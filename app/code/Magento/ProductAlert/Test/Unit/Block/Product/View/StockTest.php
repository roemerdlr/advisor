...
class StockTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_block = $objectManager->getObject ( 'Magento\ProductAlert\Block\Product\View\Stock', [ 
				'helper' => $this->_helper,
	...
	public function testSetTemplateStockUrlAllowed() {
		...
		$this->_helper->expects ( $this->once () )->method ( 'getSaveUrl' )->with ( 'stock' )->will ( $this->returnValue ( 'http://url' ) );
	...
	public function setTemplateStockUrlNotAllowedDataProvider() {
		return [ 
				'stock alert not allowed' => [ 
						false,
						false 
				],
				'product is available (no alert)' => [ 
						true,
						true 
				],
				'stock alert not allowed and product is available' => [ 
						false,
						true 
				] 
		];
