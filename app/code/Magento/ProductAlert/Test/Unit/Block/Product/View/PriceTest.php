...
class PriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_helper = $this->getMock ( 'Magento\ProductAlert\Helper\Data', [ 
				'isPriceAlertAllowed',
				'getSaveUrl' 
		], [ ], '', false );
		...
		$this->_registry = $this->getMockBuilder ( 'Magento\Framework\Registry' )->disableOriginalConstructor ()->setMethods ( [ 
				'registry' 
		] )->getMock ();
		$this->_block = $objectManager->getObject ( 'Magento\ProductAlert\Block\Product\View\Price', [ 
				'helper' => $this->_helper,
	...
	public function testSetTemplatePriceAlertAllowed() {
		...
		$this->_helper->expects ( $this->once () )->method ( 'getSaveUrl' )->with ( 'price' )->will ( $this->returnValue ( 'http://url' ) );
	...
	public function setTemplatePriceAlertNotAllowedDataProvider() {
		return [ 
				...
				'price alert is not allowed and no product price' => [ 
						false,
						false 
				] 
		];
	...
	public function testSetTemplateNoProduct() {
		...
		$this->_registry->expects ( $this->once () )->method ( 'registry' )->with ( 'current_product' )->will ( $this->returnValue ( null ) );
