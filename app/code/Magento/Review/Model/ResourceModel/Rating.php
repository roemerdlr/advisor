...
class Rating extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object) {
		...
		// load rating titles
		$select = $connection->select ()->from ( $this->getTable ( 'rating_title' ), [ 
				'store_id',
				'value' 
	...
	public function getStores($ratingId) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'rating_store' ), 'store_id' )->where ( 'rating_id = ?', $ratingId );
	...
	protected function processRatingCodes(\Magento\Framework\Model\AbstractModel $object) {
		...
		$old = $connection->fetchPairs ( $select, [ 
				':rating_id' => $ratingId 
		] );
	...
	protected function processRatingStores(\Magento\Framework\Model\AbstractModel $object) {
		...
		$old = $connection->fetchCol ( $select, [ 
				':rating_id' => $ratingId 
		] );
	...
	protected function _getEntitySummaryData($object) {
		...
		], 'rating_vote.review_id=review.review_id', [ ] )->joinLeft ( [ 
		...
		], 'review.status_id = review_status.status_id', [ ] )->where ( 'review_status.status_code = :status_code' )->group ( 'rating_vote.entity_pk_value' )->group ( 'review_store.store_id' );
		$bind = [ 
				':status_code' => self::RATING_STATUS_APPROVED 
		];
	...
	public function getEntityIdByCode($entityCode) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'rating_entity' ), [ 
				'entity_id' 
	...
	public function deleteAggregatedRatingsByProductId($productId) {
		...
		$ratingIds = $connection->fetchCol ( $select, [ 
				':entity_id' => $entityId 
		] );
