...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function setStoreFilter($storeId) {
		...
		], $this->getConnection ()->quoteInto ( 'main_table.review_id=rstore.review_id AND rstore.store_id=?', ( int ) $storeId ), [ ] );
		return $this;
	}
	...
	public function addRatingInfo($storeId = null) {
		...
		], $connection->quoteInto ( 'main_table.rating_id=title.rating_id AND title.store_id = ?', ( int ) $this->_storeManager->getStore ()->getId () ), [ 
		...
		if (! $this->_storeManager->isSingleStoreMode ()) {
			...
			if (is_array ( $storeId )) {
				$condition = $connection->prepareSqlCondition ( 'store.store_id', [ 
						'in' => $storeId 
				] );
			} else {
