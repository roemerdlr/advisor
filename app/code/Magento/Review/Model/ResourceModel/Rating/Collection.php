...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function addEntityFilter($entity) {
		...
		$this->getSelect ()->join ( $this->getTable ( 'rating_entity' ), 'main_table.entity_id=' . $this->getTable ( 'rating_entity' ) . '.entity_id', [ 
				'entity_code' 
		] );
		
		if (is_numeric ( $entity )) {
			$this->addFilter ( 'entity', $connection->quoteInto ( $this->getTable ( 'rating_entity' ) . '.entity_id=?', $entity ), 'string' );
		} elseif (is_string ( $entity )) {
			$this->addFilter ( 'entity', $connection->quoteInto ( $this->getTable ( 'rating_entity' ) . '.entity_code=?', $entity ), 'string' );
		}
		return $this;
	}
	...
	public function setStoreFilter($storeId) {
		...
		$inCondition = $connection->prepareSqlCondition ( 'store.store_id', [ 
				'in' => $storeId 
		] );
	...
	public function addEntitySummaryToItem($entityPkValue, $storeId) {
		...
		$inCond = $connection->prepareSqlCondition ( 'rating_option_vote.rating_id', [ 
				'in' => $arrRatingId 
		] );
		...
		], 'review_store.review_id=review.review_id AND review.status_id=1', [ ] )->where ( $inCond )->where ( 'rating_option_vote.entity_pk_value=:pk_value' )->group ( 'rating_option_vote.rating_id' );
		$bind = [ 
	...
	public function load($printQuery = false, $logQuery = false) {
		...
		$this->_eventManager->dispatch ( 'rating_rating_collection_load_before', [ 
				'collection' => $this 
		] );
	...
	protected function _addStoreData() {
		...
		$inCondition = $connection->prepareSqlCondition ( 'rating_id', [ 
				'in' => $ratingIds 
		] );
		...
		if (is_array ( $data ) && count ( $data ) > 0) {
			foreach ( $data as $row ) {
				...
				$item->setStores ( array_merge ( $item->getStores (), [ 
						$row ['store_id'] 
				] ) );
			}
		}
