...
class Option extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function aggregateEntityByRatingId($ratingId, $entityPkValue) {
		...
		$select = $connection->select ()->from ( $this->_aggregateTable, [ 
				'store_id',
				'primary_id' 
		] )->where ( 'rating_id = :rating_id' )->where ( 'entity_pk_value = :pk_value' );
		$bind = [ 
				':rating_id' => $ratingId,
		...
		], 'vote.review_id=review.review_id', [ ] )->joinLeft ( [ 
				'store' => $this->_reviewStoreTable 
		], 'vote.review_id=store.review_id', 'store_id' )->join ( [ 
				'rstore' => $this->_ratingStoreTable 
		], 'vote.rating_id=rstore.rating_id AND rstore.store_id=store.store_id', [ ] )->where ( 'vote.rating_id = :rating_id' )->where ( 'vote.entity_pk_value = :pk_value' )->group ( [ 
				'vote.rating_id',
		...
		foreach ( $perStoreInfo as $row ) {
			...
			if (isset ( $oldData [$row ['store_id']] )) {
				$condition = [ 
						'primary_id = ?' => $oldData [$row ['store_id']] 
				];
