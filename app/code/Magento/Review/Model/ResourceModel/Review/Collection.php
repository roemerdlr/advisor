...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function addStoreFilter($storeId) {
		$inCond = $this->getConnection ()->prepareSqlCondition ( 'store.store_id', [ 
				'in' => $storeId 
		] );
		$this->getSelect ()->join ( [ 
	...
	public function addEntityFilter($entity, $pkValue) {
		...
		if (is_numeric ( $entity )) {
			...
			$this->_select->join ( $reviewEntityTable, 'main_table.entity_id=' . $reviewEntityTable . '.entity_id', [ 
					'entity_code' 
			] );
			
			$this->addFilter ( 'entity', $this->getConnection ()->quoteInto ( $reviewEntityTable . '.entity_code=?', $entity ), 'string' );
		}
		
		$this->addFilter ( 'entity_pk_value', $this->getConnection ()->quoteInto ( 'main_table.entity_pk_value=?', $pkValue ), 'string' );
	...
	public function load($printQuery = false, $logQuery = false) {
		...
		$this->_eventManager->dispatch ( 'review_review_collection_load_before', [ 
				'collection' => $this 
		] );
	...
	protected function _addStoreData() {
		...
		if (count ( $reviewsIds ) > 0) {
			$inCond = $connection->prepareSqlCondition ( 'review_id', [ 
					'in' => $reviewsIds 
			] );
