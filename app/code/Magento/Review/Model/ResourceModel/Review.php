...
class Review extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\Stdlib\DateTime\DateTime $date, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Review\Model\RatingFactory $ratingFactory, Rating\Option $ratingOptions, $connectionName = null) {
		$this->_date = $date;
	...
	protected function _afterLoad(AbstractModel $object) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->_reviewStoreTable, [ 
				'store_id' 
	...
	public function getTotalReviews($entityPkValue, $approvedOnly = false, $storeId = 0) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->_reviewTable, [ 
	...
	protected function aggregateReviewSummary($object, $ratingSummaryObject) {
		...
		$select = $connection->select ()->from ( $this->_aggregateTable )->where ( 'entity_pk_value = :pk_value' )->where ( 'entity_type = :entity_type' )->where ( 'store_id = :store_id' );
		$bind = [ 
	...
	protected function writeReviewSummary($oldData, \Magento\Framework\DataObject $data) {
		...
		try {
			if (isset ( $oldData ['primary_id'] ) && $oldData ['primary_id'] > 0) {
				$condition = [ 
						"{$this->_aggregateTable}.primary_id = ?" => $oldData ['primary_id'] 
				];
	...
	protected function _loadVotedRatingIds($reviewId) {
		...
		], 'r.rating_id' )->joinInner ( [ 
				'r' => $this->getTable ( 'rating' ) 
		], 'v.rating_id=r.rating_id' )->where ( 'v.review_id = :revire_id' );
	...
	public function getEntityIdByCode($entityCode) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->_reviewEntityTable, [ 
				'entity_id' 
