...
class Edit extends RatingController {
	...
	public function execute() {
		...
		$resultPage->getConfig ()->getTitle ()->prepend ( $ratingModel->getId () ? $ratingModel->getRatingCode () : __ ( 'New Rating' ) );
		$resultPage->addBreadcrumb ( __ ( 'Manage Ratings' ), __ ( 'Manage Ratings' ) );
		$resultPage->addContent ( $resultPage->getLayout ()->createBlock ( 'Magento\Review\Block\Adminhtml\Rating\Edit' ) )->addLeft ( $resultPage->getLayout ()->createBlock ( 'Magento\Review\Block\Adminhtml\Rating\Edit\Tabs' ) );
		return $resultPage;
	}
}
