...
class Rss extends \Magento\Backend\Block\AbstractBlock implements DataProviderInterface {
	...
	public function getRssData() {
		$newUrl = $this->getUrl ( 'rss/catalog/review', [ 
				'_secure' => true,
				'_nosecret' => true 
		] );
		...
		foreach ( $this->rssModel->getProductCollection () as $item ) {
			...
			$url = $this->getUrl ( 'catalog/product/view', [ 
					'id' => $item->getId () 
			] );
			...
			$description = '<p>' . __ ( 'Product: <a href="%1" target="_blank">%2</a> <br/>', $url, $item->getName () ) . __ ( 'Summary of review: %1 <br/>', $item->getTitle () ) . __ ( 'Review: %1 <br/>', $item->getDetail () ) . __ ( 'Store: %1 <br/>', $storeName ) . __ ( 'Click <a href="%1">here</a> to see the review.', $reviewUrl ) . '</p>';
