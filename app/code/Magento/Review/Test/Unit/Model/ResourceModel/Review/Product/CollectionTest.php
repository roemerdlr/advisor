...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->dbSelect = $this->getMock ( 'Magento\Framework\DB\Select', [ 
				'where',
				'from',
				'join' 
		], [ ], '', false );
		...
		$entity = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product', [ 
				'getConnection',
				'getTable',
		...
		$universalFactory = $this->getMock ( '\Magento\Framework\Validator\UniversalFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$fetchStrategy = $this->getMock ( '\Magento\Framework\Data\Collection\Db\FetchStrategy\Query', [ 
				'fetchAll' 
		], [ ], '', false );
		$fetchStrategy->expects ( $this->any () )->method ( 'fetchAll' )->will ( $this->returnValue ( [ ] ) );
		$this->model = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( '\Magento\Review\Model\ResourceModel\Review\Product\Collection', [ 
	...
	public function testAddAttributeToFilter($attribute) {
		...
		$this->connectionMock->expects ( $this->once () )->method ( 'prepareSqlCondition' )->with ( $attribute, $condition )->will ( $this->returnValue ( $conditionSqlQuery ) );
	...
	public function testAddAttributeToFilterWithAttributeStore() {
		$storeId = 1;
		$this->connectionMock->expects ( $this->at ( 0 ) )->method ( 'quoteInto' )->with ( 'rt.review_id=store.review_id AND store.store_id = ?', $storeId )->will ( $this->returnValue ( 'sqlQuery' ) );
		$this->model->addAttributeToFilter ( 'stores', [ 
				'eq' => $storeId 
		] );
		$this->model->load ();
	}
	...
	public function testAddAttributeToFilterWithAttributeType($condition, $sqlConditionWith, $sqlConditionWithSec, $doubleConditionSqlQuery) {
		$conditionSqlQuery = 'sqlQuery';
		$this->connectionMock->expects ( $this->at ( 0 ) )->method ( 'prepareSqlCondition' )->with ( 'rdt.customer_id', $sqlConditionWith )->will ( $this->returnValue ( $conditionSqlQuery ) );
		if ($sqlConditionWithSec) {
			$this->connectionMock->expects ( $this->at ( 1 ) )->method ( 'prepareSqlCondition' )->with ( 'rdt.store_id', $sqlConditionWithSec )->will ( $this->returnValue ( $conditionSqlQuery ) );
		}
