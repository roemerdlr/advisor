...
class ReviewTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productFactoryMock = $this->getMock ( 'Magento\Review\Model\ResourceModel\Review\Product\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->statusFactoryMock = $this->getMock ( 'Magento\Review\Model\ResourceModel\Review\Status\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->reviewSummaryMock = $this->getMock ( 'Magento\Review\Model\ResourceModel\Review\Summary\CollectionFactory', [ ], [ ], '', false );
		$this->summaryModMock = $this->getMock ( 'Magento\Review\Model\Review\SummaryFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->review = $this->objectManagerHelper->getObject ( 'Magento\Review\Model\Review', [ 
	...
	public function testGetEntitySummary() {
		...
		$summaryData = $this->getMock ( 'Magento\Review\Model\Review\Summary', [ 
				'load',
				'getData',
				'setStoreId',
				'__wakeup' 
	...
	public function testGetReviewUrl() {
		...
		$this->urlInterfaceMock->expects ( $this->once () )->method ( 'getUrl' )->with ( $this->equalTo ( 'review/product/view' ), $this->equalTo ( [ 
				'id' => $this->reviewId 
	...
	public function testGetProductUrl($productId, $storeId, $result) {
		...
		$this->urlInterfaceMock->expects ( $this->once () )->method ( 'getUrl' )->with ( $this->equalTo ( 'catalog/product/view' ), $this->equalTo ( [ 
				'id' => $productId 
		] ) )->will ( $this->returnValue ( $result ) );
	...
	public function isAvailableOnStoreDataProvider() {
		return [ 
				'store id is set and not in list' => [ 
						1,
						false 
				],
				'store id is set' => [ 
