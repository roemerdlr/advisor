...
class PagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$sessionMock = $this->getMockBuilder ( 'Magento\Backend\Model\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'setData',
				'getData' 
		] )->getMock ();
		$sessionMock->expects ( $this->any () )->method ( 'setData' )->with ( $this->equalTo ( 'search_result_idsreviews' ), $this->anything () );
		$sessionMock->expects ( $this->any () )->method ( 'getData' )->with ( $this->equalTo ( 'search_result_idsreviews' ) )->will ( $this->returnValue ( [ 
				3,
				2,
				6,
				5 
		...
		$contextMock = $this->getMock ( 'Magento\Framework\App\Helper\Context', [ 
				'getModuleManager',
				'getRequest' 
		], [ ], '', false );
