...
class PostTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->request = $this->getMock ( '\Magento\Framework\App\Request\Http', [ 
				'getParam' 
		], [ ], '', false );
		$this->response = $this->getMock ( '\Magento\Framework\App\Response\Http', [ 
				'setRedirect' 
		], [ ], '', false );
		$this->formKeyValidator = $this->getMock ( 'Magento\Framework\Data\Form\FormKey\Validator', [ 
				'validate' 
		], [ ], '', false );
		$this->reviewSession = $this->getMock ( '\Magento\Framework\Session\Generic', [ 
				'getFormData',
				'getRedirectUrl' 
		], [ ], '', false );
		...
		$this->review = $this->getMock ( '\Magento\Review\Model\Review', [ 
				'setData',
				'validate',
				'setEntityId',
				'getEntityIdByCode',
		...
		$reviewFactory = $this->getMock ( '\Magento\Review\Model\ReviewFactory', [ 
				'create' 
		], [ ], '', false, false );
		...
		$this->rating = $this->getMock ( '\Magento\Review\Model\Rating', [ 
				'setRatingId',
				'setReviewId',
				'setCustomerId',
				'addOptionVote' 
		], [ ], '', false, false );
		$ratingFactory = $this->getMock ( '\Magento\Review\Model\RatingFactory', [ 
				'create' 
		], [ ], '', false, false );
		...
		$this->store = $this->getMock ( '\Magento\Store\Model\Store', [ 
				'getId' 
		], [ ], '', false );
		...
		$this->context = $objectManagerHelper->getObject ( 'Magento\Framework\App\Action\Context', [ 
				'request' => $this->request,
				'resultFactory' => $this->resultFactoryMock,
				'messageManager' => $this->messageManager 
		] );
		$this->model = $objectManagerHelper->getObject ( '\Magento\Review\Controller\Product\Post', [ 
	...
	public function testExecute() {
		...
		$this->reviewSession->expects ( $this->any () )->method ( 'getFormData' )->with ( true )->willReturn ( $ratingsData );
		...
		$this->review->expects ( $this->once () )->method ( 'setData' )->with ( $ratingsData )->willReturnSelf ();
		...
		$this->review->expects ( $this->once () )->method ( 'setStores' )->with ( [ 
				$storeId 
		] )->willReturnSelf ();
