...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->ratingOptionCollection = $this->getMock ( '\Magento\Review\Model\ResourceModel\Rating\Option\Collection', [ ], [ ], '', false );
		$this->element = $this->getMock ( '\Magento\Framework\Data\Form\Element\Text', [ 
				'setValue',
				'setIsChecked' 
		], [ ], '', false );
		$this->session = $this->getMock ( '\Magento\Framework\Session\Generic', [ 
				'getRatingData',
				'setRatingData' 
		], [ ], '', false );
		...
		$this->optionFactory = $this->getMock ( '\Magento\Review\Model\Rating\OptionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->ratingOptionCollection->expects ( $this->any () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$this->optionRating 
		] ) );
		...
		$this->systemStore->expects ( $this->any () )->method ( 'getStoreCollection' )->will ( $this->returnValue ( [ 
				'0' => $this->store 
		] ) );
		...
		$this->block = $objectManagerHelper->getObject ( 'Magento\Review\Block\Adminhtml\Rating\Edit\Tab\Form', [ 
