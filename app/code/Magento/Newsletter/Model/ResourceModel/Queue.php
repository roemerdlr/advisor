...
class Queue extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Newsletter\Model\ResourceModel\Subscriber\Collection $subscriberCollection, $connectionName = null) {
	...
	public function removeSubscribersFromQueue(ModelQueue $queue) {
		...
		try {
			$connection->beginTransaction ();
			$connection->delete ( $this->getTable ( 'newsletter_queue_link' ), [ 
	...
	public function setStores(ModelQueue $queue) {
		$connection = $this->getConnection ();
		$connection->delete ( $this->getTable ( 'newsletter_queue_store_link' ), [ 
				'queue_id = ?' => $queue->getId () 
		] );
		...
		$subscribers = $this->_subscriberCollection->addFieldToFilter ( 'store_id', [ 
				'in' => $stores 
	...
	public function getStores(ModelQueue $queue) {
		...
		if (! ($result = $connection->fetchCol ( $select, [ 
				'queue_id' => $queue->getId () 
		] ))) {
			$result = [ ];
		}
