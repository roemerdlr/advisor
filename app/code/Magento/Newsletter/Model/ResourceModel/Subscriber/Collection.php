...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function useQueue(ModelQueue $queue) {
		...
		], "link.subscriber_id = main_table.subscriber_id", [ ] )->where ( "link.queue_id = ? ", $queue->getId () );
	...
	public function useOnlyUnsent() {
		if ($this->_queueJoinedFlag) {
			$this->addFieldToFilter ( 'link.letter_sent_at', [ 
					'null' => 1 
			] );
		}
	...
	public function addSubscriberTypeField() {
		$this->getSelect ()->columns ( [ 
				'type' => new \Zend_Db_Expr ( $this->_getMappedField ( 'type' ) ) 
		] );
		return $this;
	}
	...
	public function useOnlyCustomers() {
		$this->addFieldToFilter ( 'main_table.customer_id', [ 
				'gt' => 0 
		] );
