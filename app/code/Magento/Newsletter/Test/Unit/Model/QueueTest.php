...
class QueueTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->templateFilter = $this->getMockBuilder ( '\Magento\Newsletter\Model\Template\Filter' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->date = $this->getMockBuilder ( '\Magento\Framework\Stdlib\DateTime\DateTime' )->disableOriginalConstructor ()->getMock ();
		$this->templateFactory = $this->getMockBuilder ( '\Magento\Newsletter\Model\TemplateFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create',
				'load' 
		] )->getMock ();
		$this->problemFactory = $this->getMockBuilder ( '\Magento\Newsletter\Model\ProblemFactory' )->disableOriginalConstructor ()->getMock ();
		$this->transportBuilder = $this->getMockBuilder ( '\Magento\Newsletter\Model\Queue\TransportBuilder' )->disableOriginalConstructor ()->setMethods ( [ 
				'setTemplateData',
				'setTemplateOptions',
				'setTemplateVars',
				'setFrom',
		...
		$this->subscribersCollection = $this->getMockBuilder ( '\Magento\Newsletter\Model\ResourceModel\Subscriber\Collection' )->disableOriginalConstructor ()->getMock ();
		$this->resource = $this->getMockBuilder ( '\Magento\Newsletter\Model\ResourceModel\Queue' )->disableOriginalConstructor ()->getMock ();
		$this->subscribersCollectionFactory = $this->getMockBuilder ( '\Magento\Newsletter\Model\ResourceModel\Subscriber\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->queue = $this->objectManager->getObject ( '\Magento\Newsletter\Model\Queue', [ 
	...
	public function testSendPerSubscriber2() {
		$this->queue->setQueueStatus ( 1 );
		$this->queue->setQueueStartAt ( 1 );
		$collection = $this->getMockBuilder ( '\Magento\Framework\Data\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'getItems' 
		] )->getMock ();
		$item = $this->getMockBuilder ( '\Magento\Newsletter\Model\Subscriber' )->disableOriginalConstructor ()->setMethods ( [ 
				'getStoreId',
				'getSubscriberEmail',
		...
		$collection->expects ( $this->atLeastOnce () )->method ( 'getItems' )->willReturn ( [ 
				$item 
		] );
