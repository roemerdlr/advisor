...
class FilterTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$scopeConfig = $this->getMockForAbstractClass ( '\Magento\Framework\App\Config\ScopeConfigInterface', [ ], '', false );
		...
		$coreVariableFactory = $this->getMock ( '\Magento\Variable\Model\VariableFactory', [ 
				'create' 
		], [ ], '', false );
		$layoutFactory = $this->getMock ( '\Magento\Framework\View\LayoutFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->filter = new \Magento\Newsletter\Model\Template\Filter ( $string, $logger, $escaper, $assetRepo, $scopeConfig, $coreVariableFactory, $this->storeManager, $layout, $layoutFactory, $this->appState, $urlModel, $emogrifier, $configVariables, $widgetResource, $widget );
	}
	public function testWidgetDirective() {
		...
		$this->appState->expects ( $this->once () )->method ( 'emulateAreaCode' )->with ( 'frontend', [ 
				$this->filter,
				'generateWidget' 
		], [ 
				[ 
