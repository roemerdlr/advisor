...
class PreviewTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->template = $this->getMock ( 'Magento\Newsletter\Model\Template', [ 
				'setTemplateType',
				'setTemplateText',
				'setTemplateStyles',
				'isPlain',
		...
		$this->subscriberFactory = $this->getMock ( 'Magento\Newsletter\Model\SubscriberFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->preview = $this->objectManagerHelper->getObject ( 'Magento\Newsletter\Block\Adminhtml\Template\Preview', [ 
	...
	public function testToHtml() {
		...
		$this->appState->expects ( $this->atLeastOnce () )->method ( 'emulateAreaCode' )->with ( \Magento\Newsletter\Model\Template::DEFAULT_DESIGN_AREA, [ 
				$this->template,
				'getProcessedTemplate' 
		], [ 
				[ 
	...
	public function testToHtmlForNewTemplate() {
		...
		$this->template->expects ( $this->once () )->method ( 'setTemplateType' )->with ( TemplateTypesInterface::TYPE_TEXT )->willReturnSelf ();
		$this->template->expects ( $this->once () )->method ( 'setTemplateText' )->with ( 'Processed Template' )->willReturnSelf ();
		$this->template->expects ( $this->once () )->method ( 'setTemplateStyles' )->with ( '.class-name{color:red;}' )->willReturnSelf ();
		...
		$this->storeManager->expects ( $this->atLeastOnce () )->method ( 'getStores' )->willReturn ( [ 
				$store 
		] );
