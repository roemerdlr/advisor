...
class Grid extends \Magento\Newsletter\Controller\Adminhtml\Problem {
	...
	public function execute() {
		if ($this->getRequest ()->getParam ( '_unsubscribe' )) {
			...
			if (count ( $problems ) > 0) {
				$collection = $this->_objectManager->create ( 'Magento\Newsletter\Model\ResourceModel\Problem\Collection' );
				$collection->addSubscriberInfo ()->addFieldToFilter ( $collection->getResource ()->getIdFieldName (), [ 
						'in' => $problems 
				] )->load ();
		...
		if ($this->getRequest ()->getParam ( '_delete' )) {
			...
			if (count ( $problems ) > 0) {
				$collection = $this->_objectManager->create ( 'Magento\Newsletter\Model\ResourceModel\Problem\Collection' );
				$collection->addFieldToFilter ( $collection->getResource ()->getIdFieldName (), [ 
						'in' => $problems 
				] )->load ();
