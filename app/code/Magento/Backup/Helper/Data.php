...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
	 ...
	 */
	public function __construct(\Magento\Framework\App\Helper\Context $context, Filesystem $filesystem, \Magento\Framework\AuthorizationInterface $authorization, \Magento\Framework\App\Cache\TypeListInterface $cacheTypeList) {
	...
	public function generateBackupDownloadName(\Magento\Backup\Model\Backup $backup) {
		...
		return $backup->getTime () . '_' . $backup->getType () . '_' . $backup->getName () . $additionalExtension . '.' . $this->getExtensionByType ( $backup->getType () );
	}
	...
	public function getBackupIgnorePaths() {
		return [ 
				'.git',
				'.svn',
				$this->_filesystem->getDirectoryRead ( MaintenanceMode::FLAG_DIR )->getAbsolutePath ( MaintenanceMode::FLAG_FILENAME ),
	...
	public function getCreateSuccessMessageByType($type) {
		$messagesMap = [ 
				...
				\Magento\Framework\Backup\Factory::TYPE_SNAPSHOT_WITHOUT_MEDIA => __ ( 'You created the system backup (excluding media).' ),
