...
class Helper extends \Magento\Framework\DB\Helper {
	 ...
	 */
	public function __construct(\Magento\Framework\App\ResourceConnection $resource, $modulePrefix, \Magento\Framework\Stdlib\DateTime\DateTime $coreDate) {
	...
	protected function _buildForeignKeysAlterTableSql($tableName, $foreignKeys) {
		...
		return sprintf ( "ALTER TABLE %s\n  %s;\n", $this->getConnection ()->quoteIdentifier ( $tableName ), join ( ",\n  ", $foreignKeys ) );
	}
	...
	public function getTableCreateSql($tableName, $withForeignKeys = false) {
		...
		if (is_array ( $matches )) {
			foreach ( $matches as $match ) {
				$this->_foreignKeys [$tableName] [] = sprintf ( 'ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s (%s)%s%s', $connection->quoteIdentifier ( $match [1] ), $connection->quoteIdentifier ( $match [2] ), $connection->quoteIdentifier ( $match [3] ), $connection->quoteIdentifier ( $match [4] ), isset ( $match [5] ) ? $match [5] : '', isset ( $match [7] ) ? $match [7] : '' );
			}
		}
