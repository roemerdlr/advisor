...
class Change extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	protected function _reEncryptSystemConfigurationValues() {
		...
		$paths = $configStructure->getFieldPathsByAttribute ( 'backend_model', 'Magento\Config\Model\Config\Backend\Encrypted' );
		
		// walk through found data and re-encrypt it
		if ($paths) {
			$table = $this->getTable ( 'core_config_data' );
			$values = $this->getConnection ()->fetchPairs ( $this->getConnection ()->select ()->from ( $table, [ 
					'config_id',
					'value' 
			] )->where ( 'path IN (?)', $paths )->where ( 'value NOT LIKE ?', '' ) );
			foreach ( $values as $configId => $value ) {
				$this->getConnection ()->update ( $table, [ 
