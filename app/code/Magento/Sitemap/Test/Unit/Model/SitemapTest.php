...
class SitemapTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_sitemapCategoryMock = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Catalog\Category' )->disableOriginalConstructor ()->getMock ();
		$this->_sitemapProductMock = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Catalog\Product' )->disableOriginalConstructor ()->getMock ();
		$this->_sitemapCmsPageMock = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Cms\Page' )->disableOriginalConstructor ()->getMock ();
		$this->_helperMockSitemap = $this->getMock ( 'Magento\Sitemap\Helper\Data', [ 
		...
		$this->_resourceMock = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Sitemap' )->setMethods ( [ 
				'_construct',
				'beginTransaction',
				'rollBack',
				'save',
		...
		$this->_filesystemMock = $this->getMockBuilder ( 'Magento\Framework\Filesystem' )->setMethods ( [ 
				'getDirectoryWrite' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testGenerateXml($maxLines, $maxFileSize, $expectedFile, $expectedWrites) {
		$actualData = [ ];
		$model = $this->_prepareSitemapModelMock ( $actualData, $maxLines, $maxFileSize, $expectedFile, $expectedWrites, null );
		$model->generateXml ();
		...
		foreach ( $expectedFile as $expectedFileName => $expectedFilePath ) {
			$this->assertArrayHasKey ( $expectedFileName, $actualData, sprintf ( 'File %s was not generated', $expectedFileName ) );
...
;
	...
	public function testAddSitemapToRobotsTxt($maxLines, $maxFileSize, $expectedFile, $expectedWrites, $robotsInfo) {
		$actualData = [ ];
		$model = $this->_prepareSitemapModelMock ( $actualData, $maxLines, $maxFileSize, $expectedFile, $expectedWrites, $robotsInfo );
		$model->generateXml ();
	}
	 ...
	 * @param int $expectedWrites        	
	 * @param array $robotsInfo        	
	 * @return \Magento\Sitemap\Model\Sitemap|PHPUnit_Framework_MockObject_MockObject @SuppressWarnings(PHPMD.ExcessiveMethodLength)
	 */
	...
	protected function _getModelMock($mockBeforeSave = false) {
		...
		$this->_sitemapCategoryMock->expects ( $this->any () )->method ( 'getCollection' )->will ( $this->returnValue ( [ 
		...
		$this->_sitemapProductMock->expects ( $this->any () )->method ( 'getCollection' )->will ( $this->returnValue ( [ 
				new \Magento\Framework\DataObject ( [ 
						'url' => 'product.html',
						'updated_at' => '2012-12-21 00:00:00' 
				] ),
	...
	protected function _getModelConstructorArgs() {
		$categoryFactory = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Catalog\CategoryFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$categoryFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_sitemapCategoryMock ) );
		
		$productFactory = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Catalog\ProductFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$productFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_sitemapProductMock ) );
		
		$cmsFactory = $this->getMockBuilder ( 'Magento\Sitemap\Model\ResourceModel\Cms\PageFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$cmsFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_sitemapCmsPageMock ) );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$constructArguments = $objectManager->getConstructArguments ( 'Magento\Sitemap\Model\Sitemap', [ 
	...
	public function testGetSitemapUrl($storeBaseUrl, $documentRoot, $baseDir, $sitemapPath, $sitemapFileName, $result) {
		/** @var $model \Magento\Sitemap\Model\Sitemap */
		$model = $this->getMockBuilder ( 'Magento\Sitemap\Model\Sitemap' )->setMethods ( [ 
				'_getStoreBaseUrl',
				'_getDocumentRoot',
				'_getBaseDir',
				'_construct' 
		] )->setConstructorArgs ( $this->_getModelConstructorArgs () )->getMock ();
