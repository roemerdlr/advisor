...
class SaveTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->context = $this->objectManagerHelper->getObject ( 'Magento\Backend\App\Action\Context', [ 
				'resultFactory' => $this->resultFactoryMock,
				'request' => $this->requestMock,
				'messageManager' => $this->messageManagerMock,
				'objectManager' => $this->objectManagerMock 
		] );
		$this->saveController = $this->objectManagerHelper->getObject ( 'Magento\Sitemap\Controller\Adminhtml\Sitemap\Save', [ 
	...
	public function testSaveEmptyDataShouldRedirectToDefault() {
		...
		$this->resultRedirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( 'adminhtml/*/' )->willReturnSelf ();
	...
	public function testTryToSaveInvalidDataShouldFailWithErrors() {
		...
		$session = $this->getMock ( $sessionClass, [ 
				'setFormData' 
		], [ ], '', false );
		...
		$this->messageManagerMock->expects ( $this->at ( 0 ) )->method ( 'addError' )->withConsecutive ( [ 
				$messages [0] 
		], [ 
				$messages [1] 
		] )->willReturnSelf ();
		
		$this->resultRedirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( 'adminhtml/*/edit', [ 
