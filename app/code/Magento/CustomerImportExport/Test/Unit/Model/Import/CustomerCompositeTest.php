...
class CustomerCompositeTest extends \PHPUnit_Framework_TestCase {
	...
	/**
	 *
	 * @var \Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorFactory |\PHPUnit_Framework_MockObject_MockObject
	 */
	...
	protected function setUp() {
		...
		$context = $this->getMock ( 'Magento\Framework\App\Helper\Context', [ 
				'getTranslateInline' 
		], [ ], '', false );
		...
		$this->errorFactory = $this->getMock ( '\Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->error = $this->getMock ( '\Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingError', [ 
				'init' 
		], [ ], '', false );
	...
	protected function _createModelMock($data) {
		return new \Magento\CustomerImportExport\Model\Import\CustomerComposite ( $this->_string, $this->_scopeConfigMock, $this->_importFactory, $this->_resourceHelper, $this->_resource, $this->errorAggregator, $this->_dataFactory, $this->_customerFactory, $this->_addressFactory, $data );
	}
	...
	protected function _getModelMockForPrepareRowForDb() {
		...
		$customerStorage = $this->getMock ( 'stdClass', [ 
				'getCustomerId' 
		] );
		...
		$dataSourceMock = $this->getMock ( 'stdClass', [ 
				'cleanBunches',
				'saveBunch' 
		] );
		$dataSourceMock->expects ( $this->any () )->method ( 'saveBunch' )->will ( $this->returnCallback ( [ 
				$this,
	...
	protected function _getCustomerEntityMock(array $mockedMethods = null) {
		...
		$customerEntity = $this->getMock ( 'Magento\CustomerImportExport\Model\Import\Customer', $mockedMethods, [ ], '', false );
		...
		$customerEntity->expects ( $this->once () )->method ( 'getAttributeCollection' )->will ( $this->returnValue ( $attributeList ) );
	...
	protected function _getAddressEntityMock(array $mockedMethods = null) {
		...
		$addressEntity = $this->getMock ( 'Magento\CustomerImportExport\Model\Import\Address', $mockedMethods, [ ], '', false );
		...
		$addressEntity->expects ( $this->once () )->method ( 'getAttributeCollection' )->will ( $this->returnValue ( $attributeList ) );
	...
	public function testIsAttributeParticular() {
		...
		foreach ( $this->_addressAttributes as $code ) {
			$this->assertTrue ( $modelUnderTest->isAttributeParticular ( CustomerComposite::COLUMN_ADDRESS_PREFIX . $code ), 'Attribute must be particular' );
		}
	...
	public function testValidateRow(array $rows, array $calls, $validationReturn, array $expectedErrors, $behavior) {
		...
		$customerEntity->expects ( $this->exactly ( $calls ['customerValidationCalls'] ) )->method ( 'validateRow' )->will ( $this->returnValue ( $validationReturn ) );
		
		$customerEntity->expects ( $this->any () )->method ( 'getErrorMessages' )->will ( $this->returnValue ( [ ] ) );
		
		$addressEntity->expects ( $this->exactly ( $calls ['addressValidationCalls'] ) )->method ( 'validateRow' )->will ( $this->returnValue ( $validationReturn ) );
	...
	public function testPrepareAddressRowData() {
		...
		$addressEntity->expects ( $this->once () )->method ( 'validateRow' )->will ( $this->returnCallback ( [ 
				$this,
	...
	public function validateAddressRowParams(array $rowData, $rowNumber) {
		...
		$this->assertArrayHasKey ( CustomerComposite::COLUMN_DEFAULT_BILLING, $rowData );
		$this->assertArrayHasKey ( CustomerComposite::COLUMN_DEFAULT_SHIPPING, $rowData );
	...
	public function testSetParameters() {
		...
		$customerEntity->expects ( $this->once () )->method ( 'setParameters' )->will ( $this->returnCallback ( [ 
				$this,
				'callbackCheckParameters' 
		] ) );
		$addressEntity->expects ( $this->once () )->method ( 'setParameters' )->will ( $this->returnCallback ( [ 
				$this,
				'callbackCheckParameters' 
		] ) );
		$data = $this->_getModelDependencies ();
	...
	public function testPrepareRowForDb() {
		...
		$directoryMock->expects ( $this->any () )->method ( 'openFile' )->will ( $this->returnValue ( new Read ( $pathToCsvFile, new File () ) ) );
