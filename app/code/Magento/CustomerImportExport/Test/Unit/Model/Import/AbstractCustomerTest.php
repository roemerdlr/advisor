...
class AbstractCustomerTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	protected function _getModelMock() {
		$customerCollection = new \Magento\Framework\Data\Collection ( $this->getMock ( 'Magento\Framework\Data\Collection\EntityFactory', [ ], [ ], '', false ) );
		foreach ( $this->_customers as $customer ) {
			$customerCollection->addItem ( new \Magento\Framework\DataObject ( $customer ) );
		}
		
		$modelMock = $this->getMockBuilder ( 'Magento\CustomerImportExport\Model\Import\AbstractCustomer' )->disableOriginalConstructor ()->setMethods ( [ 
				'getErrorAggregator',
				...
				'no website' => [ 
						...
						'$errors' => [ 
								AbstractCustomer::ERROR_WEBSITE_IS_EMPTY => [ 
										[ 
												1,
												AbstractCustomer::COLUMN_WEBSITE 
										] 
								] 
						] 
				],
	...
	public function testCheckUniqueKey(array $rowData, array $errors, $isValid = false) {
		$checkUniqueKey = new \ReflectionMethod ( 'Magento\CustomerImportExport\Model\Import\AbstractCustomer', '_checkUniqueKey' );
	...
	protected function _clearValidatedRows() {
		// clear array
		$validatedRows = new \ReflectionProperty ( 'Magento\CustomerImportExport\Model\Import\AbstractCustomer', '_validatedRows' );
		$validatedRows->setAccessible ( true );
		$validatedRows->setValue ( $this->_model, [ ] );
		$validatedRows->setAccessible ( false );
		
		// reset counter
		$entitiesCount = new \ReflectionProperty ( 'Magento\CustomerImportExport\Model\Import\AbstractCustomer', '_processedEntitiesCount' );
