...
class AddressTest extends \PHPUnit_Framework_TestCase {
	...
	/**
	 *
	 * @var \Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorAggregatorInterface |\PHPUnit_Framework_MockObject_MockObject
	 */
	...
	protected function setUp() {
		...
		$this->_storeManager = $this->getMockBuilder ( 'Magento\Store\Model\StoreManager' )->disableOriginalConstructor ()->setMethods ( [ 
				'getWebsites' 
		] )->getMock ();
		$this->_storeManager->expects ( $this->any () )->method ( 'getWebsites' )->will ( $this->returnCallback ( [ 
				$this,
				'getWebsites' 
		] ) );
		$this->_model = $this->_getModelMock ();
		$this->errorAggregator = $this->getMock ( 'Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorAggregator', [ 
				'hasToBeTerminated' 
		], [ ], '', false );
	}
	...
	protected function _getModelDependencies() {
		$dataSourceModel = $this->getMock ( 'stdClass', [ 
				'getNextBunch' 
		] );
	...
	protected function _createAttrCollectionMock() {
		...
		$attributeCollection = $this->getMock ( 'Magento\Framework\Data\Collection', [ 
				'getEntityTypeCode' 
		], [ 
				$entityFactory 
		] );
		foreach ( $this->_attributes as $attributeData ) {
			$arguments = $this->_objectManagerMock->getConstructArguments ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute', [ 
			...
			$attribute = $this->getMockForAbstractClass ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute', $arguments, '', true, true, true, [ 
					'_construct',
					'getBackend' 
			] );
	...
	protected function _createCustomerStorageMock() {
		$customerStorage = $this->getMock ( 'Magento\CustomerImportExport\Model\ResourceModel\Import\Customer\Storage', [ 
				'load' 
		], [ ], '', false );
		$resourceMock = $this->getMock ( 'Magento\Customer\Model\ResourceModel\Customer', [ 
				'getIdFieldName' 
		], [ ], '', false );
	...
	protected function _createCustomerEntityMock() {
		$customerEntity = $this->getMock ( 'stdClass', [ 
	...
	protected function _getModelMockForTestImportDataWithCustomBehaviour() {
		...
		// entity adapter mock
		$modelMock = $this->getMock ( 'Magento\CustomerImportExport\Model\Import\Address', [ 
				'validateRow',
		...
		$dataSourceMock = $this->getMock ( 'Magento\ImportExport\Model\ResourceModel\Import\Data', [ 
				'getNextBunch',
				'__wakeup' 
		], [ ], '', false );
		...
		$dataSourceModel = new \ReflectionProperty ( 'Magento\CustomerImportExport\Model\Import\Address', '_dataSourceModel' );
		...
		$modelMock->expects ( $this->any () )->method ( 'getErrorAggregator' )->will ( $this->returnValue ( $this->errorAggregator ) );
		
		$modelMock->expects ( $this->any () )->method ( '_prepareDataForUpdate' )->will ( $this->returnValue ( $updateResult ) );
		
		$modelMock->expects ( $this->any () )->method ( '_saveAddressEntities' )->will ( $this->returnCallback ( [ 
				$this,
		...
		$modelMock->expects ( $this->any () )->method ( '_deleteAddressEntities' )->will ( $this->returnCallback ( [ 
				$this,
	...
	protected function _getModelMock() {
		...
		$modelMock = new \Magento\CustomerImportExport\Model\Import\Address ( $this->_stringLib, $scopeConfig, $this->getMock ( 'Magento\ImportExport\Model\ImportFactory', [ ], [ ], '', false ), $this->getMock ( 'Magento\ImportExport\Model\ResourceModel\Helper', [ ], [ ], '', false ), $this->getMock ( 'Magento\Framework\App\ResourceConnection', [ ], [ ], '', false ), $this->getMock ( 'Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorAggregatorInterface', [ ], [ ], '', false ), $this->_storeManager, $this->getMock ( 'Magento\ImportExport\Model\Export\Factory', [ ], [ ], '', false ), $this->getMock ( 'Magento\Eav\Model\Config', [ ], [ ], '', false ), $this->getMock ( 'Magento\CustomerImportExport\Model\ResourceModel\Import\Customer\StorageFactory', [ ], [ ], '', false ), $this->getMock ( 'Magento\Customer\Model\AddressFactory', [ ], [ ], '', false ), $this->getMock ( 'Magento\Directory\Model\ResourceModel\Region\CollectionFactory', [ ], [ ], '', false ), $this->getMock ( 'Magento\Customer\Model\CustomerFactory', [ ], [ ], '', false ), $this->getMock ( 'Magento\Customer\Model\ResourceModel\Address\CollectionFactory', [ ], [ ], '', false ), $this->getMock ( 'Magento\Customer\Model\ResourceModel\Address\Attribute\CollectionFactory', [ ], [ ], '', false ), new \Magento\Framework\Stdlib\DateTime (), $this->_getModelDependencies () );
	...
	public function testGetDefaultAddressAttributeMapping() {
		...
		$this->assertArrayHasKey ( Address::COLUMN_DEFAULT_BILLING, $attributeMapping, 'Default address attribute mapping array must have a default billing column.' );
		$this->assertArrayHasKey ( Address::COLUMN_DEFAULT_SHIPPING, $attributeMapping, 'Default address attribute mapping array must have a default shipping column.' );
	}
	...
	public function testImportDataWithCustomBehaviour() {
		...
		$this->_model->setParameters ( [ 
				'behavior' => \Magento\ImportExport\Model\Import::BEHAVIOR_CUSTOM 
		] );
