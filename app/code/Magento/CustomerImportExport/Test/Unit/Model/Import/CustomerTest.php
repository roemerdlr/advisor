...
class CustomerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getModelMockForTestImportDataWithCustomBehaviour() {
		// entity adapter mock
		$modelMock = $this->getMockBuilder ( 'Magento\CustomerImportExport\Model\Import\Customer' )->disableOriginalConstructor ()->setMethods ( [ 
				'validateRow',
				'_getCustomerId',
		...
		$errorAggregator = $this->getMock ( 'Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorAggregator', [ 
				'hasToBeTerminated' 
		], [ ], '', false );
		...
		$dataSourceModelMock = $this->getMockBuilder ( 'Magento\ImportExport\Model\ResourceModel\Import\Data' )->disableOriginalConstructor ()->setMethods ( [ 
				'getNextBunch',
				'__wakeup' 
		] )->getMock ();
		
		$dataSourceModelMock->expects ( $this->at ( 0 ) )->method ( 'getNextBunch' )->will ( $this->returnValue ( $this->_inputRows ) );
		$dataSourceModelMock->expects ( $this->at ( 1 ) )->method ( 'getNextBunch' )->will ( $this->returnValue ( null ) );
		
		$property = new \ReflectionProperty ( 'Magento\CustomerImportExport\Model\Import\Customer', '_dataSourceModel' );
		...
		$modelMock->expects ( $this->any () )->method ( '_getCustomerId' )->will ( $this->returnValue ( $this->_customerIds ['delete'] ) );
		
		$modelMock->expects ( $this->any () )->method ( '_prepareDataForUpdate' )->will ( $this->returnCallback ( [ 
				$this,
				'prepareForUpdateMock' 
		] ) );
		
		$modelMock->expects ( $this->any () )->method ( '_saveCustomerEntities' )->will ( $this->returnCallback ( [ 
				$this,
		...
		$modelMock->expects ( $this->any () )->method ( '_deleteCustomerEntities' )->will ( $this->returnCallback ( [ 
				$this,
	...
	public function prepareForUpdateMock(array $rowData) {
		$preparedResult = [ 
				...
				Customer::ATTRIBUTES_TO_SAVE_KEY => [ 
						'table' => [ ] 
				] 
		];
