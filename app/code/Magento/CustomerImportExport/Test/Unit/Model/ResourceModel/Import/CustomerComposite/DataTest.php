...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getDependencies($entityType, $bunchData) {
		...
		$statementMock = $this->getMock ( 'Magento\Framework\DB\Statement\Pdo\Mysql', [ 
				'setFetchMode',
				'getIterator' 
		], [ ], '', false );
		$statementMock->expects ( $this->any () )->method ( 'getIterator' )->will ( $this->returnValue ( new \ArrayIterator ( $bunchData ) ) );
		...
		$resourceModelMock = $this->getMock ( 'Magento\Framework\App\ResourceConnection', [ ], [ ], '', false );
	...
	public function testGetNextBunch($entityType, $bunchData, $expectedData) {
		$dependencies = $this->_getDependencies ( $entityType, [ 
				[ 
						$bunchData 
				] 
		] );
		...
		$jsonDecoderMock = $this->getMockBuilder ( 'Magento\Framework\Json\DecoderInterface' )->disableOriginalConstructor ()->getMock ();
		$jsonDecoderMock->expects ( $this->once () )->method ( 'decode' )->willReturn ( \Zend_Json::decode ( $bunchData ) );
		$jsonHelper = $helper->getObject ( 'Magento\Framework\Json\Helper\Data', [ 
		...
		$object = $objectManager->getObject ( '\Magento\CustomerImportExport\Model\ResourceModel\Import\CustomerComposite\Data', [ 
				'context' => $contextMock,
