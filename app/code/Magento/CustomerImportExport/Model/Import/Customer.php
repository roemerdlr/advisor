...
class Customer extends AbstractCustomer {
	...
	protected function _saveCustomerEntities(array $entitiesToCreate, array $entitiesToUpdate) {
		...
		if ($entitiesToUpdate) {
			$this->_connection->insertOnDuplicate ( $this->_entityTable, $entitiesToUpdate, $this->customerFields );
		}
		...
		foreach ( array_intersect_key ( $rowData, $this->_attributes ) as $attributeCode => $value ) {
			...
			if (! $this->_attributes [$attributeCode] ['is_static']) {
				...
				if ('select' == $attributeParameters ['type']) {
					$value = isset ( $attributeParameters ['options'] [strtolower ( $value )] ) ? $attributeParameters ['options'] [strtolower ( $value )] : 0;
		...
		while ( $bunch = $this->_dataSourceModel->getNextBunch () ) {
			...
			foreach ( $bunch as $rowNumber => $rowData ) {
				...
				if ($this->getBehavior ( $rowData ) == \Magento\ImportExport\Model\Import::BEHAVIOR_DELETE) {
					$entitiesToDelete [] = $this->_getCustomerId ( $rowData [self::COLUMN_EMAIL], $rowData [self::COLUMN_WEBSITE] );
					...
					foreach ( $processedData [self::ATTRIBUTES_TO_SAVE_KEY] as $tableName => $customerAttributes ) {
						...
						$attributesToSave [$tableName] = array_diff_key ( $attributesToSave [$tableName], $customerAttributes ) + $customerAttributes;
					}
				}
			}
	...
	protected function _validateRowForUpdate(array $rowData, $rowNumber) {
		if ($this->_checkUniqueKey ( $rowData, $rowNumber )) {
			...
			// check password
			if (isset ( $rowData ['password'] ) && strlen ( $rowData ['password'] ) && $this->string->strlen ( $rowData ['password'] ) < self::MIN_PASSWORD_LENGTH) {
	...
	public function getValidColumnNames() {
		$this->validColumnNames = array_merge ( $this->validColumnNames, $this->customerFields );
