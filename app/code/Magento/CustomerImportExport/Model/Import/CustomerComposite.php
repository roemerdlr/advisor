...
class CustomerComposite extends \Magento\ImportExport\Model\Import\AbstractEntity {
	...
	public function __construct(\Magento\Framework\Stdlib\StringUtils $string, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\ImportExport\Model\ImportFactory $importFactory, \Magento\ImportExport\Model\ResourceModel\Helper $resourceHelper, \Magento\Framework\App\ResourceConnection $resource, ProcessingErrorAggregatorInterface $errorAggregator, \Magento\CustomerImportExport\Model\ResourceModel\Import\CustomerComposite\DataFactory $dataFactory, \Magento\CustomerImportExport\Model\Import\CustomerFactory $customerFactory, \Magento\CustomerImportExport\Model\Import\AddressFactory $addressFactory, array $data = []) {
		...
		if (isset ( $data ['customer_data_source_model'] )) {
			...
			$this->_dataSourceModels ['customer'] = $dataFactory->create ( [ 
					'arguments' => $arguments 
			] );
		}
		if (isset ( $data ['customer_entity'] )) {
			...
			$this->_customerEntity = $customerFactory->create ( [ 
					'data' => $data 
			] );
		...
		if (isset ( $data ['address_data_source_model'] )) {
			...
			$this->_dataSourceModels ['address'] = $dataFactory->create ( [ 
					'arguments' => $arguments 
			] );
		}
		if (isset ( $data ['address_entity'] )) {
			...
			$this->_addressEntity = $addressFactory->create ( [ 
					'data' => $data 
			] );
			unset ( $data ['data_source_model'] );
		}
	...
	public function validateRow(array $rowData, $rowNumber) {
		...
		if ($rowScope == self::SCOPE_DEFAULT) {
			if ($this->_customerEntity->validateRow ( $rowData, $rowNumber )) {
				$this->_currentWebsiteCode = $rowData [Customer::COLUMN_WEBSITE];
				$this->_currentEmail = strtolower ( $rowData [Customer::COLUMN_EMAIL] );
	...
	protected function _prepareAddressRowData(array $rowData) {
		...
		unset ( $rowData [Customer::COLUMN_WEBSITE], $rowData [Customer::COLUMN_STORE] );
	...
	protected function _getRowScope(array $rowData) {
		...
		return strlen ( trim ( $rowData [Customer::COLUMN_EMAIL] ) ) ? self::SCOPE_DEFAULT : self::SCOPE_ADDRESS;
	}
