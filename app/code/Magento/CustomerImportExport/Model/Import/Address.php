...
class Address extends AbstractCustomer {
	...
	protected function _importData() {
		while ( $bunch = $this->_dataSourceModel->getNextBunch () ) {
			...
			$this->_saveAddressEntities ( $newRows, $updateRows )->_saveAddressAttributes ( $attributes )->_saveCustomerDefaults ( $defaults );
	...
	protected function _saveAddressEntities(array $addRows, array $updateRows) {
		...
		if ($updateRows) {
			...
			foreach ( $updateRows as $row ) {
				$fields = array_diff ( array_keys ( $row ), [ 
						'entity_id',
						'parent_id',
						'created_at' 
				] );
	 ...
	 *
	 * @param array $entityRowIds
	 *        	Row IDs for delete
	 * @return $this
	 */
	protected function _deleteAddressEntities(array $entityRowIds) {
		if ($entityRowIds) {
			$this->_connection->delete ( $this->_entityTable, [ 
	...
	protected function _isOptionalAddressEmpty(array $rowData) {
		...
		unset ( $rowData [Customer::COLUMN_WEBSITE], $rowData [Customer::COLUMN_STORE], $rowData ['_email'] );
			...
			if ($customerId === false) {
				...
				if ($this->_checkRowDuplicate ( $customerId, $addressId )) {
					...
					foreach ( $this->_attributes as $attributeCode => $attributeParams ) {
						...
						} elseif ($attributeParams ['is_required'] && (! isset ( $this->_addresses [$customerId] ) || ! in_array ( $addressId, $this->_addresses [$customerId] ))) {
							$this->addRowError ( self::ERROR_VALUE_IS_REQUIRED, $rowNumber, $attributeCode );
						}
					}
					
					if (isset ( $rowData [self::COLUMN_COUNTRY_ID] ) && isset ( $rowData [self::COLUMN_REGION] )) {
						$countryRegions = isset ( $this->_countryRegions [strtolower ( $rowData [self::COLUMN_COUNTRY_ID] )] ) ? $this->_countryRegions [strtolower ( $rowData [self::COLUMN_COUNTRY_ID] )] : [ ];
						
						if (! empty ( $rowData [self::COLUMN_REGION] ) && ! empty ( $countryRegions ) && ! isset ( $countryRegions [strtolower ( $rowData [self::COLUMN_REGION] )] )) {
