...
class ConfigurableTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	protected function setUp() {
		...
		$this->setCollectionFactory = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->setCollection = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\Collection', [ 
				'setEntityTypeFilter' 
		], [ ], '', false );
		...
		$this->attrCollectionFactory = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->attrCollection = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\Attribute\Collection', [ 
				'setAttributeSetFilter' 
		], [ ], '', false );
		
		$superAttributes = [ ];
		foreach ( $this->_getSuperAttributes () as $superAttribute ) {
			$item = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\AbstractAttribute', [ 
					'isStatic' 
			], $superAttribute, '', false );
		...
		$this->_entityModel = $this->getMock ( 'Magento\CatalogImportExport\Model\Import\Product', [ 
				'getNewSku',
				'getOldSku',
				'getNextBunch',
		...
		$this->productCollection = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\Collection', [ 
				'addFieldToFilter',
		...
		foreach ( $testProducts as $product ) {
			$item = $this->getMock ( '\Magento\Framework\DataObject', [ 
					'getAttributeSetId' 
			], [ ], '', false );
		...
		$this->configurable = $this->objectManagerHelper->getObject ( 'Magento\ConfigurableImportExport\Model\Import\Product\Type\Configurable', [ 
	...
	public function testSaveData() {
		...
		$this->_connection->expects ( $this->any () )->method ( 'fetchAll' )->with ( $this->select )->will ( $this->returnValue ( [ 
				[ 
						...
						'product_id' => 1,
						'option_id' => 1,
						'product_super_attribute_id' => 131 
				],
				...
				[ 
						...
						'product_id' => 1,
						'option_id' => 1,
						'product_super_attribute_id' => 132 
				],
				[ 
						'attribute_id' => 132,
						'product_id' => 1,
		...
		$this->_entityModel->expects ( $this->any () )->method ( 'isRowAllowedToImport' )->will ( $this->returnCallback ( [ 
				$this,
				'isRowAllowedToImport' 
		] ) );
		...
		$this->_entityModel->expects ( $this->any () )->method ( 'getAttrSetIdToName' )->willReturn ( [ 
				4 => 'Default' 
		] );
