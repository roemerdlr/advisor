...
class Configurable extends \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType {
	 ...
	 */
	public function __construct(\Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory $attrSetColFac, \Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory $prodAttrColFac, \Magento\Framework\App\ResourceConnection $resource, array $params, \Magento\Catalog\Model\ProductTypes\ConfigInterface $productTypesConfig, \Magento\ImportExport\Model\ResourceModel\Helper $resourceHelper, \Magento\Catalog\Model\ResourceModel\Product\CollectionFactory $_productColFac) {
	 ...
	 *
	 * @param array $bunch
	 *        	- portion of products to process
	 * @param array $newSku
	 *        	- imported variations list
	 * @param array $oldSku
	 *        	- present variations list
	...
	protected function _loadSkuSuperAttributeValues($bunch, $newSku, $oldSku) {
		if ($this->_superAttributes) {
			...
			foreach ( $this->_productColFac->create ()->addFieldToFilter ( 'type_id', $this->_productTypesConfig->getComposableTypes () )->addFieldToFilter ( 'entity_id', [ 
					'in' => $productIds 
	...
	protected function _loadSkuSuperDataForBunch(array $bunch) {
		...
		if (! empty ( $productIds )) {
			...
			$select = $this->_connection->select ()->from ( [ 
					...
					'o' => $optionTable 
			], $this->_connection->quoteIdentifier ( 'o.attribute_id' ) . ' = ' . $this->_connection->quoteIdentifier ( 'o.attribute_id' ), [ 
					'option_id' 
	...
	protected function _deleteData() {
		...
		if (($this->_entityModel->getBehavior () == \Magento\ImportExport\Model\Import::BEHAVIOR_APPEND) && ! empty ( $this->_productSuperData ['product_id'] ) && ! empty ( $this->_simpleIdsToDelete )) {
			$quoted = $this->_connection->quoteInto ( 'IN (?)', [ 
					$this->_productSuperData ['product_id'] 
			] );
	...
	protected function _collectSuperData($rowData) {
		...
		$this->_productSuperData = [ 
				...
				'used_attributes' => empty ( $this->_skuSuperData [$productId] ) ? [ ] : $this->_skuSuperData [$productId],
				'assoc_ids' => [ ] 
		];
	 ...
	 *
	 * @throws \Exception
	 * @return \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	protected function configurableInBunch($bunch) {
		...
		foreach ( $bunch as $rowNum => $rowData ) {
			...
			if (($this->_type == $productData ['type_id']) && ($rowData == $this->_entityModel->isRowAllowedToImport ( $rowData, $rowNum ))) {
				return true;
			}
		}
		return false;
