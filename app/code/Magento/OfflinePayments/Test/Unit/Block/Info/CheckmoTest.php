...
class CheckmoTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetPayableTo($details, $expected) {
		$info = $this->getMock ( 'Magento\Payment\Model\Info', [ 
				'getAdditionalData' 
		], [ ], '', false );
		$info->expects ( $this->once () )->method ( 'getAdditionalData' )->willReturn ( serialize ( $details ) );
	...
	public function testConvertAdditionalDataIsNeverCalled() {
		...
		$info->expects ( $this->once () )->method ( 'getAdditionalData' )->willReturn ( serialize ( [ 
				'mailing_address' => 'blah@blah.com' 
		] ) );
		$this->_model->setData ( 'info', $info );
