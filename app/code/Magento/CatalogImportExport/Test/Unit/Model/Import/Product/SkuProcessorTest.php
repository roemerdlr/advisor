...
class SkuProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->skuProcessor = $this->getMock ( 'Magento\CatalogImportExport\Model\Import\Product\SkuProcessor', [ 
				'_getSkus' 
		], [ 
				$this->productFactory 
		], '' );
	...
	public function testReloadOldSkus() {
		...
		$this->skuProcessor->expects ( $this->once () )->method ( '_getSkus' )->willReturn ( $skuValue );
	...
	public function testGetOldSkusIfNotSet() {
		...
		$this->skuProcessor->expects ( $this->once () )->method ( '_getSkus' )->willReturn ( $expectedOldSkus );
