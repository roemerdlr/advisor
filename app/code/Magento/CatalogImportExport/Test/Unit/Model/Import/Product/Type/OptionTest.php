...
class OptionTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	protected function setUp() {
		...
		$catalogDataMock = $this->getMock ( 'Magento\Catalog\Helper\Data', [ 
				'__construct' 
		], [ ], '', false );
		...
		$modelClassArgs = [ 
				...
				$this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Option\CollectionFactory', [ ], [ ], '', false ),
				$this->getMock ( 'Magento\ImportExport\Model\ResourceModel\CollectionByPagesIteratorFactory', [ ], [ ], '', false ),
				$catalogDataMock,
				$scopeConfig,
				$timezoneInterface,
				$this->getMock ( 'Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorAggregatorInterface', [ ], [ ], '', false ),
		...
		$this->_modelMock = $this->getMockBuilder ( $modelClassName )->setConstructorArgs ( $modelClassArgs )->setMethods ( [ 
				'_getMultiRowFormat' 
		] )->getMock ();
	}
	...
	protected function _getModelDependencies($addExpectations = false, $deleteBehavior = false, $doubleOptions = false) {
		...
		if ($addExpectations) {
			if ($deleteBehavior) {
				$connection->expects ( $this->exactly ( 2 ) )->method ( 'quoteInto' )->will ( $this->returnCallback ( [ 
						$this,
						'stubQuoteInto' 
				] ) );
				$connection->expects ( $this->exactly ( 2 ) )->method ( 'delete' )->will ( $this->returnCallback ( [ 
						$this,
						'verifyDelete' 
				] ) );
			} else {
				$connection->expects ( $this->once () )->method ( 'insertMultiple' )->will ( $this->returnCallback ( [ 
						$this,
						'verifyInsertMultiple' 
				] ) );
				$connection->expects ( $this->exactly ( 6 ) )->method ( 'insertOnDuplicate' )->will ( $this->returnCallback ( [ 
						$this,
		...
		$resourceHelper = $this->getMock ( 'stdClass', [ 
	...
	protected function _getSourceDataMocks($addExpectations, $doubleOptions) {
		...
		if ($addExpectations) {
			$dataSourceModel->expects ( $this->at ( 0 ) )->method ( 'getNextBunch' )->will ( $this->returnValue ( $csvData ['data'] ) );
		...
		$this->_productEntity = $this->getMock ( 'Magento\CatalogImportExport\Model\Import\Product', [ 
				'getErrorAggregator' 
		], [ ], '', false );
		$this->_productEntity->method ( 'getErrorAggregator' )->willReturn ( $this->getErrorAggregatorObject () );
		
		$productModelMock = $this->getMock ( 'stdClass', [ 
				'getProductEntitiesInfo' 
		] );
		$productModelMock->expects ( $this->any () )->method ( 'getProductEntitiesInfo' )->will ( $this->returnValue ( $products ) );
		...
		$optionCollection = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\AbstractDb' )->setConstructorArgs ( [ 
				$entityFactory,
				$logger,
				$fetchStrategy 
		] )->setMethods ( [ 
		...
		$select = $this->getMock ( 'Magento\Framework\DB\Select', [ 
				'join',
				'where' 
		], [ ], '', false );
		...
		$optionCollection->expects ( $this->any () )->method ( 'getNewEmptyItem' )->will ( $this->returnCallback ( [ 
				$this,
				'getNewOptionMock' 
		] ) );
		...
		$collectionIterator = $this->getMock ( 'stdClass', [ 
				'iterate' 
		] );
		$collectionIterator->expects ( $this->any () )->method ( 'iterate' )->will ( $this->returnCallback ( [ 
				$this,
				'iterate' 
		] ) );
	...
	public function getNewOptionMock() {
		return $this->getMock ( 'Magento\Catalog\Model\Product\Option', [ 
				'__wakeup' 
		], [ ], '', false );
	}
	...
	private function _bypassModelMethodGetMultiRowFormat($rowData) {
		$this->_modelMock->expects ( $this->any () )->method ( '_getMultiRowFormat' )->will ( $this->returnValue ( [ 
				$rowData 
		] ) );
	}
	...
	public function testParseRequiredData() {
		$modelData = $this->getMock ( 'stdClass', [ 
				'getNextBunch' 
		] );
		$modelData->expects ( $this->at ( 0 ) )->method ( 'getNextBunch' )->will ( $this->returnValue ( [ 
				[ 
						'sku' => 'simple3',
		...
		$productModel = $this->getMock ( 'stdClass', [ 
				'getProductEntitiesInfo' 
		] );
		$productModel->expects ( $this->any () )->method ( 'getProductEntitiesInfo' )->will ( $this->returnValue ( [ ] ) );
		
		$productEntity = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product', [ ], [ ], '', false );
		
		$model = $this->objectManagerHelper->getObject ( 'Magento\CatalogImportExport\Model\Import\Product\Option', [ 
				'data' => [ 
