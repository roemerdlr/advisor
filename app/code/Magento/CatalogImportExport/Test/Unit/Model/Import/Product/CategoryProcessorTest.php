...
class CategoryProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$categoryCollection = $this->objectManagerHelper->getCollectionMock ( 'Magento\Catalog\Model\ResourceModel\Category\Collection', [ 
		...
		$categoryCollection->expects ( $this->exactly ( 3 ) )->method ( 'addAttributeToSelect' )->withConsecutive ( [ 
				'name' 
		], [ 
				'url_key' 
		], [ 
		...
		$categoryColFactory = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Category\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$categoryColFactory->method ( 'create' )->will ( $this->returnValue ( $categoryCollection ) );
		
		$categoryFactory = $this->getMock ( 'Magento\Catalog\Model\CategoryFactory', [ 
				'create' 
		], [ ], '', false );
