...
class ProductTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	protected function setUp() {
		...
		$this->_resourceFactory = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Proxy\Product\ResourceModelFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_setColFactory = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_productTypeFactory = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product\Type\Factory', [ 
				'create' 
		], [ ], '', false );
		$this->_linkFactory = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\LinkFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_proxyProdFactory = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Proxy\ProductFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_uploaderFactory = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\UploaderFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->_stockResItemFac = $this->getMock ( '\Magento\CatalogInventory\Model\ResourceModel\Stock\ItemFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->storeResolver = $this->getMockBuilder ( '\Magento\CatalogImportExport\Model\Import\Product\StoreResolver' )->setMethods ( [ 
				'getStoreCodeToId' 
		...
		$this->validator = $this->getMockBuilder ( '\Magento\CatalogImportExport\Model\Import\Product\Validator' )->setMethods ( [ 
				'isAttributeValid',
				'getMessages',
				'isValid' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	protected function _objectConstructor() {
		$this->optionFactory = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product\OptionFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testSaveProductAttributes() {
		...
		$this->skuProcessor->expects ( $this->once () )->method ( 'getNewSku' )->with ( $testSku )->willReturn ( [ 
				'entity_id' => self::ENTITY_ID 
		] );
		$this->_connection->expects ( $this->any () )->method ( 'quoteInto' )->willReturnCallback ( [ 
				$this,
				'returnQuoteCallback' 
		] );
		$this->_connection->expects ( $this->once () )->method ( 'delete' )->with ( $this->equalTo ( $testTable ), $this->equalTo ( '(store_id NOT IN (' . $storeId . ') AND attribute_id = ' . $attributeId . ' AND entity_id = ' . self::ENTITY_ID . ')' ) );
		...
		$this->_connection->expects ( $this->once () )->method ( 'insertOnDuplicate' )->with ( $testTable, $tableData, [ 
				'value' 
		] );
		$object = $this->invokeMethod ( $this->importProduct, '_saveProductAttributes', [ 
				$attributesData 
		] );
	...
	public function testGetMultipleValueSeparatorDefault() {
		...
		$this->assertEquals ( Import::DEFAULT_GLOBAL_MULTI_VALUE_SEPARATOR, $this->importProduct->getMultipleValueSeparator () );
	}
	...
	public function testDeleteProductsForReplacement() {
		$importProduct = $this->getMockBuilder ( '\Magento\CatalogImportExport\Model\Import\Product' )->disableOriginalConstructor ()->setMethods ( [ 
				'setParameters',
				'_deleteProducts' 
		] )->getMock ();
	...
	public function testGetMediaGalleryAttributeIdIfNotSetYet() {
		...
		$expectedId = '100';
		$attribute = $this->getMockBuilder ( '\Magento\Eav\Model\Entity\Attribute\AbstractAttribute' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId' 
		] )->getMockForAbstractClass ();
		$attribute->expects ( $this->once () )->method ( 'getId' )->willReturn ( $expectedId );
		$resource = $this->getMockBuilder ( '\Magento\CatalogImportExport\Model\Import\Proxy\Product\ResourceModel' )->disableOriginalConstructor ()->setMethods ( [ 
				'getAttribute' 
		] )->getMock ();
	...
	public function testValidateRowIsAlreadyValidated() {
		$rowNum = 0;
		$this->setPropertyValue ( $this->importProduct, '_validatedRows', [ 
				$rowNum => true 
		] );
	...
	public function testValidateRow($rowScope, $oldSku, $expectedResult, $behaviour = Import::BEHAVIOR_DELETE) {
		...
		$importProduct->expects ( $this->any () )->method ( 'getBehavior' )->willReturn ( $behaviour );
		...
		$this->setPropertyValue ( $importProduct, '_oldSku', [ 
	...
	public function testValidateRowDeleteBehaviourAddRowErrorCall() {
		...
		$importProduct->expects ( $this->exactly ( 2 ) )->method ( 'getBehavior' )->willReturn ( \Magento\ImportExport\Model\Import::BEHAVIOR_DELETE );
		$importProduct->expects ( $this->once () )->method ( 'getRowScope' )->willReturn ( \Magento\CatalogImportExport\Model\Import\Product::SCOPE_DEFAULT );
	...
	public function testGetStoreIdByCode($storeCode, $expectedResult) {
		$this->storeResolver->expects ( $this->any () )->method ( 'getStoreCodeToId' )->willReturn ( 'getStoreCodeToId value' );
	...
	public function testValidateRowProcessEntityIncrement() {
		...
		$rowData = [ 
				\Magento\CatalogImportExport\Model\Import\Product::COL_SKU => false 
		];
		// suppress validator
	...
	public function testValidateRowValidateExistingProductTypeAddNewSku() {
		...
		$expectedData = [ 
				...
				'attr_set_code' => $_attrSetIdToName [$oldSku [$sku] ['attr_set_id']] 
		] // attr_set_id_val
;
	...
	public function testValidateRowValidateExistingProductTypeAddErrorRowCall() {
		...
		$this->setPropertyValue ( $importProduct, '_oldSku', $oldSku );
		$importProduct->expects ( $this->once () )->method ( 'addRowError' )->with ( \Magento\CatalogImportExport\Model\Import\Product\Validator::ERROR_TYPE_UNSUPPORTED, $rowNum );
	 ...
	 */
	public function testValidateRowValidateNewProductTypeAddRowErrorCall($colType, $productTypeModelsColType, $colAttrSet, $attrSetNameToIdColAttrSet, $error) {
		$sku = 'sku';
		$rowNum = 0;
		$rowData = [ 
				\Magento\CatalogImportExport\Model\Import\Product::COL_SKU => $sku,
		...
		$importProduct->expects ( $this->once () )->method ( 'addRowError' )->with ( $error, $rowNum );
		$this->_suppressValidateRowOptionValidatorInvalidRows ( $importProduct );
	...
	public function testValidateRowValidateNewProductTypeGetNewSkuCall() {
		...
		$expectedData = [ 
				...
				'attr_set_code' => $rowData [\Magento\CatalogImportExport\Model\Import\Product::COL_ATTR_SET] 
		] // value
;
		$importProduct = $this->createModelMockWithErrorAggregator ( [ 
				'addRowError',
	...
	public function testValidateDefaultScopeNotValidAttributesResetSku() {
		...
		$scopeMock = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product', [ 
				'getRowScope' 
		], [ ], '', false );
	...
	public function testValidateValidateOptionEntity() {
		...
		$option = $this->getMockBuilder ( '\Magento\CatalogImportExport\Model\Import\Product\Option' )->disableOriginalConstructor ()->getMock ();
	...
	protected function createModelMockWithErrorAggregator(array $methods = [], array $errorAggregatorMethods = []) {
		...
		$importProduct = $this->getMockBuilder ( '\Magento\CatalogImportExport\Model\Import\Product' )->disableOriginalConstructor ()->setMethods ( $methods )->getMock ();
