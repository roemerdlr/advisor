...
class Product extends \Magento\ImportExport\Model\Export\Entity\AbstractEntity {
	...
	protected function prepareLinks(array $productIds) {
		...
		], '(cpe.entity_id = cpl.linked_product_id)', [ ] )->joinLeft ( [ 
				'cpla' => $this->_resourceModel->getTableName ( 'catalog_product_link_attribute' ) 
		], $this->_connection->quoteInto ( '(cpla.link_type_id = cpl.link_type_id AND cpla.product_link_attribute_code = ?)', 'position' ), [ ] )->joinLeft ( [ 
				'cplaq' => $this->_resourceModel->getTableName ( 'catalog_product_link_attribute' ) 
		], $this->_connection->quoteInto ( '(cplaq.link_type_id = cpl.link_type_id AND cplaq.product_link_attribute_code = ?)', 'qty' ), [ ] )->joinLeft ( [ 
				'cplai' => $this->_resourceModel->getTableName ( 'catalog_product_link_attribute_int' ) 
		], '(cplai.link_id = cpl.link_id AND cplai.product_link_attribute_id = cpla.product_link_attribute_id)', [ ] )->joinLeft ( [ 
				'cplad' => $this->_resourceModel->getTableName ( 'catalog_product_link_attribute_decimal' ) 
		], '(cplad.link_id = cpl.link_id AND cplad.product_link_attribute_id = cplaq.product_link_attribute_id)', [ ] )->where ( 'cpl.link_type_id IN (?)', array_values ( $this->_linkTypeProvider->getLinkTypes () ) )->where ( 'cpl.product_id IN (?)', $productIds );
	...
	/**
	 * Update data row with information about categories.
	 * Return true, if data row was updated
	 *
	 * @param
	 *        	array &$dataRow
	 * @param
			...
			foreach ( $collection as $itemId => $item ) {
				...
				foreach ( $this->_getExportAttrCodes () as $code ) {
					...
					if ($this->_attributeTypes [$code] === 'datetime') {
						$attrValue = $this->_localeDate->formatDateTime ( new \DateTime ( $attrValue ), \IntlDateFormatter::SHORT, \IntlDateFormatter::SHORT );
					}
					
					if ($storeId != Store::DEFAULT_STORE_ID && isset ( $data [$itemId] [Store::DEFAULT_STORE_ID] [$fieldName] ) && $data [$itemId] [Store::DEFAULT_STORE_ID] [$fieldName] == $attrValue) {
						continue;
					}
					...
					if ($this->_attributeTypes [$code] !== 'multiselect') {
						...
						if (! empty ( $this->collectedMultiselectsData [$storeId] [$itemId] [$code] )) {
							$additionalAttributes [$code] = $fieldName . ImportProduct::PAIR_NAME_VALUE_SEPARATOR . implode ( ImportProduct::PSEUDO_MULTI_LINE_SEPARATOR, $this->collectedMultiselectsData [$storeId] [$itemId] [$code] );
						}
					}
				}
				
				if (! empty ( $additionalAttributes )) {
					$data [$itemId] [$storeId] [self::COL_ADDITIONAL_ATTRIBUTES] = implode ( Import::DEFAULT_GLOBAL_MULTI_VALUE_SEPARATOR, $additionalAttributes );
				} else {
	...
	protected function collectMultirawData() {
		...
		foreach ( $collection as $item ) {
			...
			$rowWebsites [$item->getId ()] = array_intersect ( array_keys ( $this->_websiteIdToCode ), $item->getWebsites () );
	...
	protected function collectMultiselectValues($item, $attrCode, $storeId) {
		...
		$options = array_intersect_key ( $this->_attributeValues [$attrCode], array_flip ( $optionIds ) );
		if (! (isset ( $this->collectedMultiselectsData [Store::DEFAULT_STORE_ID] [$item->getId ()] [$attrCode] ) && $this->collectedMultiselectsData [Store::DEFAULT_STORE_ID] [$item->getId ()] [$attrCode] == $options)) {
