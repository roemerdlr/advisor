...
abstract class AbstractType {
	...
	public function __construct(\Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory $attrSetColFac, \Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory $prodAttrColFac, \Magento\Framework\App\ResourceConnection $resource, array $params) {
		...
		if ($this->isSuitable ()) {
			...
			$this->initMessageTemplates ( array_merge ( $this->_genericMessageTemplates, $this->_messageTemplates ) );
	 ...
	 *        	Refined attribute parameters.
	 * @param mixed $attribute        	
	 * @return \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	 ...
	 *
	 * @param array|string $attrSetData
	 *        	Product row data or simply attribute set name.
	 * @return array
	 */
	...
	protected function _initAttributes() {
		...
		$entityAttributes = $this->_connection->fetchAll ( $this->_connection->select ()->from ( [ 
		...
		] )->where ( $this->_connection->quoteInto ( 'attr.entity_type_id IN (?)', $entityId ) ) );
		$absentKeys = [ ];
		...
		foreach ( $entityAttributes as $attributeRow ) {
			if (isset ( self::$commonAttributesCache [$attributeRow ['attribute_id']] )) {
				...
				$this->_addAttributeParams ( $attributeRow ['attribute_set_name'], self::$commonAttributesCache [$attributeRow ['attribute_id']], $attribute );
			}
		}
		return $this;
	}
	...
	protected function attachAttributesById($attributeSetName, $attributeIds) {
		foreach ( $this->_prodAttrColFac->create ()->addFieldToFilter ( 'main_table.attribute_id', [ 
				'in' => $attributeIds 
		] ) as $attribute ) {
	...
	public function isRowValid(array $rowData, $rowNum, $isNewProduct = true) {
		...
		if (\Magento\CatalogImportExport\Model\Import\Product::SCOPE_NULL != $rowScope && ! empty ( $rowData [\Magento\CatalogImportExport\Model\Import\Product::COL_SKU] )) {
			foreach ( $this->_getProductAttributes ( $rowData ) as $attrCode => $attrParams ) {
				...
				if (isset ( $rowData [$attrCode] ) && strlen ( $rowData [$attrCode] )) {
					...
					if (\Magento\CatalogImportExport\Model\Import\Product::SCOPE_DEFAULT == $rowScope && ($isNewProduct || array_key_exists ( $attrCode, $rowData ))) {
						$this->_entityModel->addRowError ( RowValidatorInterface::ERROR_VALUE_IS_REQUIRED, $rowNum, $attrCode );
						$error = true;
					}
				}
			}
	 ...
	 * If method returns FALSE - model is not suitable for data processing.
	 *
	 * @return bool
	 */
	...
	public function prepareAttributesWithDefaultValueForSave(array $rowData, $withDefaultValue = true) {
		...
		foreach ( $this->_getProductAttributes ( $rowData ) as $attrCode => $attrParams ) {
			if (! $attrParams ['is_static']) {
				if (isset ( $rowData [$attrCode] ) && strlen ( $rowData [$attrCode] )) {
					$resultAttrs [$attrCode] = in_array ( $attrParams ['type'], [ 
							'select',
							'boolean' 
