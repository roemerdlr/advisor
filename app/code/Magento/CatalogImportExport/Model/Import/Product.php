...
class Product extends \Magento\ImportExport\Model\Import\Entity\AbstractEntity {
	...
	/**
	 * Value that means all entities (e.g.
	 * websites, groups etc.)
	 */
	const VALUE_ALL = 'all';
	 ...
	 * @param array $data        	
	 * @throws \Magento\Framework\Exception\LocalizedException @SuppressWarnings(PHPMD.ExcessiveParameterList)
	 */
	public function __construct(\Magento\Framework\Json\Helper\Data $jsonHelper, \Magento\ImportExport\Helper\Data $importExportData, \Magento\ImportExport\Model\ResourceModel\Import\Data $importData, \Magento\Eav\Model\Config $config, \Magento\Framework\App\ResourceConnection $resource, \Magento\ImportExport\Model\ResourceModel\Helper $resourceHelper, \Magento\Framework\Stdlib\StringUtils $string, ProcessingErrorAggregatorInterface $errorAggregator, \Magento\Framework\Event\ManagerInterface $eventManager, \Magento\CatalogInventory\Api\StockRegistryInterface $stockRegistry, \Magento\CatalogInventory\Api\StockConfigurationInterface $stockConfiguration, \Magento\CatalogInventory\Model\Spi\StockStateProviderInterface $stockStateProvider, \Magento\Catalog\Helper\Data $catalogData, \Magento\ImportExport\Model\Import\Config $importConfig, \Magento\CatalogImportExport\Model\Import\Proxy\Product\ResourceModelFactory $resourceFactory, \Magento\CatalogImportExport\Model\Import\Product\OptionFactory $optionFactory, \Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory $setColFactory, \Magento\CatalogImportExport\Model\Import\Product\Type\Factory $productTypeFactory, \Magento\Catalog\Model\ResourceModel\Product\LinkFactory $linkFactory, \Magento\CatalogImportExport\Model\Import\Proxy\ProductFactory $proxyProdFactory, \Magento\CatalogImportExport\Model\Import\UploaderFactory $uploaderFactory, \Magento\Framework\Filesystem $filesystem, \Magento\CatalogInventory\Model\ResourceModel\Stock\ItemFactory $stockResItemFac, \Magento\Framework\Stdlib\DateTime\TimezoneInterface $localeDate, DateTime $dateTime, \Psr\Log\LoggerInterface $logger, \Magento\Framework\Indexer\IndexerRegistry $indexerRegistry, Product\StoreResolver $storeResolver, Product\SkuProcessor $skuProcessor, Product\CategoryProcessor $categoryProcessor, Product\Validator $validator, ObjectRelationProcessor $objectRelationProcessor, TransactionManagerInterface $transactionManager, Product\TaxClassProcessor $taxClassProcessor, array $data = []) {
		...
		$this->_optionEntity = isset ( $data ['option_entity'] ) ? $data ['option_entity'] : $optionFactory->create ( [ 
				'data' => [ 
						'product_entity' => $this 
				] 
		] );
	...
	protected function _deleteProducts() {
		...
		while ( $bunch = $this->_dataSourceModel->getNextBunch () ) {
			...
			if ($idToDelete) {
				...
				try {
					$this->objectRelationProcessor->delete ( $this->transactionManager, $this->_connection, $productEntityTable, $this->_connection->quoteInto ( 'entity_id IN (?)', $idToDelete ), [ 
				...
				$this->_eventManager->dispatch ( 'catalog_product_import_bunch_delete_after', [ 
						'adapter' => $this,
						'bunch' => $bunch 
				] );
			}
	...
	protected function _importData() {
		...
		$this->_eventManager->dispatch ( 'catalog_product_import_finish_before', [ 
				'adapter' => $this 
		] );
		return true;
	}
	...
	protected function _initTypeModels() {
		...
		foreach ( $productTypes as $productTypeName => $productTypeConfig ) {
			...
			if (! ($model = $this->_productTypeFactory->create ( $productTypeConfig ['model'], [ 
					'params' => $params 
			] ))) {
	...
	protected function _saveLinks() {
		...
		foreach ( $this->_linkNameToId as $linkName => $linkId ) {
			$select = $this->_connection->select ()->from ( $resource->getTable ( 'catalog_product_link_attribute' ), [ 
		...
		while ( $bunch = $this->_dataSourceModel->getNextBunch () ) {
				...
				foreach ( $this->_linkNameToId as $linkName => $linkId ) {
					...
					if (isset ( $rowData [$linkName . 'sku'] )) {
						...
						foreach ( $linkSkus as $linkedSku ) {
							...
							if ((! is_null ( $this->skuProcessor->getNewSku ( $linkedSku ) ) || isset ( $this->_oldSku [$linkedSku] )) && $linkedSku != $sku) {
								...
								if ($linkedId == null) {
									...
									$this->_logger->critical ( new \Exception ( sprintf ( 'WARNING: Orphaned link skipped: From SKU %s (ID %d) to SKU %s, ' . 'Link type id: %d', $sku, $productId, $linkedSku, $linkId ) ) );
									continue;
								}
			...
			if (Import::BEHAVIOR_APPEND != $this->getBehavior () && $productIds) {
				$this->_connection->delete ( $mainTable, $this->_connection->quoteInto ( 'product_id IN (?)', array_unique ( $productIds ) ) );
			}
			if ($linkRows) {
				$this->_connection->insertOnDuplicate ( $mainTable, $linkRows, [ 
						'link_id' 
				] );
			}
			if ($positionRows) {
				// process linked product positions
				$this->_connection->insertOnDuplicate ( $resource->getAttributeTypeTable ( 'int' ), $positionRows, [ 
						'value' 
				] );
			}
		}
		...
		foreach ( $attributesData as $tableName => $skuData ) {
			...
			foreach ( $skuData as $sku => $attributes ) {
				...
				foreach ( $attributes as $attributeId => $storeValues ) {
					 ...
					 */
					$where [] = $this->_connection->quoteInto ( '(store_id NOT IN (?)', array_keys ( $storeValues ) ) . $this->_connection->quoteInto ( ' AND attribute_id = ?', $attributeId ) . $this->_connection->quoteInto ( ' AND entity_id = ?)', $productId );
			...
			$this->_connection->insertOnDuplicate ( $tableName, $tableData, [ 
					'value' 
			] );
		}
		return $this;
	...
	protected function _saveProductCategories(array $categoriesData) {
		...
		if ($categoriesData) {
			...
			if ($categoriesIn) {
				$this->_connection->insertOnDuplicate ( $tableName, $categoriesIn, [ 
						'position' 
				] );
			}
		}
	...
	protected function _saveProductEntity(array $entityRowsIn, array $entityRowsUp) {
		...
		if ($entityRowsIn) {
			...
			$newProducts = $this->_connection->fetchPairs ( $this->_connection->select ()->from ( $entityTable, [ 
					'sku',
					'entity_id' 
	...
	public function getImagesFromRow(array $rowData) {
		...
		foreach ( $this->_imagesArrayKeys as $column ) {
			...
			if (! empty ( $rowData [$column] )) {
				$images [$column] = array_unique ( explode ( $this->getMultipleValueSeparator (), $rowData [$column] ) );
			}
			...
			foreach ( $bunch as $rowNum => $rowData ) {
				...
				if (! empty ( $rowData ['_tier_price_website'] )) {
					$tierPrices [$rowSku] [] = [ 
							...
							'customer_group_id' => $rowData ['_tier_price_customer_group'] == self::VALUE_ALL ? 0 : $rowData ['_tier_price_customer_group'],
							'qty' => $rowData ['_tier_price_qty'],
							'value' => $rowData ['_tier_price_price'],
							'website_id' => self::VALUE_ALL == $rowData ['_tier_price_website'] || $priceIsGlobal ? 0 : $this->storeResolver->getWebsiteCodeToId ( $rowData ['_tier_price_website'] ) 
					];
				}
	...
	protected function _saveProductTierPrices(array $tierPriceData) {
		...
		if ($tierPriceData) {
			...
			if (Import::BEHAVIOR_APPEND != $this->getBehavior ()) {
				$this->_connection->delete ( $tableName, $this->_connection->quoteInto ( 'entity_id IN (?)', $delProductId ) );
			}
			if ($tierPriceIn) {
				$this->_connection->insertOnDuplicate ( $tableName, $tierPriceIn, [ 
						'value' 
				] );
			}
		}
	...
	protected function _saveMediaGallery(array $mediaGalleryData) {
		...
		$mediaGalleryTableName = $mediaGalleryTableName ?  : $this->_resourceFactory->create ()->getTable ( 'catalog_product_entity_media_gallery' );
		$mediaValueTableName = $mediaValueTableName ?  : $this->_resourceFactory->create ()->getTable ( 'catalog_product_entity_media_gallery_value' );
		$mediaEntityToValueTableName = $mediaEntityToValueTableName ?  : $this->_resourceFactory->create ()->getTable ( 'catalog_product_entity_media_gallery_value_to_entity' );
		$productIds = [ ];
		$imageNames = [ ];
		$multiInsertData = [ ];
		$valueToProductId = [ ];
		foreach ( $mediaGalleryData as $productSku => $mediaGalleryRows ) {
			...
			$insertedGalleryImgs = [ ];
			if (Import::BEHAVIOR_APPEND != $this->getBehavior ()) {
				$this->_connection->delete ( $mediaGalleryTableName, $this->_connection->quoteInto ( 'entity_id IN (?)', $productId ) );
			}
		...
		$oldMediaValues = $this->_connection->fetchAssoc ( $this->_connection->select ()->from ( $mediaGalleryTableName, [ 
				'value_id',
				'value' 
		] )->where ( 'value IN (?)', $imageNames ) );
		$this->_connection->insertOnDuplicate ( $mediaGalleryTableName, $multiInsertData, [ ] );
		$multiInsertData = [ ];
		$newMediaSelect = $this->_connection->select ()->from ( $mediaGalleryTableName, [ 
				'value_id',
				'value' 
		] )->where ( 'value IN (?)', $imageNames );
		...
		try {
			$this->_connection->insertOnDuplicate ( $mediaValueTableName, $multiInsertData, [ 
					'value_id',
					'store_id',
					'entity_id',
					'label',
			...
			$this->_connection->insertOnDuplicate ( $mediaEntityToValueTableName, $dataForSkinnyTable, [ 
					'value_id' 
			] );
		} catch ( \Exception $e ) {
			$this->_connection->delete ( $mediaGalleryTableName, $this->_connection->quoteInto ( 'value_id IN (?)', $newMediaValues ) );
		}
		return $this;
	}
	 ...
	 * @param array $rowData        	
	 * @param int $rowNum        	
	 * @return boolean @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 *         @SuppressWarnings(PHPMD.ExcessiveMethodLength)
	 */
	...
	private function _parseAdditionalAttributes($rowData) {
		...
		foreach ( $attributeNameValuePairs as $attributeNameValuePair ) {
			...
			if ($separatorPosition !== false) {
				...
				$value = substr ( $attributeNameValuePair, $separatorPosition + strlen ( self::PAIR_NAME_VALUE_SEPARATOR ) );
	...
	private function _customFieldsMapping($rowData) {
		...
		if (array_key_exists ( 'status', $rowData ) && $rowData ['status'] != \Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED) {
