...
class CcGenericConfigProvider implements ConfigProviderInterface {
			...
			if ($method->isAvailable ()) {
				$config = array_merge_recursive ( $config, [ 
						'payment' => [ 
								'ccform' => [ 
										'availableTypes' => [ 
												$methodCode => $this->getCcAvailableTypes ( $methodCode ) 
										],
										'months' => [ 
												$methodCode => $this->getCcMonths () 
										],
										'years' => [ 
												$methodCode => $this->getCcYears () 
										],
										'hasVerification' => [ 
												$methodCode => $this->hasVerification ( $methodCode ) 
										],
										'hasSsCardType' => [ 
												$methodCode => $this->hasSsCardType ( $methodCode ) 
										],
										'ssStartYears' => [ 
												$methodCode => $this->getSsStartYears () 
										],
										'cvvImageUrl' => [ 
												$methodCode => $this->getCvvImageUrl () 
										] 
								] 
						] 
				] );
	...
	protected function hasSsCardType($methodCode) {
		...
		$ssPresentations = array_intersect ( [ 
				'SS',
				'SM',
				'SO' 
		], $availableTypes );
