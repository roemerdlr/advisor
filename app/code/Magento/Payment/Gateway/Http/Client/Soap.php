...
class Soap implements ClientInterface {
	...
	public function placeRequest(TransferInterface $transferObject) {
		...
		$client = $this->clientFactory->create ( $transferObject->getClientConfig () ['wsdl'], [ 
				'trace' => true 
		] );
		...
		try {
			...
			$response = $client->__soapCall ( $transferObject->getMethod (), [ 
					$transferObject->getBody () 
			] );
			
			$result = $this->converter ? $this->converter->convert ( $response ) : [ 
					$response 
			];
