...
class Info extends \Magento\Framework\View\Element\Template {
	...
	public function getInfo() {
		...
		if (! $info instanceof \Magento\Payment\Model\InfoInterface) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'We cannot retrieve the payment info model object.' ) );
		}
		return $info;
	}
	...
	/**
	 * Getter for children PDF, as array.
	 * Analogue of $this->getChildHtml()
	 *
	...
	public function getChildPdfAsArray() {
		...
		foreach ( $this->getLayout ()->getChildBlocks ( $this->getNameInLayout () ) as $child ) {
			if (method_exists ( $child, 'toPdf' ) && is_callable ( [ 
					$child,
					'toPdf' 
			] )) {
				$result [] = $child->toPdf ();
