...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->config = $this->objectManagerHelper->getObject ( 'Magento\Payment\Model\Config', [ 
	...
	public function testGetActiveMethods($isActive) {
		$abstractMethod = $this->getMockBuilder ( 'Magento\Payment\Model\Method\AbstractMethod' )->disableOriginalConstructor ()->setMethods ( [ 
				'setId',
				'setStore',
				'getConfigData' 
		] )->getMock ();
		...
		$this->assertEquals ( $isActive ? [ 
				'active_method' => $abstractMethod 
		] : [ ], $this->config->getActiveMethods () );
	}
	...
	public function testGetMethodsInfo() {
		$expected = [ ];
		$this->dataStorage->expects ( $this->once () )->method ( 'get' )->with ( 'methods' )->will ( $this->returnValue ( $expected ) );
