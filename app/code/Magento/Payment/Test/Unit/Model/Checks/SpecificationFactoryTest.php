...
class SpecificationFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->_compositeFactory = $this->getMockBuilder ( 'Magento\Payment\Model\Checks\CompositeFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
	}
	public function testCreate() {
		...
		$specificationMapping = [ 
				self::SPECIFICATION_KEY => $specification 
		];
		...
		$this->assertEquals ( $expectedComposite, $modelFactory->create ( [ 
				self::SPECIFICATION_KEY 
		] ) );
	}
}
