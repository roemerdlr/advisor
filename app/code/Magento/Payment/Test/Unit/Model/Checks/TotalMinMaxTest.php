...
class TotalMinMaxTest extends \PHPUnit_Framework_TestCase {
	...
	public function testIsApplicable($baseGrandTotal, $expectation) {
		...
		$paymentMethod->expects ( $this->at ( 0 ) )->method ( 'getConfigData' )->with ( TotalMinMax::MIN_ORDER_TOTAL )->will ( $this->returnValue ( self::PAYMENT_MIN_TOTAL ) );
		$paymentMethod->expects ( $this->at ( 1 ) )->method ( 'getConfigData' )->with ( TotalMinMax::MAX_ORDER_TOTAL )->will ( $this->returnValue ( self::PAYMENT_MAX_TOTAL ) );
