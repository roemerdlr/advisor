...
class QuoteTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetDataUsingMethod() {
		$this->_quoteMock->expects ( $this->once () )->method ( 'getDataUsingMethod' )->with ( 'any key', 'any args' )->will ( $this->returnValue ( 'some value' ) );
		$this->assertEquals ( 'some value', $this->_model->getDataUsingMethod ( 'any key', 'any args' ) );
	}
	public function testGetTaxContainer() {
		$this->_quoteMock->expects ( $this->any () )->method ( 'getBillingAddress' )->will ( $this->returnValue ( 'billing address' ) );
		$this->_quoteMock->expects ( $this->any () )->method ( 'getShippingAddress' )->will ( $this->returnValue ( 'shipping address' ) );
	...
	public function testGetAllItems($pItem, $name, $qty, $price) {
		...
		$this->_quoteMock->expects ( $this->once () )->method ( 'getAllItems' )->will ( $this->returnValue ( [ 
				$itemMock 
		] ) );
	...
	public function testGetter($isVirtual, $getterMethod) {
		...
		$address->expects ( $this->any () )->method ( '__call' )->with ( $getterMethod )->will ( $this->returnValue ( $getterMethod ) );
