...
class CartTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAddCustomItem() {
		$this->_salesModelMock->expects ( $this->once () )->method ( 'getAllItems' )->will ( $this->returnValue ( $this->_getSalesModelItems () ) );
		...
		$this->assertTrue ( $customItem->getName () == 'test' && $customItem->getQty () == 10 && $customItem->getAmount () == 10.5 && $customItem->getId () == 'some_id' );
	}
	...
	/**
	 * Test all amount specific methods i.e.
	 * add...(), set...(), get...()
	 */
	...
	public function cartDataProvider() {
		return [
				...
				[ 
						[ 
								'transfer_shipping' => false,
								'transfer_discount' => false 
						],
						$this->_getSalesModelItems (),
						[ 
								'BaseDiscountAmount' => 15.0,
	...
	/**
	 * Return true if arrays of cart sales items are equal, false otherwise.
	 * Elements order not considered
	 *
	...
	protected function _collectItemsAndAmounts($transferFlags, $salesModelItems, $salesModelAmounts) {
		...
		$this->_eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( $this->equalTo ( 'payment_cart_collect_items_and_amounts' ), $this->equalTo ( [ 
				'cart' => $this->_model 
		] ) );
