...
class CctypeTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * @var array
	 */
	protected $_cctypesArray = [ 
			'allowed_cc_type' => 'name' 
	];
	 ...
	 * @var array
	 */
	protected $_expectedToOptionsArray = [ 
			[ 
					'value' => 'allowed_cc_type',
					'label' => 'name' 
			] 
	];
	public function setUp() {
		$this->_paymentConfig = $this->getMockBuilder ( 'Magento\Payment\Model\Config' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
	...
	private function _preparePaymentConfig() {
		$this->_paymentConfig->expects ( $this->once () )->method ( 'getCcTypes' )->will ( $this->returnValue ( $this->_cctypesArray ) );
	}
}
