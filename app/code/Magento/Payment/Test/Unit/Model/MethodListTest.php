...
class MethodListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->methodList = $this->objectManager->getObject ( 'Magento\Payment\Model\MethodList', [ 
	...
	public function testGetAvailableMethods() {
		...
		$methodMock = $this->getMock ( 'Magento\Payment\Model\Method\AbstractMethod', [ 
				'setInfoInstance',
				'getCode' 
		], [ ], '', false );
		$methodMock->expects ( $this->once () )->method ( 'getCode' )->willReturn ( Free::PAYMENT_METHOD_FREE_CODE );
		...
		$this->assertEquals ( [ 
				$methodMock 
		], $this->methodList->getAvailableMethods ( $quoteMock ) );
	}
}
