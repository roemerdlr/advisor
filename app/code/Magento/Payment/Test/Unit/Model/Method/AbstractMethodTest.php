...
class AbstractMethodTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->eventManagerMock = $this->getMockBuilder ( 'Magento\Framework\Event\ManagerInterface' )->setMethods ( [ 
				'dispatch' 
		] )->getMockForAbstractClass ();
		$this->quoteMock = $this->getMockBuilder ( 'Magento\Quote\Api\Data\CartInterface' )->setMethods ( [ 
				'getStoreId' 
		] )->getMockForAbstractClass ();
		$contextMock = $this->getMockBuilder ( 'Magento\Framework\Model\Context' )->disableOriginalConstructor ()->setMethods ( [ 
				'getEventDispatcher' 
		] )->getMock ();
		$contextMock->expects ( $this->once () )->method ( 'getEventDispatcher' )->willReturn ( $this->eventManagerMock );
		$this->loggerMock = $this->getMockBuilder ( '\Magento\Payment\Model\Method\Logger' )->setConstructorArgs ( [ 
		...
		$this->payment = $helper->getObject ( 'Magento\Payment\Test\Unit\Model\Method\AbstractMethod\Stub', [ 
	...
	public function testDebugData() {
		...
		$this->loggerMock->expects ( $this->once () )->method ( 'debug' )->with ( $this->equalTo ( $debugData ) );
	...
	public function testIsAvailable($result) {
		...
		$this->eventManagerMock->expects ( $result ? $this->once () : $this->never () )->method ( 'dispatch' )->with ( $this->equalTo ( 'payment_method_is_active' ), $this->countOf ( 3 ) );
