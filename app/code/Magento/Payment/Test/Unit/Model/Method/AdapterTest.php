...
class AdapterTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->valueHandlerPool = $this->getMock ( 'Magento\Payment\Gateway\Config\ValueHandlerPoolInterface' );
		$this->validatorPool = $this->getMock ( 'Magento\Payment\Gateway\Validator\ValidatorPoolInterface' );
		$this->commandPool = $this->getMock ( 'Magento\Payment\Gateway\Command\CommandPoolInterface' );
		$this->paymentDataObjectFactory = $this->getMockBuilder ( 'Magento\Payment\Gateway\Data\PaymentDataObjectFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->code = 'CODE';
		$this->formBlockType = '\FormBlock';
		$this->infoBlockType = '\InfoBlock';
		
		$this->adapter = new Adapter ( $this->eventManager, $this->valueHandlerPool, $this->paymentDataObjectFactory, $this->code, $this->formBlockType, $this->infoBlockType, $this->commandPool, $this->validatorPool );
	}
	public function testIsAvailableNotActive() {
		...
		$activeValueHandler->expects ( static::once () )->method ( 'handle' )->with ( [ 
				'field' => 'active' 
		] )->willReturn ( false );
		
		$this->eventManager->expects ( static::never () )->method ( 'dispatch' );
