...
class GatewayCommandTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->requestBuilderMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Request\BuilderInterface' )->getMockForAbstractClass ();
		$this->transferFactoryMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Http\TransferFactoryInterface' )->getMockForAbstractClass ();
		$this->clientMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Http\ClientInterface' )->getMockForAbstractClass ();
		$this->responseHandlerMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Response\HandlerInterface' )->getMockForAbstractClass ();
		$this->validatorMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Validator\ValidatorInterface' )->getMockForAbstractClass ();
		
		$this->command = new GatewayCommand ( $this->requestBuilderMock, $this->transferFactoryMock, $this->clientMock, $this->responseHandlerMock, $this->validatorMock );
	}
	public function testExecute() {
		...
		$this->validatorMock->expects ( static::once () )->method ( 'validate' )->with ( array_merge ( $commandSubject, [ 
				'response' => $response 
	...
	public function testExecuteValidationFail() {
		...
		$validationResult->expects ( static::once () )->method ( 'getFailsDescription' )->willReturn ( [ ] );
