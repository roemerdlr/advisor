...
class BuilderCompositeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testBuild() {
		...
		$customerBuilder->expects ( static::once () )->method ( 'build' )->willReturn ( [ 
				'user' => 'Mrs G. Crump',
				'address' => '46 Egernon Crescent' 
		] );
		$productBuilder->expects ( static::once () )->method ( 'build' )->willReturn ( [ 
				'amount' => 10.00,
				'currency' => 'pound',
				'item' => 'gas cooker',
				'quantity' => 1 
		] );
		$magentoBuilder->expects ( static::once () )->method ( 'build' )->willReturn ( [ 
				'url' => 'https://url.in' 
		] );
