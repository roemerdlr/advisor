...
class CountryValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resultFactoryMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Validator\ResultInterfaceFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->resultMock = $this->getMockBuilder ( 'Magento\Payment\Gateway\Validator\Result' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = new \Magento\Payment\Gateway\Validator\CountryValidator ( $this->resultFactoryMock, $this->configMock );
	}
	...
	public function validateAllowspecificTrueDataProvider() {
		return [ 
				[ 
						...
						'US,UK,CA',
						true 
				], // $storeId, $country, $allowspecific, $specificcountry, $isValid
				[ 
						1,
						'BJ',
						1,
