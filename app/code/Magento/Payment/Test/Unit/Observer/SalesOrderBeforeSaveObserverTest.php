...
class SalesOrderBeforeSaveObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->salesOrderBeforeSaveObserver = $this->objectManagerHelper->getObject ( 'Magento\Payment\Observer\SalesOrderBeforeSaveObserver', [ ] );
	...
	public function testSalesOrderBeforeSaveMethodNotFree() {
		...
		$order = $this->_getPreparedOrderMethod ( 'not_free', $neverInvokedMethods );
	...
	public function testSalesOrderBeforeSaveCantUnhold() {
		...
		$order = $this->_getPreparedOrderMethod ( 'free', [ 
				'canUnhold' 
		] + $neverInvokedMethods );
		$this->_prepareNeverInvokedOrderMethods ( $order, $neverInvokedMethods );
		$this->eventMock->expects ( $this->once () )->method ( 'getOrder' )->will ( $this->returnValue ( $order ) );
	...
	public function testSalesOrderBeforeSaveIsClosed() {
		...
		$order->expects ( $this->once () )->method ( 'getState' )->will ( $this->returnValue ( \Magento\Sales\Model\Order::STATE_CLOSED ) );
		$this->salesOrderBeforeSaveObserver->execute ( $this->observerMock );
	}
	...
	private function _getPreparedOrderMethod($methodCode, $orderMethods = []) {
		$order = $this->getMockBuilder ( 'Magento\Sales\Model\Order' )->disableOriginalConstructor ()->setMethods ( array_merge ( [ 
				'__wakeup',
				'getPayment' 
		], $orderMethods ) )->getMock ();
		$paymentMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Payment' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
