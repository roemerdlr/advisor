...
class UpdateOrderStatusForPaymentMethodsObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->updateOrderStatusForPaymentMethodsObserver = $this->objectManagerHelper->getObject ( 'Magento\Payment\Observer\UpdateOrderStatusForPaymentMethodsObserver', [ 
				'salesOrderConfig' => $this->orderConfigMock,
				'paymentConfig' => $this->paymentConfigMock,
				'resourceConfig' => $this->coreResourceConfigMock 
		] );
		
		$this->observerMock = $this->getMockBuilder ( 'Magento\Framework\Event\Observer' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
	}
	public function testUpdateOrderStatusForPaymentMethodsNotNewState() {
		$this->_prepareEventMockWithMethods ( [ 
				'getState' 
		] );
	...
	private function _getPreparedActiveMethods() {
		$method1 = $this->getMockBuilder ( 'Magento\Payment\Model\MethodInterface' )->getMockForAbstractClass ();
		$method1->expects ( $this->once () )->method ( 'getConfigData' )->with ( 'order_status' )->will ( $this->returnValue ( self::ORDER_STATUS ) );
		$method1->expects ( $this->once () )->method ( 'getCode' )->will ( $this->returnValue ( self::METHOD_CODE ) );
		
		$method2 = $this->getMockBuilder ( 'Magento\Payment\Model\MethodInterface' )->getMockForAbstractClass ();
		$method2->expects ( $this->once () )->method ( 'getConfigData' )->with ( 'order_status' )->will ( $this->returnValue ( 'not_a_status' ) );
