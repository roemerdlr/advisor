...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_storeManager = $this->getMockBuilder ( '\Magento\Store\Model\StoreManager' )->setMethods ( [ 
				'getStore' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->_eventManager = $this->getMockBuilder ( '\Magento\Framework\Event\ManagerInterface' )->setMethods ( [ 
				'dispatch' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->_escaper = $this->getMock ( '\Magento\Framework\Escaper', null, [ ], '', true );
		$context = $helper->getObject ( 'Magento\Framework\View\Element\Template\Context', [ 
				'storeManager' => $this->_storeManager,
				'eventManager' => $this->_eventManager,
				'escaper' => $this->_escaper 
		] );
		$this->_object = $helper->getObject ( 'Magento\Payment\Block\Form', [ 
				'context' => $context 
		] );
	}
	...
	public function testGetMethodCode() {
		...
		$method->expects ( $this->once () )->method ( 'getCode' )->will ( $this->returnValue ( 'method_code' ) );
		$this->_object->setData ( 'method', $method );
	...
	public function testGetInfoData($field, $value, $expected) {
		$methodInstance = $this->getMockBuilder ( '\Magento\Payment\Model\Method\AbstractMethod' )->setMethods ( [ 
				'getData' 
		] )->disableOriginalConstructor ()->getMock ();
