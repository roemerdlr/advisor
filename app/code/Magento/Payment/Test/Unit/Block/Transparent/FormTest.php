...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->urlBuilderMock = $this->getMockBuilder ( '\Magento\Framework\UrlInterface' )->setMethods ( [ 
				'getUrl' 
		] )->getMockForAbstractClass ();
		...
		$this->methodMock = $this->getMockBuilder ( 'Magento\Payment\Model\Method\TransparentInterface' )->getMock ();
		
		$this->checkoutSessionMock = $this->getMockBuilder ( 'Magento\Checkout\Model\Session' )->setMethods ( [ ] )->disableOriginalConstructor ()->getMock ();
		
		$paymentConfigMock = $this->getMockBuilder ( 'Magento\Payment\Model\Config' )->setMethods ( [ ] )->disableOriginalConstructor ()->getMock ();
		
		$this->form = new FormTesting ( $context, $paymentConfigMock, $this->checkoutSessionMock );
	}
	public function testIsAjaxRequest() {
		$this->requestMock->expects ( $this->exactly ( 2 ) )->method ( 'getParam' )->with ( 'isAjax' )->willReturnOnConsecutiveCalls ( true, false );
	...
	private function initializeMethodWithConfigMock(array $configMap = []) {
		$configInterface = $this->getMockBuilder ( 'Magento\Payment\Model\Method\ConfigInterface' )->getMock ();
		
		$configInterface->expects ( $this->any () )->method ( 'getValue' )->willReturnMap ( $configMap );
		
		$this->methodMock->expects ( $this->any () )->method ( 'getConfigInterface' )->willReturn ( $configInterface );
	}
	
	/**
	 * Data provider for testGetMethodConfigData
	 *
	 * @see testGetMethodConfigData @case #1 Set string value
	 ...
	 *
	 * @see testGetCgiUrl @case #1 The sandboxFlag is 1 we expected cgi_url_test_mode_value
	...
	public function testGetOrderUrl() {
		...
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( $orderUrlPattern )->willReturn ( $builtOrderUrl );
	...
	public function testToHtmlShouldRender() {
		...
		$paymentMock->expects ( $this->once () )->method ( 'getMethodInstance' )->willReturn ( $this->methodMock );
		
		$this->form->toHtml ();
	}
	public function testToHtmlShouldNotRenderEmptyQuote() {
		$this->checkoutSessionMock->expects ( $this->once () )->method ( 'getQuote' )->willReturn ( null );
	...
	public function testToHtmlShouldNotRenderEmptyPayment() {
		...
		$this->checkoutSessionMock->expects ( $this->once () )->method ( 'getQuote' )->willReturn ( $quoteMock );
		$quoteMock->expects ( $this->once () )->method ( 'getPayment' )->willReturn ( null );
	...
	public function testGetMethodNotTransparentInterface() {
		$this->setExpectedException ( 'Magento\Framework\Exception\LocalizedException', __ ( 'We cannot retrieve the transparent payment method model object.' ) );
