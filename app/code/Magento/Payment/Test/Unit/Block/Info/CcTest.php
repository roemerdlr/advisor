...
class CcTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$context->expects ( $this->any () )->method ( 'getLocaleDate' )->will ( $this->returnValue ( $this->localeDate ) );
		$this->model = $this->objectManager->getObject ( 'Magento\Payment\Block\Info\Cc', [ 
	...
	public function testGetCcTypeName($configCcTypes, $ccType, $expected) {
		...
		$paymentInfo = $this->getMock ( 'Magento\Payment\Model\Info', [ 
				'getCcType' 
		], [ ], '', false );
	...
	public function testGetCcExpDate($ccExpMonth, $ccExpYear) {
		...
		$paymentInfo->expects ( $this->any () )->method ( 'getCcExpMonth' )->willReturn ( $ccExpMonth );
		$paymentInfo->expects ( $this->any () )->method ( 'getCcExpYear' )->willReturn ( $ccExpYear );
		$this->model->setData ( 'info', $paymentInfo );
