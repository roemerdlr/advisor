...
class Carrier extends \Magento\Dhl\Model\AbstractDhl implements \Magento\Shipping\Model\Carrier\CarrierInterface {
	...
	protected $_requestVariables = [ 
			...
			'shipping_key' => [ 
					'code' => 'dhl_shipping_key',
					'setCode' => 'shipping_key' 
			],
			'shipping_intlkey' => [ 
					'code' => 'dhl_shipping_intl_key',
					'setCode' => 'shipping_intl_key' 
			],
			'shipment_type' => [ 
					'code' => 'dhl_shipment_type',
					'setCode' => 'shipment_type' 
			],
			'dutiable' => [ 
					'code' => 'dhl_dutiable',
					'setCode' => 'dutiable' 
			...
			'dutypaymenttype' => [ 
					'code' => 'dhl_duty_payment_type',
					'setCode' => 'duty_payment_type' 
			],
			'contentdesc' => [ 
					'code' => 'dhl_content_desc',
	...
	protected function _getDefaultValue($origValue, $pathToValue) {
		if (! $origValue) {
			$origValue = $this->_scopeConfig->getValue ( $pathToValue, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->getStore () );
		}
	...
	public function collectRates(RateRequest $request) {
		...
		$origCompanyName = $this->_getDefaultValue ( $requestDhl->getOrigCompanyName (), \Magento\Store\Model\Information::XML_PATH_STORE_INFO_NAME );
		...
		$requestDhl->setOrigCompanyName ( $origCompanyName )->setCountryId ( $origCountryId )->setOrigState ( $origState )->setOrigCity ( $origCity )->setOrigPostal ( $origPostcode );
	...
	public function getAllowedMethods() {
		...
		if ($this->_isDomestic) {
			$allowedMethods = array_merge ( explode ( ',', $this->getConfigData ( 'doc_methods' ) ), explode ( ',', $this->getConfigData ( 'nondoc_methods' ) ) );
		} else {
			switch ($contentType) {
	 ...
	 *
	 * @param string $doc
	 *        	Package content type (doc/non-doc) see DHL_CONTENT_TYPE_* constants
	 * @return array
	 */
	 ...
	 *
	 * @param string $code
	 *        	One-symbol code (see getDhlProducts())
	...
	protected function _getDimension($dimension, $configWeightUnit = false) {
		...
		if ($configDimensionUnit != $countryDimensionUnit) {
			$dimension = $this->_carrierHelper->convertMeasureDimension ( round ( $dimension, 3 ), $configDimensionUnit, $countryDimensionUnit );
		}
	...
	protected function _buildQuotesRequestXml() {
		...
		$nodeBkgDetails->addChild ( 'Date', (new \DateTime ())->format ( \Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT ) );
				...
				if (is_object ( $xml )) {
					if (in_array ( $xml->getName (), [ 
						...
						if ($this->_isShippingLabelFlag) {
							...
							throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Error #%1 : %2', trim ( $code ), trim ( $data ) ) );
						}
	...
	protected function _addRate(\SimpleXMLElement $shipmentDetails) {
		if (isset ( $shipmentDetails->ProductShortName ) && isset ( $shipmentDetails->ShippingCharge ) && isset ( $shipmentDetails->GlobalProductCode ) && isset ( $shipmentDetails->CurrencyCode ) && array_key_exists ( ( string ) $shipmentDetails->GlobalProductCode, $this->getAllowedMethods () )) {
			...
			if ($currencyCode != $baseCurrencyCode) {
				...
				$rates = $currency->getCurrencyRates ( $currencyCode, [ 
						$baseCurrencyCode 
				] );
				if (! empty ( $rates ) && isset ( $rates [$baseCurrencyCode] )) {
					...
					$rates = $currency->getCurrencyRates ( $baseCurrencyCode, [ 
							$currencyCode 
					] );
	...
	public function proccessAdditionalValidation(\Magento\Framework\DataObject $request) {
		...
		$countryParams = $this->getCountryParams ( $this->_scopeConfig->getValue ( Shipment::XML_PATH_STORE_COUNTRY_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $request->getStoreId () ) );
	...
	protected function _doRequest() {
		...
		/**
		 * Commodity
		 * The CommodityCode element contains commodity code for shipment contents.
		 * Its
		...
		/**
		 * Reference
		 * This element identifies the reference information.
		 * It is an optional field in the
	...
	protected function _parseXmlTrackingResponse($trackings, $response) {
		...
		if (strlen ( trim ( $response ) ) > 0) {
			...
			if (is_object ( $xml ) && (isset ( $xml->Response->Status->ActionStatus ) && $xml->Response->Status->ActionStatus == 'Failure' || isset ( $xml->GetQuoteResponse->Note->Condition ))) {
				if (isset ( $xml->Response->Status->Condition )) {
					$nodeCondition = $xml->Response->Status->Condition;
				}
				$code = isset ( $nodeCondition->ConditionCode ) ? ( string ) $nodeCondition->ConditionCode : 0;
				...
				foreach ( $xml->AWBInfo as $awbinfo ) {
					...
					if (isset ( $shipmentInfo->ShipmentEvent )) {
						foreach ( $shipmentInfo->ShipmentEvent as $shipmentEvent ) {
							$shipmentEventArray = [ ];
							$shipmentEventArray ['activity'] = ( string ) $shipmentEvent->ServiceEvent->EventCode . ' ' . ( string ) $shipmentEvent->ServiceEvent->Description;
							$shipmentEventArray ['deliverydate'] = ( string ) $shipmentEvent->Date;
							$shipmentEventArray ['deliverytime'] = ( string ) $shipmentEvent->Time;
							$shipmentEventArray ['deliverylocation'] = ( string ) $shipmentEvent->ServiceArea->Description . ' [' . ( string ) $shipmentEvent->ServiceArea->ServiceAreaCode . ']';
	...
	protected function isDutiable($origCountryId, $destCountryId) {
		...
		return self::DHL_CONTENT_TYPE_NON_DOC == $this->getConfigData ( 'content_type' ) && ! $this->_isDomestic;
	}
}
