...
abstract class Checkout extends \Magento\Checkout\Controller\Action implements \Magento\Checkout\Controller\Express\RedirectLoginInterface {
	...
	public function dispatch(RequestInterface $request) {
		...
		} elseif (! $checkoutSessionQuote->getIsMultiShipping () && ! in_array ( $action, [ 
				'login',
				'register',
				'success' 
		] )) {
		...
		if ($this->_getCheckoutSession ()->getCartWasUpdated ( true ) && ! in_array ( $action, [ 
				'index',
				'login',
				'register',
				'addresses',
	...
	public function getCustomerBeforeAuthUrl() {
		return $this->_objectManager->create ( 'Magento\Framework\UrlInterface' )->getUrl ( '*/*', [ 
				'_secure' => true 
		] );
	}
