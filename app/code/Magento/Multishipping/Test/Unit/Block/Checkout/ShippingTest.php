...
class ShippingTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->multiShippingMock = $this->getMock ( 'Magento\Multishipping\Model\Checkout\Type\Multishipping', [ ], [ ], '', false );
		...
		$this->model = $objectManager->getObject ( 'Magento\Multishipping\Block\Checkout\Shipping', [ 
	...
	public function testGetAddresses() {
		...
		$quoteMock->expects ( $this->once () )->method ( 'getAllShippingAddresses' )->will ( $this->returnValue ( [ 
				'expected array' 
		] ) );
		$this->assertEquals ( [ 
				'expected array' 
	...
	public function testGetAddressShippingMethod() {
		...
		$addressMock->expects ( $this->once () )->method ( 'getShippingMethod' )->will ( $this->returnValue ( 'expected shipping method' ) );
	...
	public function testGetShippingRates() {
		...
		$addressMock->expects ( $this->once () )->method ( 'getGroupedAllShippingRates' )->will ( $this->returnValue ( [ 
				'expected array' 
		] ) );
		$this->assertEquals ( [ 
				'expected array' 
		], $this->model->getShippingRates ( $addressMock ) );
	}
	public function testGetCarrierName() {
		$carrierCode = 'some carrier code';
		$name = 'some name';
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( 'carriers/' . $carrierCode . '/title', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->will ( $this->returnValue ( $name ) );
		
		$this->assertEquals ( $name, $this->model->getCarrierName ( $carrierCode ) );
	}
	public function testGetCarrierNameWithEmptyName() {
		$carrierCode = 'some carrier code';
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( 'carriers/' . $carrierCode . '/title', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->will ( $this->returnValue ( null ) );
