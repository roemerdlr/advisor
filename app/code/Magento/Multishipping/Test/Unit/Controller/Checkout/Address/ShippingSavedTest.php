...
class ShippingSavedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->objectManagerMock->expects ( $this->any () )->method ( 'get' )->with ( 'Magento\Multishipping\Model\Checkout\Type\Multishipping' )->willReturn ( $this->checkoutMock );
		...
		$this->controller = new \Magento\Multishipping\Controller\Checkout\Address\ShippingSaved ( $this->contextMock, $this->addressRepositoryMock, $this->filterBuilderMock, $this->criteriaBuilderMock );
	}
	...
	private function mockCustomerAddressRepository($customerId, array $addresses) {
		...
		$this->criteriaBuilderMock->expects ( $this->once () )->method ( 'addFilters' )->with ( [ 
				$filterMock 
		] )->willReturnSelf ();
		...
		$this->addressRepositoryMock->expects ( $this->once () )->method ( 'getList' )->with ( $searchCriteriaMock )->willReturn ( $searchResultMock );
