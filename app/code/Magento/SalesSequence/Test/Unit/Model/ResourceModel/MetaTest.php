...
class MetaTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->dbContext = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\Context', [ ], [ ], '', false );
		$this->metaFactory = $this->getMock ( 'Magento\SalesSequence\Model\MetaFactory', [ 
				'create' 
		], [ ], '', false );
		$this->resourceProfile = $this->getMock ( 'Magento\SalesSequence\Model\ResourceModel\Profile', [ 
				'loadActiveProfile',
				'save' 
		], [ ], '', false );
		...
		$this->meta = $this->getMock ( 'Magento\SalesSequence\Model\Meta', [ ], [ ], '', false );
		$this->profile = $this->getMock ( 'Magento\SalesSequence\Model\Profile', [ ], [ ], '', false );
		$this->resource = new Meta ( $this->dbContext, $this->metaFactory, $this->resourceProfile );
	}
	public function testLoadBy() {
		...
		$this->select->expects ( $this->at ( 0 ) )->method ( 'from' )->with ( $metaTableName, [ 
				$metaIdFieldName 
		] )->willReturn ( $this->select );
		$this->select->expects ( $this->at ( 1 ) )->method ( 'where' )->with ( 'entity_type = :entity_type AND store_id = :store_id' )->willReturn ( $this->select );
		$this->connectionMock->expects ( $this->once () )->method ( 'fetchOne' )->with ( $this->select, [ 
	...
	private function stepCheckSaveWithActiveProfile($metaData) {
		$this->select->expects ( $this->at ( 2 ) )->method ( 'from' )->with ( 'sequence_meta', '*', null )->willReturn ( $this->select );
