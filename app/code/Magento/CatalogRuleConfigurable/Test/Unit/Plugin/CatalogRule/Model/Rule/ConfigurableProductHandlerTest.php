...
class ConfigurableProductHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->configurableMock = $this->getMock ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable', [ 
				'getChildrenIds' 
		], [ ], '', false );
		$this->configurableProductsProviderMock = $this->getMock ( 'Magento\CatalogRuleConfigurable\Plugin\CatalogRule\Model\ConfigurableProductsProvider', [ 
				'getIds' 
		], [ ], '', false );
		$this->ruleMock = $this->getMock ( 'Magento\CatalogRule\Model\Rule', [ ], [ ], '', false );
		
		$this->configurableProductHandler = new ConfigurableProductHandler ( $this->configurableMock, $this->configurableProductsProviderMock );
	}
	...
	public function testAfterGetMatchingProductIdsWithSimpleProduct() {
		...
		$productIds = [ 
				'product' => 'valid results' 
		];
		$this->assertEquals ( $productIds, $this->configurableProductHandler->afterGetMatchingProductIds ( $this->ruleMock, $productIds ) );
	}
	...
	public function testAfterGetMatchingProductIdsWithConfigurableProduct() {
		...
		], $this->configurableProductHandler->afterGetMatchingProductIds ( $this->ruleMock, [ 
				'conf1' => [ 
						0 => true,
						1 => true 
				],
