...
class FactoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_vbFactoryMock = $this->getMock ( 'Magento\Framework\Validator\UniversalFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_vbMock = $this->getMock ( 'Magento\Framework\Validator\Builder', [ ], [ ], '', false );
		$this->_validatorMock = $this->getMock ( 'Magento\Framework\Validator\ValidatorInterface', [ ], [ ], '', false );
		
		$objectManager = new ObjectManager ( $this );
		$this->_factory = $objectManager->getObject ( 'Magento\GoogleAdwords\Model\Validator\Factory', [ 
				'validatorBuilderFactory' => $this->_vbFactoryMock 
		] );
	}
	public function testCreateColorValidator() {
		$currentColor = 'fff';
		$message = sprintf ( 'Conversion Color value is not valid "%s". Please set hexadecimal 6-digit value.', $currentColor );
		
		$this->_vbFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\Validator\Builder', [ 
				'constraints' => [ 
						[ 
								'alias' => 'Regex',
								'type' => '',
		...
		$this->_vbMock->expects ( $this->once () )->method ( 'createValidator' )->will ( $this->returnValue ( $this->_validatorMock ) );
								...
								'options' => [ 
										'methods' => [ 
												[ 
														...
														'arguments' => [ 
																[ 
																		IntUtils::NOT_INT => $message,
																		IntUtils::INVALID => $message 
																] 
														] 
												] 
										] 
