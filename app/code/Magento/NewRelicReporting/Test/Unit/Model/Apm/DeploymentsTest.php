...
class DeploymentsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->zendClientFactoryMock = $this->getMockBuilder ( 'Magento\Framework\HTTP\ZendClientFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->zendClientMock = $this->getMockBuilder ( 'Magento\Framework\HTTP\ZendClient' )->setMethods ( [ 
				'request',
				'setUri',
				'setMethod',
				'setHeaders',
		...
		$this->configMock = $this->getMockBuilder ( 'Magento\NewRelicReporting\Model\Config' )->setMethods ( [ 
				'getNewRelicApiUrl',
				'getNewRelicApiKey',
				'getNewRelicAppName',
				'getNewRelicAppId' 
		...
		$this->model = new Deployments ( $this->configMock, $this->loggerMock, $this->zendClientFactoryMock );
	}
	...
	public function testSetDeploymentRequestOk() {
		...
		$this->zendClientMock->expects ( $this->once () )->method ( 'setUri' )->with ( $data ['self_uri'] )->willReturnSelf ();
		...
		$this->configMock->expects ( $this->once () )->method ( 'getNewRelicApiUrl' )->willReturn ( '' );
		...
		$this->assertInternalType ( 'string', $this->model->setDeployment ( $data ['description'], $data ['change'], $data ['user'] ) );
	}
