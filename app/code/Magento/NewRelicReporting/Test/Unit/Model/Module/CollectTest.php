...
class CollectTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->moduleListMock = $this->getMockBuilder ( 'Magento\Framework\Module\ModuleListInterface' )->setMethods ( [ 
				'getNames',
				'has',
				'getAll',
				'getOne' 
		...
		$this->fullModuleListMock = $this->getMockBuilder ( 'Magento\Framework\Module\FullModuleList' )->setMethods ( [ 
				'getNames',
				'getAll' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->moduleManagerMock = $this->getMockBuilder ( 'Magento\Framework\Module\Manager' )->setMethods ( [ 
				'isOutputEnabled' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->moduleFactoryMock = $this->getMock ( 'Magento\NewRelicReporting\Model\ModuleFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->moduleCollectionFactoryMock = $this->getMock ( 'Magento\NewRelicReporting\Model\ResourceModel\Module\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->model = new Collect ( $this->moduleListMock, $this->fullModuleListMock, $this->moduleManagerMock, $this->moduleFactoryMock, $this->moduleCollectionFactoryMock );
	}
	...
	public function testGetModuleDataWithoutRefresh() {
		...
		$moduleCollectionMock->expects ( $this->any () )->method ( 'getData' )->willReturn ( $testChangesMockArray );
		
		$this->fullModuleListMock->expects ( $this->once () )->method ( 'getAll' )->willReturn ( $modulesMockArray );
		
		$this->fullModuleListMock->expects ( $this->once () )->method ( 'getNames' )->willReturn ( $enabledModulesMockArray );
		
		$this->moduleListMock->expects ( $this->once () )->method ( 'getNames' )->willReturn ( $enabledModulesMockArray );
	...
	public function testGetModuleDataRefresh($data) {
		...
		$itemMock = $this->getMock ( 'Magento\NewRelicReporting\Model\Module', [ 
				'getName',
				'getData',
				'setData',
				'getState',
