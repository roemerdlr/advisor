...
class ReportOrderPlacedTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->ordersFactory = $this->getMockBuilder ( 'Magento\NewRelicReporting\Model\OrdersFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->ordersModel = $this->getMockBuilder ( 'Magento\NewRelicReporting\Model\Orders' )->disableOriginalConstructor ()->getMock ();
		$this->ordersFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->ordersModel );
		
		$this->model = new ReportOrderPlaced ( $this->config, $this->ordersFactory );
	}
	...
	public function testReportOrderPlaced() {
		...
		$this->ordersModel->expects ( $this->once () )->method ( 'setData' )->with ( [ 
