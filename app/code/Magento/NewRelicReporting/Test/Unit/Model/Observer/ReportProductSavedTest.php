...
class ReportProductSavedTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->systemFactory = $this->getMockBuilder ( 'Magento\NewRelicReporting\Model\SystemFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->model = new ReportProductSaved ( $this->config, $this->systemFactory, $this->jsonEncoder );
	}
	...
	public function testReportProductSaved() {
		...
		$this->systemModel->expects ( $this->once () )->method ( 'setData' )->with ( [ 
				'type' => $testType,
				'action' => $testAction 
		] )->willReturnSelf ();
