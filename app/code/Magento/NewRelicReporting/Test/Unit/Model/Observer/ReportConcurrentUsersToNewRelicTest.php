...
class ReportConcurrentUsersToNewRelicTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->newRelicWrapper = $this->getMockBuilder ( 'Magento\NewRelicReporting\Model\NewRelicWrapper' )->disableOriginalConstructor ()->setMethods ( [ 
				'addCustomParameter' 
		] )->getMock ();
		
		$this->model = new ReportConcurrentUsersToNewRelic ( $this->config, $this->customerSession, $this->customerRepository, $this->storeManager, $this->newRelicWrapper );
	}
	...
	public function testReportConcurrentUsersToNewRelicUserIsNotLoggedIn() {
		...
		$this->newRelicWrapper->expects ( $this->exactly ( 2 ) )->method ( 'addCustomParameter' )->willReturn ( true );
