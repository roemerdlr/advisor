...
class ReportNewRelicCron {
	 ...
	 */
	public function __construct(Config $config, Collect $collect, Counter $counter, CronEventFactory $cronEventFactory, DeploymentsFactory $deploymentsFactory, \Psr\Log\LoggerInterface $logger) {
		$this->config = $config;
	...
	protected function reportModules() {
		...
		$this->addCustomParameters ( [ 
				Config::MODULES_ENABLED => $moduleData [Config::ENABLED] 
		] );
		$this->addCustomParameters ( [ 
				Config::MODULES_DISABLED => $moduleData [Config::DISABLED] 
		] );
		$this->addCustomParameters ( [ 
				Config::MODULES_INSTALLED => $moduleData [Config::INSTALLED] 
		] );
	}
