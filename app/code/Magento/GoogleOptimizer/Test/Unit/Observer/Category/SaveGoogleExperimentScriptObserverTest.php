...
class SaveGoogleExperimentScriptObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_storeId = 0;
		$this->_categoryMock->expects ( $this->atLeastOnce () )->method ( 'getStoreId' )->will ( $this->returnValue ( $this->_storeId ) );
		...
		$this->_modelObserver = $objectManagerHelper->getObject ( 'Magento\GoogleOptimizer\Observer\Category\SaveGoogleExperimentScriptObserver', [ 
	...
	public function testCreatingCodeIfRequestIsValid() {
		...
		$this->_helperMock->expects ( $this->once () )->method ( 'isGoogleExperimentActive' )->with ( $this->_storeId )->will ( $this->returnValue ( true ) );
		
		$this->_requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( 'google_experiment' )->will ( $this->returnValue ( [ 
				'code_id' => '',
				'experiment_script' => $experimentScript 
		] ) );
		
		$this->_codeMock->expects ( $this->once () )->method ( 'addData' )->with ( [ 
	...
	public function testCreatingCodeIfRequestIsNotValid($params) {
		...
		$this->_requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( 'google_experiment' )->will ( $this->returnValue ( $params ) );
	...
	public function testManagingCodeIfGoogleExperimentIsDisabled() {
		$this->_helperMock->expects ( $this->once () )->method ( 'isGoogleExperimentActive' )->with ( $this->_storeId )->will ( $this->returnValue ( false ) );
