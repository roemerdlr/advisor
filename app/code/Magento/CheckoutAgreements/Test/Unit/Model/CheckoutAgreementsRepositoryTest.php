...
class CheckoutAgreementsRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->factoryMock = $this->getMock ( 'Magento\CheckoutAgreements\Model\ResourceModel\Agreement\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->agrFactoryMock = $this->getMock ( 'Magento\CheckoutAgreements\Model\AgreementFactory', [ 
				'create' 
		], [ ], '', false );
		$methods = [ 
				'addData',
		...
		$this->agreementMock = $this->getMock ( '\Magento\CheckoutAgreements\Model\Agreement', $methods, [ ], '', false );
		$this->storeMock = $this->getMock ( 'Magento\Store\Model\Store', [ ], [ ], '', false );
		$this->extensionAttributesJoinProcessorMock = $this->getMock ( '\Magento\Framework\Api\ExtensionAttribute\JoinProcessor', [ 
				'process' 
		], [ ], '', false );
		
		$this->model = new \Magento\CheckoutAgreements\Model\CheckoutAgreementsRepository ( $this->factoryMock, $this->storeManagerMock, $this->scopeConfigMock, $this->resourceMock, $this->agrFactoryMock, $this->extensionAttributesJoinProcessorMock );
	}
	public function testGetListReturnsEmptyListIfCheckoutAgreementsAreDisabledOnFrontend() {
		$this->extensionAttributesJoinProcessorMock->expects ( $this->never () )->method ( 'process' );
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'isSetFlag' )->with ( 'checkout/options/enable_agreements', ScopeInterface::SCOPE_STORE, null )->will ( $this->returnValue ( false ) );
	...
	public function testGetListReturnsTheListOfActiveCheckoutAgreements() {
		$this->extensionAttributesJoinProcessorMock->expects ( $this->once () )->method ( 'process' )->with ( $this->isInstanceOf ( 'Magento\CheckoutAgreements\Model\ResourceModel\Agreement\Collection' ) );
		...
		$collectionMock = $this->objectManager->getCollectionMock ( 'Magento\CheckoutAgreements\Model\ResourceModel\Agreement\Collection', [ 
				$this->agreementMock 
		] );
	...
	public function testUpdate() {
		...
		$this->resourceMock->expects ( $this->once () )->method ( 'load' )->with ( $this->agreementMock, $agreementId );
		$this->storeManagerMock->expects ( $this->never () )->method ( 'getStore' );
		$this->agreementMock->expects ( $this->once () )->method ( 'setStores' );
		$this->agreementMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( $agreementId );
		$this->agreementMock->expects ( $this->any () )->method ( 'getData' )->willReturn ( [ 
				'data' 
		] );
		$this->agreementMock->expects ( $this->once () )->method ( 'addData' )->with ( [ 
				'data' 
