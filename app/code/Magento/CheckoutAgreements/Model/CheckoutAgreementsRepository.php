...
class CheckoutAgreementsRepository implements CheckoutAgreementsRepositoryInterface {
	 ...
	 *        	@codeCoverageIgnore
	 */
	public function __construct(AgreementCollectionFactory $collectionFactory, StoreManagerInterface $storeManager, ScopeConfigInterface $scopeConfig, AgreementResource $agreementResource, AgreementFactory $agreementFactory, JoinProcessorInterface $extensionAttributesJoinProcessor) {
	...
	public function save(\Magento\CheckoutAgreements\Api\Data\AgreementInterface $data, $storeId = null) {
		...
		try {
			...
			throw new \Magento\Framework\Exception\CouldNotSaveException ( __ ( 'Unable to save checkout agreement %1', $data->getAgreementId () ) );
		}
		return $data;
	}
	...
	public function delete(\Magento\CheckoutAgreements\Api\Data\AgreementInterface $data) {
		try {
			...
			throw new \Magento\Framework\Exception\CouldNotDeleteException ( __ ( 'Unable to remove checkout agreement %1', $data->getAgreementId () ) );
		}
		return true;
	}
