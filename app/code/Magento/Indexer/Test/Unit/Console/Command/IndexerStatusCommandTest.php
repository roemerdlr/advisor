...
class IndexerStatusCommandTest extends AbstractIndexerCommandCommonSetup {
	...
	public function testExecuteAll() {
		...
		$indexerOne->expects ( $this->once () )->method ( 'getStatus' )->willReturn ( \Magento\Framework\Indexer\StateInterface::STATUS_VALID );
		...
		$indexerTwo->expects ( $this->once () )->method ( 'getStatus' )->willReturn ( \Magento\Framework\Indexer\StateInterface::STATUS_INVALID );
		...
		$indexerThree->expects ( $this->once () )->method ( 'getStatus' )->willReturn ( \Magento\Framework\Indexer\StateInterface::STATUS_WORKING );
		...
		$expectedValue = sprintf ( '%-50s ', 'Title_indexerOne' . ':' ) . 'Ready' . PHP_EOL . sprintf ( '%-50s ', 'Title_indexerTwo' . ':' ) . 'Reindex required' . PHP_EOL . sprintf ( '%-50s ', 'Title_indexerThree' . ':' ) . 'Processing' . PHP_EOL . sprintf ( '%-50s ', 'Title_indexerFour' . ':' ) . 'unknown' . PHP_EOL;
