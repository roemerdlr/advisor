...
class InvalidTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$collectionMock = $this->getMock ( '\Magento\Indexer\Model\Indexer\Collection', [ 
				'getItems' 
		], [ ], '', false );
		
		$this->indexerMock = $this->getMock ( '\Magento\Indexer\Model\Indexer', [ 
				'getStatus' 
		], [ ], '', false );
		
		$urlBuilder = $this->getMock ( '\Magento\Framework\UrlInterface', [ ], [ ], '', false );
		
		$collectionMock->expects ( $this->any () )->method ( 'getItems' )->with ()->willReturn ( [ 
				$this->indexerMock 
		] );
		
		$this->model = new \Magento\Indexer\Model\Message\Invalid ( $collectionMock, $urlBuilder );
	}
	public function testDisplayMessage() {
		$this->indexerMock->expects ( $this->any () )->method ( 'getStatus' )->with ()->willReturn ( \Magento\Framework\Indexer\StateInterface::STATUS_INVALID );
