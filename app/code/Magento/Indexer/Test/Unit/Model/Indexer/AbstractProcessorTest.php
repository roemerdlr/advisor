...
class AbstractProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_indexerRegistryMock = $this->getMock ( '\Magento\Framework\Indexer\IndexerRegistry', [ 
				'isScheduled',
				'get',
				'reindexRow',
				'reindexList',
		...
		$this->model = new \Magento\Indexer\Test\Unit\Model\Indexer\AbstractProcessorStub ( $this->_indexerRegistryMock );
	}
	...
	public function testReindexRow($scheduled) {
		...
		if ($scheduled) {
			...
			$this->_indexerRegistryMock->expects ( $this->exactly ( 2 ) )->method ( 'get' )->with ( self::INDEXER_ID )->willReturnSelf ();
			$this->_indexerRegistryMock->expects ( $this->once () )->method ( 'isScheduled' )->willReturn ( $scheduled );
