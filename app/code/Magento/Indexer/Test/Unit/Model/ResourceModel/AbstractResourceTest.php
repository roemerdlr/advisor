...
class AbstractResourceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_resourceMock = $this->getMockBuilder ( 'Magento\Framework\App\ResourceConnection' )->disableOriginalConstructor ()->getMock ();
		
		$this->_tableStrategyInterface = $this->getMock ( 'Magento\Framework\Indexer\Table\StrategyInterface', [ ], [ ], '', false );
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$arguments = $objectManager->getConstructArguments ( '\Magento\Indexer\Test\Unit\Model\ResourceModel\AbstractResourceStub', [ 
				'resource' => $this->_resourceMock,
				'tableStrategy' => $this->_tableStrategyInterface 
		] );
		$this->model = $objectManager->getObject ( '\Magento\Indexer\Test\Unit\Model\ResourceModel\AbstractResourceStub', $arguments );
	}
	...
	public function testSyncData() {
		...
		$selectMock->expects ( $this->once () )->method ( 'insertFromSelect' )->with ( $resultTable, $resultColumns )->will ( $this->returnSelf () );
		
		$this->_resourceMock->expects ( $this->any () )->method ( 'getConnection' )->will ( $this->returnValue ( $connectionMock ) );
		$this->_resourceMock->expects ( $this->any () )->method ( 'getTableName' )->will ( $this->returnArgument ( 0 ) );
		
		$this->assertInstanceOf ( 'Magento\Indexer\Test\Unit\Model\ResourceModel\AbstractResourceStub', $this->model->syncData () );
	}
	...
	public function testInsertFromTable($readToIndex) {
		...
		if ($readToIndex) {
			$connectionCustomMock = $this->getMock ( 'Magento\Framework\DB\Adapter\CustomAdapterInterface', [ 
					'describeTable',
					'query',
					'select',
					'insertArray' 
			...
			$connectionCustomMock->expects ( $this->any () )->method ( 'insertArray' )->with ( $destTable, $resultColumns )->will ( $this->returnValue ( 1 ) );
			$connectionMock->expects ( $this->any () )->method ( 'query' )->will ( $this->returnValue ( $pdoMock ) );
			$pdoMock->expects ( $this->any () )->method ( 'fetch' )->will ( $this->returnValue ( [ 
					$tableColumns 
			] ) );
		...
		$this->assertInstanceOf ( 'Magento\Indexer\Test\Unit\Model\ResourceModel\AbstractResourceStub', $this->model->insertFromTable ( $sourceTable, $destTable, $readToIndex ) );
	}
