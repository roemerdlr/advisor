...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->reader = $this->getMock ( 'Magento\Framework\Indexer\Config\Reader', [ 
				'read' 
		], [ ], '', false );
		...
		$this->stateCollection = $this->getMock ( 'Magento\Indexer\Model\ResourceModel\Indexer\State\Collection', [ 
				'getItems' 
		], [ ], '', false );
	}
	public function testConstructorWithCache() {
		$this->cache->expects ( $this->once () )->method ( 'test' )->with ( $this->cacheId )->will ( $this->returnValue ( true ) );
		$this->cache->expects ( $this->once () )->method ( 'load' )->with ( $this->cacheId )->will ( $this->returnValue ( serialize ( $this->indexers ) ) );
	...
	public function testConstructorWithoutCache() {
		...
		$stateExistent = $this->getMock ( 'Magento\Indexer\Model\Indexer\State', [ 
				'getIndexerId',
				'__wakeup',
				'delete' 
		], [ ], '', false );
		...
		$stateNonexistent = $this->getMock ( 'Magento\Indexer\Model\Indexer\State', [ 
				'getIndexerId',
				'__wakeup',
				'delete' 
		], [ ], '', false );
