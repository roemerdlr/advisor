...
class IndexerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->configMock = $this->getMockForAbstractClass ( 'Magento\Framework\Indexer\ConfigInterface', [ ], '', false, false, true, [ 
				'getIndexer' 
		] );
		$this->actionFactoryMock = $this->getMock ( 'Magento\Framework\Indexer\ActionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->viewMock = $this->getMockForAbstractClass ( 'Magento\Framework\Mview\ViewInterface', [ ], '', false, false, true, [ 
				'load',
				'isEnabled',
				'getUpdated',
				'getStatus',
		...
		$this->stateFactoryMock = $this->getMock ( 'Magento\Indexer\Model\Indexer\StateFactory', [ 
				'create' 
		], [ ], '', false );
		$this->indexFactoryMock = $this->getMock ( 'Magento\Indexer\Model\Indexer\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$structureFactory = $this->getMockBuilder ( 'Magento\Framework\Indexer\StructureFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		/** @var \Magento\Framework\Indexer\StructureFactory $structureFactory */
		$this->model = new \Magento\Indexer\Model\Indexer ( $this->configMock, $this->actionFactoryMock, $structureFactory, $this->viewMock, $this->stateFactoryMock, $this->indexFactoryMock );
	}
	...
	public function testGetState() {
		$indexId = 'indexer_internal_name';
		$stateMock = $this->getMock ( '\Magento\Indexer\Model\Indexer\State', [ 
				'loadByIndexer',
				'getId',
				'__wakeup' 
		], [ ], '', false );
	...
	public function testReindexAll() {
		...
		$actionMock = $this->getMock ( 'Magento\Framework\Indexer\ActionInterface', [ 
				'executeFull',
				'executeList',
				'executeRow' 
		], [ ], '', false );
		$this->actionFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( 'Some\Class\Name' )->will ( $this->returnValue ( $actionMock ) );
	...
	public function testInvalidate() {
		...
		$this->stateFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $stateMock ) );
		$stateMock->expects ( $this->once () )->method ( 'setStatus' )->with ( StateInterface::STATUS_INVALID )->will ( $this->returnSelf () );
		$stateMock->expects ( $this->once () )->method ( 'save' )->will ( $this->returnSelf () );
