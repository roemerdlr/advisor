...
abstract class AbstractIndexerManageCommand extends AbstractIndexerCommand {
	...
	protected function getIndexers(InputInterface $input) {
		...
		if (empty ( $requestedTypes )) {
			...
			if ($unsupportedTypes) {
				...
				throw new \InvalidArgumentException ( "The following requested index types are not supported: '" . join ( "', '", $unsupportedTypes ) . "'." . PHP_EOL . 'Supported types: ' . join ( ", ", $availableTypes ) );
			}
		}
		return $indexers;
	}
	...
	public function getInputList() {
		return [ 
				new InputArgument ( self::INPUT_KEY_INDEXERS, InputArgument::OPTIONAL | InputArgument::IS_ARRAY, 'Space-separated list of index types or omit to apply to all indexes.' ) 
		];
	}
}
