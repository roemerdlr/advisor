...
class ReorderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->scopeConfigMock = $this->getMockBuilder ( 'Magento\Framework\App\Config' )->setMethods ( [ 
				'getValue' 
		...
		$this->repositoryMock = $this->getMockBuilder ( 'Magento\Sales\Api\OrderRepositoryInterface' )->getMockForAbstractClass ();
		$this->helper = new \Magento\Sales\Helper\Reorder ( $contextMock, $this->customerSessionMock, $this->repositoryMock );
		
		$this->storeParam = $this->getMockBuilder ( 'Magento\Sales\Model\Store' )->disableOriginalConstructor ()->getMock ();
	...
	protected function setupScopeConfigMock($returnValue) {
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( Reorder::XML_PATH_SALES_REORDER_ALLOW, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->storeParam )->will ( $this->returnValue ( $returnValue ) );
	}
	 ...
	 * @return void @dataProvider getOrderCanReorder
	 */
	...
	protected function setupOrderMock($storeScopeReturnValue) {
		...
		$this->orderMock->expects ( $this->once () )->method ( 'getStore' )->will ( $this->returnValue ( $this->storeParam ) );
	}
