...
class AddVatRequestParamsOrderCommentTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->customerAddressHelperMock = $this->getMockBuilder ( 'Magento\Customer\Helper\Address' )->disableOriginalConstructor ()->getMock ();
		
		$this->observer = new AddVatRequestParamsOrderComment ( $this->customerAddressHelperMock );
	}
	...
	public function testAddVatRequestParamsOrderComment($configAddressType, $vatRequestId, $vatRequestDate, $orderHistoryComment) {
		...
		$orderAddressMock = $this->getMock ( 'Magento\Sales\Model\Order\Address', [ 
				'getVatRequestId',
				'getVatRequestDate',
				'__wakeup' 
		], [ ], '', false );
