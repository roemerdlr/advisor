...
class DownloadCustomOptionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$resultForwardFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Controller\Result\ForwardFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->resultForwardMock = $this->getMockBuilder ( '\Magento\Framework\Controller\Result\Forward' )->disableOriginalConstructor ()->setMethods ( [ 
				'forward' 
		] )->getMock ();
		$resultForwardFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->resultForwardMock );
		
		$this->downloadMock = $this->getMockBuilder ( 'Magento\Sales\Model\Download' )->disableOriginalConstructor ()->setMethods ( [ 
				'downloadFile' 
		] )->getMock ();
		
		$this->unserializeMock = $this->getMockBuilder ( 'Magento\Framework\Unserialize\Unserialize' )->disableOriginalConstructor ()->setMethods ( [ 
				'unserialize' 
		] )->getMock ();
		
		$requestMock = $this->getMockBuilder ( 'Magento\Framework\App\Request\Http' )->disableOriginalConstructor ()->setMethods ( [ 
				'getParam' 
		] )->getMock ();
		...
		$this->itemOptionMock = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item\Option' )->disableOriginalConstructor ()->setMethods ( [ 
				'load',
				'getId',
				'getCode',
				'getProductId',
		...
		$this->productOptionMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Option' )->disableOriginalConstructor ()->setMethods ( [ 
				'load',
				'getId',
				'getProductId',
				'getType' 
		...
		$objectManagerMock = $this->getMockBuilder ( 'Magento\Sales\Model\Download' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$objectManagerMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValueMap ( [ 
				[ 
		...
		$contextMock = $this->getMockBuilder ( 'Magento\Backend\App\Action\Context' )->disableOriginalConstructor ()->setMethods ( [ 
				'getRequest',
				'getObjectManager' 
		] )->getMock ();
		...
		$this->objectMock = $this->getMockBuilder ( 'Magento\Sales\Controller\Download\DownloadCustomOption' )->setMethods ( [ 
				'endExecute' 
		] )->setConstructorArgs ( [ 
				'context' => $contextMock,
	...
	public function testExecuteBadSecretKey() {
		...
		$this->unserializeMock->expects ( $this->once () )->method ( 'unserialize' )->with ( self::OPTION_VALUE )->willReturn ( [ 
				self::SECRET_KEY => 'bad_test_secret_key' 
		] );
