...
class MassCancelTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->orderCollectionFactoryMock = $this->getMockBuilder ( $resourceCollection )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->massAction = $objectManagerHelper->getObject ( 'Magento\Sales\Controller\Adminhtml\Order\MassCancel', [ 
	...
	public function testExecuteCanCancelOneOrder() {
		...
		$order1->expects ( $this->once () )->method ( 'cancel' );
		...
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( '1 order(s) cannot be canceled.' );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addSuccess' )->with ( 'We canceled 1 order(s).' );
	...
	public function testExcludedCannotCancelOrders() {
		...
		$this->orderCollectionMock->expects ( $this->any () )->method ( 'getItems' )->willReturn ( [ 
				$order1,
				$order2 
		] );
