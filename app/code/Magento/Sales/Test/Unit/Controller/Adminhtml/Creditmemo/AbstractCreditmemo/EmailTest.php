...
class EmailTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->context = $this->getMock ( 'Magento\Backend\App\Action\Context', [ 
				'getRequest',
				'getResponse',
				'getMessageManager',
				'getRedirect',
		...
		$this->messageManager = $this->getMock ( 'Magento\Framework\Message\Manager', [ 
				'addSuccess' 
		], [ ], '', false );
		...
		$this->helper = $this->getMock ( '\Magento\Backend\Helper\Data', [ 
				'getUrl' 
		], [ ], '', false );
		...
		$this->creditmemoEmail = $objectManagerHelper->getObject ( 'Magento\Sales\Controller\Adminhtml\Creditmemo\AbstractCreditmemo\Email', [ 
	...
	public function testEmail() {
		...
		$this->objectManager->expects ( $this->once () )->method ( 'create' )->with ( $cmManagement )->willReturn ( $cmManagementMock );
		...
		$this->assertInstanceOf ( 'Magento\Backend\Model\View\Result\Redirect', $this->creditmemoEmail->execute () );
		$this->assertEquals ( $this->response, $this->creditmemoEmail->getResponse () );
	}
	public function testEmailNoCreditmemoId() {
		$this->request->expects ( $this->once () )->method ( 'getParam' )->with ( 'creditmemo_id' )->will ( $this->returnValue ( null ) );
	...
	protected function prepareRedirect($cmId) {
		...
		$this->resultRedirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( 'sales/order_creditmemo/view', [ 
				'creditmemo_id' => $cmId 
		] )->willReturnSelf ();
	}
}
