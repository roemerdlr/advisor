...
class SaveTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$contextMock->expects ( $this->any () )->method ( 'getResultRedirectFactory' )->will ( $this->returnValue ( $this->resultPageFactoryMock ) );
		$contextMock->expects ( $this->any () )->method ( 'getFormKeyValidator' )->will ( $this->returnValue ( $this->formKeyValidatorMock ) );
		$contextMock->expects ( $this->any () )->method ( 'getMessageManager' )->will ( $this->returnValue ( $this->messageManagerMock ) );
		
		$this->controller = $objectManager->getObject ( 'Magento\Sales\Controller\Adminhtml\Order\Invoice\Save', [ 
				'context' => $contextMock 
		] );
	}
	...
	public function testExecuteNotValidPost() {
		...
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( 'We can\'t save the invoice right now.' );
		$redirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( 'sales/order/index' )->willReturnSelf ();
