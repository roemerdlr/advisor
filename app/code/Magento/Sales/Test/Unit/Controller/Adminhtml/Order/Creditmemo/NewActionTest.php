...
class NewActionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->creditmemoLoaderMock = $this->getMock ( 'Magento\Sales\Controller\Adminhtml\Order\CreditmemoLoader', [ 
				'setOrderId',
				'setCreditmemoId',
				'setCreditmemo',
				'setInvoiceId',
		...
		$this->requestMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\RequestInterface', [ ], '', false, false, true, [ ] );
		$this->responseMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\ResponseInterface', [ ], '', false, false, true, [ ] );
		...
		$this->backendSessionMock = $this->getMock ( 'Magento\Backend\Model\Session', [ 
				'getCommentText' 
		], [ ], '', false );
		$this->layoutMock = $this->getMockForAbstractClass ( 'Magento\Framework\View\LayoutInterface', [ ], '', false, false, true, [ ] );
		...
		$this->contextMock->expects ( $this->once () )->method ( 'getResponse' )->will ( $this->returnValue ( $this->responseMock ) );
		$this->contextMock->expects ( $this->once () )->method ( 'getObjectManager' )->will ( $this->returnValue ( $this->objectManagerMock ) );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->controller = $objectManager->getObject ( 'Magento\Sales\Controller\Adminhtml\Order\Creditmemo\NewAction', [ 
	...
	public function testExecute() {
		$this->requestMock->expects ( $this->exactly ( 4 ) )->method ( 'getParam' )->will ( $this->returnValueMap ( [ 
				[ 
						'order_id',
						null,
						'order_id' 
		...
		$this->titleMock->expects ( $this->exactly ( 2 ) )->method ( 'prepend' )->will ( $this->returnValueMap ( [ 
				[ 
						'Credit Memos',
						null 
				],
				[ 
						'New Memo for #invoice-increment-id',
						null 
				],
				[ 
						'item-title',
