...
class MassUnholdTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->orderCollectionFactoryMock = $this->getMockBuilder ( $orderCollection )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->sessionMock = $this->getMock ( 'Magento\Backend\Model\Session', [ 
				'setIsUrlNotice' 
		], [ ], '', false );
		$this->actionFlagMock = $this->getMock ( 'Magento\Framework\App\ActionFlag', [ 
				'get',
				'set' 
		], [ ], '', false );
		...
		$this->massAction = $objectManagerHelper->getObject ( 'Magento\Sales\Controller\Adminhtml\Order\MassUnhold', [ 
	...
	public function testExecuteOneOrdersReleasedFromHold() {
		...
		$order1->expects ( $this->once () )->method ( 'canUnhold' )->willReturn ( true );
		$order1->expects ( $this->once () )->method ( 'unhold' );
		$order1->expects ( $this->once () )->method ( 'save' );
		...
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( '1 order(s) were not released from on hold status.' );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addSuccess' )->with ( '1 order(s) have been released from on hold status.' );
