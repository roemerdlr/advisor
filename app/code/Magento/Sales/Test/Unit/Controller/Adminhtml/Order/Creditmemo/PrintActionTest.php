...
class PrintActionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->creditmemoLoaderMock = $this->getMockBuilder ( 'Magento\Sales\Controller\Adminhtml\Order\CreditmemoLoader' )->disableOriginalConstructor ()->setMethods ( [ 
				'setOrderId',
				'setCreditmemoId',
				'setCreditmemo',
				'setInvoiceId',
		...
		$this->creditmemoRepositoryMock = $this->getMock ( 'Magento\Sales\Api\CreditmemoRepositoryInterface', [ ], [ ], '', false );
		$this->creditmemoMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Creditmemo' )->disableOriginalConstructor ()->getMock ();
		$this->creditmemoPdfMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Pdf\Creditmemo' )->disableOriginalConstructor ()->getMock ();
		$this->pdfMock = $this->getMockBuilder ( 'Zend_Pdf' )->disableOriginalConstructor ()->getMock ();
		$this->dateTimeMock = $this->getMockBuilder ( 'Magento\Framework\Stdlib\DateTime\DateTime' )->disableOriginalConstructor ()->getMock ();
		...
		$this->responseMock = $this->getMockBuilder ( 'Magento\Framework\App\ResponseInterface' )->getMock ();
		...
		$this->printAction = $objectManager->getObject ( 'Magento\Sales\Controller\Adminhtml\Order\Creditmemo\PrintAction', [ 
	...
	public function testExecute() {
		...
		$this->objectManagerMock->expects ( $this->any () )->method ( 'create' )->willReturnMap ( [ 
				...
				[ 
						...
						[ ],
						$this->creditmemoPdfMock 
				] 
		] );
		$this->creditmemoRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $creditmemoId )->willReturn ( $this->creditmemoMock );
		$this->creditmemoPdfMock->expects ( $this->once () )->method ( 'getPdf' )->with ( [ 
				$this->creditmemoMock 
	...
	protected function prepareTestExecute($creditmemoId = null) {
		...
		$this->creditmemoLoaderMock->expects ( $this->once () )->method ( 'setOrderId' )->with ( $orderId )->willReturnSelf ();
		$this->creditmemoLoaderMock->expects ( $this->once () )->method ( 'setCreditmemoId' )->with ( $creditmemoId )->willReturnSelf ();
		$this->creditmemoLoaderMock->expects ( $this->once () )->method ( 'setCreditmemo' )->with ( $creditmemo )->willReturnSelf ();
		$this->creditmemoLoaderMock->expects ( $this->once () )->method ( 'setInvoiceId' )->with ( $invoiceId )->willReturnSelf ();
		$this->creditmemoLoaderMock->expects ( $this->once () )->method ( 'load' );
	}
}
