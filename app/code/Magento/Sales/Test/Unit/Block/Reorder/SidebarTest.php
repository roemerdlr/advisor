...
class SidebarTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->orderCollectionFactory = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->customerSession = $this->getMock ( 'Magento\Customer\Model\Session', [ 
				'getCustomerId' 
		], [ ], '', false );
		$this->orderConfig = $this->getMock ( 'Magento\Sales\Model\Order\Config', [ 
				'getVisibleOnFrontStatuses' 
		], [ ], '', false );
		$this->orderCollection = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Collection', [ 
	...
	protected function createBlockObject() {
		$this->block = $this->objectManagerHelper->getObject ( 'Magento\Sales\Block\Reorder\Sidebar', [ 
	...
	public function testGetIdentities() {
		...
		$store = $this->getMock ( 'Magento\Store\Model', [ 
				'getWebsiteId' 
		], [ ], '', false );
		...
		$product->expects ( $this->atLeastOnce () )->method ( 'getWebsiteIds' )->will ( $this->returnValue ( [ 
				$websiteId 
		] ) );
		
		$item = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Item', [ 
				'__wakeup',
				'getProduct' 
		], [ ], '', false );
		...
		$order->expects ( $this->atLeastOnce () )->method ( 'getParentItemsRandomCollection' )->with ( $this->equalTo ( $limit ) )->will ( $this->returnValue ( [ 
				$item 
		] ) );
		
		$this->createBlockObject ();
		$this->assertSame ( $this->block, $this->block->setOrders ( [ 
				$order 
		] ) );
	...
	public function testInitOrders() {
		...
		$this->orderCollection->expects ( $this->at ( 1 ) )->method ( 'addAttributeToFilter' )->with ( $attribute [1], $this->equalTo ( [ 
				'in' => $statuses 
	...
	public function testIsItemAvailableForReorder() {
		...
		$this->stockItemMock->expects ( $this->once () )->method ( 'getIsInStock' )->will ( $this->returnValue ( $result ) );
		$this->stockRegistry->expects ( $this->any () )->method ( 'getStockItem' )->will ( $this->returnValue ( $this->stockItemMock ) );
