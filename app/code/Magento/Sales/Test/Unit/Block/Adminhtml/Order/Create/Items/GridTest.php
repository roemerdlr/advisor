...
class GridTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$sessionMock = $this->getMockBuilder ( 'Magento\Backend\Model\Session\Quote' )->disableOriginalConstructor ()->setMethods ( [ 
				'getQuote',
				'__wakeup' 
		] )->getMock ();
		...
		$wishlistFactoryMock = $this->getMockBuilder ( 'Magento\Wishlist\Model\WishlistFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'methods',
				'__wakeup' 
		] )->getMock ();
		
		$giftMessageSave = $this->getMockBuilder ( 'Magento\Giftmessage\Model\Save' )->setMethods ( [ 
				'__wakeup' 
		...
		$this->stockState = $this->getMock ( 'Magento\CatalogInventory\Model\StockState', [ 
				'checkQuoteItemQty',
				'__wakeup' 
		], [ ], '', false );
		...
		$this->block = $this->objectManager->getObject ( 'Magento\Sales\Block\Adminhtml\Order\Create\Items\Grid', [ 
		...
		$this->priceRenderBlock = $this->getMockBuilder ( '\Magento\Backend\Block\Template' )->disableOriginalConstructor ()->setMethods ( [ 
				'setItem',
				'toHtml' 
		] )->getMock ();
		
		$this->layoutMock = $this->getMockBuilder ( '\Magento\Framework\View\Layout' )->disableOriginalConstructor ()->setMethods ( [ 
				'getBlock' 
		] )->getMock ();
		
		$this->itemMock = $this->getMockBuilder ( '\Magento\Quote\Model\Quote\Item' )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup' 
		] )->getMock ();
	}
	...
	public function testGetItems() {
		...
		$blockMock = $this->getMock ( 'Magento\Framework\View\Element\AbstractBlock', [ 
				'getItems' 
		], [ ], '', false );
		...
		$checkMock = $this->getMock ( 'Magento\Framework\DataObject', [ 
				'getMessage',
				'getHasError' 
		], [ ], '', false );
		...
		$blockMock->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$itemMock 
		] ) );
		...
		$this->stockState->expects ( $this->once () )->method ( 'checkQuoteItemQty' )->with ( $productId, $itemQty, $itemQty, $itemQty, null )->will ( $this->returnValue ( $checkMock ) );
	...
	protected function getGrid() {
		...
		$grid = $this->objectManager->getObject ( 'Magento\Sales\Block\Adminhtml\Order\Create\Items\Grid', [ 
				'context' => $this->objectManager->getObject ( 'Magento\Backend\Block\Template\Context', [ 
	...
	public function testGetItemUnitPriceHtml() {
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'item_unit_price' )->will ( $this->returnValue ( $this->priceRenderBlock ) );
	...
	public function testGetItemRowTotalHtml() {
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'item_row_total' )->will ( $this->returnValue ( $this->priceRenderBlock ) );
	...
	public function testGetItemRowTotalWithDiscountHtml() {
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'item_row_total_with_discount' )->will ( $this->returnValue ( $this->priceRenderBlock ) );
	...
	public function testGetSubtotalWithDiscount($orderData, $displayTotalsIncludeTax, $expected) {
		$quoteAddressMock = $this->getMock ( '\Magento\Quote\Model\Quote\Address', [ 
				'getSubtotal',
				'getTaxAmount',
				'getDiscountTaxCompensationAmount',
				'getDiscountAmount' 
		], [ ], '', false );
		$gridMock = $this->getMock ( 'Magento\Sales\Block\Adminhtml\Order\Create\Items\Grid', [ 
				'getQuoteAddress',
				'displayTotalsIncludeTax' 
		], [ ], '', false );
		
		$gridMock->expects ( $this->any () )->method ( 'getQuoteAddress' )->will ( $this->returnValue ( $quoteAddressMock ) );
		$gridMock->expects ( $this->any () )->method ( 'displayTotalsIncludeTax' )->will ( $this->returnValue ( $displayTotalsIncludeTax ) );
		
		$quoteAddressMock->expects ( $this->once () )->method ( 'getSubtotal' )->will ( $this->returnValue ( $orderData ['subTotal'] ) );
		
		$quoteAddressMock->expects ( $this->any () )->method ( 'getTaxAmount' )->will ( $this->returnValue ( $orderData ['taxAmount'] ) );
		
		$quoteAddressMock->expects ( $this->any () )->method ( 'getDiscountTaxCompensationAmount' )->will ( $this->returnValue ( $orderData ['discountTaxCompensationAmount'] ) );
		
		$quoteAddressMock->expects ( $this->once () )->method ( 'getDiscountAmount' )->will ( $this->returnValue ( $orderData ['discountAmount'] ) );
