...
class ItemsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->stockConfiguration = $this->getMock ( 'Magento\CatalogInventory\Model\Configuration', [ 
				'__wakeup',
				'canSubtractQty' 
		], [ ], '', false );
		...
		$this->items = $this->objectManagerHelper->getObject ( 'Magento\Sales\Block\Adminhtml\Order\Creditmemo\Create\Items', [ 
	...
	public function testCanReturnItemsToStock($canReturnToStock, $manageStock, $result) {
		...
		if ($canReturnToStock) {
			...
			$creditMemoItem = $this->getMock ( 'Magento\Sales\Model\Order\Creditmemo\Item', [ 
					'setCanReturnToStock',
					'getOrderItem',
					'__wakeup' 
			], [ ], '', false );
			...
			$creditMemo->expects ( $this->once () )->method ( 'getAllItems' )->will ( $this->returnValue ( [ 
					$creditMemoItem 
			] ) );
