...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->formFactory = $this->getMock ( 'Magento\Framework\Data\FormFactory', [ 
				'create' 
		], [ ], '', false );
		$this->collectionFactory = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Status\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->orderConfig = $this->getMock ( 'Magento\Sales\Model\Order\Config', [ ], [ ], '', false );
		
		$this->block = $objectManager->getObject ( 'Magento\Sales\Block\Adminhtml\Order\Status\Assign\Form', [ 
	...
	public function testToHtml() {
		...
		array_unshift ( $statusesForField, [ 
				'value' => '',
				'label' => '' 
		] );
		$statesForField = array_merge ( [ 
				'' => '' 
		], $states );
		...
		$collection->expects ( $this->once () )->method ( 'toOptionArray' )->will ( $this->returnValue ( $statuses ) );
		$this->collectionFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $collection ) );
		
		$this->orderConfig->expects ( $this->once () )->method ( 'getStates' )->will ( $this->returnValue ( $states ) );
		
		$fieldset->expects ( $this->at ( 0 ) )->method ( 'addField' )->with ( 'status', 'select', [ 
				'name' => 'status',
		...
		$fieldset->expects ( $this->at ( 1 ) )->method ( 'addField' )->with ( 'state', 'select', [ 
				'name' => 'state',
		...
		$fieldset->expects ( $this->at ( 2 ) )->method ( 'addField' )->with ( 'is_default', 'checkbox', [ 
				'name' => 'is_default',
				'label' => __ ( 'Use Order Status As Default' ),
				'value' => 1 
		] );
		$fieldset->expects ( $this->at ( 3 ) )->method ( 'addField' )->with ( 'visible_on_front', 'checkbox', [ 
