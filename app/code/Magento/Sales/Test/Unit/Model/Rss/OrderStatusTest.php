...
class OrderStatusTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->orderStatusFactory = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Order\Rss\OrderStatusFactory' )->setMethods ( [ 
				'create' 
		...
		$this->order = $this->getMockBuilder ( 'Magento\Sales\Model\Order' )->setMethods ( [ 
				'__sleep',
				'__wakeup',
				'getIncrementId',
				'getId',
		...
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Sales\Model\Rss\OrderStatus', [ 
	...
	public function testGetRssData() {
		...
		$resource = $this->getMockBuilder ( '\Magento\Sales\Model\ResourceModel\Order\Rss\OrderStatus' )->setMethods ( [ 
		...
		$resource->expects ( $this->once () )->method ( 'getAllCommentCollection' )->willReturn ( [ 
				$comment 
		] );
		$this->orderStatusFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $resource );
		$this->urlInterface->expects ( $this->any () )->method ( 'getUrl' )->with ( 'sales/order/view', [ 
				'order_id' => 1 
	...
	public function testIsAllowed() {
		$this->scopeConfigInterface->expects ( $this->once () )->method ( 'getValue' )->with ( 'rss/order/status', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->will ( $this->returnValue ( true ) );
	...
	public function getCacheKeyDataProvider() {
		return [ 
				[ 
						base64_encode ( '{"order_id":1,"increment_id":"100000001","customer_id":1}' ),
						md5 ( '11000000011' ) 
				],
				[ 
