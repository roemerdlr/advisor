...
class ShipmentRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->searchResultFactory = $this->getMock ( 'Magento\Sales\Api\Data\ShipmentSearchResultInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->subject = $objectManager->getObject ( 'Magento\Sales\Model\Order\ShipmentRepository', [ 
	...
	public function testGet($id, $entityId) {
		if (! $id) {
			...
			$shipment = $this->getMock ( 'Magento\Sales\Model\Order\Shipment', [ 
					'load',
					'getEntityId' 
			], [ ], '', false );
	...
	public function testGetList() {
		...
		$collection = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Shipment\Collection', [ 
				'addFieldToFilter' 
		], [ ], '', false );
	...
	public function testDelete() {
		...
		$shipment->expects ( $this->once () )->method ( 'getEntityId' )->willReturn ( 1 );
		
		$mapper = $this->getMockForAbstractClass ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ ], '', false, true, true, [ 
				'delete' 
		] );
		$mapper->expects ( $this->once () )->method ( 'delete' )->with ( $shipment );
	...
	public function testDeleteWithException() {
		...
		$shipment->expects ( $this->never () )->method ( 'getEntityId' );
	...
	public function testSave() {
		...
		$shipment->expects ( $this->any () )->method ( 'getEntityId' )->willReturn ( 1 );
		
		$mapper = $this->getMockForAbstractClass ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ ], '', false, true, true, [ 
				'save' 
		] );
		$mapper->expects ( $this->once () )->method ( 'save' )->with ( $shipment );
