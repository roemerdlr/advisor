...
class CreditmemoSenderTest extends AbstractSenderTest {
	...
	protected function setUp() {
		...
		$this->creditmemoResourceMock = $this->getMock ( '\Magento\Sales\Model\ResourceModel\Order\Creditmemo', [ 
				'saveAttribute' 
		], [ ], '', false );
		...
		$this->creditmemoMock->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( $this->storeMock ) );
		$this->creditmemoMock->expects ( $this->any () )->method ( 'getOrder' )->will ( $this->returnValue ( $this->orderMock ) );
		
		$this->identityContainerMock = $this->getMock ( '\Magento\Sales\Model\Order\Email\Container\CreditmemoIdentity', [ 
				'getStore',
				'isEnabled',
				'getConfigValue',
				'getTemplateId',
		...
		$this->sender = new CreditmemoSender ( $this->templateContainerMock, $this->identityContainerMock, $this->senderBuilderFactoryMock, $this->loggerMock, $this->addressRenderer, $this->paymentHelper, $this->creditmemoResourceMock, $this->globalConfig, $this->eventManagerMock );
	}
	...
	public function testSend($configValue, $forceSyncMode, $customerNoteNotify, $emailSendingResult) {
		...
		if (! $configValue || $forceSyncMode) {
			...
			$this->addressRenderer->expects ( $this->exactly ( 2 ) )->method ( 'format' )->with ( $addressMock, 'html' )->willReturn ( $address );
			...
			$this->templateContainerMock->expects ( $this->once () )->method ( 'setTemplateVars' )->with ( [ 
			...
			if ($emailSendingResult) {
				...
				$this->creditmemoResourceMock->expects ( $this->once () )->method ( 'saveAttribute' )->with ( $this->creditmemoMock, [ 
						'send_email',
						'email_sent' 
				] );
