...
class CaptureOperationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->transactionManager = $this->getMockBuilder ( $transactionClass )->disableOriginalConstructor ()->getMock ();
		$this->eventManager = $this->getMockBuilder ( 'Magento\Framework\Event\ManagerInterface' )->disableOriginalConstructor ()->getMock ();
		$this->transactionBuilder = $this->getMockBuilder ( $transactionBuilderClass )->disableOriginalConstructor ()->getMock ();
		$this->stateCommand = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Payment\State\CommandInterface' )->disableOriginalConstructor ()->getMock ();
		$objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $objectManagerHelper->getObject ( 'Magento\Sales\Model\Order\Payment\Operations\CaptureOperation', [ 
