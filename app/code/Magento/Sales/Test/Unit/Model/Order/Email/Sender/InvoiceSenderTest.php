...
class InvoiceSenderTest extends AbstractSenderTest {
	...
	protected function setUp() {
		...
		$this->invoiceResourceMock = $this->getMock ( '\Magento\Sales\Model\ResourceModel\Order\Invoice', [ 
				'saveAttribute' 
		], [ ], '', false );
		...
		$this->invoiceMock->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( $this->storeMock ) );
		$this->invoiceMock->expects ( $this->any () )->method ( 'getOrder' )->will ( $this->returnValue ( $this->orderMock ) );
		
		$this->identityContainerMock = $this->getMock ( '\Magento\Sales\Model\Order\Email\Container\InvoiceIdentity', [ 
				'getStore',
				'isEnabled',
				'getConfigValue',
				'getTemplateId',
		...
		$this->sender = new InvoiceSender ( $this->templateContainerMock, $this->identityContainerMock, $this->senderBuilderFactoryMock, $this->loggerMock, $this->addressRenderer, $this->paymentHelper, $this->invoiceResourceMock, $this->globalConfig, $this->eventManagerMock );
	}
	...
	public function testSend($configValue, $forceSyncMode, $customerNoteNotify, $emailSendingResult) {
		...
		if (! $configValue || $forceSyncMode) {
			...
			if ($emailSendingResult) {
				...
				$this->invoiceResourceMock->expects ( $this->once () )->method ( 'saveAttribute' )->with ( $this->invoiceMock, [ 
						'send_email',
						'email_sent' 
				] );
