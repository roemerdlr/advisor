...
class ShipmentSenderTest extends AbstractSenderTest {
	...
	protected function setUp() {
		...
		$this->shipmentResourceMock = $this->getMock ( '\Magento\Sales\Model\ResourceModel\Order\Shipment', [ 
				'saveAttribute' 
		], [ ], '', false );
		...
		$this->shipmentMock->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( $this->storeMock ) );
		$this->shipmentMock->expects ( $this->any () )->method ( 'getOrder' )->will ( $this->returnValue ( $this->orderMock ) );
		...
		$this->sender = new ShipmentSender ( $this->templateContainerMock, $this->identityContainerMock, $this->senderBuilderFactoryMock, $this->loggerMock, $this->addressRenderer, $this->paymentHelper, $this->shipmentResourceMock, $this->globalConfig, $this->eventManagerMock );
	}
	...
	public function testSend($configValue, $forceSyncMode, $customerNoteNotify, $emailSendingResult) {
		...
		if (! $configValue || $forceSyncMode) {
			...
			if ($emailSendingResult) {
				...
				$this->shipmentResourceMock->expects ( $this->once () )->method ( 'saveAttribute' )->with ( $this->shipmentMock, [ 
						'send_email',
						'email_sent' 
				] );
