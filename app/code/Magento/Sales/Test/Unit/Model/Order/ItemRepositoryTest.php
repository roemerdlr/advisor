...
class ItemRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->objectFactory = $this->getMockBuilder ( 'Magento\Framework\DataObject\Factory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->metadata = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Metadata' )->disableOriginalConstructor ()->getMock ();
		
		$this->searchResultFactory = $this->getMockBuilder ( 'Magento\Sales\Api\Data\OrderItemSearchResultInterfaceFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->productOptionFactory = $this->getMockBuilder ( 'Magento\Catalog\Model\ProductOptionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->extensionFactory = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\ProductOptionExtensionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
	}
	...
	public function testGetWithNoId() {
		$model = new ItemRepository ( $this->objectFactory, $this->metadata, $this->searchResultFactory, $this->productOptionFactory, $this->extensionFactory );
	...
	public function testGetEmptyEntity() {
		...
		$orderItemMock->expects ( $this->once () )->method ( 'getItemId' )->willReturn ( null );
	...
	public function testGetList() {
		...
		$filterMock->expects ( $this->once () )->method ( 'getConditionType' )->willReturn ( null );
		...
		$filterGroupMock = $this->getMockBuilder ( 'Magento\Framework\Api\Search\FilterGroup' )->disableOriginalConstructor ()->getMock ();
		$filterGroupMock->expects ( $this->once () )->method ( 'getFilters' )->willReturn ( [ 
				$filterMock 
		] );
		
		$searchCriteriaMock = $this->getMockBuilder ( 'Magento\Framework\Api\SearchCriteria' )->disableOriginalConstructor ()->getMock ();
		...
		$searchResultMock = $this->getMockBuilder ( 'Magento\Sales\Api\Data\OrderItemSearchResultInterface' )->setMethods ( [ 
				'addFieldToFilter',
				'getItems' 
		] )->getMockForAbstractClass ();
		$searchResultMock->expects ( $this->once () )->method ( 'addFieldToFilter' )->with ( $field, [ 
				'eq' => $value 
		] )->willReturnSelf ();
		...
		$this->searchResultFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $searchResultMock );
	 ...
	 */
	protected function getModel(\PHPUnit_Framework_MockObject_MockObject $orderItemMock, $productType, array $data = []) {
		$requestMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->getMock ();
		
		$requestUpdateMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->getMock ();
		$requestUpdateMock->expects ( $this->any () )->method ( 'getData' )->willReturn ( $data );
		
		$this->productOptionProcessorMock = $this->getMockBuilder ( 'Magento\Catalog\Model\ProductOptionProcessorInterface' )->getMockForAbstractClass ();
		$this->productOptionProcessorMock->expects ( $this->any () )->method ( 'convertToProductOption' )->with ( $requestMock )->willReturn ( $this->productOptionData );
		$this->productOptionProcessorMock->expects ( $this->any () )->method ( 'convertToBuyRequest' )->with ( $orderItemMock )->willReturn ( $requestUpdateMock );
	...
	protected function getOrderMock($productType, $productOption) {
		...
		$orderItemMock->expects ( $this->once () )->method ( 'getProductType' )->willReturn ( $productType );
		$orderItemMock->expects ( $this->once () )->method ( 'getBuyRequest' )->willReturn ( $requestMock );
		$orderItemMock->expects ( $this->any () )->method ( 'getProductOption' )->willReturn ( null );
		$orderItemMock->expects ( $this->any () )->method ( 'setProductOption' )->with ( $productOption )->willReturnSelf ();
	...
	protected function getProductOptionExtensionMock() {
		$productOptionExtension = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\ProductOptionExtensionInterface' )->setMethods ( [ 
				'setData' 
		] )->getMockForAbstractClass ();
