...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_fileResolverMock->expects ( $this->once () )->method ( 'get' )->with ( 'pdf.xml', 'scope' )->will ( $this->returnValue ( [ 
				file_get_contents ( __DIR__ . '/_files/pdf_one.xml' ),
				file_get_contents ( __DIR__ . '/_files/pdf_two.xml' ) 
		] ) );
		
		$this->_converter = $this->getMock ( 'Magento\Sales\Model\Order\Pdf\Config\Converter', [ 
				'convert' 
		] );
