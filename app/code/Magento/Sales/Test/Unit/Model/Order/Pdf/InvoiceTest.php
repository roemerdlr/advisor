...
class InvoiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_pdfConfigMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Pdf\Config' )->disableOriginalConstructor ()->getMock ();
		$directoryMock = $this->getMock ( 'Magento\Framework\Filesystem\Directory\Write', [ ], [ ], '', false, false );
		$directoryMock->expects ( $this->any () )->method ( 'getAbsolutePath' )->will ( $this->returnCallback ( function ($argument) {
			return BP . '/' . $argument;
		} ) );
		...
		$this->_model = $helper->getObject ( 'Magento\Sales\Model\Order\Pdf\Invoice', [ 
				'filesystem' => $filesystemMock,
				'pdfConfig' => $this->_pdfConfigMock 
		] );
	}
	public function testGetPdfInitRenderer() {
		$this->_pdfConfigMock->expects ( $this->once () )->method ( 'getRenderersPerProduct' )->with ( 'invoice' )->will ( $this->returnValue ( [ 
