...
class StatusTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->model = $objectManager->getObject ( 'Magento\Sales\Model\Order\Status', [ 
	...
	public function testUnassignStateStateIsLast() {
		...
		$this->resourceMock->expects ( $this->once () )->method ( 'checkIsStateLast' )->with ( $this->equalTo ( $params ['state'] ) )->will ( $this->returnValue ( true ) );
		$this->assertEquals ( $this->model, $this->model->unassignState ( $params ['state'] ) );
	}
	...
	protected function _getPreparedModel($resource = null, $eventDispatcher = null) {
		...
		$model = $helper->getObject ( 'Magento\Sales\Model\Order\Status', [ 
				'resource' => $resource,
	...
	public function testAssignState() {
		...
		$resource->expects ( $this->once () )->method ( 'assignState' )->with ( $this->equalTo ( $status ), $this->equalTo ( $state ) );
