...
class OrderSenderTest extends AbstractSenderTest {
	...
	protected function setUp() {
		...
		$this->orderResourceMock = $this->getMock ( '\Magento\Sales\Model\ResourceModel\Order', [ 
				'saveAttribute' 
		], [ ], '', false );
		
		$this->identityContainerMock = $this->getMock ( '\Magento\Sales\Model\Order\Email\Container\OrderIdentity', [ 
				'getStore',
				'isEnabled',
				'getConfigValue',
				'getTemplateId',
		...
		$this->sender = new OrderSender ( $this->templateContainerMock, $this->identityContainerMock, $this->senderBuilderFactoryMock, $this->loggerMock, $this->addressRenderer, $this->paymentHelper, $this->orderResourceMock, $this->globalConfig, $this->eventManagerMock );
	}
	...
	public function testSend($configValue, $forceSyncMode, $emailSendingResult) {
		...
		if (! $configValue || $forceSyncMode) {
			...
			if ($emailSendingResult) {
				...
				$this->orderResourceMock->expects ( $this->once () )->method ( 'saveAttribute' )->with ( $this->orderMock, [ 
						'send_email',
						'email_sent' 
				] );
