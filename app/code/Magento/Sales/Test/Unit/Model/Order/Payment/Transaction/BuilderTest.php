...
class BuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->repositoryMock = $this->getMock ( 'Magento\Sales\Model\Order\Payment\Transaction\Repository', [ ], [ ], '', false );
		$this->paymentMock = $this->getMock ( 'Magento\Sales\Model\Order\Payment', [ 
		...
		$this->builder = $objectManager->getObject ( 'Magento\Sales\Model\Order\Payment\Transaction\Builder', [ 
	...
	public function testCreate($transactionId, $orderId, $paymentId, $failSafe, $type, $isPaymentTransactionClosed, $additionalInfo, $document, $isTransactionExists) {
		...
		if ($isTransactionExists) {
			$this->repositoryMock->method ( 'getByTransactionId' )->withConsecutive ( [ 
					$transactionId,
					$paymentId,
					$orderId 
			], [ 
					...
					$paymentId,
					$orderId 
			] )->willReturnOnConsecutiveCalls ( $transaction, $parentTransaction );
		} else {
			$this->repositoryMock->method ( 'getByTransactionId' )->withConsecutive ( [ 
					$transactionId,
					$paymentId,
	 ...
	 */
	protected function expectsLinkWithParentTransaction($transaction, $parentTransactionId, $shouldCloseParentTransaction, $parentTransaction, $parentTransactionIsClosed) {
		...
		if ($parentTransactionId) {
			...
			$this->paymentMock->expects ( $this->once () )->method ( 'getShouldCloseParentTransaction' )->willReturn ( $shouldCloseParentTransaction );
			if ($shouldCloseParentTransaction) {
				$parentTransaction->expects ( $this->once () )->method ( 'getIsClosed' )->willReturn ( $parentTransactionIsClosed );
				if (! $parentTransactionIsClosed) {
					$parentTransaction->expects ( $this->once () )->method ( 'isFailsafe' )->willReturnSelf ();
					$parentTransaction->expects ( $this->once () )->method ( 'close' )->with ( false )->willReturnSelf ();
				}
	...
	protected function expectTransaction($orderId, $paymentId) {
		$newTransaction = $this->getMock ( 'Magento\Sales\Model\Order\Payment\Transaction', [ 
				'getId',
				'setOrderId',
				'setPaymentId',
				'loadByTxnId',
	...
	protected function expectDocument($transactionId) {
		$document = $this->getMock ( 'Magento\Sales\Model\Order', [ 
				'setTransactionId' 
		], [ ], '', false );
	...
	protected function expectSetPaymentObject($newTransaction, $type, $failSafe) {
		$newTransaction->expects ( $this->once () )->method ( 'setOrderId' )->willReturnSelf ();
		$newTransaction->expects ( $this->once () )->method ( 'setPaymentId' )->willReturnSelf ();
		$newTransaction->expects ( $this->once () )->method ( 'setTxnType' )->with ( $type )->willReturnSelf ();
		$newTransaction->expects ( $this->once () )->method ( 'isFailsafe' )->with ( $failSafe )->willReturnSelf ();
	}
	...
	protected function expectsIsPaymentTransactionClosed($isPaymentTransactionClosed, $newTransaction) {
		$this->paymentMock->expects ( $this->once () )->method ( 'hasIsTransactionClosed' )->willReturn ( $isPaymentTransactionClosed );
		$newTransaction->expects ( $isPaymentTransactionClosed ? $this->once () : $this->never () )->method ( 'setIsClosed' )->with ( ( int ) $isPaymentTransactionClosed );
		$this->paymentMock->expects ( $isPaymentTransactionClosed ? $this->once () : $this->never () )->method ( 'getIsTransactionClosed' )->willReturn ( $isPaymentTransactionClosed );
	}
