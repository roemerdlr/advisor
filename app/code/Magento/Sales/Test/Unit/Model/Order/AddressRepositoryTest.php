...
class AddressRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->metadata = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Metadata', [ 
				'getNewInstance',
				'getMapper' 
		], [ ], '', false );
		
		$this->searchResultFactory = $this->getMock ( 'Magento\Sales\Api\Data\OrderAddressSearchResultInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->subject = $objectManager->getObject ( 'Magento\Sales\Model\Order\AddressRepository', [ 
	...
	public function testGet($id, $entityId) {
		if (! $id) {
			...
			$address = $this->getMock ( 'Magento\Sales\Model\Order\Address', [ 
					'load',
					'getEntityId' 
			], [ ], '', false );
			$address->expects ( $this->once () )->method ( 'load' )->with ( $id )->willReturn ( $address );
			$address->expects ( $this->once () )->method ( 'getEntityId' )->willReturn ( $entityId );
			...
			if (! $entityId) {
				...
				$address->expects ( $this->never () )->method ( 'load' )->with ( $id )->willReturn ( $address );
				$address->expects ( $this->never () )->method ( 'getEntityId' )->willReturn ( $entityId );
				
				$this->metadata->expects ( $this->never () )->method ( 'getNewInstance' )->willReturn ( $address );
	...
	public function testGetList() {
		...
		$filterGroup = $this->getMock ( 'Magento\Framework\Api\Search\FilterGroup', [ 
				'getFilters' 
		], [ ], '', false );
		...
		$searchCriteria = $this->getMock ( 'Magento\Framework\Api\SearchCriteria', [ 
				'getFilterGroups' 
		], [ ], '', false );
		...
		$collection = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Address\Collection', [ 
				'addFieldToFilter' 
		], [ ], '', false );
	...
	public function testDelete() {
		...
		$address->expects ( $this->once () )->method ( 'getEntityId' )->willReturn ( 1 );
		
		$mapper = $this->getMockForAbstractClass ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ ], '', false, true, true, [ 
				'delete' 
		] );
		$mapper->expects ( $this->once () )->method ( 'delete' )->with ( $address );
		
		$this->metadata->expects ( $this->any () )->method ( 'getMapper' )->willReturn ( $mapper );
	...
	public function testDeleteWithException() {
		...
		$address->expects ( $this->never () )->method ( 'getEntityId' );
		
		$mapper = $this->getMockForAbstractClass ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ ], '', false, true, true, [ 
				'delete' 
		] );
		$mapper->expects ( $this->once () )->method ( 'delete' )->willThrowException ( new \Exception ( 'error' ) );
	...
	public function testSave() {
		...
		$address->expects ( $this->any () )->method ( 'getEntityId' )->willReturn ( 1 );
		
		$mapper = $this->getMockForAbstractClass ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ ], '', false, true, true, [ 
				'save' 
		] );
		$mapper->expects ( $this->once () )->method ( 'save' )->with ( $address );
