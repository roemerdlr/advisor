...
class ShipmentFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->converter = $this->getMock ( 'Magento\Sales\Model\Convert\Order', [ 
				'toShipment',
				'itemToShipmentItem' 
		], [ ], '', false );
		
		$convertOrderFactory = $this->getMock ( 'Magento\Sales\Model\Convert\OrderFactory', [ 
				'create' 
		], [ ], '', false );
		$convertOrderFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->converter );
		
		$this->trackFactory = $this->getMock ( 'Magento\Sales\Model\Order\Shipment\TrackFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->subject = $objectManager->getObject ( 'Magento\Sales\Model\Order\ShipmentFactory', [ 
	...
	public function testCreate($tracks) {
		...
		$shipmentItem = $this->getMock ( 'Magento\Sales\Model\Order\Shipment\Item', [ 
				'setQty' 
		], [ ], '', false );
		...
		$order->expects ( $this->any () )->method ( 'getAllItems' )->willReturn ( [ 
				$orderItem 
		] );
		...
		if ($tracks) {
			$shipmentTrack = $this->getMock ( 'Magento\Sales\Model\Order\Shipment\Track', [ 
					'addData' 
			], [ ], '', false );
		...
		$this->assertEquals ( $shipment, $this->subject->create ( $order, [ 
				'1' => 5 
		], $tracks ) );
	}
