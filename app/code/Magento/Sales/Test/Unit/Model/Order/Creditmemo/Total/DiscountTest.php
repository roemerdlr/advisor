...
class DiscountTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->creditmemoItemMock = $this->getMock ( '\Magento\Sales\Model\Order\Creditmemo\Item', [ 
				'getHasChildren',
				'getBaseCost',
				'getQty',
				'getOrderItem',
	...
	public function testCollect() {
		...
		$this->creditmemoMock->expects ( $this->once () )->method ( 'getAllItems' )->willReturn ( [ 
		...
		$this->creditmemoMock->expects ( $this->exactly ( 2 ) )->method ( 'roundPrice' )->willReturnMap ( [ 
				[ 
						1,
						'regular',
						true,
	...
	public function testCollectZeroShipping() {
		...
		$this->creditmemoMock->expects ( $this->exactly ( 2 ) )->method ( 'setBaseDiscountAmount' )->willReturnSelf ();
		$this->creditmemoMock->expects ( $this->once () )->method ( 'getOrder' )->willReturn ( $this->orderMock );
		$this->creditmemoMock->expects ( $this->once () )->method ( 'getBaseShippingAmount' )->willReturn ( '0.0000' );
		$this->orderMock->expects ( $this->never () )->method ( 'getBaseShippingDiscountAmount' );
		$this->orderMock->expects ( $this->never () )->method ( 'getBaseShippingAmount' );
		$this->orderMock->expects ( $this->never () )->method ( 'getShippingAmount' );
		$this->creditmemoMock->expects ( $this->once () )->method ( 'getAllItems' )->willReturn ( [ 
				$this->creditmemoItemMock 
		] );
		$this->creditmemoItemMock->expects ( $this->atLeastOnce () )->method ( 'getOrderItem' )->willReturn ( $this->orderItemMock );
