...
class ManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->manager = $objectManager->getObject ( 'Magento\Sales\Model\Order\Payment\Transaction\Manager', [ 
	...
	public function testGetAuthorizationTransaction($parentTransactionId, $paymentId, $orderId) {
		...
		if ($parentTransactionId) {
			$this->repositoryMock->expects ( $this->once () )->method ( 'getByTransactionId' )->with ( $parentTransactionId, $paymentId, $orderId )->willReturn ( $transaction );
		} else {
			$this->repositoryMock->expects ( $this->once () )->method ( 'getByTransactionType' )->with ( Transaction::TYPE_AUTH, $paymentId, $orderId )->willReturn ( $transaction );
		}
		$this->assertEquals ( $transaction, $this->manager->getAuthorizationTransaction ( $parentTransactionId, $paymentId, $orderId ) );
	}
	...
	public function testIsTransactionExists($transactionId, $isRepositoryReturnTransaction, $expectedResult) {
		...
		$this->assertEquals ( $expectedResult, $this->manager->isTransactionExists ( $transactionId, $paymentId, $orderId ) );
	}
	 ...
	 */
	public function testGenerateTransactionId($transactionId, $parentTransactionId, $transactionBasedTxnId, $type, $expectedResult) {
		...
		if (! $parentTransactionId && ! $transactionId && $transactionBasedTxnId) {
			$transactionBasedOn = $this->getMock ( 'Magento\Sales\Model\Order\Payment\Transaction', [ ], [ ], '', false );
		...
		$payment->expects ( $this->exactly ( 2 ) )->method ( 'getParentTransactionId' )->willReturnOnConsecutiveCalls ( $parentTransactionId, $transactionBasedOn ? $transactionBasedTxnId : $parentTransactionId );
		
		$this->assertEquals ( $expectedResult, $this->manager->generateTransactionId ( $payment, $type, $transactionBasedOn ) );
	}
	...
	public function isTransactionExistsDataProvider() {
		return [ 
				'withTransactionIdAndTransaction' => [ 
						"100-refund",
						true,
						true 
				],
				'withoutTransactionIdAndWithTransaction' => [ 
						null,
						true,
						false 
				],
				'withTransactionIdAndWithoutTransaction' => [ 
						"100-refund",
						false,
						false 
				],
				'withoutTransactionIdAndWithoutTransaction' => [ 
						null,
						false,
						false 
				] 
