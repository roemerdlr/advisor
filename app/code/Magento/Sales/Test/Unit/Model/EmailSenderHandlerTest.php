...
class EmailSenderHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->emailSender = $this->getMock ( 'Magento\Sales\Model\Order\Email\Sender', [ 
				'send' 
		], [ ], '', false );
		
		$this->entityResource = $this->getMockForAbstractClass ( 'Magento\Sales\Model\ResourceModel\EntityAbstract', [ ], '', false, false, true, [ 
				'save' 
		] );
		
		$this->entityCollection = $this->getMockForAbstractClass ( 'Magento\Sales\Model\ResourceModel\Collection\AbstractCollection', [ ], '', false, false, true, [ 
				'addFieldToFilter',
				'getItems' 
		] );
		
		$this->globalConfig = $this->getMock ( 'Magento\Framework\App\Config', [ ], [ ], '', false );
		
		$this->object = $objectManager->getObject ( 'Magento\Sales\Model\EmailSenderHandler', [ 
	...
	public function testExecute($configValue, $collectionItems, $emailSendingResult) {
		...
		if ($configValue) {
			$this->entityCollection->expects ( $this->at ( 0 ) )->method ( 'addFieldToFilter' )->with ( 'send_email', [ 
					'eq' => 1 
			] );
			
			$this->entityCollection->expects ( $this->at ( 1 ) )->method ( 'addFieldToFilter' )->with ( 'email_sent', [ 
					'null' => true 
			] );
	...
	public function executeDataProvider() {
		$entityModel = $this->getMockForAbstractClass ( 'Magento\Sales\Model\AbstractModel', [ ], '', false, false, true, [ 
				'setEmailSent' 
		] );
