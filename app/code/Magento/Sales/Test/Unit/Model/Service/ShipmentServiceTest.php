...
class ShipmentServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->commentRepositoryMock = $this->getMockForAbstractClass ( 'Magento\Sales\Api\ShipmentCommentRepositoryInterface', [ 
				'getList' 
		], '', false );
		...
		$this->filterBuilderMock = $this->getMock ( 'Magento\Framework\Api\FilterBuilder', [ 
				'setField',
				'setValue',
				'setConditionType',
				'create' 
		], [ ], '', false );
		$this->repositoryMock = $this->getMockForAbstractClass ( 'Magento\Sales\Api\ShipmentRepositoryInterface', [ 
				'get' 
		], '', false );
		$this->notifierMock = $this->getMock ( 'Magento\Shipping\Model\ShipmentNotifier', [ 
				'notify' 
		], [ ], '', false );
		
		$this->shipmentService = $objectManager->getObject ( 'Magento\Sales\Model\Service\ShipmentService', [ 
	...
	public function testGetLabel() {
		...
		$shipmentMock = $this->getMock ( 'Magento\Sales\Model\Order\Shipment', [ 
				'getShippingLabel' 
		], [ ], '', false );
		
		$this->repositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $id )->will ( $this->returnValue ( $shipmentMock ) );
		$shipmentMock->expects ( $this->once () )->method ( 'getShippingLabel' )->will ( $this->returnValue ( $returnValue ) );
	...
	public function testGetCommentsList() {
		...
		$this->filterBuilderMock->expects ( $this->once () )->method ( 'setConditionType' )->with ( 'eq' )->will ( $this->returnSelf () );
		...
		$this->searchCriteriaBuilderMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $searchCriteriaMock ) );
