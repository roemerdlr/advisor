...
class OrderServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->orderStatusHistoryRepositoryMock = $this->getMockBuilder ( 'Magento\Sales\Api\OrderStatusHistoryRepositoryInterface' )->disableOriginalConstructor ()->getMock ();
		$this->searchCriteriaBuilderMock = $this->getMockBuilder ( 'Magento\Framework\Api\SearchCriteriaBuilder' )->disableOriginalConstructor ()->getMock ();
		$this->searchCriteriaMock = $this->getMockBuilder ( 'Magento\Framework\Api\SearchCriteria' )->disableOriginalConstructor ()->getMock ();
		$this->filterBuilderMock = $this->getMockBuilder ( 'Magento\Framework\Api\FilterBuilder' )->disableOriginalConstructor ()->getMock ();
		$this->filterMock = $this->getMockBuilder ( 'Magento\Framework\Api\Filter' )->disableOriginalConstructor ()->getMock ();
		$this->orderNotifierMock = $this->getMockBuilder ( 'Magento\Sales\Model\OrderNotifier' )->disableOriginalConstructor ()->getMock ();
		$this->orderMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order' )->disableOriginalConstructor ()->getMock ();
		$this->orderStatusHistoryMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Status\History' )->disableOriginalConstructor ()->getMock ();
		$this->orderSearchResultMock = $this->getMockBuilder ( 'Magento\Sales\Api\Data\OrderStatusHistorySearchResultInterface' )->disableOriginalConstructor ()->getMock ();
		$this->eventManagerMock = $this->getMockBuilder ( 'Magento\Framework\Event\ManagerInterface' )->disableOriginalConstructor ()->getMock ();
		$this->orderCommentSender = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Email\Sender\OrderCommentSender' )->disableOriginalConstructor ()->getMock ();
		
		$this->orderService = new \Magento\Sales\Model\Service\OrderService ( $this->orderRepositoryMock, $this->orderStatusHistoryRepositoryMock, $this->searchCriteriaBuilderMock, $this->filterBuilderMock, $this->orderNotifierMock, $this->eventManagerMock, $this->orderCommentSender );
	}
	...
	public function testGetStatus() {
		$this->orderRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( 123 )->willReturn ( $this->orderMock );
		$this->orderMock->expects ( $this->once () )->method ( 'getStatus' )->willReturn ( 'test-status' );
	...
	public function testHold() {
		...
		$this->orderMock->expects ( $this->once () )->method ( 'hold' )->willReturn ( $this->orderMock );
