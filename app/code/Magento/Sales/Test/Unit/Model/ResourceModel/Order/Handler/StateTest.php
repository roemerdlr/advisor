...
class StateTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->addressCollectionMock = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Address\Collection', [ ], [ ], '', false );
	...
	public function testCheckOrderEmpty() {
		$this->orderMock->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( null ) );
	...
	public function testCheckSetStateComplete() {
		...
		$this->orderMock->expects ( $this->once () )->method ( 'canShip' )->will ( $this->returnValue ( false ) );
		$this->orderMock->expects ( $this->once () )->method ( 'getBaseGrandTotal' )->will ( $this->returnValue ( 100 ) );
		$this->orderMock->expects ( $this->once () )->method ( 'canCreditmemo' )->will ( $this->returnValue ( true ) );
		$this->orderMock->expects ( $this->exactly ( 2 ) )->method ( 'getState' )->will ( $this->returnValue ( Order::STATE_PROCESSING ) );
		$this->orderMock->expects ( $this->once () )->method ( 'setState' )->with ( Order::STATE_COMPLETE )->will ( $this->returnSelf () );
	...
	public function testCheckSetStateClosed() {
		...
		$this->orderMock->expects ( $this->once () )->method ( 'canShip' )->will ( $this->returnValue ( false ) );
		$this->orderMock->expects ( $this->once () )->method ( 'getBaseGrandTotal' )->will ( $this->returnValue ( 100 ) );
		$this->orderMock->expects ( $this->once () )->method ( 'canCreditmemo' )->will ( $this->returnValue ( false ) );
		$this->orderMock->expects ( $this->exactly ( 2 ) )->method ( 'getTotalRefunded' )->will ( $this->returnValue ( null ) );
		$this->orderMock->expects ( $this->once () )->method ( 'hasForcedCanCreditmemo' )->will ( $this->returnValue ( true ) );
		$this->orderMock->expects ( $this->exactly ( 2 ) )->method ( 'getState' )->will ( $this->returnValue ( Order::STATE_PROCESSING ) );
		$this->orderMock->expects ( $this->once () )->method ( 'setState' )->with ( Order::STATE_CLOSED )->will ( $this->returnSelf () );
		$this->assertEquals ( $this->state, $this->state->check ( $this->orderMock ) );
	}
