...
class OrderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->orderItemMock = $this->getMock ( 'Magento\Sales\Model\Order\Item', [ 
				'getQuoteParentItemId',
				'setTotalItemCount',
				'getChildrenItems' 
		], [ ], '', false );
		$this->storeMock = $this->getMock ( 'Magento\Store\Model\Store', [ ], [ ], '', false );
		$this->storeGroupMock = $this->getMock ( 'Magento\Store\Model\Group', [ 
				'getName',
				'getDefaultStoreId' 
		], [ ], '', false );
		...
		$this->salesSequenceManagerMock = $this->getMock ( 'Magento\SalesSequence\Model\Manager', [ ], [ ], '', false );
		...
		$this->relationCompositeMock = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\VersionControl\RelationComposite', [ ], [ ], '', false );
		$this->objectRelationProcessorMock = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\ObjectRelationProcessor', [ ], [ ], '', false );
		$contextMock = $this->getMock ( '\Magento\Framework\Model\ResourceModel\Db\Context', [ ], [ ], '', false );
		$contextMock->expects ( $this->once () )->method ( 'getResources' )->willReturn ( $this->resourceMock );
		$contextMock->expects ( $this->once () )->method ( 'getObjectRelationProcessor' )->willReturn ( $this->objectRelationProcessorMock );
		
		$objectManager = new ObjectManagerHelper ( $this );
		$this->resource = $objectManager->getObject ( 'Magento\Sales\Model\ResourceModel\Order', [ 
				'context' => $contextMock,
	...
	public function testSave() {
		...
		$this->orderMock->expects ( $this->once () )->method ( 'getData' )->willReturn ( [ 
				'increment_id' => '10000001' 
		] );
		$this->objectRelationProcessorMock->expects ( $this->once () )->method ( 'validateDataIntegrity' )->with ( null, [ 
				'increment_id' => '10000001' 
		] );
