...
class RelationTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->invoiceItemResourceMock = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Order\Invoice\Item' )->disableOriginalConstructor ()->setMethods ( [ 
				'save' 
		] )->getMock ();
		$this->invoiceCommentResourceMock = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Order\Invoice\Comment' )->disableOriginalConstructor ()->setMethods ( [ 
				'save' 
		] )->getMock ();
		...
		$this->invoiceItemMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Invoice\Item' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$this->invoiceCommentMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Invoice\Comment' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$this->orderItemMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Item' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$this->relationProcessor = new \Magento\Sales\Model\ResourceModel\Order\Invoice\Relation ( $this->invoiceItemResourceMock, $this->invoiceCommentResourceMock );
	}
	public function testProcessRelation() {
		$this->invoiceMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( 'invoice-id-value' );
		$this->invoiceMock->expects ( $this->exactly ( 2 ) )->method ( 'getItems' )->willReturn ( [ 
		...
		$this->invoiceMock->expects ( $this->exactly ( 2 ) )->method ( 'getComments' )->willReturn ( [ 
				$this->invoiceCommentMock 
		] );
