...
class CreateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->formFactoryMock = $this->getMock ( 'Magento\Customer\Model\Metadata\FormFactory', [ 
				'create' 
		], [ ], '', false );
		$this->customerFactoryMock = $this->getMock ( 'Magento\Customer\Api\Data\CustomerInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->customerMapper = $this->getMockBuilder ( 'Magento\Customer\Model\Customer\Mapper' )->setMethods ( [ 
				'toFlatArray' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->quoteInitializerMock = $this->getMock ( 'Magento\Sales\Model\AdminOrder\Product\Quote\Initializer', [ ], [ ], '', false );
		...
		$this->addressFactoryMock = $this->getMock ( 'Magento\Customer\Api\Data\AddressInterfaceFactory', [ ], [ ], '', false );
		$this->groupRepositoryMock = $this->getMockForAbstractClass ( 'Magento\Customer\Api\GroupRepositoryInterface', [ ], '', false );
		$this->scopeConfigMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\Config\ScopeConfigInterface', [ ], '', false );
		$this->emailSenderMock = $this->getMock ( 'Magento\Sales\Model\AdminOrder\EmailSender', [ ], [ ], '', false );
	...
	public function testSetAccountData() {
		...
		$customerGroupMock = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\GroupInterface', [ ], '', false, true, true, [ 
				'getTaxClassId' 
		] );
		...
		$this->customerMapper->expects ( $this->any () )->method ( 'toFlatArray' )->will ( $this->returnValue ( [ 
				'email' => 'user@example.com',
				'group_id' => 1 
		] ) );
		$quoteMock = $this->getMock ( 'Magento\Quote\Model\Quote', [ ], [ ], '', false );
		$quoteMock->expects ( $this->any () )->method ( 'getCustomer' )->will ( $this->returnValue ( $customerMock ) );
		$quoteMock->expects ( $this->once () )->method ( 'addData' )->with ( [ 
	...
	public function testUpdateQuoteItemsEmptyConfiguredOption() {
		...
		$quoteMock = $this->getMock ( 'Magento\Quote\Model\Quote', [ ], [ ], '', false );
		$quoteMock->expects ( $this->once () )->method ( 'getItemById' )->will ( $this->returnValue ( $itemMock ) );
		
		$this->sessionQuoteMock->expects ( $this->any () )->method ( 'getQuote' )->will ( $this->returnValue ( $quoteMock ) );
		$this->itemUpdater->expects ( $this->once () )->method ( 'update' )->with ( $this->equalTo ( $itemMock ), $this->equalTo ( $items [1] ) )->will ( $this->returnSelf () );
