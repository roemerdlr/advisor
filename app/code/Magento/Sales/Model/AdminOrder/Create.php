...
class Create extends \Magento\Framework\DataObject implements \Magento\Checkout\Model\Cart\CartInterface {
	 ...
	 *
	 * @return boolean @SuppressWarnings(PHPMD.BooleanGetMethodName)
	 */
	...
	public function initRuleData() {
		$this->_coreRegistry->register ( 'rule_data', new \Magento\Framework\DataObject ( [ 
	...
	public function initFromOrder(\Magento\Sales\Model\Order $order) {
		...
		$this->_eventManager->dispatch ( 'sales_convert_order_to_quote', [ 
				'order' => $order,
				'quote' => $quote 
		] );
	...
	public function initFromOrderItem(\Magento\Sales\Model\Order\Item $orderItem, $qty = null) {
		...
		if ($product->getId ()) {
			...
			if ($additionalOptions = $orderItem->getProductOptionByCode ( 'additional_options' )) {
				$item->addOption ( new \Magento\Framework\DataObject ( [ 
			...
			$this->_eventManager->dispatch ( 'sales_convert_order_item_to_quote_item', [ 
	 ...
	 *        	pass cached wishlist object and get new one
	...
	protected function _parseOptions(\Magento\Quote\Model\Quote\Item $item, $additionalOptions) {
		$productOptions = $this->_objectManager->get ( 'Magento\Catalog\Model\Product\Option\Type\DefaultType' )->setProduct ( $item->getProduct () )->getProductOptions ();
		...
		foreach ( explode ( "\n", $additionalOptions ) as $_additionalOption ) {
			if (strlen ( trim ( $_additionalOption ) )) {
				...
				if (array_key_exists ( $label, $productOptions )) {
					...
					$group = $this->_objectManager->get ( 'Magento\Catalog\Model\Product\Option' )->groupFactory ( $option->getType () )->setOption ( $option )->setProduct ( $item->getProduct () );
	...
	protected function _createCustomerForm(\Magento\Customer\Api\Data\CustomerInterface $customer) {
		$customerForm = $this->_metadataFormFactory->create ( \Magento\Customer\Api\CustomerMetadataInterface::ENTITY_TYPE_CUSTOMER, 'adminhtml_checkout', $this->customerMapper->toFlatArray ( $customer ), false, CustomerForm::DONT_IGNORE_INVISIBLE );
	...
	public function setShippingAddress($address) {
		if (is_array ( $address )) {
			$shippingAddress = $this->_objectManager->create ( 'Magento\Quote\Model\Quote\Address' )->setData ( $address )->setAddressType ( \Magento\Quote\Model\Quote\Address::TYPE_SHIPPING );
	 ...
	 *
	 * @param bool $flag
	 *        	If true - don't save in address book and actually copy data across billing and shipping
	 *        	addresses
	 * @return $this
	 */
	...
	public function setBillingAddress($address) {
		if (is_array ( $address )) {
			$billingAddress = $this->_objectManager->create ( 'Magento\Quote\Model\Quote\Address' )->setData ( $address )->setAddressType ( \Magento\Quote\Model\Quote\Address::TYPE_BILLING );
	 ...
	 * Return the updated Data Object merged with the account data
	 *
	 * @param \Magento\Customer\Api\Data\CustomerInterface $customer        	
	 ...
	 * @return void
	 * @throws \InvalidArgumentException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	public function createOrder() {
		...
		$this->_eventManager->dispatch ( 'checkout_submit_all_after', [ 
				'order' => $order,
				'quote' => $quote 
		] );
	...
	protected function _getNewCustomerEmail() {
		...
		if (empty ( $email )) {
			$host = $this->_scopeConfig->getValue ( self::XML_PATH_DEFAULT_EMAIL_DOMAIN, \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
			$account = time ();
			$email = $account . '@' . $host;
