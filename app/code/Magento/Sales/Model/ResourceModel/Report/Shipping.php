	...
	protected function _aggregateByOrderCreatedAt($from, $to) {
		...
		try {
			...
			$columns = [ 
					...
					'total_shipping' => new \Zend_Db_Expr ( "SUM((base_shipping_amount - {$shippingCanceled}) * base_to_global_rate)" ),
					'total_shipping_actual' => new \Zend_Db_Expr ( "SUM((base_shipping_invoiced - {$shippingRefunded}) * base_to_global_rate)" ) 
			];
			
			$select = $connection->select ();
			$select->from ( $sourceTable, $columns )->where ( 'state NOT IN (?)', [ 
	...
	protected function _aggregateByShippingCreatedAt($from, $to) {
		...
		try {
			...
			$columns = [ 
					...
					'total_shipping' => new \Zend_Db_Expr ( 'SUM((order_table.base_shipping_amount - ' . "{$shippingCanceled}) * order_table.base_to_global_rate)" ),
					'total_shipping_actual' => new \Zend_Db_Expr ( 'SUM((order_table.base_shipping_invoiced - ' . "{$shippingRefunded}) * order_table.base_to_global_rate)" ) 
			];
			...
			], $connection->quoteInto ( 'source_table.order_id = order_table.entity_id AND order_table.state != ?', \Magento\Sales\Model\Order::STATE_CANCELED ), [ ] )->useStraightJoin ();
			
			$filterSubSelect = $connection->select ()->from ( [ 
					'filter_source_table' => $sourceTable 
			], 'MIN(filter_source_table.entity_id)' )->where ( 'filter_source_table.order_id = source_table.order_id' );
