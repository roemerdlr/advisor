...
class Collection extends \Magento\Sales\Model\ResourceModel\Report\Collection\AbstractCollection {
	...
	protected function _makeBoundarySelect($from, $to) {
		...
		$select = $connection->select ()->from ( $this->getResource ()->getMainTable (), $cols )->where ( 'period >= ?', $from )->where ( 'period <= ?', $to )->group ( 'product_id' )->order ( $this->getOrderedField () . ' DESC' )->limit ( $this->_ratingLimit );
	...
	protected function _applyAggregatedTable() {
		...
		if (! $this->_period) {
			...
			$select->where ( new \Zend_Db_Expr ( $mainTable . '.product_id IS NOT NULL' ) )->group ( 'product_id' )->order ( $this->getOrderedField () . ' ' . \Magento\Framework\DB\Select::SQL_DESC )->limit ( $this->_ratingLimit );
	...
	public function addStoreRestrictions($storeIds) {
		...
		if (isset ( $currentStoreIds ) && $currentStoreIds != \Magento\Store\Model\Store::DEFAULT_STORE_ID && $currentStoreIds != [ 
	 ...
	 *
	 * @return $this|\Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	protected function _beforeLoad() {
		...
		if ($this->_period) {
			...
			if ('year' == $this->_period) {
				if ($periodFrom) {
					...
					if ($periodFrom->format ( 'm' ) != 1 || $periodFrom->format ( 'd' ) != 1) {
						...
						if (! $periodTo || $dtTo < $periodTo) {
							$selectUnions [] = $this->_makeBoundarySelect ( $dtFrom->format ( 'Y-m-d' ), $dtTo->format ( 'Y-m-d' ) );
				...
				if ($periodFrom && $periodTo) {
					// the same month
					if ($periodTo->format ( 'Y' ) == $periodFrom->format ( 'Y' ) && $periodTo->format ( 'm' ) == $periodFrom->format ( 'm' )) {
						$dtFrom = clone $periodFrom;
						$dtTo = clone $periodTo;
						$selectUnions [] = $this->_makeBoundarySelect ( $dtFrom->format ( 'Y-m-d' ), $dtTo->format ( 'Y-m-d' ) );
