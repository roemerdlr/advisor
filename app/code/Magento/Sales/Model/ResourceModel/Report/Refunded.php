...
class Refunded extends AbstractReport {
	...
	protected function _aggregateByOrderCreatedAt($from, $to) {
		...
		try {
			...
			$select->from ( $sourceTable, $columns )->where ( 'state != ?', \Magento\Sales\Model\Order::STATE_CANCELED )->where ( 'base_total_refunded > ?', 0 );
	...
	protected function _aggregateByRefundCreatedAt($from, $to) {
		...
		try {
			...
			], 'source_table.order_id = order_table.entity_id AND ' . $connection->quoteInto ( 'order_table.state != ?', \Magento\Sales\Model\Order::STATE_CANCELED ) . ' AND order_table.base_total_refunded > 0', [ ] );
			
			$filterSubSelect = $connection->select ();
			$filterSubSelect->from ( [ 
					'filter_source_table' => $sourceTable 
			], new \Zend_Db_Expr ( 'MAX(filter_source_table.entity_id)' ) )->where ( 'filter_source_table.order_id = source_table.order_id' );
