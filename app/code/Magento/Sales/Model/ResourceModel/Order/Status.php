...
class Status extends \Magento\Framework\Model\ResourceModel\Db\VersionControl\AbstractDb {
	...
	protected function _getLoadSelect($field, $value, $object) {
		if ($field == 'default_state') {
			...
			], $this->getMainTable () . '.status = state_table.status', 'status' )->where ( 'state_table.state = ?', $value )->order ( 'state_table.is_default DESC' )->limit ( 1 );
		} else {
	...
	public function getStoreLabels(\Magento\Sales\Model\Order\Status $status) {
		...
		], [ ] )->where ( 'status = ?', $status->getStatus () )->columns ( [ 
				'store_id',
				'label' 
		] );
	...
	protected function _afterSave(\Magento\Framework\Model\AbstractModel $object) {
		if ($object->hasStoreLabels ()) {
			...
			$this->getConnection ()->delete ( $this->labelsTable, [ 
	...
	public function assignState($status, $state, $isDefault, $visibleOnFront = false) {
		if ($isDefault) {
			$this->getConnection ()->update ( $this->stateTable, [ 
					'is_default' => 0 
			], [ 
					'state = ?' => $state 
			] );
		}
		$this->getConnection ()->insertOnDuplicate ( $this->stateTable, [ 
				'status' => $status,
				'state' => $state,
	...
	public function unassignState($status, $state) {
		...
		try {
			...
			$this->getConnection ()->delete ( $this->stateTable, [ 
					'state = ?' => $state,
					'status = ?' => $status 
			] );
			if ($isStateDefault) {
	...
	public function checkIsStateLast($state) {
		return (1 == $this->getConnection ()->fetchOne ( $this->getConnection ()->select ()->from ( [ 
				'sss' => $this->stateTable 
		], [ ] )->where ( 'state = ?', $state )->columns ( [ 
	...
	public function checkIsStatusUsed($status) {
		return ( bool ) $this->getConnection ()->fetchOne ( $this->getConnection ()->select ()->from ( [ 
				'sfo' => $this->getTable ( 'sales_order' ) 
		], [ ] )->where ( 'status = ?', $status )->limit ( 1 )->columns ( [ 
				new \Zend_Db_Expr ( 1 ) 
		] ) );
	}
	...
	protected function checkIsStateDefault($state, $status) {
		return ( bool ) $this->getConnection ()->fetchOne ( $this->getConnection ()->select ()->from ( [ 
				'sss' => $this->stateTable 
		], [ ] )->where ( 'state = ?', $state )->where ( 'status = ?', $status )->limit ( 1 )->columns ( [ 
				'is_default' 
		] ) );
	}
	 ...
	 *
	 * @param string $state        	
	 * @return string @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	protected function getStatusByState($state) {
		return ( string ) $this->getConnection ()->fetchOne ( $select = $this->getConnection ()->select ()->from ( [ 
				'sss' => $this->stateTable,
				[ ] 
		] )->where ( 'state = ?', $state )->limit ( 1 )->columns ( [ 
				'status' 
		] ) );
	}
}
