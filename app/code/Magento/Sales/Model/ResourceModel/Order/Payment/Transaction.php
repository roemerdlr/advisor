...
class Transaction extends EntityAbstract implements TransactionResourceInterface {
	...
	public function injectAsParent(\Magento\Sales\Model\Order\Payment\Transaction $transaction) {
		...
		if ($txnId && \Magento\Sales\Model\Order\Payment\Transaction::TYPE_PAYMENT === $transaction->getTxnType () && ($id = $transaction->getId ())) {
			...
			$verificationRow = $connection->fetchRow ( $connection->select ()->from ( $this->getMainTable (), [ 
					'payment_id',
					'order_id' 
	...
	public function getOrderWebsiteId($orderId) {
		...
		], 'cs.website_id' )->joinInner ( [ 
				'cs' => $this->getTable ( 'store' ) 
		], 'cs.store_id = so.store_id' )->where ( 'so.entity_id = :entity_id' );
	 ...
	 * @param string $txnId        	
	 * @param
	 *        	mixed (array|string|object) $columns
	 * @param bool $isRow        	
	...
	private function _getLoadByUniqueKeySelect($orderId, $paymentId, $txnId, $columns = '*') {
		return $this->getConnection ()->select ()->from ( $this->getMainTable (), $columns )->where ( 'order_id = ?', $orderId )->where ( 'payment_id = ?', $paymentId )->where ( 'txn_id = ?', $txnId );
	}
}
