...
class Order extends AbstractModel implements EntityInterface, OrderInterface {
	...
	/**
	 * Load order by custom attribute value.
	 ...
	 * In this state order cannot be normally processed.
	...
	public function place() {
		$this->_eventManager->dispatch ( 'sales_order_place_before', [ 
				'order' => $this 
		] );
		$this->_placePayment ();
	...
	public function hold() {
		...
		$this->setState ( self::STATE_HOLDED )->setStatus ( $this->getConfig ()->getStateDefaultStatus ( self::STATE_HOLDED ) );
		return $this;
	}
	...
	public function unhold() {
		...
		$this->setState ( $this->getHoldBeforeState () )->setStatus ( $this->getHoldBeforeStatus () );
	...
	public function cancel() {
		if ($this->canCancel ()) {
			...
			$this->_eventManager->dispatch ( 'order_cancel_after', [ 
					'order' => $this 
			] );
		}
	...
	public function isFraudDetected() {
		return $this->getOrigData ( self::STATE ) == self::STATE_PAYMENT_REVIEW && $this->getOrigData ( self::STATUS ) == self::STATUS_FRAUD;
	}
	...
	public function registerCancellation($comment = '', $graceful = true) {
		if ($this->canCancel () || $this->isPaymentReview () || $this->isFraudDetected ()) {
			...
			$this->setState ( $state )->setStatus ( $this->getConfig ()->getStateDefaultStatus ( $state ) );
			if (! empty ( $comment )) {
	 ...
	 *
	 * @param bool $asObject
	 *        	return carrier code and shipping method data as object
	...
	public function addAddress(\Magento\Sales\Model\Order\Address $address) {
		...
		if (! $address->getId ()) {
			$this->setAddresses ( array_merge ( $this->getAddresses (), [ 
					$address 
			] ) );
	...
	public function addStatusHistory(\Magento\Sales\Model\Order\Status\History $history) {
		...
		if (! $history->getId ()) {
			$this->setStatusHistories ( array_merge ( $this->getStatusHistories (), [ 
					$history 
			] ) );
			$this->setDataChanges ( true );
		}
	 ...
	 * Will be used currency with which order placed
	 *
	 * @return Currency
	 */
	 ...
	 *
	 * @param string $format
	 *        	date format type (short|medium|long|full)
	 * @return string
	 */
	...
	public function getItems() {
		if ($this->getData ( OrderInterface::ITEMS ) == null) {
			$this->setData ( OrderInterface::ITEMS, $this->getItemsCollection ()->getItems () );
		}
	...
	public function getStatusHistories() {
		if ($this->getData ( OrderInterface::STATUS_HISTORIES ) == null) {
			$this->setData ( OrderInterface::STATUS_HISTORIES, $this->getStatusHistoryCollection ()->getItems () );
		}
