...
class CreditmemoSender extends Sender {
	...
	public function send(Creditmemo $creditmemo, $forceSyncMode = false) {
		...
		if (! $this->globalConfig->getValue ( 'sales_email/general/async_sending' ) || $forceSyncMode) {
			...
			$this->eventManager->dispatch ( 'email_creditmemo_set_template_vars_before', [ 
					'sender' => $this,
					'transport' => $transport 
			] );
			...
			if ($this->checkAndSend ( $order )) {
				...
				$this->creditmemoResource->saveAttribute ( $creditmemo, [ 
						'send_email',
						'email_sent' 
				] );
				return true;
