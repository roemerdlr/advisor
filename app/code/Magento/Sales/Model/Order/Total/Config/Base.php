...
class Base extends \Magento\Sales\Model\Config\Ordered {
	 ...
	 */
	public function __construct(\Magento\Framework\App\Cache\Type\Config $configCacheType, \Psr\Log\LoggerInterface $logger, \Magento\Sales\Model\Config $salesConfig, \Magento\Sales\Model\Order\TotalFactory $orderTotalFactory, $sourceData = null) {
	...
	protected function _initModelInstance($class, $totalCode, $totalConfig) {
		...
		if (! $model instanceof \Magento\Sales\Model\Order\Total\AbstractTotal) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'The total model should be extended from \Magento\Sales\Model\Order\Total\AbstractTotal.' ) );
		}
