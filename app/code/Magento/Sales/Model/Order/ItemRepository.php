...
class ItemRepository implements OrderItemRepositoryInterface {
	 ...
	 */
	public function __construct(DataObjectFactory $objectFactory, Metadata $metadata, OrderItemSearchResultInterfaceFactory $searchResultFactory, ProductOptionFactory $productOptionFactory, ProductOptionExtensionFactory $extensionFactory, array $processorPool = []) {
	...
	public function save(OrderItemInterface $entity) {
		if ($entity->getProductOption ()) {
			...
			$entity->setProductOptions ( [ 
					'info_buyRequest' => $request->toArray () 
			] );
		}
	...
	protected function addProductOption(OrderItemInterface $orderItem) {
		...
		if (isset ( $this->processorPool [$productType] ) && ! $orderItem->getParentItemId ()) {
		...
		if (isset ( $this->processorPool ['custom_options'] ) && ! $orderItem->getParentItemId ()) {
