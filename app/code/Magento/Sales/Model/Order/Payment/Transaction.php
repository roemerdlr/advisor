...
class Transaction extends AbstractModel implements TransactionInterface {
	...
	public function getParentTransaction($shouldLoad = true) {
		if (null === $this->_parentTransaction) {
			...
			if ($parentId) {
				...
				if ($shouldLoad) {
					$this->_parentTransaction->setOrderId ( $this->getOrderId () )->setPaymentId ( $this->getPaymentId () )->load ( $parentId );
	 ...
	 * @return Transaction[] @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function close($shouldSave = true) {
		...
		if (1 == $this->getIsClosed () && $this->_isFailsafe) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'The transaction "%1" (%2) is already closed.', $this->getTxnId (), $this->getTxnType () ) );
		}
		$this->setIsClosed ( 1 );
		if ($shouldSave) {
			$this->save ();
	...
	protected function _loadChildren() {
		...
		$children = $this->getResourceCollection ()->setOrderFilter ( $orderFilter )->addPaymentIdFilter ( $this->getPaymentId () )->addParentIdFilter ( $this->getId () );
	...
	protected function _verifyPaymentObject($dryRun = false) {
		if (! $this->getPaymentId () || ! $this->getOrderId ()) {
			if (! $dryRun) {
				throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Please set a proper payment and order id.' ) );
			}
		}
	}
