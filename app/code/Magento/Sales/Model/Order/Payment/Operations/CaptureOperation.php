...
class CaptureOperation extends AbstractOperation {
	...
	public function capture(OrderPaymentInterface $payment, $invoice) {
		...
		$this->eventManager->dispatch ( 'sales_order_payment_capture', [ 
				'payment' => $payment,
				'invoice' => $invoice 
		] );
		...
		if ($invoice->getTransactionId ()) {
			...
			$method->fetchTransactionInfo ( $payment, $invoice->getTransactionId () );
		}
		
		if ($invoice->getIsPaid ()) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'The transaction "%1" cannot be captured yet.', $invoice->getTransactionId () ) );
		}
