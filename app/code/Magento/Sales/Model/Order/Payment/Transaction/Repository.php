...
class Repository implements TransactionRepositoryInterface {
	 ...
	 */
	public function __construct(SearchResultFactory $searchResultFactory, FilterBuilder $filterBuilder, SearchCriteriaBuilder $searchCriteriaBuilder, SortOrderBuilder $sortOrderBuilder, Metadata $metaData, EntityStorageFactory $entityStorageFactory) {
	 ...
	 * @throws \Magento\Framework\Exception\InputException @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	...
	public function getByTransactionId($transactionId, $paymentId, $orderId) {
		...
		if (! $entity) {
			...
			$this->metaData->getMapper ()->loadObjectByTxnId ( $entity, $orderId, $paymentId, $transactionId );
	...
	public function getList(\Magento\Framework\Api\SearchCriteria $searchCriteria) {
		...
		$collection->addPaymentInformation ( [ 
				'method' 
		] );
		$collection->addOrderInformation ( [ 
				'increment_id' 
		] );
		return $collection;
	}
