...
class Item extends AbstractModel implements CreditmemoItemInterface {
	...
	public function setQty($qty) {
		...
		if ($qty <= $this->getOrderItem ()->getQtyToRefund () || $this->getOrderItem ()->isDummy ()) {
			...
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'We found an invalid quantity to refund item "%1".', $this->getName () ) );
		}
		return $this;
	}
	...
	public function register() {
		...
		$orderItem->setDiscountTaxCompensationRefunded ( $orderItem->getDiscountTaxCompensationRefunded () + $this->getDiscountTaxCompensationAmount () );
		$orderItem->setBaseDiscountTaxCompensationRefunded ( $orderItem->getBaseDiscountTaxCompensationRefunded () + $this->getBaseDiscountTaxCompensationAmount () );
	...
	public function cancel() {
		...
		$this->getOrderItem ()->setTaxRefunded ( $this->getOrderItem ()->getTaxRefunded () - $this->getOrderItem ()->getBaseTaxAmount () * $this->getQty () / $this->getOrderItem ()->getQtyOrdered () );
		$this->getOrderItem ()->setDiscountTaxCompensationRefunded ( $this->getOrderItem ()->getDiscountTaxCompensationRefunded () - $this->getOrderItem ()->getDiscountTaxCompensationAmount () * $this->getQty () / $this->getOrderItem ()->getQtyOrdered () );
		return $this;
	}
	...
	public function calcRowTotal() {
		...
		if ($rowTotalInclTax && $baseRowTotalInclTax) {
			...
			$this->setRowTotalInclTax ( $creditmemo->roundPrice ( $rowTotalInclTax / $orderItemQty * $this->getQty (), 'including' ) );
			$this->setBaseRowTotalInclTax ( $creditmemo->roundPrice ( $baseRowTotalInclTax / $orderItemQty * $this->getQty (), 'including_base' ) );
		}
		return $this;
	}
	 ...
	 * @return $this
	 */
	public function setExtensionAttributes(\Magento\Sales\Api\Data\CreditmemoItemExtensionInterface $extensionAttributes) {
