...
abstract class Create extends \Magento\Backend\App\Action {
	...
	protected function _processActionData($action = null) {
		...
		if (! $this->_getOrderCreateModel ()->getQuote ()->isVirtual ()) {
			...
			if ($syncFlag === null && $this->_getOrderCreateModel ()->getShippingAddress ()->getSameAsBilling () && empty ( $shippingMethod )) {
		...
		if ($data = $this->getRequest ()->getPost ( 'add_products' )) {
			$this->_getGiftmessageSaveModel ()->importAllowQuoteItemsFromProducts ( $this->_objectManager->get ( 'Magento\Framework\Json\Helper\Data' )->jsonDecode ( $data ) );
		}
		...
		if (! empty ( $couponCode )) {
			...
			if (! $isApplyDiscount) {
				$this->messageManager->addError ( __ ( '"%1" coupon code was not applied. Do not apply discount is selected for item(s)', $this->escaper->escapeHtml ( $couponCode ) ) );
			} else {
				if ($this->_getQuote ()->getCouponCode () !== $couponCode) {
					$this->messageManager->addError ( __ ( '"%1" coupon code is not valid.', $this->escaper->escapeHtml ( $couponCode ) ) );
				} else {
