...
class Guest extends \Magento\Framework\App\Helper\AbstractHelper {
	 ...
	 * @return \Magento\Framework\Controller\Result\Redirect|bool @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function loadValidOrder(App\RequestInterface $request) {
		...
		if (empty ( $post ) && ! $fromCookie) {
			...
			if (empty ( $incrementId ) || empty ( $lastName ) || empty ( $type ) || empty ( $storeId ) || ! in_array ( $type, [ 
					'email',
					'zip' 
