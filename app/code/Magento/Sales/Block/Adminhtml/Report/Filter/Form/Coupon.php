...
class Coupon extends \Magento\Sales\Block\Adminhtml\Report\Filter\Form {
	...
	protected function _afterToHtml($html) {
		if ($this->_renderDependentElement) {
			 ...
			 * form creates appropriate child block and uses this alias.
			 * In this case we can't use the same alias
			...
			$formAfterBlock = $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Form\Element\Dependence', 'adminhtml.block.widget.form.element.dependence' );
			$formAfterBlock->addFieldMap ( $htmlIdPrefix . 'price_rule_type', 'price_rule_type' )->addFieldMap ( $htmlIdPrefix . 'rules_list', 'rules_list' )->addFieldDependence ( 'rules_list', 'price_rule_type', '1' );
