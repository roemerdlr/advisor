...
class Detail extends \Magento\Backend\Block\Widget\Container {
	...
	protected function _construct() {
		...
		if ($fetchTransactionAllowed && $canFetchTransaction) {
			$fetchUrl = $this->getUrl ( 'sales/*/fetch', [ 
					'_current' => true 
			] );
	...
	public function getHeaderText() {
		return __ ( "Transaction # %1 | %2", $this->_txn->getTxnId (), $this->formatDate ( $this->_txn->getCreatedAt (), \IntlDateFormatter::MEDIUM, true ) );
	}
	...
	protected function _toHtml() {
		$this->setTxnIdHtml ( $this->adminHelper->escapeHtmlWithLinks ( $this->_txn->getHtmlTxnId (), [ 
				'a' 
		] ) );
		
		$this->setParentTxnIdUrlHtml ( $this->escapeHtml ( $this->getUrl ( 'sales/transactions/view', [ 
		...
		$this->setOrderIdUrlHtml ( $this->escapeHtml ( $this->getUrl ( 'sales/order/view', [ 
