...
class View extends \Magento\Backend\Block\Widget\Form\Container {
	...
	protected function _construct() {
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::actions_edit' ) && $order->canEdit ()) {
			...
			$this->buttonList->add ( 'order_edit', [ 
					'label' => __ ( 'Edit' ),
					'class' => 'edit primary',
					'onclick' => $onclickJs,
					'data_attribute' => [ 
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::cancel' ) && $order->canCancel ()) {
			$this->buttonList->add ( 'order_cancel', [ 
					'label' => __ ( 'Cancel' ),
					'class' => 'cancel',
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::hold' ) && $order->canHold ()) {
			$this->buttonList->add ( 'order_hold', [ 
					'label' => __ ( 'Hold' ),
					'class' => __ ( 'hold' ),
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::unhold' ) && $order->canUnhold ()) {
			$this->buttonList->add ( 'order_unhold', [ 
					'label' => __ ( 'Unhold' ),
					'class' => __ ( 'unhold' ),
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::review_payment' )) {
			if ($order->canReviewPayment ()) {
				...
				$this->buttonList->add ( 'accept_payment', [ 
			...
			if ($order->canFetchPaymentReviewUpdate ()) {
				$this->buttonList->add ( 'get_review_payment_update', [ 
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::invoice' ) && $order->canInvoice ()) {
			...
			$this->buttonList->add ( 'order_invoice', [ 
					'label' => $_label,
		...
		if ($this->_isAllowedAction ( 'Magento_Sales::ship' ) && $order->canShip () && ! $order->getForcedShipmentWithInvoice ()) {
			$this->buttonList->add ( 'order_ship', [ 
					'label' => __ ( 'Ship' ),
	...
	public function getHeaderText() {
		...
		return __ ( 'Order # %1 %2 | %3', $this->getOrder ()->getRealOrderId (), $_extOrderId, $this->formatDate ( $this->_localeDate->date ( new \DateTime ( $this->getOrder ()->getCreatedAt () ) ), \IntlDateFormatter::MEDIUM, true ) );
	}
	...
	public function getReviewPaymentUrl($action) {
		return $this->getUrl ( 'sales/*/reviewPayment', [ 
				'action' => $action 
		] );
	}
	...
	protected function getEditMessage($order) {
		...
		if (! empty ( $nonEditableTypes )) {
			return __ ( 'This order contains (%1) items and therefore cannot be edited through the admin interface. ' . 'If you wish to continue editing, the (%2) items will be removed, ' . ' the order will be canceled and a new order will be placed.', implode ( ', ', $nonEditableTypes ), implode ( ', ', $nonEditableTypes ) );
		}
