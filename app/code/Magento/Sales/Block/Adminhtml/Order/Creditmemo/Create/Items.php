...
class Items extends \Magento\Sales\Block\Adminhtml\Items\AbstractItems {
	...
	protected function _prepareLayout() {
		...
		$this->addChild ( 'update_button', 'Magento\Backend\Block\Widget\Button', [ 
		...
		if ($this->getCreditmemo ()->canRefund ()) {
			if ($this->getCreditmemo ()->getInvoice () && $this->getCreditmemo ()->getInvoice ()->getTransactionId ()) {
				$this->addChild ( 'submit_button', 'Magento\Backend\Block\Widget\Button', [ 
						'label' => __ ( 'Refund' ),
						'class' => 'save submit-button refund primary',
						'onclick' => 'disableElements(\'submit-button\');submitCreditMemo()' 
				] );
			}
			$this->addChild ( 'submit_offline', 'Magento\Backend\Block\Widget\Button', [ 
	...
	public function getOrderTotalbarData() {
		...
		$totalBarData [] = [ 
				__ ( 'Paid Amount' ),
				$this->displayPriceAttribute ( 'total_invoiced' ),
				false 
		];
		$totalBarData [] = [ 
				__ ( 'Refund Amount' ),
				$this->displayPriceAttribute ( 'total_refunded' ),
				false 
		];
		$totalBarData [] = [ 
				__ ( 'Shipping Amount' ),
				$this->displayPriceAttribute ( 'shipping_invoiced' ),
				false 
		];
		$totalBarData [] = [ 
				__ ( 'Shipping Refund' ),
				$this->displayPriceAttribute ( 'shipping_refunded' ),
				false 
		];
		$totalBarData [] = [ 
				__ ( 'Order Grand Total' ),
				$this->displayPriceAttribute ( 'grand_total' ),
				true 
		];
		return $totalBarData;
	}
	...
	public function getUpdateUrl() {
		return $this->getUrl ( 'sales/*/updateQty', [ 
		...
		if ($this->_canReturnToStock === null) {
			...
			if ($this->_canReturnToStock) {
				...
				foreach ( $this->getCreditmemo ()->getAllItems () as $item ) {
					...
					$stockItem = $this->stockRegistry->getStockItem ( $productId, $item->getOrderItem ()->getStore ()->getWebsiteId () );
