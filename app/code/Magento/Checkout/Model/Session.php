...
class Session extends \Magento\Framework\Session\SessionManager {
	 ...
	 *
	 * @return Quote @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	public function getQuote() {
		$this->_eventManager->dispatch ( 'custom_quote_process', [ 
		...
		if ($this->_quote === null) {
			...
			if ($this->getQuoteId ()) {
				try {
					 ...
					 * need recalculate totals of quote.
					 * It is possible if customer use currency switcher or
					 * store switcher.
					 */
			...
			if (! $this->getQuoteId ()) {
				if ($this->_customerSession->isLoggedIn () || $this->_customer) {
					...
					$this->_eventManager->dispatch ( 'checkout_quote_init', [ 
							'quote' => $quote 
					] );
				}
			}
	...
	public function loadCustomerQuote() {
		...
		$this->_eventManager->dispatch ( 'load_customer_quote_before', [ 
				'checkout_session' => $this 
		] );
	...
	public function clearQuote() {
		$this->_eventManager->dispatch ( 'checkout_quote_destroy', [ 
				'quote' => $this->getQuote () 
		] );
		$this->_quote = null;
		$this->setQuoteId ( null );
	...
	public function restoreQuote() {
		...
		if ($order->getId ()) {
			try {
				...
				$this->_eventManager->dispatch ( 'restore_quote', [ 
						'order' => $order,
						'quote' => $quote 
				] );
				return true;
	...
	/**
	 *
	 * @return bool|null @codeCoverageIgnore
	 */
