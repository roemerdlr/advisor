...
class AttributeMerger {
	 ...
	 *        	name of the storage container used by UI component
	...
	public function merge($elements, $providerName, $dataScopePrefix, array $fields = []) {
		foreach ( $elements as $attributeCode => $attributeConfig ) {
			...
			$fields [$attributeCode] = $this->getFieldConfig ( $attributeCode, $attributeConfig, $additionalConfig, $providerName, $dataScopePrefix );
		}
		return $fields;
	}
	 ...
	 *        	field configuration provided via layout XML
	 * @param string $providerName
	 *        	name of the storage container used by UI component
	 * @param string $dataScopePrefix        	
	 ...
	 */
	protected function getFieldConfig($attributeCode, array $attributeConfig, array $additionalConfig, $providerName, $dataScopePrefix) {
		...
		$element = [ 
				...
				'config' => [
						...
						'customEntry' => isset ( $additionalConfig ['config'] ['customEntry'] ) ? $additionalConfig ['config'] ['customEntry'] : null,
						'template' => 'ui/form/field',
						'elementTmpl' => isset ( $additionalConfig ['config'] ['elementTmpl'] ) ? $additionalConfig ['config'] ['elementTmpl'] : $elementTemplate,
						'tooltip' => isset ( $additionalConfig ['config'] ['tooltip'] ) ? $additionalConfig ['config'] ['tooltip'] : null 
				],
	...
	protected function getMultilineFieldConfig($attributeCode, array $attributeConfig, $providerName, $dataScopePrefix) {
		...
		for($lineIndex = 0; $lineIndex < ( int ) $attributeConfig ['size']; $lineIndex ++) {
			...
			$line = [ 
					...
					], $attributeConfig ['validation'] ) : $attributeConfig ['validation'],
	...
	/**
	 * Order country options.
	 * Move top countries to the beginning of the list.
	 *
