...
class Cart extends \Magento\Checkout\Block\Cart\AbstractCart {
	...
	public function prepareItemUrls() {
		...
		foreach ( $this->getItems () as $item ) {
			...
			if ($item->getStoreId () != $this->_storeManager->getStore ()->getId () && ! $item->getRedirectUrl () && ! $product->isVisibleInSiteVisibility ()) {
	...
	public function isWishlistActive() {
		...
		if ($isActive === null) {
			$isActive = $this->_scopeConfig->getValue ( 'wishlist/general/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE ) && $this->httpContext->getValue ( Context::CONTEXT_AUTH );
	 ...
	 *
	 * @param string $alias
	 *        	Container block alias in layout
	 * @return array
	 */
