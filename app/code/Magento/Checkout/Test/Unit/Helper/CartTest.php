...
class CartTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetShouldRedirectToCart() {
		$storeId = 1;
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'isSetFlag' )->with ( Cart::XML_PATH_REDIRECT_TO_CART, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId )->will ( $this->returnValue ( true ) );
	...
	public function testGetAddUrl() {
		...
		$productMock->expects ( $this->any () )->method ( 'getUrlDataObject' )->will ( $this->returnValue ( new DataObject ( [ 
				'store_id' => $storeId 
		] ) ) );
		...
		$this->helper->getAddUrl ( $productMock, [ 
				'custom_param' => 'value' 
		] );
	}
