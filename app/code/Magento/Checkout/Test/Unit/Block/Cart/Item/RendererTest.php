...
class RendererTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$context->expects ( $this->once () )->method ( 'getLayout' )->will ( $this->returnValue ( $this->layout ) );
	...
	public function testGetIdentities() {
		...
		$product->expects ( $this->exactly ( 2 ) )->method ( 'getIdentities' )->will ( $this->returnValue ( $identities ) );
	...
	public function testGetProductPriceHtml() {
		...
		$priceRender->expects ( $this->once () )->method ( 'render' )->with ( \Magento\Catalog\Pricing\Price\ConfiguredPriceInterface::CONFIGURED_PRICE_CODE, $product, [ 
	...
	public function testGetActions() {
		 ...
		 */
		$blockMock = $this->getMockBuilder ( 'Magento\Checkout\Block\Cart\Item\Renderer\Actions' )->disableOriginalConstructor ()->getMock ();
		
		$this->layout->expects ( $this->once () )->method ( 'getChildName' )->with ( $this->_renderer->getNameInLayout (), 'actions' )->willReturn ( $blockNameInLayout );
		$this->layout->expects ( $this->once () )->method ( 'getBlock' )->with ( $blockNameInLayout )->willReturn ( $blockMock );
		...
		$blockMock->expects ( $this->once () )->method ( 'setItem' )->with ( $itemMock );
