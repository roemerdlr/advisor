...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetUrl() {
		...
		$link = $this->_objectManagerHelper->getObject ( 'Magento\Checkout\Block\Cart\Link', [ 
				'context' => $context 
		] );
	...
	public function testToHtml() {
		...
		$block = $this->_objectManagerHelper->getObject ( 'Magento\Checkout\Block\Cart\Link', [ 
				'cartHelper' => $helper,
				'moduleManager' => $moduleManager 
		] );
		$moduleManager->expects ( $this->any () )->method ( 'isOutputEnabled' )->with ( 'Magento_Checkout' )->will ( $this->returnValue ( true ) );
	...
	public function testGetLabel($productCount, $label) {
		$helper = $this->getMockBuilder ( 'Magento\Checkout\Helper\Cart' )->disableOriginalConstructor ()->setMethods ( [ 
				'getSummaryCount' 
		] )->getMock ();
