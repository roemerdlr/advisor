...
class AbstractCartTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetItemRenderer($type, $expectedType) {
		...
		$renderer->expects ( $this->once () )->method ( 'getRenderer' )->with ( $expectedType, AbstractCart::DEFAULT_TYPE )->will ( $this->returnValue ( 'rendererObject' ) );
		...
		$layout->expects ( $this->once () )->method ( 'getBlock' )->with ( 'renderer.list' )->will ( $this->returnValue ( $renderer ) );
		
		/** @var $block \Magento\Sales\Block\Items\AbstractItems */
		$block = $this->_objectManager->getObject ( 'Magento\Checkout\Block\Cart\AbstractCart', [ 
				'context' => $this->_objectManager->getObject ( 'Magento\Backend\Block\Template\Context', [ 
						'layout' => $layout 
				] ) 
		] );
	...
	public function testGetTotalsCache($expectedResult, $isVirtual) {
		...
		$model = $this->_objectManager->getObject ( 'Magento\Checkout\Block\Cart\AbstractCart', [ 
