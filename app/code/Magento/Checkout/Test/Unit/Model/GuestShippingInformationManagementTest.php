...
class GuestShippingInformationManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->quoteIdMaskFactoryMock = $this->getMock ( '\Magento\Quote\Model\QuoteIdMaskFactory', [ 
				'create' 
		], [ ], '', false );
		$this->shippingInformationManagementMock = $this->getMock ( '\Magento\Checkout\Api\ShippingInformationManagementInterface' );
		
		$this->model = $objectManager->getObject ( 'Magento\Checkout\Model\GuestShippingInformationManagement', [ 
	...
	public function testSaveAddressInformation() {
		...
		$this->shippingInformationManagementMock->expects ( $this->once () )->method ( 'saveAddressInformation' )->with ( $quoteId, $addressInformationMock )->willReturn ( $paymentInformationMock );
