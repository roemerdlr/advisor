...
class SidebarTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->sidebar = new Sidebar ( $this->cartMock, $this->checkoutHelperMock, $this->resolverMock );
	}
	...
	public function testCheckQuoteItem() {
		...
		$itemMock = $this->getMockBuilder ( 'Magento\Quote\Api\Data\CartItemInterface' )->getMock ();
		
		$quoteMock = $this->getMockBuilder ( 'Magento\Quote\Model\Quote' )->disableOriginalConstructor ()->getMock ();
		$quoteMock->expects ( $this->once () )->method ( 'getItemById' )->with ( $itemId )->willReturn ( $itemMock );
	...
	public function testUpdateQuoteItem() {
		...
		$this->resolverMock->expects ( $this->once () )->method ( 'getLocale' )->willReturn ( 'en' );
		
		$this->cartMock->expects ( $this->once () )->method ( 'updateItems' )->with ( [ 
				$itemId => [ 
						'qty' => $itemQty 
				] 
		] )->willReturnSelf ();
