...
class CartTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->cart = $this->objectManagerHelper->getObject ( 'Magento\Checkout\Model\Cart', [ 
	...
	public function testSuggestItemsQty() {
		...
		$this->quoteMock->expects ( $this->any () )->method ( 'getItemById' )->will ( $this->returnValueMap ( [ 
				[ 
						2,
						$this->prepareQuoteItemMock ( 2 ) 
				],
				[ 
						3,
						$this->prepareQuoteItemMock ( 3 ) 
				],
				[ 
						4,
						$this->prepareQuoteItemMock ( 4 ) 
				],
				[ 
						5,
						$this->prepareQuoteItemMock ( 5 ) 
				] 
		] ) );
	...
	public function testUpdateItems() {
		...
		$infoDataObject = $this->objectManagerHelper->getObject ( 'Magento\Framework\DataObject', [ 
				'data' => $data 
		] );
		
		$this->checkoutSessionMock->expects ( $this->once () )->method ( 'getQuote' )->will ( $this->returnValue ( $this->quoteMock ) );
		$this->eventManagerMock->expects ( $this->at ( 0 ) )->method ( 'dispatch' )->with ( 'checkout_cart_update_items_before', [ 
				'cart' => $this->cart,
				'info' => $infoDataObject 
		] );
		$this->eventManagerMock->expects ( $this->at ( 1 ) )->method ( 'dispatch' )->with ( 'checkout_cart_update_items_after', [ 
				'cart' => $this->cart,
				'info' => $infoDataObject 
		] );
	...
	public function prepareQuoteItemMock($itemId) {
		...
		switch ($itemId) {
			case 2 :
				...
				$product->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( 4 ) );
			...
			case 3 :
				...
				$product->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( 5 ) );
				$product->expects ( $this->once () )->method ( 'getStore' )->will ( $this->returnValue ( $store ) );
				break;
			case 4 :
				$product = false;
	...
	public function testGetSummaryQty($useQty) {
		...
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( 'checkout/cart_link/use_qty', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->will ( $this->returnValue ( $useQty ) );
