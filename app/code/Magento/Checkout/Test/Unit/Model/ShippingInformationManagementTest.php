...
class ShippingInformationManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->paymentDetailsFactoryMock = $this->getMock ( '\Magento\Checkout\Model\PaymentDetailsFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = $objectManager->getObject ( '\Magento\Checkout\Model\ShippingInformationManagement', [ 
		...
		$this->shippingAddressMock = $this->getMock ( '\Magento\Quote\Model\Quote\Address', [ 
	 ...
	 * @expectedExceptionMessage Cart contains virtual product(s) only.
	 * Shipping address is not applicable.
	 */
	public function testSaveAddressInformationIfCartIsVirtual() {
		...
		$addressInformationMock->expects ( $this->once () )->method ( 'getShippingAddress' )->willReturn ( $this->shippingAddressMock );
	...
	public function testSaveAddressInformationIfShippingAddressNotSet() {
		...
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getCustomerAddressId' )->willReturn ( $customerAddressId );
		
		$this->addressValidatorMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->shippingAddressMock )->willReturn ( true );
		
		$this->quoteMock->expects ( $this->once () )->method ( 'setShippingAddress' )->with ( $this->shippingAddressMock )->willReturnSelf ();
		$this->quoteMock->expects ( $this->exactly ( 2 ) )->method ( 'getShippingAddress' )->willReturn ( $this->shippingAddressMock );
		
		$customerAddressMock = $this->getMock ( '\Magento\Customer\Api\Data\AddressInterface' );
		$this->addressRepositoryMock->expects ( $this->once () )->method ( 'getById' )->with ( $customerAddressId )->willReturn ( $customerAddressMock );
		
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'importCustomerAddressData' )->with ( $customerAddressMock )->willReturnSelf ();
		...
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'setCollectShippingRates' )->with ( true )->willReturnSelf ();
	...
	/**
	 * @expectedException \Magento\Framework\Exception\InputException
	 * @expectedExceptionMessage Unable to save address.
	...
	public function testSaveAddressInformationThrowExceptionWhileAddressSaving() {
		...
		$this->totalsCollectorMock->expects ( $this->once () )->method ( 'collectAddressTotals' )->willThrowException ( $exception );
		$this->addressValidatorMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->shippingAddressMock )->willReturn ( true );
	...
	public function testSaveAddressInformationIfCarrierCodeIsInvalid() {
		...
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'setCollectShippingRates' )->with ( true )->willReturnSelf ();
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getCountryId' )->willReturn ( 1 );
		$this->totalsCollectorMock->expects ( $this->once () )->method ( 'collectAddressTotals' )->with ( $this->quoteMock, $this->shippingAddressMock );
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getShippingMethod' )->willReturn ( $shippingMethod );
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getShippingRateByCode' )->with ( $shippingMethod )->willReturn ( false );
	...
	public function testSaveAddressInformationIfMinimumAmountIsNotValid() {
		...
		$this->totalsCollectorMock->expects ( $this->once () )->method ( 'collectAddressTotals' )->with ( $this->quoteMock, $this->shippingAddressMock );
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getShippingMethod' )->willReturn ( $shippingMethod );
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getShippingRateByCode' )->with ( $shippingMethod )->willReturn ( $this->getMock ( '\Magento\Quote\Model\Quote\Address\Rate', [ ], [ ], '', false ) );
		
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( 'sales/minimum_order/error_message', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId )->willReturn ( $minAmountExceptionMessage );
	...
	/**
	 * @expectedException \Magento\Framework\Exception\InputException
	 * @expectedExceptionMessage Unable to save shipping information.
	 * Please, check input data.
	 */
	public function testSaveAddressInformationIfCanNotSaveQuote() {
		...
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'getActive' )->with ( $cartId )->willReturn ( $this->quoteMock );
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'save' )->with ( $this->quoteMock )->willThrowException ( $exception );
		$addressInformationMock = $this->getMock ( '\Magento\Checkout\Api\Data\ShippingInformationInterface' );
		$addressInformationMock->expects ( $this->once () )->method ( 'getShippingAddress' )->willReturn ( $this->shippingAddressMock );
		$addressInformationMock->expects ( $this->once () )->method ( 'getShippingCarrierCode' )->willReturn ( $carrierCode );
		$addressInformationMock->expects ( $this->once () )->method ( 'getShippingMethodCode' )->willReturn ( $shippingMethod );
	...
	public function testSaveAddressInformation() {
		...
		$this->paymentMethodManagementMock->expects ( $this->once () )->method ( 'getList' )->with ( $cartId )->willReturn ( [ 
				$paymentMethodMock 
		] );
		...
		$paymentDetailsMock->expects ( $this->once () )->method ( 'setPaymentMethods' )->with ( [ 
				$paymentMethodMock 
		] )->willReturnSelf ();
		$paymentDetailsMock->expects ( $this->once () )->method ( 'setTotals' )->with ( $totalsMock )->willReturnSelf ();
		
		$this->assertEquals ( $paymentDetailsMock, $this->model->saveAddressInformation ( $cartId, $addressInformationMock ) );
	}
}
