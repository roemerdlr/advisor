...
class ItemPoolTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $objectManager->getObject ( 'Magento\Checkout\CustomerData\ItemPool', [ 
	...
	public function testGetItemDataIfItemNotExistInMap() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( $this->defaultItemId )->willReturn ( $itemMock );
	...
	public function testGetItemDataIfItemExistInMap() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( $this->itemMap [$productType] )->willReturn ( $itemMock );
	...
	public function testGetItemDataIfItemNotValid() {
		...
		$quoteItemMock = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ ], [ ], '', false );
		$quoteItemMock->expects ( $this->once () )->method ( 'getProductType' )->willReturn ( $productType );
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( $this->defaultItemId )->willReturn ( $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ ], [ ], '', false ) );
		$this->assertEquals ( $itemData, $this->model->getItemData ( $quoteItemMock ) );
	}
}
