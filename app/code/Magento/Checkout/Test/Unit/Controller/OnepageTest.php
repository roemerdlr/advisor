...
class OnepageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$objectManagerMock->expects ( $this->at ( 0 ) )->method ( 'get' )->with ( 'Magento\Checkout\Model\Session' )->willReturn ( $this->checkoutSession );
		$objectManagerMock->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( 'Magento\Customer\Model\Session' )->willReturn ( $this->customerSession );
		
		$context = $this->getMock ( 'Magento\Framework\App\Action\Context', [ ], [ ], '', false );
		$context->expects ( $this->once () )->method ( 'getObjectManager' )->willReturn ( $objectManagerMock );
		...
		$this->controller = $objectManager->getObject ( 'Magento\Checkout\Test\Unit\Controller\Stub\OnepageStub', [ 
				'context' => $context 
		] );
	}
