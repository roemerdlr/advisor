...
class IndexTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$resultPageFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Result\PageFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$resultPageFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->resultPageMock );
		
		$resultRedirectFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Controller\Result\RedirectFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$resultRedirectFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->resultRedirectMock );
		...
		$objectManagerReturns = [ 
				...
				[ 
						...
						$this->basicMock ( 'Magento\Checkout\Model\Session' ) 
				],
				[ 
						'Magento\Customer\Model\Session',
						$this->basicMock ( 'Magento\Customer\Model\Session' ) 
				] 
		]
		;
		$this->objectManagerMock->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValueMap ( $objectManagerReturns ) );
		...
		$this->basicStub ( $this->contextMock, 'getMessageManager' )->willReturn ( $this->basicMock ( '\Magento\Framework\Message\ManagerInterface' ) );
		...
		// SUT
		$this->model = $this->objectManager->getObject ( 'Magento\Checkout\Controller\Index\Index', [ 
	...
	private function basicStub($mock, $method) {
		return $mock->expects ( $this->any () )->method ( $method )->withAnyParameters ();
	}
