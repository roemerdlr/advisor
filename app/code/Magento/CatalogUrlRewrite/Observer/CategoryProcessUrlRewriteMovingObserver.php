...
class CategoryProcessUrlRewriteMovingObserver implements ObserverInterface {
	 ...
	 */
	public function __construct(CategoryUrlRewriteGenerator $categoryUrlRewriteGenerator, UrlPersistInterface $urlPersist, ScopeConfigInterface $scopeConfig, UrlRewriteHandler $urlRewriteHandler) {
	...
	public function execute(\Magento\Framework\Event\Observer $observer) {
		...
		if ($category->dataHasChangedFor ( 'parent_id' )) {
			...
			$urlRewrites = array_merge ( $this->categoryUrlRewriteGenerator->generate ( $category, true ), $this->urlRewriteHandler->generateProductUrlRewrites ( $category ) );
