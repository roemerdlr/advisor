...
class AfterImportDataObserver implements ObserverInterface {
	 ...
	 * @throws \InvalidArgumentException @SuppressWarnings(PHPMD.ExcessiveParameterList)
	 */
	public function __construct(\Magento\Catalog\Model\ProductFactory $catalogProductFactory, \Magento\Eav\Model\Config $eavConfig, \Magento\CatalogUrlRewrite\Model\ObjectRegistryFactory $objectRegistryFactory, \Magento\CatalogUrlRewrite\Model\ProductUrlPathGenerator $productUrlPathGenerator, \Magento\Framework\App\ResourceConnection $resource, \Magento\CatalogUrlRewrite\Service\V1\StoreViewService $storeViewService, \Magento\Store\Model\StoreManagerInterface $storeManager, UrlPersistInterface $urlPersist, UrlRewriteFactory $urlRewriteFactory, UrlFinderInterface $urlFinder) {
		...
		if (! $attribute) {
			throw new \InvalidArgumentException ( sprintf ( 'Cannot retrieve attribute for entity type "%s"', Product::ENTITY ) );
		}
	 ...
	 * @param array $rowData        	
	 *
	 * @return ImportExport @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	protected function generateUrls() {
		 ...
		 */
		$urls = array_merge ( $this->canonicalUrlRewriteGenerate (), $this->categoriesUrlRewriteGenerate (), $this->currentUrlRewritesRegenerate () );
	...
	protected function cleanOverriddenUrlKey() {
		...
		$select = $this->connection->select ()->from ( $this->urlKeyAttributeBackendTable, [ 
				'store_id',
				'entity_id' 
		] )->where ( 'attribute_id = ?', $this->urlKeyAttributeId )->where ( implode ( ' OR ', $this->entityStoresToCheckOverridden ) );
		...
		foreach ( $this->products as $productId => $productsByStores ) {
			foreach ( $productsByStores as $storeId => $product ) {
				foreach ( $this->categoryCache [$productId] as $categoryId ) {
					...
					$urls [] = $this->urlRewriteFactory->create ()->setEntityType ( ProductUrlRewriteGenerator::ENTITY_TYPE )->setEntityId ( $productId )->setRequestPath ( $requestPath )->setTargetPath ( $this->productUrlPathGenerator->getCanonicalUrlPath ( $product, $category ) )->setStoreId ( $storeId )->setMetadata ( [ 
	...
	protected function generateForAutogenerated($url, $category) {
		...
		if (isset ( $this->products [$productId] [$storeId] )) {
			...
			return [ 
					$this->urlRewriteFactory->create ()->setEntityType ( ProductUrlRewriteGenerator::ENTITY_TYPE )->setEntityId ( $productId )->setRequestPath ( $url->getRequestPath () )->setTargetPath ( $targetPath )->setRedirectType ( OptionProvider::PERMANENT )->setStoreId ( $storeId )->setDescription ( $url->getDescription () )->setIsAutogenerated ( 0 )->setMetadata ( $url->getMetadata () ) 
			];
		}
		return [ ];
	}
	...
	protected function generateForCustom($url, $category) {
		...
		if (isset ( $this->products [$productId] [$storeId] )) {
			...
			return [ 
					$this->urlRewriteFactory->create ()->setEntityType ( ProductUrlRewriteGenerator::ENTITY_TYPE )->setEntityId ( $productId )->setRequestPath ( $url->getRequestPath () )->setTargetPath ( $targetPath )->setRedirectType ( $url->getRedirectType () )->setStoreId ( $storeId )->setDescription ( $url->getDescription () )->setIsAutogenerated ( 0 )->setMetadata ( $url->getMetadata () ) 
			];
		}
		return [ ];
	}
