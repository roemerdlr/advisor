...
class CategoryUrlPathAutogeneratorObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->category = $this->getMock ( 'Magento\Catalog\Model\Category', [ 
				'setUrlKey',
				'setUrlPath',
				'dataHasChangedFor',
				'isObjectNew',
		...
		$this->childrenCategoriesProvider = $this->getMock ( 'Magento\CatalogUrlRewrite\Model\Category\ChildrenCategoriesProvider' );
		
		$this->storeViewService = $this->getMock ( 'Magento\CatalogUrlRewrite\Service\V1\StoreViewService', [ ], [ ], '', false );
		
		$this->categoryUrlPathAutogeneratorObserver = (new ObjectManagerHelper ( $this ))->getObject ( 'Magento\CatalogUrlRewrite\Observer\CategoryUrlPathAutogeneratorObserver', [ 
	...
	public function testUrlKeyAndUrlPathUpdating() {
		$this->categoryUrlPathGenerator->expects ( $this->once () )->method ( 'getUrlKey' )->with ( $this->category )->willReturn ( 'url_key' );
		$this->categoryUrlPathGenerator->expects ( $this->once () )->method ( 'getUrlPath' )->with ( $this->category )->willReturn ( 'url_path' );
	...
	public function testChildrenUrlPathAttributeUpdatingForSpecificStore() {
		...
		$childCategory = $this->getMockBuilder ( 'Magento\Catalog\Model\Category' )->setMethods ( [ 
				'getUrlPath',
				'setUrlPath',
				'getResource',
				'getStore',
		...
		$this->childrenCategoriesProvider->expects ( $this->once () )->method ( 'getChildren' )->willReturn ( [ 
				$childCategory 
		] );
