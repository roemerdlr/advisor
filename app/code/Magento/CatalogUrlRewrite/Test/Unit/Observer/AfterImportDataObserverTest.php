...
class AfterImportDataObserverTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->importProduct = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product', [ 
				'getNewSku',
		...
		$this->catalogProductFactory = $this->getMock ( '\Magento\Catalog\Model\ProductFactory', [ 
				'create' 
		], [ ], '', false );
		$this->storeManager = $this->getMockBuilder ( '\Magento\Store\Model\StoreManagerInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getWebsite' 
		] )->getMockForAbstractClass ();
		$this->event = $this->getMock ( '\Magento\Framework\Event', [ 
				'getAdapter',
				'getBunch' 
		], [ ], '', false );
		...
		$this->observer = $this->getMock ( '\Magento\Framework\Event\Observer', [ 
				'getEvent' 
		], [ ], '', false );
		...
		$this->productUrlRewriteGenerator = $this->getMockBuilder ( '\Magento\CatalogUrlRewrite\Model\ProductUrlRewriteGenerator' )->disableOriginalConstructor ()->setMethods ( [ 
				'generate' 
		] )->getMock ();
		...
		$this->eavConfig = $this->getMock ( '\Magento\Eav\Model\Config', [ 
				'getAttribute' 
		], [ ], '', false );
		...
		$this->connection = $this->getMockBuilder ( '\Magento\Framework\DB\Adapter\AdapterInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'quoteInto',
				'select',
				'fetchAll' 
		] )->getMockForAbstractClass ();
		$this->resource->expects ( $this->any () )->method ( 'getConnection' )->willReturn ( $this->connection );
		$this->select = $this->getMock ( '\Magento\Framework\DB\Select', [ 
				'from',
				'where' 
		], [ ], '', false );
		...
		$this->urlFinder = $this->getMockBuilder ( '\Magento\UrlRewrite\Model\UrlFinderInterface' )->setMethods ( [ 
				'findAllByData' 
		] )->disableOriginalConstructor ()->getMockForAbstractClass ();
		...
		$categoryProcessor = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product\CategoryProcessor', [ 
				'getCategoryById' 
		], [ ], '', false );
		$category = $this->getMock ( 'Magento\Catalog\Model\Category', [ 
				'getId' 
		], [ ], '', false );
		...
		$this->import = $this->objectManager->getObject ( '\Magento\CatalogUrlRewrite\Observer\AfterImportDataObserver', [ 
	...
	public function testAfterImportData() {
		...
		$websiteMock = $this->getMock ( '\Magento\Store\Model\Website', [ 
				'getStoreIds' 
		], [ ], '', false );
		$storeIds = [ 
				1,
		...
		$this->importProduct->expects ( $this->exactly ( $productsCount ) )->method ( 'getNewSku' )->withConsecutive ( [ 
				$this->products [0] [ImportProduct::COL_SKU] 
		], [ 
				$this->products [1] [ImportProduct::COL_SKU] 
		] )->will ( $this->onConsecutiveCalls ( $newSku [0], $newSku [1] ) );
		$this->importProduct->expects ( $this->exactly ( $productsCount ) )->method ( 'getProductCategories' )->withConsecutive ( [ 
				$this->products [0] [ImportProduct::COL_SKU] 
		], [ 
				$this->products [1] [ImportProduct::COL_SKU] 
		] )->willReturn ( [ ] );
		$getProductWebsitesCallsCount = $productsCount * 2;
		$this->importProduct->expects ( $this->exactly ( $getProductWebsitesCallsCount ) )->method ( 'getProductWebsites' )->willReturnOnConsecutiveCalls ( [ 
		...
		$product->expects ( $this->exactly ( $productsCount ) )->method ( 'setId' )->withConsecutive ( [ 
				$newSku [0] ['entity_id'] 
		], [ 
				$newSku [1] ['entity_id'] 
		] );
		...
		$product->expects ( $this->exactly ( $productsCount ) )->method ( 'setStoreId' )->withConsecutive ( [ 
				$this->products [0] [ImportProduct::COL_STORE] 
		], [ 
				$this->products [1] [ImportProduct::COL_STORE] 
		] );
		$this->catalogProductFactory->expects ( $this->exactly ( $productsCount ) )->method ( 'create' )->willReturn ( $product );
		$this->connection->expects ( $this->exactly ( 4 ) )->method ( 'quoteInto' )->withConsecutive ( [ 
				'(store_id = ?',
				$storeIds [0] 
		], [ 
				' AND entity_id = ?)',
	...
	public function testCleanOverriddenUrlKey() {
		...
		$this->select->expects ( $this->exactly ( 2 ) )->method ( 'where' )->withConsecutive ( [ 
				'attribute_id = ?',
				$urlKeyAttributeId 
		], [ 
		...
		$this->connection->expects ( $this->once () )->method ( 'fetchAll' )->willReturn ( $entityStoresToClean );
	...
	public function testCleanOverriddenUrlKeyEmptyEntityStoresToCheckOverridden() {
		...
		$this->select->expects ( $this->never () )->method ( 'from' );
	...
	public function testCanonicalUrlRewriteGenerateWithUrlPath() {
		...
		$this->productUrlPathGenerator->expects ( $this->once () )->method ( 'getUrlPathWithSuffix' )->will ( $this->returnValue ( $requestPath ) );
		$this->productUrlPathGenerator->expects ( $this->once () )->method ( 'getUrlPath' )->will ( $this->returnValue ( 'urlPath' ) );
		$this->productUrlPathGenerator->expects ( $this->once () )->method ( 'getCanonicalUrlPath' )->will ( $this->returnValue ( $targetPath ) );
		$this->urlRewrite->expects ( $this->once () )->method ( 'setStoreId' )->with ( $storeId )->will ( $this->returnSelf () );
		$this->urlRewrite->expects ( $this->once () )->method ( 'setEntityId' )->with ( $productId )->will ( $this->returnSelf () );
		$this->urlRewrite->expects ( $this->once () )->method ( 'setEntityType' )->with ( ProductUrlRewriteGenerator::ENTITY_TYPE )->will ( $this->returnSelf () );
		$this->urlRewrite->expects ( $this->once () )->method ( 'setRequestPath' )->with ( $requestPath )->will ( $this->returnSelf () );
		$this->urlRewrite->expects ( $this->once () )->method ( 'setTargetPath' )->with ( $targetPath )->will ( $this->returnSelf () );
	...
	public function testCanonicalUrlRewriteGenerateWithEmptyUrlPath() {
		...
		$this->productUrlPathGenerator->expects ( $this->once () )->method ( 'getUrlPath' )->will ( $this->returnValue ( '' ) );
	...
	public function testCategoriesUrlRewriteGenerate() {
		...
		$this->urlRewrite->expects ( $this->any () )->method ( 'setMetadata' )->with ( [ 
	...
	protected function getImportMock($methods = []) {
		return $this->getMock ( '\Magento\CatalogUrlRewrite\Observer\AfterImportDataObserver', $methods, [ 
	 ...
	 */
	protected function currentUrlRewritesRegeneratorPrepareUrlRewriteMock($storeId, $productId, $requestPath, $targetPath, $redirectType, $metadata, $description) {
		$this->urlRewrite->expects ( $this->any () )->method ( 'setStoreId' )->with ( $storeId )->will ( $this->returnSelf () );
		$this->urlRewrite->expects ( $this->any () )->method ( 'setEntityId' )->with ( $productId )->will ( $this->returnSelf () );
		$this->urlRewrite->expects ( $this->any () )->method ( 'setEntityType' )->with ( ProductUrlRewriteGenerator::ENTITY_TYPE )->will ( $this->returnSelf () );
	...
	protected function currentUrlRewritesRegeneratorGetCurrentRewritesMocks($currentRewrites) {
		...
		foreach ( $currentRewrites as $urlRewrite ) {
			 ...
			 */
			$url = $this->getMockBuilder ( 'Magento\UrlRewrite\Service\V1\Data\UrlRewrite' )->disableOriginalConstructor ()->getMock ();
			foreach ( $urlRewrite as $key => $value ) {
				$url->expects ( $this->any () )->method ( 'get' . str_replace ( ' ', '', ucwords ( str_replace ( '_', ' ', $key ) ) ) )->will ( $this->returnValue ( $value ) );
			}
			$rewrites [] = $url;
		}
		return $rewrites;
