...
class ChildrenCategoriesProviderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$categoryCollection = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Collection\AbstractCollection' )->disableOriginalConstructor ()->setMethods ( [ 
		...
		$categoryCollection->expects ( $this->any () )->method ( 'addIdFilter' )->with ( [ 
				'id' 
		] )->willReturnSelf ();
		...
		$this->childrenCategoriesProvider = (new ObjectManager ( $this ))->getObject ( 'Magento\CatalogUrlRewrite\Model\Category\ChildrenCategoriesProvider' );
	}
	public function testGetChildrenRecursive() {
		...
		$this->connection->expects ( $this->any () )->method ( 'fetchCol' )->with ( $this->select, $bind )->willReturn ( [ 
				'id' 
		] );
