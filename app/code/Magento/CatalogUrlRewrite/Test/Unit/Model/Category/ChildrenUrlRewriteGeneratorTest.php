...
class ChildrenUrlRewriteGeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->childrenCategoriesProvider = $this->getMockBuilder ( 'Magento\CatalogUrlRewrite\Model\Category\ChildrenCategoriesProvider' )->disableOriginalConstructor ()->getMock ();
		$this->category = $this->getMockBuilder ( 'Magento\Catalog\Model\Category' )->disableOriginalConstructor ()->getMock ();
		$this->categoryUrlRewriteGeneratorFactory = $this->getMockBuilder ( 'Magento\CatalogUrlRewrite\Model\CategoryUrlRewriteGeneratorFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->categoryUrlRewriteGenerator = $this->getMockBuilder ( 'Magento\CatalogUrlRewrite\Model\CategoryUrlRewriteGenerator' )->disableOriginalConstructor ()->getMock ();
		$this->childrenUrlRewriteGenerator = (new ObjectManager ( $this ))->getObject ( 'Magento\CatalogUrlRewrite\Model\Category\ChildrenUrlRewriteGenerator', [ 
	...
	public function testNoChildrenCategories() {
		$this->childrenCategoriesProvider->expects ( $this->once () )->method ( 'getChildren' )->with ( $this->category, false )->will ( $this->returnValue ( [ ] ) );
	...
	public function testGenerate() {
		...
		$this->categoryUrlRewriteGenerator->expects ( $this->once () )->method ( 'generate' )->with ( $childCategory )->will ( $this->returnValue ( [ 
				[ 
						'url-1',
						'url-2' 
				] 
