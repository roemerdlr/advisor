...
class ProductUrlPathGeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productUrlPathGenerator = (new ObjectManager ( $this ))->getObject ( 'Magento\CatalogUrlRewrite\Model\ProductUrlPathGenerator', [ 
	...
	public function getUrlPathDataProvider() {
		return [ 
				'path based on url key' => [ 
						'url-key',
						null,
						'url-key' 
				],
				'path based on product name 1' => [ 
						'',
						'product-name',
						'product-name' 
				],
				'path based on product name 2' => [ 
						null,
						'product-name',
						'product-name' 
				],
				'path based on product name 3' => [ 
						false,
						'product-name',
						'product-name' 
				] 
	...
	public function testGetUrlPath($urlKey, $productName, $result) {
		$this->product->expects ( $this->once () )->method ( 'getData' )->with ( 'url_path' )->will ( $this->returnValue ( null ) );
	...
	public function testGetUrlPathWithSuffix() {
		...
		$this->assertEquals ( 'product-path.html', $this->productUrlPathGenerator->getUrlPathWithSuffix ( $this->product, null ) );
	}
	...
	public function testGetCanonicalUrlPath() {
		...
		$this->assertEquals ( 'catalog/product/view/id/1', $this->productUrlPathGenerator->getCanonicalUrlPath ( $this->product ) );
	}
