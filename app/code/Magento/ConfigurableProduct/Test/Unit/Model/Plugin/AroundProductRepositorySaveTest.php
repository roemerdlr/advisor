...
class AroundProductRepositorySaveTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productOptionRepositoryMock = $this->getMock ( 'Magento\ConfigurableProduct\Api\OptionRepositoryInterface' );
		$this->configurableTypeFactoryMock = $this->getMock ( '\Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\ConfigurableFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->productFactoryMock = $this->getMockBuilder ( '\Magento\Catalog\Model\ProductFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->plugin = $objectManager->getObject ( 'Magento\ConfigurableProduct\Model\Plugin\AroundProductRepositorySave', [ 
				'optionRepository' => $this->productOptionRepositoryMock,
				'productFactory' => $this->productFactoryMock,
				'typeConfigurableFactory' => $this->configurableTypeFactoryMock 
		] );
		
		$this->productExtensionMock = $this->getMock ( 'Magento\Catalog\Api\Data\ProductExtension', [ 
	...
	public function testAroundSaveWithoutOptions() {
		...
		$this->productInterfaceMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( $this->productExtensionMock );
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getConfigurableProductOptions' )->willReturn ( null );
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getConfigurableProductLinks' )->willReturn ( null );
		
		$this->assertEquals ( $this->productMock, $this->plugin->aroundSave ( $this->productRepositoryMock, $this->closureMock, $this->productInterfaceMock ) );
	}
	protected function setupProducts($productIds, $attributeCode, $additionalProductId = null) {
		...
		foreach ( $productIds as $productId ) {
			...
			$productMock->expects ( $this->once () )->method ( 'load' )->with ( $productId )->willReturnSelf ();
			$productMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( $productId );
			$productMock->expects ( $this->any () )->method ( 'getData' )->with ( $attributeCode )->willReturn ( $productId );
			$this->productFactoryMock->expects ( $this->at ( $count ) )->method ( 'create' )->willReturn ( $productMock );
			$products [] = $productMock;
			$count ++;
		}
		
		if ($additionalProductId) {
			$nonExistingProductMock = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->getMock ();
			$nonExistingProductMock->expects ( $this->once () )->method ( 'load' )->with ( $additionalProductId )->willReturnSelf ();
			$this->productFactoryMock->expects ( $this->at ( $count ) )->method ( 'create' )->willReturn ( $nonExistingProductMock );
	...
	protected function setupConfigurableProductAttributes($attributeCodes) {
		...
		$this->productMock->expects ( $this->once () )->method ( 'getTypeInstance' )->willReturn ( $configurableProductTypeMock );
		
		$configurableAttributes = [ ];
		foreach ( $attributeCodes as $attributeCode ) {
			$configurableAttribute = $this->getMockBuilder ( '\Magento\ConfigurableProduct\Model\Product\Type\Configurable\Attribute' )->setMethods ( [ 
					'getProductAttribute' 
			] )->disableOriginalConstructor ()->getMock ();
			$productAttributeMock = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Eav\Attribute' )->disableOriginalConstructor ()->getMock ();
			$productAttributeMock->expects ( $this->once () )->method ( 'getAttributeCode' )->willReturn ( $attributeCode );
			$configurableAttribute->expects ( $this->once () )->method ( 'getProductAttribute' )->willReturn ( $productAttributeMock );
			$configurableAttributes [] = $configurableAttribute;
		}
		
		$configurableProductTypeMock->expects ( $this->once () )->method ( 'getConfigurableAttributes' )->with ( $this->productMock )->willReturn ( $configurableAttributes );
	...
	public function testAroundSaveWithLinks() {
		...
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getConfigurableProductOptions' )->willReturn ( null );
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getConfigurableProductLinks' )->willReturn ( $links );
		...
		$configurableTypeMock = $this->getMockBuilder ( '\Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable' )->disableOriginalConstructor ()->getMock ();
		$this->configurableTypeFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $configurableTypeMock );
		$configurableTypeMock->expects ( $this->once () )->method ( 'saveProducts' )->with ( $this->productMock, $links );
	...
	public function testAroundSaveWithNonExistingLinks() {
		...
		$nonExistingProductMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( null );
		$links [] = $nonExistingId;
		...
		$configurableTypeMock = $this->getMockBuilder ( '\Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable' )->disableOriginalConstructor ()->getMock ();
		$this->configurableTypeFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $configurableTypeMock );
		$configurableTypeMock->expects ( $this->never () )->method ( 'saveProducts' )->with ( $this->productMock, $links );
	...
	public function testAroundSaveWithLinksWithMissingAttribute() {
		...
		$simpleProductMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( $simpleProductId );
		$simpleProductMock->expects ( $this->any () )->method ( 'getData' )->with ( $configurableAttributeCode )->willReturn ( null );
	...
	public function testAroundSaveWithLinksWithDuplicateAttributes() {
		...
		$simpleProductMock = $productMocks [2];
		$simpleProductMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( $simpleProductId );
		$simpleProductMock->expects ( $this->any () )->method ( 'getData' )->with ( $configurableAttributeCode )->willReturn ( 4 );
	...
	protected function setupExistingOptions(array $existingOptionIds) {
		...
		foreach ( $existingOptionIds as $existingOptionId ) {
			...
			$optionMock->expects ( $this->any () )->method ( 'getId' )->willReturn ( $existingOptionId );
			$options [] = $optionMock;
		}
		
		$productExtensionMock = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\ProductExtension' )->disableOriginalConstructor ()->setMethods ( [ 
				'getConfigurableProductOptions' 
		] )->getMock ();
		$productExtensionMock->expects ( $this->any () )->method ( 'getConfigurableProductOptions' )->willReturn ( $options );
		
		$this->productMock->expects ( $this->any () )->method ( 'getExtensionAttributes' )->willReturn ( $productExtensionMock );
	}
	protected function setupOptions(array $optionIds) {
		$options = [ ];
		foreach ( $optionIds as $optionId ) {
			$optionMock = $this->getMock ( '\Magento\ConfigurableProduct\Api\Data\OptionInterface' );
			$optionMock->expects ( $this->any () )->method ( 'getId' )->willReturn ( $optionId );
			$options [] = $optionMock;
		}
		...
		$productExtensionMock->expects ( $this->any () )->method ( 'getConfigurableProductOptions' )->willReturn ( $options );
		$productExtensionMock->expects ( $this->any () )->method ( 'getConfigurableProductLinks' )->willReturn ( null );
		
		$this->productInterfaceMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( $productExtensionMock );
		return $options;
	}
}
