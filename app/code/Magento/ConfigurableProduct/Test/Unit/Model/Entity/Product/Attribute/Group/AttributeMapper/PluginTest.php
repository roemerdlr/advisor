...
class PluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->attributeFactory = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable\AttributeFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->attribute = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable\Attribute' )->setMethods ( [ 
				'getUsedAttributes',
				'getAttributeId',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->magentoObject = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getId' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->model = $helper->getObject ( 'Magento\ConfigurableProduct\Model\Entity\Product\Attribute\Group\AttributeMapper\Plugin', [ 
	...
	public function testAroundMap() {
		...
		$this->attribute->expects ( $this->once () )->method ( 'getUsedAttributes' )->with ( $this->equalTo ( $attrSetId ) )->will ( $this->returnValue ( [ 
				$attrSetId 
		] ) );
