...
class LinkManagementTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->productFactory = $this->getMock ( '\Magento\Catalog\Api\Data\ProductInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->dataObjectHelperMock = $this->getMockBuilder ( '\Magento\Framework\Api\DataObjectHelper' )->disableOriginalConstructor ()->getMock ();
		
		$this->configurableType = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable' )->disableOriginalConstructor ()->getMock ();
		
		$this->object = $this->objectManagerHelper->getObject ( '\Magento\ConfigurableProduct\Model\LinkManagement', [ 
	...
	public function testGetChildren() {
		...
		$productTypeInstance->expects ( $this->any () )->method ( 'getUsedProducts' )->with ( $product )->willReturn ( [ 
				$childProduct 
		] );
		...
		$childProduct->expects ( $this->once () )->method ( 'getAttributes' )->willReturn ( [ 
				$attribute 
		] );
		
		$productMock = $this->getMock ( '\Magento\Catalog\Api\Data\ProductInterface' );
		
		$this->dataObjectHelperMock->expects ( $this->once () )->method ( 'populateWithArray' )->with ( $productMock, [ 
				'store_id' => 1,
				'code' => 10 
		], '\Magento\Catalog\Api\Data\ProductInterface' )->willReturnSelf ();
	...
	public function testAddChild() {
		...
		$simple = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->getMock ();
		...
		$this->configurableType->expects ( $this->once () )->method ( 'getChildrenIds' )->with ( 666 )->will ( $this->returnValue ( [ 
				0 => [ 
						1,
						2,
						3 
		...
		$configurable->expects ( $this->once () )->method ( '__call' )->with ( 'setAssociatedProductIds', [ 
				[ 
						1,
						2,
						3,
	...
	public function testRemoveChild() {
		...
		$productType = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\Product\Type\Configurable' )->setMethods ( [ 
				'getUsedProducts' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->productRepository->expects ( $this->any () )->method ( 'get' )->with ( $productSku )->will ( $this->returnValue ( $product ) );
		
		$option = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->setMethods ( [ 
				'getSku',
				'getId',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$productType->expects ( $this->once () )->method ( 'getUsedProducts' )->will ( $this->returnValue ( [ 
				$option 
		] ) );
		
		$product->expects ( $this->once () )->method ( 'addData' )->with ( [ 
	...
	public function testRemoveChildForbidden() {
		...
		$product->expects ( $this->any () )->method ( 'getTypeId' )->will ( $this->returnValue ( \Magento\Catalog\Model\Product\Type::TYPE_SIMPLE ) );
