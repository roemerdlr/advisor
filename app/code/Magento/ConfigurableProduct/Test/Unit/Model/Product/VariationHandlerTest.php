...
class VariationHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_productFactoryMock = $this->getMock ( 'Magento\Catalog\Model\ProductFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_entityFactoryMock = $this->getMock ( 'Magento\Eav\Model\EntityFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_attributeSetFactory = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute\SetFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_stockConfiguration = $this->getMock ( 'Magento\CatalogInventory\Api\StockConfigurationInterface', [ ], [ ], '', false );
		$this->configurableProduct = $this->getMock ( 'Magento\ConfigurableProduct\Model\Product\Type\Configurable', [ ], [ ], '', false );
		
		$this->_product = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getMediaGallery' 
		], [ ], '', false );
		
		$this->_model = $this->_objectHelper->getObject ( 'Magento\ConfigurableProduct\Model\Product\VariationHandler', [ 
	...
	public function testGenerateSimpleProducts() {
		...
		$parentProductMock = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->setMethods ( [ 
				'__wakeup',
		...
		$newSimpleProductMock = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->setMethods ( [ 
				'__wakeup',
				'save',
				'getId',
				'setStoreId',
		...
		$attributeMock = $this->getMockBuilder ( '\Magento\Eav\Model\Entity\Attribute' )->setMethods ( [ 
				'isInSet',
				'setAttributeSetId',
				'setAttributeGroupId',
				'save' 
		] )->disableOriginalConstructor ()->getMock ();
		$attributeSetMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute\Set' )->setMethods ( [ 
				'load',
				'addSetInfo',
				'getDefaultGroupId' 
		] )->disableOriginalConstructor ()->getMock ();
		$eavEntityMock = $this->getMockBuilder ( '\Magento\Eav\Model\Entity' )->setMethods ( [ 
				'setType',
				'getTypeId' 
		] )->disableOriginalConstructor ()->getMock ();
		$productTypeMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Type' )->setMethods ( [ 
				'getEditableAttributes' 
		] )->disableOriginalConstructor ()->getMock ();
		$editableAttributeMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute' )->setMethods ( [ 
				'getIsUnique',
				'getAttributeCode',
				'getFrontend',
				'getIsVisible' 
		] )->disableOriginalConstructor ()->getMock ();
		$frontendAttributeMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute\Frontend' )->setMethods ( [ 
				'getInputType' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->configurableProduct->expects ( $this->once () )->method ( 'getUsedProductAttributes' )->willReturn ( [ 
				$attributeMock 
		] );
		...
		$attributeSetMock->expects ( $this->once () )->method ( 'addSetInfo' )->with ( 'type_id', [ 
				$attributeMock 
		] );
		...
		$productTypeMock->expects ( $this->once () )->method ( 'getEditableAttributes' )->with ( $newSimpleProductMock )->willReturn ( [ 
				$editableAttributeMock 
		] );
		...
		$parentProductMock->expects ( $this->once () )->method ( 'getQuantityAndStockStatus' )->willReturn ( [ 
				'is_in_stock' => 1 
		] );
		...
		$this->assertEquals ( [ 
				'product_id' 
		], $this->_model->generateSimpleProducts ( $parentProductMock, $productsData ) );
	}
