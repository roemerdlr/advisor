...
class PluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->jsonHelperMock = $this->getMock ( 'Magento\Framework\Json\Helper\Data', [ 
				'jsonDecode' 
		], [ ], '', false );
		...
		$this->responseMock = $this->getMock ( 'Magento\Framework\DataObject', [ 
				'setError',
				'setMessage',
				'setAttributes' 
		], [ ], '', false );
		...
		$this->plugin = new \Magento\ConfigurableProduct\Model\Product\Validator\Plugin ( $this->eventManagerMock, $this->productFactoryMock, $this->jsonHelperMock );
	}
	public function testAroundValidateWithVariationsValid() {
		...
		$plugin = $this->getMock ( 'Magento\ConfigurableProduct\Model\Product\Validator\Plugin', [ 
		...
		$plugin->expects ( $this->once () )->method ( '_validateProductVariations' )->with ( $this->productMock, $matrix, $this->requestMock )->will ( $this->returnValue ( null ) );
		...
		$this->assertEquals ( $this->proceedResult, $plugin->aroundValidate ( $this->subjectMock, $this->closureMock, $this->productMock, $this->requestMock, $this->responseMock ) );
	}
	public function testAroundValidateWithVariationsInvalid() {
		...
		$plugin->expects ( $this->once () )->method ( '_validateProductVariations' )->with ( $this->productMock, $matrix, $this->requestMock )->will ( $this->returnValue ( true ) );
	 ...
	 * @return \PHPUnit_Framework_MockObject_MockObject|\Magento\Catalog\Model\Product @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	private function createProduct($index, $id, $isValid = true) {
		...
		$this->productFactoryMock->expects ( $this->at ( $index ) )->method ( 'create' )->will ( $this->returnValue ( $productMock ) );
		$productMock->expects ( $this->once () )->method ( 'validate' )->will ( $this->returnValue ( $isValid ) );
	...
	private function createAttribute($attributeCode, $isUserDefined, $isRequired) {
		...
		$attribute->expects ( $this->any () )->method ( 'getIsRequired' )->will ( $this->returnValue ( $isRequired ) );
		$attribute->expects ( $this->any () )->method ( 'getIsUserDefined' )->will ( $this->returnValue ( $isUserDefined ) );
