...
class OptionRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->configurableTypeResource = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable' )->disableOriginalConstructor ()->getMock ();
		$this->optionResource = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable\Attribute' )->disableOriginalConstructor ()->getMock ();
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $objectManager->getObject ( '\Magento\ConfigurableProduct\Model\OptionRepository', [ 
	...
	public function testGet() {
		...
		$productExtensionMock = $this->getMockBuilder ( '\Magento\Catalog\Api\Data\ProductExtension' )->setMethods ( [ 
				'getConfigurableProductOptions' 
		] )->getMock ();
		$productExtensionMock->expects ( $this->once () )->method ( 'getConfigurableProductOptions' )->willReturn ( [ 
				$optionMock 
		] );
	...
	public function testGetNotProductById() {
		...
		$option = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Api\Data\OptionInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getProductId' 
		] )->getMockForAbstractClass ();
		$option->expects ( $this->once () )->method ( 'getProductId' )->willReturn ( $productId );
	...
	public function testDeleteCantSaveProducts() {
		...
		$this->configurableTypeResource->expects ( $this->once () )->method ( 'saveProducts' )->with ( $this->productMock )->willThrowException ( new \Exception () );
		$this->model->delete ( $option );
	}
	...
	public function testDeleteCantDeleteOption() {
		...
		$option = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\Product\Type\Configurable\Attribute' )->disableOriginalConstructor ()->setMethods ( [ 
				'getProductId',
				'getId' 
		] )->getMockForAbstractClass ();
		$option->expects ( $this->any () )->method ( 'getProductId' )->willReturn ( $productId );
		$option->expects ( $this->once () )->method ( 'getId' )->willReturn ( $optionId );
		
		$this->productRepositoryMock->expects ( $this->once () )->method ( 'getById' )->with ( $productId )->willReturn ( $this->productMock );
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->willReturn ( Configurable::TYPE_CODE );
		
		$this->configurableTypeResource->expects ( $this->once () )->method ( 'saveProducts' )->with ( $this->productMock );
		$this->optionResource->expects ( $this->once () )->method ( 'delete' )->with ( $option )->willThrowException ( new \Exception () );
		$this->model->delete ( $option );
	}
