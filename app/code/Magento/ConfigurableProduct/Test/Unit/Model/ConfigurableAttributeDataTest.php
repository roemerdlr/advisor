...
class ConfigurableAttributeDataTest extends \PHPUnit_Framework_TestCase {
	...
	/**
	 *
	 * @var \Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable\Attribute \PHPUnit_Framework_MockObject_MockObject
	 */
	...
	protected function setUp() {
		...
		$this->attributeMock = $this->getMock ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable\Attribute', [ ], [ ], '', false );
	...
	public function testPrepareJsonAttributes() {
		...
		$attributeOptions = [ 
				[ 
						'value_index' => 'option_id_1',
						'label' => 'label_1' 
				],
				[ 
						'value_index' => 'option_id_2',
						'label' => 'label_2' 
				] 
		];
		$expected = [ 
		...
		$options = [ 
				$attributeId => [ 
						'option_id_1' => 'option_products_1',
						'option_id_2' => 'option_products_2' 
				] 
		];
		
		$productAttributeMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Entity\Attribute' )->disableOriginalConstructor ()->setMethods ( [ 
				'getLabel',
				'__wakeup',
				'getAttributeCode',
				'getId',
		...
		$attributeMock = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\Product\Type\Configurable\Attribute' )->disableOriginalConstructor ()->setMethods ( [ 
				'getProductAttribute',
				'__wakeup',
				'getLabel',
				'getOptions',
		...
		$attributeMock->expects ( $this->once () )->method ( 'getLabel' )->willReturn ( $expected ['attributes'] [$attributeId] ['label'] );
		...
		$configurableProduct->expects ( $this->once () )->method ( 'getConfigurableAttributes' )->with ( $this->product )->willReturn ( [ 
				$attributeMock 
		] );
