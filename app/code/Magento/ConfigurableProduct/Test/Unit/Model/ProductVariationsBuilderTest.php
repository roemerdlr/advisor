...
class ProductVariationsBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->variationMatrix = $this->getMock ( '\Magento\ConfigurableProduct\Model\Product\Type\VariationMatrix', [ ], [ ], '', false );
		
		$this->model = new \Magento\ConfigurableProduct\Model\ProductVariationsBuilder ( $this->productFactory, $this->customAttributeFactory, $this->variationMatrix );
	}
	public function testCreate() {
		$output = $this->getMock ( '\Magento\Catalog\Model\Product', [ 
				'setPrice',
				'__wakeup',
				'setData',
				'getCustomAttributes',
		...
		$output->expects ( $this->at ( 2 ) )->method ( 'setData' )->with ( 'custom_attributes', [ 
		...
		$this->assertEquals ( [ 
				$output 
		], $this->model->create ( $this->product, $attributes ) );
	}
}
