...
class ConfigurableTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->configurable = $this->objectManagerHelper->getObject ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable', [ 
				'resource' => $this->resource,
				'catalogProductRelation' => $this->relation 
		] );
	}
	public function testSaveProducts() {
		$mainProduct = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
				'getIsDuplicate',
				'__sleep',
				'__wakeup',
				'getTypeInstance',
		...
		$typeInstance = $this->getMockBuilder ( 'Magento\ConfigurableProduct\Model\Product\Type\Configurable' )->disableOriginalConstructor ()->getMock ();
		$typeInstance->expects ( $this->once () )->method ( 'getUsedProductIds' )->will ( $this->returnValue ( [ 
				1 
		] ) );
		
		$mainProduct->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $typeInstance ) );
		
		$this->configurable->saveProducts ( $mainProduct, [ 
				1,
				2,
				3 
		] );
	...
	public function testGetConfigurableOptions() {
		$configurable = $this->getMock ( 'Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable', [ 
				'getTable',
				'getConnection' 
		], [ 
				$this->resource,
		...
		$product->expects ( $this->once () )->method ( 'getId' )->willReturn ( 'getId value' );
		
		$configurable->expects ( $this->exactly ( 6 ) )->method ( 'getTable' )->will ( $this->returnValueMap ( [ 
				[ 
		...
		$superAttribute = $this->getMock ( '\Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\Configurable\Attribute', [ 
				'getBackendTable',
				'getAttributeId' 
		], [ ], '', false );
		...
		$select->expects ( $this->exactly ( 4 ) )->method ( 'joinInner' )->will ( $this->returnSelf () )->withConsecutive ( [ 
				[ 
		...
		$select->expects ( $this->exactly ( 2 ) )->method ( 'joinLeft' )->will ( $this->returnSelf () )->withConsecutive ( [ 
				[ 
		...
		$readerAdapter = $this->getMockBuilder ( '\Magento\Framework\DB\Adapter\AdapterInterface' )->setMethods ( [ 
				'select',
				'fetchAll' 
