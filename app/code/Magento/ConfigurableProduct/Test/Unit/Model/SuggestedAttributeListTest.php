...
class SuggestedAttributeListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->configurableAttributeHandler = $this->getMock ( 'Magento\ConfigurableProduct\Model\ConfigurableAttributeHandler', [ ], [ ], '', false );
		$this->resourceHelperMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Helper', [ ], [ ], '', false );
		$this->collectionMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Attribute\Collection', [ ], [ ], '', false );
		$this->resourceHelperMock->expects ( $this->once () )->method ( 'addLikeEscape' )->with ( $this->labelPart, [ 
				'position' => 'any' 
		] )->will ( $this->returnValue ( $this->labelPart ) );
		$this->configurableAttributeHandler->expects ( $this->once () )->method ( 'getApplicableAttributes' )->will ( $this->returnValue ( $this->collectionMock ) );
		$valueMap = [ 
				[ 
						'frontend_label',
						[ 
		...
		$this->collectionMock->expects ( $this->any () )->method ( 'addFieldToFilter' )->will ( $this->returnValueMap ( $valueMap ) );
		$methods = [ 
				'getId',
				'getFrontendLabel',
				'getAttributeCode',
		...
		$this->attributeMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Eav\Attribute', $methods, [ ], '', false );
		$this->collectionMock->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				'id' => $this->attributeMock 
		] ) );
		$this->suggestedListModel = new \Magento\ConfigurableProduct\Model\SuggestedAttributeList ( $this->configurableAttributeHandler, $this->resourceHelperMock );
	}
	public function testGetSuggestedAttributesIfTheyApplicable() {
		...
		$this->configurableAttributeHandler->expects ( $this->once () )->method ( 'isAttributeApplicable' )->with ( $this->attributeMock )->willReturn ( true );
