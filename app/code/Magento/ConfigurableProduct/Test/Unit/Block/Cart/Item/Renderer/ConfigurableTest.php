...
class ConfigurableTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_imageHelper = $this->getMock ( 'Magento\Catalog\Helper\Image', [ 
				'init',
				'resize',
				'__toString' 
		], [ ], '', false );
		$this->_scopeConfig = $this->getMock ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		$this->productConfigMock = $this->getMock ( 'Magento\Catalog\Helper\Product\Configuration', [ ], [ ], '', false );
		$this->_renderer = $objectManagerHelper->getObject ( 'Magento\ConfigurableProduct\Block\Cart\Item\Renderer\Configurable', [ 
	...
	public function testGetProductForThumbnail() {
		...
		$this->assertSame ( $products ['childProduct'], $productForThumbnail, 'Child product was expected to be returned.' );
	}
	...
	public function testGetProductForThumbnailChildThumbnailNotAvailable() {
		...
		$productForThumbnail = $this->_renderer->getProductForThumbnail ();
		$this->assertSame ( $products ['parentProduct'], $productForThumbnail, 'Parent product was expected to be returned.' );
	}
	...
	public function testGetProductForThumbnailConfigUseParent() {
		...
		$productForThumbnail = $this->_renderer->getProductForThumbnail ();
		$this->assertSame ( $products ['parentProduct'], $productForThumbnail, 'Parent product was expected to be returned ' . 'if "checkout/cart/configurable_product_image option" is set to "parent" in system config.' );
	}
	...
	protected function _initProducts($childHasThumbnail = true, $useParentThumbnail = false) {
		...
		$this->_scopeConfig->expects ( $this->any () )->method ( 'getValue' )->with ( Renderer::CONFIG_THUMBNAIL_SOURCE )->will ( $this->returnValue ( $thumbnailToBeUsed ) );
		...
		$childProduct = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getThumbnail',
				'__wakeup' 
		], [ ], '', false );
		...
		$item->expects ( $this->any () )->method ( 'getOptionByCode' )->with ( 'simple_product' )->will ( $this->returnValue ( $itemOption ) );
