...
class AttributeSelectorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->attributeSelector = $helper->getObject ( 'Magento\ConfigurableProduct\Block\Product\Configurable\AttributeSelector', [ 
				'urlBuilder' => $this->urlBuilder 
		] );
	}
	public function testGetAttributeSetCreationUrl() {
		$this->urlBuilder->expects ( $this->once () )->method ( 'getUrl' )->with ( '*/product_set/save' )->will ( $this->returnValue ( 'some_url' ) );
		$this->assertEquals ( 'some_url', $this->attributeSelector->getAttributeSetCreationUrl () );
	}
	public function testGetSuggestWidgetOptions() {
		$source = 'source_url';
		$this->urlBuilder->expects ( $this->once () )->method ( 'getUrl' )->with ( '*/product_attribute/suggestConfigurableAttributes' )->will ( $this->returnValue ( $source ) );
		$expected = [ 
				'source' => $source,
				'minLength' => 0,
