...
class AroundProductRepositorySave {
	 ...
	 */
	public function __construct(\Magento\ConfigurableProduct\Api\OptionRepositoryInterface $optionRepository, \Magento\Catalog\Model\ProductFactory $productFactory, \Magento\ConfigurableProduct\Model\ResourceModel\Product\Type\ConfigurableFactory $typeConfigurableFactory) {
	 ...
	 * @throws \Magento\Framework\Exception\CouldNotSaveException @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	...
	protected function validateProductLinks(array $attributeCodes, array $linkIds) {
		...
		if (empty ( $attributeCodes ) && ! empty ( $linkIds )) {
			throw new InputException ( __ ( 'The configurable product does not have any variation attribute.' ) );
		}
		...
		foreach ( $linkIds as $productId ) {
			...
			foreach ( $attributeCodes as $attributeCode ) {
				if (! $variation->getData ( $attributeCode )) {
					throw new InputException ( __ ( 'Product with id "%1" does not contain required attribute "%2".', $productId, $attributeCode ) );
				}
				$valueKey = $valueKey . $attributeCode . ':' . $variation->getData ( $attributeCode ) . ';';
			}
			if (isset ( $valueMap [$valueKey] )) {
				throw new InputException ( __ ( 'Products "%1" and %2 have the same set of attribute values.', $productId, $valueMap [$valueKey] ) );
			}
