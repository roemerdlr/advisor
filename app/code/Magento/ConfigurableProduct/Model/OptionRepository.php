...
class OptionRepository implements \Magento\ConfigurableProduct\Api\OptionRepositoryInterface {
	...
	public function save($sku, OptionInterface $option) {
		...
		if ($option->getId ()) {
			...
			$configurableAttribute->setValues ( $option->getValues () !== null ? $option->getValues () : $configurableAttribute->getOptions () );
			...
			try {
				...
				$product->setConfigurableAttributesData ( [ 
						$configurableAttributesData 
				] );
	 ...
	 * @return void
	 * @throws InputException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
