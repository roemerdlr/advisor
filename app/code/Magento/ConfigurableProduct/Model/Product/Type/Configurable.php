...
class Configurable extends \Magento\Catalog\Model\Product\Type\AbstractType {
	...
	public function setUsedProductAttributeIds($ids, $product) {
		...
		foreach ( $ids as $attributeId ) {
			...
			$configurableAttributes [] = $this->_configurableAttributeFactory->create ()->setProductAttribute ( $this->getAttributeById ( $attributeId, $product ) );
		}
	...
	public function getConfigurableAttributes($product) {
		\Magento\Framework\Profiler::start ( 'CONFIGURABLE:' . __METHOD__, [ 
				'group' => 'CONFIGURABLE',
				'method' => __METHOD__ 
		] );
			...
			if (is_array ( $requiredAttributeIds )) {
				foreach ( $requiredAttributeIds as $attributeId ) {
					...
					if (! is_null ( $attribute )) {
						$collection->addAttributeToFilter ( $attribute->getAttributeCode (), [ 
								'notnull' => 1 
						] );
					}
				}
	...
	public function save($product) {
		...
		if ($data) {
			...
			$configurableAttributesCollection->addFieldToFilter ( 'attribute_id', [ 
