...
class Form extends \Magento\Backend\Block\Widget\Form\Generic {
	...
	protected function _prepareForm() {
		...
		$fieldset->addField ( 'current-affected-attribute-set', 'radio', [ 
				'after_element_html' => __ ( 'Add configurable attributes to the current Attribute Set ("%1")', sprintf ( '<span data-role="name-container">%s</span>', $this->getCurrentAttributeSetName () ) ),
				'name' => 'affected-attribute-set',
				'class' => 'admin__control-radio',
				'checked' => true,
				'value' => 'current' 
		] );
		$fieldset->addField ( 'new-affected-attribute-set', 'radio', [ 
				'after_element_html' => __ ( 'Add configurable attributes to the new Attribute Set based on current' ),
				'name' => 'affected-attribute-set',
				'class' => 'admin__control-radio',
				'value' => 'new' 
		] );
		$fieldset->addField ( 'new-attribute-set-name', 'text', [ 
		...
		$fieldset->addField ( 'existing-affected-attribute-set', 'radio', [ 
		...
		$fieldset->addField ( 'choose-affected-attribute-set', 'select', [ 
	...
	protected function getCurrentAttributeSetName() {
		return $this->attributeSetRepository->get ( $this->_coreRegistry->registry ( 'current_product' )->getAttributeSetId () )->getAttributeSetName ();
	}
}
