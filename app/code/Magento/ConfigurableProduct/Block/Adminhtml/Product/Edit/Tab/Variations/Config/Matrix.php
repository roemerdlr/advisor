...
class Matrix extends \Magento\Backend\Block\Template {
	 ...
	 * @param array $data        	
	 */
	public function __construct(\Magento\Backend\Block\Template\Context $context, \Magento\ConfigurableProduct\Model\Product\Type\Configurable $configurableType, \Magento\Framework\Registry $coreRegistry, \Magento\CatalogInventory\Api\StockRegistryInterface $stockRegistry, \Magento\ConfigurableProduct\Model\Product\Type\VariationMatrix $variationMatrix, ProductRepositoryInterface $productRepository, \Magento\Catalog\Helper\Image $image, \Magento\Framework\Locale\CurrencyInterface $localeCurrency, array $data = []) {
	...
	public function getEditProductUrl($id) {
		return $this->getUrl ( 'catalog/*/edit', [ 
				'id' => $id 
		] );
	}
			...
			if (isset ( $productData ['configurable_attributes_data'] )) {
				...
				foreach ( $attributes as $key => $attribute ) {
					if (isset ( $configurableData [$key] )) {
						...
						$attributes [$key] ['values'] = array_merge ( isset ( $attribute ['values'] ) ? $attribute ['values'] : [ ], isset ( $configurableData [$key] ['values'] ) ? array_filter ( $configurableData [$key] ['values'] ) : [ ] );
					}
				}
			}
	...
	/**
	 * Retrieve actual list of associated products (i.e.
	 * if product contains variations matrix form data
	 ...
	 * 
	 * @return void TODO: move to class
	 */
