...
class ConfigurationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->helper = $this->objectManagerHelper->getObject ( 'Magento\Downloadable\Helper\Catalog\Product\Configuration', [ 
	...
	public function testGetOptions() {
		...
		$productType = $this->getMockBuilder ( '\Magento\Downloadable\Model\Product\Type' )->disableOriginalConstructor ()->setMethods ( [ 
				'getLinks' 
		] )->getMock ();
		$productLink = $this->getMockBuilder ( '\Magento\Downloadable\Model\Link' )->disableOriginalConstructor ()->setMethods ( [ 
				'getTitle' 
		] )->getMock ();
		...
		$productType->expects ( $this->once () )->method ( 'getLinks' )->with ( $product )->willReturn ( [ 
				1 => $productLink 
		] );
