...
class DownloadTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_handleMock = $this->getMock ( 'Magento\Framework\Filesystem\File\ReadInterface', [ ], [ ], '', false );
		$this->_workingDirectoryMock = $this->getMock ( 'Magento\Framework\Filesystem\Directory\ReadInterface', [ ], [ ], '', false );
		...
		$this->_helper = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Downloadable\Helper\Download', [ 
	...
	public function testGetContentTypeThroughHelper($functionExistsResult, $mimeContentTypeResult) {
		...
		$this->_downloadableFileMock->expects ( $this->once () )->method ( 'getFileType' )->will ( $this->returnValue ( self::MIME_TYPE ) );
	...
	public function testGetFileNameUrlWithContentDisposition() {
		...
		$this->_setupUrlMocks ( self::FILE_SIZE, self::URL, [ 
	...
	protected function _setupFileMocks($doesExist = true, $size = self::FILE_SIZE, $path = self::FILE_PATH) {
		$this->_handleMock->expects ( $this->any () )->method ( 'stat' )->will ( $this->returnValue ( [ 
				'size' => $size 
		] ) );
	...
	protected function _setupUrlMocks($size = self::FILE_SIZE, $url = self::URL, $additionalStatData = []) {
		$this->_handleMock->expects ( $this->any () )->method ( 'stat' )->will ( $this->returnValue ( array_merge ( [ 
				'size' => $size,
				'type' => self::MIME_TYPE 
		], $additionalStatData ) ) );
