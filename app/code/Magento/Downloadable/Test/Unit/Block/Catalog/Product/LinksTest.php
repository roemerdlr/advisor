...
class LinksTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$contextMock->expects ( $this->once () )->method ( 'getLayout' )->will ( $this->returnValue ( $this->layout ) );
		...
		$this->linksBlock = $objectManager->getObject ( 'Magento\Downloadable\Block\Catalog\Product\Links', [ 
				'context' => $contextMock,
	...
	public function testGetJsonConfig() {
		...
		$linkAmountMock->expects ( $this->once () )->method ( 'getBaseAmount' )->will ( $this->returnValue ( $linkPrice ) );
		
		$typeInstanceMock = $this->getMock ( 'Magento\Catalog\Model\Product\Type\Simple', [ 
				'getLinks' 
		], [ ], '', false );
		$typeInstanceMock->expects ( $this->once () )->method ( 'getLinks' )->will ( $this->returnValue ( [ 
				$this->getLinkMock ( $linkPrice, $linkId ) 
		] ) );
	...
	protected function getLinkMock($linkPrice, $linkId) {
		...
		$linkMock->expects ( $this->any () )->method ( 'getPrice' )->will ( $this->returnValue ( $linkPrice ) );
