...
class NameTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$contextMock = $this->getMockBuilder ( 'Magento\Backend\Block\Template\Context' )->disableOriginalConstructor ()->getMock ();
		$this->purchasedFactory = $this->getMockBuilder ( 'Magento\Downloadable\Model\Link\PurchasedFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->itemsFactory = $this->getMockBuilder ( 'Magento\Downloadable\Model\ResourceModel\Link\Purchased\Item\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->block = $objectManager->getObject ( 'Magento\Downloadable\Block\Adminhtml\Sales\Items\Column\Downloadable\Name', [ 
				'context' => $contextMock,
	...
	public function testGetLinks() {
		...
		$linkPurchased = $this->getMockBuilder ( 'Magento\Downloadable\Model\Link\Purchased' )->disableOriginalConstructor ()->setMethods ( [ 
				'load' 
		] )->getMock ();
		$itemCollection = $this->getMockBuilder ( 'Magento\Downloadable\Model\ResourceModel\Link\Purchased\Item\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'addFieldToFilter' 
		] )->getMock ();
