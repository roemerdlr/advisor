...
class SamplesTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->urlBuilder = $this->getMock ( 'Magento\Backend\Model\Url', [ 
				'getUrl' 
		], [ ], '', false );
		...
		$this->downloadableProductModel = $this->getMock ( '\Magento\Downloadable\Model\Product\Type', [ 
				'__wakeup',
				'getSamples' 
		], [ ], '', false );
		$this->downloadableSampleModel = $this->getMock ( '\Magento\Downloadable\Model\Sample', [ 
				'__wakeup',
				'getId',
				'getTitle',
				'getSampleFile',
		...
		$this->block = $objectManagerHelper->getObject ( 'Magento\Downloadable\Block\Adminhtml\Catalog\Product\Edit\Tab\Downloadable\Samples', [ 
	...
	public function testGetSampleData() {
		...
		$this->downloadableProductModel->expects ( $this->any () )->method ( 'getSamples' )->will ( $this->returnValue ( [ 
				$this->downloadableSampleModel 
		] ) );
