...
class LinkSampleTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->helperData = $this->getMock ( 'Magento\Downloadable\Helper\Data', [ 
				'getIsShareable' 
		], [ ], '', false );
		$this->downloadHelper = $this->getMock ( 'Magento\Downloadable\Helper\Download', [ 
				'setResource',
				'getFilename',
				'getContentType',
				'getFileSize',
		...
		$this->urlInterface = $this->getMock ( 'Magento\Framework\UrlInterface', [ ], [ ], '', false );
		$this->objectManager = $this->getMock ( '\Magento\Framework\ObjectManager\ObjectManager', [ 
				'create',
				'get' 
		], [ ], '', false );
		$this->linkSample = $this->objectManagerHelper->getObject ( 'Magento\Downloadable\Controller\Download\LinkSample', [ 
	...
	public function testExecuteLinkTypeUrl() {
		$linkMock = $this->getMockBuilder ( 'Magento\Downloadable\Model\Link' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'load',
				'getSampleType',
				'getSampleUrl' 
		...
		$this->objectManager->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Downloadable\Model\Link' )->willReturn ( $linkMock );
		...
		$linkMock->expects ( $this->once () )->method ( 'getSampleType' )->willReturn ( \Magento\Downloadable\Helper\Download::LINK_TYPE_URL );
		$linkMock->expects ( $this->once () )->method ( 'getSampleUrl' )->willReturn ( 'sample_url' );
		$this->objectManager->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( 'Magento\Downloadable\Helper\Download' )->willReturn ( $this->downloadHelper );
	...
	public function testExecuteLinkTypeFile() {
		...
		$fileMock = $this->getMockBuilder ( 'Magento\Downloadable\Helper\File' )->disableOriginalConstructor ()->setMethods ( [ 
				'getFilePath',
				'load',
				'getSampleType',
				'getSampleUrl' 
		...
		$this->request->expects ( $this->once () )->method ( 'getParam' )->with ( 'link_id', 0 )->willReturn ( 'some_link_id' );
		$this->objectManager->expects ( $this->at ( 0 ) )->method ( 'create' )->with ( 'Magento\Downloadable\Model\Link' )->willReturn ( $linkMock );
		$linkMock->expects ( $this->once () )->method ( 'load' )->with ( 'some_link_id' )->willReturnSelf ();
		$linkMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( 'some_link_id' );
		$linkMock->expects ( $this->any () )->method ( 'getSampleType' )->willReturn ( \Magento\Downloadable\Helper\Download::LINK_TYPE_FILE );
		$this->objectManager->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( 'Magento\Downloadable\Helper\File' )->willReturn ( $fileMock );
		$this->objectManager->expects ( $this->at ( 2 ) )->method ( 'get' )->with ( 'Magento\Downloadable\Model\Link' )->willReturn ( $linkMock );
