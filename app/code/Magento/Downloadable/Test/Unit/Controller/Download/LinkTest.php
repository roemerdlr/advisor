...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->session = $this->getMock ( 'Magento\Customer\Model\Session', [ 
				'getCustomerId',
				'authenticate',
				'setBeforeAuthUrl' 
		], [ ], '', false );
		...
		$this->linkPurchasedItem = $this->getMock ( 'Magento\Downloadable\Model\Link\Purchased\Item', [ 
				'load',
				'getId',
				'getProductId',
				'getPurchasedId',
		...
		$this->linkPurchased = $this->getMock ( 'Magento\Downloadable\Model\Link\Purchased', [ 
				'load',
				'getCustomerId' 
		], [ ], '', false );
		...
		$this->link = $this->objectManagerHelper->getObject ( 'Magento\Downloadable\Controller\Download\Link', [ 
	...
	public function testAbsentLinkId() {
		$this->objectManager->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Customer\Model\Session' )->willReturn ( $this->session );
		$this->request->expects ( $this->once () )->method ( 'getParam' )->with ( 'id', 0 )->willReturn ( 'some_id' );
		$this->objectManager->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Downloadable\Model\Link\Purchased\Item' )->willReturn ( $this->linkPurchasedItem );
	...
	public function testGetLinkForGuestCustomer() {
		...
		$this->objectManager->expects ( $this->at ( 3 ) )->method ( 'create' )->with ( 'Magento\Catalog\Model\Product' )->willReturn ( $this->product );
		...
		$this->messageManager->expects ( $this->once () )->method ( 'addNotice' )->with ( 'Please sign in to download your product or purchase <a href="product_url">product_name</a>.' );
		$this->session->expects ( $this->once () )->method ( 'authenticate' )->willReturn ( true );
		$this->objectManager->expects ( $this->at ( 4 ) )->method ( 'create' )->with ( 'Magento\Framework\UrlInterface' )->willReturn ( $this->urlInterface );
		$this->urlInterface->expects ( $this->once () )->method ( 'getUrl' )->with ( 'downloadable/customer/products/', [ 
				'_secure' => true 
