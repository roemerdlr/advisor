...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->fileHelper = $this->getMock ( '\Magento\Downloadable\Helper\File', [ 
				'getFilePath' 
		], [ ], '', false );
		...
		$this->linkModel = $this->getMock ( '\Magento\Downloadable\Controller\Adminhtml\Downloadable\Product\Edit\Link', [ 
				'load',
				'getId',
				'getLinkType',
				'getLinkUrl',
		...
		$this->link = $this->objectManagerHelper->getObject ( 'Magento\Downloadable\Controller\Adminhtml\Downloadable\Product\Edit\Link', [ 
	...
	public function testExecuteFile($fileType) {
		...
		$this->objectManager->expects ( $this->at ( 2 ) )->method ( 'get' )->with ( 'Magento\Downloadable\Model\Link' )->will ( $this->returnValue ( $this->linkModel ) );
		...
		$this->fileHelper->expects ( $this->once () )->method ( 'getFilePath' )->will ( $this->returnValue ( 'filepath/' . $fileType . '.jpg' ) );
		...
		$this->linkModel->expects ( $this->any () )->method ( 'get' . $fileType . 'Type' )->will ( $this->returnValue ( 'file' ) );
