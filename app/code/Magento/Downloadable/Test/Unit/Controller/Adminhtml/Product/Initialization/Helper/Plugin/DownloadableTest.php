...
class DownloadableTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->subjectMock = $this->getMock ( 'Magento\Catalog\Controller\Adminhtml\Product\Initialization\Helper', [ ], [ ], '', false );
		$this->downloadablePlugin = new \Magento\Downloadable\Controller\Adminhtml\Product\Initialization\Helper\Plugin\Downloadable ( $this->requestMock );
	}
	public function testAfterInitializeIfDownloadableExist() {
		$this->requestMock->expects ( $this->once () )->method ( 'getPost' )->with ( 'downloadable' )->will ( $this->returnValue ( 'downloadable' ) );
	...
	public function testAfterInitializeIfDownloadableNotExist() {
		$this->requestMock->expects ( $this->once () )->method ( 'getPost' )->with ( 'downloadable' )->will ( $this->returnValue ( false ) );
