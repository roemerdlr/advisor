...
class CreditmemoTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$arguments = [ 
				...
				'orderItemCollectionFactory' => $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Item\CollectionFactory', [ ], [ ], '', false ),
				'serviceOrderFactory' => $this->getMock ( 'Magento\Sales\Model\Service\OrderFactory', [ ], [ ], '', false ),
				'currencyFactory' => $this->getMock ( 'Magento\Directory\Model\CurrencyFactory', [ ], [ ], '', false ),
				'orderHistoryFactory' => $this->getMock ( 'Magento\Sales\Model\Order\Status\HistoryFactory', [ ], [ ], '', false ),
				'orderTaxCollectionFactory' => $this->getMock ( 'Magento\Tax\Model\ResourceModel\Sales\Order\Tax\CollectionFactory', [ ], [ ], '', false ) 
		];
		$orderConstructorArgs = $objectManager->getConstructArguments ( 'Magento\Sales\Model\Order', $arguments );
		$this->_order = $this->getMock ( 'Magento\Sales\Model\Order', [ 
				'formatPriceTxt' 
		], $orderConstructorArgs );
		$this->_order->expects ( $this->any () )->method ( 'formatPriceTxt' )->will ( $this->returnCallback ( [ 
				$this,
				'formatPrice' 
		] ) );
		
		$this->_pdf = $this->getMock ( 'Magento\Sales\Model\Order\Pdf\AbstractPdf', [ 
				'drawLineBlocks',
				'getPdf' 
		], [ ], '', false, false );
		
		$filterManager = $this->getMock ( 'Magento\Framework\Filter\FilterManager', [ 
				'stripTags' 
		], [ ], '', false );
		$filterManager->expects ( $this->any () )->method ( 'stripTags' )->will ( $this->returnArgument ( 0 ) );
		
		$modelConstructorArgs = $objectManager->getConstructArguments ( 'Magento\Downloadable\Model\Sales\Order\Pdf\Items\Creditmemo', [ 
				'string' => new \Magento\Framework\Stdlib\StringUtils (),
				'filterManager' => $filterManager 
		] );
		
		$this->_model = $this->getMock ( 'Magento\Downloadable\Model\Sales\Order\Pdf\Items\Creditmemo', [ 
				'getLinks',
				'getLinksTitle' 
	...
	public function testDraw() {
		...
		$this->_model->setItem ( new \Magento\Framework\DataObject ( [ 
		...
		$this->_model->expects ( $this->any () )->method ( 'getLinks' )->will ( $this->returnValue ( new \Magento\Framework\DataObject ( [ 
				'purchased_items' => [ 
						new \Magento\Framework\DataObject ( [ 
								'link_title' => 'Magento User Guide' 
						] ) 
				] 
		] ) ) );
