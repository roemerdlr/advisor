...
class ContentValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function getLinkMock(array $linkData) {
		...
		if (isset ( $linkData ['sample_type'] )) {
			$linkMock->expects ( $this->any () )->method ( 'getSampleType' )->will ( $this->returnValue ( $linkData ['sample_type'] ) );
		}
		...
		if (isset ( $linkData ['sample_file_content'] )) {
			$linkMock->expects ( $this->any () )->method ( 'getSampleFileContent' )->willReturn ( $linkData ['sample_file_content'] );
		}
