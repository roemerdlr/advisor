...
class CartItemProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->optionFactoryMock = $this->getMock ( '\Magento\Quote\Model\Quote\ProductOptionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->objectHelperMock = $this->getMock ( '\Magento\Framework\Api\DataObjectHelper', [ ], [ ], '', false );
		$this->extensionFactoryMock = $this->getMock ( '\Magento\Quote\Api\Data\ProductOptionExtensionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->downloadableOptionFactoryMock = $this->getMock ( '\Magento\Downloadable\Model\DownloadableOptionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->model = new CartItemProcessor ( $this->objectFactoryMock, $this->objectHelperMock, $this->downloadableOptionFactoryMock, $this->optionFactoryMock, $this->extensionFactoryMock );
	}
	...
	public function testConvertToBuyRequest() {
		...
		$cartItemMock = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ 
				'getProductOption',
				'setProductOption',
				'getOptionByCode',
				'getQty' 
		...
		$extAttributesMock->expects ( $this->any () )->method ( 'getDownloadableOption' )->willReturn ( $downloadableOptionMock );
		...
		$this->objectFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( $buyRequestData )->willReturn ( $buyRequestMock );
	...
	public function testProcessProductOptions() {
		...
		$cartItemMock->expects ( $this->once () )->method ( 'getOptionByCode' )->with ( 'downloadable_link_ids' )->willReturn ( $customOption );
		
		$cartItemMock->expects ( $this->any () )->method ( 'getProductOption' )->willReturn ( null );
		
		$downloadableOptionMock = $this->getMock ( '\Magento\Downloadable\Api\Data\DownloadableOptionInterface' );
		$this->downloadableOptionFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $downloadableOptionMock );
		...
		$extAttributeMock = $this->getMock ( '\Magento\Quote\Api\Data\ProductOptionExtension', [ 
				'setDownloadableOption' 
		], [ ], '', false );
		
		$this->objectHelperMock->expects ( $this->once () )->method ( 'populateWithArray' )->with ( $downloadableOptionMock, [ 
		...
		$extAttributeMock->expects ( $this->once () )->method ( 'setDownloadableOption' )->with ( $downloadableOptionMock );
		$productOptionMock->expects ( $this->once () )->method ( 'setExtensionAttributes' )->with ( $extAttributeMock );
	...
	public function testProcessProductOptionsWhenItemDoesNotHaveDownloadableLinks() {
		...
		$cartItemMock->expects ( $this->once () )->method ( 'getOptionByCode' )->with ( 'downloadable_link_ids' );
		
		$extAttributeMock = $this->getMock ( '\Magento\Quote\Api\Data\ProductOptionExtension', [ 
				'setDownloadableOption' 
		], [ ], '', false );
		$productOptionMock = $this->getMock ( '\Magento\Quote\Api\Data\ProductOptionInterface' );
		$productOptionMock->expects ( $this->any () )->method ( 'getExtensionAttributes' )->willReturn ( $extAttributeMock );
		$cartItemMock->expects ( $this->any () )->method ( 'getProductOption' )->willReturn ( $productOptionMock );
