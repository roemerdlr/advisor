...
class AroundProductRepositorySaveTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productExtensionMock = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\ProductExtension' )->setMethods ( [ 
				'getDownloadableProductLinks',
				'getDownloadableProductSamples' 
		] )->getMock ();
		$this->existingProductExtensionMock = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\ProductExtension' )->setMethods ( [ 
				'getDownloadableProductLinks',
				'getDownloadableProductSamples' 
		] )->getMock ();
	}
	public function testAroundSaveWhenProductIsSimple() {
		...
		$this->assertEquals ( $this->savedProductMock, $this->model->aroundSave ( $this->productRepositoryMock, $this->closureMock, $this->productMock ) );
	}
	public function testAroundSaveWhenProductHasNoExtensionAttributes() {
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->willReturn ( \Magento\Downloadable\Model\Product\Type::TYPE_DOWNLOADABLE );
		$this->productMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( null );
	...
	public function testAroundSave() {
		...
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductLinks' )->willReturn ( [ 
				$newLinkMock,
				$updateLinkMock 
		] );
		...
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductSamples' )->willReturn ( [ 
				$updateSampleMock,
				$newSampleMock 
		] );
		...
		$this->savedProductMock->expects ( $this->exactly ( 2 ) )->method ( 'getExtensionAttributes' )->willReturn ( $this->existingProductExtensionMock );
		$this->existingProductExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductLinks' )->willReturn ( [ 
				$existingLinkMock,
				$toBeDeletedLinkMock 
		] );
		$this->existingProductExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductSamples' )->willReturn ( [ 
				$existingSampleMock,
				$toBeDeletedSampleMock 
		] );
		
		$this->linkRepositoryMock->expects ( $this->at ( 0 ) )->method ( 'save' )->with ( $productSku, $updateLinkMock );
		$this->linkRepositoryMock->expects ( $this->at ( 1 ) )->method ( 'save' )->with ( $productSku, $newLinkMock );
		$this->linkRepositoryMock->expects ( $this->at ( 2 ) )->method ( 'delete' )->with ( $toBeDeletedLinkId );
		
		$this->sampleRepositoryMock->expects ( $this->at ( 0 ) )->method ( 'save' )->with ( $productSku, $updateSampleMock );
		$this->sampleRepositoryMock->expects ( $this->at ( 1 ) )->method ( 'save' )->with ( $productSku, $newSampleMock );
		$this->sampleRepositoryMock->expects ( $this->at ( 2 ) )->method ( 'delete' )->with ( $toBeDeletedSampleId );
		...
		$this->assertEquals ( $newProductMock, $this->model->aroundSave ( $this->productRepositoryMock, $this->closureMock, $this->productMock ) );
	}
	...
	public function testAroundSaveWithOnlyLinks() {
		...
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductSamples' )->willReturn ( null );
		...
		$this->savedProductMock->expects ( $this->any () )->method ( 'getSku' )->willReturn ( $productSku );
		$this->savedProductMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( $this->existingProductExtensionMock );
		$this->existingProductExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductLinks' )->willReturn ( [ 
				$existingLinkMock,
				$toBeDeletedLinkMock 
		] );
		$this->existingProductExtensionMock->expects ( $this->never () )->method ( 'getDownloadableProductSamples' );
	...
	public function testAroundSaveWithOnlySamples() {
		...
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->willReturn ( \Magento\Downloadable\Model\Product\Type::TYPE_DOWNLOADABLE );
		$this->productMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( $this->productExtensionMock );
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductLinks' )->willReturn ( null );
		...
		$this->savedProductMock->expects ( $this->any () )->method ( 'getSku' )->willReturn ( $productSku );
		$this->savedProductMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( $this->existingProductExtensionMock );
		$this->existingProductExtensionMock->expects ( $this->never () )->method ( 'getDownloadableProductLinks' );
		$this->existingProductExtensionMock->expects ( $this->once () )->method ( 'getDownloadableProductSamples' )->willReturn ( [ 
				$existingSampleMock,
				$toBeDeletedSampleMock 
		] );
