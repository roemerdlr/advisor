...
class DownloadableTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_productTypeMock = $this->getMock ( '\Magento\Downloadable\Model\Product\Type', [ ], [ ], '', false );
	...
	public function testBuild() {
		...
		$this->_productMock->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( \Magento\Downloadable\Model\Product\Type::TYPE_DOWNLOADABLE ) );
		
		$this->_productMock->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $this->_productTypeMock ) );
		
		$this->_productTypeMock->expects ( $this->once () )->method ( 'getLinks' )->with ( $this->_productMock )->will ( $this->returnValue ( [ 
				$this->_linkMock 
		] ) );
		
		$this->_productTypeMock->expects ( $this->once () )->method ( 'getSamples' )->with ( $this->_productMock )->will ( $this->returnValue ( [ 
				$this->_sampleMock 
		] ) );
