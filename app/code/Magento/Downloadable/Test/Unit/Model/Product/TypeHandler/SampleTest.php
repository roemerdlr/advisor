...
class SampleTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->sampleFactory = $this->getMockBuilder ( '\Magento\Downloadable\Model\SampleFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->sampleResource = $this->getMockBuilder ( '\Magento\Downloadable\Model\ResourceModel\Sample' )->disableOriginalConstructor ()->setMethods ( [ 
				'deleteItems' 
		] )->getMock ();
		$sampleResourceFactory = $this->getMockBuilder ( '\Magento\Downloadable\Model\ResourceModel\SampleFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$sampleResourceFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->sampleResource ) );
		$this->target = $objectManagerHelper->getObject ( 'Magento\Downloadable\Model\Product\TypeHandler\Sample', [ 
	...
	public function saveDataProvider() {
		return [ 
				[ 
						'product' => $this->createProductMock ( 100500, 1, 10, [ 
								10 
						] ),
						'data' => [ 
								'sample' => [ 
	...
	public function deleteDataProvider() {
		return [ 
				[ 
						'product' => $this->createProductMock ( 1, 1, 1, [ 
								1 
						] ),
						'data' => [ 
								'sample' => [ 
	...
	private function createSampleModel($product, array $modelData) {
		$sample = $this->getMockBuilder ( '\Magento\Downloadable\Model\Sample' )->disableOriginalConstructor ()->setMethods ( [ 
				'setData',
				'setSampleType',
				'setProductId',
				'setStoreId',
	...
	private function createProductMock($id, $storeId, $storeWebsiteId, array $websiteIds) {
		...
		$product->expects ( $this->any () )->method ( 'getWebsiteIds' )->will ( $this->returnValue ( $websiteIds ) );
		$store = $this->getMockBuilder ( '\Magento\Store\Model\Store' )->disableOriginalConstructor ()->setMethods ( [ 
				'getWebsiteId' 
		] )->getMock ();
		$store->expects ( $this->any () )->method ( 'getWebsiteId' )->will ( $this->returnValue ( $storeWebsiteId ) );
