...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->linkFactory = $this->getMockBuilder ( '\Magento\Downloadable\Model\LinkFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->linkResource = $this->getMockBuilder ( '\Magento\Downloadable\Model\ResourceModel\Link' )->disableOriginalConstructor ()->setMethods ( [ 
				'deleteItems' 
		] )->getMock ();
		$this->target = $objectManagerHelper->getObject ( 'Magento\Downloadable\Model\Product\TypeHandler\Link', [ 
	...
	private function createLinkkModel($product, array $modelData, $isUnlimited) {
		$link = $this->getMockBuilder ( '\Magento\Downloadable\Model\Link' )->disableOriginalConstructor ()->setMethods ( [ 
				'setData',
				'setLinkType',
				'setProductId',
				'setStoreId',
	...
	private function createProductMock($id, $storeId, $storeWebsiteId, array $websiteIds) {
		...
		$product->expects ( $this->any () )->method ( 'getLinksPurchasedSeparately' )->will ( $this->returnValue ( true ) );
		return $product;
	}
}
