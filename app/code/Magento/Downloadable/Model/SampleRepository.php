...
class SampleRepository implements \Magento\Downloadable\Api\SampleRepositoryInterface {
	 ...
	 */
	public function __construct(ProductRepositoryInterface $productRepository, Type $downloadableType, SampleInterfaceFactory $sampleDataObjectFactory, ContentValidator $contentValidator, ContentUploaderInterface $fileContentUploader, EncoderInterface $jsonEncoder, SampleFactory $sampleFactory) {
	...
	public function save($sku, SampleInterface $sample, $isGlobalScopeContent = true) {
		...
		if ($sampleId) {
			...
			if (! in_array ( $sample->getSampleType (), [ 
					'url',
					'file' 
			] )) {
	 ...
	 * @throws NoSuchEntityException @SuppressWarnings(PHPMD.CyclomaticComplexity)
