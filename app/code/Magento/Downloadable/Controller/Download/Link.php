...
class Link extends \Magento\Downloadable\Controller\Download {
	 ...
	 *
	 * @return void|ResponseInterface @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function execute() {
		...
		$linkPurchasedItem = $this->_objectManager->create ( 'Magento\Downloadable\Model\Link\Purchased\Item' )->load ( $id, 'link_hash' );
		...
		if (! $this->_objectManager->get ( 'Magento\Downloadable\Helper\Data' )->getIsShareable ( $linkPurchasedItem )) {
			...
			if (! $customerId) {
				...
				$product = $this->_objectManager->create ( 'Magento\Catalog\Model\Product' )->load ( $linkPurchasedItem->getProductId () );
				if ($product->getId ()) {
					$notice = __ ( 'Please sign in to download your product or purchase <a href="%1">%2</a>.', $product->getProductUrl (), $product->getName () );
				} else {
					$notice = __ ( 'Please sign in to download your product.' );
				}
				$this->messageManager->addNotice ( $notice );
				$session->authenticate ();
				$session->setBeforeAuthUrl ( $this->_objectManager->create ( 'Magento\Framework\UrlInterface' )->getUrl ( 'downloadable/customer/products/', [ 
						'_secure' => true 
				] ) );
				return;
			}
		...
		if ($status == PurchasedLink::LINK_STATUS_AVAILABLE && ($downloadsLeft || $linkPurchasedItem->getNumberOfDownloadsBought () == 0)) {
			$resource = '';
			$resourceType = '';
			if ($linkPurchasedItem->getLinkType () == DownloadHelper::LINK_TYPE_URL) {
				...
				$resource = $this->_objectManager->get ( 'Magento\Downloadable\Helper\File' )->getFilePath ( $this->_getLink ()->getBasePath (), $linkPurchasedItem->getLinkFile () );
