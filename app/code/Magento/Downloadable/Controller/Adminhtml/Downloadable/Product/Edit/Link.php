...
class Link extends \Magento\Catalog\Controller\Adminhtml\Product\Edit {
	...
	protected function _processDownload($resource, $resourceType) {
		...
		$this->getResponse ()->setHttpResponseCode ( 200 )->setHeader ( 'Pragma', 'public', true )->setHeader ( 'Cache-Control', 'must-revalidate, post-check=0, pre-check=0', true )->setHeader ( 'Content-type', $contentType, true );
		...
		if ($link->getId ()) {
			...
			if ($type == 'link') {
				if ($link->getLinkType () == DownloadHelper::LINK_TYPE_URL) {
					...
					$resource = $this->_objectManager->get ( 'Magento\Downloadable\Helper\File' )->getFilePath ( $this->_getLink ()->getBasePath (), $link->getLinkFile () );
				...
				if ($link->getSampleType () == DownloadHelper::LINK_TYPE_URL) {
					...
					$resource = $this->_objectManager->get ( 'Magento\Downloadable\Helper\File' )->getFilePath ( $this->_getLink ()->getBaseSamplePath (), $link->getSampleFile () );
					$resourceType = DownloadHelper::LINK_TYPE_FILE;
				}
			}
			try {
