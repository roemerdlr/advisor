...
class Directpost extends \Magento\Authorizenet\Model\Authorizenet implements TransparentInterface, ConfigInterface {
	...
	protected function processCapture($result, $payment) {
		switch ($result->getXResponseCode ()) {
			...
			case self::RESPONSE_CODE_HELD :
				if (in_array ( $result->getXResponseReasonCode (), [ 
	...
	public function generateRequestFromOrder(\Magento\Sales\Model\Order $order) {
		$request = $this->requestFactory->create ()->setConstantData ( $this )->setDataFromOrder ( $order, $this )->signRequestData ();
	...
	public function validateResponse() {
		...
		// md5 check
		if (! $this->getConfigData ( 'trans_md5' ) || ! $this->getConfigData ( 'login' ) || ! $response->isValidHash ( $this->getConfigData ( 'trans_md5' ), $this->getConfigData ( 'login' ) )) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'The transaction was declined because the response hash validation failed.' ) );
		}
		return true;
	}
	 ...
	 *        	data from Authorize.net from $_POST
	 * @return void
	...
	public function process(array $responseData) {
		...
		if ($orderIncrementId) {
			...
			if (! $payment || $payment->getMethod () != $this->getCode ()) {
				throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'This payment didn\'t work out because we can\'t find this order.' ) );
			}
			if ($order->getId ()) {
				// operate with order
		...
		if ($isError) {
			$responseText = $responseText && ! $response->isApproved () ? $responseText : __ ( 'This payment didn\'t work out because we can\'t find this order.' );
	...
	public function checkResponseCode() {
		switch ($this->getResponse ()->getXResponseCode ()) {
			...
			case self::RESPONSE_CODE_DECLINED :
			case self::RESPONSE_CODE_ERROR :
				throw new \Magento\Framework\Exception\LocalizedException ( $this->dataHelper->wrapGatewayError ( $this->getResponse ()->getXResponseReasonText () ) );
			default :
				throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'There was a payment authorization error.' ) );
		}
	}
	...
	public function checkTransId() {
		if (! $this->getResponse ()->getXTransId ()) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Please enter a transaction ID to authorize this payment.' ) );
		}
		return true;
	}
	 ...
	 * @return void
	 * @throws \Magento\Framework\Exception\LocalizedException
	 * @throws \Exception @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	 ...
	 * Return money to customer if needed.
	 *
	 * @param \Magento\Sales\Model\Order $order        	
	 ...
	 * @return $this
	 */
	protected function addStatusCommentOnUpdate(\Magento\Sales\Model\Order\Payment $payment, \Magento\Framework\DataObject $response, $transactionId) {
		if ($payment->getIsTransactionApproved ()) {
			$message = __ ( 'Transaction %1 has been approved. Amount %2. Transaction status is "%3"', $transactionId, $payment->getOrder ()->getBaseCurrency ()->formatTxt ( $payment->getAmountAuthorized () ), $this->dataHelper->getTransactionStatusLabel ( $response->getTransactionStatus () ) );
			$payment->getOrder ()->addStatusHistoryComment ( $message );
		} elseif ($payment->getIsTransactionDenied ()) {
			$message = __ ( 'Transaction %1 has been voided/declined. Transaction status is "%2". Amount %3.', $transactionId, $this->dataHelper->getTransactionStatusLabel ( $response->getTransactionStatus () ), $payment->getOrder ()->getBaseCurrency ()->formatTxt ( $payment->getAmountAuthorized () ) );
			$payment->getOrder ()->addStatusHistoryComment ( $message );
		}
		return $this;
	}
	...
	protected function getTransactionResponse($transactionId) {
		...
		$response->setXResponseCode ( ( string ) $responseXmlDocument->transaction->responseCode )->setXResponseReasonCode ( ( string ) $responseXmlDocument->transaction->responseReasonCode )->setTransactionStatus ( ( string ) $responseXmlDocument->transaction->transactionStatus );
