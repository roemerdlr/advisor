...
class AddFieldsToResponseObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->sessionMock = $this->getMockBuilder ( 'Magento\Authorizenet\Model\Directpost\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'setLastOrderIncrementId',
				'addCheckoutOrderIncrementId' 
		] )->getMock ();
		$this->storeManagerMock = $this->getMockBuilder ( 'Magento\Store\Model\StoreManagerInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		$this->actionMock = $this->getMockBuilder ( 'Magento\Checkout\Controller\Onepage\SaveOrder' )->disableOriginalConstructor ()->getMock ();
		$this->resultMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->getMock ();
		
		$this->addFieldsToResponseObserver = $helper->getObject ( 'Magento\Authorizenet\Observer\AddFieldsToResponseObserver', [ 
	...
	public function testAddFieldsToResponseSuccess() {
		...
		$requestToAuthorizenetMock = $this->getMockBuilder ( 'Magento\Authorizenet\Model\Directpost\Request' )->disableOriginalConstructor ()->setMethods ( [ 
