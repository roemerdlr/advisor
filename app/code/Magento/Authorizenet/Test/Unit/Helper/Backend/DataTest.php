...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->urlBuilderMock = $this->getMock ( 'Magento\Backend\Model\Url', [ 
				'getUrl' 
		], [ ], '', false );
		...
		$this->dataHelper = $helper->getObject ( 'Magento\Authorizenet\Helper\Backend\Data', [ 
				'context' => $contextMock,
	...
	public function testGetPlaceOrderAdminUrl() {
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( 'adminhtml/authorizenet_directpost_payment/place' )->willReturn ( 'some value' );
	...
	public function testGetSuccessOrderUrl() {
		...
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( 'sales/order/view', [ 
				'order_id' => 'order id' 
		] )->willReturn ( 'some value' );
		
		$this->assertEquals ( 'some value', $this->dataHelper->getSuccessOrderUrl ( [ 
	...
	public function testGetRelayUrl() {
		...
		$this->storeManagerMock->expects ( $this->once () )->method ( 'getStores' )->willReturn ( [ 
				$defaultStoreMock 
		] );
