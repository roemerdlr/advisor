...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->dataHelper = $helper->getObject ( 'Magento\Authorizenet\Helper\Data', [ 
	...
	public function testGetTransactionMessage($type, $amount, $exception, $additionalMessage, $expected) {
		$currency = $this->getMock ( 'Magento\Directory\Model\Currency', [ 
				'formatTxt',
				'__wakeup' 
		], [ ], '', false );
		...
		$message = $this->dataHelper->getTransactionMessage ( $payment, $type, self::TRID, $card, $amount, $exception, $additionalMessage );
	...
	public function getMessagesParamDataProvider() {
		...
		return [ 
				[ 
						...
						false,
						$additionalMessage,
						'Credit Card: xxxx-' . self::LAST4 . ' amount 12.3 authorize - successful. ' . 'Authorize.Net Transaction ID ' . self::TRID . '. Addition message.' 
				],
				[ 
						'AUTH_CAPTURE',
						$amount,
						'some exception',
						false,
						'Credit Card: xxxx-' . self::LAST4 . ' amount 12.3 authorize and capture - failed. ' . 'Authorize.Net Transaction ID ' . self::TRID . '. some exception' 
				],
				[ 
						'CREDIT',
						false,
						false,
						$additionalMessage,
						'Credit Card: xxxx-' . self::LAST4 . ' refund - successful. ' . 'Authorize.Net Transaction ID ' . self::TRID . '. Addition message.' 
				] 
		];
	}
