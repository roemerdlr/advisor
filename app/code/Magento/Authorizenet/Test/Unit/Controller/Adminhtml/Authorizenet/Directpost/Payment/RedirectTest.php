...
class RedirectTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->directpostSessionMock = $this->getMockBuilder ( 'Magento\Authorizenet\Model\Directpost\Session' )->setMethods ( [ 
		...
		$this->adminOrderCreateMock = $this->getMockBuilder ( 'Magento\Sales\Model\AdminOrder\Create' )->setMethods ( [ 
				'getSession' 
		] )->disableOriginalConstructor ()->getMock ();
		$sessionMock = $this->getMockBuilder ( 'Magento\Backend\Model\Session' )->disableOriginalConstructor ()->getMock ();
		$this->sessionQuoteMock = $this->getMockBuilder ( 'Magento\Backend\Model\Session\Quote' )->setMethods ( [ 
				'getOrder',
				'clearStorage' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->contextMock = $this->getMockBuilder ( 'Magento\Backend\App\Action\Context' )->setMethods ( [ 
				'getObjectManager',
				'getRequest',
				'getResponse',
				'getRedirect',
		...
		$this->forwardFactoryMock = $this->getMockBuilder ( 'Magento\Backend\Model\View\Result\ForwardFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->coreRegistryMock = $this->getMockBuilder ( 'Magento\Framework\Registry' )->setMethods ( [ 
				'register' 
		] )->disableOriginalConstructor ()->getMock ();
		$resultLayoutMock = $this->getMockBuilder ( 'Magento\Framework\View\Result\Layout' )->disableOriginalConstructor ()->getMock ();
		$this->resultLayoutFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Result\LayoutFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->resultLayoutFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $resultLayoutMock );
		$this->helperMock = $this->getMockBuilder ( 'Magento\Authorizenet\Helper\Backend\Data' )->setMethods ( [ 
				'getSuccessOrderUrl' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testExecuteErrorMsgWithoutCancelOrder() {
		...
		$this->directpostSessionMock->expects ( $this->once () )->method ( 'getLastOrderIncrementId' )->willReturn ( $incrementId );
		$this->directpostSessionMock->expects ( $this->once () )->method ( 'isCheckoutOrderIncrementIdExist' )->with ( $incrementId )->willReturn ( true );
		
		$this->orderMock->expects ( $this->once () )->method ( 'loadByIncrementId' )->with ( $incrementId )->willReturnSelf ();
		$this->orderMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( true );
		$this->orderMock->expects ( $this->once () )->method ( 'getIncrementId' )->willReturn ( $incrementId );
		$this->orderMock->expects ( $this->once () )->method ( 'getState' )->willReturn ( \Magento\Sales\Model\Order::STATE_PENDING_PAYMENT );
		$this->orderMock->expects ( $this->once () )->method ( 'registerCancellation' )->with ( $params ['error_msg'] )->willReturnSelf ();
		$this->orderMock->expects ( $this->once () )->method ( 'save' );
		
		$this->directpostSessionMock->expects ( $this->once () )->method ( 'removeCheckoutOrderIncrementId' )->with ( $incrementId );
