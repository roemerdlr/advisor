...
class SuggestTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->autocomplete = $this->getMockBuilder ( 'Magento\Search\Model\AutocompleteInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getItems' 
		...
		$this->url = $this->getMockBuilder ( 'Magento\Framework\UrlInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getBaseUrl' 
		] )->getMockForAbstractClass ();
		...
		$this->controller = $this->objectManagerHelper->getObject ( 'Magento\Search\Controller\Ajax\Suggest', [ 
	...
	public function testExecute() {
		$searchString = "simple";
		$firstItemMock = $this->getMockBuilder ( 'Magento\Search\Model\Autocomplete\Item' )->disableOriginalConstructor ()->setMockClassName ( 'FirstItem' )->setMethods ( [ 
				'toArray' 
		] )->getMock ();
		$secondItemMock = $this->getMockBuilder ( 'Magento\Search\Model\Autocomplete\Item' )->disableOriginalConstructor ()->setMockClassName ( 'SecondItem' )->setMethods ( [ 
				'toArray' 
		] )->getMock ();
		
		$this->request->expects ( $this->once () )->method ( 'getParam' )->with ( 'q' )->will ( $this->returnValue ( $searchString ) );
		
		$this->autocomplete->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$firstItemMock,
				$secondItemMock 
		] ) );
