...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function setQueryFilter($query) {
		...
		$this->getSelect ()->reset ( \Magento\Framework\DB\Select::FROM )->distinct ( true )->from ( [ 
				'main_table' => $this->getTable ( 'search_query' ) 
		], [ 
				'query' => $ifSynonymFor,
				'num_results' 
		] )->where ( 'num_results > 0 AND display_in_terms = 1 AND query_text LIKE ?', $this->_resourceHelper->addLikeEscape ( $query, [ 
				'position' => 'start' 
	...
	public function setPopularQueryFilter($storeIds = null) {
		$ifSynonymFor = new \Zend_Db_Expr ( $this->getConnection ()->getCheckSql ( "synonym_for IS NOT NULL AND synonym_for != ''", 'synonym_for', 'query_text' ) );
		
		$this->getSelect ()->reset ( \Magento\Framework\DB\Select::FROM )->reset ( \Magento\Framework\DB\Select::COLUMNS )->distinct ( true )->from ( [ 
				'main_table' => $this->getTable ( 'search_query' ) 
		], [ 
				'name' => $ifSynonymFor,
				'num_results',
