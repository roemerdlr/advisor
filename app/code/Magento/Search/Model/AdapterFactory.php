...
class AdapterFactory {
	 ...
	 */
	public function __construct(\Magento\Framework\ObjectManagerInterface $objectManager, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, array $adapters, $path, $scopeType) {
	...
	public function create(array $data = []) {
		...
		if (! isset ( $this->adapterPool [$currentAdapter] )) {
			throw new \LogicException ( 'There is no such adapter: ' . $currentAdapter );
		}
		...
		if (! ($adapter instanceof \Magento\Framework\Search\AdapterInterface)) {
			throw new \InvalidArgumentException ( 'Adapter must implement \Magento\Framework\Search\AdapterInterface' );
		}
		return $adapter;
	}
}
