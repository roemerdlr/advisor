...
class RendererTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->taxHelper = $this->getMockBuilder ( '\Magento\Tax\Helper\Data' )->disableOriginalConstructor ()->setMethods ( [ 
		...
		$this->renderer = $objectManager->getObject ( 'Magento\Tax\Block\Item\Price\Renderer', [ 
	...
	protected function getItemMockWithStoreId($storeId) {
		...
		$itemMock->expects ( $this->once () )->method ( 'getStoreId' )->will ( $this->returnValue ( $storeId ) );
	...
	public function testFormatPriceQuoteItem() {
		...
		$itemMock = $this->getMockBuilder ( '\Magento\Quote\Model\Quote\Item' )->disableOriginalConstructor ()->setMethods ( [ 
				'getStore',
				'__wakeup' 
		] )->getMock ();
	...
	public function testFormatPriceOrderItem() {
		...
		$itemMock->expects ( $this->once () )->method ( 'getOrder' )->will ( $this->returnValue ( $orderMock ) );
	...
	public function testFormatPriceInvoiceItem() {
		...
		$invoiceItemMock = $this->getMockBuilder ( '\Magento\Sales\Model\Invoice\Item' )->disableOriginalConstructor ()->setMethods ( [ 
				'getOrderItem',
				'__wakeup',
				'getStoreId' 
		] )->getMock ();
	...
	public function testGetItemDisplayPriceExclTaxQuoteItem() {
		...
		$quoteItemMock->expects ( $this->once () )->method ( 'getCalculationPrice' )->will ( $this->returnValue ( $price ) );
