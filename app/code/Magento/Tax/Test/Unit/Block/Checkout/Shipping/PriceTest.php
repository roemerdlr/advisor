...
class PriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$checkoutSession = $this->getMockBuilder ( '\Magento\Checkout\Model\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'getQuote',
				'__wakeup' 
		] )->getMock ();
		...
		$this->priceObj = $objectManager->getObject ( 'Magento\Tax\Block\Checkout\Shipping\Price', [ 
	...
	protected function setupShippingRate($shippingPrice) {
		$shippingRateMock = $this->getMockBuilder ( '\Magento\Quote\Model\Quote\Address\Rate' )->disableOriginalConstructor ()->setMethods ( [ 
				'getPrice',
				'__wakeup' 
		] )->getMock ();
	...
	public function testGetShippingPriceExclTax() {
		...
		$this->taxHelper->expects ( $this->once () )->method ( 'getShippingPrice' )->will ( $this->returnValue ( $shippingPriceExclTax ) );
		
		$this->priceCurrency->expects ( $this->once () )->method ( 'convertAndFormat' )->with ( $this->logicalOr ( $shippingPriceExclTax, true, $this->store ) )->willReturn ( $convertedPrice );
	...
	public function testDisplayShippingPriceInclTax() {
		$this->taxHelper->expects ( $this->once () )->method ( 'displayShippingPriceIncludingTax' );
