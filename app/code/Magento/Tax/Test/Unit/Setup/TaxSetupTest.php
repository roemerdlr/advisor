...
class TaxSetupTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$salesSetupFactory = $this->getMock ( 'Magento\Sales\Setup\SalesSetupFactory', [ 
				'create' 
		], [ ], '', false );
		$salesSetupFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $salesSetup ) );
		
		$helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->taxSetup = $helper->getObject ( 'Magento\Tax\Setup\TaxSetup', [ 
	...
	public function testGetTaxableItems() {
		...
		$this->typeConfigMock->expects ( $this->once () )->method ( 'filter' )->with ( 'taxable' )->will ( $this->returnValue ( $refundable ) );
