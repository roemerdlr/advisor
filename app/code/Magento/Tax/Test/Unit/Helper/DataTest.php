...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->orderTaxManagementMock = $this->getMockBuilder ( 'Magento\Tax\Api\OrderTaxManagementInterface' )->disableOriginalConstructor ()->getMock ();
		...
		$this->helper = $objectManager->getObject ( 'Magento\Tax\Helper\Data', [ 
	...
	public function testGetCalculatedTaxesForOrder() {
		...
		$orderDetailsItem = $this->getMockBuilder ( 'Magento\Tax\Api\Data\OrderTaxDetailsAppliedTaxInterface' )->disableOriginalConstructor ()->getMock ();
		$orderDetailsItem->expects ( $this->once () )->method ( 'getCode' )->willReturn ( $itemCode );
		$orderDetailsItem->expects ( $this->once () )->method ( 'getAmount' )->willReturn ( $itemAmount );
		$orderDetailsItem->expects ( $this->once () )->method ( 'getBaseAmount' )->willReturn ( $itemBaseAmount );
		$orderDetailsItem->expects ( $this->once () )->method ( 'getTitle' )->willReturn ( $itemTitle );
		$orderDetailsItem->expects ( $this->once () )->method ( 'getPercent' )->willReturn ( $itemPercent );
		...
		$this->priceCurrencyMock->expects ( $this->exactly ( 2 ) )->method ( 'round' )->will ( $this->returnValueMap ( $roundValues ) );
		
		$appliedTaxes = [ 
				$orderDetailsItem 
		];
		
		$orderDetails = $this->getMockBuilder ( 'Magento\Tax\Api\Data\OrderTaxDetailsInterface' )->disableOriginalConstructor ()->getMock ();
		$orderDetails->expects ( $this->once () )->method ( 'getAppliedTaxes' )->willReturn ( $appliedTaxes );
		$this->orderTaxManagementMock->expects ( $this->once () )->method ( 'getOrderTaxDetails' )->with ( $orderId )->willReturn ( $orderDetails );
	 ...
	 * @param
	 *        	$inputArray
	 * @return \PHPUnit_Framework_MockObject_MockObject|\Magento\Tax\Api\Data\OrderTaxDetailsInterface @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	protected function mapOrderTaxItemDetail($inputArray) {
		$orderTaxItemDetailsMock = $this->getMockBuilder ( '\Magento\Tax\Api\Data\OrderTaxDetailsInterface' )->getMock ();
		$itemMocks = [ ];
		foreach ( $inputArray ['items'] as $orderTaxDetailsItemData ) {
			...
			$associatedItemId = isset ( $orderTaxDetailsItemData ['associated_item_id'] ) ? $orderTaxDetailsItemData ['associated_item_id'] : null;
			...
			foreach ( $appliedTaxesData as $appliedTaxData ) {
				$appliedTaxesMock = $this->getMockBuilder ( '\Magento\Tax\Api\Data\OrderTaxDetailsAppliedTaxInterface' )->getMock ();
				$appliedTaxesMock->expects ( $this->any () )->method ( 'getAmount' )->will ( $this->returnValue ( $appliedTaxData ['amount'] ) );
				$appliedTaxesMock->expects ( $this->any () )->method ( 'getBaseAmount' )->will ( $this->returnValue ( $appliedTaxData ['base_amount'] ) );
				$appliedTaxesMock->expects ( $this->any () )->method ( 'getCode' )->will ( $this->returnValue ( $appliedTaxData ['code'] ) );
				$appliedTaxesMock->expects ( $this->any () )->method ( 'getTitle' )->will ( $this->returnValue ( $appliedTaxData ['title'] ) );
				$appliedTaxesMock->expects ( $this->any () )->method ( 'getPercent' )->will ( $this->returnValue ( $appliedTaxData ['percent'] ) );
				$appliedTaxesMocks [] = $appliedTaxesMock;
			}
			$orderTaxDetailsItemMock = $this->getMockBuilder ( '\Magento\Tax\Api\Data\OrderTaxDetailsItemInterface' )->getMock ();
			$orderTaxDetailsItemMock->expects ( $this->any () )->method ( 'getItemId' )->will ( $this->returnValue ( $itemId ) );
			$orderTaxDetailsItemMock->expects ( $this->any () )->method ( 'getAssociatedItemId' )->will ( $this->returnValue ( $associatedItemId ) );
			$orderTaxDetailsItemMock->expects ( $this->any () )->method ( 'getType' )->will ( $this->returnValue ( $itemType ) );
			$orderTaxDetailsItemMock->expects ( $this->any () )->method ( 'getAppliedTaxes' )->will ( $this->returnValue ( $appliedTaxesMocks ) );
			
			$itemMocks [] = $orderTaxDetailsItemMock;
		}
		$orderTaxItemDetailsMock->expects ( $this->any () )->method ( 'getItems' )->will ( $this->returnValue ( $itemMocks ) );
