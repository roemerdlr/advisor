...
class AdjustmentTest extends \PHPUnit_Framework_TestCase {
	...
	public function testIsIncludedInBasePrice($expectedResult) {
		$this->taxHelper->expects ( $this->once () )->method ( 'priceIncludesTax' )->will ( $this->returnValue ( $expectedResult ) );
	...
	public function testIsIncludedInDisplayPrice($displayPriceIncludingTax, $displayBothPrices, $expectedResult) {
		$this->taxHelper->expects ( $this->once () )->method ( 'displayPriceIncludingTax' )->will ( $this->returnValue ( $displayPriceIncludingTax ) );
		if (! $displayPriceIncludingTax) {
			$this->taxHelper->expects ( $this->once () )->method ( 'displayBothPrices' )->will ( $this->returnValue ( $displayBothPrices ) );
		}
	...
	public function testExtractAdjustment($isPriceIncludesTax, $amount, $price, $expectedResult) {
		...
		$this->catalogHelper->expects ( $this->any () )->method ( 'getTaxPrice' )->with ( $object, $amount )->will ( $this->returnValue ( $price ) );
