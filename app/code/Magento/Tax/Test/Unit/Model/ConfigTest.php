...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function dataProviderGetCalculationSequence() {
		return [ 
				[ 
						true,
						true,
						\Magento\Tax\Model\Calculation::CALC_TAX_AFTER_DISCOUNT_ON_INCL 
				],
				[ 
						true,
						false,
						\Magento\Tax\Model\Calculation::CALC_TAX_AFTER_DISCOUNT_ON_EXCL 
				],
				[ 
						false,
						true,
						\Magento\Tax\Model\Calculation::CALC_TAX_BEFORE_DISCOUNT_ON_INCL 
				],
				[ 
						false,
						false,
						\Magento\Tax\Model\Calculation::CALC_TAX_BEFORE_DISCOUNT_ON_EXCL 
				] 
		];
	}
	...
	public function testScopeConfigMethods($method, $path, $configValue, $expectedValue) {
		$scopeConfigMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		$scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( $path, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->will ( $this->returnValue ( $configValue ) );
