...
class RepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->searchResultFactory = $this->getMock ( '\Magento\Tax\Api\Data\TaxClassSearchResultsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->searchResultMock = $this->getMock ( '\Magento\Tax\Api\Data\TaxClassSearchResultsInterface', [ ], [ ], '', false );
		
		$this->classModelRegistryMock = $this->getMock ( '\Magento\Tax\Model\ClassModelRegistry', [ ], [ ], '', false );
		
		$this->taxClassCollectionFactory = $this->getMock ( '\Magento\Tax\Model\ResourceModel\TaxClass\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = $this->objectManager->getObject ( 'Magento\Tax\Model\TaxClass\Repository', [ 
	...
	public function testDeleteResourceException() {
		$taxClass = $this->getMock ( '\Magento\Tax\Model\ClassModel', [ ], [ ], '', false );
		$taxClass->expects ( $this->once () )->method ( 'getClassId' )->willReturn ( 1 );
		$this->taxClassResourceMock->expects ( $this->once () )->method ( 'delete' )->willThrowException ( new CouldNotDeleteException ( __ ( 'Some Message' ) ) );
	...
	public function testDeleteWithException() {
		$taxClass = $this->getMock ( '\Magento\Tax\Model\ClassModel', [ ], [ ], '', false );
		$taxClass->expects ( $this->once () )->method ( 'getClassId' )->willReturn ( 1 );
		$this->taxClassResourceMock->expects ( $this->once () )->method ( 'delete' )->willThrowException ( new \Exception ( 'Some Message' ) );
	...
	public function testGetList() {
		...
		$searchCriteria->expects ( $this->exactly ( 2 ) )->method ( 'getSortOrders' )->willReturn ( [ 
				$sortOrder 
		] );
		...
		$collection->expects ( $this->any () )->method ( 'setItems' )->with ( [ 
				$taxClassOne,
				$taxClassTwo 
		] );
	...
	public function testSaveWithLocalizedException() {
		...
		$this->taxClassResourceMock->expects ( $this->once () )->method ( 'save' )->with ( $taxClass )->willThrowException ( new LocalizedException ( __ ( "Something went wrong" ) ) );
		$this->model->save ( $taxClass );
	}
	...
	public function testSaveWithSameClassException() {
		...
		$this->taxClassResourceMock->expects ( $this->once () )->method ( 'save' )->with ( $taxClass )->willThrowException ( new LocalizedException ( __ ( 'Class name and class type' ) ) );
		$this->model->save ( $taxClass );
	}
	 ...
	 * @return void @dataProvider validateTaxClassDataProvider
