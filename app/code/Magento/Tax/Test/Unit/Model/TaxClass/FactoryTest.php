...
class FactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreate($classType, $className, $classTypeMock) {
		$classMock = $this->getMock ( 'Magento\Tax\Model\ClassModel', [ 
				'getClassType',
				'getId',
				'__wakeup' 
		], [ ], '', false );
		...
		$objectManager->expects ( $this->once () )->method ( 'create' )->with ( $this->equalTo ( $className ), $this->equalTo ( [ 
				'data' => [ 
						'id' => 1 
				] 
	...
	public function testCreateWithWrongClassType() {
		...
		$this->setExpectedException ( 'Magento\Framework\Exception\LocalizedException', sprintf ( 'Invalid type of tax class "%s"', $wrongClassType ) );
