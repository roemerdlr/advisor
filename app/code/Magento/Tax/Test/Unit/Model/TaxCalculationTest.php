...
class TaxCalculationTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->calculatorFactory = $this->getMock ( '\Magento\Tax\Model\Calculation\CalculatorFactory', [ ], [ ], '', false );
		$this->configMock = $this->getMock ( '\Magento\Tax\Model\Config', [ ], [ ], '', false );
		$this->taxDetailsDataObjectFactory = $this->getMock ( '\Magento\Tax\Api\Data\TaxDetailsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->taxDetailsItemDataObjectFactory = $this->getMock ( '\Magento\Tax\Api\Data\TaxDetailsItemInterfaceFactory', [ ], [ ], '', false );
		...
		$this->taxCalculationService = $objectManager->getObject ( 'Magento\Tax\Model\TaxCalculation', [ 
	...
	public function testGetCalculatedRate() {
		...
		$rateRequestMock = $this->getMock ( '\Magento\Framework\DataObject', [ 
				'setProductClassId' 
		], [ ], '', false );
	...
	public function testCalculateTaxIfNoItemsInQuote() {
		...
		$taxDetailsMock->expects ( $this->once () )->method ( 'setSubtotal' )->willReturnSelf ();
		$taxDetailsMock->expects ( $this->once () )->method ( 'setTaxAmount' )->willReturnSelf ();
		$taxDetailsMock->expects ( $this->once () )->method ( 'setDiscountTaxCompensationAmount' )->willReturnSelf ();
		$taxDetailsMock->expects ( $this->once () )->method ( 'setAppliedTaxes' )->willReturnSelf ();
		$taxDetailsMock->expects ( $this->once () )->method ( 'setItems' )->willReturnSelf ();
	...
	public function testCalculateTax() {
		...
		$quoteDetailsMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
