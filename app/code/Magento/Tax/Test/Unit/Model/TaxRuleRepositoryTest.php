...
class TaxRuleRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->searchResultsMock = $this->getMock ( '\Magento\Tax\Api\Data\TaxRuleSearchResultsInterface', [ ], [ ], '', false );
		$this->ruleFactory = $this->getMock ( '\Magento\Tax\Model\Calculation\RuleFactory', [ ], [ ], '', false );
		$this->collectionFactory = $this->getMock ( '\Magento\Tax\Model\ResourceModel\Calculation\Rule\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = new TaxRuleRepository ( $this->taxRuleRegistry, $this->searchResultFactory, $this->ruleFactory, $this->collectionFactory, $this->resource, $this->extensionAttributesJoinProcessorMock );
	}
	...
	public function testSaveWithExceptions($exceptionObject, $exceptionName, $exceptionMessage) {
		...
		$this->taxRuleRegistry->expects ( $this->once () )->method ( 'retrieveTaxRule' )->with ( 10 )->willReturn ( $rule );
		$this->resource->expects ( $this->once () )->method ( 'save' )->with ( $rule )->willThrowException ( $exceptionObject );
	...
	public function testGetList() {
		...
		$this->extensionAttributesJoinProcessorMock->expects ( $this->once () )->method ( 'process' )->with ( $collectionMock );
		...
		$filterGroupMock->expects ( $this->exactly ( 2 ) )->method ( 'getFilters' )->willReturn ( [ 
				$filterMock 
		] );
		$filterMock->expects ( $this->exactly ( 2 ) )->method ( 'getConditionType' )->willReturn ( 'eq' );
		$filterMock->expects ( $this->exactly ( 2 ) )->method ( 'getField' )->willReturnOnConsecutiveCalls ( 'rate.tax_calculation_rate_id', 'cd.customer_tax_class_id' );
		$filterMock->expects ( $this->once () )->method ( 'getValue' )->willReturn ( 'value' );
		$collectionMock->expects ( $this->exactly ( 2 ) )->method ( 'joinCalculationData' )->withConsecutive ( [ 
				'rate' 
		], [ 
				'cd' 
		] );
		$collectionMock->expects ( $this->once () )->method ( 'addFieldToFilter' )->with ( [ 
				0 => 'rate.tax_calculation_rate_id' 
		], [ 
				0 => [ 
						'eq' => 'value' 
				] 
