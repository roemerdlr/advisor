...
class TotalBaseCalculatorTest extends RowBaseAndTotalBaseCalculatorTestCase {
	...
	public function testCalculateWithTaxInPrice() {
		...
		$this->totalBaseCalculator->expects ( $this->exactly ( 3 ) )->method ( 'deltaRound' )->will ( $this->returnValue ( 0 ) );
		$this->initMocks ( true );
		
		$this->assertSame ( $this->taxDetailsItem, $this->calculate ( $this->totalBaseCalculator ) );
	...
	public function testCalculateWithTaxInPriceNoRounding() {
		...
		$this->assertSame ( $this->taxDetailsItem, $this->calculate ( $this->totalBaseCalculator, false ) );
		$this->assertEquals ( self::UNIT_PRICE_INCL_TAX, $this->taxDetailsItem->getPriceInclTax () );
	}
	public function testCalculateWithTaxNotInPrice() {
		$this->initTotalBaseCalculator ();
		$this->totalBaseCalculator->expects ( $this->exactly ( 2 ) )->method ( 'deltaRound' )->will ( $this->returnValue ( 0 ) );
	...
	private function initTotalBaseCalculator() {
		...
		$this->totalBaseCalculator = $this->getMock ( 'Magento\Tax\Model\Calculation\TotalBaseCalculator', [ 
				'deltaRound' 
		], [ 
