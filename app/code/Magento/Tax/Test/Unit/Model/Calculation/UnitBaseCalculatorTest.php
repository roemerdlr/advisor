...
class UnitBaseCalculatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->taxDetailsItemDataObjectFactoryMock = $this->getMockBuilder ( 'Magento\Tax\Api\Data\TaxDetailsItemInterfaceFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->taxDetailsItemDataObjectFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->taxDetailsItem );
		
		$this->mockCalculationTool = $this->getMockBuilder ( '\Magento\Tax\Model\Calculation' )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup',
				'round',
				'getRate',
				'getStoreRate',
		...
		$this->mockCalculationTool->expects ( $this->any () )->method ( 'round' )->withAnyParameters ()->willReturnCallback ( function ($price) {
			return round ( $price, 2 );
		} );
		$this->mockConfig = $this->getMockBuilder ( '\Magento\Tax\Model\Config' )->disableOriginalConstructor ()->getMock ();
		...
		$this->appliedTaxRateDataObjectFactoryMock = $this->getMock ( 'Magento\Tax\Api\Data\AppliedTaxRateInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->appliedTaxRateDataObjectFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->appliedTaxRate );
		
		$appliedTaxDataObject = $objectManager->getObject ( 'Magento\Tax\Model\TaxDetails\AppliedTax' );
		$appliedTaxDataObjectFactoryMock = $this->getMock ( 'Magento\Tax\Api\Data\AppliedTaxInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$appliedTaxDataObjectFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $appliedTaxDataObject );
	...
	public function testCalculateWithTaxNotInPrice() {
		...
		$this->mockCalculationTool->expects ( $this->once () )->method ( 'getAppliedRates' )->withAnyParameters ()->will ( $this->returnValue ( [ 
				[ 
						'id' => 0,
						'percent' => 0,
						'rates' => [ ] 
	...
	protected function getMockItem() {
		...
		$mockItem = $this->getMockBuilder ( 'Magento\Tax\Api\Data\QuoteDetailsItemInterface' )->disableOriginalConstructor ()->getMock ();
		$mockItem->expects ( $this->atLeastOnce () )->method ( 'getDiscountAmount' )->will ( $this->returnValue ( 1 ) );
		$mockItem->expects ( $this->atLeastOnce () )->method ( 'getCode' )->will ( $this->returnValue ( self::CODE ) );
		$mockItem->expects ( $this->atLeastOnce () )->method ( 'getType' )->will ( $this->returnValue ( self::TYPE ) );
		$mockItem->expects ( $this->atLeastOnce () )->method ( 'getUnitPrice' )->will ( $this->returnValue ( self::UNIT_PRICE ) );
