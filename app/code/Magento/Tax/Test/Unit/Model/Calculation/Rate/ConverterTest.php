...
class ConverterTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->taxRateDataObjectFactory = $this->getMockBuilder ( '\Magento\Tax\Api\Data\TaxRateInterfaceFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->taxRateTitleDataObjectFactory = $this->getMockBuilder ( '\Magento\Tax\Api\Data\TaxRateTitleInterfaceFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
	...
	public function testCreateTitlesFromServiceObject() {
		...
		$this->assertEquals ( [ 
				1 => 'Value' 
		], $this->converter->createTitleArrayFromServiceObject ( $taxRateMock ) );
	}
	...
	public function testCreateArrayFromServiceObject() {
		...
		$taxRateMock->expects ( $this->atLeastOnce () )->method ( 'getTitles' )->willReturn ( [ 
				$titlesMock 
		] );
	...
	public function testPopulateTaxRateData() {
		$rateTitles = [ 
				$this->objectManager->getObject ( '\Magento\Tax\Model\Calculation\Rate\Title', [ 
						'data' => [ 
								'store_id' => 1,
								'value' => 'texas' 
						] 
		...
		$taxRate = $this->objectManager->getObject ( 'Magento\Tax\Model\Calculation\Rate', [ 
				'data' => $dataArray 
		] );
