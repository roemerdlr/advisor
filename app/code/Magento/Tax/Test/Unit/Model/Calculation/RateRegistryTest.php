...
class RateRegistryTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->rateModelFactoryMock = $this->getMockBuilder ( 'Magento\Tax\Model\Calculation\RateFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->rateRegistry = $objectManager->getObject ( 'Magento\Tax\Model\Calculation\RateRegistry', [ 
				'taxModelRateFactory' => $this->rateModelFactoryMock 
		] );
		$this->rateModelMock = $this->getMockBuilder ( 'Magento\Tax\Model\Calculation\Rate' )->disableOriginalConstructor ()->getMock ();
	}
	...
	public function testRetrieveTaxRate() {
		$this->rateModelMock->expects ( $this->once () )->method ( 'load' )->with ( self::TAX_RATE_ID )->will ( $this->returnValue ( $this->rateModelMock ) );
		$this->rateModelMock->expects ( $this->any () )->method ( 'getId' )->will ( $this->returnValue ( self::TAX_RATE_ID ) );
		$this->rateModelFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->rateModelMock ) );
	...
	public function testRetrieveException() {
		$this->rateModelMock->expects ( $this->once () )->method ( 'load' )->with ( self::TAX_RATE_ID )->will ( $this->returnValue ( $this->rateModelMock ) );
		$this->rateModelMock->expects ( $this->any () )->method ( 'getId' )->will ( $this->returnValue ( null ) );
		$this->rateModelFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->rateModelMock ) );
