...
class CalculationTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreateSearchPostCodeTemplates($postalCode, $exactPostalcode) {
		...
		$taxData->expects ( $this->any () )->method ( 'getPostCodeSubStringLength' )->will ( $this->returnValue ( 10 ) );
		
		$objectManager = new ObjectManager ( $this );
		$calcMock = $objectManager->getObject ( 'Magento\Tax\Model\ResourceModel\Calculation', [ 
				'resource' => $resource,
				'taxData' => $taxData,
		...
		$method = new \ReflectionMethod ( 'Magento\Tax\Model\ResourceModel\Calculation', '_createSearchPostCodeTemplates' );
		$method->setAccessible ( true );
		
		// test & verify
		$resultsArr = $method->invokeArgs ( $calcMock, [ 
				$postalCode,
				$exactPostalcode 
		] );
	...
	private function verifyResults($resultsArr, $code1, $code2 = null) {
		...
		$this->assertEquals ( $expectedSize, $actualSize, 'Expected size of the result array was ' . $expectedSize . ' but actual was ' . $actualSize );
