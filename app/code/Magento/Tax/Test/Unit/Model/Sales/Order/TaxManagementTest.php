...
class TaxManagementTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->taxItemResourceMock = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Tax\Item', [ ], [ ], '', false );
		$this->taxItemFactoryMock = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Order\Tax\ItemFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->appliedTaxDataObjectFactoryMock = $this->getMock ( 'Magento\Tax\Api\Data\OrderTaxDetailsAppliedTaxInterfaceFactory', $methods, [ ], '', false );
		$this->appliedTaxDataObjectFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->appliedTaxDataObject );
		
		$this->itemDataObject = $objectManager->getObject ( 'Magento\Sales\Model\Order\Tax\Item' );
		$this->itemDataObjectFactoryMock = $this->getMock ( 'Magento\Tax\Api\Data\OrderTaxDetailsItemInterfaceFactory', $methods, [ ], '', false );
		$this->itemDataObjectFactoryMock->expects ( $this->atLeastOnce () )->method ( 'create' )->willReturn ( $this->itemDataObject );
		
		$this->orderTaxDetailsDataObject = $objectManager->getObject ( 'Magento\Tax\Model\Sales\Order\Details' );
		$this->orderTaxDetailsDataObjectFactoryMock = $this->getMock ( 'Magento\Tax\Api\Data\OrderTaxDetailsInterfaceFactory', $methods, [ ], '', false );
		$this->orderTaxDetailsDataObjectFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->orderTaxDetailsDataObject );
		
		$this->taxManagement = $objectManager->getObject ( 'Magento\Tax\Model\Sales\Order\TaxManagement', [ 
	 ...
	 * @return void @dataProvider getOrderTaxDetailsDataProvider
	 */
