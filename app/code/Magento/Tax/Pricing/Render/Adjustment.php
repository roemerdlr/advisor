...
class Adjustment extends AbstractAdjustment {
	 ...
	 * @param array $data        	
	 */
	public function __construct(Template\Context $context, PriceCurrencyInterface $priceCurrency, \Magento\Tax\Helper\Data $helper, array $data = []) {
	...
	protected function apply() {
		if ($this->displayBothPrices ()) {
			...
			$this->amountRender->setPriceId ( $this->buildIdWithPrefix ( 'price-including-tax-' ) );
		} elseif ($this->displayPriceExcludingTax ()) {
			$this->amountRender->setDisplayValue ( $this->amountRender->getAmount ()->getValue ( $this->getAdjustmentCode () ) );
	...
	public function getDisplayAmountExclTax($exclude = null, $includeContainer = false) {
		...
		return $this->formatCurrency ( $this->getRawAmount ( $exclude ), $includeContainer );
	}
