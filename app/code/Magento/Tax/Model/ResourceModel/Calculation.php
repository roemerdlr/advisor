...
class Calculation extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Tax\Helper\Data $taxData, \Magento\Store\Model\StoreManagerInterface $storeManager, $connectionName = null) {
	...
	protected function _getRates($request) {
		...
		$ids = is_array ( $productClassId ) ? $productClassId : [ 
				$productClassId 
		];
		...
		if (! isset ( $this->_ratesCache [$cacheKey] )) {
			...
			] )->where ( 'rate.tax_country_id = ?', $countryId )->where ( "rate.tax_region_id IN(?)", [ 
					0,
					( int ) $regionId 
			] );
