...
class Rule extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	/**
	 * Fetches rules by rate, customer tax classes and product tax classes.
	...
	public function fetchRuleCodes($rateId, $customerTaxClassIds, $productTaxClassIds) {
		...
		$select = $connection->select ()->from ( [ 
				'main' => $this->getTable ( 'tax_calculation' ) 
		], null )->joinLeft ( [ 
				'd' => $this->getTable ( 'tax_calculation_rule' ) 
		], 'd.tax_calculation_rule_id = main.tax_calculation_rule_id', [ 
				'd.code' 
		] )->where ( 'main.tax_calculation_rate_id in (?)', $rateId )->where ( 'main.customer_tax_class_id in (?)', $customerTaxClassIds )->where ( 'main.product_tax_class_id in (?)', $productTaxClassIds )->distinct ( true );
