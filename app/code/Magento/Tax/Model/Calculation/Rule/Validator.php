...
class Validator extends \Magento\Framework\Validator\AbstractValidator {
	...
	public function isValid($value) {
		...
		if (! \Zend_Validate::is ( trim ( $value->getPosition () ), 'NotEmpty' )) {
			$this->addErrorMessage ( $messages, InputException::REQUIRED_FIELD, [ 
					'fieldName' => 'position' 
			] );
		}
		if (! \Zend_Validate::is ( trim ( $value->getPosition () ), 'GreaterThan', [ 
				- 1 
		] )) {
			$this->addErrorMessage ( $messages, InputException::INVALID_FIELD_MIN_VALUE, [ 
					'fieldName' => 'position',
		...
		if (! \Zend_Validate::is ( trim ( $value->getPriority () ), 'GreaterThan', [ 
				- 1 
		] )) {
			$this->addErrorMessage ( $messages, InputException::INVALID_FIELD_MIN_VALUE, [ 
					'fieldName' => 'priority',
			...
			foreach ( $customerTaxClassIds as $customerTaxClassId ) {
				try {
					...
					if ($taxClass === null || ! ($taxClass->getClassType () == TaxClassModel::TAX_CLASS_TYPE_CUSTOMER)) {
						$this->addErrorMessage ( $messages, NoSuchEntityException::MESSAGE_SINGLE_FIELD, [ 
								'fieldName' => 'customer_tax_class_ids',
