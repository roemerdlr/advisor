...
class Calculation extends \Magento\Framework\Model\AbstractModel {
	...
	public function getRate($request) {
		...
		if (! isset ( $this->_rateCache [$cacheKey] )) {
			...
			$this->_eventManager->dispatch ( 'tax_rate_data_fetch', [ 
					'request' => $request,
					'sender' => $this 
			] );
	...
	protected function getRateOriginRequest($store = null) {
		...
		$request->setCountryId ( $this->_scopeConfig->getValue ( \Magento\Shipping\Model\Config::XML_PATH_ORIGIN_COUNTRY_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $store ) )->setRegionId ( $this->_scopeConfig->getValue ( \Magento\Shipping\Model\Config::XML_PATH_ORIGIN_REGION_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $store ) )->setPostcode ( $this->_scopeConfig->getValue ( \Magento\Shipping\Model\Config::XML_PATH_ORIGIN_POSTCODE, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $store ) )->setCustomerClassId ( $this->getDefaultCustomerTaxClass ( $store ) )->setStore ( $store );
		return $request;
	}
	
	/**
	 * Return the default rate request.
	 * It can be either based on store address or customer address
	 *
	...
	public function getTaxRates($billingAddress, $shippingAddress, $customerTaxClassId) {
		...
		$searchCriteria = $this->searchCriteriaBuilder->addFilters ( [ 
				$this->filterBuilder->setField ( ClassModel::KEY_TYPE )->setValue ( \Magento\Tax\Api\TaxClassManagementInterface::TYPE_PRODUCT )->create () 
		] )->create ();
