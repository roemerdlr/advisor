...
class TaxManagement implements \Magento\Tax\Api\OrderTaxManagementInterface {
	 ...
	 * @return AppliedTax
	 */
	protected function convertToAppliedTaxDataObject(TaxDetailsDataObjectFactory $appliedTaxDataObjectFactory, $itemAppliedTax) {
		return $appliedTaxDataObjectFactory->create ()->setCode ( $itemAppliedTax ['code'] )->setTitle ( $itemAppliedTax ['title'] )->setPercent ( $itemAppliedTax ['tax_percent'] )->setAmount ( $itemAppliedTax ['real_amount'] )->setBaseAmount ( $itemAppliedTax ['real_base_amount'] );
	}
	...
	protected function aggregateAppliedTaxes(TaxDetailsDataObjectFactory $appliedTaxDataObjectFactory, $items) {
		...
		foreach ( $orderAppliedTaxesData as $orderAppliedTaxData ) {
			$orderAppliedTaxes [] = $appliedTaxDataObjectFactory->create ()->setCode ( $orderAppliedTaxData [Tax::KEY_CODE] )->setTitle ( $orderAppliedTaxData [Tax::KEY_TITLE] )->setPercent ( $orderAppliedTaxData [Tax::KEY_PERCENT] )->setAmount ( $orderAppliedTaxData [Tax::KEY_AMOUNT] )->setBaseAmount ( $orderAppliedTaxData [Tax::KEY_BASE_AMOUNT] );
		}
	...
	public function getOrderTaxDetails($orderId) {
		...
		if (! $order) {
			throw new NoSuchEntityException ( __ ( NoSuchEntityException::MESSAGE_DOUBLE_FIELDS, [ 
					'fieldName' => 'orderId',
					'fieldValue' => $orderId 
			] ) );
		}
		...
		foreach ( $orderItemAppliedTaxes as $itemAppliedTax ) {
			...
			if (isset ( $itemAppliedTax ['item_id'] )) {
				...
				$itemsData [$itemId] ['applied_taxes'] [$itemAppliedTax ['code']] = $this->convertToAppliedTaxDataObject ( $this->appliedTaxDataObjectFactory, $itemAppliedTax );
				...
				$itemsData [$key] ['applied_taxes'] [$itemAppliedTax ['code']] = $this->convertToAppliedTaxDataObject ( $this->appliedTaxDataObjectFactory, $itemAppliedTax );
			} else {
				...
				$itemsData [$key] [Item::KEY_APPLIED_TAXES] [$itemAppliedTax ['code']] = $this->convertToAppliedTaxDataObject ( $this->appliedTaxDataObjectFactory, $itemAppliedTax );
			}
		}
		
		$items = [ ];
		foreach ( $itemsData as $itemData ) {
			$items [] = $this->itemDataObjectFactory->create ()->setType ( $itemData [Item::KEY_TYPE] )->setItemId ( $itemData [Item::KEY_ITEM_ID] )->setAssociatedItemId ( $itemData [Item::KEY_ASSOCIATED_ITEM_ID] )->setAppliedTaxes ( $itemData [Item::KEY_APPLIED_TAXES] );
		}
		$orderAppliedTaxesDOs = $this->aggregateAppliedTaxes ( $this->appliedTaxDataObjectFactory, $items );
		return $this->orderTaxDetailsDataObjectFactory->create ()->setItems ( $items )->setAppliedTaxes ( $orderAppliedTaxesDOs );
	}
}
