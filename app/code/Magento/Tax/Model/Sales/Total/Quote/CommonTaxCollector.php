...
class CommonTaxCollector extends AbstractTotal {
	 ...
	 * @return \Magento\Tax\Api\Data\QuoteDetailsItemInterface[]
	 */
	public function mapItemExtraTaxables(\Magento\Tax\Api\Data\QuoteDetailsItemInterfaceFactory $itemDataObjectFactory, AbstractItem $item, $priceIncludesTax, $useBaseCurrency) {
		$itemDataObjects = [ ];
		...
		foreach ( $extraTaxables as $extraTaxable ) {
			$extraTaxableIncludesTax = isset ( $extraTaxable ['price_includes_tax'] ) ? $extraTaxable ['price_includes_tax'] : $priceIncludesTax;
	 ...
	 * @return \Magento\Tax\Api\Data\QuoteDetailsItemInterface[]
	 */
	public function mapItems(ShippingAssignmentInterface $shippingAssignment, $priceIncludesTax, $useBaseCurrency) {
		...
		foreach ( $items as $item ) {
			...
			if ($item->getHasChildren () && $item->isChildrenCalculated ()) {
				...
				foreach ( $item->getChildren () as $child ) {
					$childItemDataObject = $this->mapItem ( $itemDataObjectFactory, $child, $priceIncludesTax, $useBaseCurrency, $parentItemDataObject->getCode () );
					$itemDataObjects [] = $childItemDataObject;
					$extraTaxableItems = $this->mapItemExtraTaxables ( $itemDataObjectFactory, $item, $priceIncludesTax, $useBaseCurrency );
	...
	protected function prepareQuoteDetails(ShippingAssignmentInterface $shippingAssignment, $itemDataObjects) {
		...
		$quoteDetails->setCustomerTaxClassKey ( $this->taxClassKeyDataObjectFactory->create ()->setType ( TaxClassKeyInterface::TYPE_ID )->setValue ( $address->getQuote ()->getCustomerTaxClassId () ) );
	 ...
	 * @return array
	 */
	protected function organizeItemTaxDetailsByType(TaxDetailsInterface $taxDetails, TaxDetailsInterface $baseTaxDetails) {
		...
		foreach ( $keyedItems as $code => $item ) {
			...
			$itemsByType [$itemType] [$code] = [ 
					self::KEY_ITEM => $item,
					self::KEY_BASE_ITEM => $baseItem 
			];
		}
			...
			foreach ( $items as $itemTaxCalculationId => $itemTaxDetails ) {
				...
				foreach ( $appliedTaxesArray as $appliedTaxArray ) {
					$this->_saveAppliedTaxes ( $total, [ 
							$appliedTaxArray 
					], $appliedTaxArray ['amount'], $appliedTaxArray ['base_amount'], $appliedTaxArray ['percent'] );
				}
			}
		}
	 ...
	 * This function is intended to be used to generate temporary
	 * id for an item.
	 *
	 * @return int
	 */
