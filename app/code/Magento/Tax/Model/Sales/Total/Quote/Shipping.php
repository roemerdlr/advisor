...
class Shipping extends CommonTaxCollector {
	/**
	 * Collect tax totals for shipping.
	 * The result can be used to calculate discount on shipping
	 *
	...
	public function collect(\Magento\Quote\Model\Quote $quote, \Magento\Quote\Api\Data\ShippingAssignmentInterface $shippingAssignment, \Magento\Quote\Model\Quote\Address\Total $total) {
		...
		$quoteDetails = $this->prepareQuoteDetails ( $shippingAssignment, [ 
				$shippingDataObject 
		] );
		$taxDetails = $this->taxCalculationService->calculateTax ( $quoteDetails, $storeId );
		
		$baseQuoteDetails = $this->prepareQuoteDetails ( $shippingAssignment, [ 
