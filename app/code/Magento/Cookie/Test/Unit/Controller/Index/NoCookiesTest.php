...
class NoCookiesTest extends \PHPUnit_Framework_TestCase {
	...
	public function setup() {
		...
		$this->redirectResponseMock = $this->getMockBuilder ( 'Magento\Framework\App\Response\RedirectInterface' )->getMock ();
		$this->viewMock = $this->getMock ( 'Magento\Framework\App\ViewInterface' );
		
		$this->controller = $objectManager->getObject ( 'Magento\Cookie\Controller\Index\NoCookies', [ 
	...
	public function testExecuteRedirectUrl() {
		...
		$this->eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( $this->equalTo ( 'controller_action_nocookies' ), $this->callback ( function ($dataArray) {
		...
		$this->responseMock->expects ( $this->once () )->method ( 'setRedirect' )->with ( self::REDIRECT_URL );
	...
	public function testExecuteRedirectPath() {
		...
		$this->redirectResponseMock->expects ( $this->once () )->method ( 'redirect' )->with ( $this->responseMock, $this->equalTo ( '\a\path' ), $this->equalTo ( '&arg1key=arg1value' ) );
	...
	public function testExecuteDefault() {
		...
		$this->viewMock->expects ( $this->once () )->method ( 'loadLayout' )->with ( [ 
				'default',
				'noCookie' 
		] );
