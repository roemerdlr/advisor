...
class DomainTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$contextMock->expects ( $this->any () )->method ( 'getEventDispatcher' )->will ( $this->returnValue ( $eventDispatcherMock ) );
		...
		$this->validatorMock = $this->getMockBuilder ( 'Magento\Framework\Session\Config\Validator\CookieDomainValidator' )->disableOriginalConstructor ()->getMock ();
		$helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->domain = $helper->getObject ( 'Magento\Cookie\Model\Config\Backend\Domain', [ 
				'context' => $contextMock,
	...
	public function testBeforeSave($value, $isValid, $callNum, $callGetMessages = 0) {
		...
		$this->validatorMock->expects ( $this->exactly ( $callNum ) )->method ( 'isValid' )->will ( $this->returnValue ( $isValid ) );
		$this->validatorMock->expects ( $this->exactly ( $callGetMessages ) )->method ( 'getMessages' )->will ( $this->returnValue ( [ 
				'message' 
		] ) );
