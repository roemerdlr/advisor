...
class CookieTest extends \PHPUnit_Framework_TestCase {
	...
	public function testIsUserNotAllowSaveCookie() {
		...
		$this->_object = new \Magento\Cookie\Helper\Cookie ( $context, $this->getMock ( 'Magento\Store\Model\StoreManager', [ ], [ ], '', false, false ), [ 
	...
	public function testGetAcceptedSaveCookiesWebsiteIds() {
		...
		$this->assertEquals ( $this->_object->getAcceptedSaveCookiesWebsiteIds (), json_encode ( [ 
				1 => 1 
		] ) );
	}
	public function testGetCookieRestrictionLifetime() {
		...
		$scopeConfig->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnCallback ( [ 
				$this,
				'getConfigMethodStub' 
		] ) )->with ( $this->equalTo ( 'web/cookie/cookie_restriction_lifetime' ) );
		$this->_context = $this->getMock ( 'Magento\Framework\App\Helper\Context', [ 
				'getRequest',
				'getScopeConfig' 
		], [ ], '', false, false );
		...
		$this->_object = new \Magento\Cookie\Helper\Cookie ( $this->_context, $this->getMock ( 'Magento\Store\Model\StoreManager', [ ], [ ], '', false, false ), [ 
	...
	protected function _getConfigStub() {
		$scopeConfig = $this->getMock ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		$scopeConfig->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnCallback ( [ 
				$this,
				'getConfigMethodStub' 
		] ) );
	...
	protected function _getCookieStub($cookieString = []) {
		$this->_request->expects ( $this->any () )->method ( 'getCookie' )->will ( $this->returnValue ( json_encode ( $cookieString ) ) );
	}
