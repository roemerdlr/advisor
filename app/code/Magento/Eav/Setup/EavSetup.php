...
class EavSetup {
	 ...
	 * @var array
	 */
	private $defaultGroupIdAssociations = [ 
			'General' => 1 
	];
	...
	public function installDefaultGroupIds() {
		...
		foreach ( $this->defaultGroupIdAssociations as $defaultGroupName => $defaultGroupId ) {
			foreach ( $setIds as $set ) {
				$groupId = $this->setup->getTableRow ( 'eav_attribute_group', 'attribute_group_name', $defaultGroupName, 'attribute_group_id', 'attribute_set_id', $set );
				if (! $groupId) {
					$groupId = $this->setup->getTableRow ( 'eav_attribute_group', 'attribute_set_id', $set, 'attribute_group_id' );
				}
				$this->setup->updateTableRow ( 'eav_attribute_group', 'attribute_group_id', $groupId, 'default_id', $defaultGroupId );
			}
		}
	...
	public function getEntityType($id, $field = null) {
		return $this->setup->getTableRow ( 'eav_entity_type', is_numeric ( $id ) ? 'entity_type_id' : 'entity_type_code', $id, $field );
	}
	...
	public function getAttributeSetSortOrder($entityTypeId, $sortOrder = null) {
		if (! is_numeric ( $sortOrder )) {
			...
			$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_attribute_set' ), 'MAX(sort_order)' )->where ( 'entity_type_id = :entity_type_id' );
	...
	public function updateAttributeSet($entityTypeId, $id, $field, $value = null) {
		$this->setup->updateTableRow ( 'eav_attribute_set', 'attribute_set_id', $this->getAttributeSetId ( $entityTypeId, $id ), $field, $value, 'entity_type_id', $this->getEntityTypeId ( $entityTypeId ) );
		return $this;
	}
	...
	public function getAttributeSet($entityTypeId, $id, $field = null) {
		return $this->setup->getTableRow ( 'eav_attribute_set', is_numeric ( $id ) ? 'attribute_set_id' : 'attribute_set_name', $id, $field, 'entity_type_id', $this->getEntityTypeId ( $entityTypeId ) );
	}
	...
	public function getDefaultAttributeSetId($entityType) {
		...
		$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_entity_type' ), 'default_attribute_set_id' )->where ( $where );
	...
	public function getAttributeGroupSortOrder($entityTypeId, $setId, $sortOrder = null) {
		if (! is_numeric ( $sortOrder )) {
			...
			$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_attribute_group' ), 'MAX(sort_order)' )->where ( 'attribute_set_id = :attribute_set_id' );
	...
	public function updateAttributeGroup($entityTypeId, $setId, $id, $field, $value = null) {
		$this->setup->updateTableRow ( 'eav_attribute_group', 'attribute_group_id', $this->getAttributeGroupId ( $entityTypeId, $setId, $id ), $field, $value, 'attribute_set_id', $this->getAttributeSetId ( $entityTypeId, $setId ) );
	...
	public function getAttributeGroup($entityTypeId, $setId, $id, $field = null) {
		...
		return $this->setup->getTableRow ( 'eav_attribute_group', $searchField, $searchId, $field, 'attribute_set_id', $this->getAttributeSetId ( $entityTypeId, $setId ) );
	}
	...
	public function removeAttributeGroup($entityTypeId, $setId, $id) {
		$this->setup->deleteTableRow ( 'eav_attribute_group', 'attribute_group_id', $this->getAttributeGroupId ( $entityTypeId, $setId, $id ) );
		return $this;
	}
	...
	public function getDefaultAttributeGroupId($entityType, $attributeSetId = null) {
		...
		$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_attribute_group' ), 'attribute_group_id' )->where ( 'attribute_set_id = :attribute_set_id' )->order ( [ 
				'default_id ' . \Magento\Framework\DB\Select::SQL_DESC,
				'sort_order' 
		] )->limit ( 1 );
	...
	public function getAttributesNumberInGroup($entityTypeId, $setId, $groupId) {
		$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_entity_attribute' ), [ 
				'count' => 'COUNT(*)' 
		] )->where ( 'attribute_group_id = ?', $this->getAttributeGroupId ( $entityTypeId, $setId, $groupId ) )->where ( 'entity_type_id = ?', $entityTypeId )->where ( 'attribute_set_id = ?', $setId );
	...
	private function _validateAttributeData($data) {
		...
		if (isset ( $data ['attribute_code'] ) && ! \Zend_Validate::is ( $data ['attribute_code'], 'StringLength', [ 
				'max' => $attributeCodeMaxLength 
		] )) {
			throw new LocalizedException ( __ ( 'An attribute code must not be more than %1 characters.', $attributeCodeMaxLength ) );
		}
	...
	public function addAttribute($entityTypeId, $code, array $attr) {
		...
		if (! empty ( $attr ['group'] ) || empty ( $attr ['user_defined'] )) {
			$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_attribute_set' ) )->where ( 'entity_type_id = :entity_type_id' );
			$sets = $this->setup->getConnection ()->fetchAll ( $select, [ 
					'entity_type_id' => $entityTypeId 
			] );
			foreach ( $sets as $set ) {
				if (! empty ( $attr ['group'] )) {
		...
		if (isset ( $option ['value'] )) {
			foreach ( $option ['value'] as $optionId => $values ) {
				...
				if (! $intOptionId) {
					...
					$this->setup->getConnection ()->update ( $optionTable, $data, [ 
							'option_id=?' => $intOptionId 
					] );
				}
	...
	private function _updateAttribute($entityTypeId, $id, $field, $value = null, $sortOrder = null) {
		if ($sortOrder !== null) {
			$this->setup->updateTableRow ( 'eav_entity_attribute', 'attribute_id', $this->getAttributeId ( $entityTypeId, $id ), 'sort_order', $sortOrder );
		}
		...
		$this->setup->updateTableRow ( 'eav_attribute', 'attribute_id', $this->getAttributeId ( $entityTypeId, $id ), $field, $value, 'entity_type_id', $this->getEntityTypeId ( $entityTypeId ) );
	...
	private function updateCachedRow($field, $value, $attribute) {
		...
		if (is_array ( $field )) {
			$oldRow = $setupCache->has ( $mainTable, $attribute ['entity_type_id'], $attribute ['attribute_code'] ) ? $setupCache->get ( $mainTable, $attribute ['entity_type_id'], $attribute ['attribute_code'] ) : [ ];
	...
	public function getAttribute($entityTypeId, $id, $field = null) {
		...
		if (! $setupCache->has ( $mainTable, $entityTypeId, $id )) {
			...
			], 'main.attribute_id = additional.attribute_id' )->where ( "main.{$idField} = :id" )->where ( 'main.entity_type_id = :entity_type_id' );
	 ...
	 *
	 * @param int|string $entityTypeId
	 *        	Entity Type id or Entity Type code
	 * @param int|string $id
	 *        	Attribute id or Attribute code
	 * @return string
	 */
	public function getAttributeTable($entityTypeId, $id) {
		...
		$select = $this->setup->getConnection ()->select ()->from ( [ 
				...
				'backend_type' 
		] )->where ( "entity_type.{$entityKeyName} = :entity_type_id" )->where ( "attribute.{$attributeKeyName} = :id" )->limit ( 1 );
	...
	public function getAttributeSortOrder($entityTypeId, $setId, $groupId, $sortOrder = null) {
		if (! is_numeric ( $sortOrder )) {
			$bind = [ 
					'attribute_group_id' => $this->getAttributeGroupId ( $entityTypeId, $setId, $groupId ) 
			];
			$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_entity_attribute' ), 'MAX(sort_order)' )->where ( 'attribute_group_id = :attribute_group_id' );
	...
	public function addAttributeToSet($entityTypeId, $setId, $groupId, $attributeId, $sortOrder = null) {
		...
		$select = $this->setup->getConnection ()->select ()->from ( $table )->where ( 'attribute_set_id = :attribute_set_id' )->where ( 'attribute_id = :attribute_id' );
		$result = $this->setup->getConnection ()->fetchRow ( $select, $bind );
		
		if ($result) {
			if ($result ['attribute_group_id'] != $groupId) {
				$where = [ 
						'entity_attribute_id =?' => $result ['entity_attribute_id'] 
				];
				$data = [ 
	...
	public function addAttributeToGroup($entityType, $setId, $groupId, $attributeId, $sortOrder = null) {
		...
		$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_entity_attribute' ) )->where ( 'entity_type_id = :entity_type_id' )->where ( 'attribute_set_id = :attribute_set_id' )->where ( 'attribute_id = :attribute_id' );
		$row = $this->setup->getConnection ()->fetchRow ( $select, $bind );
		if ($row) {
			// update
			if ($sortOrder !== null) {
			...
			$this->setup->getConnection ()->update ( $this->setup->getTable ( 'eav_entity_attribute' ), $data, $this->setup->getConnection ()->quoteInto ( 'entity_attribute_id=?', $row ['entity_attribute_id'] ) );
		} else {
			if ($sortOrder === null) {
				$select = $this->setup->getConnection ()->select ()->from ( $this->setup->getTable ( 'eav_entity_attribute' ), 'MAX(sort_order)' )->where ( 'entity_type_id = :entity_type_id' )->where ( 'attribute_set_id = :attribute_set_id' )->where ( 'attribute_id = :attribute_id' );
	...
	private function _insertAttribute(array $data) {
		...
		$this->_insertAttributeAdditionalData ( $data ['entity_type_id'], array_merge ( [ 
				'attribute_id' => $attributeId 
		], $data ) );
