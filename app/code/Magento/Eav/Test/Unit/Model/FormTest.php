...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_model = $this->getMockBuilder ( 'Magento\Eav\Model\Form' )->setMethods ( [ 
		...
		$this->_model->expects ( $this->any () )->method ( '_getFilteredFormAttributeCollection' )->will ( $this->returnValue ( $this->_attributes ) );
		
		$this->_entity = new \Magento\Framework\DataObject ( [ 
				'id' => 1,
				'attribute_visible_user' => 'abc' 
		] );
	...
	public function testValidateDataPassed($isValid, $expected, $messages = null) {
		$validator = $this->getMockBuilder ( 'Magento\Eav\Model\Validator\Attribute\Data' )->disableOriginalConstructor ()->setMethods ( [ 
				'isValid',
				'getMessages' 
		] )->getMock ();
