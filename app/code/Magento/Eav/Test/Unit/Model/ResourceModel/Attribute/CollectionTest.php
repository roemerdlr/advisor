...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->entityTypeMock = $this->getMock ( 'Magento\Eav\Model\Entity\Type', [ 
				'__wakeup' 
		], [ ], '', false );
		...
		$this->connectionMock->expects ( $this->any () )->method ( 'describeTable' )->will ( $this->returnvalueMap ( [ 
				[ 
						'some_main_table',
						null,
						[ 
	...
	public function initSelectDataProvider() {
		return [ 
				'main_table_expression' => [ 
						'col2',
						'1',
						'SELECT COUNT(DISTINCT main_table.attribute_id) FROM `some_main_table` AS `main_table`' . "\n" . ' INNER JOIN `some_extra_table` AS `additional_table`' . ' ON additional_table.attribute_id = main_table.attribute_id' . "\n" . ' LEFT JOIN `some_extra_table` AS `scope_table`' . ' ON scope_table.attribute_id = main_table.attribute_id' . ' AND scope_table.website_id = :scope_website_id' . ' WHERE (main_table.entity_type_id = :mt_entity_type_id)' . ' AND (IF(main_table.col2 IS NULL, scope_table.col2, main_table.col2) = 1)' 
				],
				'additional_table_expression' => [ 
						'col3',
						'2',
						'SELECT COUNT(DISTINCT main_table.attribute_id) FROM `some_main_table` AS `main_table`' . "\n" . ' INNER JOIN `some_extra_table` AS `additional_table`' . ' ON additional_table.attribute_id = main_table.attribute_id' . "\n" . ' LEFT JOIN `some_extra_table` AS `scope_table`' . ' ON scope_table.attribute_id = main_table.attribute_id' . ' AND scope_table.website_id = :scope_website_id' . ' WHERE (main_table.entity_type_id = :mt_entity_type_id)' . ' AND (IF(additional_table.col3 IS NULL, scope_table.col3, additional_table.col3) = 2)' 
				] 
		];
	}
}
