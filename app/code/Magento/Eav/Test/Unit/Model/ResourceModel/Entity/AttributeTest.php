...
class AttributeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->contextMock = $this->getMock ( '\Magento\Framework\Model\Context', [ 
				'getCacheManager',
				'getEventDispatcher',
				'getLogger',
				'getAppState',
	...
	public function testSaveOptionSystemAttribute() {
		...
		$connectionMock->expects ( $this->once () )->method ( 'insert' )->will ( $this->returnValueMap ( [ 
				[ 
						'eav_attribute',
						$attributeData,
						1 
		...
		$connectionMock->expects ( $this->once () )->method ( 'fetchRow' )->will ( $this->returnValueMap ( [ 
				[ 
						'SELECT `eav_attribute`.* FROM `eav_attribute` ' . 'WHERE (attribute_code="status") AND (entity_type_id="4")',
						$attributeData 
				] 
		] ) );
		$connectionMock->expects ( $this->once () )->method ( 'update' )->with ( 'eav_attribute', [ 
				'default_value' => 2 
		], [ 
				'attribute_id = ?' => null 
		] );
	...
	public function testSaveOptionNewUserDefinedAttribute() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'lastInsertId' )->will ( $this->returnValueMap ( [ 
				[ 
						'eav_attribute',
						123 
				],
		...
		$connectionMock->expects ( $this->once () )->method ( 'update' )->will ( $this->returnValueMap ( [ 
				[ 
						'eav_attribute',
						[ 
								'default_value' => '' 
		...
		$connectionMock->expects ( $this->once () )->method ( 'fetchRow' )->will ( $this->returnValueMap ( [ 
				[ 
						'SELECT `eav_attribute`.* FROM `eav_attribute` ' . 'WHERE (attribute_code="a_dropdown") AND (entity_type_id="4")',
						false 
				] 
		] ) );
		$connectionMock->expects ( $this->once () )->method ( 'delete' )->will ( $this->returnValueMap ( [ 
				[ 
						'eav_attribute_option_value',
						[ 
								'option_id = ?' => '' 
		...
		$connectionMock->expects ( $this->exactly ( 4 ) )->method ( 'insert' )->will ( $this->returnValueMap ( [ 
				[ 
						'eav_attribute',
						$attributeData,
						1 
	...
	protected function _prepareResourceModel() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'quote' )->will ( $this->returnValueMap ( [ 
				[ 
						123,
						123 
				],
		...
		$storeManager->expects ( $this->any () )->method ( 'getStores' )->with ( true )->will ( $this->returnValue ( [ 
		...
		$resourceModelMock = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute', [ 
