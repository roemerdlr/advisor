...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->cacheMock = $this->getMockBuilder ( 'Magento\Framework\App\CacheInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'load',
				'getFrontend',
				'save',
				'remove',
		...
		$this->typeFactoryMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\TypeFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->collectionFactoryMock = $this->getMockBuilder ( 'Magento\Eav\Model\ResourceModel\Entity\Type\CollectionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->stateMock = $this->getMockBuilder ( 'Magento\Framework\App\Cache\StateInterface' )->setMethods ( [ 
				'isEnabled',
				'setEnabled',
				'persist' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->universalFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Validator\UniversalFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->config = new Config ( $this->cacheMock, $this->typeFactoryMock, $this->collectionFactoryMock, $this->stateMock, $this->universalFactoryMock );
	}
	...
	public function testGetAttributeCache($cacheEnabled, $loadCalls, $cachedValue) {
		$attributeCollectionMock = $this->getMockBuilder ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'getData',
				'setEntityTypeFilter' 
		] )->getMock ();
		...
		$entityAttributeMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute' )->setMethods ( [ 
				'setData' 
		] )->disableOriginalConstructor ()->getMock ();
		$factoryCalls = [ 
				[ 
	...
	public function testClear() {
		$this->cacheMock->expects ( $this->once () )->method ( 'clean' )->with ( $this->equalTo ( [ 
