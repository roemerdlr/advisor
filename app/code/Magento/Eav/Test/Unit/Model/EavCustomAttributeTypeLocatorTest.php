...
class EavCustomAttributeTypeLocatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->attributeRepository = $this->getMock ( 'Magento\Eav\Model\AttributeRepository', [ 
				'get' 
		], [ ], '', false );
	}
	...
	public function getTypeDataProvider() {
		...
		$serviceBackendModelDataInterfaceMapData = [ 
				$serviceInterface => [ 
						$mediaBackEndModelClass => $mediaAttributeDataInterface 
				] 
		];
		...
		$attribute->expects ( $this->any () )->method ( 'getBackendModel' )->willReturn ( $mediaBackEndModelClass );
		
		$attributeNoBackendModel = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Eav\Attribute', [ 
				'getBackendModel',
				'getFrontendInput' 
		], [ ], '', false );
		
		$attributeNoBackendModel->expects ( $this->any () )->method ( 'getBackendModel' )->willReturn ( null );
		
		$attributeNoBackendModel->expects ( $this->any () )->method ( 'getFrontendInput' )->willReturn ( 'image' );
		...
		return [ 
				[ 
						...
						'serviceEntityTypeMapData' => [ 
								$serviceInterface => $eavEntityType 
						],
	...
	public function testGetAllServiceDataInterfaceEmpty() {
		$stringUtility = new \Magento\Framework\Stdlib\StringUtils ();
		$this->eavCustomAttributeTypeLocator = new EavCustomAttributeTypeLocator ( $this->attributeRepository, $stringUtility );
	...
	public function testGetAllServiceDataInterface() {
		$serviceBackendModelDataInterfaceMapData = [ 
				'ServiceA' => [ 
						'BackendA' => 'ServiceDataInterfaceA' 
				],
				'ServiceB' => [ 
						'BackendB' => 'ServiceDataInterfaceB',
						'BackendC' => 'ServiceDataInterfaceC' 
				],
				'ServiceC' => [ 
						'BackendD' => 'ServiceDataInterfaceD' 
				] 
		];
		$stringUtility = new \Magento\Framework\Stdlib\StringUtils ();
		...
		$this->assertEquals ( [ 
				'ServiceDataInterfaceA',
				'ServiceDataInterfaceB',
				'ServiceDataInterfaceC',
				'ServiceDataInterfaceD' 
