...
class GroupRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->groupResourceMock = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Group', [ 
				'delete',
				'__wakeup',
				'load',
				'save' 
		], [ ], '', false );
		$this->groupFactoryMock = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\GroupFactory', [ 
				'create' 
		], [ ], '', false );
		$this->setRepositoryMock = $this->getMock ( '\Magento\Eav\Api\AttributeSetRepositoryInterface' );
		$this->searchResultsFactoryMock = $this->getMock ( '\Magento\Eav\Api\Data\AttributeGroupSearchResultsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->groupListFactoryMock = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Group\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $objectManager->getObject ( 'Magento\Eav\Model\Attribute\GroupRepository', [ 
	...
	public function testSaveThrowExceptionIfAttributeSetDoesNotExist() {
		$attributeSetId = - 1;
		$groupMock = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\Group', [ 
				'getAttributeSetId' 
		], [ ], '', false );
		$groupMock->expects ( $this->exactly ( 2 ) )->method ( 'getAttributeSetId' )->willReturn ( $attributeSetId );
		$this->setRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $attributeSetId )->will ( $this->throwException ( new \Magento\Framework\Exception\NoSuchEntityException ( __ ( 'AttributeSet does not exist.' ) ) ) );
	...
	public function testSaveThrowExceptionIfCannotSaveGroup() {
		...
		$existingGroupMock->expects ( $this->any () )->method ( 'getId' )->willReturn ( $groupId );
		$existingGroupMock->expects ( $this->once () )->method ( 'getAttributeSetId' )->willReturn ( $attributeSetId );
		$this->groupResourceMock->expects ( $this->once () )->method ( 'save' )->will ( $this->throwException ( new \Exception () ) );
		$this->model->save ( $groupMock );
	}
	...
	public function testGetList() {
		...
		$groupCollectionMock = $this->getMock ( '\Magento\Eav\Model\Entity\Collection\AbstractCollection', [ 
				'setAttributeSetFilter',
				'setSortOrder',
				'getItems',
				'getSize' 
		], [ ], '', false );
		$groupCollectionMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$groupMock 
		] );
		...
		$searchResultsMock = $this->getMock ( '\Magento\Eav\Api\Data\AttributeGroupSearchResultsInterface', [ ], [ ], '', false );
	...
	public function testGetListWithNoSuchEntityException() {
		...
		$searchCriteriaMock->expects ( $this->once () )->method ( 'getFilterGroups' )->willReturn ( [ ] );
		$this->setRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $attributeSetId )->willThrowException ( new \Exception () );
		$this->model->getList ( $searchCriteriaMock );
	}
	...
	public function testDeleteThrowExceptionIfProvidedGroupDoesNotExist() {
		$groupMock = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\Group', [ ], [ ], '', false );
		$this->groupResourceMock->expects ( $this->once () )->method ( 'delete' )->with ( $groupMock )->will ( $this->throwException ( new \Exception () ) );
