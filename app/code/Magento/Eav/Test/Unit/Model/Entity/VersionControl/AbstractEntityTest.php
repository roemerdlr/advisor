...
class AbstractEntityTest extends \Magento\Eav\Test\Unit\Model\Entity\AbstractEntityTest {
	...
	protected function setUp() {
		$this->entitySnapshot = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\VersionControl\Snapshot', [ 
				'isModified',
				'registerSnapshot' 
		], [ ], '', false );
		
		$this->entityRelationComposite = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\VersionControl\RelationComposite', [ 
				'processRelations' 
		], [ ], '', false );
	...
	public function testSave($attributeCode, $attributeSetId, $productData, $productOrigData) {
		...
		$arguments = $objectManager->getConstructArguments ( 'Magento\Eav\Model\Entity\VersionControl\AbstractEntity', [ 
				'eavConfig' => $eavConfig,
		...
		$model = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\VersionControl\AbstractEntity' )->setConstructorArgs ( $arguments )->setMethods ( [ 
				'_getValue',
				'beginTransaction',
				'commit',
				'rollback',
