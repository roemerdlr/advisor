...
class AbstractEntityTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$arguments = $objectManager->getConstructArguments ( 'Magento\Eav\Model\Entity\AbstractEntity', [ 
				'eavConfig' => $this->eavConfig 
		] );
		$this->_model = $this->getMockForAbstractClass ( 'Magento\Eav\Model\Entity\AbstractEntity', $arguments );
	}
	...
	public function testCompareAttributes($attribute1Sort, $attribute2Sort, $expected) {
		$attribute1 = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute', [ 
				'__wakeup' 
		], [ ], '', false );
		$attribute1->setAttributeSetInfo ( [ 
				0 => $attribute1Sort 
		] );
		$attribute2 = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute', [ 
				'__wakeup' 
		], [ ], '', false );
		$attribute2->setAttributeSetInfo ( [ 
				0 => $attribute2Sort 
		] );
	...
	protected function _getAttributes() {
		...
		foreach ( $codes as $code ) {
			...
			$backendModel = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute\Backend\AbstractBackend', [ 
					'getBackend',
					'getBackendTable' 
			] );
	...
	protected function _getConnectionMock() {
		$connection = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'describeTable',
				'lastInsertId',
				'insert',
				'prepareColumnValue',
		...
		$connection->expects ( $this->any () )->method ( 'describeTable' )->will ( $this->returnValue ( [ 
				'value' => [ 
						'test' 
				] 
		] ) );
		...
		$connection->expects ( $this->once () )->method ( 'delete' )->with ( $this->equalTo ( 'test_table' ) )->will ( $this->returnValue ( true ) );
	...
	protected function _getAttributeMock($attributeCode, $attributeSetId) {
		...
		$attribute->expects ( $this->any () )->method ( 'getBackendTable' )->will ( $this->returnValue ( $attributeCode . '_table' ) );
		
		$attribute->expects ( $this->any () )->method ( 'isInSet' )->with ( $this->equalTo ( $attributeSetId ) )->will ( $this->returnValue ( false ) );
	...
	public function testSave($attributeCode, $attributeSetId, $productData, $productOrigData) {
		$object = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getOrigData',
				'__wakeup',
				'beforeSave',
				'afterSave',
		...
		$backendModel->expects ( $this->once () )->method ( 'getAffectedFields' )->will ( $this->returnValue ( [ 
				'test_table' => [ 
						[ 
								'value_id' => 0,
		...
		$model = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\AbstractEntity' )->setConstructorArgs ( $arguments )->setMethods ( [ 
				'_getValue',
				'beginTransaction',
				'commit',
				'rollback',
