...
class OptionManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resourceModelMock = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute', [ ], [ ], '', false );
		$this->model = new \Magento\Eav\Model\Entity\Attribute\OptionManagement ( $this->attributeRepositoryMock, $this->resourceModelMock );
	}
	public function testAdd() {
		$entityType = 42;
		$attributeCode = 'atrCde';
		$optionMock = $this->getMockForAbstractClass ( '\Magento\Eav\Api\Data\AttributeOptionInterface', [ ], '', false, false, true, [ 
				'getSourceLabels' 
		] );
		$attributeMock = $this->getMockForAbstractClass ( '\Magento\Framework\Model\AbstractModel', [ ], '', false, false, true, [ 
				'usesSource',
				'setDefault',
				'setOption' 
		] );
		...
		$this->attributeRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $entityType, $attributeCode )->willReturn ( $attributeMock );
		...
		$optionMock->expects ( $this->exactly ( 2 ) )->method ( 'getStoreLabels' )->willReturn ( [ 
				$labelMock 
		] );
		...
		$attributeMock->expects ( $this->once () )->method ( 'setDefault' )->with ( [ 
				'new_option' 
		] );
	...
	public function testAddWithCannotSaveException() {
		...
		$attributeMock->expects ( $this->once () )->method ( 'setOption' )->with ( $option );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'save' )->with ( $attributeMock )->willThrowException ( new \Exception () );
		$this->model->add ( $entityType, $attributeCode, $optionMock );
	}
	...
	public function testGetItems() {
		...
		$optionsMock = [ 
				$this->getMock ( '\Magento\Eav\Api\Data\AttributeOptionInterface' ) 
		];
		$this->attributeRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $entityType, $attributeCode )->willReturn ( $attributeMock );
