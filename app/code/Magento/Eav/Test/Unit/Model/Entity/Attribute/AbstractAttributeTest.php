...
class AbstractAttributeTest extends \PHPUnit_Framework_TestCase {
	public function testGetOptionWhenOptionsAreSet() {
		$model = $this->getMockForAbstractClass ( '\Magento\Eav\Model\Entity\Attribute\AbstractAttribute', [ ], '', false, false, true, [ 
				'getData',
				'usesSource',
				'getSource',
				'convertToObjects' 
		...
		$model->expects ( $this->once () )->method ( 'getData' )->with ( \Magento\Eav\Model\Entity\Attribute\AbstractAttribute::OPTIONS )->willReturn ( [ 
				'options' 
		] );
		$model->expects ( $this->never () )->method ( 'usesSource' );
		$model->expects ( $this->once () )->method ( 'convertToObjects' )->with ( [ 
				'options' 
	...
	public function testGetOptionWhenOptionsAreEmptyWithoutSource() {
		...
		$model->expects ( $this->once () )->method ( 'convertToObjects' )->with ( [ ] )->willReturn ( 'expected value' );
	...
	public function testGetOptionWhenOptionsAreEmptyWithSource() {
		...
		$model->expects ( $this->once () )->method ( 'getAllOptions' )->willReturn ( [ 
				'source value' 
		] );
		$model->expects ( $this->once () )->method ( 'convertToObjects' )->with ( [ 
				'source value' 
	...
	public function testConvertToObjects() {
		...
		$dataFactoryMock = $this->getMock ( 'Magento\Eav\Api\Data\AttributeOptionInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$dataObjectHelperMock = $this->getMockBuilder ( '\Magento\Framework\Api\DataObjectHelper' )->disableOriginalConstructor ()->getMock ();
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$model = $objectManagerHelper->getObject ( '\Magento\Catalog\Model\Entity\Attribute', [ 
		...
		$dataObjectHelperMock->expects ( $this->once () )->method ( 'populateWithArray' )->with ( $attributeOptionMock, [ 
				'some value' 
		], '\Magento\Eav\Api\Data\AttributeOptionInterface' )->willReturnSelf ();
