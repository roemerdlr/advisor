...
class AttributeManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->setRepositoryMock = $this->getMock ( 'Magento\Eav\Api\AttributeSetRepositoryInterface', [ ], [ ], '', false );
		$this->attributeCollectionMock = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Collection', [ ], [ ], '', false );
		$this->eavConfigMock = $this->getMock ( 'Magento\Eav\Model\Config', [ ], [ ], '', false );
		$this->entityTypeFactoryMock = $this->getMock ( 'Magento\Eav\Model\ConfigFactory', [ 
				'create',
				'__wakeup' 
		], [ ], '', false );
		$this->groupRepositoryMock = $this->getMock ( 'Magento\Eav\Api\AttributeGroupRepositoryInterface', [ ], [ ], '', false );
		$this->attributeRepositoryMock = $this->getMock ( 'Magento\Eav\Api\AttributeRepositoryInterface', [ ], [ ], '', false );
		$this->attributeResourceMock = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute', [ ], [ ], '', false );
		
		$this->model = new \Magento\Eav\Model\AttributeManagement ( $this->setRepositoryMock, $this->attributeCollectionMock, $this->eavConfigMock, $this->entityTypeFactoryMock, $this->groupRepositoryMock, $this->attributeRepositoryMock, $this->attributeResourceMock );
	}
	...
	public function testAssignNoSuchEntityException() {
		...
		$this->setRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $attributeSetId )->will ( $this->throwException ( new \Magento\Framework\Exception\NoSuchEntityException () ) );
	...
	public function testAssignInputException() {
		...
		$this->setRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $attributeSetId )->willReturn ( $attributeSetMock );
	...
	public function testAssignInputExceptionGroupInSet() {
		...
		$attributeGroup = $this->getMockBuilder ( 'Magento\Eav\Api\Data\AttributeGroupInterface' )->setMethods ( [ 
				'getAttributeSetId' 
	...
	public function testAssign() {
		...
		$this->attributeRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $entityTypeCode, $attributeCode )->willReturn ( $attributeMock );
		$attributeMock->expects ( $this->once () )->method ( 'getAttributeId' )->willReturn ( 16 );
		$this->attributeResourceMock->expects ( $this->once () )->method ( 'saveInSetIncluding' )->with ( $attributeMock, 16, $attributeSetId, $attributeGroupId, $sortOrder )->willReturn ( $attributeMock );
		...
		$this->assertEquals ( $attributeMock, $this->model->assign ( $entityTypeCode, $attributeSetId, $attributeGroupId, $attributeCode, $sortOrder ) );
	}
	public function testUnassign() {
		...
		$entityTypeMock = $this->getMock ( 'Magento\Eav\Model\Entity\Type', [ ], [ ], '', false );
		$this->eavConfigMock->expects ( $this->once () )->method ( 'getEntityType' )->with ( 66 )->willReturn ( $entityTypeMock );
		$attributeMock = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute', [ 
		...
		$this->attributeRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( 'entity type code', $attributeCode )->willReturn ( $attributeMock );
	...
	public function testUnassignWithWrongAttributeSet() {
		...
		$this->setRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $attributeSetId )->willThrowException ( new NoSuchEntityException ( __ ( 'hello' ) ) );
	...
	public function testGetAttributes() {
		...
		$this->setRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $attributeSetId )->willReturn ( $attributeSetMock );
		$entityTypeMock = $this->getMock ( 'Magento\Eav\Model\Entity\Type', [ ], [ ], '', false );
		$this->eavConfigMock->expects ( $this->once () )->method ( 'getEntityType' )->with ( $entityType )->willReturn ( $entityTypeMock );
		...
		$this->attributeCollectionMock->expects ( $this->once () )->method ( 'setAttributeSetFilter' )->with ( 88 )->willReturnSelf ();
		...
		$this->attributeCollectionMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$attributeMock 
		] );
