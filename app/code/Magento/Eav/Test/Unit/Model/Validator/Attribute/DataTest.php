...
class DataTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * @param array $data        	
	 */
	public function testIsValid($attributeData, $result, $expected, $messages, $data = ['attribute' => 'new_test']) {
		$entity = $this->_getEntityMock ();
		$attribute = $this->_getAttributeMock ( $attributeData );
		$attrDataFactory = $this->getMock ( 'Magento\Eav\Model\AttributeDataFactory', [ 
				'create' 
		], [ 
		...
		if ($attribute->getDataModel () || $attribute->getFrontendInput ()) {
			...
			$attrDataFactory->expects ( $this->any () )->method ( 'create' )->with ( $attribute, $entity )->will ( $this->returnValue ( $dataModel ) );
		}
	...
	public function testIsValidAttributesFromCollection() {
		...
		$collection = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getItems' 
		] )->getMock ();
		$collection->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$attribute 
		] ) );
		$entityType = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
		...
		$attrDataFactory->expects ( $this->once () )->method ( 'create' )->with ( $attribute, $entity )->will ( $this->returnValue ( $dataModel ) );
		$validator = new \Magento\Eav\Model\Validator\Attribute\Data ( $attrDataFactory );
	...
	public function whiteBlackListProvider() {
		$whiteCallback = function ($validator) {
			$validator->setAttributesWhiteList ( [ 
					'attribute' 
			] );
		};
		
		$blackCallback = function ($validator) {
			$validator->setAttributesBlackList ( [ 
					'attribute2' 
			] );
		};
		return [ 
	...
	public function testAddErrorMessages() {
		...
		$firstAttribute = $this->_getAttributeMock ( [ 
				...
				'data_model' => $firstDataModel = $this->_getDataModelMock ( [ 
						'Error1' 
				] ),
				'frontend_input' => 'text',
				'is_visible' => true 
		...
		$secondAttribute = $this->_getAttributeMock ( [ 
				'attribute_code' => 'attribute2',
				'data_model' => $secondDataModel = $this->_getDataModelMock ( [ 
						'Error2' 
				] ),
				'frontend_input' => 'text',
				'is_visible' => true 
		...
		$factory = $this->getMock ( 'Magento\Eav\Model\AttributeDataFactory', [ 
				'create' 
		], [ 
				'objectManager' => $this->getMock ( 'Magento\Framework\ObjectManagerInterface' ),
				'string' => $this->getMock ( 'Magento\Framework\Stdlib\StringUtils' ) 
		...
		$factory->expects ( $this->at ( 0 ) )->method ( 'create' )->with ( $firstAttribute, $entity )->will ( $this->returnValue ( $firstDataModel ) );
		$factory->expects ( $this->at ( 1 ) )->method ( 'create' )->with ( $secondAttribute, $entity )->will ( $this->returnValue ( $secondDataModel ) );
		$factory->expects ( $this->at ( 2 ) )->method ( 'create' )->with ( $firstAttribute, $entity )->will ( $this->returnValue ( $firstDataModel ) );
		$factory->expects ( $this->at ( 3 ) )->method ( 'create' )->with ( $secondAttribute, $entity )->will ( $this->returnValue ( $secondDataModel ) );
	...
	protected function _getAttributeMock($attributeData) {
		$attribute = $this->getMockBuilder ( 'Magento\Eav\Model\Attribute' )->setMethods ( [ 
				'getAttributeCode',
				'getDataModel',
				'getFrontendInput',
				'__wakeup',
		...
		if (isset ( $attributeData ['data_model'] )) {
			$attribute->expects ( $this->any () )->method ( 'getDataModel' )->will ( $this->returnValue ( $attributeData ['data_model'] ) );
		}
		if (isset ( $attributeData ['frontend_input'] )) {
			$attribute->expects ( $this->any () )->method ( 'getFrontendInput' )->will ( $this->returnValue ( $attributeData ['frontend_input'] ) );
		}
		if (isset ( $attributeData ['is_visible'] )) {
			$attribute->expects ( $this->any () )->method ( 'getIsVisible' )->willReturn ( $attributeData ['is_visible'] );
		}
		return $attribute;
	}
	...
	protected function _getDataModelMock($returnValue, $argument = null) {
		$dataModel = $this->getMockBuilder ( 'Magento\Eav\Model\Attribute\Data\AbstractData' )->disableOriginalConstructor ()->setMethods ( [ 
				'validateValue' 
		] )->getMockForAbstractClass ();
		if ($argument) {
			$dataModel->expects ( $this->once () )->method ( 'validateValue' )->with ( $argument )->will ( $this->returnValue ( $returnValue ) );
		} else {
	...
	protected function _getEntityMock() {
		$entity = $this->getMockBuilder ( 'Magento\Framework\Model\AbstractModel' )->setMethods ( [ 
				'getAttribute',
				'getResource',
				'getEntityType',
				'__wakeup' 
