...
class BackendTest extends \PHPUnit_Framework_TestCase {
	...
	public function testisValidIfProvidedModelIsIncorrect() {
		$this->model->isValid ( $this->getMock ( '\Magento\Framework\DataObject', [ ], [ ], '', false ) );
	}
	...
	public function testisValidIfAttributeValueNotValid() {
		...
		$resourceMock->expects ( $this->once () )->method ( 'getAttributesByCode' )->willReturn ( [ 
				$attributeMock 
		] );
		...
		$backendMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->entityMock )->willThrowException ( new \Magento\Framework\Exception\LocalizedException ( $exceptionMessage ) );
