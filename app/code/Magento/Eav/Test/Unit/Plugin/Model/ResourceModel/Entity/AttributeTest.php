...
class AttributeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetStoreLabelsByAttributeIdOnCacheDisabled() {
		...
		$this->assertEquals ( 'attributeId', $this->getAttribute ( false )->aroundGetStoreLabelsByAttributeId ( $this->subject, $this->mockPluginProceed ( 'attributeId' ), 'attributeId' ) );
	}
	public function testGetStoreLabelsByAttributeIdFromCache() {
		...
		$this->assertEquals ( $attributes, $this->getAttribute ( true )->aroundGetStoreLabelsByAttributeId ( $this->subject, $this->mockPluginProceed (), $attributeId ) );
	}
	public function testGetStoreLabelsByAttributeIdWithCacheSave() {
		...
		$this->cache->expects ( $this->any () )->method ( 'save' )->with ( serialize ( [ 
				$attributeId 
		] ), $cacheId, [ 
		...
		$this->assertEquals ( [ 
				$attributeId 
		], $this->getAttribute ( true )->aroundGetStoreLabelsByAttributeId ( $this->subject, $this->mockPluginProceed ( [ 
				$attributeId 
		] ), $attributeId ) );
	}
	...
	protected function getAttribute($cacheEnabledFlag) {
		...
		return (new ObjectManager ( $this ))->getObject ( 'Magento\Eav\Plugin\Model\ResourceModel\Entity\Attribute', [ 
				'cache' => $this->cache,
