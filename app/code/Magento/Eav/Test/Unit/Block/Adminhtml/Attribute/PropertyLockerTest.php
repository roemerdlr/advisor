...
class PropertyLockerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->attributeMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute' )->setMethods ( [ 
				'getId' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$registryMock = $this->getMockBuilder ( '\Magento\Framework\Registry' )->setMethods ( [ 
				'registry' 
		] )->disableOriginalConstructor ()->getMock ();
		$registryMock->expects ( $this->atLeastOnce () )->method ( 'registry' )->willReturn ( $this->attributeMock );
		
		$this->attributeConfigMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute\Config' )->setMethods ( [ 
				'getLockedFields' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->formMock = $this->getMockBuilder ( 'Magento\Framework\Data\Form' )->setMethods ( [ 
				'getElement' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testLock() {
		...
		$elementMock = $this->getMockBuilder ( '\Magento\Framework\Data\Form\Element\AbstractElement' )->setMethods ( [ 
				'setDisabled',
				'setReadonly' 
