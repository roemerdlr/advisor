...
class Config {
	 ...
	 * Get cache interface
	 *
	 * @return \Magento\Framework\App\CacheInterface @codeCoverageIgnore
	 */
	...
	protected function _initEntityTypes() {
		...
		\Magento\Framework\Profiler::start ( 'EAV: ' . __METHOD__, [ 
				'group' => 'EAV',
				'method' => __METHOD__ 
		] );
		...
		if ($this->isCacheEnabled ()) {
			$this->_cache->save ( serialize ( $this->_entityTypeData ), self::ENTITIES_CACHE_ID, [ 
					\Magento\Eav\Model\Cache\Type::CACHE_TAG,
					\Magento\Eav\Model\Entity\Attribute::CACHE_TAG 
			] );
		}
	...
	protected function _initAttributes($entityType) {
		...
		$attributes = $this->_universalFactory->create ( $entityType->getEntityAttributeCollection () )->setEntityTypeFilter ( $entityType )->getData ();
		...
		foreach ( $attributes as $attribute ) {
			...
			$this->_attributeData [$entityTypeCode] [$attribute ['attribute_code']] = $attributeObject->load ( $attributeObject->getId () )->toArray ();
		}
		if ($this->isCacheEnabled ()) {
			$this->_cache->save ( serialize ( $this->_attributeData [$entityTypeCode] ), $cacheKey, [ 
					\Magento\Eav\Model\Cache\Type::CACHE_TAG,
					\Magento\Eav\Model\Entity\Attribute::CACHE_TAG 
			] );
		}
	...
	public function getAttribute($entityType, $code) {
		...
		if (! $attribute) {
			...
			if ($entity && in_array ( $attribute->getAttributeCode (), $entity->getDefaultAttributes () )) {
				$attribute->setBackendType ( \Magento\Eav\Model\Entity\Attribute\AbstractAttribute::TYPE_STATIC )->setIsGlobal ( 1 );
			}
	...
	public function getEntityAttributeCodes($entityType, $object = null) {
		...
		if ($this->isCacheEnabled ()) {
			$this->_cache->save ( serialize ( $attributes ), $cacheKey, [ 
					\Magento\Eav\Model\Cache\Type::CACHE_TAG,
					\Magento\Eav\Model\Entity\Attribute::CACHE_TAG 
			] );
		}
