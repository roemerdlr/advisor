...
abstract class AbstractEntity extends AbstractResource implements EntityInterface, DefaultAttributesProvider {
	 ...
	 * Get connection
	 *
	 * @return \Magento\Framework\DB\Adapter\AdapterInterface @codeCoverageIgnore
	 */
	 ...
	 * @return AbstractAttribute|false @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	public function getAttribute($attribute) {
		...
		if (is_numeric ( $attribute )) {
			...
			if (! $attributeInstance->getAttributeCode () && in_array ( $attribute, $this->getDefaultAttributes () )) {
				$attributeInstance->setAttributeCode ( $attribute )->setBackendType ( AbstractAttribute::TYPE_STATIC )->setIsGlobal ( 1 )->setEntity ( $this )->setEntityType ( $this->getEntityType () )->setEntityTypeId ( $this->getEntityType ()->getId () );
			}
		...
		if (empty ( $attributeInstance ) || ! $attributeInstance instanceof AbstractAttribute || ! $attributeInstance->getId () && ! in_array ( $attributeInstance->getAttributeCode (), $this->getDefaultAttributes () )) {
			return false;
		}
	...
	protected function _getDefaultAttribute($attributeCode) {
		...
		if (! isset ( self::$_defaultAttributes [$entityTypeId] [$attributeCode] )) {
			$attribute = $this->_universalFactory->create ( $this->getEntityType ()->getAttributeModel () )->setAttributeCode ( $attributeCode )->setBackendType ( AbstractAttribute::TYPE_STATIC )->setIsGlobal ( 1 )->setEntityType ( $this->getEntityType () )->setEntityTypeId ( $this->getEntityType ()->getId () );
	...
	public function walkAttributes($partMethod, array $args = [], $collectExceptionMessages = null) {
		...
		foreach ( $this->getAttributesByCode () as $attrCode => $attribute ) {
			...
			try {
				$results [$attrCode] = call_user_func_array ( [ 
						$instance,
						$method 
				], $args );
				...
				if ($collectExceptionMessages) {
					...
					$e = $this->_universalFactory->create ( 'Magento\Eav\Model\Entity\Attribute\Exception', [ 
	...
	protected function _isCallableAttributeInstance($instance, $method, $args) {
		if (! is_object ( $instance ) || ! method_exists ( $instance, $method ) || ! is_callable ( [ 
				$instance,
				$method 
		] )) {
			return false;
	...
	public function validate($object) {
		...
		$result = $this->walkAttributes ( 'backend/validate', [ 
				$object 
		], $object->getCollectExceptionMessages () );
		$errors = [ ];
	...
	public function checkAttributeUniqueValue(AbstractAttribute $attribute, $object) {
		...
		if ($attribute->getBackend ()->getType () === 'static') {
			...
			$select->from ( $this->getEntityTable (), $this->getEntityIdField () )->where ( $attribute->getAttributeCode () . ' = :attribute_code' );
		} else {
			$value = $object->getData ( $attribute->getAttributeCode () );
			...
			$select->from ( $attribute->getBackend ()->getTable (), $attribute->getBackend ()->getEntityIdField () )->where ( 'attribute_id = :attribute_id' )->where ( 'value = :value' );
		}
	 ...
	 * @return \Magento\Framework\DB\Select @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	protected function _getLoadRowSelect($object, $rowId) {
		$select = $this->getConnection ()->select ()->from ( $this->getEntityTable () )->where ( $this->getEntityIdField () . ' =?', $rowId );
	...
	protected function _getLoadAttributesSelect($object, $table) {
		$select = $this->getConnection ()->select ()->from ( $table, [ ] )->where ( $this->getEntityIdField () . ' =?', $object->getId () );
...
;
	 ...
	 * @param AbstractAttribute $attribute        	
	 * @param mixed $v
	 *        	New value of the attribute. Can be used in subclasses.
	...
	protected function _getStaticFieldProperties($field) {
		if (empty ( $this->_describeTable [$this->getEntityTable ()] )) {
			$this->_describeTable [$this->getEntityTable ()] = $this->getConnection ()->describeTable ( $this->getEntityTable () );
		}
	 ...
	 *        	array('newObject', 'entityRow', 'insert', 'update', 'delete')
	 ...
	 * @param string $table        	
	 * @param array $info        	
	 * @return \Magento\Framework\DataObject @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	 ...
	 * @return $this
	 * @throws \Exception @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	...
	protected function _afterLoad(\Magento\Framework\DataObject $object) {
		...
		$this->walkAttributes ( 'backend/afterLoad', [ 
				$object 
		] );
	...
	protected function _beforeSave(\Magento\Framework\DataObject $object) {
		$this->walkAttributes ( 'backend/beforeSave', [ 
				$object 
		] );
		return $this;
	}
	...
	protected function _afterSave(\Magento\Framework\DataObject $object) {
		$this->walkAttributes ( 'backend/afterSave', [ 
				$object 
		] );
		return $this;
	}
	...
	protected function _beforeDelete(\Magento\Framework\DataObject $object) {
		$this->walkAttributes ( 'backend/beforeDelete', [ 
				$object 
		] );
		return $this;
	}
	...
	protected function _afterDelete(\Magento\Framework\DataObject $object) {
		$this->walkAttributes ( 'backend/afterDelete', [ 
				$object 
		] );
		return $this;
	}
	...
	public function getDefaultAttributes() {
		return array_unique ( array_merge ( $this->_getDefaultAttributes (), [ 
