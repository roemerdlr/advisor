...
abstract class AbstractCollection extends AbstractDb implements SourceProviderInterface {
	 ...
	 *
	 * @return \Magento\Framework\Model\ResourceModel\Db\AbstractDb @codeCoverageIgnore
	 */
	...
	public function addAttributeToFilter($attribute, $condition = null, $joinType = 'inner') {
		...
		if (is_numeric ( $attribute )) {
			...
			if (! $attributeModel) {
				throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Invalid attribute identifier for filter (%1)', get_class ( $attribute ) ) );
			}
	 ...
	 * @return $this|AbstractDb @codeCoverageIgnore
	 */
	 ...
	 * @param string $alias
	 *        	alias for the joined attribute
	 * @param string|\Magento\Eav\Model\Entity\Attribute\AbstractAttribute $attribute        	
	 * @param string $bind
	 *        	attribute of the main entity to link with joined $filter
	 * @param string $filter
	 *        	primary key for the joined entity (entity_id default)
	 * @param string $joinType
	 *        	inner|left
	 * @param null $storeId        	
	 * @return $this
	 ...
	 *        	'country_name'
	 * @param string $table
	 *        	'directory_country_name'
	 * @param string $field
	 *        	'name'
	 * @param string $bind
	 *        	'PK(country_id)=FK(shipping_country_id)'
	 * @param string|array $cond
	 *        	"{{table}}.language_code='en'" OR array('language_code'=>'en')
	 * @param string $joinType
	 *        	'left'
	 * @return $this
	 * @throws LocalizedException
	...
	public function load($printQuery = false, $logQuery = false) {
		...
		$this->_eventManager->dispatch ( 'eav_collection_abstract_load_before', [ 
				'collection' => $this 
		] );
		$this->_beforeLoad ();
	...
	protected function _getLoadAttributesSelect($table, $attributeIds = []) {
		...
		$select = $this->getConnection ()->select ()->from ( $table, [ 
				$entityIdField,
				'attribute_id' 
		] )->where ( "{$entityIdField} IN (?)", array_keys ( $this->_itemsById ) )->where ( 'attribute_id IN (?)', $attributeIds );
		
		if ($entity->getEntityTable () == \Magento\Eav\Model\Entity::DEFAULT_ENTITY_TABLE && $entity->getTypeId ()) {
			$select->where ( 'entity_type_id =?', $entity->getTypeId () );
		}
		return $select;
	}
	 ...
	 * @param string $table        	
	 * @param string $type        	
	 * @return Select @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 *         @codeCoverageIgnore
	 */
	...
	protected function _setItemAttributeValue($valueInfo) {
		...
		if (! $attributeCode) {
			$attribute = $this->_eavConfig->getAttribute ( $this->getEntity ()->getType (), $valueInfo ['attribute_id'] );
	 ...
	 *        	inner|left
	 * @return $this
	 * @throws LocalizedException @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	protected function _addAttributeJoin($attributeCode, $joinType = 'inner') {
		...
		if (! $attribute->getBackend ()->isStatic ()) {
			$condArr [] = $this->getConnection ()->quoteInto ( $connection->quoteColumnAs ( "{$attrTable}.attribute_id", null ) . ' = ?', $attribute->getId () );
		}
	 ...
	 * @param string $alias
	 *        	@SuppressWarnings(PHPMD.UnusedFormalParameter)
	 * @return $this|\Magento\Framework\Data\Collection\AbstractDb @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 *         @codeCoverageIgnore
	 */
	 ...
	 *
	 * @param string $field        	
	 * @return $this|\Magento\Framework\Data\Collection\AbstractDb @codeCoverageIgnore
	 */
