...
abstract class AbstractAttribute extends \Magento\Framework\Model\AbstractExtensibleModel implements AttributeInterface, \Magento\Eav\Api\Data\AttributeInterface {
	...
	/**
	 *
	 * @return string|int|bool|float @codeCoverageIgnore
	 */
	 ...
	 *
	 * @return \Magento\Eav\Model\Entity\Type @codeCoverageIgnore
	 */
	...
	public function isValueEmpty($value) {
		...
		$isEmpty = (is_array ( $value ) && count ( $value ) == 0) || $value === null || $value === false && $attrType != 'int' || $value === '' && ($attrType == 'int' || $attrType == 'decimal' || $attrType == 'datetime');
		...
		if ($this->_dataTable === null) {
			if ($this->isStatic ()) {
				...
				if (empty ( $backendTable )) {
					$entityTable = [ 
							$this->getEntity ()->getEntityTablePrefix (),
							$this->getBackendType () 
					];
	...
	public function setOptions(array $options = null) {
		if ($options !== null) {
			...
			foreach ( $options as $option ) {
				$optionData = $this->dataObjectProcessor->buildOutputDataArray ( $option, '\Magento\Eav\Api\Data\AttributeOptionInterface' );
	 ...
	 * {@inheritdoc}
	 *
	 * @return \Magento\Eav\Api\Data\AttributeExtensionInterface|null @codeCoverageIgnore
	 */
