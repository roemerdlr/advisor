...
class Fieldset extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function getLabels($object) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'eav_form_fieldset_label' ), [ 
				'store_id',
				'label' 
	...
	protected function _getLoadSelect($field, $value, $object) {
		...
		], $this->getMainTable () . '.fieldset_id = default_label.fieldset_id AND default_label.store_id=0', [ ] )->joinLeft ( [ 
				'store_label' => $this->getTable ( 'eav_form_fieldset_label' ) 
		], $this->getMainTable () . '.fieldset_id = store_label.fieldset_id AND default_label.store_id=' . ( int ) $object->getStoreId (), [ 
				'label' => $labelExpr 
		] );
