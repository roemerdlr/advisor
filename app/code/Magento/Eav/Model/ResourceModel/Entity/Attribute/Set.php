...
class Set extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 *        	@codeCoverageIgnore
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Eav\Model\ResourceModel\Entity\Attribute\GroupFactory $attrGroupFactory, \Magento\Eav\Model\Config $eavConfig, $connectionName = null) {
	...
	public function validate($object, $attributeSetName) {
		...
		$select = $connection->select ()->from ( $this->getMainTable () )->where ( 'attribute_set_name = :attribute_set_name' )->where ( 'entity_type_id = :entity_type_id' );
	...
	public function getSetInfo(array $attributeIds, $setId = null) {
		...
		if ($this->eavConfig->isCacheEnabled () && ($cache = $this->eavConfig->getCache ()->load ( $cacheKey ))) {
			...
			if ($this->eavConfig->isCacheEnabled ()) {
				$this->eavConfig->getCache ()->save ( serialize ( $setInfoData ), $cacheKey, [ 
	...
	public function getDefaultGroupId($setId) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'eav_attribute_group' ), 'attribute_group_id' )->where ( 'attribute_set_id = :attribute_set_id' )->where ( 'default_id = 1' )->limit ( 1 );
