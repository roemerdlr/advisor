...
class Group extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function itemExists($object) {
		...
		$select = $connection->select ()->from ( $this->getMainTable () )->where ( 'attribute_set_id = :attribute_set_id' )->where ( 'attribute_group_name = :attribute_group_name' );
	...
	protected function _getMaxSortOrder($object) {
		...
		$select = $connection->select ()->from ( $this->getMainTable (), new \Zend_Db_Expr ( "MAX(sort_order)" ) )->where ( 'attribute_set_id = :attribute_set_id' );
	...
	public function updateDefaultGroup($attributeSetId) {
		...
		$select = $connection->select ()->from ( $this->getMainTable (), $this->getIdFieldName () )->where ( 'attribute_set_id = :attribute_set_id' )->order ( 'default_id ' . \Magento\Framework\Data\Collection::SORT_ORDER_DESC )->limit ( 1 );
