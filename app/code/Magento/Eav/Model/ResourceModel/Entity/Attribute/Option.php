...
class Option extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function addOptionValueToCollection($collection, $attribute, $valueExpr) {
		...
		$tableJoinCond2 = $connection->quoteInto ( "{$optionTable2}.option_id={$valueExpr} AND {$optionTable2}.store_id=?", $collection->getStoreId () );
		$valueExpr = $connection->getCheckSql ( "{$optionTable2}.value_id IS NULL", "{$optionTable1}.value", "{$optionTable2}.value" );
		
		$collection->getSelect ()->joinLeft ( [ 
				$optionTable1 => $this->getTable ( 'eav_attribute_option_value' ) 
		], $tableJoinCond1, [ ] )->joinLeft ( [ 
	 ...
	 * @param int $store        	
	 * @param bool $hasValueField
	 *        	flag which require option value
	...
	public function getFlatUpdateSelect(\Magento\Eav\Model\Entity\Attribute\AbstractAttribute $attribute, $store, $hasValueField = true) {
		...
		if ($attribute->getFrontend ()->getInputType () != 'multiselect' && $hasValueField) {
			...
			], "to1.option_id = {$valueExpr} AND to1.store_id = 0", [ ] )->joinLeft ( [ 
