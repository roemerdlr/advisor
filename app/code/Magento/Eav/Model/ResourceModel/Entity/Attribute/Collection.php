...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function setAttributeSetFilter($setId) {
		if (is_array ( $setId )) {
			if (! empty ( $setId )) {
				...
				], 'entity_attribute.attribute_id = main_table.attribute_id', 'attribute_id' );
				$this->addFieldToFilter ( 'entity_attribute.attribute_set_id', [ 
						'in' => $setId 
				] );
	...
	public function setInAllAttributeSetsFilter(array $setIds) {
		foreach ( $setIds as $setId ) {
			...
			$joinCondition = $this->getConnection ()->quoteInto ( "{$alias}.attribute_id = main_table.attribute_id AND {$alias}.attribute_set_id =?", $setId );
			$this->join ( [ 
					$alias => 'eav_entity_attribute' 
			], $joinCondition, 'attribute_id' );
		}
	...
	public function setAttributesExcludeFilter($attributes) {
		return $this->addFieldToFilter ( 'main_table.attribute_id', [ 
				'nin' => $attributes 
		] );
	}
	...
	public function addIsUniqueFilter() {
		return $this->addFieldToFilter ( 'is_unique', [ 
				'gt' => 0 
		] );
	}
	...
	public function addHasOptionsFilter() {
		...
		], 'ao.attribute_id = main_table.attribute_id', 'option_id' )->group ( 'main_table.attribute_id' )->where ( $orWhere );
		return $this;
	}
	...
	public function setCodeFilter($code) {
		...
		return $this->addFieldToFilter ( 'attribute_code', [ 
				'in' => $code 
		] );
	}
	...
	public function addStoreLabel($storeId) {
		$connection = $this->getConnection ();
		$joinExpression = $connection->quoteInto ( 'al.attribute_id = main_table.attribute_id AND al.store_id = ?', ( int ) $storeId );
		$this->getSelect ()->joinLeft ( [ 
