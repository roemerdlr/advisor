...
class Attribute extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 *        	@codeCoverageIgnore
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Store\Model\StoreManagerInterface $storeManager, Type $eavEntityType, $connectionName = null) {
	...
	private function _getMaxSortOrder(AbstractModel $object) {
		if (intval ( $object->getAttributeGroupId () ) > 0) {
			...
			$select = $connection->select ()->from ( $this->getTable ( 'eav_entity_attribute' ), new \Zend_Db_Expr ( "MAX(sort_order)" ) )->where ( 'attribute_set_id = :attribute_set_id' )->where ( 'attribute_group_id = :attribute_group_id' );
	...
	public function deleteEntity(AbstractModel $object) {
		...
		$this->getConnection ()->delete ( $this->getTable ( 'eav_entity_attribute' ), [ 
	...
	protected function _saveAdditionalAttributeData(AbstractModel $object) {
		...
		if ($additionalTable) {
			...
			$select = $connection->select ()->from ( $this->getTable ( $additionalTable ), [ 
					'attribute_id' 
	...
	public function getIdByCode($entityType, $code) {
		...
		], 'a.entity_type_id = t.entity_type_id', [ ] )->where ( 't.entity_type_code = :entity_type_code' )->where ( 'a.attribute_code = :attribute_code' );
	...
	public function getAttributeCodesByFrontendType($frontendType) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'eav_attribute' ), 'attribute_code' )->where ( 'frontend_input = :frontend_input' );
	...
	public function getStoreLabelsByAttributeId($attributeId) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'eav_attribute_label' ), [ 
				'store_id',
				'value' 
		] )->where ( 'attribute_id = :attribute_id' );
