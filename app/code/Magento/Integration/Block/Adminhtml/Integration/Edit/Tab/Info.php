...
class Info extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface {
	...
	protected function _addGeneralFieldset($form, $integrationData) {
		...
		if (isset ( $integrationData [self::DATA_ID] )) {
			$fieldset->addField ( self::DATA_ID, 'hidden', [ 
					'name' => 'id' 
			] );
		...
		$fieldset->addField ( self::DATA_NAME, 'text', [ 
				'label' => __ ( 'Name' ),
				'name' => self::DATA_NAME,
				'required' => true,
				'disabled' => $disabled,
		...
		$fieldset->addField ( self::DATA_EMAIL, 'text', [ 
				'label' => __ ( 'Email' ),
		...
		$fieldset->addField ( self::DATA_ENDPOINT, 'text', [ 
				'label' => __ ( 'Callback URL' ),
				'name' => self::DATA_ENDPOINT,
				'disabled' => $disabled,
				// @codingStandardsIgnoreStart
				'note' => __ ( 'Enter URL where Oauth credentials can be sent when using Oauth for token exchange. We strongly recommend using https://.' ) 
		]
		// @codingStandardsIgnoreEnd
		 );
		$fieldset->addField ( self::DATA_IDENTITY_LINK_URL, 'text', [ 
				'label' => __ ( 'Identity link URL' ),
				'name' => self::DATA_IDENTITY_LINK_URL,
				'disabled' => $disabled,
				'note' => __ ( 'URL to redirect user to link their 3rd party account with this Magento integration credentials.' ) 
		] );
	}
	
	/**
	 * Add fieldset with integration details.
	 * This fieldset is available for existing integrations only.
	 *
	 * @param \Magento\Framework\Data\Form $form        	
	 * @param array $integrationData        	
	 * @return void
