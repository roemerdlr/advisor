...
class Link extends AbstractRenderer {
	...
	public function render(DataObject $row) {
		...
		$html = sprintf ( '<a href="%s" %s>%s</a>', $this->_getUrl ( $row ), $this->_getAttributesHtml (), $this->getCaption () );
	...
	/**
	 * Return URL pattern for action associated with the link e.g.
	 * "(star)(slash)(star)(slash)activate" ->
	...
	public function getCaption() {
		return $this->isDisabled () ? $this->getColumn ()->getDisabledCaption () ?  : $this->getColumn ()->getCaption () : $this->getColumn ()->getCaption ();
	}
	
	/**
	 * Return additional HTML parameters for tag, e.g.
	 * 'style'
	 *
	 * @return string
	 */
	...
	protected function _getUrl(DataObject $row) {
		return $this->isDisabled ( $row ) ? '#' : $this->getUrl ( $this->getUrlPattern (), [ 
				'id' => $row->getId () 
		] );
	}
}
