...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->configReaderMock = $this->getMockBuilder ( 'Magento\Integration\Model\Config\Reader' )->disableOriginalConstructor ()->getMock ();
	...
	public function testGetIntegrationsFromConfigCacheType() {
		...
		$this->configCacheTypeMock->expects ( $this->once () )->method ( 'load' )->with ( Config::CACHE_ID )->will ( $this->returnValue ( serialize ( $integrations ) ) );
	...
	public function testGetIntegrationsFromConfigReader() {
		...
		$this->configCacheTypeMock->expects ( $this->once () )->method ( 'save' )->with ( serialize ( $integrations ), Config::CACHE_ID, [ 
				Type::CACHE_TAG 
