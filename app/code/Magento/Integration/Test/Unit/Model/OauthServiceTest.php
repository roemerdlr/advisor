...
class OauthServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_tokenFactoryMock = $this->getMock ( 'Magento\Integration\Model\Oauth\TokenFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->_emptyConsumerMock = $this->getMockBuilder ( 'Magento\Integration\Model\Integration' )->disableOriginalConstructor ()->setMethods ( [ 
				'getData',
				'load',
				'getId',
				'save',
	...
	public function testDelete() {
		$this->_consumerMock->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( self::VALUE_CONSUMER_ID ) );
		$this->_consumerMock->expects ( $this->once () )->method ( 'load' )->with ( self::VALUE_CONSUMER_ID )->will ( $this->returnValue ( $this->_consumerMock ) );
	...
	/**
	 *
	 * @return void @expectedException \Magento\Framework\Exception\IntegrationException
	...
	public function testCreateAccessTokenInvalidConsumerId() {
		...
		$this->_tokenProviderMock->expects ( $this->any () )->method ( 'getIntegrationTokenByConsumerId' )->will ( $this->throwException ( new \Magento\Framework\Oauth\Exception ( __ ( 'A token with consumer ID 0 does not exist' ) ) ) );
	...
	/**
	 *
	 * @return void @expectedException \Magento\Framework\Oauth\Exception
	 *         @expectedExceptionMessage Unexpected error. Unable to load oAuth consumer account.
	 */
	public function testLoadConsumerException() {
		$this->_consumerMock->expects ( $this->once () )->method ( 'load' )->will ( $this->throwException ( new \Magento\Framework\Oauth\Exception ( __ ( 'Unexpected error. Unable to load oAuth consumer account.' ) ) ) );
	...
	public function testLoadConsumerByKey() {
		$this->_consumerMock->expects ( $this->once () )->method ( 'load' )->with ( self::VALUE_CONSUMER_KEY, 'key' )->will ( $this->returnValue ( $this->_consumerMock ) );
		$this->_consumerMock->expects ( $this->any () )->method ( 'getData' )->will ( $this->returnValue ( $this->_consumerData ) );
