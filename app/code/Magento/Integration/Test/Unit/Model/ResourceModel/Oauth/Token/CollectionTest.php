...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$resource = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb' )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup',
				'getConnection' 
		...
		$arguments = $objectManagerHelper->getConstructArguments ( 'Magento\Integration\Model\ResourceModel\Oauth\Token\Collection', [ 
				'resource' => $resource 
		] );
		
		$this->collection = $this->getMockBuilder ( 'Magento\Integration\Model\ResourceModel\Oauth\Token\Collection' )->setConstructorArgs ( $arguments )->setMethods ( [ 
				'addFilter',
				'getSelect',
				'getTable',
				'_initSelect' 
	...
	public function testAddFilterByCustomerId() {
		$id = 1;
		$this->collection->expects ( $this->once () )->method ( 'addFilter' )->with ( 'main_table.customer_id', $id )->willReturn ( $this->collection );
		$this->collection->addFilterByCustomerId ( $id );
	}
	public function testAddFilterByConsumerId() {
		$id = 1;
		$this->collection->expects ( $this->once () )->method ( 'addFilter' )->with ( 'main_table.consumer_id', $id )->willReturn ( $this->collection );
		$this->collection->addFilterByConsumerId ( $id );
	}
	public function testAddFilterByType() {
		$type = 'type';
		$this->collection->expects ( $this->once () )->method ( 'addFilter' )->with ( 'main_table.type', $type )->willReturn ( $this->collection );
		$this->collection->addFilterByType ( $type );
	}
	public function testAddFilterById() {
		$id = 1;
		$this->collection->expects ( $this->once () )->method ( 'addFilter' )->with ( 'main_table.entity_id', [ 
				'in' => $id 
		], 'public' )->willReturn ( $this->collection );
