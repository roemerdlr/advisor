...
class TokenTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->contextMock = $this->getMockBuilder ( 'Magento\Framework\Model\Context' )->setMethods ( [ 
				'getEventDispatcher' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->registryMock = $this->getMockBuilder ( 'Magento\Framework\Registry' )->disableOriginalConstructor ()->getMock ();
		
		$this->validatorKeyLengthMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\Consumer\Validator\KeyLength' )->setMethods ( [ 
				'isValid',
				'setLength',
				'setName',
				'getMessages' 
		...
		$this->keyLengthFactoryMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\Consumer\Validator\KeyLengthFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->oauthDataMock = $this->getMockBuilder ( 'Magento\Integration\Helper\Oauth\Data' )->setMethods ( [ 
		...
		$this->resourceMock = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\AbstractResource' )->setMethods ( [ 
				'getIdFieldName',
				'deleteOldEntries',
				'_construct',
				'getConnection',
		...
		$eventManagerMock = $this->getMockBuilder ( 'Magento\Framework\Event\ManagerInterface' )->setMethods ( [ 
				'dispatch' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testCreateVerifierToken() {
		...
		$this->resourceMock->expects ( $this->once () )->method ( 'selectTokenByType' )->with ( $consumerId, Token::TYPE_VERIFIER )->willReturn ( [ 
				'id' => 123 
		] );
	...
	public function testValidateIfNotCallbackEstablishedAndNotValid() {
		...
		$this->validatorMock->expects ( $this->once () )->method ( 'getMessages' )->willReturn ( [ 
				$exceptionMessage 
		] );
	...
	public function testValidateIfSecretNotValid() {
		...
		$this->validatorKeyLengthMock->expects ( $this->once () )->method ( 'getMessages' )->willReturn ( [ 
				$exceptionMessage 
		] );
