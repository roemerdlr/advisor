...
class KeyLengthTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$options = [ 
				'length' => KeyLengthTest::KEY_LENGTH 
		];
	...
	public function testIsValidLong() {
		...
		$expected = [ 
				'stringLengthTooLong' => "Key '{$invalidToken}' is more than 32 characters long" 
		];
	...
	public function testIsValidShort() {
		...
		$this->keyLengthValidator->isValid ( $invalidToken );
		$expected = [ 
				'stringLengthTooShort' => "Key '{$invalidToken}' is less than 32 characters long" 
		];
		$this->assertEquals ( $expected, $this->keyLengthValidator->getMessages () );
	}
	public function testIsValidShortCustomKeyName() {
		...
		$this->keyLengthValidator->isValid ( $invalidToken );
		$expected = [ 
				'stringLengthTooShort' => "Custom Key '{$invalidToken}' is less than 32 characters long" 
		];
		$this->assertEquals ( $expected, $this->keyLengthValidator->getMessages () );
	}
	 ...
	 * @expectedExceptionMessage Invalid type given for Key.
	 * String expected
	 */
