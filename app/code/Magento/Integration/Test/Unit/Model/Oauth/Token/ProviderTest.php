...
class ProviderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->consumerFactoryMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\ConsumerFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->tokenFactoryMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\TokenFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->loggerMock = $this->getMockBuilder ( 'Psr\Log\LoggerInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->consumerMock = $this->getMockBuilder ( 'Magento\Framework\Oauth\ConsumerInterface' )->setMethods ( [ 
				'load',
				'loadByKey',
				'validate',
				'getId',
		...
		$this->requestTokenMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\Token' )->setMethods ( [ 
		...
		$this->accessTokenMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\Token' )->setMethods ( [ 
				'getToken',
				'getSecret',
				'load',
				'getId',
		...
		$this->tokenProvider = $objectManagerHelper->getObject ( 'Magento\Integration\Model\Oauth\Token\Provider', [ 
	...
	public function testCreateRequestToken() {
		...
		$tokenMock = $this->getMockBuilder ( 'Magento\Integration\Model\Oauth\Token' )->setMethods ( [ 
				'loadByConsumerIdAndUserType',
				'getId',
				'getType',
				'createRequestToken' 
		...
		$tokenMock->expects ( $this->once () )->method ( 'createRequestToken' )->willReturn ( $this->requestTokenMock );
	...
	public function testValidateRequestToken() {
		...
		$this->requestTokenMock->expects ( $this->once () )->method ( 'load' )->with ( $requestTokenString, 'token' )->willReturnSelf ();
		$this->tokenFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->requestTokenMock );
		...
		$this->assertEquals ( $secret, $this->tokenProvider->validateRequestToken ( $requestTokenString, $this->consumerMock, $oauthVerifier ) );
	}
	...
	public function testValidateAccessTokenRequest() {
		...
		$this->assertEquals ( $secret, $this->tokenProvider->validateAccessTokenRequest ( $accessTokenString, $this->consumerMock ) );
	}
	...
	public function testValidateAccessToken() {
		...
		$this->accessTokenMock->expects ( $this->once () )->method ( 'getType' )->willReturn ( Token::TYPE_ACCESS );
		$this->accessTokenMock->expects ( $this->once () )->method ( 'getRevoked' )->willReturn ( 0 );
		$this->assertEquals ( $consumerId, $this->tokenProvider->validateAccessToken ( $accessTokenString ) );
	}
