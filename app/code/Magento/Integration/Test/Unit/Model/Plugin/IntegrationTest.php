...
class IntegrationTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->integrationAuthServiceMock = $this->getMock ( 'Magento\Integration\Api\AuthorizationServiceInterface', [ 
				'removePermissions',
				'grantAllPermissions',
				'grantPermissions' 
		], [ ], '', false );
		$this->aclRetrieverMock = $this->getMock ( 'Magento\Authorization\Model\Acl\AclRetriever', [ 
				'getAllowedResourcesByUser' 
		], [ ], '', false );
		$this->integrationPlugin = new \Magento\Integration\Model\Plugin\Integration ( $this->integrationAuthServiceMock, $this->aclRetrieverMock );
	}
	...
	public function testAfterCreateSomeResources() {
		...
		$integrationModelMock->expects ( $this->at ( 2 ) )->method ( 'getData' )->with ( 'resource' )->will ( $this->returnValue ( [ 
				'testResource' 
		] ) );
		$integrationModelMock->expects ( $this->at ( 4 ) )->method ( 'getData' )->with ( 'resource' )->will ( $this->returnValue ( [ 
				'testResource' 
		] ) );
	...
	public function testAfterGet() {
		...
		$integrationModelMock = $this->getMockBuilder ( 'Magento\Integration\Model\Integration' )->disableOriginalConstructor ()->getMock ();
		$integrationModelMock->expects ( $this->exactly ( 2 ) )->method ( 'getId' )->will ( $this->returnValue ( $integrationId ) );
		$integrationModelMock->expects ( $this->once () )->method ( 'setData' )->with ( 'resource', [ 
				'testResource' 
		] );
		
		$this->aclRetrieverMock->expects ( $this->once () )->method ( 'getAllowedResourcesByUser' )->with ( \Magento\Authorization\Model\UserContextInterface::USER_TYPE_INTEGRATION, $integrationId )->will ( $this->returnValue ( [ 
				'testResource' 
		] ) );
