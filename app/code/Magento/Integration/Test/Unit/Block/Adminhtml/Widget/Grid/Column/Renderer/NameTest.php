...
class NameTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->contextMock = $this->getMock ( 'Magento\Backend\Block\Context', [ 
				'getEscaper',
				'getUrlBuilder' 
		], [ ], '', false );
		...
		$this->nameRenderer = $this->objectManagerHelper->getObject ( 'Magento\Integration\Block\Adminhtml\Widget\Grid\Column\Renderer\Name', [ 
	...
	public function testRender($endpoint, $name, $expectedResult) {
		$column = $this->getMockBuilder ( 'Magento\Backend\Block\Widget\Grid\Column' )->disableOriginalConstructor ()->setMethods ( [ 
				'getIndex',
				'getEditable',
				'getGetter' 
		] )->getMock ();
		$column->expects ( $this->any () )->method ( 'getIndex' )->willReturn ( 'name' );
		$column->expects ( $this->any () )->method ( 'getEditable' )->willReturn ( false );
		$column->expects ( $this->any () )->method ( 'getGetter' )->willReturn ( 'getName' );
