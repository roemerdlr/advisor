...
class WebapiTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->aclResourceProvider = $this->getMockBuilder ( 'Magento\Framework\Acl\AclResource\ProviderInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		
		$this->integrationHelper = $this->getMockBuilder ( 'Magento\Integration\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		
		$this->integrationService = $this->getMockBuilder ( 'Magento\Integration\Model\IntegrationService' )->disableOriginalConstructor ()->getMock ();
	}
	...
	public function testGetTree() {
		...
		$this->integrationHelper->expects ( $this->once () )->method ( 'mapResources' )->with ( [ 
				1,
				2,
				3 
	...
	private function getWebapiBlock($integrationData = [], array $selectedResources = []) {
		...
		return $this->objectManager->getObject ( 'Magento\Integration\Block\Adminhtml\Integration\Edit\Tab\Webapi', [ 
