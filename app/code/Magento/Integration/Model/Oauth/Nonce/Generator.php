...
class Generator implements NonceGeneratorInterface {
	 ...
	 */
	public function __construct(\Magento\Framework\Oauth\Helper\Oauth $oauthHelper, \Magento\Integration\Model\Oauth\NonceFactory $nonceFactory, \Magento\Framework\Stdlib\DateTime\DateTime $date, $nonceLength = \Magento\Framework\Oauth\Helper\Oauth::LENGTH_NONCE) {
	...
	public function validateNonce(ConsumerInterface $consumer, $nonce, $timestamp) {
		try {
			...
			if ($nonceObj->getNonce ()) {
				throw new \Magento\Framework\Oauth\Exception ( __ ( 'The nonce is already being used by the consumer with ID %1', [ 
						$consumer->getId () 
				] ) );
			}
