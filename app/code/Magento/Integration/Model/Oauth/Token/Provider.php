...
class Provider implements TokenProviderInterface {
	 ...
	 */
	public function __construct(\Magento\Integration\Model\Oauth\ConsumerFactory $consumerFactory, \Magento\Integration\Model\Oauth\TokenFactory $tokenFactory, \Psr\Log\LoggerInterface $logger) {
	...
	public function validateConsumer($consumer) {
		...
		if (! $consumer->isValidForTokenExchange ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Consumer key has expired' ) );
		}
		return true;
	}
	...
	public function createRequestToken($consumer) {
		...
		if ($token->getType () != Token::TYPE_VERIFIER) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Cannot create request token because consumer token is not a verifier token' ) );
		}
	...
	public function validateRequestToken($requestToken, $consumer, $oauthVerifier) {
		...
		if (! $this->_isTokenAssociatedToConsumer ( $token, $consumer )) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Request token is not associated with the specified consumer' ) );
		}
		...
		if (Token::TYPE_REQUEST != $token->getType ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Token is already being used' ) );
		}
	...
	public function getAccessToken($consumer) {
		...
		if (Token::TYPE_REQUEST != $token->getType ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Cannot get access token because consumer token is not a request token' ) );
		}
		$accessToken = $token->convertToAccess ();
		$this->logger->info ( 'Request token ' . $token->getToken () . ' was exchanged to obtain access token for consumer ' . $consumerId );
		return [ 
	...
	public function validateAccessTokenRequest($accessToken, $consumer) {
		...
		if (! $this->_isTokenAssociatedToConsumer ( $token, $consumer )) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Token is not associated with the specified consumer' ) );
		}
		if (Token::TYPE_ACCESS != $token->getType ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Token is not an access token' ) );
		}
		if ($token->getRevoked ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Access token has been revoked' ) );
		}
	...
	public function getConsumerByKey($consumerKey) {
		if (strlen ( $consumerKey ) != \Magento\Framework\Oauth\Helper\Oauth::LENGTH_CONSUMER_KEY) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Consumer key is not the correct length' ) );
		}
		
		$consumer = $this->_consumerFactory->create ()->loadByKey ( $consumerKey );
		
		if (! $consumer->getId ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'A consumer having the specified key does not exist' ) );
		}
	...
	protected function _getConsumer($consumerId) {
		...
		if (! $consumer->getId ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'A consumer with the ID %1 does not exist', [ 
					$consumerId 
			] ) );
		}
	...
	protected function _getToken($token) {
		if (! $this->validateOauthToken ( $token )) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Token is not the correct length' ) );
		}
		
		$tokenObj = $this->_tokenFactory->create ()->load ( $token, 'token' );
		
		if (! $tokenObj->getId ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'Specified token does not exist' ) );
		}
	...
	public function getIntegrationTokenByConsumerId($consumerId) {
		...
		if (! $token->getId ()) {
			throw new \Magento\Framework\Oauth\Exception ( __ ( 'A token with consumer ID %1 does not exist', [ 
					$consumerId 
			] ) );
		}
