...
class Consumer extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function _afterDelete(\Magento\Framework\Model\AbstractModel $object) {
		...
		$connection->delete ( $this->getTable ( 'oauth_nonce' ), [ 
				'consumer_id' => $object->getId () 
		] );
		$connection->delete ( $this->getTable ( 'oauth_token' ), [ 
				'consumer_id' => $object->getId () 
		] );
	...
	public function getTimeInSecondsSinceCreation($consumerId) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->getMainTable () )->reset ( \Magento\Framework\DB\Select::COLUMNS )->columns ( new \Zend_Db_Expr ( 'CURRENT_TIMESTAMP() - created_at' ) )->where ( 'entity_id = ?', $consumerId );
