...
class AuthorizationService implements \Magento\Integration\Api\AuthorizationServiceInterface {
	 ...
	 */
	public function __construct(AclBuilder $aclBuilder, RoleFactory $roleFactory, RoleCollectionFactory $roleCollectionFactory, RulesFactory $rulesFactory, RulesCollectionFactory $rulesCollectionFactory, Logger $logger, RootAclResource $rootAclResource) {
	...
	public function grantPermissions($integrationId, $resources) {
		try {
			...
			throw new LocalizedException ( __ ( 'Sorry, something went wrong granting permissions. You can find out more in the exceptions log.' ) );
		}
	}
	...
	public function grantAllPermissions($integrationId) {
		$this->grantPermissions ( $integrationId, [ 
				$this->_rootAclResource->getId () 
		] );
	}
	...
	public function removePermissions($integrationId) {
		try {
			...
			$this->_logger->critical ( $e );
			throw new LocalizedException ( __ ( 'Something went wrong while deleting roles and permissions.' . ' You can find out more in the exceptions log.' ) );
		}
	}
	...
	protected function _createRole($integrationId) {
		...
		$role->setRoleName ( $roleName )->setUserType ( UserContextInterface::USER_TYPE_INTEGRATION )->setUserId ( $integrationId )->setRoleType ( \Magento\Authorization\Model\Acl\Role\User::ROLE_TYPE )->setParentId ( 0 )->save ();
		return $role;
	}
	
	/**
	 * Remove integration role.
	 * This deletes the cascading permissions
	 *
	 * @param int $integrationId        	
	 * @return \Magento\Authorization\Model\Role
	 */
	...
	/**
	 * Associate resources with the specified role.
	 * All resources previously assigned to the role will be unassigned.
	 *
