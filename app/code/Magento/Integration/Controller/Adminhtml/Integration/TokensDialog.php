...
class TokensDialog extends \Magento\Integration\Controller\Adminhtml\Integration {
	 ...
	 *
	 * @param boolean $isReauthorize
	 *        	Is a re-authorization flow
	...
	protected function _setActivationSuccessMsg($isReauthorize, $integrationName) {
		...
		$successMsg = $isReauthorize ? __ ( "The integration '%1' has been re-authorized.", $integrationName ) : __ ( "The integration '%1' has been activated.", $integrationName );
	...
	public function execute() {
		try {
			...
			$this->_registry->register ( self::REGISTRY_KEY_CURRENT_INTEGRATION, $this->_integrationService->get ( $integrationId )->getData () );
