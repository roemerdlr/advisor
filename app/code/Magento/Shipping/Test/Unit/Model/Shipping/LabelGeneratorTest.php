...
class LabelGeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->carrierFactory = $this->getMockBuilder ( 'Magento\Shipping\Model\CarrierFactory' )->disableOriginalConstructor ()->getMock ();
		$this->labelsFactory = $this->getMockBuilder ( 'Magento\Shipping\Model\Shipping\LabelsFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->scopeConfig = $this->getMock ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		$this->trackFactory = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Shipment\TrackFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->filesystem = $this->getMockBuilder ( 'Magento\Framework\Filesystem' )->disableOriginalConstructor ()->getMock ();
		
		$this->labelGenerator = new \Magento\Shipping\Model\Shipping\LabelGenerator ( $this->carrierFactory, $this->labelsFactory, $this->scopeConfig, $this->trackFactory, $this->filesystem );
	}
	...
	public function testAddTrackingNumbersToShipment(array $info) {
		...
		$trackMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Shipment\Track' )->setMethods ( [ 
				'setNumber',
				'setCarrierCode',
				'setTitle' 
		...
		$i = 0;
		$trackingNumbers = is_array ( $info ['tracking_number'] ) ? $info ['tracking_number'] : [ 
	...
	private function getShippingMethodMock() {
		...
		$shippingMethod->expects ( static::once () )->method ( 'getCarrierCode' )->willReturn ( self::CARRIER_CODE );
	...
	private function getCarrierMock() {
		$carrierMock = $this->getMockBuilder ( 'Magento\Shipping\Model\Carrier\AbstractCarrier' )->disableOriginalConstructor ()->setMethods ( [ 
				'isShippingLabelsAvailable',
				'getCarrierCode' 
		] )->getMockForAbstractClass ();
		$carrierMock->expects ( static::once () )->method ( 'isShippingLabelsAvailable' )->willReturn ( true );
		$carrierMock->expects ( static::once () )->method ( 'getCarrierCode' )->willReturn ( self::CARRIER_CODE );
	...
	private function getResponseMock(array $info) {
		$responseMock = $this->getMockBuilder ( '\Magento\Framework\DataObject' )->setMethods ( [ 
				'hasErrors',
				'hasInfo',
				'getInfo' 
		] )->disableOriginalConstructor ()->getMock ();
		$responseMock->expects ( static::once () )->method ( 'hasErrors' )->willReturn ( false );
		$responseMock->expects ( static::once () )->method ( 'hasInfo' )->willReturn ( true );
		$responseMock->expects ( static::once () )->method ( 'getInfo' )->willReturn ( [ 
				$info 
		] );
