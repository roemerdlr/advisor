...
class AbstractCarrierOnlineTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$carrierArgs = $objectManagerHelper->getConstructArguments ( 'Magento\Shipping\Model\Carrier\AbstractCarrierOnline', [ 
				'stockRegistry' => $this->stockRegistry,
				'xmlSecurity' => new \Magento\Framework\Xml\Security () 
		] );
		$this->carrier = $this->getMockBuilder ( 'Magento\Shipping\Model\Carrier\AbstractCarrierOnline' )->setConstructorArgs ( $carrierArgs )->setMethods ( [ 
				'getConfigData',
				'_doShipmentRequest',
				'collectRates' 
		] )->getMock ();
	...
	public function testParseXmlXXEXml() {
		$xmlString = '<!DOCTYPE scan [
            <!ENTITY test SYSTEM "php://filter/read=convert.base64-encode/resource=' . __DIR__ . '/AbstractCarrierOnline/xxe-xml.txt">]><scan>&test;</scan>';
