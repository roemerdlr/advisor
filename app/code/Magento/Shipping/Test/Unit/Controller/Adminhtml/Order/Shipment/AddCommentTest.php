...
class AddCommentTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->shipmentCommentSenderMock = $this->getMock ( 'Magento\Sales\Model\Order\Email\Sender\ShipmentCommentSender', [ 
				'send',
				'__wakeup' 
		], [ ], '', false );
		...
		$this->resultLayoutFactoryMock = $this->getMock ( 'Magento\Framework\View\Result\LayoutFactory', [ 
				'create' 
		], [ ], '', false );
	...
	protected function exceptionResponse() {
		$dataMock = $this->getMock ( 'Magento\Framework\Json\Helper\Data', [ 
				'jsonEncode' 
		], [ ], '', false );
	...
	public function testExecute() {
		...
		$resultLayoutMock = $this->getMock ( 'Magento\Framework\View\Result\Layout', [ 
				'getBlock',
		...
		$this->requestMock->expects ( $this->any () )->method ( 'getParam' )->will ( $this->returnValueMap ( [ 
				[ 
						'id',
						null,
						$shipmentId 
		...
		$blockMock = $this->getMock ( 'Magento\Shipping\Block\Adminhtml\View\Comments', [ 
				'toHtml' 
		], [ ], '', false );
	...
	public function testEmptyCommentData() {
		$shipmentId = 1;
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( 'id' )->will ( $this->returnValue ( $shipmentId ) );
		$this->requestMock->expects ( $this->once () )->method ( 'setParam' )->with ( 'shipment_id', $shipmentId );
