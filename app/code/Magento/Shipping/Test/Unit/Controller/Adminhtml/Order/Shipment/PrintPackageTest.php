...
class PrintPackageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->sessionMock = $this->getMock ( 'Magento\Backend\Model\Session', [ 
				'setIsUrlNotice' 
		], [ ], '', false );
		$this->actionFlag = $this->getMock ( 'Magento\Framework\App\ActionFlag', [ 
				'get' 
		], [ ], '', false );
		$this->shipmentMock = $this->getMock ( 'Magento\Sales\Model\Order\Shipment', [ 
				'__wakeup' 
		], [ ], '', false );
		$this->fileFactoryMock = $this->getMock ( 'Magento\Framework\App\Response\Http\FileFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testExecute() {
		...
		$packagingMock = $this->getMock ( 'Magento\Shipping\Model\Order\Pdf\Packaging', [ 
				'getPdf' 
		], [ ], '', false );
		$pdfMock = $this->getMock ( 'Zend_Pdf', [ 
				'render' 
		], [ ], '', false );
		$dateTimeMock = $this->getMock ( 'Magento\Framework\Stdlib\DateTime\DateTime', [ 
				'date' 
		], [ ], '', false );
		
		$this->shipmentLoaderMock->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( $this->shipmentMock ) );
		$this->objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Shipping\Model\Order\Pdf\Packaging' )->will ( $this->returnValue ( $packagingMock ) );
		$packagingMock->expects ( $this->once () )->method ( 'getPdf' )->with ( $this->shipmentMock )->will ( $this->returnValue ( $pdfMock ) );
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Framework\Stdlib\DateTime\DateTime' )->will ( $this->returnValue ( $dateTimeMock ) );
		...
		$this->fileFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( 'packingslip' . $date . '.pdf', $content, DirectoryList::VAR_DIR, 'application/pdf' )->will ( $this->returnValue ( 'result-pdf-content' ) );
