...
class ShipmentLoaderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->shipmentRepositoryMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\ShipmentRepository' )->disableOriginalConstructor ()->setMethods ( [ 
				'get' 
		] )->getMock ();
		$this->registryMock = $this->getMockBuilder ( 'Magento\Framework\Registry' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$this->shipmentFactory = $this->getMockBuilder ( 'Magento\Sales\Model\Order\ShipmentFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->trackFactoryMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Shipment\TrackFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->orderRepository = $this->getMockBuilder ( 'Magento\Sales\Api\OrderRepositoryInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		...
		$this->loader = new \Magento\Shipping\Controller\Adminhtml\Order\ShipmentLoader ( $this->messageManagerMock, $this->registryMock, $this->shipmentRepositoryMock, $this->shipmentFactory, $this->trackFactoryMock, $this->orderRepository, $data );
	}
	...
	public function testLoadOrderId() {
		...
		$trackMock->expects ( $this->any () )->method ( 'addData' )->will ( $this->returnValueMap ( [ 
				[ 
						$this->loader->getTracking () [0],
						$trackMock 
				],
				[ 
						$this->loader->getTracking () [1],
						$trackMock 
				] 
		] ) );
