...
class SaveTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->resultRedirect = $this->getMock ( 'Magento\Framework\Controller\Result\Redirect', [ 
				'setPath' 
		], [ ], '', false );
		$this->resultRedirect->expects ( $this->any () )->method ( 'setPath' )->willReturn ( $this->resultRedirect );
		
		$resultRedirectFactory = $this->getMock ( 'Magento\Framework\Controller\Result\RedirectFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->saveAction = $objectManagerHelper->getObject ( 'Magento\Shipping\Controller\Adminhtml\Order\Shipment\Save', [ 
	...
	protected function prepareRedirect($path, array $arguments = []) {
		...
		$this->helper->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->with ( $path, $arguments )->will ( $this->returnValue ( $url ) );
		$this->response->expects ( $this->atLeastOnce () )->method ( 'setRedirect' )->with ( $url );
	}
}
