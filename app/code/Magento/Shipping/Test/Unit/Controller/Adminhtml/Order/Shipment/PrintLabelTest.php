...
class PrintLabelTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->labelGenerator = $this->getMock ( 'Magento\Shipping\Model\Shipping\LabelGenerator', [ 
		...
		$this->helperMock = $this->getMock ( 'Magento\Backend\Helper\Data', [ 
				'getUrl' 
		], [ ], '', false );
		...
		$this->controller = new \Magento\Shipping\Controller\Adminhtml\Order\Shipment\PrintLabel ( $contextMock, $this->shipmentLoaderMock, $this->labelGenerator, $this->fileFactoryMock );
	}
	...
	public function testExecuteFromImageString() {
		...
		$pdfPageMock = $this->getMock ( 'Zend_Pdf_Page', [ 
				'render',
				'getPageDictionary' 
		], [ ], '', false );
		$pageDictionaryMock = $this->getMock ( 'Zend_Pdf_Element_Dictionary', [ 
				'touch',
				'getObject' 
		], [ ], '', false );
		...
		$this->shipmentLoaderMock->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( $this->shipmentMock ) );
		$this->shipmentMock->expects ( $this->once () )->method ( 'getShippingLabel' )->will ( $this->returnValue ( $labelContent ) );
		$this->labelGenerator->expects ( $this->once () )->method ( 'createPdfPageFromImageString' )->with ( $labelContent )->will ( $this->returnValue ( $pdfPageMock ) );
