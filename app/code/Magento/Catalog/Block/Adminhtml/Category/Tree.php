...
class Tree extends \Magento\Catalog\Block\Adminhtml\Category\AbstractCategory {
	...
	protected function _prepareLayout() {
		...
		$this->addChild ( 'add_sub_button', 'Magento\Backend\Block\Widget\Button', [ 
		...
		if ($this->canAddRootCategory ()) {
			$this->addChild ( 'add_root_button', 'Magento\Backend\Block\Widget\Button', [ 
	...
	public function getSuggestedCategoriesJson($namePart) {
		...
		$escapedNamePart = $this->_resourceHelper->addLikeEscape ( $namePart, [ 
				'position' => 'any' 
		] );
		$matchingNamesCollection->addAttributeToFilter ( 'name', [ 
				'like' => $escapedNamePart 
		] )->addAttributeToFilter ( 'entity_id', [ 
				'neq' => \Magento\Catalog\Model\Category::TREE_ROOT_ID 
		] )->addAttributeToSelect ( 'path' )->setStoreId ( $storeId );
	...
	public function getBreadcrumbsJavascript($path, $javascriptVarName) {
		...
		return '<script>require(["prototype"], function(){' . $javascriptVarName . ' = ' . $this->_jsonEncoder->encode ( $categories ) . ';' . ($this->canAddSubCategory () ? '$("add_subcategory_button").show();' : '$("add_subcategory_button").hide();') . '});</script>';
	}
	...
	public function canAddRootCategory() {
		...
		$this->_eventManager->dispatch ( 'adminhtml_catalog_category_tree_can_add_root_category', [ 
	...
	public function canAddSubCategory() {
		...
		$this->_eventManager->dispatch ( 'adminhtml_catalog_category_tree_can_add_sub_category', [ 
				'category' => $this->getCategory (),
				'options' => $options,
				'store' => $this->getStore ()->getId () 
		] );
