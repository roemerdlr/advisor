...
class Advanced extends Generic {
	 ...
	 *
	 * @return $this @SuppressWarnings(PHPMD)
	 */
	protected function _prepareForm() {
		...
		$fieldset = $form->addFieldset ( 'advanced_fieldset', [ 
		...
		$fieldset->addField ( 'attribute_code', 'text', [ 
				...
				'note' => __ ( 'This is used internally. Make sure you don\'t use spaces or more than %1 symbols.', \Magento\Eav\Model\Entity\Attribute::ATTRIBUTE_CODE_MAX_LENGTH ),
				'class' => $validateClass 
		] );
		...
		$fieldset->addField ( 'is_visible_in_grid', 'hidden', [ 
				'name' => 'is_visible_in_grid',
				'value' => $attributeObject->getData ( 'is_visible_in_grid' ) ?  : 1 
		] );
		
		$fieldset->addField ( 'is_filterable_in_grid', 'select', [ 
		...
		$this->_eventManager->dispatch ( 'product_attribute_form_build', [ 
				'form' => $form 
		] );
