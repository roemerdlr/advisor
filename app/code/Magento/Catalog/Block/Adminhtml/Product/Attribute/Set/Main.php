...
class Main extends \Magento\Backend\Block\Template {
	...
	protected function _prepareLayout() {
		...
		if (! $this->getIsCurrentSetDefault ()) {
			$this->getToolbar ()->addChild ( 'delete_button', 'Magento\Backend\Block\Widget\Button', [ 
					'label' => __ ( 'Delete' ),
					'onclick' => 'deleteConfirm(\'' . $this->escapeJsQuote ( __ ( 'You are about to delete all products in this attribute set. ' . 'Are you sure you want to do that?' ) ) . '\', \'' . $this->getUrl ( 'catalog/*/delete', [ 
							'id' => $setId 
					] ) . '\')',
					'class' => 'delete' 
			] );
	...
	public function getMoveUrl() {
		return $this->getUrl ( 'catalog/product_set/save', [ 
				'id' => $this->_getSetId () 
		] );
	}
	...
	public function getGroupUrl() {
		return $this->getUrl ( 'catalog/product_group/save', [ 
				'id' => $this->_getSetId () 
		] );
	}
	...
	protected function _toHtml() {
		$this->_eventManager->dispatch ( 'adminhtml_catalog_product_attribute_set_main_html_before', [ 
				'block' => $this 
		] );
