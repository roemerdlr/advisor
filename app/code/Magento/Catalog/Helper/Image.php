...
class Image extends AbstractHelper {
	...
	public function init($product, $imageId, $attributes = []) {
		...
		$this->attributes = array_merge ( $this->getConfigView ()->getMediaAttributes ( 'Magento_Catalog', self::MEDIA_TYPE_CONFIG_NODE, $imageId ), $attributes );
	...
	public function watermark($fileName, $position, $size = null, $imageOpacity = null) {
		$this->setWatermark ( $fileName )->setWatermarkPosition ( $position )->setWatermarkSize ( $size )->setWatermarkImageOpacity ( $imageOpacity );
		return $this;
	}
	...
	public function getPlaceholder($placeholder = null) {
		if ($placeholder) {
			...
			$placeholderFullPath = $this->_placeholder ?  : 'Magento_Catalog::images/product/placeholder/' . $this->_getModel ()->getDestinationSubdir () . '.jpg';
		}
	...
	protected function isScheduledActionsAllowed() {
		$model = $this->_getModel ();
		if ($model->isBaseFilePlaceholder () && $model->getNewFile () === true || $model->isCached ()) {
			return false;
		}
		return true;
	}
