...
class Save extends \Magento\Catalog\Controller\Adminhtml\Product\Set {
	...
	public function execute() {
		...
		$model = $this->_objectManager->create ( 'Magento\Eav\Model\Entity\Attribute\Set' )->setEntityTypeId ( $entityTypeId );
		...
		try {
			if ($isNewSet) {
				...
				$data = $this->_objectManager->get ( 'Magento\Framework\Json\Helper\Data' )->jsonDecode ( $this->getRequest ()->getPost ( 'data' ) );
		...
		if ($isNewSet) {
			if ($this->getRequest ()->getPost ( 'return_session_messages_only' )) {
				...
				if ($hasError) {
					...
					$resultRedirect->setPath ( 'catalog/*/edit', [ 
							'id' => $model->getId () 
					] );
				}
