...
class Save extends \Magento\Catalog\Controller\Adminhtml\Category {
	 ...
	 */
	public function __construct(\Magento\Backend\App\Action\Context $context, \Magento\Framework\Controller\Result\RawFactory $resultRawFactory, \Magento\Framework\Controller\Result\JsonFactory $resultJsonFactory, \Magento\Framework\View\LayoutFactory $layoutFactory) {
	 ...
	 * Category save
	 *
	 * @return \Magento\Framework\Controller\ResultInterface @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function execute() {
		...
		if ($data) {
			...
			if (! $category->getId ()) {
				...
				if (! $parentId) {
					if ($storeId) {
						$parentId = $this->_objectManager->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ( $storeId )->getRootCategoryId ();
					} else {
			...
			if (isset ( $data ['category_products'] ) && is_string ( $data ['category_products'] ) && ! $category->getProductsReadonly ()) {
				$products = json_decode ( $data ['category_products'], true );
				$category->setPostedProducts ( $products );
			}
			$this->_eventManager->dispatch ( 'catalog_category_prepare_save', [ 
					'category' => $category,
			...
			try {
				...
				if (isset ( $data ['general'] ['entity_id'] )) {
					throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Something went wrong while saving the category.' ) );
				}
		...
		return $resultRedirect->setPath ( 'catalog/*/edit', $redirectParams );
	}
}
