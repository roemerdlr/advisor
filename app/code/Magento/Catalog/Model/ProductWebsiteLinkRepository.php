...
class ProductWebsiteLinkRepository implements \Magento\Catalog\Api\ProductWebsiteLinkRepositoryInterface {
	...
	public function save(ProductWebsiteLinkInterface $productWebsiteLink) {
		...
		$product->setWebsiteIds ( array_merge ( $product->getWebsiteIds (), [ 
				$productWebsiteLink->getWebsiteId () 
		] ) );
		try {
			$product->save ();
		} catch ( \Exception $e ) {
			throw new CouldNotSaveException ( __ ( 'Could not assign product "%1" to websites "%2"', $product->getId (), $productWebsiteLink->getWebsiteId () ), $e );
		}
		return true;
	}
	...
	public function deleteById($sku, $websiteId) {
		...
		$product->setWebsiteIds ( array_diff ( $product->getWebsiteIds (), [ 
				$websiteId 
		] ) );
