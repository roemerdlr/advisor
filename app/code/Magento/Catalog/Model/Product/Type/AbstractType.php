...
abstract class AbstractType {
	...
	public function getSetAttributes($product) {
		if (! $product->hasData ( $this->_cacheProductSetAttributes )) {
			$setAttributes = $product->getResource ()->loadAllAttributes ( $product )->getSortedAttributes ( $product->getAttributeSetId () );
	 ...
	 * @return array|string
	 */
	public function processConfiguration(\Magento\Framework\DataObject $buyRequest, $product, $processMode = self::PROCESS_MODE_LITE) {
	...
	protected function _prepareOptions(\Magento\Framework\DataObject $buyRequest, $product, $processMode) {
		...
		$this->_eventManager->dispatch ( $eventName, [ 
				'transport' => $transport,
		...
		if ($optionIds) {
			foreach ( explode ( ',', $optionIds->getValue () ) as $optionId ) {
				...
				if ($option) {
					...
					$group = $option->groupFactory ( $option->getType () )->setOption ( $option )->setProduct ( $product )->setConfigurationItemOption ( $confItemOption );
	 ...
	 * (applicable for complicated product types.
	 ...
	 * @param int|float $qty        	
	 * @param \Magento\Catalog\Model\Product $product        	
	 * @return float @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
