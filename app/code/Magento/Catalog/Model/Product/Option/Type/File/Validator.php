...
abstract class Validator {
	 ...
	 */
	public function __construct(\Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\Framework\Filesystem $filesystem, \Magento\Framework\File\Size $fileSize) {
	...
	protected function getConfigData($key) {
		return $this->scopeConfig->getValue ( 'catalog/custom_options/' . $key, \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
	}
	 ...
	 *
	 * @param string[] $errors
	 *        	Array of validation failure message codes @see \Zend_Validate::getErrors()
	...
	protected function buildImageValidator($object, $option, $fileFullPath = null) {
		...
		$object->addValidator ( new \Zend_Validate_File_FilesSize ( [ 
				'max' => $this->fileSize->getMaxFileSize () 
		] ) );
		return $object;
	}
	 ...
	 *        	- either file data from \Zend_File_Transfer or file path
	 * @return boolean
