...
class File extends \Magento\Catalog\Model\Product\Option\Type\DefaultType {
	 ...
	 *
	 * @param array $values
	 *        	All product option values, i.e. array (option_id => mixed, option_id => mixed...)
	 * @return $this
	...
	public function validateUserValue($values) {
		...
		if ($fileInfo !== null) {
			try {
				$value = $this->validatorInfo->setUseQuotePath ( $this->getUseQuotePath () )->validate ( $fileInfo, $option ) ? $fileInfo : null;
		...
		try {
			$value = $this->validatorFile->setProduct ( $this->getProduct () )->validate ( $this->_getProcessingParams (), $option );
			$this->setUserValue ( $value );
	 ...
	 *        	Prepared for cart option value
	 * @return string
	 */
	public function getFormattedOptionValue($optionValue) {
		if ($this->_formattedOptionValue === null) {
			try {
				...
				$value ['url'] = [ 
						'route' => $this->_customOptionDownloadUrl,
						'params' => $customOptionUrlParams 
				];
	 ...
	 *        	Serialized string of option data or its data array
	 * @return string
	...
	protected function _getOptionHtml($optionValue) {
		...
		try {
			...
			return sprintf ( '<a href="%s" target="_blank">%s</a> %s', $this->_getOptionDownloadUrl ( $urlRoute, $urlParams ), $this->_escaper->escapeHtml ( $title ), $sizes );
		} catch ( \Exception $e ) {
	 ...
	 * @return string|null @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
