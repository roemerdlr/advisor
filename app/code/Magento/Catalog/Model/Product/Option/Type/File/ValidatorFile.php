...
class ValidatorFile extends Validator {
	 ...
	 * @throws \Zend_File_Transfer_Exception @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function validate($processingParams, $option) {
		...
		try {
			...
			if (! $runValidation) {
				throw new \Magento\Framework\Validator\Exception ( __ ( 'Validation failed. Required options were not filled or the file was not uploaded.' ) );
			}
		...
		if ($upload->isUploaded ( $file ) && $upload->isValid ( $file )) {
			...
			$upload->addFilter ( new \Zend_Filter_File_Rename ( [ 
					'target' => $fileFullPath,
					'overwrite' => true 
			] ) );
