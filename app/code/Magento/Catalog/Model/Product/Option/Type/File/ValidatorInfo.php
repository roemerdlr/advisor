...
class ValidatorInfo extends Validator {
	...
	public function validate($optionValue, $option) {
		...
		if ($validatorChain->isValid ( $this->fileFullPath )) {
			$result = $this->rootDirectory->isReadable ( $this->fileRelativePath ) && isset ( $optionValue ['secret_key'] ) && $this->buildSecretKey ( $this->fileRelativePath ) == $optionValue ['secret_key'];
	...
	protected function initFilePath($optionValue) {
		/**
		 *
		 * @see \Magento\Catalog\Model\Product\Option\Type\File\ValidatorFile::validate There setUserValue() sets correct fileFullPath only for
