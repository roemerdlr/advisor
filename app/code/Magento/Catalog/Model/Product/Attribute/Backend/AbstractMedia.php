 ...
 */
class AbstractMedia extends \Magento\Eav\Model\Entity\Attribute\Backend\AbstractBackend {
	/**
	 * Resource model
	 *
	...
	/**
	 * Check whether file to move exists.
	 * Getting unique name
	 *
	 * @param string $file        	
	 * @param bool $forTmp        	
	...
	protected function getNotDuplicatedFilename($fileName, $dispretionPath) {
		$fileMediaName = $dispretionPath . '/' . \Magento\MediaStorage\Model\File\Uploader::getNewFileName ( $this->mediaConfig->getMediaPath ( $fileName ) );
		$fileTmpMediaName = $dispretionPath . '/' . \Magento\MediaStorage\Model\File\Uploader::getNewFileName ( $this->mediaConfig->getTmpMediaPath ( $fileName ) );
