...
class Attribute extends \Magento\Eav\Model\Entity\Attribute implements \Magento\Catalog\Api\Data\ProductAttributeInterface, \Magento\Eav\Model\Entity\Attribute\ScopedAttributeInterface {
	...
	protected function _isEnabledInFlat() {
		return $this->getData ( 'backend_type' ) == 'static' || $this->_productFlatIndexerHelper->isAddFilterableAttributes () && $this->getData ( 'is_filterable' ) > 0 || $this->getData ( 'used_in_product_listing' ) == 1 || $this->getData ( 'used_for_sort_by' ) == 1;
	}
	...
	protected function _isOriginalIndexable() {
		...
		if (! $this->getOrigData ( 'is_filterable_in_search' ) && ! $this->getOrigData ( 'is_visible_in_advanced_search' ) && ! $this->getOrigData ( 'is_filterable' )) {
			return false;
		}
