...
class Decimal extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function applyFilterToCollection(\Magento\Catalog\Model\Layer\Filter\FilterInterface $filter, $range, $index) {
		...
		$collection->getSelect ()->where ( "{$tableAlias}.value >= ?", $range * ($index - 1) )->where ( "{$tableAlias}.value < ?", $range * $index );
	...
	protected function _getSelect($filter) {
		...
		], 'e.entity_id = decimal_index.entity_id' . ' AND ' . $this->getConnection ()->quoteInto ( 'decimal_index.attribute_id = ?', $attributeId ) . ' AND ' . $this->getConnection ()->quoteInto ( 'decimal_index.store_id = ?', $storeId ), [ ] );
