...
class Flat extends \Magento\Indexer\Model\ResourceModel\AbstractResource {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\Indexer\Table\StrategyInterface $tableStrategy, \Magento\Catalog\Model\CategoryFactory $categoryFactory, \Magento\Catalog\Model\ResourceModel\Category\CollectionFactory $categoryCollectionFactory, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Catalog\Model\Config $catalogConfig, \Magento\Framework\Event\ManagerInterface $eventManager, $connectionName = null) {
	...
	protected function _initInactiveCategoryIds() {
		...
		$this->_eventManager->dispatch ( 'catalog_category_tree_init_inactive_category_ids', [ 
				'tree' => $this 
		] );
		return $this;
	}
	...
	protected function _loadNodes($parentNode = null, $recursionLevel = 0, $storeId = 0, $skipMenuFilter = false) {
		...
		], 'url_rewrite.entity_id = main_table.entity_id AND url_rewrite.is_autogenerated = 1' . $_conn->quoteInto ( ' AND url_rewrite.store_id = ?', $storeId ) . $_conn->quoteInto ( ' AND url_rewrite.entity_type = ?', CategoryUrlRewriteGenerator::ENTITY_TYPE ), [ 
		...
		if (isset ( $children [$path] )) {
			foreach ( $children [$path] as $child ) {
				...
				if ($childrenNodes && isset ( $childrenNodes [$child->getId ()] )) {
					$childrenNodes [$child ['entity_id']]->setChildrenNodes ( [ 
							$child->getId () => $child 
					] );
				} else {
					if ($childrenNodes) {
	...
	public function getNodes($parentId, $recursionLevel = 0, $storeId = 0) {
		if (! $this->_loaded) {
			$selectParent = $this->getConnection ()->select ()->from ( $this->getMainStoreTable ( $storeId ) )->where ( 'entity_id = ?', $parentId );
	...
	public function getChildrenAmount($category, $isActiveFlag = true) {
		...
		$select = $this->getConnection ()->select ()->from ( $_table, "COUNT({$_table}.entity_id)" )->where ( "{$_table}.path LIKE ?", $category->getPath () . '/%' )->where ( "{$_table}.is_active = ?", ( int ) $isActiveFlag );
	...
	public function getProductCount($category) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'catalog_category_product' ), "COUNT({$this->getTable('catalog_category_product')}.product_id)" )->where ( "{$this->getTable('catalog_category_product')}.category_id = ?", $category->getId () )->group ( "{$this->getTable('catalog_category_product')}.category_id" );
		return ( int ) $this->getConnection ()->fetchOne ( $select );
	}
	...
	public function getParentCategories($category, $isActive = true) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'url_rewrite' => $this->getTable ( 'url_rewrite' ) 
		], 'url_rewrite.entity_id = main_table.entity_id AND url_rewrite.is_autogenerated = 1' . $connection->quoteInto ( ' AND url_rewrite.store_id = ?', $category->getStoreId () ) . $connection->quoteInto ( ' AND url_rewrite.entity_type = ?', CategoryUrlRewriteGenerator::ENTITY_TYPE ), [ 
				'request_path' => 'url_rewrite.request_path' 
	...
	public function getParentDesignCategory($category) {
		...
		$collection->setMainTable ( $this->getMainStoreTable ( $category->getStoreId () ) )->addFieldToSelect ( '*' )->addFieldToFilter ( 'entity_id', [ 
				'in' => $pathIds 
		] )->addFieldToFilter ( 'custom_use_parent_settings', [ 
				[ 
						'eq' => 0 
				],
				[ 
	...
	public function getChildren($category, $recursive = true, $isActive = true) {
		$select = $this->getConnection ()->select ()->from ( $this->getMainStoreTable ( $category->getStoreId () ), 'entity_id' )->where ( 'path LIKE ?', "{$category->getPath()}/%" );
		if (! $recursive) {
	...
	public function checkId($id) {
		$select = $this->getConnection ()->select ()->from ( $this->getMainStoreTable ( $this->getStoreId () ), 'entity_id' )->where ( 'entity_id=?', $id );
	...
	public function getAnchorsAbove(array $filterIds, $storeId = 0) {
		...
		], 'entity_id' )->where ( 'is_anchor = ?', 1 )->where ( 'entity_id IN (?)', $filterIds );
	...
	public function getProductsPosition($category) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'catalog_category_product' ), [ 
				'product_id',
				'position' 
