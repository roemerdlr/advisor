...
class Collection extends \Magento\Catalog\Model\ResourceModel\Collection\AbstractCollection {
	 ...
	 *
	 * @param array $items        	
	 * @param boolean $countRegular
	 *        	get product count for regular (non-anchor) categories
	 * @param boolean $countAnchor
	 *        	get product count for anchor categories
	...
	public function loadProductCount($items, $countRegular = true, $countAnchor = true) {
		...
		if ($countRegular) {
			...
			if (! empty ( $regularIds )) {
				...
				] )->where ( $this->_conn->quoteInto ( 'main_table.category_id IN(?)', $regularIds ) )->group ( 'main_table.category_id' );
				if ($websiteId) {
					$select->join ( [ 
							'w' => $this->getProductWebsiteTable () 
					], 'main_table.product_id = w.product_id', [ ] )->where ( 'w.website_id = ?', $websiteId );
				}
		...
		if ($countAnchor) {
			...
			foreach ( $anchor as $item ) {
				if ($allChildren = $item->getAllChildren ()) {
					...
					$select->from ( [ 
							'main_table' => $this->getProductTable () 
					], new \Zend_Db_Expr ( 'COUNT(DISTINCT main_table.product_id)' ) )->joinInner ( [ 
							'e' => $this->getTable ( 'catalog_category_entity' ) 
					], 'main_table.category_id=e.entity_id', [ ] )->where ( 'e.entity_id = :entity_id' )->orWhere ( 'e.path LIKE :c_path' );
					if ($websiteId) {
						$select->join ( [ 
								'w' => $this->getProductWebsiteTable () 
						], 'main_table.product_id = w.product_id', [ ] )->where ( 'w.website_id = ?', $websiteId );
	...
	public function joinUrlRewrite() {
		$this->joinTable ( 'url_rewrite', 'entity_id = entity_id', [ 
				'request_path' 
		], sprintf ( '{{table}}.is_autogenerated = 1 AND {{table}}.store_id = %d AND {{table}}.entity_type = \'%s\'', $this->_storeManager->getStore ()->getId (), CategoryUrlRewriteGenerator::ENTITY_TYPE ), 'left' );
		return $this;
	}
