...
class Tree extends \Magento\Framework\Data\Tree\Dbp {
	...
	public function __construct(\Magento\Catalog\Model\ResourceModel\Category $catalogCategory, \Magento\Framework\App\CacheInterface $cache, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\App\ResourceConnection $resource, \Magento\Framework\Event\ManagerInterface $eventManager, \Magento\Catalog\Model\Attribute\Config $attributeConfig, \Magento\Catalog\Model\ResourceModel\Category\Collection\Factory $collectionFactory) {
		...
		parent::__construct ( $resource->getConnection ( 'catalog' ), $resource->getTableName ( 'catalog_category_entity' ), [ 
	...
	protected function _getInactiveItemIds($collection, $storeId) {
		...
		$select = $this->_conn->select ()->from ( [ 
				...
				'd.entity_id' 
		] )->where ( 'd.attribute_id = :attribute_id' )->where ( 'd.store_id = :zero_store_id' )->where ( 'd.entity_id IN (?)', new \Zend_Db_Expr ( $filter ) )->joinLeft ( [ 
				'c' => $table 
		], 'c.attribute_id = :attribute_id AND c.store_id = :store_id AND c.entity_id = d.entity_id', [ ] )->where ( $conditionSql . ' = :cond' );
	 ...
	 * @return $this|bool @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	public function loadByIds($ids, $addCollectionData = true, $updateAnchorProductCount = true) {
		...
		$select = $this->_conn->select ()->from ( $this->_table, [ 
				'path',
				'level' 
	...
	public function loadBreadcrumbsArray($path, $addCollectionData = true, $withRootNode = false) {
		...
		if (! empty ( $pathIds )) {
			...
			$select->where ( 'e.entity_id IN(?)', $pathIds )->order ( $this->_conn->getLengthSql ( 'e.path' ) . ' ' . \Magento\Framework\DB\Select::SQL_ASC );
	...
	protected function _createCollectionDataSelect($sorted = true, $optionalAttributes = []) {
		...
		foreach ( $attributes as $attributeCode ) {
			...
			if (! $attribute->getBackend ()->isStatic ()) {
				...
				], sprintf ( '%1$s.entity_id=e.entity_id AND %1$s.attribute_id=%2$d AND %1$s.store_id=%3$d', $tableDefault, $attribute->getId (), \Magento\Store\Model\Store::DEFAULT_STORE_ID ), [ 
						$attributeCode => 'value' 
				] )->joinLeft ( [ 
						$tableStore => $attribute->getBackend ()->getTable () 
				], sprintf ( '%1$s.entity_id=e.entity_id AND %1$s.attribute_id=%2$d AND %1$s.store_id=%3$d', $tableStore, $attribute->getId (), $this->getStoreId () ), [ 
		...
		$subConcat = $this->_conn->getConcatSql ( [ 
				'e.path',
				$this->_conn->quote ( '/%' ) 
		] );
		...
		] )->where ( 'see.entity_id = e.entity_id' )->orWhere ( 'see.path LIKE ?', $subConcat );
		$select->columns ( [ 
				'product_count' => $subSelect 
		] );
		
		$subSelect = $this->_conn->select ()->from ( [ 
				'cp' => $categoriesProductsTable 
		], 'COUNT(cp.product_id)' )->where ( 'cp.category_id = e.entity_id' );
		
		$select->columns ( [ 
				'self_product_count' => $subSelect 
		] );
