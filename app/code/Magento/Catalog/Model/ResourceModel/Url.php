...
class Url extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Eav\Model\Config $eavConfig, Product $productResource, \Magento\Catalog\Model\Category $catalogCategory, \Psr\Log\LoggerInterface $logger, $connectionName = null) {
	...
	protected function _getCategoryAttribute($attributeCode, $categoryIds, $storeId) {
		...
		if ($this->_categoryAttributes [$attributeCode] ['is_static']) {
			...
			$select->from ( $attributeTable, [ 
					'entity_id',
					'value' 
			] )->where ( 'attribute_id = :attribute_id' )->where ( 'store_id = ?', 0 )->where ( 'entity_id IN(?)', $categoryIds );
			...
			$select->from ( [ 
					...
					't2' => $attributeTable 
			], 't1.entity_id = t2.entity_id AND t1.attribute_id = t2.attribute_id AND t2.store_id = :store_id', [ ] )->where ( 't1.store_id = ?', 0 )->where ( 't1.attribute_id = :attribute_id' )->where ( 't1.entity_id IN(?)', $categoryIds );
	...
	public function _getProductAttribute($attributeCode, $productIds, $storeId) {
		...
		if ($this->_productAttributes [$attributeCode] ['is_global'] || $storeId == 0) {
			$select->from ( $attributeTable, [ 
					'entity_id',
					'value' 
			] )->where ( 'attribute_id = :attribute_id' )->where ( 'store_id = ?', 0 )->where ( 'entity_id IN(?)', $productIds );
		} else {
			$valueExpr = $connection->getCheckSql ( 't2.value_id > 0', 't2.value', 't1.value' );
			$select->from ( [ 
					't1' => $attributeTable 
					...
					't2' => $attributeTable 
			], 't1.entity_id = t2.entity_id AND t1.attribute_id = t2.attribute_id AND t2.store_id=:store_id', [ ] )->where ( 't1.store_id = ?', 0 )->where ( 't1.attribute_id = :attribute_id' )->where ( 't1.entity_id IN(?)', $productIds );
			$bind ['store_id'] = $storeId;
		}
	...
	protected function _getCategories($categoryIds, $storeId = null, $path = null) {
		...
		$select->joinLeft ( [ 
				'd' => $table 
		], 'd.attribute_id = :attribute_id AND d.store_id = 0 AND d.entity_id = main_table.entity_id', [ ] )->joinLeft ( [ 
				'c' => $table 
	...
	/**
	 * Retrieve categories data objects by their ids.
	 * Return only categories that belong to specified store.
	 *
	 * @param int|array $categoryIds        	
	 * @param int $storeId        	
	 * @return array|false
	...
	protected function _getProducts($productIds, $storeId, $entityId, &$lastEntityId) {
		...
		$select = $connection->select ()->useStraightJoin ( true )->from ( [ 
				'e' => $this->getTable ( 'catalog_product_entity' ) 
		], [ 
				'entity_id' 
		] )->join ( [ 
				'w' => $this->getTable ( 'catalog_product_website' ) 
		], 'e.entity_id = w.product_id AND w.website_id = :website_id', [ ] )->where ( 'e.entity_id > :entity_id' )->order ( 'e.entity_id' )->limit ( $this->_productLimit );
		if ($productIds !== null) {
		...
		if ($products) {
			$select = $connection->select ()->from ( $this->getTable ( 'catalog_category_product' ), [ 
					'product_id',
					'category_id' 
	...
	public function getRewriteByProductStore(array $products) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'u' => $this->getMainTable () 
		], 'i.product_id = u.entity_id AND i.store_id = u.store_id' . ' AND u.entity_type = "' . ProductUrlRewriteGenerator::ENTITY_TYPE . '"', [ 
				'request_path' 
		] )->joinLeft ( [ 
