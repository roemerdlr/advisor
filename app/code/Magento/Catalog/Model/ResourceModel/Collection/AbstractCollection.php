	...
	protected function _getLoadAttributesSelect($table, $attributeIds = []) {
		...
		if ($storeId) {
			...
			$select = $connection->select ()->from ( [ 
					...
					$entityIdField,
					'attribute_id' 
			] )->where ( "t_d.{$entityIdField} IN (?)", array_keys ( $this->_itemsById ) )->where ( 't_d.attribute_id IN (?)', $attributeIds )->joinLeft ( [ 
					't_s' => $table 
			], implode ( ' AND ', $joinCondition ), [ ] )->where ( 't_d.store_id = ?', $connection->getIfNullSql ( 't_s.store_id', \Magento\Store\Model\Store::DEFAULT_STORE_ID ) );
		} else {
			$select = $connection->select ()->from ( $table, [ 
					$entityIdField,
					'attribute_id' 
			] )->where ( "{$entityIdField} IN (?)", array_keys ( $this->_itemsById ) )->where ( 'attribute_id IN (?)', $attributeIds )->where ( 'store_id = ?', $this->getDefaultStoreId () );
		}
		return $select;
	}
