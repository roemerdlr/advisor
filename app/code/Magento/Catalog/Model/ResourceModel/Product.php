...
class Product extends AbstractResource {
	...
	public function getWebsiteIds($product) {
		...
		$select = $connection->select ()->from ( $this->getProductWebsiteTable (), 'website_id' )->where ( 'product_id = ?', ( int ) $productId );
	...
	public function getWebsiteIdsByProductIds($productIds) {
		$select = $this->getConnection ()->select ()->from ( $this->getProductWebsiteTable (), [ 
				'product_id',
				'website_id' 
	...
	public function getCategoryIds($product) {
		...
		$select = $connection->select ()->from ( $this->getProductCategoryTable (), 'category_id' )->where ( 'product_id = ?', ( int ) $product->getId () );
	...
	public function delete($object) {
		...
		$this->eventManager->dispatch ( 'catalog_product_delete_after_done', [ 
				'product' => $object 
		] );
		return $result;
	}
	...
	public function getAvailableInCategories($object) {
		...
		$select = $this->getConnection ()->select ()->distinct ()->from ( $this->getTable ( 'catalog_category_product_index' ), [ 
				'category_id' 
		] )->where ( 'product_id = ? AND is_parent = 1', ( int ) $object->getEntityId () )->where ( 'visibility != ?', \Magento\Catalog\Model\Product\Visibility::VISIBILITY_NOT_VISIBLE );
	...
	public function canBeShowInCategory($product, $categoryId) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'catalog_category_product_index' ), 'product_id' )->where ( 'product_id = ?', ( int ) $product->getId () )->where ( 'category_id = ?', ( int ) $categoryId );
	...
	public function duplicate($oldId, $newId) {
		...
		foreach ( $eavTables as $suffix ) {
			...
			$select = $connection->select ()->from ( $tableName, [ 
					...
					'value' 
			] )->where ( 'entity_id = ?', $oldId )->where ( 'store_id > ?', 0 );
		...
		$connection->update ( $statusAttributeTable, [ 
	...
	public function getProductsSku(array $productIds) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'catalog_product_entity' ), [ 
				'entity_id',
				'sku' 
	...
	public function getAssignedImages($product, $storeIds) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'attribute_code' 
		] )->where ( 'entity_id = ?', $product->getId () )->where ( 'store_id IN (?)', $storeIds )->where ( 'attribute_code IN (?)', [ 
				'small_image',
				'thumbnail',
				'image' 
		] );
	...
	public function validate($object) {
		...
		if ($attributeSet->getEntityTypeId () != $entityType->getId ()) {
			return [ 
					'attribute_set' => 'Invalid attribute set entity type' 
			];
		}
