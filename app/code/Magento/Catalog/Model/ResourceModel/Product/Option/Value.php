...
class Value extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	protected function _saveValuePrices(\Magento\Framework\Model\AbstractModel $object) {
		...
		if ($object->getPrice () && $priceType) {
			// save for store_id = 0
			$select = $this->getConnection ()->select ()->from ( $priceTable, 'option_type_id' )->where ( 'option_type_id = ?', ( int ) $object->getId () )->where ( 'store_id = ?', \Magento\Store\Model\Store::DEFAULT_STORE_ID );
		...
		if ($scope == \Magento\Store\Model\Store::PRICE_SCOPE_WEBSITE && $priceType && $object->getPrice () && $object->getStoreId () != \Magento\Store\Model\Store::DEFAULT_STORE_ID) {
			...
			if (is_array ( $storeIds )) {
				foreach ( $storeIds as $storeId ) {
					...
					$select = $this->getConnection ()->select ()->from ( $priceTable, 'option_type_id' )->where ( 'option_type_id = ?', ( int ) $object->getId () )->where ( 'store_id = ?', ( int ) $storeId );
					$optionTypeId = $this->getConnection ()->fetchOne ( $select );
			...
			if ($scope == \Magento\Store\Model\Store::PRICE_SCOPE_WEBSITE && ! $object->getPrice () && ! $priceType) {
				$storeIds = $this->_storeManager->getStore ( $object->getStoreId () )->getWebsite ()->getStoreIds ();
				foreach ( $storeIds as $storeId ) {
					$where = [ 
							'option_type_id = ?' => ( int ) $object->getId (),
	...
	protected function _saveValueTitles(\Magento\Framework\Model\AbstractModel $object) {
		foreach ( [ 
			...
			$select = $this->getConnection ()->select ()->from ( $titleTable, [ 
					'option_type_id' 
			] )->where ( 'option_type_id = ?', ( int ) $object->getId () )->where ( 'store_id = ?', ( int ) $storeId );
			$optionTypeId = $this->getConnection ()->fetchOne ( $select );
	...
	protected function getOptionIdFromOptionTable($tableName, $optionId, $storeId) {
		...
		$select = $connection->select ()->from ( $tableName, [ 
				'option_type_id' 
		] )->where ( 'option_type_id = ?', $optionId )->where ( 'store_id = ?', ( int ) $storeId );
	...
	public function deleteValue($optionId) {
		$statement = $this->getConnection ()->select ()->from ( $this->getTable ( 'catalog_product_option_type_value' ) )->where ( 'option_id = ?', $optionId );
	...
	public function duplicate(\Magento\Catalog\Model\Product\Option\Value $object, $oldOptionId, $newOptionId) {
		...
		foreach ( $valueCond as $oldTypeId => $newTypeId ) {
			...
			$select = $connection->select ()->from ( $priceTable, [ ] )->where ( 'option_type_id = ?', $oldTypeId )->columns ( $columns );
			$insertSelect = $connection->insertFromSelect ( $select, $priceTable, [ 
					'option_type_id',
					'store_id',
					'price',
					'price_type' 
			...
			$select = $this->getConnection ()->select ()->from ( $titleTable, [ ] )->where ( 'option_type_id = ?', $oldTypeId )->columns ( $columns );
			$insertSelect = $connection->insertFromSelect ( $select, $titleTable, [ 
					'option_type_id',
					'store_id',
					'title' 
			] );
