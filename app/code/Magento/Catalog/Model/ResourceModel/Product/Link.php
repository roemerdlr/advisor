...
class Link extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, Relation $catalogProductRelation, $connectionName = null) {
	...
	public function saveProductLinks($product, $data, $typeId) {
		...
		$select = $connection->select ()->from ( $this->getMainTable (), [ 
				'linked_product_id',
				'link_id' 
		] )->where ( 'product_id = :product_id' )->where ( 'link_type_id = :link_type_id' );
			...
			foreach ( $attributes as $attributeInfo ) {
				...
				if ($attributeTable) {
					if (isset ( $linkInfo [$attributeInfo ['code']] )) {
						...
						$connection->insertOnDuplicate ( $attributeTable, $bind, [ 
								'value' 
						] );
					} else {
						$connection->delete ( $attributeTable, [ 
								'link_id = ?' => $linkId,
								'product_link_attribute_id = ?' => $attributeInfo ['id'] 
						] );
					}
	...
	public function getAttributesByType($typeId) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->_attributesTable, [ 
	...
	public function getParentIdsByChild($childId, $typeId) {
		...
		$select = $connection->select ()->from ( $this->getMainTable (), [ 
				'product_id',
				'linked_product_id' 
		] )->where ( 'linked_product_id IN(?)', $childId )->where ( 'link_type_id = ?', $typeId );
