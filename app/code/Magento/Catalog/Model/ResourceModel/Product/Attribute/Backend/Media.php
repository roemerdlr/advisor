...
class Media extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function loadDataFromTableByValueId($tableNameAlias, array $ids, $storeId = null, array $cols = null, array $leftJoinTables = [], $whereCondition = null) {
		...
		], $cols )->where ( $mainTableAlias . '.value_id IN(?)', $ids );
		if (null !== $storeId) {
	...
	protected function createBaseLoadSelect($entityId, $storeId, $attributeId) {
		...
		$positionCheckSql = $connection->getCheckSql ( 'value.position IS NULL', 'default_value.position', 'value.position' );
		...
		] )->where ( $mainTableAlias . '.attribute_id = ?', $attributeId )->where ( $mainTableAlias . '.disabled = 0' )->where ( 'entity.entity_id = ?', $entityId )->order ( $positionCheckSql . ' ' . \Magento\Framework\DB\Select::SQL_ASC );
	...
	public function duplicate($attributeId, $newFiles, $originalProductId, $newProductId) {
		...
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( self::GALLERY_VALUE_TABLE ) )->where ( 'value_id IN(?)', array_keys ( $valueIdMap ) );
	...
	public function loadMediaGalleryEntitiesbyId($valueIds) {
		$select = $this->getConnection ()->select ()->from ( $this->getMainTable () )->where ( 'value_id IN(?)', $valueIds );
	...
	public function loadMediaGalleryEntities($attributeId, $productId) {
		...
		$select = $this->getConnection ()->select ()->from ( [ 
				...
				'entity_id' => 'entity_id' 
		] )->where ( 'attribute_id = ?', $attributeId )->where ( 'entity.entity_id = ?', $productId );
