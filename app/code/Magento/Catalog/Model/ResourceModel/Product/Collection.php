...
class Collection extends \Magento\Catalog\Model\ResourceModel\Collection\AbstractCollection {
	...
	protected function _afterLoad() {
		...
		if (count ( $this )) {
			$this->_eventManager->dispatch ( 'catalog_product_collection_load_after', [ 
					'collection' => $this 
			] );
		}
	...
	protected function doAddWebsiteNamesToResult() {
		...
		if (! empty ( $productWebsites )) {
			$select = $this->getConnection ()->select ()->from ( [ 
					...
					'name' 
			] )->where ( 'product_website.product_id IN (?)', array_keys ( $productWebsites ) )->where ( 'website.website_id > ?', 0 );
	...
	public function addWebsiteFilter($websites = null) {
		if (! is_array ( $websites )) {
			$websites = [ 
					$this->_storeManager->getWebsite ( $websites )->getId () 
			];
		}
	...
	public function addCategoriesFilter(array $categoriesFilter) {
		foreach ( $categoriesFilter as $conditionType => $values ) {
			...
			], 'cat.product_id' )->where ( $this->getConnection ()->prepareSqlCondition ( 'cat.category_id', [ 
					'in' => $values 
			] ) );
			$selectCondition = [ 
	...
	public function getMaxAttributeValue($attribute) {
		...
		$condition = 'e.entity_id = ' . $tableAlias . '.entity_id AND ' . $this->_getConditionSql ( $tableAlias . '.attribute_id', $attribute->getId () );
	...
	public function getAllAttributeValues($attribute) {
		...
		$select->reset ()->from ( $attribute->getBackend ()->getTable (), [ 
				'entity_id',
				'store_id',
				'value' 
	...
	public function getProductCountSelect() {
		if ($this->_productCountSelect === null) {
			...
			$this->_productCountSelect->reset ( \Magento\Framework\DB\Select::COLUMNS )->reset ( \Magento\Framework\DB\Select::GROUP )->reset ( \Magento\Framework\DB\Select::ORDER )->distinct ( false )->join ( [ 
			...
			] )->where ( 'count_table.store_id = ?', $this->getStoreId () )->group ( 'count_table.category_id' );
		}
	...
	public function addCountToCategories($categoryCollection) {
		...
		if ($isAnchor || $isNotAnchor) {
			...
			$this->_eventManager->dispatch ( 'catalog_product_collection_before_add_count_to_categories', [ 
					'collection' => $this 
			] );
	...
	public function joinUrlRewrite() {
		$this->joinTable ( 'url_rewrite', 'entity_id = entity_id', [ 
				'request_path' 
		], '{{table}}.entity_type = \'' . ProductUrlRewriteGenerator::ENTITY_TYPE . '\'', 'left' );
		return $this;
	}
	...
	public function addUrlRewrite($categoryId = '') {
		...
		$useCategoryUrl = $this->_scopeConfig->getValue ( \Magento\Catalog\Helper\Product::XML_PATH_PRODUCT_URL_USE_CATEGORY, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->getStoreId () );
		if ($useCategoryUrl) {
	...
	protected function _addUrlRewrite() {
		...
		$select = $this->getConnection ()->select ()->from ( [ 
				'u' => $this->getTable ( 'url_rewrite' ) 
		], [ 
				'u.entity_id',
				'u.request_path' 
		] )->where ( 'u.store_id = ?', $this->_storeManager->getStore ( $this->getStoreId () )->getId () )->where ( 'u.is_autogenerated = 1' )->where ( 'u.entity_type = ?', ProductUrlRewriteGenerator::ENTITY_TYPE )->where ( 'u.entity_id IN(?)', $productIds );
		
		if ($this->_urlRewriteCategory) {
			$select->joinInner ( [ 
					'cu' => $this->getTable ( 'catalog_url_rewrite_product_category' ) 
			], 'u.url_rewrite_id=cu.url_rewrite_id' )->where ( 'cu.category_id IN (?)', $this->_urlRewriteCategory );
		}
				...
				if ($alias == 'is_saleable') {
					if ($column instanceof \Zend_Db_Expr) {
						...
						if (empty ( $correlationName )) {
							...
							$field = $connection->quoteColumnAs ( [ 
									$correlationName,
									$column 
							], $alias, true );
						}
	...
	protected function _prepareProductLimitationFilters() {
		...
		if (isset ( $this->_productLimitationFilters ['category_id'] ) && ! isset ( $this->_productLimitationFilters ['store_id'] )) {
			$this->_productLimitationFilters ['store_id'] = $this->getStoreId ();
		}
		if (isset ( $this->_productLimitationFilters ['store_id'] ) && isset ( $this->_productLimitationFilters ['visibility'] ) && ! isset ( $this->_productLimitationFilters ['category_id'] )) {
			$this->_productLimitationFilters ['category_id'] = $this->_storeManager->getStore ( $this->_productLimitationFilters ['store_id'] )->getRootCategoryId ();
		}
	...
	protected function _productLimitationJoinWebsite() {
		...
		if (isset ( $filters ['website_ids'] )) {
			...
			$conditions [] = $this->getConnection ()->quoteInto ( 'product_website.website_id IN(?)', $filters ['website_ids'] );
		} elseif (isset ( $filters ['store_id'] ) && (! isset ( $filters ['visibility'] ) && ! isset ( $filters ['category_id'] )) && ! $this->isEnabledFlat ()) {
			$joinWebsite = true;
	...
	protected function _applyProductLimitations() {
		...
		$this->_eventManager->dispatch ( 'catalog_product_collection_apply_limitations_after', [ 
				'collection' => $this 
		] );
		return $this;
	}
	...
	public function addCategoryIds() {
		...
		$select->from ( $this->_productCategoryTable, [ 
				'product_id',
				'category_id' 
		] );
	...
	public function addTierPriceData() {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'catalog_product_entity_tier_price' ), $columns )->where ( 'entity_id IN(?)', $productIds )->order ( [ 
				'entity_id',
				'qty' 
		] );
	...
	public function addPriceDataFieldFilter($comparisonFormat, $fields) {
		...
		$this->_priceDataFieldFilters [] = array_merge ( [ 
				$comparisonFormat 
		], $fields );
		return $this;
	}
