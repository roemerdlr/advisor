...
abstract class AbstractIndexer extends \Magento\Indexer\Model\ResourceModel\AbstractResource {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\Indexer\Table\StrategyInterface $tableStrategy, \Magento\Eav\Model\Config $eavConfig, $connectionName = null) {
	 ...
	 *        	the entity field or expression for condition
	 * @param string|\Zend_Db_Expr $store
	 *        	the store field or expression for condition
	 * @param \Zend_Db_Expr $condition
	 *        	the limitation condition
	 * @param bool $required
	 *        	if required or has condition used INNER join, else - LEFT
	 ...
	 *        	the select object
	 * @param bool $store
	 *        	add default store join
	 * @param string|\Zend_Db_Expr $joinCondition
	 *        	the limitation for website_id
	 * @return $this
	 */
	 ...
	 * @param \Magento\Framework\DB\Select $select
	 *        	the select object
	 * @param string|\Zend_Db_Expr $website
	 *        	the limitation of website_id
	 * @param string|\Zend_Db_Expr $product
	 *        	the limitation of product_id
	 * @return $this
	 */
	...
	public function getRelationsByChild($childIds) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->getTable ( 'catalog_product_relation' ), 'parent_id' )->where ( 'child_id IN(?)', $childIds );
	...
	public function getRelationsByParent($parentIds) {
		...
		if (! empty ( $parentIds )) {
			$connection = $this->getConnection ();
			$select = $connection->select ()->from ( $this->getTable ( 'catalog_product_relation' ), 'child_id' )->where ( 'parent_id IN(?)', $parentIds );
