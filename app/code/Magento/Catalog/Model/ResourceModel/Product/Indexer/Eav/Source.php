...
class Source extends AbstractEav {
	...
	protected function _getIndexableAttributes($multiSelect) {
		...
		], 'ca.attribute_id = ea.attribute_id', [ ] )->where ( $this->_getIndexableAttributesCondition () );
	...
	protected function _prepareSelectIndex($entityIds = null, $attributeId = null) {
		...
		], sprintf ( 'd.entity_id = d2.entity_id AND d2.attribute_id = %s AND d2.value = %s AND d.store_id = d2.store_id', $this->_eavConfig->getAttribute ( \Magento\Catalog\Model\Product::ENTITY, 'status' )->getId (), ProductStatus::STATUS_ENABLED ), [ ] )->where ( 's.store_id != 0' )->where ( 'd.value IS NOT NULL' )->where ( 'd2.value IS NOT NULL' )->group ( [ 
				's.store_id',
				's.website_id',
				'd.entity_id',
				'd.attribute_id',
		...
		], 'pis.entity_id = pid.entity_id AND pis.attribute_id = pid.attribute_id AND pis.store_id = pid.store_id', [ ] )->columns ( [ 
				'pid.entity_id',
				'pid.attribute_id',
				'pid.store_id',
				'value' => $ifNullSql 
	...
	protected function _prepareMultiselectIndex($entityIds = null, $attributeId = null) {
		...
		$options = [ ];
		$select = $connection->select ()->from ( $this->getTable ( 'eav_attribute_option' ), [ 
				'attribute_id',
				'option_id' 
		...
		] )->where ( 'pvd.store_id=?', $connection->getIfNullSql ( 'pvs.store_id', \Magento\Store\Model\Store::DEFAULT_STORE_ID ) )->where ( 'cs.store_id!=?', \Magento\Store\Model\Store::DEFAULT_STORE_ID )->where ( 'pvd.attribute_id IN(?)', $attrIds );
	...
	protected function _saveIndexData(array $data) {
		...
		$connection = $this->getConnection ();
		$connection->insertArray ( $this->getIdxTable (), [ 
				'entity_id',
				'attribute_id',
				'store_id',
				'value' 
