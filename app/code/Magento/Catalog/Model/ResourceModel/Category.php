...
class Category extends AbstractResource {
	...
	protected function _beforeDelete(\Magento\Framework\DataObject $object) {
		...
		if ($parentIds) {
			...
			// +1 is itself
			$data = [ 
					'children_count' => new \Zend_Db_Expr ( 'children_count - ' . $childDecrease ) 
			];
			$where = [ 
	...
	public function deleteChildren(\Magento\Framework\DataObject $object) {
		...
		$select = $connection->select ()->from ( $this->getEntityTable (), [ 
				'entity_id' 
		] )->where ( $pathField . ' LIKE :c_path' );
		
		$childrenIds = $connection->fetchCol ( $select, [ 
				'c_path' => $object->getPath () . '/%' 
		] );
		
		if (! empty ( $childrenIds )) {
			$connection->delete ( $this->getEntityTable (), [ 
	...
	protected function _beforeSave(\Magento\Framework\DataObject $object) {
		...
		if ($object->isObjectNew ()) {
			...
			$this->getConnection ()->update ( $this->getEntityTable (), [ 
	...
	protected function _getMaxPosition($path) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'catalog_category_entity' ), 'MAX(' . $positionField . ')' )->where ( $connection->quoteIdentifier ( 'path' ) . ' LIKE :c_path' )->where ( $connection->quoteIdentifier ( 'level' ) . ' = :c_level' );
	...
	protected function _saveCategoryProducts($category) {
		...
		if (! empty ( $insert ) || ! empty ( $delete )) {
			...
			$this->_eventManager->dispatch ( 'catalog_category_change_products', [ 
					'category' => $category,
	...
	public function getProductsPosition($category) {
		$select = $this->getConnection ()->select ()->from ( $this->getCategoryProductTable (), [ 
				'product_id',
				'position' 
	...
	public function getChildrenCount($categoryId) {
		$select = $this->getConnection ()->select ()->from ( $this->getEntityTable (), 'children_count' )->where ( 'entity_id = :entity_id' );
		$bind = [ 
	...
	public function checkId($entityId) {
		$select = $this->getConnection ()->select ()->from ( $this->getEntityTable (), 'entity_id' )->where ( 'entity_id = :entity_id' );
		$bind = [ 
				'entity_id' => $entityId 
		];
	...
	public function verifyIds(array $ids) {
		...
		$select = $this->getConnection ()->select ()->from ( $this->getEntityTable (), 'entity_id' )->where ( 'entity_id IN(?)', $ids );
	...
	public function getChildrenAmount($category, $isActiveFlag = true) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'd' => $table 
		], 'd.attribute_id = :attribute_id AND d.store_id = 0 AND d.entity_id = m.entity_id', [ ] )->joinLeft ( [ 
				'c' => $table 
		], "c.attribute_id = :attribute_id AND c.store_id = :store_id AND c.entity_id = m.entity_id", [ ] )->where ( 'm.path LIKE :c_path' )->where ( $checkSql . ' = :active_flag' );
	...
	public function getIsActiveAttributeId() {
		if ($this->_isActiveAttributeId === null) {
			$this->_isActiveAttributeId = ( int ) $this->_eavConfig->getAttribute ( $this->getEntityType (), 'is_active' )->getAttributeId ();
		}
	...
	public function findWhereAttributeIs($entityIdsFilter, $attribute, $expectedValue) {
		...
		$select = $this->getConnection ()->select ()->from ( $attribute->getBackend ()->getTable (), [ 
				'entity_id' 
		] )->where ( 'attribute_id = :attribute_id' )->where ( 'value = :value' )->where ( 'entity_id IN(?)', $entityIdsFilter );
	...
	public function getParentCategories($category) {
		...
		return $categories->setStore ( $this->_storeManager->getStore () )->addAttributeToSelect ( 'name' )->addAttributeToSelect ( 'url_key' )->addFieldToFilter ( 'entity_id', [ 
				'in' => $pathIds 
		] )->addFieldToFilter ( 'is_active', 1 )->load ()->getItems ();
	}
	...
	public function getParentDesignCategory($category) {
		...
		$collection = $category->getCollection ()->setStore ( $this->_storeManager->getStore () )->addAttributeToSelect ( 'custom_design' )->addAttributeToSelect ( 'custom_design_from' )->addAttributeToSelect ( 'custom_design_to' )->addAttributeToSelect ( 'page_layout' )->addAttributeToSelect ( 'custom_layout_update' )->addAttributeToSelect ( 'custom_apply_to_products' )->addFieldToFilter ( 'entity_id', [ 
				'in' => $pathIds 
		] )->addAttributeToFilter ( 'custom_use_parent_settings', [ 
				[ 
						'eq' => 0 
				],
				[ 
		...
		], 'left' )->addFieldToFilter ( 'level', [ 
				'neq' => 0 
		] )->setOrder ( 'level', 'DESC' )->load ();
	...
	public function getChildren($category, $recursive = true) {
		...
		$select = $this->getConnection ()->select ()->from ( [ 
				...
				'c' => $backendTable 
		], 'c.attribute_id = :attribute_id AND c.store_id = :store_id AND c.entity_id = m.entity_id', [ ] )->where ( $checkSql . ' = :scope' )->where ( $connection->quoteIdentifier ( 'path' ) . ' LIKE :c_path' );
		if (! $recursive) {
	...
	public function isForbiddenToDelete($categoryId) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'store_group' ), [ 
				'group_id' 
	...
	public function getCategoryPathById($categoryId) {
		$select = $this->getConnection ()->select ()->from ( $this->getEntityTable (), [ 
				'path' 
	...
	public function changeParent(\Magento\Catalog\Model\Category $category, \Magento\Catalog\Model\Category $newParent, $afterCategoryId = null) {
		 ...
		 */
		$connection->update ( $table, [ 
				'path' => new \Zend_Db_Expr ( 'REPLACE(' . $pathField . ',' . $connection->quote ( $category->getPath () . '/' ) . ', ' . $connection->quote ( $newPath . '/' ) . ')' ),
