...
class QuoteItemProductOption {
	 ...
	 * @return \Magento\Sales\Model\Order\Item @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
			...
			foreach ( $item->getOptions () as $itemOption ) {
				...
				if (isset ( $code [1] ) && is_numeric ( $code [1] )) {
					...
					if ($option && $option->getType () == \Magento\Catalog\Model\Product\Option::OPTION_TYPE_FILE) {
						try {
							$option->groupFactory ( $option->getType () )->setQuoteItemOption ( $itemOption )->copyQuoteToOrder ();
						} catch ( \Exception $e ) {
							continue;
						}
					}
