...
class Product extends \Magento\Catalog\Model\AbstractModel implements IdentityInterface, SaleableInterface, ProductInterface {
	 ...
	 *
	 * @param \Magento\Catalog\Model\Product\Type\AbstractType|null $instance
	 ...
	 *        	Not used
	 * @return \Magento\Eav\Model\Entity\Attribute\AbstractAttribute[] @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
		...
		if (empty ( $this->_links )) {
			...
			foreach ( array_keys ( $linkTypes ) as $linkTypeName ) {
				...
				foreach ( $collection as $item ) {
					...
					$productLink->setSku ( $this->getSku () )->setLinkType ( $linkTypeName )->setLinkedProductSku ( $item ['sku'] )->setLinkedProductType ( $item ['type'] )->setPosition ( $item ['position'] );
	...
	public function getGalleryAttributeBackend() {
		...
		if (! isset ( $attributes ['media_gallery'] ) || ! ($attributes ['media_gallery'] instanceof \Magento\Eav\Model\Entity\Attribute\AbstractAttribute)) {
			return null;
		}
	 ...
	 *        	code of attribute with type 'media_image',
	...
	public function isSalable() {
		$this->_eventManager->dispatch ( 'catalog_product_is_salable_before', [ 
				'product' => $this 
		] );
		...
		$this->_eventManager->dispatch ( 'catalog_product_is_salable_after', [ 
				'product' => $this,
				'salable' => $object 
		] );
	...
	public function getAttributeText($attributeCode) {
		return $this->getResource ()->getAttribute ( $attributeCode )->getSource ()->getOptionText ( $this->getData ( $attributeCode ) );
	}
	...
	protected function _clearData() {
		foreach ( $this->_data as $data ) {
			if (is_object ( $data ) && method_exists ( $data, 'reset' ) && is_callable ( [ 
					$data,
					'reset' 
			] )) {
				$data->reset ();
