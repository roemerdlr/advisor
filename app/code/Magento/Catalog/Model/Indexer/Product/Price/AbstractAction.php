...
abstract class AbstractAction {
	...
	protected function _syncData(array $processIds = []) {
		...
		], 'index_price.entity_id = ip_tmp.entity_id AND index_price.website_id = ip_tmp.website_id', [ ] )->where ( 'ip_tmp.entity_id IS NULL' );
		if (! empty ( $processIds )) {
		...
		$this->_insertFromTable ( $this->_defaultIndexerResource->getIdxTable (), $this->_defaultIndexerResource->getTable ( 'catalog_product_index_price' ) );
		return $this;
	}
	...
	protected function _prepareTierPriceIndex($entityIds = null) {
		...
		$select = $this->_connection->select ()->from ( [ 
				'tp' => $this->_defaultIndexerResource->getTable ( [ 
		...
		], 'cw.website_id = cwd.website_id', [ ] )->where ( 'cw.website_id != 0' )->columns ( new \Zend_Db_Expr ( "MIN({$websiteExpression})" ) )->group ( [ 
				'tp.entity_id',
	...
	protected function _copyRelationIndexData($parentIds, $excludeIds = null) {
		$select = $this->_connection->select ()->from ( $this->_defaultIndexerResource->getTable ( 'catalog_product_relation' ), [ 
				'child_id' 
