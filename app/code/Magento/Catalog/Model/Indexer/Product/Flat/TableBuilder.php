...
class TableBuilder {
	 ...
	 */
	public function __construct(\Magento\Catalog\Helper\Product\Flat\Indexer $productIndexerHelper, \Magento\Framework\App\ResourceConnection $resource) {
	 ...
	 *        	Table name
	 * @param array $columns
	 *        	array('columnName' => \Magento\Catalog\Model\ResourceModel\Eav\Attribute, ...)
	 * @param string $valueFieldSuffix        	
	 *
	 * @return array
	 */
	...
	protected function _addPrimaryKeyToTable($tableName, $columnName = 'entity_id') {
		$this->_connection->addIndex ( $tableName, 'entity_id', [ 
				$columnName 
		...
		if (! empty ( $tableColumns )) {
			...
			foreach ( $columnsChunks as $columnsList ) {
				...
				foreach ( $columnsList as $columnName => $attribute ) {
					...
					$joinCondition = sprintf ( 'e.entity_id = %1$s.entity_id AND %1$s.attribute_id = %2$d AND %1$s.store_id = 0', $countTableName, $attribute->getId () );
					...
					if ($attribute->getFlatUpdateSelect ( $storeId ) instanceof \Magento\Framework\DB\Select) {
						...
						if (isset ( $flatColumns [$columnValueName] )) {
							...
							$selectValue->joinLeft ( [ 
									$countTableName => $this->_productIndexerHelper->getTable ( 'eav_attribute_option_value' ) 
							], $valueJoinCondition, [ 
