...
class Indexer {
	 ...
	 */
	public function __construct(\Magento\Framework\App\ResourceConnection $resource, \Magento\Catalog\Helper\Product\Flat\Indexer $productHelper) {
	 ...
	 * @return \Magento\Catalog\Model\Indexer\Product\Flat @SuppressWarnings(PHPMD.CyclomaticComplexity)
		...
		foreach ( $eavAttributes as $tableName => $tableColumns ) {
			...
			foreach ( $columnsChunks as $columns ) {
				...
				if ($tableName != $this->_productIndexerHelper->getTable ( 'catalog_product_entity' )) {
					...
					$select->joinLeft ( [ 
							't' => $tableName 
					], 'e.entity_id = t.entity_id ' . $this->_connection->quoteInto ( ' AND t.attribute_id IN (?)', array_keys ( $ids ) ) . ' AND t.store_id = 0', [ ] )->joinLeft ( [ 
							't2' => $tableName 
					], 't.entity_id = t2.entity_id ' . ' AND t.attribute_id = t2.attribute_id  ' . $this->_connection->quoteInto ( ' AND t2.store_id = ?', $storeId ), [ ] )->where ( 'e.entity_id = ' . $productId )->where ( 't.attribute_id IS NOT NULL' );
					...
					if (! empty ( $valueColumns )) {
						...
						$select = $this->_connection->select ()->from ( [ 
								...
								't.option_id',
								't.value' 
						] )->where ( $this->_connection->quoteInto ( 't.option_id IN (?)', $valueIds ) );
						$cursor = $this->_connection->query ( $select );
						while ( $row = $cursor->fetch ( \Zend_Db::FETCH_ASSOC ) ) {
					...
					$select->from ( [ 
							'e' => $this->_productIndexerHelper->getTable ( 'catalog_product_entity' ) 
					], $columnNames )->where ( 'e.entity_id = ' . $productId );
					$cursor = $this->_connection->query ( $select );
