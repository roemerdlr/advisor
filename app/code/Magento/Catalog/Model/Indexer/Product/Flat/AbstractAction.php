...
abstract class AbstractAction {
	 ...
	 */
	public function __construct(\Magento\Framework\App\ResourceConnection $resource, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Catalog\Helper\Product\Flat\Indexer $productHelper, \Magento\Catalog\Model\Product\Type $productType, TableBuilder $tableBuilder, FlatTableBuilder $flatTableBuilder) {
	 ...
	 *        	Update child product(s) only
	 * @return \Magento\Catalog\Model\Indexer\Product\Flat\AbstractAction @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	protected function _updateRelationProducts($storeId, $productIds = null) {
		...
		foreach ( $this->_getProductTypeInstances () as $typeInstance ) {
			...
			if ($relation && $relation->getTable () && $relation->getParentFieldName () && $relation->getChildFieldName ()) {
				...
				], "e.entity_id = t.{$relation->getChildFieldName()}", array_keys ( $columns ) );
		...
		foreach ( $this->_getProductTypeInstances () as $typeInstance ) {
			...
			if ($relation && $relation->getTable () && $relation->getParentFieldName () && $relation->getChildFieldName ()) {
				...
				$select = $this->_connection->select ()->from ( [ 
						...
						't' => $this->_productIndexerHelper->getTable ( $relation->getTable () ) 
				], implode ( ' AND ', $joinLeftCond ), [ ] )->where ( 'e.is_child = ?', 1 )->where ( 'e.entity_id IN(?)', $entitySelect )->where ( "t.{$relation->getChildFieldName()} IS NULL" );
