...
class Row extends \Magento\Catalog\Model\Indexer\Product\Flat\AbstractAction {
	 ...
	 */
	public function __construct(\Magento\Framework\App\ResourceConnection $resource, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Catalog\Helper\Product\Flat\Indexer $productHelper, \Magento\Catalog\Model\Product\Type $productType, TableBuilder $tableBuilder, FlatTableBuilder $flatTableBuilder, Indexer $flatItemWriter, Eraser $flatItemEraser) {
		parent::__construct ( $resource, $storeManager, $productHelper, $productType, $tableBuilder, $flatTableBuilder );
	...
	public function execute($id = null) {
		...
		foreach ( $this->_storeManager->getStores () as $store ) {
			...
			if (isset ( $ids [0] )) {
				if (! $tableExists) {
					$this->_flatTableBuilder->build ( $store->getId (), [ 
							$ids [0] 
					], $this->_valueFieldSuffix, $this->_tableDropSuffix, false );
				}
