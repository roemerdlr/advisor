...
class FlatTableBuilder {
	...
	protected function _fillTemporaryFlatTable(array $tables, $storeId, $valueFieldSuffix) {
		...
		], $allColumns )->joinInner ( [ 
				'wp' => $this->_productIndexerHelper->getTable ( 'catalog_product_website' ) 
		], 'wp.product_id = e.entity_id AND wp.website_id = ' . $websiteId, [ ] )->joinLeft ( [ 
				'dstatus' => $status->getBackend ()->getTable () 
		], implode ( ' AND ', $statusConditions ), [ ] )->where ( $statusExpression . ' = ' . \Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED );
		...
		foreach ( $tables as $tableName => $columns ) {
			...
			$select->joinLeft ( $temporaryTableName, 'e.entity_id = ' . $temporaryTableName . '.entity_id', $columnsNames );
			...
			if (! empty ( $columnValueNames )) {
				$select->joinLeft ( $temporaryValueTableName, 'e.entity_id = ' . $temporaryValueTableName . '.entity_id', $columnValueNames );
	...
	/**
	 * Apply diff.
	 * between 0 store and current store to temporary flat table
	 *
	 * @param array $tables        	
	 * @param array $changedIds        	
	 * @param int|string $storeId        	
		...
		foreach ( $tables as $tableName => $columns ) {
			foreach ( $columns as $attribute ) {
				...
				if ($attribute->getBackend ()->getType () != 'static') {
					$joinCondition = 't.entity_id = e.entity_id' . ' AND t.attribute_id=' . $attribute->getId () . ' AND t.store_id = ' . $storeId . ' AND t.value IS NOT NULL';
