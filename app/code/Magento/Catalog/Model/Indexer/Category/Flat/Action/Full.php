...
class Full extends \Magento\Catalog\Model\Indexer\Category\Flat\AbstractAction {
	...
	protected function populateFlatTables(array $stores) {
		...
		foreach ( $stores as $store ) {
			if (! isset ( $categories [$store->getRootCategoryId ()] )) {
				$select = $this->connection->select ()->from ( $this->connection->getTableName ( $this->getTableName ( 'catalog_category_entity' ) ) )->where ( 'path = ?', ( string ) $rootId )->orWhere ( 'path = ?', "{$rootId}/{$store->getRootCategoryId()}" )->orWhere ( 'path LIKE ?', "{$rootId}/{$store->getRootCategoryId()}/%" );
			...
			foreach ( $categoriesIdsChunks as $categoriesIdsChunk ) {
				...
				foreach ( $categories [$store->getRootCategoryId ()] as $category ) {
					...
					$data [] = $this->prepareValuesToInsert ( array_merge ( $category, $attributesData [$category ['entity_id']] ) );
				}
				$this->connection->insertMultiple ( $this->addTemporaryTableSuffix ( $this->getMainStoreTable ( $store->getId () ) ), $data );
			}
		}
	 ...
	 *
	 * @param \Magento\Store\Model\Store[] $stores
	 *        	if empty, create tables for all stores of the application
	 * @return Full
	 */
