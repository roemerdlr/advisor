...
class Rows extends \Magento\Catalog\Model\Indexer\Category\Flat\AbstractAction {
	 ...
	 */
	public function __construct(\Magento\Framework\App\ResourceConnection $resource, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Catalog\Model\ResourceModel\Helper $resourceHelper, CategoryRepositoryInterface $categoryRepository) {
		...
		foreach ( $stores as $store ) {
			...
			foreach ( $categoriesIdsChunks as $categoriesIdsChunk ) {
				...
				foreach ( $categoriesIdsChunk as $categoryId ) {
					...
					$data [] = $this->prepareValuesToInsert ( array_merge ( $category->getData (), $attributesData [$categoryId], [ 
	...
	protected function deleteNonStoreCategories(\Magento\Store\Model\Store $store, $useTempTable) {
		...
		], 'cf.path = ce.path', [ ] )->where ( "cf.path = {$rootIdExpr} OR cf.path = {$rootCatIdExpr} OR cf.path like {$catIdExpr}" )->where ( 'ce.entity_id IS NULL' );
	...
	protected function filterIdsByStore(array $ids, $store) {
		...
		$select = $this->connection->select ()->from ( $this->getTableName ( 'catalog_category_entity' ), [ 
				'entity_id' 
		] )->where ( "path = {$rootIdExpr} OR path = {$rootCatIdExpr} OR path like {$catIdExpr}" )->where ( 'entity_id IN (?)', $ids );
