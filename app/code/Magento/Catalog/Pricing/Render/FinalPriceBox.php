...
class FinalPriceBox extends BasePriceBox {
	...
	protected function _toHtml() {
		...
		if ($msrpPriceType->canApplyMsrp ( $product ) && $msrpPriceType->isMinimalPriceLessMsrp ( $product )) {
			...
			$msrpBlock = $this->rendererPool->createPriceRender ( MsrpPrice::PRICE_CODE, $this->getSaleableItem (), [ 
	...
	protected function wrapResult($html) {
		return '<div class="price-box ' . $this->getData ( 'css_classes' ) . '" ' . 'data-role="priceBox" ' . 'data-product-id="' . $this->getSaleableItem ()->getId () . '"' . '>' . $html . '</div>';
	}
	...
	public function renderAmountMinimal() {
		...
		return $this->renderAmount ( $price->getMinimalPrice (), [ 
	...
	public function showMinimalPrice() {
		...
		return $this->getDisplayMinimalPrice () && $minimalPriceAValue && $minimalPriceAValue < $finalPriceValue;
	}
}
