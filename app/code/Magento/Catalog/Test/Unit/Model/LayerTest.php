...
class LayerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->category = $this->getMockBuilder ( 'Magento\Catalog\Model\Category' )->setMethods ( [ 
				'getId',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->registry = $this->getMockBuilder ( 'Magento\Framework\Registry' )->setMethods ( [ 
				'registry' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->stateKeyGenerator = $this->getMockBuilder ( 'Magento\Catalog\Model\Layer\Category\StateKey' )->setMethods ( [ 
				'toString' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->collectionFilter = $this->getMockBuilder ( 'Magento\Catalog\Model\Layer\Category\CollectionFilter' )->setMethods ( [ 
				'filter' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->filter = $this->getMockBuilder ( 'Magento\Catalog\Model\Layer\Filter\Item' )->setMethods ( [ 
				'getFilter',
				'getValueString' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->abstractFilter = $this->getMockBuilder ( 'Magento\Catalog\Model\Layer\Filter\AbstractFilter' )->setMethods ( [ 
				'getRequestVar' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->stateFactory = $this->getMockBuilder ( 'Magento\Catalog\Model\Layer\StateFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->stateFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->state ) );
		
		$this->collection = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Product\Collection' )->disableOriginalConstructor ()->getMock ();
		
		$this->categoryRepository = $this->getMock ( 'Magento\Catalog\Api\CategoryRepositoryInterface' );
		$this->currentCategory = $this->getMock ( 'Magento\Catalog\Model\Category', [ 
				'getId',
				'__wakeup' 
		], [ ], '', false );
		
		$this->model = $helper->getObject ( 'Magento\Catalog\Model\Layer', [ 
	...
	public function testApply() {
		...
		$this->state->expects ( $this->any () )->method ( 'getFilters' )->will ( $this->returnValue ( [ 
				$this->filter 
		] ) );
	...
	public function testSetNewCurrentCategoryIfCurrentCategoryIsAnother() {
		...
		$this->categoryRepository->expects ( $this->once () )->method ( 'get' )->with ( $categoryId )->willReturn ( $this->category );
		
		$this->currentCategory->expects ( $this->any () )->method ( 'getId' )->willReturn ( $currentCategoryId );
		$this->registry->expects ( $this->once () )->method ( 'registry' )->with ( 'current_category' )->willReturn ( $this->currentCategory );
	...
	public function testSetNewCurrentCategoryIfCurrentCategoryIsSame() {
		...
		$this->categoryRepository->expects ( $this->once () )->method ( 'get' )->with ( $categoryId )->willReturn ( $this->category );
		$this->registry->expects ( $this->once () )->method ( 'registry' )->with ( 'current_category' )->willReturn ( $this->category );
	...
	public function testSetNewCurrentCategoryIfCategoryIsNotFound() {
		$this->categoryRepository->expects ( $this->once () )->method ( 'get' )->will ( $this->throwException ( new NoSuchEntityException () ) );
