...
class CategoryLinkManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productLinkFactoryMock = $this->getMock ( '\Magento\Catalog\Api\Data\CategoryProductLinkInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testGetAssignedProducts() {
		...
		$categoryProductLinkMock->expects ( $this->once () )->method ( 'setPosition' )->with ( $position )->willReturnSelf ();
		$categoryProductLinkMock->expects ( $this->once () )->method ( 'setCategoryId' )->with ( $categoryId )->willReturnSelf ();
		$this->assertEquals ( [ 
				$categoryProductLinkMock 
		], $this->model->getAssignedProducts ( $categoryId ) );
	}
}
