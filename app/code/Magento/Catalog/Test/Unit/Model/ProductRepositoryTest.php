...
class ProductRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->initializedProductMock = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getWebsiteIds',
				'setProductOptions',
				'load',
				'getOptions',
		...
		$this->collectionFactoryMock = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->searchResultsFactoryMock = $this->getMock ( '\Magento\Catalog\Api\Data\ProductSearchResultsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->contentFactoryMock = $this->getMock ( 'Magento\Framework\Api\Data\ImageContentInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->storeManagerMock = $this->getMockBuilder ( 'Magento\Store\Model\StoreManagerInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMockForAbstractClass ();
		$storeMock = $this->getMockBuilder ( 'Magento\Store\Api\Data\StoreInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMockForAbstractClass ();
		$storeMock->expects ( $this->any () )->method ( 'getWebsiteId' )->willReturn ( '1' );
		$this->storeManagerMock->expects ( $this->any () )->method ( 'getStore' )->willReturn ( $storeMock );
		
		$this->model = $this->objectManager->getObject ( 'Magento\Catalog\Model\ProductRepository', [ 
	...
	public function testGetAbsentProduct() {
		...
		$this->resourceModelMock->expects ( $this->once () )->method ( 'getIdBySku' )->with ( 'test_sku' )->will ( $this->returnValue ( null ) );
		$this->productFactoryMock->expects ( $this->never () )->method ( 'setData' );
		$this->model->get ( 'test_sku' );
	}
	public function testCreateCreatesProduct() {
		$this->productFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->productMock ) );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'getIdBySku' )->with ( 'test_sku' )->will ( $this->returnValue ( 'test_id' ) );
	 ...
	 * @param bool $editMode        	
	 * @param mixed $storeId        	
	 * @return void @dataProvider cacheKeyDataProvider
	 */
	...
	public function testGetByIdForcedReload() {
		...
		$this->productFactoryMock->expects ( $this->exactly ( 2 ) )->method ( 'create' )->will ( $this->returnValue ( $this->productMock ) );
	...
	public function testGetForcedReload() {
		...
		$this->resourceModelMock->expects ( $this->exactly ( 2 ) )->method ( 'getIdBySku' )->with ( $sku )->willReturn ( $id );
	...
	public function testSaveExisting() {
		...
		$this->productFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->productMock ) );
		$this->initializationHelperMock->expects ( $this->once () )->method ( 'initialize' )->with ( $this->productMock );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->productMock )->willReturn ( true );
	...
	public function testSaveUnableToSaveException() {
		...
		$this->initializationHelperMock->expects ( $this->never () )->method ( 'initialize' )->with ( $this->productMock );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->productMock )->willReturn ( true );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'save' )->with ( $this->productMock )->willThrowException ( new \Exception () );
		$this->extensibleDataObjectConverterMock->expects ( $this->once () )->method ( 'toNestedArray' )->will ( $this->returnValue ( $this->productData ) );
		$this->productMock->expects ( $this->once () )->method ( 'getWebsiteIds' )->willReturn ( [ ] );
	...
	public function testSaveException() {
		...
		$this->initializationHelperMock->expects ( $this->never () )->method ( 'initialize' )->with ( $this->productMock );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->productMock )->willReturn ( true );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'save' )->with ( $this->productMock )->willThrowException ( new \Magento\Eav\Model\Entity\Attribute\Exception ( __ ( '123' ) ) );
	...
	public function testSaveInvalidProductException() {
		...
		$this->productFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->productMock ) );
		$this->initializationHelperMock->expects ( $this->never () )->method ( 'initialize' )->with ( $this->productMock );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->productMock )->willReturn ( [ 
				'error1',
				'error2' 
		] );
		$this->productMock->expects ( $this->never () )->method ( 'getId' );
	...
	public function testDelete() {
		...
		$this->resourceModelMock->expects ( $this->once () )->method ( 'delete' )->with ( $this->productMock )->willReturn ( true );
	...
	public function testDeleteException() {
		$this->productMock->expects ( $this->once () )->method ( 'getSku' )->willReturn ( 'product-42' );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'delete' )->with ( $this->productMock )->willThrowException ( new \Exception () );
	...
	public function testGetList($fieldName) {
		...
		$productAttributeSearchResultsMock = $this->getMockForAbstractClass ( '\Magento\Catalog\Api\Data\ProductAttributeInterface', [ ], '', false, false, false, [ 
				'getItems' 
		] );
		...
		$productAttributeSearchResultsMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
		...
		$filterGroupMock->expects ( $this->once () )->method ( 'getFilters' )->willReturn ( [ 
				$filterGroupFilterMock 
		] );
		...
		$searchCriteriaMock->expects ( $this->once () )->method ( 'getSortOrders' )->willReturn ( [ 
				$sortOrderMock 
		] );
	...
	public function cacheKeyDataProvider() {
		$anyObject = $this->getMock ( 'stdClass', [ 
				'getId' 
		], [ ], '', false );
		$anyObject->expects ( $this->any () )->method ( 'getId' )->willReturn ( 123 );
	...
	public function testSaveExistingWithOptions(array $newOptions, array $existingOptions, array $expectedData) {
		...
		$this->initializationHelperMock->expects ( $this->once () )->method ( 'initialize' )->with ( $this->initializedProductMock );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'validate' )->with ( $this->initializedProductMock )->willReturn ( true );
		$this->resourceModelMock->expects ( $this->once () )->method ( 'save' )->with ( $this->initializedProductMock )->willReturn ( true );
		// option data
		$this->productData ['options'] = $newOptions;
		$this->extensibleDataObjectConverterMock->expects ( $this->once () )->method ( 'toNestedArray' )->will ( $this->returnValue ( $this->productData ) );
		
		$this->initializedProductMock->expects ( $this->once () )->method ( 'getOptions' )->willReturn ( $existingOptions );
		
		$this->initializedProductMock->expects ( $this->once () )->method ( 'setProductOptions' )->with ( $expectedData );
	...
	public function saveExistingWithOptionsDataProvider() {
		...
		$existingOption1 = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Option' )->disableOriginalConstructor ()->setMethods ( null )->getMock ();
		$existingOption1->setData ( [ 
				"option_id" => 10,
				"type" => "drop_down" 
		] );
		/** @var \Magento\Catalog\Model\Product\Option\Value $existingOptionValue1 */
		$existingOptionValue1 = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Option\Value' )->disableOriginalConstructor ()->setMethods ( null )->getMock ();
		$existingOptionValue1->setData ( [ 
				"option_type_id" => "8",
				"title" => "DropdownOptions_1",
				"price" => 5 
		] );
		$existingOptionValue2 = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Option\Value' )->disableOriginalConstructor ()->setMethods ( null )->getMock ();
		...
		$existingOption2 = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Option' )->disableOriginalConstructor ()->setMethods ( null )->getMock ();
	...
	public function testSaveWithLinks(array $newLinks, array $existingLinks, array $expectedData) {
		...
		if (! empty ( $newLinks )) {
			...
			$this->resourceModelMock->expects ( $this->any () )->method ( 'getProductsIdsBySkus' )->willReturn ( [ 
					$newLinks ['linked_product_sku'] => $newLinks ['linked_product_sku'] 
			] );
			...
			$this->initializedProductMock->expects ( $this->any () )->method ( 'getProductLinks' )->willReturn ( [ 
					$inputLink 
			] );
		} else {
			$this->resourceModelMock->expects ( $this->any () )->method ( 'getProductsIdsBySkus' )->willReturn ( [ ] );
			...
			$this->initializedProductMock->expects ( $this->never () )->method ( 'getProductLinks' )->willReturn ( [ ] );
		}
		
		$this->extensibleDataObjectConverterMock->expects ( $this->at ( 0 ) )->method ( 'toNestedArray' )->will ( $this->returnValue ( $this->productData ) );
		
		if (! empty ( $newLinks )) {
			$this->extensibleDataObjectConverterMock->expects ( $this->at ( 1 ) )->method ( 'toNestedArray' )->will ( $this->returnValue ( $newLinks ) );
		}
	...
	public function testSaveExistingWithNewMediaGalleryEntries() {
		...
		$this->initializedProductMock->expects ( $this->any () )->method ( 'getMediaAttributes' )->willReturn ( [ 
				"image" => "imageAttribute",
				"small_image" => "small_image_attribute" 
		] );
		...
		$galleryAttributeBackendMock->expects ( $this->once () )->method ( 'addImage' )->with ( $this->initializedProductMock, $mediaTmpPath . $absolutePath, [ 
				'image',
				'small_image' 
		], true, false )->willReturn ( $imageFileUri );
		$galleryAttributeBackendMock->expects ( $this->once () )->method ( 'updateImage' )->with ( $this->initializedProductMock, $imageFileUri, [ 
				'label' => 'label_text',
				'position' => 10,
				'disabled' => false,
				'media_type' => 'media_type' 
	...
	public function testSaveExistingWithMediaGalleryEntries() {
		...
		$this->initializedProductMock->expects ( $this->once () )->method ( 'getGalleryAttributeBackend' )->willReturn ( $galleryAttributeBackendMock );
		$galleryAttributeBackendMock->expects ( $this->once () )->method ( 'setMediaAttribute' )->with ( $this->initializedProductMock, [ 
				'image',
				'small_image' 
		], 'filename1' );
		$this->initializedProductMock->expects ( $this->once () )->method ( 'getWebsiteIds' )->willReturn ( [ ] );
	...
	public function expectAddToFilter($fieldName, $collectionMock) {
		if ($fieldName == 'category_id') {
			$collectionMock->expects ( $this->once () )->method ( 'addCategoriesFilter' )->with ( [ 
					'eq' => [ 
							'value' 
					] 
			] );
