...
class SortbyTest extends \PHPUnit_Framework_TestCase {
	...
	public function testValidate($attributeData, $data, $expected) {
		...
		$this->_attribute->expects ( $this->at ( 1 ) )->method ( 'getIsRequired' )->will ( $this->returnValue ( $attributeData ['isRequired'] ) );
		$this->_attribute->expects ( $this->any () )->method ( 'isValueEmpty' )->will ( $this->returnValue ( $attributeData ['isValueEmpty'] ) );
		$object = new \Magento\Framework\DataObject ( $data );
	...
	public function testValidateUnique() {
		...
		$entityMock = $this->getMockForAbstractClass ( 'Magento\Eav\Model\Entity\AbstractEntity', [ ], '', false, true, true, [ 
	...
	public function testValidateUniqueException() {
		...
		$frontMock = $this->getMockForAbstractClass ( 'Magento\Eav\Model\Entity\Attribute\Frontend\AbstractFrontend', [ ], '', false, true, true, [ 
				'getLabel' 
		] );
		$this->_attribute->expects ( $this->any () )->method ( 'getEntity' )->will ( $this->returnValue ( $entityMock ) );
