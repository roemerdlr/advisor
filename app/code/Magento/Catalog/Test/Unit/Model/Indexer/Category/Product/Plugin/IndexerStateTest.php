...
class IndexerStateTest extends \PHPUnit_Framework_TestCase {
	public function testAfterSave() {
		...
		$testableState = $this->getMockBuilder ( 'Magento\Indexer\Model\Indexer\State' )->setMethods ( [ 
				'getIndexerId',
				'getStatus',
				'__wakeup' 
		...
		$state->expects ( $this->at ( 1 ) )->method ( 'setData' )->with ( $this->logicalOr ( $this->equalTo ( 'status' ), $this->equalTo ( $testableStatus ) ) )->will ( $this->returnSelf () );
