...
class TableDataTest extends \PHPUnit_Framework_TestCase {
	...
	public function testMoveWithNonExistentFlatTable() {
		...
		$this->_connectionMock->expects ( $this->once () )->method ( 'isTableExists' )->with ( $flatTable )->will ( $this->returnValue ( false ) );
		
		$this->_connectionMock->expects ( $this->once () )->method ( 'renameTablesBatch' )->with ( [ 
				[ 
						'oldName' => 'flat_tmp',
						'newName' => 'flat_table' 
				] 
		...
		$model = $this->_objectManager->getObject ( 'Magento\Catalog\Model\Indexer\Product\Flat\Action\Rows\TableData', [ 
	...
	public function testMoveWithExistentFlatTable() {
		...
		$sql = md5 ( time () );
		$selectMock->expects ( $this->once () )->method ( 'insertFromSelect' )->with ( $flatTable, [ 
				'column_2',
				'column_3' 
		...
		$this->_resourceMock->expects ( $this->any () )->method ( 'getConnection' )->will ( $this->returnValue ( $this->_connectionMock ) );
