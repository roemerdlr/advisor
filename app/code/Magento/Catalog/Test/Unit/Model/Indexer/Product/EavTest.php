...
class EavTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_productEavIndexerRow = $this->getMockBuilder ( 'Magento\Catalog\Model\Indexer\Product\Eav\Action\Row' )->disableOriginalConstructor ()->getMock ();
		
		$this->_productEavIndexerRows = $this->getMockBuilder ( 'Magento\Catalog\Model\Indexer\Product\Eav\Action\Rows' )->disableOriginalConstructor ()->getMock ();
		
		$this->_productEavIndexerFull = $this->getMockBuilder ( 'Magento\Catalog\Model\Indexer\Product\Eav\Action\Full' )->disableOriginalConstructor ()->getMock ();
		
		$this->_model = new \Magento\Catalog\Model\Indexer\Product\Eav ( $this->_productEavIndexerRow, $this->_productEavIndexerRows, $this->_productEavIndexerFull );
	}
	public function testExecuteAndExecuteList() {
		...
		$this->_productEavIndexerRow->expects ( $this->any () )->method ( 'execute' )->with ( $ids );
