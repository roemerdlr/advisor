...
class MviewStateTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAfterSetStatusSuspend($stateStatus, $relatedStatus) {
		...
		$state = $this->getMockBuilder ( 'Magento\Framework\Mview\View\StateInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		...
		$relatedViewState = $this->getMockBuilder ( 'Magento\Framework\Mview\View\StateInterface' )->disableOriginalConstructor ()->getMock ();
		
		$relatedViewState->expects ( $this->once () )->method ( 'loadByView' )->with ( $this->equalTo ( $relatedViewId ) )->will ( $this->returnSelf () );
		...
		$relatedViewState->expects ( $this->once () )->method ( 'setVersionId' )->with ( $this->equalTo ( $relatedVersion ) )->will ( $this->returnSelf () );
		
		$relatedViewState->expects ( $this->once () )->method ( 'setStatus' )->with ( $this->equalTo ( $stateStatus ) )->will ( $this->returnSelf () );
		
		$relatedViewChangelog = $this->getMockBuilder ( 'Magento\Framework\Mview\View\ChangelogInterface' )->disableOriginalConstructor ()->getMock ();
		
		$relatedViewChangelog->expects ( $this->once () )->method ( 'setViewId' )->with ( $this->equalTo ( $relatedViewId ) )->will ( $this->returnSelf () );
		
		$relatedViewChangelog->expects ( $this->once () )->method ( 'getVersion' )->will ( $this->returnValue ( $relatedVersion ) );
		
		$model = new \Magento\Catalog\Model\Indexer\Category\Product\Plugin\MviewState ( $relatedViewState, $relatedViewChangelog );
	...
	public function testAfterSetStatusResume($stateStatus, $relatedStatus) {
		...
		$relatedViewState->expects ( $this->once () )->method ( 'setStatus' )->with ( $this->equalTo ( 'idle' ) )->will ( $this->returnSelf () );
