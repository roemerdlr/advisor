...
class EraserTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->indexerHelper = $this->getMock ( 'Magento\Catalog\Helper\Product\Flat\Indexer', [ ], [ ], '', false );
		...
		$this->model = new \Magento\Catalog\Model\Indexer\Product\Flat\Action\Eraser ( $resource, $this->indexerHelper, $this->storeManager );
	}
	public function testRemoveDeletedProducts() {
		...
		$this->connection->expects ( $this->once () )->method ( 'delete' )->with ( 'store_1_flat', [ 
				'entity_id IN(?)' => [ 
						1 
				] 
		] );
	...
	public function testDeleteProductsFromStoreForAllStores() {
		...
		$this->storeManager->expects ( $this->once () )->method ( 'getStores' )->will ( $this->returnValue ( [ 
				$store1,
				$store2 
		] ) );
		$this->connection->expects ( $this->at ( 0 ) )->method ( 'delete' )->with ( 'store_1_flat', [ 
				'entity_id IN(?)' => [ 
						1 
				] 
		] );
		$this->connection->expects ( $this->at ( 1 ) )->method ( 'delete' )->with ( 'store_2_flat', [ 
				'entity_id IN(?)' => [ 
						1 
				] 
		] );
