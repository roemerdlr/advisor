...
class FilterListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->attributeListMock = $this->getMock ( 'Magento\Catalog\Model\Layer\Category\FilterableAttributeList', [ ], [ ], '', false );
	...
	public function testGetFilters($method, $value, $expectedClass) {
		...
		$this->objectManagerMock->expects ( $this->at ( 1 ) )->method ( 'create' )->with ( $expectedClass, [ 
				'data' => [ 
						'attribute_model' => $this->attributeMock 
				],
		...
		$this->attributeListMock->expects ( $this->once () )->method ( 'getList' )->will ( $this->returnValue ( [ 
				$this->attributeMock 
		] ) );
		...
		$this->assertEquals ( [ 
				'filter',
				'filter' 
		], $this->model->getFilters ( $this->layerMock ) );
	}
