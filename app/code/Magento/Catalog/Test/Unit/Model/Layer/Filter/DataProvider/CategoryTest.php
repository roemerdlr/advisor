...
class CategoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->coreRegistry = $var = $this->getMockBuilder ( '\Magento\Framework\Registry' )->disableOriginalConstructor ()->setMethods ( [ 
				'register' 
		] )->getMock ();
		...
		$this->categoryFactory = $this->getMockBuilder ( 'Magento\Catalog\Model\CategoryFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->target = $objectManagerHelper->getObject ( 'Magento\Catalog\Model\Layer\Filter\DataProvider\Category', [ 
	...
	public function testGetCategoryWithAppliedId() {
		...
		$this->category->expects ( $this->any () )->method ( 'getPathIds' )->will ( $this->returnValue ( [ 
				20,
				10 
		] ) );
