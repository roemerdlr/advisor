...
class DecimalTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resource = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Layer\Filter\Decimal' )->disableOriginalConstructor ()->setMethods ( [ 
				'getMinMax',
				'getCount' 
		] )->getMock ();
		$objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->target = $objectManagerHelper->getObject ( 'Magento\Catalog\Model\Layer\Filter\DataProvider\Decimal', [ 
				'resource' => $this->resource 
		] );
	}
	public function testGetMaxValue() {
		$this->resource->expects ( $this->once () )->method ( 'getMinMax' )->with ( $this->filter )->will ( $this->returnValue ( [ 
				10,
				20 
		] ) );
	...
	public function testGetRangeItemCounts() {
		$range = 100500;
		$this->resource->expects ( $this->once () )->method ( 'getCount' )->with ( $this->filter, $range )->will ( $this->returnValue ( 350 ) );
