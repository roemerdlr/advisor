...
class CategoryRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->categoryFactoryMock = $this->getMock ( '\Magento\Catalog\Model\CategoryFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testUpdateExistingCategory() {
		...
		$this->categoryFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $categoryMock );
		$categoryMock->expects ( $this->atLeastOnce () )->method ( 'toFlatArray' )->willReturn ( [ 
				'image' => [ ] 
		] );
		...
		$this->categoryResourceMock->expects ( $this->once () )->method ( 'save' )->willReturn ( '\Magento\Framework\DataObject' );
	...
	public function testCreateNewCategory() {
		...
		$categoryMock->expects ( $this->any () )->method ( 'getId' )->will ( $this->onConsecutiveCalls ( $categoryId, $newCategoryId ) );
	...
	public function testSaveWithValidateCategoryException($error, $expectedException, $expectedExceptionMessage) {
		...
		$categoryMock->expects ( $this->once () )->method ( 'getParentId' )->willReturn ( 3 );
		$categoryMock->expects ( $this->once () )->method ( 'validate' )->willReturn ( [ 
				42 => $error 
		] );
	...
	/**
	 *
	 * @throws \Magento\Framework\Exception\StateException @expectedException \Magento\Framework\Exception\StateException
