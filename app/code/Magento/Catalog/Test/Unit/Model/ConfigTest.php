...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLoadAttributeSets() {
		$setCollectionFactory = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$setItem = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\Set', [ 
				'getEntityTypeId',
				'getAttributeSetName',
				'__wakeup' 
		], [ ], '', false );
		...
		$setCollection = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\Collection', [ 
				'load' 
		], [ ], '', false );
		$setCollection->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( [ 
				1 => $setItem 
		] ) );
	...
	public function testLoadAttributeGroups() {
		$groupCollectionFactory = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Group\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$model = $objectManager->getObject ( 'Magento\Catalog\Model\Config', [ 
		...
		$setItem = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\Group', [ 
				'getAttributeSetId',
		...
		$groupCollection = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Group\Collection', [ 
				'load' 
		], [ ], '', false );
		$groupCollection->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( [ 
				1 => $setItem 
		] ) );
		$groupCollectionFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $groupCollection ) );
	...
	public function testLoadProductTypes() {
		$productTypeFactory = $this->getMock ( '\Magento\Catalog\Model\Product\TypeFactory', [ 
				'create' 
		], [ ], '', false );
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$model = $objectManager->getObject ( 'Magento\Catalog\Model\Config', [ 
		...
		$typeCollection = $this->getMock ( '\Magento\Catalog\Model\Product\Type', [ 
				'getOptionArray' 
		], [ ], '', false );
		$typeCollection->expects ( $this->once () )->method ( 'getOptionArray' )->will ( $this->returnValue ( [ 
				1 => 'name' 
		] ) );
		$productTypeFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $typeCollection ) );
	...
	protected function prepareConfigModelForAttributes() {
		...
		$config = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Config', [ 
				'setStoreId',
		...
		$configFactory = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\ConfigFactory', [ 
				'create' 
		], [ ], '', false );
		$configFactory->expects ( $this->atLeastOnce () )->method ( 'create' )->will ( $this->returnValue ( $config ) );
		
		$eavConfig = $this->getMock ( '\Magento\Eav\Model\Config', [ 
				'getAttribute',
				'importAttributesData' 
		], [ ], '', false );
		$eavConfig->expects ( $this->once () )->method ( 'importAttributesData' )->with ( $entityType, $attributesData )->will ( $this->returnSelf () );
		$eavConfig->expects ( $this->once () )->method ( 'getAttribute' )->with ( $entityType, $attributeData ['attribute_code'] )->will ( $this->returnValue ( $attribute ) );
	...
	public function testGetAttributesUsedInProductListing() {
		...
		$this->assertEquals ( [ 
				1 => $attribute 
		], $model->getAttributesUsedInProductListing () );
		return $model;
	}
	...
	public function testGetAttributesUsedForSortBy() {
		...
		$this->assertEquals ( [ 
				1 => $attribute 
		], $model->getAttributesUsedForSortBy () );
	}
	...
	public function testGetAttributeUsedForSortByArray() {
		...
		$this->assertEquals ( [ 
				'position' => 'Position',
				'code' => 'label' 
		], $model->getAttributeUsedForSortByArray () );
	}
	...
	public function testGetProductListDefaultSortBy() {
		...
		$scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'catalog/frontend/default_sort_by', 'store', null )->will ( $this->returnValue ( 1 ) );
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$model = $objectManager->getObject ( 'Magento\Catalog\Model\Config', [ 
