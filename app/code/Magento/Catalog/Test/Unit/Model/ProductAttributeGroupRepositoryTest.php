...
class ProductAttributeGroupRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->groupFactoryMock = $this->getMock ( '\Magento\Catalog\Model\Product\Attribute\GroupFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = $objectManager->getObject ( 'Magento\Catalog\Model\ProductAttributeGroupRepository', [ 
	...
	public function testSave() {
		...
		$this->groupRepositoryMock->expects ( $this->once () )->method ( 'save' )->with ( $groupMock )->willReturn ( $expectedResult );
	...
	public function testGetList() {
		...
		$expectedResult = $this->getMock ( '\Magento\Eav\Api\Data\AttributeGroupInterface' );
		$this->groupRepositoryMock->expects ( $this->once () )->method ( 'getList' )->with ( $searchCriteriaMock )->willReturn ( $expectedResult );
	...
	public function testDeleteById() {
		$groupId = 42;
		$groupMock = $this->getMock ( '\Magento\Catalog\Model\Product\Attribute\Group', [ 
				'hasSystemAttributes',
				'getId' 
		], [ ], '', false );
		$this->groupFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $groupMock );
