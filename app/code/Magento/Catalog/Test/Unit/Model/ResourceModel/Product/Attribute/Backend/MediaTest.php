...
class MediaTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->resource = $objectManager->getObject ( 'Magento\Catalog\Model\ResourceModel\Product\Attribute\Backend\Media', [ 
				'resource' => $resource 
		] );
	...
	public function testLoadDataFromTableByValueId() {
		...
		$this->select->expects ( $this->at ( 1 ) )->method ( 'where' )->with ( 'main.value_id IN(?)', $ids )->willReturnSelf ();
		$this->select->expects ( $this->at ( 2 ) )->method ( 'where' )->with ( 'main.store_id = ?', $storeId )->willReturnSelf ();
		$resultRow = [ 
				[ 
						'value_id' => '4',
						'store_id' => 1,
		...
		$this->connection->expects ( $this->once () )->method ( 'fetchAll' )->with ( $this->select )->willReturn ( $resultRow );
		
		$methodResult = $this->resource->loadDataFromTableByValueId ( $tableNameAlias, $ids, $storeId, $cols, $leftJoinTables, $whereCondition );
	...
	public function testLoadDataFromTableByValueIdNoColsWithWhere() {
		...
		$this->select->expects ( $this->at ( 3 ) )->method ( 'where' )->with ( $whereCondition )->willReturnSelf ();
	...
	public function testLoadGallery() {
		...
		$attributeId = 6;
		$getTableReturnValue = 'table';
		$quoteInfoReturnValue = 'main.value_id = value.value_id AND value.store_id = ' . $storeId . ' AND value.entity_id = ' . $productId;
		...
		$this->connection->expects ( $this->once () )->method ( 'getCheckSql' )->with ( 'value.position IS NULL', 'default_value.position', 'value.position' )->will ( $this->returnValue ( $positionCheckSql ) );
		$this->connection->expects ( $this->once () )->method ( 'select' )->will ( $this->returnValue ( $this->select ) );
		$this->select->expects ( $this->at ( 0 ) )->method ( 'from' )->with ( [ 
				'main' => $getTableReturnValue 
		], [ 
		...
		$this->connection->expects ( $this->exactly ( 3 ) )->method ( 'quoteInto' )->withConsecutive ( [ 
				...
				5 
		] )->willReturnOnConsecutiveCalls ( 'value.store_id = ' . $storeId, 'value.entity_id = ' . $productId, 'default_value.entity_id = ' . $productId );
		...
		], 'main.value_id = default_value.value_id AND default_value.store_id = 0 AND default_value.entity_id = ' . $productId, [ 
				'label_default' => 'label',
				'position_default' => 'position',
				'disabled_default' => 'disabled' 
		] )->willReturnSelf ();
		$this->select->expects ( $this->at ( 4 ) )->method ( 'where' )->with ( 'main.attribute_id = ?', $attributeId )->willReturnSelf ();
		$this->select->expects ( $this->at ( 5 ) )->method ( 'where' )->with ( 'main.disabled = 0' )->willReturnSelf ();
		$this->select->expects ( $this->at ( 6 ) )->method ( 'where' )->with ( 'entity.entity_id = ?', $productId )->willReturnSelf ();
		$this->select->expects ( $this->once () )->method ( 'order' )->with ( $positionCheckSql . ' ' . \Magento\Framework\DB\Select::SQL_ASC )->willReturnSelf ();
		$this->connection->expects ( $this->once () )->method ( 'fetchAll' )->with ( $this->select )->willReturn ( $resultRow );
	...
	public function testInsertGalleryValueInStore() {
		...
		$this->connection->expects ( $this->any () )->method ( 'prepareColumnValue' )->willReturnOnConsecutiveCalls ( '8', 0, '', 'https://www.youtube.com/watch?v=abcdfghijk', 'New Title', 'New Description', 'New metadata' );
	...
	public function testDeleteGalleryValueInStore() {
		...
		$this->connection->expects ( $this->exactly ( 3 ) )->method ( 'quoteInto' )->withConsecutive ( [ 
				...
				'store_id = ?',
				( int ) $storeId 
		] )->willReturnOnConsecutiveCalls ( 'value_id = ' . $valueId, 'entity_id = ' . $entityId, 'store_id = ' . $storeId );
		
		$this->connection->expects ( $this->once () )->method ( 'delete' )->with ( 'table', 'value_id = 4 AND entity_id = 6 AND store_id = 1' )->willReturnSelf ();
