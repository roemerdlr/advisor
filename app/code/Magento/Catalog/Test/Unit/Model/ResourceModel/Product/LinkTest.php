...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $objectManager->getObject ( 'Magento\Catalog\Model\ResourceModel\Product\Link', [ 
	...
	protected function prepareAdapter() {
		...
		// method flow
		$this->resource->expects ( $this->at ( 0 ) )->method ( 'getConnection' )->will ( $this->returnValue ( $this->connection ) );
	...
	public function testGetAttributeTypeTable() {
		...
		$this->resource->expects ( $this->once () )->method ( 'getTableName' )->with ( 'catalog_product_link_attribute_' . $inputTable )->will ( $this->returnValue ( $resultTable ) );
	...
	public function testGetChildrenIds() {
		...
		$this->connection->expects ( $this->once () )->method ( 'fetchAll' )->with ( $this->dbSelect, $bind )->will ( $this->returnValue ( $fetchedData ) );
