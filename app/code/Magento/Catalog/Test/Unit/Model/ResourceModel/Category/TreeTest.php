...
class TreeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_resource->expects ( $this->once () )->method ( 'getConnection' )->with ( 'catalog' )->will ( $this->returnValue ( $connection ) );
		$this->_resource->expects ( $this->once () )->method ( 'getTableName' )->with ( 'catalog_category_entity' )->will ( $this->returnArgument ( 0 ) );
		$eventManager = $this->getMock ( 'Magento\Framework\Event\ManagerInterface', [ ], [ ], '', false );
		$this->_attributeConfig = $this->getMock ( 'Magento\Catalog\Model\Attribute\Config', [ ], [ ], '', false );
		$this->_collectionFactory = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Category\Collection\Factory', [ ], [ ], '', false );
		$this->_model = $objectHelper->getObject ( 'Magento\Catalog\Model\ResourceModel\Category\Tree', [ 
	...
	public function testGetCollection() {
		...
		$this->_attributeConfig->expects ( $this->once () )->method ( 'getAttributeNames' )->with ( 'catalog_category' )->will ( $this->returnValue ( $attributes ) );
	...
	public function testCallCleaningDuringSetCollection() {
		...
		$model = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Category\Tree', [ 
				'_clean' 
		], [ ], '', false );
	...
	public function testAddCollectionData() {
		...
		$model = $objectHelper->getObject ( 'Magento\Catalog\Model\ResourceModel\Category\Tree', [ 
		...
		$nodeMock = $this->getMock ( '\Magento\Framework\Data\Tree\Node', [ 
				'getId',
				'getPath' 
		], [ ], '', false );
