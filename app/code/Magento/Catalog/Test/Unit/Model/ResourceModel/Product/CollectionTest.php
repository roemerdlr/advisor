...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->collection = $helper->getObject ( 'Magento\Catalog\Model\ResourceModel\Product\Collection', [ 
	...
	public function testAddProductCategoriesFilter() {
		...
		$this->connectionMock->expects ( $this->exactly ( 2 ) )->method ( 'prepareSqlCondition' )->withConsecutive ( [ 
				...
				'e.entity_id',
				[ 
						$conditionType => $this->selectMock 
				] 
		] )->willReturnOnConsecutiveCalls ( $preparedSql, 'e.entity_id IN (1,2)' );
		$this->selectMock->expects ( $this->once () )->method ( 'from' )->with ( [ 
				'cat' => $tableName 
		], 'cat.product_id' )->willReturnSelf ();
		$this->selectMock->expects ( $this->exactly ( 2 ) )->method ( 'where' )->withConsecutive ( [ 
				$preparedSql 
		], [ 
				'e.entity_id IN (1,2)' 
		] )->willReturnSelf ();
		$this->collection->addCategoriesFilter ( [ 
