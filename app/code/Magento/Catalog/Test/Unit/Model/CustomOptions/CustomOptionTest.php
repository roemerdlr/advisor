...
class CustomOptionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$extensionAttributesFactory = $this->getMockBuilder ( 'Magento\Framework\Api\ExtensionAttributesFactory' )->disableOriginalConstructor ()->getMock ();
		
		$attributeValueFactory = $this->getMockBuilder ( 'Magento\Framework\Api\AttributeValueFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->fileProcessor = $this->getMockBuilder ( 'Magento\Catalog\Model\Webapi\Product\Option\Type\File\Processor' )->disableOriginalConstructor ()->getMock ();
		
		$resource = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\AbstractResource' )->disableOriginalConstructor ()->getMock ();
		
		$collection = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\AbstractDb' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = new CustomOption ( $context, $registry, $extensionAttributesFactory, $attributeValueFactory, $this->fileProcessor, $resource, $collection );
	}
	...
	public function testGetOptionValueWithFileInfo() {
		$customOption = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\CustomOptionExtensionInterface' )->setMethods ( [ 
				'getFileInfo' 
