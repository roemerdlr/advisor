...
class CustomOptionProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	/**
	 *
	 * @var \Magento\Catalog\Model\CustomOptions\CustomOptionFactory |\PHPUnit_Framework_MockObject_MockObject
	 */
	...
	public function setUp() {
		...
		$this->productOptionFactory = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\ProductOptionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->extensionFactory = $this->getMockBuilder ( 'Magento\Quote\Api\Data\ProductOptionExtensionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->customOptionFactory = $this->getMockBuilder ( 'Magento\Catalog\Model\CustomOptions\CustomOptionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->cartItem = $this->getMockBuilder ( 'Magento\Quote\Api\Data\CartItemInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getOptionByCode',
				'getProductOption',
				'setProductOption' 
		] )->getMockForAbstractClass ();
		$this->extensibleAttribute = $this->getMockBuilder ( 'Magento\Quote\Api\Data\ProductOptionExtensionInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'setCustomOptions',
				'getCustomOptions' 
		] )->getMockForAbstractClass ();
		$this->productOption = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\ProductOption' )->disableOriginalConstructor ()->getMock ();
		$this->customOption = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\CustomOptionInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		$this->buyRequest = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->getMock ();
		
		$this->processor = new CustomOptionProcessor ( $this->objectFactory, $this->productOptionFactory, $this->extensionFactory, $this->customOptionFactory );
	}
	public function testConvertToBuyRequest() {
		...
		$this->extensibleAttribute->expects ( $this->atLeastOnce () )->method ( 'getCustomOptions' )->willReturn ( [ 
				$this->customOption 
		] );
	...
	public function testProcessCustomOptions() {
		...
		$this->extensibleAttribute->expects ( $this->once () )->method ( 'setCustomOptions' )->with ( [ 
