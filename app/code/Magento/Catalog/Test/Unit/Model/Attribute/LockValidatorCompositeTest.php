...
class LockValidatorCompositeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCompositionsWithInvalidValidatorInstance() {
		...
		$this->model = new \Magento\Catalog\Model\Attribute\LockValidatorComposite ( $this->objectManagerMock, $validators );
	}
	public function testValidateWithValidValidatorInstance() {
		...
		$this->objectManagerMock->expects ( $this->any () )->method ( 'get' )->with ( 'Magento\Catalog\Model\Attribute\LockValidatorComposite' )->will ( $this->returnValue ( $lockValidatorMock ) );
