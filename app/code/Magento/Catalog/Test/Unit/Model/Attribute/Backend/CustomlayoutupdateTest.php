...
class CustomlayoutupdateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $helper->getObject ( '\Magento\Catalog\Model\Attribute\Backend\Customlayoutupdate', [ 
	...
	private function getMockedLayoutUpdateValidatorFactory() {
		...
		$mock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->getMockedValidator () ) );
	...
	private function getMockedValidator() {
		...
		$mock->expects ( $this->any () )->method ( 'isValid' )->will ( /**
		 *
		 * @param string $xml
		 *        	$return bool
		 */
		$this->returnCallback ( function ($xml) {
			if ($xml == 'exception') {
				return false;
			} else {
				return true;
		...
		$mock->expects ( $this->any () )->method ( 'getMessages' )->will ( $this->returnValue ( [ 
				'error' 
		] ) );
	...
	private function getMockedAttribute() {
		...
		$mock->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( $this->attributeName ) );
		
		$mock->expects ( $this->any () )->method ( 'getIsRequired' )->will ( $this->returnValue ( false ) );
