...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_fileResolverMock->expects ( $this->once () )->method ( 'get' )->with ( 'catalog_attributes.xml', 'scope' )->will ( $this->returnValue ( [ 
				file_get_contents ( __DIR__ . '/_files/attributes_config_one.xml' ),
				file_get_contents ( __DIR__ . '/_files/attributes_config_two.xml' ) 
		] ) );
		
		$this->_converter = $this->getMock ( 'Magento\Catalog\Model\Attribute\Config\Converter', [ 
				'convert' 
		] );
