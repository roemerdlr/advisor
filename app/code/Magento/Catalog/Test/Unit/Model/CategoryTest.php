...
class CategoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->context = $this->getMock ( 'Magento\Framework\Model\Context', [ 
				'getEventDispatcher',
				'getCacheManager' 
		], [ ], '', false );
		...
		$this->categoryTreeFactory = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Category\TreeFactory', [ 
				'create' 
		], [ ], '', false );
		$this->categoryRepository = $this->getMock ( 'Magento\Catalog\Api\CategoryRepositoryInterface' );
		$this->storeCollectionFactory = $this->getMock ( 'Magento\Store\Model\ResourceModel\Store\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->url = $this->getMock ( 'Magento\Framework\UrlInterface' );
		$this->productCollectionFactory = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->catalogConfig = $this->getMock ( 'Magento\Catalog\Model\Config', [ ], [ ], '', false );
		$this->filterManager = $this->getMock ( 'Magento\Framework\Filter\FilterManager', [ 
				'translitUrl' 
		], [ ], '', false );
		...
		$this->resource = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Category', [ ], [ ], '', false );
		$this->indexerRegistry = $this->getMock ( 'Magento\Framework\Indexer\IndexerRegistry', [ 
				'get' 
		], [ ], '', false );
	...
	public function testFormatUrlKey() {
		...
		$this->filterManager->expects ( $this->once () )->method ( 'translitUrl' )->with ( $strIn )->will ( $this->returnValue ( $resultString ) );
	...
	public function testMoveWhenCannotFindParentCategory() {
		...
		$parentCategory = $this->getMock ( 'Magento\Catalog\Model\Category', [ 
				'getId',
				'setStoreId',
				'load' 
		], [ ], '', false );
	...
	protected function getCategoryModel() {
		return (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Catalog\Model\Category', [ 
	 ...
	 * @param $expectedProductReindexCall @dataProvider
	...
	public function testReindexFlatEnabled($flatScheduled, $productScheduled, $expectedFlatReindexCalls, $expectedProductReindexCall) {
		...
		$this->flatState->expects ( $this->any () )->method ( 'isFlatEnabled' )->will ( $this->returnValue ( true ) );
		...
		$this->indexerRegistry->expects ( $this->at ( 0 ) )->method ( 'get' )->with ( Indexer\Category\Flat\State::INDEXER_ID )->will ( $this->returnValue ( $this->flatIndexer ) );
		
		$this->indexerRegistry->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( Indexer\Category\Product::INDEXER_ID )->will ( $this->returnValue ( $this->productIndexer ) );
	 ...
	 */
	public function testReindexFlatDisabled($productScheduled, $affectedIds, $isAnchorOrig, $isAnchor, $expectedProductReindexCall) {
