...
class ImageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->storeManager = $this->getMockBuilder ( 'Magento\Store\Model\StoreManager' )->disableOriginalConstructor ()->setMethods ( [ 
				'getStore',
				'getWebsite' 
		] )->getMock ();
		...
		$this->config = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Media\Config' )->setMethods ( [ 
				'getBaseMediaPath' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->config->expects ( $this->any () )->method ( 'getBaseMediaPath' )->will ( $this->returnValue ( 'catalog/product' ) );
		$this->coreFileHelper = $this->getMockBuilder ( 'Magento\MediaStorage\Helper\File\Storage\Database' )->setMethods ( [ 
				'saveFile',
				'deleteFolder' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->mediaDirectory = $this->getMockBuilder ( 'Magento\Framework\Filesystem\Directory\Write' )->disableOriginalConstructor ()->setMethods ( [ 
				'create',
				'isFile',
				'isExist',
				'getAbsolutePath' 
		...
		$this->image = $objectManagerHelper->getObject ( 'Magento\Catalog\Model\Product\Image', [ 
	...
	public function testSetBackgroundColor() {
		$result = $this->image->setBackgroundColor ( [ 
				0,
				0,
				0 
		] );
	...
	public function testSetWatermark() {
		...
		$imageProcessor = $this->getMockBuilder ( 'Magento\Framework\Image' )->disableOriginalConstructor ()->setMethods ( [ 
				'keepAspectRatio',
				'keepFrame',
				'keepTransparency',
				'constrainOnly',
		...
		$result = $this->image->setWatermark ( '/somefile.png', 'center', [ 
				'width' => 100,
				'height' => 100 
		], 100, 100, 50 );
		$this->assertSame ( $this->image, $result );
	...
	public function testGetUrl() {
		...
		$this->assertEquals ( 'http://magento.com/media/catalog/product/cache/1//beff4985b56e3afdbeabfc89641a4582/somefile.png', $url );
	}
