...
class RepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->attributeResourceMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Attribute', [ ], [ ], '', false );
		$this->productHelperMock = $this->getMock ( 'Magento\Catalog\Helper\Product', [ ], [ ], '', false );
		$this->filterManagerMock = $this->getMock ( 'Magento\Framework\Filter\FilterManager', [ ], [ ], '', false );
		$this->eavAttributeRepositoryMock = $this->getMock ( 'Magento\Eav\Api\AttributeRepositoryInterface', [ ], [ ], '', false );
		$this->eavConfigMock = $this->getMock ( 'Magento\Eav\Model\Config', [ ], [ ], '', false );
		$this->eavConfigMock->expects ( $this->any () )->method ( 'getEntityType' )->willReturn ( new \Magento\Framework\DataObject ( [ 
				'default_attribute_set_id' => 4 
		] ) );
		$this->validatorFactoryMock = $this->getMock ( 'Magento\Eav\Model\Adminhtml\System\Config\Source\Inputtype\ValidatorFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = new Repository ( $this->attributeResourceMock, $this->productHelperMock, $this->filterManagerMock, $this->eavAttributeRepositoryMock, $this->eavConfigMock, $this->validatorFactoryMock, $this->searchCriteriaBuilderMock );
	}
	...
	public function testGetList() {
		...
		$this->eavAttributeRepositoryMock->expects ( $this->once () )->method ( 'getList' )->with ( \Magento\Catalog\Api\Data\ProductAttributeInterface::ENTITY_TYPE_CODE, $searchCriteriaMock );
	...
	public function testGetCustomAttributesMetadata() {
		...
		$this->searchResultMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$itemMock 
		] );
		$expected = [ 
				$itemMock 
	...
	public function testSaveNoSuchEntityException() {
		...
		$this->eavAttributeRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( \Magento\Catalog\Api\Data\ProductAttributeInterface::ENTITY_TYPE_CODE, $attributeCode )->willReturn ( $existingModelMock );
	...
	public function testSaveInputExceptionRequiredField() {
		$attributeMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Eav\Attribute', [ 
				'getFrontendLabels',
	...
	public function testSaveInputExceptionInvalidFieldValue() {
		...
		$attributeMock->expects ( $this->exactly ( 4 ) )->method ( 'getFrontendLabels' )->willReturn ( [ 
				$labelMock 
		] );
