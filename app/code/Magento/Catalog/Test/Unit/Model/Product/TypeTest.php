...
class TypeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_model = $this->_objectHelper->getObject ( 'Magento\Catalog\Model\Product\Type', [ 
				'config' => $mockedConfig,
	...
	private function getMockedProduct() {
		$mockBuilder = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->disableOriginalConstructor ();
	...
	private function getMockedPriceInfoFactory() {
		...
		$mockBuilder = $this->getMockBuilder ( '\Magento\Framework\Pricing\PriceInfo\Factory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] );
		$mock = $mockBuilder->getMock ();
		
		$mock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $mockedPriceInfoInterface ) );
	...
	private function getMockedPriceInfoInterface() {
		$mockBuilder = $this->getMockBuilder ( '\Magento\Framework\Pricing\PriceInfoInterface' )->disableOriginalConstructor ();
	...
	private function getMockedProductTypePool() {
		$mockBuild = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Type\Pool' )->disableOriginalConstructor ()->setMethods ( [ 
				'get' 
		] );
		$mock = $mockBuild->getMock ();
		
		$mock->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValueMap ( [ 
				[ 
						'some_model',
						[ ],
						$this->getMockedProductTypeVirtual () 
				],
				[ 
						'Magento\Catalog\Model\Product\Type\Simple',
						[ ],
						$this->getMockedProductTypeSimple () 
				] 
		] ) );
	...
	private function getMockedProductTypeVirtual() {
		$mockBuilder = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Type\Virtual' )->disableOriginalConstructor ()->setMethods ( [ 
				'setConfig' 
		] );
		$mock = $mockBuilder->getMock ();
		
		$mock->expects ( $this->any () )->method ( 'setConfig' );
	...
	private function getMockedProductTypeSimple() {
		$mockBuilder = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Type\Simple' )->disableOriginalConstructor ()->setMethods ( [ 
				'setConfig' 
		] );
		$mock = $mockBuilder->getMock ();
	...
	private function getMockedConfig() {
		$mockBuild = $this->getMockBuilder ( '\Magento\Catalog\Model\ProductTypes\ConfigInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getAll' 
		] );
		$mock = $mockBuild->getMockForAbstractClass ();
		
		$mock->expects ( $this->any () )->method ( 'getAll' )->will ( $this->returnValue ( $this->_productTypes ) );
	...
	private function getMockedTypePriceFactory() {
		$mockBuild = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Type\Price\Factory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] );
		$mock = $mockBuild->getMockForAbstractClass ();
		
		$mock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValueMap ( [ 
				[ 
						'some_model',
						[ ],
						$this->getMockedProductTypeConfigurablePrice () 
				],
				[ 
						'Magento\Catalog\Model\Product\Type\Price',
						[ ],
						$this->getMockedProductTypePrice () 
				] 
		] ) );
	...
	private function getMockedProductTypePrice() {
		$mockBuild = $this->getMockBuilder ( '\Magento\Catalog\Model\Product\Type\Price' )->disableOriginalConstructor ();
		$mock = $mockBuild->getMock ();
	...
	private function getMockedProductTypeConfigurablePrice() {
		$mockBuild = $this->getMockBuilder ( '\Magento\ConfigurableProduct\Model\Product\Type\Configurable\Price' )->disableOriginalConstructor ();
		$mock = $mockBuild->getMock ();
