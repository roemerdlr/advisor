...
class TypesListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->inputTypeFactoryMock = $this->getMock ( 'Magento\Catalog\Model\Product\Attribute\Source\InputtypeFactory', [ 
				'create',
				'__wakeup' 
		], [ ], '', false );
		$this->attributeTypeFactoryMock = $this->getMock ( 'Magento\Catalog\Api\Data\ProductAttributeTypeInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->dataObjectHelperMock = $this->getMockBuilder ( '\Magento\Framework\Api\DataObjectHelper' )->disableOriginalConstructor ()->getMock ();
		$this->model = new TypesList ( $this->inputTypeFactoryMock, $this->attributeTypeFactoryMock, $this->dataObjectHelperMock );
	}
	public function testGetItems() {
		...
		$inputTypeMock->expects ( $this->once () )->method ( 'toOptionArray' )->willReturn ( [ 
				'option' => [ 
						'value' 
				] 
		] );
		$attributeTypeMock = $this->getMock ( '\Magento\Catalog\Api\Data\ProductAttributeTypeInterface' );
		$this->dataObjectHelperMock->expects ( $this->once () )->method ( 'populateWithArray' )->with ( $attributeTypeMock, [ 
				'value' 
		], '\Magento\Catalog\Api\Data\ProductAttributeTypeInterface' )->willReturnSelf ();
