...
class AbstractTypeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productResource = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Product' )->setMethods ( [ 
				'getSortedAttributes',
				'loadAllAttributes' 
		...
		$this->attribute = $this->getMockBuilder ( 'Magento\Catalog\Model\Entity\Attribute' )->setMethods ( [ 
				'getGroupSortPath',
				'getSortPath',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testGetAttributeById() {
		...
		$this->productResource->expects ( $this->any () )->method ( 'getSortedAttributes' )->will ( $this->returnValue ( [ 
				$this->attribute 
		] ) );
	...
	public function testGetEditableAttributes() {
		...
		$this->productResource->expects ( $this->any () )->method ( 'loadAllAttributes' )->will ( $this->returnValue ( $this->productResource ) );
		$this->productResource->expects ( $this->any () )->method ( 'getSortedAttributes' )->will ( $this->returnValue ( $expected ) );
