...
class ToolbarTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->toolbarModel = (new ObjectManager ( $this ))->getObject ( 'Magento\Catalog\Model\Product\ProductList\Toolbar', [ 
	...
	public function testGetLimit($param) {
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( Toolbar::LIMIT_PARAM_NAME )->will ( $this->returnValue ( $param ) );
	...
	public function testGetCurrentPage($param) {
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( Toolbar::PAGE_PARM_NAME )->will ( $this->returnValue ( $param ) );
