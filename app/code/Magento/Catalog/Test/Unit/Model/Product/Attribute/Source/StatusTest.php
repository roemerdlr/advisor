...
class StatusTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->collection = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\Collection', [ 
				'__wakeup',
				'getSelect',
				'joinLeft',
				'order',
		...
		$this->attributeModel = $this->getMock ( '\Magento\Catalog\Model\Entity\Attributee', [ 
				'__wakeup',
				'getAttributeCode',
				'getBackend',
				'getId',
		...
		$this->backendAttributeModel = $this->getMock ( '\Magento\Catalog\Model\Product\Attribute\Backend\Sku', [ 
				'__wakeup',
				'getTable' 
		], [ ], '', false );
	...
	public function testAddValueSortToCollectionGlobal() {
		$this->attributeModel->expects ( $this->any () )->method ( 'isScopeGlobal' )->will ( $this->returnValue ( true ) );
		$this->collection->expects ( $this->once () )->method ( 'order' )->with ( 'attribute_code_t.value asc' )->will ( $this->returnSelf () );
	...
	public function testGetVisibleStatusIds() {
		$this->assertEquals ( [ 
				0 => 1 
		], $this->status->getVisibleStatusIds () );
	}
	public function testGetSaleableStatusIds() {
		$this->assertEquals ( [ 
				0 => 1 
		], $this->status->getSaleableStatusIds () );
	}
