...
class ProductTest extends \PHPUnit_Framework_TestCase {
	...
	public function getData() {
		...
		$product2->expects ( $this->once () )->method ( 'getData' )->with ( $this->equalTo ( 'attribute' ) )->will ( $this->returnValue ( true ) );
		
		$product3 = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->getMock ();
		$product3->expects ( $this->once () )->method ( 'dataHasChangedFor' )->with ( $this->equalTo ( 'attribute' ) )->will ( $this->returnValue ( true ) );
