...
class IndexerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$flatHelperMock = $this->getMock ( 'Magento\Catalog\Helper\Product\Flat\Indexer', [ 
				'isAddChildData' 
		], [ ], '', false );
		...
		$resourceConfigFactoryMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\ConfigFactory', [ 
				'create' 
		], [ ], '', false );
		
		$eavFactoryMock = $this->getMock ( 'Magento\Eav\Model\Entity\AttributeFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->_storeManagerMock = $this->getMock ( 'Magento\Store\Model\StoreManagerInterface' );
		
		$this->_connectionMock = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'getTables',
				'dropTable' 
		], [ ], '', false );
		
		$this->_changelogMock = $this->getMock ( 'Magento\Framework\Mview\View\Changelog', [ 
				'getName' 
		], [ ], '', false );
		
		$this->_objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->_model = $this->_objectManager->getObject ( 'Magento\Catalog\Helper\Product\Flat\Indexer', [ 
				'context' => $contextMock,
	...
	public function testDeleteAbandonedStoreFlatTables() {
		...
		$this->_connectionMock->expects ( $this->once () )->method ( 'getTables' )->with ( 'catalog_product_flat_%' )->will ( $this->returnValue ( [ 
		...
		$this->_resourceMock->expects ( $this->once () )->method ( 'getConnection' )->will ( $this->returnValue ( $this->_connectionMock ) );
		
		$this->_setStoreManagerExpectedStores ( [ 
				1,
				2 
		] );
