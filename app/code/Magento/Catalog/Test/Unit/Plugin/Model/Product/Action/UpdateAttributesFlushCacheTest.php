...
class UpdateAttributesFlushCacheTest extends \PHPUnit_Framework_TestCase {
	public function testAroundUpdateAttributes() {
		...
		$cacheContextMock->expects ( $this->once () )->method ( 'registerEntities' )->with ( Product::CACHE_TAG, $productIds );
		
		$eventManagerMock = $this->getMock ( 'Magento\Framework\Event\ManagerInterface' );
		$eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( 'clean_cache_by_tags', [ 
				'object' => $cacheContextMock 
		] );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$model = $objectManager->getObject ( 'Magento\Catalog\Plugin\Model\Product\Action\UpdateAttributesFlushCache', [ 
