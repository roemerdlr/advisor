...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetAttributesUsedInListingOnCacheDisabled() {
		...
		$this->assertEquals ( [ 
				'attributes' 
		], $this->getConfig ( false )->aroundGetAttributesUsedInListing ( $this->subject, $this->mockPluginProceed ( [ 
				'attributes' 
		] ) ) );
	}
	public function testGetAttributesUsedInListingFromCache() {
		...
		$cacheId = \Magento\Catalog\Plugin\Model\ResourceModel\Config::PRODUCT_LISTING_ATTRIBUTES_CACHE_ID . $entityTypeId . '_' . $storeId;
	...
	public function testGetAttributesUsedInListingWithCacheSave() {
		...
		$this->cache->expects ( $this->any () )->method ( 'save' )->with ( serialize ( $attributes ), $cacheId, [ 
				\Magento\Eav\Model\Cache\Type::CACHE_TAG,
				\Magento\Eav\Model\Entity\Attribute::CACHE_TAG 
		] );
		
		$this->assertEquals ( $attributes, $this->getConfig ( true )->aroundGetAttributesUsedInListing ( $this->subject, $this->mockPluginProceed ( $attributes ) ) );
	}
	public function testGetAttributesUsedForSortByOnCacheDisabled() {
		...
		$this->assertEquals ( [ 
				'attributes' 
		], $this->getConfig ( false )->aroundGetAttributesUsedForSortBy ( $this->subject, $this->mockPluginProceed ( [ 
				'attributes' 
		] ) ) );
	}
	...
	protected function getConfig($cacheEnabledFlag) {
		...
		return (new ObjectManager ( $this ))->getObject ( 'Magento\Catalog\Plugin\Model\ResourceModel\Config', [ 
				'cache' => $this->cache,
