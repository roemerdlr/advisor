...
class BuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->wysiwygConfigMock = $this->getMock ( 'Magento\Cms\Model\Wysiwyg\Config', [ 
				'setStoreId' 
		], [ ], '', false );
		...
		$this->builder = new Builder ( $this->productFactoryMock, $this->loggerMock, $this->registryMock, $this->wysiwygConfigMock );
	}
	...
	public function testBuildWhenImpossibleLoadProduct() {
		...
		$this->productFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->productMock ) );
		$this->productMock->expects ( $this->once () )->method ( 'setStoreId' )->with ( 'some_store' )->will ( $this->returnSelf () );
		$this->productMock->expects ( $this->once () )->method ( 'setTypeId' )->with ( \Magento\Catalog\Model\Product\Type::DEFAULT_TYPE )->will ( $this->returnSelf () );
		$this->productMock->expects ( $this->once () )->method ( 'load' )->with ( 15 )->will ( $this->throwException ( new \Exception () ) );
