...
abstract class ProductTest extends \PHPUnit_Framework_TestCase {
	...
	protected function initContext(array $additionalParams = []) {
		...
		$eventManager = $this->getMockBuilder ( 'Magento\Framework\Event\Manager' )->setMethods ( [ 
				'dispatch' 
		] )->disableOriginalConstructor ()->getMock ();
		$eventManager->expects ( $this->any () )->method ( 'dispatch' )->will ( $this->returnSelf () );
		$title = $this->getMockBuilder ( '\Magento\Framework\App\Action\Title' )->setMethods ( [ 
				'add' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$sessionMock = $this->getMock ( 'Magento\Backend\Model\Session', [ 
				'getProductData',
				'setProductData' 
		], [ ], '', false );
