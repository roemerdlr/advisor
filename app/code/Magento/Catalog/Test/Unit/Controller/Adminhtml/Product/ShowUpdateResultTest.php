...
class ShowUpdateResultTest extends \PHPUnit_Framework_TestCase {
	...
	protected function getSession() {
		$session = $this->getMock ( 'Magento\Backend\Model\Session', [ 
				'hasCompositeProductResult',
				'getCompositeProductResult',
				'unsCompositeProductResult' 
		], [ ], '', false );
		$session->expects ( $this->once () )->method ( 'hasCompositeProductResult' )->willReturn ( true );
		$session->expects ( $this->once () )->method ( 'unsCompositeProductResult' );
		$session->expects ( $this->atLeastOnce () )->method ( 'getCompositeProductResult' )->willReturn ( new \Magento\Framework\DataObject () );
	...
	protected function getContext() {
		...
		$objectManagerMock->expects ( $this->any () )->method ( 'get' )->willreturn ( $productActionMock );
		
		$eventManager = $this->getMockForAbstractClass ( 'Magento\Framework\Event\Manager', [ 
				'dispatch' 
		], '', false );
		...
		$responseInterfaceMock = $this->getMock ( 'Magento\Framework\App\ResponseInterface', [ 
				'setRedirect',
				'sendResponse' 
		], [ ], '', false );
		...
		$this->context->expects ( $this->any () )->method ( 'getObjectManager' )->willReturn ( $objectManagerMock );
		
		$this->context->expects ( $this->any () )->method ( 'getMessageManager' )->willReturn ( $managerInterfaceMock );
