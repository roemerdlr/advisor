...
class ValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->fileSize = $this->getMock ( 'Magento\Framework\File\Size', [ ], [ ], '', false );
		$this->coreFileStorageDatabase = $this->getMock ( 'Magento\MediaStorage\Helper\File\Storage\Database', [ ], [ ], '', false );
		$this->validateFactory = $this->getMock ( 'Magento\Catalog\Model\Product\Option\Type\File\ValidateFactory', [ ], [ ], '', false );
		$this->option = $this->getMock ( 'Magento\Catalog\Model\Product\Option', [ ], [ ], '', false );
		$this->directoryRead = $this->getMockForAbstractClass ( 'Magento\Framework\Filesystem\Directory\ReadInterface', [ ], '', false );
		$this->zendValidator = $this->getMock ( 'Zend_Validate', [ ], [ ], '', false );
		
		$this->filesystem->expects ( $this->once () )->method ( 'getDirectoryRead' )->with ( 'base' )->willReturn ( $this->directoryRead );
		
		$this->validator = new Validator ( $this->scopeConfig, $this->filesystem, $this->fileSize, $this->coreFileStorageDatabase, $this->validateFactory );
	}
	...
	public function testValidate() {
		...
		$this->directoryRead->expects ( $this->once () )->method ( 'isReadable' )->with ( $relativePath )->willReturn ( true );
	...
	public function testValidateWithInvalidFile() {
		...
		$this->zendValidator->expects ( $this->at ( 3 ) )->method ( 'getErrors' )->willReturn ( [ 
