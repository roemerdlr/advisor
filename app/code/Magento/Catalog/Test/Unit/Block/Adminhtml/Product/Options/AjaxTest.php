...
class AjaxTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->context = $this->getMockBuilder ( 'Magento\Backend\Block\Context' )->setMethods ( [ 
				'getEventManager',
				'getScopeConfig',
				'getLayout',
				'getRequest' 
	...
	public function testToHtml() {
		$eventManager = $this->getMockBuilder ( 'Magento\Framework\Event\Manager' )->disableOriginalConstructor ()->setMethods ( [ 
				'dispatch' 
		] )->getMock ();
		$eventManager->expects ( $this->once () )->method ( 'dispatch' )->will ( $this->returnValue ( true ) );
		
		$scopeConfig = $this->getMockBuilder ( '\Magento\Framework\App\Config' )->setMethods ( [ 
				'getValue' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$optionsBlock = $this->getMockBuilder ( 'Magento\Catalog\Block\Adminhtml\Product\Edit\Tab\Options\Option' )->setMethods ( [ 
				'setIgnoreCaching',
				'setProduct',
				'getOptionValues' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$layout = $this->getMockBuilder ( 'Magento\Framework\View\Layout\Element\Layout' )->disableOriginalConstructor ()->setMethods ( [ 
				'createBlock' 
		] )->getMock ();
		...
		$request = $this->getMockBuilder ( 'Magento\Framework\App\Request\Http' )->setMethods ( [ 
				'getParam' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->registry->expects ( $this->once () )->method ( 'registry' )->with ( 'import_option_products' )->will ( $this->returnValue ( [ 
				1 
		] ) );
		$this->productFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $product ) );
		
		$this->block = $this->objectManagerHelper->getObject ( 'Magento\Catalog\Block\Adminhtml\Product\Options\Ajax', [ 
