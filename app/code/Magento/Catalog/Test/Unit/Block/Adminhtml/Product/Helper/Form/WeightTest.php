...
class WeightTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->weightSwitcher = $this->getMock ( 'Magento\Framework\Data\Form\Element\Radios', [ 
				'setId',
				'setName',
				'setLabel',
				'setForm' 
		...
		$this->factory->expects ( $this->once () )->method ( 'create' )->with ( $this->equalTo ( 'radios' ) )->will ( $this->returnValue ( $this->weightSwitcher ) );
		$this->localeFormat = $this->getMock ( '\Magento\Framework\Locale\Format', [ ], [ ], '', false );
		
		$this->collectionFactory = $this->getMock ( 'Magento\Framework\Data\Form\Element\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->_model = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Catalog\Block\Adminhtml\Product\Helper\Form\Weight', [ 
	...
	public function testSetForm() {
		...
		$this->weightSwitcher->expects ( $this->any () )->method ( 'setForm' )->with ( $this->equalTo ( $form ) )->will ( $this->returnSelf () );
		
		$this->_model->setForm ( $form );
	}
	public function testGetEscapedValue() {
		$this->localeFormat->expects ( $this->any () )->method ( 'getPriceFormat' )->willReturn ( [ 
				'precision' => 2,
				'decimalSymbol' => ',',
				'groupSymbol' => '.' 
		] );
