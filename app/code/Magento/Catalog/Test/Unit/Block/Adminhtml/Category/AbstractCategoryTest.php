...
class AbstractCategoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->category = $this->objectManager->getObject ( 'Magento\Catalog\Block\Adminhtml\Category\AbstractCategory', [ 
	...
	public function testGetRootIdsFromCache() {
		$this->category->setData ( 'root_ids', [ 
				'ids' 
		] );
		...
		$this->assertEquals ( [ 
				'ids' 
		], $this->category->getRootIds () );
	}
	public function testGetRootIds() {
		$this->storeManagerMock->expects ( $this->once () )->method ( 'getGroups' )->willReturn ( [ 
				$this->storeMock 
		] );
