...
class InventoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->contextMock = $this->getMock ( 'Magento\Backend\Block\Template\Context', [ 
				'getRequest',
				'getStoreManager' 
		], [ ], '', false );
		$this->stockConfigurationMock = $this->getMockForAbstractClass ( 'Magento\CatalogInventory\Api\StockConfigurationInterface', [ ], '', false );
		$this->stockRegistryMock = $this->getMockForAbstractClass ( 'Magento\CatalogInventory\Api\StockRegistryInterface', [ ], '', false );
		$this->backordersMock = $this->getMock ( 'Magento\CatalogInventory\Model\Source\Backorders', [ ], [ ], '', false );
		$this->stockMock = $this->getMock ( 'Magento\CatalogInventory\Model\Source\Stock', [ ], [ ], '', false );
		$this->coreRegistryMock = $this->getMock ( 'Magento\Framework\Registry', [ ], [ ], '', false );
		$this->moduleManager = $this->getMock ( 'Magento\Framework\Module\Manager', [ ], [ ], '', false );
		$this->storeManagerMock = $this->getMockForAbstractClass ( 'Magento\Store\Model\StoreManagerInterface', [ ], '', false );
		
		$this->contextMock->expects ( $this->once () )->method ( 'getStoreManager' )->will ( $this->returnValue ( $this->storeManagerMock ) );
		
		$this->inventory = $objectManager->getObject ( 'Magento\Catalog\Block\Adminhtml\Product\Edit\Tab\Inventory', [ 
	 ...
	 * @return void @dataProvider dataProviderModuleEnabled
	 */
	public function testGetBackordersOption($moduleEnabled) {
		...
		if ($moduleEnabled) {
			$this->backordersMock->expects ( $this->once () )->method ( 'toOptionArray' )->will ( $this->returnValue ( [ 
					'test-value',
					'test-value' 
			] ) );
		}
	...
	public function testGetStockOption($moduleEnabled) {
		$this->moduleManager->expects ( $this->once () )->method ( 'isEnabled' )->with ( 'Magento_CatalogInventory' )->will ( $this->returnValue ( $moduleEnabled ) );
		if ($moduleEnabled) {
			$this->stockMock->expects ( $this->once () )->method ( 'toOptionArray' )->will ( $this->returnValue ( [ 
					'test-value',
					'test-value' 
			] ) );
		}
	...
	public function testGetProduct() {
		$this->coreRegistryMock->expects ( $this->once () )->method ( 'registry' )->with ( 'product' )->will ( $this->returnValue ( 'return-value' ) );
	...
	public function testGetStockItem() {
		...
		$this->stockRegistryMock->expects ( $this->once () )->method ( 'getStockItem' )->with ( $productId, $websiteId )->will ( $this->returnValue ( 'return-value' ) );
	 ...
	 * @param array $methods        	
	 * @param string $result        	
	 * @return void @dataProvider dataProviderGetFieldValue
	 */
	 ...
	 * @param array $methods        	
	 * @param string $result        	
	 * @return void @dataProvider dataProviderGetConfigFieldValue
	 */
	...
	public function testIsReadonly() {
		...
		$this->coreRegistryMock->expects ( $this->once () )->method ( 'registry' )->with ( 'product' )->will ( $this->returnValue ( $productMock ) );
		
		$productMock->expects ( $this->once () )->method ( 'getInventoryReadonly' )->will ( $this->returnValue ( 'return-value' ) );
	 ...
	 * @param int|null $id        	
	 * @param bool $result        	
	 * @return void @dataProvider dataProviderGetId
	 */
	...
	public function testCanUseQtyDecimals() {
		...
		$typeMock = $this->getMockForAbstractClass ( 'Magento\Catalog\Model\Product\Type\AbstractType', [ ], '', false, true, true, [ 
				'canUseQtyDecimals' 
		] );
		$this->coreRegistryMock->expects ( $this->once () )->method ( 'registry' )->with ( 'product' )->will ( $this->returnValue ( $productMock ) );
		$productMock->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $typeMock ) );
		$typeMock->expects ( $this->once () )->method ( 'canUseQtyDecimals' )->will ( $this->returnValue ( 'return-value' ) );
	...
	public function testIsVirtual() {
		...
		$this->coreRegistryMock->expects ( $this->once () )->method ( 'registry' )->with ( 'product' )->will ( $this->returnValue ( $productMock ) );
		$productMock->expects ( $this->once () )->method ( 'getIsVirtual' )->will ( $this->returnValue ( 'return-value' ) );
	...
	public function testIsSingleStoreMode() {
		$this->storeManagerMock->expects ( $this->once () )->method ( 'isSingleStoreMode' )->will ( $this->returnValue ( 'return-value' ) );
