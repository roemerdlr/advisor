...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$context->expects ( $this->any () )->method ( 'getStoreManager' )->will ( $this->returnValue ( $this->storeManager ) );
	...
	public function testGetHrefWithSetStoreId() {
		...
		$this->storeManager->expects ( $this->once () )->method ( 'getStore' )->with ( 'store_id' )->
	...
	public function testGetHrefIfRewriteIsNotFound() {
		...
		$this->urlFinder->expects ( $this->once () )->method ( 'findOneByData' )->will ( $this->returnValue ( false ) );
	...
	public function testGetHrefWithoutUrlStoreSuffix($url, $separator) {
		...
		$rewrite->expects ( $this->once () )->method ( 'getRequestPath' )->will ( $this->returnValue ( $requestPath ) );
		...
		$store->expects ( $this->once () )->method ( 'getUrl' )->with ( '', [ 
				'_direct' => $requestPath 
		...
		$this->urlFinder->expects ( $this->once () )->method ( 'findOneByData' )->with ( [ 
