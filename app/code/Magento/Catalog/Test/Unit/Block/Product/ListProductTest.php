...
class ListProductTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$layerResolver->expects ( $this->any () )->method ( $this->anything () )->will ( $this->returnValue ( $this->layerMock ) );
		$this->postDataHelperMock = $this->getMock ( 'Magento\Framework\Data\Helper\PostHelper', [ ], [ ], '', false );
		$this->typeInstanceMock = $this->getMock ( 'Magento\Catalog\Model\Product\Type\Simple', [ ], [ ], '', false, false );
		$this->productMock = $this->getMock ( 'Magento\Catalog\Model\Product', [ ], [ ], '', false );
		$this->cartHelperMock = $this->getMock ( 'Magento\Checkout\Helper\Cart', [ ], [ ], '', false );
		
		$this->urlHelperMock = $this->getMockBuilder ( 'Magento\Framework\Url\Helper\Data' )->disableOriginalConstructor ()->getMock ();
	...
	public function testGetIdentities() {
		...
		$this->productMock->expects ( $this->once () )->method ( 'getIdentities' )->will ( $this->returnValue ( [ 
				$productTag 
		] ) );
		
		$itemsCollection = new \ReflectionProperty ( 'Magento\Catalog\Block\Product\ListProduct', '_productCollection' );
		$itemsCollection->setAccessible ( true );
		$itemsCollection->setValue ( $this->block, [ 
				$this->productMock 
		] );
