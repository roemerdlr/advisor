...
class ImageBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->helperFactory = $this->getMockBuilder ( 'Magento\Catalog\Helper\ImageFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->imageFactory = $this->getMockBuilder ( 'Magento\Catalog\Block\Product\ImageFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->model = new \Magento\Catalog\Block\Product\ImageBuilder ( $this->helperFactory, $this->imageFactory );
	}
	public function testSetProduct() {
		$productMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->getMock ();
		
		$this->assertInstanceOf ( 'Magento\Catalog\Block\Product\ImageBuilder', $this->model->setProduct ( $productMock ) );
	}
	public function testSetImageId() {
		$imageId = 'test_image_id';
		
		$this->assertInstanceOf ( 'Magento\Catalog\Block\Product\ImageBuilder', $this->model->setImageId ( $imageId ) );
	}
	public function testSetAttributes() {
		$attributes = [ 
				'name' => 'value' 
		];
		$this->assertInstanceOf ( 'Magento\Catalog\Block\Product\ImageBuilder', $this->model->setAttributes ( $attributes ) );
	}
