...
class GalleryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->arrayUtils = $this->getMockBuilder ( 'Magento\Framework\Stdlib\ArrayUtils' )->disableOriginalConstructor ()->getMock ();
		
		$this->jsonEncoderMock = $this->getMockBuilder ( 'Magento\Framework\Json\EncoderInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = new \Magento\Catalog\Block\Product\View\Gallery ( $this->context, $this->arrayUtils, $this->jsonEncoderMock );
	}
	...
	public function testGetGalleryImages() {
		...
		$this->imageHelper->expects ( $this->exactly ( 3 ) )->method ( 'init' )->willReturnMap ( [ 
				[ 
						$productMock,
	...
	private function getImagesCollection() {
		...
		$collectionMock->expects ( $this->any () )->method ( 'getIterator' )->willReturn ( new \ArrayIterator ( $items ) );
