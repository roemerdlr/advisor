...
class NewWidgetTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->eventManager = $this->getMock ( 'Magento\Framework\Event\Manager', [ 
				'dispatch' 
		], [ ], '', false, false );
		$this->scopeConfig = $this->getMock ( 'Magento\Framework\App\Config', [ 
				'getValue' 
		], [ ], '', false, false );
		$this->cacheState = $this->getMock ( 'Magento\Framework\App\Cache\State', [ 
				'isEnabled' 
		], [ ], '', false, false );
		$this->localDate = $this->getMock ( 'Magento\Framework\Stdlib\DateTime\Timezone', [ ], [ ], '', false, false );
		$this->catalogConfig = $this->getMockBuilder ( 'Magento\Catalog\Model\Config' )->setMethods ( [ 
		...
		$this->context = $this->getMockBuilder ( 'Magento\Catalog\Block\Product\Context' )->setMethods ( [ 
				'getEventManager',
				'getScopeConfig',
				'getLayout',
				'getRequest',
				...
				'getCatalogConfig',
				'getLocaleDate' 
		] )->disableOriginalConstructor ()->disableArgumentCloning ()->getMock ();
		
		$this->context->expects ( $this->any () )->method ( 'getLayout' )->willReturn ( $this->layout );
		$this->context->expects ( $this->any () )->method ( 'getRequest' )->willReturn ( $this->requestMock );
		
		$this->block = $this->objectManager->getObject ( 'Magento\Catalog\Block\Product\Widget\NewWidget', [ 
	...
	public function testGetProductPriceHtml() {
		...
		$priceBoxMock = $this->getMock ( 'Magento\Framework\Pricing\Render', [ 
				'render' 
		], [ ], '', false, false );
	...
	protected function generalGetProductCollection() {
		...
		$this->productCollection = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Product\Collection' )->setMethods ( [ 
				'setVisibility',
				'addMinimalPrice',
				'addFinalPrice',
				'addTaxPercents',
	...
	protected function startTestGetProductCollection($displayType, $pagerEnable, $productsCount, $productsPerPage) {
		$productCollectionFactory = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\CollectionFactory', [ 
				'create' 
		], [ ], '', false, false );
	...
	public function testGetProductNewCollection($pagerEnable, $productsCount, $productsPerPage, $expectedPageSize) {
		...
		$this->productCollection->expects ( $this->exactly ( 2 ) )->method ( 'setPageSize' )->withConsecutive ( [ 
				$productsCount 
		], [ 
				$expectedPageSize 
		] )->willReturnSelf ();
		
		$this->startTestGetProductCollection ( NewWidget::DISPLAY_TYPE_NEW_PRODUCTS, $pagerEnable, $productsCount, $productsPerPage );
	}
