...
class CategoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->categoryFactory = $this->getMockBuilder ( 'Magento\Catalog\Model\CategoryFactory' )->setMethods ( [ 
				'create' 
		...
		$this->block = $objectManagerHelper->getObject ( 'Magento\Catalog\Block\Rss\Category', [ 
	...
	public function testGetRssData() {
		$category = $this->getMockBuilder ( '\Magento\Catalog\Model\Category' )->setMethods ( [ 
				'__sleep',
				'__wakeup',
				'load',
				'getId',
		...
		$this->rssModel->expects ( $this->once () )->method ( 'getProductCollection' )->will ( $this->returnValue ( [ 
				$product 
		] ) );
		$this->imageHelper->expects ( $this->once () )->method ( 'init' )->with ( $product, 'rss_thumbnail' )->will ( $this->returnSelf () );
	...
	public function testGetFeeds() {
		...
		$collection = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Category\Collection' )->setMethods ( [ 
				'addIdFilter',
		...
		$node = new \Magento\Framework\DataObject ( [ 
				'id' => 1 
		] );
		$nodes = $this->getMockBuilder ( 'Magento\Framework\Data\Tree\Node' )->setMethods ( [ 
				'getChildren' 
		] )->disableOriginalConstructor ()->getMock ();
		$nodes->expects ( $this->once () )->method ( 'getChildren' )->will ( $this->returnValue ( [ 
				$node 
		] ) );
		
		$tree = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Category\Tree' )->setMethods ( [ 
				'loadChildren',
				'loadNode' 
		] )->disableOriginalConstructor ()->getMock ();
