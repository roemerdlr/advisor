...
class SpecialTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->httpContext = $this->getMockBuilder ( 'Magento\Framework\App\Http\Context' )->setMethods ( [ 
				'getValue' 
		...
		$this->outputHelper = $this->getMock ( 'Magento\Catalog\Helper\Output', [ 
				'productAttribute' 
		], [ ], '', false );
		$this->msrpHelper = $this->getMock ( 'Magento\Msrp\Helper\Data', [ 
				'canApplyMsrp' 
		], [ ], '', false );
		...
		$this->block = $objectManagerHelper->getObject ( 'Magento\Catalog\Block\Rss\Product\Special', [ 
	...
	public function testGetRssData() {
		$this->rssUrlBuilder->expects ( $this->once () )->method ( 'getUrl' )->with ( [ 
				'type' => 'special_products',
				'store_id' => 1 
		] )->will ( $this->returnValue ( 'http://magento.com/rss/feed/index/type/special_products/store_id/1' ) );
		$item = $this->getItemMock ();
		$this->rssModel->expects ( $this->once () )->method ( 'getProductsCollection' )->will ( $this->returnValue ( [ 
				$item 
		] ) );
	...
	protected function getItemMock() {
		$item = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->setMethods ( [ 
				'__wakeup',
				'__sleep',
				'getName',
				'getProductUrl',
		...
		$item->expects ( $this->exactly ( 2 ) )->method ( 'getProductUrl' )->will ( $this->returnValue ( 'http://magento.com/product-name.html' ) );
		
		return $item;
	}
	public function testIsAllowed() {
		$this->scopeConfig->expects ( $this->once () )->method ( 'isSetFlag' )->with ( 'rss/catalog/special', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->will ( $this->returnValue ( true ) );
