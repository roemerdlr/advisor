...
class RefreshSpecialPricesTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_storeManagerMock = $this->getMock ( 'Magento\Store\Model\StoreManagerInterface', [ ], [ ], '', false );
		...
		$this->_priceProcessorMock = $this->getMock ( 'Magento\Catalog\Model\Indexer\Product\Price\Processor', [ ], [ ], '', false );
	...
	public function testRefreshSpecialPrices() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'fetchCol' )->with ( $selectMock, [ 
				'entity_id' 
		...
		$this->_storeManagerMock->expects ( $this->once () )->method ( 'getStores' )->with ( true )->will ( $this->returnValue ( [ 
				$storeMock 
		] ) );
		...
		$attributeMock = $this->getMockForAbstractClass ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute', [ ], '', false, true, true, [ 
				'__wakeup',
				'getAttributeId' 
		] );
