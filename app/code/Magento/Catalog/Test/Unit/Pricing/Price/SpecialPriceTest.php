...
class SpecialPriceTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetValue($isValidInterval, $specialPrice, $specialPriceValue) {
		$expected = 56.34;
		$specialPriceModel = $this->objectManager->getObject ( 'Magento\Catalog\Pricing\Price\SpecialPrice', [ 
	...
	protected function prepareSaleableItem($specialPrice) {
		$saleableItemMock = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getSpecialPrice',
				'getPriceInfo',
				'getStore',
				'__wakeup' 
		...
		$saleableItemMock->expects ( $this->any () )->method ( 'getSpecialPrice' )->will ( $this->returnValue ( $specialPrice ) );
		
		$priceInfo = $this->getMockBuilder ( 'Magento\Framework\Pricing\PriceInfoInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		
		$priceInfo->expects ( $this->any () )->method ( 'getAdjustments' )->will ( $this->returnValue ( [ ] ) );
		
		$saleableItemMock->expects ( $this->any () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $priceInfo ) );
	...
	protected function prepareLocaleDate($isValidInterval) {
		...
		$localeDate->expects ( $this->any () )->method ( 'isScopeDateInInterval' )->will ( $this->returnValue ( $isValidInterval ) );
