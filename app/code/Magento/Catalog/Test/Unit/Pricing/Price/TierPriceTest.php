...
class TierPriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->session->expects ( $this->any () )->method ( 'getCustomerGroupId' )->will ( $this->returnValue ( $this->customerGroup ) );
		
		$this->calculator = $this->getMock ( 'Magento\Framework\Pricing\Adjustment\Calculator', [ ], [ ], '', false );
		$this->groupManagement = $this->getMock ( 'Magento\Customer\Api\GroupManagementInterface', [ ], [ ], '', false );
		
		$this->priceCurrencyMock = $this->getMock ( '\Magento\Framework\Pricing\PriceCurrencyInterface' );
		
		$this->model = new TierPrice ( $this->product, $this->quantity, $this->calculator, $this->priceCurrencyMock, $this->session, $this->groupManagement );
	}
	...
	public function testGetSavePercent($basePrice, $tierPrice, $savedPercent) {
		...
		$priceAmount->expects ( $this->once () )->method ( 'getBaseAmount' )->will ( $this->returnValue ( $basePrice ) );
		
		$price = $this->getMock ( 'Magento\Framework\Pricing\Price\PriceInterface' );
		$price->expects ( $this->any () )->method ( 'getAmount' )->will ( $this->returnValue ( $priceAmount ) );
		
		$this->priceInfo->expects ( $this->atLeastOnce () )->method ( 'getPrice' )->will ( $this->returnValue ( $price ) )->with ( RegularPrice::PRICE_CODE );
