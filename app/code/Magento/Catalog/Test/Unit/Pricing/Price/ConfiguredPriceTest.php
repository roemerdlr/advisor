...
class ConfiguredPriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->item = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Configuration\Item\ItemInterface' )->getMock ();
	...
	protected function createProductOptionStub($optionId) {
		...
		$option->expects ( $this->atLeastOnce () )->method ( 'groupFactory' )->will ( $this->returnValue ( $this->createOptionTypeStub ( $option ) ) );
		return $option;
	}
	...
	protected function createOptionTypeStub(\Magento\Catalog\Model\Product\Option $option) {
		$optionType = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Option\Type\DefaultType' )->setMethods ( [ 
				'setOption',
		...
		$optionType->expects ( $this->atLeastOnce () )->method ( 'getOptionPrice' )->with ( $this->anything (), $this->basePriceValue )->will ( $this->returnValue ( 10. ) );
		return $optionType;
	}
}
