...
class RenderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->object = $objectManager->getObject ( 'Magento\Catalog\Pricing\Render', [ 
				'context' => $context,
	...
	public function testToHtmlProductFromRegistry() {
		...
		$this->layout->expects ( $this->any () )->method ( 'getBlock' )->will ( $this->returnValue ( $this->pricingRenderBlock ) );
		
		$this->registry->expects ( $this->once () )->method ( 'registry' )->with ( $this->equalTo ( 'product' ) )->will ( $this->returnValue ( $product ) );
		
		$arguments = $this->object->getData ();
		$arguments ['render_block'] = $this->object;
		$this->pricingRenderBlock->expects ( $this->any () )->method ( 'render' )->with ( $this->equalTo ( 'test_price_type_code' ), $this->equalTo ( $product ), $this->equalTo ( $arguments ) )->will ( $this->returnValue ( $expectedValue ) );
	...
	public function testToHtmlProductFromParentBlock() {
		...
		$block = $this->getMock ( 'Magento\Framework\Pricing\Render', [ 
				'getProductItem',
				'render' 
		], [ ], '', false );
