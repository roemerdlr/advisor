...
class Grouped extends \Magento\Catalog\Model\Product\Type\AbstractType {
	...
	public function getChildrenIds($parentId, $required = true) {
		return $this->productLinks->getChildrenIds ( $parentId, \Magento\GroupedProduct\Model\ResourceModel\Product\Link::LINK_TYPE_GROUPED );
	}
	...
	public function getAssociatedProducts($product) {
		if (! $product->hasData ( $this->_keyAssociatedProducts )) {
			...
			$collection = $this->getAssociatedProductCollection ( $product )->addAttributeToSelect ( '*' )->addFilterByRequiredOptions ()->setPositionOrder ()->addStoreFilter ( $this->getStoreFilter ( $product ) )->addAttributeToFilter ( 'status', [ 
	...
	public function getAssociatedProductCollection($product) {
		...
		$collection = $links->getProductCollection ()->setFlag ( 'require_stock_items', true )->setFlag ( 'product_children', true )->setIsStrongMode ();
	...
	protected function _prepareProduct(\Magento\Framework\DataObject $buyRequest, $product, $processMode) {
		...
		$associatedProducts = ! $isStrictProcessMode || ! empty ( $productsInfo ) ? $this->getAssociatedProducts ( $product ) : false;
		...
		foreach ( $associatedProducts as $subProduct ) {
			...
			if ($isStrictProcessMode) {
				...
				$_result [0]->addCustomOption ( 'info_buyRequest', serialize ( [ 
