...
class GroupedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->block = $objectHelper->getObject ( 'Magento\GroupedProduct\Block\Adminhtml\Product\Composite\Fieldset\Grouped', [ 
	...
	public function testGetProductPositive() {
		$instanceMock = $this->getMock ( 'Magento\GroupedProduct\Model\Product\Type\Grouped', [ ], [ ], '', false );
	...
	public function testGetProductNegative() {
		...
		$instanceMock->expects ( $this->once () )->method ( 'getStoreFilter' )->with ( $this->productMock )->will ( $this->returnValue ( null ) );
	...
	public function testGetAssociatedProducts() {
		...
		$instanceMock->expects ( $this->once () )->method ( 'getAssociatedProducts' )->with ( $this->productMock )->will ( $this->returnValue ( [ 
				$associatedProduct 
		] ) );
		...
		$this->assertEquals ( [ 
				$associatedProduct 
		], $this->block->getAssociatedProducts () );
	}
	...
	public function testSetPreconfiguredValue() {
		...
		$this->productMock->expects ( $this->once () )->method ( 'getPreconfiguredValues' )->will ( $this->returnValue ( $objectMock ) );
	...
	public function testGetCurrencyPrice() {
		...
		$this->pricingHelperMock->expects ( $this->once () )->method ( 'currencyByStore' )->with ( $price, $storeId, false )->will ( $this->returnValue ( $expectedPrice ) );
