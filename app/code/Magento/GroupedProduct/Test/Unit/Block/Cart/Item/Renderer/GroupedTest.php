...
class GroupedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_renderer = $objectManagerHelper->getObject ( 'Magento\GroupedProduct\Block\Cart\Item\Renderer\Grouped', [ 
	...
	public function testGetProductForThumbnailConfigUseParent() {
		...
		$productForThumbnail = $this->_renderer->getProductForThumbnail ();
		$this->assertSame ( $products ['parentProduct'], $productForThumbnail, 'Parent product was expected to be returned ' . 'if "checkout/cart/grouped_product_image" is set to "parent" in system config.' );
	}
	...
	protected function _initProducts($childHasThumbnail = true, $useParentThumbnail = false) {
		...
		$item->expects ( $this->any () )->method ( 'getOptionByCode' )->with ( 'product_type' )->will ( $this->returnValue ( $itemOption ) );
