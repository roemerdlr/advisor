...
class ListAssociatedProductsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->contextMock->expects ( $this->any () )->method ( 'getStoreManager' )->will ( $this->returnValue ( $this->storeManagerMock ) );
	...
	public function testGetAssociatedProducts() {
		...
		$this->typeInstanceMock->expects ( $this->once () )->method ( 'getAssociatedProducts' )->with ( $this->productMock )->will ( $this->returnValue ( [ 
				$this->generateAssociatedProduct ( 1 ),
				$this->generateAssociatedProduct ( 2 ) 
		] ) );
	...
	protected function generateAssociatedProduct($productKey = 0) {
		$associatedProduct = $this->getMock ( 'Magento\Framework\DataObject', [ 
				'getQty',
				'getPosition',
				'getId',
				'getSku',
