...
class GroupedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->configuredValueMock = $this->getMock ( 'Magento\Framework\DataObject', [ 
				'getSuperGroup' 
		], [ ], '', false );
		$layout = $this->getMock ( 'Magento\Framework\View\LayoutInterface' );
		$this->groupedView = $helper->getObject ( 'Magento\GroupedProduct\Block\Product\View\Type\Grouped', [ 
				'data' => [ 
	...
	public function testSetPreconfiguredValue($id) {
		...
		$associatedProduct = [ 
				'key' => $this->productMock 
		];
		$this->configuredValueMock->expects ( $this->once () )->method ( 'getSuperGroup' )->will ( $this->returnValue ( $configValue ) );
		$this->productMock->expects ( $this->once () )->method ( 'getPreconfiguredValues' )->will ( $this->returnValue ( $this->configuredValueMock ) );
		
		$this->typeInstanceMock->expects ( $this->once () )->method ( 'getAssociatedProducts' )->with ( $this->productMock )->will ( $this->returnValue ( $associatedProduct ) );
	...
	public function setPreconfiguredValueDataProvider() {
		return [ 
				'item_id_exist_in_config' => [ 
						'id_one' 
				],
				'item_id_not_exist_in_config' => [ 
						'id_two' 
				] 
		];
	}
