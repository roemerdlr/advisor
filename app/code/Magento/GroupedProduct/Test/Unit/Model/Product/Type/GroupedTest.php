...
class GroupedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->catalogProductLink = $this->getMock ( '\Magento\GroupedProduct\Model\ResourceModel\Product\Link', [ ], [ ], '', false );
		$this->productStatusMock = $this->getMock ( '\Magento\Catalog\Model\Product\Attribute\Source\Status', [ ], [ ], '', false );
		$this->_model = $this->objectHelper->getObject ( 'Magento\GroupedProduct\Model\Product\Type\Grouped', [ 
	...
	public function testGetChildrenIds() {
		...
		$this->catalogProductLink->expects ( $this->once () )->method ( 'getChildrenIds' )->with ( $parentId, \Magento\GroupedProduct\Model\ResourceModel\Product\Link::LINK_TYPE_GROUPED )->will ( $this->returnValue ( $childrenIds ) );
	...
	public function testGetParentIdsByChild() {
		...
		$this->catalogProductLink->expects ( $this->once () )->method ( 'getParentIdsByChild' )->with ( $childId, \Magento\GroupedProduct\Model\ResourceModel\Product\Link::LINK_TYPE_GROUPED )->will ( $this->returnValue ( $parentIds ) );
	...
	public function testSetSaleableStatus() {
		...
		$this->productStatusMock->expects ( $this->once () )->method ( 'getSaleableStatusIds' )->will ( $this->returnValue ( $saleableIds ) );
	...
	public function testGetAssociatedProductIdsNonCached() {
		...
		$model = $this->getMock ( '\Magento\GroupedProduct\Model\Product\Type\Grouped', [ 
				'getAssociatedProducts' 
		], $args );
		
		$associatedProduct = $this->getMock ( 'Magento\Catalog\Model\Product', [ ], [ ], '', false );
		$model->expects ( $this->once () )->method ( 'getAssociatedProducts' )->with ( $this->product )->will ( $this->returnValue ( [ 
				$associatedProduct 
		] ) );
	...
	public function testGetAssociatedProductCollection() {
		...
		$link->expects ( $this->any () )->method ( 'setLinkTypeId' )->with ( \Magento\GroupedProduct\Model\ResourceModel\Product\Link::LINK_TYPE_GROUPED );
		$collection = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Link\Product\Collection', [ 
				'setFlag',
				'setIsStrongMode',
				'setProduct' 
		], [ ], '', false );
	...
	public function testProcessBuyRequest($superGroup, $result) {
		$buyRequest = $this->getMock ( '\Magento\Framework\DataObject', [ 
				'getSuperGroup' 
		], [ ], '', false );
	...
	public function testPrepareForCartAdvancedEmpty() {
		...
		$productCollection = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Link\Product\Collection', [ ], [ ], '', false );
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'setFlag' )->willReturnSelf ();
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'setIsStrongMode' )->willReturnSelf ();
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'setProduct' );
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'addAttributeToSelect' )->willReturnSelf ();
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'addFilterByRequiredOptions' )->willReturnSelf ();
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'setPositionOrder' )->willReturnSelf ();
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'addStoreFilter' )->willReturnSelf ();
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'addAttributeToFilter' )->willReturnSelf ();
		$items = [ 
				$this->getMock ( 'Magento\Catalog\Model\Product', [ ], [ ], '', false ),
				$this->getMock ( 'Magento\Catalog\Model\Product', [ ], [ ], '', false ) 
		];
		$productCollection->expects ( $this->atLeastOnce () )->method ( 'getIterator' )->willReturn ( new \ArrayIterator ( $items ) );
		
		$link = $this->getMock ( 'Magento\Catalog\Model\Product\Link', [ ], [ ], '', false );
		$link->expects ( $this->any () )->method ( 'setLinkTypeId' );
		$link->expects ( $this->atLeastOnce () )->method ( 'getProductCollection' )->willReturn ( $productCollection );
		
		$this->product->expects ( $this->atLeastOnce () )->method ( 'getLinkInstance' )->willReturn ( $link );
		
		$this->product->expects ( $this->any () )->method ( 'getData' )->willReturn ( $items );
	...
	public function testPrepareForCartAdvancedWithProductsStrictFalseStringResult() {
		...
		$typeMock = $this->getMock ( 'Magento\Catalog\Model\Product\Type\AbstractType', [ 
				'_prepareProduct',
		...
		$cached = true;
		$this->product->expects ( $this->atLeastOnce () )->method ( 'hasData' )->will ( $this->returnValue ( $cached ) );
		$this->product->expects ( $this->atLeastOnce () )->method ( 'getData' )->will ( $this->returnValue ( [ 
				$associatedProduct 
		] ) );
		
		$this->assertEquals ( $associatedPrepareResult, $this->_model->prepareForCartAdvanced ( $buyRequest, $this->product, Grouped::PROCESS_MODE_LITE ) );
	}
	public function testPrepareForCartAdvancedWithProductsStrictFalseEmptyArrayResult() {
		...
		$this->assertEquals ( $expectedMsg, $this->_model->prepareForCartAdvanced ( $buyRequest, $this->product, Grouped::PROCESS_MODE_LITE ) );
	}
	public function testPrepareForCartAdvancedWithProductsStrictFalse() {
		...
		$associatedPrepareResult = [ 
				$this->getMock ( 'Magento\Catalog\Model\Product', [ ], [ ], 'resultProduct', false ) 
		];
		$typeMock->expects ( $this->once () )->method ( '_prepareProduct' )->willReturn ( $associatedPrepareResult );
	...
	public function testPrepareForCartAdvancedWithProductsStrictTrue() {
		...
		$typeMock->expects ( $this->once () )->method ( '_prepareProduct' )->willReturn ( [ 
				$associatedPrepareResult 
		] );
