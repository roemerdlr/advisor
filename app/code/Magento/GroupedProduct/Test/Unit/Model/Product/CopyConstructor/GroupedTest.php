...
class GroupedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_productMock = $this->getMock ( '\Magento\Catalog\Model\Product', [ 
				'getTypeId',
				'__wakeup',
				'getLinkInstance' 
		], [ ], '', false );
		
		$this->_duplicateMock = $this->getMock ( '\Magento\Catalog\Model\Product', [ 
				'setGroupedLinkData',
				'__wakeup' 
		], [ ], '', false );
		
		$this->_linkMock = $this->getMock ( '\Magento\Catalog\Model\Product\Link', [ 
				'setLinkTypeId',
				'__wakeup',
				'getAttributes',
				'getLinkCollection' 
		...
		$this->_productMock->expects ( $this->any () )->method ( 'getLinkInstance' )->will ( $this->returnValue ( $this->_linkMock ) );
	}
	...
	public function testBuild() {
		...
		$productLinkMock = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\Link', [ 
				'__wakeup',
				'getLinkedProductId',
				'toArray' 
		], [ ], '', false );
		$this->_linkMock->expects ( $this->atLeastOnce () )->method ( 'setLinkTypeId' )->with ( \Magento\GroupedProduct\Model\ResourceModel\Product\Link::LINK_TYPE_GROUPED );
		
		$productLinkMock->expects ( $this->once () )->method ( 'getLinkedProductId' )->will ( $this->returnValue ( '100500' ) );
		$productLinkMock->expects ( $this->once () )->method ( 'toArray' )->with ( [ 
				'one',
				'two' 
		...
		$collectionMock = $helper->getCollectionMock ( '\Magento\Catalog\Model\ResourceModel\Product\Link\Collection', [ 
				$productLinkMock 
		] );
