...
class PriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->finalPriceModel = $helper->getObject ( 'Magento\GroupedProduct\Model\Product\Type\Grouped\Price', [ ] );
	}
	...
	public function testGetFinalPriceIfQtyIsNullAndFinalPriceExist() {
		...
		$this->productMock->expects ( $this->any () )->method ( 'getCalculatedFinalPrice' )->will ( $this->returnValue ( $finalPrice ) );
	 ...
	 * @param
	 *        	$expectedPriceCall
	 * @param $expectedFinalPrice @dataProvider
	 *        	getFinalPriceDataProvider
	 *        	@covers \Magento\GroupedProduct\Model\Product\Type\Grouped\Price::getFinalPrice
	 */
	public function testGetFinalPrice(array $associatedProducts, array $options, $expectedPriceCall, $expectedFinalPrice) {
		$rawFinalPrice = 10;
		$rawPriceCheckStep = 6;
		
		$this->productMock->expects ( $this->any () )->method ( 'getCalculatedFinalPrice' )->will ( $this->returnValue ( $rawFinalPrice ) );
		
		// mock for parent::getFinal price call
		$this->productMock->expects ( $this->any () )->method ( 'getPrice' )->will ( $this->returnValue ( $rawFinalPrice ) );
		
		$this->productMock->expects ( $this->at ( $rawPriceCheckStep ) )->method ( 'setFinalPrice' )->with ( $rawFinalPrice )->will ( $this->returnValue ( $this->productMock ) );
		
		$this->productMock->expects ( $this->at ( $expectedPriceCall ) )->method ( 'setFinalPrice' )->with ( $expectedFinalPrice );
		
		$this->productMock->expects ( $this->any () )->method ( 'getData' )->with ( 'final_price' )->will ( $this->returnValue ( $rawFinalPrice ) );
		
		// test method
		$this->productMock->expects ( $this->once () )->method ( 'hasCustomOptions' )->will ( $this->returnValue ( true ) );
		
		$productTypeMock = $this->getMock ( 'Magento\GroupedProduct\Model\Product\Type\Grouped', [ ], [ ], '', false );
		
		$this->productMock->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $productTypeMock ) );
		
		$this->productMock->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( 'store1' ) );
		
		$productTypeMock->expects ( $this->once () )->method ( 'setStoreFilter' )->with ( 'store1', $this->productMock )->will ( $this->returnValue ( $productTypeMock ) );
		
		$productTypeMock->expects ( $this->once () )->method ( 'getAssociatedProducts' )->with ( $this->productMock )->will ( $this->returnValue ( $associatedProducts ) );
	...
	public function getFinalPriceDataProvider() {
		$optionMock = $this->getMock ( 'Magento\Catalog\Model\Product\Option', [ 
				'getValue',
				'__wakeup' 
		], [ ], '', false );
		/* quantity of options */
		...
		return [ 
				...
				'custom_option_exist' => [ 
						...
						'options' => [ 
								[ 
										'associated_product_1',
										false 
								],
								[ 
										'associated_product_2',
										$optionMock 
								],
								[ 
										'associated_product_3',
										$optionMock 
								] 
						],
	...
	protected function generateAssociatedProducts() {
		$childProductMock = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getId',
				'getFinalPrice',
				'__wakeup' 
		], [ ], '', false );
