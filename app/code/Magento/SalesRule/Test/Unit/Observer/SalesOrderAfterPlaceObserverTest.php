...
class SalesOrderAfterPlaceObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $helper->getObject ( 'Magento\SalesRule\Observer\SalesOrderAfterPlaceObserver', [ 
	...
	protected function initMocks() {
		$this->couponMock = $this->getMock ( '\Magento\SalesRule\Model\Coupon', [ 
				'__wakeup',
				'save',
				'load',
				'getId',
		...
		$this->ruleCustomerFactory = $this->getMock ( 'Magento\SalesRule\Model\Rule\CustomerFactory', [ 
				'create' 
		], [ ], '', false );
	...
	protected function initOrderFromEvent($observer) {
		...
		$event->expects ( $this->any () )->method ( 'getOrder' )->will ( $this->returnValue ( $order ) );
	...
	public function testSalesOrderAfterPlace($ruleCustomerId) {
		...
		$ruleCustomer = $this->getMock ( 'Magento\SalesRule\Model\Rule\Customer', [ 
				'setCustomerId',
				'loadByCustomerRule',
				'getId',
				'setTimesUsed',
	...
	public function salesOrderAfterPlaceDataProvider() {
		return [ 
				'With customer rule id' => [ 
						1 
				],
				'Without customer rule id' => [ 
						null 
				] 
		];
	}
