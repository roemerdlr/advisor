...
class CouponRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->searchResultFactory = $this->getMock ( '\Magento\SalesRule\Api\Data\CouponSearchResultInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->searchResultsMock = $this->getMock ( '\Magento\SalesRule\Api\Data\CouponSearchResultInterface', [ ], [ ], '', false );
		$this->couponFactory = $this->getMock ( '\Magento\SalesRule\Model\CouponFactory', [ 
				'create' 
		], [ ], '', false );
		$this->ruleFactory = $this->getMock ( '\Magento\SalesRule\Model\RuleFactory', [ 
				'create' 
		], [ ], '', false );
		$this->collectionFactory = $this->getMock ( '\Magento\SalesRule\Model\ResourceModel\Coupon\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = $this->objectManager->getObject ( 'Magento\SalesRule\Model\CouponRepository', [ 
	...
	public function testSave() {
		$id = 1;
		$coupon = $this->getMock ( '\Magento\SalesRule\Model\Coupon', [ 
				'load',
				'getCouponId',
				'getById' 
		], [ ], '', false );
	...
	public function testSaveWithExceptions($exceptionObject, $exceptionName, $exceptionMessage, $id) {
		...
		if ($id) {
			$this->resource->expects ( $this->once () )->method ( 'save' )->with ( $coupon )->willThrowException ( $exceptionObject );
		}
	...
	public function testGetList() {
		...
		$this->extensionAttributesJoinProcessorMock->expects ( $this->once () )->method ( 'process' )->with ( $collectionMock, 'Magento\SalesRule\Api\Data\CouponInterface' );
		...
		$filterMock->expects ( $this->once () )->method ( 'getField' )->willReturn ( 'coupon_id' );
