...
class ValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->rulesApplier = $this->getMock ( 'Magento\SalesRule\Model\RulesApplier', [ 
				'setAppliedRuleIds',
				'applyRules',
		...
		$this->item = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ 
				'__wakeup' 
		], [ ], '', false );
		...
		$this->validators = $this->getMock ( 'Magento\SalesRule\Model\Validator\Pool', [ 
				'getValidators' 
		], [ ], '', false );
		$this->messageManager = $this->getMock ( '\Magento\Framework\Message\Manager', [ ], [ ], '', false );
		$this->ruleCollection = $this->getMockBuilder ( 'Magento\SalesRule\Model\ResourceModel\Rule\Collection' )->disableOriginalConstructor ()->getMock ();
		...
		$this->model = $this->helper->getObject ( 'Magento\SalesRule\Model\Validator', [ 
				'context' => $context,
				'registry' => $registry,
	...
	protected function getQuoteItemMock() {
		...
		$itemDownloadable = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ 
				'getAddress',
				'__wakeup' 
		], [ ], '', false );
		$itemDownloadable->expects ( $this->any () )->method ( 'getAddress' )->will ( $this->returnValue ( new \stdClass () ) );
		
		$itemSimple = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ 
				'getAddress',
				'__wakeup' 
		], [ ], '', false );
	...
	public function testCanApplyRules() {
		...
		$actionsCollection = $this->getMock ( 'Magento\Rule\Model\Action\Collection', [ 
				'validate' 
		], [ ], '', false );
	...
	public function testInit() {
		$this->assertInstanceOf ( 'Magento\SalesRule\Model\Validator', $this->model->init ( $this->model->getWebsiteId (), $this->model->getCustomerGroupId (), $this->model->getCouponCode () ) );
	}
	public function testCanApplyDiscount() {
		$validator = $this->getMockBuilder ( 'Magento\Framework\Validator\AbstractValidator' )->setMethods ( [ 
				'isValid' 
		] )->disableOriginalConstructor ()->getMockForAbstractClass ();
		
		$this->validators->expects ( $this->any () )->method ( 'getValidators' )->with ( 'discount' )->willReturn ( [ 
				$validator 
		] );
		$validator->expects ( $this->any () )->method ( 'isValid' )->with ( $this->item )->willReturn ( false );
	...
	public function testInitTotalsCanApplyDiscount() {
		...
		$rule = $this->getMock ( 'Magento\SalesRule\Model\Rule', [ 
				'getSimpleAction',
				'getActions',
				'getId' 
		], [ ], '', false );
		$item1 = $this->getMockForAbstractClass ( 'Magento\Quote\Model\Quote\Item\AbstractItem', [ ], '', false, true, true, [ 
				'__clone',
		...
		$rule->expects ( $this->any () )->method ( 'getSimpleAction' )->willReturn ( \Magento\SalesRule\Model\Rule::CART_FIXED_ACTION );
		$iterator = new \ArrayIterator ( [ 
				$rule 
		] );
		...
		$this->validators->expects ( $this->atLeastOnce () )->method ( 'getValidators' )->with ( 'discount' )->willReturn ( [ 
				$validator 
		] );
	...
	protected function prepareRuleCollectionMock($ruleCollection) {
		$this->ruleCollection->expects ( $this->once () )->method ( 'addFieldToFilter' )->with ( 'is_active', 1 )->will ( $this->returnSelf () );
		$this->ruleCollection->expects ( $this->once () )->method ( 'load' )->will ( $this->returnSelf () );
		
		$ruleCollectionFactoryMock = $this->getMockBuilder ( 'Magento\SalesRule\Model\ResourceModel\Rule\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
	...
	public function testProcessShippingAmountNoRules() {
		...
		$this->ruleCollection->expects ( $this->any () )->method ( 'getIterator' )->willReturn ( $iterator );
		$this->model->init ( $this->model->getWebsiteId (), $this->model->getCustomerGroupId (), $this->model->getCouponCode () );
		$this->assertInstanceOf ( 'Magento\SalesRule\Model\Validator', $this->model->processShippingAmount ( $this->getAddressMock () ) );
	}
	public function testProcessShippingAmountProcessDisabled() {
		$ruleMock = $this->getMockBuilder ( 'Magento\SalesRule\Model\Rule' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$iterator = new \ArrayIterator ( [ 
				$ruleMock 
		] );
		$this->ruleCollection->expects ( $this->any () )->method ( 'getIterator' )->willReturn ( $iterator );
	...
	public function testProcessShippingAmountActions($action) {
		...
		$ruleMock = $this->getMockBuilder ( 'Magento\SalesRule\Model\Rule' )->disableOriginalConstructor ()->setMethods ( [ 
				'getApplyToShipping',
				'getSimpleAction',
				'getDiscountAmount' 
		] )->getMock ();
	...
	protected function getAddressMock($shippingAmount = null) {
		...
		$addressMock = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Address' )->disableOriginalConstructor ()->setMethods ( [ 
				'getShippingAmountForDiscount',
				'getQuote',
				'getCustomAttributesCodes' 
		] )->getMock ();
		$addressMock->expects ( $this->any () )->method ( 'getShippingAmountForDiscount' )->willReturn ( $shippingAmount );
