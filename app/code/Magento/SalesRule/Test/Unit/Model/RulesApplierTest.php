...
class RulesApplierTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->calculatorFactory = $this->getMock ( 'Magento\SalesRule\Model\Rule\Action\Discount\CalculatorFactory', [ ], [ ], '', false );
		$this->eventManager = $this->getMock ( 'Magento\Framework\Event\Manager', [ 
				'dispatch' 
		], [ ], '', false );
		$this->validatorUtility = $this->getMock ( 'Magento\SalesRule\Model\Utility', [ 
				'canProcessRule',
				'minFix',
				'deltaRoundingFix',
				'getItemQty' 
		...
		$this->rulesApplier = new \Magento\SalesRule\Model\RulesApplier ( $this->calculatorFactory, $this->eventManager, $this->validatorUtility );
	}
	...
	public function testApplyRulesWhenRuleWithStopRulesProcessingIsUsed($isChildren, $isContinue) {
		 ...
		 */
		$ruleWithStopFurtherProcessing = $this->getMock ( 'Magento\SalesRule\Model\Rule', [ 
				'getStoreLabel',
				'getCouponType',
				'getRuleId',
				'__wakeup',
		...
		$ruleThatShouldNotBeRun = $this->getMock ( 'Magento\SalesRule\Model\Rule', [ 
				'getStopRulesProcessing',
				'__wakeup' 
		], [ ], '', false );
		
		$actionMock = $this->getMock ( 'Magento\Rule\Model\Action\Collection', [ 
				'validate' 
		], [ ], '', false );
		...
		if ($isChildren) {
			$item->expects ( $this->atLeastOnce () )->method ( 'getChildren' )->willReturn ( [ 
					$item 
			] );
	...
	protected function applyRule($item, $rule) {
		$qty = 2;
		$discountCalc = $this->getMock ( 'Magento\SalesRule\Model\Rule\Action\Discount', [ 
				'fixQuantity',
				'calculate' 
		], [ ], '', false );
		$discountData = $this->getMock ( 'Magento\SalesRule\Model\Rule\Action\Discount\Data', [ ], [ 
				'amount' => 30,
				'baseAmount' => 30,
				'originalAmount' => 30,
