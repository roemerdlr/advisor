...
class CouponManagementServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->ruleFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		
		$className = '\Magento\SalesRule\Model\ResourceModel\Coupon\CollectionFactory';
		$this->collectionFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		...
		$this->couponMassDeleteResultFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		$this->couponMassDeleteResultFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->couponMassDeleteResult );
		
		$this->model = $this->objectManager->getObject ( 'Magento\SalesRule\Model\Service\CouponManagementService', [ 
	...
	public function testGenerate() {
		 ...
		 */
		$couponSpec = $this->getMock ( $className, [ 
				'getRuleId',
				'getQuantity',
				'getFormat',
				'getLength',
	...
	public function testGenerateValidationException() {
		...
		$this->couponGenerator->expects ( $this->once () )->method ( 'validateData' )->willThrowException ( new \Magento\Framework\Exception\InputException () );
	...
	public function testGenerateLocalizedException() {
		...
		$rule->expects ( $this->any () )->method ( 'load' )->willReturnSelf ();
		$rule->expects ( $this->any () )->method ( 'getRuleId' )->willReturn ( 1 );
		$rule->expects ( $this->once () )->method ( 'getUseAutoGeneration' )->willThrowException ( new \Magento\Framework\Exception\LocalizedException ( __ ( 'Error occurred when generating coupons: %1', '1' ) ) );
		$this->ruleFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $rule );
	...
	public function testDeleteByIdsIgnore() {
		...
		$couponCollection->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$coupon,
				$coupon,
				$coupon 
		] );
