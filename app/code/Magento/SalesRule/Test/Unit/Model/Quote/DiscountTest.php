...
class DiscountTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->validatorMock = $this->getMockBuilder ( 'Magento\SalesRule\Model\Validator' )->disableOriginalConstructor ()->setMethods ( [ 
				'canApplyRules',
				'reset',
				'init',
				'initTotals',
		...
		$priceCurrencyMock->expects ( $this->any () )->method ( 'round' )->will ( $this->returnCallback ( function ($argument) {
			return round ( $argument, 2 );
		} ) );
		
		$this->addressMock = $this->getMock ( '\Magento\Quote\Model\Quote\Address', [ 
				'getQuote',
				'getAllItems',
				'getShippingAmount',
				'__wakeup',
		...
		$this->discount = $this->objectManager->getObject ( 'Magento\SalesRule\Model\Quote\Discount', [ 
	...
	public function testCollectItemNoDiscount() {
		$itemNoDiscount = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ 
				'getNoDiscount',
				'__wakeup' 
		], [ ], '', false );
		...
		$this->shippingAssignmentMock->expects ( $this->any () )->method ( 'getItems' )->willReturn ( [ 
				$itemNoDiscount 
		] );
		...
		$this->assertInstanceOf ( 'Magento\SalesRule\Model\Quote\Discount', $this->discount->collect ( $quoteMock, $this->shippingAssignmentMock, $totalMock ) );
	}
	public function testCollectItemHasParent() {
		$itemWithParentId = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ 
				'getNoDiscount',
				'getParentItem',
				'__wakeup' 
		], [ ], '', false );
	...
	public function testCollectItemHasChildren($childItemData, $parentData, $expectedChildData) {
		...
		$itemWithChildren = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item' )->disableOriginalConstructor ()->setMethods ( [ 
				'getNoDiscount',
				'getParentItem',
				'getHasChildren',
