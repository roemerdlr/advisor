...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->entityFactory = $this->getMock ( 'Magento\Framework\Data\Collection\EntityFactory', [ ], [ ], '', false );
		
		$this->loggerMock = $this->getMock ( 'Psr\Log\LoggerInterface' );
		
		$this->fetchStrategy = $this->getMock ( 'Magento\Framework\Data\Collection\Db\FetchStrategyInterface', [ ], [ ], '', false );
		
		$this->eventManager = $this->getMock ( 'Magento\Framework\Event\ManagerInterface', [ ], [ ], '', false );
		
		$this->reportResource = $this->getMock ( 'Magento\Sales\Model\ResourceModel\Report', [ 
				'getConnection',
				'getMainTable' 
		], [ ], '', false );
		...
		$this->connection->expects ( $this->any () )->method ( 'select' )->will ( $this->returnValue ( $this->selectMock ) );
		
		$this->reportResource->expects ( $this->any () )->method ( 'getConnection' )->will ( $this->returnValue ( $this->connection ) );
		$this->reportResource->expects ( $this->any () )->method ( 'getMainTable' )->will ( $this->returnValue ( 'test_main_table' ) );
		
		$this->ruleFactory = $this->getMock ( 'Magento\SalesRule\Model\ResourceModel\Report\RuleFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->object = new \Magento\SalesRule\Model\ResourceModel\Report\Collection ( $this->entityFactory, $this->loggerMock, $this->fetchStrategy, $this->eventManager, $this->reportResource, $this->ruleFactory );
	}
	...
	public function testApplyAggregatedTableNegativeIsTotals() {
		$this->selectMock->expects ( $this->once () )->method ( 'group' )->with ( $this->equalTo ( [ 
				null,
				'coupon_code' 
		] ) );
	...
	public function testApplyAggregatedTableIsSubTotals() {
		$this->selectMock->expects ( $this->once () )->method ( 'group' )->with ( $this->equalTo ( null ) );
	...
	public function testApplyRulesFilterWithRulesList() {
		...
		$this->selectMock->expects ( $this->at ( 3 ) )->method ( 'where' )->with ( $this->equalTo ( implode ( [ 
				'test_1',
				'test_2' 
		], ' OR ' ) ) );
	...
	protected function getRuleMock() {
		return $this->getMock ( 'Magento\SalesRule\Model\ResourceModel\Report\Rule', [ 
