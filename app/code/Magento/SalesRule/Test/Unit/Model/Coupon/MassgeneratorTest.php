...
class MassgeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerateCode(array $data, $length) {
		$salesRuleCouponMock = $this->getMock ( 'Magento\SalesRule\Helper\Coupon', [ 
				'getCharset' 
		], [ ], '', false );
		...
		$salesRuleCouponMock->expects ( $this->once () )->method ( 'getCharset' )->with ( $data ['format'] )->will ( $this->returnValue ( $this->charset ) );
	...
	public function testGetDelimiter(array $data) {
		...
		if (empty ( $data ['delimiter'] )) {
			$salesRuleCouponMock->expects ( $this->once () )->method ( 'getCodeSeparator' )->will ( $this->returnValue ( 'test-separator' ) );
	...
	public function testGeneratePool() {
		...
		$resourceMock = $this->getMock ( 'Magento\SalesRule\Model\ResourceModel\Coupon', [ 
				'exists',
				'__wakeup',
				'getIdFieldName' 
		], [ ], '', false );
		$dateMock = $this->getMock ( 'Magento\Framework\Stdlib\DateTime\DateTime', [ 
				'gmtTimestamp' 
		], [ ], '', false );
		$dateTimeMock = $this->getMock ( 'Magento\Framework\Stdlib\DateTime', [ 
				'formatDate' 
		], [ ], '', false );
		$couponFactoryMock = $this->getMock ( 'Magento\SalesRule\Model\CouponFactory', [ 
				'create' 
		], [ ], '', false );
		$couponMock = $this->getMock ( 'Magento\SalesRule\Model\Coupon', [ 
				'__wakeup',
				'setId',
				'setRuleId',
				'setUsageLimit',
		...
		$couponFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $couponMock ) );
		$salesRuleCouponMock->expects ( $this->any () )->method ( 'getCharset' )->with ( $data ['format'] )->will ( $this->returnValue ( $this->charset ) );
		/** @var \Magento\SalesRule\Model\Coupon\Massgenerator $massgenerator */
		$massgenerator = $this->objectManager->getObject ( 'Magento\SalesRule\Model\Coupon\Massgenerator', [ 
	 ...
	 * @expectedExceptionMessage We cannot create the requested Coupon Qty.
	...
	public function testGeneratePoolException() {
		...
		$couponFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $couponMock ) );
		$salesRuleCouponMock->expects ( $this->any () )->method ( 'getCharset' )->with ( $data ['format'] )->will ( $this->returnValue ( $this->charset ) );
		$resourceMock->expects ( $this->any () )->method ( 'exists' )->will ( $this->returnValue ( true ) );
