...
class UtilityTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->usageFactory = $this->getMock ( 'Magento\SalesRule\Model\ResourceModel\Coupon\UsageFactory', [ 
				'create' 
		], [ ], '', false );
		$this->couponFactory = $this->getMock ( 'Magento\SalesRule\Model\CouponFactory', [ 
				'create' 
		], [ ], '', false );
		$this->objectFactory = $this->getMock ( 'Magento\Framework\DataObjectFactory', [ 
				'create' 
		], [ ], '', false );
		$this->customerFactory = $this->getMock ( 'Magento\SalesRule\Model\Rule\CustomerFactory', [ 
				'create' 
		], [ ], '', false );
		$this->coupon = $this->getMock ( 'Magento\SalesRule\Model\Coupon', [ 
				'load',
				'getId',
				'getUsageLimit',
				'getTimesUsed',
		...
		$this->quote = $this->getMock ( 'Magento\Quote\Model\Quote', [ 
				'__wakeup',
				'getStore' 
		], [ ], '', false );
		$this->customer = $this->getMock ( 'Magento\SalesRule\Model\Rule\Customer', [ 
				'loadByCustomerRule',
				'__wakeup' 
		], [ ], '', false );
		$this->rule = $this->getMock ( 'Magento\SalesRule\Model\Rule', [ 
		...
		$this->address = $this->getMock ( 'Magento\Quote\Model\Quote\Address', [ 
				'isObjectNew',
				'getQuote',
				'setIsValidForAddress',
				'__wakeup',
		...
		$this->item = $this->getMock ( 'Magento\Quote\Model\Quote\Item\AbstractItem', [ 
		...
		$this->utility = new \Magento\SalesRule\Model\Utility ( $this->usageFactory, $this->couponFactory, $this->customerFactory, $this->objectFactory, $this->priceCurrency );
	}
	...
	public function testCanProcessRuleValidAddress() {
		$this->rule->expects ( $this->once () )->method ( 'hasIsValidForAddress' )->with ( $this->address )->will ( $this->returnValue ( true ) );
		$this->rule->expects ( $this->once () )->method ( 'getIsValidForAddress' )->with ( $this->address )->will ( $this->returnValue ( true ) );
		$this->address->expects ( $this->once () )->method ( 'isObjectNew' )->will ( $this->returnValue ( false ) );
	...
	public function testCanProcessRuleCouponUsageLimitFail() {
		...
		$this->address->expects ( $this->once () )->method ( 'getQuote' )->will ( $this->returnValue ( $this->quote ) );
		
		$this->coupon->expects ( $this->atLeastOnce () )->method ( 'getUsageLimit' )->will ( $this->returnValue ( $usageLimit ) );
		$this->coupon->expects ( $this->once () )->method ( 'getTimesUsed' )->will ( $this->returnValue ( $timesUsed ) );
		$this->coupon->expects ( $this->once () )->method ( 'load' )->with ( $couponCode, 'code' )->will ( $this->returnSelf () );
		$this->couponFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->coupon ) );
		$this->coupon->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( $couponId ) );
	...
	public function testCanProcessRuleCouponUsagePerCustomerFail() {
		...
		$this->address->expects ( $this->atLeastOnce () )->method ( 'getQuote' )->will ( $this->returnValue ( $this->quote ) );
		
		$this->coupon->expects ( $this->atLeastOnce () )->method ( 'getUsagePerCustomer' )->will ( $this->returnValue ( $usageLimit ) );
		$this->coupon->expects ( $this->once () )->method ( 'load' )->with ( $couponCode, 'code' )->will ( $this->returnSelf () );
		$this->coupon->expects ( $this->atLeastOnce () )->method ( 'getId' )->will ( $this->returnValue ( $couponId ) );
		$this->couponFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->coupon ) );
		
		$couponUsage = new \Magento\Framework\DataObject ();
		$this->objectFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $couponUsage ) );
		$couponUsageModel = $this->getMock ( '\Magento\SalesRule\Model\ResourceModel\Coupon\Usage', [ ], [ ], '', false );
		$couponUsage->setData ( [ 
				'coupon_id' => $couponId,
				'times_used' => $timesUsed 
		] );
		$this->usageFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $couponUsageModel ) );
		$this->assertFalse ( $this->utility->canProcessRule ( $this->rule, $this->address ) );
	}
	...
	public function testCanProcessRuleUsagePerCustomer() {
		...
		$this->customerFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->customer ) );
	...
	public function testGetItemPriceNull() {
		...
		$this->item->expects ( $this->once () )->method ( 'getDiscountCalculationPrice' )->will ( $this->returnValue ( $price ) );
		$this->item->expects ( $this->once () )->method ( 'getCalculationPrice' )->will ( $this->returnValue ( null ) );
		$this->assertEquals ( $price, $this->utility->getItemPrice ( $this->item ) );
	}
	...
	public function testGetBaseItemPriceCalculation() {
		$calcPrice = 5;
		$this->item->expects ( $this->once () )->method ( 'getDiscountCalculationPrice' )->will ( $this->returnValue ( null ) );
		$this->item->expects ( $this->any () )->method ( 'getBaseCalculationPrice' )->will ( $this->returnValue ( $calcPrice ) );
	...
	public function testGetItemQtyMin() {
		...
		$this->rule->expects ( $this->once () )->method ( 'getDiscountQty' )->will ( $this->returnValue ( $discountQty ) );
	...
	protected function getItemPrice() {
		...
		$this->item->expects ( $this->atLeastOnce () )->method ( 'getDiscountCalculationPrice' )->will ( $this->returnValue ( $price ) );
		$this->item->expects ( $this->once () )->method ( 'getCalculationPrice' )->will ( $this->returnValue ( $calcPrice ) );
		return $price;
	}
	...
	protected function getItemBasePrice() {
		$price = 4;
		$calcPrice = 5;
		$this->item->expects ( $this->atLeastOnce () )->method ( 'getDiscountCalculationPrice' )->will ( $this->returnValue ( $calcPrice ) );
		$this->item->expects ( $this->any () )->method ( 'getBaseDiscountCalculationPrice' )->will ( $this->returnValue ( $price ) );
		return $price;
	}
	public function testDeltaRoundignFix() {
		...
		$this->priceCurrency->expects ( $this->any () )->method ( 'round' )->will ( $this->returnValueMap ( [ 
				[ 
						$discountAmount,
						$roundedDiscount 
				],
				...
				[ 
						...
						$secondRoundedDiscount 
				], // ?
				[ 
						$baseDiscountAmount + $baseDelta,
						$secondRoundedBaseDiscount 
				] 
		] // ?
 ) );
