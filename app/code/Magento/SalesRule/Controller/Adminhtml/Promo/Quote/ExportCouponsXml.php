...
class ExportCouponsXml extends \Magento\SalesRule\Controller\Adminhtml\Promo\Quote {
	...
	public function execute() {
		...
		if ($rule->getId ()) {
			...
			$content = $this->_view->getLayout ()->createBlock ( 'Magento\SalesRule\Block\Adminhtml\Promo\Quote\Edit\Tab\Coupons\Grid' )->getExcelFile ( $fileName );
			return $this->_fileFactory->create ( $fileName, $content, DirectoryList::VAR_DIR );
		} else {
			$this->_redirect ( 'sales_rule/*/detail', [ 
					'_current' => true 
			] );
			return;
		}
