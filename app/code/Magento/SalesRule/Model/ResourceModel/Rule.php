...
class Rule extends \Magento\Rule\Model\ResourceModel\AbstractResource {
	...
	public function getCustomerUses($rule, $customerId) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'rule_customer' ), [ 
				'cnt' => 'count(*)' 
		] )->where ( 'rule_id = :rule_id' )->where ( 'customer_id = :customer_id' );
	...
	public function getStoreLabels($ruleId) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'salesrule_label' ), [ 
				'store_id',
				'label' 
	...
	public function getStoreLabel($ruleId, $storeId) {
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( 'salesrule_label' ), 'label' )->where ( 'rule_id = :rule_id' )->where ( 'store_id IN(0, :store_id)' )->order ( 'store_id DESC' );
	...
	public function getActiveAttributes() {
		...
		], new \Zend_Db_Expr ( 'DISTINCT ea.attribute_code' ) )->joinInner ( [ 
	...
	public function setActualProductAttributes($rule, $attributes) {
		$connection = $this->getConnection ();
		$connection->delete ( $this->getTable ( 'salesrule_product_attribute' ), [ 
				'rule_id=?' => $rule->getId () 
		] );
