...
class Downloadable extends \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType {
	 ...
	 * Pass VALID row data ONLY as argument.
	 *
	 * @param array $rowData        	
	 * @param int $rowNum        	
	...
	protected function isRowValidSample(array $rowData) {
		$result = false;
		if (isset ( $rowData [self::COL_DOWNLOADABLE_SAMPLES] ) && $rowData [self::COL_DOWNLOADABLE_SAMPLES] != '' && $this->sampleGroupTitle ( $rowData ) == '') {
			$this->_entityModel->addRowError ( self::ERROR_GROUP_TITLE_NOT_FOUND, $this->rowNum );
			$result = true;
		}
		if (isset ( $rowData [self::COL_DOWNLOADABLE_SAMPLES] ) && $rowData [self::COL_DOWNLOADABLE_SAMPLES] != '') {
	...
	protected function isRowValidLink(array $rowData) {
		$result = false;
		if (isset ( $rowData [self::COL_DOWNLOADABLE_LINKS] ) && $rowData [self::COL_DOWNLOADABLE_LINKS] != '' && $this->linksAdditionalAttributes ( $rowData, 'group_title', self::DEFAULT_GROUP_TITLE ) == '') {
			$this->_entityModel->addRowError ( self::ERROR_GROUP_TITLE_NOT_FOUND, $this->rowNum );
			$result = true;
		}
		if (isset ( $rowData [self::COL_DOWNLOADABLE_LINKS] ) && $rowData [self::COL_DOWNLOADABLE_LINKS] != '') {
	...
	protected function linksAdditionalAttributes(array $rowData, $attribute, $defaultValue) {
		...
		if (isset ( $rowData [self::COL_DOWNLOADABLE_LINKS] )) {
			$options = explode ( \Magento\CatalogImportExport\Model\Import\Product::PSEUDO_MULTI_LINE_SEPARATOR, $rowData [self::COL_DOWNLOADABLE_LINKS] );
			foreach ( $options as $option ) {
	...
	protected function sampleGroupTitle(array $rowData) {
		...
		if (isset ( $rowData [self::COL_DOWNLOADABLE_SAMPLES] )) {
			$options = explode ( \Magento\CatalogImportExport\Model\Import\Product::PSEUDO_MULTI_LINE_SEPARATOR, $rowData [self::COL_DOWNLOADABLE_SAMPLES] );
			foreach ( $options as $option ) {
	...
	protected function parseOptions(array $rowData, $entityId) {
		...
		if (isset ( $rowData [self::COL_DOWNLOADABLE_LINKS] )) {
			$this->cachedOptions ['link'] = array_merge ( $this->cachedOptions ['link'], $this->prepareLinkData ( $rowData [self::COL_DOWNLOADABLE_LINKS], $entityId ) );
		}
		if (isset ( $rowData [self::COL_DOWNLOADABLE_SAMPLES] )) {
			$this->cachedOptions ['sample'] = array_merge ( $this->prepareSampleData ( $rowData [self::COL_DOWNLOADABLE_SAMPLES], $entityId ), $this->cachedOptions ['sample'] );
		}
		return $this;
	}
	...
	protected function fillDataSample(array $base, array $options) {
		$result = [ ];
		$existingOptions = $this->connection->fetchAll ( $this->connection->select ()->from ( $this->_resource->getTableName ( 'downloadable_sample' ) )->where ( 'product_id in (?)', $this->productIds ) );
		foreach ( $options as $option ) {
			$isExist = false;
			foreach ( $existingOptions as $existingOption ) {
				if ($option ['sample_url'] == $existingOption ['sample_url'] && $option ['sample_file'] == $existingOption ['sample_file'] && $option ['sample_type'] == $existingOption ['sample_type'] && $option ['product_id'] == $existingOption ['product_id']) {
	...
	protected function fillDataLink(array $base, array $options) {
		$result = [ ];
		$existingOptions = $this->connection->fetchAll ( $this->connection->select ()->from ( $this->_resource->getTableName ( 'downloadable_link' ) )->where ( 'product_id in (?)', $this->productIds ) );
		foreach ( $options as $option ) {
	...
	protected function fillDataTitleLink(array $options) {
		$result = [ ];
		$existingOptions = $this->connection->fetchAll ( $this->connection->select ()->from ( [ 
	...
	protected function saveSampleOptions() {
		...
		$this->connection->insertOnDuplicate ( $this->_resource->getTableName ( 'downloadable_sample' ), $this->downloadableHelper->prepareDataForSave ( $this->dataSample, $dataSample ) );
		$titleSample = $this->fillDataSample ( $this->dataSampleTitle, $options ['sample'] );
		$this->connection->insertOnDuplicate ( $this->_resource->getTableName ( 'downloadable_sample_title' ), $this->downloadableHelper->prepareDataForSave ( $this->dataSampleTitle, $titleSample ) );
		return $this;
	}
	...
	protected function saveLinkOptions() {
		...
		$this->connection->insertOnDuplicate ( $this->_resource->getTableName ( 'downloadable_link' ), $this->downloadableHelper->prepareDataForSave ( $this->dataLink, $dataLink ) );
		$dataLink = $this->fillDataTitleLink ( $options ['link'] );
		$this->connection->insertOnDuplicate ( $this->_resource->getTableName ( 'downloadable_link_title' ), $this->downloadableHelper->prepareDataForSave ( $this->dataLinkTitle, $dataLink ['title'] ) );
		if (count ( $dataLink ['price'] )) {
			$this->connection->insertOnDuplicate ( $this->_resource->getTableName ( 'downloadable_link_price' ), $this->downloadableHelper->prepareDataForSave ( $this->dataLinkPrice, $dataLink ['price'] ) );
		}
		return $this;
	}
	...
	protected function prepareSampleData($rowCol, $entityId = null) {
		$result = [ ];
		$options = explode ( \Magento\CatalogImportExport\Model\Import\Product::PSEUDO_MULTI_LINE_SEPARATOR, $rowCol );
		foreach ( $options as $option ) {
			$result [] = array_merge ( $this->dataSample, [ 
					'product_id' => $entityId 
	...
	protected function prepareLinkData($rowCol, $entityId = null) {
		...
		$options = explode ( \Magento\CatalogImportExport\Model\Import\Product::PSEUDO_MULTI_LINE_SEPARATOR, $rowCol );
		foreach ( $options as $option ) {
			$result [] = array_merge ( $this->dataLink, [ 
					'product_id' => $entityId 
