...
class DownloadableTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	public function setUp() {
		...
		$this->connectionMock = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'select',
				'fetchAll',
				'fetchPairs',
				'joinLeft',
		...
		// 1. $attrSetColFac
		$this->attrSetColFacMock = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->attrSetColMock = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\Collection', [ 
				'setEntityTypeFilter' 
		], [ ], '', false );
		$this->attrSetColMock->expects ( $this->any () )->method ( 'setEntityTypeFilter' )->will ( $this->returnValue ( [ ] ) );
		
		// 2. $prodAttrColFac
		$this->prodAttrColFacMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		// 3. $resource
		$this->resourceMock = $this->getMock ( 'Magento\Framework\App\ResourceConnection', [ 
				'getConnection',
				'getTableName' 
		], [ ], '', false );
		...
		// 4. $params
		$this->entityModelMock = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product', [ 
				'addMessageTemplate',
				'getEntityTypeId',
				'getBehavior',
				'getNewSku',
		...
		$this->uploaderMock = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Uploader', [ 
				'move' 
		], [ ], '', false );
		...
		// 7. $fileHelper
		$this->uploaderHelper = $this->getMock ( '\Magento\DownloadableImportExport\Helper\Uploader', [ 
				'getUploader' 
		], [ ], '', false );
		$this->uploaderHelper->expects ( $this->any () )->method ( 'getUploader' )->willReturn ( $this->uploaderMock );
		$this->downloadableHelper = $this->getMock ( '\Magento\DownloadableImportExport\Helper\Data', [ 
				'prepareDataForSave' 
		], [ ], '', false );
	...
	public function testSaveDataAppend($newSku, $bunch, $allowImport, $fetchResult) {
		...
		$this->connectionMock->expects ( $this->any () )->method ( 'fetchAll' )->with ( $this->select )->will ( $this->onConsecutiveCalls ( [ 
				[ 
		...
		], $fetchResult ['sample'], $fetchResult ['sample'], $fetchResult ['link'], $fetchResult ['link'] ) );
		
		$downloadableModelMock = $this->objectManagerHelper->getObject ( '\Magento\DownloadableImportExport\Model\Import\Product\Type\Downloadable', [ 
	...
	public function testIsRowValid(array $rowData, $rowNum, $isNewProduct = true) {
		$this->connectionMock->expects ( $this->any () )->method ( 'fetchAll' )->with ( $this->select )->willReturnOnConsecutiveCalls ( [ 
				[ 
						'attribute_set_name' => '1',
						'attribute_id' => '1' 
				],
		...
		$this->downloadableModelMock = $this->objectManagerHelper->getObject ( '\Magento\DownloadableImportExport\Model\Import\Product\Type\Downloadable', [ 
				'attrSetColFac' => $this->attrSetColFacMock,
				'prodAttrColFac' => $this->prodAttrColFacMock,
				'resource' => $this->resourceMock,
				'params' => $this->paramsArray,
	...
	public function testPrepareAttributesWithDefaultValueForSave() {
		...
		$this->setPropertyValue ( $this->downloadableModelMock, '_attributes', [ 
				'Default' => [ 
						'name' => [ 
								'id' => '69',
								'code' => 'name',
