...
class Variable extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function getVariableByCode($code, $withValue = false, $storeId = 0) {
		$select = $this->getConnection ()->select ()->from ( $this->getMainTable () )->where ( $this->getMainTable () . '.code = ?', $code );
		if ($withValue) {
	...
	protected function _afterSave(\Magento\Framework\Model\AbstractModel $object) {
		...
		if ($object->getUseDefaultValue ()) {
			 ...
			 * remove store value
			 */
			$this->getConnection ()->delete ( $this->getTable ( 'variable_value' ), [ 
			...
			$this->getConnection ()->insertOnDuplicate ( $this->getTable ( 'variable_value' ), $data, [ 
					'plain_value',
					'html_value' 
			] );
		}
	...
	protected function _addValueToSelect(\Magento\Framework\DB\Select $select, $storeId = \Magento\Store\Model\Store::DEFAULT_STORE_ID) {
		...
		], 'def.variable_id = ' . $this->getMainTable () . '.variable_id AND def.store_id = 0', [ ] )->joinLeft ( [ 
				'store' => $this->getTable ( 'variable_value' ) 
		], 'store.variable_id = def.variable_id AND store.store_id = ' . $connection->quote ( $storeId ), [ ] )->columns ( [ 
