...
class Selection extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function getChildrenIds($parentId, $required = true) {
		...
		], 'e.entity_id = tbl_selection.product_id AND e.required_options=0', [ ] )->join ( [ 
	...
	public function getParentIdsByChild($childId) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->distinct ( true )->from ( $this->getMainTable (), 'parent_product_id' )->where ( 'product_id IN(?)', $childId );
	...
	public function saveSelectionPrice($item) {
		...
		if ($item->getDefaultPriceScope ()) {
			$connection->delete ( $this->getTable ( 'catalog_product_bundle_selection_price' ), [ 
			...
			$connection->insertOnDuplicate ( $this->getTable ( 'catalog_product_bundle_selection_price' ), $values, [ 
