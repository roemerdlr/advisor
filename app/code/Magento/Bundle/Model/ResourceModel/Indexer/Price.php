...
class Price extends \Magento\Catalog\Model\ResourceModel\Product\Indexer\Price\DefaultPrice {
	...
	protected function _prepareBundlePriceByType($priceType, $entityIds = null) {
		...
		$select->columns ( 'website_id', 'cw' )->join ( [ 
				'cwd' => $this->_getWebsiteDateTable () 
		], 'cw.website_id = cwd.website_id', [ ] )->joinLeft ( [ 
				'tp' => $this->_getTierPriceIndexTable () 
		], 'tp.entity_id = e.entity_id AND tp.website_id = cw.website_id' . ' AND tp.customer_group_id = cg.customer_group_id', [ ] )->where ( 'e.type_id=?', $this->getTypeId () );
		...
		$specialExpr = $connection->getCheckSql ( $connection->getCheckSql ( $specialFrom . ' IS NULL', '1', $connection->getCheckSql ( $specialFrom . ' <= ' . $curentDate, '1', '0' ) ) . " > 0 AND " . $connection->getCheckSql ( $specialTo . ' IS NULL', '1', $connection->getCheckSql ( $specialTo . ' >= ' . $curentDate, '1', '0' ) ) . " > 0 AND {$specialPrice} > 0 AND {$specialPrice} < 100 ", $specialPrice, '0' );
		
		$tierExpr = new \Zend_Db_Expr ( "tp.min_price" );
		
		if ($priceType == \Magento\Bundle\Model\Product\Price::PRICE_TYPE_FIXED) {
			$finalPrice = $connection->getCheckSql ( $specialExpr . ' > 0', 'ROUND(' . $price . ' * (' . $specialExpr . '  / 100), 4)', $price );
			$tierPrice = $connection->getCheckSql ( $tierExpr . ' IS NOT NULL', 'ROUND(' . $price . ' - ' . '(' . $price . ' * (' . $tierExpr . ' / 100)), 4)', 'NULL' );
			
			$finalPrice = $connection->getCheckSql ( "{$tierPrice} < {$finalPrice}", $tierPrice, $finalPrice );
		} else {
		 ...
		 */
		$this->_eventManager->dispatch ( 'catalog_product_prepare_index_select', [ 
				'select' => $select,
	...
	protected function _calculateBundleOptionPrice() {
		...
		$minPrice = new \Zend_Db_Expr ( $connection->getCheckSql ( 'SUM(io.min_price) = 0', 'MIN(io.alt_price)', 'SUM(io.min_price)' ) . ' + i.price' );
		$maxPrice = new \Zend_Db_Expr ( "SUM(io.max_price) + i.price" );
		$tierPrice = $connection->getCheckSql ( 'MIN(i.tier_percent) IS NOT NULL', $connection->getCheckSql ( 'SUM(io.tier_price) = 0', 'SUM(io.alt_tier_price)', 'SUM(io.tier_price)' ) . ' + MIN(i.tier_price)', 'NULL' );
		...
		], 'i.entity_id = io.entity_id AND i.customer_group_id = io.customer_group_id' . ' AND i.website_id = io.website_id', [ ] )->group ( [ 
				'io.entity_id',
				'io.customer_group_id',
				'io.website_id',
				'i.tax_class_id',
				'i.orig_price',
				'i.price' 
	...
	protected function _calculateBundleSelectionPrice($priceType) {
		...
		if ($priceType == \Magento\Bundle\Model\Product\Price::PRICE_TYPE_FIXED) {
			$selectionPriceValue = $connection->getCheckSql ( 'bsp.selection_price_value IS NULL', 'bs.selection_price_value', 'bsp.selection_price_value' );
			$selectionPriceType = $connection->getCheckSql ( 'bsp.selection_price_type IS NULL', 'bs.selection_price_type', 'bsp.selection_price_type' );
			$priceExpr = new \Zend_Db_Expr ( $connection->getCheckSql ( $selectionPriceType . ' = 1', 'ROUND(i.price * (' . $selectionPriceValue . ' / 100),4)', $connection->getCheckSql ( 'i.special_price > 0 AND i.special_price < 100', 'ROUND(' . $selectionPriceValue . ' * (i.special_price / 100),4)', $selectionPriceValue ) ) . '* bs.selection_qty' );
			
			$tierExpr = $connection->getCheckSql ( 'i.base_tier IS NOT NULL', $connection->getCheckSql ( $selectionPriceType . ' = 1', 'ROUND(i.base_tier - (i.base_tier * (' . $selectionPriceValue . ' / 100)),4)', $connection->getCheckSql ( 'i.tier_percent > 0', 'ROUND(' . $selectionPriceValue . ' - (' . $selectionPriceValue . ' * (i.tier_percent / 100)),4)', $selectionPriceValue ) ) . ' * bs.selection_qty', 'NULL' );
			
			$priceExpr = new \Zend_Db_Expr ( $connection->getCheckSql ( "{$tierExpr} < {$priceExpr}", $tierExpr, $priceExpr ) );
		} else {
			$priceExpr = new \Zend_Db_Expr ( $connection->getCheckSql ( 'i.special_price > 0 AND i.special_price < 100', 'ROUND(idx.min_price * (i.special_price / 100), 4)', 'idx.min_price' ) . ' * bs.selection_qty' );
			$tierExpr = $connection->getCheckSql ( 'i.base_tier IS NOT NULL', 'ROUND(idx.min_price * (i.base_tier / 100), 4)* bs.selection_qty', 'NULL' );
		}
		...
		], 'bs.product_id = idx.entity_id AND i.customer_group_id = idx.customer_group_id' . ' AND i.website_id = idx.website_id', [ ] )->join ( [ 
				'e' => $this->getTable ( 'catalog_product_entity' ) 
		], 'bs.product_id = e.entity_id AND e.required_options=0', [ ] )->where ( 'i.price_type=?', $priceType )->columns ( [ 
	...
	protected function _prepareTierPriceIndex($entityIds = null) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'e' => $this->getTable ( 'catalog_product_entity' ) 
		], 'i.entity_id=e.entity_id', [ ] )->where ( 'e.type_id=?', $this->getTypeId () );
		...
		$select = $connection->select ()->from ( [ 
				...
				'website_id' 
		] )->where ( 'cw.website_id != 0' )->where ( 'e.type_id=?', $this->getTypeId () )->columns ( new \Zend_Db_Expr ( 'MIN(tp.value)' ) )->group ( [ 
				'tp.entity_id',
