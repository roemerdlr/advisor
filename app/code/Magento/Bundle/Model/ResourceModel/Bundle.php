...
class Bundle extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Catalog\Model\ResourceModel\Product\Relation $productRelation, \Magento\Quote\Model\ResourceModel\Quote $quoteResource, $connectionName = null) {
	...
	protected function _getSelect($productId, $columns = []) {
		return $this->getConnection ()->select ()->from ( [ 
				...
				'type',
				'option_id' 
		] )->where ( "bundle_option.parent_id = ?", $productId )->where ( "bundle_option.required = 1" )->joinLeft ( [ 
	...
	public function dropAllQuoteChildItems($productId) {
		...
		$quoteItemIds = $connection->fetchCol ( $select->from ( $this->getTable ( 'quote_item' ), [ 
				'item_id' 
		...
		if ($quoteItemIds) {
			$connection->delete ( $this->getTable ( 'quote_item' ), [ 
