...
class Type extends \Magento\Catalog\Model\Product\Type\AbstractType {
	...
	public function getSelectionsCollection($optionIds, $product) {
		...
		if (! $product->hasData ( $key )) {
			...
			$selectionsCollection = $this->_bundleCollection->create ()->addAttributeToSelect ( $this->_config->getProductAttributes () )->addAttributeToSelect ( 'tax_class_id' )-> // used for calculation item taxes in Bundle with Dynamic Price
setFlag ( 'require_stock_items', true )->setFlag ( 'product_children', true )->setPositionOrder ()->addStoreFilter ( $this->getStoreFilter ( $product ) )->setStoreId ( $storeId )->addFilterByRequiredOptions ()->setOptionIdsFilter ( $optionIds );
	 ...
	 * @param mixed $qty        	
	 * @param \Magento\Catalog\Model\Product $product        	
	 * @return int @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	 ...
	 * @return \Magento\Framework\Phrase|array|string @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	...
	public function getProductsToPurchaseByReqGroups($product) {
		...
		foreach ( $this->getOptions ( $product ) as $option ) {
			$groupProducts = [ ];
			foreach ( $this->getSelectionsCollection ( [ 
					$option->getId () 
			], $product ) as $childProduct ) {
	...
	public function canConfigure($product) {
		return $product instanceof \Magento\Catalog\Model\Product && $product->isAvailable () && parent::canConfigure ( $product );
	}
