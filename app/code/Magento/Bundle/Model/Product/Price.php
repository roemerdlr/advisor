...
class Price extends \Magento\Catalog\Model\Product\Type\Price {
	...
	public function getFinalPrice($qty, $product) {
		...
		$this->_eventManager->dispatch ( 'catalog_product_get_final_price', [ 
				'product' => $product,
				'qty' => $qty 
		] );
	 ...
	 * @return float|array @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 ...
	 * @param null|bool $multiplyQty
	 *        	Whether to multiply selection's price by its quantity
	 * @return float
	 *
	...
	public function getSelectionFinalTotalPrice($bundleProduct, $selectionProduct, $bundleQty, $selectionQty, $multiplyQty = true, $takeTierPrice = true) {
		...
		return min ( $price, $this->_applyTierPrice ( $bundleProduct, $bundleQty, $price ), $this->_applySpecialPrice ( $bundleProduct, $price ) );
	}
	...
	public function getTierPrice($qty, $product) {
		...
		if ($qty) {
			...
			foreach ( $prices as $price ) {
				...
				if ($price ['price_qty'] == $prevQty && $prevGroup != $allCustomersGroupId && $price ['cust_group'] == $allCustomersGroupId) {
