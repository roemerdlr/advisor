...
class OptionRepository implements \Magento\Bundle\Api\ProductOptionRepositoryInterface {
	 ...
	 */
	public function __construct(\Magento\Catalog\Api\ProductRepositoryInterface $productRepository, \Magento\Bundle\Model\Product\Type $type, \Magento\Bundle\Api\Data\OptionInterfaceFactory $optionFactory, \Magento\Bundle\Model\ResourceModel\Option $optionResource, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Bundle\Api\ProductLinkManagementInterface $linkManagement, \Magento\Bundle\Model\Product\OptionList $productOptionList, \Magento\Bundle\Model\Product\LinksList $linkList, \Magento\Framework\Api\DataObjectHelper $dataObjectHelper) {
	...
	public function delete(\Magento\Bundle\Api\Data\OptionInterface $option) {
		try {
			...
			throw new \Magento\Framework\Exception\StateException ( __ ( 'Cannot delete option with id %1', $option->getOptionId () ), $exception );
		}
		return true;
	}
	 ...
	 *        	of \Magento\Bundle\Api\Data\LinkInterface
	 * @param array $secondArray
	 *        	of \Magento\Bundle\Api\Data\LinkInterface
	 * @return array @SuppressWarnings(PHPMD.UnusedPrivateMethod)
	 */
	private function compareLinks(array $firstArray, array $secondArray) {
		...
		foreach ( $firstArray as $obj ) {
			foreach ( $secondArray as $objToCompare ) {
				if ($obj->getId () != $objToCompare->getId () && $obj instanceof \Magento\Bundle\Api\Data\LinkInterface && $objToCompare instanceof \Magento\Bundle\Api\Data\LinkInterface) {
					$result [] = $obj;
				}
			}
		}
