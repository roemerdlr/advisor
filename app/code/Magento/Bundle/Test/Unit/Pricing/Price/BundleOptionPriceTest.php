...
class BundleOptionPriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->saleableItemMock->expects ( $this->once () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $this->priceInfoMock ) );
		
		$store = $this->getMockBuilder ( 'Magento\Store\Model\Store' )->disableOriginalConstructor ()->getMock ();
		$priceCurrency->expects ( $this->any () )->method ( 'round' )->will ( $this->returnArgument ( 0 ) );
		
		$this->saleableItemMock->expects ( $this->once () )->method ( 'setQty' )->will ( $this->returnSelf () );
		
		$this->saleableItemMock->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( $store ) );
		
		$this->selectionFactoryMock = $this->getMockBuilder ( 'Magento\Bundle\Pricing\Price\BundleSelectionFactory' )->disableOriginalConstructor ()->getMock ();
		...
		$this->bundleCalculatorMock = $this->getMockBuilder ( 'Magento\Bundle\Pricing\Adjustment\Calculator' )->setConstructorArgs ( [ 
				$this->baseCalculator,
				$this->amountFactory,
		...
		$this->bundleOptionPrice = $this->objectManagerHelper->getObject ( 'Magento\Bundle\Pricing\Price\BundleOptionPrice', [ 
	...
	protected function prepareOptionMocks($selectionCollection) {
		$this->saleableItemMock->expects ( $this->atLeastOnce () )->method ( 'getStoreId' )->will ( $this->returnValue ( 1 ) );
		
		$priceTypeMock = $this->getMock ( 'Magento\Bundle\Model\Product\Type', [ ], [ ], '', false );
		$priceTypeMock->expects ( $this->atLeastOnce () )->method ( 'setStoreFilter' )->with ( $this->equalTo ( 1 ), $this->equalTo ( $this->saleableItemMock ) )->will ( $this->returnSelf () );
		
		$optionIds = [ 
				'41',
				'55' 
		];
		$priceTypeMock->expects ( $this->atLeastOnce () )->method ( 'getOptionsIds' )->with ( $this->equalTo ( $this->saleableItemMock ) )->will ( $this->returnValue ( $optionIds ) );
		...
		$collection->expects ( $this->atLeastOnce () )->method ( 'appendSelections' )->with ( $this->equalTo ( $selectionCollection ), $this->equalTo ( true ), $this->equalTo ( false ) )->will ( $this->returnValue ( $selectionCollection ) );
		
		$priceTypeMock->expects ( $this->atLeastOnce () )->method ( 'getOptionsCollection' )->with ( $this->equalTo ( $this->saleableItemMock ) )->will ( $this->returnValue ( $collection ) );
		
		$this->saleableItemMock->expects ( $this->atLeastOnce () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $priceTypeMock ) );
	}
	...
	public function testGetOptionSelectionAmount($selectionQty, $selectionAmount) {
		$selection = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
				'getSelectionQty',
				'__wakeup' 
		], [ ], '', false );
	...
	protected function createAmountMock($amountData) {
		...
		$amount->expects ( $this->any () )->method ( 'getAdjustmentAmounts' )->will ( $this->returnValue ( isset ( $amountData ['adjustmentAmounts'] ) ? $amountData ['adjustmentAmounts'] : [ ] ) );
	...
	protected function createOptionMock($optionData) {
		...
		$option->expects ( $this->any () )->method ( 'isMultiSelection' )->will ( $this->returnValue ( $optionData ['isMultiSelection'] ) );
		$selections = [ ];
	...
	protected function createSelectionMock($selectionData) {
		$selection = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
				'isSalable',
				'getAmount',
				'getQuantity',
				'getProduct',
		...
		$innerProduct = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
	...
	public function testCalculation($optionList, $expected) {
		...
		$this->baseCalculator->expects ( $this->atLeastOnce () )->method ( 'getAmount' )->will ( $this->returnValue ( $this->createAmountMock ( [ 
				'amount' => 0. 
		] ) ) );
