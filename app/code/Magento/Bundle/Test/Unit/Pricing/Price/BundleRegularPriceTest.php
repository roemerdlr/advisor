...
class BundleRegularPriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->saleableInterfaceMock = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->setMethods ( [ 
				'getPriceInfo',
				'getPriceType',
				'getPrice' 
		] )->getMock ();
		...
		$this->regularPrice = new \Magento\Bundle\Pricing\Price\BundleRegularPrice ( $this->saleableInterfaceMock, $this->quantity, $this->bundleCalculatorMock, $this->priceCurrencyMock );
	}
	...
	public function testGetMaximalPrice() {
		...
		$this->bundleCalculatorMock->expects ( $this->once () )->method ( 'getMaxRegularAmount' )->with ( $expectedResult, $this->saleableInterfaceMock )->will ( $this->returnValue ( $expectedResult ) );
	...
	public function testGetMaximalPriceForFixedPriceBundleWithOption() {
		...
		$this->customOptionPriceMock->expects ( $this->once () )->method ( 'getCustomOptionRange' )->with ( false )->willReturn ( $maxOptionPrice );
		...
		$this->bundleCalculatorMock->expects ( $this->once () )->method ( 'getMaxRegularAmount' )->with ( $expectedPrice, $this->saleableInterfaceMock )->will ( $this->returnValue ( $expectedPrice ) );
