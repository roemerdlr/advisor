...
class CalculatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->saleableItem = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
				'getPriceInfo',
				'getPriceType',
				'__wakeup',
				'getStore' 
		...
		$this->selectionFactory = $this->getMockBuilder ( 'Magento\Bundle\Pricing\Price\BundleSelectionFactory' )->disableOriginalConstructor ()->getMock ();
		$this->selectionFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnArgument ( 1 ) );
		
		$this->taxData = $this->getMockBuilder ( 'Magento\Tax\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = (new ObjectManager ( $this ))->getObject ( 'Magento\Bundle\Pricing\Adjustment\Calculator', [ 
	...
	public function testEmptySelectionPriceList() {
		...
		$option->expects ( $this->any () )->method ( 'getSelections' )->will ( $this->returnValue ( null ) );
	...
	protected function createAmountMock($amountData) {
		...
		$amount = $this->getMockBuilder ( 'Magento\Framework\Pricing\Amount\Base' )->setMethods ( [ 
				'getAdjustmentAmounts',
				'getValue',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		$amount->expects ( $this->any () )->method ( 'getAdjustmentAmounts' )->will ( $this->returnValue ( $amountData ['adjustmentsAmounts'] ) );
	...
	public function testGetAmountWithoutOption() {
		...
		$calculatorMock = $this->getMockBuilder ( 'Magento\Bundle\Pricing\Adjustment\Calculator' )->disableOriginalConstructor ()->setMethods ( [ 
				'calculateBundleAmount' 
		] )->getMock ();
		
		$calculatorMock->expects ( $this->once () )->method ( 'calculateBundleAmount' )->with ( $amount, $this->saleableItem, [ ] )->will ( $this->returnValue ( $result ) );
	...
	public function testGetMinRegularAmount() {
		...
		$calculatorMock->expects ( $this->once () )->method ( 'getOptionsAmount' )->with ( $this->saleableItem, $exclude, true, $amount, true )->will ( $this->returnValue ( $expectedResult ) );
	...
	public function testGetMaxRegularAmount() {
		...
		$calculatorMock->expects ( $this->once () )->method ( 'getOptionsAmount' )->with ( $this->saleableItem, $exclude, false, $amount, true )->will ( $this->returnValue ( $expectedResult ) );
	...
	public function testGetOptionsAmount($searchMin, $useRegularPrice) {
		...
		$selections [] = $this->getMockBuilder ( '\Magento\Bundle\Pricing\Price\BundleSelectionPrice' )->disableOriginalConstructor ()->getMock ();
		$calculatorMock->expects ( $this->once () )->method ( 'getSelectionAmounts' )->with ( $this->saleableItem, $searchMin, $useRegularPrice )->will ( $this->returnValue ( $selections ) );
		
		$calculatorMock->expects ( $this->once () )->method ( 'calculateBundleAmount' )->with ( $amount, $this->saleableItem, $selections, $exclude )->will ( $this->returnValue ( $expectedResult ) );
		
		$result = $calculatorMock->getOptionsAmount ( $this->saleableItem, $exclude, $searchMin, $amount, $useRegularPrice );
