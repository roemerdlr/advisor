...
class ConfigurationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->pricingHelper = $this->getMock ( 'Magento\Framework\Pricing\Helper\Data', [ 
				'currency' 
		], [ ], '', false );
		...
		$this->item = $this->getMock ( 'Magento\Catalog\Model\Product\Configuration\Item\ItemInterface', [ 
				'getQty',
				'getProduct',
				'getOptionByCode',
	...
	public function testGetSelectionQty() {
		...
		$option = $this->getMock ( 'Magento\Catalog\Model\Product\Option', [ 
				'__wakeup',
				'getValue' 
		], [ ], '', false );
		
		$product->expects ( $this->once () )->method ( 'getCustomOption' )->with ( 'selection_qty_' . $selectionId )->will ( $this->returnValue ( $option ) );
	...
	public function testGetBundleOptionsEmptyBundleOptionsIds() {
		...
		$this->item->expects ( $this->once () )->method ( 'getProduct' )->will ( $this->returnValue ( $product ) );
		$this->item->expects ( $this->once () )->method ( 'getOptionByCode' )->with ( 'bundle_option_ids' )->will ( $this->returnValue ( null ) );
	...
	public function testGetBundleOptionsEmptyBundleSelectionIds() {
		...
		$typeInstance = $this->getMock ( 'Magento\Bundle\Model\Product\Type', [ 
				'getOptionsByIds' 
		], [ ], '', false );
		$selectionOption = $this->getMock ( '\Magento\Catalog\Model\Product\Configuration\Item\Option\OptionInterface', [ 
				'getValue' 
		] );
	...
	public function testGetOptions() {
		...
		$priceModel = $this->getMock ( 'Magento\Bundle\Model\Product\Price', [ 
				'getSelectionFinalTotalPrice' 
		], [ ], '', false );
		$selectionQty = $this->getMock ( 'Magento\Quote\Model\Quote\Item\Option', [ 
				'getValue',
				'__wakeup' 
		], [ ], '', false );
		$bundleOption = $this->getMock ( 'Magento\Bundle\Model\Option', [ 
				'getSelections',
				'getTitle',
				'__wakeup' 
		], [ ], '', false );
		...
		$collection = $this->getMock ( 'Magento\Bundle\Model\ResourceModel\Option\Collection', [ 
				'appendSelections' 
		], [ ], '', false );
		$itemOption = $this->getMock ( '\Magento\Catalog\Model\Product\Configuration\Item\Option\OptionInterface', [ 
				'getValue' 
		...
		$bundleOption->expects ( $this->any () )->method ( 'getSelections' )->will ( $this->returnValue ( [ 
				$product 
		] ) );
		...
		$collection->expects ( $this->once () )->method ( 'appendSelections' )->with ( $collection2, true )->will ( $this->returnValue ( [ 
				$bundleOption 
		] ) );
		$itemOption->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $optionIds ) );
		$typeInstance->expects ( $this->once () )->method ( 'getOptionsByIds' )->with ( unserialize ( $optionIds ), $product )->will ( $this->returnValue ( $collection ) );
		...
		$this->item->expects ( $this->at ( 1 ) )->method ( 'getOptionByCode' )->with ( 'bundle_option_ids' )->will ( $this->returnValue ( $itemOption ) );
		$this->item->expects ( $this->at ( 2 ) )->method ( 'getOptionByCode' )->with ( 'bundle_selection_ids' )->will ( $this->returnValue ( $selectionOption ) );
		$this->productConfiguration->expects ( $this->once () )->method ( 'getCustomOptions' )->with ( $this->item )->will ( $this->returnValue ( [ 
				0 => [ 
						'label' => 'title',
						'value' => 'value' 
				] 
		...
		$this->assertEquals ( [ 
				0 => [ 
						'label' => 'title',
						'value' => [ 
								0 => '1 x name <span class="price">$15.00</span>' 
						] 
				],
				1 => [ 
						'label' => 'title',
