...
class BundleTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->bundleProductPriceFactory = $this->getMockBuilder ( '\Magento\Bundle\Model\Product\PriceFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->product = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->setMethods ( [ 
				'getTypeInstance',
				'getPriceInfo',
				'getStoreId',
				'getPriceType',
	...
	public function testGetOptionHtmlNoRenderer() {
		...
		$this->assertEquals ( 'There is no defined renderer for "checkbox" option type.', $this->_bundleBlock->getOptionHtml ( $option ) );
	}
	public function testGetOptionHtml() {
		...
		$optionBlock = $this->getMock ( '\Magento\Bundle\Block\Catalog\Product\View\Type\Bundle\Option\Checkbox', [ 
				'setOption',
				'toHtml' 
		], [ ], '', false );
		...
		$layout = $this->getMock ( 'Magento\Framework\View\Layout', [ 
				'getChildName',
				'getBlock' 
		], [ ], '', false );
	...
	public function testGetJsonConfigFixedPriceBundle() {
		...
		$selections = [ 
				$this->createOptionSelection ( 1123, 'Selection 1', 23, [ 
						[ 
								'price' => new MagentoObject ( [ 
		...
		$bundleProductPrice = $this->getMockBuilder ( '\Magento\Bundle\Model\Product\Price' )->disableOriginalConstructor ()->setMethods ( [ 
				'getLowestPrice' 
		] )->getMock ();
		...
		$prices = [ 
				'bundle_option' => $this->getAmountPriceMock ( $baseAmount, $regularPriceMock, [ 
						[ 
								'item' => $selections [0],
								'value' => $basePriceValue,
								'base_amount' => 321321 
	...
	private function setupBundleBlock($options, $priceInfo, $priceType) {
		...
		$eventManager = $this->getMockBuilder ( '\Magento\Framework\Event\Manager' )->disableOriginalConstructor ()->getMock ();
		$eventManager->expects ( $this->any () )->method ( 'dispatch' )->will ( $this->returnValue ( true ) );
		
		$optionCollection = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\Option\Collection' )->disableOriginalConstructor ()->getMock ();
		...
		$typeInstance->expects ( $this->any () )->method ( 'getOptionsCollection' )->will ( $this->returnValue ( $optionCollection ) );
		$typeInstance->expects ( $this->any () )->method ( 'getStoreFilter' )->will ( $this->returnValue ( true ) );
		
		$this->product->expects ( $this->any () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $typeInstance ) );
		$this->product->expects ( $this->any () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $priceInfo ) );
		$this->product->expects ( $this->any () )->method ( 'getPriceType' )->will ( $this->returnValue ( $priceType ) );
		
		$registry = $this->getMockBuilder ( '\Magento\Framework\Registry' )->disableOriginalConstructor ()->setMethods ( [ 
				'registry' 
		] )->getMock ();
		$registry->expects ( $this->once () )->method ( 'registry' )->will ( $this->returnValue ( $this->product ) );
		
		$taxHelperMock = $this->getMockBuilder ( '\Magento\Tax\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		
		$context = $this->getMockBuilder ( '\Magento\Catalog\Block\Product\Context' )->disableOriginalConstructor ()->getMock ();
		$context->expects ( $this->any () )->method ( 'getRegistry' )->will ( $this->returnValue ( $registry ) );
		$context->expects ( $this->any () )->method ( 'getTaxData' )->will ( $this->returnValue ( $taxHelperMock ) );
		$context->expects ( $this->any () )->method ( 'getEventManager' )->will ( $this->returnValue ( $eventManager ) );
		
		$jsonEncoderMock = $this->getMockBuilder ( '\Magento\Framework\Json\Encoder' )->disableOriginalConstructor ()->getMock ();
		$jsonEncoderMock->expects ( $this->any () )->method ( 'encode' )->will ( $this->returnArgument ( 0 ) );
		
		/** @var $bundleBlock BundleBlock */
		$bundleBlock = $objectHelper->getObject ( 'Magento\Bundle\Block\Catalog\Product\View\Type\Bundle', [ 
				'context' => $context,
	...
	private function getPriceInfoMock($price) {
		$priceInfoMock = $this->getMockBuilder ( '\Magento\Framework\Pricing\PriceInfo\Base' )->disableOriginalConstructor ()->setMethods ( [ 
				'getPrice' 
		] )->getMock ();
		...
		if (is_array ( $price )) {
			...
			foreach ( $price as $priceType => $priceValue ) {
				$priceInfoMock->expects ( $this->at ( $counter ) )->method ( 'getPrice' )->with ( $priceType )->will ( $this->returnValue ( $priceValue ) );
				$counter ++;
			}
		} else {
			$priceInfoMock->expects ( $this->any () )->method ( 'getPrice' )->will ( $this->returnValue ( $price ) );
		}
	...
	private function getPriceMock($prices) {
		...
		$priceMock = $this->getMockBuilder ( 'Magento\Catalog\Pricing\Price\BasePrice' )->disableOriginalConstructor ()->setMethods ( $methods )->getMock ();
		foreach ( $prices as $methodName => $amount ) {
			$priceMock->expects ( $this->any () )->method ( $methodName )->will ( $this->returnValue ( $amount ) );
		}
	...
	private function getAmountPriceMock($value, $baseAmount, array $selectionAmounts) {
		$amountPrice = $this->getMockBuilder ( '\Magento\Framework\Pricing\Amount\AmountInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getValue',
				'getBaseAmount',
		...
		foreach ( $selectionAmounts as $selectionAmount ) {
			$amountPrice->expects ( $this->any () )->method ( 'getOptionSelectionAmount' )->with ( $selectionAmount ['item'] )->will ( $this->returnValue ( new MagentoObject ( [ 
	 ...
	 */
	private function createOption($id, $title, array $selections = [], $type = 'checkbox', $isRequired = false) {
	 ...
	 */
	private function createOptionSelection($id, $name, $qty, array $tierPriceList = [], $isCanChangeQty = true, $isDefault = false, $isSalable = true) {
		$selection = $this->getMockBuilder ( '\Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->setMethods ( [ 
				'getSelectionId',
				'getSelectionQty',
				'getPriceInfo',
		...
		$tierPrice = $this->getMockBuilder ( '\Magento\Bundle\Pricing\Price\TierPrice' )->disableOriginalConstructor ()->setMethods ( [ 
				'getTierPriceList' 
		] )->getMock ();
		$tierPrice->expects ( $this->any () )->method ( 'getTierPriceList' )->will ( $this->returnValue ( $tierPriceList ) );
		$priceInfo = $this->getMockBuilder ( '\Magento\Framework\Pricing\PriceInfo\Base' )->disableOriginalConstructor ()->setMethods ( [ 
				'getPrice' 
		] )->getMock ();
		$priceInfo->expects ( $this->any () )->method ( 'getPrice' )->will ( $this->returnValue ( $tierPrice ) );
		...
		$selection->expects ( $this->any () )->method ( 'getSelectionCanChangeQty' )->will ( $this->returnValue ( $isCanChangeQty ) );
