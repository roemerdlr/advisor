...
class BundleSaveOptionsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productBundleOptionsMock = $this->getMock ( 'Magento\Bundle\Api\Data\OptionInterface', [ ], [ ], '', false );
	}
	...
	public function testAroundSaveWhenProductIsBundleWithoutOptions() {
		...
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getBundleProductOptions' )->willReturn ( null );
	...
	public function testAroundSaveWhenProductIsBundleWithOptions() {
		...
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->willReturn ( 'bundle' );
		$this->productMock->expects ( $this->once () )->method ( 'getExtensionAttributes' )->willReturn ( $this->productExtensionMock );
		$this->productExtensionMock->expects ( $this->once () )->method ( 'getBundleProductOptions' )->willReturn ( [ 
				$option 
		] );
		...
		$this->productOptionRepositoryMock->expects ( $this->once () )->method ( 'getList' )->with ( $productSku )->will ( $this->returnValue ( [ ] ) );
