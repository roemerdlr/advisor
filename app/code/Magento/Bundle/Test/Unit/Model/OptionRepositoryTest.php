...
class OptionRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->optionFactoryMock = $this->getMockBuilder ( '\Magento\Bundle\Api\Data\OptionInterfaceFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		...
		$this->optionResourceMock = $this->getMock ( '\Magento\Bundle\Model\ResourceModel\Option', [ 
				'delete',
				'__wakeup',
				'save' 
		], [ ], '', false );
		...
		$this->model = new \Magento\Bundle\Model\OptionRepository ( $this->productRepositoryMock, $this->typeMock, $this->optionFactoryMock, $this->optionResourceMock, $this->storeManagerMock, $this->linkManagementMock, $this->optionListMock, $this->linkListMock, $this->dataObjectHelperMock );
	}
	...
	public function testGetThrowsExceptionIfProductIsSimple() {
		...
		$productMock->expects ( $this->once () )->method ( 'getTypeId' )->willReturn ( \Magento\Catalog\Model\Product\Type::TYPE_SIMPLE );
		$this->productRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $productSku )->willReturn ( $productMock );
	...
	public function testDeleteThrowsExceptionIfCannotDelete() {
		...
		$this->optionResourceMock->expects ( $this->once () )->method ( 'delete' )->with ( $optionMock )->willThrowException ( new \Exception () );
	...
	public function testSaveIfOptionIdIsNull() {
		...
		$optionMock->expects ( $this->exactly ( 2 ) )->method ( 'getProductLinks' )->willReturn ( [ 
				$linkedProductMock 
		] );
	...
	public function testUpdateIfOptionDoesNotExist() {
		...
		$optCollectionMock->expects ( $this->once () )->method ( 'getItemById' )->with ( $optionId )->willReturn ( $existingOptionMock );
	...
	public function testSaveExistingOption() {
		...
		$existingLinks = [ 
				$existingLinkToUpdate,
				$existingLinkToDelete 
		];
		$this->linkManagementMock->expects ( $this->once () )->method ( 'getChildren' )->with ( $productSku, $optionId )->willReturn ( $existingLinks );
		...
		$this->linkManagementMock->expects ( $this->once () )->method ( 'addChild' )->with ( $productMock, $optionId, $productLinkNew );
		$this->linkManagementMock->expects ( $this->once () )->method ( 'saveChild' )->with ( $productSku, $productLinkUpdate );
		$this->linkManagementMock->expects ( $this->once () )->method ( 'removeChild' )->with ( $productSku, $optionId, $productSkuToDelete );
		$this->assertEquals ( $optionId, $this->model->save ( $productMock, $optionMock ) );
	}
	...
	public function testSaveNewOption() {
		...
		$optionMock->expects ( $this->once () )->method ( 'setStoreId' )->with ( $storeId )->willReturnSelf ();
		$optionMock->expects ( $this->once () )->method ( 'setParentId' )->with ( $productId )->willReturnSelf ();
		$optionMock->method ( 'getOptionId' )->will ( $this->onConsecutiveCalls ( null, $optionId, $optionId, $optionId, $optionId ) );
		...
		$this->optionResourceMock->expects ( $this->once () )->method ( 'save' )->with ( $optionMock )->willReturnSelf ();
		$this->linkManagementMock->expects ( $this->at ( 0 ) )->method ( 'addChild' )->with ( $productMock, $optionId, $productLink1 );
		$this->linkManagementMock->expects ( $this->at ( 1 ) )->method ( 'addChild' )->with ( $productMock, $optionId, $productLink2 );
		$this->assertEquals ( $optionId, $this->model->save ( $productMock, $optionMock ) );
	}
	...
	public function testGetList() {
		...
		$this->productRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $productSku )->willReturn ( $productMock );
		$this->optionListMock->expects ( $this->once () )->method ( 'getItems' )->with ( $productMock )->willReturn ( [ 
				'object' 
		] );
		$this->assertEquals ( [ 
				'object' 
	...
	private function createOptionMock() {
		...
		$extensionAttributesFactory = $this->getMock ( 'Magento\Framework\Api\ExtensionAttributesFactory', [ ], [ ], '', false );
