...
class AbstractItemsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->orderItem = $this->getMock ( 'Magento\Sales\Model\Order\Item', [ 
				'getProductOptions',
				'__wakeup',
				'getParentItem',
				'getOrderItem',
	...
	public function testGetChildren($parentItem) {
		if ($parentItem) {
			$parentItem = $this->getMock ( 'Magento\Sales\Model\Order\Item', [ 
					'getId',
					'__wakeup' 
			], [ ], '', false );
		...
		$salesModel = $this->getMock ( 'Magento\Sales\Model\Order\Invoice', [ 
				'getAllItems',
				'__wakeup' 
		], [ ], '', false );
		$salesModel->expects ( $this->once () )->method ( 'getAllItems' )->will ( $this->returnValue ( [ 
		...
		$item = $this->getMock ( 'Magento\Sales\Model\Order\Invoice\Item', [ 
				'getInvoice',
				'getOrderItem',
				'__wakeup' 
		], [ ], '', false );
	...
	public function getSelectionAttributesDataProvider() {
		return [ 
				...
				[ 
						[ 
								'bundle_selection_attributes' => 'a:1:{i:0;i:1;}' 
						],
						[ 
								0 => 1 
						] 
