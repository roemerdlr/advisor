...
class TypeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->bundleCollection = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Selection\CollectionFactory' )->setMethods ( [ 
				'create' 
		...
		$this->bundleOptionFactory = $this->getMockBuilder ( 'Magento\Bundle\Model\OptionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->stockRegistry = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\StockRegistry' )->setMethods ( [ 
				'getStockItem' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->stockState = $this->getMockBuilder ( '\Magento\CatalogInventory\Model\StockState' )->setMethods ( [ 
				'getStockQty' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->catalogProduct = $this->getMockBuilder ( 'Magento\Catalog\Helper\Product' )->setMethods ( [ 
				'getSkipSaleableCheck' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->priceCurrency = $this->getMockBuilder ( 'Magento\Framework\Pricing\PriceCurrencyInterface' )->setMethods ( [ 
				'convert' 
		] )->disableOriginalConstructor ()->getMockForAbstractClass ();
		$this->bundleModelSelection = $this->getMockBuilder ( 'Magento\Bundle\Model\SelectionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->bundleFactory = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\BundleFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$objectHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $objectHelper->getObject ( 'Magento\Bundle\Model\Product\Type', [ 
	...
	public function testPrepareForCartAdvancedWithoutOptions() {
		...
		$group = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Option\Type\DefaultType' )->setMethods ( [ 
				'setOption',
				'setProduct',
				'setRequest',
				'setProcessMode',
		...
		$buyRequest = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'__wakeup',
				'getOptions',
		...
		$option = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Option' )->setMethods ( [ 
				'groupFactory',
				'getType',
				'getId',
				'getRequired',
		...
		$selectionCollection = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Selection\Collection' )->setMethods ( [ 
				'getItems' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$productType = $this->getMockBuilder ( '\Magento\Bundle\Model\Product\Type' )->setMethods ( [ 
				'setStoreFilter',
		...
		$optionCollection = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Option\Collection' )->setMethods ( [ 
				'getItems',
				'getItemById',
				'appendSelections' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$product->expects ( $this->any () )->method ( 'getSkipCheckRequiredOption' )->willReturn ( true );
		$product->expects ( $this->any () )->method ( 'getTypeInstance' )->willReturn ( $productType );
		$product->expects ( $this->any () )->method ( 'getData' )->willReturnCallback ( function ($key) use($optionCollection, $selectionCollection) {
			$resultValue = null;
			switch ($key) {
		...
		$optionCollection->expects ( $this->any () )->method ( 'appendSelections' )->willReturn ( [ 
				$option 
		] );
		$productType->expects ( $this->once () )->method ( 'setStoreFilter' );
		$productType->expects ( $this->once () )->method ( 'getOptionsCollection' )->willReturn ( $optionCollection );
		$productType->expects ( $this->once () )->method ( 'getOptionsIds' )->willReturn ( [ 
				1,
				2,
				3 
		] );
		$productType->expects ( $this->once () )->method ( 'getSelectionsCollection' )->willReturn ( $selectionCollection );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOption' )->willReturn ( 'options' );
		$option->expects ( $this->at ( 3 ) )->method ( 'getId' )->willReturn ( 10 );
		$option->expects ( $this->once () )->method ( 'getRequired' )->willReturn ( true );
	...
	public function testPrepareForCartAdvancedWithShoppingCart() {
		...
		$priceModel = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Type\Price' )->setMethods ( [ 
		...
		/** @var \PHPUnit_Framework_MockObject_MockObject|\Magento\Framework\DataObject $buyRequest */
		$selection = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'__wakeup',
				'isSalable',
				'getOptionId',
		...
		$product->expects ( $this->once () )->method ( 'hasData' )->willReturn ( true );
		$product->expects ( $this->any () )->method ( 'getData' )->willReturnCallback ( function ($key) use($optionCollection, $selectionCollection) {
			$resultValue = null;
			switch ($key) {
				case '_cache_instance_options_collection' :
		...
		$product->expects ( $this->any () )->method ( 'getId' )->willReturn ( 333 );
		$product->expects ( $this->once () )->method ( 'getCustomOption' )->willReturn ( $option );
		$product->expects ( $this->once () )->method ( 'getPriceModel' )->willReturn ( $priceModel );
		$optionCollection->expects ( $this->once () )->method ( 'getItemById' )->willReturn ( $option );
		$optionCollection->expects ( $this->once () )->method ( 'appendSelections' );
		$productType->expects ( $this->once () )->method ( 'setStoreFilter' );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOption' )->willReturn ( [ 
				3 => 5,
				10 => [ 
						7 => 2,
						11 => 14 
		...
		$selectionCollection->expects ( $this->any () )->method ( 'getItems' )->willReturn ( [ 
				$selection 
		] );
		$selection->expects ( $this->once () )->method ( 'isSalable' )->willReturn ( false );
		$selection->expects ( $this->any () )->method ( 'getOptionId' )->willReturn ( 3 );
		$selection->expects ( $this->any () )->method ( 'getOption' )->willReturn ( $option );
		$selection->expects ( $this->once () )->method ( 'getSelectionCanChangeQty' )->willReturn ( true );
		$selection->expects ( $this->once () )->method ( 'getSelectionId' );
		$selection->expects ( $this->once () )->method ( 'addCustomOption' )->willReturnSelf ();
		$selection->expects ( $this->any () )->method ( 'getId' )->willReturn ( 333 );
		$selection->expects ( $this->once () )->method ( 'getTypeInstance' )->willReturn ( $productType );
		$option->expects ( $this->at ( 3 ) )->method ( 'getId' )->willReturn ( 10 );
		$option->expects ( $this->at ( 9 ) )->method ( 'getId' )->willReturn ( 10 );
		$option->expects ( $this->once () )->method ( 'getRequired' )->willReturn ( true );
		$option->expects ( $this->once () )->method ( 'isMultiSelection' )->willReturn ( true );
		$option->expects ( $this->once () )->method ( 'getProduct' )->willReturn ( $product );
		$option->expects ( $this->once () )->method ( 'getValue' )->willReturn ( 4 );
		$option->expects ( $this->once () )->method ( 'getTitle' )->willReturn ( 'Title for option' );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOptionQty' )->willReturn ( [ 
				3 => 5 
		] );
		$priceModel->expects ( $this->once () )->method ( 'getSelectionFinalTotalPrice' )->willReturnSelf ();
		$productType->expects ( $this->once () )->method ( 'prepareForCart' )->willReturn ( [ 
				$productType 
		] );
		$productType->expects ( $this->once () )->method ( 'setParentProductId' )->willReturnSelf ();
		$productType->expects ( $this->any () )->method ( 'addCustomOption' )->willReturnSelf ();
		$productType->expects ( $this->once () )->method ( 'setCartQty' )->willReturnSelf ();
		$productType->expects ( $this->once () )->method ( 'getSelectionId' )->willReturn ( 314 );
		
		$this->priceCurrency->expects ( $this->once () )->method ( 'convert' )->willReturn ( 3.14 );
	...
	public function testPrepareForCartAdvancedStringInResult() {
		...
		$priceModel->expects ( $this->once () )->method ( 'getSelectionFinalTotalPrice' )->willReturnSelf ();
		$productType->expects ( $this->once () )->method ( 'prepareForCart' )->willReturn ( 'string' );
	...
	public function testPrepareForCartAdvancedWithoutSelections() {
		...
		$product->expects ( $this->once () )->method ( 'getTypeInstance' )->willReturn ( $productType );
		$product->expects ( $this->once () )->method ( 'hasData' )->willReturn ( true );
		$product->expects ( $this->any () )->method ( 'getData' )->willReturnCallback ( function ($key) use($optionCollection) {
			$resultValue = null;
			switch ($key) {
				case '_cache_instance_options_collection' :
					$resultValue = $optionCollection;
		...
		$product->expects ( $this->once () )->method ( 'getId' )->willReturn ( 333 );
		$productType->expects ( $this->once () )->method ( 'setStoreFilter' );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOption' )->willReturn ( [ ] );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOptionQty' )->willReturn ( [ 
				3 => 5 
	...
	public function testPrepareForCartAdvancedSelectionsSelectionIdsExists() {
		...
		$selectionCollection->expects ( $this->at ( 0 ) )->method ( 'getItems' )->willReturn ( [ 
				$selection 
		] );
		$selectionCollection->expects ( $this->at ( 1 ) )->method ( 'getItems' )->willReturn ( [ ] );
		$selection->expects ( $this->once () )->method ( 'isSalable' )->willReturn ( false );
		$option->expects ( $this->at ( 3 ) )->method ( 'getId' )->willReturn ( 10 );
		$option->expects ( $this->once () )->method ( 'getRequired' )->willReturn ( true );
		$option->expects ( $this->once () )->method ( 'isMultiSelection' )->willReturn ( true );
	...
	public function testPrepareForCartAdvancedSelectRequiredOptions() {
		...
		$selectionCollection->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$selection 
		] );
		$selection->expects ( $this->once () )->method ( 'isSalable' )->willReturn ( false );
		$option->expects ( $this->at ( 3 ) )->method ( 'getId' )->willReturn ( 3 );
		$option->expects ( $this->once () )->method ( 'getRequired' )->willReturn ( true );
		$option->expects ( $this->once () )->method ( 'isMultiSelection' )->willReturn ( true );
	...
	public function testPrepareForCartAdvancedParentClassReturnString() {
		...
		$product->expects ( $this->at ( 0 ) )->method ( 'getOptions' )->willThrowException ( new LocalizedException ( __ ( $exceptedResult ) ) );
	...
	public function testPrepareForCartAdvancedAllrequiredOption() {
		...
		$product->expects ( $this->any () )->method ( 'getSkipCheckRequiredOption' )->willReturn ( false );
		$product->expects ( $this->once () )->method ( 'getTypeInstance' )->willReturn ( $productType );
		$product->expects ( $this->once () )->method ( 'hasData' )->willReturn ( true );
		$product->expects ( $this->any () )->method ( 'getData' )->willReturnCallback ( function ($key) use($optionCollection) {
			$resultValue = null;
		...
		$optionCollection->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$option 
		] );
		$productType->expects ( $this->once () )->method ( 'setStoreFilter' );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOption' )->willReturn ( [ 
	...
	public function testPrepareForCartAdvancedSpecifyProductOptions() {
		...
		$product->expects ( $this->once () )->method ( 'getSkipCheckRequiredOption' )->willReturn ( true );
		$buyRequest->expects ( $this->once () )->method ( 'getBundleOption' )->willReturn ( [ 
				0,
				'',
				'str' 
	...
	public function testGetIdentities() {
		...
		$optionCollectionMock = $this->getMock ( 'Magento\Bundle\Model\ResourceModel\Option\Collection', [ ], [ ], '', false );
		...
		$optionCollectionMock->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$optionMock 
		] ) );
		$optionMock->expects ( $this->exactly ( 2 ) )->method ( 'getSelections' )->will ( $this->returnValue ( [ 
				$productMock 
		] ) );
	...
	public function testGetSkuWithType() {
		...
		$productMock->expects ( $this->at ( 0 ) )->method ( 'getData' )->with ( 'sku' )->will ( $this->returnValue ( $sku ) );
		$productMock->expects ( $this->at ( 2 ) )->method ( 'getData' )->with ( 'sku_type' )->will ( $this->returnValue ( 'some_data' ) );
	...
	public function testGetSkuWithoutType() {
		...
		$productMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
				'__wakeup',
				'getData',
				'hasCustomOptions',
				'getCustomOption' 
		] )->disableOriginalConstructor ()->getMock ();
		$customOptionMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Configuration\Item\Option' )->setMethods ( [ 
				'getValue',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		$selectionItemMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getSku',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$productMock->expects ( $this->at ( 0 ) )->method ( 'getData' )->with ( 'sku' )->will ( $this->returnValue ( $sku ) );
		$productMock->expects ( $this->at ( 1 ) )->method ( 'getCustomOption' )->with ( 'option_ids' )->will ( $this->returnValue ( false ) );
		$productMock->expects ( $this->at ( 2 ) )->method ( 'getData' )->with ( 'sku_type' )->will ( $this->returnValue ( null ) );
		$productMock->expects ( $this->once () )->method ( 'hasCustomOptions' )->will ( $this->returnValue ( true ) );
		$productMock->expects ( $this->at ( 4 ) )->method ( 'getCustomOption' )->with ( 'bundle_selection_ids' )->will ( $this->returnValue ( $customOptionMock ) );
		$customOptionMock->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( $serializeIds ) );
		$selectionMock = $this->getSelectionsByIdsMock ( $selectionIds, $productMock, 5, 6 );
		$selectionMock->expects ( ($this->any ()) )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$selectionItemMock 
		] ) );
		$selectionItemMock->expects ( $this->any () )->method ( 'getSku' )->will ( $this->returnValue ( $itemSku ) );
	...
	public function testGetWeightWithoutCustomOption() {
		...
		$productMock->expects ( $this->at ( 0 ) )->method ( 'getData' )->with ( 'weight_type' )->will ( $this->returnValue ( true ) );
		$productMock->expects ( $this->at ( 1 ) )->method ( 'getData' )->with ( 'weight' )->will ( $this->returnValue ( $weight ) );
	...
	public function testGetWeightWithCustomOption() {
		...
		$productMock->expects ( $this->at ( 0 ) )->method ( 'getData' )->with ( 'weight_type' )->will ( $this->returnValue ( false ) );
		$productMock->expects ( $this->once () )->method ( 'hasCustomOptions' )->will ( $this->returnValue ( true ) );
		$productMock->expects ( $this->at ( 2 ) )->method ( 'getCustomOption' )->with ( 'bundle_selection_ids' )->will ( $this->returnValue ( $customOptionMock ) );
		$customOptionMock->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $serializeIds ) );
		$selectionMock = $this->getSelectionsByIdsMock ( $selectionIds, $productMock, 3, 4 );
		$selectionMock->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$selectionItemMock 
		] ) );
		$selectionItemMock->expects ( $this->any () )->method ( 'getSelectionId' )->will ( $this->returnValue ( 'id' ) );
		$productMock->expects ( $this->at ( 5 ) )->method ( 'getCustomOption' )->with ( 'selection_qty_' . 'id' )->will ( $this->returnValue ( null ) );
		$selectionItemMock->expects ( $this->once () )->method ( 'getWeight' )->will ( $this->returnValue ( $weight ) );
	...
	public function testGetWeightWithSeveralCustomOption() {
		...
		$qtyOptionMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Configuration\Item\Option' )->setMethods ( [ 
				'getValue',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		$selectionItemMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
		...
		$selectionItemMock->expects ( $this->any () )->method ( 'getSelectionId' )->will ( $this->returnValue ( 'id' ) );
		$productMock->expects ( $this->at ( 5 ) )->method ( 'getCustomOption' )->with ( 'selection_qty_' . 'id' )->will ( $this->returnValue ( $qtyOptionMock ) );
		$qtyOptionMock->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $qtyOption ) );
		$selectionItemMock->expects ( $this->once () )->method ( 'getWeight' )->will ( $this->returnValue ( $weight ) );
	...
	public function testIsVirtualWithoutCustomOption() {
		...
		$productMock->expects ( $this->once () )->method ( 'hasCustomOptions' )->will ( $this->returnValue ( false ) );
	...
	public function testIsVirtual() {
		...
		$productMock->expects ( $this->once () )->method ( 'hasCustomOptions' )->will ( $this->returnValue ( true ) );
		$productMock->expects ( $this->once () )->method ( 'getCustomOption' )->with ( 'bundle_selection_ids' )->will ( $this->returnValue ( $customOptionMock ) );
		$customOptionMock->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $serializeIds ) );
		$selectionMock = $this->getSelectionsByIdsMock ( $selectionIds, $productMock, 2, 3 );
		$selectionMock->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$selectionItemMock 
		] ) );
		$selectionItemMock->expects ( $this->once () )->method ( 'isVirtual' )->will ( $this->returnValue ( true ) );
		$selectionItemMock->expects ( $this->once () )->method ( 'isVirtual' )->will ( $this->returnValue ( true ) );
		$selectionMock->expects ( $this->once () )->method ( 'count' )->will ( $this->returnValue ( 1 ) );
	...
	protected function getSelectionsByIdsMock($selectionIds, $productMock, $getSelectionsIndex, $getSelectionsIdsIndex) {
		$usedSelectionsMock = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Selection\Collection' )->disableOriginalConstructor ()->getMock ();
		
		$productMock->expects ( $this->at ( $getSelectionsIndex ) )->method ( 'getData' )->with ( '_cache_instance_used_selections' )->will ( $this->returnValue ( $usedSelectionsMock ) );
		$productMock->expects ( $this->at ( $getSelectionsIdsIndex ) )->method ( 'getData' )->with ( '_cache_instance_used_selections_ids' )->will ( $this->returnValue ( $selectionIds ) );
	 ...
	 * @param int $firstId        	
	 * @param int $secondId        	
	 * @return void @dataProvider shakeSelectionsDataProvider
	 */
	public function testShakeSelections($expected, $firstId, $secondId) {
		$firstItemMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
				'__wakeup',
				'getOption',
				'getOptionId',
				'getPosition',
		...
		$secondItemMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->setMethods ( [ 
				'__wakeup',
				'getOption',
				'getOptionId',
				'getPosition',
		...
		$optionFirstMock = $this->getMockBuilder ( 'Magento\Bundle\Model\Option' )->setMethods ( [ 
				'getPosition',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		$optionSecondMock = $this->getMockBuilder ( 'Magento\Bundle\Model\Option' )->setMethods ( [ 
				'getPosition',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$firstItemMock->expects ( $this->once () )->method ( 'getOption' )->will ( $this->returnValue ( $optionFirstMock ) );
		$optionFirstMock->expects ( $this->once () )->method ( 'getPosition' )->will ( $this->returnValue ( 'option_position' ) );
		$firstItemMock->expects ( $this->once () )->method ( 'getOptionId' )->will ( $this->returnValue ( 'option_id' ) );
		$firstItemMock->expects ( $this->once () )->method ( 'getPosition' )->will ( $this->returnValue ( 'position' ) );
		$firstItemMock->expects ( $this->once () )->method ( 'getSelectionId' )->will ( $this->returnValue ( $firstId ) );
		$secondItemMock->expects ( $this->once () )->method ( 'getOption' )->will ( $this->returnValue ( $optionSecondMock ) );
		$optionSecondMock->expects ( $this->any () )->method ( 'getPosition' )->will ( $this->returnValue ( 'option_position' ) );
		$secondItemMock->expects ( $this->once () )->method ( 'getOptionId' )->will ( $this->returnValue ( 'option_id' ) );
		$secondItemMock->expects ( $this->once () )->method ( 'getPosition' )->will ( $this->returnValue ( 'position' ) );
		$secondItemMock->expects ( $this->once () )->method ( 'getSelectionId' )->will ( $this->returnValue ( $secondId ) );
	...
	public function testGetSelectionsByIds() {
		...
		$productMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->getMock ();
		$usedSelectionsMock = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Selection\Collection' )->setMethods ( [ 
		...
		$storeMock = $this->getMockBuilder ( 'Magento\Store\Model\Store' )->setMethods ( [ 
				'getWebsiteId',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$usedSelectionsMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( $usedSelectionsIds );
	...
	public function testGetOptionsByIds() {
		...
		$storeId = 2;
		$productMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->getMock ();
		$usedOptionsMock = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Option\Collection' )->setMethods ( [ 
				'getResourceCollection' 
		] )->disableOriginalConstructor ()->getMock ();
		$resourceClassName = 'Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection';
		$dbResourceMock = $this->getMockBuilder ( $resourceClassName )->setMethods ( [ 
				'setProductIdFilter',
				'setPositionOrder',
				'joinValues',
				'setIdFilter' 
	...
	public function testIsSalableWithoutOptions() {
		$optionCollectionMock = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\Option\Collection' )->disableOriginalConstructor ()->getMock ();
	...
	public function testIsSalableWithRequiredOptionsTrue() {
		...
		$option3 = $this->getMockBuilder ( 'Magento\Bundle\Model\Option' )->setMethods ( [ 
				'getRequired',
				'getOptionId',
				'getId' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testIsSalableWithRequiredOptionsOutOfStock() {
		$option1 = $this->getRequiredOptionMock ( 10, 10 );
		$option1->expects ( $this->atLeastOnce () )->method ( 'getSelectionCanChangeQty' )->willReturn ( false );
		
		$option2 = $this->getRequiredOptionMock ( 20, 10 );
		$option2->expects ( $this->atLeastOnce () )->method ( 'getSelectionCanChangeQty' )->willReturn ( false );
		
		$this->stockRegistry->method ( 'getStockItem' )->willReturn ( $this->getStockItem ( true ) );
		$this->stockState->method ( 'getStockQty' )->will ( $this->returnValueMap ( [ 
				[ 
						10,
						10 
				],
	...
	private function getRequiredOptionMock($id, $selectionQty) {
		$option = $this->getMockBuilder ( 'Magento\Bundle\Model\Option' )->setMethods ( [ 
				'getRequired',
				'isSalable',
				'hasSelectionQty',
				'getSelectionQty',
	...
	private function getSelectionCollectionMock(array $selectedOptions) {
		$selectionCollectionMock = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\Selection\Collection' )->setMethods ( [ 
				'getItems',
				'getIterator' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$selectionCollectionMock->expects ( $this->any () )->method ( 'getIterator' )->willReturn ( new \ArrayIterator ( $selectedOptions ) );
	...
	private function getOptionCollectionMock(array $options) {
		...
		$optionCollectionMock = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\Option\Collection' )->setMethods ( [ 
				'getItems',
				'getAllIds' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$optionCollectionMock->expects ( $this->any () )->method ( 'getItems' )->willReturn ( $options );
		
		$optionCollectionMock->expects ( $this->any () )->method ( 'getAllIds' )->willReturn ( $ids );
	...
	protected function getStockItem($isManageStock) {
		$result = $this->getMockBuilder ( 'Magento\CatalogInventory\Api\Data\StockItemInterface' )->getMock ();
	...
	protected function parentClass($group, $option, $buyRequest, $product) {
		...
		$product->expects ( $this->once () )->method ( 'getOptions' )->willReturn ( [ 
				$option 
		] );
	...
	public function testSave() {
		...
		$option = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Option\Collection' )->setMethods ( [ 
				'setData',
				'setParentId',
				'setStoreId',
				'isDeleted',
		...
		$selection = $this->getMockBuilder ( 'Magento\Bundle\Model\Selection' )->setMethods ( [ 
				'setData',
				'setOptionId',
				'setParentProductId',
				'setWebsiteId',
		...
		$store = $this->getMockBuilder ( 'Magento\Store\Model\Store' )->setMethods ( [ 
				'getWebsiteId',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testGetOptionsCollection() {
		$product = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->setMethods ( [ 
				'_wakeup',
				'getStoreId',
				'getData',
				'hasData',
		...
		$store = $this->getMockBuilder ( '\Magento\Store\Model\Store' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId' 
		] )->getMock ();
	...
	public function testGetSelectionsCollection() {
		...
		$selectionCollection = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\Selection\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'addAttributeToSelect',
				'setFlag',
				'setPositionOrder',
				'addStoreFilter',
	...
	public function testProcessBuyRequest() {
		...
		$buyRequest = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->setMethods ( [ 
				'getBundleOption',
				'getBundleOptionQty' 
		] )->getMock ();
	...
	public function testGetProductsToPurchaseByReqGroups() {
		...
		$item = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'getRequired' 
		] )->getMock ();
		...
		$product->expects ( $this->at ( 1 ) )->method ( 'getData' )->with ( '_cache_instance_options_collection' )->willReturn ( $dbResourceMock );
		$dbResourceMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$item 
		] );
		$item->expects ( $this->once () )->method ( 'getId' )->willReturn ( 'itemId' );
		$product->expects ( $this->at ( 3 ) )->method ( 'getData' )->with ( '_cache_instance_selections_collectionitemId' )->willReturn ( [ 
				$selectionCollection 
		] );
		...
		], $this->model->getProductsToPurchaseByReqGroups ( $product ) );
	}
	public function testGetSearchableData() {
		...
		$option = $this->getMockBuilder ( '\Magento\Bundle\Model\Option' )->disableOriginalConstructor ()->setMethods ( [ 
				'getSearchableData' 
		] )->getMock ();
		...
		$this->bundleOptionFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $option );
		$option->expects ( $this->once () )->method ( 'getSearchableData' )->willReturn ( [ 
				'optionSearchdata' 
		] );
		
		$this->assertEquals ( [ 
				'optionSearchdata' 
		], $this->model->getSearchableData ( $product ) );
	}
	public function testHasOptions() {
		...
		$optionCollection = $this->getMockBuilder ( 'Magento\Bundle\Model\ResourceModel\Option\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'getAllIds' 
		] )->getMock ();
		$selectionCollection = $this->getMockBuilder ( '\Magento\Bundle\Model\ResourceModel\Selection\Collection' )->disableOriginalConstructor ()->getMock ();
		...
		$optionCollection->expects ( $this->once () )->method ( 'getAllIds' )->willReturn ( [ 
				'ids' 
		] );
		$product->expects ( $this->at ( 5 ) )->method ( 'getData' )->with ( '_cache_instance_selections_collectionids' )->willReturn ( [ 
				$selectionCollection 
		] );
