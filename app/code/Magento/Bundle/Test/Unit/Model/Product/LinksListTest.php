...
class LinksListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->linkFactoryMock = $this->getMock ( 'Magento\Bundle\Api\Data\LinkInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->dataObjectHelperMock = $this->getMockBuilder ( '\Magento\Framework\Api\DataObjectHelper' )->disableOriginalConstructor ()->getMock ();
		$this->selectionMock = $this->getMock ( 'Magento\Catalog\Model\Product', [ 
	...
	public function testLinksList() {
		$optionId = 665;
		$selectionId = 1345;
		$this->productTypeMock->expects ( $this->once () )->method ( 'getSelectionsCollection' )->with ( [ 
				$optionId 
		], $this->productMock )->willReturn ( [ 
				$this->selectionMock 
		] );
		...
		$this->selectionMock->expects ( $this->once () )->method ( 'getData' )->willReturn ( [ 
				'some data' 
		] );
		...
		$this->dataObjectHelperMock->expects ( $this->once () )->method ( 'populateWithArray' )->with ( $linkMock, [ 
				'some data' 
		], '\Magento\Bundle\Api\Data\LinkInterface' )->willReturnSelf ();
		...
		$this->assertEquals ( [ 
				$linkMock 
		], $this->model->getItems ( $this->productMock, $optionId ) );
	}
}
