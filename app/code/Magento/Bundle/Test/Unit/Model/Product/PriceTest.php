...
class PriceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->ruleFactoryMock = $this->getMock ( 'Magento\CatalogRule\Model\ResourceModel\RuleFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$tpFactory = $this->getMock ( 'Magento\Catalog\Api\Data\ProductTierPriceInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$scopeConfig = $this->getMock ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		
		$objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $objectManagerHelper->getObject ( 'Magento\Bundle\Model\Product\Price', [ 
	...
	public function testGetTotalBundleItemsPriceWithNoCustomOptions() {
		...
		$productMock->expects ( $this->once () )->method ( 'hasCustomOptions' )->willReturn ( false );
	...
	public function testGetTotalBundleItemsPriceWithEmptyOptions($value) {
		$dataObjectMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getValue' 
		...
		$dataObjectMock->expects ( $this->once () )->method ( 'getValue' )->willReturn ( $value );
	...
	public function testGetTotalBundleItemsPriceWithNoItems() {
		...
		$productTypeMock->expects ( $this->once () )->method ( 'getSelectionsByIds' )->with ( [ 
				1 
		], $productMock )->willReturn ( $selectionsMock );
		...
		$this->eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( 'prepare_catalog_product_collection_prices', [ 
				'collection' => $selectionsMock,
				'store_id' => $storeId 
		] )->willReturnSelf ();
