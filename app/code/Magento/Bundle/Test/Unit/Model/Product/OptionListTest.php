...
class OptionListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->optionFactoryMock = $this->getMock ( '\Magento\Bundle\Api\Data\OptionInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->extensionAttributesFactoryMock = $this->getMock ( '\Magento\Framework\Api\ExtensionAttribute\JoinProcessorInterface', [ ], [ ], '', false );
		
		$this->objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $this->objectManager->getObject ( 'Magento\Bundle\Model\Product\OptionList', [ 
				'type' => $this->typeMock,
	...
	public function testGetItems() {
		...
		$optionsCollMock = $this->objectManager->getCollectionMock ( 'Magento\Bundle\Model\ResourceModel\Option\Collection', [ 
				$optionMock 
		] );
		...
		$this->linkListMock->expects ( $this->once () )->method ( 'getItems' )->with ( $productMock, $optionId )->willReturn ( [ 
				$linkMock 
		] );
		...
		$newOptionMock->expects ( $this->once () )->method ( 'setProductLinks' )->with ( [ 
				$linkMock 
		] )->willReturnSelf ();
