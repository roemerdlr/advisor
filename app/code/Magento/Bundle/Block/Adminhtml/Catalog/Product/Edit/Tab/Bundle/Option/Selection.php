...
class Selection extends \Magento\Backend\Block\Widget {
	...
	protected function _prepareLayout() {
		$this->addChild ( 'selection_delete_button', 'Magento\Backend\Block\Widget\Button', [ 
				'label' => __ ( 'Delete' ),
				'class' => 'action-delete',
	...
	public function getPriceTypeSelectHtml() {
		...
		] )->setName ( $this->getFieldName () . '[<%- data.parentIndex %>][<%- data.index %>][selection_price_type]' )->setOptions ( $this->_priceType->toOptionArray () );
	...
	public function getQtyTypeSelectHtml() {
		$select = $this->getLayout ()->createBlock ( 'Magento\Framework\View\Element\Html\Select' )->setData ( [ 
				'id' => $this->getFieldId () . '_<%- data.index %>_can_change_qty',
				'class' => 'select' 
		] )->setName ( $this->getFieldName () . '[<%- data.parentIndex %>][<%- data.index %>][selection_can_change_qty]' )->setOptions ( $this->_yesno->toOptionArray () );
	...
	public function getCheckboxScopeHtml() {
		...
		if ($this->isUsedWebsitePrice ()) {
			...
			$checkboxHtml = '<input type="checkbox" id="' . $fieldsId . '" class="' . $class . '" name="' . $name . '"' . $disabled . ' value="1" />';
