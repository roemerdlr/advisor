...
class Collection extends \Magento\Review\Model\ResourceModel\Review\Collection {
	...
	protected function _joinCustomers() {
		...
		$customerFullname = $connection->getConcatSql ( [ 
				'customer.firstname',
				'customer.lastname' 
		], ' ' );
		$this->getSelect ()->reset ( \Magento\Framework\DB\Select::COLUMNS )->joinInner ( [ 
				'customer' => $connection->getTableName ( 'customer_entity' ) 
		], 'customer.entity_id = detail.customer_id', [ ] )->columns ( [ 
