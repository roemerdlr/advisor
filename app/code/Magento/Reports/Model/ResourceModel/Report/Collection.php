...
class Collection extends \Magento\Framework\Data\Collection {
	...
	protected function _getDayInterval(\DateTime $dateStart) {
		$interval = [ 
				'period' => $this->_localeDate->formatDateTime ( $dateStart, \IntlDateFormatter::SHORT, \IntlDateFormatter::NONE ),
	...
	protected function _getMonthInterval(\DateTime $dateStart, \DateTime $dateEnd, $firstInterval) {
		...
		if ($dateStart->diff ( $dateEnd )->m == 0) {
			$interval ['end'] = $dateStart->setDate ( $dateStart->format ( 'Y' ), $dateStart->format ( 'm' ), $dateEnd->format ( 'd' ) )->format ( 'Y-m-d 23:59:59' );
		} else {
	...
	protected function _getYearInterval(\DateTime $dateStart, \DateTime $dateEnd, $firstInterval) {
		...
		$interval ['start'] = $firstInterval ? $dateStart->format ( 'Y-m-d 00:00:00' ) : $dateStart->format ( 'Y-01-01 00:00:00' );
		
		$interval ['end'] = $dateStart->diff ( $dateEnd )->y == 0 ? $dateStart->setDate ( $dateStart->format ( 'Y' ), $dateEnd->format ( 'm' ), $dateEnd->format ( 'd' ) )->format ( 'Y-m-d 23:59:59' ) : $dateStart->format ( 'Y-12-31 23:59:59' );
