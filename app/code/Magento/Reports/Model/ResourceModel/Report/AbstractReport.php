...
abstract class AbstractReport extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 * @return $this
	 */
	protected function _clearTableByDateRange($table, $from = null, $to = null, $subSelect = null, $doNotUseTruncate = false, $connection = null) {
	 ...
	 */
	protected function _getTableDateRangeSelect($table, $column, $whereColumn, $from = null, $to = null, $additionalWhere = [], $alias = 'date_range_table') {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( [ 
				$alias => $table 
		], $connection->getDatePartSql ( $this->getStoreTZOffsetQuery ( [ 
				$alias => $table 
		], $alias . '.' . $column, $from, $to ) ) )->distinct ( true );
	...
	protected function _makeConditionFromDateRangeSelect($select, $periodColumn, $connection = null) {
		...
		foreach ( $selectResult as $date ) {
			$whereCondition [] = $connection->prepareSqlCondition ( $periodColumn, [ 
					'like' => $date 
			] );
		}
	 ...
	 * @return \Magento\Framework\DB\Select @SuppressWarnings(PHPMD.ExcessiveParameterList)
	 */
	protected function _getTableDateRangeRelatedSelect($table, $relatedTable, $joinCondition, $column, $whereColumn, $from = null, $to = null, $additionalWhere = [], $alias = 'date_range_table', $relatedAlias = 'related_date_range_table') {
		$connection = $this->getConnection ();
		$joinConditionSql = [ ];
		...
		$select = $connection->select ()->from ( [ 
				$alias => $table 
		], $connection->getDatePartSql ( $connection->quoteIdentifier ( $alias . '.' . $column ) ) )->joinInner ( [ 
				$relatedAlias => $relatedTable 
		], implode ( ' AND ', $joinConditionSql ), [ ] )->distinct ( true );
		...
		if (! empty ( $additionalWhere )) {
			foreach ( $additionalWhere as $condition ) {
				...
				$condition = str_replace ( [ 
						...
						$connection->quoteIdentifier ( $relatedAlias ) 
				], $condition );
				$select->where ( $condition );
			}
		}
	...
	public function getStoreTZOffsetQuery($table, $column, $from = null, $to = null, $store = null, $connection = null) {
		...
		if (null === $from) {
			$selectOldest = $connection->select ()->from ( $table, [ 
					"MIN({$column})" 
			] );
	...
	protected function _getTZOffsetTransitions($timezone, $from = null, $to = null) {
		...
		try {
			if (! empty ( $from )) {
				$from = $from instanceof \DateTime ? $from->getTimestamp () : (new \DateTime ( $from ))->getTimestamp ();
			}
			
			$to = $to instanceof \DateTime ? $to : new \DateTime ( $to );
			$nextPeriod = $this->getConnection ()->formatDate ( $to->format ( 'Y-m-d H:i:s' ) );
			$to = $to->getTimestamp ();
			...
			for($i = count ( $transitions ) - 1; $i >= 0; $i --) {
				...
				$tr ['time'] = $this->getConnection ()->formatDate ( (new \DateTime ( $tr ['time'] ))->format ( 'Y-m-d H:i:s' ) );
				$tzTransitions [$tr ['offset']] [] = [ 
						'from' => $tr ['time'],
						'to' => $nextPeriod 
				];
