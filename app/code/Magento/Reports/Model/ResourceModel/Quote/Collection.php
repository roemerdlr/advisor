...
class Collection extends \Magento\Quote\Model\ResourceModel\Quote\Collection {
	...
	public function prepareForAbandonedReport($storeIds, $filter = null) {
		$this->addFieldToFilter ( 'items_count', [ 
				'neq' => '0' 
		] )->addFieldToFilter ( 'main_table.is_active', '1' )->addFieldToFilter ( 'main_table.customer_id', [ 
				'neq' => null 
		] )->addSubtotal ( $storeIds, $filter )->setOrder ( 'updated_at' );
	...
	public function addCustomerData($filter = null) {
		...
		if (isset ( $filter ['customer_name'] )) {
			$customerName = $this->customerResource->getConnection ()->getConcatSql ( [ 
					'customer.firstname',
					'customer.lastname' 
			], ' ' );
	...
	public function addSubtotal($storeIds = '', $filter = null) {
		if (is_array ( $storeIds )) {
			...
			$this->getSelect ()->columns ( [ 
					'subtotal' => 'main_table.base_subtotal_with_discount' 
			] );
		...
		if ($filter && is_array ( $filter ) && isset ( $filter ['subtotal'] )) {
			if (isset ( $filter ['subtotal'] ['from'] )) {
				$this->getSelect ()->where ( $this->_joinedFields ['subtotal'] . ' >= ?', $filter ['subtotal'] ['from'], \Zend_Db::FLOAT_TYPE );
			}
			if (isset ( $filter ['subtotal'] ['to'] )) {
				$this->getSelect ()->where ( $this->_joinedFields ['subtotal'] . ' <= ?', $filter ['subtotal'] ['to'], \Zend_Db::FLOAT_TYPE );
			}
		}
