...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function prepareActiveCartItems() {
		...
		$quoteItemsSelect->reset ()->from ( [ 
				'main_table' => $this->getTable ( 'quote_item' ) 
		], '' )->columns ( 'main_table.product_id' )->columns ( [ 
				'carts' => new \Zend_Db_Expr ( 'COUNT(main_table.item_id)' ) 
		] )->columns ( 'quote.base_to_global_rate' )->joinInner ( [ 
				'quote' => $this->getTable ( 'quote' ) 
		], 'main_table.quote_id = quote.entity_id', null )->where ( 'quote.is_active = ?', 1 )->group ( 'main_table.product_id' );
	...
	protected function getOrdersData(array $productIds) {
		...
		] )->where ( 'oi.product_id IN (?)', $productIds )->group ( 'oi.product_id' );
	...
	public function getSelectCountSql() {
		...
		$countSelect->reset ( \Magento\Framework\DB\Select::COLUMNS )->reset ( \Magento\Framework\DB\Select::GROUP )->columns ( 'COUNT(DISTINCT main_table.product_id)' );
		return $countSelect;
	}
	...
	protected function getProductData(array $productIds) {
		...
		] )->useStraightJoin ( true )->joinInner ( [ 
				'product_name' => $productAttrName->getBackend ()->getTable () 
		], 'product_name.entity_id = main_table.entity_id' . ' AND product_name.attribute_id = ' . $productAttrNameId . ' AND product_name.store_id = ' . \Magento\Store\Model\Store::DEFAULT_STORE_ID, [ 
