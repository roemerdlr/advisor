...
class Collection extends \Magento\Sales\Model\ResourceModel\Order\Collection {
	...
	public function checkIsLive($range) {
		$this->_isLive = ( bool ) (! $this->_scopeConfig->getValue ( 'sales/dashboard/use_aggregated_data', \Magento\Store\Model\ScopeInterface::SCOPE_STORE ));
		return $this;
	}
	...
	protected function _getSalesAmountExpression() {
		if (null === $this->_salesAmountExpression) {
			...
			$this->_eventManager->dispatch ( 'sales_prepare_amount_expression', [ 
					'collection' => $this,
					'expression_object' => $expressionTransferObject 
			] );
			$this->_salesAmountExpression = vsprintf ( $expressionTransferObject->getExpression (), $expressionTransferObject->getArguments () );
		}
	...
	protected function _prepareSummaryLive($range, $customStart, $customEnd, $isFilter = 0) {
		...
		if ($isFilter == 0) {
			$this->getSelect ()->columns ( [ 
					'revenue' => new \Zend_Db_Expr ( sprintf ( 'SUM((%s) * %s)', $expression, $connection->getIfNullSql ( 'main_table.base_to_global_rate', 0 ) ) ) 
			] );
		} else {
			$this->getSelect ()->columns ( [ 
		...
		$tzRangeOffsetExpression = $this->_getTZRangeOffsetExpression ( $range, 'created_at', $dateRange ['from'], $dateRange ['to'] );
		...
		] )->order ( 'range', \Magento\Framework\DB\Select::SQL_ASC )->group ( $tzRangeOffsetExpression );
	...
	protected function _prepareSummaryAggregated($range, $customStart, $customEnd) {
		...
		] )->order ( 'range' )->group ( $rangePeriod );
		
		$this->getSelect ()->where ( $this->_getConditionSql ( 'main_table.period', $this->getDateRange ( $range, $customStart, $customEnd ) ) );
		...
		$this->addFieldToFilter ( 'main_table.order_status', [ 
				'nin' => $statuses 
		] );
	 ...
	 * @param string $tzFrom        	
	 * @param string $tzTo        	
	 * @return string @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	protected function _getTZRangeExpressionForAttribute($range, $attribute, $tzFrom = '+00:00', $tzTo = null) {
		...
		$periodExpr = $connection->getDateAddSql ( $attribute, $tzTo, \Magento\Framework\DB\Adapter\AdapterInterface::INTERVAL_HOUR );
	...
	public function getDateRange($range, $customStart, $customEnd, $returnObjects = false) {
		...
		switch ($range) {
			...
			case '1m' :
				$dateStart->setDate ( $dateStart->format ( 'Y' ), $dateStart->format ( 'm' ), $this->_scopeConfig->getValue ( 'reports/dashboard/mtd_start', \Magento\Store\Model\ScopeInterface::SCOPE_STORE ) );
				break;
	...
	public function calculateSales($isFilter = 0) {
		...
		if ($this->_scopeConfig->getValue ( 'sales/dashboard/use_aggregated_data', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )) {
			$this->setMainTable ( 'sales_order_aggregated_created' );
			$this->removeAllFieldsFromSelect ();
			$averageExpr = $connection->getCheckSql ( 'SUM(main_table.orders_count) > 0', 'SUM(main_table.total_revenue_amount)/SUM(main_table.orders_count)', 0 );
			$this->getSelect ()->columns ( [ 
			...
			$this->getSelect ()->columns ( [ 
					'lifetime' => "SUM({$expr})",
					'average' => "AVG({$expr})" 
			] )->where ( 'main_table.status NOT IN(?)', $statuses )->where ( 'main_table.state NOT IN(?)', [ 
					\Magento\Sales\Model\Order::STATE_NEW,
					\Magento\Sales\Model\Order::STATE_PENDING_PAYMENT 
			] );
		}
	...
	public function setDateRange($fromDate, $toDate) {
		$this->_reset ()->addFieldToFilter ( 'created_at', [ 
				'from' => $fromDate,
				'to' => $toDate 
	...
	public function setStoreIds($storeIds) {
		...
		if ($storeIds) {
			$this->getSelect ()->columns ( [ 
					...
					'profit' => "SUM({$baseSubtotalInvoiced}) " . "+ SUM({$baseDiscountRefunded}) - SUM({$baseSubtotalRefunded}) " . "- SUM({$baseDiscountInvoiced}) - SUM({$baseTotalInvocedCost})" 
			] );
		} else {
			$this->getSelect ()->columns ( [ 
					...
					'profit' => "SUM({$baseSubtotalInvoiced} *  main_table.base_to_global_rate) " . "+ SUM({$baseDiscountRefunded} * main_table.base_to_global_rate) " . "- SUM({$baseSubtotalRefunded} * main_table.base_to_global_rate) " . "- SUM({$baseDiscountInvoiced} * main_table.base_to_global_rate) " . "- SUM({$baseTotalInvocedCost} * main_table.base_to_global_rate)" 
			] );
		}
	...
	public function addSumAvgTotals($storeId = 0) {
		 ...
		 */
		$expr = $this->getTotalsExpression ( $storeId, $this->getConnection ()->getIfNullSql ( 'main_table.base_subtotal_refunded', 0 ), $this->getConnection ()->getIfNullSql ( 'main_table.base_subtotal_canceled', 0 ), $this->getConnection ()->getIfNullSql ( 'main_table.base_discount_canceled', 0 ) );
	 ...
	 * @return string
	 */
	protected function getTotalsExpression($storeId, $baseSubtotalRefunded, $baseSubtotalCanceled, $baseDiscountCanceled) {
		$template = ($storeId != 0) ? 'main_table.base_subtotal - %2$s - %1$s - ABS(main_table.base_discount_amount) - %3$s' : '(main_table.base_subtotal - %1$s - %2$s - ABS(main_table.base_discount_amount) - %3$s) ' . ' * main_table.base_to_global_rate';
