...
class CatalogProductViewObserverTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->productIndexFactoryMock = $this->getMockBuilder ( 'Magento\Reports\Model\Product\Index\ViewedFactory' )->setMethods ( [ 
				'create' 
		...
		$reportEventFactory = $this->getMockBuilder ( 'Magento\Reports\Model\EventFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->productCompFactoryMock = $this->getMockBuilder ( 'Magento\Reports\Model\Product\Index\ComparedFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->eventSaverMock = $this->getMockBuilder ( '\Magento\Reports\Observer\EventSaver' )->disableOriginalConstructor ()->setMethods ( [ 
				'save' 
		] )->getMock ();
		
		$this->observer = $objectManager->getObject ( 'Magento\Reports\Observer\CatalogProductViewObserver', [ 
	...
	protected function getObserverMock($productId) {
		$eventObserverMock = $this->getMockBuilder ( 'Magento\Framework\Event\Observer' )->disableOriginalConstructor ()->getMock ();
