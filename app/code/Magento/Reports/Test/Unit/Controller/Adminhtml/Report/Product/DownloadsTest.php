...
class DownloadsTest extends \Magento\Reports\Test\Unit\Controller\Adminhtml\Report\AbstractControllerTest {
	...
	public function testExecute() {
		...
		$titleMock->expects ( $this->once () )->method ( 'prepend' )->with ( new Phrase ( 'Downloads Report' ) );
		...
		$this->menuBlockMock->expects ( $this->once () )->method ( 'setActive' )->with ( 'Magento_Downloadable::report_products_downloads' );
		...
		$this->breadcrumbsBlockMock->expects ( $this->exactly ( 3 ) )->method ( 'addLink' )->withConsecutive ( [ 
				...
				new Phrase ( 'Downloads' ),
				new Phrase ( 'Downloads' ) 
		] );
		
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( 'Magento\Reports\Block\Adminhtml\Product\Downloads' )->willReturn ( $this->abstractBlockMock );
