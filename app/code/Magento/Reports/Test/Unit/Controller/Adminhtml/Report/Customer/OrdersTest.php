...
class OrdersTest extends \Magento\Reports\Test\Unit\Controller\Adminhtml\Report\AbstractControllerTest {
	...
	protected function setUp() {
		...
		$this->orders = new Orders ( $this->contextMock, $this->fileFactoryMock );
	}
	...
	public function testExecute() {
		...
		$titleMock->expects ( $this->once () )->method ( 'prepend' )->with ( new Phrase ( 'Order Count Report' ) );
		
		$this->viewMock->expects ( $this->any () )->method ( 'getPage' )->willReturn ( new DataObject ( [ 
				'config' => new DataObject ( [ 
						'title' => $titleMock 
				] ) 
		] ) );
		
		$this->menuBlockMock->expects ( $this->once () )->method ( 'setActive' )->with ( 'Magento_Reports::report_customers_orders' );
		...
		$this->breadcrumbsBlockMock->expects ( $this->at ( 2 ) )->method ( 'addLink' )->with ( new Phrase ( 'Customers by Number of Orders' ), new Phrase ( 'Customers by Number of Orders' ) );
		$this->orders->execute ();
	}
}
