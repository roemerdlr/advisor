...
class ViewedTest extends \Magento\Reports\Test\Unit\Controller\Adminhtml\Report\AbstractControllerTest {
	...
	protected function setUp() {
		...
		$responseMock = $this->getMockBuilder ( 'Magento\Framework\App\ResponseInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'setRedirect',
				'sendResponse' 
		] )->getMock ();
		...
		$this->viewed = $objectManager->getObject ( 'Magento\Reports\Controller\Adminhtml\Report\Product\Viewed', [ 
	...
	public function testExecute() {
		...
		$titleMock->expects ( $this->once () )->method ( 'prepend' )->with ( new Phrase ( 'Product Views Report' ) );
		
		$this->viewMock->expects ( $this->once () )->method ( 'getPage' )->willReturn ( new DataObject ( [ 
				'config' => new DataObject ( [ 
						'title' => $titleMock 
				] ) 
		] ) );
		
		$this->menuBlockMock->expects ( $this->once () )->method ( 'setActive' )->with ( 'Magento_Reports::report_products_viewed' );
		
		$this->breadcrumbsBlockMock->expects ( $this->exactly ( 3 ) )->method ( 'addLink' )->withConsecutive ( [ 
				new Phrase ( 'Reports' ),
				new Phrase ( 'Reports' ) 
		], [ 
				new Phrase ( 'Products' ),
				...
				new Phrase ( 'Products Most Viewed Report' ),
				new Phrase ( 'Products Most Viewed Report' ) 
		] );
	...
	public function testExecuteWithException() {
		$errorText = new Phrase ( 'An error occurred while showing the product views report. ' . 'Please review the log and try again.' );
		
		$logMock = $this->getMockBuilder ( 'Psr\Log\LoggerInterface' )->disableOriginalConstructor ()->getMock ();
		
		$sessionMock = $this->getMockBuilder ( 'Magento\Backend\Model\Session' )->setMethods ( [ 
				'setIsUrlNotice' 
		...
		$logMock->expects ( $this->once () )->method ( 'critical' );
		$sessionMock->expects ( $this->once () )->method ( 'setIsUrlNotice' );
		
		$this->menuBlockMock->expects ( $this->once () )->method ( 'setActive' )->willThrowException ( new \Exception () );
