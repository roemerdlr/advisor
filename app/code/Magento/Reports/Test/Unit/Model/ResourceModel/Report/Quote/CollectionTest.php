...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetSelectCountSql() {
		...
		$constructArgs = $this->objectManager->getConstructArguments ( 'Magento\Reports\Model\ResourceModel\Quote\Collection' );
		$collection = $this->getMock ( 'Magento\Reports\Model\ResourceModel\Quote\Collection', [ 
				'getSelect' 
		], $constructArgs, '', false );
		
		$collection->expects ( $this->once () )->method ( 'getSelect' )->willReturn ( $this->selectMock );
		$this->selectMock->expects ( $this->atLeastOnce () )->method ( 'reset' )->willReturnSelf ();
		$this->selectMock->expects ( $this->once () )->method ( 'columns' )->with ( 'COUNT(*)' )->willReturnSelf ();
	...
	public function testPrepareActiveCartItems() {
		...
		$collection = $this->getMock ( 'Magento\Reports\Model\ResourceModel\Quote\Item\Collection', [ 
				'getSelect',
				'getTable' 
		], $constructArgs, '', false );
	...
	public function testLoadWithFilter() {
		...
		$productResourceMock->expects ( $this->any () )->method ( 'getAttribute' )->willReturnMap ( [ 
				[ 
						'name',
		...
		$connectionMock->expects ( $this->once () )->method ( 'fetchAssoc' )->willReturn ( [ 
				1,
				2,
				3 
		] );
