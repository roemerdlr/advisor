...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->entitySnapshotMock = $this->getMockBuilder ( $snapshotClassName )->disableOriginalConstructor ()->getMock ();
		...
		$this->configMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Config' )->disableOriginalConstructor ()->getMock ();
		$this->orderFactoryMock = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Report\OrderFactory' )->setMethods ( [ 
				'create' 
		...
		$this->selectMock->expects ( $this->any () )->method ( 'order' )->willReturnSelf ();
		$this->selectMock->expects ( $this->any () )->method ( 'group' )->willReturnSelf ();
		$this->selectMock->expects ( $this->any () )->method ( 'getPart' )->willReturn ( [ ] );
		
		$this->connectionMock = $this->getMockBuilder ( 'Magento\Framework\DB\Adapter\Pdo\Mysql' )->setMethods ( [ 
				'select',
				'getIfNullSql',
				'getDateFormatSql',
				'prepareSqlCondition',
		...
		$this->resourceMock = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb' )->disableOriginalConstructor ()->getMock ();
		$this->resourceMock->expects ( $this->once () )->method ( 'getConnection' )->willReturn ( $this->connectionMock );
		
		$this->collection = new Collection ( $this->entityFactoryMock, $this->loggerMock, $this->fetchStrategyMock, $this->managerMock, $this->entitySnapshotMock, $this->helperMock, $this->scopeConfigMock, $this->storeManagerMock, $this->timezoneMock, $this->configMock, $this->orderFactoryMock, null, $this->resourceMock );
	}
	...
	public function testCheckIsLive() {
		$range = '';
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( 'sales/dashboard/use_aggregated_data', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
	...
	public function testGetDateRangeSecondPart($range, $customStart, $customEnd, $config) {
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( $config, \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->willReturn ( 1 );
	...
	public function testSetDateRange() {
		...
		$this->connectionMock->expects ( $this->at ( 0 ) )->method ( 'prepareSqlCondition' )->with ( '`created_at`', [ 
				'from' => $fromDate,
				'to' => $toDate 
		] );
	...
	public function testSetStoreIds($storeIds, $parameters) {
		$this->connectionMock->expects ( $this->any () )->method ( 'getIfNullSql' )->willReturn ( 'text' );
	...
	public function storesDataProvider() {
		$firstReturn = [ 
				...
				'profit' => 'SUM(text *  main_table.base_to_global_rate) + SUM(text * main_table.base_to_global_rate) ' . '- SUM(text * main_table.base_to_global_rate) - SUM(text * main_table.base_to_global_rate) ' . '- SUM(text * main_table.base_to_global_rate)' 
		];
