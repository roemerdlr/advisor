...
class ComparedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->visitorMock = $this->getMockBuilder ( 'Magento\Customer\Model\Visitor' )->disableOriginalConstructor ()->getMock ();
		...
		$this->catalogProductHelperMock = $this->getMockBuilder ( 'Magento\Catalog\Helper\Product\Compare' )->disableOriginalConstructor ()->getMock ();
		
		$this->resourceMock = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\AbstractResource' )->disableOriginalConstructor ()->setMethods ( [ 
				'getIdFieldName',
				'_construct',
				'getConnection' 
		] )->getMockForAbstractClass ();
		$this->dbMock = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\AbstractDb' )->disableOriginalConstructor ()->getMock ();
		
		$this->compared = new Compared ( $this->contextMock, $this->registryMock, $this->storeManagerMock, $this->visitorMock, $this->sessionMock, $this->genericMock, $this->visibilityMock, $this->dateTimeMock, $this->catalogProductHelperMock, $this->resourceMock, $this->dbMock );
	}
	...
	public function testGetExcludeProductIds() {
		$collection = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Product\Compare\Item\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'getEntityId' 
		] )->getMock ();
		...
		$this->catalogProductHelperMock->expects ( $this->once () )->method ( 'getItemCollection' )->willReturn ( [ 
				$collection 
		] );
		...
		$this->assertEquals ( [ 
				1,
				2 
		], $this->compared->getExcludeProductIds () );
	}
}
