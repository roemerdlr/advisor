...
class Grid extends \Magento\Backend\Block\Widget\Grid {
	...
	protected function _prepareCollection() {
		...
		if (is_string ( $filter )) {
			...
			if (! isset ( $data ['report_from'] )) {
				...
				$data ['report_from'] = $this->_localeDate->formatDateTime ( $date, \IntlDateFormatter::SHORT, \IntlDateFormatter::NONE );
			}
			
			if (! isset ( $data ['report_to'] )) {
				// getting all reports from 2001 year
				$date = new \DateTime ();
				$data ['report_to'] = $this->_localeDate->formatDateTime ( $date, \IntlDateFormatter::SHORT, \IntlDateFormatter::NONE );
			}
	...
	protected function _prepareFilterButtons() {
		$this->addChild ( 'refresh_button', 'Magento\Backend\Block\Widget\Button', [ 
				'label' => __ ( 'Refresh' ),
