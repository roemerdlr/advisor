...
class StockStateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->objectResult = $this->getMock ( '\Magento\Framework\DataObject', [ ], [ ], '', false );
		
		$this->stockStateProvider = $this->getMock ( 'Magento\CatalogInventory\Model\Spi\StockStateProviderInterface', [ 
				'verifyStock',
				'verifyNotification',
				'checkQty',
				'suggestQty',
		...
		$this->stockRegistryProvider = $this->getMock ( 'Magento\CatalogInventory\Model\Spi\StockRegistryProviderInterface', [ 
				'getStock',
				'getStockItem',
				'getStockStatus' 
		], [ ], '', false );
		...
		$this->stockState = $this->objectManagerHelper->getObject ( '\Magento\CatalogInventory\Model\StockState', [ 
	...
	public function testVerifyStock() {
		$this->assertEquals ( true, $this->stockState->verifyStock ( $this->productId, $this->websiteId ) );
	}
	public function testVerifyNotification() {
		$this->assertEquals ( true, $this->stockState->verifyNotification ( $this->productId, $this->websiteId ) );
	}
	public function testCheckQty() {
		$this->assertEquals ( true, $this->stockState->checkQty ( $this->productId, $this->qty, $this->websiteId ) );
	}
	public function testSuggestQty() {
		$this->assertEquals ( $this->qty, $this->stockState->suggestQty ( $this->productId, $this->qty, $this->websiteId ) );
	}
	...
	public function testCheckQtyIncrements() {
		$this->assertEquals ( $this->objectResult, $this->stockState->checkQtyIncrements ( $this->productId, $this->qty, $this->websiteId ) );
	}
