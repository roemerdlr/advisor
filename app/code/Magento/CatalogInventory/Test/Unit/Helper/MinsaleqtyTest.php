...
class MinsaleqtyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$groupManagement = $this->getMockBuilder ( 'Magento\Customer\Api\GroupManagementInterface' )->setMethods ( [ 
				'getAllCustomersGroup' 
		] )->getMockForAbstractClass ();
		
		$allGroup = $this->getMockBuilder ( 'Magento\Customer\Api\Data\GroupInterface' )->setMethods ( [ 
				'getId' 
		] )->getMockForAbstractClass ();
		...
		$this->minsaleqty = $this->objectManagerHelper->getObject ( 'Magento\CatalogInventory\Helper\Minsaleqty', [ 
	...
	public function testGetConfigValue($customerGroupId, $store, $minSaleQty, $result) {
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( $this->equalTo ( \Magento\CatalogInventory\Model\Configuration::XML_PATH_MIN_SALE_QTY ), $this->equalTo ( \Magento\Store\Model\ScopeInterface::SCOPE_STORE ), $this->equalTo ( $store ) )->will ( $this->returnValue ( $minSaleQty ) );
	...
	public function getConfigValueDataProvider() {
		return [ 
				...
				[ 
						5,
						4,
						'a:1:{i:0;a:2:{s:17:"customer_group_id";i:5;s:12:"min_sale_qty";d:40.10000000000000;}}',
						40.1 
				],
				[ 
						5,
