...
class StockTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->stockRegistryProviderMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\Spi\StockRegistryProviderInterface' )->disableOriginalConstructor ()->getMock ();
		...
		$this->statusFactoryMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\ResourceModel\Stock\StatusFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->stock = new Stock ( $this->storeManagerMock, $this->scopeConfigMock, $this->statusFactoryMock, $this->stockRegistryProviderMock );
	}
	public function testAssignStatusToProduct() {
		...
		$productMock->expects ( $this->any () )->method ( 'getStore' )->willReturn ( $storeMock );
	...
	public function filterProvider() {
		$configMock = $this->getMockBuilder ( 'Magento\Framework\App\Config' )->disableOriginalConstructor ()->getMock ();
		return [ 
				[ 
						$configMock 
				],
	...
	public function testAddIsInStockFilterToCollection() {
		...
		$stockStatusMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\ResourceModel\Stock\Status' )->disableOriginalConstructor ()->setMethods ( [ 
				'addIsInStockFilterToCollection' 
		] )->getMock ();
		$stockStatusMock->expects ( $this->once () )->method ( 'addIsInStockFilterToCollection' )->with ( $collectionMock );
