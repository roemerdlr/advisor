...
class StockStateProviderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->mathDivision = $this->getMock ( '\Magento\Framework\Math\Division', [ 
				'getExactDivision' 
		], [ ], '', false );
		
		$this->localeFormat = $this->getMockForAbstractClass ( '\Magento\Framework\Locale\FormatInterface', [ 
				'getNumber' 
		] );
		...
		$this->objectFactory = $this->getMock ( '\Magento\Framework\DataObject\Factory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->stockStateProvider = $this->objectManagerHelper->getObject ( 'Magento\CatalogInventory\Model\StockStateProvider', [ 
	...
	public function testVerifyStock(StockItemInterface $stockItem, $expectedResult) {
		$this->assertEquals ( $expectedResult, $this->stockStateProvider->verifyStock ( $stockItem ) );
	}
	...
	public function testVerifyNotification(StockItemInterface $stockItem, $expectedResult) {
		$this->assertEquals ( $expectedResult, $this->stockStateProvider->verifyNotification ( $stockItem ) );
	}
	...
	public function testSuggestQty(StockItemInterface $stockItem, $expectedResult) {
		$this->assertEquals ( $expectedResult, $this->stockStateProvider->suggestQty ( $stockItem, $this->qty ) );
	}
	...
	protected function prepareDataForMethod($methodName) {
		...
		foreach ( $this->getVariations () as $variation ) {
			...
			$stockItem->expects ( $this->any () )->method ( 'getIsSaleable' )->willReturn ( $variation ['values'] ['_is_saleable_'] );
			$stockItem->expects ( $this->any () )->method ( 'getOrderedItems' )->willReturn ( $variation ['values'] ['_ordered_items_'] );
			...
			$stockItem->expects ( $this->any () )->method ( 'getData' )->with ( 'stock_qty' )->willReturn ( $variation ['values'] ['_stock_qty_'] );
