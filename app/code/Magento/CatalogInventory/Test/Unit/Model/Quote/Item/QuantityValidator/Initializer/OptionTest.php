...
class OptionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->optionMock = $this->getMock ( 'Magento\Quote\Model\Quote\Item\Option', $optionMethods, [ ], '', false );
		...
		$this->stockItemMock = $this->getMock ( 'Magento\CatalogInventory\Api\Data\StockItem', $stockItemMethods, [ ], '', false );
		$productMethods = [ 
				'getId',
				'__wakeup',
				'getStore' 
		...
		$this->qtyItemListMock = $this->getMock ( 'Magento\CatalogInventory\Model\Quote\Item\QuantityValidator\QuoteItemQtyList', [ ], [ ], '', false );
		$resultMethods = [ 
				'getItemIsQtyDecimal',
		...
		$this->stockState = $this->getMockForAbstractClass ( 'Magento\CatalogInventory\Api\StockStateInterface', [ 
				'checkQuoteItemQty' 
		] );
		
		$this->objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->validator = $this->objectManager->getObject ( 'Magento\CatalogInventory\Model\Quote\Item\QuantityValidator\Initializer\Option', [ 
	...
	public function testInitializeWhenResultIsDecimalGetBackordersMessageHasOptionQtyUpdate() {
		...
		$this->qtyItemListMock->expects ( $this->once () )->method ( 'getQty' )->with ( $this->productId, 'quote_item_id', 'quote_id', $qtyToAdd * $optionValue )->will ( $this->returnValue ( $qtyForCheck ) );
		...
		$this->resultMock->expects ( $this->exactly ( 2 ) )->method ( 'getItemIsQtyDecimal' )->will ( $this->returnValue ( 'is_decimal' ) );
		...
		$this->resultMock->expects ( $this->exactly ( 2 ) )->method ( 'getItemBackorders' )->will ( $this->returnValue ( 'backorders' ) );
