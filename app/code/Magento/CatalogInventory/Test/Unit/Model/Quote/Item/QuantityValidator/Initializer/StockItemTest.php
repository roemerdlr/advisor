...
class StockItemTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->quoteItemQtyList = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\Quote\Item\QuantityValidator\QuoteItemQtyList' )->disableOriginalConstructor ()->getMock ();
		
		$this->typeConfig = $this->getMockBuilder ( 'Magento\Catalog\Model\ProductTypes\ConfigInterface' )->disableOriginalConstructor ()->getMock ();
		
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->stockStateMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Api\StockStateInterface' )->disableOriginalConstructor ()->getMock ();
		$this->model = $objectManagerHelper->getObject ( 'Magento\CatalogInventory\Model\Quote\Item\QuantityValidator\Initializer\StockItem', [ 
	...
	public function testInitializeWithSubitem() {
		...
		$stockItem = $this->getMock ( 'Magento\CatalogInventory\Model\Stock\Item', [ 
				'checkQuoteItemQty',
				'setProductName',
				'setIsChildItem',
				'hasIsChildItem',
		...
		$quoteItem = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item' )->setMethods ( [ 
				'getParentItem',
				'getProduct',
				'getId',
				'getQuoteId',
		...
		$parentItem = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item' )->setMethods ( [ 
				'getQty',
				'setIsQtyDecimal',
				'getProduct',
				'__wakeup' 
		...
		$productTypeInstance = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Type\AbstractType' )->disableOriginalConstructor ()->getMock ();
		...
		$result = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getItemIsQtyDecimal',
		...
		$this->quoteItemQtyList->expects ( $this->any () )->method ( 'getQty' )->with ( 'product_id', 'quote_item_id', 'quote_id', 0 )->will ( $this->returnValue ( 'summary_qty' ) );
		...
		$parentProduct->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $productTypeInstance ) );
		$productTypeInstance->expects ( $this->once () )->method ( 'getForceChildItemQtyChanges' )->with ( $product )->will ( $this->returnValue ( true ) );
	...
	public function testInitializeWithoutSubitem() {
		...
		$stockItem = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\Stock\Item' )->setMethods ( [ 
				'checkQuoteItemQty',
				'setProductName',
				'setIsChildItem',
				'hasIsChildItem',
