...
class StockStatusRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->stockStatusMock = $this->getMockBuilder ( '\Magento\CatalogInventory\Model\Stock\Status' )->disableOriginalConstructor ()->getMock ();
		$this->stockStatusResourceMock = $this->getMockBuilder ( '\Magento\CatalogInventory\Model\ResourceModel\Stock\Status' )->disableOriginalConstructor ()->getMock ();
		$this->stockStatusFactoryMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\Stock\StatusFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->stockStatusCollectionMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Api\Data\StockStatusCollectionInterfaceFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->model = new StockStatusRepository ( $this->stockStatusResourceMock, $this->stockStatusFactoryMock, $this->stockStatusCollectionMock, $this->queryBuilderFactoryMock, $this->mapperMock );
	}
	public function testSave() {
		$this->stockStatusResourceMock->expects ( $this->once () )->method ( 'save' )->with ( $this->stockStatusMock )->willReturnSelf ();
	...
	public function testSaveException() {
		$this->stockStatusResourceMock->expects ( $this->once () )->method ( 'save' )->with ( $this->stockStatusMock )->willThrowException ( new \Exception () );
