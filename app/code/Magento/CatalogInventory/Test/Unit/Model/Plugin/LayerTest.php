...
class LayerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_stockHelperMock = $this->getMock ( '\Magento\CatalogInventory\Helper\Stock', [ ], [ ], '', false );
		
		$this->_model = new \Magento\CatalogInventory\Model\Plugin\Layer ( $this->_stockHelperMock, $this->_scopeConfigMock );
	}
	...
	public function testAddStockStatusDisabledShow() {
		$this->_scopeConfigMock->expects ( $this->once () )->method ( 'isSetFlag' )->with ( 'cataloginventory/options/show_out_of_stock' )->will ( $this->returnValue ( true ) );
