...
class DefaultStockqtyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->stockState = $this->getMock ( 'Magento\CatalogInventory\Api\StockStateInterface', [ ], [ ], '', false );
		$this->stockRegistryMock = $this->getMockBuilder ( 'Magento\CatalogInventory\Api\StockRegistryInterface' )->disableOriginalConstructor ()->getMock ();
		...
		$this->block = $objectManager->getObject ( 'Magento\CatalogInventory\Block\Stockqty\DefaultStockqty', [ 
	...
	public function testGetStockQty($productStockQty, $productId, $websiteId, $dataQty, $expectedQty) {
		...
		if ($dataQty) {
			...
			if ($productId) {
				$this->stockState->expects ( $this->once () )->method ( 'getStockQty' )->with ( $this->equalTo ( $productId ), $this->equalTo ( $websiteId ) )->will ( $this->returnValue ( $productStockQty ) );
			}
		}
