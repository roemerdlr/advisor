...
class StockIndex implements StockIndexInterface {
	 ...
	 */
	public function __construct(StockRegistryProviderInterface $stockRegistryProvider, ProductRepositoryInterface $productRepository, ProductWebsite $productWebsite, ProductType $productType) {
	 ...
	 * @param int $productId        	
	 * @param int $scopeId        	
	 * @return true @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	 ...
	 */
	protected function processChildren($productId, $websiteId, $qty = 0, $status = \Magento\CatalogInventory\Model\Stock\Status::STATUS_IN_STOCK) {
			...
			foreach ( $websitesWithStores as $websiteId => $storeId ) {
				...
				foreach ( $requiredChildrenIds as $groupedChildrenIds ) {
					...
					foreach ( $groupedChildrenIds as $childId ) {
						if (isset ( $childrenStatus [$childId] ) && isset ( $childrenWebsites [$childId] ) && in_array ( $websiteId, $childrenWebsites [$childId] ) && $childrenStatus [$childId] == \Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED && isset ( $childrenStock [$childId] ) && $childrenStock [$childId] == \Magento\CatalogInventory\Model\Stock\Status::STATUS_IN_STOCK) {
							$optionStatus = true;
						}
					}
	...
	protected function getWebsitesWithDefaultStores($websiteId = null) {
		...
		if (! is_null ( $websiteId ) && isset ( $this->websites [$websiteId] )) {
			$websites = [ 
					$websiteId => $this->websites [$websiteId] 
			];
		}
		return $websites;
	}
	...
	protected function getStockStatusResource() {
		if (empty ( $this->stockStatusResource )) {
			$this->stockStatusResource = \Magento\Framework\App\ObjectManager::getInstance ()->get ( 'Magento\CatalogInventory\Model\ResourceModel\Stock\Status' );
		}
