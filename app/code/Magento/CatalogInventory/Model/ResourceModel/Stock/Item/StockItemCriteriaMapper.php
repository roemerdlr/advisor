...
class StockItemCriteriaMapper extends GenericMapper {
	...
	public function mapProductsFilter($products) {
		...
		$this->addFieldToFilter ( 'main_table.product_id', [ 
				'in' => $productIds 
		] );
	}
	...
	public function mapStockStatus($storeId = null) {
		...
		], 'main_table.product_id=status_table.product_id' . ' AND main_table.stock_id=status_table.stock_id' . $this->connection->quoteInto ( ' AND status_table.website_id=?', $websiteId ), [ 
				'stock_status' 
		] );
	}
	...
	public function mapQtyFilter($comparisonMethod, $qty) {
		...
		if (! isset ( $methods [$comparisonMethod] )) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( '%1 is not a correct comparison method.', $comparisonMethod ) );
		}
		$this->addFieldToFilter ( 'main_table.qty', [ 
				$methods [$comparisonMethod] => $qty 
		] );
	}
}
