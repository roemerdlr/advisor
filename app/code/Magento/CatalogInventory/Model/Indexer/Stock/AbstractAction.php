...
abstract class AbstractAction {
	 ...
	 */
	public function __construct(ResourceConnection $resource, \Magento\CatalogInventory\Model\ResourceModel\Indexer\StockFactory $indexerFactory, \Magento\Catalog\Model\Product\Type $catalogProductType, \Magento\Framework\Indexer\CacheContext $cacheContext, \Magento\Framework\Event\ManagerInterface $eventManager) {
	...
	public function getRelationsByChild($childIds) {
		...
		$select = $connection->select ()->from ( $this->_getTable ( 'catalog_product_relation' ), 'parent_id' )->where ( 'child_id IN(?)', $childIds );
	...
	protected function _deleteOldRelations($tableName) {
		...
		], 's.product_id = w.product_id AND s.website_id = w.website_id', [ ] )->where ( 'w.product_id IS NULL' );
	...
	protected function _reindexRows($productIds = []) {
		...
		$select = $connection->select ()->from ( $this->_getTable ( 'catalog_product_entity' ), [ 
				'entity_id',
				'type_id' 
		...
		$select = $connection->select ()->distinct ( true )->from ( $this->_getTable ( 'catalog_category_product' ), [ 
				'category_id' 
