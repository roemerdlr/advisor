...
abstract class AbstractResource extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	protected function _multiplyBunchInsert($ruleIds, $entityIds, $entityType) {
		...
		foreach ( $ruleIds as $ruleId ) {
			foreach ( $entityIds as $entityId ) {
				...
				$count ++;
				if ($count % 1000 == 0) {
					$this->getConnection ()->insertOnDuplicate ( $this->getTable ( $entityInfo ['associations_table'] ), $data, [ 
		...
		$this->getConnection ()->delete ( $this->getTable ( $entityInfo ['associations_table'] ), $this->getConnection ()->quoteInto ( $entityInfo ['rule_id_field'] . ' IN (?) AND ', $ruleIds ) . $this->getConnection ()->quoteInto ( $entityInfo ['entity_id_field'] . ' NOT IN (?)', $entityIds ) );
		return $this;
	}
	...
	public function getAssociatedEntityIds($ruleId, $entityType) {
		...
		$select = $this->getConnection ()->select ()->from ( $this->getTable ( $entityInfo ['associations_table'] ), [ 
	...
	protected function _getAssociatedEntityInfo($entityType) {
		...
		throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'There is no information about associated entity type "%1".', $entityType ) );
	}
}
