...
abstract class AbstractCondition extends \Magento\Framework\DataObject implements ConditionInterface {
	...
	public function asXml() {
		$xml = "<type>" . $this->getType () . "</type>" . "<attribute>" . $this->getAttribute () . "</attribute>" . "<operator>" . $this->getOperator () . "</operator>" . "<value>" . $this->getValue () . "</value>";
		return $xml;
	}
	...
	public function getValue() {
		if ($this->getInputType () == 'date' && ! $this->getIsValueParsed ()) {
			...
			$this->setValue ( (new \DateTime ( $this->getData ( 'value' ) ))->format ( 'Y-m-d H:i:s' ) );
	...
	/**
	 *
	 * @return array|string @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	public function asHtml() {
		$html = $this->getTypeElementHtml () . $this->getAttributeElementHtml () . $this->getOperatorElementHtml () . $this->getValueElementHtml () . $this->getRemoveLinkHtml () . $this->getChooserContainerHtml ();
		return $html;
	}
	...
	public function getValueElement() {
		...
		return $this->getForm ()->addField ( $this->getPrefix () . '__' . $this->getId () . '__value', $this->getValueElementType (), $elementParams )->setRenderer ( $this->getValueElementRenderer () );
	}
	 ...
	 *
	 * @param object|array|int|string|float|bool $validatedValue
