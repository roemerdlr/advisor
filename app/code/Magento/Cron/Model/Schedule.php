...
class Schedule extends \Magento\Framework\Model\AbstractModel {
	...
	public function trySchedule() {
		...
		$match = $this->matchCronExpression ( $e [0], strftime ( '%M', $time ) ) && $this->matchCronExpression ( $e [1], strftime ( '%H', $time ) ) && $this->matchCronExpression ( $e [2], strftime ( '%d', $time ) ) && $this->matchCronExpression ( $e [3], strftime ( '%m', $time ) ) && $this->matchCronExpression ( $e [4], strftime ( '%w', $time ) );
	 ...
	 * @param int $num        	
	 * @return bool
	 * @throws \Magento\Framework\Exception\CronException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function tryLockJob() {
		if ($this->_getResource ()->trySetJobStatusAtomic ( $this->getId (), self::STATUS_RUNNING, self::STATUS_PENDING )) {
