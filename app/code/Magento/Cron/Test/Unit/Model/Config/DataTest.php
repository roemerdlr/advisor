...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetJobs() {
		$reader = $this->getMockBuilder ( 'Magento\Cron\Model\Config\Reader\Xml' )->disableOriginalConstructor ()->getMock ();
		$cache = $this->getMock ( 'Magento\Framework\Config\CacheInterface' );
		$dbReader = $this->getMockBuilder ( 'Magento\Cron\Model\Config\Reader\Db' )->disableOriginalConstructor ()->getMock ();
		...
		$cache->expects ( $this->any () )->method ( 'load' )->with ( $this->equalTo ( 'test_cache_id' ) )->will ( $this->returnValue ( serialize ( $jobs ) ) );
