...
class SaveTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_responseMock = $this->getMock ( 'Magento\Framework\App\Response\Http', [ ], [ ], '', false, false );
		
		$configStructureMock = $this->getMock ( 'Magento\Config\Model\Config\Structure', [ ], [ ], '', false, false );
		$this->_configFactoryMock = $this->getMock ( 'Magento\Config\Model\Config\Factory', [ ], [ ], '', false, false );
		...
		$this->messageManagerMock = $this->getMock ( 'Magento\Framework\Message\Manager', [ 
				'addSuccess',
				'addException' 
		], [ ], '', false, false );
		...
		$this->resultRedirect->expects ( $this->atLeastOnce () )->method ( 'setPath' )->with ( 'adminhtml/system_config/edit' )->willReturnSelf ();
		$resultRedirectFactory = $this->getMockBuilder ( 'Magento\Backend\Model\View\Result\RedirectFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->_sectionCheckerMock = $this->getMock ( 'Magento\Config\Controller\Adminhtml\System\ConfigSectionChecker', [ ], [ ], '', false );
		
		$context = $helper->getObject ( 'Magento\Backend\App\Action\Context', $arguments );
		$this->_controller = $this->getMock ( 'Magento\Config\Controller\Adminhtml\System\Config\Save', [ 
				'deniedAction' 
		], [ 
				$context,
				$configStructureMock,
	...
	public function testIndexActionWithAllowedSection() {
		...
		$this->_configFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'data' => $params 
	...
	public function testIndexActionSaveState() {
		...
		$userMock->expects ( $this->once () )->method ( 'saveExtra' )->with ( [ 
				'configState' => $data 
		] );
		$this->_authMock->expects ( $this->once () )->method ( 'getUser' )->will ( $this->returnValue ( $userMock ) );
		
		$this->_requestMock->expects ( $this->any () )->method ( 'getPost' )->with ( 'config_state' )->will ( $this->returnValue ( $data ) );
		$this->assertEquals ( $this->resultRedirect, $this->_controller->execute () );
	}
	public function testIndexActionGetGroupForSave() {
		...
		$this->_requestMock->expects ( $this->any () )->method ( 'getPost' )->will ( $this->returnValueMap ( $requestPostMap ) );
		$this->_requestMock->expects ( $this->any () )->method ( 'getParam' )->will ( $this->returnValueMap ( $requestParamMap ) );
		$this->_requestMock->expects ( $this->once () )->method ( 'getFiles' )->with ( 'groups' )->will ( $this->returnValue ( $files ) );
