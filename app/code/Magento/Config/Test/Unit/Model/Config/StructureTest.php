...
class StructureTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_flyweightFactory = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\FlyweightFactory', [ ], [ ], '', false );
		$this->_tabIteratorMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Iterator\Tab', [ ], [ ], '', false );
		$this->_structureDataMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Data', [ ], [ ], '', false );
		$this->_scopeDefinerMock = $this->getMock ( 'Magento\Config\Model\Config\ScopeDefiner', [ ], [ ], '', false );
		...
		$this->_structureDataMock->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( $this->_structureData ['config'] ['system'] ) );
		$this->_model = new \Magento\Config\Model\Config\Structure ( $this->_structureDataMock, $this->_tabIteratorMock, $this->_flyweightFactory, $this->_scopeDefinerMock );
	}
	...
	public function testGetTabsBuildsSectionTree() {
		...
		$model = new \Magento\Config\Model\Config\Structure ( $this->_structureDataMock, $this->_tabIteratorMock, $this->_flyweightFactory, $this->_scopeDefinerMock );
	 ...
	 */
	public function testGetElementReturnsEmptyElementIfNotExistingElementIsRequested($path, $expectedType, $expectedId, $expectedPath) {
		$expectedConfig = [ 
				'id' => $expectedId,
				'path' => $expectedPath,
		...
		$this->_flyweightFactory->expects ( $this->once () )->method ( 'create' )->with ( $expectedType )->will ( $this->returnValue ( $elementMock ) );
		$this->assertEquals ( $elementMock, $this->_model->getElement ( $path ) );
	}
	public function emptyElementDataProvider() {
		return [ 
				[ 
						'someSection/group_1/nonexisting_field',
						'field',
						'nonexisting_field',
						'someSection/group_1' 
				],
				[ 
						'section_1/group_1/nonexisting_field',
						'field',
						'nonexisting_field',
						'section_1/group_1' 
				],
				[ 
						'section_1/nonexisting_group',
						'group',
						'nonexisting_group',
						'section_1' 
				],
	...
	public function testGetElementByPathPartsIfSectionDataIsEmpty() {
		...
		$structureDataMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Data', [ ], [ ], '', false );
		
		$structureDataMock->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( [ ] ) );
		
		$structureMock = new \Magento\Config\Model\Config\Structure ( $structureDataMock, $this->_tabIteratorMock, $this->_flyweightFactory, $this->_scopeDefinerMock );
	...
	public function testGetFirstSectionReturnsFirstAllowedSection() {
		$tabMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Tab', [ 
				'current',
				'getChildren',
				'rewind' 
		], [ ], '', false );
