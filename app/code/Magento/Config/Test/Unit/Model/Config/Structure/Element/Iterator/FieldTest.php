...
class FieldTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_fieldMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Field', [ ], [ ], '', false );
		$this->_groupMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false );
		$this->_model = new \Magento\Config\Model\Config\Structure\Element\Iterator\Field ( $this->_groupMock, $this->_fieldMock );
	...
	public function testIteratorInitializesCorrespondingFlyweights() {
		$this->_groupMock->expects ( $this->at ( 0 ) )->method ( 'setData' )->with ( [ 
				'_elementType' => 'group',
				'id' => 'someGroup_1' 
		], 'scope' );
		$this->_groupMock->expects ( $this->at ( 2 ) )->method ( 'setData' )->with ( [ 
				'_elementType' => 'group',
				'id' => 'someGroup_2' 
		], 'scope' );
		$this->_groupMock->expects ( $this->any () )->method ( 'isVisible' )->will ( $this->returnValue ( true ) );
		
		$this->_fieldMock->expects ( $this->at ( 0 ) )->method ( 'setData' )->with ( [ 
				'_elementType' => 'field',
				'id' => 'someField_1' 
		], 'scope' );
		$this->_fieldMock->expects ( $this->at ( 2 ) )->method ( 'setData' )->with ( [ 
				'_elementType' => 'field',
				'id' => 'someField_2' 
		], 'scope' );
