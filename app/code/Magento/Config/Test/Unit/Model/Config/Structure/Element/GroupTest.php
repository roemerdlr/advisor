...
class GroupTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_cloneFactoryMock = $this->getMock ( 'Magento\Config\Model\Config\BackendClone\Factory', [ ], [ ], '', false );
		$this->_depMapperMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Dependency\Mapper', [ ], [ ], '', false );
		
		$this->_model = $objectManager->getObject ( 'Magento\Config\Model\Config\Structure\Element\Group', [ 
	...
	public function testGetCloneModelCreatesCloneModel() {
		...
		$this->_model->setData ( [ 
				'clone_model' => 'clone_model_name' 
		], 'scope' );
		$this->assertEquals ( $cloneModel, $this->_model->getCloneModel () );
	}
	public function testGetFieldsetSetsOnlyNonArrayValuesToFieldset() {
		$fieldsetMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Fieldset', [ 
				'setOriginalData' 
		], [ ], '', false );
		$fieldsetMock->expects ( $this->once () )->method ( 'setOriginalData' )->with ( [ 
				'var1' => 'val1',
				'var2' => 'val2' 
		] );
