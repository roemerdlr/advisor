...
class MapperTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_configStructureMock = $this->getMockBuilder ( 'Magento\Config\Model\Config\Structure' )->setMethods ( [ 
				'getElement' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->_fieldFactoryMock = $this->getMockBuilder ( 'Magento\Config\Model\Config\Structure\Element\Dependency\FieldFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->_scopeConfigMock = $this->getMockBuilder ( '\Magento\Framework\App\Config\ScopeConfigInterface' )->disableOriginalConstructor ()->getMock ();
		$this->_model = new \Magento\Config\Model\Config\Structure\Element\Dependency\Mapper ( $this->_configStructureMock, $this->_fieldFactoryMock, $this->_scopeConfigMock );
	}
	...
	public function testGetDependenciesWhenDependentIsInvisible($isValueSatisfy) {
		...
		for($i = 0; $i < count ( $this->_testData ); ++ $i) {
			...
			$field = $this->_getField ( false, $dependentPath, 'Magento_Backend_Model_Config_Structure_Element_Field_' . ( string ) $isValueSatisfy . $i );
			$this->_configStructureMock->expects ( $this->at ( $i ) )->method ( 'getElement' )->with ( $data ['id'] )->will ( $this->returnValue ( $field ) );
			$dependencyField = $this->_getDependencyField ( $isValueSatisfy, false, $data ['id'], 'Magento_Backend_Model_Config_Structure_Element_Dependency_Field_' . ( string ) $isValueSatisfy . $i );
			$this->_fieldFactoryMock->expects ( $this->at ( $i ) )->method ( 'create' )->with ( [ 
					'fieldData' => $data,
					'fieldPrefix' => self::FIELD_PREFIX 
			] )->will ( $this->returnValue ( $dependencyField ) );
			$this->_scopeConfigMock->expects ( $this->at ( $i ) )->method ( 'getValue' )->with ( $dependentPath, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, self::STORE_CODE )->will ( $this->returnValue ( self::VALUE_IN_STORE ) );
			if (! $isValueSatisfy) {
	...
	protected function _getDependencyField($isValueSatisfy, $isFieldVisible, $fieldId, $mockClassName) {
		$field = $this->getMockBuilder ( 'Magento\Config\Model\Config\Structure\Element\Dependency\Field' )->setMethods ( [ 
				'isValueSatisfy',
				'getId' 
		] )->setMockClassName ( $mockClassName )->disableOriginalConstructor ()->getMock ();
		if ($isFieldVisible) {
			$field->expects ( $isFieldVisible ? $this->never () : $this->once () )->method ( 'isValueSatisfy' );
		} else {
			$field->expects ( $this->once () )->method ( 'isValueSatisfy' )->with ( self::VALUE_IN_STORE )->will ( $this->returnValue ( $isValueSatisfy ) );
		}
		$field->expects ( $isFieldVisible || ! $isValueSatisfy ? $this->once () : $this->never () )->method ( 'getId' )->will ( $this->returnValue ( $fieldId ) );
		return $field;
	}
	...
	protected function _getField($isVisible, $path, $mockClassName) {
		$field = $this->getMockBuilder ( 'Magento\Config\Model\Config\Structure\Element\Field' )->setMethods ( [ 
				'isVisible',
				'getPath' 
		] )->setMockClassName ( $mockClassName )->disableOriginalConstructor ()->getMock ();
		...
		if ($isVisible) {
			...
			$field->expects ( $isVisible ? $this->never () : $this->once () )->method ( 'getPath' )->with ( self::FIELD_PREFIX )->will ( $this->returnValue ( $path ) );
		}
		return $field;
	}
}
