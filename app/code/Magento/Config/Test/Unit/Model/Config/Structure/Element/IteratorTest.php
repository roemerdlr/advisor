...
class IteratorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_flyweightMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false );
	...
	public function testIteratorInitializesFlyweight() {
		$this->_flyweightMock->expects ( $this->at ( 0 ) )->method ( 'setData' )->with ( [ 
				'id' => 1 
		], 'scope' );
		$this->_flyweightMock->expects ( $this->at ( 2 ) )->method ( 'setData' )->with ( [ 
				'id' => 2 
		], 'scope' );
		$this->_flyweightMock->expects ( $this->at ( 4 ) )->method ( 'setData' )->with ( [ 
				'id' => 3 
		], 'scope' );
