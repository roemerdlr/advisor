...
class LoaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_configValueFactory = $this->getMock ( 'Magento\Framework\App\Config\ValueFactory', [ 
				'create',
				'getCollection' 
		], [ ], '', false );
		$this->_model = new \Magento\Config\Model\Config\Loader ( $this->_configValueFactory );
		
		$this->_configCollection = $this->getMock ( 'Magento\Config\Model\ResourceModel\Config\Data\Collection', [ ], [ ], '', false );
		$this->_configCollection->expects ( $this->once () )->method ( 'addScopeFilter' )->with ( 'scope', 'scopeId', 'section' )->will ( $this->returnSelf () );
		...
		$configDataMock->expects ( $this->any () )->method ( 'getCollection' )->will ( $this->returnValue ( $this->_configCollection ) );
