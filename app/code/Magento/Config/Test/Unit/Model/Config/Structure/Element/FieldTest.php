...
class FieldTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_backendFactoryMock = $this->getMock ( 'Magento\Config\Model\Config\BackendFactory', [ ], [ ], '', false );
		$this->_sourceFactoryMock = $this->getMock ( 'Magento\Config\Model\Config\SourceFactory', [ ], [ ], '', false );
		$this->_commentFactoryMock = $this->getMock ( 'Magento\Config\Model\Config\CommentFactory', [ ], [ ], '', false );
		$this->_blockFactoryMock = $this->getMock ( 'Magento\Framework\View\Element\BlockFactory', [ ], [ ], '', false );
		$this->_depMapperMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Dependency\Mapper', [ ], [ ], '', false );
		
		$this->_model = $objectManager->getObject ( 'Magento\Config\Model\Config\Structure\Element\Field', [ 
	...
	public function testGetTooltipCreatesTooltipBlock() {
		$this->_model->setData ( [ 
				'tooltip_block' => 'Magento\Config\Block\Tooltip' 
		], 'scope' );
	...
	public function testGetBackendModelCreatesBackendModel() {
		...
		$this->_model->setData ( [ 
				'backend_model' => 'Magento\Framework\Model\Name' 
		], 'scope' );
	...
	public function testGetSectionId() {
		$this->_model->setData ( [ 
				'id' => 'fieldId',
				'path' => 'sectionId/groupId/subgroupId' 
		], 'scope' );
	...
	public function testPopulateInput() {
		...
		$elementMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Text', [ 
				'setOriginalData' 
		], [ ], '', false );
	...
	public function testGetOptionsUsesOptionsInterfaceIfNoMethodIsProvided() {
		...
		$sourceModelMock->expects ( $this->once () )->method ( 'toOptionArray' )->with ( false )->will ( $this->returnValue ( $expected ) );
		$this->assertEquals ( $expected, $this->_model->getOptions () );
	}
	public function testGetOptionsUsesProvidedMethodOfSourceModel() {
		$this->_model->setData ( [ 
				'source_model' => 'Source_Model_Name::retrieveElements',
				'path' => 'path',
				'type' => 'multiselect' 
		], 'scope' );
		$sourceModelMock = $this->getMock ( 'Magento\Framework\DataObject', [ 
				'setPath',
				'retrieveElements' 
		] );
		$this->_sourceFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( 'Source_Model_Name' )->will ( $this->returnValue ( $sourceModelMock ) );
	...
	public function testGetOptionsParsesResultOfProvidedMethodOfSourceModelIfTypeIsNotMultiselect() {
		...
		$this->_sourceFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( 'Source_Model_Name' )->will ( $this->returnValue ( $sourceModelMock ) );
		$sourceModelMock->expects ( $this->once () )->method ( 'setPath' )->with ( 'path/' );
		$sourceModelMock->expects ( $this->once () )->method ( 'retrieveElements' )->will ( $this->returnValue ( [ 
				'var1' => 'val1',
				'var2' => [ 
						'subvar1' => 'subval1' 
				] 
