...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_structureReaderMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Reader', [ ], [ ], '', false );
		$this->_configStructure = $this->getMock ( 'Magento\Config\Model\Config\Structure', [ ], [ ], '', false );
		
		$this->_structureReaderMock->expects ( $this->any () )->method ( 'getConfiguration' )->will ( $this->returnValue ( $this->_configStructure ) );
		
		$this->_transFactoryMock = $this->getMock ( 'Magento\Framework\DB\TransactionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_appConfigMock = $this->getMock ( 'Magento\Framework\App\Config\ReinitableConfigInterface' );
		$this->_configLoaderMock = $this->getMock ( 'Magento\Config\Model\Config\Loader', [ 
				'getConfigByPath' 
		], [ ], '', false );
		$this->_dataFactoryMock = $this->getMock ( 'Magento\Framework\App\Config\ValueFactory', [ ], [ ], '', false );
		
		$this->_storeManager = $this->getMockForAbstractClass ( 'Magento\Store\Model\StoreManagerInterface' );
		
		$this->_model = new \Magento\Config\Model\Config ( $this->_appConfigMock, $this->_eventManagerMock, $this->_configStructure, $this->_transFactoryMock, $this->_configLoaderMock, $this->_dataFactoryMock, $this->_storeManager );
	}
	...
	public function testSaveToCheckScopeDataSet() {
		...
		$this->_storeManager->expects ( $this->any () )->method ( 'getWebsites' )->will ( $this->returnValue ( [ 
				$website 
		] ) );
		...
		$backendModel = $this->getMock ( 'Magento\Framework\App\Config\Value', [ 
				'setPath',
				'addData',
				'__sleep',
				'__wakeup' 
	...
	public function setDataByPathWrongDepthDataProvider() {
		return [ 
				...
				'depth 4' => [ 
						...
						"Your configuration depth is 4 for path" . " 'section/group/field/sub-field'" 
				] 
		];
	}
}
