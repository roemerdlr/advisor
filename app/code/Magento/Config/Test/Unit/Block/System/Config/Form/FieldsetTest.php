...
class FieldsetTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->authSessionMock = $this->getMockBuilder ( 'Magento\Backend\Model\Auth\Session' )->setMethods ( [ 
				'getUser' 
		...
		$this->_requestMock = $this->getMockBuilder ( 'Magento\Framework\App\RequestInterface' )->disableOriginalConstructor ()->getMock ();
		$this->_requestMock->expects ( $this->any () )->method ( 'getParam' )->willReturn ( 'Test Param' );
		...
		$groupMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false );
		...
		$this->_elementMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Text', [ 
				'getId',
				'getHtmlId',
				'getName',
				'getElements',
		...
		$this->_elementMock->expects ( $this->any () )->method ( 'getId' )->will ( $this->returnValue ( $this->testData ['htmlId'] ) );
		$this->_elementMock->expects ( $this->any () )->method ( 'getHtmlId' )->will ( $this->returnValue ( $this->testData ['htmlId'] ) );
		$this->_elementMock->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( $this->testData ['name'] ) );
		$this->_elementMock->expects ( $this->any () )->method ( 'getLegend' )->will ( $this->returnValue ( $this->testData ['legend'] ) );
		$this->_elementMock->expects ( $this->any () )->method ( 'getComment' )->will ( $this->returnValue ( $this->testData ['comment'] ) );
	}
	 ...
	 * @param
	 *        	$nested
	 * @param $extra @dataProvider
	 *        	testRenderWithStoredElementsDataProvider
	 */
	public function testRenderWithStoredElements($expanded, $nested, $extra) {
		...
		$fieldMock = $this->getMockBuilder ( 'Magento\Framework\Data\Form\Element\Text' )->setMethods ( [ 
				'getId',
				'getTooltip',
				'toHtml',
				'getHtmlId',
		...
		$fieldSetMock = $this->getMockBuilder ( 'Magento\Framework\Data\Form\Element\Fieldset' )->setMethods ( [ 
				'getId',
				'getTooltip',
				'toHtml',
				'getHtmlId',
		...
		$formMock = $this->getMock ( 'Magento\Framework\Data\Form\AbstractForm', [ ], [ 
				$factory,
				$factoryColl 
		] );
