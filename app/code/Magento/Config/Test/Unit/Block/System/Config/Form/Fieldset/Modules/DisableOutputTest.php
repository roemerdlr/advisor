...
class DisableOutputTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->moduleListMock->expects ( $this->any () )->method ( 'getNames' )->will ( $this->returnValue ( [ 
				'Test Name' 
		] ) );
		...
		$groupMock = $this->getMockBuilder ( 'Magento\Config\Model\Config\Structure\Element\Group' )->setMethods ( [ 
				'getFieldsetCss' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$context = $this->objectManager->getObject ( 'Magento\Backend\Block\Context', [ 
		...
		$this->elementMock = $this->getMockBuilder ( 'Magento\Framework\Data\Form\Element\Text' )->setMethods ( [ 
				'getId',
				'getHtmlId',
				'getName',
				'getExpanded',
				...
				'getElements',
				'getIsNested' 
		] )->disableOriginalConstructor ()->disableOriginalClone ()->enableAutoload ()->getMock ();
		...
		$this->elementMock->expects ( $this->any () )->method ( 'getElements' )->will ( $this->returnValue ( [ 
				$this->elementMock 
		] ) );
	}
