...
class AllowspecificTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetAfterElementHtml() {
		$this->_formMock->expects ( $this->exactly ( 2 ) )->method ( 'getHtmlIdPrefix' )->will ( $this->returnValue ( 'test_prefix_' ) );
		$this->_formMock->expects ( $this->exactly ( 2 ) )->method ( 'getHtmlIdSuffix' )->will ( $this->returnValue ( '_test_suffix' ) );
	...
	public function testGetHtmlWhenValueIsEmpty($value) {
		...
		$elementMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Select', [ 
				'setDisabled' 
		], [ ], '', false, false );
		...
		$this->_formMock->expects ( $this->once () )->method ( 'getElement' )->with ( $countryId )->will ( $this->returnValue ( $elementMock ) );
