...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_formFactoryMock = $this->getMock ( 'Magento\Framework\Data\FormFactory', [ 
				'create' 
		], [ ], '', false, false );
		...
		$configFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'data' => [ 
		...
		$this->_backendConfigMock->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( [ 
				'section1/group1/field1' => 'some_value' 
		] ) );
		
		$this->_formMock = $this->getMock ( 'Magento\Framework\Data\Form', [ 
				'setParent',
				'setBaseUrl',
				'addFieldset' 
		], [ ], '', false, false );
		...
		$context = $helper->getObject ( 'Magento\Backend\Block\Template\Context', [ 
		...
		$this->_objectBuilder = $this->getMockBuilder ( 'Magento\Config\Block\System\Config\Form' )->setConstructorArgs ( $objectArguments )->setMethods ( [ 
				'something' 
		] );
	...
	public function testInitForm($sectionIsVisible) {
		...
		$object = $this->_objectBuilder->setMethods ( [ 
				'_initGroup' 
		] )->getMock ();
		...
		if ($sectionIsVisible) {
			...
			$sectionMock->expects ( $this->once () )->method ( 'getChildren' )->willReturn ( [ 
					$this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false, false ) 
			] );
		}
		
		$this->_systemConfigMock->expects ( $this->once () )->method ( 'getElement' )->with ( 'section_code' )->will ( $this->returnValue ( $sectionIsVisible ? $sectionMock : null ) );
		
		if ($sectionIsVisible) {
			$object->expects ( $this->once () )->method ( '_initGroup' );
		} else {
			$object->expects ( $this->never () )->method ( '_initGroup' );
		}
	...
	public function testInitGroup($shouldCloneFields, $prefixes, $callNum) {
		...
		$cloneModelMock = $this->getMock ( 'Magento\Config\Model\Config', [ 
				'getPrefixes' 
		], [ ], '', false, false );
		...
		$sectionMock->expects ( $this->once () )->method ( 'getChildren' )->will ( $this->returnValue ( [ 
				$groupMock 
		] ) );
	...
	public function testInitFields($backendConfigValue, $configValue, $configPath, $inherit, $expectedValue) {
		...
		$this->_backendConfigMock->expects ( $this->once () )->method ( 'extendConfig' )->with ( 'some/config/path', false, [ 
				'section1/group1/field1' => 'some_value' 
		...
		$formFieldMock = $this->getMockForAbstractClass ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ ], '', false, false, true, [ 
				'setRenderer' 
		] );
