...
class Edit extends \Magento\UrlRewrite\Block\Edit {
		...
		if ($this->_getProduct ()->getId ()) {
			if ($this->_getCategory ()->getId () || ! $this->getIsCategoryMode ()) {
				...
				if ($this->_getUrlRewrite ()->getId () === null) {
					...
					$this->_updateBackButtonLink ( $this->_adminhtmlData->getUrl ( 'adminhtml/*/edit', [ 
							'product' => $productId 
					] ) . 'category' );
				}
			} else {
	...
	private function _addProductLinkBlock() {
		$this->addChild ( 'product_link', 'Magento\UrlRewrite\Block\Link', [ 
	...
	private function _addCategoryLinkBlock() {
		$this->addChild ( 'category_link', 'Magento\UrlRewrite\Block\Link', [ 
				'item_url' => $this->_adminhtmlData->getUrl ( 'adminhtml/*/*', [ 
	...
	private function _addSkipCategoriesBlock() {
		$this->addChild ( 'skip_categories', 'Magento\Backend\Block\Widget\Button', [ 
				'label' => __ ( 'Skip Category Selection' ),
				'onclick' => 'window.location = \'' . $this->_adminhtmlData->getUrl ( 'adminhtml/*/*', [ 
						'product' => $this->_getProduct ()->getId () 
				] ) . '\'',
				'class' => 'save',
				'level' => - 1 
	...
	protected function _createEditFormBlock() {
		return $this->getLayout ()->createBlock ( 'Magento\UrlRewrite\Block\Catalog\Edit\Form', '', [ 
				'data' => [ 
