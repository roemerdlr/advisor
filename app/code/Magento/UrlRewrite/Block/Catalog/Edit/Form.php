...
class Form extends \Magento\UrlRewrite\Block\Edit\Form {
	...
	protected function _formPostInit($form) {
		$form->setAction ( $this->_adminhtmlData->getUrl ( 'adminhtml/*/save', [ 
		...
		if (! $model->getId ()) {
			...
			$disablePaths = in_array ( $model->getEntityType (), [ 
	...
	protected function _getEntityStores() {
		...
		if ($product->getId ()) {
			...
			if (! $entityStores) {
				throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'We can\'t set up a URL rewrite because the product you chose is not associated with a website.' ) );
			}
			$this->_requireStoresFilter = true;
		} elseif ($category->getId ()) {
			$entityStores = ( array ) $category->getStoreIds ();
			$message = __ ( 'Please assign a website to the selected category.' );
			if (! $entityStores) {
