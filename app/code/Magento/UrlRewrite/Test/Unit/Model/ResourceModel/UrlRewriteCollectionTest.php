...
class UrlRewriteCollectionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resource = $this->getMockForAbstractClass ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ ], '', false, true, true, [ 
				'getConnection',
				'__wakeup',
				'getMainTable',
				'getTable' 
		...
		$this->collection = (new ObjectManager ( $this ))->getObject ( 'Magento\UrlRewrite\Model\ResourceModel\UrlRewriteCollection', [ 
	...
	public function testAddStoreFilterIfStoreIsArray($storeId, $withAdmin, $condition) {
		$this->connectionMock->expects ( $this->once () )->method ( 'prepareSqlCondition' )->with ( 'store_id', [ 
				'in' => $condition 
		] );
