...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->urlRewriteFactory = $this->getMock ( 'Magento\UrlRewrite\Model\UrlRewriteFactory', [ 
				'create' 
		], [ ], '', false );
		$this->urlRewriteFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->getMock ( 'Magento\UrlRewrite\Model\UrlRewrite', [ ], [ ], '', false ) );
		...
		$this->form = (new ObjectManager ( $this ))->getObject ( 'Magento\UrlRewrite\Block\Catalog\Edit\Form', [ 
				'layout' => $this->layout,
	...
	public function testAddErrorMessageWhenProductWithoutStores() {
		...
		$storeElement = $this->getMock ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ 
				'setAfterElementHtml',
				'setValues' 
		], [ ], '', false );
		$fieldset->expects ( $this->at ( 2 ) )->method ( 'addField' )->with ( 'store_id', 'select', [ 
				'label' => 'Store',
				'title' => 'Store',
				'name' => 'store_id',
				'required' => true,
