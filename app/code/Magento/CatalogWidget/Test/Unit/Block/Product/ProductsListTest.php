...
class ProductsListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->collectionFactory = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Product\CollectionFactory' )->setMethods ( [ 
				'create' 
		...
		$arguments = $objectManagerHelper->getConstructArguments ( 'Magento\CatalogWidget\Block\Product\ProductsList', [ 
	...
	public function testGetProductPriceHtml() {
		...
		$priceRenderer = $this->getMockBuilder ( '\Magento\Framework\Pricing\Render' )->setMethods ( [ 
				'render' 
		] )->disableOriginalConstructor ()->getMock ();
		$priceRenderer->expects ( $this->once () )->method ( 'render' )->with ( 'final_price', $product, [ 
		...
		$this->assertEquals ( '<html>', $this->productsList->getProductPriceHtml ( $product, 'some-price-type', \Magento\Framework\Pricing\Render::ZONE_ITEM_LIST, [ 
				'include_container' => false,
				'display_minimal_price' => false 
		] ) );
	}
	...
	public function testGetPagerHtml() {
		$collection = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Product\Collection' )->setMethods ( [ 
				'getSize' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$pagerBlock = $this->getMockBuilder ( 'Magento\Catalog\Block\Product\Widget\Html\Pager' )->setMethods ( [ 
				'toHtml',
				'setUseContainer',
				'setShowAmounts',
				'setShowPerPage',
	...
	public function testCreateCollection($pagerEnable, $productsCount, $productsPerPage, $expectedPageSize) {
		...
		$collection->expects ( $this->once () )->method ( 'setVisibility' )->with ( [ 
				Visibility::VISIBILITY_IN_CATALOG,
				Visibility::VISIBILITY_BOTH 
		] )->willReturnSelf ();
		...
		$conditions = $this->getMockBuilder ( '\Magento\Rule\Model\Condition\Combine' )->setMethods ( [ 
