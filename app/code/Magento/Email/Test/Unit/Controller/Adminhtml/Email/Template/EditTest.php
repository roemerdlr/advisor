...
class EditTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->registryMock = $this->getMockBuilder ( 'Magento\Framework\Registry' )->disableOriginalConstructor ()->setMethods ( [ 
				'registry',
				'register' 
		] )->getMock ();
		$this->requestMock = $this->getMockBuilder ( 'Magento\Framework\App\Request\Http' )->disableOriginalConstructor ()->getMock ();
		$this->viewMock = $this->getMockBuilder ( 'Magento\Framework\App\View' )->disableOriginalConstructor ()->setMethods ( [ 
				'loadLayout',
				'getLayout',
				'getPage',
				'renderLayout' 
		...
		$this->menuBlockMock = $this->getMockBuilder ( '\Magento\Backend\Block\Menu' )->disableOriginalConstructor ()->setMethods ( [ 
				'setActive',
				'getMenuModel',
				'getParentItems' 
		] )->getMock ();
		$this->breadcrumbsBlockMock = $this->getMockBuilder ( '\Magento\Backend\Block\Widget\Breadcrumbs' )->disableOriginalConstructor ()->setMethods ( [ 
				'addLink' 
		] )->getMock ();
		$this->editBlockMock = $this->getMockBuilder ( '\Magento\Backend\Block\Widget\Breadcrumbs' )->disableOriginalConstructor ()->setMethods ( [ 
				'setEditMode' 
		] )->getMock ();
		...
		$this->editController = $objectManager->getObject ( 'Magento\Email\Controller\Adminhtml\Email\Template\Edit', [ 
	...
	public function testExecuteNewTemplate() {
		...
		$this->registryMock->expects ( $this->atLeastOnce () )->method ( 'registry' )->willReturnMap ( [ 
				[ 
						'email_template',
						true 
				],
		...
		$this->pageTitleMock->expects ( $this->any () )->method ( 'prepend' )->willReturnMap ( [ 
				[ 
						'Email Templates',
						$this->returnSelf () 
				],
		...
		$this->breadcrumbsBlockMock->expects ( $this->any () )->method ( 'addLink' )->willReturnMap ( [ 
				[ 
