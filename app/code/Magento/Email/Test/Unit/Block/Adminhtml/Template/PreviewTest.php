...
class PreviewTest extends \PHPUnit_Framework_TestCase {
	...
	public function testToHtml($requestParamMap) {
		$storeId = 1;
		$template = $this->getMockBuilder ( 'Magento\Email\Model\Template' )->setMethods ( [ 
				'setDesignConfig',
				'getDesignConfig',
				'__wakeup',
		...
		$emailFactory = $this->getMock ( 'Magento\Email\Model\TemplateFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$storeManager->expects ( $this->any () )->method ( 'getStores' )->willReturn ( [ 
				$store 
		] );
		$appState = $this->getMockBuilder ( 'Magento\Framework\App\State' )->setConstructorArgs ( [ 
				$scopeConfig 
		...
		$maliciousCode = $this->getMock ( 'Magento\Framework\Filter\Input\MaliciousCode', [ 
				'filter' 
		], [ ], '', false );
		...
		$preview = $this->objectManagerHelper->getObject ( 'Magento\Email\Block\Adminhtml\Template\Preview', [ 
				'context' => $context,
				...
				[ 
						'data 1' => [ 
								...
								[ 
										'text',
										null,
										sprintf ( '<javascript>%s</javascript>', self::MALICIOUS_TEXT ) 
								],
								[ 
										'styles',
										null,
				...
				[ 
						'data 2' => [ 
								...
								[ 
										'text',
										null,
										sprintf ( '<iframe>%s</iframe>', self::MALICIOUS_TEXT ) 
								],
								[ 
										'styles',
										null,
