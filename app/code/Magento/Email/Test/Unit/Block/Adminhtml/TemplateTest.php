...
class TemplateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->itemFactoryMock = $this->getMockBuilder ( 'Magento\Backend\Block\Widget\Button\ItemFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->buttonMock = $this->getMockBuilder ( 'Magento\Backend\Block\Widget\Button\Item' )->disableOriginalConstructor ()->getMock ();
		$this->itemFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->buttonMock );
		$this->buttonList = $this->objectManager->getObject ( 'Magento\Backend\Block\Widget\Button\ButtonList', [ 
		...
		$this->template = $this->objectManager->getObject ( 'Magento\Email\Block\Adminhtml\Template', [ 
				'context' => $this->context,
				'buttonList' => $this->buttonList 
		] );
	}
	public function testAddButton() {
		$this->template->addButton ( '1', [ 
				'title' => 'My Button' 
		] );
	...
	public function testGetCreateUrl() {
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( 'adminhtml/*/new', [ ] );
