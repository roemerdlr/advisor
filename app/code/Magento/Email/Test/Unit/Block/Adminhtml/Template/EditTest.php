...
class EditTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_configStructureMock = $this->getMock ( 'Magento\Config\Model\Config\Structure', [ ], [ ], '', false, false );
		...
		$viewFilesystem = $this->getMock ( '\Magento\Framework\View\Filesystem', [ 
				'getTemplateFileName' 
		], [ ], '', false );
		$viewFilesystem->expects ( $this->any () )->method ( 'getTemplateFileName' )->will ( $this->returnValue ( DirectoryList::ROOT . '/custom/filename.phtml' ) );
	...
	public function testGetCurrentlyUsedForPaths() {
		...
		$groupMock1 = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false, false );
		$groupMock2 = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false, false );
		$groupMock3 = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Group', [ ], [ ], '', false, false );
		$filedMock = $this->getMock ( 'Magento\Config\Model\Config\Structure\Element\Field', [ ], [ ], '', false, false );
		$map = [ 
				[ 
						[ 
								'section1',
		...
		$templateMock->expects ( $this->once () )->method ( 'getSystemConfigPathsWhereCurrentlyUsed' )->will ( $this->returnValue ( $this->_fixtureConfigPath ) );
		
		$this->_registryMock->expects ( $this->once () )->method ( 'registry' )->with ( 'current_email_template' )->will ( $this->returnValue ( $templateMock ) );
	...
	public function testGetDefaultTemplatesAsOptionsArray() {
		...
		$this->_emailConfigMock->expects ( $this->once () )->method ( 'getAvailableTemplates' )->will ( $this->returnValue ( [ 
				[ 
						'value' => 'template_b2',
						'label' => 'Template B2',
