...
class FilterTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->escaper = $this->getMockBuilder ( '\Magento\Framework\Escaper' )->disableOriginalConstructor ()->enableProxyingToOriginalMethods ()->getMock ();
		...
		$this->coreVariableFactory = $this->getMockBuilder ( '\Magento\Variable\Model\VariableFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->storeManager = $this->getMockBuilder ( '\Magento\Store\Model\StoreManagerInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->layout = $this->getMockBuilder ( '\Magento\Framework\View\LayoutInterface' )->disableOriginalConstructor ()->getMock ();
		...
		$this->backendUrlBuilder = $this->getMockBuilder ( '\Magento\Backend\Model\UrlInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->emogrifier = $this->objectManager->getObject ( '\Pelago\Emogrifier' );
		
		$this->configVariables = $this->getMockBuilder ( 'Magento\Email\Model\Source\Variables' )->disableOriginalConstructor ()->getMock ();
	}
	
	/**
	 *
	 * @param array|null $mockedMethods
	 *        	Methods to mock
	...
	protected function getModel($mockedMethods = null) {
		return $this->getMockBuilder ( '\Magento\Email\Model\Template\Filter' )->setConstructorArgs ( [ 
				$this->string,
				$this->logger,
				$this->escaper,
				$this->assetRepo,
	...
	/**
	 * Tests proper parsing of the {{trans .
	 * ..}} directive used in email templates
	 *
	...
	/**
	 * Data provider for various possible {{trans .
	 * ..}} usages
	 *
	 * @return array
	 */
	...
	public function testApplyInlineCss($html, $css, $expectedResults) {
		...
		$filter->expects ( $this->exactly ( count ( $expectedResults ) ) )->method ( 'getCssFilesContent' )->will ( $this->returnValue ( $css ) );
	...
	public function testAfterFilterCallbackGetsResetWhenExceptionTriggered() {
		...
		$filter->expects ( $this->once () )->method ( 'varDirective' )->will ( $this->throwException ( $exception ) );
		
		// Callbacks must be reset after exception is thrown
		$filter->expects ( $this->once () )->method ( 'resetAfterFilterCallbacks' );
		
		// Build arbitrary object to pass into the addAfterFilterCallback method
		$callbackObject = $this->getMockBuilder ( 'stdObject' )->setMethods ( [ 
				'afterFilterCallbackMethod' 
		] )->getMock ();
		// Callback should never run due to exception happening during filtering
		$callbackObject->expects ( $this->never () )->method ( 'afterFilterCallbackMethod' );
		$filter->addAfterFilterCallback ( [ 
				$callbackObject,
				'afterFilterCallbackMethod' 
		] );
	...
	public function testConfigDirectiveAvailable() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->willReturn ( $scopeConfigValue );
