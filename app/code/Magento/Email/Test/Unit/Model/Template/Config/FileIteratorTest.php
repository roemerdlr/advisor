...
class FileIteratorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->filePaths = [ 
				'directory/path/file1',
				'directory/path/file2' 
		];
		...
		$this->fileIterator = new \Magento\Email\Model\Template\Config\FileIterator ( $this->fileReadFactory, $this->filePaths, $this->moduleDirResolverMock );
	}
	...
	public function testIterator() {
		...
		foreach ( $this->filePaths as $filePath ) {
			$this->moduleDirResolverMock->expects ( $this->at ( $index ) )->method ( 'getModuleName' )->with ( $filePath )->will ( $this->returnValue ( $moduleName ) );
			$this->fileReadFactory->expects ( $this->at ( $dirIndex ) )->method ( 'create' )->with ( $filePath )->willReturn ( $this->fileRead );
			$this->fileRead->expects ( $this->at ( $dirIndex ++ ) )->method ( 'readAll' )->will ( $this->returnValue ( $contents [$index ++] ) );
		}
		$index = 0;
	...
	public function testIteratorNegative() {
		...
		$this->moduleDirResolverMock->expects ( $this->at ( 0 ) )->method ( 'getModuleName' )->with ( $filePath )->will ( $this->returnValue ( false ) );
