...
class BackendTemplateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->structureMock->expects ( $this->any () )->method ( 'getFieldPathsByAttribute' )->willReturn ( [ 
				'path' => 'test' 
		] );
		
		$this->resourceModelMock = $this->getMock ( 'Magento\Email\Model\ResourceModel\Template', [ ], [ ], '', false );
		$this->resourceModelMock->expects ( $this->any () )->method ( 'getSystemConfigByPathsAndTemplateId' )->willReturn ( [ 
				'test_config' => 2015 
		] );
		...
		$this->model = $helper->getObject ( 'Magento\Email\Model\BackendTemplate', [ 
	...
	public function testGetSystemConfigPathsWhereCurrentlyUsedValidId() {
		...
		$this->assertEquals ( [ 
				'test_config' => 2015 
		], $this->model->getSystemConfigPathsWhereCurrentlyUsed () );
	}
}
