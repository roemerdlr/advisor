...
class Filter extends \Magento\Framework\Filter\Template {
	...
	public function blockDirective($construction) {
		...
		if (isset ( $blockParameters ['class'] )) {
			$block = $this->_layout->createBlock ( $blockParameters ['class'], null, [ 
	...
	public function layoutDirective($construction) {
		...
		if ($this->_directiveParams ['area'] != $this->_appState->getAreaCode ()) {
			return $this->_appState->emulateAreaCode ( $this->_directiveParams ['area'], [ 
					$this,
					'emulateAreaCallback' 
			] );
		} else {
	...
	protected function getTransParameters($value) {
		if (preg_match ( self::TRANS_DIRECTIVE_REGEX, $value, $matches ) !== 1) {
			return [ 
					'',
					[ ] 
			]; // malformed directive body; return without breaking list
		}
	 ...
	 *        	assumed modifier if none present
	 * @return array
	 */
	...
	public function protocolDirective($construction) {
		...
		if (isset ( $params ['store'] )) {
			try {
				...
				throw new \Magento\Framework\Exception\MailException ( __ ( 'Requested invalid store "%1"', $params ['store'] ) );
			}
		}
	...
	public function configDirective($construction) {
		...
		if (isset ( $params ['path'] ) && $this->isAvailableConfigVariable ( $params ['path'] )) {
			$configValue = $this->_scopeConfig->getValue ( $params ['path'], \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
		}
		return $configValue;
	}
	...
	public function inlinecssDirective($construction) {
		...
		if (! isset ( $params ['file'] ) || ! $params ['file']) {
			throw new \Magento\Framework\Exception\MailException ( __ ( '"file" parameter must be specified and must not be empty' ) );
		}
	...
	/**
	 * Merge HTML and CSS and return HTML that has CSS styles applied "inline" to the HTML tags.
	 * This is necessary
		...
		if ($html && $cssToInline) {
			try {
				...
				if (strpos ( $cssToInline, ContentProcessorInterface::ERROR_MESSAGE_PREFIX ) !== false) {
					throw new \Magento\Framework\Exception\MailException ( __ ( '<pre>' . PHP_EOL . $cssToInline . PHP_EOL . '</pre>' ) );
				}
				...
				if ($this->_appState->getMode () == \Magento\Framework\App\State::MODE_DEVELOPER) {
					$processedHtml = __ ( 'CSS inlining error:' ) . PHP_EOL . $e->getMessage () . PHP_EOL . $html;
				} else {
					$processedHtml = $html;
				}
	 ...
	 *
	 * @param string $value        	
	 * @return string @SuppressWarnings(PHPMD.ConstructorWithNameAsEnclosingClass)
	 */
