...
class Config implements \Magento\Framework\Mail\Template\ConfigInterface {
	 ...
	 */
	public function __construct(\Magento\Email\Model\Template\Config\Data $dataStorage, \Magento\Framework\Module\Dir\Reader $moduleReader, \Magento\Framework\View\FileSystem $viewFileSystem, ThemePackageList $themePackages, ReadFactory $readDirFactory) {
		...
		foreach ( $this->themePackages->getThemes () as $theme ) {
			if ($theme->getArea () == $area) {
				...
				if ($themeDir->isExist ( $file )) {
					$templates [] = [ 
							'value' => sprintf ( '%s/%s/%s', $templateId, $theme->getVendor (), $theme->getName () ),
							'label' => sprintf ( '%s (%s/%s)', $this->getTemplateLabel ( $templateId ), $theme->getVendor (), $theme->getName () ),
							'group' => $this->getTemplateModule ( $templateId ) 
					];
				}
			}
	 ...
	 *        	Name of a field value of which to return
	 * @return string
