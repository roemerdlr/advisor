...
class IndexBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resource = $this->getMockBuilder ( '\Magento\Framework\App\ResourceConnection' )->disableOriginalConstructor ()->setMethods ( [ 
				'getConnection',
				'getTableName' 
		] )->getMock ();
		$this->resource->expects ( $this->any () )->method ( 'getConnection' )->will ( $this->returnValue ( $this->connection ) );
		
		$this->request = $this->getMockBuilder ( '\Magento\Framework\Search\RequestInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getIndex',
				'getDimensions',
				'getQuery' 
		] )->getMockForAbstractClass ();
		
		$this->config = $this->getMockBuilder ( '\Magento\Framework\App\Config\ScopeConfigInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'isSetFlag' 
		] )->getMockForAbstractClass ();
		...
		$this->conditionManager = $this->getMockBuilder ( '\Magento\Framework\Search\Adapter\Mysql\ConditionManager' )->setMethods ( [ 
				'combineQueries',
				'wrapBrackets',
				'generateCondition' 
		...
		$this->target = $objectManagerHelper->getObject ( 'Magento\CatalogSearch\Model\Search\IndexBuilder', [ 
	...
	public function testBuildWithOutOfStock() {
		...
		$this->config->expects ( $this->once () )->method ( 'isSetFlag' )->with ( 'cataloginventory/options/show_out_of_stock' )->will ( $this->returnValue ( true ) );
		
		$this->request->expects ( $this->exactly ( 2 ) )->method ( 'getDimensions' )->willReturn ( [ ] );
	...
	public function testBuildWithoutOutOfStock() {
		...
		], 'search_index.entity_id = stock_index.product_id' . ' AND stock_index.website_id = 1', [ ] )->willReturnSelf ();
	...
	protected function mockBuild($index, $tableSuffix, $hasFilters = false) {
		...
		], 'search_index.attribute_id = cea.attribute_id', [ ] )->will ( $this->returnSelf () );
		if ($hasFilters) {
			$this->select->expects ( $this->at ( 2 ) )->method ( 'joinLeft' )->with ( [ 
					'category_index' => 'catalog_category_product_index' 
			], 'search_index.entity_id = category_index.product_id', [ ] )->will ( $this->returnSelf () );
			$this->select->expects ( $this->at ( 3 ) )->method ( 'joinLeft' )->with ( [ 
					'cpie' => $this->resource->getTableName ( 'catalog_product_index_eav' ) 
			], 'search_index.entity_id = cpie.entity_id AND search_index.attribute_id = cpie.attribute_id', [ ] )->willReturnSelf ();
		}
	}
