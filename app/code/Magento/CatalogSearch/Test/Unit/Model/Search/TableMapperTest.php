...
class TableMapperTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$attributeCollectionFactory = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$attributeCollectionFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->attributeCollection );
		$this->target = $objectManager->getObject ( '\Magento\CatalogSearch\Model\Search\TableMapper', [ 
	...
	public function testAddPriceFilter() {
		...
		], 'search_index.entity_id = price_index.entity_id AND price_index.website_id = ' . self::WEBSITE_ID, [ ] )->willReturnSelf ();
	...
	public function testAddStaticAttributeFilter() {
		...
		], 'search_index.entity_id = static_filter.entity_id', null )->willReturnSelf ();
		$select = $this->target->addTables ( $this->select, $this->request );
		$this->assertEquals ( $this->select, $select, 'Returned results isn\'t equal to passed select' );
	}
	public function testAddCategoryIds() {
		...
		], 'search_index.entity_id = category_ids_index.product_id', [ ] )->willReturnSelf ();
		$select = $this->target->addTables ( $this->select, $this->request );
		$this->assertEquals ( $this->select, $select, 'Returned results isn\'t equal to passed select' );
	}
	public function testAddTermFilter() {
		...
		], 'search_index.entity_id = color_filter.entity_id' . ' AND color_filter.attribute_id = 132' . ' AND color_filter.store_id = 2514', [ ] )->willReturnSelf ();
		$select = $this->target->addTables ( $this->select, $this->request );
		$this->assertEquals ( $this->select, $select, 'Returned results isn\'t equal to passed select' );
	}
	public function testAddBoolQueryWithTermFiltersInside() {
		...
		$this->request->expects ( $this->once () )->method ( 'getQuery' )->willReturn ( $query );
		$this->select->expects ( $this->at ( 0 ) )->method ( 'joinLeft' )->with ( [ 
				'must1_filter' => 'prefix_catalog_product_index_eav' 
		], 'search_index.entity_id = must1_filter.entity_id' . ' AND must1_filter.attribute_id = 101' . ' AND must1_filter.store_id = 2514', [ ] )->willReturnSelf ();
		$this->select->expects ( $this->at ( 1 ) )->method ( 'joinLeft' )->with ( [ 
				'should1_filter' => 'prefix_catalog_product_index_eav' 
		], 'search_index.entity_id = should1_filter.entity_id' . ' AND should1_filter.attribute_id = 102' . ' AND should1_filter.store_id = 2514', [ ] )->willReturnSelf ();
		$this->select->expects ( $this->at ( 2 ) )->method ( 'joinLeft' )->with ( [ 
				'mustNot1_filter' => 'prefix_catalog_product_index_eav' 
		], 'search_index.entity_id = mustNot1_filter.entity_id' . ' AND mustNot1_filter.attribute_id = 103' . ' AND mustNot1_filter.store_id = 2514', [ ] )->willReturnSelf ();
		$select = $this->target->addTables ( $this->select, $this->request );
		$this->assertEquals ( $this->select, $select, 'Returned results isn\'t equal to passed select' );
	}
	...
	public function testAddBoolFilterWithTermFiltersInside() {
		...
		$this->createAttributeMock ( 'should1', null, null, 102, 'select', 1 );
		$this->createAttributeMock ( 'mustNot1', null, null, 103, 'select', 2 );
		$query = $this->createFilterQuery ( $this->createBoolFilter ( [ 
	...
	private function createFilterQuery($filter) {
		$query = $this->getMockBuilder ( '\Magento\Framework\Search\Request\Query\Filter' )->disableOriginalConstructor ()->getMock ();
		$query->method ( 'getType' )->willReturn ( QueryInterface::TYPE_FILTER );
	...
	private function createBoolQuery(array $must, array $should, array $mustNot) {
		$query = $this->getMockBuilder ( '\Magento\Framework\Search\Request\Query\BoolExpression' )->disableOriginalConstructor ()->getMock ();
	...
	private function createBoolFilter(array $must, array $should, array $mustNot) {
		$query = $this->getMockBuilder ( '\Magento\Framework\Search\Request\Filter\BoolExpression' )->disableOriginalConstructor ()->getMock ();
		$query->method ( 'getType' )->willReturn ( FilterInterface::TYPE_BOOL );
		$query->method ( 'getMust' )->willReturn ( $must );
		$query->method ( 'getShould' )->willReturn ( $should );
		$query->method ( 'getMustNot' )->willReturn ( $mustNot );
		return $query;
	...
	private function createRangeFilter($field) {
		$filter = $this->createFilterMock ( '\Magento\Framework\Search\Request\Filter\Range', FilterInterface::TYPE_RANGE, $field );
		return $filter;
	}
	...
	private function createTermFilter($field) {
		$filter = $this->createFilterMock ( '\Magento\Framework\Search\Request\Filter\Term', FilterInterface::TYPE_TERM, $field );
		return $filter;
	}
	...
	private function createFilterMock($class, $type, $field) {
		$filter = $this->getMockBuilder ( $class )->disableOriginalConstructor ()->getMock ();
	...
	private function createAttributeMock($code, $backendType = null, $backendTable = null, $attributeId = 120, $frontendInput = 'select', $positionInCollection = 0) {
		$attribute = $this->getMockBuilder ( '\Magento\Catalog\Model\ResourceModel\Eav\Attribute' )->setMethods ( [ 
				'getBackendType',
				'getBackendTable',
				'getId',
				'getFrontendInput' 
