...
class FulltextTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->context = $this->getMockBuilder ( '\Magento\Framework\Model\ResourceModel\Db\Context' )->disableOriginalConstructor ()->getMock ();
		...
		$this->target = $objectManager->getObject ( '\Magento\CatalogSearch\Model\ResourceModel\Fulltext', [ 
	...
	public function testResetSearchResult() {
		...
		$this->connection->expects ( $this->once () )->method ( 'update' )->with ( 'table_name_search_query', [ 
				'is_processed' => 0 
		], [ 
				'is_processed != 0' 
		] )->willReturn ( 10 );
