...
class AttributeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->indexerMock = $this->getMockForAbstractClass ( 'Magento\Framework\Indexer\IndexerInterface', [ ], '', false, false, true, [ 
				'getId',
				'getState',
				'__wakeup' 
		] );
		...
		$this->config = $this->getMockBuilder ( \Magento\Framework\Search\Request\Config::class )->disableOriginalConstructor ()->getMock ();
	 ...
	 * @return void @dataProvider aroundSaveDataProvider
	 */
	public function testAroundSave($isObjectNew, $isSearchableChanged, $invalidateCounter) {
		...
		$attributeMock->expects ( $this->any () )->method ( 'dataHasChangedFor' )->will ( $this->returnValue ( $isSearchableChanged ) );
		...
		$this->assertEquals ( $this->subjectMock, $this->model->aroundSave ( $this->subjectMock, $closureMock, $attributeMock ) );
	}
	...
	protected function prepareIndexer($invalidateCounter) {
		$this->indexerRegistryMock->expects ( $this->exactly ( $invalidateCounter ) )->method ( 'get' )->with ( \Magento\CatalogSearch\Model\Indexer\Fulltext::INDEXER_ID )->will ( $this->returnValue ( $this->indexerMock ) );
	}
}
