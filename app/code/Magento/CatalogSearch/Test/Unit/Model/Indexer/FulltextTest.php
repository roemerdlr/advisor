...
class FulltextTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$fullActionFactory = $this->getMock ( 'Magento\CatalogSearch\Model\Indexer\Fulltext\Action\FullFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$indexerHandlerFactory = $this->getMock ( '\Magento\CatalogSearch\Model\Indexer\IndexerHandlerFactory', [ 
				'create' 
		], [ ], '', false );
		$indexerHandlerFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->saveHandler );
		
		$this->storeManager = $this->getMockForAbstractClass ( 'Magento\Store\Model\StoreManagerInterface', [ ], '', false, false, true, [ ] );
		...
		$this->model = new \Magento\CatalogSearch\Model\Indexer\Fulltext ( $fullActionFactory, $indexerHandlerFactory, $this->storeManager, $dimensionFactory, $this->fulltextResource, $this->searchRequestConfig, [ ] );
	}
