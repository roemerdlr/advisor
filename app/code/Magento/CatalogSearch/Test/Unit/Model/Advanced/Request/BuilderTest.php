...
class BuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->config = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Config' )->setMethods ( [ 
				'get' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->objectManager = $this->getMock ( 'Magento\Framework\ObjectManagerInterface' );
		
		$this->requestMapper = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Mapper' )->setMethods ( [ 
				'getRootQuery',
				'getBuckets' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->request = $this->getMockBuilder ( 'Magento\Framework\Search\Request' )->disableOriginalConstructor ()->getMock ();
		
		$this->binder = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Binder' )->setMethods ( [ 
				'bind' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->cleaner = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Cleaner' )->setMethods ( [ 
				'clean' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->requestBuilder = $helper->getObject ( 'Magento\CatalogSearch\Model\Advanced\Request\Builder', [ 
				'config' => $this->config,
	...
	public function testCreate() {
		...
		$this->requestBuilder->bindRequestValue ( 'from_to', [ 
				'from' => 10,
				'to' => 20 
		] );
		$this->requestBuilder->bindRequestValue ( 'not_array', 130 );
		$this->requestBuilder->bindRequestValue ( 'like', [ 
				'like' => 'search_text' 
		] );
		$this->requestBuilder->bindRequestValue ( 'in', [ 
				'in' => 23 
		] );
		$this->requestBuilder->bindRequestValue ( 'in_set', [ 
				'in_set' => [ 
						12,
						23,
						34,
		...
		$this->binder->expects ( $this->once () )->method ( 'bind' )->withConsecutive ( [ 
				$data,
				$bindData 
		] )->willReturn ( $data );
