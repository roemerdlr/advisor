...
class Decimal extends AbstractFilter {
	...
	public function apply(\Magento\Framework\App\RequestInterface $request) {
		...
		$this->getLayer ()->getState ()->addFilter ( $this->_createItem ( $this->renderRangeLabel ( empty ( $from ) ? 0 : $from, $to ), $filter ) );
	 ...
	 *
	 * @return array
	 * @throws \Magento\Framework\Exception\LocalizedException @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	protected function _getItemsData() {
		...
		foreach ( $facets as $key => $aggregation ) {
			...
			$label = $this->renderRangeLabel ( empty ( $from ) ? 0 : $from, empty ( $to ) ? $to : $to );
			$value = $from . '-' . $to;
