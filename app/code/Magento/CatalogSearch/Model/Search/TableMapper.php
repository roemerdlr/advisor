...
class TableMapper {
	...
	private function getMappingData(FilterInterface $filter) {
		...
		if ($fieldToTableMap) {
			...
			if ($filter->getType () === FilterInterface::TYPE_TERM && in_array ( $attribute->getFrontendInput (), [ 
					'select',
					'multiselect' 
			], true )) {
	...
	private function getFieldToTableMap($field) {
		$fieldToTableMap = [ 
				'price' => [ 
						...
						$this->resource->getConnection ()->quoteInto ( 'search_index.entity_id = price_index.entity_id AND price_index.website_id = ?', $this->getWebsiteId () ),
						[ ] 
				],
				'category_ids' => [ 
						'category_ids_index',
