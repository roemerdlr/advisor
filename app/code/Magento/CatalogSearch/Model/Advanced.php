...
class Advanced extends \Magento\Framework\Model\AbstractModel {
	...
	public function __construct(Context $context, Registry $registry, AttributeCollectionFactory $attributeCollectionFactory, Visibility $catalogProductVisibility, Config $catalogConfig, CurrencyFactory $currencyFactory, ProductFactory $productFactory, StoreManagerInterface $storeManager, ProductCollectionFactory $productCollectionFactory, AdvancedFactory $advancedFactory, array $data = []) {
		...
		parent::__construct ( $context, $registry, $advancedFactory->create (), $this->productCollectionFactory->create (), $data );
	}
	...
	public function addFilters($values) {
		...
		foreach ( $attributes as $attribute ) {
			...
			if ($attribute->getAttributeCode () == 'price') {
				...
				$value ['from'] = (isset ( $value ['from'] ) && is_numeric ( $value ['from'] )) ? ( float ) $value ['from'] / $rate : '';
				$value ['to'] = (isset ( $value ['to'] ) && is_numeric ( $value ['to'] )) ? ( float ) $value ['to'] / $rate : '';
			}
			
			if ($attribute->getBackendType () == 'datetime') {
				$value ['from'] = (isset ( $value ['from'] ) && ! empty ( $value ['from'] )) ? date ( 'Y-m-d\TH:i:s\Z', strtotime ( $value ['from'] ) ) : '';
				$value ['to'] = (isset ( $value ['to'] ) && ! empty ( $value ['to'] )) ? date ( 'Y-m-d\TH:i:s\Z', strtotime ( $value ['to'] ) ) : '';
			}
	 ...
	 * @param EntityAttribute $attribute        	
	 * @param mixed $value        	
	 * @return string|bool @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
