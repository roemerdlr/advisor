...
class Full {
	...
	protected function rebuildIndex($productIds = null) {
		$storeIds = array_keys ( $this->storeManager->getStores () );
		foreach ( $storeIds as $storeId ) {
			$dimension = $this->dimensionFactory->create ( [ 
					'name' => self::SCOPE_FIELD_NAME,
					'value' => $storeId 
			] );
			$this->indexHandler->deleteIndex ( [ 
					$dimension 
			], $this->getIterator ( $productIds ) );
	...
	protected function getProductIdsFromParents(array $entityIds) {
		return $this->connection->select ()->from ( $this->getTable ( 'catalog_product_relation' ), 'parent_id' )->distinct ( true )->where ( 'child_id IN (?)', $entityIds )->where ( 'parent_id NOT IN (?)', $entityIds )->query ()->fetchAll ( \Zend_Db::FETCH_COLUMN );
	}
	 ...
	 *        	Product Entity Id
	 * @return \Generator @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	protected function getSearchableProducts($storeId, array $staticFields, $productIds = null, $lastProductId = 0, $limit = 100) {
		...
		$select = $this->connection->select ()->useStraightJoin ( true )->from ( [ 
	...
	protected function getSearchableAttributes($backendType = null) {
		if (null === $this->searchableAttributes) {
			...
			$this->eventManager->dispatch ( 'catelogsearch_searchable_attributes_load_after', [ 
					'engine' => $this->engine,
		...
		foreach ( $attributeTypes as $backendType => $attributeIds ) {
			if ($attributeIds) {
				...
				$selects [] = $this->connection->select ()->from ( [ 
						...
						't_store' => $tableName 
				], $this->connection->quoteInto ( 't_default.entity_id=t_store.entity_id' . ' AND t_default.attribute_id=t_store.attribute_id' . ' AND t_store.store_id = ?', $storeId ), [ 
						'value' => $this->unifyField ( $ifStoreValue, $backendType ) 
				] )->where ( 't_default.store_id = ?', 0 )->where ( 't_default.attribute_id IN (?)', $attributeIds )->where ( 't_default.entity_id IN (?)', $productIds );
			}
		}
	...
	protected function getAttributeValue($attributeId, $valueId, $storeId) {
		...
		if (false !== $value && $attribute->getIsSearchable () && $attribute->usesSource () && $this->engine->allowAdvancedIndex ()) {
			...
			$pieces = array_filter ( array_merge ( [ 
					$value 
			], $valueText ) );
