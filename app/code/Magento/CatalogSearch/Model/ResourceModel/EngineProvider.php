	...
	public function get() {
		if (! $this->engine) {
			...
			if (! isset ( $this->enginePool [$currentEngine] )) {
				throw new \LogicException ( 'There is no such engine: ' . $currentEngine );
			}
			...
			if (false === $engine instanceof \Magento\CatalogSearch\Model\ResourceModel\EngineInterface) {
				throw new \LogicException ( $engineClassName . ' doesn\'t implement \Magento\CatalogSearch\Model\ResourceModel\EngineInterface' );
			}
			
			if ($engine && ! $engine->isAvailable ()) {
				throw new \LogicException ( 'Engine is not available: ' . $currentEngine );
			}
			$this->engine = $engine;
		}
