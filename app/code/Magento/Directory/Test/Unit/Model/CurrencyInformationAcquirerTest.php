...
class CurrencyInformationAcquirerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->currencyInformationFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		
		$className = '\Magento\Directory\Model\Data\ExchangeRateFactory';
		$this->exchangeRateFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		
		$className = '\Magento\Store\Model\StoreManager';
		$this->storeManager = $this->getMock ( $className, [ 
				'getStore' 
		], [ ], '', false );
		
		$this->model = $this->objectManager->getObject ( '\Magento\Directory\Model\CurrencyInformationAcquirer', [ 
	...
	public function testGetCurrencyInfo() {
		...
		$exchangeRate = $this->getMock ( '\Magento\Directory\Model\Data\ExchangeRate', [ 
				'load' 
		], [ ], '', false );
		...
		$currencyInformation = $this->getMock ( '\Magento\Directory\Model\Data\CurrencyInformation', [ 
				'load' 
		], [ ], '', false );
		...
		$baseCurrency = $this->getMock ( '\Magento\Directory\Model\Currency', [ 
				'getCode',
				'getCurrencySymbol',
				'getRate' 
		], [ ], '', false );
		...
		$store->expects ( $this->atLeastOnce () )->method ( 'getAvailableCurrencyCodes' )->willReturn ( [ 
				'AUD' 
		] );
		...
		$this->assertEquals ( [ 
				'AUD' 
		], $result->getAvailableCurrencyCodes () );
