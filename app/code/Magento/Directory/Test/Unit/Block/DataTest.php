...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->regionCollectionFactory = $this->getMockBuilder ( 'Magento\Directory\Model\ResourceModel\Region\CollectionFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->prepareCountryCollection ();
		
		$this->model = new Data ( $this->context, $this->helperData, $this->jsonEncoder, $this->cacheTypeConfig, $this->regionCollectionFactory, $this->countryCollectionFactory );
	}
	protected function prepareContext() {
		...
		$this->storeManager = $this->getMockBuilder ( 'Magento\Store\Model\StoreManagerInterface' )->getMockForAbstractClass ();
	...
	protected function prepareCountryCollection() {
		$this->countryCollection = $this->getMockBuilder ( 'Magento\Directory\Model\ResourceModel\Country\Collection' )->disableOriginalConstructor ()->getMock ();
		
		$this->countryCollectionFactory = $this->getMockBuilder ( 'Magento\Directory\Model\ResourceModel\Country\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->countryCollectionFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->countryCollection );
	}
	...
	protected function mockElementHtmlSelect($defaultCountry, $options, $resultHtml) {
		...
		$elementHtmlSelect = $this->getMockBuilder ( 'Magento\Framework\View\Element\Html\Select' )->disableOriginalConstructor ()->setMethods ( [ 
				'setName',
				'setId',
				'setTitle',
				'setValue',
		...
		$elementHtmlSelect->expects ( $this->once () )->method ( 'setName' )->with ( $name )->willReturnSelf ();
		$elementHtmlSelect->expects ( $this->once () )->method ( 'setId' )->with ( $id )->willReturnSelf ();
		$elementHtmlSelect->expects ( $this->once () )->method ( 'setTitle' )->with ( __ ( $title ) )->willReturnSelf ();
		$elementHtmlSelect->expects ( $this->once () )->method ( 'setValue' )->with ( $defaultCountry )->willReturnSelf ();
		$elementHtmlSelect->expects ( $this->once () )->method ( 'setOptions' )->with ( $options )->willReturnSelf ();
		$elementHtmlSelect->expects ( $this->once () )->method ( 'setExtraParams' )->with ( 'data-validate="{\'validate-select\':true}"' )->willReturnSelf ();
		$elementHtmlSelect->expects ( $this->once () )->method ( 'getHtml' )->willReturn ( $resultHtml );
