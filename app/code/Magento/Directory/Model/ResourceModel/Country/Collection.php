...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	public function loadByStore($store = null) {
		...
		if (! empty ( $allowCountries )) {
			$this->addFieldToFilter ( "country_id", [ 
					'in' => $allowCountries 
			] );
		}
		return $this;
			...
			if (is_array ( $countryCode )) {
				if (is_array ( $iso )) {
					...
					foreach ( $iso as $iso_curr ) {
						$whereOr [] .= $this->_getConditionSql ( "{$iso_curr}_code", [ 
								'in' => $countryCode 
						] );
					}
					$this->_select->where ( '(' . implode ( ') OR (', $whereOr ) . ')' );
				} else {
					$this->addFieldToFilter ( "{$iso}_code", [ 
							'in' => $countryCode 
					] );
				}
			} else {
	...
	public function toOptionArray($emptyLabel = ' ') {
		$options = $this->_toOptionArray ( 'country_id', 'name', [ 
				'title' => 'iso2_code' 
		] );
