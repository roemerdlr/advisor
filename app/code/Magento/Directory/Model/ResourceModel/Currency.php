...
class Currency extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	public function getRate($currencyFrom, $currencyTo) {
		...
		if (! isset ( self::$_rateCache [$currencyFrom] [$currencyTo] )) {
			...
			$select = $connection->select ()->from ( $this->_currencyRateTable, 'rate' )->where ( 'currency_from = :currency_from' )->where ( 'currency_to = :currency_to' );
	...
	public function getAnyRate($currencyFrom, $currencyTo) {
		...
		if (! isset ( self::$_rateCache [$currencyFrom] [$currencyTo] )) {
			...
			if ($rate === false) {
				$select = $connection->select ()->from ( $this->_currencyRateTable, new \Zend_Db_Expr ( '1/rate' ) )->where ( 'currency_to = :currency_from' )->where ( 'currency_from = :currency_to' );
				$rate = $connection->fetchOne ( $select, $bind );
			}
	...
	public function saveRates($rates) {
		if (is_array ( $rates ) && sizeof ( $rates ) > 0) {
			...
			if ($data) {
				$connection->insertOnDuplicate ( $this->_currencyRateTable, $data, [ 
						'rate' 
				] );
			}
		} else {
	...
	protected function _getRatesByCode($code, $toCurrencies = null) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'directory_currency_rate' ), [ 
				'currency_to',
				'rate' 
		] )->where ( 'currency_from = :currency_from' )->where ( 'currency_to IN(?)', $toCurrencies );
		$rowSet = $connection->fetchAll ( $select, $bind );
		$result = [ ];
