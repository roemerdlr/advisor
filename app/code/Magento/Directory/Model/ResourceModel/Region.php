...
class Region extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\Locale\ResolverInterface $localeResolver, $connectionName = null) {
	...
	protected function _loadByCountry($object, $countryId, $value, $field) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'name' 
		] )->where ( 'region.country_id = ?', $countryId )->where ( "region.{$field} = ?", $value );
