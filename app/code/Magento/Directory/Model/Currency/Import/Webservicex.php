...
class Webservicex extends \Magento\Directory\Model\Currency\Import\AbstractImport {
	...
	protected function _convert($currencyFrom, $currencyTo, $retry = 0) {
		...
		try {
			$response = $this->_httpClient->setUri ( $url )->setConfig ( [ 
					'timeout' => $this->_scopeConfig->getValue ( 'currency/webservicex/timeout', \Magento\Store\Model\ScopeInterface::SCOPE_STORE ) 
			] )->request ( 'GET' )->getBody ();
