...
abstract class AbstractSwatch extends \Magento\Eav\Block\Adminhtml\Attribute\Edit\Options\Options {
	...
	protected function createStoreValues($storeId, $optionId) {
		...
		$value ['swatch' . $storeId] = isset ( $swatchStoreValue [$optionId] ) ? $this->escapeHtml ( $swatchStoreValue [$optionId] ) : '';
	...
	public function getStoreOptionValues($storeId) {
		...
		if ($values === null) {
			...
			$valuesCollection->setAttributeFilter ( $this->getAttributeObject ()->getId () )->setStoreFilter ( $storeId, false );
			$valuesCollection->getSelect ()->joinLeft ( [ 
					'swatch_table' => $valuesCollection->getTable ( 'eav_attribute_option_swatch' ) 
			], 'swatch_table.option_id = main_table.option_id AND swatch_table.store_id = ' . $storeId, 'swatch_table.value AS label' );
