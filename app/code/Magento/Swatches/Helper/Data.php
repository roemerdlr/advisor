...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
	 ...
	 */
	public function __construct(Context $context, \Magento\Catalog\Model\ResourceModel\Product\CollectionFactory $productCollectionFactory, \Magento\ConfigurableProduct\Model\Product\Type\Configurable $configurable, ProductRepositoryInterface $productRepository, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Swatches\Model\ResourceModel\Swatch\CollectionFactory $swatchCollectionFactory, \Magento\Catalog\Helper\Image $imageHelper) {
	...
	protected function addFilterByAttributes(ProductCollection $productCollection, array $attributes) {
		foreach ( $attributes as $code => $option ) {
			$productCollection->addAttributeToFilter ( $code, [ 
					'eq' => $option 
			] );
		}
	}
	...
	protected function addFilterByParent(ProductCollection $productCollection, $parentId) {
		...
		], 'e.entity_id = pr.child_id' )->where ( 'pr.parent_id = ?', $parentId );
	}
	...
	protected function getProductMedia(Product $product) {
		return array_filter ( $product->getMediaAttributeValues (), function ($value) {
	...
	protected function getAllSizeImages(Product $product, $imageFile) {
		return [ 
				'large' => $this->imageHelper->init ( $product, 'product_page_image_large' )->constrainOnly ( true )->keepAspectRatio ( true )->keepFrame ( false )->setImageFile ( $imageFile )->getUrl (),
				'medium' => $this->imageHelper->init ( $product, 'product_page_image_medium' )->constrainOnly ( true )->keepAspectRatio ( true )->keepFrame ( false )->setImageFile ( $imageFile )->getUrl (),
				'small' => $this->imageHelper->init ( $product, 'product_page_image_small' )->setImageFile ( $imageFile )->getUrl () 
		];
	}
	...
	protected function createSwatchProduct($product) {
		if (gettype ( $product ) == 'integer') {
			...
			throw new InputException ( __ ( 'Swatch Helper: not valid parameter for product instantiation' ) );
		}
