...
class EavAttribute {
	 ...
	 */
	public function __construct(\Magento\Swatches\Model\ResourceModel\Swatch\CollectionFactory $collectionFactory, \Magento\Swatches\Model\SwatchFactory $swatchFactory, \Magento\Swatches\Helper\Data $swatchHelper) {
		...
		if (isset ( $swatchArray ) && is_array ( $swatchArray )) {
			foreach ( $swatchArray as $optionId => $storeValues ) {
				...
				foreach ( $storeValues as $storeId => $value ) {
					...
					$this->saveSwatchData ( $swatch, $optionId, $storeId, \Magento\Swatches\Model\Swatch::SWATCH_TYPE_TEXTUAL, $value );
					$this->isSwatchExists = null;
				}
			}
		}
	...
	/**
	 * Get option id.
	 * If it not exist get it from dependency link array
	 *
	...
	protected function saveDefaultSwatchOptionValue(Attribute $attribute) {
		...
		if (! empty ( $defaultValue )) {
			...
			if (substr ( $defaultValue, 0, 6 ) == self::BASE_OPTION_TITLE && isset ( $this->dependencyArray [$defaultValue] )) {
