...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->productCollectionFactoryMock = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->productMock = $this->getMock ( '\Magento\Catalog\Model\Product', [ ], [ ], '', false );
		
		$this->productCollectionMock = $this->objectManager->getCollectionMock ( '\Magento\Catalog\Model\ResourceModel\Product\Collection', [ 
				$this->productMock,
				$this->productMock 
		] );
		
		$this->configurableMock = $this->getMock ( '\Magento\ConfigurableProduct\Model\Product\Type\Configurable', [ ], [ ], '', false );
		$this->productModelFactoryMock = $this->getMock ( '\Magento\Catalog\Model\ProductFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->swatchCollectionFactoryMock = $this->getMock ( '\Magento\Swatches\Model\ResourceModel\Swatch\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->attributeMock = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Eav\Attribute', [ ], [ ], '', false );
		
		$this->swatchHelperObject = $this->objectManager->getObject ( '\Magento\Swatches\Helper\Data', [ 
	...
	public function testPopulateAdditionalDataEavAttribute($data, $count) {
		$this->attributeMock->expects ( $this->exactly ( $count ['getData'] ) )->method ( 'getData' )->with ( 'additional_data' )->will ( $this->returnValue ( $data ) );
	...
	/**
	 * @dataProvider dataForCreateSwatchProduct
	 */
	public function testLoadFirstVariationWithSwatchImage($product, $typesArray) {
		$this->createSwatchProduct ( $product );
		
		$this->prepareVariationCollection ();
		
		$this->productMock->method ( 'getId' )->willReturn ( 95 );
		$this->productRepoMock->expects ( $this->atLeastOnce () )->method ( 'getById' )->with ( 95 )->willReturn ( $this->productMock );
		
		$this->getProductMedia ( $typesArray );
		
		$this->swatchHelperObject->loadFirstVariationWithSwatchImage ( $this->productMock, [ 
				'color' => 31 
		] );
	}
	public function testLoadFirstVariationWithSwatchImageWithException() {
		...
		$this->swatchHelperObject->loadFirstVariationWithSwatchImage ( null, [ 
				'color' => 31 
		] );
	}
	...
	public function testLoadVariationByFallback($product) {
		...
		$this->swatchHelperObject->loadVariationByFallback ( $this->productMock, [ 
				'color' => 31 
		] );
	}
	public function testLoadVariationByFallbackByProductId() {
		$this->createSwatchProduct ( 95 );
		$this->prepareVariationCollection ();
		
		$result = $this->swatchHelperObject->loadVariationByFallback ( 95, [ 
				'color' => 31 
		] );
	 ...
	 */
	public function testLoadFirstVariationWithImage($imageTypes, $expected) {
		$this->createSwatchProduct ( $this->productMock );
		$this->prepareVariationCollection ();
		$this->productMock->method ( 'getId' )->willReturn ( 95 );
		$this->productRepoMock->expects ( $this->atLeastOnce () )->method ( 'getById' )->with ( 95 )->willReturn ( $this->productMock );
		$this->productMock->method ( 'getMediaAttributeValues' )->willReturn ( $imageTypes );
		
		$result = $this->swatchHelperObject->loadFirstVariationWithImage ( $this->productMock, [ 
				'color' => 31 
		] );
		if ($expected === false) {
			$this->assertFalse ( $result );
	...
	public function testLoadFirstVariationWithImageNoProduct() {
		$result = $this->swatchHelperObject->loadVariationByFallback ( $this->productMock, [ 
				'color' => 31 
		] );
		$this->assertFalse ( $result );
	}
	...
	public function testGetProductMediaGallery($mediaGallery, $image) {
		...
		$this->imageHelperMock->expects ( $this->any () )->method ( 'init' )->willReturnMap ( [ 
				[ 
						$this->productMock,
		...
		$mediaObject->method ( 'getData' )->withConsecutive ( [ 
				...
				'file' 
		] )->willReturnOnConsecutiveCalls ( 0, $image );
	...
	protected function createSwatchProduct($product) {
		if (gettype ( $product ) == 'integer') {
			$this->productRepoMock->expects ( $this->once () )->method ( 'getById' )->with ( 95 )->willReturn ( $this->productMock );
		}
	...
	protected function getAttributesFromConfigurable() {
		$configurable = $this->getMock ( '\Magento\ConfigurableProduct\Model\Product\Type\Configurable', [ ], [ ], '', false );
		
		$this->productMock->expects ( $this->atLeastOnce () )->method ( 'getTypeInstance' )->willReturn ( $configurable );
		
		$confAttribute = $this->getMock ( '\Magento\ConfigurableProduct\Model\Product\Type\Configurable\Attribute', [ ], [ ], '', false );
		
		$configurable->expects ( $this->atLeastOnce () )->method ( 'getConfigurableAttributes' )->with ( $this->productMock )->willReturn ( [ 
				$confAttribute,
				$confAttribute 
		] );
		
		$confAttribute->expects ( $this->atLeastOnce () )->method ( '__call' )->with ( 'getProductAttribute' )->willReturn ( $this->attributeMock );
	}
	protected function prepareVariationCollection() {
		$this->productCollectionFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->productCollectionMock );
	...
	protected function getProductMedia($typesArray) {
		$this->productMock->method ( 'getMediaAttributeValues' )->willReturn ( $typesArray );
	}
	...
	public function testGetSwatchesByOptionsIdIf1() {
		...
		$swatchCollectionMock = $this->objectManager->getCollectionMock ( '\Magento\Swatches\Model\ResourceModel\Swatch\Collection', [ 
				$swatchMock 
		] );
		$this->swatchCollectionFactoryMock->method ( 'create' )->willReturn ( $swatchCollectionMock );
		
		$swatchCollectionMock->method ( 'addFilterByOptionsIds' )->with ( [ 
				35 
		...
		$this->swatchHelperObject->getSwatchesByOptionsId ( [ 
				35 
		] );
	}
	...
	public function testIsVisualSwatchFalse($data, $count, $swatchType, $boolResult) {
		...
		$this->attributeMock->expects ( $this->exactly ( 2 ) )->method ( 'getData' )->withConsecutive ( [ 
				...
				'swatch_input_type' 
		] )->willReturnOnConsecutiveCalls ( $data, $swatchType );
