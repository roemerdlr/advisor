...
class EavAttributeTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->swatchFactory = $this->getMock ( '\Magento\Swatches\Model\SwatchFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->collectionFactory = $this->getMock ( '\Magento\Swatches\Model\ResourceModel\Swatch\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->abstractSource = $this->getMock ( '\Magento\Eav\Model\Entity\Attribute\Source\AbstractSource', [ ], [ ], '', false );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->eavAttribute = $objectManager->getObject ( 'Magento\Swatches\Model\Plugin\EavAttribute', [ 
	...
	public function testBeforeSaveVisualSwatch() {
		...
		$this->attribute->expects ( $this->exactly ( 6 ) )->method ( 'getData' )->withConsecutive ( [ 
				...
				'option/delete/0' 
		] )->will ( $this->onConsecutiveCalls ( self::ATTRIBUTE_DEFAULT_VALUE, self::ATTRIBUTE_OPTION_VALUE, self::ATTRIBUTE_SWATCH_VALUE, $option, false ) );
		
		$this->attribute->expects ( $this->exactly ( 3 ) )->method ( 'setData' )->withConsecutive ( [ 
				'option',
		...
		$this->swatchHelper->expects ( $this->once () )->method ( 'assembleAdditionalDataEavAttribute' )->with ( $this->attribute );
		$this->swatchHelper->expects ( $this->atLeastOnce () )->method ( 'isVisualSwatch' )->with ( $this->attribute )->willReturn ( true );
		$this->swatchHelper->expects ( $this->once () )->method ( 'isSwatchAttribute' )->with ( $this->attribute )->willReturn ( true );
	...
	public function testBeforeSaveTextSwatch() {
		...
		$this->swatchHelper->expects ( $this->once () )->method ( 'assembleAdditionalDataEavAttribute' )->with ( $this->attribute );
		$this->swatchHelper->expects ( $this->atLeastOnce () )->method ( 'isVisualSwatch' )->with ( $this->attribute )->willReturn ( false );
		$this->swatchHelper->expects ( $this->atLeastOnce () )->method ( 'isTextSwatch' )->with ( $this->attribute )->willReturn ( true );
		$this->swatchHelper->expects ( $this->once () )->method ( 'isSwatchAttribute' )->with ( $this->attribute )->willReturn ( true );
	...
	public function testBeforeSaveWithFailedValidation() {
		...
		$this->swatchHelper->expects ( $this->atLeastOnce () )->method ( 'isVisualSwatch' )->willReturn ( true );
		$this->attribute->expects ( $this->exactly ( 5 ) )->method ( 'getData' )->withConsecutive ( [ 
				'defaultvisual' 
		], [ 
				'optionvisual' 
		], [ 
				...
				'option/delete/0' 
		] )->will ( $this->onConsecutiveCalls ( self::ATTRIBUTE_DEFAULT_VALUE, self::ATTRIBUTE_OPTION_VALUE, self::ATTRIBUTE_SWATCH_VALUE, $optionText, false ) );
	...
	public function testBeforeSaveWithDeletedOption() {
		...
		$this->attribute->expects ( $this->exactly ( 6 ) )->method ( 'getData' )->withConsecutive ( [ 
				...
				'swatch_input_type' 
		] )->will ( $this->onConsecutiveCalls ( self::ATTRIBUTE_DEFAULT_VALUE, self::ATTRIBUTE_OPTION_VALUE, self::ATTRIBUTE_SWATCH_VALUE, $optionText, true, false ) );
		$this->eavAttribute->beforeSave ( $this->attribute );
	}
	public function testBeforeSaveNotSwatch() {
		...
		$this->attribute->expects ( $this->exactly ( 2 ) )->method ( 'getData' )->withConsecutive ( [ 
				...
				'additional_data' 
		] )->willReturnOnConsecutiveCalls ( Swatch::SWATCH_INPUT_TYPE_DROPDOWN, serialize ( $additionalData ) );
		
		$this->attribute->expects ( $this->once () )->method ( 'setData' )->with ( 'additional_data', serialize ( $shortAdditionalData ) )->will ( $this->returnSelf () );
		...
		$this->swatchHelper->expects ( $this->once () )->method ( 'isSwatchAttribute' )->with ( $this->attribute )->willReturn ( false );
	...
	public function testAfterAfterSaveVisualSwatch($swatchType, $swatchValue) {
		...
		$this->swatch->expects ( $this->once () )->method ( 'getId' )->willReturn ( EavAttribute::DEFAULT_STORE_ID );
		$this->swatch->expects ( $this->once () )->method ( 'save' );
		$this->swatch->expects ( $this->exactly ( 4 ) )->method ( 'setData' )->withConsecutive ( [ 
				'option_id',
				self::OPTION_ID 
		], [ 
				'store_id',
				EavAttribute::DEFAULT_STORE_ID 
		], [ 
				'type',
				$swatchType 
		], [ 
		...
		$this->collection->expects ( $this->exactly ( 2 ) )->method ( 'addFieldToFilter' )->withConsecutive ( [ 
				'option_id',
				self::OPTION_ID 
		], [ 
				'store_id',
		...
		$this->attribute->expects ( $this->at ( 4 ) )->method ( 'getData' )->with ( 'swatch/value' )->willReturn ( [ 
				self::STORE_ID => $swatchValue 
		] );
		...
		$this->swatchHelper->expects ( $this->once () )->method ( 'isVisualSwatch' )->with ( $this->attribute )->willReturn ( true );
		$this->swatchHelper->expects ( $this->never () )->method ( 'isTextSwatch' );
	...
	public function testAfterAfterSaveVisualSwatchIsDelete() {
		...
		$this->attribute->expects ( $this->at ( 5 ) )->method ( 'getData' )->with ( 'option/delete/' . self::OPTION_ID )->willReturn ( true );
		
		$this->swatchFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->swatch );
		$this->swatchHelper->expects ( $this->exactly ( 2 ) )->method ( 'isSwatchAttribute' )->with ( $this->attribute )->willReturn ( true );
		$this->swatchHelper->expects ( $this->once () )->method ( 'isVisualSwatch' )->with ( $this->attribute )->willReturn ( true );
		$this->swatchHelper->expects ( $this->never () )->method ( 'isTextSwatch' );
	...
	public function testAfterAfterSaveIsSwatchExists() {
		...
		$this->swatch->expects ( $this->once () )->method ( 'save' );
		$this->swatch->expects ( $this->once () )->method ( 'isDeleted' )->with ( false );
		$this->swatch->expects ( $this->exactly ( 2 ) )->method ( 'setData' )->withConsecutive ( [ 
				'type',
				Swatch::SWATCH_TYPE_TEXTUAL 
		], [ 
				'value',
	...
	public function testAfterAfterSaveNotSwatchAttribute() {
		...
		$this->attribute->expects ( $this->at ( 1 ) )->method ( 'getSource' )->willReturn ( $this->abstractSource );
		$this->attribute->expects ( $this->at ( 2 ) )->method ( 'getData' )->with ( 'swatch/value' )->willReturn ( [ 
				self::STORE_ID => [ 
						self::OPTION_ID => null 
				] 
		] );
