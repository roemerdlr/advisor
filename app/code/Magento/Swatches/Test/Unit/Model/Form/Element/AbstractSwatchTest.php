...
class AbstractSwatchTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->source = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute\Source\AbstractSource' )->getMockForAbstractClass ();
		
		$this->attribute = $this->getMockBuilder ( 'Magento\Catalog\Model\ResourceModel\Eav\Attribute' )->disableOriginalConstructor ()->getMock ();
		
		$this->swatch = $this->getMockBuilder ( 'Magento\Swatches\Model\Form\Element\AbstractSwatch' )->disableOriginalConstructor ()->setMethods ( [ 
				'getData' 
	...
	public function testGetValuesEmpty() {
		$this->swatch->expects ( $this->once () )->method ( 'getData' )->with ( 'entity_attribute' )->willReturn ( null );
