...
class AbstractSwatchTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->attrOptionCollectionFactoryMock = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Option\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->block = $this->getMock ( 'Magento\Swatches\Block\Adminhtml\Attribute\Edit\Options\AbstractSwatch', [ 
				'getData' 
		], [ 
	...
	public function testGetStoreOptionValues($values) {
		...
		if ($values === null) {
			...
			$option = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Option', [ 
					'getId',
					'getValue',
					'getLabel' 
			], [ ], '', false );
			
			$attrOptionCollectionMock = $objectManager->getCollectionMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute\Option\Collection', [ 
					$option,
					$option 
			] );
			...
			$attribute = $this->getMock ( '\Magento\Eav\Model\ResourceModel\Entity\Attribute', [ 
					'getId' 
			], [ ], '', false );
