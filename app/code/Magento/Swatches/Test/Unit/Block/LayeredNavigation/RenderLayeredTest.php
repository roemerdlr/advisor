...
class RenderLayeredTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->requestMock = $this->getMock ( '\Magento\Framework\App\Request', [ 
				'getParams' 
		], [ ], '', false );
		...
		$this->layerAttributeFactoryMock = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Layer\Filter\AttributeFactory', [ 
				'create' 
		], [ ], '', false );
		$this->layerAttributeMock = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Layer\Filter\Attribute', [ 
				'getCount' 
		], [ ], '', false );
		...
		$this->block = $this->getMock ( '\Magento\Swatches\Block\LayeredNavigation\RenderLayered', [ 
				'filter',
				'eavAttribute' 
		], [ 
				$this->contextMock,
	...
	public function testGetSwatchData() {
		...
		$item1->expects ( $this->any () )->method ( '__call' )->withConsecutive ( [ 
				...
				'getLabel' 
		] )->willReturnOnConsecutiveCalls ( 'yellow', 3, 'yellow', 3, 'Yellow' );
		...
		$item3->expects ( $this->any () )->method ( '__call' )->withConsecutive ( [ 
				...
				'getCount' 
		] )->willReturnOnConsecutiveCalls ( 'red', 0 );
		...
		$item4->expects ( $this->any () )->method ( '__call' )->withConsecutive ( [ 
				...
				'getLabel' 
		] )->willReturnOnConsecutiveCalls ( 'green', 3, 'green', 0, 'Green' );
		...
		$eavAttribute = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Eav\Attribute', [ ], [ ], '', false );
		$eavAttribute->expects ( $this->once () )->method ( 'getOptions' )->willReturn ( [ 
				$option1,
				$option2,
				$option3,
				$option4 
		...
		$this->layerAttributeMock->method ( 'getCount' )->willReturn ( $optionsCount );
	...
	public function testGetSwatchPath() {
		$this->mediaHelperMock->expects ( $this->once () )->method ( 'getSwatchAttributeImage' )->with ( 'swatch_image', '/m/a/magento.jpg' )->willReturn ( 'http://domain.com/path_to_swatch_image/m/a/magento.jpg' );
