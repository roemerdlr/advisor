...
class RegisterFormKeyFromCookieTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->cookieFormKey = $this->getMockBuilder ( 'Magento\Framework\App\PageCache\FormKey' )->disableOriginalConstructor ()->getMock ();
		$this->escaper = $this->getMockBuilder ( 'Magento\Framework\Escaper' )->disableOriginalConstructor ()->getMock ();
		$this->sessionFormKey = $this->getMockBuilder ( 'Magento\Framework\Data\Form\FormKey' )->disableOriginalConstructor ()->getMock ();
		$this->cookieMetadataFactory = $this->getMockBuilder ( 'Magento\Framework\Stdlib\Cookie\CookieMetadataFactory' )->disableOriginalConstructor ()->getMock ();
		...
		$this->observer = new RegisterFormKeyFromCookie ( $this->cookieFormKey, $this->escaper, $this->sessionFormKey, $this->cookieMetadataFactory, $this->sessionConfig );
	}
	public function testExecuteNoCookie() {
		$this->cookieFormKey->expects ( static::once () )->method ( 'get' )->willReturn ( null );
		$this->cookieFormKey->expects ( static::never () )->method ( 'set' );
		$this->sessionFormKey->expects ( static::never () )->method ( 'set' );
