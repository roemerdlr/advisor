...
class ProcessLayoutRenderElementTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->_configMock = $this->getMock ( 'Magento\PageCache\Model\Config', [ 
				'getType',
				'isEnabled' 
		], [ ], '', false );
		
		$this->_model = new \Magento\PageCache\Observer\ProcessLayoutRenderElement ( $this->_configMock );
		$this->_observerMock = $this->getMock ( 'Magento\Framework\Event\Observer', [ 
				'getEvent' 
		], [ ], '', false );
		$this->_layoutMock = $this->getMock ( 'Magento\Framework\View\Layout', [ 
				'isCacheable',
				'getBlock',
				'getUpdate',
				'getHandles' 
		], [ ], '', false );
		$this->_blockMock = $this->getMockForAbstractClass ( 'Magento\Framework\View\Element\AbstractBlock', [ ], '', false, true, true, [ 
				'getData',
				'isScopePrivate',
				'getNameInLayout',
				'getUrl' 
		] );
		$this->_transport = new \Magento\Framework\DataObject ( [ 
	...
	public function processLayoutRenderDataProvider() {
		return [ 
				...
				'full_page type is disabled, Varnish enabled' => [ 
						false,
						true,
						false,
						null,
