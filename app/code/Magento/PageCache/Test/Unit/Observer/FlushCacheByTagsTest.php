...
class FlushCacheByTagsTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_cacheMock = $this->getMock ( 'Magento\Framework\App\PageCache\Cache', [ 
				'clean' 
		], [ ], '', false );
		
		$this->_model = new \Magento\PageCache\Observer\FlushCacheByTags ( $this->_configMock, $this->_cacheMock );
	}
	...
	public function flushCacheByTagsDataProvider() {
		return [ 
				'full_page cache type is enabled' => [ 
						true 
				],
				'full_page cache type is disabled' => [ 
						false 
				] 
		];
	}
