...
class BuiltinPluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$config = $this->getMock ( 'Magento\PageCache\Model\Config', [ 
				'isEnabled',
				'getType' 
		], [ ], '', false );
		...
		$this->plugin = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\PageCache\Model\Controller\Result\BuiltinPlugin', [ 
	...
	public function testAroundResultWithoutPlugin() {
		...
		$this->assertSame ( call_user_func ( $this->closure ), $this->plugin->aroundRenderResult ( $this->subject, $this->closure, $this->response ) );
	}
	public function testAroundResultWithPlugin() {
		...
		$this->response->expects ( $this->once () )->method ( 'setHeader' )->with ( 'X-Magento-Tags', 'tag,' . \Magento\PageCache\Model\Cache\Type::CACHE_TAG );
	...
	public function testAroundResultWithPluginDeveloperMode() {
		...
		$this->response->expects ( $this->any () )->method ( 'setHeader' )->withConsecutive ( [ 
				...
				'X-Magento-Tags',
				'tag,tag2,' . \Magento\PageCache\Model\Cache\Type::CACHE_TAG 
		] );
