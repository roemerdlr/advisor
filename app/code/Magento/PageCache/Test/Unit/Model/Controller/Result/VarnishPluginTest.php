...
class VarnishPluginTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAroundResult($usePlugin, $setCacheDebugHeaderCount, $getModeCount, $processCount) {
		...
		$response->expects ( $setCacheDebugHeaderCount )->method ( 'setHeader' )->with ( 'X-Magento-Debug', 1 );
		...
		$registry->expects ( $this->once () )->method ( 'registry' )->with ( 'use_page_cache_plugin' )->will ( $this->returnValue ( $usePlugin ) );
		...
		$config->expects ( $this->once () )->method ( 'getType' )->will ( $this->returnValue ( \Magento\PageCache\Model\Config::VARNISH ) );
		...
		$state->expects ( $getModeCount )->method ( 'getMode' )->will ( $this->returnValue ( \Magento\Framework\App\State::MODE_DEVELOPER ) );
		...
		$plugin = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\PageCache\Model\Controller\Result\VarnishPlugin', [ 
				'registry' => $registry,
				'config' => $config,
				'state' => $state,
				'version' => $version 
