...
class LayoutPluginTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->layoutMock = $this->getMockForAbstractClass ( 'Magento\Framework\View\Layout', [ ], '', false, true, true, [ 
				'isCacheable',
				'getAllBlocks' 
		] );
		...
		$this->model = new \Magento\PageCache\Model\Layout\LayoutPlugin ( $this->responseMock, $this->configMock );
	}
	 ...
	 * @param
	 *        	$cacheState
	 * @param $layoutIsCacheable @dataProvider
	...
	public function afterGenerateXmlDataProvider() {
		return [ 
				'Full_cache state is true, Layout is cache-able' => [ 
						true,
						true 
				],
				'Full_cache state is true, Layout is not cache-able' => [ 
						true,
						false 
				],
				'Full_cache state is false, Layout is not cache-able' => [ 
						false,
						false 
				],
				'Full_cache state is false, Layout is cache-able' => [ 
						false,
						true 
				] 
		];
	...
	public function testAfterGetOutput($cacheState, $layoutIsCacheable, $expectedTags, $configCacheType, $ttl) {
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'isCacheable' )->will ( $this->returnValue ( $layoutIsCacheable ) );
		$this->layoutMock->expects ( $this->any () )->method ( 'getAllBlocks' )->will ( $this->returnValue ( [ 
				$blockStub 
		] ) );
	...
	public function afterGetOutputDataProvider() {
		...
		return [ 
				'Cacheable layout, Full_cache state is true' => [ 
						true,
						true,
						$tags,
						null,
