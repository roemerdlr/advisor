...
class PageCachePluginTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->subjectMock = $this->getMockBuilder ( '\Magento\Framework\App\PageCache\Cache' )->disableOriginalConstructor ()->getMock ();
	}
	public function testBeforeSaveAddTag() {
		...
		$expectedTags = array_merge ( $initTags, [ 
				Type::CACHE_TAG 
		] );
	...
	public function afterSaveDataProvider() {
		return [ 
				'Compressed cache' => [ 
						'raw-data',
						PageCachePlugin::COMPRESSION_PREFIX . gzcompress ( 'raw-data' ) 
				],
				'Non-compressed cache' => [ 
						'raw-data',
						'raw-data' 
				] 
		];
