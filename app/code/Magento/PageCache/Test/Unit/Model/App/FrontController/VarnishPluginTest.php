...
class VarnishPluginTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->frontControllerMock = $this->getMock ( 'Magento\Framework\App\FrontControllerInterface', [ ], [ ], '', false );
		...
		$this->plugin = new \Magento\PageCache\Model\App\FrontController\VarnishPlugin ( $this->configMock, $this->versionMock, $this->stateMock );
	}
	...
	public function testAroundDispatchReturnsCache($state, $countHeader, $countProcess, $countGetMode, $response) {
		...
		$this->configMock->expects ( $this->once () )->method ( 'getType' )->will ( $this->returnValue ( \Magento\PageCache\Model\Config::VARNISH ) );
		$this->versionMock->expects ( $countProcess )->method ( 'process' );
		$this->stateMock->expects ( $countGetMode )->method ( 'getMode' )->will ( $this->returnValue ( $state ) );
		$response->expects ( $countHeader )->method ( 'setHeader' )->with ( 'X-Magento-Debug' );
