...
class RenderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->action = $helperObjectManager->getObject ( 'Magento\PageCache\Controller\Block\Render', [ 
				'context' => $contextMock,
	...
	public function testExecuteNoParams() {
		...
		$this->requestMock->expects ( $this->at ( 10 ) )->method ( 'getParam' )->with ( $this->equalTo ( 'blocks' ), $this->equalTo ( '' ) )->will ( $this->returnValue ( '' ) );
		$this->requestMock->expects ( $this->at ( 11 ) )->method ( 'getParam' )->with ( $this->equalTo ( 'handles' ), $this->equalTo ( '' ) )->will ( $this->returnValue ( '' ) );
		$this->action->execute ();
	}
	public function testExecute() {
		...
		$blockInstance1 = $this->getMock ( 'Magento\PageCache\Test\Unit\Block\Controller\StubBlock', [ 
				'toHtml' 
		], [ ], '', false );
		$blockInstance1->expects ( $this->once () )->method ( 'toHtml' )->will ( $this->returnValue ( $expectedData ['block1'] ) );
		
		$blockInstance2 = $this->getMock ( 'Magento\PageCache\Test\Unit\Block\Controller\StubBlock', [ 
				'toHtml' 
		], [ ], '', false );
