...
class SetCanApplyMsrpObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->observer = new \Magento\Msrp\Observer\Frontend\Quote\SetCanApplyMsrpObserver ( $this->configMock, $this->canApplyMsrpMock, $this->msrpMock );
	}
	public function testSetQuoteCanApplyMsrpIfMsrpCanApply() {
		...
		$addressMock = $this->getMock ( '\Magento\Customer\Model\Address\AbstractAddress', [ 
				'__wakeup' 
		], [ ], '', false );
		$this->canApplyMsrpMock->expects ( $this->once () )->method ( 'isCanApplyMsrp' )->willReturn ( true );
		
		$quoteMock->expects ( $this->once () )->method ( 'getAllAddresses' )->willReturn ( [ 
				$addressMock 
		] );
