...
class MetadataProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->model = $objectManager->getObject ( 'Magento\Ui\Model\Export\MetadataProvider', [ 
				'filter' => $this->filter 
		] );
	}
	...
	protected function prepareColumns($componentName, $columnName, $columnLabel) {
		...
		$columns = $this->getMockBuilder ( 'Magento\Ui\Component\Listing\Columns' )->disableOriginalConstructor ()->getMock ();
		
		$column = $this->getMockBuilder ( 'Magento\Ui\Component\Listing\Columns\Column' )->disableOriginalConstructor ()->getMock ();
		
		$component->expects ( $this->any () )->method ( 'getName' )->willReturn ( $componentName );
		$component->expects ( $this->once () )->method ( 'getChildComponents' )->willReturn ( [ 
				$columns 
		] );
		
		$columns->expects ( $this->once () )->method ( 'getChildComponents' )->willReturn ( [ 
				$column 
		] );
		
		$column->expects ( $this->any () )->method ( 'getName' )->willReturn ( $columnName );
		$column->expects ( $this->any () )->method ( 'getData' )->with ( 'config/label' )->willReturn ( $columnLabel );
		return $component;
	}
	...
	public function testGetRowData($key, $fields, $options, $expected) {
		...
		$attribute = $this->getMockBuilder ( 'Magento\Framework\Api\AttributeInterface' )->getMockForAbstractClass ();
		
		$document->expects ( $this->once () )->method ( 'getCustomAttribute' )->with ( $fields [0] )->willReturn ( $attribute );
		
		$attribute->expects ( $this->once () )->method ( 'getValue' )->willReturn ( $key );
	...
	public function testGetOptions($filter, $options, $expected) {
		...
		$childComponent->expects ( $this->once () )->method ( 'getChildComponents' )->willReturn ( [ 
				$filters 
		] );
		
		$filters->expects ( $this->once () )->method ( 'getChildComponents' )->willReturn ( [ 
				$select 
		] );
