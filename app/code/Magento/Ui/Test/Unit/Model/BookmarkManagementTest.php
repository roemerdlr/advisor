...
class BookmarkManagementTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->bookmarkRepository = $this->getMockBuilder ( 'Magento\Ui\Api\BookmarkRepositoryInterface' )->getMockForAbstractClass ();
		$this->filterBuilder = $this->getMockBuilder ( 'Magento\Framework\Api\FilterBuilder' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->bookmarkManagement = new BookmarkManagement ( $this->bookmarkRepository, $this->filterBuilder, $this->searchCriteriaBuilder, $this->userContext );
	}
	public function testLoadByNamespace() {
		...
		$this->filterBuilder->expects ( $this->at ( 1 ) )->method ( 'create' )->willReturn ( $fieldNamespace );
	...
	public function testGetByIdentifierNamespace() {
		...
		$searchResult->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$bookmark 
		] );
		$this->bookmarkRepository->expects ( $this->once () )->method ( 'getList' )->with ( $searchCriteria )->willReturn ( $searchResult );
