...
class ManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->componentConfigProvider = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\Config\Provider\Component\Definition' )->disableOriginalConstructor ()->getMock ();
		$this->domMerger = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\Config\DomMergerInterface' )->getMockForAbstractClass ();
		$this->aggregatedFileCollector = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\Config\FileCollector\AggregatedFileCollector' )->disableOriginalConstructor ()->getMock ();
		$this->aggregatedFileCollectorFactory = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\Config\FileCollector\AggregatedFileCollectorFactory' )->disableOriginalConstructor ()->getMock ();
		$this->arrayObjectFactory = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\ArrayObjectFactory' )->disableOriginalConstructor ()->getMock ();
		$this->arrayObjectFactory->expects ( $this->at ( 0 ) )->method ( 'create' )->willReturn ( new \ArrayObject ( [ ] ) );
		$this->uiReader = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\Config\UiReaderInterface' )->getMockForAbstractClass ();
		$this->readerFactory = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponent\Config\ReaderFactory' )->disableOriginalConstructor ()->getMock ();
		$this->cacheConfig = $this->getMockBuilder ( 'Magento\Framework\Config\CacheInterface' )->getMockForAbstractClass ();
		$this->argumentInterpreter = $this->getMockBuilder ( 'Magento\Framework\Data\Argument\InterpreterInterface' )->getMockForAbstractClass ();
		$this->manager = new Manager ( $this->componentConfigProvider, $this->domMerger, $this->readerFactory, $this->arrayObjectFactory, $this->aggregatedFileCollectorFactory, $this->cacheConfig, $this->argumentInterpreter );
	}
	public function testGetReader() {
		$this->readerFactory->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'fileCollector' => $this->aggregatedFileCollector,
				'domMerger' => $this->domMerger 
		] )->willReturn ( $this->uiReader );
		$this->aggregatedFileCollectorFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->aggregatedFileCollector );
		$this->assertEquals ( $this->uiReader, $this->manager->getReader ( 'some_name' ) );
	}
	public function testPrepareDataWithException() {
		$this->setExpectedException ( 'Magento\Framework\Exception\LocalizedException', __ ( 'Initialization error component, check the spelling of the name or the correctness of the call.' ) );
	...
	public function testPrepareGetData($componentName, $componentData, $isCached, $readerData, $expectedResult) {
		$this->readerFactory->expects ( $this->any () )->method ( 'create' )->with ( [ 
				'fileCollector' => $this->aggregatedFileCollector,
				'domMerger' => $this->domMerger 
		] )->willReturn ( $this->uiReader );
