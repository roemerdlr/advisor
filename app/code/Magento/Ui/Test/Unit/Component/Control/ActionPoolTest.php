...
class ActionPoolTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->contextMock = $this->getMock ( 'Magento\Framework\View\Element\UiComponent\Context', [ 
				'getPageLayout' 
		], [ ], '', false );
		$this->toolbarBlockMock = $this->getMock ( 'Magento\Framework\View\Element\AbstractBlock', [ 
				'setChild' 
		], [ ], '', false );
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( static::ACTIONS_PAGE_TOOLBAR )->willReturn ( $this->toolbarBlockMock );
		
		$this->itemFactoryMock = $this->getMock ( 'Magento\Ui\Component\Control\ItemFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->uiComponentInterfaceMock = $this->getMockForAbstractClass ( 'Magento\Framework\View\Element\UiComponentInterface' );
		$this->items [$this->key] = $this->getMock ( 'Magento\Ui\Component\Control\Item', [ 
				'setData' 
		], [ ], '', false );
		$this->actionPool = new ActionPool ( $this->contextMock, $this->itemFactoryMock, $this->toolbarBlockMock );
	}
	public function testAdd() {
		...
		$this->contextMock->expects ( $this->any () )->method ( 'getPageLayout' )->willReturn ( $this->layoutMock );
		$toolbarContainerMock = $this->getMock ( 'Magento\Backend\Block\Widget\Button\Toolbar\Container', [ ], [ ], '', false );
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( 'Magento\Ui\Component\Control\Container', 'container-name-' . $this->key, [ 
				'data' => [ 
						'button_item' => $this->items [$this->key],
						'context' => $this->uiComponentInterfaceMock 
				] 
		] )->willReturn ( $toolbarContainerMock );
		$this->toolbarBlockMock->expects ( $this->once () )->method ( 'setChild' )->with ( $this->key, $toolbarContainerMock )->willReturnSelf ();
