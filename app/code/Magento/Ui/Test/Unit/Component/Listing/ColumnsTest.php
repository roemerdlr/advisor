...
class ColumnsTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetComponentName() {
		$columns = $this->objectManager->getObject ( 'Magento\Ui\Component\Listing\Columns', [ 
	...
	public function testPrepare() {
		...
		$this->contextMock->expects ( $this->once () )->method ( 'addComponentDefinition' )->with ( 'columns', [ 
				'extends' => 'test_config_extends' 
		] );
		
		$dataProviderMock->expects ( $this->once () )->method ( 'getFieldMetaInfo' )->with ( 'test_name', 'test_column_name' )->willReturn ( [ 
				'test_meta' => 'test_meta_value' 
		] );
		
		$columnMock->expects ( $this->once () )->method ( 'getName' )->willReturn ( 'test_column_name' );
		$columnMock->expects ( $this->once () )->method ( 'getData' )->with ( 'config' )->willReturn ( [ 
				'test_config_data' => 'test_config_value' 
		] );
		$columnMock->expects ( $this->once () )->method ( 'setData' )->with ( 'config', [ 
				'test_config_data' => 'test_config_value',
				'test_meta' => 'test_meta_value' 
		] );
