...
class FieldTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->field = new Field ( $this->contextMock, $this->uiComponentFactoryMock );
	}
	 ...
	 * @return void @dataProvider prepareSuccessDataProvider
	 */
	...
	protected function getWrappedComponentMock() {
		$wrappedComponentMock = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponentInterface' )->getMockForAbstractClass ();
		
		$wrappedComponentMock->expects ( $this->any () )->method ( 'getData' )->willReturnMap ( $this->testConfigData );
		$wrappedComponentMock->expects ( $this->once () )->method ( 'setData' )->with ( 'config', $this->logicalNot ( $this->isEmpty () ) );
		$wrappedComponentMock->expects ( $this->once () )->method ( 'prepare' );
		$wrappedComponentMock->expects ( $this->once () )->method ( 'getChildComponents' )->willReturn ( $this->getComponentsMock () );
		$wrappedComponentMock->expects ( $this->any () )->method ( 'getComponentName' )->willReturn ( self::COMPONENT_NAME );
	...
	protected function getComponentsMock() {
		$componentMock = $this->getMockBuilder ( 'Magento\Framework\View\Element\UiComponentInterface' )->getMockForAbstractClass ();
