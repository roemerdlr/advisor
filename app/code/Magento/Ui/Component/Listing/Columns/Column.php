...
class Column extends AbstractComponent implements ColumnInterface {
	...
	public function prepare() {
		...
		if ($dataType) {
			$this->wrappedComponent = $this->uiComponentFactory->create ( $this->getName (), $dataType, array_merge ( [ 
	...
	protected function applySorting() {
		...
		if ($isSortable !== false && ! empty ( $sorting ['field'] ) && ! empty ( $sorting ['direction'] ) && $sorting ['field'] === $this->getName ()) {
			$this->getContext ()->getDataProvider ()->addOrder ( $this->getName (), strtoupper ( $sorting ['direction'] ) );
		}
	}
}
