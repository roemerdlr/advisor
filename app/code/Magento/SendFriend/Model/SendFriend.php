...
class SendFriend extends \Magento\Framework\Model\AbstractModel {
	...
	public function send() {
		...
		foreach ( $this->getRecipients ()->getEmails () as $k => $email ) {
			...
			$this->_transportBuilder->setTemplateIdentifier ( $this->_sendfriendData->getEmailTemplate () )->setTemplateOptions ( [ 
					'area' => \Magento\Framework\App\Area::AREA_FRONTEND,
					'store' => $this->_storeManager->getStore ()->getId () 
			] )->setFrom ( $sender )->setTemplateVars ( [ 
					'name' => $name,
					'email' => $email,
					...
					'product_image' => $this->_catalogImage->init ( $this->getProduct (), 'sendfriend_small_image' )->getUrl () 
			] )->addTo ( $email, $name );
	...
	public function setRecipients($recipients) {
		...
		return $this->setData ( '_recipients', new \Magento\Framework\DataObject ( [ 
				'emails' => $emails,
				'names' => $names 
		] ) );
	}
	...
	public function getRecipients() {
		...
		if (! $recipients instanceof \Magento\Framework\DataObject) {
			$recipients = new \Magento\Framework\DataObject ( [ 
					'emails' => [ ],
					'names' => [ ] 
			] );
	 ...
	 *
	 * @param bool $useCache
	 *        	- flag, is allow to use value of attribute of model if it is processed last time
	 * @return int
	 */
	 ...
	 *        	- flag, increase count before return value
	 * @return int
	 */
