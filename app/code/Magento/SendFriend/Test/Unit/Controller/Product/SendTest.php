...
class SendTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\SendFriend\Controller\Product\Send', [ 
	...
	public function testExecute() {
		...
		$this->sendFriendMock->expects ( $this->once () )->method ( 'getMaxSendsToFriend' )->willReturn ( 11 );
		$this->sendFriendMock->expects ( $this->once () )->method ( 'isExceedLimit' )->willReturn ( false );
		...
		$this->eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( 'sendfriend_product', [ 
				'product' => $productMock 
		] );
		
		$this->catalogSessionMock->expects ( $this->once () )->method ( 'getSendfriendFormData' )->willReturn ( $formData );
		$this->catalogSessionMock->expects ( $this->once () )->method ( 'setSendfriendFormData' )->with ( true );
		...
		$blockMock = $this->getMockBuilder ( 'Magento\SendFriend\Block\Send' )->disableOriginalConstructor ()->getMock ();
		
		$layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'sendfriend.send' )->willReturn ( $blockMock );
		
		$blockMock->expects ( $this->once () )->method ( 'setFormData' )->with ( $formData )->willReturnSelf ();
	...
	public function testExecuteWithoutBlock() {
		...
		$layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'sendfriend.send' )->willReturn ( false );
	...
	public function testExecuteWithoutParam() {
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( 'id', null )->willReturn ( null );
	...
	public function testExecuteWithoutProduct() {
		...
		$this->productRepositoryMock->expects ( $this->once () )->method ( 'getById' )->with ( $productId, false, null, false )->willThrowException ( new \Magento\Framework\Exception\NoSuchEntityException ( __ ( 'No Product Exception.' ) ) );
