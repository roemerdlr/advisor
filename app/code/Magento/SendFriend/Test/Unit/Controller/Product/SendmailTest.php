...
class SendmailTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->requestMock = $this->getMockBuilder ( 'Magento\Framework\App\RequestInterface' )->setMethods ( [ 
				'getPost',
				'getPostValue',
				'getParam' 
		...
		$this->sendFriendMock = $this->getMockBuilder ( 'Magento\SendFriend\Model\SendFriend' )->disableOriginalConstructor ()->getMock ();
		$this->productRepositoryMock = $this->getMockBuilder ( 'Magento\Catalog\Api\ProductRepositoryInterface' )->getMockForAbstractClass ();
		$this->categoryRepositoryMock = $this->getMockBuilder ( 'Magento\Catalog\Api\CategoryRepositoryInterface' )->getMockForAbstractClass ();
		$this->catalogSessionMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Session' )->setMethods ( [ 
		...
		$this->urlBuilderMock = $this->getMockBuilder ( 'Magento\Framework\UrlInterface' )->getMock ();
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\SendFriend\Controller\Product\Sendmail', [ 
	...
	public function testExecute() {
		...
		$this->requestMock->expects ( $this->exactly ( 2 ) )->method ( 'getParam' )->willReturnMap ( [ 
				[ 
						'id',
						null,
						$productId 
		...
		$productMock = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\ProductInterface' )->setMethods ( [ 
				'isVisibleInCatalog',
				'setCategory',
				'getProductUrl' 
		] )->getMockForAbstractClass ();
		...
		$categoryMock = $this->getMockBuilder ( 'Magento\Catalog\Api\Data\CategoryInterface' )->getMockForAbstractClass ();
		
		$this->categoryRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $categoryId, null )->willReturn ( $categoryMock );
		
		$productMock->expects ( $this->once () )->method ( 'setCategory' )->with ( $categoryMock );
		
		$this->registryMock->expects ( $this->exactly ( 2 ) )->method ( 'register' )->willReturnMap ( [ 
				[ 
						'product',
						$productMock,
						false,
		...
		$this->sendFriendMock->expects ( $this->once () )->method ( 'validate' )->willReturn ( true );
		$this->sendFriendMock->expects ( $this->once () )->method ( 'send' )->willReturnSelf ();
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addSuccess' )->with ( __ ( 'The link to a friend was sent.' ) )->willReturnSelf ();
		
		$productMock->expects ( $this->once () )->method ( 'getProductUrl' )->willReturn ( $productUrl );
		
		$this->redirectMock->expects ( $this->once () )->method ( 'success' )->with ( $productUrl )->willReturnArgument ( 0 );
		
		$redirectMock->expects ( $this->once () )->method ( 'setUrl' )->with ( $productUrl )->willReturnSelf ();
	...
	public function testExecuteWithoutValidationAndCategory() {
		...
		$this->registryMock->expects ( $this->once () )->method ( 'register' )->willReturnMap ( [ 
				[ 
						'product',
						$productMock,
						false,
		...
		$this->sendFriendMock->expects ( $this->once () )->method ( 'setRecipients' )->with ( $recipients )->willReturnSelf ();
		$this->sendFriendMock->expects ( $this->once () )->method ( 'setProduct' )->with ( $productMock )->willReturnSelf ();
		$this->sendFriendMock->expects ( $this->once () )->method ( 'validate' )->willReturn ( [ 
				'Some error' 
		] );
		$this->sendFriendMock->expects ( $this->never () )->method ( 'send' );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( __ ( 'Some error' ) )->willReturnSelf ();
		
		$this->catalogSessionMock->expects ( $this->once () )->method ( 'setSendfriendFormData' )->with ( $formData );
		
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( 'sendfriend/product/send', [ 
				'_current' => true 
	...
	public function testExecuteWithoutValidationAndCategoryWithProblems() {
		...
		$this->sendFriendMock->expects ( $this->once () )->method ( 'setRecipients' )->with ( $recipients )->willReturnSelf ();
		$this->sendFriendMock->expects ( $this->once () )->method ( 'setProduct' )->with ( $productMock )->willReturnSelf ();
		$this->sendFriendMock->expects ( $this->once () )->method ( 'validate' )->willReturn ( 'Some error' );
		$this->sendFriendMock->expects ( $this->never () )->method ( 'send' );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( __ ( 'We found some problems with the data.' ) )->willReturnSelf ();
	...
	public function testExecuteWithLocalizedException() {
		...
		$this->sendFriendMock->expects ( $this->once () )->method ( 'setRecipients' )->with ( $recipients )->willReturnSelf ();
		$this->sendFriendMock->expects ( $this->once () )->method ( 'setProduct' )->with ( $productMock )->willReturnSelf ();
		$this->sendFriendMock->expects ( $this->once () )->method ( 'validate' )->willThrowException ( new \Magento\Framework\Exception\LocalizedException ( __ ( 'Localized Exception.' ) ) );
		$this->sendFriendMock->expects ( $this->never () )->method ( 'send' );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( __ ( 'Localized Exception.' ) )->willReturnSelf ();
	...
	public function testExecuteWithoutProduct() {
		...
		$this->resultFactoryMock->expects ( $this->exactly ( 2 ) )->method ( 'create' )->willReturnMap ( [ 
				[ 
		...
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->willReturnMap ( [ 
				[ 
						'id',
						null,
						null 
	...
	public function testExecuteWithoutFormKey() {
		...
		$this->resultFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturnMap ( [ 
				[ 
						\Magento\Framework\Controller\ResultFactory::TYPE_REDIRECT,
						[ ],
						$redirectMock 
		...
		$redirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( 'sendfriend/product/send', [ 
				'_current' => true 
		] )->willReturnSelf ();
