...
class ImagesTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->directoryWriteMock = $this->getMock ( '\Magento\Framework\Filesystem\Directory\Write', [ ], [ 
				'path' => $this->path 
		], '', false );
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'getAbsolutePath' )->willReturnMap ( [ 
				[ 
		...
		$this->imagesHelper = $this->objectManager->getObject ( 'Magento\Cms\Helper\Wysiwyg\Images', [ 
	...
	public function testGetStorageRoot() {
		$this->assertEquals ( $this->getAbsolutePath ( WysiwygConfig::IMAGE_DIRECTORY ), $this->imagesHelper->getStorageRoot () );
	}
	...
	public function testConvertPathToId() {
		...
		$this->assertEquals ( $this->imagesHelper->convertPathToId ( $pathOne ), $this->imagesHelper->convertPathToId ( $pathTwo ) );
	}
	...
	public function providerShortFilenameDefaultMaxLength() {
		return [ 
				...
				[ 
						'Some text for this unit test',
						'Some text for this u...' 
				] 
		];
	}
	...
	protected function generalSettingsIsUsingStaticUrlsAllowed($allowedValue) {
		...
		$this->eventManagerMock->expects ( $this->any () )->method ( 'dispatch' )->with ( 'cms_wysiwyg_images_static_urls_allowed', [ 
				'result' => $checkResult,
				'store_id' => $storeId 
	...
	public function testGetCurrentPath($pathId, $expectedPath, $isExist) {
		...
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'isDirectory' )->willReturnMap ( [ 
				[ 
						'/../wysiwyg/test_path',
						true 
				],
		...
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'getRelativePath' )->willReturnMap ( [ 
				[ 
						'PATH/wysiwyg/test_path',
						'/../wysiwyg/test_path' 
				],
				[ 
						'PATH/wysiwyg/my.jpg',
	...
	public function testGetCurrentPathThrowException() {
		...
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'create' )->willThrowException ( new \Magento\Framework\Exception\FileSystemException ( __ ( 'Could not create directory.' ) ) );
	...
	public function testGetImageHtmlDeclaration($baseUrl, $fileName, $isUsingStaticUrls, $expectedHtml) {
		...
		$this->backendDataMock->expects ( $this->any () )->method ( 'getUrl' )->with ( 'cms/wysiwyg/directive', [ 
