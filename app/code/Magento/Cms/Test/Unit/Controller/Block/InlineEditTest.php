...
class InlineEditTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->cmsBlock = $this->getMock ( 'Magento\Cms\Model\Block', [ ], [ ], '', false );
		$this->context = $helper->getObject ( 'Magento\Backend\App\Action\Context', [ 
				'request' => $this->request 
		] );
		$this->blockRepository = $this->getMockForAbstractClass ( 'Magento\Cms\Api\BlockRepositoryInterface', [ ], '', false );
		$this->resultJson = $this->getMock ( 'Magento\Framework\Controller\Result\Json', [ ], [ ], '', false );
		$this->jsonFactory = $this->getMock ( 'Magento\Framework\Controller\Result\JsonFactory', [ 
				'create' 
		], [ ], '', false );
		$this->controller = new InlineEdit ( $this->context, $this->blockRepository, $this->jsonFactory );
	}
	public function prepareMocksForTestExecute() {
		...
		$this->cmsBlock->expects ( $this->once () )->method ( 'getData' )->willReturn ( [ 
				'identifier' => 'test-identifier' 
		] );
		$this->cmsBlock->expects ( $this->once () )->method ( 'setData' )->with ( [ 
				'title' => 'Catalog Events Lister',
				'identifier' => 'Catalog Events Lister' 
		] );
