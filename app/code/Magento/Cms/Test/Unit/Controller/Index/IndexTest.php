...
class IndexTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->forwardFactoryMock = $this->getMockBuilder ( '\Magento\Framework\Controller\Result\ForwardFactory' )->setMethods ( [ 
				'create' 
		...
		$scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( \Magento\Cms\Helper\Page::XML_PATH_HOME_PAGE, \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->willReturn ( $this->pageId );
		$this->controller = $helper->getObject ( 'Magento\Cms\Controller\Index\Index', [ 
	...
	public function testExecuteResultPage() {
		$this->cmsHelperMock->expects ( $this->once () )->method ( 'prepareResultPage' )->with ( $this->controller, $this->pageId )->willReturn ( $this->resultPageMock );
