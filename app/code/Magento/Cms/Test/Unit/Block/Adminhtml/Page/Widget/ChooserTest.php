...
class ChooserTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->pageFactoryMock = $this->getMockBuilder ( 'Magento\Cms\Model\PageFactory' )->setMethods ( [ 
				'create' 
		...
		$this->cmsPageMock = $this->getMockBuilder ( 'Magento\Cms\Model\Page' )->disableOriginalConstructor ()->setMethods ( [ 
				'getTitle',
				'load',
				'getId' 
		] )->getMock ();
		...
		$this->this = $objectManager->getObject ( 'Magento\Cms\Block\Adminhtml\Page\Widget\Chooser', [ 
	...
	public function testPrepareElementHtml($elementValue, $cmsPageId) {
		...
		$this->urlBuilderMock->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->with ( 'cms/page_widget/chooser', [ 
				'uniq_id' => $uniqId 
