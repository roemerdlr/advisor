...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->model = $this->getMock ( 'Magento\Cms\Model\Block', [ 
				'getBlockId',
				'setStoreId' 
		], [ ], '', false );
		...
		$this->systemStore->expects ( $this->any () )->method ( 'getStoreValuesForForm' )->with ( false, true )->willReturn ( $this->storeValues );
		...
		$this->rootDirectory = $this->getMock ( 'Magento\Framework\Filesystem\Directory\ReadInterface', [ ], [ ], '', false );
		...
		$this->form->expects ( $this->once () )->method ( 'addFieldset' )->with ( 'base_fieldset', [ 
		...
		$this->block = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Cms\Block\Adminhtml\Block\Edit\Form', [ 
	...
	public function testPrepareFormModelHasNoBlockIdAndSingleStoreMode() {
		...
		$this->fieldSet->expects ( $this->at ( 2 ) )->method ( 'addField' )->with ( 'store_id', 'hidden', [ 
				'name' => 'stores[]',
				'value' => $storeId 
	...
	public function testPrepareFormModelHasBlockIdAndNonSingleStoreMode() {
		...
		$renderer = $this->getMock ( 'Magento\Backend\Block\Store\Switcher\Form\Renderer\Fieldset\Element', [ ], [ ], '', false );
		
		$this->model->expects ( $this->once () )->method ( 'getBlockId' )->willReturn ( $blockId );
		
		$this->layout->expects ( $this->once () )->method ( 'createBlock' )->with ( 'Magento\Backend\Block\Store\Switcher\Form\Renderer\Fieldset\Element' )->willReturn ( $renderer );
		
		$this->field->expects ( $this->once () )->method ( 'setRenderer' )->with ( $renderer );
		
		$this->fieldSet->expects ( $this->at ( 0 ) )->method ( 'addField' )->with ( 'block_id', 'hidden', [ 
				'name' => 'block_id' 
		] );
		$this->fieldSet->expects ( $this->at ( 3 ) )->method ( 'addField' )->with ( 'store_id', 'multiselect', [ 
				'name' => 'stores[]',
				'label' => __ ( 'Store View' ),
				'title' => __ ( 'Store View' ),
				'required' => true,
