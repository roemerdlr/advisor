...
class ChooserTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->escaper = $this->getMockBuilder ( 'Magento\Framework\Escaper' )->disableOriginalConstructor ()->setMethods ( [ 
				'escapeHtml' 
		] )->getMock ();
		$this->blockFactoryMock = $this->getMockBuilder ( 'Magento\Cms\Model\BlockFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->elementMock = $this->getMockBuilder ( 'Magento\Framework\Data\Form\Element\AbstractElement' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'getValue',
				'setData' 
		] )->getMock ();
		$this->modelBlockMock = $this->getMockBuilder ( 'Magento\Cms\Model\Block' )->disableOriginalConstructor ()->setMethods ( [ 
				'getTitle',
				'load',
				'getId' 
		] )->getMock ();
		$this->chooserMock = $this->getMockBuilder ( 'Magento\Framework\View\Element\BlockInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'setElement',
				'setConfig',
				'setFieldsetId',
				'setSourceUrl',
		...
		$this->this = $objectManager->getObject ( 'Magento\Cms\Block\Adminhtml\Block\Widget\Chooser', [ 
	...
	public function testPrepareElementHtml($elementValue, $modelBlockId) {
		...
		$this->urlBuilderMock->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->with ( 'cms/block_widget/chooser', [ 
				'uniq_id' => $uniqId 
