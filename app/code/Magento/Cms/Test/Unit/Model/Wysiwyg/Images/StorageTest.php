...
class StorageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->driverMock = $this->getMockForAbstractClass ( 'Magento\Framework\Filesystem\DriverInterface', [ ], '', false, false, true, [ 
				'getRealPath' 
		] );
		...
		$this->directoryMock->expects ( $this->any () )->method ( 'getDriver' )->will ( $this->returnValue ( $this->driverMock ) );
		
		$this->filesystemMock = $this->getMock ( 'Magento\Framework\Filesystem', [ 
				'getDirectoryWrite' 
		], [ ], '', false );
		...
		$this->adapterFactoryMock = $this->getMock ( 'Magento\Framework\Image\AdapterFactory', [ ], [ ], '', false );
		$this->imageHelperMock = $this->getMock ( 'Magento\Cms\Helper\Wysiwyg\Images', [ 
				'getStorageRoot' 
		], [ ], '', false );
		$this->imageHelperMock->expects ( $this->any () )->method ( 'getStorageRoot' )->will ( $this->returnValue ( self::STORAGE_ROOT_DIR ) );
		...
		$this->storageCollectionFactoryMock = $this->getMock ( 'Magento\Cms\Model\Wysiwyg\Images\Storage\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->storageFileFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\FileFactory', [ ], [ ], '', false );
		$this->storageDatabaseFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\DatabaseFactory', [ ], [ ], '', false );
		$this->directoryDatabaseFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\Directory\DatabaseFactory', [ 
				'create' 
		], [ ], '', false );
		$this->directoryCollectionMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\Directory\Database', [ ], [ ], '', false );
		
		$this->uploaderFactoryMock = $this->getMockBuilder ( 'Magento\MediaStorage\Model\File\UploaderFactory' )->disableOriginalConstructor ()->getMock ();
		...
		$this->coreFileStorageMock = $this->getMockBuilder ( 'Magento\MediaStorage\Helper\File\Storage\Database' )->disableOriginalConstructor ()->getMock ();
		
		$this->objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		
		$this->imagesStorage = $this->objectManagerHelper->getObject ( 'Magento\Cms\Model\Wysiwyg\Images\Storage', [ 
	...
	public function testDeleteDirectoryOverRoot() {
		$this->setExpectedException ( '\Magento\Framework\Exception\LocalizedException', sprintf ( 'Directory %s is not under storage root path.', self::INVALID_DIRECTORY_OVER_ROOT ) );
	...
	public function testDeleteRootDirectory() {
		$this->setExpectedException ( '\Magento\Framework\Exception\LocalizedException', sprintf ( 'We can\'t delete root directory %s right now.', self::STORAGE_ROOT_DIR ) );
	...
	public function testGetDirsCollectionCreateSubDirectories() {
		...
		$this->directoryCollectionMock->expects ( $this->once () )->method ( 'getSubdirectories' )->with ( self::STORAGE_ROOT_DIR )->willReturn ( [ 
				[ 
						'name' => $directoryName 
				] 
		] );
	...
	public function testGetDirsCollection($exclude, $include, $fileNames, $expectedRemoveKeys) {
		...
		foreach ( $fileNames as $filename ) {
			...
			$objectMock = $this->getMock ( 'Magento\Framework\DataObject', [ 
					'getFilename' 
			], [ ], '', false );
			$objectMock->expects ( $this->any () )->method ( 'getFilename' )->willReturn ( self::STORAGE_ROOT_DIR . $filename );
