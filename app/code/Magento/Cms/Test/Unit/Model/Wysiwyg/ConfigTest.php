...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->backendUrlMock = $this->getMockBuilder ( 'Magento\Backend\Model\UrlInterface' )->disableOriginalConstructor ()->getMock ();
		$this->assetRepoMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\Repository' )->disableOriginalConstructor ()->getMock ();
		$this->authorizationMock = $this->getMockBuilder ( 'Magento\Framework\AuthorizationInterface' )->disableOriginalConstructor ()->getMock ();
		$this->variableConfigMock = $this->getMockBuilder ( 'Magento\Variable\Model\Variable\Config' )->disableOriginalConstructor ()->getMock ();
		...
		$this->assetFileMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\File' )->disableOriginalConstructor ()->getMock ();
		$this->windowSize = [ 
				'width' => 1200,
				'height' => 800 
		];
		...
		$this->wysiwygConfig = $objectManager->getObject ( 'Magento\Cms\Model\Wysiwyg\Config', [ 
	...
	public function testGetConfig($data, $isAuthorizationAllowed, $expectedResults) {
		...
		$this->backendUrlMock->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->withConsecutive ( [ 
				'cms/wysiwyg/directive' 
		], [ 
				'cms/wysiwyg_images/index' 
		] );
		$this->assetRepoMock->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->withConsecutive ( [ 
