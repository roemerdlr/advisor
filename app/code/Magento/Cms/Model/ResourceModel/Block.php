...
class Block extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Store\Model\StoreManagerInterface $storeManager, $connectionName = null) {
	...
	protected function _getLoadSelect($field, $value, $object) {
		...
		if ($object->getStoreId ()) {
			...
			$select->join ( [ 
					...
					'store_id' 
			] )->where ( 'is_active = ?', 1 )->where ( 'cbs.store_id in (?)', $stores )->order ( 'store_id DESC' )->limit ( 1 );
		}
	...
	public function getIsUniqueBlockToStores(\Magento\Framework\Model\AbstractModel $object) {
		...
		$select = $this->getConnection ()->select ()->from ( [ 
				...
				'cbs' => $this->getTable ( 'cms_block_store' ) 
		], 'cb.block_id = cbs.block_id', [ ] )->where ( 'cb.identifier = ?', $object->getData ( 'identifier' ) )->where ( 'cbs.store_id IN (?)', $stores );
	...
	public function lookupStoreIds($id) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'cms_block_store' ), 'store_id' )->where ( 'block_id = :block_id' );
