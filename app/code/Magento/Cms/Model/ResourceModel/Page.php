...
class Page extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	protected function _getLoadSelect($field, $value, $object) {
		...
		if ($object->getStoreId ()) {
			...
			], $this->getMainTable () . '.page_id = cms_page_store.page_id', [ ] )->where ( 'is_active = ?', 1 )->where ( 'cms_page_store.store_id IN (?)', $storeIds )->order ( 'cms_page_store.store_id DESC' )->limit ( 1 );
		}
	...
	protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null) {
		...
		], 'cp.page_id = cps.page_id', [ ] )->where ( 'cp.identifier = ?', $identifier )->where ( 'cps.store_id IN (?)', $store );
	...
	public function lookupStoreIds($pageId) {
		...
		$select = $connection->select ()->from ( $this->getTable ( 'cms_page_store' ), 'store_id' )->where ( 'page_id = ?', ( int ) $pageId );
