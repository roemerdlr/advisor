...
class BackendAuthenticationTest extends \PHPUnit_Framework_TestCase {
	public function testAroundDispatch() {
		...
		$httpAuthentication->expects ( $this->once () )->method ( 'getCredentials' )->will ( $this->returnValue ( [ 
				$username,
				$password 
		] ) );
		...
		$plugin = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Rss\App\Action\Plugin\BackendAuthentication', [ 
				'auth' => $auth,
		...
		$this->assertSame ( $response, $plugin->aroundDispatch ( $subject, $proceed, $request ) );
	}
}
