...
class Sku extends \Magento\Backend\Block\Widget\Grid\Extended {
	...
	protected function _addColumnFilterToCollection($column) {
		...
		if ($column->getId () == 'in_products') {
			...
			if ($column->getFilter ()->getValue ()) {
				$this->getCollection ()->addFieldToFilter ( 'sku', [ 
						'in' => $selected 
				] );
			} else {
				$this->getCollection ()->addFieldToFilter ( 'sku', [ 
	...
	protected function _prepareCollection() {
		$collection = $this->_getCpCollectionInstance ()->setStoreId ( 0 )->addAttributeToSelect ( 'name', 'type_id', 'attribute_set_id' );
	...
	public function getGridUrl() {
		return $this->getUrl ( 'catalog_rule/*/chooser', [ 
				'_current' => true,
