...
class ActionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->productRuleProcessor = $this->getMockBuilder ( 'Magento\CatalogRule\Model\Indexer\Product\ProductRuleProcessor' )->disableOriginalConstructor ()->setMethods ( [ 
				'reindexList' 
		] )->getMock ();
	...
	public function testAfterUpdateAttributes() {
		...
		$result = $this->getMockBuilder ( 'Magento\Catalog\Model\Product\Action' )->disableOriginalConstructor ()->setMethods ( [ 
				'getAttributesData',
				'getProductIds' 
		] )->getMock ();
		
		$result->expects ( $this->once () )->method ( 'getAttributesData' )->willReturn ( [ ] );
		
		$result->expects ( $this->never () )->method ( 'getProductIds' );
