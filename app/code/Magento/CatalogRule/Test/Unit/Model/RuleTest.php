...
class RuleTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->storeModel = $this->getMock ( 'Magento\Store\Model\Store', [ 
				'__wakeup',
				'getId' 
		], [ ], '', false );
		$this->combineFactory = $this->getMock ( 'Magento\CatalogRule\Model\Rule\Condition\CombineFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->condition = $this->getMock ( 'Magento\Rule\Model\Condition\Combine', [ 
				'setRule',
				'validate' 
		], [ ], '', false );
		$this->websiteModel = $this->getMock ( 'Magento\Store\Model\Website', [ 
				'__wakeup',
				'getId',
				'getDefaultStore' 
		], [ ], '', false );
		$this->_ruleProductProcessor = $this->getMock ( '\Magento\CatalogRule\Model\Indexer\Rule\RuleProductProcessor', [ ], [ ], '', false );
		
		$this->_productCollectionFactory = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->_resourceIterator = $this->getMock ( '\Magento\Framework\Model\ResourceModel\Iterator', [ 
				'walk' 
		], [ ], '', false );
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->rule = $this->objectManagerHelper->getObject ( 'Magento\CatalogRule\Model\Rule', [ 
	...
	public function testCallbackValidateProduct($validate) {
		...
		$this->storeManager->expects ( $this->any () )->method ( 'getWebsites' )->with ( true )->will ( $this->returnValue ( [ 
				$this->websiteModel,
				$this->websiteModel 
		] ) );
	 ...
	 * 
	 * @param array $data
	 *        	Data for the rule actions
	 * @param bool|array $expected
	 *        	True or an array of errors
	 *        	
	 * @return void
	 */
