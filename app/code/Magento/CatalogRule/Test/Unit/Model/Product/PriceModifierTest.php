...
class PriceModifierTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->ruleFactoryMock = $this->getMock ( 'Magento\CatalogRule\Model\RuleFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testModifyPriceIfPriceExists($resultPrice, $expectedPrice) {
		...
		$this->ruleMock->expects ( $this->once () )->method ( 'calcProductPriceRule' )->with ( $this->productMock, 100 )->will ( $this->returnValue ( $resultPrice ) );
	...
	public function modifyPriceDataProvider() {
		return [ 
				'resulted_price_exists' => [ 
						150,
						150 
				],
				'resulted_price_not_exists' => [ 
						null,
						100 
				] 
		];
