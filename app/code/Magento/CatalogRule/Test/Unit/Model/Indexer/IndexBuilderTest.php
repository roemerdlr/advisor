...
class IndexBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->ruleCollectionFactory = $this->getMock ( 'Magento\CatalogRule\Model\ResourceModel\Rule\CollectionFactory', [ 
				'create',
				'addFieldToFilter' 
		], [ ], '', false );
		...
		$this->storeManager->expects ( $this->any () )->method ( 'getWebsites' )->will ( $this->returnValue ( [ 
				$this->website 
		] ) );
		...
		$this->rules->expects ( $this->any () )->method ( 'getWebsiteIds' )->will ( $this->returnValue ( [ 
				1 
		] ) );
		$this->rules->expects ( $this->any () )->method ( 'getCustomerGroupIds' )->will ( $this->returnValue ( [ 
				1 
		] ) );
		
		$this->ruleCollectionFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnSelf () );
		$this->ruleCollectionFactory->expects ( $this->any () )->method ( 'addFieldToFilter' )->will ( $this->returnValue ( [ 
				$this->rules 
		] ) );
		...
		$this->product->expects ( $this->any () )->method ( 'getWebsiteIds' )->will ( $this->returnValue ( [ 
				1 
		] ) );
	...
	public function testUpdateCatalogRuleGroupWebsiteData() {
		$priceAttrMock = $this->getMock ( 'Magento\Catalog\Model\Entity\Attribute', [ 
				'getBackend' 
		], [ ], '', false );
		$backendModelMock = $this->getMock ( 'Magento\Catalog\Model\Product\Attribute\Backend\Tierprice', [ 
				'getResource' 
		], [ ], '', false );
		$resourceMock = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Attribute\Backend\Tierprice', [ 
				'getMainTable' 
		], [ ], '', false );
		...
		$this->indexBuilder->reindexByIds ( [ 
				1 
		] );
	}
}
