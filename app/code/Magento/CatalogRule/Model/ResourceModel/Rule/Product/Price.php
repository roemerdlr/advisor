...
class Price extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 *        	the array of fields for compare with rule price and update
	...
	public function applyPriceRuleToIndexTable(\Magento\Framework\DB\Select $select, $indexTable, $entityId, $customerGroupId, $websiteId, $updateFields, $websiteDate) {
		...
		], "rp.rule_date = {$websiteDate}", [ ] )->where ( "rp.product_id = {$entityId} AND rp.website_id = {$websiteId} AND rp.customer_group_id = {$customerGroupId}" );
		
		foreach ( $updateFields as $priceField ) {
			$priceCond = $this->getConnection ()->quoteIdentifier ( [ 
					$indexAlias,
					$priceField 
			] );
			$priceExpr = $this->getConnection ()->getCheckSql ( "rp.rule_price < {$priceCond}", 'rp.rule_price', $priceCond );
			$select->columns ( [ 
					$priceField => $priceExpr 
			] );
		}
