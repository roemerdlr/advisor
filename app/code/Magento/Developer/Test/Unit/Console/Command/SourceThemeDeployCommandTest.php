...
class SourceThemeDeployCommandTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->validatorMock = $this->getMockBuilder ( Locale::class )->disableOriginalConstructor ()->getMock ();
		$this->assetPublisherMock = $this->getMockBuilder ( Publisher::class )->disableOriginalConstructor ()->getMock ();
		$this->assetRepositoryMock = $this->getMockBuilder ( Repository::class )->disableOriginalConstructor ()->getMock ();
		
		$this->sourceThemeDeployCommand = new SourceThemeDeployCommand ( $this->validatorMock, $this->assetPublisherMock, $this->assetRepositoryMock );
	}
	...
	public function testExecute() {
		...
		$this->assetRepositoryMock->expects ( self::once () )->method ( 'createAsset' )->with ( 'file-test-value/test' . DIRECTORY_SEPARATOR . 'file' . '.' . self::TYPE_TEST_VALUE, [ 
	...
	private function getInputMock() {
		...
		$inputMock->expects ( self::exactly ( 4 ) )->method ( 'getOption' )->willReturnMap ( [ 
				[ 
						'area',
		...
		$inputMock->expects ( self::once () )->method ( 'getArgument' )->with ( 'file' )->willReturn ( [ 
				self::FILE_TEST_VALUE 
		] );
