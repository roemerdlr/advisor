 ...
 *
 * @see \Magento\Developer\Model\View\Asset\PreProcessor\FrontendCompilation @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class FrontendCompilationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->alternativeSourceMock = $this->getMockBuilder ( AlternativeSourceInterface::class )->getMockForAbstractClass ();
	...
	public function testProcessException() {
		...
		$this->alternativeSourceMock->expects ( self::once () )->method ( 'getAlternativesExtensionsNames' )->willReturn ( [ 
				'less' 
		] );
		...
		$this->assetBuilderMock->expects ( self::once () )->method ( 'build' )->willThrowException ( new \Exception () );
		
		$this->assetSourceMock->expects ( self::never () )->method ( 'getContent' );
	...
	private function getChainMockExpects($content = '', $contentExactly = 1, $pathExactly = 1) {
		...
		$chainMock->expects ( self::exactly ( 3 ) )->method ( 'getAsset' )->willReturn ( $this->getAssetMockExpects ( $pathExactly ) );
	...
	private function getAssetMockExpects($pathExactly = 1) {
		...
		$assetMock->expects ( self::exactly ( $pathExactly ) )->method ( 'getFilePath' )->willReturn ( self::FILE_PATH );
