...
class PreprocessorStrategyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->frontendCompilationMock = $this->getMockBuilder ( FrontendCompilation::class )->disableOriginalConstructor ()->getMock ();
		$this->scopeConfigMock = $this->getMockBuilder ( ScopeConfigInterface::class )->getMockForAbstractClass ();
		
		$this->preprocessorStrategy = new PreprocessorStrategy ( $this->alternativeSourceMock, $this->frontendCompilationMock, $this->scopeConfigMock );
	}
	...
	public function testProcessClientSideCompilation() {
		...
		$this->scopeConfigMock->expects ( self::once () )->method ( 'getValue' )->with ( WorkflowType::CONFIG_NAME_PATH )->willReturn ( WorkflowType::CLIENT_SIDE_COMPILATION );
		
		$this->frontendCompilationMock->expects ( self::once () )->method ( 'process' )->with ( $chainMock );
		
		$this->alternativeSourceMock->expects ( self::never () )->method ( 'process' );
