...
class PublicationDecoratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerateRelatedFile() {
		...
		$fileTemporaryMock = $this->getMockBuilder ( Temporary::class )->disableOriginalConstructor ()->getMock ();
		...
		$importGeneratorMock = $this->getMockBuilder ( 'Magento\Framework\Css\PreProcessor\Instruction\Import' )->disableOriginalConstructor ()->getMock ();
		...
		$importGeneratorMock->expects ( self::any () )->method ( 'getRelatedFiles' )->will ( self::onConsecutiveCalls ( $relatedFiles, [ ] ) );
		
		$assetRepoMock->expects ( self::any () )->method ( 'createRelated' )->willReturn ( $relatedAssetMock );
		
		$publisherMock->expects ( self::once () )->method ( 'publish' )->with ( $relatedAssetMock );
		
		$model = new PublicationDecorator ( $filesystemMock, $assetRepoMock, $fileTemporaryMock, $publisherMock, $scopeConfigMock, true );
