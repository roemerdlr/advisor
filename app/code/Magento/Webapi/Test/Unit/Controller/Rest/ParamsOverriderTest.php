...
class ParamsOverriderTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *        	Data from config about which parameters to override
	 * @param array $expectedOverriddenParams
	 *        	Result of overriding $requestData when applying rules from $parameters
	 * @param int $userId
	 *        	The id of the user invoking the request
	 * @param int $userType
	 *        	The type of user invoking the request
	 *        	
	...
	public function testOverrideParams($requestData, $parameters, $expectedOverriddenParams, $userId, $userType) {
		...
		$userContextMock = $this->getMockBuilder ( 'Magento\Authorization\Model\UserContextInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getUserId',
				'getUserType' 
		...
		$paramOverriderCustomerId = $objectManager->getObject ( 'Magento\Webapi\Controller\Rest\ParamOverriderCustomerId', [ 
				'userContext' => $userContextMock 
		] );
		
		/** @var \Magento\Webapi\Controller\Rest\ParamsOverrider $paramsOverrider */
		$paramsOverrider = $objectManager->getObject ( 'Magento\Webapi\Controller\Rest\ParamsOverrider', [ 
				'paramOverriders' => [ 
