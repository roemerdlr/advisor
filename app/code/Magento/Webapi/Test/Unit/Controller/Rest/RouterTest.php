...
class RouterTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_routeMock = $this->getMockBuilder ( 'Magento\Webapi\Controller\Rest\Router\Route' )->disableOriginalConstructor ()->setMethods ( [ 
				'match' 
		] )->getMock ();
		...
		$this->_request = $objectManager->getObject ( 'Magento\Framework\Webapi\Rest\Request', [ 
		 ...
		 * Initialize SUT.
		 */
		$this->_router = $objectManager->getObject ( 'Magento\Webapi\Controller\Rest\Router', [ 
	...
	public function testMatch() {
		$this->_apiConfigMock->expects ( $this->once () )->method ( 'getRestRoutes' )->will ( $this->returnValue ( [ 
				$this->_routeMock 
		] ) );
		$this->_routeMock->expects ( $this->once () )->method ( 'match' )->with ( $this->_request )->will ( $this->returnValue ( [ ] ) );
