...
class PathProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAllStoreCode($storeCodeInPath, $storeCodeSet, $setCurrentStoreCallCtr = 1) {
		...
		$this->storeManagerMock->expects ( $this->exactly ( $setCurrentStoreCallCtr ) )->method ( 'setCurrentStore' )->with ( $storeCodeSet );
	...
	public function processPathDataProvider() {
		return [ 
				...
				'Arbitrary store code' => [ 
						$this->arbitraryStoreCode,
						$this->arbitraryStoreCode 
				],
				'Explicit default store code' => [ 
						'default',
						'default' 
				] 
		];
