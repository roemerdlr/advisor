...
class HandlerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		 ...
		 */
		$this->_apiConfigMock = $this->getMockBuilder ( 'Magento\Webapi\Model\Soap\Config' )->setMethods ( [ 
		...
		$this->_dataObjectConverter = $this->getMock ( 'Magento\Framework\Api\SimpleDataObjectConverter', [ 
				'convertStdObjectToArray' 
		], [ ], '', false );
		$this->_serviceInputProcessorMock = $this->getMock ( 'Magento\Framework\Webapi\ServiceInputProcessor', [ ], [ ], '', false );
		$this->_dataObjectProcessorMock = $this->getMock ( 'Magento\Framework\Reflection\DataObjectProcessor', [ ], [ ], '', false );
		$this->_methodsMapProcessorMock = $this->getMock ( 'Magento\Framework\Reflection\MethodsMap', [ ], [ ], '', false );
		
		/**
		 * Initialize SUT.
		 */
		$this->_handler = new \Magento\Webapi\Controller\Soap\Request\Handler ( $this->_requestMock, $this->_objectManagerMock, $this->_apiConfigMock, $this->_authorizationMock, $this->_dataObjectConverter, $this->_serviceInputProcessorMock, $this->_dataObjectProcessorMock, $this->_methodsMapProcessorMock );
		parent::setUp ();
	}
	public function testCall() {
		...
		$this->_dataObjectConverter->expects ( $this->once () )->method ( 'convertStdObjectToArray' )->will ( $this->returnValue ( [ 
				'field' => 1 
		] ) );
		...
		$aclResources = [ 
				[ 
						'Magento_TestModule::resourceA' 
				] 
		];
		$this->_apiConfigMock->expects ( $this->once () )->method ( 'getServiceMethodInfo' )->with ( $operationName, $requestedServices )->will ( $this->returnValue ( [ 
		...
		$serviceMock = $this->getMockBuilder ( $className )->disableOriginalConstructor ()->setMethods ( [ 
				$methodName 
		] )->getMock ();
		...
		], $this->_handler->__call ( $operationName, [ 
				( object ) [ 
						'field' => 1 
				] 
		] ) );
