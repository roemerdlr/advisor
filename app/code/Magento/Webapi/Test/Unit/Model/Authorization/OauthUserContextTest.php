...
class OauthUserContextTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->oauthRequestHelper = $this->getMockBuilder ( 'Magento\Framework\Oauth\Helper\Request' )->disableOriginalConstructor ()->setMethods ( [ 
				'prepareRequest',
				'getRequestUrl' 
		] )->getMock ();
		
		$this->oauthService = $this->getMockBuilder ( 'Magento\Framework\Oauth\Oauth' )->disableOriginalConstructor ()->setMethods ( [ 
				'validateAccessTokenRequest' 
		] )->getMock ();
		
		$this->oauthUserContext = $this->objectManager->getObject ( 'Magento\Webapi\Model\Authorization\OauthUserContext', [ 
	...
	public function testGetUserIdExist() {
		...
		$this->setupUserId ( $integrationId, [ 
				'oauth_token' => 'asdcfsdvanskdcalkdsjcfljldk' 
		] );
