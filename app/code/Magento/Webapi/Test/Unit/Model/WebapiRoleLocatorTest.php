...
class WebapiRoleLocatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->userContext = $this->getMockBuilder ( 'Magento\Authorization\Model\CompositeUserContext' )->disableOriginalConstructor ()->setMethods ( [ 
				'getUserId',
				'getUserType' 
		] )->getMock ();
		...
		$this->roleCollectionFactory = $this->getMockBuilder ( 'Magento\Authorization\Model\ResourceModel\Role\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->roleCollection = $this->getMockBuilder ( 'Magento\Authorization\Model\ResourceModel\Role\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'setUserFilter',
				'getFirstItem' 
		] )->getMock ();
		...
		$this->role = $this->getMockBuilder ( 'Magento\Authorization\Model\Role' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'__wakeup' 
		] )->getMock ();
		...
		$this->locator = $this->_objectManager->getObject ( 'Magento\Webapi\Model\WebapiRoleLocator', [ 
				'userContext' => $this->userContext,
				'roleCollectionFactory' => $this->roleCollectionFactory 
		] );
	}
	public function testNoRoleId() {
		$this->role->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( null ) );
