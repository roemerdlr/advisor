...
class ComplexTypeStrategyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_typeProcessor = $this->getMockBuilder ( 'Magento\Framework\Reflection\TypeProcessor' )->setMethods ( [ 
				'getTypeData' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->_wsdl = $this->getMockBuilder ( 'Magento\Webapi\Model\Soap\Wsdl' )->setMethods ( [ 
				'toDomDocument',
				'getTypes',
				'getSchema' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testAddComplexTypeSimpleParameters($type, $data) {
		...
		$this->_typeProcessor->expects ( $this->at ( 0 ) )->method ( 'getTypeData' )->with ( $type )->will ( $this->returnValue ( $data ) );
						...
						[ 
								...
								'parameters' => [ 
										'string_param' => [ 
												'type' => 'string',
												'required' => false,
												'documentation' => '{callInfo:VendorModuleACreate:requiredInput:conditionally}' . '{callInfo:allCallsExcept(VendorModuleAGet):returned:always}' 
										] 
								] 
						] 
				],
