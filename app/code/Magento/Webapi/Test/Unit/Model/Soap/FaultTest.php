...
class FaultTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$code = 111;
		$webapiException = new \Magento\Framework\Webapi\Exception ( __ ( $message ), $code, \Magento\Framework\Webapi\Exception::HTTP_INTERNAL_ERROR, $details );
...
XML;
		...
		$this->assertEquals ( $this->_sanitizeXML ( $expectedResult ), $this->_sanitizeXML ( $actualXml ), 'Wrong SOAP fault message with default parameters.' );
	}
	 ...
	 */
	public function testGetSoapFaultMessage($faultReason, $faultCode, $additionalParameters, $expectedResult, $assertMessage) {
		$actualResult = $this->_soapFault->getSoapFaultMessage ( $faultReason, $faultCode, $additionalParameters );
		$wsdlUrl = urlencode ( self::WSDL_URL );
		$this->assertEquals ( $this->_sanitizeXML ( str_replace ( '{wsdl_url}', $wsdlUrl, $expectedResult ) ), $this->_sanitizeXML ( $actualResult ), $assertMessage );
	}
