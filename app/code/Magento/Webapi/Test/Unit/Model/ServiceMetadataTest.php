...
class ServiceMetadataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$classReflection = $this->getMock ( 'Magento\Webapi\Model\Config\ClassReflector', [ 
				'reflectClassMethods',
				'extractClassDescription' 
		], [ ], '', false );
		$classReflection->expects ( $this->any () )->method ( 'reflectClassMethods' )->will ( $this->returnValue ( $interfaceParameters ) );
		$classReflection->expects ( $this->any () )->method ( 'extractClassDescription' )->will ( $this->returnValue ( 'classDescription' ) );
		...
		$this->serviceMetadata = new \Magento\Webapi\Model\ServiceMetadata ( $config, $cacheMock, $classReflection, $typeProcessor );
	...
	public function dataProviderForTestGetServiceNameInvalidName() {
		return [ 
				[ 
						'BarV1Interface',
						'V1' 
				], // Missed vendor, module, 'Service'
				[ 
						'Service\\V1Interface',
						'V1' 
				], // Missed vendor and module
				[ 
