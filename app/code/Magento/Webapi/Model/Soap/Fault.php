...
class Fault {
	...
	public function toXml() {
		if ($this->appState->getMode () == State::MODE_DEVELOPER) {
			$this->addDetails ( [ 
					self::NODE_DETAIL_TRACE => "<![CDATA[{$this->stackTrace}]]>" 
			] );
		}
		if ($this->getParameters ()) {
			$this->addDetails ( [ 
					self::NODE_DETAIL_PARAMETERS => $this->getParameters () 
			] );
		}
		if ($this->getWrappedErrors ()) {
			$this->addDetails ( [ 
					self::NODE_DETAIL_WRAPPED_ERRORS => $this->getWrappedErrors () 
			] );
		}
	 ...
	 *
	 * @param array $details
	 *        	Associative array containing details about current fault
	 * @return $this
	 */
	...
	protected function _generateDetailXml($details) {
		...
		if (is_array ( $details ) && ! empty ( $details )) {
			...
			if ($detailsXml) {
				...
				$detailsXml = "<env:Detail><m:{$errorDetailsNode}>" . $detailsXml . "</m:{$errorDetailsNode}></env:Detail>";
			} else {
				$detailsXml = '';
			}
		}
	...
	protected function _getParametersXml($parameters) {
		...
		foreach ( $parameters as $parameterName => $parameterValue ) {
			if (is_string ( $parameterName ) && (is_string ( $parameterValue ) || is_numeric ( $parameterValue ))) {
				...
				$paramsXml .= "<m:$parameterNode><m:$keyNode>$parameterName</m:$keyNode><m:$valueNode>" . htmlspecialchars ( $parameterValue ) . "</m:$valueNode></m:$parameterNode>";
			}
		}
		if (! empty ( $paramsXml )) {
	...
	protected function _generateErrorNodeXML($error) {
		...
		if (! empty ( $parameters )) {
			...
			foreach ( $parameters as $key => $value ) {
				...
				$xml .= "<m:$parameterNode>" . "<m:$keyNode>$key</m:$keyNode><m:$valueNode>$value</m:$valueNode>" . "</m:$parameterNode>";
			}
			$xml .= "</m:$parametersNode>";
		}
