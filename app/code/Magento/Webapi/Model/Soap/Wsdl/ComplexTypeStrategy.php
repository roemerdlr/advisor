...
class ComplexTypeStrategy extends AbstractComplexTypeStrategy {
	 ...
	 *        	array of callInfo from parent complex type
	 * @return string
	...
	protected function _processParameters($parameters, $callInfo) {
		...
		foreach ( $parameters as $parameterName => $parameterData ) {
			...
			if ($this->_typeProcessor->isArrayType ( $parameterType )) {
				...
				$element->setAttribute ( 'type', Wsdl::TYPES_NS . ':' . $this->_typeProcessor->translateArrayTypeName ( $parameterType ) );
				if (! $isRequired) {
	...
	public function addAnnotation(\DOMElement $element, $documentation, $default = null, $callInfo = []) {
		...
		$appInfoNode->setAttributeNS ( Wsdl::XML_NS_URI, Wsdl::XML_NS . ':' . self::APP_INF_NS, $this->getContext ()->getTargetNamespace () );
	 ...
	 *
	 * @param \DOMElement $element        	
	 * @return string|null @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
