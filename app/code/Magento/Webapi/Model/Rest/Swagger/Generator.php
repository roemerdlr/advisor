...
class Generator extends AbstractSchemaGenerator {
	...
	protected function generateMethodResponses($methodData) {
		...
		if (isset ( $methodData ['interface'] ['out'] ['parameters'] ) && is_array ( $methodData ['interface'] ['out'] ['parameters'] )) {
		...
		if (isset ( $methodData ['interface'] ['out'] ['throws'] ) && is_array ( $methodData ['interface'] ['out'] ['throws'] )) {
	...
	private function generateMethodParameters($httpMethodData) {
		 ...
		 */
		if (! isset ( $phpMethodData ['interface'] ['in'] ['parameters'] ) || ! isset ( $httpMethodData ['uri'] ) || ! isset ( $httpMethodData ['httpOperation'] )) {
			return [ ];
		}
		...
		foreach ( $phpMethodData ['interface'] ['in'] ['parameters'] as $parameterName => $parameterInfo ) {
			 ...
			 */
			if (isset ( $httpMethodData ['parameters'] [$parameterName] ['force'] ) && $httpMethodData ['parameters'] [$parameterName] ['force']) {
				continue;
			}
			...
			if (strpos ( $httpMethodData ['uri'], '{' . $parameterName . '}' ) !== false) {
				...
				$parameters = $this->generateMethodQueryParameters ( $parameterName, $parameterInfo, $description, $parameters );
			} else {
				$bodySchema = $this->generateBodySchema ( $parameterName, $parameterInfo, $description, $bodySchema );
			}
		}
		...
		$remainingPathParams = array_diff ( $allPathParams [1], array_keys ( $phpMethodData ['interface'] ['in'] ['parameters'] ) );
	...
	protected function getObjectSchema($typeName, $description = '') {
		...
		if ($simpleType == false) {
			...
			if ($simpleType = $this->getSimpleType ( $trimedTypeName )) {
				...
				if (strpos ( $typeName, '[]' )) {
					$result ['items'] = [ 
							'$ref' => $this->getDefinitionReference ( $trimedTypeName ) 
					];
				} else {
					$result = [ 
							'$ref' => $this->getDefinitionReference ( $trimedTypeName ) 
	...
	protected function generateDefinition($typeName) {
		...
		if (isset ( $typeData ['parameters'] )) {
			foreach ( $typeData ['parameters'] as $parameterName => $parameterData ) {
				$properties [$parameterName] = $this->getObjectSchema ( $parameterData ['type'], $parameterData ['documentation'] );
	 ...
	 * Get definitions
	 *
	 * @return array Todo: create interface for error response
	 */
	...
	private function handleComplex($name, $type, $prefix, $isArray) {
		...
		foreach ( $parameters as $subParameterName => $subParameterInfo ) {
			...
			$subParameterDescription = isset ( $subParameterInfo ['documentation'] ) ? $subParameterInfo ['documentation'] : null;
			$subPrefix = $prefix ? $prefix . '[' . $name . ']' : $name;
			if ($isArray) {
				$subPrefix .= self::ARRAY_SIGNIFIER;
			}
			$queryNames = array_merge ( $queryNames, $this->getQueryParamNames ( $subParameterName, $subParameterType, $subParameterDescription, $subPrefix ) );
		}
		return $queryNames;
	}
	...
	private function generateMethodQueryParameters($parameterName, $parameterInfo, $description, $parameters) {
		...
		if (count ( $queryParams ) === 1) {
			...
			$parameters [] = $this->createQueryParam ( $parameterName, $parameterInfo ['type'], $description, $parameterInfo ['required'] );
		} else {
			/**
			...
			foreach ( $queryParams as $name => $queryParamInfo ) {
				$parameters [] = $this->createQueryParam ( $name, $queryParamInfo ['type'], $queryParamInfo ['description'] );
			}
		}
		return $parameters;
	}
	...
	private function generateBodySchema($parameterName, $parameterInfo, $description, $bodySchema) {
		...
		$bodySchema ['properties'] [$parameterName] = $this->getObjectSchema ( $parameterInfo ['type'], $description );
	...
	private function generateMethodExceptionErrorResponses($exceptionClass, $responses) {
		...
		if (is_subclass_of ( $exceptionClass, '\Magento\Framework\Exception\LocalizedException' )) {
			...
			} elseif (is_subclass_of ( $exceptionClass, '\Magento\Framework\Exception\AuthorizationException' ) || is_subclass_of ( $exceptionClass, '\Magento\Framework\Exception\AuthenticationException' )) {
