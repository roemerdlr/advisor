...
class Handler {
	 ...
	 */
	public function __construct(SoapRequest $request, \Magento\Framework\ObjectManagerInterface $objectManager, SoapConfig $apiConfig, AuthorizationInterface $authorization, SimpleDataObjectConverter $dataObjectConverter, ServiceInputProcessor $serviceInputProcessor, DataObjectProcessor $dataObjectProcessor, MethodsMap $methodsMapProcessor) {
	...
	public function __call($operation, $arguments) {
		...
		if (! $isAllowed) {
			throw new AuthorizationException ( __ ( AuthorizationException::NOT_AUTHORIZED, [ 
		...
		$outputData = call_user_func_array ( [ 
				$service,
				$serviceMethod 
		], $inputData );
	...
	protected function _prepareRequestData($serviceClass, $serviceMethod, $arguments) {
		/**
		 * SoapServer wraps parameters into array.
		 * Thus this wrapping should be removed to get access to parameters.
		 */
