...
class Soap implements \Magento\Framework\App\FrontControllerInterface {
	...
	/**
	 * Check if current request is WSDL request.
	 * SOAP operation execution request is another type of requests.
	 *
	 * @return bool
	 */
	...
	/**
	 * Parse the Authorization header and return the access token e.g.
	...
	protected function _setResponseContentType($contentType = 'text/xml') {
		$this->_response->clearHeaders ()->setHeader ( 'Content-Type', "{$contentType}; charset={$this->_soapServer->getApiCharset()}" );
		return $this;
	}
