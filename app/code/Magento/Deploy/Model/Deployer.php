...
class Deployer {
	 ...
	 * @param bool $isDryRun        	
	 */
	public function __construct(Files $filesUtil, OutputInterface $output, Version\StorageInterface $versionStorage, JsTranslationConfig $jsTranslationConfig, array $alternativeSources, $isDryRun = false) {
		...
		array_map ( function (AlternativeSourceInterface $alternative) {
		}, $alternativeSources );
		...
		foreach ( $areas as $area => $themes ) {
			...
			foreach ( $locales as $locale ) {
				...
				foreach ( $themes as $themePath ) {
					...
					$assetRepo = $this->objectManager->create ( 'Magento\Framework\View\Asset\Repository', [ 
							'design' => $design 
					] );
					/** @var \Magento\RequireJs\Model\FileManager $fileManager */
					$fileManager = $this->objectManager->create ( 'Magento\RequireJs\Model\FileManager', [ 
							'config' => $this->objectManager->create ( 'Magento\Framework\RequireJs\Config', [ 
									'assetRepo' => $assetRepo,
									'design' => $design 
							] ),
							'assetRepo' => $assetRepo 
	...
	private function collectAppFiles($requestedLocales) {
		...
		if (! empty ( $locales )) {
			$langList = implode ( ', ', $locales );
			$this->output->writeln ( "WARNING: there were files for the following languages detected in the file system: {$langList}." . ' These languages were not requested, so the files will not be populated.' );
		}
	...
	private function deployFile($filePath, $area, $themePath, $locale, $module) {
		...
		try {
			$asset = $this->assetRepo->createAsset ( $filePath, [ 
					'area' => $area,
					'theme' => $themePath,
					'locale' => $locale,
					'module' => $module 
