...
class AjaxLogin {
	 ...
	 * @param array $formIds        	
	 */
	public function __construct(CaptchaHelper $helper, SessionManagerInterface $sessionManager, JsonFactory $resultJsonFactory, array $formIds) {
		$this->helper = $helper;
	 ...
	 * @return $this
	 * @throws \Zend_Json_Exception @SuppressWarnings(PHPMD.NPathComplexity)
		...
		foreach ( $this->formIds as $formId ) {
			...
			if ($captchaModel->isRequired ( $username ) && ! in_array ( $loginFormId, $this->formIds )) {
				...
				return $resultJson->setData ( [ 
						'errors' => true,
						'message' => __ ( 'Provided form does not exist' ) 
				] );
			}
