...
class DefaultModel extends \Zend_Captcha_Image implements \Magento\Captcha\Model\CaptchaInterface {
	 ...
	 * @param string $formId        	
	 */
	public function __construct(\Magento\Framework\Session\SessionManagerInterface $session, \Magento\Captcha\Helper\Data $captchaData, \Magento\Captcha\Model\ResourceModel\LogFactory $resLogFactory, $formId) {
	...
	public function isRequired($login = null) {
		if ($this->_isUserAuth () && ! $this->isShownToLoggedInUser () || ! $this->_isEnabled () || ! in_array ( $this->_formId, $this->_getTargetForms () )) {
			return false;
		}
		
		return $this->_isShowAlways () || $this->_isOverLimitAttempts ( $login ) || $this->_session->getData ( $this->_getFormIdKey ( 'show_captcha' ) );
	}
	...
	/**
	 * Returns length for generating captcha word.
	 * This value may be dynamic.
	 *
	 * @return int
	 */
	...
	protected function _isShowAlways() {
		...
		if (( string ) $this->_captchaData->getConfig ( 'mode' ) == \Magento\Captcha\Helper\Data::MODE_AFTER_FAIL && $this->_getAllowedAttemptsForSameLogin () == 0) {
			return true;
		}
	...
	protected function _setWord($word) {
		$this->_session->setData ( $this->_getFormIdKey ( self::SESSION_WORD ), [ 
				'data' => $word,
	 ...
	 *
	 * @return void Now deleting old captcha images make crontab script
