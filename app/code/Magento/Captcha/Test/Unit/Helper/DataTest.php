...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetCaptcha() {
		$this->configMock->expects ( $this->once () )->method ( 'getValue' )->with ( 'customer/captcha/type' )->will ( $this->returnValue ( 'zend' ) );
		
		$this->factoryMock->expects ( $this->once () )->method ( 'create' )->with ( $this->equalTo ( 'Zend' ) )->will ( $this->returnValue ( new \Magento\Captcha\Model\DefaultModel ( $this->getMock ( 'Magento\Framework\Session\SessionManager', [ ], [ ], '', false ), $this->getMock ( 'Magento\Captcha\Helper\Data', [ ], [ ], '', false ), $this->getMock ( 'Magento\Captcha\Model\ResourceModel\LogFactory', [ 
				'create' 
		], [ ], '', false ), 'user_create' ) ) );
	...
	public function testGetImgDir() {
		$dirWriteMock = $this->getMock ( 'Magento\Framework\Filesystem\Directory\Write', [ 
				'changePermissions',
				'create',
				'getAbsolutePath' 
		], [ ], '', false );
		
		$this->_filesystem->expects ( $this->once () )->method ( 'getDirectoryWrite' )->with ( DirectoryList::MEDIA )->will ( $this->returnValue ( $dirWriteMock ) );
		
		$dirWriteMock->expects ( $this->once () )->method ( 'getAbsolutePath' )->with ( '/captcha/base' )->will ( $this->returnValue ( TESTS_TEMP_DIR . '/captcha/base' ) );
