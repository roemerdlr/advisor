...
class DefaultTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_storeManager->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( $this->_getStoreStub () ) );
		
		// \Magento\Customer\Model\Session
		$this->_objectManager = $this->getMock ( 'Magento\Framework\ObjectManagerInterface' );
		$this->_objectManager->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValueMap ( [ 
				'Magento\Captcha\Helper\Data' => $this->_getHelperStub (),
				'Magento\Customer\Model\Session' => $this->session 
		] ) );
		
		$this->_resLogFactory = $this->getMock ( 'Magento\Captcha\Model\ResourceModel\LogFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_resLogFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_getResourceModelStub () ) );
		
		$this->_object = new \Magento\Captcha\Model\DefaultModel ( $this->session, $this->_getHelperStub (), $this->_resLogFactory, 'user_create' );
	}
	...
	public function testGetImgSrc() {
		$this->assertEquals ( $this->_object->getImgSrc (), 'http://localhost/pub/media/captcha/base/' . $this->_object->getId () . '.png' );
	}
	...
	public function testLogAttempt() {
		$captcha = new \Magento\Captcha\Model\DefaultModel ( $this->session, $this->_getHelperStub (), $this->_resLogFactory, 'user_create' );
	...
	protected function _getSessionStub() {
		...
		$sessionArgs = $helper->getConstructArguments ( 'Magento\Customer\Model\Session', [ 
	...
	protected function _getHelperStub() {
		$helper = $this->getMockBuilder ( 'Magento\Captcha\Helper\Data' )->disableOriginalConstructor ()->setMethods ( [ 
				'getConfig',
				'getFonts',
				'_getWebsiteCode',
				'getImgUrl' 
		...
		$helper->expects ( $this->any () )->method ( 'getConfig' )->will ( $this->returnCallback ( 'Magento\Captcha\Test\Unit\Model\DefaultTest::getConfigNodeStub' ) );
		...
		$helper->expects ( $this->any () )->method ( 'getImgUrl' )->will ( $this->returnValue ( 'http://localhost/pub/media/captcha/base/' ) );
	...
	protected function _getResourceModelStub() {
		$resourceModel = $this->getMock ( 'Magento\Captcha\Model\ResourceModel\Log', [ 
	...
	public function testIsShownToLoggedInUser($expectedResult, $formId) {
		$captcha = new \Magento\Captcha\Model\DefaultModel ( $this->session, $this->_getHelperStub (), $this->_resLogFactory, $formId );
