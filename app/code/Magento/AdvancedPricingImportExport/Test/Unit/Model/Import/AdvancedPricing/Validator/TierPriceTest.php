...
class TierPriceTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->groupRepository = $this->getMockBuilder ( '\Magento\Customer\Api\GroupRepositoryInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getList' 
		...
		$this->tierPrice = $this->getMock ( '\Magento\AdvancedPricingImportExport\Model\Import\AdvancedPricing\Validator\TierPrice', [ 
	...
	public function testInitInternalCalls() {
		...
		$groupTest = $this->getMockBuilder ( '\Magento\Customer\Api\Data\GroupInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getCode',
				'getId' 
		] )->getMockForAbstractClass ();
	...
	public function testIsValidAddMessagesCall($value, $hasEmptyColumns, $customerGroups, $expectedMessages) {
		$priceContextMock = $this->getMock ( '\Magento\CatalogImportExport\Model\Import\Product', [ ], [ 
	...
	public function isValidAddMessagesCallDataProvider() {
		return [
				...
				[ 
						...
						'$expectedMessages' => [ 
								AdvancedPricing\Validator::ERROR_TIER_DATA_INCOMPLETE 
						] 
				],
				// Second if condition cases.
				[ 
						...
						'$expectedMessages' => [ 
								AdvancedPricing\Validator::ERROR_INVALID_TIER_PRICE_GROUP 
						] 
				],
				// Third if condition cases.
				[ 
