...
class AdvancedPricing extends \Magento\CatalogImportExport\Model\Export\Product {
	 ...
	 *
	 * @return array|mixed @SuppressWarnings(PHPMD.CyclomaticComplexity)
		...
		try {
			...
			if (isset ( $productIds )) {
				if (! $this->_passTierPrice) {
					$exportData = array_merge ( $exportData, $this->getTierPrices ( $productIds, ImportAdvancedPricing::TABLE_TIER_PRICE ) );
				}
			}
			if ($exportData) {
			...
			foreach ( $exportRow as $keyTemplate => $valueTemplate ) {
				if (isset ( $row [$keyTemplate] )) {
					if (in_array ( $keyTemplate, $this->_priceWebsite )) {
						$exportRow [$keyTemplate] = $this->_getWebsiteCode ( $row [$keyTemplate] );
					} elseif (in_array ( $keyTemplate, $this->_priceCustomerGroup )) {
						$exportRow [$keyTemplate] = $this->_getCustomerGroupById ( $row [$keyTemplate], isset ( $row [ImportAdvancedPricing::VALUE_ALL_GROUPS] ) ? $row [ImportAdvancedPricing::VALUE_ALL_GROUPS] : null );
	 ...
	 * @param array $listSku        	
	 * @param string $table        	
	 * @return array|bool @SuppressWarnings(PHPMD.NPathComplexity)
	...
	protected function getTierPrices(array $listSku, $table) {
		...
		if ($listSku) {
			...
			try {
				...
				], $selectFields )->joinInner ( [ 
						'ap' => $this->_resource->getTableName ( $table ) 
				], 'ap.entity_id = cpe.entity_id', [ ] )->where ( 'cpe.entity_id IN (?)', $listSku );
	...
	protected function _getWebsiteCode($websiteId) {
		$storeName = ($websiteId == 0) ? ImportAdvancedPricing::VALUE_ALL_WEBSITES : $this->_storeManager->getWebsite ( $websiteId )->getName ();
		if ($websiteId == 0) {
