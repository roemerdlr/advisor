...
class AdvancedPricing extends \Magento\ImportExport\Model\Import\Entity\AbstractEntity {
	...
	protected function saveAndReplaceAdvancedPrices() {
		...
		while ( $bunch = $this->_dataSourceModel->getNextBunch () ) {
			...
			if (\Magento\ImportExport\Model\Import::BEHAVIOR_REPLACE == $behavior) {
				...
				$this->processCountExistingPrices ( $tierPrices, self::TABLE_TIER_PRICE )->processCountNewPrices ( $tierPrices );
				$this->saveProductPrices ( $tierPrices, self::TABLE_TIER_PRICE );
				if ($listSku) {
					$this->setUpdatedAt ( $listSku );
				}
	...
	protected function saveProductPrices(array $priceData, $table) {
		if ($priceData) {
			...
			if ($priceIn) {
				$this->_connection->insertOnDuplicate ( $tableName, $priceIn, [ 
						'value' 
				] );
			}
		}
	...
	protected function deleteProductTierPrices(array $listSku, $tableName) {
		if ($tableName && $listSku) {
			if (! $this->_cachedSkuToDelete) {
				$this->_cachedSkuToDelete = $this->_connection->fetchCol ( $this->_connection->select ()->from ( $this->_catalogProductEntity, 'entity_id' )->where ( 'sku IN (?)', $listSku ) );
			}
			if ($this->_cachedSkuToDelete) {
				try {
					$this->countItemsDeleted += $this->_connection->delete ( $tableName, $this->_connection->quoteInto ( 'entity_id IN (?)', $this->_cachedSkuToDelete ) );
					return true;
				} catch ( \Exception $e ) {
					return false;
				}
	...
	protected function setUpdatedAt(array $listSku) {
		...
		$this->_connection->update ( $this->_catalogProductEntity, [ 
	...
	protected function retrieveOldSkus() {
		$oldSkus = $this->_connection->fetchPairs ( $this->_connection->select ()->from ( $this->_connection->getTableName ( 'catalog_product_entity' ), [ 
				'sku',
				'entity_id' 
		] ) );
		return $oldSkus;
	...
	protected function processCountExistingPrices($prices, $table) {
		$existingPrices = $this->_connection->fetchAssoc ( $this->_connection->select ()->from ( $this->_connection->getTableName ( $table ), [ 
				'value_id',
				'entity_id',
				'all_groups',
				'customer_group_id' 
	...
	protected function incrementCounterUpdated($prices, $existingPrice) {
		foreach ( $prices as $price ) {
			if ($existingPrice ['all_groups'] == $price ['all_groups'] && $existingPrice ['customer_group_id'] == $price ['customer_group_id']) {
