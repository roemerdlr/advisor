...
class Parser implements \Magento\Framework\Translate\Inline\ParserInterface {
	...
	public function processAjaxPost(array $translateParams) {
		...
		$this->_filterTranslationParams ( $translateParams, [ 
				'custom' 
		] );
	...
	protected function _validateTranslationParams(array $translateParams) {
		foreach ( $translateParams as $param ) {
			if (! is_array ( $param ) || ! isset ( $param ['original'] ) || ! isset ( $param ['custom'] )) {
				throw new \InvalidArgumentException ( 'Both original and custom phrases are required for inline translation.' );
			}
		}
	}
	 ...
	 *        	Names of fields values of which are to be filtered
	 * @return void
	 */
	 ...
	 * Adding translate mode attribute for vde requests.
	 *
	...
	protected function _applySpecialTagsFormat($tagHtml, $tagName, $trArr) {
		$specialTags = $tagHtml . '<span class="translate-inline-' . $tagName . '" ' . $this->_getHtmlAttribute ( self::DATA_TRANSLATE, '[' . join ( ',', $trArr ) . ']' );
	 ...
	 * Added translate mode attribute for vde requests.
	 *
	 * @param string $tagHtml        	
	 * @param string $tagName        	
	 * @param array $trArr        	
	...
	protected function _applySimpleTagsFormat($tagHtml, $tagName, $trArr) {
		$simpleTags = substr ( $tagHtml, 0, strlen ( $tagName ) + 1 ) . ' ' . $this->_getHtmlAttribute ( self::DATA_TRANSLATE, htmlspecialchars ( '[' . join ( ',', $trArr ) . ']' ) );
		$additionalAttr = $this->_getAdditionalHtmlAttribute ( $tagName );
		if ($additionalAttr !== null) {
	...
	private function _prepareTagAttributesForContent(&$content) {
		...
		while ( preg_match ( $tagRegExp, $content, $tagMatch, PREG_OFFSET_CAPTURE, $nextTag ) ) {
			...
			$trArr = $this->_getTranslateData ( $attrRegExp, $tagHtml, [ 
					$this,
					'_getAttributeLocation' 
			] );
			if ($trArr) {
				$transRegExp = '# ' . $this->_getHtmlAttribute ( self::DATA_TRANSLATE, '\[([^' . preg_quote ( $quoteHtml ) . ']*)]' ) . '#i';
				if (preg_match ( $transRegExp, $tagHtml, $matches )) {
					$tagHtml = str_replace ( $matches [0], '', $tagHtml );
					$trAttr = ' ' . $this->_getHtmlAttribute ( self::DATA_TRANSLATE, '[' . htmlspecialchars ( $matches [1] ) . ',' . join ( ',', $trArr ) . ']' );
				} else {
					$trAttr = ' ' . $this->_getHtmlAttribute ( self::DATA_TRANSLATE, '[' . join ( ',', $trArr ) . ']' );
				}
	...
	private function _translateTags(&$content, $tagsList, $formatCallback) {
		...
		while ( preg_match ( $tagRegExp, $content, $tagMatch, PREG_OFFSET_CAPTURE, $nextTag ) ) {
			...
			$tagHtml = $tagMatch [0] [0] . substr ( $content, $tagMatch [0] [1] + $tagStartLength, $tagLength - $tagStartLength );
			$tagClosurePos = $tagMatch [0] [1] + strlen ( $tagHtml );
			
			$trArr = $this->_getTranslateData ( '#' . self::REGEXP_TOKEN . '#iS', $tagHtml, [ 
					$this,
					'_getTagLocation' 
			], [ 
					'tagName' => $tagName,
			...
			if (! empty ( $trArr )) {
				...
				$tagHtml = call_user_func ( [ 
						$this,
						$formatCallback 
				], $tagHtml, $tagName, $trArr );
				$tagClosurePos = $tagMatch [0] [1] + strlen ( $tagHtml );
	...
	private function _otherText() {
		...
		while ( preg_match ( '#' . self::REGEXP_TOKEN . '#', $this->_content, $matches, PREG_OFFSET_CAPTURE, $next ) ) {
			...
			$spanHtml = $this->_getDataTranslateSpan ( '[' . htmlspecialchars ( $translateProperties ) . ']', $matches [1] [0] );
