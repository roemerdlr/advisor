...
class CacheTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLoadPartnersFromCache() {
		...
		$this->cacheHelperMock->expects ( $this->once () )->method ( 'getCache' )->will ( $this->returnValue ( $cache ) );
		$cache->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( '' ) );
	...
	public function testSavePartnersToCache() {
		$cache = $this->getCacheMock ();
		$this->cacheHelperMock->expects ( $this->once () )->method ( 'getCache' )->will ( $this->returnValue ( $cache ) );
		$cache->expects ( $this->once () )->method ( 'save' )->will ( $this->returnValue ( true ) );
