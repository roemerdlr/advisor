...
class PartnersTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetPartners() {
		$partnersModelMock = $this->getPartnersModelMock ( [ 
				'getPartners' 
		] );
		$partnersModelMock->expects ( $this->once () )->method ( 'getPartners' )->will ( $this->returnValue ( [ ] ) );
		
		$this->partnersBlockMock->expects ( $this->once () )->method ( 'getPartnersModel' )->will ( $this->returnValue ( $partnersModelMock ) );
