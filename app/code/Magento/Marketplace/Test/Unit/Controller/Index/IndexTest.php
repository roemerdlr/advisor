...
class IndexTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->indexControllerMock = $this->getControllerIndexMock ( [ 
	...
	public function testExecute() {
		...
		$resultPageFactoryMock = $this->getResultPageFactoryMock ( [ 
				'create' 
		] );
		
		$resultPageFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $pageMock ) );
		
		$this->indexControllerMock->expects ( $this->once () )->method ( 'getResultPageFactory' )->will ( $this->returnValue ( $resultPageFactoryMock ) );
		
		$titleMock = $this->getTitleMock ( [ 
				'prepend' 
		] );
		...
		$configMock->expects ( $this->once () )->method ( 'getTitle' )->will ( $this->returnValue ( $titleMock ) );
		$pageMock->expects ( $this->once () )->method ( 'getConfig' )->will ( $this->returnValue ( $configMock ) );
