...
class Bundle extends \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType {
	 ...
	 * @param array $params        	
	 */
	public function __construct(\Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory $attrSetColFac, \Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory $prodAttrColFac, \Magento\Framework\App\ResourceConnection $resource, array $params) {
	...
	protected function parseSelections($rowData, $entityId) {
		$rowData ['bundle_values'] = str_replace ( self::BEFORE_OPTION_VALUE_DELIMITER, $this->_entityModel->getMultipleValueSeparator (), $rowData ['bundle_values'] );
		...
		foreach ( $selections as $selection ) {
			...
			if (isset ( $option ['sku'] ) && isset ( $option ['name'] )) {
				...
				$this->_cachedOptionSelectQuery [] = $this->connection->quoteInto ( '(parent_id = ' . ( int ) $entityId . ' AND title = ?)', $option ['name'] );
			}
		}
		return $selections;
	}
	...
	protected function populateSelectionTemplate($selection, $optionId, $parentId, $index) {
		...
		$populatedSelection = [ 
				...
				'selection_price_type' => (isset ( $selection ['price_type'] ) && $selection ['price_type'] == self::VALUE_FIXED) ? self::SELECTION_PRICE_TYPE_FIXED : self::SELECTION_PRICE_TYPE_PERCENT,
	...
	protected function retrieveProducsByCachedSkus() {
		$this->_cachedSkuToProducts = $this->connection->fetchPairs ( $this->connection->select ()->from ( $this->_resource->getTableName ( 'catalog_product_entity' ), [ 
				'sku',
				'entity_id' 
		] )->where ( 'sku IN (?)', $this->_cachedSkus ) );
		return $this;
	}
		...
		foreach ( array_keys ( $this->_customFieldsMapping ) as $oldKey ) {
			if (isset ( $rowData [$oldKey] )) {
				if ($oldKey != self::NOT_FIXED_DYNAMIC_ATTRIBUTE) {
					$resultAttrs [$oldKey] = (($rowData [$oldKey] == self::VALUE_FIXED) ? BundlePrice::PRICE_TYPE_FIXED : BundlePrice::PRICE_TYPE_DYNAMIC);
				}
			}
		}
		return $resultAttrs;
	...
	protected function populateExistingOptions() {
		$existingOptions = $this->connection->fetchAssoc ( $this->connection->select ()->from ( [ 
				...
				'value_id',
				'title' 
		] )->where ( implode ( ' OR ', $this->_cachedOptionSelectQuery ) ) );
	...
	protected function insertOptions() {
		...
		$this->connection->insertOnDuplicate ( $optionTable, $insert, [ 
				'required',
				'position',
				'type' 
		] );
		$optionIds = $this->connection->fetchAssoc ( $this->connection->select ()->from ( $optionTable, [ 
				'option_id',
				'position',
				'parent_id' 
		...
		if (! empty ( $insertValues )) {
			$this->connection->insertOnDuplicate ( $optionValueTable, $insertValues, [ 
					'title' 
			] );
		}
		return $this;
		...
		foreach ( $this->_cachedOptions as $entityId => $options ) {
			foreach ( $options as $key => $option ) {
				foreach ( $optionIds as $optionId => $assoc ) {
					if ($assoc ['position'] == $this->_cachedOptions [$entityId] [$key] ['index'] && $assoc ['parent_id'] == $entityId) {
	...
	protected function insertSelections() {
		...
		if (! empty ( $selections )) {
			$this->connection->insertOnDuplicate ( $selectionTable, $selections, [ 
					'selection_id',
					'product_id',
					'position',
					'is_default',
