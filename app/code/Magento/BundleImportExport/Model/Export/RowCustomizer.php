...
class RowCustomizer implements RowCustomizerInterface {
	...
	protected function getFormattedBundleOptionValues($product) {
		...
		$optionsCollection = $product->getTypeInstance ()->getOptionsCollection ( $product )->setOrder ( 'position', Collection::SORT_ORDER_ASC );
		
		$bundleData = '';
		foreach ( $optionsCollection as $option ) {
			$bundleData .= $this->getFormattedBundleSelections ( $this->getFormattedOptionValues ( $option ), $product->getTypeInstance ()->getSelectionsCollection ( [ 
					$option->getId () 
			], $product )->setOrder ( 'position', Collection::SORT_ORDER_ASC ) );
		}
	...
	protected function cleanNotBundleAdditionalAttributes($dataRow) {
		if (! empty ( $dataRow ['additional_attributes'] )) {
			$additionalAttributes = explode ( ImportModel::DEFAULT_GLOBAL_MULTI_VALUE_SEPARATOR, $dataRow ['additional_attributes'] );
	...
	protected function getNotBundleAttributes($additionalAttributes) {
		...
		foreach ( $additionalAttributes as $attribute ) {
			...
			if (! in_array ( 'bundle_' . $attributeCode, $this->bundleColumns )) {
				$cleanedAdditionalAttributes .= $attributeCode . ImportProductModel::PAIR_NAME_VALUE_SEPARATOR . $attributeValue . ImportModel::DEFAULT_GLOBAL_MULTI_VALUE_SEPARATOR;
			}
		}
