...
class BundleTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	protected function setUp() {
		...
		$this->entityModel = $this->getMock ( 'Magento\CatalogImportExport\Model\Import\Product', [ 
				'getErrorAggregator',
				'getBehavior',
				'getNewSku',
				'getNextBunch',
		...
		$this->connection = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'select',
				'fetchAll',
				'fetchPairs',
				'joinLeft',
		...
		$this->attrSetColFac = $this->getMock ( 'Magento\Eav\Model\ResourceModel\Entity\Attribute\Set\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->prodAttrColFac = $this->getMock ( 'Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->bundle = $this->objectManagerHelper->getObject ( 'Magento\BundleImportExport\Model\Import\Product\Type\Bundle', [ 
	 ...
	 * @param array $skus        	
	 * @param array $bunch        	
	 * @param $allowImport @dataProvider
	 *        	testSaveDataProvider
	 */
	public function testSaveData($skus, $bunch, $allowImport) {
		$this->entityModel->expects ( $this->any () )->method ( 'getBehavior' )->will ( $this->returnValue ( \Magento\ImportExport\Model\Import::BEHAVIOR_APPEND ) );
		...
		$this->entityModel->expects ( $this->any () )->method ( 'isRowAllowedToImport' )->will ( $this->returnValue ( $allowImport ) );
	...
	public function testSaveDataDelete() {
		$this->entityModel->expects ( $this->any () )->method ( 'getBehavior' )->will ( $this->returnValue ( \Magento\ImportExport\Model\Import::BEHAVIOR_DELETE ) );
	...
	public function testPrepareAttributesWithDefaultValueForSaveInsideCall() {
		$bundleMock = $this->getMock ( 'Magento\BundleImportExport\Model\Import\Product\Type\Bundle', [ 
		...
		$bundleMock->expects ( $this->once () )->method ( 'transformBundleCustomAttributes' )->with ( $rowData )->willReturn ( [ ] );
