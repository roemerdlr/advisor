...
class RowCustomizerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->productResourceCollection = $this->getMock ( '\Magento\Catalog\Model\ResourceModel\Product\Collection', [ 
		...
		$this->optionsCollection = $this->getMock ( '\Magento\Bundle\Model\ResourceModel\Option\Collection', [ 
				'setOrder',
				'getIterator' 
		], [ ], '', false );
		...
		$this->option = $this->getMock ( '\Magento\Bundle\Model\Option', [ 
				'getId',
				'getTitle',
				'getType',
				'getRequired' 
		...
		$this->selection = $this->getMock ( '\Magento\Catalog\Model\Product', [ 
				'getSku',
		...
		$this->selectionsCollection = $this->getMock ( '\Magento\Bundle\Model\ResourceModel\Selection\Collection', [ 
				'getIterator',
				'addAttributeToSort' 
		], [ ], '', false );
		$this->selectionsCollection->expects ( $this->any () )->method ( 'getIterator' )->will ( $this->returnValue ( new \ArrayIterator ( [ 
				$this->selection 
		] ) ) );
		...
		$this->productResourceCollection->expects ( $this->any () )->method ( 'getIterator' )->will ( $this->returnValue ( new \ArrayIterator ( [ 
				$this->product 
		] ) ) );
	}
	...
	public function testPrepareData() {
		$this->rowCustomizerMock->prepareData ( $this->productResourceCollection, [ 
				1 
		] );
	}
	...
	public function testAddData() {
		$preparedData = $this->rowCustomizerMock->prepareData ( $this->productResourceCollection, [ 
				1 
		] );
		$dataRow = [ 
				'sku' => 'sku1',
