...
class ToolbarEntryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getBlockInstance($unreadNotifications) {
		...
		$notificationList = $this->getMock ( 'Magento\AdminNotification\Model\ResourceModel\Inbox\Collection\Unread', [ 
				'getSize',
				'setCurPage',
				'setPageSize' 
		], [ ], '', false );
		...
		$block = $objectManagerHelper->getObject ( 'Magento\AdminNotification\Block\ToolbarEntry', [ 
	...
	public function testGetLatestUnreadNotifications() {
		...
		// 1. Create mocks
		$notificationList = $this->getMockBuilder ( 'Magento\AdminNotification\Model\ResourceModel\Inbox\Collection\Unread' )->disableOriginalConstructor ()->getMock ();
		...
		// 2. Set expectations
		$notificationList->expects ( $this->atLeastOnce () )->method ( 'setPageSize' )->with ( \Magento\AdminNotification\Block\ToolbarEntry::NOTIFICATIONS_NUMBER )->will ( $this->returnSelf () );
