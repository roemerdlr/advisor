...
class BaseurlTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$configFactoryMock = $this->getMock ( 'Magento\Framework\App\Config\ValueFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_configDataMock = $this->getMock ( 'Magento\Framework\App\Config\Value', [ 
				'getScope',
				'getScopeId',
				'getCollection',
				'__sleep',
		...
		$this->_dataCollectionMock = $this->getMock ( 'Magento\Config\Model\ResourceModel\Config\Data\Collection', [ ], [ ], '', false );
		
		$this->_iteratorMock = $this->getMock ( 'Iterator' );
		$this->_dataCollectionMock->expects ( $this->any () )->method ( 'getIterator' )->will ( $this->returnValue ( $this->_iteratorMock ) );
		
		$configFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_configDataMock ) );
		$this->_configDataMock->expects ( $this->any () )->method ( 'getCollection' )->will ( $this->returnValue ( $this->_dataCollectionMock ) );
	...
	public function testGetSeverity() {
		$this->assertEquals ( \Magento\Framework\Notification\MessageInterface::SEVERITY_CRITICAL, $this->_model->getSeverity (), 'Invalid message severity type' );
	}
	public function testgetValueUrlWithDefaultUnsecureAndSecureBaseUrl() {
		...
		$this->_urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( 'adminhtml/system_config/edit', [ 
				'section' => 'web' 
	...
	public function testgetValueUrlWithSavedDataForScopes($scope, $urlParam, $storeMethod) {
		...
		$this->_iteratorMock->expects ( $this->once () )->method ( 'current' )->will ( $this->returnValue ( $this->_configDataMock ) );
		
		$this->_configDataMock->expects ( $this->once () )->method ( 'getScopeId' )->will ( $this->returnValue ( 1 ) );
		
		$storeMock = $this->getMock ( 'Magento\Store\Model\Store', [ ], [ ], '', false );
		$this->_storeManagerMock->expects ( $this->once () )->method ( $storeMethod )->with ( 1 )->will ( $this->returnValue ( $storeMock ) );
	...
	public function testIsDisplayedWithEmptyConfigUrl() {
		$this->_configMock->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( \Magento\Store\Model\Store::BASE_URL_PLACEHOLDER ) );
