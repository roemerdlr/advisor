...
class Vault {
	 ...
	 */
	public function __construct(\Magento\Braintree\Helper\Data $braintreeHelper, \Magento\Braintree\Model\Config\Cc $config, \Psr\Log\LoggerInterface $logger, \Magento\Braintree\Helper\Error $errorHelper, \Magento\Framework\App\Cache\Type\Collection $cache, \Magento\Customer\Model\Session $customerSession, \Magento\Customer\Model\CustomerFactory $customerFactory, BraintreeCustomer $braintreeCustomer, BraintreeCreditCard $braintreeCreditCard, BraintreePaymentMethod $braintreePaymentMethod, \Magento\Directory\Model\ResourceModel\Country\CollectionFactory $countryFactory) {
		$this->config = $config;
	...
	public function currentCustomerStoredCards() {
		if ($this->config->useVault () && $this->customerSession->isLoggedIn ()) {
			...
			$customerId = $this->braintreeHelper->generateCustomerId ( $this->customerSession->getCustomerId (), $customer->getEmail () );
			try {
	...
	public function getSavedCardType($token) {
		...
		try {
			...
			if (! empty ( $cachedValues )) {
				$cachedValues = array_merge ( $cachedValues, [ 
						$token => $ccType 
				] );
			} else {
				$cachedValues = [ 
