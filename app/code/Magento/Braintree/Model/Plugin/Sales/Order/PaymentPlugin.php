...
class PaymentPlugin {
	 ...
	 */
	public function __construct(\Magento\Framework\Registry $registry, TransactionCollectionFactory $salesTransactionCollectionFactory, \Magento\Braintree\Helper\Data $paymentHelper) {
	 ...
	 *
	 * @return \Magento\Sales\Model\Order\Payment\Transaction|false @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	public function aroundGetAuthorizationTransaction(Payment $subject, \Closure $proceed) {
		...
		if ($invoice && $invoice->getId ()) {
			...
			$collection = $this->salesTransactionCollectionFactory->create ()->addFieldToFilter ( 'txn_id', [ 
					'eq' => $transactionId 
			] );
