...
class PaymentMethod extends \Magento\Payment\Model\Method\Cc {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\Context $context, \Magento\Framework\Registry $registry, \Magento\Framework\Api\ExtensionAttributesFactory $extensionFactory, \Magento\Framework\Api\AttributeValueFactory $customAttributeFactory, \Magento\Payment\Helper\Data $paymentData, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\Payment\Model\Method\Logger $logger, \Magento\Framework\Module\ModuleListInterface $moduleList, \Magento\Framework\Stdlib\DateTime\TimezoneInterface $localeDate, \Magento\Braintree\Model\Config\Cc $config, Vault $vault, BraintreeTransaction $braintreeTransaction, BraintreeCreditCard $braintreeCreditCard, \Magento\Framework\App\RequestInterface $request, \Magento\Braintree\Helper\Data $braintreeHelper, \Magento\Braintree\Helper\Error $errorHelper, TransactionCollectionFactory $salesTransactionCollectionFactory, \Magento\Framework\App\ProductMetadataInterface $productMetaData, \Magento\Directory\Model\RegionFactory $regionFactory, \Magento\Sales\Api\OrderRepositoryInterface $orderRepository, \Magento\Framework\Model\ResourceModel\AbstractResource $resource = null, \Magento\Framework\Data\Collection\AbstractDb $resourceCollection = null, array $data = []) {
	...
	protected function getTransactionsToVoid(InfoInterface $payment) {
		...
		if ($invoice && $invoice->getId () && $invoice->getTransactionId ()) {
			...
			$collection = $this->salesTransactionCollectionFactory->create ()->addFieldToSelect ( 'txn_id' )->addOrderIdFilter ( $payment->getOrder ()->getId () )->addTxnTypeFilter ( [ 
	...
	public function void(InfoInterface $payment) {
		...
		if ($errors) {
			...
			$match = true;
			foreach ( $transactionIds as $transactionId ) {
				$collection = $this->salesTransactionCollectionFactory->create ()->addFieldToFilter ( 'parent_txn_id', [ 
						'eq' => $transactionId 
	...
	public function toBraintreeAddress($address) {
		if ($address) {
			...
			] // alpha2 is the default in magento
;
		} else {
			return [ ];
		}
	...
	protected function cloneTransaction($amount, $transactionId) {
		$this->_debug ( [ 
				'clone-' . $transactionId . ' amount=' . $amount 
		] );
		$result = $this->braintreeTransaction->cloneTransaction ( $transactionId, [ 
				'amount' => $amount,
				'options' => [ 
	...
	public function canVoid() {
		if ((($order = $this->_registry->registry ( 'current_order' )) && $order->getId () && $order->hasInvoices ()) || $this->_registry->registry ( 'current_invoice' )) {
			return false;
		}
