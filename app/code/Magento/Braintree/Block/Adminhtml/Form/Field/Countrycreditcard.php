...
class Countrycreditcard extends \Magento\Config\Block\System\Config\Form\Field\FieldArray\AbstractFieldArray {
	...
	protected function getCountryRenderer() {
		if (! $this->countryRenderer) {
			$this->countryRenderer = $this->getLayout ()->createBlock ( '\Magento\Braintree\Block\Adminhtml\Form\Field\Countries', '', [ 
					'data' => [ 
	...
	protected function getCcTypesRenderer() {
		if (! $this->ccTypesRenderer) {
			$this->ccTypesRenderer = $this->getLayout ()->createBlock ( '\Magento\Braintree\Block\Adminhtml\Form\Field\Cctypes', '', [ 
					'data' => [ 
							'is_render_to_js_template' => true 
					] 
			] );
