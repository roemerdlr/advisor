...
class Data extends \Magento\Directory\Block\Data {
	...
	public function getCountryHtmlSelect($defValue = null, $name = 'country_id', $id = 'country', $title = 'Country') {
		...
		$html = $this->getLayout ()->createBlock ( 'Magento\Framework\View\Element\Html\Select' )->setName ( $name )->setId ( $id )->setTitle ( __ ( $title ) )->setClass ( 'validate-select' )->setValue ( $defValue )->setOptions ( $options )->getHtml ();
		return $html;
	}
	...
	public function getCountryCollection() {
		...
		if ($collection == null) {
			...
			$collection = $this->_countryCollectionFactory->create ()->addFieldToFilter ( 'country_id', [ 
					'nin' => $restrictedCountriesByBraintree 
			] )->loadByStore ();
