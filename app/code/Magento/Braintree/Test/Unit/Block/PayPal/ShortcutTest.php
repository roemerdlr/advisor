...
class ShortcutTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->checkoutDataMock = $this->getMockBuilder ( '\Magento\Checkout\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		...
		$this->block = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Block\PayPal\Shortcut', [ 
				'context' => $contextMock,
	...
	public function testGetClientToken() {
		...
		$this->paypalConfigMock->expects ( $this->once () )->method ( 'getClientToken' )->willReturn ( $clientToken );
	...
	public function testGetReviewPageUrl() {
		...
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( 'braintree/paypal/review' )->willReturn ( $url );
	...
	public function testMerchantName() {
		...
		$this->paypalConfigMock->expects ( $this->once () )->method ( 'getMerchantNameOverride' )->willReturn ( $merchantName );
	...
	public function testGetHtmlElementIdsMiniCart() {
		$block = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Block\PayPal\Shortcut', [ 
				'data' => [ 
