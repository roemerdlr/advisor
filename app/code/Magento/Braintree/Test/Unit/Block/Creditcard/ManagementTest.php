...
class ManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->regionCollectionFactoryMock = $this->getMockBuilder ( '\Magento\Directory\Model\ResourceModel\Region\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->ccConfigMock = $this->getMockBuilder ( '\Magento\Payment\Model\CcConfig' )->disableOriginalConstructor ()->getMock ();
		...
		$contextMock->expects ( $this->any () )->method ( 'getLayout' )->willReturn ( $this->layoutMock );
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->block = $this->objectManagerHelper->getObject ( '\Magento\Braintree\Block\Creditcard\Management', [ 
				'context' => $contextMock,
	...
	public function testCreditCard() {
		...
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( 'token' )->willReturn ( $token );
		
		$this->vaultMock->expects ( $this->once () )->method ( 'storedCard' )->with ( $token )->willReturn ( $creditCard );
	...
	public function testGetRegionIdByName() {
		...
		$regionCollectionMock->expects ( $this->once () )->method ( 'getFirstItem' )->willReturn ( new \Magento\Framework\DataObject ( [ 
				'id' => $regionId 
		] ) );
	...
	public function testGetCcMonths() {
		...
		$this->paymentConfigMock->expects ( $this->once () )->method ( 'getMonths' )->willReturn ( $months );
