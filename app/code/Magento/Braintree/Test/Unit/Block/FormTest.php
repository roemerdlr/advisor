...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->dataHelperMock = $this->getMockBuilder ( '\Magento\Braintree\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		
		$this->contextMock = $this->getMockBuilder ( '\Magento\Framework\View\Element\Template\Context' )->disableOriginalConstructor ()->setMethods ( [ 
				'getAppState' 
		] )->getMock ();
		
		$this->sessionQuoteMock = $this->getMockBuilder ( '\Magento\Backend\Model\Session\Quote' )->disableOriginalConstructor ()->setMethods ( [ 
				'getQuote' 
		] )->getMock ();
		...
		$this->paymentConfigMock = $this->getMockBuilder ( '\Magento\Payment\Model\Config' )->disableOriginalConstructor ()->getMock ();
		
		$this->objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->form = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Block\Form', [ 
	...
	public function setMethodInfo() {
		...
		$paymentMock->expects ( $this->once () )->method ( 'getMethodInstance' )->willReturn ( 'method' );
		...
		$this->onePageMock->expects ( $this->once () )->method ( 'getQuote' )->willReturn ( $quoteMock );
	...
	public function testGetStoredCards() {
		...
		$this->vaultMock->expects ( $this->once () )->method ( 'currentCustomerStoredCards' )->willReturn ( [ 
				$card 
		] );
		...
		$this->configMock->expects ( $this->once () )->method ( 'getApplicableCardTypes' )->with ( 'US' )->willReturn ( [ 
				'VI',
				'DI',
				'MC',
				'AE',
	...
	public function testGetCcAvailableTypesWithSession() {
		$this->appStateMock->expects ( $this->once () )->method ( 'getAreaCode' )->willReturn ( \Magento\Backend\App\Area\FrontNameResolver::AREA_CODE );
		...
		$this->paymentConfigMock->expects ( $this->once () )->method ( 'getCcTypes' )->willReturn ( [ 
				'VI' => 'VI',
				'DI' => 'DI',
				'MC' => 'MC',
				'AE' => 'AE',
