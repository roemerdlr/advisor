...
class CreateorderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->paymentHelper = $this->getMockBuilder ( '\Magento\Braintree\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		
		$this->braintreeCustomerAdapter = $this->getMockBuilder ( '\Magento\Braintree\Model\Adapter\BraintreeCustomer' )->disableOriginalConstructor ()->getMock ();
		
		$this->sessionQuote = $this->getMockBuilder ( '\Magento\Backend\Model\Session\Quote' )->disableOriginalConstructor ()->setMethods ( [ 
				'getStoreId',
				'getCustomerId',
				'getQuote',
				'getBillingAddress' 
		...
		$this->model = $this->objectManager->getObject ( 'Magento\Braintree\Helper\Createorder', [ 
	...
	public function testGetLoggedInCustomerCards($data, $expected) {
		...
		if ($data ['vault']) {
			...
			$this->paymentHelper->expects ( $this->any () )->method ( 'getCcAvailableCardTypes' )->willReturn ( [ 
					'VI' => "VISA",
					'MC' => "MasterCard" 
			] );
			...
			$ccobj = json_decode ( json_encode ( [ 
					'creditCards' => null 
			] ) );
			$ccobj->creditCards [] = json_decode ( json_encode ( [ 
					'cardType' => 'AE' 
			] ) );
			$ccobj->creditCards [] = json_decode ( json_encode ( [ 
					'cardType' => 'VI' 
			...
			$billing = $this->getMockBuilder ( '\Magento\Quote\Model\Quote\Address' )->disableOriginalConstructor ()->setMethods ( [ 
					'getCountryId' 
			] )->getMock ();
			...
			$this->sessionQuote->expects ( $this->any () )->method ( 'getCustomerEmail' )->willReturn ( "email@email.com" );
