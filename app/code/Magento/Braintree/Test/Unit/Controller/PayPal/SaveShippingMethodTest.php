...
class SaveShippingMethodTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->responseMock = $this->getMockBuilder ( '\Magento\Framework\App\Response' )->disableOriginalConstructor ()->setMethods ( [ 
				'setBody' 
		] )->getMock ();
		
		$urlBuilderMock = $this->getMock ( '\Magento\Framework\UrlInterface' );
		$urlBuilderMock->expects ( $this->any () )->method ( 'getUrl' )->willReturn ( self::REVIEW_URL );
		...
		$contextMock->expects ( $this->any () )->method ( 'getUrl' )->willReturn ( $urlBuilderMock );
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->controller = $this->objectManagerHelper->getObject ( '\Magento\Braintree\Controller\PayPal\SaveShippingMethod', [ 
				'context' => $contextMock,
	...
	public function testExecute() {
		...
		$this->setupCart ();
		$this->checkoutMock->expects ( $this->once () )->method ( 'updateShippingMethod' )->with ( $shippingMethod );
		
		$blockMock = $this->getMockBuilder ( '\Magento\Paypal\Block\Express\Review\Details' )->disableOriginalConstructor ()->getMock ();
		...
		$layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'page.block' )->willReturn ( $blockMock );
		$responsePageMock = $this->getMockBuilder ( '\Magento\Framework\View\Result\Page' )->disableOriginalConstructor ()->getMock ();
		$responsePageMock->expects ( $this->once () )->method ( 'addHandle' )->with ( 'paypal_express_review_details' )->willReturnSelf ();
		$responsePageMock->expects ( $this->once () )->method ( 'getLayout' )->willReturn ( $layoutMock );
		
		$this->responseMock->expects ( $this->once () )->method ( 'setBody' )->with ( $html );
		$this->resultFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( ResultFactory::TYPE_PAGE )->willReturn ( $responsePageMock );
