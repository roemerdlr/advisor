...
class ReviewTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->braintreePayPalConfigMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Config\PayPal' )->disableOriginalConstructor ()->getMock ();
		
		$this->checkoutFactoryMock = $this->getMockBuilder ( '\Magento\Braintree\Model\CheckoutFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->messageManagerMock = $this->getMock ( '\Magento\Framework\Message\ManagerInterface' );
		
		$jsonHelperMock = $this->getMockBuilder ( '\Magento\Framework\Json\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		$jsonHelperMock->expects ( $this->any () )->method ( 'jsonDecode' )->willReturnCallback ( function ($arg) {
			return json_decode ( $arg, true );
		} );
		
		$this->checkoutMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Checkout' )->disableOriginalConstructor ()->getMock ();
		
		$this->requestMock = $this->getMock ( '\Magento\Framework\App\RequestInterface' );
		
		$this->actionFlagMock = $this->getMockBuilder ( '\Magento\Framework\App\ActionFlag' )->disableOriginalConstructor ()->setMethods ( [ 
				'set' 
		] )->getMock ();
		
		$this->resultFactoryMock = $this->getMockBuilder ( '\Magento\Framework\Controller\ResultFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->paymentMethodInstanceMock = $this->getMockBuilder ( '\Magento\Braintree\Model\PaymentMethod\PayPal' )->disableOriginalConstructor ()->getMock ();
		
		$contextMock = $this->getMockBuilder ( '\Magento\Framework\App\Action\Context' )->disableOriginalConstructor ()->getMock ();
		$contextMock->expects ( $this->any () )->method ( 'getResultFactory' )->willReturn ( $this->resultFactoryMock );
		...
		$this->controller = $this->objectManagerHelper->getObject ( '\Magento\Braintree\Controller\PayPal\Review', [ 
				'context' => $contextMock,
	...
	protected function setupCart($withPaymentMethodInstance = true) {
		...
		if ($withPaymentMethodInstance) {
			$paymentMock = $this->getMockBuilder ( '\Magento\Quote\Model\Quote\Payment' )->disableOriginalConstructor ()->setMethods ( [ 
					'getMethodInstance' 
			] )->getMock ();
			
			$paymentMock->expects ( $this->any () )->method ( 'getMethodInstance' )->willReturn ( $this->paymentMethodInstanceMock );
	...
	protected function setupReviewPage($quote) {
		$shippingMethodBlockMock = $this->getMockBuilder ( '\Magento\Paypal\Block\Express\Review' )->disableOriginalConstructor ()->getMock ();
		
		$reviewBlockMock = $this->getMockBuilder ( '\Magento\Braintree\Block\Checkout\Review' )->disableOriginalConstructor ()->getMock ();
		$reviewBlockMock->expects ( $this->once () )->method ( 'setQuote' )->with ( $quote );
		$shippingMethodBlockMock->expects ( $this->once () )->method ( 'setQuote' )->with ( $quote );
		$reviewBlockMock->expects ( $this->once () )->method ( 'getChildBlock' )->with ( 'shipping_method' )->willReturn ( $shippingMethodBlockMock );
		
		$layoutMock = $this->getMockBuilder ( '\Magento\Framework\View\Layout' )->disableOriginalConstructor ()->getMock ();
		$layoutMock->expects ( $this->once () )->method ( 'getBlock' )->with ( 'braintree.paypal.review' )->willReturn ( $reviewBlockMock );
		$resultPageMock = $this->getMockBuilder ( '\Magento\Framework\View\Result\Page' )->disableOriginalConstructor ()->getMock ();
		$resultPageMock->expects ( $this->once () )->method ( 'getLayout' )->willReturn ( $layoutMock );
		
		$this->resultFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( \Magento\Framework\Controller\ResultFactory::TYPE_PAGE )->willReturn ( $resultPageMock );
	...
	public function testExecute() {
		...
		$this->checkoutMock->expects ( $this->once () )->method ( 'initializeQuoteForReview' )->with ( $paymentMethodNonce, [ 
				'email' => $email 
		] );
	...
	public function testExecuteWithBillingAddress() {
		...
		$quoteMock = $this->setupCart ();
		$this->checkoutMock->expects ( $this->once () )->method ( 'initializeQuoteForReview' )->with ( $paymentMethodNonce, $details );
		$resultPageMock = $this->setupReviewPage ( $quoteMock );
	...
	public function testExecuteRefresh() {
		$quoteMock = $this->setupCart ();
		$this->paymentMethodInstanceMock->expects ( $this->once () )->method ( 'getCode' )->willReturn ( \Magento\Braintree\Model\PaymentMethod\PayPal::METHOD_CODE );
		$resultPageMock = $this->setupReviewPage ( $quoteMock );
	...
	public function testExecuteNoPayment() {
		...
		$paymentMock->expects ( $this->once () )->method ( 'getMethodInstance' )->willReturn ( null );
		$quoteMock->expects ( $this->once () )->method ( 'getPayment' )->willReturn ( $paymentMock );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addErrorMessage' )->with ( new \Magento\Framework\Phrase ( 'Incorrect payment method.' ) );
	...
	public function testExecuteIncorrectPaymentMathod() {
		$this->setupCart ();
		$this->paymentMethodInstanceMock->expects ( $this->once () )->method ( 'getCode' )->willReturn ( 'incorrect_method' );
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addErrorMessage' )->with ( new \Magento\Framework\Phrase ( 'Incorrect payment method.' ) );
	...
	public function testExecuteValidationFailure() {
		...
		$this->paymentMethodInstanceMock->expects ( $this->once () )->method ( 'validate' )->willThrowException ( $exception );
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addExceptionMessage' )->with ( $exception, $errorMessage );
