...
class MyCreditCardsTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_response = $this->getMockBuilder ( '\Magento\Framework\App\ResponseInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->config = $this->getMockBuilder ( '\Magento\Braintree\Model\Config\Cc' )->disableOriginalConstructor ()->getMock ();
		...
		$this->actionFlag = $this->getMockBuilder ( '\Magento\Framework\App\ActionFlag' )->disableOriginalConstructor ()->setMethods ( [ 
				'set' 
		] )->getMock ();
	}
	...
	public function testDispatchNoVault() {
		...
		$this->config->expects ( $this->once () )->method ( 'useVault' )->willReturn ( false );
		/**
		 *
		 * @var \Magento\Framework\App\RequestInterface $requestInterface
		 */
		$requestInterface = $this->getMockBuilder ( '\Magento\Framework\App\RequestInterface' )->getMock ();
		
		$notification = $objectManager->getObject ( 'Magento\Braintree\Test\Unit\Controller\Stub\MyCreditCardsStub', [ 
