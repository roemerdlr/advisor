...
class AddPaypalShortcutsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->addPaypalShortcutsObserver = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Observer\AddPaypalShortcuts', [ 
	...
	public function testAddPaypalShortcuts() {
		...
		$this->paypalConfigMock->expects ( $this->once () )->method ( 'isShortcutCheckoutEnabled' )->willReturn ( true );
		
		$shortcutMock = $this->getMockBuilder ( '\Magento\Braintree\Block\PayPal\Shortcut' )->disableOriginalConstructor ()->setMethods ( [ 
				'setShowOrPosition',
				'skipShortcutForGuest' 
		] )->getMock ();
		$shortcutMock->expects ( $this->once () )->method ( 'skipShortcutForGuest' )->willReturn ( false );
		$shortcutMock->expects ( $this->once () )->method ( 'setShowOrPosition' )->with ( $orPosition );
		
		$layoutMock = $this->getMock ( '\Magento\Framework\View\LayoutInterface' );
		$layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( AddPaypalShortcuts::PAYPAL_SHORTCUT_BLOCK, '', [ 
				'data' => [ 
						Shortcut::MINI_CART_FLAG_KEY => $isInMiniCart 
				] 
		] )->willReturn ( $shortcutMock );
		
		$containerMock->expects ( $this->once () )->method ( 'getLayout' )->willReturn ( $layoutMock );
		
		$containerMock->expects ( $this->once () )->method ( 'addShortcut' )->with ( $shortcutMock );
	...
	public function testAddPaypalShortcutsWithoutMinicart() {
		...
		$this->paypalMethodMock->expects ( static::once () )->method ( 'isActive' )->willReturn ( true );
		$this->paypalConfigMock->expects ( static::once () )->method ( 'isShortcutCheckoutEnabled' )->willReturn ( true );
		
		$containerMock->expects ( static::never () )->method ( 'getLayout' );
	...
	public function testAddPaypalShortcutsNotActive() {
		...
		$this->paypalMethodMock->expects ( $this->once () )->method ( 'isActive' )->willReturn ( false );
		$this->paypalConfigMock->expects ( $this->never () )->method ( 'isShortcutCheckoutEnabled' );
