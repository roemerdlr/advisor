...
class ProcessBraintreePaymentTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->transactionFactoryMock = $this->getMockBuilder ( '\Magento\Framework\DB\TransactionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->processBraintreePaymentObserver = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Observer\ProcessBraintreePayment', [ 
	...
	public function testProcessBraintreePayment() {
		...
		$orderMock->expects ( $this->once () )->method ( 'getPayment' )->willReturn ( new \Magento\Framework\DataObject ( [ 
				'method' => PaymentMethod::METHOD_CODE 
		] ) );
		$orderMock->expects ( $this->once () )->method ( 'canInvoice' )->willReturn ( true );
		
		$this->configMock->expects ( $this->at ( 0 ) )->method ( 'getConfigData' )->with ( ProcessBraintreePayment::CONFIG_PATH_PAYMENT_ACTION )->willReturn ( AbstractMethod::ACTION_AUTHORIZE );
		$this->configMock->expects ( $this->at ( 1 ) )->method ( 'getConfigData' )->with ( ProcessBraintreePayment::CONFIG_PATH_CAPTURE_ACTION )->willReturn ( PaymentMethod::CAPTURE_ON_SHIPMENT );
		
		$shipmentMock = $this->getMockBuilder ( '\Magento\Sales\Model\Order\Shipment' )->disableOriginalConstructor ()->getMock ();
		$shipmentMock->expects ( $this->any () )->method ( 'getOrder' )->willReturn ( $orderMock );
		
		$invoiceItemQty = $this->setupOrderShipmentItems ( $orderMock, $shipmentMock );
		$invoiceMock = $this->getMockBuilder ( '\Magento\Sales\Model\Order\Invoice' )->disableOriginalConstructor ()->setMethods ( [ 
				'setRequestedCaptureCase',
				'register',
				'getOrder' 
		] )->getMock ();
		$orderMock->expects ( $this->once () )->method ( 'prepareInvoice' )->with ( $invoiceItemQty )->willReturn ( $invoiceMock );
		
		$invoiceMock->expects ( $this->once () )->method ( 'setRequestedCaptureCase' )->with ( \Magento\Sales\Model\Order\Invoice::CAPTURE_ONLINE );
		...
		$this->transactionFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $transactionMock );
	...
	protected function setupOrderShipmentItems($orderMock, $shipmentMock) {
		...
		$orderMock->expects ( $this->any () )->method ( 'getAllItems' )->willReturn ( $orderItems );
		...
		$shipmentMock->expects ( $this->any () )->method ( 'getAllItems' )->willReturn ( $shipmentItems );
