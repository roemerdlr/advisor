...
class PaymentPluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->transactionCollectionFactoryMock = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Order\Payment\Transaction\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Model\Plugin\Sales\Order\PaymentPlugin', [ 
	...
	public function testAroundGetAuthorizationTransactionNotPaymentMethod() {
		...
		$methodInstanceMock->expects ( $this->once () )->method ( 'getCode' )->willReturn ( 'PayPal' );
	...
	public function testAroundGetAuthorizationTransaction() {
		...
		$collectionMock->expects ( $this->once () )->method ( 'addFieldToFilter' )->with ( 'txn_id', [ 
				'eq' => $transactionId 
		] )->willReturnSelf ();
		$collectionMock->expects ( $this->once () )->method ( 'getSize' )->willReturn ( 1 );
		$collectionMock->expects ( $this->once () )->method ( 'getFirstItem' )->willReturn ( $transaction );
