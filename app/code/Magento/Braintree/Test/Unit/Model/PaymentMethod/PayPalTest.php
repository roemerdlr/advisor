...
class PayPalTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->salesTransactionCollectionFactoryMock = $this->getMockBuilder ( '\Magento\Sales\Model\ResourceModel\Order\Payment\Transaction\CollectionFactory' )->disableOriginalConstructor ()->getMock ();
		...
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Model\PaymentMethod\PayPal', [ 
	...
	public function testAssignData() {
		...
		$this->infoInstanceMock->expects ( $this->once () )->method ( 'setAdditionalInformation' )->with ( 'payment_method_nonce', $paymentMethodNonce );
	...
	public function testValidateInvalidCountry() {
		...
		$this->payPalConfigMock->expects ( $this->once () )->method ( 'canUseForCountry' )->with ( $countryId )->willReturn ( false );
		$this->model->validate ();
	}
	public function testValidate() {
		...
		$this->payPalConfigMock->expects ( $this->once () )->method ( 'canUseForCountry' )->with ( $countryId )->willReturn ( true );
	 ...
	 */
	public function testAuthorizeSuccess(array $configData, array $paymentInfo, array $expectedRequestAttributes, array $braintreeResponse, array $expectedPaymentFields) {
		$storeId = 3;
		$amount = self::AUTH_AMOUNT;
		$paymentObject = $this->objectManagerHelper->getObject ( 'Magento\Sales\Model\Order\Payment' );
		
		$expectedRequestAttribs = $this->setupAuthorizeRequest ( $configData, $paymentInfo, $expectedRequestAttributes, $paymentObject, $storeId, $amount );
