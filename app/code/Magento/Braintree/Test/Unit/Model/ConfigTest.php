...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->braintreeConfigurationMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Adapter\BraintreeConfiguration' )->disableOriginalConstructor ()->getMock ();
		$this->sourceCountryMock = $this->getMockBuilder ( '\Magento\Braintree\Model\System\Config\Source\Country' )->disableOriginalConstructor ()->getMock ();
		$this->braintreeClientTokenMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Adapter\BraintreeClientToken' )->getMock ();
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $this->objectManagerHelper->getObject ( '\Magento\Braintree\Model\Config', [ 
	...
	public function testConstructorActive() {
		...
		$this->scopeConfigMock->expects ( $this->at ( 1 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_ENVIRONMENT, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( $environment );
		$this->scopeConfigMock->expects ( $this->at ( 2 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_MERCHANT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( $merchantId );
		$this->scopeConfigMock->expects ( $this->at ( 3 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_PUBLIC_KEY, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( $publicKey );
		$this->scopeConfigMock->expects ( $this->at ( 4 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_PRIVATE_KEY, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( $privateKey );
		$this->scopeConfigMock->expects ( $this->at ( 5 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_MERCHANT_ACCOUNT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( $merchantAccountId );
		
		$this->braintreeConfigurationMock->expects ( $this->once () )->method ( 'environment' )->with ( $environment );
		$this->braintreeConfigurationMock->expects ( $this->once () )->method ( 'merchantId' )->with ( $merchantId );
		$this->braintreeConfigurationMock->expects ( $this->once () )->method ( 'publicKey' )->with ( $publicKey );
		$this->braintreeConfigurationMock->expects ( $this->once () )->method ( 'privateKey' )->with ( $privateKey );
		...
		$this->braintreeClientTokenMock->expects ( $this->once () )->method ( 'generate' )->willReturn ( $clientToken );
	...
	public function testConstructorInActive() {
		...
		$this->scopeConfigMock->expects ( $this->once ( 0 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_ACTIVE, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( 0 );
	...
	public function testCanUseForCountryAllowSpecific($countryId, $expected) {
		...
		$this->scopeConfigMock->expects ( $this->at ( 0 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_ALLOW_SPECIFIC, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( 1 );
		$this->scopeConfigMock->expects ( $this->at ( 1 ) )->method ( 'getValue' )->with ( $prefix . Config::KEY_SPECIFIC_COUNTRY, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, null )->willReturn ( $specificCountry );
