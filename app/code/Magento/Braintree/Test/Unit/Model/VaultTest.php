...
class VaultTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->cacheMock = $this->getMockBuilder ( '\Magento\Framework\App\Cache\Type\Collection' )->disableOriginalConstructor ()->getMock ();
		...
		$this->countryFactoryMock = $this->getMockBuilder ( '\Magento\Directory\Model\ResourceModel\Country\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->braintreeCustomerMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Adapter\BraintreeCustomer' )->disableOriginalConstructor ()->getMock ();
		$this->braintreeCreditCardMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Adapter\BraintreeCreditCard' )->disableOriginalConstructor ()->getMock ();
		$this->braintreePaymentMethodMock = $this->getMockBuilder ( '\Magento\Braintree\Model\Adapter\BraintreePaymentMethod' )->disableOriginalConstructor ()->getMock ();
		
		$this->customerMock = $this->getMockBuilder ( '\Magento\Customer\Model\Customer' )->disableOriginalConstructor ()->getMock ();
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Model\Vault', [ 
	...
	public function testExists() {
		$customerId = 1;
		$this->braintreeCustomerMock->expects ( $this->once () )->method ( 'find' )->with ( $customerId )->willReturnSelf ();
	...
	public function testExistsException() {
		...
		$customerId = 1;
		$this->braintreeCustomerMock->expects ( $this->once () )->method ( 'find' )->with ( $customerId )->willThrowException ( $exception );
	...
	public function testDeleteCustomer() {
		$customerID = 1;
		$this->braintreeCustomerMock->expects ( $this->once () )->method ( 'delete' )->with ( $customerID )->willReturnSelf ();
	...
	public function testStoredCard() {
		$cardToken = 1;
		$this->braintreeCreditCardMock->expects ( $this->once () )->method ( 'find' )->with ( $cardToken )->willReturnSelf ();
	...
	public function testStoredCardException() {
		$exception = new \Braintree_Exception ();
		$cardToken = 1;
		$this->braintreeCreditCardMock->expects ( $this->once () )->method ( 'find' )->with ( $cardToken )->willThrowException ( $exception );
	...
	public function testDeleteCard() {
		$cardToken = 1;
		$this->braintreeCreditCardMock->expects ( $this->once () )->method ( 'delete' )->with ( $cardToken )->willReturnSelf ();
	...
	public function testGeneratePaymentMethodToken() {
		...
		$this->braintreePaymentMethodMock->expects ( $this->once () )->method ( 'createNonce' )->with ( $cardToken )->willReturn ( $paymentMethodObj );
	...
	public function testGeneratePaymentMethodTokenException() {
		...
		$this->errorHelperMock->expects ( $this->once () )->method ( 'parseBraintreeError' )->willReturn ( new \Magento\Framework\Phrase ( 'exception' ) );
	...
	public function dataProviderCurrentCustomerStoredCards() {
		...
		return [ 
				[ 
						...
						'useVault' => true,
						'braintreeCustomerObject' => json_decode ( json_encode ( [ 
		...
		return [ 
				...
				[ 
						'params' => [ 
								...
								'canUseCcTypeForCountry' => new \Magento\Framework\Phrase ( 'Credit card type is not allowed for your country.' ),
								'paymentMethodObj' => false 
						],
