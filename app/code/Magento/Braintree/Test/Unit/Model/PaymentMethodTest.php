...
class PaymentMethodTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->salesTransactionCollectionFactoryMock = $this->getMockBuilder ( '\Magento\Sales\Model\ResourceModel\Order\Payment\Transaction\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Braintree\Model\PaymentMethod', [ 
		...
		$this->infoInstanceMock = $this->getMockForAbstractClass ( '\Magento\Payment\Model\InfoInterface', [ ], '', false, false, false, [ 
				'setCcType',
				'setCcOwner',
				'setCcLast4',
				'setCcNumber',
	...
	protected function setupInfoInstance($countryId, $ccType, $ccToken = null) {
		...
		$this->infoInstanceMock->expects ( $this->once () )->method ( 'getQuote' )->willReturn ( $quoteObj );
		
		if ($ccType) {
			$this->infoInstanceMock->expects ( $this->once () )->method ( 'getCcType' )->willReturn ( $ccType );
		}
		if ($ccToken) {
			$this->infoInstanceMock->expects ( $this->any () )->method ( 'getAdditionalInformation' )->with ( 'cc_token' )->willReturn ( $ccToken );
		}
		return;
	}
	...
	public function testValidateInvalidCountry() {
		...
		$this->configMock->expects ( $this->once () )->method ( 'canUseForCountry' )->with ( $countryId )->willReturn ( false );
		$this->model->validate ();
	}
	...
	public function testValidateInvalidCardForCountry() {
		...
		$this->configMock->expects ( $this->once () )->method ( 'canUseCcTypeForCountry' )->with ( $countryId, $ccType )->willReturn ( $errorMsg );
		$this->model->validate ();
	}
	...
	public function testValidateInvalidCardForCountryFromToken() {
		...
		$this->vaultMock->expects ( $this->once () )->method ( 'getSavedCardType' )->with ( $ccToken )->willReturn ( $ccType );
	...
	public function testValidateValidCardForCountryFromToken() {
		...
		$this->configMock->expects ( $this->once () )->method ( 'canUseCcTypeForCountry' )->with ( $countryId, $ccType )->willReturn ( false );
	...
	protected function setupOrderMock($billingAddress, $shippingAddress, $customerEmail, $orderId, $customerId, $storeId) {
		...
		$orderMock->expects ( $this->any () )->method ( 'getIncrementId' )->willReturn ( $orderId );
		$orderMock->expects ( $this->any () )->method ( 'getBillingAddress' )->willReturn ( $billingAddress );
		$orderMock->expects ( $this->any () )->method ( 'getShippingAddress' )->willReturn ( $shippingAddress );
		$orderMock->expects ( $this->any () )->method ( 'getCustomerEmail' )->willReturn ( $customerEmail );
		$orderMock->expects ( $this->any () )->method ( 'getCustomerId' )->willReturn ( $customerId );
		$orderMock->expects ( $this->any () )->method ( 'getStoreId' )->willReturn ( $storeId );
		$orderMock->expects ( static::any () )->method ( 'getTotalDue' )->willReturn ( self::TOTAL_AMOUNT );
		
		$this->orderRepository->expects ( static::any () )->method ( 'get' )->willReturn ( $orderMock );
	...
	protected function setupPaymentObject(\Magento\Framework\DataObject $paymentObject, $storeId) {
		...
		$regionMock = $this->getMockBuilder ( 'Magento\Directory\Model\Region' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'getCode',
				'load' 
		] )->getMock ();
	 ...
	 * @return bool @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
...
,
	...
	public function testAuthorizeCaughtLocalizedException() {
		...
		$paymentObjectMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Payment' )->disableOriginalConstructor ()->setMethods ( [ 
				'getOrder' 
		] )->getMock ();
		
		$paymentObjectMock->expects ( $this->once () )->method ( 'getOrder' )->willThrowException ( $exception );
		
		$this->psrLoggerMock->expects ( $this->never () )->method ( 'critical' );
		
		$this->registryMock->expects ( $this->once () )->method ( 'unregister' )->with ( PaymentMethod::REGISTER_NAME );
	...
	public function testAuthorizeError(array $configData, $vault, $registry, $existingCustomer, array $paymentInfo) {
		...
		$this->setupAuthorizeRequest ( $configData, $vault, $registry, $existingCustomer, $paymentInfo, [ ], $paymentObject, $storeId, $amount );
		
		// setup braintree response
		$result = $this->getMockBuilder ( '\Braintree_Result_Error' )->disableOriginalConstructor ()->getMock ();
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'sale' )->willReturn ( $result );
		
		$this->psrLoggerMock->expects ( $this->never () )->method ( 'critical' );
		
		$this->errorHelperMock->expects ( $this->once () )->method ( 'parseBraintreeError' )->with ( $result )->willReturn ( new \Magento\Framework\Phrase ( 'error' ) );
	...
	public function testAuthorizeException(array $configData, $vault, $registry, $existingCustomer, array $paymentInfo) {
		...
		$this->psrLoggerMock->expects ( $this->once () )->method ( 'critical' )->with ( $exception );
		
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'sale' )->willThrowException ( $exception );
	...
	protected function setupSalesTransaction($paymentId, $numberOfTransactions) {
		...
		$transactionCollectionMock->expects ( $this->at ( 0 ) )->method ( 'addFieldToFilter' )->with ( 'payment_id', $paymentId )->willReturnSelf ();
		$transactionCollectionMock->expects ( $this->at ( 1 ) )->method ( 'addFieldToFilter' )->with ( 'txn_type', PaymentTransaction::TYPE_CAPTURE )->willReturnSelf ();
		$transactionCollectionMock->expects ( $this->once () )->method ( 'getSize' )->willReturn ( $numberOfTransactions );
	...
	public function testCaptureSuccess() {
		...
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'submitForSettlement' )->with ( self::AUTH_TRAN_ID, $amount )->willReturn ( $successResult );
	...
	public function testCaptureSuccessAuthTransactionClosed() {
		...
		$this->braintreeTransactionMock->expects ( static::once () )->method ( 'submitForSettlement' )->with ( self::AUTH_TRAN_ID, self::TOTAL_AMOUNT )->willReturn ( $result );
		
		$this->psrLoggerMock->expects ( static::never () )->method ( 'critical' );
	...
	protected function setupAuthTransaction($paymentId, $authTransaction) {
		$authTransactionCollectionMock = $this->getMockBuilder ( 'Magento\Sales\Model\ResourceModel\Order\Payment\Transaction\Collection' )->disableOriginalConstructor ()->getMock ();
		$authTransactionCollectionMock->expects ( $this->once () )->method ( 'addPaymentIdFilter' )->with ( $paymentId )->willReturnSelf ();
		$authTransactionCollectionMock->expects ( $this->once () )->method ( 'addTxnTypeFilter' )->with ( PaymentTransaction::TYPE_AUTH )->willReturnSelf ();
		$authTransactionCollectionMock->expects ( $this->any () )->method ( 'setOrder' )->willReturnSelf ();
		$authTransactionCollectionMock->expects ( $this->any () )->method ( 'setPageSize' )->with ( 1 )->willReturnSelf ();
		$authTransactionCollectionMock->expects ( $this->any () )->method ( 'setCurPage' )->with ( 1 )->willReturnSelf ();
		$authTransactionCollectionMock->expects ( $this->any () )->method ( 'getFirstItem' )->willReturn ( $authTransaction );
		
		$this->salesTransactionCollectionFactoryMock->expects ( $this->at ( 1 ) )->method ( 'create' )->willReturn ( $authTransactionCollectionMock );
		return $this;
	}
	 ...
	 */
	public function testPartialCaptureWithToken(array $configData, $vault, $registry, $existingCustomer, array $paymentInfo, array $braintreeResponse) {
		$amount = self::AUTH_AMOUNT;
		$paymentId = 1005;
		...
		$this->braintreeCreditCardMock->expects ( $this->once () )->method ( 'find' )->with ( self::CC_TOKEN )->willReturn ( true );
		
		$this->configMock->expects ( $this->once () )->method ( 'initEnvironment' )->with ( $storeId );
		
		$this->appStateMock->expects ( $this->any () )->method ( 'getAreaCode' )->willReturn ( \Magento\Backend\App\Area\FrontNameResolver::AREA_CODE );
	 ...
	 */
	public function testPartialCaptureWithTokenException(array $configData, $vault, $registry, $existingCustomer, array $paymentInfo) {
		$amount = self::AUTH_AMOUNT;
		$paymentId = 1005;
		$authTransactionId = 1006;
		$storeId = 2;
		...
		// set up authorize
		$this->setupAuthorizeRequest ( $configData, $vault, $registry, $existingCustomer, $paymentInfo, [ ], $paymentObject, $storeId, $amount );
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'sale' )->willThrowException ( new \Exception ( 'error' ) );
	...
	public function testPartialCaptureCloneTransaction() {
		...
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'cloneTransaction' )->with ( $braintreeTransactionId, [ 
				'amount' => $amount,
				'options' => [ 
	...
	public function testRefundWithVoid($status) {
		...
		$this->helperMock->expects ( $this->once () )->method ( 'clearTransactionId' )->with ( $refundTransactionId )->willReturn ( $refundTransactionId );
		...
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'find' )->with ( $refundTransactionId )->willReturn ( $transaction );
		
		$resultSuccess = $this->setupSuccessResponse ( [ ] );
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'void' )->with ( $refundTransactionId )->willReturn ( $resultSuccess );
		$this->braintreeTransactionMock->expects ( $this->never () )->method ( 'refund' );
	...
	public function testRefundWithVoidError($status) {
		...
		$resultError = $this->getMockBuilder ( '\Braintree_Result_Error' )->disableOriginalConstructor ()->getMock ();
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'void' )->with ( $refundTransactionId )->willReturn ( $resultError );
		$this->braintreeTransactionMock->expects ( $this->never () )->method ( 'refund' );
	...
	public function testPartialRefundNotSettled() {
		...
		$resultSuccess = $this->setupSuccessResponse ( [ ] );
		$this->braintreeTransactionMock->expects ( $this->never () )->method ( 'void' )->with ( $refundTransactionId )->willReturn ( $resultSuccess );
	...
	public function testRefund() {
		...
		$resultSuccess = $this->setupSuccessResponse ( [ ] );
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'refund' )->with ( $refundTransactionId, $amount )->willReturn ( $resultSuccess );
		$this->braintreeTransactionMock->expects ( $this->never () )->method ( 'void' );
	...
	public function testRefundError() {
		...
		$resultError = $this->getMockBuilder ( '\Braintree_Result_Error' )->disableOriginalConstructor ()->getMock ();
		$this->braintreeTransactionMock->expects ( $this->once () )->method ( 'refund' )->with ( $refundTransactionId )->willReturn ( $resultError );
		$this->braintreeTransactionMock->expects ( $this->never () )->method ( 'void' );
	...
	protected function setupTransactionIds($orderId, $transactionIds) {
		...
		$transactionCollectionMock->expects ( $this->once () )->method ( 'addTxnTypeFilter' )->with ( [ 
	...
	protected function setupPaymentObjectForVoid($orderId) {
		...
		$orderMock = $this->objectManagerHelper->getObject ( '\Magento\Sales\Model\Order', [ 
				'data' => [ 
						'id' => $orderId 
				] 
		] );
	...
	public function testVoid() {
		...
		foreach ( array_keys ( $transactions ) as $id ) {
			$transactionCollectionMock = $this->getMockBuilder ( '\Magento\Sales\Model\ResourceModel\Order\Payment\Transaction\Collection' )->disableOriginalConstructor ()->getMock ();
			$transactionCollectionMock->expects ( $this->at ( 0 ) )->method ( 'addFieldToFilter' )->with ( 'parent_txn_id', [ 
					'eq' => $id 
			] )->willReturnSelf ();
	...
	protected function getPriceCurrencyMock() {
		$currencyMock = $this->getMockBuilder ( '\Magento\Directory\Model\PriceCurrency' )->disableOriginalConstructor ()->setMethods ( [ 
				'round' 
		] )->getMock ();
		$currencyMock->expects ( static::any () )->method ( 'round' )->willReturnMap ( [ 
				[ 
						self::TOTAL_AMOUNT,
