...
class NavigationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->visibilityFlagMock = $this->getMock ( '\Magento\Catalog\Model\Layer\AvailabilityFlagInterface', [ ], [ ], '', false );
		...
		$layerResolver->expects ( $this->any () )->method ( $this->anything () )->will ( $this->returnValue ( $this->catalogLayerMock ) );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $objectManager->getObject ( 'Magento\LayeredNavigation\Block\Navigation', [ 
	...
	public function testGetStateHtml() {
		...
		$this->layoutMock->expects ( $this->at ( 0 ) )->method ( 'getChildName' )->with ( null, 'renderer' );
		$this->layoutMock->expects ( $this->at ( 1 ) )->method ( 'getChildName' )->with ( null, 'state' )->will ( $this->returnValue ( 'state block' ) );
		
		$this->layoutMock->expects ( $this->once () )->method ( 'renderElement' )->with ( 'state block', true )->will ( $this->returnValue ( $stateHtml ) );
