...
class Config {
	 ...
	 */
	public function __construct(\Magento\Framework\Config\DomFactory $domFactory, \Magento\Framework\Module\Dir\Reader $moduleReader, \Magento\Framework\View\LayoutInterface $layout, \Magento\Framework\App\State $appState, \Magento\Persistent\Model\Factory $persistentFactory, \Magento\Framework\Filesystem\Directory\ReadFactory $readFactory) {
	...
	protected function _getConfigDomXPath() {
		if ($this->_configDomXPath === null) {
			...
			$configDom = $this->_domFactory->createDom ( [ 
					...
					'schemaFile' => $this->_moduleReader->getModuleDir ( Dir::MODULE_ETC_DIR, 'Magento_Persistent' ) . '/persistent.xsd' 
			] );
