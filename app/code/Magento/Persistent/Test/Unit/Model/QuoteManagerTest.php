...
class QuoteManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->sessionMock = $this->getMock ( 'Magento\Persistent\Model\Session', [ 
				'setLoadInactive',
				'setCustomerData',
				'clearQuote',
				'clearStorage',
		...
		$this->abstractCollectionMock = $this->getMock ( 'Magento\Eav\Model\Entity\Collection\AbstractCollection', [ ], [ ], '', false );
		...
		$this->model = new QuoteManager ( $this->persistentSessionMock, $this->persistentDataMock, $this->checkoutSessionMock, $this->quoteRepositoryMock );
	}
	public function testSetGuestWithEmptyQuote() {
		$this->checkoutSessionMock->expects ( $this->once () )->method ( 'getQuote' )->will ( $this->returnValue ( null ) );
	...
	public function testSetGuestWhenShoppingCartAndQuoteAreNotPersistent() {
		...
		$this->persistentDataMock->expects ( $this->once () )->method ( 'isShoppingCartPersist' )->will ( $this->returnValue ( false ) );
		$this->quoteMock->expects ( $this->once () )->method ( 'getIsPersistent' )->will ( $this->returnValue ( false ) );
		$this->checkoutSessionMock->expects ( $this->once () )->method ( 'clearQuote' )->will ( $this->returnValue ( $this->checkoutSessionMock ) );
	...
	public function testExpireWithActiveQuoteAndCustomerId() {
		...
		$this->checkoutSessionMock->expects ( $this->once () )->method ( 'setCustomerData' )->with ( null )->will ( $this->returnValue ( $this->sessionMock ) );
		
		$this->sessionMock->expects ( $this->once () )->method ( 'clearQuote' )->will ( $this->returnValue ( $this->sessionMock ) );
		$this->sessionMock->expects ( $this->once () )->method ( 'clearStorage' )->will ( $this->returnValue ( $this->sessionMock ) );
