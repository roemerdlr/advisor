...
class AdditionalTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->contextMock = $this->getMock ( 'Magento\Framework\View\Element\Template\Context', [ 
				'getEventManager',
				'getScopeConfig',
				'getCacheState',
				'getCache',
		...
		$this->persistentSessionHelperMock = $this->getMock ( 'Magento\Persistent\Helper\Session', [ 
				'getSession' 
		], [ ], '', false );
		...
		$this->scopeConfigMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\Config\ScopeConfigInterface', [ ], '', false, true, true, [ 
				'getValue' 
		] );
		$this->cacheStateMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\Cache\StateInterface', [ ], '', false, true, true, [ 
				'isEnabled' 
		] );
		$this->cacheMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\CacheInterface', [ ], '', false, true, true, [ 
				'load' 
		] );
		$this->sidResolverMock = $this->getMockForAbstractClass ( 'Magento\Framework\Session\SidResolverInterface', [ ], '', false, true, true, [ 
				'getSessionIdQueryParam' 
		] );
		$this->sessionMock = $this->getMockForAbstractClass ( 'Magento\Framework\Session\SessionManagerInterface', [ ], '', false, true, true, [ 
				'getSessionId' 
		] );
		$this->escaperMock = $this->getMockForAbstractClass ( 'Magento\Framework\Escaper', [ ], '', false, true, true, [ 
				'escapeHtml' 
		] );
		$this->urlBuilderMock = $this->getMockForAbstractClass ( 'Magento\Framework\UrlInterface', [ ], '', false, true, true, [ 
				'getUrl' 
		] );
		...
		$this->additional = $this->objectManager->getObject ( 'Magento\Persistent\Block\Header\Additional', [ 
	 ...
	 * @return void @dataProvider dataProviderToHtml
	 */
	public function testToHtml($customerId) {
		...
		$this->eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( 'view_block_abstract_to_html_before', [ 
				'block' => $this->additional 
		] );
		...
		$sessionMock = $this->getMock ( 'Magento\Persistent\Model\Session', [ 
				'getCustomerId' 
		], [ ], '', false );
