...
class RenewCookieObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = new \Magento\Persistent\Observer\RenewCookieObserver ( $this->helperMock, $this->sessionHelperMock, $this->customerSessionMock, $this->sessionFactoryMock );
	}
	public function testRenewCookie() {
		...
		$this->requestMock->expects ( $this->once () )->method ( 'getFullActionName' )->will ( $this->returnValue ( 'customer_account_logout' ) );
		$this->helperMock->expects ( $this->once () )->method ( 'getLifeTime' )->will ( $this->returnValue ( 60 ) );
		$this->customerSessionMock->expects ( $this->once () )->method ( 'getCookiePath' )->will ( $this->returnValue ( 'path/cookie' ) );
