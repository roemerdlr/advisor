...
class PreventExpressCheckoutObserverTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_customerSession = $this->getMockBuilder ( 'Magento\Customer\Model\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'isLoggedIn' 
		] )->getMock ();
		
		$this->_persistentSession = $this->getMockBuilder ( 'Magento\Persistent\Helper\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'isPersistent' 
		] )->getMock ();
		
		$this->_messageManager = $this->getMockBuilder ( 'Magento\Framework\Message\ManagerInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$this->_url = $this->getMockBuilder ( 'Magento\Framework\UrlInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$this->_expressRedirectHelper = $this->getMockBuilder ( 'Magento\Checkout\Helper\ExpressRedirect' )->disableOriginalConstructor ()->setMethods ( [ 
				'redirectLogin' 
		] )->getMock ();
		
		$this->_model = $helper->getObject ( 'Magento\Persistent\Observer\PreventExpressCheckoutObserver', [ 
	...
	public function testPreventExpressCheckoutEmpty() {
		...
		$expressRedirectMock = $this->getMockBuilder ( 'Magento\Checkout\Controller\Express\RedirectLoginInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getActionFlagList',
				'getResponse',
