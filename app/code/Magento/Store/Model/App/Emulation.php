...
class Emulation extends \Magento\Framework\DataObject {
	 ...
	 * @param string $area        	
	 * @param bool $force
	 *        	A true value will ensure that environment is always emulated, regardless of current store
	 * @return void
	 */
	public function startEnvironmentEmulation($storeId, $area = \Magento\Framework\App\Area::AREA_FRONTEND, $force = false) {
		...
		// emulate design
		$storeTheme = $this->_viewDesign->getConfigurationDesignTheme ( $area, [ 
				'store' => $storeId 
		] );
	...
	public function storeCurrentEnvironmentInfo() {
		...
		$this->initialEnvironmentInfo->setInitialTranslateInline ( $this->inlineTranslation->isEnabled () )->setInitialDesign ( [ 
