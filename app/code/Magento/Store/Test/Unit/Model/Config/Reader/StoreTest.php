...
class StoreTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_collectionFactory = $this->getMock ( 'Magento\Store\Model\ResourceModel\Config\Collection\ScopedFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_storeMock = $this->getMock ( 'Magento\Store\Model\Store', [ ], [ ], '', false );
		$placeholderProcessor = $this->getMock ( 'Magento\Store\Model\Config\Processor\Placeholder', [ ], [ ], '', false );
		$placeholderProcessor->expects ( $this->any () )->method ( 'process' )->will ( $this->returnArgument ( 0 ) );
		$this->_model = new \Magento\Store\Model\Config\Reader\Store ( $this->_initialConfigMock, $this->_scopePullMock, new \Magento\Store\Model\Config\Converter ( $placeholderProcessor ), $this->_collectionFactory, $this->_storeManagerMock );
	}
	...
	public function testRead($storeCode) {
		...
		$dataMock->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( [ 
				'config' => [ 
						'key0' => 'website_value0',
						'key1' => 'website_value1' 
				] 
		...
		$dataMock->expects ( $this->once () )->method ( 'getSource' )->will ( $this->returnValue ( [ 
				'config' => [ 
						'key0' => 'website_value0',
						'key1' => 'website_value1' 
				] 
		...
		$this->_initialConfigMock->expects ( $this->once () )->method ( 'getData' )->with ( "stores|{$storeCode}" )->will ( $this->returnValue ( [ 
				'config' => [ 
						'key1' => 'store_value1',
						'key2' => 'store_value2' 
				] 
		] ) );
		$this->_collectionFactory->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'scope' => 'stores',
				'scopeId' => $storeId 
		] )->will ( $this->returnValue ( [ 
				new \Magento\Framework\DataObject ( [ 
						'path' => 'config/key1',
						'value' => 'store_db_value1' 
				] ),
				new \Magento\Framework\DataObject ( [ 
						'path' => 'config/key3',
						'value' => 'store_db_value3' 
				] ) 
		] ) );
		
		$this->_storeManagerMock->expects ( $this->any () )->method ( 'getStore' )->with ( $storeCode )->will ( $this->returnValue ( $this->_storeMock ) );
		$expectedData = [ 
				'config' => [ 
						'key0' => 'website_value0',
