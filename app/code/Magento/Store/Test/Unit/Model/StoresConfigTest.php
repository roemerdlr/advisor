...
class StoresConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_model = new \Magento\Store\Model\StoresConfig ( $this->_storeManager, $this->_config );
	}
	public function testGetStoresConfigByPath() {
		...
		$this->_storeManager->expects ( $this->once () )->method ( 'getStores' )->with ( true )->will ( $this->returnValue ( [ 
				0 => $this->_storeOne,
				1 => $this->_storeTwo 
		] ) );
		...
		$this->assertEquals ( [ 
				0 => 0,
				1 => 1 
		], $this->_model->getStoresConfigByPath ( $path ) );
	}
}
