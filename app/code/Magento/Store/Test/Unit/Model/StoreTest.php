...
class StoreTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->store = $this->objectManagerHelper->getObject ( 'Magento\Store\Model\Store', [ 
	...
	public function testLoad($key, $field) {
		...
		$resource = $this->getMock ( '\Magento\Store\Model\ResourceModel\Store', [ 
				'load',
				'getIdFieldName',
				'__wakeup' 
		], [ ], '', false );
		$resource->expects ( $this->atLeastOnce () )->method ( 'load' )->with ( $this->isInstanceOf ( '\Magento\Store\Model\Store' ), $this->equalTo ( $key ), $this->equalTo ( $field ) )->will ( $this->returnSelf () );
	...
	public function testSetWebsite() {
		$website = $this->getMock ( '\Magento\Store\Model\Website', [ 
				'getId',
				'__wakeup' 
		], [ ], '', false );
	...
	public function testGetWebsite() {
		...
		$websiteRepository->expects ( $this->once () )->method ( 'getById' )->with ( $websiteId )->willReturn ( $website );
	...
	public function testGetGroup() {
		...
		$groupRepository->expects ( $this->once () )->method ( 'get' )->with ( $groupId )->willReturn ( $group );
	...
	public function testGetGroupIfGroupIsNotExist() {
		$groupRepository = $this->getMock ( 'Magento\Store\Api\GroupRepositoryInterface' );
		$groupRepository->expects ( $this->never () )->method ( 'getById' );
	...
	public function testGetUrl() {
		...
		$url->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->with ( $this->equalTo ( 'test/route' ), $this->equalTo ( $params ) )->will ( $this->returnValue ( 'http://test/url' ) );
	...
	public function testGetBaseUrl($type, $secure, $expectedPath, $expectedBaseUrl) {
		...
		$configMock->expects ( $this->atLeastOnce () )->method ( 'getValue' )->will ( $this->returnCallback ( function ($path, $scope, $scopeCode) use($secure, $expectedPath) {
	...
	public function testGetBaseUrlEntryPoint() {
		...
		/** @var \Magento\Framework\App\Config\ReinitableConfigInterface $configMock */
		$configMock = $this->getMockForAbstractClass ( '\Magento\Framework\App\Config\ReinitableConfigInterface' );
		$configMock->expects ( $this->atLeastOnce () )->method ( 'getValue' )->will ( $this->returnCallback ( function ($path, $scope, $scopeCode) use($expectedPath) {
		...
		$this->assertEquals ( $expectedBaseUrl, $model->getBaseUrl ( \Magento\Framework\UrlInterface::URL_TYPE_LINK, false ) );
		$_SERVER = $server;
	}
	...
	public function testGetBaseCurrency($priceScope, $currencyCode) {
		...
		$config = $this->getMockForAbstractClass ( '\Magento\Framework\App\Config\ReinitableConfigInterface' );
		$config->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValueMap ( [ 
				[ 
						'catalog/price/scope',
		...
		$currencyFactory = $this->getMock ( '\Magento\Directory\Model\CurrencyFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testGetAllowedCurrencies() {
		...
		$configMock = $this->getMockForAbstractClass ( 'Magento\Framework\App\Config\ReinitableConfigInterface', [ ], '', false );
		$configMock->expects ( $this->once () )->method ( 'getValue' )->with ( $currencyPath, 'store', null )->will ( $this->returnValue ( 'EUR,USD' ) );
	 ...
	 */
	public function testIsCurrentlySecure($expected, $value, $requestSecure = false, $useSecureInFrontend = true, $secureBaseUrl = 'https://example.com:443') {
		/* @var ReinitableConfigInterface|PHPUnit_Framework_MockObject_MockObject $configMock */
		$configMock = $this->getMockForAbstractClass ( '\Magento\Framework\App\Config\ReinitableConfigInterface' );
		$configMock->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValueMap ( [ 
				[ 
		...
		$this->requestMock->expects ( $this->any () )->method ( 'isSecure' )->willReturn ( $requestSecure );
		
		$this->requestMock->expects ( $this->any () )->method ( 'getServer' )->with ( $this->equalTo ( 'SERVER_PORT' ) )->willReturn ( $value );
	...
	public function isCurrentlySecureDataProvider() {
		return [ 
				'secure request, no server setting' => [ 
						true,
						[ ],
						true 
				],
				'unsecure request, using registered port' => [ 
						true,
						443 
				],
				'unsecure request, no secure base url registered' => [ 
						false,
						443,
						false,
						true,
				...
				'unsecure request, not using registered port' => [ 
						false,
						80 
				],
				'unsecure request, using registered port, not using secure in frontend' => [ 
						false,
						443,
						false,
						false 
				...
				'unsecure request, not using registered port, not using secure in frontend' => [ 
						false,
						80,
						false,
						false 
	...
	public function testGetBaseMediaDir() {
		...
		$this->filesystemMock->expects ( $this->once () )->method ( 'getUri' )->with ( \Magento\Framework\App\Filesystem\DirectoryList::MEDIA )->willReturn ( $expectedResult );
