...
class PathConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->urlSecurityInfoMock = $this->getMockBuilder ( 'Magento\Framework\Url\SecurityInfoInterface' )->disableOriginalConstructor ()->getMock ();
	...
	public function testShouldBeSecureUnsecureBaseUrl($base, $secure) {
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( Store::XML_PATH_UNSECURE_BASE_URL, ScopeInterface::SCOPE_STORE )->willReturn ( $base );
	...
	public function testShouldBeSecureSecureBaseUrl($base, $secure) {
		...
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'isSetFlag' )->with ( Store::XML_PATH_SECURE_IN_FRONTEND, ScopeInterface::SCOPE_STORE )->willReturn ( $secure );
		...
		$this->scopeConfigMock->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValueMap ( $getValueReturnMap ) );
