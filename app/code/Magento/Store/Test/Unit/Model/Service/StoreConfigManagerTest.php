...
class StoreConfigManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->storeConfigFactoryMock = $this->getMockBuilder ( '\Magento\Store\Model\Data\StoreConfigFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->storeCollectionFactoryMock = $this->getMockBuilder ( '\Magento\Store\Model\ResourceModel\Store\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->scopeConfigMock = $this->getMock ( '\Magento\Framework\App\Config\ScopeConfigInterface' );
		
		$this->model = new \Magento\Store\Model\Service\StoreConfigManager ( $this->storeCollectionFactoryMock, $this->scopeConfigMock, $this->storeConfigFactoryMock );
	}
	protected function getStoreMock(array $storeConfig) {
		$storeMock = $this->getMockBuilder ( 'Magento\Store\Model\Store' )->disableOriginalConstructor ()->getMock ();
		
		$storeMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( $storeConfig ['id'] );
		$storeMock->expects ( $this->any () )->method ( 'getCode' )->willReturn ( $storeConfig ['code'] );
		$storeMock->expects ( $this->once () )->method ( 'getWebsiteId' )->willReturn ( $storeConfig ['website_id'] );
		...
		$urlMap = [ 
				...
				[ 
						\Magento\Framework\UrlInterface::URL_TYPE_LINK,
						true,
						$storeConfig ['secure_base_link_url'] 
				],
				[ 
						\Magento\Framework\UrlInterface::URL_TYPE_STATIC,
						false,
						$storeConfig ['base_static_url'] 
				],
				[ 
						\Magento\Framework\UrlInterface::URL_TYPE_STATIC,
						true,
						$storeConfig ['secure_base_static_url'] 
				],
				[ 
		...
		$storeMock->expects ( $this->any () )->method ( 'getBaseUrl' )->willReturnMap ( $urlMap );
		
		return $storeMock;
	}
	protected function createStoreConfigDataObject() {
		/** @var \Magento\Framework\Api\ExtensionAttributesFactory $extensionFactoryMock */
		$extensionFactoryMock = $this->getMockBuilder ( '\Magento\Framework\Api\ExtensionAttributesFactory' )->disableOriginalConstructor ()->getMock ();
		/** @var \Magento\Framework\Api\AttributeValueFactory $attributeValueFactoryMock */
		$attributeValueFactoryMock = $this->getMockBuilder ( '\Magento\Framework\Api\AttributeValueFactory' )->disableOriginalConstructor ()->getMock ();
		
		$storeConfigDataObject = new \Magento\Store\Model\Data\StoreConfig ( $extensionFactoryMock, $attributeValueFactoryMock, [ ] );
	...
	public function testGetStoreConfigs() {
		...
		$storeCollectionMock->expects ( $this->once () )->method ( 'addFieldToFilter' )->with ( 'code', [ 
				'in' => [ 
						$code 
				] 
		] )->willReturnSelf ();
		...
		$result = $this->model->getStoreConfigs ( [ 
				$code 
		] );
