...
class RouteParamsResolverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = new \Magento\Store\Url\Plugin\RouteParamsResolver ( $this->scopeConfigMock, $this->storeManagerMock, $this->queryParamsResolverMock );
	}
	...
	public function testAroundSetRouteParamsScopeInParams() {
		...
		$routeParamsResolverMock = $this->getMockBuilder ( 'Magento\Framework\Url\RouteParamsResolver' )->setMethods ( [ 
				'setScope',
				'getScope' 
	...
	public function testAroundSetRouteParamsScopeUseStoreInUrl() {
		$storeCode = 'custom_store';
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( \Magento\Store\Model\Store::XML_PATH_STORE_IN_URL, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeCode )->will ( $this->returnValue ( true ) );
		$this->storeManagerMock->expects ( $this->any () )->method ( 'hasSingleStore' )->willReturn ( false );
		$data = [ 
				'_scope' => $storeCode,
				'_scope_to_url' => true 
