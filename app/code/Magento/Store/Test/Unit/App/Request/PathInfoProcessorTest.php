...
class PathInfoProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testProcessIfStoreExistsAndIsNotDirectAcccessToFrontName() {
		...
		$this->_storeManagerMock->expects ( $this->once () )->method ( 'getStore' )->with ( 'storeCode' )->willReturn ( $store );
		$store->expects ( $this->once () )->method ( 'isUseStoreInUrl' )->will ( $this->returnValue ( true ) );
		$this->_requestMock->expects ( $this->once () )->method ( 'isDirectAccessFrontendName' )->with ( 'storeCode' )->will ( $this->returnValue ( false ) );
	...
	public function testProcessIfStoreExistsAndDirectAcccessToFrontName() {
		...
		$this->_storeManagerMock->expects ( $this->once () )->method ( 'getStore' )->with ( 'storeCode' )->willReturn ( $store );
		$store->expects ( $this->once () )->method ( 'isUseStoreInUrl' )->will ( $this->returnValue ( true ) );
		$this->_requestMock->expects ( $this->once () )->method ( 'isDirectAccessFrontendName' )->with ( 'storeCode' )->will ( $this->returnValue ( true ) );
	...
	public function testProcessIfStoreIsEmpty() {
		$path = '/0/node_one/';
		$store = $this->getMock ( '\Magento\Store\Model\Store', [ ], [ ], '', false );
		$this->_storeManagerMock->expects ( $this->once () )->method ( 'getStore' )->with ( '0' )->willReturn ( $store );
		$store->expects ( $this->once () )->method ( 'isUseStoreInUrl' )->will ( $this->returnValue ( true ) );
		$this->_requestMock->expects ( $this->once () )->method ( 'isDirectAccessFrontendName' )->with ( '0' )->will ( $this->returnValue ( true ) );
