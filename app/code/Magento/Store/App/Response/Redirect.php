...
class Redirect implements \Magento\Framework\App\Response\RedirectInterface {
	 ...
	 */
	public function __construct(\Magento\Framework\App\RequestInterface $request, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\Encryption\UrlCoder $urlCoder, \Magento\Framework\Session\SessionManagerInterface $session, \Magento\Framework\Session\SidResolverInterface $sidResolver, \Magento\Framework\UrlInterface $urlBuilder, $canUseSessionIdInParam = true) {
	...
	protected function normalizeRefererUrl($refererUrl) {
		...
		$normalizedUrl = $redirectParsedUrl ['scheme'] . '://' . $redirectParsedUrl ['host'] . (isset ( $redirectParsedUrl ['port'] ) ? ':' . $redirectParsedUrl ['port'] : '') . $redirectParsedUrl ['path'] . ($refererQuery ? '?' . http_build_query ( $refererQuery ) : '');
	...
	protected function normalizeRefererQueryParts($refererQuery) {
		...
		if ($store && ! empty ( $refererQuery [StoreResolverInterface::PARAM_NAME] ) && ($refererQuery [StoreResolverInterface::PARAM_NAME] !== $store->getCode ())) {
