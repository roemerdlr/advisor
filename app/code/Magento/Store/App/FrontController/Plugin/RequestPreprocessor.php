...
class RequestPreprocessor {
	 ...
	 */
	public function __construct(\Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\UrlInterface $url, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\Framework\App\ResponseFactory $responseFactory) {
	 ...
	 *
	 * @return \Magento\Framework\App\ResponseInterface @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
		...
		if (! $request->isPost () && $this->_isBaseUrlCheckEnabled ()) {
			...
			if ($baseUrl) {
				...
				if (! $this->_isBaseUrlCorrect ( $uri, $request )) {
					$redirectUrl = $this->_url->getRedirectUrl ( $this->_url->getUrl ( ltrim ( $request->getPathInfo (), '/' ), [ 
							'_nosid' => true 
					] ) );
	...
	protected function _isBaseUrlCheckEnabled() {
		return ( bool ) $this->_scopeConfig->getValue ( 'web/url/redirect_to_base', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
	}
