...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
	 ...
	 */
	public function __construct(\Magento\Framework\App\Helper\Context $context, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Weee\Model\Tax $weeeTax, \Magento\Weee\Model\Config $weeeConfig, \Magento\Tax\Helper\Data $taxData, \Magento\Framework\Registry $coreRegistry) {
	 ...
	 * @return bool|int @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	public function getTotalTaxAppliedForWeeeTax($item) {
		...
		foreach ( $weeeTaxAppliedAmounts as $weeeTaxAppliedAmount ) {
			$totalTaxForWeeeTax += max ( $weeeTaxAppliedAmount ['amount_incl_tax'] - $weeeTaxAppliedAmount ['amount'], 0 );
		}
	...
	public function getBaseTotalTaxAppliedForWeeeTax($item) {
		...
		$totalTaxForWeeeTax = 0;
		foreach ( $weeeTaxAppliedAmounts as $weeeTaxAppliedAmount ) {
			$totalTaxForWeeeTax += max ( $weeeTaxAppliedAmount ['base_amount_incl_tax'] - $weeeTaxAppliedAmount ['base_amount'], 0 );
		}
		return $totalTaxForWeeeTax;
	}
	...
	/**
	 * Returns the total amount of FPT across all items.
	...
	public function isDisplayIncl($storeId = null) {
		return $this->typeOfDisplay ( WeeeDisplayConfig::DISPLAY_INCL, \Magento\Framework\Pricing\Render::ZONE_ITEM_VIEW, $storeId );
	}
	...
	public function isDisplayInclDesc($storeId = null) {
		return $this->typeOfDisplay ( WeeeDisplayConfig::DISPLAY_INCL_DESCR, \Magento\Framework\Pricing\Render::ZONE_ITEM_VIEW, $storeId );
	}
	...
	public function isDisplayExclDescIncl($storeId = null) {
		return $this->typeOfDisplay ( WeeeDisplayConfig::DISPLAY_EXCL_DESCR_INCL, \Magento\Framework\Pricing\Render::ZONE_ITEM_VIEW, $storeId );
	}
	...
	public function isDisplayExcl($storeId = null) {
		return $this->typeOfDisplay ( WeeeDisplayConfig::DISPLAY_EXCL, \Magento\Framework\Pricing\Render::ZONE_ITEM_VIEW, $storeId );
	}
	...
	public function getWeeeAttributesForBundle($product) {
		if ($product->getTypeId () == \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE) {
			...
			foreach ( $selectionCollection as $selectionItem ) {
				$weeAttributes = $this->getProductWeeeAttributes ( $selectionItem, null, null, $product->getStore ()->getWebsiteId () );
