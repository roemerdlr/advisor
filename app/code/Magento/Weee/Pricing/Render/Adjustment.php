...
class Adjustment extends AbstractAdjustment {
	 ...
	 * @param array $data        	
	 */
	public function __construct(Template\Context $context, PriceCurrencyInterface $priceCurrency, \Magento\Weee\Helper\Data $weeeHelper, array $data = []) {
	...
	protected function apply() {
		...
		$weeeTaxAmount = $this->amountRender->getAmount ()->getAdjustmentAmount ( \Magento\Weee\Pricing\TaxAdjustment::ADJUSTMENT_CODE );
		...
		if ($this->typeOfDisplay ( [ 
			...
			$this->amountRender->setDisplayValue ( $this->amountRender->getDisplayValue () - $weeeAmount - $weeeTaxAmount );
		}
		return $this->toHtml ();
	}
	...
	protected function isWeeeShown() {
		$isWeeeShown = $this->typeOfDisplay ( [ 
