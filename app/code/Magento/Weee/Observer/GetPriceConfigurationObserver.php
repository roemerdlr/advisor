...
class GetPriceConfigurationObserver implements ObserverInterface {
	 ...
	 */
	public function __construct(\Magento\Framework\Registry $registry, \Magento\Weee\Helper\Data $weeeData, \Magento\Tax\Helper\Data $taxData) {
	...
	private function recurConfigAndInsertWeeePrice($input, $searchKey, $calcPrice, $weeeAttributesForBundle = null) {
		...
		if (is_array ( $input )) {
			foreach ( $input as $key => $el ) {
				if (is_array ( $el )) {
					$holder [$key] = $this->recurConfigAndInsertWeeePrice ( $el, $searchKey, $calcPrice, $weeeAttributesForBundle );
					if ($key === $searchKey) {
						if ((! array_key_exists ( 'weeePrice', $holder [$key] )) && (array_key_exists ( $calcPrice, $holder [$key] ))) {
	...
	private function insertWeeePrice($holder, $key, $weeeAttributesForBundle) {
		if (array_key_exists ( $holder ['optionId'], $weeeAttributesForBundle )) {
			if (count ( $weeeAttributesForBundle [$holder ['optionId']] ) > 0 && is_array ( $weeeAttributesForBundle [$holder ['optionId']] )) {
				$weeeSum = 0;
