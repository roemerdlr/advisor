...
class AdjustmentTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->priceCurrencyMock = $this->getMockForAbstractClass ( 'Magento\Framework\Pricing\PriceCurrencyInterface', [ ], '', true, true, true, [ ] );
		...
		$this->model = new Adjustment ( $this->contextMock, $this->priceCurrencyMock, $this->weeeHelperMock );
	}
	...
	public function testGetFinalAmount() {
		$this->priceCurrencyMock->expects ( $this->once () )->method ( 'format' )->with ( 10, true, 2 )->will ( $this->returnValue ( "$10.00" ) );
		...
		$amountRender->expects ( $this->any () )->method ( 'getDisplayValue' )->will ( $this->returnValue ( $displayValue ) );
	...
	public function testShowInclDescr($typeOfDisplay, $amount, $expectedResult) {
		...
		$baseAmount->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( $amount ) );
