...
class UpdateProductOptionsObserverTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *        	The initial array that specifies the set of additional options
	 * @param bool $weeeEnabled
	 *        	Whether the Weee module is assumed to be enabled
	 * @param int $weeeDisplay
	 *        	Which Weee display is configured
	 * @param int $priceDisplay
	 *        	Values are: including tax, excluding tax, or both including and excluding tax
	 * @param array $expectedArray
	 *        	The revised array of the additional options
	 *        	
	...
	public function testUpdateProductOptions($initialArray, $weeeEnabled, $weeeDisplay, $priceDisplay, $expectedArray) {
		...
		$weeeHelper->expects ( $this->any () )->method ( 'getWeeeAttributesForBundle' )->will ( $this->returnValue ( [ 
				[ 
						'fpt1' => $weeeObject1 
				],
				[ 
		...
		$responseObject = $this->getMock ( 'Magento\Framework\Event\Observer', [ 
				'getResponseObject' 
		], [ ], '', false );
		...
		$product = $this->getMock ( '\Magento\Bundle\Model\Product\Type', [ 
				'getTypeId',
				'getStoreId' 
		], [ ], '', false );
		...
		$weeeObserverObject = $objectManager->getObject ( 'Magento\Weee\Observer\UpdateProductOptionsObserver', [ 
				'weeeData' => $weeeHelper,
				'taxData' => $taxHelper,
				'registry' => $registry 
		] );
		...
		return [ 
				'weee not enabled' => [ 
						'initialArray' => [ 
								...
								'optionTemplate' => '<%= data.label %><% if (data.finalPrice.value) ' . '{ %> +<%- data.finalPrice.formatted %><% } %>' 
						],
						'weeeEnabled' => false,
				...
				'weee enabled, and display with Weee included in the price, and include the Weee descriptions' => [ 
						...
						'expectedArray' => [ 
								'TOTAL_BASE_CALCULATION' => 'TOTAL_BASE_CALCULATION',
								'optionTemplate' => '<%= data.label %><% if (data.basePrice.value) ' . '{ %> +<%- data.basePrice.formatted %><% } %> <% if (data.weeePricefpt1) ' . '{ %>  (: <%- data.weeePricefpt1.formatted %>)<% } %> ' . '<% if (data.weeePricefpt2) { %>  (: <%- data.weeePricefpt2.formatted %>)<% } %>' 
						] 
				],
				...
				'weee enabled, and display with ExclDescIncl' => [ 
						...
						'expectedArray' => [ 
								'TOTAL_BASE_CALCULATION' => 'TOTAL_BASE_CALCULATION',
								'optionTemplate' => '<%= data.label %><% if (data.basePrice.value) ' . '{ %> +<%- data.basePrice.formatted %><% } %> <% if (data.weeePricefpt1) ' . '{ %>  (: <%- data.weeePricefpt1.formatted %>)<% } %> ' . '<% if (data.weeePricefpt2) { %>  (: <%- data.weeePricefpt2.formatted %>)<% } %> ' . '<% if (data.weeePrice) { %><%- data.weeePrice.formatted %><% } %>' 
						] 
				],
