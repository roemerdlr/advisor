...
class GetPriceConfigurationObserverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetPriceConfiguration($hasWeeeAttributes, $testArray, $expectedArray) {
		...
		$weeeHelper->expects ( $this->any () )->method ( 'isEnabled' )->will ( $this->returnValue ( true ) );
		...
		if ($hasWeeeAttributes) {
			...
			$weeeHelper->expects ( $this->any () )->method ( 'getWeeeAttributesForBundle' )->will ( $this->returnValue ( null ) );
		}
		
		$objectManager = new ObjectManager ( $this );
		/** @var \Magento\Weee\Observer\GetPriceConfigurationObserver $weeeObserverObject */
		$weeeObserverObject = $objectManager->getObject ( 'Magento\Weee\Observer\GetPriceConfigurationObserver', [ 
				'weeeData' => $weeeHelper,
				'registry' => $registry 
		] );
