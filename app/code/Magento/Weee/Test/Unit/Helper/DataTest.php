...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	private function setupOrderItem() {
		$orderItem = $this->getMockBuilder ( '\Magento\Sales\Model\Order\Item' )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup' 
		] )->getMock ();
		
		$orderItem->setData ( 'weee_tax_applied', \Zend_Json::encode ( [ 
				[ 
	...
	public function testGetWeeeAttributesForBundle() {
		...
		$testArray = [ 
				$prodId1 => [ 
						$fptCode1 => $weeeObject1 
				],
				$prodId2 => [ 
						$fptCode2 => $weeeObject2 
				] 
		];
		
		$this->weeeTax->expects ( $this->any () )->method ( 'getProductWeeeAttributes' )->will ( $this->returnValue ( [ 
				$weeeObject1,
				$weeeObject2 
		] ) );
		
		$productSimple = $this->getMock ( '\Magento\Catalog\Model\Product\Type\Simple', [ 
				'getId' 
		], [ ], '', false );
		...
		$productInstance->expects ( $this->any () )->method ( 'getSelectionsCollection' )->will ( $this->returnValue ( [ 
				$productSimple 
		] ) );
		...
		$product = $this->getMock ( '\Magento\Bundle\Model\Product', [ 
				'getTypeInstance',
				'getStoreId',
				'getStore',
				'getTypeId' 
	...
	public function testGetAppliedSimple() {
		...
		$itemProductSimple = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ 
				'getWeeeTaxApplied' 
		], [ ], '', false );
		$itemProductSimple->expects ( $this->any () )->method ( 'getHasChildren' )->will ( $this->returnValue ( false ) );
		
		$itemProductSimple->expects ( $this->any () )->method ( 'getWeeeTaxApplied' )->will ( $this->returnValue ( \Zend_Json::encode ( $testArray ) ) );
	...
	public function testGetAppliedBundle() {
		...
		$itemProductSimple1 = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ 
				'getWeeeTaxApplied' 
		], [ ], '', false );
		$itemProductSimple2 = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ 
				'getWeeeTaxApplied' 
		], [ ], '', false );
		...
		$itemProductBundle = $this->getMock ( '\Magento\Quote\Model\Quote\Item', [ 
				'getHasChildren',
		...
		$itemProductBundle->expects ( $this->any () )->method ( 'getChildren' )->will ( $this->returnValue ( [ 
				$itemProductSimple1,
				$itemProductSimple2 
		] ) );
	...
	public function testGetRecursiveAmountSimple() {
		...
		$itemProductSimple->expects ( $this->any () )->method ( 'getWeeeTaxAppliedAmount' )->will ( $this->returnValue ( $testAmountUnit ) );
		$itemProductSimple->expects ( $this->any () )->method ( 'getWeeeTaxAppliedRowAmount' )->will ( $this->returnValue ( $testAmountRow ) );
	...
	public function testGetTaxDisplayConfig() {
		$expected = 1;
		$taxData = $this->getMock ( '\Magento\Tax\Helper\Data', [ 
				'getPriceDisplayType' 
		], [ ], '', false );
