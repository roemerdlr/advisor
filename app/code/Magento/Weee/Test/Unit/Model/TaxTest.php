...
class TaxTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->attributeFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		...
		$this->calculationFactory = $this->getMock ( $className, [ 
				'create' 
		], [ ], '', false );
		
		$className = '\Magento\Customer\Model\Session';
		$this->customerSession = $this->getMock ( $className, [ 
				'getCustomerId',
		...
		$this->model = $this->objectManager->getObject ( '\Magento\Weee\Model\Tax', [ 
	...
	public function testGetProductWeeeAttributes() {
		...
		$attribute->expects ( $this->once () )->method ( 'getAttributeCodesByFrontendType' )->willReturn ( [ 
				'0' => 'fpt' 
		] );
