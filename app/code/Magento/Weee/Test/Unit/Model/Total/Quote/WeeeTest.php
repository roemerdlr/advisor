...
class WeeeTest extends \PHPUnit_Framework_TestCase {
	...
	/**
	 * Setup an item mock as a parent of a child item mock.
	 * Return both.
	 *
	...
	protected function setupParentItemWithChildrenMock($parentQty, $itemQty) {
		...
		$parentItemMock->expects ( $this->any () )->method ( 'getChildren' )->will ( $this->returnValue ( [ 
				$childItemMock 
		] ) );
	 ...
	 */
	public function testCollect($taxConfig, $weeeConfig, $taxRates, $itemData, $itemQty, $parentQty, $addressData, $assertSetApplied = false) {
		$items = [ ];
		if ($parentQty > 0) {
		...
		if ($assertSetApplied) {
			$weeeHelper->expects ( $this->at ( 1 ) )->method ( 'setApplied' )->with ( reset ( $items ), [ ] );
			
			$weeeHelper->expects ( $this->at ( 2 ) )->method ( 'setApplied' )->with ( end ( $items ), [ ] );
			
			$weeeHelper->expects ( $this->at ( 8 ) )->method ( 'setApplied' )->with ( end ( $items ), [ 
					[ 
							'title' => 'Recycling Fee',
							'base_amount' => '10',
							'amount' => '10',
