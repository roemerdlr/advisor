...
class WeeeTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->weeeData = $this->getMockBuilder ( '\Magento\Weee\Helper\Data' )->setMethods ( [ 
		...
		$this->model = $this->objectManager->getObject ( 'Magento\Weee\Model\Total\Invoice\Weee', [ 
	...
	protected function getInvoiceItem($invoiceItemData) {
		...
		$invoiceItem = $this->getMock ( '\Magento\Sales\Model\Order\Invoice\Item', [ 
				'getOrderItem',
				'isLast',
				'__wakeup' 
		], [ ], '', false );
		$invoiceItem->expects ( $this->any () )->method ( 'getOrderItem' )->will ( $this->returnValue ( $orderItem ) );
		$invoiceItem->expects ( $this->any () )->method ( 'isLast' )->will ( $this->returnValue ( $invoiceItemData ['is_last'] ) );
