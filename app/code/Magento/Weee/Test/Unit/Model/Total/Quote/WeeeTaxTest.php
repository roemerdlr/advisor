...
class WeeeTaxTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setupTotalMock($itemMock, $isWeeeTaxable, $itemWeeeTaxDetails, $addressData) {
		...
		$totalMock->expects ( $this->any () )->method ( 'getWeeeTotalExclTax' )->will ( $this->returnValue ( $weeeTotals ) );
		$totalMock->expects ( $this->any () )->method ( 'getWeeeBaseTotalExclTax' )->will ( $this->returnValue ( $weeeBaseTotals ) );
	...
	protected function setupShippingAssignmentMock($addressMock, $itemMock) {
		...
		$shippingAssignmentMock = $this->getMock ( '\Magento\Quote\Api\Data\ShippingAssignmentInterface', [ ], [ ], '', false );
		$itemMock = $itemMock ? [ 
				$itemMock 
		] : [ ];
	...
	public function testFetch() {
		...
		$weeeHelper = $this->setupWeeeHelper ( [ 
				'getTotalAmounts' => $weeeTotal 
		] );
		$this->weeeCollector = $this->objectManagerHelper->getObject ( 'Magento\Weee\Model\Total\Quote\WeeeTax', [ 
				'taxData' => $taxHelper,
				'weeeData' => $weeeHelper 
		] );
		$expectedResult = [ 
	...
	public function testCollect($taxConfig, $weeeConfig, $itemWeeeTaxDetails, $itemQty, $addressData = []) {
		...
		$this->weeeCollector = $this->objectManagerHelper->getObject ( 'Magento\Weee\Model\Total\Quote\WeeeTax', $arguments );
