...
class AclRetriever {
	 ...
	 * @param Logger $logger        	
	 */
	public function __construct(AclBuilder $aclBuilder, RoleCollectionFactory $roleCollectionFactory, RulesCollectionFactory $rulesCollectionFactory, Logger $logger) {
		$this->logger = $logger;
	...
	public function getAllowedResourcesByUser($userType, $userId) {
		...
		try {
			...
			if (! $role) {
				throw new AuthorizationException ( __ ( 'We can\'t find the role for the user you wanted.' ) );
			}
			...
			throw new LocalizedException ( __ ( 'Something went wrong while compiling a list of allowed resources. ' . 'You can find out more in the exceptions log.' ) );
		}
