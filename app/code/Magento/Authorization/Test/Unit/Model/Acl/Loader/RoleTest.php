...
class RoleTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_groupFactoryMock = $this->getMock ( 'Magento\Authorization\Model\Acl\Role\GroupFactory', [ 
				'create' 
		], [ ], '', false );
		$this->_roleFactoryMock = $this->getMock ( 'Magento\Authorization\Model\Acl\Role\UserFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->_resourceMock->expects ( $this->once () )->method ( 'getTableName' )->with ( $this->equalTo ( 'authorization_role' ) )->will ( $this->returnArgument ( 1 ) );
		...
		$this->_resourceMock->expects ( $this->once () )->method ( 'getConnection' )->will ( $this->returnValue ( $this->_adapterMock ) );
		
		$this->_model = new \Magento\Authorization\Model\Acl\Loader\Role ( $this->_groupFactoryMock, $this->_roleFactoryMock, $this->_resourceMock );
	}
	public function testPopulateAclAddsRolesAndTheirChildren() {
		$this->_adapterMock->expects ( $this->once () )->method ( 'fetchAll' )->will ( $this->returnValue ( [ 
				[ 
						'role_id' => 1,
						'role_type' => 'G',
						'parent_id' => null 
		...
		$this->_groupFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'roleId' => '1' 
		] );
		$this->_roleFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'roleId' => '2' 
		] );
