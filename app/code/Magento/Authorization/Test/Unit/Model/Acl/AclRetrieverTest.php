...
class AclRetrieverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetAllowedResourcesByUserTypeCustomer() {
		...
		$allowedResources = $this->aclRetriever->getAllowedResourcesByUser ( UserContextInterface::USER_TYPE_CUSTOMER, null );
	...
	protected function createAclRetriever() {
		...
		$rulesMock1 = $this->getMock ( 'Magento\Authorization\Model\Rules', [ 
				'getResourceId',
				'__wakeup' 
		], [ ], '', false );
		$rulesMock1->expects ( $this->any () )->method ( 'getResourceId' )->will ( $this->returnValue ( 'Magento_Backend::dashboard' ) );
		/** @var \PHPUnit_Framework_MockObject_MockObject|\Magento\Authorization\Model\Rules $rulesMock1 */
		$rulesMock2 = $this->getMock ( 'Magento\Authorization\Model\Rules', [ 
				'getResourceId',
				'__wakeup' 
		], [ ], '', false );
		...
		$rulesCollectionMock = $this->getMock ( 'Magento\Authorization\Model\ResourceModel\Rules\Collection', [ 
				'getByRoles',
				'load',
				'getItems' 
		], [ ], '', false );
		...
		$rulesCollectionMock->expects ( $this->any () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$rulesMock1,
				$rulesMock2 
		] ) );
		
		/** @var \PHPUnit_Framework_MockObject_MockObject|RulesCollectionFactory $rulesCollectionFactoryMock */
		$rulesCollectionFactoryMock = $this->getMock ( 'Magento\Authorization\Model\ResourceModel\Rules\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$aclBuilderMock = $this->getMock ( 'Magento\Framework\Acl\Builder', [ 
				'getAcl' 
		], [ ], '', false );
		$aclBuilderMock->expects ( $this->any () )->method ( 'getAcl' )->will ( $this->returnValue ( $aclMock ) );
		
		return new AclRetriever ( $aclBuilderMock, $roleCollectionFactoryMock, $rulesCollectionFactoryMock, $this->getMock ( 'Psr\Log\LoggerInterface' ) );
	}
}
