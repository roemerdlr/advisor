...
class SampleDataDeployCommandTest extends \PHPUnit_Framework_TestCase {
	public function testExecute() {
		...
		$filesystem->expects ( $this->atLeastOnce () )->method ( 'getDirectoryRead' )->with ( DirectoryList::ROOT )->willReturn ( $directoryRead );
		...
		$arrayInputFactory = $this->getMock ( 'Symfony\Component\Console\Input\ArrayInputFactory', [ 
				'create' 
		], [ ], '', false );
		$requireArgs = [ 
				'command' => 'require',
		...
		$arrayInputFactory->expects ( $this->atLeastOnce () )->method ( 'create' )->with ( [ 
				'parameters' => $requireArgs 
		] )->willReturn ( $arrayInput );
		$application = $this->getMock ( 'Composer\Console\Application', [ ], [ ], '', false );
		$application->expects ( $this->atLeastOnce () )->method ( 'run' )->with ( $arrayInput, $this->anything () )->willReturnCallback ( function ($input, $output) {
			$input->getFirstArgument ();
			$output->writeln ( './composer.json has been updated' );
		} );
		
		$applicationFactory = $this->getMock ( 'Composer\Console\ApplicationFactory', [ 
				'create' 
		], [ ], '', false );
