...
class Carrier extends AbstractCarrierOnline implements \Magento\Shipping\Model\Carrier\CarrierInterface {
	...
	protected function _parseXmlResponse($response) {
						...
						if (is_object ( $xml->Package ) && is_object ( $xml->Package->Postage )) {
							foreach ( $xml->Package->Postage as $postage ) {
								...
								if (in_array ( $serviceCode, $allowedMethods )) {
									...
									$priceArr [$serviceCode] = $this->getMethodPrice ( ( string ) $postage->Rate, $serviceCode );
								}
							}
							asort ( $priceArr );
						}
						...
						if (is_object ( $xml->Package ) && is_object ( $xml->Package->Service )) {
							foreach ( $xml->Package->Service as $service ) {
								...
								if (in_array ( $serviceCode, $allowedMethods )) {
									...
									$priceArr [$serviceCode] = $this->getMethodPrice ( ( string ) $service->Postage, $serviceCode );
								}
							}
							asort ( $priceArr );
						}
		...
		if (empty ( $priceArr )) {
			...
			foreach ( $priceArr as $method => $price ) {
				...
				$rate->setMethodTitle ( isset ( $serviceCodeToActualNameMap [$method] ) ? $serviceCodeToActualNameMap [$method] : $this->getCode ( 'method', $method ) );
	 ...
	 * @param string $type        	
	 * @param string $code        	
	 * @return array|false @SuppressWarnings(PHPMD.ExcessiveMethodLength)
	 */
	 ...
	 * @return string|false @SuppressWarnings(PHPMD.ExcessiveMethodLength)
	 */
	...
	protected function _formUsExpressShipmentRequest(\Magento\Framework\DataObject $request) {
		...
		if ($packageParams->getWeightUnits () != \Zend_Measure_Weight::OUNCE) {
			$packageWeight = round ( $this->_carrierHelper->convertMeasureWeight ( $request->getPackageWeight (), $packageParams->getWeightUnits (), \Zend_Measure_Weight::OUNCE ) );
		}
		...
		$xmlWrap = $this->_xmlElFactory->create ( [ 
				'data' => '<?xml version = "1.0" encoding = "UTF-8"?><wrap/>' 
		] );
	...
	protected function _formIntlShipmentRequest(\Magento\Framework\DataObject $request) {
		...
		$productCollection = $this->_productCollectionFactory->create ()->addStoreFilter ( $request->getStoreId () )->addFieldToFilter ( 'entity_id', [ 
				'in' => $productIds 
	 ...
	 *
	 * @param \Magento\Framework\DataObject $request        	
	 * @return \Magento\Framework\DataObject @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
