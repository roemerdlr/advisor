...
class CurrencysymbolTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->systemStoreMock = $this->getMock ( 'Magento\Store\Model\System\Store', [ 
				'getWebsiteCollection',
				'getGroupCollection',
				'getStoreCollection' 
		], [ ], '', false );
		$this->configFactoryMock = $this->getMock ( 'Magento\Config\Model\Config\Factory', [ 
				'create' 
		], [ ], '', false );
		$this->eventManagerMock = $this->getMock ( 'Magento\Framework\Event\ManagerInterface', [ 
				'dispatch' 
		], [ ], '', false );
		$this->coreConfigMock = $this->getMock ( 'Magento\Framework\App\Config\ReinitableConfigInterface', [ 
				'reinit',
				'setValue',
				'getValue',
				'isSetFlag' 
		...
		$this->cacheTypeListMock = $this->getMock ( 'Magento\Framework\App\Cache\TypeListInterface', [ ], [ ], '', false );
		
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\CurrencySymbol\Model\System\Currencysymbol', [ 
	...
	public function testSetCurrencySymbolData() {
		 ...
		 */
		$configMock = $this->getMock ( 'Magento\Config\Model\Config', [ 
				'setSection',
				'setWebsite',
				'setStore',
				'setGroups',
	...
	protected function prepareMocksForGetCurrencySymbolsData($websiteId, $groupId, $currencies) {
		...
		$this->systemStoreMock->expects ( $this->once () )->method ( 'getWebsiteCollection' )->willReturn ( [ 
				$websiteMock 
		] );
		$this->systemStoreMock->expects ( $this->once () )->method ( 'getGroupCollection' )->willReturn ( [ 
				$groupMock 
		] );
		$this->systemStoreMock->expects ( $this->once () )->method ( 'getStoreCollection' )->willReturn ( [ 
				$storeMock 
		] );
