...
class GiftMessageConfigProviderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->cartRepositoryMock = $this->getMock ( 'Magento\GiftMessage\Api\CartRepositoryInterface', [ ], [ ], '', false );
		$this->itemRepositoryMock = $this->getMock ( 'Magento\GiftMessage\Api\ItemRepositoryInterface', [ ], [ ], '', false );
		$contextMock->expects ( $this->atLeastOnce () )->method ( 'getScopeConfig' )->willReturn ( $this->scopeConfigMock );
		
		$this->model = new \Magento\GiftMessage\Model\GiftMessageConfigProvider ( $contextMock, $this->cartRepositoryMock, $this->itemRepositoryMock, $this->checkoutSessionMock, $this->httpContextMock, $this->storeManagerMock, $this->localeFormatMock, $this->formKeyMock );
	}
	public function testGetConfig() {
		...
		$messageMock = $this->getMockForAbstractClass ( 'Magento\GiftMessage\Api\Data\MessageInterface', [ ], '', false, false, false, [ 
				'getData' 
		] );
		...
		$this->itemRepositoryMock->expects ( $this->once () )->method ( 'get' )->with ( $quoteId, $itemId )->willReturn ( $messageMock );
		
		$quoteMock->expects ( $this->once () )->method ( 'getQuoteCurrencyCode' )->willReturn ( $currencyCode );
		$this->localeFormatMock->expects ( $this->once () )->method ( 'getPriceFormat' )->with ( null, $currencyCode )->willReturn ( $priceFormat );
		
		$quoteMock->expects ( $this->once () )->method ( 'getStore' )->willReturn ( $storeMock );
		$storeMock->expects ( $this->once () )->method ( 'getCode' )->willReturn ( $storeCode );
		
		$this->httpContextMock->expects ( $this->once () )->method ( 'getValue' )->with ( CustomerContext::CONTEXT_AUTH )->willReturn ( $isCustomerLoggedIn );
		...
		$storeMock->expects ( $this->once () )->method ( 'getBaseUrl' )->with ( UrlInterface::URL_TYPE_LINK, $isFrontUrlSecure )->willReturn ( $baseUrl );
		
		$expectedResult = [ 
				'giftMessage' => [ 
						'orderLevel' => $messageDataMock,
						'itemLevel' => [ 
								$itemId => $messageDataMock 
						] 
				],
