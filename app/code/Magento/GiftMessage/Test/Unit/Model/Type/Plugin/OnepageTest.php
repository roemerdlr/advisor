...
class OnepageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->plugin = $objectManager->getObject ( 'Magento\GiftMessage\Model\Type\Plugin\Onepage', [ 
	...
	public function testAfterSaveShippingMethodWithEmptyResult() {
		...
		$this->requestMock->expects ( $this->once () )->method ( 'getParam' )->with ( 'giftmessage' )->will ( $this->returnValue ( 'giftMessage' ) );
	...
	public function testAfterSaveShippingMethodWithNotEmptyResult() {
		...
		$this->assertEquals ( [ 
				'expected result' 
		], $this->plugin->afterSaveShippingMethod ( $subjectMock, [ 
				'expected result' 
		] ) );
	}
}
