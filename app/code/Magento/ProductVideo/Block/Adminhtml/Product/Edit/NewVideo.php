...
class NewVideo extends \Magento\Backend\Block\Widget\Form\Generic {
	...
	protected function _prepareForm() {
		...
		$fieldset->addField ( 'file_name', 'hidden', [ ] );
		
		$fieldset->addField ( 'video_provider', 'hidden', [ 
				'name' => 'video_provider' 
		] );
		
		$fieldset->addField ( 'video_url', 'text', [ 
		...
		$fieldset->addField ( 'video_title', 'text', [ 
				'class' => 'edited-data',
				'label' => __ ( 'Title' ),
				'title' => __ ( 'Title' ),
				'required' => true,
		...
		$fieldset->addField ( 'video_description', 'textarea', [ 
				'class' => 'edited-data',
		...
		$fieldset->addField ( 'new_video_screenshot', 'file', [ 
				'label' => __ ( 'Preview Image' ),
				'title' => __ ( 'Preview Image' ),
				'name' => 'image' 
		] );
		
		$fieldset->addField ( 'new_video_screenshot_preview', 'button', [ 
				'class' => 'preview_hidden_image_input_button',
				'label' => '',
				'name' => '_preview' 
		] );
		
		$fieldset->addField ( 'new_video_get', 'button', [ 
				'label' => '',
		...
		$fieldset->addField ( 'new_video_disabled', 'checkbox', [ 
				'class' => 'edited-data',
	...
	protected function addMediaRoleAttributes(Fieldset $fieldset) {
		$fieldset->addField ( 'roleLabel', 'note', [ 
				'text' => __ ( 'Role' ) 
		] );
		$mediaRoles = $this->getProduct ()->getMediaAttributes ();
		ksort ( $mediaRoles );
		foreach ( $mediaRoles as $mediaRole ) {
			$fieldset->addField ( 'video_' . $mediaRole->getAttributeCode (), 'checkbox', [ 
