...
class ExternalVideoEntryProcessor {
	 ...
	 */
	public function __construct(\Magento\Catalog\Model\ResourceModel\Product\Attribute\Backend\Media $resourceEntryMediaGallery, \Magento\ProductVideo\Model\ResourceModel\Video $videoResourceModel) {
	...
	public function afterBeforeSave(MediaBackendModel $mediaBackendModel, Product $product) {
		...
		if (! empty ( $mediaCollection )) {
			...
			$product->setData ( $attribute->getAttributeCode (), $mediaCollection + $product->getData ( $attribute->getAttributeCode () ) );
		}
	...
	protected function excludeCurrentStoreRecord(array $mediaCollection, $currentStoreId) {
		return array_filter ( $mediaCollection, function ($item) use($currentStoreId) {
	...
	protected function loadStoreViewVideoData(array $mediaCollection, $excludedStore) {
		...
		$result = [ ];
		if (! empty ( $ids )) {
			$result = $this->resourceEntryMediaGallery->loadDataFromTableByValueId ( InstallSchema::GALLERY_VALUE_VIDEO_TABLE, $ids, null, $this->videoPropertiesDbMapping );
	...
	protected function collectVideoData(array $mediaCollection) {
		...
		foreach ( $mediaCollection as $item ) {
			if (! empty ( $item ['media_type'] ) && empty ( $item ['removed'] ) && $item ['media_type'] == ExternalVideoEntryConverter::MEDIA_TYPE_CODE) {
	...
	protected function extractVideoDataFromRowData(array $rowData) {
		return array_intersect_key ( $rowData, array_merge ( $this->videoPropertiesDbMapping, [ 
				self::ADDITIONAL_STORE_DATA_KEY => '' 
		] ) );
	}
	...
	protected function collectVideoEntriesIdsToAdditionalLoad(array $mediaCollection) {
		$ids = [ ];
		foreach ( $mediaCollection as $item ) {
			if (! empty ( $item ['media_type'] ) && empty ( $item ['removed'] ) && $item ['media_type'] == ExternalVideoEntryConverter::MEDIA_TYPE_CODE && isset ( $item ['save_data_from'] )) {
	...
	protected function loadVideoDataById(array $ids, $storeId = null) {
		...
		$result = $this->resourceEntryMediaGallery->loadDataFromTableByValueId ( InstallSchema::GALLERY_VALUE_VIDEO_TABLE, $ids, \Magento\Store\Model\Store::DEFAULT_STORE_ID, [ 
				'value_id' => 'value_id',
