...
class MediaTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetPlayIfBaseAttribute() {
		$return = 'some_value';
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( Media::XML_PATH_PLAY_IF_BASE )->will ( $this->returnValue ( $return ) );
		
		$this->assertEquals ( $return, $this->helper->getPlayIfBaseAttribute () );
	}
	...
	public function testGetVideoAutoRestartAttribute() {
		$return = 'some_value';
		$this->scopeConfigMock->expects ( $this->once () )->method ( 'getValue' )->with ( Media::XML_PATH_VIDEO_AUTO_RESTART )->will ( $this->returnValue ( $return ) );
		
		$this->assertEquals ( $return, $this->helper->getVideoAutoRestartAttribute () );
	}
