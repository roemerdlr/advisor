...
class PageTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->urlPersistMock = $this->getMockBuilder ( 'Magento\UrlRewrite\Model\UrlPersistInterface' )->getMockForAbstractClass ();
		
		$this->cmsPageMock = $this->getMockBuilder ( 'Magento\Cms\Model\Page' )->disableOriginalConstructor ()->getMock ();
		
		$this->cmsPageResourceMock = $this->getMockBuilder ( 'Magento\Cms\Model\ResourceModel\Page' )->disableOriginalConstructor ()->getMock ();
		
		$this->pageObject = $objectManager->getObject ( 'Magento\CmsUrlRewrite\Plugin\Cms\Model\ResourceModel\Page', [ 
	...
	public function testAroundDeletePositive() {
		...
		$this->cmsPageMock->expects ( $this->once () )->method ( 'getId' )->willReturn ( $productId );
		
		$this->cmsPageMock->expects ( $this->once () )->method ( 'isDeleted' )->willReturn ( true );
		
		$this->urlPersistMock->expects ( $this->once () )->method ( 'deleteByData' )->with ( [ 
