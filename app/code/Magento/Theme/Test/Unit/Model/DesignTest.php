...
class DesignTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resource = $this->getMockBuilder ( 'Magento\Theme\Model\ResourceModel\Design' )->disableOriginalConstructor ()->getMock ();
		$this->resourceCollection = $this->getMockBuilder ( 'Magento\Theme\Model\ResourceModel\Design\Collection' )->disableOriginalConstructor ()->getMock ();
		$this->cacheManager = $this->getMockBuilder ( 'Magento\Framework\App\CacheInterface' )->getMock ();
		
		$context->expects ( $this->any () )->method ( 'getCacheManager' )->willReturn ( $this->cacheManager );
		
		/**
		 *
		 * @var $context \Magento\Framework\Model\Context
		 */
		$this->model = new Design ( $context, $this->registry, $this->localeDate, $this->dateTime, $this->resource, $this->resourceCollection );
	}
	 ...
	 * @test
	 * 
	 * @return void @covers \Magento\Theme\Model\Design::loadChange
	 */
	public function testLoadChange() {
		...
		$this->cacheManager->expects ( $this->once () )->method ( 'save' )->with ( serialize ( [ ] ), $cacheId, [ 
				Design::CACHE_TAG 
		], 86400 )->willReturnSelf ();
	...
	public function testLoadChangeFromCache() {
		...
		$this->localeDate->expects ( $this->once () )->method ( 'scopeTimeStamp' )->with ( $storeId )->willReturn ( $localDate );
		$this->dateTime->expects ( $this->once () )->method ( 'formatDate' )->with ( $localDate, false )->willReturn ( $date );
		$this->cacheManager->expects ( $this->once () )->method ( 'load' )->with ( $cacheId )->willReturn ( serialize ( [ 
				'test' => 'data' 
		] ) );
	 ...
	 * @test
	 * 
	 * @return void @covers \Magento\Theme\Model\Design::getIdentities
	 *         @covers \Magento\Theme\Model\Design::__construct
	 *         @covers \Magento\Theme\Model\Design::_construct
	 */
	 ...
	 * @test
	 * 
	 * @return void @covers \Magento\Theme\Model\Design::changeDesign
	 *         @covers \Magento\Theme\Model\Design::__construct
	 *         @covers \Magento\Theme\Model\Design::_construct
	 */
