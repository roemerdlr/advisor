...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_storeManagerMock = $this->getMockForAbstractClass ( 'Magento\Store\Model\StoreManagerInterface', [ ], '', true, true, true, [ 
				'getStores',
				'isSingleStoreMode' 
		] );
		$this->_configData = $this->getMock ( 'Magento\Framework\App\Config\Value', [ 
				'getCollection',
				'addFieldToFilter',
				'__wakeup' 
		], [ ], '', false );
		...
		$this->_scopeConfigWriter = $this->getMock ( 'Magento\Framework\App\Config\Storage\WriterInterface', [ 
				'save',
				'delete' 
		] );
		
		$this->_model = new \Magento\Theme\Model\Config ( $this->_configData, $this->_scopeConfigWriter, $this->_storeManagerMock, $this->getMock ( 'Magento\Framework\Event\ManagerInterface', [ ], [ ], '', false ), $this->_configCacheMock, $this->_layoutCacheMock );
	}
	...
	public function testAssignToStoreInSingleStoreMode() {
		 ...
		 */
		$configEntity = new \Magento\Framework\DataObject ( [ 
				'value' => 6,
				'scope_id' => 8 
		] );
		...
		$this->_configData->expects ( $this->at ( 2 ) )->method ( 'addFieldToFilter' )->with ( 'path', \Magento\Framework\View\DesignInterface::XML_PATH_THEME_ID )->will ( $this->returnValue ( [ 
				$configEntity 
		] ) );
		...
		$this->_model->assignToStore ( $this->_themeMock, [ 
				2,
				3,
				5 
		] );
