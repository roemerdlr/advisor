...
class ResolverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->themeCollectionFactoryMock = $this->getMock ( 'Magento\Theme\Model\ResourceModel\Theme\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->themeCollectionMock = $this->getMock ( 'Magento\Theme\Model\ResourceModel\Theme\Collection', [ ], [ ], '', false );
		...
		$this->model = new \Magento\Theme\Model\Theme\Resolver ( $this->appStateMock, $this->designMock, $this->themeCollectionFactoryMock );
	}
	public function testGetByAreaWithThemeDefaultArea() {
		$this->designMock->expects ( $this->exactly ( 2 ) )->method ( 'getDesignTheme' )->will ( $this->returnValue ( $this->themeMock ) );
		...
		$this->appStateMock->expects ( $this->once () )->method ( 'getAreaCode' )->will ( $this->returnValue ( 'theme_area' ) );
	...
	public function testGetByAreaWithDesignDefaultArea() {
		...
		$this->appStateMock->expects ( $this->once () )->method ( 'getAreaCode' )->will ( $this->returnValue ( 'design_area' ) );
	...
	public function testGetByAreaWithOtherAreaAndStringThemeId() {
		...
		$this->designMock->expects ( $this->once () )->method ( 'getArea' )->will ( $this->returnValue ( 'design_area' ) );
		$this->designMock->expects ( $this->once () )->method ( 'getConfigurationDesignTheme' )->will ( $this->returnValue ( 'other_theme' ) );
		
		$this->themeMock->expects ( $this->once () )->method ( 'getArea' )->will ( $this->returnValue ( 'theme_area' ) );
		
		$this->themeCollectionFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->themeCollectionMock ) );
		
		$this->themeCollectionMock->expects ( $this->once () )->method ( 'getThemeByFullPath' )->with ( 'other_area' . \Magento\Framework\View\Design\ThemeInterface::PATH_SEPARATOR . 'other_theme' )->will ( $this->returnValue ( $this->themeMock ) );
