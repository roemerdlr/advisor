...
class PhysicalTest extends \PHPUnit_Framework_TestCase {
	public function testCreateVirtualTheme() {
		$physicalTheme = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup' 
		], [ ], '', false, false );
		$physicalTheme->setData ( [ 
				'parent_id' => 10,
				'theme_title' => 'Test Theme' 
		] );
		
		$copyService = $this->getMock ( 'Magento\Theme\Model\CopyService', [ 
				'copy' 
		], [ ], '', false, false );
		$copyService->expects ( $this->once () )->method ( 'copy' )->will ( $this->returnValue ( $copyService ) );
		
		$virtualTheme = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup',
				'getThemeImage',
		...
		$virtualTheme->expects ( $this->once () )->method ( 'createPreviewImageCopy' )->will ( $this->returnValue ( $virtualTheme ) );
		...
		$themeCollection = $this->getMock ( 'Magento\Theme\Model\ResourceModel\Theme\Collection', [ 
				'addTypeFilter',
				'addAreaFilter',
				'addFilter',
				'count' 
		...
		$domainModel = new \Magento\Theme\Model\Theme\Domain\Physical ( $this->getMock ( 'Magento\Framework\View\Design\ThemeInterface', [ ], [ ], '', false, false ), $themeFactory, $copyService, $themeCollection );
