...
class ThemeDependencyCheckerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setup() {
		...
		$this->themeCollection = $this->getMock ( 'Magento\Theme\Model\Theme\Data\Collection', [ ], [ ], '', false );
		$this->themeProvider = $this->getMock ( 'Magento\Theme\Model\Theme\ThemeProvider', [ ], [ ], '', false );
		
		$this->themeDependencyChecker = new ThemeDependencyChecker ( $this->themeCollection, $this->themeProvider, $this->themePackageInfo );
	}
	...
	public function testExecuteFailedChildThemeCheck($hasVirtual, $hasPhysical, array $input, $expected) {
		...
		$this->themeCollection->expects ( $this->any () )->method ( 'getIterator' )->willReturn ( new \ArrayIterator ( [ 
				$childThemeC,
				$childThemeD 
		] ) );
	...
	public function executeFailedChildThemeCheckDataProvider() {
		return [ 
				...
				[ 
						...
						[ 
								'frontend/Magento/a, frontend/Magento/b are parents of virtual theme.' . ' Parent themes cannot be uninstalled.' 
						] 
				],
				[ 
						false,
				...
				[ 
						...
						[ 
								'frontend/Magento/a, frontend/Magento/b are parents of physical theme.' . ' Parent themes cannot be uninstalled.' 
						] 
				],
				[ 
						true,
