...
class FileTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->themeFactory = $this->getMockBuilder ( 'Magento\Framework\View\Design\Theme\FlyweightFactory' )->disableOriginalConstructor ()->getMock ();
		$this->fileServiceFactory = $this->getMockBuilder ( 'Magento\Framework\View\Design\Theme\Customization\FileServiceFactory' )->disableOriginalConstructor ()->getMock ();
		$this->resource = $this->getMockBuilder ( 'Magento\Theme\Model\ResourceModel\Theme\File' )->disableOriginalConstructor ()->getMock ();
		$this->resourceCollection = $this->getMockBuilder ( 'Magento\Theme\Model\ResourceModel\Theme\File\Collection' )->disableOriginalConstructor ()->getMock ();
		$context->expects ( $this->once () )->method ( 'getEventDispatcher' )->willReturn ( $this->getMockBuilder ( 'Magento\Framework\Event\ManagerInterface' )->getMock () );
		$validator = $this->getMockBuilder ( 'Magento\Framework\Model\ActionValidator\RemoveAction' )->disableOriginalConstructor ()->getMock ();
		$validator->expects ( $this->any () )->method ( 'isAllowed' )->willReturn ( true );
		$context->expects ( $this->once () )->method ( 'getActionValidator' )->willReturn ( $validator );
		
		/** @var $context \Magento\Framework\Model\Context */
		$this->model = new File ( $context, $this->registry, $this->themeFactory, $this->fileServiceFactory, $this->resource, $this->resourceCollection );
	}
