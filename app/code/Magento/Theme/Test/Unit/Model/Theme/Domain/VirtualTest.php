...
class VirtualTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetStagingThemeExisting() {
		...
		$theme = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup',
				'getStagingVersion' 
		], [ ], '', false, false );
		$theme->expects ( $this->once () )->method ( 'getStagingVersion' )->will ( $this->returnValue ( $themeStaging ) );
		
		$themeFactory = $this->getMock ( 'Magento\Theme\Model\ThemeFactory', [ 
				'create' 
		], [ ], '', false );
		$themeFactory->expects ( $this->never () )->method ( 'create' );
		
		$themeCopyService = $this->getMock ( 'Magento\Theme\Model\CopyService', [ 
				'copy' 
		], [ ], '', false );
		...
		$object = new \Magento\Theme\Model\Theme\Domain\Virtual ( $theme, $themeFactory, $themeCopyService, $customizationConfig );
	...
	public function testGetStagingThemeNew() {
		...
		$themeStaging = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup',
				'setData',
				'save' 
		], [ ], '', false, false );
		$themeStaging->expects ( $this->at ( 0 ) )->method ( 'setData' )->with ( [ 
	...
	public function testIsAssigned() {
		$customizationConfig = $this->getMock ( 'Magento\Theme\Model\Config\Customization', [ 
				'isThemeAssignedToStore' 
		], [ ], '', false );
		$themeMock = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup',
				'getCollection',
				'getId' 
		], [ ], '', false, false );
		...
		$constructArguments = $objectManagerHelper->getConstructArguments ( 'Magento\Theme\Model\Theme\Domain\Virtual', [ 
				'theme' => $themeMock,
	...
	public function physicalThemeDataProvider() {
		$physicalTheme = $this->getMockBuilder ( 'Magento\Framework\View\Design\ThemeInterface' )->setMethods ( [ 
				'isPhysical',
				'getId' 
		] )->getMockForAbstractClass ();
		$physicalTheme->expects ( $this->once () )->method ( 'isPhysical' )->willReturn ( true );
		$physicalTheme->expects ( $this->once () )->method ( 'getId' )->willReturn ( 1 );
		return [ 
				'empty' => [ 
						null 
				],
	 ...
	 * @test
	 * 
	 * @return void @dataProvider physicalThemeDataProvider
	...
	public function testGetPhysicalTheme($data) {
		...
		$parentThemeMock = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup',
				'isPhysical',
				'getParentTheme' 
		], [ ], '', false, false );
		
		$themeMock->expects ( $this->once () )->method ( 'getParentTheme' )->willReturn ( $parentThemeMock );
		$parentThemeMock->expects ( $this->once () )->method ( 'getParentTheme' )->willReturn ( $data );
		$parentThemeMock->expects ( $this->once () )->method ( 'isPhysical' )->willReturn ( false );
		
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$object = $objectManagerHelper->getObject ( 'Magento\Theme\Model\Theme\Domain\Virtual', [ 
				'theme' => $themeMock 
		] );
