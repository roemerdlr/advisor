...
class ServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_uploaderFactory = $this->getMock ( 'Magento\MediaStorage\Model\File\UploaderFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->_fileSizeMock = $this->getMockBuilder ( 'Magento\Framework\File\Size' )->setMethods ( [ 
				'getMaxFileSize' 
	...
	public function testUploadLimitNotConfigured() {
		$this->_service = new \Magento\Theme\Model\Uploader\Service ( $this->_filesystemMock, $this->_fileSizeMock, $this->_uploaderFactory );
	...
	public function testGetFileContent() {
		...
		$this->_directoryMock->expects ( $this->once () )->method ( 'readFile' )->with ( $fileName )->will ( $this->returnValue ( 'content from my file' ) );
	...
	public function testUploadCssFile() {
		...
		$this->_uploader->expects ( $this->once () )->method ( 'validateFile' )->will ( $this->returnValue ( [ 
				'name' => $fileName,
				'tmp_name' => $fileName 
		] ) );
	...
	public function testUploadInvalidCssFile() {
		...
		$this->_uploader->expects ( $this->once () )->method ( 'getFileSize' )->will ( $this->returnValue ( 30 * self::MB_MULTIPLIER ) );
