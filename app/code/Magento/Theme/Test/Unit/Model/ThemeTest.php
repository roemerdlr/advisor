...
class ThemeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->customizationFactory = $this->getMock ( 'Magento\Framework\View\Design\Theme\CustomizationFactory', [ 
				'create' 
		], [ ], '', false );
		$this->resourceCollection = $this->getMock ( 'Magento\Theme\Model\ResourceModel\Theme\Collection', [ ], [ ], '', false );
		$this->_imageFactory = $this->getMock ( 'Magento\Framework\View\Design\Theme\ImageFactory', [ 
				'create' 
		], [ ], '', false );
		$this->themeFactory = $this->getMock ( 'Magento\Framework\View\Design\Theme\FlyweightFactory', [ 
				'create' 
		], [ ], '', false );
		$this->domainFactory = $this->getMock ( 'Magento\Framework\View\Design\Theme\Domain\Factory', [ 
				'create' 
		], [ ], '', false );
		...
		$arguments = $objectManagerHelper->getConstructArguments ( 'Magento\Theme\Model\Theme', [ 
	...
	public function testThemeImageGetter() {
		$this->_imageFactory->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'theme' => $this->_model 
		] );
	...
	public function testIsVirtual($type, $isVirtual) {
		...
		$themeModel = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup' 
		], [ ], '', false );
	...
	public function testValidate() {
		$this->validator->expects ( $this->once () )->method ( 'validate' )->with ( $this->_model )->willReturn ( false );
		$this->validator->expects ( $this->once () )->method ( 'getErrorMessages' )->willReturn ( [ 
				[ 
						__ ( 'testMessage' ) 
				] 
		] );
	...
	public function testHasChildThemes() {
		...
		$this->resourceCollection->expects ( $this->once () )->method ( 'addFieldToFilter' )->with ( 'parent_id', [ 
				'eq' => 1 
		] )->willReturnSelf ();
