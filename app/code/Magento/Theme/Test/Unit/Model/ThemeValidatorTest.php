...
class ThemeValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->themeProvider = $this->getMock ( 'Magento\Framework\View\Design\Theme\ThemeProviderInterface', [ ], [ ], '', false );
		$this->configData = $this->getMock ( 'Magento\Framework\App\Config\Value', [ 
				'getCollection',
				'addFieldToFilter' 
		], [ ], '', false );
		$this->themeValidator = new \Magento\Theme\Model\ThemeValidator ( $this->storeManager, $this->themeProvider, $this->configData );
	}
	public function testValidateIsThemeInUse() {
		$theme = $this->getMock ( 'Magento\Theme\Model\Theme', [ ], [ ], '', false );
		$theme->expects ( $this->once () )->method ( 'getId' )->willReturn ( 6 );
		$defaultEntity = new \Magento\Framework\DataObject ( [ 
				'value' => 6,
				'scope' => 'default',
				'scope_id' => 8 
		] );
		$websitesEntity = new \Magento\Framework\DataObject ( [ 
				'value' => 6,
				'scope' => 'websites',
				'scope_id' => 8 
		] );
		$storesEntity = new \Magento\Framework\DataObject ( [ 
				'value' => 6,
				'scope' => 'stores',
				'scope_id' => 8 
		] );
		...
		$this->configData->expects ( $this->at ( 2 ) )->method ( 'addFieldToFilter' )->willReturn ( [ 
				$defaultEntity,
				$websitesEntity,
				$storesEntity 
		] );
		...
		$result = $this->themeValidator->validateIsThemeInUse ( [ 
				'frontend/Magento/a' 
		] );
		$this->assertEquals ( [ 
