...
class CustomizationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$collectionFactory = $this->getMockBuilder ( 'Magento\Theme\Model\ResourceModel\Theme\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$collectionFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->themeCollection ) );
		
		$this->themeProviderMock = $this->getMockBuilder ( '\Magento\Theme\Model\Theme\ThemeProvider' )->disableOriginalConstructor ()->setMethods ( [ 
				'getThemeCustomizations',
				'getThemeByFullPath' 
		] )->getMock ();
		
		$this->model = new \Magento\Theme\Model\Config\Customization ( $this->storeManager, $this->designPackage, $this->themeProviderMock );
	}
	...
	public function testGetAssignedThemeCustomizations() {
		...
		$this->themeProviderMock->expects ( $this->once () )->method ( 'getThemeCustomizations' )->with ( Area::AREA_FRONTEND )->willReturn ( [ 
				$this->getAssignedTheme (),
				$this->getUnassignedTheme () 
		] );
