...
class BuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->configFactory = $this->getMockBuilder ( 'Magento\Framework\View\PageLayout\ConfigFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->fileCollector = $this->getMockBuilder ( 'Magento\Framework\View\PageLayout\File\Collector\Aggregated' )->disableOriginalConstructor ()->getMock ();
		...
		$this->themeCollection->expects ( $this->once () )->method ( 'setItemObjectClass' )->with ( 'Magento\Theme\Model\Theme\Data' )->willReturnSelf ();
	...
	public function testGetPageLayoutsConfig() {
		$files1 = [ 
				'content layouts_1.xml',
				'content layouts_2.xml' 
		];
		$files2 = [ 
				'content layouts_3.xml',
				'content layouts_4.xml' 
		];
		...
		$this->themeCollection->expects ( $this->any () )->method ( 'loadRegisteredThemes' )->willReturn ( [ 
				$theme1,
				$theme2 
		] );
		
		$this->fileCollector->expects ( $this->exactly ( 2 ) )->method ( 'getFilesContent' )->willReturnMap ( [ 
				[ 
						$theme1,
						'layouts.xml',
						$files1 
		...
		$this->configFactory->expects ( $this->once () )->method ( 'create' )->with ( [ 
