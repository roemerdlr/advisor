...
class CleanThemeRelatedContentObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->themeImageFactory = $this->getMockBuilder ( 'Magento\Framework\View\Design\Theme\ImageFactory' )->setMethods ( [ 
				'create',
				'removePreviewImage' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->updateCollection = $this->getMockBuilder ( 'Magento\Widget\Model\ResourceModel\Layout\Update\Collection' )->setMethods ( [ 
				'addThemeFilter',
				'delete' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->themeObserver = $objectManagerHelper->getObject ( 'Magento\Theme\Observer\CleanThemeRelatedContentObserver', [ 
	...
	public function testCleanThemeRelatedContent() {
		...
		$this->themeImageFactory->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'theme' => $themeMock 
		] )->willReturnSelf ();
