...
class ThemeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetCssAssets($layoutStr, $expectedResult) {
		...
		$assetRepo = $this->getMock ( 'Magento\Framework\View\Asset\Repository', [ 
				'createAsset' 
		], [ ], '', false );
		$assetRepo->expects ( $this->any () )->method ( 'createAsset' )->will ( $this->returnArgument ( 0 ) );
		$helper = new \Magento\Theme\Helper\Theme ( $this->getMock ( 'Magento\Framework\App\Helper\Context', [ ], [ ], '', false ), $layoutMergeFactory, $assetRepo );
	...
	protected function _getLayoutMergeFactory($theme, $layoutStr) {
		...
		$layoutProcessor->expects ( $this->any () )->method ( 'getFileLayoutUpdatesXml' )->will ( $this->returnValue ( $layoutElement ) );
		
		/** @var $processorFactory \Magento\Framework\View\Layout\ProcessorFactory */
		$processorFactory = $this->getMock ( 'Magento\Framework\View\Layout\ProcessorFactory', [ 
				'create' 
		], [ ], '', false );
		$processorFactory->expects ( $this->any () )->method ( 'create' )->with ( [ 
				'theme' => $theme 
