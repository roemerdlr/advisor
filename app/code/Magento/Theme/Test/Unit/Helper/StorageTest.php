...
class StorageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->customizationPath = '/' . implode ( '/', [ 
				'var',
				'theme' 
		] );
		...
		$this->filesystem->expects ( $this->any () )->method ( 'getDirectoryWrite' )->will ( $this->returnValue ( $this->directoryWrite ) );
	...
	public function testGetThumbnailPathNotFound() {
		...
		$this->request->expects ( $this->at ( 0 ) )->method ( 'getParam' )->willReturnMap ( [ 
				[ 
	...
	public function getStorageTypeForNameDataProvider() {
		return [ 
				'font' => [ 
						...
						\Magento\Theme\Helper\Storage::FONTS 
				],
				'image' => [ 
						\Magento\Theme\Model\Wysiwyg\Storage::TYPE_IMAGE,
						\Magento\Theme\Helper\Storage::IMAGES 
				] 
		];
	}
	 ...
	 * @param string $type        	
	 * @param string $name        	
	 * @return void @dataProvider getStorageTypeForNameDataProvider
	 */
	public function testGetStorageTypeName($type, $name) {
		$this->request->expects ( $this->once () )->method ( 'getParam' )->with ( \Magento\Theme\Helper\Storage::PARAM_CONTENT_TYPE )->willReturn ( $type );
