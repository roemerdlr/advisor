...
class UploadJsTest extends \Magento\Theme\Test\Unit\Controller\Adminhtml\System\Design\ThemeTest {
	...
	public function setUp() {
		...
		$this->themeCustomization = $this->getMockForAbstractClass ( 'Magento\Framework\View\Design\Theme\CustomizationInterface', [ ], '', false, false, true, [ 
				'generateFileInfo',
				'getFilesByType' 
		] );
		$this->customizationJs = $this->getMock ( 'Magento\Framework\View\Design\Theme\Customization\File\Js', [ ], [ ], '', false );
	}
	public function testExecuteWithoutTheme() {
		...
		$this->_objectManagerMock->expects ( $this->at ( 3 ) )->method ( 'get' )->with ( 'Magento\Framework\Json\Helper\Data' )->WillReturn ( $this->jsonHelper );
		
		$this->themeFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( null );
		$this->jsonHelper->expects ( $this->once () )->method ( 'jsonEncode' )->with ( [ 
				'error' => true,
				'message' => "We cannot find a theme with id \"$themeId\"." 
		] )->willReturn ( '{"error":"true","message":"We cannot find a theme with id "' . $themeId . '"."}' );
		$this->response->expects ( $this->once () )->method ( 'representJson' )->with ( '{"error":"true","message":"We cannot find a theme with id "' . $themeId . '"."}' );
	...
	public function testExecuteWithException() {
		...
		$this->jsonHelper->expects ( $this->once () )->method ( 'jsonEncode' )->with ( [ 
				'error' => true,
				'message' => 'We can\'t upload the JS file right now.' 
	...
	public function testExecute() {
		...
		$jsFile = $this->getMockForAbstractClass ( 'Magento\Framework\View\Design\Theme\FileInterface', [ ], '', false, true, true, [ 
				'setTheme',
				'setFileName',
				'setData',
				'save' 
		...
		$this->serviceModel->expects ( $this->once () )->method ( 'uploadJsFile' )->with ( 'js_files_uploader' )->willReturn ( [ 
				'filename' => 'filename',
				'content' => 'content' 
		] );
		...
		$this->_objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\View\Design\Theme\CustomizationInterface', [ 
				'theme' => $theme 
		] )->willReturn ( $this->themeCustomization );
		$this->themeCustomization->expects ( $this->once () )->method ( 'getFilesByType' )->with ( \Magento\Framework\View\Design\Theme\Customization\File\Js::TYPE )->willReturn ( [ 
				$jsFile 
		] );
		$this->themeCustomization->expects ( $this->once () )->method ( 'generateFileInfo' )->with ( [ 
				$jsFile 
		] )->willReturn ( [ 
				'fileOne' => [ 
						'name' => 'name' 
