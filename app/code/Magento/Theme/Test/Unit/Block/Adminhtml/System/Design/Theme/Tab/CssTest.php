...
class CssTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$constructArguments = $objectManagerHelper->getConstructArguments ( 'Magento\Theme\Block\Adminhtml\System\Design\Theme\Edit\Tab\Css', [ 
				'formFactory' => $this->getMock ( 'Magento\Framework\Data\FormFactory', [ ], [ ], '', false ),
				'objectManager' => $this->_objectManager,
				'uploaderService' => $this->getMock ( 'Magento\Theme\Model\Uploader\Service', [ ], [ ], '', false ),
				'urlBuilder' => $this->urlBuilder,
				'urlCoder' => $this->urlCoder 
		] );
		
		$this->_model = $this->getMock ( 'Magento\Theme\Block\Adminhtml\System\Design\Theme\Edit\Tab\Css', [ 
				'_getCurrentTheme' 
		], $constructArguments, '', true );
	}
	public function testGetUploadCssFileNote() {
		...
		$this->_objectManager->expects ( $this->any () )->method ( 'get' )->with ( 'Magento\Framework\File\Size' )->will ( $this->returnValue ( $sizeModel ) );
	...
	public function testGetterDownloadUrl() {
		$fileId = 1;
		$themeId = 1;
		$this->urlCoder->expects ( $this->atLeastOnce () )->method ( 'encode' )->with ( $fileId )->will ( $this->returnValue ( 'encoded' ) );
		$this->urlBuilder->expects ( $this->atLeastOnce () )->method ( 'getUrl' )->with ( $this->anything (), [ 
				'theme_id' => $themeId,
				'file' => 'encoded' 
		] );
