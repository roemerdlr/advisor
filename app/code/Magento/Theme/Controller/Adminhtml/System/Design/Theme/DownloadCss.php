...
class DownloadCss extends \Magento\Theme\Controller\Adminhtml\System\Design\Theme {
	...
	public function execute() {
		...
		try {
			...
			$asset = $this->_assetRepo->createAsset ( $fileId, [ 
					'themeModel' => $theme 
			] );
			$relPath = $this->_appFileSystem->getDirectoryRead ( DirectoryList::ROOT )->getRelativePath ( $asset->getSourceFile () );
			return $this->_fileFactory->create ( $relPath, [ 
					'type' => 'filename',
					'value' => $relPath 
			], DirectoryList::ROOT );
		} catch ( \Exception $e ) {
