...
class Css extends \Magento\Theme\Block\Adminhtml\System\Design\Theme\Edit\AbstractTab {
	...
	protected function _prepareForm() {
		...
		$customFiles = $this->_getCurrentTheme ()->getCustomization ()->getFilesByType ( \Magento\Theme\Model\Theme\Customization\File\CustomCss::TYPE );
	...
	protected function _addThemeCssFieldset() {
		$form = $this->getForm ();
		$themeFieldset = $form->addFieldset ( 'theme_css', [ 
				'legend' => __ ( 'Theme CSS' ),
				'class' => 'fieldset-wide' 
		] );
		...
		$themeFieldset->addField ( 'theme_css_view_assets', 'links', [ 
	...
	protected function _addCustomCssFieldset() {
		$form = $this->getForm ();
		$themeFieldset = $form->addFieldset ( 'custom_css', [ 
				'legend' => __ ( 'Custom CSS' ),
				'class' => 'fieldset-wide' 
		] );
		$this->_addElementTypes ( $themeFieldset );
		
		$themeFieldset->addField ( 'css_file_uploader', 'css_file', [ 
		...
		$themeFieldset->addField ( 'css_uploader_button', 'button', [ 
		...
		$downloadButtonConfig = [ 
				...
				'onclick' => "setLocation('" . $this->getUrl ( 'adminhtml/*/downloadCustomCss', [ 
		...
		$imageButton = $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Button' )->setData ( [ 
				'id' => 'css_images_manager',
				'label' => __ ( 'Manage' ),
				'class' => 'button',
				'onclick' => "MediabrowserUtility.openDialog('" . $this->getUrl ( 'adminhtml/system_design_wysiwyg_files/index', [ 
				...
				] ) . "', null, null,'" . $this->escapeQuote ( __ ( 'Upload Images' ), true ) . "');" 
		] );
		
		$themeFieldset->addField ( 'css_browse_image_button', 'note', [ 
				'label' => __ ( "Images Assets" ),
				'text' => $imageButton->toHtml () 
		] );
		
		/** @var $fontButton \Magento\Backend\Block\Widget\Button */
		$fontButton = $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Button' )->setData ( [ 
				...
				] ) . "', null, null,'" . $this->escapeQuote ( __ ( 'Upload Fonts' ), true ) . "');" 
		] );
		
		$themeFieldset->addField ( 'css_browse_font_button', 'note', [ 
				'label' => __ ( "Fonts Assets" ),
				'text' => $fontButton->toHtml () 
		] );
		
		$themeFieldset->addField ( 'custom_css_content', 'textarea', [ 
	...
	public function getDownloadUrl($fileId, $themeId) {
		return $this->getUrl ( 'adminhtml/*/downloadCss', [ 
				'theme_id' => $themeId,
				'file' => $this->urlCoder->encode ( $fileId ) 
		] );
	}
}
