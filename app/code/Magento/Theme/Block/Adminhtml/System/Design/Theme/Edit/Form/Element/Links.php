...
class Links extends \Magento\Framework\Data\Form\Element\AbstractElement {
	...
	public function getElementHtml() {
		...
		$html = sprintf ( '<div id="%s" %s>%s%s</div><br />%s%s', $this->getHtmlId (), $this->serialize ( $this->getHtmlAttributes () ), PHP_EOL, join ( '', $links ), PHP_EOL, $this->getAfterElementHtml () );
		return $html;
	}
	...
	protected function _optionToHtml(array $option) {
		...
		$html = sprintf ( '<a %s>%s</a>%s', join ( ' ', $attributes ), $this->_escape ( $option ['label'] ), isset ( $option ['delimiter'] ) ? $option ['delimiter'] : '' );
