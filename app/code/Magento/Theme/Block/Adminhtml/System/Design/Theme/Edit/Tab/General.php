...
class General extends \Magento\Theme\Block\Adminhtml\System\Design\Theme\Edit\AbstractTab {
	...
	protected function _addThemeFieldset($form, $formData, ThemeInterface $theme) {
		$themeFieldset = $form->addFieldset ( 'theme', [ 
				'legend' => __ ( 'Theme Settings' ) 
		] );
		$this->_addElementTypes ( $themeFieldset );
		
		if (isset ( $formData ['theme_id'] )) {
			$themeFieldset->addField ( 'theme_id', 'hidden', [ 
					'name' => 'theme_id' 
			] );
		}
		...
		$onChangeScript = sprintf ( 'parentThemeOnChange(this.value, %s)', str_replace ( '"', '\'', $helper->jsonEncode ( $this->_getDefaultsInherited ( $themesCollections ) ) ) );
		...
		if ($this->_getCurrentTheme ()->isObjectNew ()) {
			$themeFieldset->addField ( 'parent_id', 'select', [ 
			...
			$themeFieldset->addField ( 'parent_title', 'note', [ 
					'label' => __ ( 'Parent Theme' ),
					'title' => __ ( 'Parent Theme' ),
					'name' => 'parent_title',
		...
		if (! empty ( $formData ['theme_path'] )) {
			$themeFieldset->addField ( 'theme_path', 'label', [ 
					'label' => __ ( 'Theme Path' ),
					'title' => __ ( 'Theme Path' ),
					'name' => 'theme_code' 
			] );
		...
		$themeFieldset->addField ( 'theme_title', $this->_getFieldTextType (), [ 
		...
		if ($this->_isThemeEditable) {
			$themeFieldset->addField ( 'preview_image', 'image', [ 
			...
			$themeFieldset->addField ( 'preview_image', 'note', [ 
					'label' => __ ( 'Theme Preview Image' ),
					'title' => __ ( 'Theme Preview Image' ),
					'name' => 'preview',
					'after_element_html' => '<a href="' . $theme->getThemeImage ()->getPreviewImageUrl () . '" onclick="imagePreview(\'theme_preview_image\'); return false;">' . '<img width="50" src="' . $theme->getThemeImage ()->getPreviewImageUrl () . '" id="theme_preview_image" /></a>' 
			] );
		}
