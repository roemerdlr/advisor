...
class AddToCartTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->checkoutSession = $this->getMockBuilder ( 'Magento\Checkout\Model\Session' )->setMethods ( [ 
				'getSharedWishlist',
		...
		$this->wishlistFactory = $this->getMockBuilder ( 'Magento\Wishlist\Model\WishlistFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
	...
	public function testExecute() {
		...
		$event = $this->getMockBuilder ( 'Magento\Framework\Event' )->setMethods ( [ 
				'getRequest',
				'getResponse' 
		] )->disableOriginalConstructor ()->getMock ();
		$request = $this->getMockBuilder ( 'Magento\Framework\App\RequestInterface' )->getMock ();
		$response = $this->getMockBuilder ( 'Magento\Framework\App\ResponseInterface' )->setMethods ( [ 
				'setRedirect' 
		...
		$loadedWishlist = $this->getMockBuilder ( 'Magento\Wishlist\Model\Wishlist\Item' )->setMethods ( [ 
				'getId',
				'delete' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->checkoutSession->expects ( $this->once () )->method ( 'getWishlistPendingMessages' )->willReturn ( [ 
				$message 
		] );
		$this->checkoutSession->expects ( $this->once () )->method ( 'getWishlistPendingUrls' )->willReturn ( [ 
				$url 
		] );
		...
		$wishlists->expects ( $this->once () )->method ( 'load' )->willReturn ( [ 
				$loadedWishlist 
		] );
