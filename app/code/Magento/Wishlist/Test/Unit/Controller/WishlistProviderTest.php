...
class WishlistProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->wishlistFactory = $this->getMock ( '\Magento\Wishlist\Model\WishlistFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->wishlistProvider = $objectManager->getObject ( 'Magento\Wishlist\Controller\WishlistProvider', [ 
	...
	public function testGetWishlistWithCustomer() {
		...
		$wishlist->expects ( $this->once () )->method ( 'loadByCustomerId' )->will ( $this->returnSelf () );
		$wishlist->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( 1 ) );
