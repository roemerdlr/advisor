...
class PluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function tearDown() {
		unset ( $this->customerSession, $this->authenticationState, $this->config, $this->redirector, $this->request );
	}
	protected function getPlugin() {
		return new \Magento\Wishlist\Controller\Index\Plugin ( $this->customerSession, $this->authenticationState, $this->config, $this->redirector );
	}
	...
	public function testBeforeDispatch() {
		...
		$this->customerSession->expects ( $this->at ( 2 ) )->method ( '__call' )->with ( 'setBeforeWishlistUrl', [ 
				'http://referer-url.com' 
		] )->willReturn ( false );
		$this->customerSession->expects ( $this->at ( 3 ) )->method ( '__call' )->with ( 'setBeforeWishlistRequest', [ 
				[ 
						'product' => 1 
				] 
		] )->willReturn ( true );
