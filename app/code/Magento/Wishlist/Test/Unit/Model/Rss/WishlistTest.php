...
class WishlistTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->wishlistHelperMock = $this->getMock ( 'Magento\Wishlist\Helper\Rss', [ 
				'getWishlist',
				'getCustomer',
				'getCustomerName' 
		], [ ], '', false );
		...
		$this->layoutMock = $this->getMockForAbstractClass ( 'Magento\Framework\View\LayoutInterface', [ ], '', true, true, true, [ 
				'getBlock' 
		] );
		
		$this->customerFactory = $this->getMockBuilder ( 'Magento\Customer\Model\CustomerFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$requestMock = $this->getMock ( '\Magento\Framework\App\RequestInterface' );
		$requestMock->expects ( $this->any () )->method ( 'getParam' )->with ( 'sharing_code' )->will ( $this->returnValue ( 'somesharingcode' ) );
		
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $objectManager->getObject ( 'Magento\Wishlist\Model\Rss\Wishlist', [ 
	...
	public function testGetRssData() {
		...
		$wishlistModelMock = $this->getMock ( 'Magento\Wishlist\Model\Wishlist', [ 
				'getId',
				'__wakeup',
				'getCustomerId',
				'getItemCollection',
		...
		$customer = $this->getMockBuilder ( 'Magento\Customer\Model\Customer' )->setMethods ( [ 
				'getName',
				'__wakeup',
				'load' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	protected function processWishlistItemDescription($wishlistModelMock, $staticArgs) {
		...
		$this->imageHelperMock->expects ( $this->once () )->method ( 'init' )->with ( $productMock, 'rss_thumbnail' )->will ( $this->returnSelf () );
		$this->imageHelperMock->expects ( $this->once () )->method ( 'getUrl' )->will ( $this->returnValue ( $imgThumbSrc ) );
		$priceRendererMock = $this->getMock ( 'Magento\Framework\Pricing\Render', [ 
				'render' 
		], [ ], '', false );
		...
		$this->wishlistBlock->expects ( $this->any () )->method ( 'getProductUrl' )->with ( $productMock, [ 
				'_rss' => true 
	...
	public function testIsAllowed() {
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'rss/wishlist/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->will ( $this->returnValue ( true ) );
