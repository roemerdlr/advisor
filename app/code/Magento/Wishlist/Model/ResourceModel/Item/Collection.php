...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
	...
	protected function _assignProducts() {
		\Magento\Framework\Profiler::start ( 'WISHLIST:' . __METHOD__, [ 
				'group' => 'WISHLIST',
				'method' => __METHOD__ 
		] );
		$productIds = [ ];
		...
		$this->_eventManager->dispatch ( 'wishlist_item_collection_products_after_load', [ 
	...
	public function addCustomerIdFilter($customerId) {
		...
		], 'main_table.wishlist_id = wishlist.wishlist_id', [ ] )->where ( 'wishlist.customer_id = ?', $customerId );
		return $this;
	}
	...
	protected function _joinProductNameTable() {
		if (! $this->_isProductNameJoined) {
			...
			], 'product_name_table.entity_id=main_table.product_id' . ' AND product_name_table.store_id=' . $storeId . ' AND product_name_table.attribute_id=' . $attribute->getId (), [ ] );
