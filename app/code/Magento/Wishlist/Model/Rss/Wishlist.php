...
class Wishlist implements DataProviderInterface {
	...
	public function isAllowed() {
		return ( bool ) $this->scopeConfig->getValue ( 'rss/wishlist/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
	}
	...
	public function getRssData() {
		...
		if ($wishlist->getId ()) {
			...
			foreach ( $wishlist->getItemCollection () as $wishlistItem ) {
				...
				$productUrl = $this->wishlistBlock->getProductUrl ( $product, [ 
						'_rss' => true 
				] );
	...
	public function getHeader() {
		...
		$newUrl = $this->urlBuilder->getUrl ( 'wishlist/shared/index', [ 
	...
	public function getProductPriceHtml(\Magento\Catalog\Model\Product $product) {
		...
		if (! $priceRender) {
			$priceRender = $this->layout->createBlock ( 'Magento\Framework\Pricing\Render', 'product.price.render.default', [ 
					'data' => [ 
		...
		if ($priceRender) {
			$price = $priceRender->render ( \Magento\Catalog\Pricing\Price\FinalPrice::PRICE_CODE, $product, [ 
