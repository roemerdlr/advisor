...
class Item extends AbstractModel implements ItemInterface {
	...
	/**
	 * Set quantity.
	 * If quantity is less than 0 - set it to 1
	 *
	 * @param int $qty        	
	 * @return $this
	 */
	 ...
	 * @param bool $delete
	 *        	delete the item after successful add to cart
	 * @return bool
	...
	public function addToCart(\Magento\Checkout\Model\Cart $cart, $delete = false) {
		...
		if (! $product->isVisibleInSiteVisibility ()) {
			...
			$urlData = $this->_catalogUrl->getRewriteByProductStore ( [ 
					$product->getId () => $storeId 
			] );
	...
	public function addOption($option) {
		if (is_array ( $option )) {
			...
			$option = $this->_wishlistOptFactory->create ()->setData ( $option->getData () )->setProduct ( $option->getProduct () )->setItem ( $this );
		} else {
