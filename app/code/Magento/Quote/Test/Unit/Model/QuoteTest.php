...
class QuoteTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->quoteAddressFactoryMock = $this->getMock ( 'Magento\Quote\Model\Quote\AddressFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->quoteAddressCollectionMock = $this->getMock ( 'Magento\Quote\Model\ResourceModel\Quote\Address\Collection', [ ], [ ], '', false );
		$this->extensibleDataObjectConverterMock = $this->getMock ( 'Magento\Framework\Api\ExtensibleDataObjectConverter', [ 
				'toFlatArray' 
		], [ ], '', false );
		$this->customerRepositoryMock = $this->getMockForAbstractClass ( 'Magento\Customer\Api\CustomerRepositoryInterface', [ ], '', false, true, true, [ 
				'getById' 
		] );
		$this->objectCopyServiceMock = $this->getMock ( 'Magento\Framework\DataObject\Copy', [ 
				'copyFieldsetToTarget' 
		], [ ], '', false );
		$this->productMock = $this->getMock ( '\Magento\Catalog\Model\Product', [ ], [ ], '', false );
		$this->objectFactoryMock = $this->getMock ( '\Magento\Framework\DataObject\Factory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->resourceMock = $this->getMockBuilder ( 'Magento\Quote\Model\ResourceModel\Quote' )->disableOriginalConstructor ()->getMock ();
		$this->contextMock = $this->getMockBuilder ( 'Magento\Framework\Model\Context' )->disableOriginalConstructor ()->getMock ();
		$this->customerFactoryMock = $this->getMockBuilder ( 'Magento\Customer\Model\CustomerFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->quoteItemCollectionFactoryMock = $this->getMock ( 'Magento\Quote\Model\ResourceModel\Quote\Item\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->quotePaymentCollectionFactoryMock = $this->getMock ( 'Magento\Quote\Model\ResourceModel\Quote\Payment\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->paymentFactoryMock = $this->getMock ( 'Magento\Quote\Model\Quote\PaymentFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->quote = (new ObjectManager ( $this ))->getObject ( 'Magento\Quote\Model\Quote', [ 
	...
	public function testIsMultipleShippingAddresses($addresses, $expected) {
		$this->quoteAddressCollectionMock->expects ( $this->any () )->method ( 'setQuoteFilter' )->will ( $this->returnValue ( $this->quoteAddressCollectionMock ) );
		$this->quoteAddressCollectionMock->expects ( $this->once () )->method ( 'getIterator' )->will ( $this->returnValue ( new \ArrayIterator ( $addresses ) ) );
	...
	public function testGetCustomerGroupIdNotSet() {
		$this->assertEquals ( \Magento\Customer\Model\GroupManagement::NOT_LOGGED_IN_ID, $this->quote->getCustomerGroupId (), "Customer group ID is invalid" );
	}
	...
	public function dataProviderForTestIsMultipleShippingAddresses() {
		return [ 
				[ 
						[ 
								$this->getAddressMock ( Address::TYPE_SHIPPING ),
								$this->getAddressMock ( Address::TYPE_SHIPPING ) 
						],
						true 
				],
				[ 
						[ 
								$this->getAddressMock ( Address::TYPE_SHIPPING ),
								$this->getAddressMock ( Address::TYPE_BILLING ) 
						],
						false 
				] 
		];
	 ...
	 *
	 * @param string $type
	 *        	One of \Magento\Customer\Model\Address\AbstractAddress::TYPE_ const
	 * @return \PHPUnit_Framework_MockObject_MockObject
	 */
	protected function getAddressMock($type) {
		$shippingAddressMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address', [ 
				'getAddressType',
				'__wakeup' 
		], [ ], '', false );
	...
	public function testLoadActive() {
		...
		$this->resourceMock->expects ( $this->once () )->method ( 'loadActive' )->with ( $this->quote, $quoteId );
	...
	public function testSetCustomerAddressData() {
		$customerId = 1;
		$addressMock = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\AddressInterface', [ ], '', false, true, true, [ 
				'getId' 
		] );
		...
		$result = $this->quote->setCustomerAddressData ( [ 
				$addressMock 
		] );
		$this->assertInstanceOf ( 'Magento\Quote\Model\Quote', $result );
	 ...
	 * @param
	 *        	$customerAddressId
	 * @param $expected @dataProvider
	 *        	dataProviderGetAddressByCustomer
	 */
	...
	public function testGetShippingAddressByCustomerAddressId($isDeleted, $addressType, $customerAddressId, $expected) {
		...
		$this->quoteAddressMock->expects ( $this->once () )->method ( 'isDeleted' )->will ( $this->returnValue ( $isDeleted ) );
		$this->quoteAddressMock->expects ( $this->once () )->method ( 'getCustomerAddressId' )->will ( $this->returnValue ( $customerAddressId ) );
		$this->quoteAddressMock->expects ( $this->once () )->method ( 'getAddressType' )->will ( $this->returnValue ( $addressType ) );
	...
	public function testRemoveAllAddresses() {
		...
		$this->quoteAddressMock->expects ( $this->any () )->method ( 'setData' )->will ( $this->returnSelf () );
		$this->quoteAddressMock->expects ( $this->once () )->method ( 'getId' )->will ( $this->returnValue ( $id ) );
	...
	public function testAddProductNoCandidates() {
		...
		$this->objectFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( $this->equalTo ( [ 
				'qty' => 1 
		...
		$typeInstanceMock->expects ( $this->once () )->method ( 'prepareForCartAdvanced' )->will ( $this->returnValue ( $expectedResult ) );
	...
	public function testAddProductItemPreparation() {
		...
		$collectionMock = $this->getMock ( 'Magento\Quote\Model\ResourceModel\Quote\Item\Collection', [ ], [ ], '', false );
		...
		$typeInstanceMock->expects ( $this->once () )->method ( 'prepareForCartAdvanced' )->will ( $this->returnValue ( [ 
				$productMock 
		] ) );
		$this->productMock->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $typeInstanceMock ) );
	...
	public function testValidateMiniumumAmount() {
		...
		$this->quoteAddressMock->expects ( $this->once () )->method ( 'validateMinimumAmount' )->willReturn ( true );
		
		$this->quoteAddressCollectionMock->expects ( $this->once () )->method ( 'setQuoteFilter' )->willReturn ( [ 
				$this->quoteAddressMock 
		] );
	...
	public function testGetPaymentIsNotDeleted() {
		$this->quote->setId ( 1 );
		$payment = $this->getMock ( 'Magento\Quote\Model\Quote\Payment', [ 
				'setQuote',
				'isDeleted',
				'__wakeup' 
		], [ ], '', false );
		$payment->expects ( $this->once () )->method ( 'setQuote' );
		$payment->expects ( $this->once () )->method ( 'isDeleted' )->willReturn ( false );
		$quotePaymentCollectionMock = $this->getMock ( 'Magento\Quote\Model\ResourceModel\Quote\Payment\Collection', [ 
				'setQuoteFilter',
				'getFirstItem' 
		], [ ], '', false );
	...
	public function testAddItem() {
		$item = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ 
				'setQuote',
				'getId' 
		], [ ], '', false );
		$item->expects ( $this->once () )->method ( 'setQuote' );
		$item->expects ( $this->once () )->method ( 'getId' )->willReturn ( false );
		$itemsMock = $this->getMock ( 'Magento\Eav\Model\Entity\Collection\AbstractCollection', [ 
				'setQuote',
				'addItem' 
		], [ ], '', false );
	...
	public function testGetItemsCollection() {
		$itemCollectionMock = $this->getMockBuilder ( 'Magento\Quote\Model\ResourceModel\Quote\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'setQuote' 
		] )->getMock ();
	...
	public function testGetAllItems() {
		$itemOneMock = $this->getMockBuilder ( 'Magento\Quote\Model\ResourceModel\Quote\Item' )->setMethods ( [ 
				'isDeleted' 
		] )->disableOriginalConstructor ()->getMock ();
		$itemOneMock->expects ( $this->once () )->method ( 'isDeleted' )->willReturn ( false );
		
		$itemTwoMock = $this->getMockBuilder ( 'Magento\Quote\Model\ResourceModel\Quote\Item' )->setMethods ( [ 
				'isDeleted' 
		] )->disableOriginalConstructor ()->getMock ();
