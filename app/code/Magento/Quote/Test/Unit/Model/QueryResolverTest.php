...
class QueryResolverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->quoteResolver = new \Magento\Quote\Model\QueryResolver ( $this->configMock, $this->cacheMock, 'connection_config_cache' );
	}
	public function testIsSingleQueryWhenDataWereCached() {
		$queryData ['checkout'] = true;
		$this->cacheMock->expects ( $this->once () )->method ( 'load' )->with ( 'connection_config_cache' )->willReturn ( serialize ( $queryData ) );
	...
	public function testIsSingleQueryWhenDataNotCached() {
		...
		$this->configMock->expects ( $this->once () )->method ( 'getConnectionName' )->with ( 'checkout_setup' )->willReturn ( 'default' );
