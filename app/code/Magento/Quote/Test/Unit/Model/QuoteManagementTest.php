...
class QuoteManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->orderFactory = $this->getMock ( 'Magento\Sales\Api\Data\OrderInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->quoteAddressToOrder = $this->getMock ( 'Magento\Quote\Model\Quote\Address\ToOrder', [ ], [ ], '', false );
		$this->quotePaymentToOrderPayment = $this->getMock ( 'Magento\Quote\Model\Quote\Payment\ToOrderPayment', [ ], [ ], '', false );
		$this->quoteAddressToOrderAddress = $this->getMock ( 'Magento\Quote\Model\Quote\Address\ToOrderAddress', [ ], [ ], '', false );
		...
		$this->storeManagerMock = $this->getMockForAbstractClass ( 'Magento\Store\Model\StoreManagerInterface', [ ], '', false, true, true, [ 
				'getStore',
				'getStoreId' 
		] );
		...
		$this->quoteAddressFactory = $this->getMock ( 'Magento\Quote\Model\Quote\AddressFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->accountManagementMock = $this->getMock ( '\Magento\Customer\Api\AccountManagementInterface', [ ], [ ], '', false );
		
		$this->quoteFactoryMock = $this->getMock ( '\Magento\Quote\Model\QuoteFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->model = $objectManager->getObject ( '\Magento\Quote\Model\QuoteManagement', [ 
	...
	public function testCreateEmptyCartForCustomer() {
		...
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'getActiveForCustomer' )->with ( $userId )->willThrowException ( new NoSuchEntityException () );
	 ...
	 * @expectedExceptionMessage Cannot assign customer to the given cart.
	 * The cart belongs to different store
	 */
	public function testAssignCustomerFromAnotherStore() {
		...
		$customerModelMock = $this->getMock ( '\Magento\Customer\Model\Customer', [ 
				'load',
				'getSharedStoreIds' 
		], [ ], '', false );
		...
		$customerModelMock->expects ( $this->once () )->method ( 'getSharedStoreIds' )->willReturn ( [ ] );
	 ...
	 * @expectedException \Magento\Framework\Exception\StateException
	 * @expectedExceptionMessage Cannot assign customer to the given cart.
	 * Customer already has active cart.
	 */
	...
	public function testSubmit() {
		...
		$this->quoteAddressToOrderAddress->expects ( $this->at ( 0 ) )->method ( 'convert' )->with ( $shippingAddress, [ 
				'address_type' => 'shipping',
				'email' => 'customer@example.com' 
		] )->willReturn ( $convertedShippingAddress );
		$this->quoteAddressToOrderAddress->expects ( $this->at ( 1 ) )->method ( 'convert' )->with ( $billingAddress, [ 
				'address_type' => 'billing',
				'email' => 'customer@example.com' 
		] )->willReturn ( $convertedBillingAddress );
		$this->quoteItemToOrderItem->expects ( $this->once () )->method ( 'convert' )->with ( $quoteItem, [ 
				'parent_item' => null 
		...
		$order = $this->prepareOrderFactory ( $baseOrder, $convertedBillingAddress, $addresses, $convertedPayment, $convertedItems, $quoteId, $convertedShippingAddress );
		
		$this->orderManagement->expects ( $this->once () )->method ( 'place' )->with ( $order )->willReturn ( $order );
		
		$this->eventManager->expects ( $this->at ( 0 ) )->method ( 'dispatch' )->with ( 'sales_model_service_quote_submit_before', [ 
				'order' => $order,
				'quote' => $quote 
		] );
		$this->eventManager->expects ( $this->at ( 1 ) )->method ( 'dispatch' )->with ( 'sales_model_service_quote_submit_success', [ 
				'order' => $order,
				'quote' => $quote 
		] );
	...
	public function testPlaceOrderIfCustomerIsGuest() {
		...
		$this->quoteMock->expects ( $this->once () )->method ( 'getCheckoutMethod' )->willReturn ( \Magento\Checkout\Model\Type\Onepage::METHOD_GUEST );
		...
		$addressMock = $this->getMock ( '\Magento\Quote\Model\Quote\Address', [ 
				'getEmail' 
		], [ ], '', false );
		...
		$this->quoteMock->expects ( $this->once () )->method ( 'setCustomerGroupId' )->with ( \Magento\Customer\Api\Data\GroupInterface::NOT_LOGGED_IN_ID );
		
		/** @var \PHPUnit_Framework_MockObject_MockObject|\Magento\Quote\Model\QuoteManagement $service */
		$service = $this->getMock ( '\Magento\Quote\Model\QuoteManagement', [ 
				'submit' 
		], [ 
				'eventManager' => $this->eventManager,
				'quoteValidator' => $this->quoteValidator,
	...
	public function testPlaceOrder() {
		...
		$quotePayment->expects ( $this->once () )->method ( 'setQuote' );
		$quotePayment->expects ( $this->once () )->method ( 'importData' );
		$this->quoteMock->expects ( $this->atLeastOnce () )->method ( 'getPayment' )->willReturn ( $quotePayment );
		
		$this->quoteMock->expects ( $this->once () )->method ( 'getCheckoutMethod' )->willReturn ( \Magento\Checkout\Model\Type\Onepage::METHOD_CUSTOMER );
		$this->quoteMock->expects ( $this->never () )->method ( 'setCustomerIsGuest' )->with ( true );
		...
		$paymentMethod = $this->getMock ( 'Magento\Quote\Model\Quote\Payment', [ 
				'setChecks',
				'getData' 
		], [ ], '', false );
		$paymentMethod->expects ( $this->once () )->method ( 'setChecks' );
		$paymentMethod->expects ( $this->once () )->method ( 'getData' )->willReturn ( [ 
				'additional_data' => [ ] 
		] );
	 ...
	 */
	protected function getQuote($isGuest, $isVirtual, \Magento\Quote\Model\Quote\Address $billingAddress, \Magento\Quote\Model\Quote\Payment $payment, $customerId, $id, array $quoteItems, \Magento\Quote\Model\Quote\Address $shippingAddress = null) {
		$quote = $this->getMock ( 'Magento\Quote\Model\Quote', [ 
				'setIsActive',
				'getCustomerEmail',
				'getAllVisibleItems',
				'getCustomerIsGuest',
		...
		$quote->expects ( $this->once () )->method ( 'getCustomerIsGuest' )->willReturn ( $isGuest );
		$quote->expects ( $this->once () )->method ( 'isVirtual' )->willReturn ( $isVirtual );
		if ($shippingAddress) {
			$quote->expects ( $this->exactly ( 3 ) )->method ( 'getShippingAddress' )->willReturn ( $shippingAddress );
		}
		...
		$quote->expects ( $this->atLeastOnce () )->method ( 'getCustomerEmail' )->willReturn ( 'customer@example.com' );
		$quote->expects ( $this->any () )->method ( 'getCustomer' )->willReturn ( $customer );
		$quote->expects ( $this->once () )->method ( 'getId' )->willReturn ( $id );
	 ...
	 * @return \PHPUnit_Framework_MockObject_MockObject
	 */
	protected function prepareOrderFactory(\Magento\Sales\Api\Data\OrderInterface $baseOrder, \Magento\Sales\Api\Data\OrderAddressInterface $billingAddress, array $addresses, $payment, array $items, $quoteId, \Magento\Sales\Api\Data\OrderAddressInterface $shippingAddress = null, $customerId = null) {
