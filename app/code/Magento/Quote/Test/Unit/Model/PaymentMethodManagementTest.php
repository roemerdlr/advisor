...
class PaymentMethodManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->quoteRepositoryMock = $this->getMockForAbstractClass ( 'Magento\Quote\Api\CartRepositoryInterface', [ ], '', false, true, true, [ ] );
		...
		$this->model = $this->objectManager->getObject ( 'Magento\Quote\Model\PaymentMethodManagement', [ 
	...
	public function testGetList() {
		...
		$this->methodListMock->expects ( $this->once () )->method ( 'getAvailableMethods' )->with ( $quoteMock )->will ( $this->returnValue ( [ 
				$paymentMethod 
		] ) );
		$this->assertEquals ( [ 
				$paymentMethod 
	...
	public function testSetVirtualProduct() {
		...
		$methodMock = $this->getMock ( 'Magento\Quote\Model\Quote\Payment', [ 
				'setChecks',
				'getData' 
		], [ ], '', false );
		$methodMock->expects ( $this->once () )->method ( 'setChecks' )->with ( [ 
		...
		$billingAddressMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address', [ 
				'getCountryId',
				'setPaymentMethod' 
		], [ ], '', false );
		$billingAddressMock->expects ( $this->once () )->method ( 'setPaymentMethod' )->with ( $paymentMethod )->willReturnSelf ();
		...
		$this->zeroTotalMock->expects ( $this->once () )->method ( 'isApplicable' )->with ( $methodInstance, $quoteMock )->willReturn ( true );
	...
	public function testSetVirtualProductThrowsExceptionIfPaymentMethodNotAvailable() {
		...
		$this->zeroTotalMock->expects ( $this->once () )->method ( 'isApplicable' )->with ( $methodInstance, $quoteMock )->willReturn ( false );
	...
	public function testSetSimpleProduct() {
		...
		$shippingAddressMock->expects ( $this->once () )->method ( 'setPaymentMethod' )->with ( $paymentMethod )->willReturnSelf ();
