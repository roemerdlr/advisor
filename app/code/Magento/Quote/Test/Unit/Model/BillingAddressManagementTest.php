...
class BillingAddressManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $this->objectManager->getObject ( '\Magento\Quote\Model\BillingAddressManagement', [ 
	...
	public function testSetAddressValidationFailed() {
		...
		$this->validatorMock->expects ( $this->once () )->method ( 'validate' )->will ( $this->throwException ( new \Magento\Framework\Exception\NoSuchEntityException ( __ ( 'error123' ) ) ) );
	...
	public function testSetAddress() {
		...
		$this->validatorMock->expects ( $this->once () )->method ( 'validate' )->with ( $address )->willReturn ( true );
		...
		$this->addressRepository->expects ( $this->once () )->method ( 'getById' )->with ( $customerAddressId )->willReturn ( $customerAddressMock );
		...
		$quoteBillingAddress->expects ( $this->once () )->method ( 'importCustomerAddressData' )->with ( $customerAddressMock )->willReturnSelf ();
		
		$quoteShippingAddress = $this->getMock ( 'Magento\Quote\Model\Quote\Address', [ 
				'setSaveInAddressBook',
				'setSameAsBilling',
		...
		$quoteShippingAddress->expects ( $this->once () )->method ( 'importCustomerAddressData' )->with ( $customerAddressMock )->willReturnSelf ();
	...
	public function testSetAddressWithInabilityToSaveQuote() {
		...
		$this->validatorMock->expects ( $this->once () )->method ( 'validate' )->with ( $address )->will ( $this->returnValue ( true ) );
