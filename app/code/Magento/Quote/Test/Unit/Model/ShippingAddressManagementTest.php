...
class ShippingAddressManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->service = $this->objectManager->getObject ( '\Magento\Quote\Model\ShippingAddressManagement', [ 
	...
	public function testSetAddressValidationFailed() {
		...
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'getActive' )->with ( 'cart654' )->will ( $this->returnValue ( $quoteMock ) );
		
		$this->validatorMock->expects ( $this->once () )->method ( 'validate' )->will ( $this->throwException ( new \Magento\Framework\Exception\NoSuchEntityException ( __ ( 'error345' ) ) ) );
	...
	public function testSetAddress() {
		...
		$quoteMock->expects ( $this->any () )->method ( 'setShippingAddress' )->with ( $this->quoteAddressMock )->willReturnSelf ();
		$quoteMock->expects ( $this->any () )->method ( 'getShippingAddress' )->will ( $this->returnValue ( $this->quoteAddressMock ) );
