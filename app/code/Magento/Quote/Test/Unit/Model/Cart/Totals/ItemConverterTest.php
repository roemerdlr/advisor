...
class ItemConverterTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->totalsFactoryMock = $this->getMock ( 'Magento\Quote\Api\Data\TotalsItemInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->model = new \Magento\Quote\Model\Cart\Totals\ItemConverter ( $this->configPoolMock, $this->eventManagerMock, $this->totalsFactoryMock, $this->dataObjectHelperMock );
	}
	public function testModelToDataObject() {
		$productType = 'simple';
		
		$itemMock = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ ], [ ], '', false );
		$itemMock->expects ( $this->once () )->method ( 'toArray' )->will ( $this->returnValue ( [ 
				'options' => [ ] 
		] ) );
		...
		$this->configPoolMock->expects ( $this->any () )->method ( 'getByProductType' )->will ( $this->returnValueMap ( [ 
				[ 
						'simple',
						$simpleConfigMock 
				],
		...
		$simpleConfigMock->expects ( $this->once () )->method ( 'getOptions' )->with ( $itemMock )->will ( $this->returnValue ( $options ) );
		...
		$this->eventManagerMock->expects ( $this->once () )->method ( 'dispatch' )->with ( 'items_additional_data', [ 
				'item' => $itemMock 
		] );
		...
		$this->dataObjectHelperMock->expects ( $this->once () )->method ( 'populateWithArray' )->with ( null, $expectedData, '\Magento\Quote\Api\Data\TotalsItemInterface' );
