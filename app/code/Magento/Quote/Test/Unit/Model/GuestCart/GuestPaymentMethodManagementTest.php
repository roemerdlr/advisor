...
class GuestPaymentMethodManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->paymentMethodManagementMock = $this->getMock ( 'Magento\Quote\Api\PaymentMethodManagementInterface', [ ], [ ], '', false );
		...
		$this->model = $objectManager->getObject ( 'Magento\Quote\Model\GuestCart\GuestPaymentMethodManagement', [ 
	...
	public function testGetList() {
		...
		$this->paymentMethodManagementMock->expects ( $this->once () )->method ( 'getList' )->willReturn ( [ 
				$paymentMethod 
		] );
		$this->assertEquals ( [ 
				$paymentMethod 
		], $this->model->getList ( $this->maskedCartId ) );
	}
