...
class QuoteRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->searchResultsDataFactory = $this->getMock ( '\Magento\Quote\Api\Data\CartSearchResultsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->quoteCollectionMock = $this->getMock ( 'Magento\Quote\Model\ResourceModel\Quote\Collection', [ ], [ ], '', false );
		...
		$this->model = $objectManager->getObject ( 'Magento\Quote\Model\QuoteRepository', [ 
	...
	public function testGetListSuccess($direction, $expectedDirection) {
		...
		$filterGroupMock->expects ( $this->any () )->method ( 'getFilters' )->will ( $this->returnValue ( [ 
				$filterMock 
		] ) );
		...
		$sortOrderMock = $this->getMockBuilder ( 'Magento\Framework\Api\SortOrder' )->setMethods ( [ 
				'getField',
				'getDirection' 
		] )->disableOriginalConstructor ()->getMock ();
		
		// foreach cycle
		$searchCriteriaMock->expects ( $this->once () )->method ( 'getSortOrders' )->will ( $this->returnValue ( [ 
				$sortOrderMock 
		] ) );
		...
		$this->quoteCollectionMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$cartMock 
		] );
