...
class ShippingMethodManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->methodDataFactoryMock = $this->getMock ( '\Magento\Quote\Api\Data\ShippingMethodInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->converterMock = $this->getMock ( '\Magento\Quote\Model\Cart\ShippingMethodConverter', [ ], [ ], '', false );
		
		$this->model = $this->objectManager->getObject ( 'Magento\Quote\Model\ShippingMethodManagement', [ 
	...
	public function testGetMethod() {
		...
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getShippingRateByCode' )->with ( 'one_two' )->willReturn ( $shippingRateMock );
		
		$this->shippingMethodMock = $this->getMock ( '\Magento\Quote\Api\Data\ShippingMethodInterface' );
		$this->converterMock->expects ( $this->once () )->method ( 'modelToDataObject' )->with ( $shippingRateMock, $currencyCode )->willReturn ( $this->shippingMethodMock );
	...
	public function testGetList() {
		...
		$shippingRateMock = $this->getMock ( '\Magento\Quote\Model\Quote\Address\Rate', [ ], [ ], '', false );
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getGroupedAllShippingRates' )->will ( $this->returnValue ( [ 
				[ 
						$shippingRateMock 
				] 
		] ) );
	...
	public function testSetMethodWithNotFoundMethod() {
		...
		$this->shippingAddressMock->expects ( $this->once () )->method ( 'getShippingRateByCode' )->will ( $this->returnValue ( false ) );
	 ...
	 * @expectedExceptionMessage Cannot set shipping method.
	 * Custom Error
	 */
