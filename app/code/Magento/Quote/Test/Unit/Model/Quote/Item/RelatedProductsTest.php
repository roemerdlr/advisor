...
class RelatedProductsTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetRelatedProductIds($optionValue, $productId, $expectedResult) {
		...
		$itemOptionMock = $this->getMock ( '\Magento\Quote\Model\Quote\Item\Option', [ 
				'getValue',
				'getProductId',
				'__wakeup' 
		], [ ], '', false );
		
		$quoteItemMock->expects ( $this->once () )->method ( 'getOptionByCode' )->with ( 'product_type' )->will ( $this->returnValue ( $itemOptionMock ) );
		...
		$this->assertEquals ( $expectedResult, $this->model->getRelatedProductIds ( [ 
				$quoteItemMock 
		] ) );
	}
	...
	public function testGetRelatedProductIdsNoOptions() {
		...
		$this->assertEquals ( [ ], $this->model->getRelatedProductIds ( [ 
				$quoteItemMock 
		] ) );
	}
}
