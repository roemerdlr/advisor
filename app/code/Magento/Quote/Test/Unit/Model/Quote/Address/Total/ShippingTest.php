...
class ShippingTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->freeShipping = $this->getMockForAbstractClass ( 'Magento\Quote\Model\Quote\Address\FreeShippingInterface', [ ], '', false );
		$this->priceCurrency = $this->getMockForAbstractClass ( 'Magento\Framework\Pricing\PriceCurrencyInterface', [ ], '', false );
		...
		$this->total = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Total', [ 
				'setShippingAmount',
		...
		$this->shippingAssignment = $this->getMockForAbstractClass ( 'Magento\Quote\Api\Data\ShippingAssignmentInterface', [ ], '', false );
		...
		$this->cartItem = $this->getMockForAbstractClass ( 'Magento\Quote\Api\Data\CartItemInterface', [ ], '', false, false, true, [ 
				'getFreeShipping',
				'getProduct',
				'getParentItem',
				'getHasChildren',
		...
		$this->rate = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Rate', [ 
				'getPrice',
				'getCode',
				'getCarrierTitle',
				'getMethodTitle' 
	...
	public function testFetch() {
		...
		$totalMock = $this->getMock ( '\Magento\Quote\Model\Quote\Address\Total', [ 
				'getShippingAmount',
				'getShippingDescription' 
		], [ ], '', false );
	...
	public function testCollect() {
		...
		$this->shippingAssignment->expects ( $this->atLeastOnce () )->method ( 'getItems' )->willReturn ( [ 
				$this->cartItem 
		] );
		$this->freeShipping->expects ( $this->once () )->method ( 'isFreeShipping' )->with ( $this->quote, [ 
				$this->cartItem 
		] )->willReturn ( true );
		...
		$this->address->expects ( $this->once () )->method ( 'getAllShippingRates' )->willReturn ( [ 
				$this->rate 
		] );
		...
		$this->priceCurrency->expects ( $this->once () )->method ( 'convert' )->with ( 5, $this->store )->willReturn ( 10 );
		$this->total->expects ( $this->once () )->method ( 'setShippingAmount' )->with ( 10 );
		$this->total->expects ( $this->once () )->method ( 'setBaseShippingAmount' )->with ( 5 );
