...
class RepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->itemDataFactoryMock = $this->getMock ( 'Magento\Quote\Api\Data\CartItemInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->customOptionProcessor = $this->getMock ( 'Magento\Catalog\Model\CustomOptions\CustomOptionProcessor', [ ], [ ], '', false );
		
		$this->repository = new \Magento\Quote\Model\Quote\Item\Repository ( $this->quoteRepositoryMock, $this->productRepositoryMock, $this->itemDataFactoryMock, [ 
	 ...
	 * @return void @expectedException \Magento\Framework\Exception\InputException
	...
	/**
	 *
	 * @return void @expectedException \Magento\Framework\Exception\LocalizedException
	...
	public function testSaveCouldNotAddProduct() {
		...
		$buyRequest->expects ( $this->once () )->method ( 'setData' )->with ( 'qty', '12' );
		...
		$this->productRepositoryMock->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( $this->productMock ) );
		$this->dataMock->expects ( $this->once () )->method ( 'getSku' );
		$this->quoteMock->expects ( $this->once () )->method ( 'addProduct' )->with ( $this->productMock, $buyRequest )->willReturn ( 'Please specify all the required information.' );
		...
		$this->quoteMock->expects ( $this->never () )->method ( 'getAllItems' );
	...
	/**
	 *
	 * @return void @expectedException \Magento\Framework\Exception\CouldNotSaveException
	...
	public function testSaveCouldNotSaveException() {
		...
		$this->productRepositoryMock->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( $this->productMock ) );
		$this->dataMock->expects ( $this->once () )->method ( 'getSku' );
		$this->quoteMock->expects ( $this->once () )->method ( 'addProduct' )->with ( $this->productMock, $buyRequest )->willReturn ( $this->productMock );
		$this->quoteMock->expects ( $this->never () )->method ( 'getItemById' );
	...
	public function testSave() {
		...
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'save' )->with ( $this->quoteMock );
		$this->dataMock->expects ( $this->once () )->method ( 'getItemId' )->will ( $this->returnValue ( null ) );
		$this->quoteMock->expects ( $this->once () )->method ( 'getAllItems' )->willReturn ( [ 
				$this->quoteItemMock 
		] );
	...
	/**
	 *
	 * @return void @expectedException \Magento\Framework\Exception\NoSuchEntityException
	...
	public function testUpdateItemWithInvalidQuoteItem() {
		...
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'getActive' )->with ( $cartId )->will ( $this->returnValue ( $this->quoteMock ) );
		$this->quoteMock->expects ( $this->once () )->method ( 'getItemById' )->with ( $itemId )->will ( $this->returnValue ( false ) );
	...
	public function testUpdateItemQty() {
		...
		$this->customOptionProcessor->expects ( $this->once () )->method ( 'convertToBuyRequest' )->with ( $this->dataMock )->willReturn ( $buyRequest );
		$buyRequest->expects ( $this->once () )->method ( 'setData' )->with ( 'qty', '12' );
		$this->quoteMock->expects ( $this->once () )->method ( 'updateItem' )->with ( $itemId, $buyRequest )->willReturn ( $this->dataMock );
		$this->assertEquals ( $this->quoteItemMock, $this->repository->save ( $this->dataMock ) );
	}
	...
	public function testDeleteWithCouldNotSaveException() {
		...
		$this->quoteRepositoryMock->expects ( $this->once () )->method ( 'getActive' )->with ( $cartId )->will ( $this->returnValue ( $this->quoteMock ) );
		$this->quoteMock->expects ( $this->once () )->method ( 'getItemById' )->with ( $itemId )->will ( $this->returnValue ( $this->quoteItemMock ) );
		$this->quoteMock->expects ( $this->once () )->method ( 'removeItem' )->with ( $itemId )->will ( $this->returnValue ( $this->quoteMock ) );
		$this->quoteMock->expects ( $this->once () )->method ( 'collectTotals' )->will ( $this->returnValue ( $this->quoteMock ) );
	...
	public function testGetList() {
		...
		$quoteMock->expects ( $this->any () )->method ( 'getAllItems' )->will ( $this->returnValue ( [ 
				$itemMock 
		] ) );
		
		$this->assertEquals ( [ 
				$itemMock 
		], $this->repository->getList ( 33 ) );
	}
