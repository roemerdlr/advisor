...
class CompareTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->comparedMock = $this->getMock ( 'Magento\Quote\Model\Quote\Item', [ 
				'__wakeup',
				'getProductId',
				'getOptions' 
		], [ ], '', false );
		$this->optionMock = $this->getMock ( 'Magento\Quote\Model\Quote\Item\Option', [ 
				'__wakeup',
				'getCode',
				'getValue' 
		], [ ], '', false );
	...
	protected function getOptionMock($code, $value) {
		...
		$optionMock->expects ( $this->once () )->method ( 'getCode' )->will ( $this->returnValue ( $code ) );
		$optionMock->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $value ) );
		return $optionMock;
	}
	...
	public function testCompareDifferentProduct() {
		$this->itemMock->expects ( $this->once () )->method ( 'getProductId' )->will ( $this->returnValue ( 1 ) );
		$this->itemMock->expects ( $this->once () )->method ( 'getProductId' )->will ( $this->returnValue ( 2 ) );
	...
	public function testCompareProductWithDifferentOptions() {
		...
		$this->comparedMock->expects ( $this->any () )->method ( 'getOptions' )->will ( $this->returnValue ( [ 
