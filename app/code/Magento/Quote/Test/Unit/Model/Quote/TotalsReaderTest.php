...
class TotalsReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->totalMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Total', [ 
				'setData' 
		], [ ], '', false );
		$this->quoteMock = $this->getMock ( '\Magento\Quote\Model\Quote', [ ], [ ], '', false );
		$this->collectorMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Total\AbstractTotal', [ ], [ ], '', false );
		$this->model = new \Magento\Quote\Model\Quote\TotalsReader ( $this->totalFactoryMock, $this->collectionListMock );
	}
	public function testFetch() {
		$total = [ ];
		$storeId = 1;
		$testedTotalMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Total', [ 
				'setData',
				'getCode' 
		], [ ], '', false );
		$expected = [ 
		...
		$this->totalFactoryMock->expects ( $this->at ( 0 ) )->method ( 'create' )->willReturn ( $this->totalMock );
		$this->totalFactoryMock->expects ( $this->at ( 1 ) )->method ( 'create' )->willReturn ( $testedTotalMock );
		$this->collectionListMock->expects ( $this->once () )->method ( 'getCollectors' )->with ( $storeId )->willReturn ( [ 
				$this->collectorMock 
		] );
		$this->collectorMock->expects ( $this->once () )->method ( 'fetch' )->with ( $this->quoteMock, $this->totalMock )->willReturn ( $data );
	...
	public function testFetchWithEmptyData() {
		...
		$this->collectorMock->expects ( $this->once () )->method ( 'fetch' )->with ( $this->quoteMock, $this->totalMock )->willReturn ( [ ] );
		$this->assertEquals ( [ ], $this->model->fetch ( $this->quoteMock, $total ) );
	}
	public function testFetchSeveralCollectors() {
		$total = [ ];
		$storeId = 1;
		$firstTotalMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Total', [ 
				'setData',
				'getCode' 
		], [ ], '', false );
		$secondTotalMock = $this->getMock ( 'Magento\Quote\Model\Quote\Address\Total', [ 
				'setData',
				'getCode' 
		], [ ], '', false );
		$expected = [ 
		...
		$data = [ 
				[ 
						'code' => 'first_total_type' 
				],
				[ 
						'code' => 'second_total_type' 
				] 
		];
		$this->totalMock->expects ( $this->once () )->method ( 'setData' )->with ( [ ] )->willReturnSelf ();
		$this->quoteMock->expects ( $this->once () )->method ( 'getStoreId' )->willReturn ( $storeId );
