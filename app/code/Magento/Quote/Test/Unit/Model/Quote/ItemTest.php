...
class ItemTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->localeFormat = $this->getMockBuilder ( 'Magento\Framework\Locale\FormatInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->modelContext = $this->getMockBuilder ( 'Magento\Framework\Model\Context' )->disableOriginalConstructor ()->setMethods ( [ 
				'getEventDispatcher' 
		] )->getMock ();
		
		$this->eventDispatcher = $this->getMockBuilder ( 'Magento\Framework\Event\ManagerInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'dispatch' 
		] )->getMock ();
		
		$this->modelContext->expects ( $this->any () )->method ( 'getEventDispatcher' )->will ( $this->returnValue ( $this->eventDispatcher ) );
		
		$this->errorInfos = $this->getMockBuilder ( 'Magento\Sales\Model\Status\ListStatus' )->disableOriginalConstructor ()->setMethods ( [ 
				'clear',
				'addItem',
				'getItems',
				'removeItemsByParams' 
		...
		$statusListFactory = $this->getMockBuilder ( 'Magento\Sales\Model\Status\ListFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$statusListFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->errorInfos ) );
		
		$this->itemOptionFactory = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item\OptionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->compareHelper = $this->getMock ( 'Magento\Quote\Model\Quote\Item\Compare', [ ], [ ], '', false );
		
		$this->stockItemMock = $this->getMock ( 'Magento\CatalogInventory\Model\Stock\Item', [ 
				'getIsQtyDecimal',
				'__wakeup' 
		], [ ], '', false );
		
		$this->stockRegistry = $this->getMockBuilder ( 'Magento\CatalogInventory\Model\StockRegistry' )->disableOriginalConstructor ()->setMethods ( [ 
				'getStockItem',
				'__wakeup' 
		] )->getMock ();
		$this->stockRegistry->expects ( $this->any () )->method ( 'getStockItem' )->will ( $this->returnValue ( $this->stockItemMock ) );
		
		$this->model = $this->objectManagerHelper->getObject ( '\Magento\Quote\Model\Quote\Item', [ 
	...
	public function testGetAddress() {
		$quote = $this->getMockBuilder ( 'Magento\Quote\Model\Quote' )->setMethods ( [ 
				'getShippingAddress',
				'getBillingAddress',
				'getStoreId',
				'__wakeup' 
	...
	public function testAddQtyNormal() {
		...
		$this->localeFormat->expects ( $this->at ( 0 ) )->method ( 'getNumber' )->with ( $quantityToAdd )->will ( $this->returnValue ( $preparedQuantityToAdd ) );
		
		$this->localeFormat->expects ( $this->at ( 1 ) )->method ( 'getNumber' )->with ( $preparedQuantityToAdd + $existingQuantity )->will ( $this->returnValue ( $preparedQuantityToAdd + $existingQuantity ) );
	...
	public function testAddQtyExistingParentItemAndId() {
		...
		$parentItemMock = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item' )->setMethods ( [ 
				'addChild',
				'__wakeup' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testSetQty() {
		...
		$this->eventDispatcher->expects ( $this->once () )->method ( 'dispatch' )->with ( 'sales_quote_item_qty_set_after', [ 
				'item' => $this->model 
		] );
	...
	public function testSetQtyQuoteIgnoreOldQuantity() {
		...
		$quoteMock->expects ( $this->once () )->method ( 'getIgnoreOldQty' )->will ( $this->returnValue ( true ) );
		$quoteMock->expects ( $this->any () )->method ( 'getStoreId' )->will ( $this->returnValue ( 1 ) );
	...
	public function testSetProduct() {
		...
		$this->eventDispatcher->expects ( $this->once () )->method ( 'dispatch' )->with ( 'sales_quote_item_set_product', [ 
				'product' => $productMock,
				'quote_item' => $this->model 
		] );
	...
	private function generateProductMock($productId, $productType, $productSku, $productName, $productWeight, $productTaxClassId, $productCost) {
		$productMock = $this->getMockBuilder ( 'Magento\Catalog\Model\Product' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'getTypeId',
				'getSku',
				'getName',
		...
		$productMock->expects ( $this->any () )->method ( 'getTypeId' )->will ( $this->returnValue ( $productType ) );
		$productMock->expects ( $this->any () )->method ( 'getSku' )->will ( $this->returnValue ( $productSku ) );
		$productMock->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( $productName ) );
		$productMock->expects ( $this->any () )->method ( 'getWeight' )->will ( $this->returnValue ( $productWeight ) );
		$productMock->expects ( $this->any () )->method ( 'getTaxClassId' )->will ( $this->returnValue ( $productTaxClassId ) );
		$productMock->expects ( $this->any () )->method ( 'getCost' )->will ( $this->returnValue ( $productCost ) );
		$store = $this->getMock ( 'Magento\Store\Model\Store', [ 
				'getWebsiteId' 
		], [ ], '', false );
		$store->expects ( $this->any () )->method ( 'getWebsiteId' )->will ( $this->returnValue ( 10 ) );
		
		$productMock->expects ( $this->any () )->method ( 'getStore' )->will ( $this->returnValue ( $store ) );
	...
	public function testRepresentProductStickWithinParentNotSameAsParentItem() {
		...
		$productMock->expects ( $this->once () )->method ( 'getStickWithinParent' )->will ( $this->returnValue ( true ) );
	...
	public function testRepresentProductItemOptionsNotInProductOptions() {
		...
		$productMock->expects ( $this->once () )->method ( 'getCustomOptions' )->will ( $this->returnValue ( [ 
				$optionCode1 => $optionMock1 
		] ) );
	...
	public function testCompareOptionsEqual() {
		...
		$this->assertTrue ( $this->model->compareOptions ( [ 
				$optionCode1 => $optionMock1 
		], [ 
				$optionCode1 => $optionMock1 
		] ) );
	...
	public function testCompareOptionsDifferentValues() {
		...
		$this->assertFalse ( $this->model->compareOptions ( [ 
				$optionCode1 => $optionMock1 
		], [ 
				$optionCode2 => $optionMock2 
		] ) );
	...
	public function testCompareOptionsNullValues() {
		...
		$optionCode2 = 1234;
		$optionMock2 = $this->createOptionMock ( $optionCode1 );
		$optionMock2->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( null ) );
	...
	public function testGetQtyOptions() {
		...
		$this->assertEquals ( [ 
				self::PRODUCT_ID => $optionMock2 
		], $this->model->getQtyOptions () );
	}
	...
	public function testGetProductTypeOption() {
		...
		$optionMock->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $optionProductTypeValue ) );
	...
	public function testSetOptions() {
		...
		$this->assertSame ( $this->model, $this->model->setOptions ( [ 
				$optionMock1,
				$optionMock2 
		] ) );
		$this->assertEquals ( [ 
	...
	private function createOptionMock($optionCode, $optionData = []) {
		$optionMock = $this->getMockBuilder ( 'Magento\Quote\Model\Quote\Item\Option' )->setMethods ( [ 
				'setData',
				'setItem',
				'getCode',
				'__wakeup',
		...
		$optionMock->expects ( $this->any () )->method ( 'setData' )->with ( $optionData )->will ( $this->returnValue ( $optionMock ) );
		$optionMock->expects ( $this->any () )->method ( 'setItem' )->with ( $this->model )->will ( $this->returnValue ( $optionMock ) );
		$optionMock->expects ( $this->any () )->method ( 'getCode' )->will ( $this->returnValue ( $optionCode ) );
	...
	public function testAddOptionArray() {
		...
		$this->assertEquals ( [ 
				$optionCode => $optionMock 
		], $this->model->getOptionsByCode () );
		$this->assertEquals ( $optionMock, $this->model->getOptionByCode ( $optionCode ) );
	}
	public function testUpdateQtyOption() {
		$productMock = $this->generateProductMock ( self::PRODUCT_ID, self::PRODUCT_TYPE, self::PRODUCT_SKU, self::PRODUCT_NAME, self::PRODUCT_WEIGHT, self::PRODUCT_TAX_CLASS_ID, self::PRODUCT_COST );
		
		$typeInstanceMock = $this->getMockForAbstractClass ( 'Magento\Catalog\Model\Product\Type\AbstractType', [ ], '', false, false, true, [ 
				'updateQtyOption' 
		] );
		$productMock->expects ( $this->once () )->method ( 'getTypeInstance' )->will ( $this->returnValue ( $typeInstanceMock ) );
		
		$optionMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->setMethods ( [ 
				'getProduct' 
		] )->getMock ();
		$optionMock->expects ( $this->once () )->method ( 'getProduct' )->will ( $this->returnValue ( $productMock ) );
		
		$quantityValue = 12;
		
		$this->model->setProduct ( $productMock );
		$typeInstanceMock->expects ( $this->once () )->method ( 'updateQtyOption' )->with ( $this->model->getOptions (), $optionMock, $quantityValue, $productMock );
	...
	public function testRemoveOption() {
		...
		$optionMock->expects ( $this->once () )->method ( 'setItem' )->with ( $this->model )->will ( $this->returnValue ( $optionMock ) );
		$optionMock->expects ( $this->exactly ( 3 ) )->method ( 'getCode' )->will ( $this->returnValue ( $optionCode ) );
		$optionMock->expects ( $this->at ( 0 ) )->method ( 'isDeleted' )->will ( $this->returnValue ( false ) );
		$optionMock->expects ( $this->at ( 1 ) )->method ( 'isDeleted' )->will ( $this->returnValue ( true ) );
	...
	public function testGetOptionByCodeDeletedCode() {
		...
		$optionMock->expects ( $this->once () )->method ( 'setItem' )->with ( $this->model )->will ( $this->returnValue ( $optionMock ) );
		$optionMock->expects ( $this->exactly ( 3 ) )->method ( 'getCode' )->will ( $this->returnValue ( $optionCode ) );
		$optionMock->expects ( $this->once () )->method ( 'isDeleted' )->will ( $this->returnValue ( true ) );
	...
	public function testGetBuyRequestOptionByCode() {
		...
		$optionMock->expects ( $this->once () )->method ( 'setItem' )->with ( $this->model )->will ( $this->returnValue ( $optionMock ) );
		$optionMock->expects ( $this->exactly ( 3 ) )->method ( 'getCode' )->will ( $this->returnValue ( $optionCode ) );
		$optionMock->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( serialize ( [ 
				'qty' => $buyRequestQuantity 
		] ) ) );
	...
	public function testSetHasErrorFalse() {
		$this->errorInfos->expects ( $this->once () )->method ( 'clear' );
	...
	public function testSetHasErrorTrue() {
		$this->errorInfos->expects ( $this->once () )->method ( 'addItem' )->with ( null, null, null, null );
	...
	public function testAddErrorInfo() {
		...
		$this->errorInfos->expects ( $this->once () )->method ( 'addItem' )->with ( $origin, $code, $message, $additionalData );
	...
	public function testRemoveErrorInfosByParams() {
		...
		$this->errorInfos->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( true ) );
