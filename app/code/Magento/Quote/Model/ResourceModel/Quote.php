...
class Quote extends AbstractDb {
	...
	public function loadByCustomerId($quote, $customerId) {
		...
		$select = $this->_getLoadSelect ( 'customer_id', $customerId, $quote )->where ( 'is_active = ?', 1 )->order ( 'updated_at ' . \Magento\Framework\DB\Select::SQL_DESC )->limit ( 1 );
	...
	public function markQuotesRecollectOnCatalogRules() {
		...
		], [ ] )->where ( 't2.product_id = t3.product_id' )->group ( 'quote_id' );
		...
		$updateQuery = $select->crossUpdateFromSelect ( [ 
				't1' => $tableQuote 
		] );
	...
	public function substractProductFromQuotes($product) {
		...
		$subSelect->from ( false, [ 
				'items_qty' => new \Zend_Db_Expr ( $connection->quoteIdentifier ( 'q.items_qty' ) . ' - ' . $connection->quoteIdentifier ( 'qi.qty' ) ),
		...
		$updateQuery = $connection->updateFromSelect ( $subSelect, [ 
	...
	public function markQuotesRecollect($productIds) {
		...
		$subSelect = $this->getConnection ()->select ()->from ( $tableItem, [ 
				'entity_id' => 'quote_id' 
		] )->where ( 'product_id IN ( ? )', $productIds )->group ( 'quote_id' );
