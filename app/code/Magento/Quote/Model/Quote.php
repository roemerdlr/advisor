...
class Quote extends AbstractExtensibleModel implements \Magento\Quote\Api\Data\CartInterface {
	...
	public function getCustomerTaxClassId() {
		/**
		 * tax class can vary at any time.
		 * so instead of using the value from session,
	...
	public function getShippingAddressByCustomerAddressId($addressId) {
		...
		foreach ( $this->getAddressesCollection () as $address ) {
			if (! $address->isDeleted () && $address->getAddressType () == Address::TYPE_SHIPPING && $address->getCustomerAddressId () == $addressId) {
				return $address;
			}
		}
		return false;
	 ...
	 *
	 * @param bool $useCache        	
	 * @return \Magento\Eav\Model\Entity\Collection\AbstractCollection @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	...
	/**
	 * Delete quote item.
	 * If it does not have identifier then it will be only removed from collection
	 *
	...
	public function removeItem($itemId) {
		...
		if ($item) {
			...
			$this->_eventManager->dispatch ( 'sales_quote_remove_item', [ 
					'quote_item' => $item 
			] );
		}
	...
	public function addItem(\Magento\Quote\Model\Quote\Item $item) {
		...
		if (! $item->getId ()) {
			...
			$this->_eventManager->dispatch ( 'sales_quote_add_item', [ 
					'quote_item' => $item 
			] );
		}
		return $this;
	...
	public function addProduct(\Magento\Catalog\Model\Product $product, $request = null, $processMode = \Magento\Catalog\Model\Product\Type\AbstractType::PROCESS_MODE_FULL) {
		...
		$this->_eventManager->dispatch ( 'sales_quote_product_add_after', [ 
				'items' => $items 
		] );
	 ...
	 * @param int $qty        	
	 * @return \Magento\Quote\Model\Quote\Item @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	 ...
	 * @throws \Magento\Framework\Exception\LocalizedException
	 *
	 * @see \Magento\Catalog\Helper\Product::addParamsToBuyRequest() @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	 ...
	 *
	 * @param string $type
	 *        	An internal error type ('error', 'qty', etc.), passed then to adding messages routine
	 * @param string|null $origin
	 *        	Usually a name of module, that embeds error
	 * @param int|null $code
	 *        	Error code, unique for origin, that sets it
	 * @param string|null $message
	 *        	Error message
	 * @param \Magento\Framework\DataObject|null $additionalData
	 *        	Any additional data, that caller would like to store
	 * @return $this
	 */
	...
	public function validateMinimumAmount($multishipping = false) {
		...
		$minOrderActive = $this->_scopeConfig->isSetFlag ( 'sales/minimum_order/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
		if (! $minOrderActive) {
			return true;
		}
		$minOrderMulti = $this->_scopeConfig->isSetFlag ( 'sales/minimum_order/multi_address', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
		$minAmount = $this->_scopeConfig->getValue ( 'sales/minimum_order/amount', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
		$taxInclude = $this->_scopeConfig->getValue ( 'sales/minimum_order/tax_including', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
	...
	public function merge(Quote $quote) {
		$this->_eventManager->dispatch ( $this->_eventPrefix . '_merge_before', [ 
		...
		$this->_eventManager->dispatch ( $this->_eventPrefix . '_merge_after', [ 
				$this->_eventObject => $this,
				'source' => $quote 
		] );
	 ...
	 *
	 * @param boolean $originalMethod
	 *        	if true return defined method from beginning
	 * @return string
	 */
