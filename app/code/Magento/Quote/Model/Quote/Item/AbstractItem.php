...
abstract class AbstractItem extends \Magento\Framework\Model\AbstractExtensibleModel implements \Magento\Catalog\Model\Product\Configuration\Item\ItemInterface {
	...
	public function getProduct() {
		...
		if ($product === null && $this->getProductId ()) {
			$product = clone $this->productRepository->getById ( $this->getProductId (), false, $this->getQuote ()->getStoreId () );
	...
	/**
	 * Adds message(s) for quote item.
	 * Duplicated messages are not added.
	 *
	 ...
	 *
	 * @param bool $string
	 *        	flag for converting messages to string
	 * @return array|string
	 */
	...
	public function checkData() {
		...
		try {
			...
			$this->getQuote ()->setHasError ( true )->addMessage ( __ ( 'Some of the products below do not have all the required options.' ) );
		} catch ( \Exception $e ) {
		...
		if ($this->getHasConfigurationUnavailableError ()) {
			$this->setHasError ( true )->setMessage ( __ ( 'Selected option(s) or their combination is not currently available.' ) );
			$this->getQuote ()->setHasError ( true )->addMessage ( __ ( 'Some item options or their combination are not currently available.' ), 'unavailable-configuration' );
	...
	/**
	 * Get item price.
	 * Item price currency is website base currency.
	 *
	 * @return float
	 */
	...
	public function isShipSeparately() {
		...
		if (null !== $shipmentType && ( int ) $shipmentType === \Magento\Catalog\Model\Product\Type\AbstractType::SHIPMENT_SEPARATELY) {
			return true;
		}
		return false;
	}
	 ...
	 * If there are no children, returns the discount
	 * amount of this item.
	 *
	 * @return float
	 */
