...
class QuoteManagement implements \Magento\Quote\Api\CartManagementInterface {
	...
	public function placeOrder($cartId, PaymentInterface $paymentMethod = null) {
		...
		$this->eventManager->dispatch ( 'checkout_submit_before', [ 
				'quote' => $quote 
		] );
	...
	protected function resolveItems(QuoteEntity $quote) {
		...
		foreach ( $quoteItems as $quoteItem ) {
			...
			$orderItems [$quoteItem->getId ()] = $this->quoteItemToOrderItem->convert ( $quoteItem, [ 
	...
	protected function submitQuote(QuoteEntity $quote, $orderData = []) {
		...
		if ($quote->isVirtual ()) {
			...
			$shippingAddress = $this->quoteAddressToOrderAddress->convert ( $quote->getShippingAddress (), [ 
