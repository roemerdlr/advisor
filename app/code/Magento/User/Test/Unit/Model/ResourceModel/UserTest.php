...
class UserTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->userMock = $this->getMockBuilder ( 'Magento\User\Model\User' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		...
		$this->aclCacheMock = $this->getMockBuilder ( 'Magento\Framework\Acl\CacheInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$this->roleFactoryMock = $this->getMockBuilder ( 'Magento\Authorization\Model\RoleFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$this->roleMock = $this->getMockBuilder ( 'Magento\Authorization\Model\Role' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$this->dateTimeMock = $this->getMockBuilder ( 'Magento\Framework\Stdlib\DateTime' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$this->selectMock = $this->getMockBuilder ( 'Magento\Framework\DB\Select' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$this->dbAdapterMock = $this->getMockBuilder ( 'Magento\Framework\DB\Adapter\AdapterInterface' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		
		$helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->model = $helper->getObject ( 'Magento\User\Model\ResourceModel\User', [ 
	...
	public function testIsUserUnique() {
		...
		$this->dbAdapterMock->expects ( $this->once () )->method ( 'fetchRow' )->willReturn ( [ 
				true 
		] );
	...
	public function testHasAssigned2RolePassAnObject() {
		$methodUserMock = $this->getMockBuilder ( '\Magento\Framework\Model\AbstractModel' )->disableOriginalConstructor ()->setMethods ( [ 
				'getUserId' 
		] )->getMock ();
		$returnData = [ 
				1,
	...
	public function testGetRolesReturnFilledArray() {
		...
		$this->dbAdapterMock->expects ( $this->once () )->method ( 'fetchCol' )->willReturn ( [ 
				1,
				2,
				3 
		] );
		$this->assertEquals ( [ 
				...
				2,
				3 
		], $this->model->getRoles ( $this->userMock ) );
	}
	...
	public function testSaveExtraEmptyId() {
		...
		$this->assertInstanceOf ( 'Magento\User\Model\ResourceModel\User', $this->model->saveExtra ( $this->userMock, [ 
				1,
				2,
				3 
		] ) );
	...
	public function testInitUniqueFields() {
		$this->assertInstanceOf ( '\Magento\User\Model\ResourceModel\User', $this->invokeMethod ( $this->model, '_initUniqueFields', [ ] ) );
	}
	public function testAfterSave() {
		$roleId = 123;
		$methodUserMock = $this->getMockBuilder ( '\Magento\User\Model\User' )->disableOriginalConstructor ()->setMethods ( [ 
				'hasRoleId',
				'getRoleId' 
		] )->getMock ();
		...
		$this->dbAdapterMock->expects ( $this->once () )->method ( 'describeTable' )->willReturn ( [ 
				1,
				2,
				3 
		] );
		
		$this->assertInstanceOf ( '\Magento\User\Model\ResourceModel\User', $this->invokeMethod ( $this->model, '_afterSave', [ 
				$methodUserMock 
		] ) );
	}
