...
class EditRole extends \Magento\User\Controller\Adminhtml\User\Role {
	...
	public function execute() {
		...
		$this->_view->getPage ()->getConfig ()->getTitle ()->prepend ( $role->getId () ? $role->getRoleName () : __ ( 'New Role' ) );
		
		$this->_addBreadcrumb ( $breadCrumb, $breadCrumbTitle );
		
		$this->_view->getLayout ()->getBlock ( 'adminhtml.user.role.buttons' )->setRoleId ( $role->getId () )->setRoleInfo ( $role );
