...
class User extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\Acl\CacheInterface $aclCache, \Magento\Authorization\Model\RoleFactory $roleFactory, \Magento\Framework\Stdlib\DateTime $dateTime, $connectionName = null) {
	...
	public function getRoles(\Magento\Framework\Model\AbstractModel $user) {
		...
		$select = $connection->select ()->from ( $table, [ ] )->joinLeft ( [ 
				'ar' => $table 
	...
	public function getValidationRulesBeforeSave() {
		$userIdentity = new \Zend_Validate_Callback ( [ 
				$this,
				'isUserUnique' 
		] );
	...
	public function unlock($userIds) {
		...
		return $this->getConnection ()->update ( $this->getMainTable (), [ 
				'failures_num' => 0,
				'first_failure' => null,
				'lock_expires' => null 
	...
	public function updateFailure($user, $setLockExpires = false, $setFirstFailure = false) {
		$update = [ 
				'failures_num' => new \Zend_Db_Expr ( 'failures_num + 1' ) 
		];
		...
		$this->getConnection ()->update ( $this->getMainTable (), $update, $this->getConnection ()->quoteInto ( "{$this->getIdFieldName()} = ?", $user->getId () ) );
	}
	...
	public function getOldPasswords($user, $retainLimit = 4) {
		...
		$retainPasswordIds = $this->getConnection ()->fetchCol ( $this->getConnection ()->select ()->from ( $table, 'password_id' )->where ( 'user_id = :user_id' )->order ( 'expires ' . \Magento\Framework\DB\Select::SQL_DESC )->order ( 'password_id ' . \Magento\Framework\DB\Select::SQL_DESC )->limit ( $retainLimit ), [ 
				':user_id' => $userId 
		] );
		$where = [ 
				'user_id = ?' => $userId,
		...
		return $this->getConnection ()->fetchCol ( $this->getConnection ()->select ()->from ( $table, 'password_hash' )->where ( 'user_id = :user_id' ), [ 
				':user_id' => $userId 
		] );
	}
	...
	public function trackPassword($user, $passwordHash, $lifetime) {
		$now = time ();
		$this->getConnection ()->insert ( $this->getTable ( 'admin_passwords' ), [ 
				'user_id' => $user->getId (),
	...
	public function getLatestPassword($userId) {
		return $this->getConnection ()->fetchRow ( $this->getConnection ()->select ()->from ( $this->getTable ( 'admin_passwords' ) )->where ( 'user_id = :user_id' )->order ( 'password_id ' . \Magento\Framework\DB\Select::SQL_DESC )->limit ( 1 ), [ 
				':user_id' => $userId 
		] );
	}
}
