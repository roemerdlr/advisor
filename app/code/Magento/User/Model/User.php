...
class User extends AbstractModel implements StorageInterface, UserInterface {
	...
	public function sendPasswordResetConfirmationEmail() {
		...
		$transport = $this->_transportBuilder->setTemplateIdentifier ( $templateId )->setTemplateModel ( 'Magento\Email\Model\BackendTemplate' )->setTemplateOptions ( [ 
				'area' => FrontNameResolver::AREA_CODE,
				'store' => Store::DEFAULT_STORE_ID 
		] )->setTemplateVars ( [ 
				'user' => $this,
				'store' => $this->_storeManager->getStore ( Store::DEFAULT_STORE_ID ) 
		] )->setFrom ( $this->_config->getValue ( self::XML_PATH_FORGOT_EMAIL_IDENTITY ) )->addTo ( $this->getEmail (), $this->getName () )->getTransport ();
	...
	public function authenticate($username, $password) {
		...
		try {
			$this->_eventManager->dispatch ( 'admin_user_authenticate_before', [ 
					'username' => $username,
					'user' => $this 
			] );
