...
class ItemTest extends \PHPUnit_Framework_TestCase {
	...
	public function configure($amountOption, $withoutOptions = false) {
		...
		$product->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( null ) );
		$product->expects ( $this->once () )->method ( 'getName' )->will ( $this->returnValue ( 'testProductName' ) );
		
		$this->item = $this->getMock ( 'Magento\Wishlist\Model\Item', [ 
				'getProduct' 
		], [ ], '', false );
		$this->item->expects ( $this->atLeastOnce () )->method ( 'getProduct' )->will ( $this->returnValue ( $product ) );
		
		$productConfig = $this->getMock ( 'Magento\Catalog\Helper\Product\Configuration', null, [ ], '', false );
		$productConfigPool = $this->getMock ( 'Magento\Catalog\Helper\Product\ConfigurationPool', [ 
				'get' 
		], [ ], '', false );
		
		$helper = $this->getMock ( 'Magento\Bundle\Helper\Catalog\Product\Configuration', [ 
				'getOptions' 
		], [ ], '', false );
		...
		if ($withoutOptions) {
			$helper->expects ( $this->once () )->method ( 'getOptions' )->will ( $this->returnValue ( null ) );
			$escaper->expects ( $this->once () )->method ( 'escapeHtml' )->with ( 'testProductName' )->will ( $this->returnValue ( 'testProductName' ) );
		} else {
			$helper->expects ( $this->once () )->method ( 'getOptions' )->will ( $this->returnValue ( $options ) );
			
			$escaper->expects ( $this->at ( 0 ) )->method ( 'escapeHtml' )->with ( 'testProductName' )->will ( $this->returnValue ( 'testProductName' ) );
			for($i = 1; $i <= count ( $options ); $i ++) {
				$escaper->expects ( $this->at ( $i ) )->method ( 'escapeHtml' )->will ( $this->returnValue ( "testLabel{$i}" ) );
			}
		}
		
		$context = $this->getMock ( 'Magento\Backend\Block\Context', [ 
				'getEscaper' 
		], [ ], '', false );
		$context->expects ( $this->once () )->method ( 'getEscaper' )->will ( $this->returnValue ( $escaper ) );
		
		$productConfigPool->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Catalog\Helper\Product\Configuration' )->will ( $this->returnValue ( $helper ) );
