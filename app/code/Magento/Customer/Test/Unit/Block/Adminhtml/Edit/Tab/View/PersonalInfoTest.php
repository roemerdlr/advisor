...
class PersonalInfoTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$customerDataFactory = $this->getMock ( 'Magento\Customer\Api\Data\CustomerInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$customerDataFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $customer );
		
		$backendSession = $this->getMock ( 'Magento\Backend\Model\Session', [ 
				'getCustomerData' 
		], [ ], '', false );
		$backendSession->expects ( $this->any () )->method ( 'getCustomerData' )->willReturn ( [ 
				'account' => [ ] 
		] );
		
		$this->customerLog = $this->getMock ( 'Magento\Customer\Model\Log', [ 
				'getLastLoginAt',
				'getLastVisitAt',
				'getLastLogoutAt' 
		], [ ], '', false );
		...
		$customerLogger = $this->getMock ( 'Magento\Customer\Model\Logger', [ 
				'get' 
		], [ ], '', false );
		...
		$this->block = $objectManagerHelper->getObject ( 'Magento\Customer\Block\Adminhtml\Edit\Tab\View\PersonalInfo', [ 
	...
	public function testGetStoreLastLoginDateTimezone() {
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( $this->pathToDefaultTimezone, \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->willReturn ( $this->defaultTimezone );
	 ...
	 * @return void @dataProvider getCurrentStatusDataProvider
	 */
	public function testGetCurrentStatus($status, $lastLoginAt, $lastVisitAt, $lastLogoutAt) {
		$this->scopeConfig->expects ( $this->any () )->method ( 'getValue' )->with ( 'customer/online_customers/online_minutes_interval', \Magento\Store\Model\ScopeInterface::SCOPE_STORE )->willReturn ( 240 ); // TODO: it's value mocked because unit tests run data providers before all testsuite
