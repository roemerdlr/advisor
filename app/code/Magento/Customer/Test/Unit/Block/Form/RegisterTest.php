...
class RegisterTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_customerSession = $this->getMock ( 'Magento\Customer\Model\Session', [ 
				'getCustomerFormData' 
		], [ ], '', false );
	...
	public function getConfigProvider() {
		return [ 
				...
				[ 
						'/path/to/config/value/that/does/not/exist',
						null 
				] 
		];
	}
	...
	public function testGetBackUrlNullData() {
		$this->_customerUrl->expects ( $this->once () )->method ( 'getLoginUrl' )->will ( $this->returnValue ( self::LOGIN_URL ) );
	...
	/**
	 * Form data has not been set on the block and there is no customer data in the customer session.
	 * So
	...
	public function testGetFormDataCustomerFormDataRegionId() {
		...
		$customerFormData = [ 
				self::REGION_ID_ATTRIBUTE_CODE => self::REGION_ID_ATTRIBUTE_VALUE 
		];
		$this->_customerSession->expects ( $this->once () )->method ( 'getCustomerFormData' )->will ( $this->returnValue ( $customerFormData ) );
	...
	public function testGetCountryIdParentNullData() {
		$this->directoryHelperMock->expects ( $this->once () )->method ( 'getDefaultCountry' )->will ( $this->returnValue ( self::COUNTRY_ID ) );
		$this->assertEquals ( self::COUNTRY_ID, $this->_block->getCountryId () );
	}
	...
	public function testIsNewsletterEnabled($isNewsletterEnabled, $expectedValue) {
		$this->_moduleManager->expects ( $this->once () )->method ( 'isOutputEnabled' )->with ( 'Magento_Newsletter' )->will ( $this->returnValue ( $isNewsletterEnabled ) );
