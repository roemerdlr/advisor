...
class DobTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *        	Determines whether the 'dob' attribute is visible or enabled
	 * @param bool $expectedValue
	 *        	The value we expect from Dob::isEnabled()
	 *        	
	 ...
	 *        	Determines whether the 'dob' attribute is required
	 * @param bool $expectedValue
	 *        	The value we expect from Dob::isRequired()
	 *        	
	 ...
	 *        	Date (e.g. '01/01/2020' or false for no date)
	 * @param int|bool $expectedTime
	 *        	The value we expect from Dob::getTime()
	 * @param string|bool $expectedDate
	 *        	The value we expect from Dob::getData('date')
	 *        	
	 ...
	 *
	 * @param string|bool $date
	 *        	The date (e.g. '01/01/2020' or false for no date)
	 * @param string $expectedDay
	 *        	The value we expect from Dob::getDay()
	 *        	
	 ...
	 *        	The value we expect from Dob::getMonth()
	 *        	
	 ...
	 *        	The value we expect from Dob::getYear()
	 *        	
	...
	/**
	 * This tests the Dob::setDateInput() method.
	 * The Dob::getSortedDateInputs() uses the value of
	...
	public function testGetSortedDateInputsWithoutStrippingNonInputChars() {
		...
		$this->assertEquals ( self::MONTH_HTML . '/' . self::DAY_HTML . '/' . self::YEAR_HTML, $this->_block->getSortedDateInputs ( false ) );
	}
	
	/**
	 *
	 * @param array $validationRules
	 *        	The date Min/Max validation rules
	 * @param int $expectedValue
	 *        	The value we expect from Dob::getMinDateRange()
	 *        	
	...
	public function getMinDateRangeDataProvider() {
		$emptyValidationRule = $this->getMockBuilder ( 'Magento\Customer\Api\Data\ValidationRuleInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getName',
				'getValue' 
		] )->getMockForAbstractClass ();
		
		$validationRule = $this->getMockBuilder ( 'Magento\Customer\Api\Data\ValidationRuleInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getName',
				'getValue' 
		] )->getMockForAbstractClass ();
		$validationRule->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( Dob::MIN_DATE_RANGE_KEY ) );
		$validationRule->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( strtotime ( self::MIN_DATE ) ) );
	 ...
	 *        	The date Min/Max validation rules
	 * @param int $expectedValue
	 *        	The value we expect from Dob::getMaxDateRange()
	 *        	
	...
	public function getMaxDateRangeDataProvider() {
		...
		$validationRule->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( Dob::MAX_DATE_RANGE_KEY ) );
		$validationRule->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( strtotime ( self::MAX_DATE ) ) );
		return [ 
				[ 
						[ 
								$validationRule 
