...
class NameTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_block = new \Magento\Customer\Block\Widget\Name ( $context, $addressHelper, $this->customerMetadata, $this->_options, $this->addressMetadata );
	}
	...
	public function testShowPrefix() {
		$this->_setUpShowAttribute ( [ 
				\Magento\Customer\Model\Data\Customer::PREFIX => self::PREFIX 
		] );
	...
	public function testShowPrefixWithException() {
		$this->customerMetadata->expects ( $this->any () )->method ( 'getAttributeMetadata' )->will ( $this->throwException ( new NoSuchEntityException ( __ ( NoSuchEntityException::MESSAGE_SINGLE_FIELD, [ 
				'fieldName' => 'field',
				'fieldValue' => 'value' 
		] ) ) ) );
		$this->assertFalse ( $this->_block->showPrefix () );
	...
	public function testShowMiddlename() {
		$this->_setUpShowAttribute ( [ 
				\Magento\Customer\Model\Data\Customer::MIDDLENAME => self::MIDDLENAME 
		] );
	...
	public function testShowSuffix() {
		$this->_setUpShowAttribute ( [ 
				\Magento\Customer\Model\Data\Customer::SUFFIX => self::SUFFIX 
		] );
	...
	public function testGetPrefixOptionsNotEmpty() {
		/**
		 * Added some padding so that the trim() call on Customer::getPrefix() will remove it.
		 * Also added
		...
		$this->_options->expects ( $this->once () )->method ( 'getNamePrefixOptions' )->will ( $this->returnValue ( $prefixOptions ) );
	...
	public function testGetSuffixOptionsNotEmpty() {
		...
		$this->_options->expects ( $this->once () )->method ( 'getNameSuffixOptions' )->will ( $this->returnValue ( $suffixOptions ) );
	...
	/**
	 *
	 * @param bool $isPrefixVisible
	 *        	Value returned by Name::showPrefix()
	 * @param bool $isMiddlenameVisible
	 *        	Value returned by Name::showMiddlename()
	 * @param bool $isSuffixVisible
	 *        	Value returned by Name::showSuffix()
	 * @param string $expectedValue
	 *        	The expected value of Name::getContainerClassName()
	 *        	
	...
	public function testGetContainerClassName($isPrefixVisible, $isMiddlenameVisible, $isSuffixVisible, $expectedValue) {
		$this->attribute->expects ( $this->at ( 0 ) )->method ( 'isVisible' )->will ( $this->returnValue ( $isPrefixVisible ) );
		$this->attribute->expects ( $this->at ( 1 ) )->method ( 'isVisible' )->will ( $this->returnValue ( $isMiddlenameVisible ) );
		$this->attribute->expects ( $this->at ( 2 ) )->method ( 'isVisible' )->will ( $this->returnValue ( $isSuffixVisible ) );
	...
	public function getContainerClassNameProvider() {
		return [ 
				...
				[ 
						...
						true,
						false,
						self::DEFAULT_CLASS_NAME . self::CONTAINER_CLASS_NAME_MIDDLENAME 
				],
				[ 
						false,
						false,
						true,
						self::DEFAULT_CLASS_NAME . self::CONTAINER_CLASS_NAME_SUFFIX 
				],
				[ 
						true,
						true,
						true,
						self::DEFAULT_CLASS_NAME . self::CONTAINER_CLASS_NAME_PREFIX . self::CONTAINER_CLASS_NAME_MIDDLENAME . self::CONTAINER_CLASS_NAME_SUFFIX 
				] 
		];
	}
	 ...
	 *        	The attribute's store label
	 * @param string $expectedValue
	 *        	The expected value of Name::getStoreLabel()
	 *        	
	...
	/**
	 * This data provider provides two data sets.
	 * One tests that an empty string is returned for an invalid
	...
	public function getStoreLabelProvider() {
		return [ 
				...
				[ 
						self::PREFIX_ATTRIBUTE_CODE,
						self::PREFIX_STORE_LABEL,
						self::PREFIX_STORE_LABEL 
				] 
		];
	}
	...
	private function _setUpShowAttribute(array $data) {
		 ...
		 * Subsequent calls may return true or false depending on the returnValue
