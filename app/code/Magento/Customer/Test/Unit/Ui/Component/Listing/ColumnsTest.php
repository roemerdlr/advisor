...
class ColumnsTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->columnFactory = $this->getMock ( 'Magento\Customer\Ui\Component\ColumnFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testPrepareWithAddColumn() {
		...
		$this->attributeRepository->expects ( $this->atLeastOnce () )->method ( 'getList' )->willReturn ( [ 
				$attributeCode => [ 
	...
	public function testPrepareWithUpdateColumn() {
		...
		$this->columnFactory->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->column );
		$this->column->expects ( $this->once () )->method ( 'prepare' );
		$this->column->expects ( $this->atLeastOnce () )->method ( 'getData' )->with ( 'config' )->willReturn ( [ ] );
		$this->column->expects ( $this->at ( 3 ) )->method ( 'setData' )->with ( 'config', [ 
				'options' => [ 
						[ 
								'label' => 'Label',
								'value' => 'Value' 
		...
		$this->column->expects ( $this->at ( 5 ) )->method ( 'setData' )->with ( 'config', [ 
				'name' => $attributeCode,
	...
	public function testPrepareWithUpdateStaticColumn() {
		...
		$this->column->expects ( $this->at ( 6 ) )->method ( 'setData' )->with ( 'config', [ 
				'editor' => 'text',
				'visible' => true 
		] );
