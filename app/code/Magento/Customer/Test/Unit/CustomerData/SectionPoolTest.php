...
class SectionPoolTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = new SectionPool ( $this->objectManagerMock, $this->identifierMock, $this->sectionSourceMap );
	}
	public function testGetSectionsDataAllSections() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( 'b' )->willReturn ( $sectionSourceMock );
		$sectionSourceMock->expects ( $this->once () )->method ( 'getSectionData' )->willReturn ( $sectionsData );
		
		$this->identifierMock->expects ( $this->once () )->method ( 'markSections' )->
		// check also default value for $updateIds = false
		with ( $allSectionsData, $sectionNames, false )->willReturn ( $identifierResult );
	...
	public function testGetSectionsDataAllSectionsException() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( 'b' )->willReturn ( $this->model );
		$modelResult = $this->model->getSectionsData ( $sectionNames );
		$this->assertEquals ( $identifierResult, $modelResult );
	}
}
