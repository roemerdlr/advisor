...
class ValidateTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->customerDataFactory = $this->getMock ( 'Magento\Customer\Api\Data\CustomerInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->formFactory = $this->getMock ( 'Magento\Customer\Model\Metadata\FormFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->controller = $objectHelper->getObject ( 'Magento\Customer\Controller\Adminhtml\Index\Validate', [ 
	...
	public function testExecute() {
		$this->request->expects ( $this->once () )->method ( 'getPost' )->willReturn ( [ 
				'_template_' => null,
				'address_index' => null 
		] );
		...
		$this->form->expects ( $this->once () )->method ( 'validateData' )->willReturn ( [ 
				$error 
		] );
		...
		$validationResult->expects ( $this->once () )->method ( 'getMessages' )->willReturn ( [ 
				'Error message' 
		] );
	...
	public function testExecuteWithoutAddresses() {
		...
		$error = $this->getMock ( 'Magento\Framework\Message\Error', [ ], [ ], '', false );
		$this->form->expects ( $this->never () )->method ( 'validateData' )->willReturn ( [ 
				$error 
		] );
	...
	public function testExecuteWithException() {
		...
		$exception->expects ( $this->once () )->method ( 'getMessages' )->willReturn ( [ 
				$error 
		] );
