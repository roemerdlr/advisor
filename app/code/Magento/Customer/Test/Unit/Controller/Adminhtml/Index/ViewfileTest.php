...
class ViewfileTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->resultRawFactoryMock = $this->getMock ( 'Magento\Framework\Controller\Result\RawFactory', [ 
				'create' 
		], [ ], '', false );
	}
	...
	/**
	 *
	 * @throws \Magento\Framework\Exception\NotFoundException @expectedException \Magento\Framework\Exception\NotFoundException
	 */
	...
	public function testExecuteParamFile() {
		...
		$fileFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( $path, [ 
				'type' => 'filename',
				'value' => $fileName 
		], \Magento\Framework\App\Filesystem\DirectoryList::MEDIA )->willReturn ( $fileResponse );
	...
	public function testExecuteGetParamImage() {
		...
		$this->resultRawMock->expects ( $this->any () )->method ( 'setHeader' )->willReturnMap ( [ 
				[ 
						'Pragma',
						'public',
						true,
