...
class EditPostTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->customerAccountManagement = $this->getMockBuilder ( 'Magento\Customer\Api\AccountManagementInterface' )->getMockForAbstractClass ();
		...
		$this->model = new EditPost ( $this->context, $this->session, $this->customerAccountManagement, $this->customerRepository, $this->validator, $this->customerExtractor );
	}
	public function testInvalidFormKey() {
		$this->validator->expects ( $this->once () )->method ( 'validate' )->with ( $this->request )->willReturn ( false );
	...
	public function testGeneralSave() {
		...
		$this->request->expects ( $this->once () )->method ( 'getParam' )->with ( 'change_password' )->willReturn ( false );
		...
		$this->messageManager->expects ( $this->once () )->method ( 'getMessages' )->willReturn ( $this->messageCollection );
		$this->messageManager->expects ( $this->once () )->method ( 'addSuccess' )->with ( __ ( 'You saved the account information.' ) )->willReturnSelf ();
		
		$this->messageCollection->expects ( $this->once () )->method ( 'getCount' )->willReturn ( 0 );
	 ...
	 */
	public function testChangePassword($currentPassword, $newPassword, $confirmationPassword, $errors) {
		$customerId = 1;
		...
		$currentCustomerMock = $this->getCurrentCustomerMock ( $address );
		$currentCustomerMock->expects ( $this->once () )->method ( 'getEmail' )->willReturn ( $customerEmail );
		...
		$this->request->expects ( $this->exactly ( 3 ) )->method ( 'getPost' )->willReturnMap ( [ 
				[ 
						'current_password',
						null,
						$currentPassword 
		...
		if ($errors ['counter'] > 0) {
			...
			$this->customerAccountManagement->expects ( $this->once () )->method ( 'changePassword' )->with ( $customerEmail, $currentPassword, $newPassword )->willReturnSelf ();
		...
		$this->messageCollection->expects ( $this->once () )->method ( 'getCount' )->willReturn ( $errors ['counter'] );
	 ...
	 */
	public function testGeneralException($counter, $message, $exception) {
		$customerId = 1;
		...
		$this->session->expects ( $this->once () )->method ( 'getCustomerId' )->willReturn ( $customerId );
		$this->session->expects ( $this->once () )->method ( 'setCustomerFormData' )->with ( true )->willReturnSelf ();
		...
		$this->messageCollection->expects ( $this->once () )->method ( 'getCount' )->willReturn ( $counter );
	...
	protected function prepareContext() {
		...
		$this->resultRedirectFactory = $this->getMockBuilder ( 'Magento\Framework\Controller\Result\RedirectFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->messageCollection = $this->getMockBuilder ( 'Magento\Framework\Message\Collection' )->disableOriginalConstructor ()->getMock ();
	...
	protected function getNewCustomerMock($customerId, $address) {
		$newCustomerMock = $this->getMockBuilder ( 'Magento\Customer\Api\Data\CustomerInterface' )->getMockForAbstractClass ();
		
		$newCustomerMock->expects ( $this->once () )->method ( 'setId' )->with ( $customerId )->willReturnSelf ();
		$newCustomerMock->expects ( $this->once () )->method ( 'getAddresses' )->willReturn ( null );
		$newCustomerMock->expects ( $this->once () )->method ( 'setAddresses' )->with ( [ 
				$address 
		] )->willReturn ( null );
	...
	protected function getCurrentCustomerMock($address) {
		$currentCustomerMock = $this->getMockBuilder ( 'Magento\Customer\Api\Data\CustomerInterface' )->getMockForAbstractClass ();
		
		$currentCustomerMock->expects ( $this->once () )->method ( 'getAddresses' )->willReturn ( [ 
				$address 
		] );
	...
	protected function mockChangePasswordErrors($currentPassword, $newPassword, $errors, $customerEmail) {
		if (! empty ( $errors ['exception'] )) {
			...
			$this->messageManager->expects ( $this->any () )->method ( 'addException' )->with ( $exception, __ ( 'Something went wrong while changing the password.' ) )->willReturnSelf ();
		}
