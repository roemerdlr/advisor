...
class CreatePasswordTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Customer\Controller\Account\CreatePassword', [ 
	...
	public function testExecuteWithLink() {
		...
		$this->sessionMock->expects ( $this->once () )->method ( 'setRpToken' )->with ( $token );
		$this->sessionMock->expects ( $this->once () )->method ( 'setRpCustomerId' )->with ( $customerId );
		...
		$redirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( '*/*/createpassword', [ ] )->willReturnSelf ();
	...
	public function testExecuteWithSession() {
		...
		$blockMock = $this->getMockBuilder ( 'Magento\Customer\Block\Account\Resetpassword' )->disableOriginalConstructor ()->setMethods ( [ 
				'setCustomerId',
