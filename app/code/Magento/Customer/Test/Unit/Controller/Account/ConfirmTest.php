...
class ConfirmTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$resultFactoryMock = $this->getMock ( 'Magento\Framework\Controller\ResultFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = $objectManagerHelper->getObject ( 'Magento\Customer\Controller\Account\Confirm', [ 
	...
	public function testNoCustomerIdInRequest($customerId, $key) {
		...
		$this->urlMock->expects ( $this->once () )->method ( 'getUrl' )->with ( $this->equalTo ( '*/*/index' ), [ 
				'_secure' => true 
	...
	public function getSuccessMessageDataProvider() {
		return [ 
				...
				[ 
						...
						__ ( 'enter your billing address for proper VAT calculation' ) 
				],
				[ 
						1,
						1,
						...
						__ ( 'enter your shipping address for proper VAT calculation' ) 
				] 
		];
	}
	...
	public function testSuccessRedirect($customerId, $key, $backUrl, $successUrl, $resultUrl, $isSetFlag, $successMessage) {
		...
		$this->urlMock->expects ( $this->any () )->method ( 'getUrl' )->with ( $this->equalTo ( '*/*/index' ), [ 
				'_secure' => true 
