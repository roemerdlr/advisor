...
class ResetPasswordPostTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->sessionMock = $this->getMockBuilder ( 'Magento\Customer\Model\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'unsRpToken',
				'unsRpCustomerId' 
		] )->getMock ();
		$this->pageFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Result\PageFactory' )->disableOriginalConstructor ()->getMock ();
		$this->accountManagementMock = $this->getMockBuilder ( 'Magento\Customer\Api\AccountManagementInterface' )->getMockForAbstractClass ();
		...
		$this->redirectFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Controller\Result\RedirectFactory' )->disableOriginalConstructor ()->getMock ();
		$this->messageManagerMock = $this->getMockBuilder ( 'Magento\Framework\Message\ManagerInterface' )->getMockForAbstractClass ();
		
		$this->objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $this->objectManagerHelper->getObject ( 'Magento\Customer\Controller\Account\ResetPasswordPost', [ 
	...
	public function testExecute() {
		...
		$this->requestMock->expects ( $this->exactly ( 2 ) )->method ( 'getQuery' )->willReturnMap ( [ 
				[ 
						'token',
						$token 
				],
		...
		$this->accountManagementMock->expects ( $this->once () )->method ( 'resetPassword' )->with ( $email, $token, $password )->willReturn ( true );
		
		$this->sessionMock->expects ( $this->once () )->method ( 'unsRpToken' );
		$this->sessionMock->expects ( $this->once () )->method ( 'unsRpCustomerId' );
		
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addSuccess' )->with ( __ ( 'You updated your password.' ) )->willReturnSelf ();
		...
		$redirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( '*/*/login', [ ] )->willReturnSelf ();
	...
	public function testExecuteWithException() {
		...
		$redirectMock->expects ( $this->once () )->method ( 'setPath' )->with ( '*/*/createPassword', [ 
				'id' => $customerId,
				'token' => $token 
		] )->willReturnSelf ();
	...
	public function testExecuteWithWrongConfirmation() {
		...
		$this->messageManagerMock->expects ( $this->once () )->method ( 'addError' )->with ( __ ( 'New Password and Confirm New Password values didn\'t match.' ) )->willReturnSelf ();
