...
class CreatePostTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$urlFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->urlMock ) );
		
		$this->customerMock = $this->getMock ( 'Magento\Customer\Api\Data\CustomerInterface', [ ], [ ], '', false );
		$this->customerDetailsMock = $this->getMock ( 'Magento\Customer\Api\Data\CustomerInterface', [ ], [ ], '', false );
		$this->customerDetailsFactoryMock = $this->getMock ( 'Magento\Customer\Api\Data\CustomerInterfaceFactory', [ ], [ ], '', false );
		...
		$subscriberFactoryMock = $this->getMock ( 'Magento\Newsletter\Model\SubscriberFactory', [ 
				'create' 
		], [ ], '', false );
		$subscriberFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->subscriberMock ) );
		
		$regionFactoryMock = $this->getMock ( 'Magento\Customer\Api\Data\RegionInterfaceFactory', [ ], [ ], '', false );
		$addressFactoryMock = $this->getMock ( 'Magento\Customer\Api\Data\AddressInterfaceFactory', [ ], [ ], '', false );
		...
		$this->resultRedirectFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Controller\Result\RedirectFactory' )->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$contextMock->expects ( $this->any () )->method ( 'getRedirect' )->willReturn ( $this->redirectMock );
		...
		$this->model = $objectManager->getObject ( 'Magento\Customer\Controller\Account\CreatePost', [ 
				'context' => $contextMock,
	...
	public function testRegenerateIdOnExecution() {
		...
		$this->customerExtractorMock->expects ( $this->once () )->method ( 'extract' )->willReturn ( $this->customerMock );
		$this->accountManagement->expects ( $this->once () )->method ( 'createAccount' )->willReturn ( $this->customerMock );
	 ...
	 * @param
	 *        	$addressType
	 * @param $successMessage @dataProvider
	...
	public function testSuccessRedirect($customerId, $password, $confirmationStatus, $successUrl, $isSetFlag, $successMessage) {
		...
		$this->urlMock->expects ( $this->any () )->method ( 'getUrl' )->willReturnMap ( [ 
				[ 
						'*/*/index',
						[ 
								'_secure' => true 
