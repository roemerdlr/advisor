...
class GridTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->flatScopeResolver = $this->getMock ( 'Magento\Framework\Indexer\ScopeResolver\FlatScopeResolver', [ ], [ ], '', false );
		...
		$this->queryResult = $this->getMockForAbstractClass ( 'Zend_Db_Statement_Interface', [ ], '', false );
		
		$this->observer = new Grid ( $this->resource, $this->indexerRegistry, $this->flatScopeResolver );
	}
	public function testSyncCustomerGrid() {
		...
		$this->resource->expects ( $this->exactly ( 2 ) )->method ( 'getTableName' )->willReturnMap ( [ 
				[ 
						$gridTable 
				],
				[ 
		...
		$this->queryResult->expects ( $this->once () )->method ( 'fetchAll' )->willReturn ( [ 
				[ 
						'customer_id' => 23 
				],
				[ 
		...
		$this->indexer->expects ( $this->once () )->method ( 'reindexList' )->with ( [ 
				'23',
				'65' 
		] );
