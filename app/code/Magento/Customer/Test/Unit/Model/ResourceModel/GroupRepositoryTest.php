...
class GroupRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->groupRegistry = $this->getMock ( 'Magento\Customer\Model\GroupRegistry', [ ], [ ], '', false );
		$this->groupFactory = $this->getMock ( 'Magento\Customer\Model\GroupFactory', [ 
				'create' 
		], [ ], '', false );
		$this->groupModel = $this->getMock ( 'Magento\Customer\Model\Group', [ 
				'getTaxClassId',
				'getTaxClassName',
				'getId',
				'getCode',
		...
		$this->groupDataFactory = $this->getMock ( 'Magento\Customer\Api\Data\GroupInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->group = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\GroupInterface', [ ], 'group', false );
		
		$this->groupResourceModel = $this->getMock ( 'Magento\Customer\Model\ResourceModel\Group', [ ], [ ], '', false );
		$this->dataObjectProcessor = $this->getMock ( 'Magento\Framework\Reflection\DataObjectProcessor', [ ], [ ], '', false );
		$this->searchResultsFactory = $this->getMock ( 'Magento\Customer\Api\Data\GroupSearchResultsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->searchResults = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\GroupSearchResultsInterface', [ ], '', false );
		$this->taxClassRepository = $this->getMockForAbstractClass ( 'Magento\Tax\Api\TaxClassRepositoryInterface', [ ], '', false );
		...
		$this->model = new \Magento\Customer\Model\ResourceModel\GroupRepository ( $this->groupRegistry, $this->groupFactory, $this->groupDataFactory, $this->groupResourceModel, $this->dataObjectProcessor, $this->searchResultsFactory, $this->taxClassRepository, $this->extensionAttributesJoinProcessor );
	}
	public function testSave() {
		...
		$this->dataObjectProcessor->expects ( $this->once () )->method ( 'buildOutputDataArray' )->with ( $this->group, '\Magento\Customer\Api\Data\GroupInterface' )->willReturn ( [ 
	...
	public function testGetList() {
		...
		$this->extensionAttributesJoinProcessor->expects ( $this->once () )->method ( 'extractExtensionAttributes' )->with ( 'Magento\Customer\Api\Data\GroupInterface', [ ] )->willReturn ( [ 
				'extension_attributes' => $groupExtension 
		] );
