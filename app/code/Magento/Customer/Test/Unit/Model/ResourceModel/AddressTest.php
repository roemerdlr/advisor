...
class AddressTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->entityRelationCompositeMock = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\VersionControl\RelationComposite', [ ], [ ], '', false );
		
		$this->addressResource = (new ObjectManagerHelper ( $this ))->getObject ( 'Magento\Customer\Model\ResourceModel\Address', [ 
	...
	public function testSave($addressId, $isDefaultBilling, $isDefaultShipping) {
		...
		$address = $this->getMock ( 'Magento\Customer\Model\Address', [ 
				'__wakeup',
				'getId',
				'getEntityTypeId',
				'getIsDefaultBilling',
	...
	protected function prepareResource() {
		...
		$dbAdapter = $this->getMockBuilder ( 'Magento\Framework\DB\Adapter\Pdo\Mysql' )->disableOriginalConstructor ()->getMock ();
		
		$dbAdapter->expects ( $this->any () )->method ( 'describeTable' )->with ( 'customer_address_entity' )->willReturn ( [ 
				'entity_type_id',
				'attribute_set_id',
				'created_at',
				'updated_at',
	...
	protected function prepareEavConfig() {
		$attributeMock = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute', [ 
				'getAttributeCode',
				'getBackend',
				'__wakeup' 
		], [ ], '', false );
		...
		$this->eavConfigType = $this->getMock ( 'Magento\Eav\Model\Entity\Type', [ 
				'getEntityIdField',
				'getId',
				'getEntityTable',
				'__wakeup' 
		...
		$eavConfig->expects ( $this->any () )->method ( 'getEntityAttributeCodes' )->with ( $this->eavConfigType )->willReturn ( [ 
				'entity_type_id',
				'attribute_set_id',
				'created_at',
				'updated_at',
		...
		$eavConfig->expects ( $this->any () )->method ( 'getAttribute' )->willReturnMap ( [ 
				[ 
						$this->eavConfigType,
						'entity_type_id',
						$attributeMock 
	...
	protected function prepareValidatorFactory() {
		$validatorMock = $this->getMock ( 'Magento\Framework\Validator', [ 
				'isValid' 
		], [ ], '', false );
		$validatorMock->expects ( $this->any () )->method ( 'isValid' )->willReturn ( true );
		
		$validatorFactory = $this->getMock ( 'Magento\Framework\Validator\Factory', [ 
				'createValidator' 
		], [ ], '', false );
