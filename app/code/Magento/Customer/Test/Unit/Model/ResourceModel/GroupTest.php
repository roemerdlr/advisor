...
class GroupTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->customersFactory = $this->getMock ( 'Magento\Customer\Model\ResourceModel\Customer\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->groupManagement = $this->getMock ( 'Magento\Customer\Api\GroupManagementInterface', [ 
				'getDefaultGroup',
				'getNotLoggedInGroup',
				'isReadOnly',
				'getLoggedInGroups',
		...
		$transactionManagerMock->expects ( $this->once () )->method ( 'start' )->willReturn ( $this->getMock ( '\Magento\Framework\DB\Adapter\AdapterInterface' ) );
		$contextMock->expects ( $this->once () )->method ( 'getTransactionManager' )->willReturn ( $transactionManagerMock );
		$contextMock->expects ( $this->once () )->method ( 'getObjectRelationProcessor' )->willReturn ( $this->relationProcessorMock );
		
		$this->groupResourceModel = (new ObjectManagerHelper ( $this ))->getObject ( 'Magento\Customer\Model\ResourceModel\Group', [ 
				'context' => $contextMock,
	...
	public function testDelete() {
		...
		$customer = $this->getMock ( 'Magento\Customer\Model\Customer', [ 
				'__wakeup',
				'load',
				'getId',
				'getStoreId',
		...
		$defaultCustomerGroup = $this->getMock ( 'Magento\Customer\Model\Group', [ 
				'getId' 
		], [ ], '', false );
		...
		$customerCollection->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( [ 
				$customer 
		] ) );
