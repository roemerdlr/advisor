...
class CustomerRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->customerFactory = $this->getMock ( 'Magento\Customer\Model\CustomerFactory', [ 
				'create' 
		], [ ], '', false );
		$this->customerSecureFactory = $this->getMock ( 'Magento\Customer\Model\Data\CustomerSecureFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->addressRepository = $this->getMock ( 'Magento\Customer\Model\ResourceModel\AddressRepository', [ ], [ ], '', false );
		
		$this->customerMetadata = $this->getMockForAbstractClass ( 'Magento\Customer\Api\CustomerMetadataInterface', [ ], '', false );
		$this->searchResultsFactory = $this->getMock ( 'Magento\Customer\Api\Data\CustomerSearchResultsInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->imageProcessor = $this->getMockForAbstractClass ( 'Magento\Framework\Api\ImageProcessorInterface', [ ], '', false );
		$this->extensionAttributesJoinProcessor = $this->getMockForAbstractClass ( 'Magento\Framework\Api\ExtensionAttribute\JoinProcessorInterface', [ ], '', false );
		$this->customer = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\CustomerInterface', [ ], '', false );
		
		$this->model = new \Magento\Customer\Model\ResourceModel\CustomerRepository ( $this->customerFactory, $this->customerSecureFactory, $this->customerRegistry, $this->addressRepository, $this->customerResourceModel, $this->customerMetadata, $this->searchResultsFactory, $this->eventManager, $this->storeManager, $this->extensibleDataObjectConverter, $this->dataObjectHelper, $this->imageProcessor, $this->extensionAttributesJoinProcessor );
	}
	...
	protected function prepareMocksForValidation($isValid = false) {
		$attributeMetaData = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\AttributeMetadataInterface', [ ], '', false );
		$attributeMetaData->expects ( $this->atLeastOnce () )->method ( 'isRequired' )->willReturn ( true );
		$this->customerMetadata->expects ( $this->atLeastOnce () )->method ( 'getAttributeMetadata' )->willReturn ( $attributeMetaData );
		
		$this->customer->expects ( $this->once () )->method ( 'getFirstname' )->willReturn ( $isValid ? 'Firstname' : false );
		$this->customer->expects ( $this->once () )->method ( 'getLastname' )->willReturn ( $isValid ? 'Lastname' : false );
		$this->customer->expects ( $this->atLeastOnce () )->method ( 'getEmail' )->willReturn ( $isValid ? 'example@example.com' : false );
		$this->customer->expects ( $this->once () )->method ( 'getDob' )->willReturn ( $isValid ? '12/12/2015' : false );
		$this->customer->expects ( $this->atLeastOnce () )->method ( 'getTaxvat' )->willReturn ( $isValid ? 'taxvat' : false );
		$this->customer->expects ( $this->once () )->method ( 'getGender' )->willReturn ( $isValid ? 'gender' : false );
	}
	...
	public function testSave() {
		...
		$address = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\AddressInterface', [ ], '', false, false, true, [ 
				'setCustomerId',
				'setRegion',
				'getRegion',
				'getId' 
		] );
		$address2 = $this->getMockForAbstractClass ( 'Magento\Customer\Api\Data\AddressInterface', [ ], '', false, false, true, [ 
				'setCustomerId',
				'setRegion',
				'getRegion',
				'getId' 
		] );
		$customerModel = $this->getMock ( 'Magento\Customer\Model\Customer', [ 
				'getId',
				'setId',
				'setStoreId',
				'getStoreId',
		...
		$customerAttributesMetaData = $this->getMockForAbstractClass ( 'Magento\Framework\Api\CustomAttributesDataInterface', [ ], '', false, false, true, [ 
				'getId',
				'getEmail',
				'getWebsiteId',
				'getAddresses',
		...
		$customerSecureData = $this->getMock ( 'Magento\Customer\Model\Data\CustomerSecure', [ 
				'getRpToken',
				'getRpTokenCreatedAt',
				'getPasswordHash' 
		], [ ], '', false );
		...
		$customerAttributesMetaData->expects ( $this->atLeastOnce () )->method ( 'getAddresses' )->willReturn ( [ 
				$address 
		] );
		$customerAttributesMetaData->expects ( $this->at ( 1 ) )->method ( 'setAddresses' )->with ( [ ] );
		$customerAttributesMetaData->expects ( $this->at ( 2 ) )->method ( 'setAddresses' )->with ( [ 
				$address 
		] );
		$this->extensibleDataObjectConverter->expects ( $this->once () )->method ( 'toNestedArray' )->with ( $customerAttributesMetaData, [ ], '\Magento\Customer\Api\Data\CustomerInterface' )->willReturn ( [ 
				'customerData' 
		] );
		...
		$customerModel->expects ( $this->once () )->method ( 'setId' )->with ( $customerId );
		...
		$customerAttributesMetaData->expects ( $this->atLeastOnce () )->method ( 'getId' )->willReturn ( $customerId );
		$this->customerRegistry->expects ( $this->once () )->method ( 'retrieveSecureData' )->with ( $customerId )->willReturn ( $customerSecureData );
		$customerSecureData->expects ( $this->once () )->method ( 'getRpToken' )->willReturn ( 'rpToken' );
		$customerSecureData->expects ( $this->once () )->method ( 'getRpTokenCreatedAt' )->willReturn ( 'rpTokenCreatedAt' );
		$customerSecureData->expects ( $this->once () )->method ( 'getPasswordHash' )->willReturn ( 'passwordHash' );
		
		$customerModel->expects ( $this->exactly ( 2 ) )->method ( 'setRpToken' )->willReturnMap ( [ 
				[ 
						'rpToken',
						$customerModel 
				],
		...
		$customerModel->expects ( $this->exactly ( 2 ) )->method ( 'setRpTokenCreatedAt' )->willReturnMap ( [ 
				[ 
						'rpTokenCreatedAt',
						$customerModel 
				],
		...
		$this->customer->expects ( $this->once () )->method ( 'getAddresses' )->willReturn ( [ 
				$address,
				$address2 
		] );
		...
		$this->eventManager->expects ( $this->once () )->method ( 'dispatch' )->with ( 'customer_save_after_data_object', [ 
				'customer_data_object' => $this->customer,
				'orig_customer_data_object' => $customerAttributesMetaData 
		] );
	...
	public function testSaveWithPasswordHash() {
		...
		$address->expects ( $this->atLeastOnce () )->method ( 'getId' )->willReturn ( 7 );
		$address->expects ( $this->once () )->method ( 'setRegion' )->with ( $region );
		$customerAttributesMetaData->expects ( $this->any () )->method ( 'getAddresses' )->willReturn ( [ 
				$address 
		] );
		$customerAttributesMetaData->expects ( $this->at ( 1 ) )->method ( 'setAddresses' )->with ( [ ] );
		$customerAttributesMetaData->expects ( $this->at ( 2 ) )->method ( 'setAddresses' )->with ( [ 
		...
		$customerModel->expects ( $this->once () )->method ( 'save' );
		$this->customerRegistry->expects ( $this->once () )->method ( 'push' )->with ( $customerModel );
		$this->customer->expects ( $this->any () )->method ( 'getAddresses' )->willReturn ( [ 
				$address,
				$address2 
		] );
		$this->addressRepository->expects ( $this->once () )->method ( 'save' )->with ( $address );
	...
	public function testGetList() {
		...
		$filterGroup->expects ( $this->once () )->method ( 'getFilters' )->willReturn ( [ 
				$filter 
		] );
		...
		$searchCriteria->expects ( $this->atLeastOnce () )->method ( 'getSortOrders' )->willReturn ( [ 
				$sortOrder 
		] );
