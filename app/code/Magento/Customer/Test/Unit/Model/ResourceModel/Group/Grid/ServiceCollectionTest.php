...
class ServiceCollectionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->searchCriteriaBuilder = $this->objectManager->getObject ( 'Magento\Framework\Api\SearchCriteriaBuilder', [ 
		...
		$this->searchResults = $this->getMockForAbstractClass ( 'Magento\Framework\Api\SearchResultsInterface', [ 
				'getTotalCount',
				'getItems' 
		] );
		...
		$this->serviceCollection = $this->objectManager->getObject ( 'Magento\Customer\Model\ResourceModel\Group\Grid\ServiceCollection', [ 
	...
	public function testGetSearchCriteriaImplicitEq() {
		...
		$expectedSearchCriteria = $this->searchCriteriaBuilder->setCurrentPage ( 1 )->setPageSize ( false )->addSortOrder ( $sortOrder )->addFilters ( [ 
	...
	public function testGetSearchCriteriaOneField() {
		...
		$expectedSearchCriteria = $this->searchCriteriaBuilder->setCurrentPage ( 1 )->setPageSize ( 0 )->addSortOrder ( $sortOrder )->addFilters ( [ 
				$filter 
		] )->create ();
		...
		// Now call service collection to load the data. This causes it to create the search criteria Data Object
		$this->serviceCollection->addFieldToFilter ( $field, [ 
				$conditionType => $value 
		] );
		$this->serviceCollection->setOrder ( 'name', ServiceCollection::SORT_ORDER_ASC );
		$this->serviceCollection->loadData ();
	...
	public function testGetSearchCriteriaAnd() {
		...
		// Now call service collection to load the data. This causes it to create the search criteria Data Object
		$this->serviceCollection->addFieldToFilter ( $fieldA, [ 
				'gt' => $value 
		] );
		$this->serviceCollection->addFieldToFilter ( $fieldB, [ 
				'gt' => $value 
		] );
		$this->serviceCollection->setOrder ( 'name', ServiceCollection::SORT_ORDER_ASC );
		$this->serviceCollection->loadData ();
