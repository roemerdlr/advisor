...
class StoreTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$storeManager = $this->storeManager = $this->getMockBuilder ( 'Magento\Store\Model\StoreManagerInterface' )->getMock ();
	...
	public function testBeforeSaveWithId() {
		$object = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId' 
		] )->getMock ();
		...
		$this->assertInstanceOf ( 'Magento\Customer\Model\Customer\Attribute\Backend\Store', $this->testable->beforeSave ( $object ) );
	}
	public function testBeforeSave() {
		...
		$store = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getId',
				'getName' 
		] )->getMock ();
		...
		$this->storeManager->expects ( $this->exactly ( 2 ) )->method ( 'getStore' )->will ( $this->returnValue ( $store ) );
		...
		$object->expects ( $this->once () )->method ( 'setData' )->with ( $this->logicalOr ( 'created_in', $storeName ) )->will ( $this->returnSelf () );
		/** @var \Magento\Framework\DataObject $object */
