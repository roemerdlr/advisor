...
class RegionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRender($regionCollection) {
		$countryFactoryMock = $this->getMock ( 'Magento\Directory\Model\CountryFactory', [ 
				'create' 
		], [ ], '', false );
		$directoryHelperMock = $this->getMock ( 'Magento\Directory\Helper\Data', [ 
				'isRegionRequired' 
		], [ ], '', false );
		$escaperMock = $this->getMock ( 'Magento\Framework\Escaper', [ ], [ ], '', false );
		$elementMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ 
				'getForm',
				'getHtmlAttributes' 
		], [ ], '', false );
		$countryMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ 
				'getValue' 
		], [ ], '', false );
		$regionMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ ], [ ], '', false );
		$countryModelMock = $this->getMock ( 'Magento\Directory\Model\Country', [ 
				'setId',
				'getLoadedRegionCollection',
				'toOptionArray',
				'__wakeup' 
		], [ ], '', false );
		$formMock = $this->getMock ( 'Magento\Framework\Data\Form', [ 
				'getElement' 
		], [ ], '', false );
		
		$elementMock->expects ( $this->any () )->method ( 'getForm' )->will ( $this->returnValue ( $formMock ) );
		$elementMock->expects ( $this->any () )->method ( 'getHtmlAttributes' )->will ( $this->returnValue ( [ 
				'title',
				'class',
				'style',
				'onclick',
		...
		$formMock->expects ( $this->any () )->method ( 'getElement' )->will ( $this->returnValueMap ( [ 
				[ 
						'country_id',
						$countryMock 
				],
		...
		$directoryHelperMock->expects ( $this->any () )->method ( 'isRegionRequired' )->will ( $this->returnValueMap ( [ 
				[ 
						'GE',
						true 
				] 
