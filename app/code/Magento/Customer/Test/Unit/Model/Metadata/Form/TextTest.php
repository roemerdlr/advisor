...
class TextTest extends AbstractFormTestCase {
	...
	protected function getClass($value) {
		return new \Magento\Customer\Model\Metadata\Form\Text ( $this->localeMock, $this->loggerMock, $this->attributeMetadataMock, $this->localeResolverMock, $value, 0, false, $this->stringHelper );
	}
	...
	public function testValidateValueLength($value, $expected) {
		$minTextLengthRule = $this->getMockBuilder ( 'Magento\Customer\Api\Data\ValidationRuleInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getName',
				'getValue' 
		] )->getMockForAbstractClass ();
		$minTextLengthRule->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( 'min_text_length' ) );
		$minTextLengthRule->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( 4 ) );
		
		$maxTextLengthRule = $this->getMockBuilder ( 'Magento\Customer\Api\Data\ValidationRuleInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getName',
				'getValue' 
		] )->getMockForAbstractClass ();
		$maxTextLengthRule->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( 'max_text_length' ) );
		$maxTextLengthRule->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( 8 ) );
