...
class AbstractDataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_localeMock = $this->getMockBuilder ( 'Magento\Framework\Stdlib\DateTime\TimezoneInterface' )->disableOriginalConstructor ()->getMock ();
		...
		$this->_model = new ExtendsAbstractData ( $this->_localeMock, $this->_loggerMock, $this->_attributeMock, $this->_localeResolverMock, $this->_value, $this->_entityTypeCode, $this->_isAjax );
	}
	...
	public function testDateFilterFormat($format, $output) {
		...
		if ($format === null) {
			$this->_localeMock->expects ( $this->once () )->method ( 'getDateFormat' )->with ( $this->equalTo ( \IntlDateFormatter::SHORT ) )->will ( $this->returnValue ( $output ) );
		}
	...
	public function testValidateInputRule($value, $label, $inputValidation, $expectedOutput) {
		...
		$this->_attributeMock->expects ( $this->any () )->method ( 'getValidationRules' )->will ( $this->returnValue ( [ 
				$validationRule 
		] ) );
	...
	public function testGetIsAjaxRequest($ajaxRequest) {
		$this->_model = new ExtendsAbstractData ( $this->_localeMock, $this->_loggerMock, $this->_attributeMock, $this->_localeResolverMock, $this->_value, $this->_entityTypeCode, $ajaxRequest );
	...
	public function testGetRequestValue($request, $attributeCode, $requestScope, $requestScopeOnly, $expectedValue) {
		$this->_attributeMock->expects ( $this->once () )->method ( 'getAttributeCode' )->will ( $this->returnValue ( $attributeCode ) );
	...
	public function getRequestValueDataProvider() {
		...
		$requestMockOne->expects ( $this->any () )->method ( 'getParam' )->with ( 'ATTR_CODE' )->will ( $this->returnValue ( $expectedValue ) );
		
		$requestMockTwo = $this->getMockBuilder ( '\Magento\Framework\App\RequestInterface' )->getMock ();
		$requestMockTwo->expects ( $this->at ( 0 ) )->method ( 'getParam' )->with ( 'REQUEST_SCOPE' )->will ( $this->returnValue ( [ 
				'ATTR_CODE' => $expectedValue 
		] ) );
		$requestMockTwo->expects ( $this->at ( 1 ) )->method ( 'getParam' )->with ( 'REQUEST_SCOPE' )->will ( $this->returnValue ( [ ] ) );
		
		$requestMockThree = $this->getMockBuilder ( '\Magento\Framework\App\Request\Http' )->disableOriginalConstructor ()->getMock ();
		$requestMockThree->expects ( $this->once () )->method ( 'getParams' )->will ( $this->returnValue ( [ 
				'REQUEST' => [ 
						'SCOPE' => [ 
								'ATTR_CODE' => $expectedValue 
						] 
