...
class AddressMetadataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->attributeConverterMock = $this->getMockBuilder ( 'Magento\Customer\Model\AttributeMetadataConverter' )->disableOriginalConstructor ()->getMock ();
		$this->attributeProviderMock = $this->getMockBuilder ( 'Magento\Customer\Model\AttributeMetadataDataProvider' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = new AddressMetadata ( $this->attributeConverterMock, $this->attributeProviderMock );
	}
	public function testGetAttributes() {
		...
		$attributeMock = $this->getMockBuilder ( 'Magento\Customer\Model\Attribute' )->disableOriginalConstructor ()->getMock ();
		$attributes = [ 
				$attributeMock 
		];
		
		/** @var Collection|\PHPUnit_Framework_MockObject_MockObject $collectionMock */
		$collectionMock = $this->getMockBuilder ( 'Magento\Customer\Model\ResourceModel\Form\Attribute\Collection' )->disableOriginalConstructor ()->getMock ();
		
		$this->attributeProviderMock->expects ( $this->once () )->method ( 'loadAttributesCollection' )->with ( AddressMetadataInterface::ENTITY_TYPE_ADDRESS, $formCode )->willReturn ( $collectionMock );
		
		$collectionMock->expects ( $this->once () )->method ( 'getIterator' )->willReturn ( new \ArrayIterator ( $attributes ) );
	...
	public function testGetAttributeMetadata() {
		...
		$attributeMock = $this->getMockBuilder ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId' 
	...
	public function testGetAttributeMetadataWithoutAttribute() {
		...
		$this->attributeProviderMock->expects ( $this->once () )->method ( 'getAttribute' )->with ( AddressMetadataInterface::ENTITY_TYPE_ADDRESS, $attributeCode )->willReturn ( null );
	...
	public function testGetCustomAttributesMetadata() {
		...
		$metadataMock->expects ( $this->once () )->method ( 'isSystem' )->willReturn ( false );
	...
	public function testGetCustomAttributesMetadataWithSystemAttribute() {
		...
		$metadataMock->expects ( $this->once () )->method ( 'getAttributeCode' )->willReturn ( $attributeCode );
		$metadataMock->expects ( $this->once () )->method ( 'isSystem' )->willReturn ( true );
