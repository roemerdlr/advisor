...
class FileTest extends AbstractFormTestCase {
	...
	protected function setUp() {
		parent::setUp ();
		$this->urlEncode = $this->getMockBuilder ( 'Magento\Framework\Url\EncoderInterface' )->disableOriginalConstructor ()->getMock ();
	...
	public function testExtractValueNoRequestScope($expected, $attributeCode = '', $isAjax = false, $delete = '') {
		...
		$this->requestMock->expects ( $this->any () )->method ( 'getParam' )->will ( $this->returnValue ( [ 
				'delete' => $delete 
		] ) );
		
		$this->attributeMetadataMock->expects ( $this->any () )->method ( 'getAttributeCode' )->will ( $this->returnValue ( $attributeCode ) );
	...
	public function testExtractValueWithRequestScope($expected, $requestScope, $mainScope = false) {
		...
		$this->requestMock->expects ( $this->any () )->method ( 'getParams' )->will ( $this->returnValue ( [ 
				'delete' => true 
		] ) );
	...
	public function testValidateValueNotToUpload($expected, $value, $isAjax = false, $isRequired = true) {
		$fileForm = $this->getClass ( $value, $isAjax );
		$this->attributeMetadataMock->expects ( $this->any () )->method ( 'isRequired' )->will ( $this->returnValue ( $isRequired ) );
		$this->attributeMetadataMock->expects ( $this->any () )->method ( 'getStoreLabel' )->will ( $this->returnValue ( 'attributeLabel' ) );
	...
	public function validateValueNotToUploadDataProvider() {
		return [ 
				...
				'null' => [ 
						[ 
								'"attributeLabel" is a required value.' 
						],
						null 
				] 
		];
	...
	public function testValidateValueToUpload($expected, $value, $parameters = []) {
		...
		$this->attributeMetadataMock->expects ( $this->any () )->method ( 'getStoreLabel' )->will ( $this->returnValue ( 'File Input Field Label' ) );
		
		$this->fileValidatorMock->expects ( $this->any () )->method ( 'getMessages' )->will ( $this->returnValue ( [ 
				'Validation error message.' 
		] ) );
		$this->fileValidatorMock->expects ( $this->any () )->method ( 'isValid' )->will ( $this->returnValue ( $parameters ['valid'] ) );
		$this->assertEquals ( $expected, $fileForm->validateValue ( $value ) );
	}
	...
	public function testCompactValueDelete() {
		...
		$mediaDirMock->expects ( $this->once () )->method ( 'delete' )->with ( self::ENTITY_TYPE . 'value' );
		...
		$this->assertSame ( '', $fileForm->compactValue ( [ 
				'delete' => true 
		] ) );
	}
	public function testCompactValueTmpFile() {
		...
		$this->uploaderFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'fileId' => $value 
	...
	public function testOutputValueJson() {
		...
		$urlKey = 'url_key';
		$fileForm = $this->getClass ( $value, false );
		$this->urlEncode->expects ( $this->once () )->method ( 'encode' )->with ( $this->equalTo ( $value ) )->will ( $this->returnValue ( $urlKey ) );
		$expected = [ 
				'value' => $value,
				'url_key' => $urlKey 
		];
	 ...
	 *        	The value undergoing testing by a given test
	 * @param bool $isAjax        	
