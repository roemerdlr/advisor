...
class MultiselectTest extends AbstractFormTestCase {
	...
	protected function getClass($value) {
		return new \Magento\Customer\Model\Metadata\Form\Multiselect ( $this->localeMock, $this->loggerMock, $this->attributeMetadataMock, $this->localeResolverMock, $value, 0 );
	}
	 ...
	 *        	to assign to boolean
	 * @param bool $expected
	 *        	text output
	 *        	
	 * @return void @dataProvider extractValueDataProvider
	 */
	public function testExtractValue($value, $expected) {
		/** @var \PHPUnit_Framework_MockObject_MockObject | Multiselect $multiselect */
		$multiselect = $this->getMockBuilder ( 'Magento\Customer\Model\Metadata\Form\Multiselect' )->disableOriginalConstructor ()->setMethods ( [ 
				'_getRequestValue' 
		] )->getMock ();
	 ...
	 *        	text output
	 *        	
	 * @return void @dataProvider compactValueDataProvider
	 */
	 ...
	 *
	 * @return void @dataProvider outputValueTextDataProvider
	 */
	 ...
	 * @param string[] $expected        	
	 *
	 * @return void @dataProvider outputValueJsonDataProvider
	 */
	...
	protected function runOutputValueTest($value, $expected, $format) {
		$option1 = $this->getMockBuilder ( 'Magento\Customer\Api\Data\OptionInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getLabel',
				'getValue' 
		] )->getMockForAbstractClass ();
		$option1->expects ( $this->any () )->method ( 'getLabel' )->will ( $this->returnValue ( 'fourteen' ) );
		$option1->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( '14' ) );
		
		$option2 = $this->getMockBuilder ( 'Magento\Customer\Api\Data\OptionInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getLabel',
				'getValue' 
		] )->getMockForAbstractClass ();
		$option2->expects ( $this->any () )->method ( 'getLabel' )->will ( $this->returnValue ( 'some string' ) );
		$option2->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValue ( 'some key' ) );
		
		$this->attributeMetadataMock->expects ( $this->any () )->method ( 'getOptions' )->will ( $this->returnValue ( [ 
				$option1,
				$option2 
		] ) );
		$multiselect = $this->getClass ( $value );
