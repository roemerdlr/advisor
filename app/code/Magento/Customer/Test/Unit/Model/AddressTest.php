...
class AddressTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->customer = $this->getMockBuilder ( 'Magento\Customer\Model\Customer' )->disableOriginalConstructor ()->getMock ();
		$this->customer->expects ( $this->any () )->method ( 'getId' )->will ( $this->returnValue ( self::ORIG_CUSTOMER_ID ) );
		$this->customer->expects ( $this->any () )->method ( 'load' )->will ( $this->returnSelf () );
		
		$this->customerFactory = $this->getMockBuilder ( 'Magento\Customer\Model\CustomerFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->customerFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->customer ) );
		
		$this->resource = $this->getMockBuilder ( 'Magento\Customer\Model\ResourceModel\Address' )->disableOriginalConstructor ()->getMock ();
		
		$this->address = $this->objectManager->getObject ( 'Magento\Customer\Model\Address', [ 
