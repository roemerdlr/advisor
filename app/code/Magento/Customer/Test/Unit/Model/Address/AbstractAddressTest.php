...
class AbstractAddressTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$regionCollectionMock = $this->getMock ( 'Magento\Directory\Model\ResourceModel\Region\Collection', [ ], [ ], '', false );
		...
		$this->model = $objectManager->getObject ( 'Magento\Customer\Model\Address\AbstractAddress', [ 
	...
	protected function prepareGetRegion($countryId, $regionName = 'RegionName') {
		$region = $this->getMock ( 'Magento\Directory\Model\Region', [ 
				'getCountryId',
				'getName',
				'__wakeup',
				'load' 
		], [ ], '', false );
		$region->expects ( $this->once () )->method ( 'getName' )->will ( $this->returnValue ( $regionName ) );
	...
	public function validateDataProvider() {
		...
		return [ 
				'firstname' => [ 
						array_merge ( array_diff_key ( $data, [ 
								'firstname' => '' 
						] ), [ 
								'country_id' => $countryId ++ 
						] ),
