...
class AccountManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->accountManagement = $this->objectManagerHelper->getObject ( 'Magento\Customer\Model\AccountManagement', [ 
	...
	public function testCreateAccountWithPasswordHashWithExistingCustomer() {
		...
		$website->expects ( $this->once () )->method ( 'getStoreIds' )->willReturn ( [ 
				1,
				2,
				3 
		] );
	...
	public function testCreateAccountWithPasswordHashWithCustomerWithoutStoreId() {
		...
		$customer->expects ( $this->once () )->method ( 'getAddresses' )->willReturn ( [ 
				$address 
		] );
		...
		$this->customerRepository->expects ( $this->once () )->method ( 'save' )->with ( $customer, $hash )->willThrowException ( $exception );
	...
	public function testCreateAccountWithoutPassword() {
		...
		$store->expects ( $this->once () )->method ( 'getId' )->willReturn ( $defaultStoreId );
		$website = $this->getMockBuilder ( 'Magento\Store\Model\Website' )->disableOriginalConstructor ()->getMock ();
		$website->expects ( $this->atLeastOnce () )->method ( 'getStoreIds' )->willReturn ( [ 
				1,
				2,
				3 
		] );
		...
		$customerSecure->expects ( $this->any () )->method ( 'getPasswordHash' )->willReturn ( null );
		...
		$this->scopeConfig->expects ( $this->at ( 1 ) )->method ( 'getValue' )->with ( AccountManagement::XML_PATH_REGISTER_NO_PASSWORD_EMAIL_TEMPLATE, ScopeInterface::SCOPE_STORE, $defaultStoreId )->willReturn ( $templateIdentifier );
	...
	protected function prepareEmailSend($email, $templateIdentifier, $sender, $storeId, $customerName) {
		...
		$this->transportBuilder->expects ( $this->any () )->method ( 'setTemplateOptions' )->with ( [ 
				'area' => Area::AREA_FRONTEND,
				'store' => $storeId 
		] )->willReturnSelf ();
		$this->transportBuilder->expects ( $this->any () )->method ( 'setTemplateVars' )->with ( [ 
				'customer' => $this->customerSecure,
				'store' => $this->store 
		] )->willReturnSelf ();
	...
	public function testInitiatePasswordResetEmailReset() {
		...
		$this->scopeConfig->expects ( $this->at ( 0 ) )->method ( 'getValue' )->with ( AccountManagement::XML_PATH_FORGOT_EMAIL_TEMPLATE, ScopeInterface::SCOPE_STORE, $storeId )->willReturn ( $templateIdentifier );
		$this->scopeConfig->expects ( $this->at ( 1 ) )->method ( 'getValue' )->with ( AccountManagement::XML_PATH_FORGOT_EMAIL_IDENTITY, ScopeInterface::SCOPE_STORE, $storeId )->willReturn ( $sender );
	...
	private function reInitModel() {
		$this->customerSecure = $this->getMockBuilder ( 'Magento\Customer\Model\Data\CustomerSecure' )->disableOriginalConstructor ()->setMethods ( [ 
				'getRpToken',
				'getRpTokenCreatedAt' 
		] )->getMock ();
		
		$this->customerSecure->expects ( $this->any () )->method ( 'getRpToken' )->willReturn ( 'newStringToken' );
		
		$date = date ( 'Y-m-d', strtotime ( '-1 year' ) );
		
		$this->customerSecure->expects ( $this->any () )->method ( 'getRpTokenCreatedAt' )->willReturn ( $date );
		
		$this->customer = $this->getMockBuilder ( 'Magento\Customer\Model\Customer' )->disableOriginalConstructor ()->setMethods ( [ 
