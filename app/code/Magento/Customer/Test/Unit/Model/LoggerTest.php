...
class LoggerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->logFactory = $this->getMock ( '\Magento\Customer\Model\LogFactory', [ 
				'create' 
		], [ ], '', false );
		
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		
		$this->logger = $objectManagerHelper->getObject ( '\Magento\Customer\Model\Logger', [ 
	...
	public function testLog($customerId, $data) {
		...
		$this->connection->expects ( $this->once () )->method ( 'insertOnDuplicate' )->with ( $tableName, array_merge ( [ 
	...
	public function testGet($customerId, $data) {
		...
		$this->resource->expects ( $this->once () )->method ( 'getConnection' )->willReturn ( $this->connection );
		$this->connection->expects ( $this->any () )->method ( 'fetchRow' )->with ( $select )->willReturn ( $data );
