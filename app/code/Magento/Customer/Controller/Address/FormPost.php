...
class FormPost extends \Magento\Customer\Controller\Address {
	...
	protected function _extractAddress() {
		...
		$this->dataObjectHelper->populateWithArray ( $addressDataObject, array_merge ( $existingAddressData, $attributeValues ), '\Magento\Customer\Api\Data\AddressInterface' );
		$addressDataObject->setCustomerId ( $this->_getSession ()->getCustomerId () )->setIsDefaultBilling ( $this->getRequest ()->getParam ( 'default_billing', false ) )->setIsDefaultShipping ( $this->getRequest ()->getParam ( 'default_shipping', false ) );
	...
	protected function getExistingAddressData() {
		...
		if ($addressId = $this->getRequest ()->getParam ( 'id' )) {
			...
			$existingAddressData = $this->_dataProcessor->buildOutputDataArray ( $existingAddress, '\Magento\Customer\Api\Data\AddressInterface' );
	...
	public function execute() {
		...
		if (! $this->getRequest ()->isPost ()) {
			...
			return $this->resultRedirectFactory->create ()->setUrl ( $this->_redirect->error ( $this->_buildUrl ( '*/*/edit' ) ) );
		}
