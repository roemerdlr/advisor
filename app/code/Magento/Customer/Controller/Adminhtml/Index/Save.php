...
class Save extends \Magento\Customer\Controller\Adminhtml\Index {
	...
	protected function _extractCustomerData() {
		...
		if ($this->getRequest ()->getPost ( 'customer' )) {
			...
			$customerData = $this->_extractData ( $this->getRequest (), 'adminhtml_customer', \Magento\Customer\Api\CustomerMetadataInterface::ENTITY_TYPE_CUSTOMER, $serviceAttributes, 'customer' );
		}
		
		if (isset ( $customerData ['disable_auto_group_change'] )) {
			$customerData ['disable_auto_group_change'] = ( int ) filter_var ( $customerData ['disable_auto_group_change'], FILTER_VALIDATE_BOOLEAN );
		}
	 ...
	 *        	The code of EAV form to take the list of attributes from
	 * @param string $entityType
	 *        	entity type for the form
	 * @param string[] $additionalAttributes
	 *        	The list of attribute codes to skip filtration for
	 * @param string $scope
	...
	protected function saveDefaultFlags(array $addressIdList, array & $extractedCustomerData) {
		...
		foreach ( $addressIdList as $addressId ) {
			...
			$addressData = $this->_extractData ( $this->getRequest (), 'adminhtml_customer_address', \Magento\Customer\Api\AddressMetadataInterface::ENTITY_TYPE_ADDRESS, [ 
					'default_billing',
					'default_shipping' 
			], $scope );
	...
	public function execute() {
		...
		$customerId = isset ( $originalRequestData ['customer'] ['entity_id'] ) ? $originalRequestData ['customer'] ['entity_id'] : null;
