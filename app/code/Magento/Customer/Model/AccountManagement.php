...
class AccountManagement implements AccountManagementInterface {
	...
	public function resendConfirmation($email, $websiteId = null, $redirectUrl = '') {
		...
		try {
			$this->sendNewAccountEmail ( $customer, self::NEW_ACCOUNT_EMAIL_CONFIRMATION, $redirectUrl, $this->storeManager->getStore ()->getId () );
	...
	public function authenticate($username, $password) {
		...
		$this->eventManager->dispatch ( 'customer_customer_authenticated', [ 
				'model' => $customerModel,
				'password' => $password 
		] );
		...
		try {
			switch ($template) {
				...
				case AccountManagement::EMAIL_RESET :
					...
					break;
				default :
					throw new InputException ( __ ( InputException::INVALID_FIELD_VALUE, [ 
							'value' => $template,
	 ...
	 * @throws LocalizedException
	 */
	protected function sendNewAccountEmail($customer, $type = self::NEW_ACCOUNT_EMAIL_REGISTERED, $backUrl = '', $storeId = '0', $sendemailStoreId = null) {
		...
		$this->sendEmailTemplate ( $customer, $types [$type], self::XML_PATH_REGISTER_EMAIL_IDENTITY, [ 
	...
	protected function sendPasswordResetNotificationEmail($customer) {
		...
		$this->sendEmailTemplate ( $customer, self::XML_PATH_RESET_PASSWORD_TEMPLATE, self::XML_PATH_FORGOT_EMAIL_IDENTITY, [ 
				'customer' => $customerEmailData,
	...
	protected function sendEmailTemplate($customer, $template, $sender, $templateParams = [], $storeId = null) {
		...
		$transport = $this->transportBuilder->setTemplateIdentifier ( $templateId )->setTemplateOptions ( [ 
				...
				'store' => $storeId 
		] )->setTemplateVars ( $templateParams )->setFrom ( $this->scopeConfig->getValue ( $sender, ScopeInterface::SCOPE_STORE, $storeId ) )->addTo ( $customer->getEmail (), $this->customerViewHelper->getCustomerName ( $customer ) )->getTransport ();
	...
	public function sendPasswordReminderEmail($customer) {
		...
		$this->sendEmailTemplate ( $customer, self::XML_PATH_REMIND_EMAIL_TEMPLATE, self::XML_PATH_FORGOT_EMAIL_IDENTITY, [ 
				'customer' => $customerEmailData,
				'store' => $this->storeManager->getStore ( $storeId ) 
		], $storeId );
	...
	public function sendPasswordResetConfirmationEmail($customer) {
		...
		$this->sendEmailTemplate ( $customer, self::XML_PATH_FORGOT_EMAIL_TEMPLATE, self::XML_PATH_FORGOT_EMAIL_IDENTITY, [ 
				'customer' => $customerEmailData,
				'store' => $this->storeManager->getStore ( $storeId ) 
		], $storeId );
