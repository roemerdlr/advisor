...
class Logger {
	 ...
	 */
	public function __construct(ResourceConnection $resource, \Magento\Customer\Model\LogFactory $logFactory) {
	...
	public function log($customerId, array $data) {
		...
		$connection->insertOnDuplicate ( $this->resource->getTableName ( 'customer_log' ), array_merge ( [ 
	...
	protected function loadLogData($customerId) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'last_visit_at' 
		] )->where ( 'cl.customer_id = ?', $customerId )->order ( 'cv.visitor_id DESC' )->limit ( 1 );
