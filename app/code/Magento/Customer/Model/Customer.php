...
class Customer extends \Magento\Framework\Model\AbstractModel {
	...
	public function updateData($customer) {
		$customerDataAttributes = $this->dataObjectProcessor->buildOutputDataArray ( $customer, '\Magento\Customer\Api\Data\CustomerInterface' );
	 ...
	 * @return bool
	 * @throws \Magento\Framework\Exception\LocalizedException Use \Magento\Customer\Api\AccountManagementInterface::authenticate
	 */
	public function authenticate($login, $password) {
		...
		$this->_eventManager->dispatch ( 'customer_customer_authenticated', [ 
				'model' => $this,
				'password' => $password 
		] );
	...
	public function getAddressesCollection() {
		if ($this->_addressesCollection === null) {
			$this->_addressesCollection = $this->getAddressCollection ()->setCustomerFilter ( $this )->addAttributeToSelect ( '*' );
	...
	public function sendNewAccountEmail($type = 'registered', $backUrl = '', $storeId = '0') {
		...
		if (! isset ( $types [$type] )) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Please correct the transactional account email type.' ) );
		}
	...
	public function isConfirmationRequired() {
		...
		return ( bool ) $this->_scopeConfig->getValue ( self::XML_PATH_IS_CONFIRM, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
	}
	 ...
	 *        	configuration path of email template
	 * @param string $sender
	 *        	configuration path of email identity
	...
	protected function _sendEmailTemplate($template, $sender, $templateParams = [], $storeId = null) {
		...
		$transport = $this->_transportBuilder->setTemplateIdentifier ( $this->_scopeConfig->getValue ( $template, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId ) )->setTemplateOptions ( [ 
				'area' => \Magento\Framework\App\Area::AREA_FRONTEND,
				'store' => $storeId 
		] )->setTemplateVars ( $templateParams )->setFrom ( $this->_scopeConfig->getValue ( $sender, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId ) )->addTo ( $this->getEmail (), $this->getName () )->getTransport ();
	...
	public function getGroupId() {
		if (! $this->hasData ( 'group_id' )) {
			...
			$groupId = $this->_scopeConfig->getValue ( GroupManagement::XML_PATH_DEFAULT_ID, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId );
	 ...
	 *
	 * @return bool|string[] @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function validate() {
		...
		$this->_eventManager->dispatch ( 'customer_validate', [ 
				'customer' => $this,
				'transport' => $transport 
		] );
	...
	public function afterSave() {
		...
		if ($indexer->getState ()->getStatus () == StateInterface::STATUS_VALID) {
			$this->_getResource ()->addCommitCallback ( [ 
					$this,
					'reindex' 
			] );
		}
	...
	public function changeResetPasswordLinkToken($passwordLinkToken) {
		if (! is_string ( $passwordLinkToken ) || empty ( $passwordLinkToken )) {
			throw new AuthenticationException ( __ ( 'Please enter a valid password reset token.' ) );
		}
	...
	public function getResetPasswordLinkExpirationPeriod() {
		return ( int ) $this->_scopeConfig->getValue ( self::XML_PATH_CUSTOMER_RESET_PASSWORD_LINK_EXPIRATION_PERIOD, ScopeConfigInterface::SCOPE_TYPE_DEFAULT );
	}
