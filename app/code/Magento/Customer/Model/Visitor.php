...
class Visitor extends \Magento\Framework\Model\AbstractModel {
	...
	public function initByRequest($observer) {
		...
		if (! $this->getId ()) {
			...
			$this->save ();
			$this->_eventManager->dispatch ( 'visitor_init', [ 
					'visitor' => $this 
			] );
	...
	public function saveByRequest($observer) {
		...
		try {
			$this->save ();
			$this->_eventManager->dispatch ( 'visitor_activity_save', [ 
					'visitor' => $this 
			] );
			$this->session->setVisitorData ( $this->getData () );
		} catch ( \Exception $e ) {
	 ...
	 *
	 * @param \Magento\Framework\Event\Observer $observer        	
	 * @return \Magento\Customer\Model\Visitor @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	...
	public function getCleanTime() {
		return $this->scopeConfig->getValue ( \Magento\Framework\Session\Config::XML_PATH_COOKIE_LIFETIME, \Magento\Store\Model\ScopeInterface::SCOPE_STORE ) + 86400;
	}
	...
	public function getOnlineInterval() {
		$configValue = intval ( $this->scopeConfig->getValue ( static::XML_PATH_ONLINE_INTERVAL, \Magento\Store\Model\ScopeInterface::SCOPE_STORE ) );
