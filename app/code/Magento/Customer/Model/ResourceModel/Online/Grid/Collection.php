...
class Collection extends SearchResult {
	 ...
	 */
	public function __construct(EntityFactory $entityFactory, Logger $logger, FetchStrategy $fetchStrategy, EventManager $eventManager, $mainTable, $resourceModel, Visitor $visitorModel, \Magento\Framework\Stdlib\DateTime\DateTime $date) {
		$this->date = $date;
	...
	protected function _initSelect() {
		...
		$this->getSelect ()->joinLeft ( [ 
				...
				'firstname',
				'lastname' 
		] )->where ( 'main_table.last_visit_at >= ?', $connection->formatDate ( $lastDate ) );
	...
	public function addFieldToFilter($field, $condition = null) {
		if ($field == 'visitor_type') {
			...
			if (is_array ( $condition ) && isset ( $condition ['eq'] )) {
				$condition = $condition ['eq'] == Visitor::VISITOR_TYPE_CUSTOMER ? [ 
						'gt' => 0 
				] : [ 
						'null' => true 
				];
