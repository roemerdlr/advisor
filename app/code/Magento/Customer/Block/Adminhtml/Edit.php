...
class Edit extends \Magento\Backend\Block\Widget\Form\Container {
	...
	protected function _construct() {
		...
		if ($customerId) {
			$url = $this->getUrl ( 'customer/index/resetPassword', [ 
					'customer_id' => $customerId 
			] );
			$this->buttonList->add ( 'reset_password', [ 
		...
		if ($customerId) {
			$url = $this->getUrl ( 'customer/customer/invalidateToken', [ 
					'customer_id' => $customerId 
			] );
			$deleteConfirmMsg = __ ( "Are you sure you want to revoke the customer\'s tokens?" );
			$this->buttonList->add ( 'invalidate_token', [ 
	...
	/**
	 * Prepare form Html.
	 * Add block for configurable product modification interface.
	 *
	 * @return string
	 */
	...
	public function getValidationUrl() {
		return $this->getUrl ( 'customer/*/validate', [ 
				'_current' => true 
		] );
	}
