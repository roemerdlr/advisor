...
class Database extends \Magento\MediaStorage\Model\ResourceModel\File\Storage\AbstractStorage {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\DB\Helper $resourceHelper, $connectionName = null) {
	...
	public function createDatabaseScheme() {
		...
		$ddlTable = $connection->newTable ( $table )->addColumn ( 'file_id', \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER, null, [ 
				'identity' => true,
				'unsigned' => true,
				'nullable' => false,
				'primary' => true 
		], 'File Id' )->addColumn ( 'content', \Magento\Framework\DB\Ddl\Table::TYPE_VARBINARY, \Magento\Framework\DB\Ddl\Table::MAX_VARBINARY_SIZE, [ 
				'nullable' => false 
		], 'File Content' )->addColumn ( 'upload_time', \Magento\Framework\DB\Ddl\Table::TYPE_TIMESTAMP, null, [ 
				'nullable' => false,
				'default' => \Magento\Framework\DB\Ddl\Table::TIMESTAMP_INIT 
		], 'Upload Timestamp' )->addColumn ( 'filename', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, 100, [ 
				'nullable' => false 
		], 'Filename' )->addColumn ( 'directory_id', \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER, null, [ 
				'unsigned' => true,
				'default' => null 
		], 'Identifier of Directory where File is Located' )->addColumn ( 'directory', \Magento\Framework\DB\Ddl\Table::TYPE_TEXT, 255, [ 
				'default' => null 
		], 'Directory Path' )->addIndex ( $connection->getIndexName ( $table, [ 
				'filename',
				'directory_id' 
		...
		] )->addIndex ( $connection->getIndexName ( $table, [ 
				'directory_id' 
		] ), [ 
				'directory_id' 
		] )->addForeignKey ( $connection->getForeignKeyName ( $table, 'directory_id', $dirStorageTable, 'directory_id' ), 'directory_id', $dirStorageTable, 'directory_id', \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE )->setComment ( 'File Storage' );
	 ...
	 *
	 * @param array $row
	 *        	Table row with 'content' key in it
	 * @return array
	 */
	 ...
	 *
	 * @param array $rows
	 *        	Array of table rows (files), each containing 'content' key
	 * @return array
	 */
	...
	public function loadByFilename(\Magento\MediaStorage\Model\File\Storage\Database $object, $filename, $path) {
		...
		$select = $connection->select ()->from ( [ 
				'e' => $this->getMainTable () 
		] )->where ( 'filename = ?', $filename )->where ( $connection->prepareSqlCondition ( 'directory', [ 
				'seq' => $path 
		] ) );
	...
	public function getFiles($offset = 0, $count = 100) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'content',
				'directory' 
		] )->order ( 'file_id' )->limit ( $count, $offset );
	...
	public function renameFile($oldFilename, $oldPath, $newFilename, $newPath) {
		...
		$dataWhere [] = new \Zend_Db_Expr ( $connection->prepareSqlCondition ( 'directory', [ 
				'seq' => $oldPath 
		] ) );
	...
	public function copyFile($oldFilename, $oldPath, $newFilename, $newPath) {
		...
		$select = $connection->select ()->from ( [ 
				'e' => $this->getMainTable () 
		] )->where ( 'filename = ?', $oldFilename )->where ( $connection->prepareSqlCondition ( 'directory', [ 
				'seq' => $oldPath 
		] ) );
	...
	public function deleteFolder($folderName = '') {
		...
		$likeExpression = $this->_resourceHelper->addLikeEscape ( $folderName . '/', [ 
				'position' => 'start' 
		] );
	...
	public function deleteFile($filename, $directory) {
		...
		$where [] = new \Zend_Db_Expr ( $connection->prepareSqlCondition ( 'directory', [ 
				'seq' => $directory 
		] ) );
	...
	public function getDirectoryFiles($directory) {
		...
		$select = $connection->select ()->from ( [ 
				...
				'directory',
				'content' 
		] )->where ( $connection->prepareSqlCondition ( 'directory', [ 
				'seq' => $directory 
		] ) )->order ( 'file_id' );
