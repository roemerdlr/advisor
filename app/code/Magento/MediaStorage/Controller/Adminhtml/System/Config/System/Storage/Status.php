...
class Status extends \Magento\MediaStorage\Controller\Adminhtml\System\Config\System\Storage {
	 ...
	 *
	 * @return \Magento\Framework\Controller\Result\Json @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
				...
				case \Magento\MediaStorage\Model\File\Storage\Flag::STATE_RUNNING :
					if (! $flag->getLastUpdate () || time () <= strtotime ( $flag->getLastUpdate () ) + \Magento\MediaStorage\Model\File\Storage\Flag::FLAG_TTL) {
						...
						if (is_array ( $flagData ) && ! (isset ( $flagData ['timeout_reached'] ) && $flagData ['timeout_reached'])) {
							$this->_objectManager->get ( 'Psr\Log\LoggerInterface' )->critical ( new \Magento\Framework\Exception\LocalizedException ( __ ( 'The timeout limit for response from synchronize process was reached.' ) ) );
