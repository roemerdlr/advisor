...
class SynchronizationTest extends \PHPUnit_Framework_TestCase {
	public function testSynchronize() {
		...
		$storageFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\DatabaseFactory', [ 
				'create',
				'_wakeup' 
		], [ ], '', false );
		$storageMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\Database', [ 
				'getContent',
				'getId',
				'loadByFilename',
				'__wakeup' 
		...
		$file = $this->getMock ( 'Magento\Framework\Filesystem\File\Write', [ 
				'lock',
				'write',
				'unlock',
				'close' 
		...
		$directory->expects ( $this->once () )->method ( 'openFile' )->with ( $relativeFileName )->will ( $this->returnValue ( $file ) );
