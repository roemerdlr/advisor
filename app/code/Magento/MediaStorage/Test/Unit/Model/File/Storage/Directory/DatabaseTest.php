...
class DatabaseTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->helperStorageDatabase = $this->getMock ( 'Magento\MediaStorage\Helper\File\Storage\Database', [ ], [ ], '', false );
		$this->dateModelMock = $this->getMock ( 'Magento\Framework\Stdlib\DateTime\DateTime', [ ], [ ], '', false );
		$this->directoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\Directory\Database', [ 
				'setPath',
				'setName',
				'__wakeup',
				'save',
		...
		$this->directoryFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\Directory\DatabaseFactory', [ 
				'create' 
		], [ ], '', false );
		$this->resourceDirectoryDatabaseMock = $this->getMock ( 'Magento\MediaStorage\Model\ResourceModel\File\Storage\Directory\Database', [ ], [ ], '', false );
		$this->loggerMock = $this->getMock ( 'Psr\Log\LoggerInterface' );
		
		$this->directoryFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->directoryMock ) );
		
		$this->configMock = $this->getMock ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		$this->configMock->expects ( $this->any () )->method ( 'getValue' )->with ( \Magento\MediaStorage\Model\File\Storage::XML_PATH_STORAGE_MEDIA_DATABASE, 'default' )->will ( $this->returnValue ( $this->customConnectionName ) );
		
		$this->contextMock->expects ( $this->once () )->method ( 'getLogger' )->will ( $this->returnValue ( $this->loggerMock ) );
		
		$this->directoryDatabase = new \Magento\MediaStorage\Model\File\Storage\Directory\Database ( $this->contextMock, $this->registryMock, $this->helperStorageDatabase, $this->dateModelMock, $this->configMock, $this->directoryFactoryMock, $this->resourceDirectoryDatabaseMock, null, $this->customConnectionName, [ ] );
	}
	...
	public function testImportDirectories() {
		...
		$this->directoryMock->expects ( $this->exactly ( 2 ) )->method ( 'setPath' )->with ( $this->logicalOr ( $this->equalTo ( 'path/number/one' ), $this->equalTo ( 'path/number/two' ) ) );
