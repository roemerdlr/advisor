...
class MediaTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->configFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\ConfigFactory', [ 
				'create' 
		], [ ], '', false );
		$this->configFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->configMock ) );
		$this->syncFactoryMock = $this->getMock ( 'Magento\MediaStorage\Model\File\Storage\SynchronizationFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testProcessRequestCreatesConfigFileMediaDirectoryIsNotProvided() {
		$this->model = new \Magento\MediaStorage\App\Media ( $this->configFactoryMock, $this->syncFactoryMock, $this->responseMock, $this->closure, false, self::CACHE_FILE_PATH, self::RELATIVE_FILE_PATH, $this->filesystemMock );
		$filePath = '/absolute/path/to/test/file.png';
		$this->directoryMock->expects ( $this->any () )->method ( 'getAbsolutePath' )->will ( $this->returnValueMap ( [ 
				[ 
						null,
		...
		$this->directoryMock->expects ( $this->once () )->method ( 'isReadable' )->with ( self::RELATIVE_FILE_PATH )->will ( $this->returnValue ( true ) );
