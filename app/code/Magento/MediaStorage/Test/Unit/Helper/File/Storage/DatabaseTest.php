...
class DatabaseTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->dbStorageFactoryMock = $this->getMockBuilder ( 'Magento\MediaStorage\Model\File\Storage\DatabaseFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
	...
	public function testGetResourceStorageModel() {
		$dbModelMock = $this->getMockBuilder ( 'Magento\MediaStorage\Model\File\Storage\Database' )->disableOriginalConstructor ()->getMock ();
		$this->dbStorageFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $dbModelMock ) );
		$resourceModelMock = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb' )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup' 
		] )->getMockForAbstractClass ();
		$dbModelMock->expects ( $this->once () )->method ( 'getResource' )->will ( $this->returnValue ( $resourceModelMock ) );
	...
	public function testSaveFileToFileSystem($expected, $storage, $callNum, $id = 0, $callSaveFile = 0) {
		...
		$dbModelMock->expects ( $this->exactly ( $callSaveFile ) )->method ( 'getData' )->will ( $this->returnValue ( [ 
				'data' 
		] ) );
	...
	public function testDeleteFolder($storage, $callNum) {
		...
		$dbModelMock->expects ( $this->exactly ( $callNum ) )->method ( 'getResource' )->will ( $this->returnValue ( $resourceModelMock ) );
		$resourceModelMock->expects ( $this->exactly ( $callNum ) )->method ( 'deleteFolder' )->with ( 'folder' );
	...
	public function testGetMediaBaseDir() {
		...
		$this->helper = $this->objectManager->getObject ( 'Magento\MediaStorage\Helper\File\Storage\Database', [ 
