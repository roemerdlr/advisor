...
class StorageTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetCurrentStorageCode() {
		$currentStorage = '10';
		$this->configMock->expects ( $this->once () )->method ( 'getValue' )->with ( \Magento\MediaStorage\Model\File\Storage::XML_PATH_STORAGE_MEDIA, 'default' )->will ( $this->returnValue ( $currentStorage ) );
	...
	public function testGetStorageModel() {
		$storageModelMock = $this->getMockBuilder ( 'Magento\Framework\Model\AbstractModel' )->disableOriginalConstructor ()->getMock ();
		$this->storageMock->expects ( $this->once () )->method ( 'getStorageModel' )->will ( $this->returnValue ( $storageModelMock ) );
	...
	public function testProcessStorageFile($expected, $storage, $callNum, $callSaveFileNum, $fileId = null) {
		...
		$storageModelMock = $this->getMockBuilder ( 'Magento\Framework\Model\AbstractModel' )->disableOriginalConstructor ()->setMethods ( [ 
				'loadByFileName',
				'__wakeup' 
		] )->getMock ();
		$this->storageMock->expects ( $this->exactly ( $callNum ) )->method ( 'getStorageModel' )->will ( $this->returnValue ( $storageModelMock ) );
		$fileMock = $this->getMockBuilder ( 'Magento\MediaStorage\Model\File\Storage\Database' )->disableOriginalConstructor ()->setMethods ( [ 
				'getId',
				'__wakeup' 
		] )->getMock ();
	...
	public function processStorageFileDataProvider() {
		return [ 
				...
				'external storage, no file' => [ 
						false,
						5,
						1,
						0 
				],
				'external storage, with file' => [ 
						1,
						5,
						1,
						1,
