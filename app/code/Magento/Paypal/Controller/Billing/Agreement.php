...
abstract class Agreement extends \Magento\Framework\App\Action\Action {
	...
	protected function _initAgreement() {
		...
		if ($agreementId) {
			...
			$billingAgreement = $this->_objectManager->create ( 'Magento\Paypal\Model\Billing\Agreement' )->load ( $agreementId );
		...
		$this->messageManager->addErrorMessage ( __ ( 'Please specify the correct billing agreement ID and try again.' ) );
		$this->_redirect ( '*/*/' );
		return false;
	}
