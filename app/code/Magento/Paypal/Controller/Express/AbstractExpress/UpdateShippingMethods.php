...
class UpdateShippingMethods extends \Magento\Paypal\Controller\Express\AbstractExpress {
	...
	public function execute() {
		try {
			...
			$this->getResponse ()->setBody ( $this->_view->getLayout ()->getBlock ( 'express.review.shipping.method' )->setQuote ( $this->_getQuote () )->toHtml () );
			return;
		...
		$this->getResponse ()->setBody ( '<script>window.location.href = ' . $this->_url->getUrl ( '*/*/review' ) . ';</script>' );
	}
}
