...
abstract class AbstractExpress extends AppAction implements RedirectLoginInterface {
	...
	protected function _initToken($setToken = null) {
		if (null !== $setToken) {
			if (false === $setToken) {
				...
				if (! $this->_getSession ()->getExpressCheckoutToken ()) {
					throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'PayPal Express Checkout Token does not exist.' ) );
				}
		...
		if ($setToken) {
			if ($setToken !== $this->_getSession ()->getExpressCheckoutToken ()) {
				throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'A wrong PayPal Express Checkout Token is specified.' ) );
			}
		} else {
	...
	public function redirectLogin() {
		...
		$this->getResponse ()->setRedirect ( $this->_urlHelper->addRequestParam ( $this->_customerUrl->getLoginUrl (), [ 
				'context' => 'checkout' 
		] ) );
	}
}
