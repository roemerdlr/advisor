	...
	protected function getFrameActionUrl($code) {
		...
		switch ($code) {
			case Config::METHOD_PAYFLOWADVANCED :
				$url = $this->urlBuilder->getUrl ( 'paypal/payflowadvanced/form', [ 
						'_secure' => true 
				] );
				break;
			case Config::METHOD_PAYFLOWLINK :
				$url = $this->urlBuilder->getUrl ( 'paypal/payflow/form', [ 
						'_secure' => true 
				] );
				break;
			case Config::METHOD_HOSTEDPRO :
				$url = $this->urlBuilder->getUrl ( 'paypal/hostedpro/redirect', [ 
						'_secure' => true 
				] );
				break;
		}
