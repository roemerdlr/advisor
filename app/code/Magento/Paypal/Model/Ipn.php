...
class Ipn extends \Magento\Paypal\Model\AbstractIpn implements IpnInterface {
	 ...
	 * @param array $data        	
	 */
	public function __construct(\Magento\Paypal\Model\ConfigFactory $configFactory, \Psr\Log\LoggerInterface $logger, \Magento\Framework\HTTP\Adapter\CurlFactory $curlFactory, \Magento\Sales\Model\OrderFactory $orderFactory, Info $paypalInfo, OrderSender $orderSender, CreditmemoSender $creditmemoSender, array $data = []) {
	...
	protected function _getConfig() {
		...
		if (strtolower ( $merchantEmail ) != strtolower ( $receiver )) {
			throw new Exception ( sprintf ( 'The requested %s and configured %s merchant emails do not match.', $receiver, $merchantEmail ) );
		}
	 ...
	 * Everything should be added to order comments.
	 * In positive processing cases customer will get email notifications.
	...
	protected function _registerPaymentCapture($skipFraudDetection = false) {
		...
		$payment->registerCaptureNotification ( $this->getRequestData ( 'mc_gross' ), $skipFraudDetection && $parentTransactionId );
		$this->_order->save ();
	...
	protected function _registerPaymentDenial() {
		$this->_importPaymentInformation ();
		$this->_order->getPayment ()->setTransactionId ( $this->getRequestData ( 'txn_id' ) )->setNotificationResult ( true )->setIsTransactionClosed ( true )->deny ( false );
		$this->_order->save ();
	}
	...
	public function _registerPaymentPending() {
		...
		if (\Magento\Sales\Model\Order::STATE_PENDING_PAYMENT == $this->_order->getState () && ! $this->getRequestData ( 'transaction_entity' )) {
		...
		$this->_order->getPayment ()->setPreparedMessage ( $this->_createIpnComment ( $this->_paypalInfo->explainPendingReason ( $reason ) ) )->setTransactionId ( $this->getRequestData ( 'txn_id' ) )->setIsTransactionClosed ( 0 )->update ( false );
		$this->_order->save ();
	}
