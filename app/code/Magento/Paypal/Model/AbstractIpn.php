...
class AbstractIpn {
	 ...
	 * @param array $data        	
	 */
	public function __construct(\Magento\Paypal\Model\ConfigFactory $configFactory, \Psr\Log\LoggerInterface $logger, \Magento\Framework\HTTP\Adapter\CurlFactory $curlFactory, array $data = []) {
	...
	protected function _postBack() {
		...
		$httpAdapter->write ( \Zend_Http_Client::POST, $postbackUrl, '1.1', [ 
				'Connection: close' 
		], $postbackQuery );
		try {
			$postbackResult = $httpAdapter->read ();
		} catch ( \Exception $e ) {
			$this->_addDebugData ( 'http_error', [ 
					'error' => $e->getMessage (),
					'code' => $e->getCode () 
			] );
			throw $e;
		...
		if (empty ( $postbackResult ) || in_array ( $responseCode, [ 
				'500',
				'502',
				'503' 
		] )) {
