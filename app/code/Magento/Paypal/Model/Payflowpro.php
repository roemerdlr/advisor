...
class Payflowpro extends \Magento\Payment\Model\Method\Cc implements GatewayInterface {
	...
	public function isActive($storeId = null) {
		...
		return ( bool ) ( int ) $this->_scopeConfig->getValue ( $pathPayflowPro, ScopeInterface::SCOPE_STORE, $storeId ) || ( bool ) ( int ) $this->_scopeConfig->getValue ( $pathPaymentPro, ScopeInterface::SCOPE_STORE, $storeId );
	}
	...
	public function void(\Magento\Payment\Model\InfoInterface $payment) {
		...
		if ($response->getResultCode () == self::RESPONSE_CODE_APPROVED) {
			$payment->setTransactionId ( $response->getPnref () )->setIsTransactionClosed ( 1 )->setShouldCloseParentTransaction ( 1 );
		}
	...
	protected static function _isTransactionUnderReview($status) {
		if (in_array ( $status, [ 
				...
				self::RESPONSE_CODE_DECLINED_BY_MERCHANT 
		] )) {
			return false;
		}
		return true;
	...
	public function setBilling(DataObject $request, $billing) {
		$request->setFirstname ( $billing->getFirstname () )->setLastname ( $billing->getLastname () )->setStreet ( implode ( ' ', $billing->getStreet () ) )->setCity ( $billing->getCity () )->setState ( $billing->getRegionCode () )->setZip ( $billing->getPostcode () )->setCountry ( $billing->getCountryId () );
		return $request;
	}
