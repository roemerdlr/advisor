...
class Pro {
	 ...
	 */
	public function __construct(\Magento\Paypal\Model\Config\Factory $configFactory, \Magento\Paypal\Model\Api\Type\Factory $apiFactory, \Magento\Paypal\Model\InfoFactory $infoFactory, TransactionRepositoryInterface $transactionRepository) {
	...
	/**
	 * Payment method code setter.
	 * Also instantiates/updates config
	 *
	 * @param string $code        	
	...
	public function setMethod($code, $storeId = null) {
		if (null === $this->_config) {
			...
			$this->_config = $this->_configFactory->create ( $this->_configType, [ 
					'params' => $params 
			] );
		} else {
	...
	public function void(\Magento\Framework\DataObject $payment) {
		...
		if ($authTransactionId) {
			...
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'You need an authorization transaction to void.' ) );
		}
	}
	...
	public function capture(\Magento\Framework\DataObject $payment, $amount) {
		...
		$api = $this->getApi ()->setAuthorizationId ( $authTransactionId )->setIsCaptureComplete ( $payment->getShouldCloseParentTransaction () )->setAmount ( $amount );
		
		$order = $payment->getOrder ();
		$orderIncrementId = $order->getIncrementId ();
		$api->setCurrencyCode ( $order->getBaseCurrencyCode () )->setInvNum ( $orderIncrementId )->setCustref ( $orderIncrementId )->setPonum ( $order->getId () );
	...
	public function refund(\Magento\Framework\DataObject $payment, $amount) {
		...
		if ($captureTxnId) {
			...
			$api->setRefundType ( $isFullRefund ? \Magento\Paypal\Model\Config::REFUND_TYPE_FULL : \Magento\Paypal\Model\Config::REFUND_TYPE_PARTIAL );
			$api->callRefundTransaction ();
			$this->_importRefundResultToPayment ( $api, $payment, $canRefundMore );
		} else {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'We can\'t issue a refund transaction because there is no capture transaction.' ) );
		}
	}
	...
	protected function _importRefundResultToPayment($api, $payment, $canRefundMore) {
		$payment->setTransactionId ( $api->getRefundTransactionId () )->setIsTransactionClosed ( 1 ) // refund initiated by merchant
