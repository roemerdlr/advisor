...
class Config extends AbstractConfig {
	...
	public function getPaypalUrl(array $params = []) {
		return sprintf ( 'https://www.%spaypal.com/cgi-bin/webscr%s', $this->getValue ( 'sandboxFlag' ) ? 'sandbox.' : '', $params ? '?' . http_build_query ( $params ) : '' );
	}
	 ...
	 *        	encrypted summary about merchant
	 * @return string
	...
	public function getPaymentMarkImageUrl($localeCode, $orderTotal = null, $pal = null, $staticSize = null) {
		...
		return sprintf ( 'https://www.paypalobjects.com/webstatic/en_US/i/buttons/pp-acceptance-%s.png', $staticSize );
	}
	...
	public function shouldAskToCreateBillingAgreement() {
		return $this->getValue ( 'allow_ba_signup' ) === self::EC_BA_SIGNUP_ASK && ! $this->shouldUseUnilateralPayments ();
	}
	...
	protected function _getDynamicImageUrl($type, $localeCode, $orderTotal, $pal) {
		...
		return sprintf ( 'https://fpdbs%s.paypal.com/dynamicimageweb?%s', $this->getValue ( 'sandboxFlag' ) ? '.sandbox' : '', http_build_query ( $params ) );
	}
	
	/**
	 * Check whether specified locale code is supported.
	 * Fallback to en_US
	 *
	...
	protected function _mapWpukFieldset($fieldName) {
		...
		} elseif ($this->_methodCode == self::METHOD_WPP_PE_EXPRESS && $this->isMethodAvailable ( self::METHOD_PAYFLOWADVANCED )) {
	...
	public function getBmlPublisherId() {
		return $this->_scopeConfig->getValue ( 'payment/' . self::METHOD_WPP_BML . '/publisher_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId );
	}
	...
	public function getBmlDisplay($section) {
		$display = $this->_scopeConfig->getValue ( 'payment/' . self::METHOD_WPP_BML . '/' . $section . '_display', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId );
		$bmlActive = $this->_scopeConfig->getValue ( 'payment/' . self::METHOD_WPP_BML . '/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId );
		$bmlUkActive = $this->_scopeConfig->getValue ( 'payment/' . self::METHOD_WPP_PE_BML . '/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId );
		return (($bmlActive && $this->isMethodActive ( self::METHOD_WPP_EXPRESS )) || ($bmlUkActive && $this->isMethodActive ( self::METHOD_WPP_PE_EXPRESS ))) ? $display : 0;
	}
	...
	public function getBmlPosition($section) {
		return $this->_scopeConfig->getValue ( 'payment/' . self::METHOD_WPP_BML . '/' . $section . '_position', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId );
	}
	...
	public function getBmlSize($section) {
		return $this->_scopeConfig->getValue ( 'payment/' . self::METHOD_WPP_BML . '/' . $section . '_size', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId );
	}
}
