...
class Nvp extends \Magento\Paypal\Model\Api\AbstractApi {
	 ...
	 * @var string[]
	 */
	protected $_setExpressCheckoutResponse = [ 
			'TOKEN' 
	];
	 ...
	 * @var string[]
	 */
	protected $_getExpressCheckoutDetailsRequest = [ 
			'TOKEN',
			'SUBJECT' 
	];
	 ...
	 * @var string[]
	 */
	protected $_doReauthorizationRequest = [ 
			'AUTHORIZATIONID',
			'AMT',
			'CURRENCYCODE' 
	];
	 ...
	 * @var string[]
	 */
	protected $_doAuthorizationRequest = [ 
			'TRANSACTIONID',
			'AMT',
			'CURRENCYCODE' 
	];
	 ...
	 * @var string[]
	 */
	protected $_doAuthorizationResponse = [ 
			'TRANSACTIONID',
			'AMT' 
	];
	 ...
	 * @var string[]
	 */
	protected $_getTransactionDetailsRequest = [ 
			'TRANSACTIONID' 
	];
	 ...
	 * @var string[]
	 */
	protected $_refundTransactionRequest = [ 
			'TRANSACTIONID',
			'REFUNDTYPE',
			'CURRENCYCODE',
			'NOTE' 
	 ...
	 * @var string[]
	 */
	protected $_refundTransactionResponse = [ 
			'REFUNDTRANSACTIONID',
			'GROSSREFUNDAMT' 
	];
	 ...
	 * @var string[]
	 */
	protected $_managePendingTransactionStatusRequest = [ 
			'TRANSACTIONID',
			'ACTION' 
	];
	 ...
	 * @var string[]
	 */
	protected $_managePendingTransactionStatusResponse = [ 
			'TRANSACTIONID',
			'STATUS' 
	];
	 ...
	 * @var string[]
	 */
	protected $_getPalDetailsResponse = [ 
			'PAL' 
	];
	 ...
	 * @var string[]
	 */
	protected $_customerBillingAgreementRequest = [ 
			'RETURNURL',
			'CANCELURL',
			'BILLINGTYPE' 
	];
	 ...
	 * @var string[]
	 */
	protected $_customerBillingAgreementResponse = [ 
			'TOKEN' 
	];
	 ...
	 * @var string[]
	 */
	protected $_billingAgreementCustomerDetailsRequest = [ 
			'TOKEN' 
	];
	 ...
	 * @var string[]
	 */
	protected $_createBillingAgreementRequest = [ 
			'TOKEN' 
	];
	 ...
	 * @var string[]
	 */
	protected $_createBillingAgreementResponse = [ 
			'BILLINGAGREEMENTID' 
	];
	 ...
	 * @var string[]
	 */
	protected $_doReferenceTransactionResponse = [ 
			'BILLINGAGREEMENTID',
			'TRANSACTIONID' 
	];
	 ...
	 * @param array $request        	
	 * @return array
	 * @throws \Magento\Framework\Exception\LocalizedException|\Exception @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	public function call($methodName, array $request) {
		...
		try {
			...
			$config = [ 
					'timeout' => 60,
					'verifypeer' => $this->_config->getValue ( 'verifyPeer' ) 
			];
			if ($this->getUseProxy ()) {
		...
		if ($http->getErrno ()) {
			$this->_logger->critical ( new \Exception ( sprintf ( 'PayPal NVP CURL connection error #%s: %s', $http->getErrno (), $http->getError () ) ) );
			$http->close ();
		...
		if (! $this->_validateResponse ( $methodName, $response )) {
			...
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Something went wrong while processing your order.' ) );
		}
	 ...
	 * @param array $response        	
	 * @return void
	 * @throws \Magento\Paypal\Model\Api\ProcessableException|\Magento\Framework\Exception\LocalizedException @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	protected function _handleCallErrors($response) {
		...
		$exceptionLogMessage = sprintf ( 'PayPal NVP gateway errors: %s Correlation ID: %s. Version: %s.', $errorMessages, isset ( $response ['CORRELATIONID'] ) ? $response ['CORRELATIONID'] : '', isset ( $response ['VERSION'] ) ? $response ['VERSION'] : '' );
	...
	protected function _formatErrorMessage($errorCode, $shortErrorMessage, $longErrorMessage) {
		...
		return $longErrorMessage ? sprintf ( '%s (#%s: %s).', $longErrorMessage, $errorCode, $shortErrorMessage ) : sprintf ( '#%s: %s.', $errorCode, $shortErrorMessage );
	}
	...
	protected function _extractErrorsFromResponse($response) {
		...
		for($i = 0; isset ( $response ["L_ERRORCODE{$i}"] ); $i ++) {
			...
			$errorMessage = $this->_formatErrorMessage ( $errorCode, $response ["L_SHORTMESSAGE{$i}"], isset ( $response ["L_LONGMESSAGE{$i}"] ) ? $response ["L_LONGMESSAGE{$i}"] : null );
			$errors [] = [ 
					'code' => $errorCode,
	...
	protected function _applyStreetAndRegionWorkarounds(\Magento\Framework\DataObject $address) {
		...
		if ($address->getCountryId () && $address->getRegion ()) {
			...
			foreach ( $regions as $region ) {
				...
				$address->setExportedKeys ( array_merge ( $address->getExportedKeys (), [ 
						'region_id' 
				] ) );
				break;
			}
	 ...
	 *
	 * @param string $value        	
	 * @return string|null @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	protected function _prepareEachCallRequest($methodName) {
		...
		return array_diff ( $this->_eachCallRequest, [ 
				'USER',
				'PWD',
				'SIGNATURE' 
		] );
