...
abstract class AbstractApi extends \Magento\Framework\DataObject {
	 ...
	 * @param array $data        	
	 */
	public function __construct(\Magento\Customer\Helper\Address $customerAddress, \Psr\Log\LoggerInterface $logger, Logger $customLogger, \Magento\Framework\Locale\ResolverInterface $localeResolver, \Magento\Directory\Model\RegionFactory $regionFactory, array $data = []) {
	...
	public function import($to, array $publicMap = []) {
		return \Magento\Framework\DataObject\Mapper::accumulateByMap ( [ 
				$this,
				'getDataUsingMethod' 
		], $to, $publicMap );
	}
	...
	protected function &_exportToRequest(array $privateRequestMap, array $request = []) {
		...
		$result = \Magento\Framework\DataObject\Mapper::accumulateByMap ( [ 
				$this,
				'getDataUsingMethod' 
		], $request, $map );
		foreach ( $privateRequestMap as $key ) {
		...
		foreach ( $privateResponseMap as $key ) {
			...
			if (isset ( $response [$key] ) && isset ( $this->_importFromRequestFilters [$key] )) {
				...
				$response [$key] = call_user_func ( [ 
						$this,
						$callback 
				], $response [$key], $key, $map [$key] );
			}
		}
		\Magento\Framework\DataObject\Mapper::accumulateByMap ( $response, [ 
				$this,
				'setDataUsingMethod' 
		], $map );
	}
	 ...
	 *        	array &$request
	 * @param int $i        	
	 * @return true|null @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	 ...
	 * Paypal note: The value for quantity must be a positive integer.
