...
class PayflowNvp extends \Magento\Paypal\Model\Api\Nvp {
	...
	public function getApiEndpoint() {
		return sprintf ( 'https://%spayflowpro.paypal.com/transaction', $this->_config->getValue ( 'sandboxFlag' ) ? 'pilot-' : '' );
	}
	...
	protected function _mapPaypalMethodName($methodName) {
		switch ($methodName) {
			...
			case \Magento\Paypal\Model\Api\Nvp::DO_DIRECT_PAYMENT :
				return $this->_config->getValue ( 'payment_action' ) == \Magento\Paypal\Model\Config::PAYMENT_ACTION_AUTH ? self::TRXTYPE_AUTH_ONLY : self::TRXTYPE_SALE;
	...
	protected function _handleCallErrors($response) {
		if ($response ['RESULT'] != self::RESPONSE_CODE_APPROVED) {
			...
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'The PayPal gateway rejected the request. %1', $message ) );
		}
	}
