...
class Info {
	...
	public function importToPayment($from, \Magento\Payment\Model\InfoInterface $payment) {
		...
		\Magento\Framework\DataObject\Mapper::accumulateByMap ( $from, [ 
				$payment,
	...
	public static function isPaymentSuccessful(\Magento\Payment\Model\InfoInterface $payment) {
		...
		$pendingReason = $payment->getAdditionalInformation ( self::PENDING_REASON_GLOBAL );
		return self::PAYMENTSTATUS_PENDING === $paymentStatus && in_array ( $pendingReason, [ 
				'authorization',
				'order' 
		] );
	}
	...
	public static function explainReasonCode($code) {
		$comments = [ 
				...
				'guarantee' => __ ( 'A reversal has occurred on this transaction due to your customer triggering a money-back guarantee.' ),
				...
				'refund' => __ ( 'A reversal has occurred on this transaction because you have given the customer a refund.' ),
		...
		return isset ( $comments [$code] ) ? $comments [$code] : __ ( 'Unknown reason. Please contact PayPal customer service.' );
	}
	...
	protected function _getLabel($key) {
		...
		return isset ( $this->_labelCodesCache [self::ITEM_LABELS] [$key] ) ? $this->_labelCodesCache [self::ITEM_LABELS] [$key] : '';
	}
	...
	protected function _getAvsLabel($value) {
		...
		return isset ( $this->_labelCodesCache [self::PAYPAL_AVS_CODE] [$value] ) ? $this->_labelCodesCache [self::PAYPAL_AVS_CODE] [$value] : $value;
	}
	...
	protected function _getCvv2Label($value) {
		...
		return isset ( $this->_labelCodesCache [self::PAYPAL_CVV_2_MATCH] [$value] ) ? $this->_labelCodesCache [self::PAYPAL_CVV_2_MATCH] [$value] : $value;
	}
	...
	protected function _getBuyerIdTypeValue($code) {
		...
		return isset ( $this->_labelCodesCache [self::BUYER_TAX_ID_TYPE] [$code] ) ? $this->_labelCodesCache [self::BUYER_TAX_ID_TYPE] [$code] : '';
	}
}
