...
class Iframe extends \Magento\Payment\Block\Form {
	...
	protected function _construct() {
		...
		if (in_array ( $paymentCode, $this->_hssHelper->getHssMethods () )) {
			...
			$file = $this->resolver->getTemplateFileName ( $templateFile, [ 
	...
	protected function _getBlock() {
		if (! $this->_block) {
			$this->_block = $this->getLayout ()->createBlock ( 'Magento\\Paypal\\Block\\' . str_replace ( ' ', '\\', ucwords ( str_replace ( '_', ' ', $this->_paymentMethodCode ) ) ) . '\\Iframe' );
	...
	protected function _isAfterPaymentSave() {
		...
		if ($quote->getPayment ()->getMethod () == $this->_paymentMethodCode && $quote->getIsActive () && $this->getTemplate () && $this->getRequest ()->getActionName () == 'savePayment') {
			return true;
		}
