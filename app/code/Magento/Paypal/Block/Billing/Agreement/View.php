...
class View extends \Magento\Framework\View\Element\Template {
	...
	public function getRelatedOrders() {
		if ($this->_relatedOrders === null) {
			...
			$this->_relatedOrders = $this->_orderCollectionFactory->create ()->addFieldToSelect ( '*' )->addFieldToFilter ( 'customer_id', ( int ) $this->_customerSession->getCustomerId () )->addFieldToFilter ( 'status', [ 
	...
	public function getOrderItemValue(\Magento\Sales\Model\Order $order, $key) {
		...
		switch ($key) {
			...
			case 'status_label' :
				...
				break;
			case 'view_url' :
				$value = $this->getUrl ( 'sales/order/view', [ 
	...
	protected function _prepareLayout() {
		...
		$pager = $this->getLayout ()->createBlock ( 'Magento\Theme\Block\Html\Pager' )->setCollection ( $this->getRelatedOrders () )->setIsOutputRequired ( false );
	...
	protected function _toHtml() {
		...
		if ($billingAgreement) {
			...
			$this->setCancelUrl ( $this->getUrl ( '*/billing_agreement/cancel', [ 
					'_current' => true,
			...
			$this->setAgreementCreatedAt ( $createdAt ? $this->formatDate ( $createdAt, \IntlDateFormatter::SHORT, true ) : __ ( 'N/A' ) );
			if ($updatedAt) {
