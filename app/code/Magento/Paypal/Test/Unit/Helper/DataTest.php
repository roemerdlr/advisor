...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->_paymentDataMock = $this->getMockBuilder ( 'Magento\Payment\Helper\Data' )->disableOriginalConstructor ()->setMethods ( [ 
				'getStoreMethods',
				'getPaymentMethods' 
		] )->getMock ();
		
		$this->configMock = $this->getMock ( 'Magento\Paypal\Model\Config', [ ], [ ], '', false );
		$configMockFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\ConfigFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$this->_helper = $objectManager->getObject ( 'Magento\Paypal\Helper\Data', [ 
				'paymentData' => $this->_paymentDataMock,
				'methodCodes' => [ 
						'expressCheckout' => 'paypal_express',
						'hostedPro' => 'hosted_pro' 
				],
	...
	public function getBillingAgreementMethodsDataProvider() {
		$quoteMock = $this->getMockBuilder ( 'Magento\Quote\Model\Quote' )->disableOriginalConstructor ()->setMethods ( null );
		$methodInterfaceMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Billing\Agreement\MethodInterface' )->getMock ();
	...
	public function testGetHtmlTransactionIdProvider() {
		$htmlTransactionId = '<a target="_blank" href="https://www.%1$s.paypal.com/cgi-bin/webscr?cmd=_view-a-trans&id=%2$s">%2$s</a>';
		return [ 
				[ 
						'paypal_express',
						$htmlTransactionId 
