...
class PlaceOrderTest extends \Magento\Paypal\Test\Unit\Controller\ExpressTest {
	...
	public function testExecuteNonProcessableException($isGeneral) {
		if (! $isGeneral) {
			$this->request->expects ( $this->once () )->method ( 'getPost' )->with ( 'agreement', [ ] )->will ( $this->returnValue ( [ ] ) );
		}
	...
	protected function _expectRedirect($path = '*/*/review') {
		$this->redirect->expects ( $this->once () )->method ( 'redirect' )->with ( $this->anything (), $path, [ ] );
	}
	...
	public function testExecuteProcessableException($code, $paymentAction = null) {
		...
		$this->objectManagerCallback = function ($className) use($code, $oldCallback) {
			...
			if ($className == '\Magento\CheckoutAgreements\Model\AgreementsValidator') {
				$exception = $this->getMock ( 'Magento\Paypal\Model\Api\ProcessableException', [ 
						'getUserMessage' 
				], [ 
						'message',
						$code 
				] );
				$exception->expects ( $this->any () )->method ( 'getUserMessage' )->will ( $this->returnValue ( 'User Message' ) );
				$instance->expects ( $this->once () )->method ( 'isValid' )->will ( $this->throwException ( $exception ) );
			}
			return $instance;
		};
		if (isset ( $paymentAction )) {
			$this->config->expects ( $this->once () )->method ( 'getPaymentAction' )->will ( $this->returnValue ( $paymentAction ) );
		}
	...
	protected function _expectErrorCodes($code, $paymentAction) {
		...
		if (in_array ( $code, [ 
			...
			$this->quote->expects ( $this->once () )->method ( 'getPayment' )->will ( $this->returnValue ( $payment ) );
		}
		if ($code == \Magento\Paypal\Model\Api\ProcessableException::API_UNABLE_TRANSACTION_COMPLETE && $paymentAction == \Magento\Payment\Model\Method\AbstractMethod::ACTION_ORDER) {
			$this->config->expects ( $this->once () )->method ( 'getExpressCheckoutOrderUrl' )->will ( $this->returnValue ( $redirectUrl ) );
		}
		if ($code == \Magento\Paypal\Model\Api\ProcessableException::API_DO_EXPRESS_CHECKOUT_FAIL || $code == \Magento\Paypal\Model\Api\ProcessableException::API_UNABLE_TRANSACTION_COMPLETE && $paymentAction != \Magento\Payment\Model\Method\AbstractMethod::ACTION_ORDER) {
			$this->config->expects ( $this->once () )->method ( 'getExpressCheckoutStartUrl' )->will ( $this->returnValue ( $redirectUrl ) );
		...
		if (in_array ( $code, [ 
			...
			$this->response->expects ( $this->once () )->method ( 'setRedirect' )->with ( $redirectUrl );
		} else {
			$this->messageManager->expects ( $this->once () )->method ( 'addError' )->with ( 'User Message' );
