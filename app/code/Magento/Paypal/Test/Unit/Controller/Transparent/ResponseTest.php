...
class ResponseTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resultLayoutMock = $this->getMockBuilder ( 'Magento\Framework\View\Result\Layout' )->setMethods ( [ 
				'addDefaultHandle',
				'getLayout',
				'getUpdate',
				'load' 
		...
		$this->transactionMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Payflow\Service\Response\Transaction' )->setMethods ( [ 
				'getResponseObject',
				'validateResponse',
				'savePaymentInQuote' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->contextMock = $this->getMockBuilder ( '\Magento\Framework\App\Action\Context' )->setMethods ( [ 
				'getRequest' 
		] )->disableOriginalConstructor ()->getMock ();
	...
	public function testExecute() {
		...
		$this->responseValidatorMock->expects ( $this->once () )->method ( 'validate' )->with ( $objectMock );
		$this->transactionMock->expects ( $this->once () )->method ( 'savePaymentInQuote' )->with ( $objectMock );
		$this->coreRegistryMock->expects ( $this->once () )->method ( 'register' )->with ( 'transparent_form_params', $this->logicalNot ( $this->arrayHasKey ( 'error' ) ) );
	...
	private function getLayoutMock() {
		$processorInterfaceMock = $this->getMockBuilder ( '\Magento\Framework\View\Layout\ProcessorInterface' )->getMockForAbstractClass ();
		$layoutMock = $this->getMockBuilder ( '\Magento\Framework\View\Layout' )->setMethods ( [ 
				'getUpdate' 
		] )->disableOriginalConstructor ()->getMock ();
		$layoutMock->expects ( $this->once () )->method ( 'getUpdate' )->willReturn ( $processorInterfaceMock );
