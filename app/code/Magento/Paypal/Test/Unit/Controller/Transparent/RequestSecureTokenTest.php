...
class RequestSecureTokenTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resultJsonFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Controller\Result\JsonFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->sessionTransparentMock = $this->getMockBuilder ( 'Magento\Framework\Session\Generic' )->setMethods ( [ 
				'setQuoteId' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->secureTokenServiceMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Payflow\Service\Request\SecureToken' )->setMethods ( [ 
				'requestToken' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->sessionManagerMock = $this->getMockBuilder ( 'Magento\Framework\Session\SessionManager' )->setMethods ( [ 
				'getQuote' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->transparentMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Payflow\Transparent' )->setMethods ( [ 
				'getCode' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->controller = new \Magento\Paypal\Controller\Transparent\RequestSecureToken ( $this->contextMock, $this->resultJsonFactoryMock, $this->sessionTransparentMock, $this->secureTokenServiceMock, $this->sessionManagerMock, $this->transparentMock );
	}
	public function testExecuteSuccess() {
		...
		$tokenMock->expects ( $this->atLeastOnce () )->method ( 'getData' )->willReturnMap ( [ 
				[ 
						'',
						null,
						$tokenFields 
