...
class ReturnUrlTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->checkoutSessionMock = $this->getMockBuilder ( '\Magento\Checkout\Model\Session' )->setMethods ( [ 
				'getLastRealOrderId',
				'getLastRealOrder',
				'restoreQuote' 
	...
	/**
	 *
	 * @param $state @dataProvider
	 *        	testAllowedOrderStateDataProvider
	 */
	public function testExecuteAllowedOrderState($state) {
		...
		$this->checkoutSessionMock->expects ( $this->exactly ( 2 ) )->method ( 'getLastRealOrderId' )->will ( $this->returnValue ( $lastRealOrderId ) );
		
		$this->orderFactoryMock->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->orderMock ) );
		...
		$this->blockMock->expects ( $this->once () )->method ( 'setData' )->with ( 'goto_success_page', true )->will ( $this->returnSelf () );
	 ...
	 * @param
	 *        	$restoreQuote
	 * @param $expectedGotoSection @dataProvider
	 *        	testNotAllowedOrderStateDataProvider
	 */
