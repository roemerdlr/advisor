...
class SwitcherPluginTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAroundGetUrl($countryParam, $getUrlParams) {
		...
		$subjectRequest->expects ( $this->once () )->method ( 'getParam' )->with ( \Magento\Paypal\Model\Config\StructurePlugin::REQUEST_PARAM_COUNTRY )->will ( $this->returnValue ( $countryParam ) );
		$subject = $this->getMock ( 'Magento\Backend\Block\Store\Switcher', [ 
				'getRequest' 
		], [ ], '', false );
		...
		$this->assertEquals ( [ 
				'',
				$getUrlParams 
		], $this->_model->aroundGetUrl ( $subject, $getUrl, '', [ ] ) );
	}
	public function aroundGetUrlDataProvider() {
		return [ 
				[ 
						'any value',
						[ 
								\Magento\Paypal\Model\Config\StructurePlugin::REQUEST_PARAM_COUNTRY => null 
						] 
				],
				[ 
						null,
