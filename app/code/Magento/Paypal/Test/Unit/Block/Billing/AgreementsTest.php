...
class AgreementsTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * 
	 * @var \Magento\Paypal\Model\ResourceModel\Billing\Agreement\CollectionFactory|\PHPUnit_Framework_MockObject_MockObject @codingStandardsIgnoreEnd
	 */
	...
	public function setUp() {
		...
		$this->agreementCollection = $this->getMockBuilder ( 'Magento\Paypal\Model\ResourceModel\Billing\Agreement\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$this->helper = $this->getMock ( 'Magento\Paypal\Helper\Data', [ ], [ ], '', false );
		$objectManager = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		
		$this->block = $objectManager->getObject ( 'Magento\Paypal\Block\Billing\Agreements', [ 
	...
	public function testGetItemValueEditUrl() {
		$this->escaper->expects ( $this->once () )->method ( 'escapeHtml' );
		$item = $this->getMock ( 'Magento\Paypal\Model\Billing\Agreement', [ 
				'getAgreementId' 
		], [ ], '', false );
		$item->expects ( $this->once () )->method ( 'getAgreementId' )->willReturn ( 1 );
		$this->urlBuilder->expects ( $this->once () )->method ( 'getUrl' )->with ( 'paypal/billing_agreement/view', [ 
				'agreement' => 1 
		] );
	...
	public function testGetWizardPaymentMethodOptions() {
		$method1 = $this->getMock ( 'Magento\Paypal\Model\Method\Agreement', [ 
				'getConfigData',
				'getCode',
				'getTitle' 
		], [ ], '', false );
		$method2 = $this->getMock ( 'Magento\Paypal\Model\Method\Agreement', [ 
				'getConfigData',
				'getCode',
				'getTitle' 
		], [ ], '', false );
		$method3 = $this->getMock ( 'Magento\Paypal\Model\Method\Agreement', [ 
				'getConfigData',
				'getCode',
				'getTitle' 
		], [ ], '', false );
		...
		$this->assertEquals ( [ 
				'code1' => 'title1',
				'code3' => 'title3' 
		], $this->block->getWizardPaymentMethodOptions () );
	}
	public function testToHtml() {
		$this->eventManager->expects ( $this->once () )->method ( 'dispatch' )->with ( 'view_block_abstract_to_html_before', [ 
				'block' => $this->block 
		] );
