...
class HtmlTransactionIdObserverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->paypalDataMock = $this->getMock ( '\Magento\Paypal\Helper\Data', [ 
				'getHtmlTransactionId' 
		], [ ], '', false );
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->_model = $objectManagerHelper->getObject ( 'Magento\Paypal\Observer\HtmlTransactionIdObserver', [ 
				'paypalData' => $this->paypalDataMock 
		] );
	}
	public function testObserveHtmlTransactionId() {
		$observerMock = $this->getMockBuilder ( 'Magento\Framework\Event\Observer' )->setMethods ( [ 
				'getDataObject' 
		] )->disableOriginalConstructor ()->getMock ();
		$transactionMock = $this->getMockBuilder ( '\Magento\Sales\Model\Order\Payment\Transaction' )->setMethods ( [ 
				'getOrder',
				'getTxnId',
				'setData' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$methodInstanceMock = $this->getMockBuilder ( '\Magento\Payment\Model\MethodInterface' )->setMethods ( [ 
				'getCode' 
