...
class PayflowproTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$configFactoryMock = $this->getMock ( 'Magento\Payment\Model\Method\ConfigInterfaceFactory', [ 
				'create' 
		], [ ], '', false );
		$this->configMock = $this->getMock ( 'Magento\Paypal\Model\PayflowConfig', [ ], [ ], '', false );
		$client = $this->getMock ( 'Magento\Framework\HTTP\ZendClient', [ ], [ ], '', false );
		$this->storeManagerMock = $this->getMockForAbstractClass ( 'Magento\Store\Model\StoreManagerInterface', [ ], '', false, true, true, [ 
				'getStore' 
		] );
		$this->gatewayMock = $this->getMock ( 'Magento\Paypal\Model\Payflow\Service\Gateway', [ ], [ ], '', false );
		$this->scopeConfigMock = $this->getMockBuilder ( 'Magento\Framework\App\Config\ScopeConfigInterface' )->setMethods ( [ 
				'getValue' 
		...
		$clientFactory = $this->getMock ( 'Magento\Framework\HTTP\ZendClientFactory', [ 
				'create' 
		], [ ], '', false );
		$clientFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $client ) );
		
		$this->helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->payflowpro = $this->helper->getObject ( 'Magento\Paypal\Model\Payflowpro', [ 
	...
	public function testFetchTransactionInfoForBN() {
		...
		$payment = $this->getMock ( 'Magento\Payment\Model\Info', [ 
				'setTransactionId',
				'__wakeup' 
		], [ ], '', false );
	...
	protected function initStoreMock() {
		...
		$this->storeManagerMock->expects ( static::once () )->method ( 'getStore' )->willReturn ( $storeMock );
		$storeMock->expects ( static::once () )->method ( 'getId' )->willReturn ( $storeId );
	}
	...
	protected function getPaymentMock() {
		...
		$paymentMock->expects ( static::any () )->method ( 'getCcNumber' )->willReturn ( $cardData ['number'] );
		$paymentMock->expects ( static::any () )->method ( 'getCcExpMonth' )->willReturn ( $cardData ['month'] );
		$paymentMock->expects ( static::any () )->method ( 'getCcExpYear' )->willReturn ( $cardData ['year'] );
		$paymentMock->expects ( static::any () )->method ( 'getCcCid' )->willReturn ( $cardData ['cvv'] );
		return $paymentMock;
	}
