...
class ResponseValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->validatorMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Payflow\Service\Response\ValidatorInterface' )->setMethods ( [ 
				'validate' 
	...
	public function testValidate(DataObject $response, $exactlyCount) {
		$this->validatorMock->expects ( $this->exactly ( $exactlyCount ) )->method ( 'validate' )->with ( $response );
	...
	public function dataProviderForTestValidate() {
		return [ 
				...
				[ 
						'response' => new DataObject ( [ 
								'result' => Payflowpro::RESPONSE_CODE_FRAUDSERVICE_FILTER 
						] ),
						'exactlyCount' => 1 
				],
				[ 
						'response' => new DataObject ( [ 
								'result' => Payflowpro::RESPONSE_CODE_INVALID_AMOUNT 
						] ),
						'exactlyCount' => 0 
				] 
		];
	...
	public function testValidateFail() {
		...
		$this->validatorMock->expects ( $this->once () )->method ( 'validate' )->with ( $response )->willReturn ( false );
