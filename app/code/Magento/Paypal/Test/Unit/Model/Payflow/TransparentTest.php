...
class TransparentTest extends \PHPUnit_Framework_TestCase {
	...
	/**
	 *
	 * @var \PHPUnit_Framework_MockObject_MockObject \Magento\Paypal\Model\Payflow\Service\Response\Validator\ResponseValidator
	 */
	protected $responseValidator;
	protected function setUp() {
		$this->paymentMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order\Payment\Info' )->setMethods ( [ 
				'getOrder',
		...
		$this->gatewayMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Payflow\Service\Gateway' )->setMethods ( [ 
				'postRequest' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->storeManagerMock = $this->getMockBuilder ( '\Magento\Store\Model\StoreManagerInterface' )->setMethods ( [ 
				'getStore',
				'getId' 
		...
		$this->configFactoryMock = $this->getMockBuilder ( '\Magento\Payment\Model\Method\ConfigInterfaceFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->configFactoryMock->expects ( $this->once () )->method ( 'create' )->willReturn ( $this->configMock );
		$this->responseMock = $this->getMockBuilder ( '\Magento\Framework\DataObject' )->setMethods ( [ 
				'getResultCode',
				'getOrigresult',
				'getRespmsg',
				'getPnref' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->responseValidator = $this->getMockBuilder ( 'Magento\Paypal\Model\Payflow\Service\Response\Validator\ResponseValidator' )->disableOriginalConstructor ()->setMethods ( [ 
				'validate' 
		] )->getMock ();
		
		$objectHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->object = $objectHelper->getObject ( 'Magento\Paypal\Model\Payflow\Transparent', [ 
	...
	protected function initializationAuthorizeMock() {
		$this->orderMock = $this->getMockBuilder ( 'Magento\Sales\Model\Order' )->setMethods ( [ 
				'getCustomerId',
				'getBillingAddress',
				'getShippingAddress',
				'getCustomerEmail',
		...
		$this->addressBillingMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getFirstname',
				'getLastname',
				'getStreet',
				'getCity',
		...
		$this->addressShippingMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getFirstname',
				'getLastname',
				'getStreet',
				'getCity',
	...
	protected function buildRequestData() {
		...
		$this->addressBillingMock->expects ( $this->once () )->method ( 'getStreet' )->willReturn ( [ 
				'street-1',
				'street-2' 
		] );
		...
		$this->orderMock->expects ( $this->once () )->method ( 'getCustomerEmail' )->willReturn ( 'customer@email.com' );
		...
		$this->addressShippingMock->expects ( $this->once () )->method ( 'getStreet' )->willReturn ( [ 
				'street-1',
				'street-2' 
		] );
	...
	protected function crateVoidResponseMock() {
		$voidResponseMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->setMethods ( [ 
				'getResultCode',
				'getPnref' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$voidResponseMock->expects ( $this->any () )->method ( 'getResultCode' )->willReturn ( Transparent::RESPONSE_CODE_APPROVED );
		$voidResponseMock->expects ( $this->any () )->method ( 'getPnref' )->willReturn ( 'test-pnref' );
		
		$this->responseMock->expects ( $this->once () )->method ( 'getPnref' )->willReturn ( 'test-pnref' );
		$this->paymentMock->expects ( $this->once () )->method ( 'setParentTransactionId' )->with ( 'test-pnref' );
		$this->paymentMock->expects ( $this->once () )->method ( 'getParentTransactionId' )->willReturn ( 'test-pnref' );
		$this->paymentMock->expects ( $this->once () )->method ( 'setTransactionId' )->with ( 'test-pnref' )->willReturnSelf ();
		$this->paymentMock->expects ( $this->once () )->method ( 'setIsTransactionClosed' )->with ( 1 )->willReturnSelf ();
		$this->paymentMock->expects ( $this->once () )->method ( 'setShouldCloseParentTransaction' )->with ( 1 );
	...
	public function testAuthorizeException() {
		...
		$this->gatewayMock->expects ( $this->once () )->method ( 'postRequest' )->willThrowException ( new \Exception () );
	...
	public function testAuthorizeValidationException() {
		...
		$this->gatewayMock->expects ( $this->at ( 0 ) )->method ( 'postRequest' )->willReturn ( $this->responseMock );
		
		$this->responseValidator->expects ( $this->once () )->method ( 'validate' )->with ( $this->responseMock )->willThrowException ( new \Magento\Framework\Exception\LocalizedException ( __ ( 'Error' ) ) );
		
		$this->gatewayMock->expects ( $this->at ( 1 ) )->method ( 'postRequest' )->willReturn ( $voidResponseMock );
		
		$this->paymentMock->expects ( $this->once () )->method ( 'getAdditionalInformation' )->with ( 'pnref' )->willReturn ( 'test-pnref' );
		
		$this->responseMock->expects ( $this->any () )->method ( 'getResultCode' )->willReturn ( Payflowpro::RESPONSE_CODE_FRAUDSERVICE_FILTER );
	 ...
	 */
	public function testAuthorizeLocalizedException($resultCodeExactlyCall, $resultCode, $origResultExactlyCall, $origResult) {
		$this->initializationAuthorizeMock ();
		$this->buildRequestData ();
		
		$this->responseMock->expects ( $this->exactly ( $resultCodeExactlyCall ) )->method ( 'getResultCode' )->willReturn ( $resultCode );
		$this->responseMock->expects ( $this->exactly ( $origResultExactlyCall ) )->method ( 'getOrigresult' )->willReturn ( $origResult );
		
		$this->gatewayMock->expects ( $this->exactly ( 1 ) )->method ( 'postRequest' )->willReturn ( $this->responseMock );
		$this->object->authorize ( $this->paymentMock, 33 );
	}
