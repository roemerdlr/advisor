...
class TransactionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->sessionTransparent = $this->getMock ( 'Magento\Framework\Session\Generic', [ 
				'getQuoteId' 
		], [ ], '', false );
		...
		$this->paymentMethodManagementInterface = $this->getMock ( 'Magento\Quote\Api\PaymentMethodManagementInterface', [ ], [ ], '', false );
		$this->errorHandlerMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Payflow\Service\Response\Handler\HandlerInterface' )->getMock ();
		
		$this->loggerMock = $this->getMockBuilder ( 'Magento\Payment\Model\Method\Logger' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = new Transaction ( $this->sessionTransparent, $this->quoteRepository, $this->transparent, $this->paymentMethodManagementInterface, $this->errorHandlerMock, $this->loggerMock );
	}
	public function testGetResponseObject() {
		$gatewayTransactionResponse = [ ];
		$result = new \Magento\Framework\DataObject ();
		
		$this->transparent->expects ( $this->once () )->method ( 'getDebugReplacePrivateDataKeys' )->willReturn ( [ 
				'key1',
				'key2' 
		] );
		...
		$this->loggerMock->expects ( $this->once () )->method ( 'debug' )->with ( $gatewayTransactionResponse, [ 
				'key1',
				'key2' 
		], true );
