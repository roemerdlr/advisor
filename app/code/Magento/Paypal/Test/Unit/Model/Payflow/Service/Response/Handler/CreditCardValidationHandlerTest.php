...
class CreditCardValidationHandlerTest extends \PHPUnit_Framework_TestCase {
	public function testHandleCreditCardValidationFields() {
		...
		$paypalInfoManager = $this->getMockBuilder ( 'Magento\Paypal\Model\Info' )->disableOriginalConstructor ()->getMock ();
		...
		$responseMock->expects ( $this->exactly ( count ( $expectedHandleResult ) * 2 ) )->method ( 'getData' )->willReturnMap ( [ 
				[ 
		...
		$paypalInfoManager->expects ( $this->once () )->method ( 'importToPayment' )->with ( $expectedHandleResult, $paymentMock );
