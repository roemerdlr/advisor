...
class FraudHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->responseMock = $this->getMockBuilder ( 'Magento\Framework\DataObject' )->disableOriginalConstructor ()->getMock ();
		$this->paypalInfoManagerMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Info' )->disableOriginalConstructor ()->getMock ();
		
		$this->fraudHandler = new FraudHandler ( $this->paypalInfoManagerMock, new \Magento\Framework\Xml\Security () );
	}
	public function testHandleApprovedTransaction() {
		$this->responseMock->expects ( $this->once () )->method ( 'getData' )->with ( 'result' )->willReturn ( Payflowpro::STATUS_APPROVED );
		
		$this->paypalInfoManagerMock->expects ( $this->never () )->method ( 'importToPayment' );
	...
	public function testHandle($message, $rulesString, $existingFrauds, $expectedMessage) {
		$this->responseMock->expects ( $this->atLeastOnce () )->method ( 'getData' )->willReturnMap ( [ 
				[ 
						FraudHandler::RESPONSE_MESSAGE,
						null,
						$message 
				],
				[ 
						FraudHandler::FRAUD_RULES_XML,
						null,
						$rulesString 
				],
				[ 
		...
		$this->paypalInfoManagerMock->expects ( $this->once () )->method ( 'importToPayment' )->with ( [ 
	...
	public function handleMessagesDataProvider() {
		return [ 
				...
				[ 
						...
						array_merge ( $this->getRulesExpectedDictionary (), [ 
								'Total Purchase Price Ceiling' => 'Existing fraud message',
								'RESPMSG' => 'Existing fraud message' 
						] ) 
				] 
	...
	public function testHandleXXEXml() {
		...
		$this->paymentMock->expects ( $this->once () )->method ( 'getAdditionalInformation' )->with ( Info::FRAUD_FILTERS )->willReturn ( [ 
				'Total Purchase Price Ceiling' => 'Existing fraud message',
				'RESPMSG' => 'Existing fraud message' 
		] );
