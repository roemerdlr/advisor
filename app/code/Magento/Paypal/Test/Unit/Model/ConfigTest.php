...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->directoryHelper = $this->getMockBuilder ( 'Magento\Directory\Helper\Data' )->disableOriginalConstructor ()->getMock ();
		
		$this->storeManager = $this->getMock ( 'Magento\Store\Model\StoreManagerInterface' );
		
		$this->ccTypeFactory = $this->getMockBuilder ( 'Magento\Payment\Model\Source\CctypeFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->certFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\CertFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->model = new Config ( $this->scopeConfig, $this->directoryHelper, $this->storeManager, $this->ccTypeFactory, $this->certFactory );
	}
	...
	public function testGetBuildNotationCode() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'paypal/bncode', ScopeInterface::SCOPE_STORE, 123 )->will ( $this->returnValue ( 'some BN code' ) );
	...
	public function testIsMethodAvailableForIsMethodActive($methodName, $expected) {
		$this->scopeConfig->expects ( $this->any () )->method ( 'getValue' )->with ( 'paypal/general/merchant_country' )->will ( $this->returnValue ( 'US' ) );
		$this->scopeConfig->expects ( $this->exactly ( 2 ) )->method ( 'isSetFlag' )->withAnyParameters ()->will ( $this->returnValue ( true ) );
	...
	public function testGetMerchantCountryPaypal() {
		$this->scopeConfig->expects ( static::once () )->method ( 'getValue' )->with ( 'paypal/general/merchant_country', ScopeInterface::SCOPE_STORE, null )->willReturn ( 'US' );
		
		$this->directoryHelper->expects ( static::never () )->method ( 'getDefaultCountry' );
		
		static::assertEquals ( 'US', $this->model->getMerchantCountry () );
	}
	public function testGetMerchantCountryGeneral() {
		$this->scopeConfig->expects ( static::once () )->method ( 'getValue' )->with ( 'paypal/general/merchant_country', ScopeInterface::SCOPE_STORE, null )->willReturn ( null );
		
		$this->directoryHelper->expects ( static::once () )->method ( 'getDefaultCountry' )->with ( null )->willReturn ( 'US' );
	...
	public function testGetSpecificConfigPath() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/' . Config::METHOD_WPP_EXPRESS . '/allow_ba_signup' )->will ( $this->returnValue ( 1 ) );
	...
	public function testGetSpecificConfigPathPayflow() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/' . Config::METHOD_WPP_PE_EXPRESS . '/allow_ba_signup' )->will ( $this->returnValue ( 1 ) );
		$this->assertEquals ( 1, $this->model->getValue ( 'allow_ba_signup' ) );
	}
	public function testGetSpecificConfigPathPayflowAdvancedLink() {
		// _mapWpukFieldset
		$this->model->setMethod ( Config::METHOD_PAYFLOWADVANCED );
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/' . Config::METHOD_PAYFLOWADVANCED . '/payment_action' )->willReturn ( 'Authorization' );
	...
	public function testGetPayPalBasicStartUrl($value, $url) {
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/paypal_express/skip_order_review_step' )->will ( $this->returnValue ( $value ) );
	...
	public function testGetBmlPublisherId() {
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/' . Config::METHOD_WPP_BML . '/publisher_id' )->will ( $this->returnValue ( '12345' ) );
	...
	public function testGetBmlPosition($section, $expected) {
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/' . Config::METHOD_WPP_BML . '/' . $section . '_position' )->will ( $this->returnValue ( $expected ) );
	...
	public function testGetBmlSize($section, $expected) {
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'payment/' . Config::METHOD_WPP_BML . '/' . $section . '_size' )->will ( $this->returnValue ( $expected ) );
	...
	public function testGetBmlDisplay($section, $expectedValue, $expectedFlag, $expected) {
		...
		$this->scopeConfig->expects ( $this->any () )->method ( 'isSetFlag' )->will ( $this->returnValue ( $expectedFlag ) );
		$this->scopeConfig->expects ( $this->any () )->method ( 'getValue' )->will ( $this->returnValueMap ( [ 
				[ 
						'payment/' . Config::METHOD_WPP_BML . '/' . $section . '_display',
						'store',
						1,
						$expectedValue 
				],
				...
				[ 
						'payment/' . Config::METHOD_WPP_PE_BML . '/active',
						'store',
						1,
						$expectedValue 
				] 
	 ...
	 */
	public function testGetExpressCheckoutShortcutImageUrl($localeCode, $orderTotal, $pal, $areButtonDynamic, $sandboxFlag, $buttonType, $result) {
		...
		$this->assertEquals ( $result, $this->model->getExpressCheckoutShortcutImageUrl ( $localeCode, $orderTotal, $pal ) );
	}
	...
	public function dataProviderGetExpressCheckoutShortcutImageUrl() {
		return [ 
				[ 
						...
						null,
						null,
						Config::EC_FLAVOR_DYNAMIC,
						true,
						Config::EC_BUTTON_TYPE_SHORTCUT,
				...
				[ 
						...
						null,
						null,
						Config::EC_FLAVOR_STATIC,
						false,
						Config::EC_BUTTON_TYPE_MARK,
	...
	public function testGetPaymentMarkImageUrl($localeCode, $orderTotal, $pal, $staticSize, $areButtonDynamic, $sandboxFlag, $result) {
		...
		$this->assertEquals ( $result, $this->model->getPaymentMarkImageUrl ( $localeCode, $orderTotal, $pal, $staticSize ) );
	}
