...
class BillingAgreementConfigProviderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->currentCustomerMock = $this->getMockBuilder ( 'Magento\Customer\Helper\Session\CurrentCustomer' )->setMethods ( [ 
				'getCustomerId' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->agreementFactoryMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Billing\AgreementFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->configProvider = new BillingAgreementConfigProvider ( $this->currentCustomerMock, $this->agreementFactoryMock );
	}
	public function testGetConfig() {
		...
		$agreements = [ 
				new \Magento\Framework\DataObject ( [ 
						'id' => 1,
						'reference_id' => 'DFG123ER' 
				] ),
				new \Magento\Framework\DataObject ( [ 
						'id' => 2,
						'reference_id' => 'JKT153ER' 
				] ) 
		];
				...
				'payment' => [ 
						'paypalBillingAgreement' => [ 
								'agreements' => [ 
										[ 
												'id' => 1,
												'referenceId' => 'DFG123ER' 
										],
										[ 
												'id' => 2,
												'referenceId' => 'JKT153ER' 
										] 
								],
		...
		$agreementMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Billing\Agreement' )->disableOriginalConstructor ()->getMock ();
		
		$agreementMock->expects ( $this->once () )->method ( 'getAvailableCustomerBillingAgreements' )->with ( $customerId )->willReturn ( $agreements );
