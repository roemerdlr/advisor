...
class ProTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$args = $objectHelper->getConstructArguments ( 'Magento\Paypal\Model\Pro', [ 
				'configFactory' => $configFactory,
				'infoFactory' => $infoFactory,
				'apiFactory' => $apiFactory 
		] );
		/** @var $pro \Magento\Paypal\Model\Pro */
		$this->pro = $this->getMock ( 'Magento\Paypal\Model\Pro', [ 
	...
	public function testCanReviewPayment($pendingReason, $isReviewRequired, $expected) {
		$this->pro->expects ( $this->any () )->method ( '_isPaymentReviewRequired' )->will ( $this->returnValue ( $isReviewRequired ) );
		$payment = $this->getMockBuilder ( 'Magento\Payment\Model\Info' )->disableOriginalConstructor ()->setMethods ( [ 
	...
	public function canReviewPaymentDataProvider() {
		return [ 
				[ 
						\Magento\Paypal\Model\Info::PAYMENTSTATUS_REVIEW,
						true,
						false 
				],
				[ 
	...
	protected function getInfoFactory() {
		$infoFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\InfoFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$infoMock = $this->getMockBuilder ( 'Magento\Paypal\Model\Info' )->disableOriginalConstructor ()->setMethods ( [ 
	...
	protected function getConfigFactory($storeId) {
		...
		$configMock = $this->getMockBuilder ( $configType )->disableOriginalConstructor ()->getMock ();
		$configFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\Config\Factory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$configFactory->expects ( static::any () )->method ( 'create' )->with ( $configType, [ 
				'params' => [ 
	...
	protected function getApiFactory(ObjectManager $objectHelper) {
		$apiFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\Api\Type\Factory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		...
		$curlFactory = $this->getMockBuilder ( 'Magento\Framework\HTTP\Adapter\CurlFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$curlFactory->expects ( static::any () )->method ( 'create' )->willReturn ( $httpClient );
		
		$apiType = 'Magento\Paypal\Model\Api\Nvp';
		$args = $objectHelper->getConstructArguments ( $apiType, [ 
				'curlFactory' => $curlFactory 
		] );
		$this->apiMock = $this->getMockBuilder ( $apiType )->setConstructorArgs ( $args )->setMethods ( [ 
				'__wakeup',
				'getTransactionId',
				'getDataUsingMethod' 
		] )->getMock ();
	...
	protected function getPaymentMock() {
		$paymentMock = $this->getMockBuilder ( 'Magento\Payment\Model\Info' )->disableOriginalConstructor ()->setMethods ( [ 
				'getParentTransactionId',
				'getOrder',
				'getShouldCloseParentTransaction' 
		] )->getMock ();
		$parentTransactionId = 43;
		$paymentMock->expects ( static::once () )->method ( 'getParentTransactionId' )->willReturn ( $parentTransactionId );
		$paymentMock->expects ( static::once () )->method ( 'getShouldCloseParentTransaction' )->willReturn ( true );
		return $paymentMock;
	}
	...
	protected function getOrderMock() {
		...
		$orderMock->expects ( static::once () )->method ( 'getId' )->willReturn ( $orderData ['id'] );
		$orderMock->expects ( static::once () )->method ( 'getBaseCurrencyCode' )->willReturn ( $orderData ['currency'] );
		$orderMock->expects ( static::atLeastOnce () )->method ( 'getIncrementId' )->willReturn ( $orderData ['increment_id'] );
		return $orderMock;
	}
}
