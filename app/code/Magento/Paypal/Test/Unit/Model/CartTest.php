...
class CartTest extends \PHPUnit_Framework_TestCase {
	...
	public function __construct($name = null, array $data = [], $dataName = '') {
		...
		$this->_validItem = new \Magento\Framework\DataObject ( [ 
				...
				'qty' => 3,
				'name' => 'valid item',
				'original_item' => new \Magento\Framework\DataObject ( [ 
						'base_row_total' => 6.0 
				] ) 
		] );
	}
	protected function setUp() {
		$this->_salesModel = $this->getMockForAbstractClass ( 'Magento\Payment\Model\Cart\SalesModel\SalesModelInterface' );
		$factoryMock = $this->getMock ( 'Magento\Payment\Model\Cart\SalesModel\Factory', [ 
				'create' 
		], [ ], '', false );
		$factoryMock->expects ( $this->once () )->method ( 'create' )->with ( 'sales model' )->will ( $this->returnValue ( $this->_salesModel ) );
	...
	public function testInvalidGetAllItems($items) {
		$taxContainer = new \Magento\Framework\DataObject ( [ 
				...
				'base_shipping_discount_tax_compensation_amnt' => 0.1 
		] );
	...
	public function testInvalidTotalsGetAllItems($values, $transferDiscount) {
		...
		$this->assertEquals ( $values ['base_tax_amount'] + $values ['base_discount_tax_compensation_amount'] + $values ['base_shipping_discount_tax_compensation_amnt'], $this->_model->getTax () );
	...
	public function testInvalidGetAmounts($values, $transferDiscount, $transferShipping) {
		...
		$this->assertEquals ( [ 
				Cart::AMOUNT_SUBTOTAL => $expectedSubtotal 
		], $result );
	}
	...
	protected function _prepareInvalidModelData($values, $transferDiscount) {
		...
		$this->_salesModel->expects ( $this->once () )->method ( 'getTaxContainer' )->will ( $this->returnValue ( $taxContainer ) );
		$this->_salesModel->expects ( $this->once () )->method ( 'getAllItems' )->will ( $this->returnValue ( [ 
				$this->_validItem 
		] ) );
		$this->_salesModel->expects ( $this->once () )->method ( 'getBaseSubtotal' )->will ( $this->returnValue ( $values ['base_subtotal'] ) );
		$this->_salesModel->expects ( $this->once () )->method ( 'getBaseTaxAmount' )->will ( $this->returnValue ( $values ['base_tax_amount'] ) );
		$this->_salesModel->expects ( $this->once () )->method ( 'getBaseShippingAmount' )->will ( $this->returnValue ( $values ['base_shipping_amount'] ) );
		$this->_salesModel->expects ( $this->once () )->method ( 'getBaseDiscountAmount' )->will ( $this->returnValue ( $values ['base_discount_amount'] ) );
		$this->_salesModel->expects ( $this->once () )->method ( 'getDataUsingMethod' )->with ( 'base_grand_total' )->will ( $this->returnValue ( $values ['base_grand_total'] ) );
