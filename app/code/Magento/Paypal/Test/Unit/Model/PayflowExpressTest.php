...
class PayflowExpressTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$proFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\ProFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$api = $this->getMock ( 'Magento\Paypal\Model\Api\Nvp', [ ], [ ], '', false );
		$paypalPro = $this->getMockBuilder ( 'Magento\Paypal\Model\Pro' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$this->transactionRepository = $this->getMockBuilder ( '\Magento\Sales\Api\TransactionRepositoryInterface' )->disableOriginalConstructor ()->setMethods ( [ 
		...
		$this->_model = $objectManager->getObject ( 'Magento\Paypal\Model\PayflowExpress', [ 
	...
	public function testCanRefundCaptureNotExist() {
		...
		$this->transactionRepository->expects ( $this->once () )->method ( 'getByTransactionType' )->with ( Transaction::TYPE_CAPTURE )->willReturn ( false );
	...
	public function testCanRefundCaptureExistNoAdditionalInfo() {
		...
		$captureTransaction->expects ( $this->once () )->method ( 'getAdditionalInformation' )->with ( Payflow\Pro::TRANSPORT_PAYFLOW_TXN_ID )->will ( $this->returnValue ( null ) );
		$paymentInfo->expects ( $this->once () )->method ( 'getOrder' )->willReturnSelf ();
		$this->transactionRepository->expects ( $this->once () )->method ( 'getByTransactionType' )->with ( Transaction::TYPE_CAPTURE )->willReturn ( $captureTransaction );
		$this->assertFalse ( $this->_model->canRefund () );
	}
	...
	protected function _getCaptureTransaction() {
		return $this->getMockBuilder ( 'Magento\Sales\Model\Order\Payment\Transaction' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
	}
