...
class PayflowConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->methodInterfaceMock = $this->getMockBuilder ( 'Magento\Payment\Model\MethodInterface' )->getMockForAbstractClass ();
		
		$om = new ObjectManager ( $this );
		$this->config = $om->getObject ( 'Magento\Paypal\Model\PayflowConfig', [ 
	...
	public function testGetTrxType($paymentAction, $expectedValue) {
		$this->scopeConfigMock->expects ( $this->any () )->method ( 'getValue' )->willReturn ( $paymentAction );
	...
	public function getTrxTypeDataProvider() {
		return [ 
				[ 
						PayflowConfig::PAYMENT_ACTION_AUTH,
						PayflowConfig::TRXTYPE_AUTH_ONLY 
				],
				[ 
						PayflowConfig::PAYMENT_ACTION_SALE,
						PayflowConfig::TRXTYPE_SALE 
				],
				[ 
						'other',
						null 
	...
	public function testGetTransactionUrlWithTestModeOn() {
		...
		$this->methodInterfaceMock->expects ( $this->once () )->method ( 'getConfigData' )->with ( 'transaction_url_test_mode' )->willReturn ( 'transaction_url_test_mode' );
	...
	public function testIsMethodActive(array $expectsMethods, $currentMethod, $result) {
		...
		$this->scopeConfigMock->expects ( $this->any () )->method ( 'getValue' )->with ( 'paypal/general/merchant_country' )->will ( $this->returnValue ( 'US' ) );
