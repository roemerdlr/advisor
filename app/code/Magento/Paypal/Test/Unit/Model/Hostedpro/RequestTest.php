...
class RequestTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->localeResolverMock = $this->getMockBuilder ( 'Magento\Framework\Locale\Resolver' )->disableOriginalConstructor ()->getMock ();
		
		$this->taxData = $this->helper->getObject ( 'Magento\Tax\Helper\Data' );
		
		$this->_model = $this->helper->getObject ( 'Magento\Paypal\Model\Hostedpro\Request', [ 
	 ...
	 * @param
	 *        	$state
	 * @param $countryId @dataProvider
	 *        	addressesDataProvider
	 */
	public function testSetOrderAddresses($billing, $shipping, $billingState, $state, $countryId) {
		$payment = $this->getMockBuilder ( Payment::class )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup' 
		] )->getMock ();
		...
		$order->expects ( static::any () )->method ( 'getPayment' )->will ( $this->returnValue ( $payment ) );
		$order->expects ( static::any () )->method ( 'getBillingAddress' )->will ( $this->returnValue ( $billing ) );
		$order->expects ( static::any () )->method ( 'getShippingAddress' )->will ( $this->returnValue ( $shipping ) );
	...
	public function testSetAmountWithoutTax($total, $subtotal, $tax, $shipping, $discount) {
		...
		$payment->expects ( static::once () )->method ( 'getBaseAmountAuthorized' )->willReturn ( $total );
		
		$order->expects ( static::once () )->method ( 'getPayment' )->willReturn ( $payment );
		
		$order->expects ( static::once () )->method ( 'getBaseDiscountAmount' )->willReturn ( $discount );
		
		$order->expects ( static::once () )->method ( 'getBaseTaxAmount' )->willReturn ( $tax );
		
		$order->expects ( static::once () )->method ( 'getBaseShippingAmount' )->willReturn ( $shipping );
		
		$order->expects ( static::once () )->method ( 'getBaseSubtotal' )->willReturn ( $subtotal );
