...
class StructurePluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_model = $objectManagerHelper->getObject ( 'Magento\Paypal\Model\Config\StructurePlugin', [ 
	...
	public function testAroundGetElementByPathPartsNonPayment($pathParts, $returnResult) {
		$result = $returnResult ? $this->getMockForAbstractClass ( 'Magento\Config\Model\Config\Structure\ElementInterface' ) : null;
		$this->_aroundGetElementByPathPartsAssertResult ( $result, $this->_getElementByPathPartsCallback ( $pathParts, $result ), $pathParts );
	}
	...
	public function testAroundGetElementByPathPartsNoResult($pathParts, $countryCode, $expectedPathParts) {
		...
		$this->_aroundGetElementByPathPartsAssertResult ( null, $this->_getElementByPathPartsCallback ( $expectedPathParts, null ), $pathParts );
	}
	...
	public function testAroundGetElementByPathParts($pathParts, $countryCode, $expectedPathParts) {
		...
		$this->_aroundGetElementByPathPartsAssertResult ( $result, $this->_getElementByPathPartsCallback ( $expectedPathParts, $result ), $pathParts );
	}
	...
	public function testAroundGetSectionByPathParts($pathParts, $countryCode, $expectedPathParts) {
		...
		$getElementByPathParts = function ($pathParts) use($self, $expectedPathParts, $result) {
			...
			$scope = 'any scope';
			$self->_scopeDefiner->expects ( $self->once () )->method ( 'getScope' )->will ( $self->returnValue ( $scope ) );
			$result->expects ( $self->once () )->method ( 'getData' )->will ( $self->returnValue ( [ ] ) );
			$result->expects ( $self->once () )->method ( 'setData' )->with ( [ 
					'showInDefault' => true,
					'showInWebsite' => true,
					'showInStore' => true 
			], $scope )->will ( $self->returnSelf () );
			return $result;
		};
