...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->converter = $this->getMock ( '\Magento\Paypal\Model\Config\Rules\Converter', [ ], [ ], '', false );
		$this->schemaLocator = $this->getMockForAbstractClass ( '\Magento\Framework\Config\SchemaLocatorInterface' );
		$this->validationState = $this->getMockForAbstractClass ( '\Magento\Framework\Config\ValidationStateInterface' );
		$this->helper = $this->getMock ( '\Magento\Paypal\Helper\Backend', [ ], [ ], '', false );
	}
	...
	public function testReadExistingCountryConfig($countryCode, $xml, $expected) {
		...
		$this->fileResolver->expects ( $this->once () )->method ( 'get' )->with ( $this->equalTo ( $expected ) )->willReturn ( $xml );
		
		$this->reader = new \Magento\Paypal\Model\Config\Rules\Reader ( $this->fileResolver, $this->converter, $this->schemaLocator, $this->validationState, $this->helper );
