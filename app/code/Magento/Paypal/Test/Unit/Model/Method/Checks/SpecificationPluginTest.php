...
class SpecificationPluginTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->agreementFactory = $this->getMockBuilder ( 'Magento\Paypal\Model\Billing\AgreementFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$objectManagerHelper = new ObjectManagerHelper ( $this );
		$this->model = $objectManagerHelper->getObject ( 'Magento\Paypal\Model\Method\Checks\SpecificationPlugin', [ 
	...
	public function testAroundIsApplicable($count) {
		...
		$proceed = $this->getProceedClosure ( true, $paymentMethod, $quote );
		$agreementCollection = $this->getMock ( 'Magento\Paypal\Model\ResourceModel\Billing\Agreement\Collection', [ ], [ ], '', false );
	...
	private function getPaymentMethod($code) {
		...
		$paymentMethod->expects ( $this->any () )->method ( 'getCode' )->will ( $this->returnValue ( $code ) );
	 ...
	 * @return bool
	 */
	private function callAroundIsApplicable(\Closure $proceed, MethodInterface $paymentMethod, Quote $quote) {
