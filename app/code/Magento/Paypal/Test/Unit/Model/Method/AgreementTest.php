...
class AgreementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$paypalConfigMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Config' )->disableOriginalConstructor ()->setMethods ( [ ] )->getMock ();
		$this->_apiNvpMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Api\Nvp' )->disableOriginalConstructor ()->setMethods ( [ 
				'callDoReferenceTransaction',
				'callGetTransactionDetails' 
		] )->getMock ();
		$proMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Pro' )->setMethods ( [ 
				'getApi',
				'setMethod',
				'getConfig',
				'importPaymentInfo' 
		...
		$billingAgreementMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Billing\Agreement' )->disableOriginalConstructor ()->setMethods ( [ 
				'load',
				'__wakeup' 
		] )->getMock ();
		$billingAgreementMock->expects ( $this->any () )->method ( 'load' )->will ( $this->returnValue ( $billingAgreementMock ) );
		
		$agreementFactoryMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Billing\AgreementFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		$agreementFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $billingAgreementMock ) );
		
		$cartMock = $this->getMockBuilder ( '\Magento\Paypal\Model\Cart' )->disableOriginalConstructor ()->getMock ();
		$cartFactoryMock = $this->getMockBuilder ( '\Magento\Paypal\Model\CartFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
	...
	public function testAuthorizeWithBaseCurrency() {
		$payment = $this->getMockBuilder ( '\Magento\Sales\Model\Order\Payment' )->disableOriginalConstructor ()->setMethods ( [ 
				'__wakeup' 
		] )->getMock ();
