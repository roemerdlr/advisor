...
class NvpTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->customLoggerMock = $this->getMockBuilder ( '\Magento\Payment\Model\Method\Logger' )->setConstructorArgs ( [ 
				$this->getMockForAbstractClass ( 'Psr\Log\LoggerInterface' ) 
		] )->setMethods ( [ 
				'debug' 
		] )->getMock ();
		...
		$processableExceptionFactory = $this->getMock ( 'Magento\Paypal\Model\Api\ProcessableExceptionFactory', [ 
				'create' 
		], [ ], '', false );
		$processableExceptionFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnCallback ( function ($arguments) {
			$this->processableException = $this->getMock ( 'Magento\Paypal\Model\Api\ProcessableException', null, [ 
					$arguments ['phrase'],
					null,
					$arguments ['code'] 
			] );
		...
		$exceptionFactory = $this->getMock ( 'Magento\Framework\Exception\LocalizedExceptionFactory', [ 
				'create' 
		], [ ], '', false );
		$exceptionFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnCallback ( function ($arguments) {
			$this->exception = $this->getMock ( 'Magento\Framework\Exception\LocalizedException', null, [ 
					$arguments ['phrase'] 
			] );
			return $this->exception;
		} ) );
		$this->curl = $this->getMock ( 'Magento\Framework\HTTP\Adapter\Curl', [ ], [ ], '', false );
		$curlFactory = $this->getMock ( 'Magento\Framework\HTTP\Adapter\CurlFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->model = $helper->getObject ( 'Magento\Paypal\Model\Api\Nvp', [ 
	...
	public function callDataProvider() {
		return [ 
				...
				[ 
						"\r\n" . 'ACK[7]=Failure&L_ERRORCODE0[5]=10417' . '&L_SHORTMESSAGE0[8]=Message.&L_LONGMESSAGE0[15]=Long%20Message.',
						[ 
								10417,
								10422 
						],
