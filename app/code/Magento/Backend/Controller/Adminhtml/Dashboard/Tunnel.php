...
class Tunnel extends \Magento\Backend\Controller\Adminhtml\Dashboard {
	 ...
	 */
	public function __construct(Action\Context $context, Result\RawFactory $resultRawFactory) {
			...
			if (Security::compareStrings ( $newHash, $gaHash )) {
				...
				if ($params) {
					try {
						...
						$response = $httpClient->setUri ( \Magento\Backend\Block\Dashboard\Graph::API_URL )->setParameterGet ( $params )->setConfig ( [ 
								'timeout' => 5 
						] )->request ( 'GET' );
