			...
			try {
				switch ($postData ['store_type']) {
					...
					case 'group' :
						...
						$this->_eventManager->dispatch ( 'store_group_save', [ 
								'group' => $groupModel 
						] );
					...
					case 'store' :
						...
						$groupModel = $this->_objectManager->create ( 'Magento\Store\Model\Group' )->load ( $storeModel->getGroupId () );
				...
				$this->messageManager->addException ( $e, __ ( 'Something went wrong while saving. Please review the error log.' ) );
				$this->_getSession ()->setPostData ( $postData );
			}
