...
abstract class Store extends Action {
	...
	protected function _backupDatabase() {
		...
		try {
			...
			$backup->setTime ( time () )->setType ( 'db' )->setPath ( $filesystem->getDirectoryRead ( DirectoryList::VAR_DIR )->getAbsolutePath ( 'backups' ) );
			...
			$this->messageManager->addException ( $e, __ ( 'We can\'t create a backup right now. Please try again later.' ) );
			return false;
		}
		return true;
	}
	...
	protected function _addDeletionNotice($typeTitle) {
		$this->messageManager->addNotice ( __ ( 'Deleting a %1 will not delete the information associated with the %1 (e.g. categories, products, etc.), but the %1 will not be able to be restored. It is suggested that you create a database backup before deleting the %1.', $typeTitle ) );
		return $this;
	}
}
