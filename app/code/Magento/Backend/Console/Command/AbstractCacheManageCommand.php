...
abstract class AbstractCacheManageCommand extends AbstractCacheCommand {
	...
	protected function getRequestedTypes(InputInterface $input) {
		...
		if (empty ( $requestedTypes )) {
			...
			if ($unsupportedTypes) {
				throw new \InvalidArgumentException ( "The following requested cache types are not supported: '" . join ( "', '", $unsupportedTypes ) . "'." . PHP_EOL . 'Supported types: ' . join ( ", ", $availableTypes ) );
			}
