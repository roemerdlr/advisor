...
class CleanCacheTest extends \PHPUnit_Framework_TestCase {
	public function testCleanCache() {
		...
		$cacheFrontendMock->expects ( $this->once () )->method ( 'getBackend' )->will ( $this->returnValue ( $cacheBackendMock ) );
		
		$frontendPoolMock->expects ( $this->any () )->method ( 'valid' )->will ( $this->onConsecutiveCalls ( true, false ) );
		
		$frontendPoolMock->expects ( $this->any () )->method ( 'current' )->will ( $this->returnValue ( $cacheFrontendMock ) );
		 ...
		 */
		$model = $objectManagerHelper->getObject ( 'Magento\Backend\Cron\CleanCache', [ 
