...
class ColumnSetTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetRowUrl() {
		...
		$rowUrlGenerator = $this->getMock ( 'Magento\Backend\Model\Widget\Grid\Row\UrlGenerator', [ 
				'getUrl' 
		], [ ], '', false );
		...
		$factoryMock = $this->getMock ( 'Magento\Backend\Model\Widget\Grid\Row\UrlGeneratorFactory', [ 
				'createUrlGenerator' 
		], [ ], '', false );
		$factoryMock->expects ( $this->once () )->method ( 'createUrlGenerator' )->with ( $this->equalTo ( $generatorClass ), $this->equalTo ( [ 
				'args' => [ 
	...
	public function testItemHasMultipleRows() {
		...
		$subCollection->addItem ( new \Magento\Framework\DataObject ( [ 
				'test4' => '1',
				'test5' => '2' 
		] ) );
		$subCollection->addItem ( new \Magento\Framework\DataObject ( [ 
	...
	public function testShouldRenderTotalWithEmptyCollection() {
		$this->_prepareLayoutWithGrid ( $this->_prepareGridMock ( new \Magento\Framework\Data\Collection ( $this->getMock ( 'Magento\Framework\Data\Collection\EntityFactory', [ ], [ ], '', false ) ) ) );
	...
	public function testUpdateItemByFirstMultiRow() {
		...
		$expectedItem = new \Magento\Framework\DataObject ( [ 
				'test1' => '1' 
		] );
		$expectedItem->addData ( [ 
				'test4' => '1',
				'test5' => '2' 
		] );
	...
	public function testGetSubTotals() {
		...
		$this->_subtotalsMock->expects ( $this->once () )->method ( 'countTotals' )->with ( $subCollection )->will ( $this->returnValue ( new \Magento\Framework\DataObject ( [ 
				'column' => '2' 
		] ) ) );
		...
		$this->assertEquals ( new \Magento\Framework\DataObject ( [ 
				'column' => '2' 
	...
	public function testGetTotals() {
		...
		$this->_totalsMock->expects ( $this->once () )->method ( 'countTotals' )->with ( $collection )->will ( $this->returnValue ( new \Magento\Framework\DataObject ( [ 
				'test1' => '3',
				'test2' => '2' 
		] ) ) );
	...
	protected function _prepareGridMock($collection) {
		// prepare block grid
		$gridMock = $this->getMock ( 'Magento\Backend\Model\Widget\Grid', [ 
				'getCollection' 
		], [ ], '', true );
	...
	protected function _prepareLayoutWithGrid($gridMock) {
		$this->_layoutMock->expects ( $this->any () )->method ( 'getParentName' )->with ( 'grid.columnSet' )->will ( $this->returnValue ( 'grid' ) );
		$this->_layoutMock->expects ( $this->any () )->method ( 'getBlock' )->with ( 'grid' )->will ( $this->returnValue ( $gridMock ) );
	}
}
