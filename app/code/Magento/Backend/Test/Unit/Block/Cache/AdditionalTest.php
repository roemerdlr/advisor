...
class AdditionalTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->additonalBlock = $objectHelper->getObject ( 'Magento\Backend\Block\Cache\Additional', [ 
				'context' => $context 
		] );
	}
	public function testGetCleanImagesUrl() {
		$expectedUrl = 'cleanImagesUrl';
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( '*/*/cleanImages' )->will ( $this->returnValue ( $expectedUrl ) );
	...
	public function testGetCleanMediaUrl() {
		...
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( '*/*/cleanMedia' )->will ( $this->returnValue ( $expectedUrl ) );
	...
	public function testGetCleanStaticFiles() {
		...
		$this->urlBuilderMock->expects ( $this->once () )->method ( 'getUrl' )->with ( '*/*/cleanStaticFiles' )->will ( $this->returnValue ( $expectedUrl ) );
	...
	public function testIsInProductionMode($mode, $expected) {
		$this->appStateMock->expects ( $this->once () )->method ( 'getMode' )->willReturn ( $mode );
