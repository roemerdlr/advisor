...
class UrlTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_formKey = $this->getMock ( 'Magento\Framework\Data\Form\FormKey', [ 
				'getFormKey' 
		], [ ], '', false );
		...
		$mockItem->expects ( $this->any () )->method ( 'getId' )->will ( $this->returnValue ( 'Magento_Backend::system_acl_roles' ) );
		$mockItem->expects ( $this->any () )->method ( 'getAction' )->will ( $this->returnValue ( 'adminhtml/user_role' ) );
		
		$this->_menuMock->expects ( $this->any () )->method ( 'get' )->with ( $this->equalTo ( 'Magento_Backend::system_acl_roles' ) )->will ( $this->returnValue ( $mockItem ) );
		...
		$this->_scopeConfigMock->expects ( $this->any () )->method ( 'getValue' )->with ( \Magento\Backend\Model\Url::XML_PATH_STARTUP_MENU_ITEM )->will ( $this->returnValue ( 'Magento_Backend::system_acl_roles' ) );
		
		$this->_authSessionMock = $this->getMock ( 'Magento\Backend\Model\Auth\Session', [ ], [ ], '', false, false );
		$helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->_encryptor = $this->getMock ( 'Magento\Framework\Encryption\Encryptor', null, [ ], '', false );
		$this->_paramsResolverMock = $this->getMock ( 'Magento\Framework\Url\RouteParamsResolverFactory', [ ], [ ], '', false );
		$this->_paramsResolverMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->getMock ( 'Magento\Framework\Url\RouteParamsResolver', [ ], [ ], '', false ) ) );
		$this->_model = $helper->getObject ( 'Magento\Backend\Model\Url', [ 
	...
	public function testGetAreaFrontName() {
		$helperMock = $this->getMock ( 'Magento\Backend\Helper\Data', [ ], [ ], '', false );
		$helperMock->expects ( $this->once () )->method ( 'getAreaFrontName' )->will ( $this->returnValue ( $this->_areaFrontName ) );
		
		$helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$urlModel = $helper->getObject ( 'Magento\Backend\Model\Url', [ 
				'backendHelper' => $helperMock,
				'authSession' => $this->_authSessionMock,
				'routeParamsResolverFactory' => $this->_paramsResolverMock 
		] );
