...
class ItemTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_model = $helper->getObject ( 'Magento\Backend\Model\Menu\Item', [ 
	...
	public function testGetUrlWithEmptyActionReturnsHashSign() {
		$this->_params ['action'] = '';
		$helper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$item = $helper->getObject ( 'Magento\Backend\Model\Menu\Item', [ 
				'menuFactory' => $this->_menuFactoryMock,
				'data' => $this->_params 
		] );
	...
	public function testGetUrlWithValidActionReturnsUrl() {
		$this->_urlModelMock->expects ( $this->once () )->method ( 'getUrl' )->with ( $this->equalTo ( '/system/config' ) )->will ( $this->returnValue ( 'Url' ) );
	...
	public function testIsAllowedReturnsTrueIfResourceIsAvailable() {
		$this->_aclMock->expects ( $this->once () )->method ( 'isAllowed' )->with ( 'Magento_Config::config' )->will ( $this->returnValue ( true ) );
		$this->assertTrue ( $this->_model->isAllowed () );
	}
	public function testIsAllowedReturnsFalseIfResourceIsNotAvailable() {
		$this->_aclMock->expects ( $this->once () )->method ( 'isAllowed' )->with ( 'Magento_Config::config' )->will ( $this->throwException ( new \Magento\Framework\Exception\LocalizedException ( __ ( 'Error' ) ) ) );
