...
class PathInfoProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_subjectMock = $this->getMock ( '\Magento\Store\App\Request\PathInfoProcessor', [ ], [ ], '', false );
		$this->_backendHelperMock = $this->getMock ( '\Magento\Backend\Helper\Data', [ ], [ ], '', false );
		$this->_model = new \Magento\Backend\App\Request\PathInfoProcessor ( $this->_subjectMock, $this->_backendHelperMock );
	}
	public function testProcessIfStoreCodeEqualToAreaFrontName() {
		$this->_backendHelperMock->expects ( $this->once () )->method ( 'getAreaFrontName' )->will ( $this->returnValue ( 'storeCode' ) );
	...
	public function testProcessIfStoreCodeNotEqualToAreaFrontName() {
		$this->_backendHelperMock->expects ( $this->once () )->method ( 'getAreaFrontName' )->will ( $this->returnValue ( 'store' ) );
		$this->_subjectMock->expects ( $this->once () )->method ( 'process' )->with ( $this->_requestMock, $this->_pathInfo )->will ( $this->returnValue ( 'Expected' ) );
