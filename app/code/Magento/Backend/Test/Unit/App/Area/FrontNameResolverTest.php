...
class FrontNameResolverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$deploymentConfigMock->expects ( $this->once () )->method ( 'get' )->with ( ConfigOptionsList::CONFIG_PATH_BACKEND_FRONTNAME )->will ( $this->returnValue ( $this->_defaultFrontName ) );
	...
	public function testIfCustomPathUsed() {
		$this->configMock->expects ( $this->at ( 0 ) )->method ( 'getValue' )->with ( 'admin/url/use_custom_path' )->will ( $this->returnValue ( true ) );
		$this->configMock->expects ( $this->at ( 1 ) )->method ( 'getValue' )->with ( 'admin/url/custom_path' )->will ( $this->returnValue ( 'expectedValue' ) );
	...
	public function testIsHostBackend($url, $host, $useCustomAdminUrl, $customAdminUrl, $expectedValue) {
		...
		$this->scopeConfigMock->expects ( $this->exactly ( 2 ) )->method ( 'getValue' )->will ( $this->returnValueMap ( [ 
				[ 
