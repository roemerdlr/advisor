...
class AuthenticationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->auth = $this->getMock ( 'Magento\Backend\Model\Auth', [ 
				'getUser',
				'isLoggedIn',
				'getAuthStorage' 
		], [ ], '', false );
		$objectManager = new ObjectManager ( $this );
		$this->plugin = $objectManager->getObject ( 'Magento\Backend\App\Action\Plugin\Authentication', [ 
				'auth' => $this->auth 
		] );
	}
	...
	public function testAroundDispatchProlongStorage() {
		...
		$user = $this->getMock ( 'Magento\User\Model\User', [ 
				'reload',
				'__wakeup' 
		], [ ], '', false );
		$storage = $this->getMock ( 'Magento\Backend\Model\Auth\Session', [ 
				'prolong',
				'refreshAcl' 
		], [ ], '', false );
		...
		$this->auth->expects ( $this->any () )->method ( 'getUser' )->will ( $this->returnValue ( $user ) );
		$this->auth->expects ( $this->once () )->method ( 'isLoggedIn' )->will ( $this->returnValue ( $loggedIn ) );
		...
		$user->expects ( $this->once () )->method ( 'reload' );
		
		$storage->expects ( $this->at ( 0 ) )->method ( 'prolong' );
		$storage->expects ( $this->at ( 1 ) )->method ( 'refreshAcl' );
	...
	public function testProcessNotLoggedInUser($isIFrameParam, $isAjaxParam, $isForwardedFlag) {
		$subject = $this->getMockBuilder ( 'Magento\Backend\Controller\Adminhtml\Index' )->disableOriginalConstructor ()->getMock ();
		$request = $this->getMockBuilder ( 'Magento\Framework\App\Request\Http' )->disableOriginalConstructor ()->getMock ();
		$storage = $this->getMockBuilder ( 'Magento\Backend\Model\Auth\Session' )->disableOriginalConstructor ()->getMock ();
