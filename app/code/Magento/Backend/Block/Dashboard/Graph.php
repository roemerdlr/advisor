...
class Graph extends \Magento\Backend\Block\Dashboard\AbstractDashboard {
	...
	public function getChartUrl($directUrl = true) {
				...
				if ($idx == 'x') {
					...
					foreach ( $this->_axisLabels [$idx] as $_index => $_label ) {
						if ($_label != '') {
							...
							switch ($this->getDataHelper ()->getParam ( 'period' )) {
								case '24h' :
									$this->_axisLabels [$idx] [$_index] = $this->_localeDate->formatDateTime ( $period->setTime ( $period->format ( 'H' ), 0, 0 ), \IntlDateFormatter::NONE, \IntlDateFormatter::SHORT );
									break;
								case '7d' :
								case '1m' :
									$this->_axisLabels [$idx] [$_index] = $this->_localeDate->formatDateTime ( $period, \IntlDateFormatter::SHORT, \IntlDateFormatter::NONE );
									break;
								case '1y' :
								case '2y' :
		...
		if ($directUrl) {
			...
			return $this->getUrl ( 'adminhtml/*/tunnel', [ 
					'_query' => $params 
			] );
		}
	}
	...
	protected function _prepareData() {
		if ($this->_dataHelper !== null) {
			...
			$this->getDataHelper ()->setParam ( 'period', $period && in_array ( $period, $availablePeriods ) ? $period : '24h' );
		}
	}
}
