...
class Menu extends \Magento\Backend\Block\Template {
	...
	protected function _renderItemCssClass($menuItem, $level) {
		...
		$output = ($this->_isItemActive ( $menuItem, $level ) ? '_current _active' : '') . ' ' . ($menuItem->hasChildren () ? 'parent' : '') . ' ' . $isLast . ' ' . 'level-' . $level;
		return $output;
	}
	...
	protected function _renderAnchor($menuItem, $level) {
		if ($level == 1 && $menuItem->getUrl () == '#') {
			$output = '<strong class="submenu-group-title" role="presentation">' . '<span>' . $this->_getAnchorLabel ( $menuItem ) . '</span>' . '</strong>';
		} else {
			$output = '<a href="' . $menuItem->getUrl () . '" ' . $this->_renderItemAnchorTitle ( $menuItem ) . $this->_renderItemOnclickFunction ( $menuItem ) . ' class="' . $this->_renderAnchorCssClass ( $menuItem, $level ) . '">' . '<span>' . $this->_getAnchorLabel ( $menuItem ) . '</span>' . '</a>';
		}
	...
	protected function _afterToHtml($html) {
		$html = preg_replace_callback ( '#' . \Magento\Backend\Model\UrlInterface::SECRET_KEY_PARAM_NAME . '/\$([^\/].*)/([^\/].*)/([^\$].*)\$#U', [ 
				$this,
				'_callbackSecretKey' 
		], $html );
	...
	public function renderMenu($menu, $level = 0) {
		...
		foreach ( $this->_getMenuIterator ( $menu ) as $menuItem ) {
			$output .= '<li ' . $this->_renderMouseEvent ( $menuItem ) . ' class="' . $this->_renderItemCssClass ( $menuItem, $level ) . '"' . $this->getUiId ( $menuItem->getId () ) . 'role="menuitem">';
