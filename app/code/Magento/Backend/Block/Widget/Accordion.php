...
class Accordion extends \Magento\Backend\Block\Widget {
	...
	public function addItem($itemId, $config) {
		$this->_items [$itemId] = $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Accordion\Item', $this->getNameInLayout () . '-' . $itemId )->setData ( $config )->setAccordion ( $this )->setId ( $itemId );
