...
class Extended extends \Magento\Backend\Block\Widget\Grid implements \Magento\Backend\Block\Widget\Grid\ExportInterface {
	...
	protected function _prepareLayout() {
		$this->setChild ( 'export_button', $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Button' )->setData ( [ 
				'label' => __ ( 'Export' ),
				'onclick' => $this->getJsObjectName () . '.doExport()',
				'class' => 'task' 
		] ) );
		$this->setChild ( 'reset_filter_button', $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Button' )->setData ( [ 
		...
		$this->setChild ( 'search_button', $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Button' )->setData ( [ 
				'label' => __ ( 'Search' ),
	...
	public function addColumn($columnId, $column) {
		if (is_array ( $column )) {
			$this->getColumnSet ()->setChild ( $columnId, $this->getLayout ()->createBlock ( 'Magento\Backend\Block\Widget\Grid\Column\Extended' )->setData ( $column )->setId ( $columnId )->setGrid ( $this ) );
	...
	public function sortColumnsByOrder() {
		foreach ( $this->getColumnsOrder () as $columnId => $after ) {
			$this->getLayout ()->reorderChild ( $this->getColumnSet ()->getNameInLayout (), $this->getColumn ( $columnId )->getNameInLayout (), $this->getColumn ( $after )->getNameInLayout () );
		}
	...
	protected function _prepareMassactionColumn() {
		...
		$this->getColumnSet ()->insert ( $massactionColumn, count ( $this->getColumnSet ()->getColumns () ) + 1, false, $columnId );
		return $this;
	}
	...
	public function getCsvFile() {
		...
		$this->_exportIterateCollection ( '_exportCsvItem', [ 
				$stream 
		] );
...
;
	...
	public function getExcelFile($sheetName = '') {
		...
		$convert = new \Magento\Framework\Convert\Excel ( $this->getCollection ()->getIterator (), [ 
				$this,
				'getRowRecord' 
		] );
