		...
		if ($this->hasData ( 'exportTypes' )) {
			foreach ( $this->getData ( 'exportTypes' ) as $type ) {
				if (! isset ( $type ['urlPath'] ) || ! isset ( $type ['label'] )) {
					throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'Invalid export type supplied for grid export block' ) );
				}
...
;
	...
	public function getExcelFile($sheetName = '') {
		...
		$convert = new \Magento\Framework\Convert\Excel ( $collection->getIterator (), [ 
				$this,
				'getRowRecord' 
		] );
