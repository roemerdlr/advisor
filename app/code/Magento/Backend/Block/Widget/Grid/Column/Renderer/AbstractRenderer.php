...
abstract class AbstractRenderer extends \Magento\Backend\Block\AbstractBlock implements RendererInterface {
	...
	public function render(DataObject $row) {
		if ($this->getColumn ()->getEditable ()) {
			...
			$result .= $this->getColumn ()->getEditOnly () ? '' : '<span class="admin__grid-control-value">' . $this->_getValue ( $row ) . '</span>';
	...
	public function renderHeader() {
		if (false !== $this->getColumn ()->getSortable ()) {
			...
			$out = '<th data-sort="' . $this->getColumn ()->getId () . '" data-direction="' . $nDir . '" class="data-grid-th _sortable ' . $className . ' ' . $this->getColumn ()->getHeaderCssClass () . '"><span>' . $this->getColumn ()->getHeader () . '</span></th>';
		} else {
			$out = '<th class="data-grid-th ' . $this->getColumn ()->getHeaderCssClass () . '"><span>' . $this->getColumn ()->getHeader () . '</span></th>';
		}
		return $out;
	}
