...
class Tabs extends \Magento\Backend\Block\Widget {
	...
	public function setActiveTab($tabId) {
		if (isset ( $this->_tabs [$tabId] ) && $this->canShowTab ( $this->_tabs [$tabId] ) && ! $this->getTabIsHidden ( $this->_tabs [$tabId] )) {
			$this->_activeTab = $tabId;
			...
			foreach ( $tabs as $tabId ) {
				foreach ( $tabs as $tabToId ) {
					if ($tabId !== $tabToId) {
						...
						$this->_tabs [$tabToId]->setData ( 'shadow_tabs', array_merge ( $this->_tabs [$tabToId]->getData ( 'shadow_tabs' ), [ 
								$blockId . '_' . $tabId 
						] ) );
					}
				}
