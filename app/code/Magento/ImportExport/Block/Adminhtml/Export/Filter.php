...
class Filter extends \Magento\Backend\Block\Widget\Grid\Extended {
	...
	protected function _getDateFromToHtmlWithValue(Attribute $attribute, $value) {
		...
		$dateBlock = $this->_layout->createBlock ( 'Magento\Framework\View\Element\Html\Date', '', [ 
				'data' => $arguments 
		] );
		$fromValue = null;
		$toValue = null;
		...
		return '<strong class="admin__control-support-text">' . __ ( 'From' ) . ':</strong>&nbsp;' . $dateBlock->setValue ( $fromValue )->getHtml () . '&nbsp;<strong class="admin__control-support-text">' . __ ( 'To' ) . ':</strong>&nbsp;' . $dateBlock->setId ( $dateBlock->getId () . '_to' )->setValue ( $toValue )->getHtml ();
	}
	...
	protected function _getInputHtmlWithValue(Attribute $attribute, $value) {
		$html = '<input type="text" name="' . $this->getFilterElementName ( $attribute->getAttributeCode () ) . '" class="admin__control-text input-text input-text-export-filter"';
		if ($value) {
	...
	protected function _getMultiSelectHtmlWithValue(Attribute $attribute, $value) {
		...
		if ($size = count ( $options )) {
			...
			$selectBlock = $this->_layout->createBlock ( 'Magento\Framework\View\Element\Html\Select', '', [ 
					'data' => $arguments 
			] );
	...
	protected function _getNumberFromToHtmlWithValue(Attribute $attribute, $value) {
		...
		return '<strong class="admin__control-support-text">' . __ ( 'From' ) . ':</strong>&nbsp;' . '<input type="text" name="' . $name . '[]" class="admin__control-text input-text input-text-range"' . ' value="' . $fromValue . '"/>&nbsp;' . '<strong class="admin__control-support-text">' . __ ( 'To' ) . ':</strong>&nbsp;<input type="text" name="' . $name . '[]" class="admin__control-text input-text input-text-range" value="' . $toValue . '" />';
	}
	 ...
	 * @param Attribute $attribute        	
	 * @param mixed $value        	
	 * @return \Magento\Framework\Phrase|string @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	 ...
	 *
	 * @param Attribute $row        	
	 * @return string|false @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
