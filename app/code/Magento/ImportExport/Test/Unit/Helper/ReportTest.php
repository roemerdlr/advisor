...
class ReportTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->timezone = $this->getMock ( 'Magento\Framework\Stdlib\DateTime\Timezone', [ 
				'date',
				'getConfigTimezone',
				'diff',
				'format' 
		], [ ], '', false );
		$this->varDirectory = $this->getMock ( 'Magento\Framework\Filesystem\Directory\Write', [ 
				'getRelativePath',
				'readFile',
				'isFile',
				'stat' 
		...
		$this->report = $this->objectManagerHelper->getObject ( 'Magento\ImportExport\Helper\Report', [ 
	...
	public function testGetSummaryStats() {
		...
		$importConfig = $this->getMock ( 'Magento\ImportExport\Model\Import\Config', [ 
				'getEntities' 
		], [ ], '', false );
		$importConfig->expects ( $this->any () )->method ( 'getEntities' )->willReturn ( [ 
				'catalog_product' => [ 
						'model' => 'catalog_product' 
				] 
		] );
		$entityFactory = $this->getMock ( 'Magento\ImportExport\Model\Import\Entity\Factory', [ 
				'create' 
		], [ ], '', false );
		$product = $this->getMock ( 'Magento\CatalogImportExport\Model\Import\Product', [ 
				'getEntityTypeCode',
				'setParameters' 
		], [ ], '', false );
