...
class AbstractEavTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_model = $this->getMockForAbstractClass ( 'Magento\ImportExport\Model\Export\Entity\AbstractEav', [ ], '', false, true, true, [ 
		...
		$this->_model->expects ( $this->once () )->method ( '_getExportAttributeCodes' )->will ( $this->returnValue ( $this->_expectedAttributes ) );
	}
	...
	public function testAddAttributesToCollection() {
		...
		$stubCollection = $this->getMock ( 'Magento\Eav\Model\Entity\Collection\AbstractCollection', [ 
	...
	public function testAddAttributeValuesToRow() {
		...
		$testAttribute = $this->getMockForAbstractClass ( 'Magento\Eav\Model\Entity\Attribute\AbstractAttribute', [ ], '', false, false, false, [ 
				'__wakeup' 
		] );
		$testAttribute->setAttributeCode ( $testAttributeCode );
		
		$this->_model->expects ( $this->any () )->method ( 'getAttributeCollection' )->will ( $this->returnValue ( [ 
				$testAttribute 
		] ) );
		...
		$item = $this->getMockForAbstractClass ( 'Magento\Framework\Model\AbstractModel', [ ], '', false, true, true, [ 
				'getData',
				'__wakeup' 
		] );
