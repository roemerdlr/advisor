...
class ExportTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getMageImportExportModelExportMock() {
		...
		$abstractMockEntity = $this->getMockForAbstractClass ( 'Magento\ImportExport\Model\Export\AbstractEntity', [ ], '', false );
		
		/** @var $mockAdapterTest \Magento\ImportExport\Model\Export\Adapter\AbstractAdapter */
		$mockAdapterTest = $this->getMockForAbstractClass ( 'Magento\ImportExport\Model\Export\Adapter\AbstractAdapter', [ ], '', false, true, true, [ 
				'getFileExtension' 
		] );
		$mockAdapterTest->expects ( $this->any () )->method ( 'getFileExtension' )->will ( $this->returnValue ( $this->_exportFileExtension ) );
		
		$logger = $this->getMock ( 'Psr\Log\LoggerInterface' );
		$filesystem = $this->getMock ( 'Magento\Framework\Filesystem', [ ], [ ], '', false );
		$entityFactory = $this->getMock ( 'Magento\ImportExport\Model\Export\Entity\Factory', [ ], [ ], '', false );
		$exportAdapterFac = $this->getMock ( 'Magento\ImportExport\Model\Export\Adapter\Factory', [ ], [ ], '', false );
		/** @var $mockModelExport \Magento\ImportExport\Model\Export */
		$mockModelExport = $this->getMock ( 'Magento\ImportExport\Model\Export', [ 
				'getEntityAdapter',
				'_getEntityAdapter',
				'_getWriter' 
		], [ 
		...
		$mockModelExport->expects ( $this->any () )->method ( 'getEntityAdapter' )->will ( $this->returnValue ( $abstractMockEntity ) );
		$mockModelExport->expects ( $this->any () )->method ( '_getEntityAdapter' )->will ( $this->returnValue ( $abstractMockEntity ) );
