...
class ZipTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->directory = $this->getMockBuilder ( '\Magento\Framework\Filesystem\Directory\Write' )->disableOriginalConstructor ()->setMethods ( [ 
				'getRelativePath' 
		] )->getMock ();
	}
	...
	protected function _invokeConstructor($fileName) {
		try {
			$this->zip = $this->getMockBuilder ( '\Magento\ImportExport\Model\Import\Source\Zip' )->setConstructorArgs ( [ 
					$fileName,
					$this->directory,
					[ ] 
			] )->getMock ();
			
			$reflectedClass = new \ReflectionClass ( '\Magento\ImportExport\Model\Import\Source\Zip' );
