...
class AbstractTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	protected function setUp() {
		...
		$this->_model = $this->getMockBuilder ( 'Magento\ImportExport\Model\Import\Entity\AbstractEntity' )->disableOriginalConstructor ()->setMethods ( [ 
	 ...
	 *
	 * @param array $columns
	 *        	value which will be returned by method getColNames()
	...
	protected function _createSourceAdapterMock(array $columns) {
		...
		$source = $this->getMockForAbstractClass ( 'Magento\ImportExport\Model\Import\AbstractSource', [ ], '', false, true, true, [ 
				'getColNames' 
		] );
	...
	public function testValidateDataEmptyColumnName() {
		...
		$this->assertArrayHasKey ( AbstractEntity::ERROR_CODE_COLUMN_EMPTY_HEADER, $errorAggregator->getRowsGroupedByErrorCode () );
	}
	...
	public function testValidateDataAttributeNames() {
		...
		$errorAggregator = $this->_model->validateData ();
		$this->assertArrayHasKey ( AbstractEntity::ERROR_CODE_COLUMN_NAME_INVALID, $errorAggregator->getRowsGroupedByErrorCode () );
	}
	...
	public function isAttributeValidDataProvider() {
		return [ 
				...
				[ 
						...
						[ 
								'created_at' => '29.02.2012 11:12:59' 
						],
						1,
						true 
				],
