...
class ImportTest extends \Magento\ImportExport\Test\Unit\Model\Import\AbstractImportTestCase {
	...
	public function setUp() {
		...
		$this->_importConfig = $this->getMockBuilder ( '\Magento\ImportExport\Model\Import\Config' )->disableOriginalConstructor ()->setMethods ( [ 
				'getEntityTypeCode',
				'getBehavior',
				'getEntities' 
		...
		$this->_entityAdapter = $this->getMockBuilder ( '\Magento\ImportExport\Model\Import\Entity\AbstractEntity' )->disableOriginalConstructor ()->setMethods ( [ 
				'importData',
				'_saveValidatedBunches',
				'getErrorAggregator' 
		] )->getMockForAbstractClass ();
		$this->_entityAdapter->method ( 'getErrorAggregator' )->willReturn ( $this->getErrorAggregatorObject ( [ 
		...
		$this->historyModel = $this->getMockBuilder ( '\Magento\ImportExport\Model\History' )->disableOriginalConstructor ()->setMethods ( [ 
				'updateReport',
				'invalidateReport',
				'addReport' 
		] )->getMock ();
		...
		$this->import = $this->getMockBuilder ( '\Magento\ImportExport\Model\Import' )->setConstructorArgs ( [ 
				$logger,
				$this->_filesystem,
	...
	public function testImportSource() {
		...
		$this->_entityAdapter->expects ( $this->any () )->method ( 'importData' )->will ( $this->returnSelf () );
		$this->import->expects ( $this->any () )->method ( '_getEntityAdapter' )->will ( $this->returnValue ( $this->_entityAdapter ) );
		$this->_importConfig->expects ( $this->any () )->method ( 'getEntities' )->willReturn ( [ 
				$entityTypeCode => [ 
	...
	public function testIsReportEntityType($entity, $getEntityResult, $expectedResult) {
		$importMock = $this->getMockBuilder ( '\Magento\ImportExport\Model\Import' )->disableOriginalConstructor ()->setMethods ( [ 
				'getEntity',
				'_getEntityAdapter',
				'getEntityTypeCode',
	...
	public function testCreateHistoryReportEmptyReportEntityType() {
		...
		$this->import->expects ( $this->once () )->method ( 'isReportEntityType' )->with ( $entity )->willReturn ( false );
		$this->_varDirectory->expects ( $this->never () )->method ( 'getRelativePath' );
		$this->_varDirectory->expects ( $this->never () )->method ( 'copyFile' );
		$this->dateTime->expects ( $this->never () )->method ( 'gmtTimestamp' );
		$this->historyModel->expects ( $this->never () )->method ( 'addReport' );
	...
	public function testCreateHistoryReportSourceFileRelativeIsArray() {
		...
		$this->_varDirectory->expects ( $this->once () )->method ( 'getRelativePath' )->with ( \Magento\ImportExport\Model\Import::IMPORT_DIR . $fileName )->willReturn ( $sourceFileRelativeNew );
