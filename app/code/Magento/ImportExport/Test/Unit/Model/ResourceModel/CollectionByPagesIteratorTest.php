...
class CollectionByPagesIteratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testIterate() {
		...
		$callbackMock = $this->getMock ( 'stdClass', [ 
				'callback' 
		] );
		...
		$collectionMock = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\AbstractDb' )->setConstructorArgs ( [ 
				$entityFactory,
				$logger,
				$fetchStrategy 
		] )->setMethods ( [ 
		...
		$this->_resourceModel->iterate ( $collectionMock, $pageSize, [ 
				[ 
						$callbackMock,
						'callback' 
				] 
