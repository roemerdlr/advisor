...
class ProcessingErrorAggregator implements ProcessingErrorAggregatorInterface {
	 ...
	 */
	public function __construct(\Magento\ImportExport\Model\Import\ErrorProcessing\ProcessingErrorFactory $errorFactory) {
	...
	public function initValidationStrategy($validationStrategy, $allowedErrorCount = 0) {
		...
		if (! in_array ( $validationStrategy, $allowedStrategy )) {
			throw new \Magento\Framework\Exception\LocalizedException ( __ ( 'ImportExport: Import Data validation - Validation strategy not found' ) );
		}
	...
	public function isErrorLimitExceeded() {
		...
		if ($errorsCount > 0 && $this->validationStrategy == self::VALIDATION_STRATEGY_STOP_ON_ERROR && $errorsCount >= $this->allowedErrorsCount) {
			$isExceeded = true;
		}
	 ...
	 * @return array
	 */
	public function getRowsGroupedByErrorCode(array $errorCode = [], array $excludedCodes = [], $replaceCodeWithMessage = true) {
		$result = [ ];
		foreach ( $this->items as $error ) {
			if ((! empty ( $errorCode ) && in_array ( $error->getErrorCode (), $errorCode )) || in_array ( $error->getErrorCode (), $excludedCodes )) {
				continue;
			}
	 ...
	 * @return int
	 */
	public function getErrorsCount(array $errorLevels = [
        ProcessingError::ERROR_LEVEL_CRITICAL,
        ProcessingError::ERROR_LEVEL_NOT_CRITICAL
        ]) {
		$result = 0;
	...
	protected function processErrorStatistics($errorLevel) {
		if (! empty ( $errorLevel )) {
			isset ( $this->errorStatistics [$errorLevel] ) ? $this->errorStatistics [$errorLevel] ++ : $this->errorStatistics [$errorLevel] = 1;
		}
