...
abstract class AbstractEntity {
	/**
	 * Attribute code to its values.
	 * Only attributes with options and only default store values used.
	 *
	 * @var array
	 */
	 ...
	 */
	public function __construct(\Magento\Framework\Stdlib\DateTime\TimezoneInterface $localeDate, \Magento\Eav\Model\Config $config, ResourceConnection $resource, \Magento\Store\Model\StoreManagerInterface $storeManager) {
	...
	protected function _getExportAttrCodes() {
		if (null === self::$attrCodes) {
			if (! empty ( $this->_parameters [\Magento\ImportExport\Model\Export::FILTER_ELEMENT_SKIP] ) && is_array ( $this->_parameters [\Magento\ImportExport\Model\Export::FILTER_ELEMENT_SKIP] )) {
	 ...
	 * @return \Magento\Eav\Model\Entity\Collection\AbstractCollection @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
			...
			if (isset ( $exportFilter [$attrCode] )) {
				...
				if (\Magento\ImportExport\Model\Export::FILTER_TYPE_SELECT == $attrFilterType) {
					if (is_scalar ( $exportFilter [$attrCode] ) && trim ( $exportFilter [$attrCode] )) {
						$collection->addAttributeToFilter ( $attrCode, [ 
								'eq' => $exportFilter [$attrCode] 
						] );
					}
				} elseif (\Magento\ImportExport\Model\Export::FILTER_TYPE_INPUT == $attrFilterType) {
					if (is_scalar ( $exportFilter [$attrCode] ) && trim ( $exportFilter [$attrCode] )) {
						$collection->addAttributeToFilter ( $attrCode, [ 
								'like' => "%{$exportFilter[$attrCode]}%" 
						] );
					}
	 ...
	 *        	Error code or simply column name
	...
	/**
	 * Returns attributes all values in label-value or value-value pairs form.
		...
		if ($attribute->usesSource ()) {
			...
			try {
				foreach ( $attribute->getSource ()->getAllOptions ( false ) as $option ) {
					foreach ( is_array ( $option ['value'] ) ? $option ['value'] : [ 
							$option 
					] as $innerOption ) {
	...
	public function getErrorMessages() {
		...
		foreach ( $this->_errors as $errorCode => $errorRows ) {
			$message = isset ( $this->_messageTemplates [$errorCode] ) ? __ ( $this->_messageTemplates [$errorCode] ) : __ ( 'Please correct the value for "%1" column.', $errorCode );
