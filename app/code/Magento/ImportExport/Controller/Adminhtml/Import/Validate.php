...
class Validate extends ImportResultController {
	...
	public function execute() {
		...
		if ($data) {
			...
			$source = ImportAdapter::findAdapterFor ( $import->uploadSource (), $this->_objectManager->create ( 'Magento\Framework\Filesystem' )->getDirectoryWrite ( DirectoryList::ROOT ), $data [$import::FIELD_FIELD_SEPARATOR] );
			...
			if (! $import->getProcessedRowsCount ()) {
				...
				if (! $validationResult) {
					$resultBlock->addError ( __ ( 'Data validation is failed. Please fix errors and re-upload the file..' ) );
