...
class Update extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	 ...
	 */
	public function __construct(\Magento\Framework\Model\ResourceModel\Db\Context $context, \Magento\Framework\Cache\FrontendInterface $cache, $connectionName = null) {
	...
	protected function _getFetchUpdatesByHandleSelect($loadAllUpdates = false) {
		...
		], 'link.layout_update_id=layout_update.layout_update_id', '' )->where ( 'link.store_id IN (0, :store_id)' )->where ( 'link.theme_id = :theme_id' )->where ( 'layout_update.handle = :layout_update_handle' )->order ( 'layout_update.sort_order ' . \Magento\Framework\DB\Select::SQL_ASC );
	...
	protected function _afterSave(\Magento\Framework\Model\AbstractModel $object) {
		...
		if (isset ( $data ['store_id'] ) && isset ( $data ['theme_id'] )) {
			$this->getConnection ()->insertOnDuplicate ( $this->getTable ( 'layout_link' ), [ 
