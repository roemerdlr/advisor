...
class Instance extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
	...
	protected function _afterSave(AbstractModel $object) {
		...
		if (is_array ( $pageIds ) && count ( $pageIds ) > 0) {
			$inCond = $connection->prepareSqlCondition ( 'page_id', [ 
					'in' => $pageIds 
			] );
	...
	protected function _deleteLayoutUpdates($layoutUpdateIds) {
		$connection = $this->getConnection ();
		if ($layoutUpdateIds) {
			$inCond = $connection->prepareSqlCondition ( 'layout_update_id', [ 
					'in' => $layoutUpdateIds 
			] );
	...
	public function lookupStoreIds($id) {
		$connection = $this->getConnection ();
		$select = $connection->select ()->from ( $this->getMainTable (), 'store_ids' )->where ( "{$this->getIdFieldName()} = ?", ( int ) $id );
