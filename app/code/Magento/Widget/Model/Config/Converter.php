...
class Converter implements \Magento\Framework\Config\ConverterInterface {
				...
				switch ($widgetSubNode->nodeName) {
					...
					case 'containers' :
						...
						foreach ( $widgetSubNode->childNodes as $container ) {
							...
							$widgetArray ['supported_containers'] = array_merge ( $widgetArray ['supported_containers'], $this->_convertContainer ( $container ) );
						}
						break;
					case "#text" :
						break;
					case '#comment' :
						break;
					default :
						throw new \LogicException ( sprintf ( "Unsupported child xml node '%s' found in the 'widget' node", $widgetSubNode->nodeName ) );
				}
			}
	...
	protected function _convertContainer($source) {
		...
		foreach ( $source->childNodes as $containerTemplate ) {
			...
			$template [$templateAttributes->getNamedItem ( 'name' )->nodeValue] = $templateAttributes->getNamedItem ( 'value' )->nodeValue;
		}
		$supportedContainers [] = [ 
	 ...
	 * @param \DOMNode $source        	
	 * @return array
	 * @throws \LogicException @SuppressWarnings(PHPMD.NPathComplexity)
	 *         @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	protected function _convertDepends($source) {
		...
		foreach ( $source->childNodes as $childNode ) {
			...
			if ($childNode->nodeName !== 'parameter') {
				throw new \LogicException ( sprintf ( "Only 'parameter' node can be child of 'depends' node, %s found", $childNode->nodeName ) );
			}
	...
	protected function _convertBlock($source) {
		...
		foreach ( $source->childNodes as $blockSubNode ) {
			...
			if ($blockSubNode->nodeName !== 'data') {
				throw new \LogicException ( sprintf ( "Only 'data' node can be child of 'block' node, %s found", $blockSubNode->nodeName ) );
			}
