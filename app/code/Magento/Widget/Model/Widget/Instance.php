...
class Instance extends \Magento\Framework\Model\AbstractModel {
		...
		if ($pageGroups) {
			foreach ( $pageGroups as $pageGroup ) {
				if (isset ( $pageGroup [$pageGroup ['page_group']] )) {
					...
					if (in_array ( $pageGroup ['page_group'], [ 
							'pages',
							'page_layouts' 
					] )) {
	...
	/**
	 * Return widget instance code.
	 * If not set, derive value from type (namespace\class).
	 *
	 * @return string
	 */
	...
	public function getWidgetConfigAsArray() {
		if ($this->_widgetConfigXml === null) {
			...
			if ($this->_widgetConfigXml) {
				$configFile = $this->_viewFileSystem->getFilename ( 'widget.xml', [ 
						'area' => $this->getArea (),
						'theme' => $this->getThemeId (),
						'module' => $this->_namespaceResolver->determineOmittedNamespace ( preg_replace ( '/^(.+?)\/.+$/', '\\1', $this->getType () ), true ) 
				] );
	...
	public function generateLayoutUpdateXml($container, $templatePath = '') {
		$templateFilename = $this->_viewFileSystem->getTemplateFileName ( $templatePath, [ 
				'area' => $this->getArea (),
