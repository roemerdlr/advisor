...
class Layout extends \Magento\Backend\Block\Template implements \Magento\Framework\Data\Form\Element\Renderer\RendererInterface {
	 ...
	 * @param array $data        	
	 */
	public function __construct(\Magento\Backend\Block\Template\Context $context, \Magento\Catalog\Model\Product\Type $productType, array $data = []) {
	...
	public function getCategoriesChooserUrl() {
		return $this->getUrl ( 'adminhtml/*/categories', [ 
				'_current' => true 
		] );
	}
	...
	public function getProductsChooserUrl() {
		return $this->getUrl ( 'adminhtml/*/products', [ 
				'_current' => true 
		] );
	}
	...
	public function getBlockChooserUrl() {
		return $this->getUrl ( 'adminhtml/*/blocks', [ 
				'_current' => true 
		] );
	}
	...
	public function getTemplateChooserUrl() {
		return $this->getUrl ( 'adminhtml/*/template', [ 
				'_current' => true 
		] );
	}
	...
	public function getDisplayOnSelectHtml() {
		$selectBlock = $this->getLayout ()->createBlock ( 'Magento\Framework\View\Element\Html\Select' )->setName ( 'widget_instance[<%- data.id %>][page_group]' )->setId ( 'widget_instance[<%- data.id %>][page_group]' )->setClass ( 'required-entry page_group_select select' )->setExtraParams ( "onchange=\"WidgetInstance.displayPageGroup(this.value+\'_<%- data.id %>\')\"" )->setOptions ( $this->_getDisplayOnOptions () );
	 ...
	 *
	 * @return array @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	public function getDisplayOnContainers() {
		...
		foreach ( $this->_productType->getTypes () as $typeId => $type ) {
			$container [$typeId] = [ 
					...
					'layout_handle' => str_replace ( '{{TYPE}}', $typeId, \Magento\Widget\Model\Widget\Instance::PRODUCT_TYPE_LAYOUT_HANDLE ),
					'is_anchor_only' => '',
	...
	public function getLayoutsChooser() {
		$chooserBlock = $this->getLayout ()->createBlock ( 'Magento\Widget\Block\Adminhtml\Widget\Instance\Edit\Chooser\Layout' )->setName ( 'widget_instance[<%- data.id %>][pages][layout_handle]' )->setId ( 'layout_handle' )->setClass ( 'required-entry select' )->setExtraParams ( "onchange=\"WidgetInstance.loadSelectBoxByType(\'block_reference\', " . "this.up(\'div.pages\'), this.value)\"" )->setArea ( $this->getWidgetInstance ()->getArea () )->setTheme ( $this->getWidgetInstance ()->getThemeId () );
	...
	public function getPageLayoutsPageChooser() {
		$chooserBlock = $this->getLayout ()->createBlock ( 'Magento\Widget\Block\Adminhtml\Widget\Instance\Edit\Chooser\DesignAbstraction' )->setName ( 'widget_instance[<%- data.id %>][page_layouts][layout_handle]' )->setId ( 'layout_handle' )->setClass ( 'required-entry select' )->setExtraParams ( "onchange=\"WidgetInstance.loadSelectBoxByType(\'block_reference\', " . "this.up(\'div.pages\'), this.value)\"" )->setArea ( $this->getWidgetInstance ()->getArea () )->setTheme ( $this->getWidgetInstance ()->getThemeId () );
		return $chooserBlock->toHtml ();
	}
