...
abstract class AbstractContainerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->eventManagerMock = $this->getMockBuilder ( 'Magento\Framework\Event\Manager' )->setMethods ( [ 
				'dispatch' 
		...
		$this->themeCollectionMock = $this->getMockBuilder ( 'Magento\Theme\Model\ResourceModel\Theme\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'getItemById' 
		] )->getMock ();
		$this->themeMock = $this->getMockBuilder ( 'Magento\Theme\Model\Theme' )->disableOriginalConstructor ()->getMock ();
		
		$this->layoutProcessorFactoryMock = $this->getMock ( 'Magento\Framework\View\Layout\ProcessorFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->layoutMergeMock = $this->getMockBuilder ( 'Magento\Framework\View\Model\Layout\Merge' )->setMethods ( [ 
				'addPageHandles',
				'load',
				'getContainers' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->contextMock = $this->getMockBuilder ( 'Magento\Backend\Block\Context' )->setMethods ( [ 
				'getEventManager',
				'getScopeConfig',
				'getEscaper' 
		] )->disableOriginalConstructor ()->getMock ();
