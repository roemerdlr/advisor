...
class ChosenOption extends SimpleElement {
	...
	public function setValue($value) {
		...
		if (isset ( $value ['filter_url_key'] )) {
			$this->getClassBlock ( $this->chooserClasses ['page'] )->searchAndOpen ( [ 
					'chooser_identifier' => $value ['filter_url_key'] 
			] );
		}
		if (isset ( $value ['filter_identifier'] )) {
			$this->getClassBlock ( $this->chooserClasses ['page'] )->searchAndOpen ( [ 
					'chooser_identifier' => $value ['filter_identifier'] 
			] );
		}
		if (isset ( $value ['category_path'] )) {
			if (isset ( $value ['filter_sku'] )) {
				$this->getClassBlock ( $this->chooserClasses ['category'] )->selectCategoryByName ( $value ['category_path'] );
				$this->getClassBlock ( $this->chooserClasses ['product'] )->searchAndOpen ( [ 
						'chooser_sku' => $value ['filter_sku'] 
				] );
			} else {
				$this->getClassBlock ( $this->chooserClasses ['category'] )->selectCategoryByName ( $value ['category_path'] );
			}
	...
	protected function waitLoader() {
		...
		$this->waitUntil ( function () use($browser, $loaderSelector) {
	...
	protected function getClassBlock($class) {
		return \Magento\Mtf\ObjectManager::getInstance ()->create ( $class, [ 
