...
abstract class Conditions extends Curl {
	...
	protected function prepareCondition($conditions) {
		$decodeConditions = empty ( $conditions ) ? $this->decodeValue ( "[Conditions combination]" ) : $this->decodeValue ( "{Conditions combination:[{$conditions}]}" );
	...
	private function convertConditionsCombination($combination, $conditions, $nesting) {
		$combination = [ 
				$nesting => $this->convertSingleCondition ( $combination ) 
		];
