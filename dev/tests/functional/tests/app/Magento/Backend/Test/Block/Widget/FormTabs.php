...
class FormTabs extends Form {
	...
	protected function fillMissedFields() {
		...
		if (! empty ( $this->unassignedFields )) {
			throw new \Exception ( 'Could not find all elements on the tabs: ' . implode ( ', ', array_keys ( $this->unassignedFields ) ) );
		}
	}
	...
	public function getTab($tabName) {
		...
		/** @var Tab $tab */
		$tab = $this->blockFactory->create ( $tabClass, [ 
	...
	protected function getTabElement($tabName) {
		...
		$strategy = isset ( $this->tabs [$tabName] ['strategy'] ) ? $this->tabs [$tabName] ['strategy'] : Locator::SELECTOR_CSS;
