...
class Footer extends Block {
	...
	public function selectStoreGroup(Store $store) {
		...
		$storeGroup = $this->_rootElement->find ( sprintf ( $this->storeGroupSelector, $storeGroupName ), Locator::SELECTOR_XPATH );
	...
	public function isStoreGroupVisible(Store $store) {
		$storeGroupName = explode ( "/", $store->getGroupId () ) [1];
		$this->_rootElement->find ( $this->storeGroupSwitch )->click ();
		return $this->_rootElement->find ( sprintf ( $this->storeGroupSelector, $storeGroupName ), Locator::SELECTOR_XPATH )->isVisible ();
	}
