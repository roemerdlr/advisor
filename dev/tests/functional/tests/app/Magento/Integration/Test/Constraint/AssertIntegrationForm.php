...
class AssertIntegrationForm extends AbstractAssertForm {
	 ...
	 * @param bool $strictResourcesVerify
	 *        	[optional]
	 * @return void
	 */
	public function processAssert(IntegrationIndex $integrationIndexPage, IntegrationNew $integrationNewPage, Integration $integration, Integration $initialIntegration = null, $strictResourcesVerify = false) {
	...
	protected function verifyForm(array $formData, array $fixtureData) {
		...
		foreach ( $fixtureData as $key => $value ) {
			if (in_array ( $key, $this->skippedFields )) {
				...
				$errorMessages = array_merge ( $errorMessages, $this->checkResources ( $formData [$key], $fixtureData [$key] ) );
	...
	protected function getResourcesDifferentData(array $formData, $fixtureData) {
		$fixtureData = is_array ( $fixtureData ) ? $fixtureData : [ 
				$fixtureData 
		];
		return $this->strictResourcesVerify ? array_diff ( $formData, $fixtureData ) : $this->notStrictVerification ( $formData, $fixtureData );
	}
