...
class AssertRefundedGrandTotalOnFrontend extends AbstractAssertOrderOnFrontend {
	 ...
	 * @return void
	 */
	public function processAssert(OrderHistory $orderHistory, OrderInjectable $order, CustomerOrderView $customerOrderView, CreditMemoView $creditMemoView, array $ids) {
		...
		foreach ( $ids ['creditMemoIds'] as $key => $creditMemoId ) {
			\PHPUnit_Framework_Assert::assertEquals ( number_format ( $order->getPrice () [$key] ['grand_creditmemo_total'], 2 ), $creditMemoView->getCreditMemoBlock ()->getItemBlock ( $creditMemoId )->getGrandTotal () );
		}
	}
