...
class MoveShoppingCartProductsOnOrderPageTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(CustomerAccountLogout $customerAccountLogout, CatalogProductView $catalogProductView, CustomerIndex $customerIndex, CustomerIndexEdit $customerIndexEdit, BrowserInterface $browser, OrderCreateIndex $orderCreateIndex) {
	...
	public function test(Customer $customer, $product) {
		// Preconditions
		// Create product
		$product = $this->objectManager->create ( '\Magento\Catalog\Test\TestStep\CreateProductStep', [ 
				'product' => $product 
		] )->run () ['product'];
		...
		$this->orderCreateIndex->getCustomerActivitiesBlock ()->getShoppingCartItemsBlock ()->addProductsToOrder ( [ 
				$product 
		] );
