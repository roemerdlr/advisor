...
class CreateInvoiceStep implements TestStepInterface {
	 ...
	 *        	[optional]
	 */
	public function __construct(OrderIndex $orderIndex, SalesOrderView $salesOrderView, OrderInvoiceNew $orderInvoiceNew, OrderInvoiceView $orderInvoiceView, OrderInjectable $order, OrderShipmentView $orderShipmentView, $data = null) {
	...
	public function run() {
		...
		if (! empty ( $this->data )) {
			$this->orderInvoiceNew->getFormBlock ()->fillProductData ( $this->data, $this->order->getEntityId () ['products'] );
