...
class AssertOrderInOrdersGrid extends AbstractConstraint {
	...
	public function assert(OrderInjectable $order, OrderIndex $orderIndex, $status, $orderId = '') {
		...
		\PHPUnit_Framework_Assert::assertTrue ( $orderIndex->getSalesOrderGrid ()->isRowVisible ( array_filter ( $filter ) ), 'Order with following data \'' . $errorMessage . '\' is absent in Orders grid.' );
	}
