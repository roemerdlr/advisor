 ...
 * Preconditions:
 * 1.
 * Custom Order Status is created.
...
class UpdateCustomOrderStatusTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(OrderStatusIndex $orderStatusIndex, OrderStatusEdit $orderStatusEdit, OrderIndex $orderIndex) {
	...
	public function test(OrderStatus $orderStatusInitial, OrderStatus $orderStatus, OrderInjectable $order, FixtureFactory $fixtureFactory, $orderExist) {
			...
			$config = $fixtureFactory->createByCode ( 'configData', [ 
					...
					'data' => [ 
							'payment/checkmo/order_status' => [ 
									'value' => $orderStatusInitial->getStatus () 
							] 
					] 
			] );
			$config->persist ();
		...
		$this->orderStatusIndex->getOrderStatusGrid ()->searchAndOpen ( [ 
				'label' => $orderStatusInitial->getLabel () 
		] );
		...
		$orderStatus = $fixtureFactory->createByCode ( 'orderStatus', [ 
	...
	public function tearDown() {
		if ($this->order->hasData ( 'id' )) {
			$this->orderIndex->open ()->getSalesOrderGrid ()->massaction ( [ 
					[ 
							'id' => $this->order->getId () 
					] 
			], 'Cancel' );
