...
class MoveRecentlyViewedProductsOnOrderPageTest extends Injectable {
	...
	public function test(Customer $customer, $products) {
		// Preconditions
		$products = $this->objectManager->create ( 'Magento\Catalog\Test\TestStep\CreateProductsStep', [ 
				'products' => $products 
		] )->run () ['products'];
		$this->objectManager->create ( 'Magento\Catalog\Test\TestStep\OpenProductsOnFrontendStep', [ 
				'products' => $products 
		] )->run ();
		
		// Steps
		$this->objectManager->create ( 'Magento\Customer\Test\TestStep\OpenCustomerOnBackendStep', [ 
				'customer' => $customer 
		] )->run ();
		...
		$this->objectManager->create ( 'Magento\Sales\Test\TestStep\AddRecentlyViewedProductsToCartStep', [ 
				'products' => $products 
		] )->run ();
		$this->objectManager->create ( 'Magento\Sales\Test\TestStep\ConfigureProductsStep', [ 
				'products' => $products 
		] )->run ();
