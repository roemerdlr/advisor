...
class CreateCreditMemoEntityTest extends Injectable {
	...
	public function __prepare(FixtureFactory $fixtureFactory) {
		...
		$setupConfigurationStep = $this->objectManager->create ( 'Magento\Config\Test\TestStep\SetupConfigurationStep', [ 
	...
	public function test(OrderInjectable $order, array $data) {
		// Preconditions
		$order->persist ();
		$this->objectManager->create ( 'Magento\Sales\Test\TestStep\CreateInvoiceStep', [ 
				'order' => $order 
		] )->run ();
		
		// Steps
		$createCreditMemoStep = $this->objectManager->create ( 'Magento\Sales\Test\TestStep\CreateCreditMemoStep', [ 
				'order' => $order,
				'data' => $data 
		] );
		...
		return [ 
				'ids' => [ 
						'creditMemoIds' => $result ['creditMemoIds'] 
				],
	...
	protected function getProduct(OrderInjectable $order, array $data, $index = 0) {
		...
		return $this->fixtureFactory->create ( get_class ( $product ), [ 
				'data' => $productData 
		] );
	}
}
