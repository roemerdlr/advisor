...
class Result extends Block {
	...
	public function isVisibleMessages($text) {
		return $this->_rootElement->find ( sprintf ( $this->messagesSelector, $text ), Locator::SELECTOR_XPATH )->isVisible ();
	}
	...
	public function getSearchSummaryItems() {
		...
		while ( $element->find ( sprintf ( $this->searchSummaryItems, $index ), Locator::SELECTOR_XPATH )->isVisible () ) {
			...
			$childIndex = 1;
			while ( $parentElement->find ( sprintf ( $this->searchSummaryItem, $childIndex ), Locator::SELECTOR_XPATH )->isVisible () ) {
				$result [] = $parentElement->find ( sprintf ( $this->searchSummaryItem, $childIndex ), Locator::SELECTOR_XPATH )->getText ();
				++ $childIndex;
			}
			++ $index;
		}
