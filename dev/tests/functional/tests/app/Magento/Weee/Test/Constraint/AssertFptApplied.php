...
class AssertFptApplied extends AbstractConstraint {
	 ...
	 * @return void
	 */
	public function processAssert(CatalogProductSimple $product, CmsIndex $cmsIndex, CatalogCategoryView $catalogCategoryView, CatalogProductView $catalogProductView, CheckoutCart $checkoutCart, array $prices) {
		$this->cmsIndex = $cmsIndex;
		$this->catalogCategoryView = $catalogCategoryView;
		$this->catalogProductView = $catalogProductView;
		$this->checkoutCart = $checkoutCart;
		$this->fptLabel = $product->getDataFieldConfig ( 'attribute_set_id' ) ['source']->getAttributeSet ()->getDataFieldConfig ( 'assigned_attributes' ) ['source']->getAttributes () [0]->getFrontendLabel ();
		$this->clearShoppingCart ();
		$actualPrices = $this->getPrices ( $product );
		// Prices verification
		\PHPUnit_Framework_Assert::assertEquals ( $prices, $actualPrices, 'Prices on front should be equal to defined in dataset' );
	}
