...
class AssertShipmentInShipmentsTab extends AbstractConstraint {
	...
	public function processAssert(SalesOrderView $salesOrderView, OrderIndex $orderIndex, OrderInjectable $order, array $ids) {
		...
		foreach ( $ids ['shipmentIds'] as $key => $shipmentId ) {
			...
			\PHPUnit_Framework_Assert::assertTrue ( $salesOrderView->getOrderForm ()->getTab ( 'shipments' )->getGridBlock ()->isRowVisible ( $filter ), 'Shipment is absent on shipments tab.' );
		}
	}
