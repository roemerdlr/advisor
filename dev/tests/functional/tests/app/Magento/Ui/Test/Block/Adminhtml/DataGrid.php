...
class DataGrid extends Grid {
	...
	protected function openFilterBlock() {
		...
		if ($toggleFilterButton->isVisible () && ! $searchButton->isVisible ()) {
			...
			$browser = $this->_rootElement;
			$browser->waitUntil ( function () use($searchButton) {
	...
	public function massaction(array $items, $action, $acceptAlert = false, $massActionSelection = '') {
		...
		if ($massActionSelection) {
			...
			$this->_rootElement->find ( sprintf ( $this->massActionToggleList, $massActionSelection ), Locator::SELECTOR_XPATH )->click ();
		}
		$actionType = is_array ( $action ) ? key ( $action ) : $action;
		$this->_rootElement->find ( $this->massActionButton )->click ();
		$this->_rootElement->find ( sprintf ( $this->massActionToggleList, $actionType ), Locator::SELECTOR_XPATH )->click ();
		if (is_array ( $action )) {
			$this->_rootElement->find ( sprintf ( $this->massActionToggleList, end ( $action ) ), Locator::SELECTOR_XPATH )->click ();
		}
		if ($acceptAlert) {
