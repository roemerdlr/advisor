...
class AssertTermRequireMessageOnMultishippingCheckout extends AbstractConstraint {
	...
	public function processAssert(MultishippingCheckoutOverview $page, TestStepFactory $stepFactory, $products, $payment, $shipping) {
		$customer = [ 
				'customer' => [ 
						'dataset' => 'johndoe_with_multiple_addresses' 
				] 
		];
		$customer = $stepFactory->create ( '\Magento\Customer\Test\TestStep\CreateCustomerStep', $customer )->run ();
		$products = $stepFactory->create ( '\Magento\Catalog\Test\TestStep\CreateProductsStep', [ 
				'products' => $products 
		] )->run ();
		...
		$stepFactory->create ( '\Magento\Multishipping\Test\TestStep\FillShippingInformationStep', array_merge ( [ 
				'shippingMethod' => $shipping 
		], $customer ) )->run ();
		$stepFactory->create ( '\Magento\Multishipping\Test\TestStep\SelectPaymentMethodStep', [ 
				'payment' => $payment 
		] )->run ();
		$stepFactory->create ( '\Magento\CheckoutAgreements\Test\TestStep\CheckTermOnMultishippingStep', [ 
				'agreementValue' => 'No' 
		] )->run ();
