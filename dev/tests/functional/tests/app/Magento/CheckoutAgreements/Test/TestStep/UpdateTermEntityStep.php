...
class UpdateTermEntityStep implements TestStepInterface {
	 ...
	 */
	public function __construct(CheckoutAgreementIndex $agreementIndex, CheckoutAgreementNew $agreementNew, CheckoutAgreement $agreement, CheckoutAgreement $agreementUpdated) {
	...
	public function run() {
		...
		$this->agreementIndex->getAgreementGridBlock ()->searchAndOpen ( [ 
				'name' => $this->agreement->getName () 
		] );
		$this->agreementNew->getAgreementsForm ()->fill ( $this->agreementUpdated );
		$this->agreementNew->getPageActionsBlock ()->save ();
		return [ 
				'agreement' => $this->agreementUpdated 
		];
	}
}
