 ...
 * Preconditions:
 * 1.
 * Admin user with assigned full access role is created.
...
class UpdateAdminUserEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(UserIndex $userIndex, UserEdit $userEdit, Dashboard $dashboard, AdminAuthLogin $adminAuth, FixtureFactory $fixtureFactory) {
	...
	public function testUpdateAdminUser(User $initialUser, User $user, $loginAsDefaultAdmin) {
		...
		return [ 
				'user' => $this->mergeUsers ( $initialUser, $user ) 
		];
	}
	...
	protected function mergeUsers(User $initialUser, User $user) {
		...
		if (isset ( $data ['role_id'] )) {
			$data ['role_id'] = [ 
					'role' => ($user->hasData ( 'role_id' )) ? $user->getDataFieldConfig ( 'role_id' ) ['source']->getRole () : $initialUser->getDataFieldConfig ( 'role_id' ) ['source']->getRole () 
			];
		}
		
		return $this->fixtureFactory->createByCode ( 'user', [ 
				'data' => $data 
		] );
	}
