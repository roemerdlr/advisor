...
class UpdateAdminUserRoleEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(UserRoleIndex $rolePage, UserRoleEditRole $userRoleEditRole, AdminAuthLogin $adminAuthLogin, Dashboard $dashboard) {
	...
	public function testUpdateAdminUserRolesEntity(Role $roleInit, Role $role, User $user) {
		...
		return [ 
				'user' => $role->hasData ( 'in_role_users' ) ? $role->getDataFieldConfig ( 'in_role_users' ) ['source']->getAdminUsers () [0] : $user 
		];
	}
