...
class Attribute extends Form {
	...
	public function getAttributesGrid() {
		return $this->blockFactory->create ( 'Magento\ConfigurableProduct\Test\Block\Adminhtml\Product\AttributesGrid', [ 
	...
	protected function createNewVariationSet(array $attribute) {
		$attributeFixture = ObjectManager::getInstance ()->create ( 'Magento\Catalog\Test\Fixture\CatalogProductAttribute', [ 
				'data' => $attribute 
		] );
	...
	protected function waitBlock($selector, $browser = null, $strategy = Locator::SELECTOR_CSS) {
		...
		return $browser->waitUntil ( function () use($browser, $selector, $strategy) {
	...
	protected function fillOptions(array $attribute) {
		$attributeBlock = $this->browser->find ( sprintf ( $this->attributeBlockByName, $attribute ['frontend_label'] ), Locator::SELECTOR_XPATH );
		$count = 0;
		...
		foreach ( $attribute ['options'] as $option ) {
			...
			$count ++;
			$optionContainer = $attributeBlock->find ( sprintf ( $this->optionContainerByNumber, $count ), Locator::SELECTOR_XPATH );
	...
	protected function updateOptions(array $attribute) {
		...
		foreach ( $attribute ['options'] as $option ) {
			...
			if (! $optionContainer->isVisible ()) {
				...
				$this->getElement ( $optionContainer, $mapping ['label'] )->setValue ( $mapping ['label'] ['value'] );
				$this->getTemplateBlock ()->waitLoader ();
	...
	protected function isVisibleOption(SimpleElement $attributeBlock, $number) {
		return $attributeBlock->find ( sprintf ( $this->optionContainerByNumber, $number ), Locator::SELECTOR_XPATH )->isVisible ();
	}
	...
	protected function getEditAttributeForm() {
		return $this->blockFactory->create ( 'Magento\Catalog\Test\Block\Adminhtml\Product\Attribute\AttributeForm', [ 
	...
	public function getAttributeSelector() {
		return $this->_rootElement->find ( $this->variationSearchBlock, Locator::SELECTOR_CSS, 'Magento\ConfigurableProduct\Test\Block\Adminhtml\Product\Edit\Tab\Variations\Config\Attribute' . '\AttributeSelector' );
	}
	...
	protected function isExistAttribute($label) {
		return $this->_rootElement->find ( sprintf ( $this->attributeBlockByName, $label ), Locator::SELECTOR_XPATH )->isVisible ();
	}
	...
	protected function getTemplateBlock() {
		return $this->blockFactory->create ( 'Magento\Backend\Test\Block\Template', [ 
