...
class ConfigurableAttributesData extends DataSource {
	...
	protected function prepareAttributes(array $data) {
		...
		foreach ( $data ['attributes'] as $key => $attribute ) {
			if (is_string ( $attribute )) {
				...
				$attribute = $this->fixtureFactory->createByCode ( $fixture, [ 
						'dataset' => $dataset 
				] );
			}
	...
	protected function prepareAttributesData(array $data) {
		...
		$this->attributesData = array_replace_recursive ( isset ( $data ['attributes_data'] ) ? $data ['attributes_data'] : [ ], $this->attributesData );
	}
	...
	protected function prepareProducts(array $data) {
		...
		if (! empty ( $this->attributes )) {
			$attributeSetData ['attribute_set_id'] = [ 
					'attribute_set' => $this->createAttributeSet () 
			];
		}
		...
		foreach ( $data ['products'] as $key => $product ) {
			if (is_string ( $product )) {
				...
				$attributeData = [ 
						'attributes' => $this->getProductAttributeData ( $key ) 
				];
				$stock = [ ];
				...
				$product = $this->fixtureFactory->createByCode ( $fixture, [ 
						'dataset' => $dataset,
	...
	protected function createAttributeSet() {
		$attributeSet = $this->fixtureFactory->createByCode ( 'catalogAttributeSet', [ 
	...
	protected function getAttributeOptionId($compositeKey) {
		...
		return isset ( $this->attributesData [$attributeKey] ['options'] [$optionKey] ['id'] ) ? $this->attributesData [$attributeKey] ['options'] [$optionKey] ['id'] : null;
	}
	 ...
	 *
	 * @param array $data        	
	 * @return void @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	protected function prepareVariationsMatrix(array $data) {
		...
		$this->variationsMatrix = isset ( $data ['matrix'] ) ? array_replace_recursive ( $variationsMatrix, $data ['matrix'] ) : $variationsMatrix;
		...
		foreach ( $this->variationsMatrix as $key => $row ) {
			if (isset ( $this->products [$key] )) {
				...
				$this->variationsMatrix [$key] = array_replace_recursive ( $this->variationsMatrix [$key], [ 
						'weight' => 1,
						'quantity_and_stock_status' => [ 
								'qty' => 10 
						] 
	...
	protected function prepareData() {
		...
		foreach ( $this->attributesData as $attributeKey => $attribute ) {
			...
			$attribute ['label'] = isset ( $attribute ['label'] ) ? $attribute ['label'] : (isset ( $attribute ['frontend_label'] ) ? $attribute ['frontend_label'] : null);
