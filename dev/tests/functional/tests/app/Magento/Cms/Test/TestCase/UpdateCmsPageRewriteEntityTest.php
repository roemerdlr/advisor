...
class UpdateCmsPageRewriteEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(UrlRewriteIndex $urlRewriteIndex, UrlRewriteEdit $urlRewriteEdit, StoreIndex $storeIndex, StoreNew $storeNew, StoreDelete $storeDelete) {
	...
	public function test(UrlRewrite $urlRewrite, UrlRewrite $cmsPageRewrite) {
		...
		$filter = [ 
				'request_path' => $cmsPageRewrite->getRequestPath () 
		];
		...
		return [ 
				'cmsPage' => $cmsPageRewrite->getDataFieldConfig ( 'target_path' ) ['source']->getEntity () 
		];
	}
