...
class Links extends Form {
	...
	public function getRowBlock($index, SimpleElement $element = null) {
		...
		return $this->blockFactory->create ( 'Magento\Downloadable\Test\Block\Adminhtml\Catalog\Product\Edit\Tab\Downloadable\LinkRow', [ 
	...
	public function fillLinks(array $fields, SimpleElement $element = null) {
		...
		$mapping = $this->dataMapping ( [ 
				'title' => $fields ['title'],
				'links_purchased_separately' => $fields ['links_purchased_separately'] 
		] );
		$this->_fill ( $mapping );
	...
	public function getDataLinks(array $fields = null, SimpleElement $element = null) {
		...
		foreach ( $fields ['downloadable'] ['link'] as $index => $link ) {
			$newFields ['downloadable'] ['link'] [$index] = $this->getRowBlock ( $index, $element )->getDataLinkRow ( $link );
		}
		return $newFields;
	}
