...
class Samples extends Form {
	...
	public function getRowBlock($index, SimpleElement $element = null) {
		...
		return $this->blockFactory->create ( 'Magento\Downloadable\Test\Block\Adminhtml\Catalog\Product\Edit\Tab\Downloadable\SampleRow', [ 
	...
	public function getDataSamples(array $fields = null, SimpleElement $element = null) {
		...
		foreach ( $fields ['downloadable'] ['sample'] as $index => $sample ) {
			$newFields ['downloadable'] ['sample'] [$index] = $this->getRowBlock ( $index, $element )->getDataSampleRow ( $sample );
		}
		return $newFields;
	}
}
