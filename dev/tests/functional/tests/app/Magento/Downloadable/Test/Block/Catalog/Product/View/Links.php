...
class Links extends Block {
	...
	public function fill(array $data) {
		foreach ( $data as $linkData ) {
			$link = $this->_rootElement->find ( sprintf ( $this->linkByLabel, $linkData ['label'] ), Locator::SELECTOR_XPATH, 'checkbox' );
	...
	public function getLinks() {
		...
		foreach ( $choiceLinks as $choiceLink ) {
			...
			$linkData = [ 
					'links_purchased_separately' => $choiceLink->find ( $this->separatelyForChoice )->isVisible () ? 'Yes' : 'No',
					...
					'price_adjustments' => $priceAdjustments->isVisible () ? $this->escapePrice ( $priceAdjustments->getText () ) : null 
			];
