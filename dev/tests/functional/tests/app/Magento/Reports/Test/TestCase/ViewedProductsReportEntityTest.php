...
class ViewedProductsReportEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(ProductReportView $productReportView, FixtureFactory $fixtureFactory, BrowserInterface $browser, CatalogProductIndex $catalogProductIndexPage) {
	...
	protected function prepareProducts($productList) {
		...
		foreach ( $productsData as $productConfig ) {
			...
			$productFixture = $this->fixtureFactory->createByCode ( $product [0], [ 
					'dataset' => $product [1] 
			] );
