...
class SalesTaxReportEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(OrderIndex $orderIndex, OrderInvoiceNew $orderInvoiceNew, SalesOrderView $salesOrderView, Statistics $reportStatistic, SalesTaxReport $salesTaxReport, TaxRuleIndex $taxRuleIndexPage, TaxRuleNew $taxRuleNewPage) {
	...
	public function test(OrderInjectable $order, TaxRule $taxRule, array $report, $orderSteps) {
		...
		$this->orderIndex->getSalesOrderGrid ()->searchAndOpen ( [ 
				'id' => $order->getId () 
		] );
		...
		], 'Refresh Statistics for the Last Day', true );
	...
	protected function processOrder($orderSteps, OrderInjectable $order) {
		...
		foreach ( $orderStatus as $orderStep ) {
			$this->objectManager->create ( 'Magento\Sales\Test\TestStep\\Create' . ucfirst ( trim ( $orderStep ) ) . 'Step', [ 
					'order' => $order 
			] )->run ();
		}
	}
