...
class AssertStoreNotOnFrontend extends AbstractConstraint {
	...
	public function processAssert(Store $store, CmsIndex $cmsIndex) {
		...
		$isStoreViewVisible = ! $cmsIndex->getStoreSwitcherBlock ()->isStoreViewDropdownVisible () ? false : // if only one store view is assigned to store group
$cmsIndex->getStoreSwitcherBlock ()->isStoreViewVisible ( $store );
		
		\PHPUnit_Framework_Assert::assertFalse ( $isStoreViewVisible, "Store view is visible in dropdown on CmsIndex page" );
	}
