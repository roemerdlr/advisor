...
class AssertStoreGroupInGrid extends AbstractConstraint {
	...
	public function processAssert(StoreIndex $storeIndex, StoreGroup $storeGroup) {
		...
		$storeIndex->open ()->getStoreGrid ()->search ( [ 
				'group_title' => $storeGroupName 
		] );
		\PHPUnit_Framework_Assert::assertTrue ( $storeIndex->getStoreGrid ()->isStoreExists ( $storeGroupName ), 'Store group \'' . $storeGroupName . '\' is not present in grid.' );
	}
