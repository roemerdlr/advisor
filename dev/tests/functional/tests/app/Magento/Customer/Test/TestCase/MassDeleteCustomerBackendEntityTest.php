...
class MassDeleteCustomerBackendEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(FixtureFactory $fixtureFactory, CustomerIndex $customerIndexPage, CustomerIndexEdit $customerIndexEditPage) {
	...
	protected function createCustomers($customersQty) {
		...
		for($i = 0; $i < $customersQty; $i ++) {
			$customer = $this->fixtureFactory->createByCode ( 'customer', [ 
					'dataset' => 'default' 
			] );
			$customer->persist ();
			$customers [] = $customer;
