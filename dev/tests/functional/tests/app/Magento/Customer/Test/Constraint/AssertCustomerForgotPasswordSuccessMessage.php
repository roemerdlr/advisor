...
class AssertCustomerForgotPasswordSuccessMessage extends AbstractConstraint {
	const SUCCESS_MESSAGE = 'If there is an account associated with %s you will receive an email with a link to reset your password.';
	...
	public function processAssert(CustomerAccountLogin $customerLogin, Customer $customer) {
		\PHPUnit_Framework_Assert::assertEquals ( sprintf ( self::SUCCESS_MESSAGE, $customer->getEmail () ), $customerLogin->getMessages ()->getSuccessMessage (), 'Wrong forgot password message is displayed.' );
	}
