...
abstract class AbstractApplyVatIdTest extends Injectable {
	...
	public function __prepare(FixtureFactory $fixtureFactory) {
		...
		foreach ( $this->vatGroupDataSets as $group => $dataset ) {
			...
			$groupFixture = $this->fixtureFactory->createByCode ( 'customerGroup', [ 
					'dataset' => $dataset 
			] );
	...
	protected function prepareVatConfig(ConfigData $vatConfig, $customerGroup) {
		...
		$vatConfig = $this->fixtureFactory->createByCode ( 'configData', [ 
				'data' => array_replace_recursive ( $vatConfig->getSection (), $groupConfig ) 
		] );
		$vatConfig->persist ();
		
		$customerData = array_merge ( $this->customer->getData (), [ 
				'group_id' => [ 
		...
		$this->customer = $this->fixtureFactory->createByCode ( 'customer', [ 
				'data' => $customerData 
		] );
	}
