...
class AssertCustomerGroupOnProductForm extends AbstractConstraint {
	...
	public function processAssert(CatalogProductIndex $catalogProductIndex, CatalogProductNew $catalogProductNew, CustomerGroup $customerGroup) {
		...
		\PHPUnit_Framework_Assert::assertTrue ( $advancedPricingTab->getTierPriceForm ()->isVisibleCustomerGroup ( $customerGroup ), "Customer group {$customerGroup->getCustomerGroupCode()} not in tier price form on product page." );
	}
