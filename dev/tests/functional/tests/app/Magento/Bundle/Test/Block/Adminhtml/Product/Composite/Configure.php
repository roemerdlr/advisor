...
class Configure extends \Magento\Catalog\Test\Block\Adminhtml\Product\Composite\Configure {
	...
	protected function prepareOptionSelector(array $option, $selector) {
		return $this->bundleFieldsRootElement . str_replace ( '%selector%', str_replace ( '%product_name%', $option ['value'] ['name'], $selector ), str_replace ( '%option_name%', $option ['title'], $this->option ) );
	}
}
