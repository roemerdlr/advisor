...
class ManageProductReviewFromCustomerPageTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(CustomerIndexEdit $customerIndexEdit, CustomerIndex $customerIndex, CatalogProductView $catalogProductView, BrowserInterface $browser, RatingIndex $ratingIndex, RatingEdit $ratingEdit, ReviewEdit $reviewEdit) {
	...
	public function test(Review $reviewInitial, Review $review, Customer $customer) {
		...
		$this->customerIndexEdit->getCustomerForm ()->getTab ( 'product_reviews' )->getReviewsGrid ()->searchAndOpen ( $filter );
