...
class UpdateProductReviewEntityOnProductPageTest extends Injectable {
	...
	public function __prepare(FixtureFactory $fixtureFactory) {
		$this->reviewInitial = $fixtureFactory->createByCode ( 'review', [ 
	 ...
	 * @return void
	 */
	public function __inject(RatingIndex $ratingIndex, RatingEdit $ratingEdit, CatalogProductEdit $catalogProductEdit, ReviewEdit $reviewEdit) {
	...
	public function test(Review $review, $rating) {
		...
		$this->objectManager->create ( 'Magento\Catalog\Test\TestStep\OpenProductOnBackendStep', [ 
				'product' => $product 
		] )->run ();
	...
	protected function createReview($review, $rating) {
		...
		$reviewData ['ratings'] [0] = [ 
				'fixtureRating' => $fixtureRating,
				'rating' => $rating 
		];
		
		return $this->fixtureFactory->createByCode ( 'review', [ 
				'data' => $reviewData 
		] );
	}
