...
class AssertProductRatingInGrid extends AbstractConstraint {
	...
	public function processAssert(RatingIndex $ratingIndex, Rating $productRating) {
		...
		$ratingIndex->open ();
		\PHPUnit_Framework_Assert::assertTrue ( $ratingIndex->getRatingGrid ()->isRowVisible ( $filter ), "Product Rating " . $productRating->getRatingCode () . " is absent on product Rating grid." );
	}
