...
class AssertCatalogPriceRuleAppliedOnepageCheckout extends AbstractConstraint {
	...
	public function processAssert(CheckoutOnepage $checkoutOnepage, Customer $customer, array $products, array $cartPrice, array $shipping, array $payment) {
		...
		$this->objectManager->create ( '\Magento\Checkout\Test\TestStep\AddProductsToTheCartStep', [ 
				'products' => $products 
		] )->run ();
		$this->objectManager->create ( '\Magento\Checkout\Test\TestStep\ProceedToCheckoutStep' )->run ();
		$this->objectManager->create ( '\Magento\Checkout\Test\TestStep\FillBillingInformationStep', [ 
				'customer' => $customer,
				'checkoutMethod' => 'register' 
		] )->run ();
		$this->objectManager->create ( '\Magento\Checkout\Test\TestStep\FillShippingMethodStep', [ 
				'shipping' => $shipping 
		] )->run ();
		$this->objectManager->create ( '\Magento\Checkout\Test\TestStep\SelectPaymentMethodStep', [ 
				'payment' => $payment 
		] )->run ();
