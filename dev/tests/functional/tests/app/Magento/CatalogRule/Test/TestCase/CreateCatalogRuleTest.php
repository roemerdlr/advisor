...
class CreateCatalogRuleTest extends AbstractCatalogRuleEntityTest {
	...
	public function testCreate(CatalogRule $catalogPriceRule, $product, $conditionEntity, Customer $customer = null) {
		...
		$productSimple = $this->fixtureFactory->createByCode ( 'catalogProductSimple', [ 
				'dataset' => $product 
		] );
		// Prepare data
	...
	public function applyCustomerGroup(CatalogRule $catalogPriceRule, Customer $customer = null) {
		if ($customer !== null) {
			...
			$catalogPriceRule = $this->fixtureFactory->createByCode ( 'catalogRule', [ 
					'data' => array_merge ( $catalogPriceRule->getData (), [ 
	...
	protected function prepareCondition(CatalogProductSimple $productSimple, $conditionEntity) {
		...
		switch ($conditionEntity) {
			...
			case 'attribute' :
				...
				$attributes = $productSimple->getDataFieldConfig ( 'attribute_set_id' ) ['source']->getAttributeSet ()->getDataFieldConfig ( 'assigned_attributes' ) ['source']->getAttributes ();
