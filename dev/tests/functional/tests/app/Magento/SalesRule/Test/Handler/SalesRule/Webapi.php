...
class Webapi extends AbstractWebapi implements SalesRuleInterface {
	 ...
	 */
	public function __construct(DataInterface $configuration, EventManagerInterface $eventManager, WebapiDecorator $webapiTransport, Curl $handlerCurl) {
	...
	protected function createCoupon($ruleId) {
		...
		$data = [ 
				'coupon' => array_filter ( [ 
						...
						'usage_limit' => isset ( $this->data ['uses_per_coupon'] ) ? $this->data ['uses_per_coupon'] : null,
						'usage_per_customer' => isset ( $this->data ['usage_per_customer'] ) ? $this->data ['usage_per_customer'] : null,
						'is_primary' => true 
				] ) 
		];
	...
	protected function convertCondition(array $condition, $prefix = '', $indent = 1) {
		...
		while ( $isContinue ) {
			...
			$result [] = array_filter ( [ 
					...
					'aggregator_type' => isset ( $condition [$key] ['aggregator'] ) ? $condition [$key] ['aggregator'] : null,
					'attribute_name' => isset ( $condition [$key] ['attribute'] ) ? $condition [$key] ['attribute'] : null,
					'operator' => isset ( $condition [$key] ['operator'] ) ? $condition [$key] ['operator'] : null,
