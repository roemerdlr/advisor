	...
	public function processAssert(array $products, CatalogProductCompare $comparePage, CmsIndex $cmsIndex) {
		...
		foreach ( $products as $key => $product ) {
			foreach ( $this->attributeProduct as $attributeKey => $attribute ) {
				...
				$attributeValue = $attribute != 'price' ? ($product->hasData ( $attribute ) ? $product->getData ( $attribute ) : 'N/A') : ($product->getDataFieldConfig ( 'price' ) ['source']->getPriceData () !== null ? $product->getDataFieldConfig ( 'price' ) ['source']->getPriceData () ['compare_price'] : number_format ( $product->getPrice (), 2 ));
