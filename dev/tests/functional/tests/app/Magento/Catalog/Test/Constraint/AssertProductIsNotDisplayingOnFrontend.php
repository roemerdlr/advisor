...
class AssertProductIsNotDisplayingOnFrontend extends AbstractConstraint {
	 ...
	 */
	public function processAssert(CatalogProductView $catalogProductView, CatalogsearchResult $catalogSearchResult, CatalogCategoryView $catalogCategoryView, CmsIndex $cmsIndex, $product, BrowserInterface $browser, Category $category = null) {
		...
		$products = is_array ( $product ) ? $product : [ 
				$product 
		];
		$errors = [ ];
		...
		\PHPUnit_Framework_Assert::assertEmpty ( $errors, "In the process of checking product availability on the frontend, found the following errors:\n" . implode ( "\n", $errors ) );
	}
