...
class Options extends Tab {
	...
	public function fillFormTab(array $fields, SimpleElement $element = null) {
		...
		foreach ( $fields ['value'] as $keyRoot => $field ) {
			...
			if (isset ( $field ['type'] ) && ! empty ( $options )) {
				...
				$optionsForm = $this->blockFactory->create ( __NAMESPACE__ . '\Options\Type\\' . $this->optionNameConvert ( $field ['type'] ), [ 
						'element' => $rootElement 
				] );
				
				foreach ( $options as $key => $option ) {
					++ $key;
					$optionsForm->fillOptions ( $option, $rootElement->find ( '.fieldset .data-table tbody tr:nth-child(' . $key . ')' ) );
				}
			}
		}
	...
	protected function getSearchGridBlock() {
		return $this->blockFactory->create ( 'Magento\Catalog\Test\Block\Adminhtml\Product\Edit\Tab\Options\Search\Grid', [ 
	...
	public function getDataFormTab($tabFields = null, SimpleElement $element = null) {
		...
		foreach ( $fields ['value'] as $keyRoot => $field ) {
			...
			if (isset ( $field ['type'] ) && ! empty ( $options )) {
				...
				foreach ( $options as $key => $option ) {
					$formDataItem ['options'] [$key ++] = $optionsForm->getDataOptions ( $option, $rootElement->find ( '.fieldset .data-table tbody tr:nth-child(' . $key . ')' ) );
				}
			}
