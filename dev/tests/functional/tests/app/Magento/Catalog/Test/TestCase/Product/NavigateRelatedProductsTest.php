...
class NavigateRelatedProductsTest extends AbstractProductPromotedProductsTest {
	 ...
	 * @return void
	 */
	public function test($products, $selectable, $promotedProducts, $navigateProductsOrder, $productsToVerify, CheckoutCart $checkoutCart) {
		// Preconditions
	...
	protected function assertAbsentRelatedSellSection() {
		\PHPUnit_Framework_Assert::assertFalse ( $this->catalogProductView->getRelatedProductBlock ()->isVisible (), "Related section is present." );
	}
	...
	protected function assertRelatedSection($product) {
		...
		asort ( $fixtureData );
		asort ( $pageData );
		\PHPUnit_Framework_Assert::assertEquals ( $pageData, $fixtureData, 'Wrong products are displayed in related section.' );
	}
