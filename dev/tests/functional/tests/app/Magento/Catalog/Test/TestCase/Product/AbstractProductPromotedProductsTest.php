...
abstract class AbstractProductPromotedProductsTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(CatalogProductIndex $catalogProductIndex, CatalogProductEdit $catalogProductEdit, CatalogProductView $catalogProductView) {
	...
	protected function createProducts($products) {
		...
		foreach ( $list as $item ) {
			...
			$product = $this->fixtureFactory->createByCode ( $fixtureCode, [ 
					'dataset' => $dataset 
			] );
	...
	protected function assignPromotedProducts($promotedProducts, $type) {
		...
		foreach ( $promotedProducts as $productName => $assignedNames ) {
			...
			$product = $this->fixtureFactory->create ( get_class ( $initialProduct ), [ 
					'data' => [ 
							$type => [ 
	...
	protected function parseProductsToVerify($productsToVerify) {
		...
		foreach ( $list as $item ) {
			...
			$result [$step] = empty ( $products ) ? [ ] : array_map ( 'trim', explode ( ',', $products ) );
		}
