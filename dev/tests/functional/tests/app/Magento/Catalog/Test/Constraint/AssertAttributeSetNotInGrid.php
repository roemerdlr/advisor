...
class AssertAttributeSetNotInGrid extends AbstractConstraint {
	...
	public function processAssert(CatalogProductSetIndex $productSetPage, CatalogAttributeSet $attributeSet) {
		...
		$productSetPage->open ();
		\PHPUnit_Framework_Assert::assertFalse ( $productSetPage->getGrid ()->isRowVisible ( $filterAttributeSet ), 'An attribute set named "' . $filterAttributeSet ['set_name'] . '" is present in Attribute Set grid.' );
	}
