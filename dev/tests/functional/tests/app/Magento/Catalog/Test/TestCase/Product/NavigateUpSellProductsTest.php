...
class NavigateUpSellProductsTest extends AbstractProductPromotedProductsTest {
	 ...
	 * @return void
	 */
	public function test($products, $promotedProducts, $navigateProductsOrder, $productsToVerify) {
		// Preconditions
	...
	protected function assertAbsentUpSellSection() {
		\PHPUnit_Framework_Assert::assertFalse ( $this->catalogProductView->getUpsellBlock ()->isVisible (), "Up-sell section is present." );
	}
	...
	protected function assertUpSellSection(array $promotedProductNames) {
		...
		\PHPUnit_Framework_Assert::assertEquals ( $productNames, $pageProductNames, 'Wrong products are displayed in up-sell section.' );
	}
}
