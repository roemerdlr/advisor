...
class Product extends Tab {
	...
	public function fillFormTab(array $fields, SimpleElement $element = null) {
		...
		foreach ( $fields ['category_products'] ['source']->getData () as $productName ) {
			$this->getProductGrid ()->searchAndSelect ( [ 
					'name' => $productName 
			] );
		}
	}
	...
	public function getDataFormTab($fields = null, SimpleElement $element = null) {
		...
		if (isset ( $data ['category_products'] )) {
			$this->getProductGrid ()->search ( [ 
					'in_category' => 'Yes' 
			] );
			$rows = $this->getProductGrid ()->getRowsData ( [ 
					'name' 
			] );
	...
	public function getProductGrid() {
		return $this->blockFactory->create ( 'Magento\Catalog\Test\Block\Adminhtml\Category\Tab\ProductGrid', [ 
