...
class ProductForm extends FormTabs {
	 ...
	 *        	[optional]
	 * @return FormTabs @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	...
	protected function waitPageToLoad() {
		...
		$this->_rootElement->waitUntil ( function () use($browser, $advancedSettingTrigger) {
			return $browser->find ( $advancedSettingTrigger )->isVisible () == true ? true : null;
		} );
		
		$this->_rootElement->waitUntil ( function () use($browser, $element) {
	...
	public function checkAttributeLabel($productAttribute) {
		$frontendLabel = (is_array ( $productAttribute )) ? $productAttribute ['frontend_label'] : $productAttribute->getFrontendLabel ();
	...
	protected function getAttributesSearchForm() {
		return $this->_rootElement->find ( $this->attributeSearch, Locator::SELECTOR_CSS, 'Magento\Catalog\Test\Block\Adminhtml\Product\Edit\Tab\Attributes\Search' );
	}
	...
	public function saveAttributeForm() {
		...
		$this->_rootElement->waitUntil ( function () use($browser, $loader) {
