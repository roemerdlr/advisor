...
class AssertProductAttributeIsUnique extends AbstractConstraint {
	...
	public function processAssert(CatalogProductIndex $catalogProductIndex, CatalogProductEdit $catalogProductEdit, CatalogProductAttribute $attribute, CatalogProductSimple $product, FixtureFactory $fixtureFactory) {
		...
		\PHPUnit_Framework_Assert::assertEquals ( sprintf ( self::UNIQUE_MESSAGE, $attributeLabel ), $actualMessage, 'JS error notice on product edit page is not equal to expected.' );
	}
	...
	protected function getActualMessage(array $errors, $attributeLabel) {
		return isset ( $errors ['product-details'] [$attributeLabel] ) ? $errors ['product-details'] [$attributeLabel] : null;
	}
	...
	protected function createSimpleProductFixture(CatalogProductSimple $product, CatalogProductAttribute $attribute) {
		return $this->fixtureFactory->createByCode ( 'catalogProductSimple', [ 
