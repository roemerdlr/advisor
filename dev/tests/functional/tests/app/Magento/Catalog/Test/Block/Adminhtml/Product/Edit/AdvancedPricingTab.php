...
class AdvancedPricingTab extends ProductTab {
	...
	public function fillFormTab(array $fields, SimpleElement $element = null) {
		...
		foreach ( $fields as $fieldName => $field ) {
			...
			if (isset ( $this->childrenForm [$fieldName] ) && is_array ( $field ['value'] )) {
				...
				$optionsForm = $this->blockFactory->create ( __NAMESPACE__ . '\\' . $this->childrenForm [$fieldName], [ 
						'element' => $context 
				] );
				
				foreach ( $field ['value'] as $key => $option ) {
					++ $key;
					$optionsForm->fillOptions ( $option, $context->find ( '#attribute-' . $fieldName . '-container tbody tr:nth-child(' . $key . ')' ) );
				}
	...
	public function getDataFormTab($fields = null, SimpleElement $element = null) {
		...
		foreach ( $fields as $fieldName => $field ) {
			...
			if (isset ( $this->childrenForm [$fieldName] ) && is_array ( $field ['value'] )) {
				...
				foreach ( $field ['value'] as $key => $option ) {
					$formData [$fieldName] [$key ++] = $optionsForm->getDataOptions ( $option, $this->_rootElement->find ( '#attribute-' . $fieldName . '-container tbody tr:nth-child(' . $key . ')' ) );
				}
			} elseif (! empty ( $field ['value'] )) {
				$data = $this->dataMapping ( [ 
						$fieldName => $field 
	...
	public function getTierPriceForm() {
		return $this->blockFactory->create ( 'Magento\Catalog\Test\Block\Adminhtml\Product\Edit\AdvancedPricingTab\OptionTier', [ 
