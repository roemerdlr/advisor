...
class MassProductUpdateTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(CatalogProductIndex $productGrid, CatalogProductActionAttributeEdit $attributeMassActionPage) {
	...
	public function test(CatalogProductSimple $initialProduct, CatalogProductSimple $product, $configData) {
		...
		$this->productGrid->getProductGrid ()->updateAttributes ( [ 
				[ 
		...
		$product = $this->objectManager->create ( 'Magento\Catalog\Test\Fixture\CatalogProductSimple', [ 
				'data' => $data 
		] );
