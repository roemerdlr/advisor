...
class Curl extends AbstractCurl implements CatalogProductSimpleInterface {
	...
	protected function prepareStockData(array $fields) {
		...
		if (! isset ( $fields ['stock_data'] ['is_in_stock'] )) {
			$fields ['stock_data'] ['is_in_stock'] = isset ( $fields ['quantity_and_stock_status'] ['is_in_stock'] ) ? $fields ['quantity_and_stock_status'] ['is_in_stock'] : (isset ( $fields ['inventory_manage_stock'] ) ? $fields ['inventory_manage_stock'] : null);
		}
		if (! isset ( $fields ['stock_data'] ['qty'] )) {
			$fields ['stock_data'] ['qty'] = isset ( $fields ['quantity_and_stock_status'] ['qty'] ) ? $fields ['quantity_and_stock_status'] ['qty'] : null;
		}
		
		if (! isset ( $fields ['stock_data'] ['manage_stock'] )) {
			$fields ['stock_data'] ['manage_stock'] = ( int ) (! empty ( $fields ['stock_data'] ['qty'] ) || ! empty ( $fields ['stock_data'] ['is_in_stock'] ));
		}
	...
	protected function createProduct(array $data, array $config) {
		$config ['create_url_params'] ['set'] = isset ( $data ['product'] ['attribute_set_id'] ) ? $data ['product'] ['attribute_set_id'] : $config ['create_url_params'] ['set'];
