...
abstract class AbstractConfigureBlock extends Form {
	...
	public function getCustomOptionsBlock() {
		return $this->blockFactory->create ( 'Magento\Catalog\Test\Block\Product\View\CustomOptions', [ 
	...
	public function fillOptions(FixtureInterface $product) {
		...
		if ($this->hasRender ( $typeId )) {
			$this->callRender ( $typeId, 'fillOptions', [ 
					'product' => $product 
			] );
		}
		...
		foreach ( $checkoutData as $checkoutOption ) {
			...
			if (isset ( $options [$attribute] )) {
				$result [] = [ 
						...
						'title' => isset ( $options [$attribute] ['title'] ) ? $options [$attribute] ['title'] : $attribute,
						'value' => isset ( $options [$attribute] ['options'] [$option] ['title'] ) ? $options [$attribute] ['options'] [$option] ['title'] : $option 
				];
			}
		}
