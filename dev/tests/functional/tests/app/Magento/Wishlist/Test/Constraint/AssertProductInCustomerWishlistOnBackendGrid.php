...
class AssertProductInCustomerWishlistOnBackendGrid extends AbstractConstraint {
	...
	public function processAssert(CustomerIndexEdit $customerIndexEdit, FixtureInterface $product) {
		...
		\PHPUnit_Framework_Assert::assertTrue ( $wishlistGrid->isRowVisible ( $filter ), 'Product ' . $product->getName () . ' is absent in grid with configure option.' );
	}
		...
		if (isset ( $checkoutData ['custom_options'] )) {
			foreach ( $checkoutData ['custom_options'] as $option ) {
				...
				$productOptions [] = [ 
						...
						'value' => isset ( $customOptions [$optionKey] ['options'] [$valueKey] ['title'] ) ? $customOptions [$optionKey] ['options'] [$valueKey] ['title'] : $valueKey 
				];
			}
		}
