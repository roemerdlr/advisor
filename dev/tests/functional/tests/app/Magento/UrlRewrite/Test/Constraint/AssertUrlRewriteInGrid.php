...
class AssertUrlRewriteInGrid extends AbstractConstraint {
	...
	public function processAssert(UrlRewriteIndex $urlRewriteIndex, UrlRewrite $urlRewrite) {
		...
		\PHPUnit_Framework_Assert::assertTrue ( $urlRewriteIndex->getUrlRedirectGrid ()->isRowVisible ( $filter ), 'URL Rewrite with request path \'' . $urlRewrite->getRequestPath () . '\' is absent in grid.' );
	}
