...
class AssertUrlRewriteUpdatedProductInGrid extends AbstractConstraint {
	...
	public function processAssert(CatalogProductSimple $product, CatalogProductSimple $initialProduct, UrlRewriteIndex $urlRewriteIndex) {
		...
		\PHPUnit_Framework_Assert::assertFalse ( $urlRewriteIndex->getUrlRedirectGrid ()->isRowVisible ( [ 
				'target_path' => $targetPath 
		], true, false ), "URL Rewrite with target path '$targetPath' is present in grid." );
	}
