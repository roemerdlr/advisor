...
class UpdateProductFromMiniShoppingCartEntityTest extends Injectable {
	 ...
	 * @return void
	 */
	public function __inject(CmsIndex $cmsIndex, CatalogProductView $catalogProductView, FixtureFactory $fixtureFactory) {
	...
	public function test($originalProduct, $checkoutData) {
		...
		$newProduct = $this->createProduct ( explode ( '::', $originalProduct ) [0], [ 
				$productData 
		] );
	...
	protected function addToCart(FixtureInterface $product) {
		$addToCartStep = $this->objectManager->create ( 'Magento\Checkout\Test\TestStep\AddProductsToTheCartStep', [ 
				'products' => [ 
						$product 
				] 
		] );
