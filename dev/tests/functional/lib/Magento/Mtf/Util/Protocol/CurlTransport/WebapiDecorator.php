...
class WebapiDecorator implements CurlInterface {
	 ...
	 */
	public function __construct(ObjectManager $objectManager, CurlTransport $transport, DataInterface $configuration, FixtureFactory $fixtureFactory) {
	...
	protected function init() {
		...
		if (null === $integrationToken || ! $this->isValidIntegration ()) {
			...
			$integration = $this->fixtureFactory->create ( 'Magento\Integration\Test\Fixture\Integration', [ 
	...
	protected function disableSecretKey() {
		$config = $this->fixtureFactory->create ( 'Magento\Config\Test\Fixture\ConfigData', [ 
