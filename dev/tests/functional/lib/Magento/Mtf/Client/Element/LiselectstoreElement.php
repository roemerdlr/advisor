	...
	public function getValues() {
		...
		foreach ( $dropdownData as $key => $dropdownElement ) {
			if ($dropdownElement ['storeView']) {
				$data [] = $this->findNearestElement ( 'website', $key, $dropdownData ) . "/" . $this->findNearestElement ( 'store', $key, $dropdownData ) . "/" . $dropdownElement ['element']->getText ();
			}
		}
		return $data;
	}
	...
	public function getValue() {
		...
		foreach ( $elements as $key => $element ) {
			if ($element ['current'] == true) {
				...
				$path = $this->findNearestElement ( 'website', $key, $elements ) . "/" . $this->findNearestElement ( 'store', $key, $elements ) . "/" . $element ['element']->getText ();
				return $path;
			}
		}
