...
class PhtmlTemplateTest extends \PHPUnit_Framework_TestCase {
	public function testBlockVariableInsteadOfThis() {
		 ...
		 */
		function ($file) {
			$this->assertNotRegExp ( '/this->(?!helper)\S*/iS', file_get_contents ( $file ), 'Access to members and methods of Block class through $this is ' . 'obsolete in phtml templates. Use only $block instead of $this.' );
	...
	public function testObsoleteBlockMethods() {
		 ...
		 */
		function ($file) {
			$this->assertNotRegexp ( '/block->_[^_]+\S*\(/iS', file_get_contents ( $file ), 'Access to protected and private members of Block class is ' . 'obsolete in phtml templates. Use only public members.' );
		}, \Magento\Framework\App\Utility\Files::init ()->getPhtmlFiles () );
	}
