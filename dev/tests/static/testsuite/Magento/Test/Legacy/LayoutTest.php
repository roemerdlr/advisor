		...
		foreach ( $layoutXml as $handle ) {
			if (isset ( $this->_obsoleteReferences [$handle->getName ()] )) {
				foreach ( $handle->xpath ( 'reference' ) as $reference ) {
					$this->assertNotContains ( ( string ) $reference ['name'], $this->_obsoleteReferences [$handle->getName ()], 'The block being referenced is removed.' );
				}
			}
		}
	}
	...
	public function testActionNodeMethods() {
		...
		function ($layoutFile) {
			...
			foreach ( $layoutXml->xpath ( '//action[' . $methodFilter . ']' ) as $node ) {
				...
				$this->fail ( sprintf ( 'Call of method "%s" via layout instruction <action> is not allowed.', $attributes ['method'] ) );
			}
		}, \Magento\Framework\App\Utility\Files::init ()->getLayoutFiles () );
	}
	
	/**
	 * List of currently allowed (i.e.
	 * not refactored yet) methods for use in <action method="someMethod"/> layout
	 * instruction.
	 *
