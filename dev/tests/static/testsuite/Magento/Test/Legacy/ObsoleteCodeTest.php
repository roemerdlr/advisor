...
class ObsoleteCodeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testTemplateMageCalls() {
		...
		}, Files::init ()->getPhpFiles ( Files::INCLUDE_TEMPLATES | Files::INCLUDE_TESTS | Files::AS_DATA_SET ) );
	}
	...
	protected function _testObsoleteClasses($content) {
		foreach ( self::$_classes as $row ) {
			...
			$this->_assertNotRegExp ( '/[^a-z\d_]' . preg_quote ( $class, '/' ) . '[^a-z\d_\\\\]/iS', $content, $this->_suggestReplacement ( sprintf ( "Class '%s' is obsolete.", $class ), $replacement ) );
		}
	}
	...
	protected function _testObsoleteNamespaces($content) {
		foreach ( self::$_namespaces as $row ) {
			...
			$this->_assertNotRegExp ( '/namespace\s+' . preg_quote ( $namespace, '/' ) . ';/S', $content, $this->_suggestReplacement ( sprintf ( "Namespace '%s' is obsolete.", $namespace ), $replacement ) );
			$this->_assertNotRegExp ( '/[^a-zA-Z\d_]' . preg_quote ( $namespace . '\\', '/' ) . '/S', $content, $this->_suggestReplacement ( sprintf ( "Namespace '%s' is obsolete.", $namespace ), $replacement ) );
		}
	}
	...
	protected function _testObsoleteMethods($content, $file) {
		foreach ( self::$_methods as $row ) {
			...
			if ($class) {
				$message = $this->_suggestReplacement ( "Method '{$class}::{$method}()' is obsolete in file '{$file}'.", $replacement );
				// without opening parentheses to match static callbacks notation
				$this->_assertNotRegExp ( '/' . preg_quote ( $class, '/' ) . '::\s*' . $quotedMethod . '[^a-z\d_]/iS', $content, $message );
				...
				$message = $this->_suggestReplacement ( "Function or method '{$method}()' is obsolete in file '{$file}'.", $replacement );
	...
	protected function _testObsoletePaths($file) {
		foreach ( self::$_paths as $row ) {
			...
			$message = $this->_suggestReplacement ( "Path '{$obsoletePath}' is obsolete.", $replacementPath );
	...
	protected function _testGetChildSpecialCase($content, $file) {
		if (0 === strpos ( $file, Files::init ()->getPathToSource () . '/app/' )) {
			$this->_assertNotRegexp ( '/[^a-z\d_]getChild\s*\(/iS', $content, 'Block method getChild() is obsolete. ' . 'Replacement suggestion: \Magento\Framework\View\Element\AbstractBlock::getChildBlock()' );
		}
	}
	...
	protected function _testGetOptionsSpecialCase($content) {
		$this->_assertNotRegexp ( '/getOptions\(\)\s*->get(Base|App|Code|Design|Etc|Lib|Locale|Js|Media' . '|Var|Tmp|Cache|Log|Session|Upload|Export)?Dir\(/S', $content, 'The class \Magento\Core\Model\Config\Options is obsolete. ' . 'Replacement suggestion: \Magento\Framework\Filesystem' );
	}
	...
	protected function _testObsoleteMethodArguments($content) {
		$this->_assertNotRegExp ( '/[^a-z\d_]getTypeInstance\s*\(\s*[^\)]+/iS', $content, 'Backwards-incompatible change: method getTypeInstance() is not supposed to be invoked with any arguments.' );
		$this->_assertNotRegExp ( '/\->getUsedProductIds\(([^\)]+,\s*[^\)]+)?\)/', $content, 'Backwards-incompatible change: method getUsedProductIds($product)' . ' must be invoked with one and only one argument - product model object' );
		
		$this->_assertNotRegExp ( '#->_setActiveMenu\([\'"]([\w\d/_]+)[\'"]\)#Ui', $content, 'Backwards-incompatible change: method _setActiveMenu()' . ' must be invoked with menu item identifier than xpath for menu item' );
	}
	...
	protected function _testObsoleteProperties($content) {
		foreach ( self::$_attributes as $row ) {
			...
			$this->_assertNotRegExp ( '/[^a-z\d_]' . preg_quote ( $attribute, '/' ) . '[^a-z\d_]/iS', $content, $this->_suggestReplacement ( sprintf ( "Class attribute '%s' is obsolete.", $fullyQualified ), $replacement ) );
		}
	}
	...
	protected function _testObsoleteActions($content) {
		...
		$this->_assertNotRegExp ( '#[^a-z\d_/]catalog/product/image[^a-z\d_/]#iS', $content, "Action 'catalog/product/image' is obsolete. {$suggestion}" );
	}
	...
	private function buildRegExFromObsoleteConstant($classPartialPath, $content, $constant) {
		...
		if ($this->_isClassOrInterface ( $content, $classPartialPath )) {
			$regex .= '|' . $this->_getClassConstantDefinitionRegExp ( $constant ) . '|' . preg_quote ( "self::{$constant}", '/' ) . '|' . preg_quote ( "static::{$constant}", '/' );
	...
	private function _checkConstantWithFullClasspath($constant, $class, $replacement, $content) {
		...
		$this->_checkExistenceOfObsoleteConstants ( $constantRegex, $classRegex, $content, "{$class}::{$constant}", $replacement, $class );
	}
	...
	private function _checkConstantWithClasspath($constant, $class, $replacement, $content) {
		...
		for($i = count ( $classPathParts ) - 1; $i >= 0; $i --) {
			...
			$constantRegexPartial = '/\b(?P<classWithConst>([a-zA-Z0-9_' . preg_quote ( '\\' ) . ']*))(' . preg_quote ( '::' ) . ')*' . '(' . preg_quote ( $constant, '/' ) . '\b)(\s*|;)/';
			...
			if (($matchNamespaceClass === 1) && ($matchConstantPartial === 1) && ($match ['classWithConst'] === '')) {
				$this->assertSame ( 0, 1, $this->_suggestReplacement ( sprintf ( "Constant '%s' is obsolete.", $constant ), $replacement ) );
			} else {
				$this->_checkExistenceOfObsoleteConstants ( $constantRegex, $classRegex, $content, "{$classPartialPath}::{$constant}", $replacement, $class );
			}
		}
	}
	 ...
	 * @param string $class        	
	 */
	private function _checkExistenceOfObsoleteConstants($constantRegex, $classRegex, $content, $constant, $replacement, $class) {
		$constantRegexFull = '/\b(?P<constPart>((?P<classWithConst>([a-zA-Z0-9_' . preg_quote ( '\\' ) . ']*))(' . preg_quote ( '::' ) . ')*' . '(' . $constantRegex . '\b)))(\s*|;)/';
		$matchConstant = preg_match_all ( $constantRegexFull, $content, $matchConstantString );
		$result = 0;
		if ($matchConstant === 1) {
			if ($classRegex !== '') {
				$classRegexFull = '/(?P<useOrNamespace>(use|namespace))\s+(?P<classPath>(' . preg_quote ( '\\' ) . '?(' . $classRegex . ')))(\s+as\s+(?P<classAlias>([\w\d_]+)))?(\s|;)/';
				$matchClass = preg_match ( $classRegexFull, $content, $matchClassString );
				if ($matchClass === 1) {
					if ($matchClassString ['classAlias']) {
						$result = $this->_checkAliasUseNamespace ( $constantRegex, $matchConstantString, $matchClassString, $class );
					} else {
		...
		$this->assertSame ( 0, $result, $this->_suggestReplacement ( sprintf ( "Constant '%s' is obsolete.", $constant ), $replacement ) );
	}
	 ...
	 * @return int
	 */
	private function _checkAliasUseNamespace($constantRegex, $matchConstantString, $matchClassString, $class) {
		...
		foreach ( $matchConstantString ['constPart'] as $constantMatch ) {
			...
			$expectedConstPartialClass = '/' . $asComponent . preg_quote ( '\\' ) . $constantRegex . '/';
			if ((preg_match ( $expectedOnlyConst, $constantMatch ) === 1) || (preg_match ( $expectedConstPartialClass, $constantMatch ) === 1)) {
				$foundAsComponent = true;
			}
			if (strpos ( $constantMatch, '::' ) !== false) {
				$foundProperUse = $this->_checkCompletePathOfClass ( $constantMatch, $matchClassString, $class, $foundAsComponent, $asComponent );
				if ($foundProperUse) {
					break;
				}
			}
	 ...
	 * @return int
	 */
	private function _checkNoAliasUseNamespace($matchConstantString, $matchClassString, $class) {
		$foundProperUse = false;
		foreach ( $matchConstantString ['constPart'] as $constantMatch ) {
			$foundProperUse = $this->_checkCompletePathOfClass ( $constantMatch, $matchClassString, $class );
			if ($foundProperUse) {
				break;
			}
		}
	 ...
	 * @return bool
	 */
	private function _checkCompletePathOfClass($constantMatch, $matchClassString, $class, $foundAsComponent = false, $asComponent = '') {
		...
		$pathInUseNamespaceTruncated = trim ( trim ( preg_replace ( '/' . preg_quote ( $pathWithConstParts [0] ) . '$/', '', $pathInUseNamespace ), '\\' ) );
		if ($this->_checkClasspathProperDivisionNoConstantPath ( $pathInUseNamespaceTruncated, $pathInUseNamespace, $matchClassString, $class, $foundAsComponent )) {
			return true;
		} else {
			return $this->_checkClasspathProperDivisionWithConstantPath ( $pathInUseNamespaceTruncated, $pathInUseNamespace, $pathWithConst, $class, $foundAsComponent );
		}
	}
	 ...
	 * @return bool
	 */
	private function _checkClasspathProperDivisionNoConstantPath($pathInUseNamespaceTruncated, $pathInUseNamespace, $matchClassString, $class, $foundAsComponent) {
		if ($pathInUseNamespaceTruncated === $pathInUseNamespace && $pathInUseNamespaceTruncated !== $class && ($foundAsComponent || (strpos ( $matchClassString ['useOrNamespace'], 'namespace' ) !== false))) {
			return true;
		} else {
			return false;
		}
	 ...
	 * @return bool
	 */
	private function _checkClasspathProperDivisionWithConstantPath($pathInUseNamespaceTruncated, $pathInUseNamespace, $pathWithConst, $class, $foundAsComponent) {
		if ((($pathInUseNamespaceTruncated . '\\' . $pathWithConst === $class) && ($pathInUseNamespaceTruncated !== $pathInUseNamespace) && ! $foundAsComponent) || (($pathInUseNamespaceTruncated === $class) && (strpos ( $pathWithConst, '\\' ) === false) && $foundAsComponent)) {
			return true;
		} else {
			return false;
		}
	...
	protected function _testObsoletePropertySkipCalculate($content) {
		$this->_assertNotRegExp ( '/[^a-z\d_]skipCalculate[^a-z\d_]/iS', $content, "Configuration property 'skipCalculate' is obsolete." );
	}
	...
	protected function _isDirectDescendant($content, $name) {
		$name = preg_quote ( $name, '/' );
		return ( bool ) preg_match ( '/\s+extends\s+' . $name . '\b|\s+implements\s+[^{]*\b' . $name . '\b[^{^\\\\]*\{/iS', $content );
	}
