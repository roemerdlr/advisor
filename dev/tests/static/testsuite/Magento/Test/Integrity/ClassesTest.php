...
class ClassesTest extends \PHPUnit_Framework_TestCase {
	...
	protected static $_keywordsBlacklist = [ 
			"String",
			"Array",
			"Boolean",
			"Element" 
	...
	public function testPhpFiles() {
		...
		function ($file) {
			...
			$classes = Classes::getAllMatches ( $contents, '/
			...
			Classes::getAllMatches ( $contents, '/(?:\-> | parent\:\:)(?:_init | setType)\(\s*
	...
	protected function _assertClassNamespace($file, $relativePath, $contents, $className) {
		...
		if (preg_match ( $formalPattern, $contents, $formalNamespaceArray ) != 0) {
			...
			if ($namespaceFolders != null && $foundNamespace != null) {
				$this->assertEquals ( $namespaceFolders, $foundNamespace, "Location of {$file} does not match formal namespace: {$expectedNamespace}\n" );
			}
		} else {
	...
	public function testClassReferences() {
		...
		function ($file) {
			...
			$newObjectPattern = '/^' . '.*new\s(?<venderClass>\\\\Magento(?:\\\\[a-zA-Z0-9_]+)+)\(.*\)' . '|.*new\s(?<badClass>[A-Z][a-zA-Z0-9]+[a-zA-Z0-9_\\\\]*)\(.*\)\;' . '|use [A-Z][a-zA-Z0-9_\\\\]+ as (?<aliasClass>[A-Z][a-zA-Z0-9]+)' . '/m';
			$result1 = [ ];
			preg_match_all ( $newObjectPattern, $contents, $result1 );
			
			// Static function/variable, for example: "Foo::someStaticFunction();"
			$staticCallPattern = '/^' . '((?!Magento).)*(?<venderClass>\\\\Magento(?:\\\\[a-zA-Z0-9_]+)+)\:\:.*\;' . '|[^\\\\^a-z^A-Z^0-9^_^:](?<badClass>[A-Z][a-zA-Z0-9_]+)\:\:.*\;' . '|use [A-Z][a-zA-Z0-9_\\\\]+ as (?<aliasClass>[A-Z][a-zA-Z0-9]+)' . '/m';
			$result2 = [ ];
			...
			$annotationPattern = '/^' . '[\s]*\*\s\@(?:return|throws)\s(?<venderClass>\\\\Magento(?:\\\\[a-zA-Z0-9_]+)+)' . '|[\s]*\*\s\@return\s(?<badClass>[A-Z][a-zA-Z0-9_\\\\]+)' . '|[\s]*\*\s\@throws\s(?<exception>[A-Z][a-zA-Z0-9_\\\\]+)' . '|use [A-Z][a-zA-Z0-9_\\\\]+ as (?<aliasClass>[A-Z][a-zA-Z0-9]+)' . '/m';
			$result3 = [ ];
			preg_match_all ( $annotationPattern, $contents, $result3 );
			
			$vendorClasses = array_unique ( array_merge_recursive ( $result1 ['venderClass'], $result2 ['venderClass'], $result3 ['venderClass'] ) );
			
			$badClasses = array_unique ( array_merge_recursive ( $result1 ['badClass'], $result2 ['badClass'], $result3 ['badClass'] ) );
			
			$aliasClasses = array_unique ( array_merge_recursive ( $result1 ['aliasClass'], $result2 ['aliasClass'], $result3 ['aliasClass'] ) );
	...
	private function removeSpecialCasesNonFullyQualifiedClassNames($namespacePath, &$badClasses, $badClass) {
		...
		if ($moduleDir) {
			$fullPath = $moduleDir . '/' . implode ( '/', $namespaceParts ) . '/' . str_replace ( '\\', '/', $badClass ) . '.php';
	...
	public function testCoversAnnotation() {
		...
		if ($errors) {
			$this->fail ( 'Nonexistent classes/methods were found in @covers annotations: ' . PHP_EOL . implode ( PHP_EOL, $errors ) );
		}
	}
	...
	private function isNonexistentEntityCovered($matches) {
		return ! empty ( $matches [2] ) && ! class_exists ( $matches [2] ) || ! empty ( $matches [4] ) && ! method_exists ( $matches [2], $matches [4] );
	}
}
