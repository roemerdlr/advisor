...
class TranslationFilesTest extends TranslationFiles {
	...
	public function testDefaultLocale($file, $phrases) {
		...
		$this->assertEmpty ( $failures, $this->printMessage ( [ 
				$file => $failures 
		] ) );
	}
	...
	protected function prepareParser() {
		...
		$parserContextual = new \Magento\Setup\Module\I18n\Parser\Contextual ( $filesCollector, new \Magento\Setup\Module\I18n\Factory (), new \Magento\Setup\Module\I18n\Context ( new ComponentRegistrar () ) );
	...
	public function testPhrasePlaceHolders($placePath) {
		...
		$this->assertEmpty ( $failures, $this->printMessage ( $failures, 'Found discrepancy between keys and translations in count of numeric placeholders' ) );
	}
}
