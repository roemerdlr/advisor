...
class DependencyTest extends \PHPUnit_Framework_TestCase {
	...
	public static function setUpBeforeClass() {
		...
		if (preg_match ( '/magento\/project-*/', $rootJson ['name'] ) == 1) {
			...
			self::markTestSkipped ( 'MAGETWO-43654: The build is running from vendor/magento. DependencyTest is skipped.' );
		}
	...
	protected static function _initRules() {
		...
		self::$_rulesInstances = [ 
				...
				new \Magento\TestFramework\Dependency\LayoutRule ( self::$_mapRouters, self::$_mapLayoutBlocks, self::$_mapLayoutHandles ) 
		];
	}
	...
	protected function _getCleanedFileContents($fileType, $file) {
		...
		switch ($fileType) {
			...
			case 'template' :
				// Removing html
				$contentsWithoutHtml = '';
				preg_replace_callback ( '~(<\?php\s+.*\?>)~sU', function ($matches) use($contents, &$contentsWithoutHtml) {
	...
	public function testRedundant() {
		...
		foreach ( array_keys ( self::$_mapDependencies ) as $module ) {
			...
			if (count ( $redundant )) {
				$result [] = sprintf ( "\r\nModule %s: %s [%s]", $module, self::TYPE_HARD, implode ( ', ', array_values ( $redundant ) ) );
			}
	...
	public function getAllFiles() {
		...
		// Get all php files
		$files = array_merge ( $files, $this->_prepareFiles ( 'php', Files::init ()->getPhpFiles ( Files::INCLUDE_APP_CODE | Files::AS_DATA_SET | Files::INCLUDE_NON_CLASSES ), true ) );
		
		// Get all configuration files
		$files = array_merge ( $files, $this->_prepareFiles ( 'config', Files::init ()->getConfigFiles () ) );
		
		// Get all layout updates files
		$files = array_merge ( $files, $this->_prepareFiles ( 'layout', Files::init ()->getLayoutFiles () ) );
		
		// Get all template files
		$files = array_merge ( $files, $this->_prepareFiles ( 'template', Files::init ()->getPhtmlFiles () ) );
	...
	protected static function _prepareMapRouters() {
		$pattern = '/(?<namespace>[A-Z][a-z]+)[_\/\\\\](?<module>[A-Z][a-zA-Z]+)\/Controller\/' . '(?<path>[\/\w]*).php/';
	 ...
	 *        	The name of a composer json component or dependency e.g. 'magento/module-theme'
		...
		foreach ( $jsonFiles as $file ) {
			...
			if (isset ( $require ) && ! empty ( $require )) {
				foreach ( $require as $requiredModule => $version ) {
					if (0 === strpos ( $requiredModule, 'magento/' ) && 'magento/magento-composer-installer' != $requiredModule) {
						$type = self::TYPE_HARD;
						self::_addDependencies ( $moduleName, $type, self::MAP_TYPE_DECLARED, self::convertModuleName ( $requiredModule ) );
					}
				}
			}
