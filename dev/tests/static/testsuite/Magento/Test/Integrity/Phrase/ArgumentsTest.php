...
class ArgumentsTest extends \Magento\Test\Integrity\Phrase\AbstractTestCase {
	...
	protected function setUp() {
		...
		$this->blackList = [
				...
				$componentRegistrar->getPath ( ComponentRegistrar::MODULE, 'Magento_Translation' ) . '/Model/Js/DataProvider.php' 
		];
	}
	public function testArguments() {
		...
		$this->assertEmpty ( $missedPhraseErrors, sprintf ( "\n%d missed phrases were discovered: \n%s", count ( $missedPhraseErrors ), implode ( "\n\n", $missedPhraseErrors ) ) );
		$this->assertEmpty ( $incorrectNumberOfArgumentsErrors, sprintf ( "\n%d usages of inconsistency the number of arguments and placeholders were discovered: \n%s", count ( $incorrectNumberOfArgumentsErrors ), implode ( "\n\n", $incorrectNumberOfArgumentsErrors ) ) );
	}
