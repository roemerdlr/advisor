...
class ObserverImplementationTest extends \PHPUnit_Framework_TestCase {
	...
	public static function setUpBeforeClass() {
		self::$observerClasses = array_merge ( self::getObserverClasses ( '{*/events.xml,events.xml}', '//observer' ) );
	}
	...
	public function testObserverHasNoExtraPublicMethods() {
		...
		if ($errors) {
			$errors = array_unique ( $errors );
			sort ( $errors );
			$this->fail ( sprintf ( '%d of observers have extra public methods: %s', count ( $errors ), implode ( "\n", $errors ) ) );
		}
	}
