...
class ComposerTest extends \PHPUnit_Framework_TestCase {
	...
	private function validateComposerJsonFile($path) {
		if (self::$isComposerAvailable) {
			self::$shell->execute ( self::$composerPath . ' validate --working-dir=%s', [ 
					$path 
			] );
		}
	}
	...
	private function assertConsistentModuleName(\SimpleXMLElement $xml, $packageName) {
		...
		$this->assertEquals ( $packageName, $this->convertModuleToPackageName ( $moduleName ), "For the module '{$moduleName}', the expected package name is '{$packageName}'" );
	}
	...
	private function assertDependsOnFramework(\StdClass $json) {
		$this->assertObjectHasAttribute ( 'magento/framework', $json, 'This component is expected to depend on magento/framework' );
	}
	...
	private function assertPhpVersionInSync($name, $phpVersion) {
		if (isset ( self::$rootJson ['require'] ['php'] )) {
			$this->assertEquals ( self::$rootJson ['require'] ['php'], $phpVersion, "PHP version {$phpVersion} in component {$name} is inconsistent with version " . self::$rootJson ['require'] ['php'] . ' in root composer.json' );
		}
	}
	...
	private function assertRequireInSync(\StdClass $json) {
		...
		if (isset ( $json->require )) {
			...
			foreach ( array_keys ( ( array ) $json->require ) as $depName ) {
				...
				if (! isset ( self::$rootJson ['require-dev'] [$depName] ) && ! isset ( self::$rootJson ['require'] [$depName] ) && ! isset ( self::$rootJson ['replace'] [$depName] )) {
					$errors [] = "'$name' depends on '$depName'";
				}
			}
			if (! empty ( $errors )) {
				$this->fail ( "The following dependencies are missing in root 'composer.json'," . " while declared in child components.\n" . "Consider adding them to 'require-dev' section (if needed for child components only)," . " to 'replace' section (if they are present in the project)," . " to 'require' section (if needed for the skeleton).\n" . join ( "\n", $errors ) );
			}
		}
	}
