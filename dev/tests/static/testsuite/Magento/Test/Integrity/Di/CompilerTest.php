...
class CompilerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _buildInheritanceHierarchyTree($className, array $allowedFiles) {
		...
		if ($parent && in_array ( $file, $allowedFiles )) {
			$output = array_merge ( $this->_buildInheritanceHierarchyTree ( $parent->getName (), $allowedFiles ), [ 
					$className 
			], $output );
		} else {
			$output [] = $className;
	...
	public function testConstructorIntegrity() {
		$generatorIo = new \Magento\Framework\Code\Generator\Io ( new \Magento\Framework\Filesystem\Driver\File (), $this->_generationDir );
		$generator = new \Magento\Framework\Code\Generator ( $generatorIo, [ 
		...
		spl_autoload_register ( [ 
				$generationAutoloader,
				'load' 
		] );
		
		$invoker = new \Magento\Framework\App\Utility\AggregateInvoker ( $this );
		$invoker ( function ($className) {
