...
class ReferentialTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGroupsExists() {
		...
		if (! empty ( $missing )) {
			$message = sprintf ( "The groups, referenced in store configuration for the payment, " . "don't correspond to any payment group declared in payment.xml: %s", implode ( ', ', $missing ) );
			$this->fail ( $message );
		}
	}
}
