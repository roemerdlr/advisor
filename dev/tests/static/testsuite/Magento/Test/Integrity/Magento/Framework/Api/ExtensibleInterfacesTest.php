...
class ExtensibleInterfacesTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * @return array
	 */
	private function checkGetExtensionAttributes(\ReflectionClass $interfaceReflection, $extensionInterfaceName, $fullInterfaceName) {
		$errors = [ ];
		try {
			...
			if (! preg_match ( $pattern, $methodDocBlock )) {
				$errors [] = "'{$fullInterfaceName}::getExtensionAttributes()' must be declared " . "with a return type of '{$extensionInterfaceName}'.";
			}
		} catch ( \ReflectionException $e ) {
			$errors [] = "The following method should be declared in " . "'{$extensionInterfaceName}'. '{$extensionInterfaceName}' must be specified as" . " a return type for '{$fullInterfaceName}::getExtensionAttributes()'";
		}
	 ...
	 * @return array
	 */
	private function checkSetExtensionAttributes(\ReflectionClass $interfaceReflection, $extensionInterfaceName, $fullInterfaceName) {
		$errors = [ ];
		try {
			...
			if (empty ( $methodParameters )) {
				$errors [] = "'{$extensionInterfaceName}' must be specified as the parameter type " . "in '{$fullInterfaceName}::setExtensionAttributes()'.";
			} else {
			...
			$errors [] = "'{$fullInterfaceName}::setExtensionAttributes()' must be declared " . "with a '{$extensionInterfaceName}' parameter type.";
		}
	...
	public function getInterfacesFiles() {
		...
		foreach ( $filesToCheck as $file ) {
			$interfaces [substr ( $file, strlen ( BP ) )] = [ 
					$file 
			];
		}
		return $interfaces;
	...
	public function getPhpFiles() {
		...
		foreach ( $filesToCheck as $file ) {
			$phpFiles [substr ( $file, strlen ( BP ) )] = [ 
					$file 
			];
		}
		return $phpFiles;
