...
class CodeCheck {
	...
	public static function isNamespaceUsed($namespace, $content) {
		return self::_isRegexpMatched ( '/namespace\s+' . preg_quote ( $namespace, '/' ) . ';/S', $content ) || self::_isRegexpMatched ( '/[^a-zA-Z\d_]' . preg_quote ( $namespace . '\\', '/' ) . '/S', $content );
	}
	...
	public static function isFunctionCalled($method, $content, $class = null) {
		...
		if (! $class) {
			return self::_isRegexpMatched ( '/(?<![a-z\d_:]|->|function\s)' . $quotedMethod . '\s*\(/iS', $content );
		}
		// without opening parentheses to match static callbacks notation
		if (self::_isRegexpMatched ( '/' . preg_quote ( $class, '/' ) . '::\s*' . $quotedMethod . '[^a-z\d_]/iS', $content )) {
			return true;
		}
		if (self::isClassOrInterface ( $content, $class ) || self::isDirectDescendant ( $content, $class )) {
			return self::_isRegexpMatched ( '/this->' . $quotedMethod . '\s*\(/iS', $content ) || self::_isRegexpMatched ( '/(self|static|parent)::\s*' . $quotedMethod . '\s*\(/iS', $content );
		}
	}
	...
	public static function isPropertyUsed($property, $content, $class = null) {
		...
		return self::_isRegexpMatched ( '/[^a-z\d_]' . preg_quote ( $property, '/' ) . '[^a-z\d_]/iS', $content );
	}
	...
	public static function isDirectDescendant($content, $name) {
		...
		return self::_isRegexpMatched ( '/\s+extends\s+' . $name . '\b|\s+implements\s+[^{]*\b' . $name . '\b[^{^\\\\]*\{/iS', $content );
	}
