...
class WordsFinder {
	 ...
	 * @var array
	 */
	protected $copyrightSkipExtensions = [ 
			'csv',
			'json',
			'lock',
			'md',
	...
	/**
	 * Checks the file content and name against the list of words.
	 * Do not check content of binary files.
	...
	protected function _findWords($file) {
		...
		if ($contents && $this->isCopyrightChecked && ! $this->isCopyrightCheckSkipped ( $file ) && (($copyrightStringPosition = mb_strpos ( $contents, $this->copyrightString )) === false || ($copyingStringPosition = strpos ( $contents, $this->copyingString )) === false || $copyingStringPosition - $copyrightStringPosition - mb_strlen ( $this->copyrightString ) > 10)) {
