...
class PhpRule implements RuleInterface {
	...
	public function getDependencyInfo($currentModule, $fileType, $file, &$contents) {
		...
		$pattern = '~\b(?<class>(?<module>(' . implode ( '_|', Files::init ()->getNamespaces () ) . '[_\\\\])[a-zA-Z0-9]+)[a-zA-Z0-9_\\\\]*)\b~';
		...
		if (preg_match_all ( $pattern, $contents, $matches )) {
			...
			foreach ( $matches ['module'] as $i => $referenceModule ) {
				...
				$dependencyType = $this->isPluginDependency ( $currentClass, $dependencyClass ) ? RuleInterface::TYPE_SOFT : RuleInterface::TYPE_HARD;
	...
	private function getClassFromFilepath($filepath, $module) {
		$class = strstr ( $filepath, str_replace ( [ 
				'_',
				'\\',
				'/' 
	...
	private function isPluginDependency($dependent, $dependency) {
		...
		$subject = isset ( $pluginMap [$dependent] ) ? $pluginMap [$dependent] : null;
