...
abstract class AbstractConfig extends \PHPUnit_Framework_TestCase {
	 ...
	 *        	a known bad xml file.
	 * @param $schemaFile string
	 *        	schema that should find errors in the known bad xml file.
	 * @param $fileSchemaFile string
	 *        	schema that should find errors in the known bad xml file
	 */
	protected function _validateFileExpectSuccess($xmlFile, $schemaFile, $fileSchemaFile = null) {
		...
		if ($errors) {
			...
			$this->fail ( 'There is a problem with the schema.  A known good XML file failed validation: ' . PHP_EOL . implode ( PHP_EOL . PHP_EOL, $errors ) );
		}
	}
	 ...
	 *
	 * @param $xmlFile string
	 *        	a known good xml file.
	 * @param $schemaFile string
	 *        	schema that should find no errors in the known good xml file.
	 * @param $expectedErrors null|array
	 *        	that may contain a list of expected errors. Each element can be a substring
	...
	protected function _validateFileExpectFailure($xmlFile, $schemaFile, $expectedErrors = null) {
		...
		if (isset ( $expectedErrors )) {
			$this->assertNotEmpty ( $actualErrors, 'No schema validation errors found, expected errors: ' . PHP_EOL . implode ( PHP_EOL, $expectedErrors ) );
			foreach ( $expectedErrors as $expectedError ) {
				...
				$this->assertTrue ( $found, 'Failed asserting that ' . $expectedError . " is in: \n" . implode ( PHP_EOL, $actualErrors ) );
			}
