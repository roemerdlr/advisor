...
class Helper {
	...
	protected static $reportingLevel = [ 
			...
			self::MISSING => [ 
					self::LEVEL => self::ERROR,
					self::MESSAGE => 'Missing %s doc comment' 
			],
			self::WRONG_STYLE => [ 
					self::LEVEL => self::WARNING,
			...
			self::EMPTY_DOC => [ 
					self::LEVEL => self::WARNING,
					self::MESSAGE => '%s doc comment is empty' 
			],
	 ...
	 * @param int $severity
	 *        	The severity level for this error. A value of 0
	 * @return void
	 */
	 ...
	 * @param string $text        	
	 * @param
	 *        	array &$matches Type that was detected as ambiguous is in result.
	 * @return bool
	 */
