	...
	public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
		...
		if ($code === T_COMMENT) {
			...
			if ($tokens [$commentEnd] ['line'] === $tokens [$commentEnd] ['line']) {
				$this->helper->addMessage ( $stackPtr, Helper::MISSING, [ 
						'function' 
				] );
			} else {
				$this->helper->addMessage ( $stackPtr, Helper::WRONG_STYLE, [ 
						'function' 
				] );
			}
			return;
			...
			$this->helper->addMessage ( $commentEnd, Helper::WRONG_END, [ 
		...
		try {
			...
			$this->helper->addMessage ( $line, Helper::FAILED_PARSE, [ 
					$e->getMessage () 
			] );
			return;
		}
		...
		if (($comment === null) === true) {
			$this->helper->addMessage ( $commentStart, Helper::EMPTY_DOC, [ 
					'Function' 
			] );
			return;
		}
		...
		if (trim ( $short ) === '') {
			$this->helper->addMessage ( $commentStart, Helper::MISSING_SHORT, [ 
					'function' 
			] );
			return;
		}
		...
		if ($short !== '' && $newlineSpan > 0) {
			$this->helper->addMessage ( $commentStart + 1, Helper::SPACING_BEFORE_SHORT, [ 
					'function' 
			] );
		}
		...
		if (empty ( $long ) === false) {
			...
			if ($newlineBetween !== 2) {
				$this->helper->addMessage ( $commentStart + $newlineCount + 1, Helper::SPACING_BETWEEN, [ 
						'function' 
				] );
			}
			...
			if (preg_match ( '|\p{Lu}|u', $testLong [0] ) === 0) {
				$this->helper->addMessage ( $commentStart + $newlineCount, Helper::LONG_NOT_CAPITAL, [ 
						'Function' 
				] );
			}
		}
		...
		if (count ( $params ) > 1) {
			...
			if ($newlineSpan !== 2) {
				...
				$this->helper->addMessage ( $commentStart + $newlineCount, Helper::SPACING_BEFORE_TAGS, [ 
						'function' 
				] );
		...
		if (substr_count ( $testShort, $phpcsFile->eolChar ) !== 0) {
			$this->helper->addMessage ( $commentStart + 1, Helper::SHORT_SINGLE_LINE, [ 
					'Function' 
			] );
		}
		
		if (preg_match ( '|\p{Lu}|u', $testShort [0] ) === 0) {
			$this->helper->addMessage ( $commentStart + 1, Helper::SHORT_NOT_CAPITAL, [ 
					'Function' 
			] );
		}
		
		if ($lastChar !== '.') {
			$this->helper->addMessage ( $commentStart + 1, Helper::SHORT_FULL_STOP, [ 
					'Function' 
			] );
		}
		...
		if (trim ( $words [$lastPos - 1] ) !== '' || strpos ( $words [$lastPos - 1], $this->helper->getCurrentFile ()->eolChar ) === false || trim ( $words [$lastPos - 2] ) === '') {
			$this->helper->addMessage ( $commentEnd, Helper::SPACING_AFTER, [ 
					'function' 
			] );
		}
	}
		...
		if (empty ( $sees ) === false) {
			...
			foreach ( $sees as $i => $see ) {
				...
				if (empty ( $content ) === true) {
					$this->helper->addMessage ( $errorPos, Helper::EMPTY_SEE, [ 
							'function' 
					] );
					continue;
				}
		...
		if (empty ( $params ) === false) {
			...
			foreach ( $params as $param ) {
				...
				foreach ( $typeNames as $typeName ) {
					...
					if ($typeName !== $suggestedName) {
						...
						$data = [ 
								$matches [1],
								$paramName,
								' at position ' . $pos . ' is NOT recommended' 
						];
				...
				if (isset ( $realParams [$pos - 1] ) === true) {
					...
					$this->helper->addMessage ( $errorPos, Helper::EXTRA_PARAM_COMMENT, [ 
							$pos 
					] );
				}
				
				if ($param->getVarName () === '') {
					$this->helper->addMessage ( $errorPos, Helper::MISSING_PARAM_NAME, [ 
							$pos 
					] );
				}
				
				if ($param->getType () === '') {
					$this->helper->addMessage ( $errorPos, Helper::MISSING_PARAM_TYPE, [ 
							$pos 
					] );
				}
