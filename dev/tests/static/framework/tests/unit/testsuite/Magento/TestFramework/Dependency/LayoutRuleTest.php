...
class LayoutRuleTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLayoutGetDependencyInfo($contents, $type, $isHandle) {
		...
		$data = [ 
				'frontend' => [ 
						'any_handle_name' => [ 
								'Magento\AnyHandleModule' => 'Magento\AnyHandleModule' 
						] 
				],
				'default' => [ 
						'singlechunk' => [ 
								'Magento\DefaultHandleModule' => 'Magento\DefaultHandleModule' 
						] 
				] 
		];
		$model = $isHandle ? new LayoutRule ( [ ], [ ], $data ) : new LayoutRule ( [ ], $data, [ ] );
		$this->assertEquals ( [ ], $model->getDependencyInfo ( 'Magento\AnyHandleModule', 'layout', 'path/frontend/file.txt', $contents ) );
		$this->assertEquals ( [ ], $model->getDependencyInfo ( 'Magento\DefaultHandleModule', 'layout', 'any', $contents ) );
		$this->assertEquals ( [ 
				[ 
		...
		$this->assertEquals ( [ ], $model->getDependencyInfo ( 'Magento\HandleModule', 'layout', 'path/frontend/file.txt', $contents ) );
	}
	...
	public function layoutGetDependencyInfoWithReferenceDataProvider() {
		return array_merge ( $this->layoutGetDependencyInfoDataProvider (), [ 
				[ 
