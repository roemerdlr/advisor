...
class TokensTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->parseFactory = $this->getMockBuilder ( 'Magento\TestFramework\Integrity\Library\PhpParser\ParserFactory' )->disableOriginalConstructor ()->getMock ();
	}
	...
	public function testParseContent() {
		...
		$this->parseFactory->expects ( $this->any () )->method ( 'createParsers' )->will ( $this->returnValue ( [ 
				$parser 
		] ) );
	...
	public function testGetDependencies() {
		...
		$staticCalls->expects ( $this->once () )->method ( 'getDependencies' )->will ( $this->returnValue ( [ 
				'StaticDependency' 
		] ) );
		...
		$throws->expects ( $this->once () )->method ( 'getDependencies' )->will ( $this->returnValue ( [ 
				'ThrowDependency' 
		] ) );
