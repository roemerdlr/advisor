...
class CommandTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_cmd = $this->getMock ( 'Magento\TestFramework\Inspection\JsHint\Command', [ 
				'_getHostScript',
				'_fileExists',
				'_getJsHintPath',
				'_executeCommand',
	...
	public function testCanRun() {
		...
		$this->_cmd->expects ( $this->any () )->method ( '_executeCommand' )->with ( $this->stringContains ( 'cscript' ) )->will ( $this->returnValue ( [ 
				'output',
				0 
		] ) );
		$this->_cmd->expects ( $this->any () )->method ( '_getJsHintPath' )->will ( $this->returnValue ( 'jshint-path' ) );
		$this->_cmd->expects ( $this->any () )->method ( '_fileExists' )->with ( $this->isType ( 'string' ) )->will ( $this->returnValue ( true ) );
