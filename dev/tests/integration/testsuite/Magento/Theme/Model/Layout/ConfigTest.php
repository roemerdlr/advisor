...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$fileResolverMock = $this->getMockBuilder ( 'Magento\Framework\Config\FileResolverInterface' )->disableOriginalConstructor ()->getMock ();
		$fileResolverMock->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValue ( [ 
				$configFile 
		] ) );
		$reader = $objectManager->create ( 'Magento\Theme\Model\Layout\Config\Reader', [ 
				'fileResolver' => $fileResolverMock 
		] );
		$dataStorage = $objectManager->create ( 'Magento\Theme\Model\Layout\Config\Data', [ 
				'reader' => $reader 
		] );
		$this->_model = $objectManager->create ( 'Magento\Theme\Model\Layout\Config', [ 
