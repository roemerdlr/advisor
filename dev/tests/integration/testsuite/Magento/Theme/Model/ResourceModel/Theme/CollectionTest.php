...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	protected static function _getThemesCollection() {
		return \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Theme\Model\ResourceModel\Theme\Collection' );
	}
	...
	public function testAddAreaFilter($area, $themeCount) {
		...
		$themeCollection = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Theme\Model\ResourceModel\Theme\Collection' );
	...
	public function addTypeFilterDataProvider() {
		return [ 
				[ 
						'themeType' => ThemeInterface::TYPE_PHYSICAL,
						'themeCount' => 1 
				],
				[ 
						'themeType' => ThemeInterface::TYPE_VIRTUAL,
						'themeCount' => 1 
				],
				[ 
						'themeType' => ThemeInterface::TYPE_STAGING,
						'themeCount' => 1 
				],
				[ 
						'themeType' => false,
	...
	public function testFilterVisibleThemes() {
		...
		/** @var $theme \Magento\Framework\View\Design\ThemeInterface */
		foreach ( $themeCollection as $theme ) {
			$this->assertTrue ( in_array ( $theme->getType (), [ 
	...
	public function testCheckParentInThemes() {
		...
		foreach ( self::getInheritedThemeList () as $themeData ) {
			...
			if ($themeData ['parent_id']) {
				...
				$this->assertEquals ( $parentIdActual, $parentIdExpected, sprintf ( 'Invalid parent_id for theme "%s"', $fullPath ) );
			} else {
				$parentIdExpected = 0;
				$this->assertEquals ( $parentIdExpected, $parentIdActual, sprintf ( 'Parent id should be null for "%s"', $fullPath ) );
			}
		}
	}
	...
	public function testFilterPhysicalThemesPerPage() {
		...
		$this->assertLessThanOrEqual ( \Magento\Theme\Model\ResourceModel\Theme\Collection::DEFAULT_PAGE_SIZE, $collection->count () );
