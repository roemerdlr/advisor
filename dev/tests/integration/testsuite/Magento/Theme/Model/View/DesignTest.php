...
class DesignTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetConfigurationDesignThemeDefaults() {
		...
		$design = $objectManager->create ( 'Magento\Theme\Model\View\Design', [ 
				'themes' => $themes 
		] );
	...
	public function testGetConfigurationDesignThemeStore() {
		...
		$this->assertEquals ( 'one', $this->_model->getConfigurationDesignTheme ( null, [ 
				'store' => $storeId 
		] ) );
		$this->assertEquals ( 'one', $this->_model->getConfigurationDesignTheme ( 'frontend', [ 
				'store' => $storeId 
		] ) );
		$this->assertEquals ( 'two', $this->_model->getConfigurationDesignTheme ( null, [ 
				'store' => 'fixturestore' 
		] ) );
		$this->assertEquals ( 'two', $this->_model->getConfigurationDesignTheme ( 'frontend', [ 
				'store' => 'fixturestore' 
		] ) );
	}
	...
	public function testGetViewConfig() {
		...
		$this->assertEquals ( [ 
				'var1' => 'value1',
				'var2' => 'value2' 
		], $config->getVars ( 'Namespace_Module' ) );
	}
