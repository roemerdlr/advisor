...
class BeforeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$importModel = $this->getMock ( 'Magento\ImportExport\Model\Import', [ 
				'getEntityBehaviors',
				'getUniqueEntityBehaviors' 
		], [ ], '', false );
		$importModel->expects ( $this->any () )->method ( 'getEntityBehaviors' )->will ( $this->returnValue ( $this->_sourceEntities ) );
		$importModel->expects ( $this->any () )->method ( 'getUniqueEntityBehaviors' )->will ( $this->returnValue ( $this->_sourceBehaviors ) );
