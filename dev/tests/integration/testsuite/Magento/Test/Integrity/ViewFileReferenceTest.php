...
class ViewFileReferenceTest extends \PHPUnit_Framework_TestCase {
	...
	public static function setUpBeforeClass() {
		...
		self::$_fallbackRule = $fallbackPool->getRule ( $fallbackPool::TYPE_STATIC_FILE );
...
(strpos ( $patternPath, $localePlaceholder ) !== false)) // It is localized directory
{
	...
	public function testModularFallback($modularCall, array $usages, $area) {
		...
		if ($wrongResolutions) {
			...
			$message = sprintf ( "Found modular call:\n  %s in\n  %s\n  which may resolve to non-modular location(s):\n  %s", $modularCall, implode ( ', ', $usages ), implode ( ', ', $wrongResolutions ) );
			$this->fail ( $message );
		}
	}
	...
	public static function modularFallbackDataProvider() {
		...
		foreach ( self::_getFilesToProcess () as $file ) {
			...
			$pattern = '#' . $modulePattern . preg_quote ( \Magento\Framework\View\Asset\Repository::FILE_ID_SEPARATOR ) . $filePattern . '#S';
	...
	protected static function _getFilesToProcess() {
		...
		$dirs = array_merge ( $componentRegistrar->getPaths ( ComponentRegistrar::MODULE ), $componentRegistrar->getPaths ( ComponentRegistrar::THEME ) );
		foreach ( $dirs as $dir ) {
