...
class TemplateFilesTest extends \PHPUnit_Framework_TestCase {
	...
	public function widgetTemplatesDataProvider() {
		...
		foreach ( $model->getWidgetsArray () as $row ) {
			...
			$instance = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Widget\Model\Widget\Instance' );
			$config = $instance->setType ( $row ['type'] )->getWidgetConfigAsArray ();
			$class = $row ['type'];
			if (is_subclass_of ( $class, 'Magento\Framework\View\Element\Template' )) {
				if (isset ( $config ['parameters'] ) && isset ( $config ['parameters'] ['template'] ) && isset ( $config ['parameters'] ['template'] ['values'] )) {
