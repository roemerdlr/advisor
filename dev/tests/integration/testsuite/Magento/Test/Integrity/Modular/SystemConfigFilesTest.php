...
class SystemConfigFilesTest extends \PHPUnit_Framework_TestCase {
	public function testConfiguration() {
		...
		$configMock = $this->getMock ( 'Magento\Framework\Module\Dir\Reader', [ 
				'getConfigurationFiles',
				'getModuleDir' 
		], [ ], '', false );
		$configMock->expects ( $this->any () )->method ( 'getConfigurationFiles' )->will ( $this->returnValue ( $fileList ) );
		$configMock->expects ( $this->any () )->method ( 'getModuleDir' )->with ( 'etc', 'Magento_Backend' )->will ( $this->returnValue ( $modulesDir->getAbsolutePath () . '/app/code/Magento/Backend/etc' ) );
		try {
			$objectManager->create ( 'Magento\Config\Model\Config\Structure\Reader', [ 
