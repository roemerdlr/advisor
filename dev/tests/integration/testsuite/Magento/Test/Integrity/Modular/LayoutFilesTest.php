...
class LayoutFilesTest extends \PHPUnit_Framework_TestCase {
	...
	public function layoutArgumentsDataProvider() {
		...
		foreach ( $areas as $area ) {
			$layoutFiles = \Magento\Framework\App\Utility\Files::init ()->getLayoutFiles ( [ 
					'area' => $area 
			], false );
			foreach ( $layoutFiles as $layoutFile ) {
				$data [substr ( $layoutFile, strlen ( BP ) )] = [ 
						$area,
						$layoutFile 
				];
			}
	...
	protected function isSkippedArgument(array $argumentData) {
		...
		$ignoredArguments = [ 
				...
				[ 
						$typeAttr => 'object',
						'value' => 'Magento\Solr\Model\ResourceModel\Search\Grid\Collection' 
				],
				[ 
						$typeAttr => 'object',
						'value' => 'Magento\Wishlist\Model\ResourceModel\Item\Collection\Grid' 
				],
				[ 
						$typeAttr => 'object',
				...
				[ 
						$typeAttr => 'options',
						'model' => 'Magento\Solr\Model\Adminhtml\Search\Grid\Options' 
				],
				[ 
						$typeAttr => 'options',
						'model' => 'Magento\Logging\Model\ResourceModel\Grid\ActionsGroup' 
				],
				[ 
						$typeAttr => 'options',
						'model' => 'Magento\Logging\Model\ResourceModel\Grid\Actions' 
				] 
		];
