...
class RouteConfigFilesTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRouteConfigsValidation() {
		...
		$mergedConfig = new \Magento\Framework\Config\Dom ( '<config></config>', $this->validationStateMock, $this->_idAttributes );
		
		foreach ( $files as $file ) {
			$content = file_get_contents ( $file );
			try {
				new \Magento\Framework\Config\Dom ( $content, $this->validationStateMock, $this->_idAttributes, null, $this->schemaFile );
