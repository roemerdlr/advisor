...
class DiConfigFilesTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *
	 * @param string $xml        	
	 * @return void @dataProvider linearFilesProvider
	 */
	public function testDiConfigFileWithoutMerging($xml) {
		/** @var \Magento\Framework\ObjectManager\Config\SchemaLocator $schemaLocator */
		$schemaLocator = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\ObjectManager\Config\SchemaLocator' );
	...
	public function testMergedDiConfig(array $files) {
		...
		new \Magento\Framework\ObjectManager\Config\Reader\Dom ( $fileResolverMock, $mapperMock, $schemaLocator, $validationStateMock );
	}
	public function mixedFilesProvider() {
		...
		$primaryFiles ['all primary config files'] = [ 
				self::$_primaryFiles->toArray () 
		];
		...
		$moduleFiles ['all module global config files'] = [ 
				self::$_moduleGlobalFiles->toArray () 
		];
		...
		foreach ( self::$_moduleAreaFiles as $area => $files ) {
			...
			$areaFiles ["all {$area} config files"] = [ 
					self::$_moduleAreaFiles [$area]->toArray () 
			];
		}
