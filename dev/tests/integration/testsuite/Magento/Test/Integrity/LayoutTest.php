...
class LayoutTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _composeXml(\Magento\Framework\View\Design\ThemeInterface $theme) {
		...
		$layoutUpdate = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\View\Layout\ProcessorInterface', [ 
				'theme' => $theme 
		] );
	...
	public function areasAndThemesDataProvider() {
		$result = [ ];
		$themeCollection = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\View\Design\ThemeInterface' )->getCollection ();
		/** @var $theme \Magento\Framework\View\Design\ThemeInterface */
		foreach ( $themeCollection as $theme ) {
			$result [$theme->getFullPath () . ' [' . $theme->getId () . ']'] = [ 
					$theme 
			];
		}
		return $result;
	...
	public function testHandleLabels() {
		...
		function (\Magento\Framework\View\Design\ThemeInterface $theme) {
			...
			if ($errors) {
				$this->fail ( 'The following handles must have label, but they don\'t have it:' . PHP_EOL . var_export ( $errors, true ) );
			}
	...
	public function testPageTypesDeclaration() {
		...
		function (\Magento\Framework\View\File $layout) {
			...
			$this->assertEmpty ( $content->xpath ( \Magento\Framework\View\Model\Layout\Merge::XPATH_HANDLE_DECLARATION ), "Theme layout update '" . $layout->getFilename () . "' contains page type declaration(s)" );
	...
	public function pageTypesDeclarationDataProvider() {
		...
		$themeUpdates = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\View\File\Collector\ThemeModular', [ 
				'subDir' => 'layout' 
		] );
		/** @var $themeUpdatesOverride \Magento\Framework\View\File\Collector\Override\ThemeModular */
		$themeUpdatesOverride = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\View\File\Collector\Override\ThemeModular', [ 
				'subDir' => 'layout/override/theme' 
		] );
		/** @var $themeCollection \Magento\Theme\Model\Theme\Collection */
		$themeCollection = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Theme\Model\Theme\Collection' );
		...
		foreach ( $themeLayouts as $layout ) {
			$result [$layout->getFileIdentifier ()] = [ 
					$layout 
			];
		}
		return $result;
	...
	public function testOverrideBaseFiles() {
		...
		function ($themeFile, $theme) {
			$baseFiles = self::_getCachedFiles ( $theme->getArea (), 'Magento\Framework\View\File\Collector\Base', $theme );
			$fileKey = $themeFile->getModule () . '/' . $themeFile->getName ();
			$this->assertArrayHasKey ( $fileKey, $baseFiles, sprintf ( "Could not find base file, overridden by theme file '%s'.", $themeFile->getFilename () ) );
	...
	public function testOverrideThemeFiles() {
		...
		function ($themeFile, $theme) {
			...
			$this->assertNotNull ( $ancestorTheme, sprintf ( 'Could not find ancestor theme "%s", ' . 'its layout file is supposed to be overridden by file "%s".', $themeFile->getTheme ()->getCode (), $themeFile->getFilename () ) );
			
			// Search for the overridden file in the ancestor theme
			$ancestorFiles = self::_getCachedFiles ( $ancestorTheme->getFullPath (), 'Magento\Framework\View\File\Collector\ThemeModular', $ancestorTheme );
			$fileKey = $themeFile->getModule () . '/' . $themeFile->getName ();
			$this->assertArrayHasKey ( $fileKey, $ancestorFiles, sprintf ( "Could not find original file in '%s' theme, overridden by file '%s'.", $themeFile->getTheme ()->getCode (), $themeFile->getFilename () ) );
	...
	protected static function _getCachedFiles($cacheKey, $sourceClass, \Magento\Framework\View\Design\ThemeInterface $theme) {
		if (! isset ( self::$_cachedFiles [$cacheKey] )) {
			...
			$fileList = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( $sourceClass, [ 
					'subDir' => 'layout' 
			] )->getFiles ( $theme, '*.xml' );
			$files = [ ];
	...
	public function overrideBaseFilesDataProvider() {
		return $this->_retrieveFilesForEveryTheme ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\View\File\Collector\Override\Base', [ 
	...
	public function overrideThemeFilesDataProvider() {
		return $this->_retrieveFilesForEveryTheme ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\View\File\Collector\Override\ThemeModular', [ 
				'subDir' => 'layout/override/theme' 
		] ) );
	}
	...
	protected function _retrieveFilesForEveryTheme(\Magento\Framework\View\File\CollectorInterface $filesRetriever) {
		...
		foreach ( $themeCollection as $theme ) {
			foreach ( $filesRetriever->getFiles ( $theme, '*.xml' ) as $file ) {
				$result ['theme: ' . $theme->getFullPath () . ', ' . $file->getFilename ()] = [ 
						$file,
						$theme 
				];
			}
