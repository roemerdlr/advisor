...
class ManagestockTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *
	 * @param int $newStockValue
	 *        	new value for stock status
	 * @param int $callCount
	 *        	count matcher
	 */
	public function testSaveAndRebuildIndex($newStockValue, $callCount) {
		/** @var \Magento\CatalogInventory\Model\StockIndex */
		$stockManagement = $this->getMock ( '\Magento\CatalogInventory\Model\StockIndex', [ 
				'rebuild' 
		], [ ], '', false );
		
		$stockManagement->expects ( $this->exactly ( $callCount ) )->method ( 'rebuild' );
