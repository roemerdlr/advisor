...
class TaxTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testAjaxSaveAction($postData, $expectedData) {
		...
		$result = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\Json\Helper\Data' )->jsonDecode ( $jsonBody );
		
		$this->assertArrayHasKey ( 'class_id', $result );
		
		$classId = $result ['class_id'];
		/** @var $class \Magento\Tax\Model\ClassModel */
		$class = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Tax\Model\ClassModel' )->load ( $classId, 'class_id' );
	...
	public function testAjaxDeleteAction($taxClassData) {
		...
		$taxClassService = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Tax\Api\TaxClassRepositoryInterface' );
		
		$taxClassFactory = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Tax\Api\Data\TaxClassInterfaceFactory' );
		$taxClass = $taxClassFactory->create ();
		$taxClass->setClassName ( $taxClassData ['class_name'] )->setClassType ( $taxClassData ['class_type'] );
		
		$taxClassId = $taxClassService->save ( $taxClass );
		
		/** @var $class \Magento\Tax\Model\ClassModel */
		$class = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Tax\Model\ClassModel' )->load ( $taxClassId, 'class_id' );
