...
class SetupUtil {
	...
	protected function createCustomerTaxClass() {
		foreach ( array_keys ( $this->customerTaxClasses ) as $className ) {
			$this->customerTaxClasses [$className] = $this->objectManager->create ( 'Magento\Tax\Model\ClassModel' )->setClassName ( $className )->setClassType ( \Magento\Tax\Model\ClassModel::TAX_CLASS_TYPE_CUSTOMER )->save ()->getId ();
		}
	...
	protected function createProductTaxClass() {
		foreach ( array_keys ( $this->productTaxClasses ) as $className ) {
			$this->productTaxClasses [$className] = $this->objectManager->create ( 'Magento\Tax\Model\ClassModel' )->setClassName ( $className )->setClassType ( \Magento\Tax\Model\ClassModel::TAX_CLASS_TYPE_PRODUCT )->save ()->getId ();
		}
	...
	protected function setConfig($configData) {
		...
		foreach ( $configData as $path => $value ) {
			...
			$config->saveConfig ( $path, $value, ScopeConfigInterface::SCOPE_TYPE_DEFAULT, 0 );
		}
	...
	protected function createTaxRates($overrides) {
		...
		foreach ( array_keys ( $this->taxRates ) as $taxRateCode ) {
			...
			$this->taxRates [$taxRateCode] ['id'] = $this->objectManager->create ( 'Magento\Tax\Model\Calculation\Rate' )->setData ( $this->taxRates [$taxRateCode] ['data'] )->save ()->getId ();
		}
		return $this;
	}
		...
		if (empty ( $overrides [self::TAX_RULE_OVERRIDES] )) {
			...
			$shippingTaxRuleData = [ 
					...
					'product_tax_class_ids' => [ 
							$this->productTaxClasses [self::SHIPPING_TAX_CLASS] 
					],
					'tax_rate_ids' => [ 
							$this->taxRates [self::TAX_RATE_SHIPPING] ['id'] 
					] 
			];
			$this->taxRules [$shippingTaxRuleData ['code']] = $this->objectManager->create ( 'Magento\Tax\Model\Calculation\Rule' )->setData ( $shippingTaxRuleData )->save ()->getId ();
			
			// Create a default tax rule
			$this->taxRules [$taxRuleDefaultData ['code']] = $this->objectManager->create ( 'Magento\Tax\Model\Calculation\Rule' )->setData ( $taxRuleDefaultData )->save ()->getId ();
		} else {
			foreach ( $overrides [self::TAX_RULE_OVERRIDES] as $taxRuleOverrideData ) {
				...
				$this->taxRules [$mergedTaxRuleData ['code']] = $this->objectManager->create ( 'Magento\Tax\Model\Calculation\Rule' )->setData ( $mergedTaxRuleData )->save ()->getId ();
			}
		}
	...
	public function createSimpleProduct($sku, $price, $taxClassId) {
		...
		$product->setTypeId ( 'simple' )->setAttributeSetId ( 4 )->setName ( 'Simple Product' . $sku )->setSku ( $sku )->setPrice ( $price )->setTaxClassId ( $taxClassId )->setStockData ( [ 
				'use_config_manage_stock' => 1,
				'qty' => 100,
				'is_qty_decimal' => 0,
				'is_in_stock' => 1 
		] )->setMetaTitle ( 'meta title' )->setMetaKeyword ( 'meta keyword' )->setMetaDescription ( 'meta description' )->setVisibility ( \Magento\Catalog\Model\Product\Visibility::VISIBILITY_BOTH )->setStatus ( \Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED )->save ();
	...
	protected function createCustomerGroup($customerTaxClassId) {
		...
		$customerGroup = $customerGroupFactory->create ()->setCode ( 'custom_group' )->setTaxClassId ( $customerTaxClassId );
	...
	protected function createCustomer() {
		...
		$customer->setWebsiteId ( 1 )->setEntityTypeId ( 1 )->setAttributeSetId ( 1 )->setEmail ( 'customer@example.com' )->setPassword ( 'password' )->setGroupId ( $customerGroupId )->setStoreId ( 1 )->setIsActive ( 1 )->setFirstname ( 'Firstname' )->setLastname ( 'Lastname' )->save ();
	...
	protected function createCustomerAddress($addressOverride, $customerId) {
		...
		$customerAddress->setData ( $addressData )->setCustomerId ( $customerId )->save ();
