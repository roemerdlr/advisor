...
class TaxRuleFixtureFactory {
	 ...
	 *        	array of rate data, keys are 'country', 'region' and 'percentage'
	...
	public function createTaxRates($ratesData) {
		...
		foreach ( $ratesData as $rateData ) {
			...
			$taxRate->setTaxCountryId ( $rateData ['country'] )->setTaxRegionId ( $rateData ['region'] )->setTaxPostcode ( $postcode )->setCode ( $code )->setRate ( $rateData ['percentage'] );
	...
	public function createTaxClasses($classesData) {
		...
		foreach ( $classesData as $classData ) {
			...
			$class = $this->objectManager->create ( 'Magento\Tax\Model\ClassModel' )->setClassName ( $classData ['name'] )->setClassType ( $classData ['type'] )->save ();
