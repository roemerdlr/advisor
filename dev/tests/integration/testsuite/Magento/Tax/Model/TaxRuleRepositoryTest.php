...
class TaxRuleRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSave() {
		...
		$this->assertEquals ( $taxRuleDataObject->getCustomerTaxClassIds (), $taxRule->getCustomerTaxClassIds () );
	...
	public function testSaveThrowsExceptionIdTargetTaxRulDoesNotExist() {
		...
		$taxRuleDataObject->setId ( 9999 )->setCode ( 'code' )->setCustomerTaxClassIds ( [ 
				3 
		] )->setProductTaxClassIds ( [ 
				2 
		] )->setTaxRateIds ( [ 
				2 
		] )->setPriority ( 0 )->setPosition ( 1 );
	...
	public function testSaveThrowsExceptionIfProvidedTaxClassIdsAreInvalid() {
		...
		$this->dataObjectHelper->populateWithArray ( $taxRule, $taxRuleData, '\Magento\Tax\Api\Data\TaxRuleInterface' );
	 ...
	 * @param string[] $expectedRuleCodes
	 *        	The codes of the tax rules that are expected to be found
	 *        	
	 *        	@magentoDbIsolation enabled
	...
	public function searchTaxRulesDataProvider() {
		...
		return [ 
				...
				'code like "%Rule"' => [ 
						[ 
								$filterBuilder->setField ( 'code' )->setValue ( '%Rule' )->setConditionType ( 'like' )->create () 
						],
						[ ],
						[ 
								'Default Rule',
	...
	private function setUpDefaultRules() {
		...
		$this->taxRules = $this->taxRuleFixtureFactory->createTaxRules ( [ 
				...
				[ 
						...
						'product_tax_class_ids' => [ 
								$this->taxClasses ['HigherProductClass'] 
						],
	...
	private function createTaxRuleDataObject() {
		$taxRule = $this->taxRuleFactory->create ();
		$taxRule->setCode ( 'code' )->setCustomerTaxClassIds ( [ 
				3 
		] )->setProductTaxClassIds ( [ 
				2 
		] )->setTaxRateIds ( [ 
