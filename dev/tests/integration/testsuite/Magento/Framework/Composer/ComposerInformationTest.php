...
class ComposerInformationTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *
	 * @param $composerDir string
	 *        	Directory under _files that contains composer files
	 */
	private function setupDirectory($composerDir) {
		$directories = [ 
				DirectoryList::CONFIG => [ 
						DirectoryList::PATH => __DIR__ . '/_files/' 
				],
				DirectoryList::ROOT => [ 
						DirectoryList::PATH => __DIR__ . '/_files/' . $composerDir 
				],
		...
		$this->directoryList = $this->objectManager->create ( 'Magento\Framework\App\Filesystem\DirectoryList', [ 
				'root' => __DIR__ . '/_files/' . $composerDir,
				'config' => $directories 
		] );
	...
	public function testGetRequiredPhpVersion($composerDir) {
		...
		$composerInfo = $this->objectManager->create ( 'Magento\Framework\Composer\ComposerInformation', [ 
	...
	public function getRequiredPhpVersionDataProvider() {
		return [ 
				...
				'Composer.json from git clone' => [ 
						'testFromClone' 
				],
				'Composer.json from git create project' => [ 
	...
	public function testNoLock() {
		...
		$this->objectManager->create ( 'Magento\Framework\Composer\ComposerInformation', [ 
				'applicationFactory' => new MagentoComposerApplicationFactory ( $this->composerJsonFinder, $this->directoryList ) 
		] );
	}
