...
class MinifierTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCSSminLibrary() {
		...
		$this->assertEquals ( file_get_contents ( dirname ( __DIR__ ) . '/_files/static/expected/styles.magento.min.css' ), $adapter->minify ( file_get_contents ( dirname ( __DIR__ ) . '/_files/static/theme/web/css/styles.css' ) ), 'Minified CSS differs from initial minified CSS snapshot. ' . 'Ensure that new CSS is fully valid for all supported browsers ' . 'and replace old minified snapshot with new one.' );
	}
	...
	public function testJshrinkLibrary() {
		...
		$this->assertEquals ( file_get_contents ( dirname ( __DIR__ ) . '/_files/static/expected/test.min.js' ), $adapter->minify ( file_get_contents ( dirname ( __DIR__ ) . '/_files/static/theme/web/js/test.js' ) ), 'Minified JS differs from initial minified JS snapshot. ' . 'Ensure that new JS is fully valid for all supported browsers ' . 'and replace old minified snapshot with new one.' );
	}
	...
	protected function _testCssMinification($requestedUri, $assertionCallback) {
		...
		$response = $this->getMockBuilder ( 'Magento\Framework\App\Response\FileInterface' )->setMethods ( [ 
				'setFilePath' 
		...
		$staticResourceApp = $this->objectManager->create ( 'Magento\Framework\App\StaticResource', [ 
				'response' => $response 
		] );
	...
	public function testCssMinification() {
		$this->_testCssMinification ( '/frontend/FrameworkViewMinifier/default/en_US/css/styles.min.css', function ($path) {
			$this->assertEquals ( file_get_contents ( dirname ( __DIR__ ) . '/_files/static/expected/styles.magento.min.css' ), file_get_contents ( $path ), 'Minified files are not equal or minification did not work for requested CSS' );
		} );
	}
	...
	public function testCssMinificationOff() {
		$this->_testCssMinification ( '/frontend/FrameworkViewMinifier/default/en_US/css/styles.css', function ($path) {
			$content = file_get_contents ( $path );
			$this->assertNotEmpty ( $content );
			$this->assertContains ( 'FrameworkViewMinifier/frontend', $content );
			$this->assertNotEquals ( file_get_contents ( dirname ( __DIR__ ) . '/_files/static/expected/styles.magento.min.css' ), $content, 'CSS is minified when minification turned off' );
		} );
	}
	...
	public function testCssMinificationForMinifiedFiles() {
		$this->_testCssMinification ( '/frontend/FrameworkViewMinifier/default/en_US/css/preminified-styles.min.css', function ($path) {
			$content = file_get_contents ( $path );
			$this->assertNotEmpty ( $content );
	...
	public function testDeploymentWithMinifierEnabled() {
		...
		$omFactory = $this->getMock ( '\Magento\Framework\App\ObjectManagerFactory', [ 
				'create' 
		], [ ], '', false );
		$omFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->objectManager ) );
		
		$output = $this->objectManager->create ( 'Symfony\Component\Console\Output\ConsoleOutput' );
		...
		$filesUtil->expects ( $this->any () )->method ( 'getStaticPreProcessingFiles' )->will ( $this->returnValue ( [ 
				[ 
						'frontend',
						'FrameworkViewMinifier/default',
						'',
						'',
						'css/styles.css',
						$fileToTestPublishing 
				] 
		] ) );
		
		/** @var \Magento\Deploy\Model\Deployer $deployer */
		$deployer = $this->objectManager->create ( 'Magento\Deploy\Model\Deployer', [ 
				'filesUtil' => $filesUtil,
				'output' => $output,
				'isDryRun' => false 
		] );
		
		$deployer->deploy ( $omFactory, [ 
				'en_US' 
		] );
