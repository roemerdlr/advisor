...
class MergeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_layoutValidator = $this->getMock ( 'Magento\Framework\View\Model\Layout\Update\Validator', [ ], [ ], '', false );
		...
		$fileReader->expects ( $this->any () )->method ( 'readAll' )->will ( $this->returnCallback ( function ($filename) use($fileDriver) {
			return $fileDriver->fileGetContents ( __DIR__ . '/_mergeFiles/layout/' . $filename );
		} ) );
		
		$this->_model = $objectHelper->getObject ( 'Magento\Framework\View\Model\Layout\Merge', [ 
				'design' => $design,
	...
	public function testAddPageHandles() {
		...
		$this->assertFalse ( $this->_model->addPageHandles ( [ 
				'non_existing_handle' 
		] ) );
		...
		$this->assertTrue ( $this->_model->addPageHandles ( [ 
				'default',
				'checkout_index_index' 
		] ) );
		$this->assertEquals ( $expectedPageHandles, $this->_model->getPageHandles () );
	...
	public function testLoadFileSystemWithPageLayout() {
		$handles = [ 
				'fixture_handle_with_page_layout' 
		];
		$expectedHandles = [ 
				'fixture_handle_with_page_layout' 
		];
	...
	public function testLoadCache() {
		$this->_cache->expects ( $this->at ( 0 ) )->method ( 'load' )->with ( 'LAYOUT_area_STORE20_100c6a4ccd050e33acef0553f24ef399961' )->will ( $this->returnValue ( self::FIXTURE_LAYOUT_XML ) );
	...
	public function testLoadDbApp() {
		$this->_resource->expects ( $this->any () )->method ( 'fetchUpdatesByHandle' )->with ( 'fixture_handle', $this->_theme, $this->scope )->will ( $this->returnValue ( self::FIXTURE_LAYOUT_XML ) );
		$this->assertEmpty ( $this->_model->getHandles () );
		$this->assertEmpty ( $this->_model->asString () );
		$handles = [ 
				'fixture_handle_one' 
	...
	public function testLoadWithInvalidLayout() {
		...
		$this->_layoutValidator->expects ( $this->any () )->method ( 'getMessages' )->will ( $this->returnValue ( [ 
				'testMessage1',
				'testMessage2' 
		] ) );
		...
		$this->_logger->expects ( $this->once () )->method ( 'info' )->with ( 'Cache file with merged layout: ' . $cacheId . ': ' . array_shift ( $messages ) );
