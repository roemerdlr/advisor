...
class Layout {
	...
	public function getLayoutUpdateFromFixture($layoutUpdatesFile) {
		...
		$pageLayoutFileSource = $this->_testCase->getMockForAbstractClass ( 'Magento\Framework\View\File\CollectorInterface' );
		$pageLayoutFileSource->expects ( \PHPUnit_Framework_TestCase::any () )->method ( 'getFiles' )->willReturn ( [ ] );
		$cache = $this->_testCase->getMockForAbstractClass ( 'Magento\Framework\Cache\FrontendInterface' );
		return $objectManager->create ( 'Magento\Framework\View\Layout\ProcessorInterface', [ 
	...
	public function getLayoutFromFixture($layoutUpdatesFile, array $args = []) {
		$layout = $this->_testCase->getMock ( 'Magento\Framework\View\Layout', [ 
				'getUpdate' 
		], $args );
