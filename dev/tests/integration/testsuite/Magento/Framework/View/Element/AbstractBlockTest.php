...
class AbstractBlockTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_block = $this->getMockForAbstractClass ( 'Magento\Framework\View\Element\AbstractBlock', [ 
	...
	public function testCssWithWrongImage() {
		$cssUrl = $this->_block->getViewFileUrl ( 'css/wrong.css', [ 
				'area' => 'frontend',
				'theme' => 'Magento/luma',
				'locale' => 'en_US' 
		] );
	...
	public function testAddChild() {
		...
		$child = $parentBlock->addChild ( 'testAddChildAlias', 'Magento\Framework\View\Element\Text', [ 
				'content' => 'content' 
		] );
	...
	public function testInsertContainer() {
		...
		$this->assertEquals ( [ 
				$name 
		], $layout->getChildNames ( $parentName ) );
	}
	...
	public function testGetUrl() {
		...
		$this->assertEquals ( $withRoute, $this->_block->getUrl ( 'catalog/product/view', [ 
				'id' => 10 
		] ) );
	}
	...
	public function testGetCacheKeyInfo() {
		$name = uniqid ( 'block.' );
		$block = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\View\LayoutInterface' )->createBlock ( 'Magento\Framework\View\Element\Text' );
		$block->setNameInLayout ( $name );
		$this->assertEquals ( [ 
				$name 
	...
	protected function _createBlockWithLayout($name = 'block', $alias = null, $type = 'Magento\Framework\View\Element\AbstractBlock') {
		...
		if (! isset ( self::$_mocks [$mockClass] )) {
			self::$_mocks [$mockClass] = $this->getMockForAbstractClass ( $type, [ 
					\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\View\Element\Context' ),
					[ 
							'module_name' => 'Magento_Theme' 
					] 
