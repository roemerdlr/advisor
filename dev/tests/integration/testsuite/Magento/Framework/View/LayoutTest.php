...
class LayoutTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerateXml() {
		...
		$merge = $this->getMock ( 'StdClass', [ 
				'asSimplexml' 
		] );
		$merge->expects ( $this->once () )->method ( 'asSimplexml' )->will ( $this->returnValue ( simplexml_load_string ( '<layout><container name="container1"></container></layout>', 'Magento\Framework\View\Layout\Element' ) ) );
	 ...
	 * 
	 * @see \Magento\Framework\View\LayoutDirectivesTest @magentoAppIsolation enabled
	 */
	...
	public function testGetElementProperty() {
		$name = 'test';
		$this->_layout->addContainer ( $name, 'Test', [ 
				'option1' => 1,
				'option2' => 2 
		] );
		$this->assertEquals ( 'Test', $this->_layout->getElementProperty ( $name, \Magento\Framework\View\Layout\Element::CONTAINER_OPT_LABEL ) );
		$this->assertEquals ( \Magento\Framework\View\Layout\Element::TYPE_CONTAINER, $this->_layout->getElementProperty ( $name, 'type' ) );
		...
		$this->assertEquals ( \Magento\Framework\View\Layout\Element::TYPE_BLOCK, $this->_layout->getElementProperty ( 'text', 'type' ) );
		$this->assertSame ( [ 
				'text' => 'text' 
	...
	public function testCreateBlock($blockType, $blockName, array $blockData, $expectedName) {
		...
		$block = $this->_layout->createBlock ( $blockType, $blockName, [ 
				'data' => $blockData 
		] );
	...
	public function createBlockDataProvider() {
		return [ 
				...
				'no name block' => [ 
						...
						'',
						[ 
								'type' => 'Magento\Framework\View\Element\Text\ListText',
								'key1' => 'value1' 
						],
						'/text\\\\list/' 
				] 
	...
	public function testAddBlock() {
		$this->assertInstanceOf ( 'Magento\Framework\View\Element\Text', $this->_layout->addBlock ( 'Magento\Framework\View\Element\Text', 'block1' ) );
	...
	public function testAddContainerInvalidHtmlTag() {
		$msg = 'Html tag "span" is forbidden for usage in containers. ' . 'Consider to use one of the allowed: dd, div, dl, fieldset, main, header, ' . 'footer, ol, p, section, table, tfoot, ul.';
	...
	public function testSetChild() {
		...
		$this->assertSame ( [ 
				'two',
				'three' 
		], $this->_layout->getChildNames ( 'one' ) );
		return $this->_layout;
	}
	...
	public function testGetChildBlocks() {
		...
		$this->assertSame ( [ 
				'block1' => $block1,
				'block2' => $block2 
		], $this->_layout->getChildBlocks ( 'parent' ) );
	}
	...
	public function testIsCacheable() {
		$this->_layout->setXml ( simplexml_load_file ( __DIR__ . '/_files/layout/cacheable.xml', 'Magento\Framework\View\Layout\Element' ) );
		$this->_layout->generateElements ();
