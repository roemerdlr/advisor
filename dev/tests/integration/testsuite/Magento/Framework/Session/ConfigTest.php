...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->deploymentConfigMock->expects ( $this->at ( 0 ) )->method ( 'get' )->with ( $this->equalTo ( Config::PARAM_SESSION_SAVE_METHOD ), $this->anything () )->will ( $this->returnValue ( 'files' ) );
		$this->deploymentConfigMock->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( Config::PARAM_SESSION_SAVE_PATH )->will ( $this->returnValue ( null ) );
		$this->deploymentConfigMock->expects ( $this->at ( 2 ) )->method ( 'get' )->with ( Config::PARAM_SESSION_CACHE_LIMITER )->will ( $this->returnValue ( $this->_cacheLimiter ) );
		
		$this->_model = $this->_objectManager->create ( 'Magento\Framework\Session\Config', [ 
	...
	public function testDefaultConfiguration() {
		...
		$path = ini_get ( 'session.save_path' ) ?  : $filesystem->getDirectoryRead ( DirectoryList::SESSION )->getAbsolutePath ();
		
		$this->assertEquals ( $path, $this->_model->getSavePath () );
	...
	public function testConstructorSavePath($existing, $given, $expected) {
		...
		$this->deploymentConfigMock->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( Config::PARAM_SESSION_SAVE_PATH )->will ( $this->returnValue ( $given ) );
