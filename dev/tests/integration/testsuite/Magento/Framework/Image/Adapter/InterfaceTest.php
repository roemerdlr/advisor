...
class InterfaceTest extends \PHPUnit_Framework_TestCase {
	...
	public function testImageSize($image, $adapterType) {
		...
		try {
			$adapter->open ( $image );
			$this->assertEquals ( $this->_getFixtureImageSize (), [ 
	...
	public function testResize($image, $dims, $adapterType) {
		...
		try {
			...
			$result = $dims [0] !== null && $dims [0] <= 0 || $dims [1] !== null && $dims [1] <= 0 || empty ( ${$dims [0]} ) && empty ( ${$dims [1]} );
	...
	public function testRotate($image, $angle, $pixel, $adapterType) {
		...
		$newPixel = $this->_convertCoordinates ( $pixel, $angle, $size, [ 
				$adapter->getOriginalWidth (),
				$adapter->getOriginalHeight () 
		] );
	...
	public function testWatermark($image, $watermark, $width, $height, $opacity, $position, $colorX, $colorY, $adapterType) {
		$adapter = $this->_getAdapter ( $adapterType );
		$adapter->open ( $image );
		$pixel = $this->_prepareColor ( [ 
				'x' => $colorX,
				'y' => $colorY 
		], $position, $adapter );
		
		$colorBefore = $adapter->getColorAt ( $pixel ['x'], $pixel ['y'] );
		$adapter->setWatermarkWidth ( $width )->setWatermarkHeight ( $height )->setWatermarkImageOpacity ( $opacity )->setWatermarkPosition ( $position )->watermark ( $watermark );
	...
	public function testCreatePngFromString($pixel1, $expectedColor1, $pixel2, $expectedColor2, $adapterType) {
		...
		$readFactory = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\Filesystem\Directory\ReadFactory' );
