...
class TranslateCachingTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLoadDataCaching() {
		...
		$this->assertEquals ( 'Fixture Db Translation', new Phrase ( 'Fixture String' ), 'Translation is expected to be cached' );
		
		$model->loadData ( \Magento\Framework\App\Area::AREA_FRONTEND, true );
		$this->assertEquals ( 'New Db Translation', new Phrase ( 'Fixture String' ), 'Forced load should not use cache' );
	}
}
