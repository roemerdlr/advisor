...
class CookieScopeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetSensitiveCookieMetadataEmpty() {
		$serverVal = $_SERVER;
		$this->request->setServer ( new Parameters ( array_merge ( $_SERVER, [ 
				'HTTPS' => 'on' 
		] ) ) );
	...
	protected function createSensitiveMetadata($metadata = []) {
		return $this->objectManager->create ( 'Magento\Framework\Stdlib\Cookie\SensitiveCookieMetadata', [ 
				'metadata' => $metadata 
		] );
	}
	...
	protected function createPublicMetadata($metadata = []) {
		return $this->objectManager->create ( 'Magento\Framework\Stdlib\Cookie\PublicCookieMetadata', [ 
				'metadata' => $metadata 
		] );
	}
	...
	protected function createCookieMetadata($metadata = []) {
		return $this->objectManager->create ( 'Magento\Framework\Stdlib\Cookie\CookieMetadata', [ 
				'metadata' => $metadata 
		] );
	}
}
