...
class CopyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_service = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\DataObject\Copy' );
	}
	...
	public function testCopyFieldsetWithExtensionAttributes() {
		...
		$fieldsetConfigMock = $this->getMockBuilder ( '\Magento\Framework\DataObject\Copy\Config' )->disableOriginalConstructor ()->setMethods ( [ 
				'getFieldSet' 
		] )->getMock ();
		
		$service = $objectManager->create ( 'Magento\Framework\DataObject\Copy', [ 
		...
		$fieldsetConfigMock->expects ( $this->once () )->method ( 'getFieldSet' )->willReturn ( $data );
		...
		$data = [ 
				...
				'extension_attributes' => [ 
						'test_group_code' => $groupCode 
				] 
		];
		$dataWithExtraField = array_merge ( $data, [ 
				'undeclared_field' => 'will be omitted' 
		] );
		...
		$dataObjectHelper->populateWithArray ( $source, $dataWithExtraField, 'Magento\Wonderland\Api\Data\FakeCustomerInterface' );
		...
		$this->assertNull ( $target->getEmail (), "Email should not be set because it is not defined in the fieldset." );
		$this->assertEquals ( $firstName, $target->getFirstname (), "First name was not copied." );
		$this->assertEquals ( $groupCode, $target->getExtensionAttributes ()->getTestGroupCode (), "Extension attribute was not copied." );
	}
