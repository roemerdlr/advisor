...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->model = $objectManager->create ( 'Magento\Framework\DataObject\Copy\Config\Reader', [ 
				'fileResolver' => $this->fileResolver 
		] );
	}
	public function testRead() {
		$this->fileResolver->expects ( $this->once () )->method ( 'get' )->with ( 'fieldset.xml', 'global' )->willReturn ( [ 
				file_get_contents ( __DIR__ . '/_files/fieldset.xml' ) 
		] );
	...
	public function testMergeCompleteAndPartial() {
		...
		$this->fileResolver->expects ( $this->once () )->method ( 'get' )->with ( 'fieldset.xml', 'global' )->willReturn ( $fileList );
		$expected = [ 
				'global' => [ 
						'quote_convert_item' => [ 
								'event_id' => [ 
								...
								'event_description' => [ 
										'to_order_item' => "complexDescription" 
								] 
						] 
				] 
		];
