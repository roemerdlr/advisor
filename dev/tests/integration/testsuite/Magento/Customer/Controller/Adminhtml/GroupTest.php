...
class GroupTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testNewActionNoCustomerGroupDataInSession() {
		...
		$expected = '<input id="customer_group_code" name="code"  ' . 'data-ui-id="group-form-fieldset-element-text-code"  value=""';
	...
	public function testNewActionWithCustomerGroupDataInSession() {
		...
		$customerGroupFactory = $this->_objectManager->get ( 'Magento\Customer\Api\Data\GroupInterfaceFactory' );
		/** @var \Magento\Customer\Api\Data\GroupInterface $customerGroup */
		$customerGroup = $customerGroupFactory->create ()->setCode ( self::CUSTOMER_GROUP_CODE )->setTaxClassId ( self::TAX_CLASS_ID );
		...
		$customerGroupData = $dataObjectProcessor->buildOutputDataArray ( $customerGroup, 'Magento\Customer\Api\Data\GroupInterface' );
		...
		$responseBody = $this->getResponse ()->getBody ();
		$this->assertRegExp ( '/<h1 class\="page-title">\s*New Customer Group\s*<\/h1>/', $responseBody );
		$expected = '<input id="customer_group_code" name="code"  ' . 'data-ui-id="group-form-fieldset-element-text-code"  value="' . self::CUSTOMER_GROUP_CODE . '"';
		$this->assertContains ( $expected, $responseBody );
	}
	...
	public function testSaveActionExistingGroup() {
		...
		$customerGroupData = $simpleDataObjectConverter->toFlatArray ( $this->groupRepository->getById ( $groupId ), 'Magento\Customer\Api\Data\GroupInterface' );
