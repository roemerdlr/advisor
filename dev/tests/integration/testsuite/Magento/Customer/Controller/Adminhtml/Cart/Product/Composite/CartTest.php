...
class CartTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testConfigureActionNoQuoteId() {
		...
		$this->dispatch ( 'backend/customer/cart_product_composite_cart/configure' );
		$this->assertEquals ( '{"error":true,"message":"Please correct the quote items and try again."}', $this->getResponse ()->getBody () );
	}
	...
	public function testConfigureAction() {
		...
		$this->dispatch ( 'backend/customer/cart_product_composite_cart/configure' );
		$this->assertContains ( '<input id="product_composite_configure_input_qty" class="input-text admin__control-text qty"' . ' type="text" name="qty" value="1">', $this->getResponse ()->getBody () );
	}
