...
class ViewTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_helper = Bootstrap::getObjectManager ()->create ( 'Magento\Customer\Helper\View', [ 
	...
	public function testGetCustomerName($customerData, $expectedCustomerName, $isPrefixAllowed = false, $isMiddleNameAllowed = false, $isSuffixAllowed = false) {
		...
		$this->_customerMetadataService->expects ( $this->any () )->method ( 'getAttributeMetadata' )->will ( $this->returnValueMap ( [ 
				[ 
						'prefix',
						$isPrefixAllowed ? $visibleAttribute : $invisibleAttribute 
				],
				[ 
						'middlename',
						$isMiddleNameAllowed ? $visibleAttribute : $invisibleAttribute 
				],
				[ 
						'suffix',
						$isSuffixAllowed ? $visibleAttribute : $invisibleAttribute 
				] 
		] ) );
	...
	public function getCustomerNameDataProvider() {
		...
		return [ 
				'With disabled prefix, middle name, suffix' => [ 
						$customerFactory->create ()->setPrefix ( 'prefix' )->setFirstname ( 'FirstName' )->setMiddlename ( 'MiddleName' )->setLastname ( 'LastName' )->setSuffix ( 'suffix' ),
						'FirstName LastName' 
				],
...
,
				'Empty prefix and suffix, not empty middle name' => [ 
						$customerFactory->create ()->setFirstname ( 'FirstName' )->setMiddlename ( 'MiddleName' )->setLastname ( 'LastName' ),
