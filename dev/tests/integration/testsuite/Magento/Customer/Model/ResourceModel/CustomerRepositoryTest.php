...
class CustomerRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testUpdateCustomerAddress() {
		...
		$newAddress = array_merge ( $addresses [0]->__toArray (), [ 
				'city' => $city 
		] );
		...
		$newCustomerEntity->setId ( $customerId )->setAddresses ( [ 
	...
	public function testUpdateCustomerPreserveAllAddresses() {
		...
		$newCustomerEntity = $this->customerFactory->create ();
		$this->dataObjectHelper->populateWithArray ( $newCustomerEntity, $customerDetails, '\Magento\Customer\Api\Data\CustomerInterface' );
		$newCustomerEntity->setId ( $customer->getId () )->setAddresses ( null );
		$this->customerRepository->save ( $newCustomerEntity );
	...
	public function testSearchCustomersOrder() {
		...
		$sortOrder = $sortOrderBuilder->setField ( 'lastname' )->setDirection ( SortOrder::SORT_ASC )->create ();
		...
		$sortOrder = $sortOrderBuilder->setField ( 'lastname' )->setDirection ( SortOrder::SORT_DESC )->create ();
		$searchBuilder->addSortOrder ( $sortOrder );
		$searchResults = $this->customerRepository->getList ( $searchBuilder->create () );
	...
	public function searchCustomersDataProvider() {
		...
		return [ 
				...
				'Customers created since' => [ 
						[ 
								$builder->setField ( 'created_at' )->setValue ( '2011-02-28 15:52:26' )->setConditionType ( 'gt' )->create () 
						],
						[ ],
						[ 
								1 => [ 
	 ...
	 *        	$defaultShipping
	 */
	protected function expectedDefaultShippingsInCustomerModelAttributes($customerId, $defaultBilling, $defaultShipping) {
		/**
		 *
		...
		$this->assertEquals ( $defaultBilling, $customer->getDefaultBilling (), 'default_billing customer attribute did not updated' );
		$this->assertEquals ( $defaultShipping, $customer->getDefaultShipping (), 'default_shipping customer attribute did not updated' );
	}
}
