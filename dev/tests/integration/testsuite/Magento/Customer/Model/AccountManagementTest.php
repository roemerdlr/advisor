...
class AccountManagementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$address->setId ( '1' )->setCountryId ( 'US' )->setCustomerId ( '1' )->setPostcode ( '75477' )->setRegion ( $regionFactory->create ()->setRegionCode ( 'AL' )->setRegion ( 'Alabama' )->setRegionId ( 1 ) )->setRegionId ( 1 )->setCompany ( 'CompanyName' )->setStreet ( [ 
				'Green str, 67' 
		] )->setTelephone ( '3468676' )->setCity ( 'CityM' )->setFirstname ( 'John' )->setLastname ( 'Smith' )->setIsDefaultShipping ( true )->setIsDefaultBilling ( true );
		
		$address2 = $this->addressFactory->create ();
		$address2->setId ( '2' )->setCountryId ( 'US' )->setCustomerId ( '1' )->setPostcode ( '47676' )->setRegion ( $regionFactory->create ()->setRegionCode ( 'AL' )->setRegion ( 'Alabama' )->setRegionId ( 1 ) )->setRegionId ( 1 )->setCompany ( 'Company' )->setStreet ( [ 
				'Black str, 48' 
		] )->setCity ( 'CityX' )->setTelephone ( '3234676' )->setFirstname ( 'John' )->setLastname ( 'Smith' );
		
		$this->_expectedAddresses = [ 
				$address,
				$address2 
		];
	...
	public function testActivateCustomerWrongAccount() {
		...
		try {
			...
			$this->assertEquals ( 'No such entity with email = 1234customer@needAconfirmation.com, websiteId = 1', $nsee->getMessage () );
		}
	}
	...
	public function testSendPasswordResetLinkBadEmailOrWebsite() {
		...
		try {
			$this->accountManagement->initiatePasswordReset ( $email, AccountManagement::EMAIL_RESET, 0 );
			$this->fail ( 'Expected exception not thrown.' );
	...
	public function testResetPasswordTokenWrongUser() {
		...
		try {
			...
			$this->assertEquals ( 'No such entity with email = invalid-customer@example.com, websiteId = 1', $nsee->getMessage () );
		}
	}
	...
	public function testResendConfirmationBadWebsiteId() {
		try {
			...
			$this->assertEquals ( 'No such entity with email = customer@needAconfirmation.com, websiteId = notAWebsiteId', $nsee->getMessage () );
		}
	}
	...
	public function testResendConfirmationNoEmail() {
		try {
			...
			$this->assertEquals ( 'No such entity with email = wrongemail@example.com, websiteId = 1', $nsee->getMessage () );
		}
	}
	...
	public function testCreateCustomerInServiceVsInModel() {
		...
		$customerModel->save ();
		/** @var \Magento\Customer\Model\Customer $customerModel */
		$savedModel = $this->objectManager->create ( 'Magento\Customer\Model\CustomerFactory' )->create ()->load ( $customerModel->getId () );
		$dataInModel = $savedModel->getData ();
		
		$newCustomerEntity = $this->customerFactory->create ()->setEmail ( $email2 )->setFirstname ( $firstname )->setLastname ( $lastname )->setGroupId ( $groupId );
		...
		$simpleDataObjectConverter = Bootstrap::getObjectManager ()->get ( 'Magento\Framework\Api\SimpleDataObjectConverter' );
		
		$dataInService = $simpleDataObjectConverter->toFlatArray ( $savedCustomer, 'Magento\Customer\Api\Data\CustomerInterface' );
		$expectedDifferences = [ 
				'created_at',
				'updated_at',
				'email',
	...
	public function testGetDefaultBillingAddress() {
		...
		$expected = $this->dataProcessor->buildOutputDataArray ( $this->_expectedAddresses [0], 'Magento\Customer\Api\Data\AddressInterface' );
