...
class EditTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->editBlock = Bootstrap::getObjectManager ()->create ( 'Magento\Integration\Block\Adminhtml\Integration\Edit' );
	}
	public function testGetHeaderTextNewIntegration() {
		$this->assertEquals ( 'New Integration', $this->editBlock->getHeaderText ()->getText () );
		$buttonList = Bootstrap::getObjectManager ()->get ( 'Magento\Backend\Block\Widget\Context' )->getButtonList ()->getItems ();
		...
		foreach ( $buttonList as $button ) {
			foreach ( $button as $key => $value ) {
				if ($key === 'save') {
					...
					$this->assertEquals ( 'activate', $value->getDataByKey ( 'options' ) ['save_activate'] ['id'], "'Activate' button is expected when creating a new integration." );
				}
			}
		}
	...
	public function testGetHeaderTextEditIntegrationConfigType() {
		...
		$editBlock = Bootstrap::getObjectManager ()->create ( 'Magento\Integration\Block\Adminhtml\Integration\Edit' );
