...
class AuthorizationServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_service = $objectManager->create ( 'Magento\Integration\Model\AuthorizationService', [ 
				'logger' => $loggerMock 
		] );
		
		$this->userContextMock = $this->getMockForAbstractClass ( 'Magento\Authorization\Model\UserContextInterface' );
		$this->userContextMock->expects ( $this->any () )->method ( 'getUserType' )->will ( $this->returnValue ( UserContextInterface::USER_TYPE_INTEGRATION ) );
		$roleLocator = $objectManager->create ( 'Magento\Webapi\Model\WebapiRoleLocator', [ 
				'userContext' => $this->userContextMock 
		] );
		$this->libAuthorization = $objectManager->create ( 'Magento\Framework\Authorization', [ 
	...
	public function testGrantAllPermissions() {
		...
		$this->_ensurePermissionsAreGranted ( $integrationId, [ 
				'Magento_Backend::all' 
		] );
	}
	...
	protected function _ensurePermissionsAreGranted($integrationId, $resources) {
		...
		foreach ( $resources as $resource ) {
			$this->assertTrue ( $this->libAuthorization->isAllowed ( $resource ), "Access to resource '{$resource}' is prohibited while it is expected to be granted." );
		}
	}
