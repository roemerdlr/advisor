...
class OrderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLoad($query, $limit, $start, $expectedResult) {
		...
		foreach ( $expectedResult as $itemIndex => $expectedItem ) {
			...
			$this->assertContains ( "order/view/order_id/{$orderIdByIncrementId[$orderIncrementId]}", $searchResults [$itemIndex] ['url'], 'Item URL is invalid.' );
			...
			foreach ( $expectedItem as $field => $value ) {
				$this->assertEquals ( $value, ( string ) $searchResults [$itemIndex] [$field], "Data of item #$itemIndex is invalid." );
			}
		}
	}
