...
class CacheTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testFlushAllAction() {
		...
		$cachePool = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\App\Cache\Frontend\Pool' );
	...
	public function testFlushSystemAction() {
		...
		/** @var $cacheFrontend \Magento\Framework\Cache\FrontendInterface */
		foreach ( $cachePool as $cacheFrontend ) {
			$this->assertSame ( 'non-application cache data', $cacheFrontend->getBackend ()->load ( 'NON_APPLICATION_FIXTURE' ) );
		}
	}
	...
	public function testMassActionsInvalidTypes($action) {
		...
		$this->assertSessionMessages ( $this->contains ( "Specified cache type(s) don't exist: invalid_type_1, invalid_type_2" ), \Magento\Framework\Message\MessageInterface::TYPE_ERROR );
	}
