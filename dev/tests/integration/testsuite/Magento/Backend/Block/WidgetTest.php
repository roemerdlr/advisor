...
class WidgetTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetButtonHtml() {
		...
		$this->assertRegExp ( '/<button.*onclick\=\"this.form.submit\(\)\".*\>[\s\S]*Button Label[\s\S]*<\/button>/iu', $widget->getButtonHtml ( 'Button Label', 'this.form.submit()' ) );
	}
	...
	public function testGetButtonHtmlForTwoButtonsInOneBlock() {
		...
		$this->assertRegExp ( '/<button.*onclick\=\"this.form.submit\(\)\".*\>[\s\S]*Button Label2[\s\S]*<\/button>/iu', $widget->getButtonHtml ( 'Button Label2', 'this.form.submit()' ) );
	}
