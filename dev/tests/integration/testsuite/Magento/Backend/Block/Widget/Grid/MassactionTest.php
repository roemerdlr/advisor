...
class MassactionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_layout = $objectManager->create ( 'Magento\Framework\View\LayoutInterface', [ 
				'area' => 'adminhtml' 
		] );
	...
	public function testMassactionDefaultValues() {
		...
		$blockEmpty = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\View\LayoutInterface' )->createBlock ( 'Magento\Backend\Block\Widget\Grid\Massaction' );
	...
	public function testGetJavaScript() {
		...
		$expectedItemFirst = '#"option_id1":{"label":"Option One",' . '"url":"http:\\\/\\\/localhost\\\/index\.php\\\/(?:key\\\/([\w\d]+)\\\/)?",' . '"complete":"Test","id":"option_id1"}#';
		$this->assertRegExp ( $expectedItemFirst, $javascript );
		
		$expectedItemSecond = '#"option_id2":{"label":"Option Two",' . '"url":"http:\\\/\\\/localhost\\\/index\.php\\\/(?:key\\\/([\w\d]+)\\\/)?",' . '"confirm":"Are you sure\?","id":"option_id2"}#';
	 ...
	 * @param
	 *        	$itemId
	 * @param $expectedItem @dataProvider
