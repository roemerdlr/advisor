...
class PaymentTest extends \Magento\TestFramework\TestCase\AbstractController {
	public function testResponseActionValidationFailed() {
		...
		$this->assertContains ( 'authorizenet/directpost_payment/redirect/success/0/error_msg/The transaction was' . ' declined because the response hash validation failed.', 
	...
	public function testBackendResponseActionValidationFailed() {
		...
		$this->dispatch ( 'authorizenet/directpost_payment/backendresponse' );
		// @codingStandardsIgnoreStart
		$this->assertContains ( 'authorizenet_directpost_payment/redirect/success/0/error_msg/The transaction was declined' . ' because the response hash validation failed./controller_action_name/action_name/', 
				// @codingStandardsIgnoreEnd
				$this->getResponse ()->getBody () );
	}
}
