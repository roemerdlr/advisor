...
class SourceThemeDeployCommandTest extends \PHPUnit_Framework_TestCase {
	...
	private function collectFiles($path) {
		...
		return new \RegexIterator ( $iterator, '#\.less$#', \RegexIterator::MATCH, \RegexIterator::USE_KEY );
	}
	...
	private function getInputMock() {
		...
		$inputMock->expects ( self::once () )->method ( 'getArgument' )->with ( 'file' )->willReturn ( $this->compiledFiles );
