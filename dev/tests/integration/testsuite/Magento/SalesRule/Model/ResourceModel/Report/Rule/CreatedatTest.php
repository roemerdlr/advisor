...
class CreatedatTest extends \PHPUnit_Framework_TestCase {
	...
	public function testTotals($orderParams) {
		...
		$order->loadByIncrementId ( '100000001' )->setBaseGrandTotal ( $orderParams ['base_subtotal'] )->setSubtotal ( $orderParams ['base_subtotal'] )->setBaseSubtotal ( $orderParams ['base_subtotal'] )->setBaseDiscountAmount ( $orderParams ['base_discount_amount'] )->setBaseTaxAmount ( $orderParams ['base_tax_amount'] )->setBaseSubtotalInvoiced ( $orderParams ['base_subtotal_invoiced'] )->setBaseDiscountInvoiced ( $orderParams ['base_discount_invoiced'] )->setBaseTaxInvoiced ( $orderParams ['base_tax_invoiced'] )->setBaseShippingAmount ( 0 )->setBaseToGlobalRate ( 1 )->setCouponCode ( '1234567890' )->setCreatedAt ( '2014-10-25 10:10:10' )->save ();
		// refresh report statistics
		/** @var \Magento\SalesRule\Model\ResourceModel\Report\Rule $reportResource */
		$reportResource = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\SalesRule\Model\ResourceModel\Report\Rule' );
		$reportResource->aggregate ();
		/** @var \Magento\SalesRule\Model\ResourceModel\Report\Collection $reportCollection */
		$reportCollection = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\SalesRule\Model\ResourceModel\Report\Collection' );
	...
	private function getTotalAmountActual(\Magento\Sales\Model\Order $order) {
		return ($order->getBaseSubtotalInvoiced () - $order->getSubtotalRefunded () - abs ( $order->getBaseDiscountInvoiced () ) - abs ( $order->getBaseDiscountRefunded () ) + $order->getBaseTaxInvoiced () - $order->getBaseTaxRefunded ()) * $order->getBaseToGlobalRate ();
	}
