...
class JsTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetAllRatesByProductClassJson() {
		...
		$fixtureTaxRule->setCustomerTaxClassIds ( array_merge ( $fixtureTaxRule->getCustomerTaxClasses (), [ 
				$defaultCustomerTaxClass 
		] ) )->setProductTaxClassIds ( $fixtureTaxRule->getProductTaxClasses () )->setTaxRateIds ( $fixtureTaxRule->getRates () )->saveCalculationData ();
		...
		$expectedProductTaxClasses = array_unique ( array_merge ( $fixtureTaxRule->getProductTaxClasses (), [ 
				$defaultProductTaxClass,
				$noneTaxClass 
		] ) );
		$this->assertCount ( count ( $expectedProductTaxClasses ), $taxClassesArray, 'Invalid quantity of rates for tax classes.' );
		foreach ( $expectedProductTaxClasses as $taxClassId ) {
			$this->assertArrayHasKey ( "value_{$taxClassId}", $taxClassesArray, "Rates for tax class with ID '{$taxClassId}' is missing." );
		}
