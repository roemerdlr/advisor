...
class CategoryTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testSaveActionFromProductCreationPage($postData) {
		...
		if (empty ( $postData ['return_session_messages_only'] )) {
			$this->assertRedirect ( $this->stringContains ( 'http://localhost/index.php/backend/catalog/category/edit/id/' ) );
		} else {
			$result = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\Json\Helper\Data' )->jsonDecode ( $body );
	...
	public function testSuggestCategoriesActionDefaultCategoryFound() {
		...
		$this->assertEquals ( '[{"id":"2","children":[],"is_active":"1","label":"Default Category"}]', $this->getResponse ()->getBody () );
	}
	...
	public function testMoveAction($parentId, $childId, $childUrlKey, $grandChildId, $grandChildUrlKey, $error) {
		...
		foreach ( $urlKeys as $categoryId => $urlKey ) {
			...
			$category = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Category' );
			if ($categoryId > 0) {
				$category->load ( $categoryId )->setUrlKey ( $urlKey )->save ();
			}
		}
		$this->getRequest ()->setPostValue ( 'id', $grandChildId )->setPostValue ( 'pid', $parentId );
