...
class CompareTest extends \Magento\TestFramework\TestCase\AbstractController {
	public function testAddAction() {
		...
		$this->assertContains ( 'Simple Product 1 Name', ( string ) $messageManager->getMessages ()->getLastAddedMessage ()->getText () );
	...
	public function testRemoveAction() {
		...
		$this->assertInstanceOf ( 'Magento\Framework\Message\Success', $messageManager->getMessages ()->getLastAddedMessage () );
		$this->assertContains ( 'Simple Product 2 Name', ( string ) $messageManager->getMessages ()->getLastAddedMessage ()->getText () );
	...
	public function testRemoveActionProductNameXss() {
		...
		$messages = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\Message\Manager' )->getMessages ()->getItems ();
	...
	protected function _requireCustomerWithTwoProducts() {
		...
		$customer->setWebsiteId ( 1 )->setId ( 1 )->setEntityTypeId ( 1 )->setAttributeSetId ( 1 )->setEmail ( 'customer@example.com' )->setPassword ( 'password' )->setGroupId ( 1 )->setStoreId ( 1 )->setIsActive ( 1 )->setFirstname ( 'Firstname' )->setLastname ( 'Lastname' )->setDefaultBilling ( 1 )->setDefaultShipping ( 1 );
		...
		/** @var $item \Magento\Catalog\Model\Product\Compare\Item */
		$item = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Product\Compare\Item' );
		$item->setVisitorId ( $visitor->getId () )->setCustomerId ( 1 )->setProductId ( 1 )->save ();
	...
	protected function _assertCompareListEquals(array $expectedProductIds) {
		...
		$compareItems = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\ResourceModel\Product\Compare\Item\Collection' );
		$compareItems->useProductItem ( true );
		// important
		$compareItems->setVisitorId ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Customer\Model\Visitor' )->getId () );
		$actualProductIds = [ ];
