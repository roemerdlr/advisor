...
class AlgorithmBaseTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPricesSegmentation($categoryId, array $entityIds, array $intervalItems) {
		...
		$termBucket = $objectManager->create ( 'Magento\Framework\Search\Request\Aggregation\TermBucket', [ 
				'name' => 'name',
				'field' => 'price',
				'metrics' => [ ] 
		] );
		...
		$dimensions = [ 
				'scope' => $objectManager->create ( 'Magento\Framework\Search\Request\Dimension', [ 
						'name' => 'someName',
						'value' => 'default' 
				] ) 
		];
		...
		$entityMetadata = $objectManager->create ( 'Magento\Framework\Search\EntityMetadata', [ 
				'entityId' => 'id' 
		] );
		$idKey = $entityMetadata->getEntityId ();
		
		/** @var \Magento\Framework\Search\Adapter\Mysql\DocumentFactory $documentFactory */
		$documentFactory = $objectManager->create ( 'Magento\Framework\Search\Adapter\Mysql\DocumentFactory', [ 
