...
class AlgorithmAdvancedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _prepareFilter($layer, $priceResource, $request = null) {
		...
		$model = Bootstrap::getObjectManager ()->create ( 'Magento\Framework\Search\Dynamic\Algorithm' );
		/** @var $filter \Magento\Catalog\Model\Layer\Filter\Price */
		$filter = Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Layer\Filter\Price', [ 
				'layer' => $layer,
				'resource' => $priceResource,
				'priceAlgorithm' => $model 
		] );
		$filter->setLayer ( $layer )->setAttributeModel ( new DataObject ( [ 
				'attribute_code' => 'price' 
		] ) );
		if ($request !== null) {
			$filter->apply ( $request, Bootstrap::getObjectManager ()->get ( 'Magento\Framework\View\LayoutInterface' )->createBlock ( 'Magento\Framework\View\Element\Text' ) );
		...
		$model->setStatistics ( $collection->getMinPrice (), $collection->getMaxPrice (), $collection->getPriceStandardDeviation (), $collection->getPricesCount () );
		return $model;
	}
	...
	protected function createLayer() {
		...
		$layer->setState ( Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Layer\State' ) );
		return $layer;
	}
	...
	protected function createPriceResource($layer) {
		return Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\ResourceModel\Layer\Filter\Price', [ 
				'layer' => $layer 
		] );
	}
	...
	protected function createInterval($priceResource) {
		return Bootstrap::getObjectManager ()->create ( 'Magento\CatalogSearch\Model\Price\Interval', [ 
				'resource' => $priceResource 
		] );
	}
}
