...
class AbstractTypeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$productRepository = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Catalog\Api\ProductRepositoryInterface' );
		$catalogProductOption = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Catalog\Model\Product\Option' );
	...
	public function testAttributesCompare() {
		$attribute [1] = new \Magento\Framework\DataObject ( [ 
				'group_sort_path' => 1,
				'sort_path' => 10 
		] );
		$attribute [2] = new \Magento\Framework\DataObject ( [ 
				'group_sort_path' => 1,
				'sort_path' => 5 
		] );
		$attribute [3] = new \Magento\Framework\DataObject ( [ 
				'group_sort_path' => 2,
				'sort_path' => 10 
		] );
	...
	public function testPrepareForCartOptionsException() {
		...
		$product->load ( 1 );
		// fixture
		$this->assertEquals ( 'Please specify product\'s required option(s).', $this->_model->prepareForCart ( new \Magento\Framework\DataObject (), $product ) );
	}
	public function testGetSpecifyOptionMessage() {
		$this->assertEquals ( 'Please specify product\'s required option(s).', $this->_model->getSpecifyOptionMessage () );
	}
	...
	public function testGetOrderOptions() {
		...
		$product->load ( 1 );
		// fixture
		$product->addCustomOption ( 'info_buyRequest', serialize ( new \Magento\Framework\DataObject ( [ 
				'qty' => 2 
		] ) ) );
	...
	public function testGetForceChildItemQtyChanges() {
		$this->assertFalse ( $this->_model->getForceChildItemQtyChanges ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Product' ) ) );
	}
	...
	public function testSetConfig() {
		$this->assertFalse ( $this->_model->isComposite ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Product' ) ) );
		$this->assertTrue ( $this->_model->canUseQtyDecimals () );
		$config = [ 
				'composite' => 1,
				'can_use_qty_decimals' => 0 
		];
		$this->_model->setConfig ( $config );
		$this->assertTrue ( $this->_model->isComposite ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Product' ) ) );
	...
	public function testGetProductsToPurchaseByReqGroups() {
		...
		], $this->_model->getProductsToPurchaseByReqGroups ( $product ) );
