...
class ImageTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSetBaseFilePlaceholder() {
		...
		$model = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Product\Image' );
	...
	public function testSaveFilePlaceholder($model) {
		$processor = $this->getMock ( 'Magento\Framework\Image', [ 
				'save' 
		], [ ], '', false );
	...
	public function testSetWatermark() {
		...
		$viewFileSystem->expects ( $this->once () )->method ( 'getStaticFileName' )->with ( $inputFile )->will ( $this->returnValue ( $expectedFile ) );
		...
		$processor->expects ( $this->once () )->method ( 'watermark' )->with ( $expectedFile );
