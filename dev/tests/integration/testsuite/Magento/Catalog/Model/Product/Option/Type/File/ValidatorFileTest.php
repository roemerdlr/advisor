...
class ValidatorFileTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->httpFactoryMock = $this->getMock ( 'Magento\Framework\HTTP\Adapter\FileTransferFactory', [ 
				'create' 
		] );
		...
		$this->model = $this->objectManager->create ( 'Magento\Catalog\Model\Product\Option\Type\File\ValidatorFile', [ 
	...
	public function testRunValidationException() {
		$httpAdapterMock = $this->getMock ( 'Zend_File_Transfer_Adapter_Http', [ 
				'isValid' 
		] );
	...
	public function testLargeSizeFile() {
		$this->setExpectedException ( '\Magento\Framework\Exception\LocalizedException', sprintf ( 'The file you uploaded is larger than %s Megabytes allowed by server', $this->maxFileSizeInMb ) );
		$this->prepareEnv ();
	...
	public function testInvalidateFile() {
		$this->setExpectedException ( '\Magento\Framework\Exception\LocalizedException', "The file 'test.jpg' for 'MediaOption' has an invalid extension.\n" . "The file 'test.jpg' for 'MediaOption' has an invalid extension.\n" . "The maximum allowed image size for 'MediaOption' is 2000x2000 px.\n" . sprintf ( "The file 'test.jpg' you uploaded is larger than the %s megabytes allowed by our server.", $this->maxFileSizeInMb ) );
		$this->prepareEnv ();
		$httpAdapterMock = $this->getMock ( 'Zend_File_Transfer_Adapter_Http', [ 
				'isValid',
				'getErrors' 
	...
	public function testValidate() {
		...
		$result = $this->model->validate ( $this->objectManager->create ( 'Magento\Framework\DataObject' ), $this->getProductOption () );
