...
class AddressTest extends \PHPUnit_Framework_TestCase {
	...
	protected $_updateData = [ 
			'address' => [  // address records
					...
					'new_no_address_id' => '32301' 
			] // new address without address id
,
...
;
	...
	protected function setUp() {
		...
		$this->customerResource = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Customer\Model\ResourceModel\Customer' );
		$this->_entityAdapter = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( $this->_testClassName );
	}
	...
	public function testConstruct() {
		// check entity table
		$this->assertAttributeInternalType ( 'string', '_entityTable', $this->_entityAdapter, 'Entity table must be a string.' );
		$this->assertAttributeNotEmpty ( '_entityTable', $this->_entityAdapter, 'Entity table must not be empty' );
		
		// check message templates
		$this->assertAttributeInternalType ( 'array', 'errorMessageTemplates', $this->_entityAdapter, 'Templates must be an array.' );
		$this->assertAttributeNotEmpty ( 'errorMessageTemplates', $this->_entityAdapter, 'Templates must not be empty' );
		
		// check attributes
		$this->assertAttributeInternalType ( 'array', '_attributes', $this->_entityAdapter, 'Attributes must be an array.' );
		$this->assertAttributeNotEmpty ( '_attributes', $this->_entityAdapter, 'Attributes must not be empty' );
		
		// check addresses
		$this->assertAttributeInternalType ( 'array', '_addresses', $this->_entityAdapter, 'Addresses must be an array.' );
		$this->assertAttributeNotEmpty ( '_addresses', $this->_entityAdapter, 'Addresses must not be empty' );
		
		// check country regions and regions
		$this->assertAttributeInternalType ( 'array', '_countryRegions', $this->_entityAdapter, 'Country regions must be an array.' );
	...
	public function testSaveAddressEntities() {
		...
		// check DB
		$testAddress = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Customer\Model\Address' );
	...
	protected function _addTestAddress(Address $entityAdapter) {
		...
		$addressModel = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Customer\Model\Address' );
		$tableName = $addressModel->getResource ()->getEntityTable ();
		$addressId = $objectManager->get ( 'Magento\ImportExport\Model\ResourceModel\Helper' )->getNextAutoincrement ( $tableName );
