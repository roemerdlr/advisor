...
class UpdateOrderStatusForPaymentMethodsObserverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testUpdateOrderStatusForPaymentMethodsEvent() {
		...
		$this->_objectManager->create ( 'Magento\Config\Model\Config' )->setSection ( 'payment' )->setWebsite ( 'base' )->setGroups ( [ 
				'groups' => $data ['groups'] 
		] )->save ();
		...
		$defaultStatus = ( string ) $scopeConfig->getValue ( 'payment/checkmo/order_status', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
		
		/** @var \Magento\Config\Model\ResourceModel\Config $config */
		$config = $this->_objectManager->get ( 'Magento\Config\Model\ResourceModel\Config' );
		$config->saveConfig ( 'payment/checkmo/order_status', $statusCode, ScopeConfigInterface::SCOPE_TYPE_DEFAULT, 0 );
		
		$this->_resetConfig ();
		
		$newStatus = ( string ) $scopeConfig->getValue ( 'payment/checkmo/order_status', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
		
		$status->unassignState ( \Magento\Sales\Model\Order::STATE_NEW );
		
		$this->_resetConfig ();
		
		$unassignedStatus = $scopeConfig->getValue ( 'payment/checkmo/order_status', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
	...
	public function testUpdateOrderStatusForPaymentMethods() {
		...
		$observer = $this->_objectManager->create ( 'Magento\Payment\Observer\UpdateOrderStatusForPaymentMethodsObserver' );
		...
		/** @var $scopeConfig \Magento\Framework\App\Config\ScopeConfigInterface */
		$scopeConfig = $this->_objectManager->get ( 'Magento\Framework\App\Config\ScopeConfigInterface' );
		$unassignedStatus = ( string ) $scopeConfig->getValue ( 'payment/checkmo/order_status', \Magento\Store\Model\ScopeInterface::SCOPE_STORE );
		$this->assertEquals ( 'pending', $unassignedStatus );
	}
	...
	protected function _createEventObserver() {
		...
		$event = $this->_objectManager->create ( 'Magento\Framework\Event', [ 
				'data' => $data 
		] );
		return $this->_objectManager->create ( 'Magento\Framework\Event\Observer', [ 
				'data' => [ 
						'event' => $event 
				] 
		] );
