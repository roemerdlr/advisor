...
class TemplateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function mockModel($filesystem = null) {
		...
		$this->mail = $this->getMock ( 'Zend_Mail', [ 
				'send',
				'addTo',
				'addBcc',
				'setReturnPath',
		...
		$this->model = $this->getMockBuilder ( 'Magento\Email\Model\Template' )->setMethods ( [ 
				'_getMail' 
		] )->setConstructorArgs ( [ 
		...
		$this->model->expects ( $this->any () )->method ( '_getMail' )->will ( $this->returnCallback ( [ 
				$this,
				'getMail' 
		] ) );
	...
	public function testSetGetTemplateFilter() {
		...
		$this->assertEquals ( $this->objectManager->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ()->getId (), $filter->getStoreId () );
	...
	public function testGetProcessedTemplate() {
		...
		$this->model->setDesignConfig ( [ 
				'area' => 'frontend',
				'store' => $this->objectManager->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ( 'fixturestore' )->getId () 
		] );
	...
	public function testTemplateStylesVariable($area, $expectedOutput, $unexpectedOutputs, $templateForDatabase = []) {
		if (count ( $templateForDatabase )) {
			...
			$themeDirectory = $this->getMockBuilder ( 'Magento\Framework\Filesystem\Directory\ReadInterface' )->disableOriginalConstructor ()->setMethods ( [ 
					'readFile' 
			] )->getMockForAbstractClass ();
			
			$themeDirectory->expects ( $this->once () )->method ( 'readFile' )->will ( $this->returnValue ( '<!--@styles p { color: #111; } @--> {{var template_styles}}' ) );
			
			$filesystem = $this->getMockBuilder ( '\Magento\Framework\Filesystem' )->disableOriginalConstructor ()->setMethods ( [ 
					'getDirectoryRead' 
			] )->getMock ();
	...
	protected function setUpAdminThemeFallback() {
		$themes = [ 
				BackendFrontNameResolver::AREA_CODE => 'Vendor_EmailTest/custom_theme' 
		];
	...
	protected function setUpThemeFallback($area) {
		...
		$this->objectManager->get ( 'Magento\Framework\App\Config\MutableScopeConfigInterface' )->setValue ( DesignInterface::XML_PATH_THEME_ID, $themeId, ScopeInterface::SCOPE_STORE, 'fixturestore' );
	...
	protected function setNotDefaultThemeForFixtureStore() {
		...
		\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\App\Config\MutableScopeConfigInterface' )->setValue ( \Magento\Framework\View\DesignInterface::XML_PATH_THEME_ID, $theme->getId (), \Magento\Store\Model\ScopeInterface::SCOPE_STORE, 'fixturestore' );
	}
	...
	public function testGetDefaultEmailLogo() {
		...
		$this->assertStringEndsWith ( 'static/frontend/Magento/luma/en_US/Magento_Email/logo_email.png', $this->model->getDefaultEmailLogo () );
	}
	
	/**
	 *
	 * @param $config @dataProvider
	...
	public function setDesignConfigExceptionDataProvider() {
		$storeId = Bootstrap::getObjectManager ()->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ()->getId ();
