...
class ProductTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_model = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\CatalogImportExport\Model\Import\Product' );
	}
	...
	public function testSaveProductsVisibility() {
		...
		$source = new \Magento\ImportExport\Model\Import\Source\Csv ( __DIR__ . '/_files/products_to_import.csv', $directory );
		...
		foreach ( $productsBeforeImport as $productBeforeImport ) {
			...
			$productAfterImport = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\Product' );
	...
	public function testSaveDatetimeAttribute() {
		...
		$source = new \Magento\ImportExport\Model\Import\Source\Csv ( __DIR__ . '/_files/products_to_import_with_datetime.csv', $directory );
		$errors = $this->_model->setParameters ( [ 
				'behavior' => \Magento\ImportExport\Model\Import::BEHAVIOR_APPEND,
				'entity' => 'catalog_product' 
		] )->setSource ( $source )->validateData ();
		...
		foreach ( $productData ['data'] as $data ) {
			...
			if (! empty ( $data ['_custom_option_row_title'] ) && empty ( $data ['_custom_option_store'] )) {
				...
				foreach ( $this->_assertOptionValues as $assertKey ) {
					$valueKey = \Magento\CatalogImportExport\Model\Import\Product\Option::COLUMN_PREFIX . 'row_' . $assertKey;
	 ...
	 * @return array
	 */
	protected function mergeWithExistingData(array $expected, \Magento\Catalog\Model\ResourceModel\Product\Option\Collection $options) {
	...
	public function testSaveMediaImage() {
		...
		$source = new \Magento\ImportExport\Model\Import\Source\Csv ( __DIR__ . '/_files/import_media.csv', $directory );
	...
	public function testInvalidSkuLink() {
		...
		$this->assertEquals ( 'Invalid value for Attribute Set column (set doesn\'t exist?)', $errors->getErrorByRowNumber ( 1 ) [0]->getErrorMessage () );
		$this->_model->importData ();
		
		$productCollection = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Catalog\Model\ResourceModel\Product\Collection' );
	...
	public function testValidateInvalidMultiselectValues() {
		$filesystem = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Framework\Filesystem' );
		$directory = $filesystem->getDirectoryWrite ( DirectoryList::ROOT );
		$source = new \Magento\ImportExport\Model\Import\Source\Csv ( __DIR__ . '/_files/products_with_invalid_multiselect_values.csv', $directory );
		$errors = $this->_model->setParameters ( [ 
				'behavior' => \Magento\ImportExport\Model\Import::BEHAVIOR_APPEND,
				'entity' => 'catalog_product' 
		] )->setSource ( $source )->validateData ();
		...
		$this->assertTrue ( $errors->getErrorsCount () == 1 );
		$this->assertEquals ( "Value for 'multiselect_attribute' attribute contains incorrect value, " . "see acceptable values on settings specified for Admin", $errors->getErrorByRowNumber ( 1 ) [0]->getErrorMessage () );
	}
	...
	public function testProductWithInvalidWeight() {
		...
		$this->assertTrue ( $errors->getErrorsCount () == 1 );
		$this->assertEquals ( "Value for 'weight' attribute contains incorrect value", $errors->getErrorByRowNumber ( 1 ) [0]->getErrorMessage () );
	}
