...
class ChangeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testChangeEncryptionKeyConfigNotWritable() {
		...
		$keyChangeModel = $this->objectManager->create ( 'Magento\EncryptionKey\Model\ResourceModel\Key\Change', [ 
				'writer' => $writerMock 
		] );
	...
	public function testChangeEncryptionKey() {
		...
		$structureMock->expects ( $this->once () )->method ( 'getFieldPathsByAttribute' )->will ( $this->returnValue ( [ 
				$testPath 
		] ) );
		...
		$configModel = $this->objectManager->create ( 'Magento\Config\Model\ResourceModel\Config' );
		...
		$values1 = $connection->fetchPairs ( $connection->select ()->from ( $keyChangeModel->getTable ( 'core_config_data' ), [ 
				'config_id',
				'value' 
		] )->where ( 'path IN (?)', [ 
				$testPath 
		...
		$values2 = $connection->fetchPairs ( $connection->select ()->from ( $keyChangeModel->getTable ( 'sales_order_payment' ), [ 
				'entity_id',
				'cc_number_enc' 
		] ) );
