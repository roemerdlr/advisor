...
class SaveTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testSaveActionWithEmptyKey() {
		...
		$request->setPostValue ( 'generate_random', $ifGenerateRandom )->setPostValue ( 'crypt_key', $encryptionKey );
		$this->dispatch ( 'backend/admin/crypt_key/save' );
		$this->assertSessionMessages ( $this->contains ( 'Please enter an encryption key.' ), \Magento\Framework\Message\MessageInterface::TYPE_ERROR );
	}
	...
	public function testSaveActionWithProvidedKey() {
		...
		$this->assertRedirect ();
		$this->assertSessionMessages ( $this->contains ( 'The encryption key has been changed.' ), \Magento\Framework\Message\MessageInterface::TYPE_SUCCESS );
	}
	...
	public function testSaveActionWithInvalidKey() {
		...
		$this->assertRedirect ();
		$this->assertSessionMessages ( $this->contains ( 'The encryption key format is invalid.' ), \Magento\Framework\Message\MessageInterface::TYPE_ERROR );
	}
