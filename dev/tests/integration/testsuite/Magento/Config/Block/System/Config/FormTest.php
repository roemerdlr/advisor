...
class FormTest extends \PHPUnit_Framework_TestCase {
	 ...
	 *        	uses the test_field_use_config field if true
	 * @param bool $isConfigDataEmpty
	 *        	if the config data array should be empty or not
	 * @param string $configDataValue
	 *        	The value that the field path should be set to in the config data
	 * @param int $valueSelCtr
	 *        	Number of time that value is selected
	 * @param bool $expectedUseDefault
	 *        	@dataProvider initFieldsUseDefaultCheckboxDataProvider
	 */
	public function testInitFieldsUseDefaultCheckbox($fieldId, $isConfigDataEmpty, $configDataValue, $expectedUseDefault, $valueSelCtr = 1) {
		...
		$useDefaultSel = sprintf ( 'input#%s_%s_%s_inherit.checkbox', $this->_section->getId (), $this->_group->getId (), $this->_field->getId () );
		...
		$this->assertSelectCount ( $useDefaultSel, $valueSelCtr, $fieldsetHtml, '"Use Default" checkbox should appear' . $valueSelCtr . ' times  in fieldset HTML.' );
		
		if ($expectedUseDefault) {
			$this->assertSelectCount ( $useDefaultCheckedSel, true, $fieldsetHtml, '"Use Default" checkbox should be checked' );
			$this->assertSelectCount ( $valueDisabledSel, true, $fieldsetHtml, 'Field input should be disabled' );
		} else {
			$this->assertSelectCount ( $useDefaultCheckedSel, false, $fieldsetHtml, '"Use Default" checkbox should not be checked' );
	...
	public static function initFieldsUseDefaultCheckboxDataProvider() {
		return [ 
				...
				[ 
						'test_field_use_config_module_1',
						false,
						'config value',
						false 
				],
				[ 
						'test_field_use_config_module_0',
						false,
						'config value',
						false,
						0 
	 ...
	 *        	if the config data array should be empty or not
	 * @param string $configDataValue
	 *        	Value that the field path should be set to in the config data
	 * @param int $valueSelCtr
	 *        	Number of time that value is selected
	...
	protected function _setupFieldsInheritCheckbox($fieldId, $isConfigDataEmpty, $configDataValue) {
		...
		$structureReader = $objectManager->create ( 'Magento\Config\Model\Config\Structure\Reader', [ 
				'fileResolver' => $fileResolverMock 
		] );
		$structureData = $objectManager->create ( 'Magento\Config\Model\Config\Structure\Data', [ 
				'reader' => $structureReader 
		] );
		/** @var \Magento\Config\Model\Config\Structure $structure  */
		$structure = $objectManager->create ( 'Magento\Config\Model\Config\Structure', [ 
		...
		if ($isConfigDataEmpty) {
			...
			$this->_configData = [ 
					$fieldPath => $configDataValue 
			];
		}
	}
	public function testInitFormAddsFieldsets() {
		\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\App\ResponseInterface' )->headersSentThrowsException = false;
		\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\App\RequestInterface' )->setParam ( 'section', 'general' );
		/** @var $block \Magento\Config\Block\System\Config\Form */
		$block = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\View\LayoutInterface' )->createBlock ( 'Magento\Config\Block\System\Config\Form' );
		$block->initForm ();
		$expectedIds = [ 
				'general_country' => [ 
