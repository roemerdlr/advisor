...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getFormInstance($args = []) {
		...
		$block = $layout->createBlock ( 'Magento\UrlRewrite\Block\Catalog\Edit\Form', 'block', [ 
				'data' => $args 
		] );
	...
	public function testFormPostInitNew($productData, $categoryData, $action, $requestPath, $targetPath) {
		$args = [ ];
		if ($productData) {
			$args ['product'] = $this->objectManager->create ( 'Magento\Catalog\Model\Product', [ 
					'data' => $productData 
			] );
		}
		if ($categoryData) {
			$args ['category'] = $this->objectManager->create ( 'Magento\Catalog\Model\Category', [ 
					'data' => $categoryData 
			] );
		}
	...
	public function testGetEntityStoresProductStoresException() {
		...
		$this->assertEquals ( 'We can\'t set up a URL rewrite because the product you chose is not associated with a website.', $form->getElement ( 'store_id' )->getAfterElementHtml () );
	}
	...
	public function testGetEntityStoresCategoryStoresException() {
		...
		$form = $this->_getFormInstance ( $args );
		$this->assertEquals ( [ ], $form->getElement ( 'store_id' )->getValues () );
		$this->assertEquals ( 'Please assign a website to the selected category.', $form->getElement ( 'store_id' )->getAfterElementHtml () );
	}
