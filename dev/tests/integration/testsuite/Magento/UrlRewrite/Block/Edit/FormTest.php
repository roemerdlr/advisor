...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getFormInstance($args = []) {
		...
		$block = $layout->createBlock ( 'Magento\UrlRewrite\Block\Edit\Form', 'block', [ 
				'data' => $args 
		] );
	...
	public function testPrepareForm() {
		...
		$form = $this->_getFormInstance ( [ 
				'url_rewrite' => new \Magento\Framework\DataObject ( [ 
						'id' => 3 
				] ) 
		] );
	...
	public function testSessionRestore() {
		...
		\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Backend\Model\Session' )->setUrlRewriteData ( $sessionValues );
	...
	public function testStoreElementSingleStore() {
		...
		$defaultStore = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ( true )->getId ();
	...
	public function testStoreElementMultiStores() {
		...
		$this->assertInstanceOf ( 'Magento\Backend\Block\Store\Switcher\Form\Renderer\Fieldset\Element', $storeElement->getRenderer () );
		
		// Check store elements has expected values
		$storesList = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Store\Model\System\Store' )->getStoreValuesForForm ();
