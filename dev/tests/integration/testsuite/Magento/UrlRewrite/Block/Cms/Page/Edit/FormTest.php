...
class FormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getFormInstance($args = []) {
		...
		$block = $layout->createBlock ( 'Magento\UrlRewrite\Block\Cms\Page\Edit\Form', 'block', [ 
				'data' => $args 
		] );
	...
	public function testFormPostInit($cmsPageData, $action, $requestPath, $targetPath) {
		$args = [ ];
		if ($cmsPageData) {
			$args ['cms_page'] = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Cms\Model\Page', [ 
					'data' => $cmsPageData 
			] );
		}
	...
	public function testGetEntityStores() {
		$args = [ 
				'cms_page' => $this->_getCmsPageWithStoresMock ( [ 
						1 
				] ) 
		];
		$form = $this->_getFormInstance ( $args );
	...
	public function testGetEntityStoresProductStoresException() {
		...
		$this->assertEquals ( 'Please assign a website to the selected CMS page.', $form->getElement ( 'store_id' )->getAfterElementHtml () );
	}
	...
	protected function _getCmsPageWithStoresMock($stores) {
		$resourceMock = $this->getMockBuilder ( 'Magento\Cms\Model\ResourceModel\Page' )->setMethods ( [ 
				'lookupStoreIds' 
		] )->disableOriginalConstructor ()->getMock ();
		$resourceMock->expects ( $this->any () )->method ( 'lookupStoreIds' )->will ( $this->returnValue ( $stores ) );
		
		$cmsPageMock = $this->getMockBuilder ( 'Magento\Cms\Model\Page' )->setMethods ( [ 
				'getResource',
				'getId' 
		] )->disableOriginalConstructor ()->getMock ();
