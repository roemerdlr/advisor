	...
	public function testGet() {
		...
		$fileResolver->expects ( $this->exactly ( 3 ) )->method ( 'get' )->will ( $this->returnValueMap ( [ 
				[ 
						...
						'global',
						[ 
								file_get_contents ( __DIR__ . '/_files/orders_and_returns.xml' ) 
						] 
				],
				[ 
						'widget.xml',
		...
		$reader = $objectManager->create ( 'Magento\Widget\Model\Config\Reader', [ 
				'fileResolver' => $fileResolver 
		] );
		/** @var \Magento\Widget\Model\Config\Data $configData */
		$configData = $objectManager->create ( 'Magento\Widget\Model\Config\Data', [ 
				'reader' => $reader 
		] );
