...
class InfoTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testCustomerGridAction() {
		...
		$infoBlock = $layout->createBlock ( 'Magento\Sales\Block\Adminhtml\Order\View\Info', 'info_block' . mt_rand (), [ ] );
	...
	public function testGetCustomerGroupName() {
		...
		$customerGroupBlock = $layout->createBlock ( 'Magento\Sales\Block\Adminhtml\Order\View\Info', 'info_block' . mt_rand (), [ 
	...
	protected function _putOrderIntoRegistry(array $additionalOrderData = []) {
		...
		$order = $this->_objectManager->get ( 'Magento\Sales\Model\Order' )->load ( '100000001' )->setData ( array_merge ( [ 
				'customer_group_id' => 0 
		], $additionalOrderData ) );
