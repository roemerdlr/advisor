...
class AbstractTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAddAttributesToForm() {
		...
		$block = $this->getMockForAbstractClass ( 'Magento\Sales\Block\Adminhtml\Order\Create\Form\AbstractForm', $arguments );
		$block->setLayout ( $objectManager->create ( 'Magento\Framework\View\Layout' ) );
		
		$method = new \ReflectionMethod ( 'Magento\Sales\Block\Adminhtml\Order\Create\Form\AbstractForm', '_addAttributesToForm' );
		...
		$attributeMetadataFactory = $objectManager->create ( 'Magento\Customer\Api\Data\AttributeMetadataInterfaceFactory' );
		$dateAttribute = $attributeMetadataFactory->create ()->setAttributeCode ( 'date' )->setBackendType ( 'datetime' )->setFrontendInput ( 'date' )->setFrontendLabel ( 'Date' );
		$attributes = [ 
				'date' => $dateAttribute 
		];
