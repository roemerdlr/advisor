...
class CreateTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetCustomerWishlistNoCustomerId() {
		...
		$this->assertFalse ( $this->_model->getCustomerWishlist ( true ), 'If customer ID is not set to session, false is expected to be returned.' );
	}
	...
	public function testSetBillingAddress() {
		...
		$expectedAddressData = array_merge ( $addressData, [ 
	...
	public function testSetBillingAddressValidationErrors() {
		...
		$session = Bootstrap::getObjectManager ()->create ( 'Magento\Backend\Model\Session\Quote' );
		$session->setCustomerId ( $customerIdFromFixture );
		$invalidAddressData = array_merge ( $this->_getValidAddressData (), [ 
				'firstname' => '',
				'lastname' => '' 
		] );
		/**
		...
		$this->assertTrue ( in_array ( 'Billing Address: "First Name" is a required value.', $errorMessages ), 'Expected validation message is absent.' );
		$this->assertTrue ( in_array ( 'Billing Address: "Last Name" is a required value.', $errorMessages ), 'Expected validation message is absent.' );
	}
	...
	public function testCreateOrderNewCustomerDifferentAddresses() {
		...
		$orderData = [ 
				...
				'billing_address' => array_merge ( $this->_getValidAddressData (), [ 
						'save_in_address_book' => '1' 
				] ),
				'shipping_address' => array_merge ( $this->_getValidAddressData (), [ 
						'save_in_address_book' => '1',
						'firstname' => $firstNameForShippingAddress 
				] ),
		...
		$this->_preparePreconditionsForCreateOrder ( $productIdFromFixture, $customerEmail, $shippingMethod, $shippingAddressAsBilling, $paymentData, $orderData, $paymentMethod );
		...
		$this->assertEquals ( $firstNameForShippingAddress, $customer->getPrimaryShippingAddress ()->getFirstname (), 'Shipping address is saved incorrectly.' );
	}
	...
	public function testCreateOrderExistingCustomerDifferentAddresses() {
		...
		$this->assertEquals ( $firstNameForShippingAddress, $address->getFirstname (), 'Shipping address is saved incorrectly.' );
	}
	...
	protected function _preparePreconditionsForCreateOrder($productIdFromFixture, $customerEmail, $shippingMethod, $shippingAddressAsBilling, $paymentData, $orderData, $paymentMethod, $customerIdFromFixture = null) {
		...
		$this->_model->addProduct ( $productIdFromFixture, [ 
				'qty' => 1 
		] );
