...
class QuoteTest extends \PHPUnit_Framework_TestCase {
	private function convertToArray($entity) {
		return Bootstrap::getObjectManager ()->create ( 'Magento\Framework\Api\ExtensibleDataObjectConverter' )->toFlatArray ( $entity );
	}
	...
	public function testSetCustomerData() {
		...
		$dataObjectHelper->populateWithArray ( $customer, $expected, '\Magento\Customer\Api\Data\CustomerInterface' );
	...
	public function testUpdateCustomerData() {
		...
		$dataObjectHelper->populateWithArray ( $customerDataUpdated, $expected, '\Magento\Customer\Api\Data\CustomerInterface' );
	...
	public function testAssignCustomerWithAddressChangeAddressesNotSpecified() {
		...
		foreach ( $expectedBillingAddressData as $field => $value ) {
			$this->assertEquals ( $value, $billingAddress->getData ( $field ), "'{$field}' value in quote billing address is invalid." );
		}
		...
		foreach ( $expectedShippingAddressData as $field => $value ) {
			$this->assertEquals ( $value, $shippingAddress->getData ( $field ), "'{$field}' value in quote shipping address is invalid." );
		}
	}
	...
	public function testAddProductUpdateItem() {
		...
		$this->setExpectedException ( '\Magento\Framework\Exception\LocalizedException', 'We don\'t have as many "Simple Product" as you requested.' );
