...
class AgreementTest extends \Magento\TestFramework\TestCase\AbstractController {
	...
	public function testReturnWizardAction() {
		...
		$paymentMethodMock = $this->getMock ( 'Magento\Paypal\Model\Express', [ 
				'getTitle',
				'setStore',
		...
		$paymentHelperMock = $this->getMock ( 'Magento\Payment\Helper\Data', [ 
				'getMethodInstance' 
		], [ ], '', false );
		$paymentHelperMock->expects ( $this->any () )->method ( 'getMethodInstance' )->will ( $this->returnValue ( $paymentMethodMock ) );
		$billingAgreement = $objectManager->create ( 'Magento\Paypal\Model\Billing\Agreement', [ 
		...
		$objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Paypal\Model\Billing\Agreement', [ ] )->will ( $this->returnValue ( $billingAgreement ) );
		$storeManager = $objectManager->get ( 'Magento\Store\Model\StoreManager' );
		$customerSession = $objectManager->get ( 'Magento\Customer\Model\Session' );
		$objectManagerMock->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValueMap ( [ 
				[ 
		...
		$contextMock = $objectManager->create ( 'Magento\Framework\App\Action\Context', [ 
				'objectManager' => $objectManagerMock,
				'request' => $requestMock 
		] );
		/** @var \Magento\Paypal\Controller\Billing\Agreement $billingAgreementController */
		$billingAgreementController = $objectManager->create ( 'Magento\Paypal\Controller\Billing\Agreement\ReturnWizard', [ 
				'context' => $contextMock 
		] );
		...
		$billingAgreementCollection = $objectManager->create ( 'Magento\Paypal\Model\ResourceModel\Billing\Agreement\Collection' );
