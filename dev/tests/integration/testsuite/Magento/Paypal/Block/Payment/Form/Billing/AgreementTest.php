...
class AgreementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$layout->expects ( $this->once () )->method ( 'getBlock' )->will ( $this->returnValue ( new \Magento\Framework\DataObject ( [ 
				'quote' => $quote 
		] ) ) );
		$layout->expects ( $this->once () )->method ( 'getParentName' )->will ( $this->returnValue ( 'billing_agreement_form' ) );
		
		$this->_block = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Paypal\Block\Payment\Form\Billing\Agreement' );
