...
class SettlementTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFetchAndSave() {
		...
		$model = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\Paypal\Model\Report\Settlement' );
		$connection = $this->getMock ( 'Magento\Framework\Filesystem\Io\Sftp', [ 
				'rawls',
				'read' 
		], [ ], '', false );
		$filename = 'STL-00000000.00.abc.CSV';
		$connection->expects ( $this->once () )->method ( 'rawls' )->will ( $this->returnValue ( [ 
				$filename => [ ] 
		] ) );
