...
class CheckoutTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPlaceGuestQuote() {
		...
		$this->assertEquals ( \Magento\Customer\Model\GroupManagement::NOT_LOGGED_IN_ID, $quote->getCustomerGroupId () );
	...
	protected function _getCheckout(Quote $quote) {
		return $this->_objectManager->create ( 'Magento\Paypal\Model\Express\Checkout', [ 
				'params' => [ 
	...
	public function testReturnFromPaypal() {
		...
		$checkoutModel = $this->_objectManager->create ( 'Magento\Paypal\Model\Express\Checkout', [ 
				'params' => [ 
						'quote' => $quote,
		...
		$api = $this->getMock ( 'Magento\Paypal\Model\Api\Nvp', [ 
				'call',
				'getExportedShippingAddress',
		...
		$api->expects ( $this->any () )->method ( 'getExportedBillingAddress' )->will ( $this->returnValue ( $exportedBillingAddress ) );
		
		$exportedShippingAddress = $this->_getExportedAddressFixture ( $quote->getShippingAddress ()->getData () );
		$api->expects ( $this->any () )->method ( 'getExportedShippingAddress' )->will ( $this->returnValue ( $exportedShippingAddress ) );
