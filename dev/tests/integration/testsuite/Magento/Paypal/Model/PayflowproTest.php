...
class PayflowproTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$httpClientFactoryMock = $this->getMockBuilder ( 'Magento\Framework\HTTP\ZendClientFactory' )->setMethods ( [ 
				'create' 
		...
		$httpClientFactoryMock->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_httpClientMock ) );
		
		$mathRandomMock = $this->getMock ( 'Magento\Framework\Math\Random', [ ], [ ], '', false );
		$loggerMock = $this->getMock ( 'Magento\Payment\Model\Method\Logger', [ ], [ ], '', false );
		$this->gatewayMock = $this->_objectManager->create ( 'Magento\Paypal\Model\Payflow\Service\Gateway', [ 
				'httpClientFactory' => $httpClientFactoryMock,
				'mathRandom' => $mathRandomMock,
				'logger' => $loggerMock 
		] );
		$this->_model = $this->_objectManager->create ( 'Magento\Paypal\Model\Payflowpro', [ 
	...
	public function testReviewPaymentNullResponce() {
		...
		$this->_httpClientMock->expects ( $this->any () )->method ( 'request' )->will ( $this->returnValue ( new \Magento\Framework\DataObject ( [ 
				'body' => 'RESULTval=12&val2=34' 
		] ) ) );
		$expectedResult = [ 
				'resultval' => '12',
				'val2' => '34',
