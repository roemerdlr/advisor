...
class SaveTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testSaveAction($currencyCode, $inputCurrencySymbol, $outputCurrencySymbol) {
		...
		$currencySymbol = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\CurrencySymbol\Model\System\Currencysymbol' );
		
		$currencySymbolOriginal = $currencySymbol->getCurrencySymbol ( $currencyCode );
		
		$request = $this->getRequest ();
		$request->setParam ( 'custom_currency_symbol', [ 
		...
		$this->assertEquals ( $outputCurrencySymbol, $currencySymbol->getCurrencySymbol ( $currencyCode ), 'Currency symbol has not been saved' );
		
		// restore current symbol
		$currencySymbol->setCurrencySymbolsData ( [ 
				$currencyCode => $currencySymbolOriginal 
		] );
	}
