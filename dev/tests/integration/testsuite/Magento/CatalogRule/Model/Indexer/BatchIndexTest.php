...
class BatchIndexTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPriceForSmallBatch($batchCount, $price, $expectedPrice) {
		 ...
		 */
		$indexerBuilder = Bootstrap::getObjectManager ()->create ( 'Magento\CatalogRule\Model\Indexer\IndexBuilder', [ 
	...
	protected function prepareProducts($price) {
		...
		$productSecond->setId ( null )->setUrlKey ( null )->setSku ( uniqid ( $this->product->getSku () . '-' ) )->setName ( uniqid ( $this->product->getName () . '-' ) )->setWebsiteIds ( [ 
				1 
		] );
		$productSecond->save ();
		$productSecond->setPrice ( $price )->save ();
		$productThird = clone $this->product;
		$productThird->setId ( null )->setUrlKey ( null )->setSku ( uniqid ( $this->product->getSku () . '-' ) )->setName ( uniqid ( $this->product->getName () . '-' ) )->setWebsiteIds ( [ 
				1 
		] )->save ();
