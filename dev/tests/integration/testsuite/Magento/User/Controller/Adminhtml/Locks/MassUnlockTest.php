...
class MassUnlockTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testMassUnlockAction() {
		...
		$request->setPostValue ( 'unlock', $userIds );
		$this->dispatch ( 'backend/admin/locks/massunlock' );
		
		$this->assertSessionMessages ( $this->contains ( ( string ) __ ( 'Unlocked %1 user(s).', count ( $userIds ) ) ), \Magento\Framework\Message\MessageInterface::TYPE_SUCCESS );
		$this->assertRedirect ();
	}
}
