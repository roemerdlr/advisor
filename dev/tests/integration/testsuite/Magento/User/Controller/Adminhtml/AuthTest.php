...
class AuthTest extends \Magento\TestFramework\TestCase\AbstractBackendController {
	...
	public function testResetPasswordAction() {
		...
		$user = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->create ( 'Magento\User\Model\User' )->loadByUsername ( 'dummy_username' );
		$this->assertNotEmpty ( $user->getId (), 'Broken fixture' );
		$resetPasswordToken = \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\User\Helper\Data' )->generateResetPasswordLinkToken ();
	...
	public function testResetPasswordPostAction($password, $passwordConfirmation, $isPasswordChanged) {
		...
		$this->getRequest ()->setQueryValue ( 'token', $resetPasswordToken )->setQueryValue ( 'id', $user->getId () )->setPostValue ( 'password', $password )->setPostValue ( 'confirmation', $passwordConfirmation );
		...
		if ($isPasswordChanged) {
			...
			$this->assertRedirect ( $this->stringContains ( 'backend/admin/auth/resetpassword' ) );
		}
	...
	public function testResetPasswordPostActionWithInvalidPassword() {
		...
		$this->getRequest ()->setQueryValue ( 'token', $resetPasswordToken )->setQueryValue ( 'id', $user->getId () )->setPostValue ( 'password', $newDummyPassword )->setPostValue ( 'confirmation', 'invalid' );
