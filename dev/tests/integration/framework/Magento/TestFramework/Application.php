...
class Application {
	...
	public function initialize($overriddenParams = []) {
		...
		$logger = $objectManager->create ( 'Magento\TestFramework\ErrorLog\Logger', [ 
				'name' => 'integration-tests',
				'handlers' => [ 
						'system' => $objectManager->create ( 'Magento\Framework\Logger\Handler\System', [ 
								'exceptionHandler' => $objectManager->create ( 'Magento\Framework\Logger\Handler\Exception', [ 
										'filePath' => $this->installDir 
								] ),
								'filePath' => $this->installDir 
						] ),
						'debug' => $objectManager->create ( 'Magento\Framework\Logger\Handler\Debug', [ 
								'filePath' => $this->installDir 
						] ) 
				] 
		] );
		...
		if ($this->loadTestExtensionAttributes) {
			$objectManagerConfiguration = array_merge ( $objectManagerConfiguration, [ 
					'Magento\Framework\Api\ExtensionAttribute\Config\Reader' => [ 
							'arguments' => [ 
									'fileResolver' => [ 
											'instance' => 'Magento\TestFramework\Api\Config\Reader\FileResolver' 
									] 
							] 
					] 
			] );
		...
		$objectManager->create ( 'Magento\TestFramework\Config', [ 
	...
	public function cleanup() {
		 ...
		 */
		$this->_shell->execute ( 'php -f %s setup:uninstall -n --magento-init-params=%s', [ 
				BP . '/bin/magento',
				$this->getInitParamsQuery () 
		] );
	}
	...
	public function install() {
		...
		// run install script
		$this->_shell->execute ( 'php -f %s setup:install ' . implode ( ' ', array_keys ( $installParams ) ), array_merge ( [ 
				BP . '/bin/magento' 
		], array_values ( $installParams ) ) );
		
		// enable only specified list of caches
		$initParamsQuery = $this->getInitParamsQuery ();
		$this->_shell->execute ( 'php -f %s cache:disable --bootstrap=%s', [ 
				BP . '/bin/magento',
				$initParamsQuery 
		] );
		$this->_shell->execute ( 'php -f %s cache:enable %s %s %s %s --bootstrap=%s', [ 
				BP . '/bin/magento',
	...
	private function copyAppConfigFiles() {
		$globalConfigFiles = glob ( $this->_globalConfigDir . '/{di.xml,vendor_path.php}', GLOB_BRACE );
	...
	public function loadArea($areaCode) {
		...
		\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->configure ( \Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\App\ObjectManager\ConfigLoader' )->load ( $areaCode ) );
	...
	protected function getCustomDirs() {
		...
		$customDirs = [ 
				DirectoryList::CONFIG => [ 
						$path => "{$this->installDir}/etc" 
				],
				DirectoryList::VAR_DIR => [ 
						$path => $var 
				],
				DirectoryList::MEDIA => [ 
						$path => "{$this->installDir}/pub/media" 
				],
				DirectoryList::STATIC_VIEW => [ 
						$path => "{$this->installDir}/pub/static" 
				],
