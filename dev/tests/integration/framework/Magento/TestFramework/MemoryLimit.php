...
class MemoryLimit {
	...
	public function printStats() {
		...
		$msg = sprintf ( "Memory usage (OS):\t%s (%.2F%% of %s reported by PHP", $this->_toMb ( $usage ), 100 * $usage / ($usage - $leak), $this->_toMb ( $usage - $leak ) );
	...
	public function validateUsage() {
		...
		if ($this->_memCap && $usage >= $this->_memCap) {
			throw new \LogicException ( "Memory limit of {$this->_toMb($this->_memCap)} ({$this->_memCap} bytes) has been reached." );
		}
		if ($this->_leakCap && $leak >= $this->_leakCap) {
			throw new \LogicException ( "Estimated memory leak limit of {$this->_toMb(
                    $this->_leakCap
