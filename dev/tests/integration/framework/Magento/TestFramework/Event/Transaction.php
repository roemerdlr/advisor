...
class Transaction {
	...
	protected function _processTransactionRequests($eventName, \PHPUnit_Framework_TestCase $test) {
		...
		$this->_eventManager->fireEvent ( $eventName . 'TransactionRequest', [ 
				$test,
				$param 
		] );
	...
	protected function _startTransaction(\PHPUnit_Framework_TestCase $test) {
		if (! $this->_isTransactionActive) {
			...
			try {
				$this->_eventManager->fireEvent ( 'startTransaction', [ 
						$test 
				] );
			} catch ( \Exception $e ) {
				$test->getTestResultObject ()->addFailure ( $test, new \PHPUnit_Framework_AssertionFailedError ( ( string ) $e ), 0 );
			}
		}
	}
