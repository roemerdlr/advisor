...
class Api {
	 ...
	 * @param string $path        	
	 * @param array $params
	 *        	Order of items matters as they are passed to call_user_func_array
	 * @return mixed
	 */
	public static function call(\PHPUnit_Framework_TestCase $testCase, $path, $params = []) {
		$soapAdapterMock = $testCase->getMock ( 'Magento\Api\Model\Server\Adapter\Soap', [ 
				'fault' 
		] );
		$soapAdapterMock->expects ( $testCase->any () )->method ( 'fault' )->will ( $testCase->returnCallback ( [ 
				__CLASS__,
				'soapAdapterFaultCallback' 
		] ) );
		
		$serverMock = $testCase->getMock ( 'Magento\Api\Model\Server', [ 
				'getAdapter' 
		] );
		$serverMock->expects ( $testCase->any () )->method ( 'getAdapter' )->will ( $testCase->returnValue ( $soapAdapterMock ) );
		
		$apiSessionMock = $testCase->getMock ( 'Magento\Api\Model\Session', [ 
				'isAllowed',
				'isLoggedIn' 
		], [ ], '', false );
		...
		self::$_previousHandler = set_error_handler ( [ 
				$handlerMock,
				'handlePhpError' 
		] );
	 ...
	 * @return \SoapFault
	 */
	public static function callWithException(\PHPUnit_Framework_TestCase $testCase, $path, $params = [], $expectedMessage = '') {
		try {
	 ...
	 * Callback for 'fault' method of API.
	 *
	 ...
	 * @return object In XML notation we can't have nodes with digital names in other words fallowing XML will be not valid:
	 *         &lt;24&gt;
	 *         Default category
	 *         &lt;/24&gt;
	 *        
	 ...
	 *        	To be able to compare fields from loaded model with fields from API response
	 ...
	 *        	);
	 */
	public static function checkEntityFields(\PHPUnit_Framework_TestCase $testCase, array $expectedData, array $actualData, array $fieldsToCompare = []) {
		...
		foreach ( $fieldsToCompare as $entityField => $field ) {
			$testCase->assertEquals ( $expectedData [is_numeric ( $entityField ) ? $field : $entityField], $actualData [$field], sprintf ( '"%s" filed has invalid value.', $field ) );
		}
	}
}
