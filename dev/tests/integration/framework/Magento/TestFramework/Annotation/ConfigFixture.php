...
class ConfigFixture {
	...
	protected function _getConfigValue($configPath, $scopeCode = null) {
		...
		if ($scopeCode !== false) {
			...
			$result = $scopeConfig->getValue ( $configPath, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $scopeCode );
		}
		return $result;
	}
	...
	protected function _setConfigValue($configPath, $value, $storeCode = false) {
		...
		if ($storeCode === false) {
			if (strpos ( $configPath, 'default/' ) === 0) {
				...
				$objectManager->get ( 'Magento\Framework\App\Config\MutableScopeConfigInterface' )->setValue ( $configPath, $value, ScopeConfigInterface::SCOPE_TYPE_DEFAULT );
			}
		} else {
			\Magento\TestFramework\Helper\Bootstrap::getObjectManager ()->get ( 'Magento\Framework\App\Config\MutableScopeConfigInterface' )->setValue ( $configPath, $value, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeCode );
		}
	}
