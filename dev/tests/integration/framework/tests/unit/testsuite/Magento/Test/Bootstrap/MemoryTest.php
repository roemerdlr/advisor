...
class MemoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_activationPolicy = $this->getMock ( 'stdClass', [ 
				'register_shutdown_function' 
		] );
		$this->_object = new \Magento\TestFramework\Bootstrap\Memory ( $this->_memoryLimit, [ 
	...
	public function testDisplayStats() {
		...
		$this->_memoryLimit->expects ( $this->once () )->method ( 'printStats' )->will ( $this->returnValue ( 'Dummy Statistics' ) );
		$this->_object->displayStats ();
	}
	public function testActivateStatsDisplaying() {
		$this->_activationPolicy->expects ( $this->once () )->method ( 'register_shutdown_function' )->with ( $this->identicalTo ( [ 
				$this->_object,
				'displayStats' 
		] ) );
