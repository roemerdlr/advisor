...
class MemoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetRealMemoryUsageUnix() {
		...
		$this->_shell->expects ( $this->at ( 0 ) )->method ( 'execute' )->with ( $this->stringStartsWith ( 'tasklist.exe ' ) )->will ( $this->throwException ( new \Magento\Framework\Exception\LocalizedException ( __ ( 'command not found' ) ) ) );
		$this->_shell->expects ( $this->at ( 1 ) )->method ( 'execute' )->with ( $this->stringStartsWith ( 'ps ' ) )->will ( $this->returnValue ( '26321' ) );
		$this->assertEquals ( 26952704, $object->getRealMemoryUsage () );
	}
	public function testGetRealMemoryUsageWin() {
		$this->_shell->expects ( $this->once () )->method ( 'execute' )->with ( $this->stringStartsWith ( 'tasklist.exe ' ) )->will ( $this->returnValue ( '"php.exe","12345","N/A","0","26,321 K"' ) );
		$object = new \Magento\TestFramework\Helper\Memory ( $this->_shell );
		$this->assertEquals ( 26952704, $object->getRealMemoryUsage () );
	}
