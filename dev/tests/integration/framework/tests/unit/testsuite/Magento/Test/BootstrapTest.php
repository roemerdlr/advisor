...
class BootstrapTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_envBootstrap = $this->getMock ( 'Magento\TestFramework\Bootstrap\Environment', [ 
				'emulateHttpRequest',
				'emulateSession' 
		] );
		$this->_docBlockBootstrap = $this->getMock ( 'Magento\TestFramework\Bootstrap\DocBlock', [ 
				'registerAnnotations' 
		], [ 
				__DIR__ 
		] );
		$profilerDriver = $this->getMock ( 'Magento\Framework\Profiler\Driver\Standard', [ 
				'registerOutput' 
		] );
		$this->_profilerBootstrap = $this->getMock ( 'Magento\TestFramework\Bootstrap\Profiler', [ 
				'registerFileProfiler',
				'registerBambooProfiler' 
		], [ 
				$profilerDriver 
		] );
		$this->_shell = $this->getMock ( 'Magento\Framework\Shell', [ 
				'execute' 
		], [ ], '', false );
		...
		$this->_object = new \Magento\TestFramework\Bootstrap ( $this->_settings, $this->_envBootstrap, $this->_docBlockBootstrap, $this->_profilerBootstrap, $this->_shell, $this->application, $this->memoryFactory );
	}
	...
	public function testRunBootstrap() {
		...
		$memoryBootstrap = $this->getMock ( 'Magento\TestFramework\Bootstrap\Memory', [ 
	...
	public function testRunBootstrapProfilerEnabled() {
		...
		$settingsMap = [ 
				...
				[ 
						'TESTS_BAMBOO_PROFILER_FILE',
						'',
						'profiler_bamboo.csv' 
				],
				[ 
						'TESTS_BAMBOO_PROFILER_METRICS_FILE',
						'',
						'profiler_metrics.php' 
				] 
		];
