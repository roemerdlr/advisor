...
class EntityTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_model = $this->getMock ( 'Magento\Framework\Model\AbstractModel', [ 
				'load',
				'save',
				'delete',
				'getIdFieldName',
	...
	public function testTestCrud($saveCallback, $expectedException = null) {
		...
		$this->_model->expects ( $this->atLeastOnce () )->method ( 'save' )->will ( $this->returnCallback ( [ 
				$this,
				$saveCallback 
		] ) );
		/* It's important that 'delete' should be always called to guarantee the cleanup */
		$this->_model->expects ( $this->atLeastOnce () )->method ( 'delete' )->will ( $this->returnCallback ( [ 
				$this,
		...
		$test = $this->getMock ( 'Magento\TestFramework\Entity', [ 
				'_getEmptyModel' 
		], [ 
				$this->_model,
				[ 
