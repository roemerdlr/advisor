...
class DataFixtureTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_object = $this->getMock ( 'Magento\TestFramework\Annotation\DataFixture', [ 
				'_applyOneFixture' 
		], [ 
				__DIR__ . '/_files' 
		] );
	...
	public function testStartTransactionClassAnnotation() {
		$this->_object->expects ( $this->once () )->method ( '_applyOneFixture' )->with ( [ 
				__CLASS__,
				'sampleFixtureOne' 
		] );
		$this->_object->startTransaction ( $this );
	...
	public function testStartTransactionMethodAnnotation() {
		$this->_object->expects ( $this->at ( 0 ) )->method ( '_applyOneFixture' )->with ( [ 
				__CLASS__,
				'sampleFixtureTwo' 
		] );
		$this->_object->expects ( $this->at ( 1 ) )->method ( '_applyOneFixture' )->with ( $this->stringEndsWith ( 'path/to/fixture/script.php' ) );
		$this->_object->startTransaction ( $this );
	}
	...
	public function testRollbackTransactionRevertFixtureFile() {
		$this->_object->startTransaction ( $this );
		$this->_object->expects ( $this->once () )->method ( '_applyOneFixture' )->with ( $this->stringEndsWith ( 'sample_fixture_two_rollback.php' ) );
		$this->_object->rollbackTransaction ();
	}
}
