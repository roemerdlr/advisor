...
class TransactionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_eventManager = $this->getMock ( 'Magento\TestFramework\EventManager', [ 
				'fireEvent' 
		], [ 
				[ ] 
		] );
		$this->_adapter = $this->getMock ( '\Magento\TestFramework\Db\Adapter\Mysql', [ 
				'beginTransaction',
				'rollBack' 
		], [ ], '', false );
		$this->_object = $this->getMock ( 'Magento\TestFramework\Event\Transaction', [ 
				'_getConnection' 
		], [ 
				$this->_eventManager 
		] );
	...
	protected function _imitateTransactionStartRequest($eventName) {
		...
		$this->_eventManager->expects ( $this->at ( 0 ) )->method ( 'fireEvent' )->with ( $eventName )->will ( $this->returnCallback ( $callback ) );
	}
