...
class ObjectManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testClearCache() {
		...
		$configMock = $this->getMockBuilder ( 'Magento\TestFramework\ObjectManager\Config' )->disableOriginalConstructor ()->setMethods ( [ 
				'getPreference',
				'clean' 
		] )->getMock ();
		
		$configMock->expects ( $this->atLeastOnce () )->method ( 'getPreference' )->will ( $this->returnCallback ( function ($className) {
			return $className;
		} ) );
		...
		$primaryLoaderMock = $this->getMock ( 'Magento\Framework\App\ObjectManager\ConfigLoader\Primary', [ ], [ ], '', false );
		$factory = $this->getMock ( 'Magento\Framework\ObjectManager\FactoryInterface' );
		$factory->expects ( $this->exactly ( 2 ) )->method ( 'create' )->will ( $this->returnCallback ( function ($className) {
			if ($className === 'Magento\Framework\DataObject') {
				return $this->getMock ( 'Magento\Framework\DataObject', [ ], [ ], '', false );
			}
		} ) );
		
		$connectionMock = $this->getMockBuilder ( 'Magento\Framework\App\ResourceConnection' )->disableOriginalConstructor ()->getMock ();
		$sharedInstances = [ 
				...
				'Magento\Framework\App\ResourceConnection\Config' => $this->getMock ( 'Magento\Framework\App\ResourceConnection\Config', [ ], [ ], '', false ) 
		];
		$model = new \Magento\TestFramework\ObjectManager ( $factory, $configMock, $sharedInstances, $primaryLoaderMock );
