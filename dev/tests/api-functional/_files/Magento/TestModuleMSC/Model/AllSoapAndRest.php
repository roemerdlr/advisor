...
class AllSoapAndRest implements \Magento\TestModuleMSC\Api\AllSoapAndRestInterface {
	 ...
	 */
	public function __construct(ItemInterfaceFactory $itemDataFactory, CustomAttributeDataObjectInterfaceFactory $customAttributeNestedDataObjectFactory) {
	...
	public function update(\Magento\TestModuleMSC\Api\Data\ItemInterface $entityItem) {
		return $this->itemDataFactory->create ()->setItemId ( $entityItem->getItemId () )->setName ( 'Updated' . $entityItem->getName () );
	}
	...
	public function getPreconfiguredItem() {
		$customAttributeDataObject = $this->customAttributeDataObjectDataFactory->create ()->setName ( 'nameValue' )->setCustomAttribute ( 'custom_attribute_int', 1 );
		
		$item = $this->itemDataFactory->create ()->setItemId ( 1 )->setName ( 'testProductAnyType' )->setCustomAttribute ( 'custom_attribute_data_object', $customAttributeDataObject )->setCustomAttribute ( 'custom_attribute_string', 'someStringValue' );
