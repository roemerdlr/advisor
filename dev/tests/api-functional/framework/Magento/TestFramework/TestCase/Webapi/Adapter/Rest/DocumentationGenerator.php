...
HTML;
		$endHtml = $this->generateHtmlFooter ();
		$content = "{$beginningHtml}{$headingHtml}<pre>{$arguments}</pre>{$responseHtml}<pre>{$response}" . "</pre>{$requestResponseParametersHtml}{$requestParametersHtml}{$responseParametersHtml}{$endHtml}";
		return $content;
	}
	...
	protected function generateFileName() {
		...
		foreach ( $debugBackTrace as $traceItem ) {
			...
			if (count ( $traceItem ) == 3) {
				/**
				 * Remove 'test' prefix from method name, e.g.
				 * testCreate => create
				 */
	...
	protected function retrieveParametersAsHtml($parameters) {
		...
		if (is_array ( $parameters )) {
			foreach ( array_keys ( $parameters ) as $parameter ) {
				$parametersAsHtml = $parametersAsHtml . '<li><strong>' . $parameter . '</strong> (<em>Change type manually!</em>) TBD.</li>';
			}
		} else {
			$parametersAsHtml = '<li><strong>' . 'scalar_value' . '</strong> (<em>Change type manually!</em>) TBD.</li>';
		}
