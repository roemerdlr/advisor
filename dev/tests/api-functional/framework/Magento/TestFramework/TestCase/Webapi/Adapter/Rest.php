...
class Rest implements \Magento\TestFramework\TestCase\Webapi\AdapterInterface {
	...
	public function __construct() {
		...
		$this->documentationGenerator = $objectManager->get ( 'Magento\TestFramework\TestCase\Webapi\Adapter\Rest\DocumentationGenerator' );
		$this->defaultStoreCode = Bootstrap::getObjectManager ()->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ()->getCode ();
	}
	 ...
	 * {@inheritdoc}
	 *
	 * @throws \LogicException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function call($serviceInfo, $arguments = [], $storeCode = null, $integration = null) {
		...
		if (array_key_exists ( 'token', $restServiceInfo )) {
			...
			$authHeader = $oAuthClient->buildOauthAuthorizationHeader ( $this->curlClient->constructResourceUrl ( $resourcePath ), $accessCredentials ['key'], $accessCredentials ['secret'], ($httpMethod == 'PUT' || $httpMethod == 'POST') && $urlFormEncoded ? $arguments : [ ], $httpMethod );
		}
		$authHeader = array_merge ( $authHeader, [ 
