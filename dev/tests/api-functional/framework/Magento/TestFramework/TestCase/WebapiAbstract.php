...
abstract class WebapiAbstract extends \PHPUnit_Framework_TestCase {
	...
	public static function tearDownAfterClass() {
		...
		if (isset ( self::$_classLevelFixtures [$fixtureNamespace] ) && count ( self::$_classLevelFixtures [$fixtureNamespace] )) {
	...
	protected function tearDown() {
		$fixtureNamespace = self::_getFixtureNamespace ();
		if (isset ( self::$_methodLevelFixtures [$fixtureNamespace] ) && count ( self::$_methodLevelFixtures [$fixtureNamespace] )) {
	 ...
	 */
	protected function _webApiCall($serviceInfo, $arguments = [], $webApiAdapterCode = null, $storeCode = null, $integration = null) {
	...
	protected function _getWebApiAdapter($webApiAdapterCode) {
		if (! isset ( $this->_webApiAdapters [$webApiAdapterCode] )) {
			if (! isset ( $this->_webApiAdaptersMap [$webApiAdapterCode] )) {
				throw new \LogicException ( sprintf ( 'Declaration of the requested Web API adapter "%s" was not found.', $webApiAdapterCode ) );
			}
	 ...
	 *
	 * @param string $path
	 *        	Config path with the form "section/group/node"
	 * @param string|int|null $value
	 *        	Value of config item
	 * @param bool $cleanAppCache
	 *        	If TRUE application cache will be refreshed
	 * @param bool $updateLocalConfig
	 *        	If TRUE local config object will be updated too
	 * @param bool $restore
	 *        	If TRUE config value will be restored after test run
	 * @return \Magento\TestFramework\TestCase\WebapiAbstract
	 * @throws \RuntimeException
	 */
	protected function _updateAppConfig($path, $value, $cleanAppCache = true, $updateLocalConfig = false, $restore = false) {
		list ( $section, $group, $node ) = explode ( '/', $path );
		
		if (! $section || ! $group || ! $node) {
			throw new \RuntimeException ( sprintf ( 'Config path must have view as "section/group/node" but now it "%s"', $path ) );
		}
		...
		if ($restore && ! isset ( $this->_origConfigValues [$path] )) {
			$this->_origConfigValues [$path] = ( string ) $objectManager->get ( 'Magento\Framework\App\Config\ScopeConfigInterface' )->getNode ( $path, 'default' );
		}
	...
	protected function _checkWrappedErrors($expectedWrappedErrors, $errorDetails) {
		...
		if ($expectedWrappedErrors) {
			...
			$this->assertEquals ( $expectedWrappedErrors, $actualWrappedErrors, "Wrapped errors in fault details are invalid." );
		} else {
