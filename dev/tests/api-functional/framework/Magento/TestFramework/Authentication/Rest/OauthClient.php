...
class OauthClient extends AbstractService {
	...
	public function getTestApiEndpoint() {
		$defaultStoreCode = Bootstrap::getObjectManager ()->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ()->getCode ();
	 ...
	 *        	used to verify the passed token
	...
	public function validateAccessToken($token, $method = 'GET') {
		...
		$authorizationHeader = [ 
				'Authorization' => $this->buildAuthorizationHeaderForAPIRequest ( $method, $this->getTestApiEndpoint (), $token, [ ] ) 
		];
