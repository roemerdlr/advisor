...
class BlockRepositoryTest extends WebapiAbstract {
	...
	public function testGet() {
		...
		$block = $this->_webApiCall ( $serviceInfo, [ 
				BlockInterface::BLOCK_ID => $this->currentBlock->getId () 
		] );
	...
	public function testUpdate() {
		...
		$blockDataObject->setTitle ( $blockTitle )->setIdentifier ( $blockIdentifier );
		$this->currentBlock = $this->blockRepository->save ( $blockDataObject );
		$this->dataObjectHelper->populateWithArray ( $this->currentBlock, [ 
				BlockInterface::TITLE => $newBlockTitle 
		], 'Magento\Cms\Api\Data\BlockInterface' );
		$blockData = $this->dataObjectProcessor->buildOutputDataArray ( $this->currentBlock, 'Magento\Cms\Api\Data\BlockInterface' );
