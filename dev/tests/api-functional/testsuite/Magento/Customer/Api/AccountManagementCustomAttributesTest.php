...
class AccountManagementCustomAttributesTest extends WebapiAbstract {
	...
	protected function createCustomerWithDefaultImageAttribute() {
		...
		$image = $this->imageFactory->create ()->setType ( 'image/jpeg' )->setName ( 'sample.jpeg' )->setBase64EncodedData ( $imageData );
		
		$imageData = $this->dataObjectProcessor->buildOutputDataArray ( $image, '\Magento\Framework\Api\Data\ImageContentInterface' );
	...
	public function testCreateCustomerWithInvalidImageAttribute() {
		$image = $this->imageFactory->create ()->setType ( 'image/jpeg' )->setName ( 'sample.jpeg' )->setBase64EncodedData ( 'INVALID_IMAGE_DATA' );
	...
	public function testUpdateCustomerWithImageAttribute() {
		...
		$imageData = base64_encode ( file_get_contents ( $testImagePath ) );
		$image = $this->imageFactory->create ()->setType ( 'image/png' )->setName ( 'buttons.png' )->setBase64EncodedData ( $imageData );
		$imageData = $this->dataObjectProcessor->buildOutputDataArray ( $image, '\Magento\Framework\Api\Data\ImageContentInterface' );
		...
		$mediaDirectory = $this->fileSystem->getDirectoryWrite ( DirectoryList::MEDIA );
		$customerMediaPath = $mediaDirectory->getAbsolutePath ( CustomerMetadataInterface::ENTITY_TYPE_CUSTOMER );
		$previousImagePath = $previousCustomerData [CustomAttributesDataInterface::CUSTOM_ATTRIBUTES] [0] [AttributeValue::VALUE];
