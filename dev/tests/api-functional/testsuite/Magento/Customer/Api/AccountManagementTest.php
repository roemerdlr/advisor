	...
	public function testCreateCustomerWithErrors() {
		...
		try {
			...
			if (TESTS_WEB_API_ADAPTER == self::ADAPTER_SOAP) {
				...
				$expectedException->addError ( __ ( InputException::INVALID_FIELD_VALUE, [ 
						'fieldName' => 'email',
						'value' => $invalidEmail 
				] ) );
				$this->assertInstanceOf ( 'SoapFault', $e );
				$this->checkSoapFault ( $e, $expectedException->getRawMessage (), 'env:Sender', $expectedException->getParameters () ) // expected error parameters
;
			} else {
	...
	public function testActivateCustomer() {
		...
		$this->assertTrue ( ! isset ( $result [Customer::CONFIRMATION] ) || $result [Customer::CONFIRMATION] === null, 'Customer is not activated!' );
	}
	...
	public function testValidateResetPasswordLinkTokenInvalidToken() {
		...
		try {
			if (TESTS_WEB_API_ADAPTER == self::ADAPTER_SOAP) {
				$this->_webApiCall ( $serviceInfo, [ 
						'customerId' => $customerData ['id'],
						'resetPasswordLinkToken' => 'invalid' 
				] );
			} else {
				$this->_webApiCall ( $serviceInfo );
			}
			...
			$this->assertContains ( $expectedMessage, $e->getMessage (), "Exception message does not match" );
		} catch ( \Exception $e ) {
	...
	public function testSendPasswordResetLinkBadEmailOrWebsite() {
		...
		try {
			...
			if (TESTS_WEB_API_ADAPTER == self::ADAPTER_REST) {
				$errorObj = $this->processRestExceptionResult ( $e );
				$this->assertEquals ( NoSuchEntityException::MESSAGE_DOUBLE_FIELDS, $errorObj ['message'] );
				$this->assertEquals ( $expectedErrorParameters, $errorObj ['parameters'] );
				$this->assertEquals ( HTTPExceptionCodes::HTTP_NOT_FOUND, $e->getCode () );
			} else {
				$this->assertInstanceOf ( 'SoapFault', $e );
				$this->checkSoapFault ( $e, NoSuchEntityException::MESSAGE_DOUBLE_FIELDS, 'env:Sender', $expectedErrorParameters );
			}
		}
	}
	...
	public function testValidateCustomerData() {
		...
		$customerData = $this->dataObjectProcessor->buildOutputDataArray ( $customerData, '\Magento\Customer\Api\Data\CustomerInterface' );
		$requestData = [ 
	...
	public function testGetDefaultBillingAddress() {
		...
		$this->assertEquals ( $this->getFirstFixtureAddressData (), $addressData, "Default billing address data is invalid." );
	}
	...
	public function testGetDefaultShippingAddress() {
		...
		$addressData = $this->_webApiCall ( $serviceInfo, $requestData );
		$this->assertEquals ( $this->getFirstFixtureAddressData (), $addressData, "Default shipping address data is invalid." );
	}
