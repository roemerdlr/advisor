...
class GroupRepositoryTest extends WebapiAbstract {
	 ...
	 *        	The group data for the group being retrieved.
	 *        	
	...
	public function testCreateGroupRest() {
		...
		$this->assertEquals ( $groupData [CustomerGroup::TAX_CLASS_ID], $newGroup->getTaxClassId (), 'The group tax class id does not match.' );
	}
	...
	public function testCreateGroupDuplicateGroupRest() {
		...
		try {
			...
			$this->assertEquals ( 'Customer Group already exists.', $errorData ['message'] );
	...
	public function testCreateGroupDefaultTaxClassIdRest() {
		...
		$this->assertEquals ( $groupId, $newGroup->getId (), 'The group id does not match.' );
		$this->assertEquals ( $groupData [CustomerGroup::CODE], $newGroup->getCode (), 'The group code does not match.' );
		$this->assertEquals ( GroupRepository::DEFAULT_TAX_CLASS_ID, $newGroup->getTaxClassId (), 'The group tax class id does not match.' );
	}
	...
	public function testCreateGroupNoCodeExpectExceptionRest() {
		...
		try {
			...
			$this->assertContains ( '{"message":"%fieldName is a required field.","parameters":{"fieldName":"code"}', $e->getMessage (), "Exception does not contain expected message." );
	...
	public function testCreateGroupInvalidTaxClassIdRest() {
		...
		try {
			...
			// @codingStandardsIgnoreStart
			$this->assertContains ( '{"message":"Invalid value of \"%value\" provided for the %fieldName field.","parameters":{"fieldName":"taxClassId","value":9999}', $e->getMessage (), "Exception does not contain expected message." );
			// codingStandardsIgnoreEnd
		}
	}
	...
	public function testCreateGroupWithIdRest() {
		...
		try {
			...
			$this->assertContains ( '{"message":"No such entity with %fieldName = %fieldValue","parameters":{"fieldName":"id","fieldValue":88}', $e->getMessage (), "Exception does not contain expected message." );
		}
	}
	...
	public function testCreateGroupWithIdSoap() {
		...
		try {
			...
			$this->assertContains ( 'No such entity with %fieldName = %fieldValue', $e->getMessage (), "SoapFault does not contain expected message." );
		}
	}
	...
	public function testUpdateGroupRest() {
		...
		$this->assertEquals ( $groupData [CustomerGroup::TAX_CLASS_ID], $group->getTaxClassId (), 'The group tax class id did not change' );
	}
	...
	public function testUpdateGroupNotExistingGroupRest() {
		...
		try {
			...
			$expectedMessage = '{"message":"No such entity with %fieldName = %fieldValue",' . '"parameters":{"fieldName":"id","fieldValue":9999}';
	...
	public function testCreateGroupSoap() {
		...
		$this->assertEquals ( $groupId, $newGroup->getId (), "The group id does not match." );
		$this->assertEquals ( $groupData [CustomerGroup::CODE], $newGroup->getCode (), "The group code does not match." );
		$this->assertEquals ( $groupData ['taxClassId'], $newGroup->getTaxClassId (), "The group tax class id does not match." );
	}
	...
	public function testCreateGroupNoCodeExpectExceptionSoap() {
		...
		try {
			...
			$this->assertContains ( '%fieldName is a required field.', $e->getMessage (), "SoapFault does not contain expected message." );
		}
	}
	...
	public function testUpdateGroupSoap() {
		...
		$group = $this->groupRepository->getById ( $groupId );
		$this->assertEquals ( $groupData [CustomerGroup::CODE], $group->getCode (), 'The group code did not change.' );
		$this->assertEquals ( $groupData ['taxClassId'], $group->getTaxClassId (), 'The group tax class id did not change' );
	}
	...
	public function testDeleteGroupCannotDelete() {
		...
		$requestData = [ 
				CustomerGroup::ID => $groupIdAssignedDefault 
		];
	...
	public function testSearchGroupsDataProvider() {
		return [ 
				...
				[ 
						'code',
						md5 ( mt_rand ( 0, 10000000000 ) . time () ),
						null 
				],
				[ 
						'id',
	 ...
	 *        	Customer Group field to filter by
	 * @param string $filterValue
	 *        	Value of the field to be filtered by
	...
	public function testSearchGroups($filterField, $filterValue, $expectedResult) {
		...
		$filter = $filterBuilder->setField ( $filterField )->setValue ( $filterValue )->create ();
