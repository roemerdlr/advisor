...
class CustomerRepositoryTest extends WebapiAbstract {
	...
	public function testDeleteCustomer() {
		...
		$this->setExpectedException ( 'Magento\Framework\Exception\NoSuchEntityException', sprintf ( "No such entity with customerId = %s", $customerData [Customer::ID] ) );
	...
	public function testUpdateCustomer() {
		...
		$requestData = [ 
				'customer' => $newCustomerDataObject 
		];
	...
	public function testSearchCustomers() {
		...
		$customerData = $this->_createCustomer ();
		$filter = $builder->setField ( Customer::EMAIL )->setValue ( $customerData [Customer::EMAIL] )->create ();
		$this->searchCriteriaBuilder->addFilters ( [ 
				$filter 
		] );
		$searchData = $this->dataObjectProcessor->buildOutputDataArray ( $this->searchCriteriaBuilder->create (), 'Magento\Framework\Api\SearchCriteriaInterface' );
		$requestData = [ 
	...
	public function testSearchCustomersMultipleFiltersWithSort() {
		...
		$sortOrderBuilder = Bootstrap::getObjectManager ()->create ( 'Magento\Framework\Api\SortOrderBuilder' );
	...
	public function testSearchCustomersMultipleFiltersWithSortUsingGET() {
		...
		$this->searchCriteriaBuilder->setSortOrders ( [ 
				Customer::EMAIL => SortOrder::SORT_ASC 
		] );
		$searchCriteria = $this->searchCriteriaBuilder->create ();
		$searchData = $searchCriteria->__toArray ();
		$requestData = [ 
	...
	public function testSearchCustomersNonExistentMultipleFilters() {
		...
		$customerData1 = $this->_createCustomer ();
		$customerData2 = $this->_createCustomer ();
		$filter1 = $filter1 = $builder->setField ( Customer::EMAIL )->setValue ( $customerData1 [Customer::EMAIL] )->create ();
		$filter2 = $builder->setField ( Customer::EMAIL )->setValue ( $customerData2 [Customer::EMAIL] )->create ();
		$filter3 = $builder->setField ( Customer::LASTNAME )->setValue ( 'INVALID' )->create ();
		$this->searchCriteriaBuilder->addFilters ( [ 
				$filter1,
				$filter2 
		] );
