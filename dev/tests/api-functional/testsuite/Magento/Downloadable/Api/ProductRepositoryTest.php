...
class ProductRepositoryTest extends WebapiAbstract {
	...
	protected function createDownloadableProduct() {
		...
		$this->assertEquals ( \Magento\Downloadable\Model\Product\Type::TYPE_DOWNLOADABLE, $response ['type_id'] );
		return $response;
	}
	...
	public function testCreateDownloadableProduct() {
		...
		$this->assertTrue ( isset ( $response [ExtensibleDataInterface::EXTENSION_ATTRIBUTES_KEY] ["downloadable_product_samples"] ) );
	...
	public function testUpdateDownloadableProductLinks() {
		...
		$updatedLink1Data = [ 
				...
				'sample_file' => 'http://www.example.com/invalid' 
...
;
	...
	public function testUpdateDownloadableProductSamples() {
		...
		$resultSample = $response [ExtensibleDataInterface::EXTENSION_ATTRIBUTES_KEY] ["downloadable_product_samples"];
	...
	protected function getProduct($productSku) {
		...
		$response = (TESTS_WEB_API_ADAPTER == self::ADAPTER_SOAP) ? $this->_webApiCall ( $serviceInfo, [ 
				'sku' => $productSku 
