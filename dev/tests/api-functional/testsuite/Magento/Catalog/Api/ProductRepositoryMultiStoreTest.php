...
class ProductRepositoryMultiStoreTest extends WebapiAbstract {
	...
	public function testGetMultiStore() {
		...
		$this->assertEquals ( self::STORE_NAME_FROM_FIXTURE, $store->getName (), 'Precondition failed: fixture store was not created.' );
		...
		$this->assertEquals ( $nameInDefaultStore, $defaultStoreResponse [Product::NAME], 'Product name in default store is invalid.' );
		$fixtureStoreResponse = $this->_webApiCall ( $serviceInfo, $requestData, null, self::STORE_CODE_FROM_FIXTURE );
		$this->assertEquals ( $nameInFixtureStore, $fixtureStoreResponse [Product::NAME], 'Product name in fixture store is invalid.' );
	}
