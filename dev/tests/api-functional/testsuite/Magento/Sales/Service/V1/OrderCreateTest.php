...
class OrderCreateTest extends WebapiAbstract {
	...
	protected function prepareOrder() {
		...
		if (TESTS_WEB_API_ADAPTER == self::ADAPTER_SOAP) {
			$orderItem->setData ( 'parent_item', $orderItem->getData () + [ 
					'parent_item' => null 
			] );
			$orderItem->setAdditionalData ( 'test' );
		} else {
			$orderItem->setData ( 'parent_item', [ 
					'weight' => 1 
			] );
		}
	...
	public function testOrderCreate() {
		...
		$this->assertNotEmpty ( $this->_webApiCall ( $serviceInfo, [ 
				'entity' => $order 
		] ) );
