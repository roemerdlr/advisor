...
class TaxRateRepositoryTest extends WebapiAbstract {
	...
	public function testUpdateTaxRate() {
		...
		$this->assertEquals ( $expectedRateData ['tax_region_id'], $taxRateModel->getTaxRegionId (), 'Tax rate was not updated in  DB.' );
		$this->assertEquals ( $expectedRateData ['tax_country_id'], $taxRateModel->getTaxCountryId (), 'Tax rate was not updated in  DB.' );
		$this->assertEquals ( $expectedRateData ['tax_postcode'], $taxRateModel->getTaxPostcode (), 'Tax rate was not updated in  DB.' );
		$this->assertEquals ( $expectedRateData ['code'], $taxRateModel->getCode (), 'Tax rate was not updated in  DB.' );
		$this->assertEquals ( $expectedRateData ['rate'], $taxRateModel->getRate (), 'Tax rate was not updated in  DB.' );
	}
	...
	public function testSearchTaxRates() {
		...
		$filter = $this->filterBuilder->setField ( Rate::KEY_CODE )->setValue ( 'codeUs12' )->create ();
	...
	private function setupTaxRatesForSearch() {
		...
		$rates ['codeUs12'] = $objectManager->create ( 'Magento\Tax\Model\Calculation\Rate' )->setData ( $taxRateUs12 )->save ();
		...
		$rates ['codeUs14'] = $objectManager->create ( 'Magento\Tax\Model\Calculation\Rate' )->setData ( $taxRateUs14 )->save ();
		$taxRateBr13 = [ 
				'tax_country_id' => 'BR',
				'tax_region_id' => 13,
				'tax_postcode' => '*',
		...
		$rates ['codeBr13'] = $objectManager->create ( 'Magento\Tax\Model\Calculation\Rate' )->setData ( $taxRateBr13 )->save ();
		...
		$rates ['codeCz1'] = $objectManager->create ( 'Magento\Tax\Model\Calculation\Rate' )->setData ( $taxRateCz1 )->save ();
		$taxRateCz2 = [ 
				'tax_country_id' => 'CZ',
				'tax_postcode' => '250 00',
				'code' => 'codeCz2',
		...
		$rates ['codeCz2'] = $objectManager->create ( 'Magento\Tax\Model\Calculation\Rate' )->setData ( $taxRateCz2 )->save ();
