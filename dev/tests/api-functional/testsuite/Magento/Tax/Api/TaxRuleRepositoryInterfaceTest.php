...
class TaxRuleRepositoryInterfaceTest extends WebapiAbstract {
	...
	public function setUp() {
		...
		$this->filterBuilder = Bootstrap::getObjectManager ()->create ( 'Magento\Framework\Api\FilterBuilder' );
	...
	public function testSearchTaxRulesSimple() {
		...
		$filter = $this->filterBuilder->setField ( 'code' )->setValue ( 'Test Rule' )->create ();
	...
	public function testSearchTaxRulesCodeLike() {
		...
		$filter = $this->filterBuilder->setField ( 'code' )->setValue ( 'Test Rule%' )->setConditionType ( 'like' )->create ();
		
		$sortFilter = $this->filterBuilder->setField ( 'position' )->setValue ( 0 )->create ();
	...
	public function testUpdateTaxRule() {
		...
		$this->assertEquals ( $expectedRuleData ['code'], $taxRuleModel->getCode (), 'Tax rule code was updated incorrectly.' );
		$this->assertEquals ( $expectedRuleData ['position'], $taxRuleModel->getPosition (), 'Tax rule sort order was updated incorrectly.' );
		$this->assertEquals ( $expectedRuleData ['priority'], $taxRuleModel->getPriority (), 'Tax rule priority was updated incorrectly.' );
		$this->assertEquals ( $expectedRuleData ['customer_tax_class_ids'], array_values ( array_unique ( $taxRuleModel->getCustomerTaxClasses () ) ), 'Customer Tax classes were updated incorrectly' );
		$this->assertEquals ( $expectedRuleData ['product_tax_class_ids'], array_values ( array_unique ( $taxRuleModel->getProductTaxClasses () ) ), 'Product Tax classes were updated incorrectly.' );
		$this->assertEquals ( $expectedRuleData ['tax_rate_ids'], array_values ( array_unique ( $taxRuleModel->getRates () ) ), 'Tax rates were updated incorrectly.' );
	}
	...
	private function getFixtureTaxClasses() {
		if ($this->fixtureTaxClasses === null) {
			...
			if ($this->getFixtureTaxRules ()) {
				$taxClassIds = array_merge ( ( array ) $this->getFixtureTaxRules () [0]->getCustomerTaxClasses (), ( array ) $this->getFixtureTaxRules () [0]->getProductTaxClasses () );
