...
class RestErrorHandlingTest extends \Magento\TestFramework\TestCase\WebapiAbstract {
	...
	public function testNotFound() {
		...
		$this->_errorTest ( $serviceInfo, [ 
				'resourceY' 
		], WebapiException::HTTP_NOT_FOUND, 'Resource with ID "%1" not found.' );
	}
	public function testUnauthorized() {
		...
		$this->_errorTest ( $serviceInfo, [ ], WebapiException::HTTP_UNAUTHORIZED, 'Consumer is not authorized to access %1', [ 
				'resourceN' 
		] );
	}
	public function testOtherException() {
		...
		$this->_errorTest ( $serviceInfo, [ ], WebapiException::HTTP_INTERNAL_ERROR, $expectedMessage, null, 'Magento\TestModule3\Service\V1\Error->otherException()' ) // Check if trace contains proper error source
;
	}
	 ...
	 *        	- REST Service information (i.e. resource path and HTTP method)
	 * @param array $data
	 *        	- Data for the cal
	 ...
	 *        	- Optional parameters array, or null if no parameters
	 * @param string $traceString
	 *        	- Optional trace string to verify
	 */
	protected function _errorTest($serviceInfo, $data, $httpStatus, $errorMessage, $parameters = [], $traceString = null) {
		try {
			...
			$errorMessages = is_array ( $errorMessage ) ? $errorMessage : [ 
					$errorMessage 
			];
			...
			$this->assertContains ( $actualMessage, $errorMessages, "Message is invalid. Actual: '{$actualMessage}'. Expected one of: {'" . implode ( "', '", $errorMessages ) . "'}" );
