...
class SoapErrorHandlingTest extends \Magento\TestFramework\TestCase\WebapiAbstract {
	...
	public function testWebapiException() {
		...
		try {
			...
			$this->checkSoapFault ( $e, 'Service not found', 'env:Sender' );
		}
	}
	public function testUnknownException() {
		...
		try {
			...
			if (strpos ( $e->getMessage (), 'Internal Error' ) === false) {
				$this->checkSoapFault ( $e, 'Non service exception', 'env:Receiver', null, null, 'Magento\TestModule3\Service\V1\Error->otherException()' );
			} else {
				$this->checkSoapFault ( $e, 'Internal Error. Details are available in Magento log file. Report ID:', 'env:Receiver' );
			}
		}
	}
	...
	public function testUnauthorized() {
		...
		$expectedException = new AuthorizationException ( __ ( AuthorizationException::NOT_AUTHORIZED, [ 
...
;
	...
	protected function _testWrappedError($parameters) {
		...
		foreach ( $parameters as $error ) {
			$expectedException->addError ( __ ( \Magento\Framework\Exception\InputException::INVALID_FIELD_VALUE, $error ) );
		}
