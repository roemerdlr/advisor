...
abstract class BaseService extends \Magento\TestFramework\TestCase\WebapiAbstract {
	...
	protected function _assertRestUnauthorizedException($serviceInfo, $requestData = null) {
		try {
			...
			$this->assertContains ( '{"message":"' . AuthorizationException::NOT_AUTHORIZED . '"', $e->getMessage (), sprintf ( 'REST routing did not fail as expected for the method "%s" of service "%s"', $serviceInfo ['rest'] ['httpMethod'], $serviceInfo ['rest'] ['resourcePath'] ) );
	...
	protected function _assertSoapException($serviceInfo, $requestData = null, $expectedMessage = '') {
		try {
			...
			if (get_class ( $e ) !== 'SoapFault') {
				$this->fail ( sprintf ( 'Expected SoapFault exception not generated for Service - "%s" and Operation - "%s"', $serviceInfo ['soap'] ['service'], $serviceInfo ['soap'] ['operation'] ) );
			}
