...
class ServiceVersionV1Test extends \Magento\Webapi\Routing\BaseService {
	...
	protected function setUp() {
		...
		$this->valueFactory = Bootstrap::getObjectManager ()->create ( 'Magento\Framework\Api\AttributeValueFactory' );
		
		$this->itemFactory = Bootstrap::getObjectManager ()->create ( 'Magento\TestModule1\Service\V1\Entity\ItemFactory' );
	}
	...
	public function testItemAnyType() {
		...
		$item = $this->itemFactory->create ()->setItemId ( 1 )->setName ( 'testProductAnyType' )->setCustomAttributes ( [ 
				$attributeValue1,
				$attributeValue2,
				$attributeValue3 
		] );
		...
		$serviceInfo = [ 
				...
				'soap' => [ 
						'service' => $this->_soapService,
						'operation' => $this->_soapService . 'ItemAnyType' 
				] 
		];
		$requestData = $item->__toArray ();
		$item = $this->_webApiCall ( $serviceInfo, [ 
				'entityItem' => $requestData 
		] );
