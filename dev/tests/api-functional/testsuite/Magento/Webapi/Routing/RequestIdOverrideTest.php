...
class RequestIdOverrideTest extends \Magento\Webapi\Routing\BaseService {
	...
	public function testOverride() {
		...
		$item = $this->itemFactory->create ()->setEntityId ( $incorrectItemId )->setName ( 'test' );
		$requestData = [ 
				'entityItem' => $item->__toArray () 
		];
		$item = $this->_webApiCall ( $serviceInfo, $requestData );
		$this->assertEquals ( $itemId, $item [\Magento\TestModule5\Service\V1\Entity\AllSoapAndRest::ID], 'Identifier overriding failed.' );
	}
	public function testOverrideNested() {
		...
		$item = $this->_webApiCall ( $serviceInfo, $requestData );
		$this->assertEquals ( $secondItemId, $item [\Magento\TestModule5\Service\V1\Entity\AllSoapAndRest::ID], 'Identifier overriding failed for nested resource request.' );
	}
	...
	public function testOverrideWithScalarValues() {
		...
		$serviceInfo = [ 
				'rest' => [ 
						'resourcePath' => "/{$this->_version}/TestModule5/OverrideService/" . $firstItemId . '/nestedResource/' . $secondItemId,
						'httpMethod' => \Magento\Framework\Webapi\Rest\Request::HTTP_METHOD_PUT 
				] 
		];
		
		$requestData = [ 
				'entity_id' => $incorrectItemId,
				'name' => 'test',
				'orders' => true 
		];
		$item = $this->_webApiCall ( $serviceInfo, $requestData );
		$this->assertEquals ( $secondItemId, $item [\Magento\TestModule5\Service\V1\Entity\AllSoapAndRest::ID], 'Identifier overriding failed.' );
	}
}
