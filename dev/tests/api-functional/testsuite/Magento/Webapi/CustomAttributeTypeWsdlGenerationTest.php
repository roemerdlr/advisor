...
class CustomAttributeTypeWsdlGenerationTest extends \Magento\TestFramework\TestCase\WebapiAbstract {
	...
	public function testCustomAttributeTypesInWsdl() {
		...
		foreach ( $types as $type ) {
			if (strpos ( $type, 'TestModuleMSCDataCustomAttributeDataObjectInterface' ) !== false || strpos ( $type, 'TestModuleMSCDataCustomAttributeNestedDataObjectInterface' ) !== false) {
				$testCustomTypeCount ++;
			}
		}
		
		$this->assertEquals ( 2, $testCustomTypeCount, 'Incorrect count for Custom attribute types. Found "' . $testCustomTypeCount . ' type(s)" expected 2.' );
	}
}
