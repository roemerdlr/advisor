...
class Processor {
	...
	public function __construct(\Magento\Framework\App\Response\Http $response) {
		...
		if (! empty ( $_SERVER ['SCRIPT_NAME'] )) {
			if (in_array ( basename ( $_SERVER ['SCRIPT_NAME'], '.php' ), [ 
					'404',
					'503',
					'report' 
			] )) {
	...
	public function getViewFileUrl() {
		...
		return $this->getBaseUrl () . str_replace ( str_replace ( '\\', '/', $this->_indexDir ), '', str_replace ( '\\', '/', $this->_errorDir ) ) . $this->_config->skin . '/';
	}
	...
	public function getHostUrl() {
		...
		if (! empty ( $_SERVER ['SERVER_PORT'] ) && ! in_array ( $_SERVER ['SERVER_PORT'], [ 
				80,
				433 
		] ) && ! preg_match ( '/.*?\:[0-9]+$/', $url )) {
	...
	protected function _validate() {
		$email = preg_match ( '/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/', $this->postData ['email'] );
	...
	protected function _setReportUrl() {
		if ($this->reportId && $this->_config && isset ( $this->_config->skin )) {
			$this->reportUrl = "{$this->getBaseUrl(true)}pub/errors/report.php?" . http_build_query ( [ 
					'id' => $this->reportId,
