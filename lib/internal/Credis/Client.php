...
class Credis_Client {
	 ...
	 * @var array
	 */
	protected $wrapperMethods = array (
			'delete' => 'del',
			'getkeys' => 'keys',
			'sremove' => 'srem' 
	);
	 ...
	 * Use 0 to disable timeouts entirely (or use a very long timeout
	 * if not supported).
	 *
	 * @param int $timeout
	 *        	0 (or -1) for no timeout, otherwise number of seconds
	 * @throws CredisException
	 * @return Credis_Client
	 */
	...
	/**
	 * Enabled command renaming and provide mapping method.
	...
	public function auth($password) {
		$response = $this->__call ( 'auth', array (
				$password 
		) );
		$this->authPassword = $password;
		return $response;
	...
	public function select($index) {
		$response = $this->__call ( 'select', array (
				$index 
		) );
	...
	public function pSubscribe($patterns, $callback) {
		if (! $this->standalone) {
			return $this->__call ( 'pSubscribe', array (
					( array ) $patterns,
					$callback 
			) );
		}
		...
		while ( $patternCount -- ) {
			if (isset ( $status )) {
				...
				list ( $command, $pattern, $status ) = $this->__call ( 'psubscribe', array (
						$patterns 
				) );
			}
			if (! $status) {
	...
	public function subscribe($channels, $callback) {
		if (! $this->standalone) {
			return $this->__call ( 'subscribe', array (
					( array ) $channels,
					$callback 
			) );
		}
		...
		while ( $channelCount -- ) {
			if (isset ( $status )) {
				...
				list ( $command, $channel, $status ) = $this->__call ( 'subscribe', array (
						$channels 
				) );
			}
			if (! $status) {
			...
			if ($this->usePipeline) {
				if ($name == 'pipeline') {
					...
					if ($this->isMulti) {
						...
						$this->commands .= self::_prepare_command ( array (
								$this->getRenamedCommand ( $name ) 
						) );
					}
			...
			try {
				...
				if ($name == 'pipeline' || $name == 'multi') {
					if ($this->isMulti) {
						...
						$this->isMulti = TRUE;
						$this->redisMulti = call_user_func_array ( array (
								$this->redis,
								$name 
						), $args );
					}
	...
	private static function _prepare_command($args) {
		return sprintf ( '*%d%s%s%s', count ( $args ), CRLF, implode ( array_map ( array (
				'self',
				'_map' 
		), $args ), CRLF ), CRLF );
	}
