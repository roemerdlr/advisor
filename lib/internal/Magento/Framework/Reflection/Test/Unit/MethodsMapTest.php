...
class MethodsMapTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$cacheMock = $this->getMockBuilder ( 'Magento\Framework\Cache\FrontendInterface' )->getMockForAbstractClass ();
		$cacheMock->expects ( $this->any () )->method ( 'save' );
		$cacheMock->expects ( $this->any () )->method ( 'load' )->will ( $this->returnValue ( null ) );
		
		$attributeTypeResolverMock = $this->getMockBuilder ( 'Magento\Framework\Api\AttributeTypeResolverInterface' )->getMockForAbstractClass ();
		$fieldNamerMock = $this->getMockBuilder ( 'Magento\Framework\Reflection\FieldNamer' )->getMockForAbstractClass ();
		$this->model = $objectManager->getObject ( 'Magento\Framework\Reflection\MethodsMap', [ 
				'cache' => $cacheMock,
	...
	public function testGetMethodsMap() {
		...
		$this->assertEquals ( [ 
				...
				'getMethodsMap' => [ 
						'type' => 'array',
						'isRequired' => true,
						'description' => "<pre> Service methods' reflection data stored in cache as 'methodName' => " . "'returnType' ex. [ 'create' => '\Magento\Customer\Api\Data\Customer', 'validatePassword' " . "=> 'boolean' ] </pre>",
						'parameterCount' => 1 
				],
				'getMethodParams' => [ 
						'type' => 'array',
