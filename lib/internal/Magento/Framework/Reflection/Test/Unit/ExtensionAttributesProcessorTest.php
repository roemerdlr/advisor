...
class ExtensionAttributesProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testBuildOutputDataArrayWithPermission($isPermissionAllowed, $expectedValue) {
		...
		$this->methodsMapProcessorMock->expects ( $this->once () )->method ( 'getMethodsMap' )->with ( $dataObjectType )->will ( $this->returnValue ( [ 
				$methodName => [ ] 
		] ) );
		...
		$this->configMock->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( [ 
				$dataObjectType => [ 
						$attributeName => [ 
