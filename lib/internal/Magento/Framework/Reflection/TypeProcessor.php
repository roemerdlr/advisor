...
class TypeProcessor {
	...
	protected function _processMethod(\Zend\Code\Reflection\MethodReflection $methodReflection, $typeName) {
		...
		if ($isGetter && ! $methodReflection->getNumberOfRequiredParameters ()) {
			...
			if ($returnMetadata ['description']) {
				...
				$description = $this->dataObjectGetterDescriptionToFieldDescription ( $methodReflection->getDocBlock ()->getShortDescription () );
			}
	 ...
	 * @param string $type
	 *        	Convert given value to the this simple type
	 * @return int|string|float|int[]|string[]|float[] Return the value which is converted to type
	 * @throws SerializationException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
		...
		if ($isArrayType && is_array ( $value )) {
			...
			foreach ( array_keys ( $value ) as $key ) {
				if ($value !== null && ! settype ( $value [$key], $arrayItemType )) {
					throw new SerializationException ( new Phrase ( SerializationException::TYPE_MISMATCH, [ 
							'value' => $value,
							'type' => $type 
					] ) );
				}
	 ...
	 * @throws \Exception If $camelCaseProperty has no corresponding setter method
	 */
	protected function findAccessorMethodName(ClassReflection $class, $camelCaseProperty, $accessorName, $boolAccessorName) {
		if ($this->classHasMethod ( $class, $accessorName )) {
			...
			throw new \LogicException ( sprintf ( 'Property "%s" does not have corresponding setter in class "%s".', $camelCaseProperty, $class->getName () ) );
		}
	}
		...
		foreach ( $interface as $direction => $interfaceData ) {
			...
			foreach ( $interfaceData ['parameters'] as $parameterData ) {
				if (! $this->isTypeSimple ( $parameterData ['type'] ) && ! $this->isTypeAny ( $parameterData ['type'] )) {
					...
					$this->setTypeData ( $parameterData ['type'], [ 
							'callInfo' => $callInfo 
					] );
				}
			}
