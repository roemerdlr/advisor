...
class ConditionManager {
	...
	public function combineQueries(array $queries, $unionOperator) {
		return implode ( ' ' . $unionOperator . ' ', array_filter ( $queries, 'strlen' ) );
	}
	...
	public function generateCondition($field, $operator, $value) {
		return sprintf ( is_array ( $value ) ? self::CONDITION_PATTERN_ARRAY : self::CONDITION_PATTERN_SIMPLE, $this->connection->quoteIdentifier ( $field ), $operator, $this->connection->quote ( $value ) );
	}
}
