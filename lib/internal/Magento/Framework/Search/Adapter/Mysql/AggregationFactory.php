...
class AggregationFactory {
	...
	public function create(array $rawAggregation) {
		...
		foreach ( $rawAggregation as $rawBucketName => $rawBucket ) {
			...
			$buckets [$rawBucketName] = $this->objectManager->create ( 'Magento\Framework\Search\Response\Bucket', [ 
					'name' => $rawBucketName,
					'values' => $this->prepareValues ( ( array ) $rawBucket ) 
			] );
		}
		return $this->objectManager->create ( 'Magento\Framework\Search\Response\Aggregation', [ 
				'buckets' => $buckets 
		] );
	}
	...
	private function prepareValues(array $values) {
		...
		foreach ( $values as $name => $value ) {
			$valuesObjects [] = $this->objectManager->create ( 'Magento\Framework\Search\Response\Aggregation\Value', [ 
					'value' => $name,
					'metrics' => $value 
			] );
		}
