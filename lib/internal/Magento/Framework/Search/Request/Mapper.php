...
class Mapper {
	 ...
	 */
	public function __construct(\Magento\Framework\ObjectManagerInterface $objectManager, array $queries, $rootQueryName, array $aggregations = [], array $filters = []) {
	 ...
	 * @throws \Exception
	 * @throws \InvalidArgumentException
	 * @throws StateException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	private function mapQuery($queryName) {
		if (! isset ( $this->queries [$queryName] )) {
			...
			throw new StateException ( new Phrase ( 'Cycle found. Query %1 already used in request hierarchy', [ 
					$queryName 
			] ) );
		}
		...
		switch ($query ['type']) {
			case QueryInterface::TYPE_MATCH :
				$query = $this->objectManager->create ( 'Magento\Framework\Search\Request\Query\Match', [ 
						'name' => $query ['name'],
						'value' => $query ['value'],
			...
			case QueryInterface::TYPE_FILTER :
				...
				$query = $this->objectManager->create ( 'Magento\Framework\Search\Request\Query\Filter', [ 
						'name' => $query ['name'],
						'boost' => isset ( $query ['boost'] ) ? $query ['boost'] : 1,
						'reference' => $reference,
			...
			case QueryInterface::TYPE_BOOL :
				...
				$query = $this->objectManager->create ( 'Magento\Framework\Search\Request\Query\BoolExpression', array_merge ( [ 
						'name' => $query ['name'],
						'boost' => isset ( $query ['boost'] ) ? $query ['boost'] : 1 
				], $aggregatedByType ) );
				break;
	...
	private function mapFilter($filterName) {
		if (! isset ( $this->filters [$filterName] )) {
			...
			throw new StateException ( new Phrase ( 'Cycle found. Filter %1 already used in request hierarchy', [ 
					$filterName 
			] ) );
		}
		...
		switch ($filter ['type']) {
			case FilterInterface::TYPE_TERM :
				$filter = $this->objectManager->create ( 'Magento\Framework\Search\Request\Filter\Term', [ 
						'name' => $filter ['name'],
						'field' => $filter ['field'],
						'value' => $filter ['value'] 
				] );
			...
			case FilterInterface::TYPE_RANGE :
				$filter = $this->objectManager->create ( 'Magento\Framework\Search\Request\Filter\Range', [ 
						'name' => $filter ['name'],
						'field' => $filter ['field'],
			...
			case FilterInterface::TYPE_WILDCARD :
				$filter = $this->objectManager->create ( 'Magento\Framework\Search\Request\Filter\Wildcard', [ 
						'name' => $filter ['name'],
						'field' => $filter ['field'],
						'value' => $filter ['value'] 
				] );
			...
			case FilterInterface::TYPE_BOOL :
				...
				$filter = $this->objectManager->create ( 'Magento\Framework\Search\Request\Filter\BoolExpression', array_merge ( [ 
						'name' => $filter ['name'] 
				], $aggregatedByType ) );
				break;
			default :
	...
	private function validateNotUsed($elements, $mappedElements, $errorMessage) {
		...
		if (! empty ( $notUsedElements )) {
			throw new StateException ( new Phrase ( $errorMessage, [ 
					$notUsedElements 
			] ) );
		}
	}
		...
		foreach ( $this->aggregations as $bucketData ) {
			...
			switch ($bucketData ['type']) {
				case BucketInterface::TYPE_TERM :
					$bucket = $this->objectManager->create ( 'Magento\Framework\Search\Request\Aggregation\TermBucket', $arguments );
					break;
				case BucketInterface::TYPE_RANGE :
					$bucket = $this->objectManager->create ( 'Magento\Framework\Search\Request\Aggregation\RangeBucket', array_merge ( $arguments, [ 
							'ranges' => $this->mapRanges ( $bucketData ) 
					] ) );
					break;
				case BucketInterface::TYPE_DYNAMIC :
					$bucket = $this->objectManager->create ( 'Magento\Framework\Search\Request\Aggregation\DynamicBucket', array_merge ( $arguments, [ 
	...
	private function mapMetrics(array $bucketData) {
		...
		if (isset ( $bucketData ['metric'] )) {
			...
			foreach ( $metrics as $metric ) {
				$metricObjects [] = $this->objectManager->create ( 'Magento\Framework\Search\Request\Aggregation\Metric', [ 
						'type' => $metric ['type'] 
				] );
			}
		}
	...
	private function mapRanges(array $bucketData) {
		...
		if (isset ( $bucketData ['range'] )) {
			...
			foreach ( $ranges as $range ) {
				$rangeObjects [] = $this->objectManager->create ( 'Magento\Framework\Search\Request\Aggregation\Range', [ 
						'from' => $range ['from'],
						'to' => $range ['to'] 
				] );
			}
