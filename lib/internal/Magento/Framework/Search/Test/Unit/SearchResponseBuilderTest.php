...
class SearchResponseBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->searchResultFactory = $this->getMockBuilder ( 'Magento\Framework\Api\Search\SearchResultFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->documentFactory = $this->getMockBuilder ( 'Magento\Framework\Api\Search\DocumentFactory' )->disableOriginalConstructor ()->getMock ();
	...
	public function testBuild() {
		...
		$responseDocument->expects ( $this->any () )->method ( 'getIterator' )->willReturn ( new \ArrayIterator ( [ 
				$field 
		] ) );
		...
		$response = $this->getMockBuilder ( 'Magento\Framework\Search\Response\QueryResponse' )->setMethods ( [ 
				'getIterator',
				'getAggregations' 
