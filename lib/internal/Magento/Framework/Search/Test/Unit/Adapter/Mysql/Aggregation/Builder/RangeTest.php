...
class RangeTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->metricsBuilder = $this->getMockBuilder ( 'Magento\Framework\Search\Adapter\Mysql\Aggregation\Builder\Metrics' )->setMethods ( [ 
				'build' 
		...
		$this->range = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Aggregation\Range' )->setMethods ( [ 
				'getFrom',
				'getTo' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->builder = $helper->getObject ( 'Magento\Framework\Search\Adapter\Mysql\Aggregation\Builder\Range', [ 
	...
	public function testBuild() {
		$this->metricsBuilder->expects ( $this->once () )->method ( 'build' )->willReturn ( [ 
				'metrics' 
		] );
		...
		$this->connectionMock->expects ( $this->once () )->method ( 'getCaseSql' )->withConsecutive ( [ 
				'' 
		], [ 
				[ 
						'`value` < 50' => "'*_50'",
