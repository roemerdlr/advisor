...
class BuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->conditionManager = $this->getMockBuilder ( 'Magento\Framework\Search\Adapter\Mysql\ConditionManager' )->disableOriginalConstructor ()->setMethods ( [ 
				'generateCondition',
				'combineQueries',
				'wrapBrackets' 
		] )->getMock ();
		...
		$rangeBuilder = $this->getMockBuilder ( '\Magento\Framework\Search\Adapter\Mysql\Filter\Builder\Range' )->setMethods ( [ 
				'buildFilter' 
		...
		$rangeBuilder->expects ( $this->any () )->method ( 'buildFilter' )->will ( $this->returnCallback ( function (FilterInterface $filter, $isNegation) {
			...
			return $this->conditionManager->combineQueries ( [ 
					$fromCondition,
					$toCondition 
			], $unionOperator );
		} ) );
		
		$termBuilder = $this->getMockBuilder ( '\Magento\Framework\Search\Adapter\Mysql\Filter\Builder\Term' )->setMethods ( [ 
				'buildFilter' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->preprocessor = $this->getMockBuilder ( 'Magento\Framework\Search\Adapter\Mysql\Filter\PreprocessorInterface' )->setMethods ( [ 
				'process' 
		...
		$this->builder = $objectManager->getObject ( 'Magento\Framework\Search\Adapter\Mysql\Filter\Builder', [ 
				'range' => $rangeBuilder,
				'term' => $termBuilder,
	...
	public function buildFilterDataProvider() {
		return array_merge ( $this->buildTermFilterDataProvider (), $this->buildRangeFilterDataProvider (), $this->buildBoolFilterDataProvider () );
	}
	...
	private function createTermFilter($field, $value) {
		$filter = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Filter\Term' )->setMethods ( [ 
				'getField',
				'getValue' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$filter->expects ( $this->exactly ( 1 ) )->method ( 'getField' )->will ( $this->returnValue ( $field ) );
		$filter->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $value ) );
		return $filter;
	}
	...
	private function createRangeFilter($field, $from, $to) {
		$filter = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Filter\Range' )->setMethods ( [ 
				'getField',
				'getFrom',
				'getTo' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$filter->expects ( $this->exactly ( 2 ) )->method ( 'getField' )->will ( $this->returnValue ( $field ) );
		$filter->expects ( $this->atLeastOnce () )->method ( 'getFrom' )->will ( $this->returnValue ( $from ) );
		$filter->expects ( $this->atLeastOnce () )->method ( 'getTo' )->will ( $this->returnValue ( $to ) );
		return $filter;
	}
	...
	public function buildBoolFilterDataProvider() {
		return [ 
				'boolFilterWithMust' => [ 
						'filter' => $this->createBoolFilter ( [  // must
...
[  // mustNot
				...
				'boolFilterWithAllFields' => [ 
						...
						'conditionType' => RequestBoolQuery::QUERY_CONDITION_MUST,
						'expectedResult' => '(((term1 = 1)) AND ((range1 >= 0 AND range1 < 10))' . ' AND (((term2 = 1)) OR ((range2 >= 0 AND range2 < 10)))' . ' AND (((term3 != 1)) AND ((range3 < 0 OR range3 >= 10))))' 
				],
				'boolFilterInBoolFilter' => [ 
						...
						'conditionType' => RequestBoolQuery::QUERY_CONDITION_MUST,
						'expectedResult' => '(((term1 = 1)) AND ((range1 >= 0 AND range1 < 10))' . ' AND (((term2 = 1)) OR ((range2 >= 0 AND range2 < 10)))' . ' AND (((term3 != 1)) AND ((range3 < 0 OR range3 >= 10))' . ' AND ((((term4 != 1)) AND ((range4 < 0 OR range4 >= 10))' . ' AND (((term5 != 1)) OR ((range5 < 0 OR range5 >= 10)))' . ' AND (((term6 = 1)) AND ((range6 >= 0 AND range6 < 10)))))))' 
				],
				'boolEmpty' => [ 
	...
	private function createBoolFilter(array $must, array $should, array $mustNot) {
		$filter = $this->getMockBuilder ( 'Magento\Framework\Search\Request\Filter\BoolExpression' )->setMethods ( [ 
				'getMust',
				'getShould',
				'getMustNot' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$filter->expects ( $this->once () )->method ( 'getMust' )->will ( $this->returnValue ( $must ) );
		$filter->expects ( $this->once () )->method ( 'getShould' )->will ( $this->returnValue ( $should ) );
		$filter->expects ( $this->once () )->method ( 'getMustNot' )->will ( $this->returnValue ( $mustNot ) );
		return $filter;
	}
	...
	public function testUnknownFilterType() {
		...
		$filter = $this->getMockBuilder ( 'Magento\Framework\Search\Request\FilterInterface' )->setMethods ( [ 
				'getType' 
