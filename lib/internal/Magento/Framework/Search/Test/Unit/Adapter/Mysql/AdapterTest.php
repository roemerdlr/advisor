...
class AdapterTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->mapper = $this->getMockBuilder ( '\Magento\Framework\Search\Adapter\Mysql\Mapper' )->setMethods ( [ 
				'buildQuery' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->aggregatioBuilder = $this->getMockBuilder ( 'Magento\Framework\Search\Adapter\Mysql\Aggregation\Builder' )->setMethods ( [ 
				'build' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$this->temporaryStorage = $this->getMockBuilder ( 'Magento\Framework\Search\Adapter\Mysql\TemporaryStorage' )->disableOriginalConstructor ()->getMock ();
		
		$temporaryStorageFactoryName = 'Magento\Framework\Search\Adapter\Mysql\TemporaryStorageFactory';
		$temporaryStorageFactory = $this->getMockBuilder ( $temporaryStorageFactoryName )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$temporaryStorageFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->temporaryStorage );
		
		$this->adapter = $this->objectManager->getObject ( 'Magento\Framework\Search\Adapter\Mysql\Adapter', [ 
				'mapper' => $this->mapper,
