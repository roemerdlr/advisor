...
class TemporaryStorageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$resource->expects ( $this->any () )->method ( 'getConnection' )->willReturn ( $this->adapter );
		$resource->expects ( $this->any () )->method ( 'getTableName' )->willReturn ( $this->tableName );
	...
	public function testStoreDocumentsFromSelect() {
		...
		$this->adapter->expects ( $this->once () )->method ( 'insertFromSelect' )->with ( $select, $this->tableName )->willReturn ( $sql );
	...
	public function testStoreDocuments() {
		...
		$result = $this->model->storeDocuments ( [ 
				$document 
		] );
	...
	private function createTemporaryTable() {
		...
		$table->expects ( $this->at ( 1 ) )->method ( 'addColumn' )->with ( TemporaryStorage::FIELD_ENTITY_ID, Table::TYPE_INTEGER, 10, [ 
				'unsigned' => true,
				'nullable' => false,
				'primary' => true 
		], 'Entity ID' );
		$table->expects ( $this->at ( 2 ) )->method ( 'addColumn' )->with ( 'score', Table::TYPE_DECIMAL, [ 
				32,
				16 
		], [ 
				'unsigned' => true,
