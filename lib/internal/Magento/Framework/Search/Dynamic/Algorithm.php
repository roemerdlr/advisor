...
class Algorithm {
		...
		for($intervalNumber = 1; $intervalNumber < $this->getIntervalsNumber (); ++ $intervalNumber) {
			...
			while ( ! empty ( $separator ) && ! array_key_exists ( $intervalNumber, $result ) ) {
				...
				if ($bestSeparator && $bestSeparator [2] > 0) {
					$isEqualValue = $intervalFirstValue == $this->_values [$bestSeparator [2] - 1] ? $this->_values [0] : false;
			...
			if (array_key_exists ( $intervalNumber, $result )) {
				...
				if ($valueIndex != - 1 && $lastSeparator > $this->_lastValueLimiter [1]) {
					$this->_lastValueLimiter = [ 
							$valueIndex + $this->_quantileInterval [0],
							$lastSeparator 
					];
				}
			}
	 ...
	 *        	should be from 1 to n-1 where n is number of intervals
	 * @param IntervalInterface $interval        	
	 * @return array|null @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	...
	protected function _findValueSeparator($quantileNumber, IntervalInterface $interval) {
		...
		if ($offset < 0) {
			...
			$values = array_slice ( $this->_values, $this->_lastValueLimiter [0] + $offset - $this->_quantileInterval [0], - $offset );
			$offset = 0;
		}
		...
		if ($intervalValuesCount >= 0) {
			$values = array_merge ( $values, $interval->load ( $intervalValuesCount + 1, $offset, $lowerValue, $this->_upperLimit ) );
		}
		...
		if ($lastValue == $values [0]) {
			if ($quantileNumber == 1 && $offset) {
				...
				if ($additionalValues) {
					...
					$bestRoundValue = $this->_findRoundValue ( $values [0] + self::MIN_POSSIBLE_VALUE / 10, $lastValue, false );
				}
			}
			if ($quantileNumber == $this->getIntervalsNumber () - 1) {
				...
				if ($values [$valuesCount - 1] > $lastValue) {
					...
					$additionalValues = $interval->loadNext ( $lastValue, $this->_count - $quantileInterval [0] - count ( $values ), $this->_upperLimit );
				}
				if ($additionalValues) {
					...
					$upperBestRoundValue = $this->_findRoundValue ( $lastValue + self::MIN_POSSIBLE_VALUE / 10, $values [count ( $values ) - 1], false );
					$this->_mergeRoundValues ( $bestRoundValue, $upperBestRoundValue );
				}
			}
		} else {
			$bestRoundValue = $this->_findRoundValue ( $values [0] + self::MIN_POSSIBLE_VALUE / 10, $lastValue );
		}
		...
		if ($values [$valuesCount - 1] > $lastValue) {
			$this->_lastValueLimiter = [ 
					$quantileInterval [0] + $valuesCount - 1,
					$values [$valuesCount - 1] 
			];
		}
	...
	protected function _getQuantileInterval($quantileNumber) {
		...
		if (array_key_exists ( $quantileNumber - 1, $this->_skippedQuantilesUpperLimits ) && $left > $this->_skippedQuantilesUpperLimits [$quantileNumber - 1]) {
	 ...
	 *        	whether empty result is acceptable
	 * @param null|float $roundingFactor
	 *        	if given, checks for range to contain the factor
	 * @return false|array @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	protected function _findRoundValue($lowerValue, $upperValue, $returnEmpty = true, $roundingFactor = null) {
		...
		while ( $tenPower >= self::MIN_POSSIBLE_VALUE ) {
			...
			foreach ( $roundingFactorCoefficients as $roundingFactorCoefficient ) {
				...
				$roundValues = $this->_findRoundValue ( $lowerValue, $upperValue, $returnEmpty, $roundingFactorCoefficient );
				if ($roundValues) {
					$index = round ( $roundingFactorCoefficient / self::MIN_POSSIBLE_VALUE );
	 ...
	 *
	 * @param
	 *        	array &$oldRoundValues
	 * @param
	 *        	array &$newRoundValues
	 * @return void
	 */
	protected function _mergeRoundValues(&$oldRoundValues, &$newRoundValues) {
		foreach ( $newRoundValues as $roundingFactor => $roundValueValues ) {
			if (array_key_exists ( $roundingFactor, $oldRoundValues )) {
				$oldRoundValues [$roundingFactor] = array_unique ( array_merge ( $oldRoundValues [$roundingFactor], $roundValueValues ) );
			} else {
	...
	protected function _findBestSeparator($quantileNumber, $separators) {
		...
		while ( $i < $valuesCount && ! empty ( $separators ) ) {
			$i = $this->_binarySearch ( $separators [0], [ 
					$i 
			] );
			if ($i == - 1) {
				break;
			...
			$deflection = abs ( $quantileNumber * $this->_count - ($this->_quantileInterval [0] + $i) * $this->_getCalculatedIntervalsNumber () );
	 ...
	 *
	 * @param float $value        	
	 * @param null|float[] $limits
	 *        	search [from, to]
	 * @return int @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
