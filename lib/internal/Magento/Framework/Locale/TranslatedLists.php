...
class TranslatedLists implements ListsInterface {
	...
	public function getOptionTimezones() {
		...
		foreach ( $zones as $code ) {
			$options [] = [ 
					'label' => \IntlTimeZone::createTimeZone ( $code )->getDisplayName ( false, \IntlTimeZone::DISPLAY_LONG, $locale ) . ' (' . $code . ')',
					'value' => $code 
			];
		}
		return $this->_sortOptionArray ( $options );
	...
	public function getOptionWeekdays($preserveCodes = false, $ucFirstCode = false) {
		$options = [ ];
		$days = (new DataBundle ())->get ( $this->localeResolver->getLocale () ) ['calendar'] ['gregorian'] ['dayNames'] ['format'] ['wide'] ?  : [ ];
