...
class CurrencyTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->mockEventManager = $this->getMockBuilder ( '\Magento\Framework\Event\Manager' )->disableOriginalConstructor ()->getMock ();
		$this->mockLocaleResolver = $this->getMockBuilder ( '\Magento\Framework\Locale\Resolver' )->disableOriginalConstructor ()->getMock ();
		$this->mockCurrencyFactory = $this->getMockBuilder ( '\Magento\Framework\CurrencyFactory' )->disableOriginalConstructor ()->getMock ();
		
		$this->testCurrencyObject = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Framework\Locale\Currency', [ 
	...
	public function testGetCurrencyNonCached() {
		...
		$this->mockCurrencyFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $options ) );
		
		$this->mockEventManager->expects ( $this->once () )->method ( 'dispatch' );
		
		$retrievedCurrencyObject = $this->testCurrencyObject->getCurrency ( self::TEST_NONCACHED_CURRENCY );
	...
	public function testGetCurrencyCached() {
		...
		$retrievedCurrencyObject = $this->testCurrencyObject->getCurrency ( self::TEST_CACHED_CURRENCY );
		...
		$this->mockCurrencyFactory->expects ( $this->never () )->method ( 'create' )->will ( $this->returnValue ( $options ) );
	...
	public function testGetNonExistantCurrency() {
		...
		$retrievedCurrencyObject = $this->testCurrencyObject->getCurrency ( self::TEST_NONEXISTANT_CURRENCY );
