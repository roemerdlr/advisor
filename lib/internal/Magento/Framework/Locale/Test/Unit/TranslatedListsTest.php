...
class TranslatedListsTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->mockConfig = $this->getMockBuilder ( '\Magento\Framework\Locale\ConfigInterface' )->disableOriginalConstructor ()->getMock ();
		$this->mockLocaleResolver = $this->getMockBuilder ( '\Magento\Framework\Locale\ResolverInterface' )->disableOriginalConstructor ()->getMock ();
		$this->mockLocaleResolver->expects ( $this->once () )->method ( 'getLocale' )->will ( $this->returnValue ( 'en_US' ) );
		
		$this->listsModel = new \Magento\Framework\Locale\TranslatedLists ( $this->mockConfig, $this->mockLocaleResolver );
	}
	...
	protected function setupForOptionLocales() {
		...
		$this->mockConfig->expects ( $this->once () )->method ( 'getAllowedLocales' )->will ( $this->returnValue ( $allowedLocales ) );
	}
}
