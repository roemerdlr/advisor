...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * @param string $name        	
	 * @param
	 *        	\Magento\Framework\Event | null $event
	...
	protected function getObserverMock($name, $event = null) {
		...
		$observer->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( $name ) );
		if ($event) {
			$observer->expects ( $this->once () )->method ( 'dispatch' )->with ( $event );
		}
		return $observer;
	}
	...
	public function testGetAllObservers() {
		...
		$this->assertEquals ( [ 
				'test_observer1' => $observer1,
				'test_observer2' => $observer2 
