...
class ObserverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testIsValidFor() {
		...
		$eventMock->expects ( $this->once () )->method ( 'getName' )->will ( $this->returnValue ( $eventName ) );
	...
	public function testDispatch() {
		...
		$callbackMock [0]->expects ( $this->once () )->method ( 'testCallback' )->will ( $this->returnValue ( true ) );
		$eventMock = $this->getMock ( 'Magento\Framework\Event', [ 
				'getName' 
		] );
		$eventMock->expects ( $this->once () )->method ( 'getName' )->will ( $this->returnValue ( $eventName ) );
