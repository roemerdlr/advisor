...
class GridStructure implements IndexStructureInterface {
	 ...
	 */
	public function __construct(ResourceConnection $resource, FlatScopeResolver $flatScopeResolver, array $columnTypesMap = []) {
	...
	protected function createFlatTable($tableName, array $fields) {
		...
		foreach ( $fields as $field ) {
			...
			$columnMap = isset ( $field ['dataType'] ) && isset ( $this->columnTypesMap [$field ['dataType']] ) ? $this->columnTypesMap [$field ['dataType']] : [ 
					'type' => $field ['dataType'],
			...
			if ($field ['type'] === 'filterable') {
				$table->addIndex ( $this->resource->getIdxName ( $tableName, $name, AdapterInterface::INDEX_TYPE_INDEX ), $name, [ 
						'type' => AdapterInterface::INDEX_TYPE_INDEX 
				] );
			}
			$table->addColumn ( $name, $type, $size );
		}
		$table->addIndex ( $this->resource->getIdxName ( $tableName, $searchableFields, AdapterInterface::INDEX_TYPE_FULLTEXT ), $searchableFields, [ 
