...
class GridStructureTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->flatScopeResolver = $this->getMockBuilder ( 'Magento\Framework\Indexer\ScopeResolver\FlatScopeResolver' )->disableOriginalConstructor ()->getMock ();
		$this->resource->expects ( $this->any () )->method ( 'getConnection' )->with ( 'write' )->willReturn ( $this->connection );
		$this->object = new GridStructure ( $this->resource, $this->flatScopeResolver );
	}
	public function testDelete() {
		$index = 'index';
		$table = 'index_table';
		
		$this->flatScopeResolver->expects ( $this->once () )->method ( 'resolve' )->with ( $index, [ ] )->willReturn ( $table );
		$this->connection->expects ( $this->once () )->method ( 'isTableExists' )->with ( $table )->willReturn ( true );
		$this->connection->expects ( $this->once () )->method ( 'dropTable' )->with ( $table );
	...
	public function testCreate() {
		...
		$table->expects ( $this->any () )->method ( 'addColumn' )->willReturnMap ( [ 
				[ 
						'entity_id',
						Table::TYPE_INTEGER,
						10,
		...
		$this->resource->expects ( $this->once () )->method ( 'getIdxName' )->with ( $tableName, [ 
				'field' 
		], AdapterInterface::INDEX_TYPE_FULLTEXT )->willReturn ( $idxName );
		$table->expects ( $this->once () )->method ( 'addIndex' )->with ( $idxName, [ 
				'field' 
		], [ 
