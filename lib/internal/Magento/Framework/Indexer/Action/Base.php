...
class Base implements ActionInterface {
	 ...
	 * @param array $data        	
	 */
	public function __construct(AppResource $resource, SourceFactory $sourceFactory, HandlerPool $handlerPool, SaveHandlerFactory $saveHandlerFactory, FieldsetPool $fieldsetPool, StdString $string, IndexStructureInterface $indexStructure, $data = []) {
	...
	protected function prepareDataSource(array $ids = []) {
		return ! count ( $ids ) ? $this->createResultCollection () : $this->createResultCollection ()->addFieldToFilter ( $this->getPrimaryResource ()->getIdFieldname (), $ids );
	}
	...
	protected function getSaveHandler() {
		if ($this->saveHandler === null) {
			$this->saveHandler = $this->saveHandlerFactory->create ( $this->data ['saveHandler'], [ 
		...
		foreach ( $this->data ['fieldsets'] as $fieldset ) {
			if (isset ( $fieldset ['references'] )) {
				foreach ( $fieldset ['references'] as $reference ) {
					...
					$alias = $this->getPrimaryFieldset () ['name'] == $reference ['fieldset'] ? $this->tableAlias : $reference ['fieldset'];
					$select->joinLeft ( [ 
							$fieldset ['name'] => $currentEntityName 
					], new \Zend_Db_Expr ( $fieldset ['name'] . '.' . $reference ['from'] . '=' . $alias . '.' . $reference ['to'] ), null );
				}
			}
			foreach ( $fieldset ['fields'] as $field ) {
				$handler = $field ['handler'];
				/** @var HandlerInterface $handler */
				$handler->prepareSql ( $this->getPrimaryResource (), $this->getPrimaryFieldset () ['name'] == $fieldset ['name'] ? $this->tableAlias : $fieldset ['name'], $field );
			}
		}
