...
class ConsoleLoggerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$outputFormatter = $this->getMock ( 'Symfony\Component\Console\Formatter\OutputFormatterInterface', [ ], [ ], '', false );
		$this->console->expects ( $this->once () )->method ( 'getFormatter' )->willReturn ( $outputFormatter );
		$this->consoleLoggerModel = new ConsoleLogger ( $this->console );
	}
	public function testLogSuccess() {
		$this->console->expects ( $this->once () )->method ( 'writeln' )->with ( '<info>[SUCCESS]: Success message.</info>' );
		$this->consoleLoggerModel->logSuccess ( 'Success message.' );
	}
	public function testLogError() {
		$exception = $this->getMock ( '\Exception', [ ], [ ], '', false );
		$this->console->expects ( $this->once () )->method ( 'writeln' )->with ( '<error>[ERROR]: </error>' );
	...
	public function testLogInline() {
		$this->console->expects ( $this->once () )->method ( 'write' )->with ( '<detail>Detail message.</detail>' );
