...
class Migration {
	...
	/**
	 * Rows per page.
	 * To split processing data from tables
	 *
	 * @var int
	 */
	 ...
	 *        	name of table to replace aliases in
	 * @param string $fieldName
	 *        	name of table column to replace aliases in
	 * @param string $entityType
	 *        	entity type of alias
	 * @param string $fieldContentType
	 *        	type of field content where class alias is used
	 * @param array $primaryKeyFields
	 *        	row pk field(s) to update by
	 ...
	 * @param string $tableName
	 *        	name of table to replace aliases in
	 * @param array $tableRules
	 *        	replacing rules for table
	 * @return void
	 */
	protected function _updateClassAliasesInTable($tableName, array $tableRules) {
		foreach ( $tableRules as $fieldName => $fieldRule ) {
			$pagesCount = ceil ( $this->_getRowsCount ( $tableName, $fieldName, $fieldRule ['additional_where'] ) / $this->_rowsPerPage );
	...
	protected function _getRowsCount($tableName, $fieldName, $additionalWhere = '') {
		...
		$query = $connection->select ()->from ( $this->setup->getTable ( $tableName ), [ 
	...
	protected function _applyFieldRule($tableName, $fieldName, array $fieldRule, $currentPage = 0) {
		...
		$tableData = $this->_getTableData ( $tableName, $fieldName, $fieldsToSelect, $fieldRule ['additional_where'], $currentPage );
		
		$fieldReplacements = [ ];
		foreach ( $tableData as $rowData ) {
			$replacement = $this->_getReplacement ( $rowData [$fieldName], $fieldRule ['content_type'], $fieldRule ['entity_type'] );
	...
	protected function _updateRowsData($tableName, $fieldName, array $fieldReplacements) {
		if (count ( $fieldReplacements ) > 0) {
			...
			foreach ( $fieldReplacements as $fieldReplacement ) {
				...
				$connection->update ( $this->setup->getTable ( $tableName ), [ 
	...
	protected function _getTableData($tableName, $fieldName, array $fieldsToSelect, $additionalWhere = '', $currPage = 0) {
		...
		$query = $connection->select ()->from ( $this->setup->getTable ( $tableName ), $fieldsToSelect )->where ( $fieldName . ' IS NOT NULL' );
