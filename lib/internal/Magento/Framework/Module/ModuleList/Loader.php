...
class Loader {
	 ...
	 * Excludes modules specified in $exclude.
	 *
	 * @param array $exclude        	
	...
	public function load(array $exclude = []) {
		...
		foreach ( $this->getModuleConfigs () as list ( $file, $contents ) ) {
			try {
				...
				throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Invalid Document: %1%2 Error: %3', [ 
						$file,
						PHP_EOL,
						$e->getMessage () 
				] ), $e );
	...
	private function getModuleConfigs() {
		...
		foreach ( $modulePaths as $modulePath ) {
			$filePath = str_replace ( [ 
					'\\',
					'/' 
			], DIRECTORY_SEPARATOR, "$modulePath/etc/module.xml" );
			yield [ 
					$filePath,
