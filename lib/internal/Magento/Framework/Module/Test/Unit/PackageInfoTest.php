...
class PackageInfoTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->componentRegistrar->expects ( $this->once () )->method ( 'getPaths' )->will ( $this->returnValue ( [ 
				'A' => 'A',
				'B' => 'B',
				'C' => 'C',
				'D' => 'D',
	...
	public function testGetRequireReturnModuleName() {
		$this->assertEquals ( [ 
				'B' 
		], $this->packageInfo->getRequire ( 'A' ) );
		$this->assertEquals ( [ 
				'D' 
		], $this->packageInfo->getRequire ( 'B' ) );
		$this->assertEquals ( [ 
				'E' 
		], $this->packageInfo->getRequire ( 'C' ) );
