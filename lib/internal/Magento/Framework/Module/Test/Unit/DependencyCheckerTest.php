...
class DependencyCheckerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->loaderMock->expects ( $this->any () )->method ( 'load' )->will ( $this->returnValue ( [ 
				'A' => [ ],
				'B' => [ ],
				'C' => [ ],
				'D' => [ ],
	...
	public function testCheckDependenciesWhenDisableModules() {
		$this->listMock->expects ( $this->any () )->method ( 'getNames' )->will ( $this->returnValue ( [ 
				'A',
				'B',
				'C',
				'D',
		...
		$actual = $this->checker->checkDependenciesWhenDisableModules ( [ 
				'B',
				'D' 
		] );
		$expected = [ 
	...
	public function testCheckDependenciesWhenEnableModules() {
		...
		$this->packageInfoMock->expects ( $this->atLeastOnce () )->method ( 'getNonExistingDependencies' )->willReturn ( [ ] );
		$this->checker = new DependencyChecker ( $this->listMock, $this->loaderMock, $this->packageInfoFactoryMock );
		$actual = $this->checker->checkDependenciesWhenEnableModules ( [ 
				'B',
				'D' 
		] );
		$expected = [ 
