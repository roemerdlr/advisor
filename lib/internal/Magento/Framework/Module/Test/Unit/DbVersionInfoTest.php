...
class DbVersionInfoTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->moduleList->expects ( $this->any () )->method ( 'getOne' )->will ( $this->returnValueMap ( [ 
				[ 
						'Module_One',
						[ 
								'name' => 'Module_One',
		...
		$this->moduleList->expects ( $this->any () )->method ( 'getNames' )->will ( $this->returnValue ( [ 
				'Module_One',
				'Module_Two' 
		] ) );
		...
		$this->dbVersionInfo = new DbVersionInfo ( $this->moduleList, $this->moduleResource );
	}
	...
	public function testIsDbSchemaUpToDate($moduleName, $dbVersion, $expectedResult) {
		$this->moduleResource->expects ( $this->once () )->method ( 'getDbVersion' )->with ( $moduleName )->will ( $this->returnValue ( $dbVersion ) );
		$this->assertEquals ( $expectedResult, $this->dbVersionInfo->isSchemaUpToDate ( $moduleName ) );
	}
	...
	public function testIsDbDataUpToDate($moduleName, $dbVersion, $expectedResult) {
		$this->moduleResource->expects ( $this->once () )->method ( 'getDataVersion' )->with ( $moduleName )->will ( $this->returnValue ( $dbVersion ) );
		$this->assertEquals ( $expectedResult, $this->dbVersionInfo->isDataUpToDate ( $moduleName ) );
	}
