...
class MigrationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getModelDependencies($tableRowsCount = 0, $tableData = [], $aliasesMap = []) {
		...
		$this->_selectMock->expects ( $this->any () )->method ( 'where' )->will ( $this->returnCallback ( [ 
				$this,
				'whereCallback' 
		] ) );
		...
		$connectionMock->expects ( $this->any () )->method ( 'update' )->will ( $this->returnCallback ( [ 
				$this,
				'updateCallback' 
		] ) );
	...
	public function whereCallback($condition) {
		...
		if (! empty ( $condition ) && false === strpos ( $condition, ' IS NOT NULL' ) && ! in_array ( $condition, $this->_actualWhere )) {
	...
	public function testAppendClassAliasReplace() {
		...
		$setupModel->appendClassAliasReplace ( 'tableName', 'fieldName', 'newEntityType', 'newFieldContentType', [ 
				'new_pk_field1',
				'new_pk_field2' 
		], 'newAdditionalWhere' );
		$this->assertAttributeEquals ( $expectedRulesList, '_replaceRules', $setupModel );
