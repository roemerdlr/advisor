...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_protFactoryMock = $this->getMock ( 'Magento\Framework\App\Config\BaseFactory', [ ], [ ], '', false, false );
		$this->_dirsMock = $this->getMock ( 'Magento\Framework\Module\Dir', [ ], [ ], '', false, false );
		$this->_baseConfigMock = $this->getMock ( 'Magento\Framework\App\Config\Base', [ ], [ ], '', false, false );
		$this->_moduleListMock = $this->getMock ( 'Magento\Framework\Module\ModuleListInterface' );
		$this->directoryReadFactoryMock = $this->getMock ( '\Magento\Framework\Filesystem\Directory\ReadFactory', [ ], [ ], '', false, false );
		$this->_fileIteratorFactory = $this->getMock ( '\Magento\Framework\Config\FileIteratorFactory', [ ], [ ], '', false, false );
		
		$this->_model = new \Magento\Framework\Module\Dir\Reader ( $this->_dirsMock, $this->_moduleListMock, $this->_fileIteratorFactory, $this->directoryReadFactoryMock );
	}
	public function testGetModuleDirWhenCustomDirIsNotSet() {
		$this->_dirsMock->expects ( $this->any () )->method ( 'getDir' )->with ( 'Test_Module', 'etc' )->will ( $this->returnValue ( 'app/code/Test/Module/etc' ) );
		$this->assertEquals ( 'app/code/Test/Module/etc', $this->_model->getModuleDir ( Dir::MODULE_ETC_DIR, 'Test_Module' ) );
	}
	...
	public function testGetConfigurationFiles() {
		...
		$this->_moduleListMock->expects ( $this->once () )->method ( 'getNames' )->will ( $this->returnValue ( [ 
				'Test_Module' 
		] ) );
