...
class ManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_moduleList->expects ( $this->any () )->method ( 'getOne' )->will ( $this->returnValueMap ( [ 
				[ 
						'Module_One',
						[ 
								'name' => 'One_Module',
		...
		$this->_model = new \Magento\Framework\Module\Manager ( $this->_outputConfig, $this->_moduleList, [ 
	...
	public function testIsOutputEnabledGenericConfigPath($configValue, $expectedResult) {
		...
		$this->_outputConfig->expects ( $this->once () )->method ( 'isEnabled' )->with ( 'Module_One' )->will ( $this->returnValue ( $configValue ) );
	...
	public function testIsOutputEnabledCustomConfigPath($configValue, $expectedResult) {
		$this->_moduleList->expects ( $this->once () )->method ( 'has' )->will ( $this->returnValue ( true ) );
		$this->_outputConfig->expects ( $this->at ( 0 ) )->method ( 'isSetFlag' )->with ( self::XML_PATH_OUTPUT_ENABLED )->will ( $this->returnValue ( $configValue ) );
	...
	public function isOutputEnabledCustomConfigPathDataProvider() {
		return [ 
				'path literal, output disabled' => [ 
						false,
						false 
				],
				'path literal, output enabled' => [ 
						true,
						true 
				] 
		];
