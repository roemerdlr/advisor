...
class RendererPool extends AbstractBlock {
	...
	protected function getAmountRenderBlockTemplate($type, $priceCode) {
		...
		if (! $template) {
			throw new \InvalidArgumentException ( 'For type "' . $type . '" amount render block not configured' );
		}
		return $template;
	}
	...
	protected function getRenderBlockTemplate($type, $priceCode) {
		...
		$template = $this->findDataByPattern ( $pattern );
		if (! $template) {
			throw new \InvalidArgumentException ( 'Price code "' . $priceCode . '" render block not configured' );
		}
		return $template;
	}
