...
class CalculatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->amountFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Pricing\Amount\AmountFactory' )->disableOriginalConstructor ()->getMock ();
	...
	public function testGetAmount() {
		...
		$this->amountFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( $this->equalTo ( $totalAmount ), $this->equalTo ( $expectedAdjustments ) )->will ( $this->returnValue ( $amountBaseMock ) );
		...
		$weeeAdjustmentMock->expects ( $this->once () )->method ( 'isIncludedInDisplayPrice' )->will ( $this->returnValue ( true ) );
		$weeeAdjustmentMock->expects ( $this->once () )->method ( 'applyAdjustment' )->with ( $this->equalTo ( $amountInclTax ), $this->equalTo ( $productMock ) )->will ( $this->returnValue ( $weeeAdjustment + $amountInclTax ) );
		...
		$taxAdjustmentMock->expects ( $this->once () )->method ( 'extractAdjustment' )->with ( $this->equalTo ( $amountInclTax ), $this->equalTo ( $productMock ) )->will ( $this->returnValue ( $taxAdjustment ) );
		$taxAdjustmentMock->expects ( $this->once () )->method ( 'applyAdjustment' )->with ( $this->equalTo ( $totalAmount ), $this->equalTo ( $productMock ) )->will ( $this->returnValue ( $totalAmount ) );
		$taxAdjustmentMock->expects ( $this->never () )->method ( 'isIncludedInDisplayPrice' );
