...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$adj1->expects ( $this->any () )->method ( 'getSortOrder' )->will ( $this->returnValue ( 10 ) );
		$adj2 = $this->getMock ( 'Magento\Framework\Pricing\Adjustment\AdjustmentInterface' );
		$adj2->expects ( $this->any () )->method ( 'getSortOrder' )->will ( $this->returnValue ( 20 ) );
		$adj3 = $this->getMock ( 'Magento\Framework\Pricing\Adjustment\AdjustmentInterface' );
		$adj3->expects ( $this->any () )->method ( 'getSortOrder' )->will ( $this->returnValue ( 5 ) );
		$adj4 = $this->getMock ( 'Magento\Framework\Pricing\Adjustment\AdjustmentInterface' );
		$adj4->expects ( $this->any () )->method ( 'getSortOrder' )->will ( $this->returnValue ( \Magento\Framework\Pricing\Adjustment\Pool::DEFAULT_SORT_ORDER ) );
		...
		$adjustmentPool = $this->getMockBuilder ( 'Magento\Framework\Pricing\Adjustment\Pool' )->disableOriginalConstructor ()->setMethods ( [ 
				'getAdjustmentByCode' 
		] )->getMock ();
		$adjustmentPool->expects ( $this->any () )->method ( 'getAdjustmentByCode' )->will ( $this->returnCallback ( function ($code) use($adjustmentsData) {
