...
class AmountTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->rendererPool = $this->getMock ( 'Magento\Framework\Pricing\Render\RendererPool', [ ], [ 
				'data' => $data 
		], '', false, false );
		...
		$this->model = $objectManager->getObject ( 'Magento\Framework\Pricing\Render\Amount', [ 
				'context' => $context,
	...
	public function testToHtmlGetAdjustmentRenders() {
		...
		$this->rendererPool->expects ( $this->once () )->method ( 'getAdjustmentRenders' )->will ( $this->returnValue ( $adjustmentRenders ) );
		$this->amount->expects ( $this->atLeastOnce () )->method ( 'getAdjustmentAmount' )->willReturn ( true );
	...
	public function testGetDisplayValue() {
		$amountValue = 100.99;
		$this->amount->expects ( $this->once () )->method ( 'getValue' )->will ( $this->returnValue ( $amountValue ) );
	...
	protected function getAdjustmentRenderMock($data = [], $html = '', $code = 'adjustment_code') {
		...
		$adjustmentRender->expects ( $this->once () )->method ( 'render' )->with ( $this->model, $data )->will ( $this->returnValue ( $html ) );
		$adjustmentRender->expects ( $this->any () )->method ( 'getAdjustmentCode' )->will ( $this->returnValue ( $code ) );
