...
class RendererPoolTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->contextMock = $this->getMockBuilder ( '\Magento\Framework\View\Element\Context' )->disableOriginalConstructor ()->getMock ();
	...
	public function testCreatePriceRenderNoClassName() {
		...
		$data = [ ];
		$type = 'simple';
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( $type ) );
	...
	public function testCreatePriceRenderNoPriceModel() {
		...
		$priceInfoMock->expects ( $this->once () )->method ( 'getPrice' )->with ( $this->equalTo ( $priceCode ) )->will ( $this->returnValue ( $priceModel ) );
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( $type ) );
		$this->productMock->expects ( $this->once () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $priceInfoMock ) );
	...
	public function testCreatePriceRenderBlockNotPriceBox() {
		...
		$priceCode = 'price_test';
		$type = 'simple';
		$className = 'Magento\Framework\View\Element\Template\Context';
		$data = [ 
				$type => [ 
						'prices' => [ 
								$priceCode => [ 
		...
		$priceInfoMock = $this->getMockBuilder ( 'Magento\Framework\Pricing\PriceInfo\Base' )->disableOriginalConstructor ()->getMock ();
		$priceInfoMock->expects ( $this->once () )->method ( 'getPrice' )->with ( $this->equalTo ( $priceCode ) )->will ( $this->returnValue ( $this->priceMock ) );
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( $type ) );
		$this->productMock->expects ( $this->once () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $priceInfoMock ) );
		
		$contextMock = $this->getMockBuilder ( 'Magento\Framework\View\Element\Template\Context' )->disableOriginalConstructor ()->getMock ();
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( $this->equalTo ( $className ), $this->equalTo ( '' ), $this->equalTo ( $arguments ) )->will ( $this->returnValue ( $block ) );
	...
	public function testCreatePriceRender() {
		...
		$renderBlock = $this->getMockBuilder ( 'Magento\Framework\Pricing\Render\PriceBox' )->disableOriginalConstructor ()->getMock ();
		$renderBlock->expects ( $this->once () )->method ( 'setTemplate' )->with ( $this->equalTo ( $template ) );
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( $this->equalTo ( $className ), $this->equalTo ( '' ), $this->equalTo ( $arguments ) )->will ( $this->returnValue ( $renderBlock ) );
		
		$result = $testedClass->createPriceRender ( $priceCode, $this->productMock, $methodData );
		$this->assertInstanceOf ( 'Magento\Framework\Pricing\Render\PriceBoxRenderInterface', $result );
	}
	...
	public function testCreateAmountRenderNoAmountClass() {
		...
		$amountMock = $this->getMockBuilder ( 'Magento\Framework\Pricing\Amount\Base' )->disableOriginalConstructor ()->getMock ();
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( $type ) );
		$this->priceMock->expects ( $this->once () )->method ( 'getPriceCode' )->will ( $this->returnValue ( $priceCode ) );
	...
	public function testCreateAmountRenderNotAmountInterface() {
		...
		$methodData = [ ];
		$priceCode = 'base_price_test';
		$amountRenderClass = 'Magento\Framework\View\Element\Template\Context';
		$data = [ 
				$type => [ 
						'prices' => [ 
								$priceCode => [ 
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( $this->equalTo ( $amountRenderClass ), $this->equalTo ( '' ), $this->equalTo ( $arguments ) )->will ( $this->returnValue ( $block ) );
	...
	public function testCreateAmountRender() {
		...
		$priceCode = 'base_price_test';
		$template = 'template.phtml';
		$amountRenderClass = 'Magento\Framework\Pricing\Render\Amount';
		$data = [ 
				$type => [ 
						'prices' => [ 
								$priceCode => [ 
		...
		$blockMock = $this->getMockBuilder ( 'Magento\Framework\Pricing\Render\Amount' )->disableOriginalConstructor ()->getMock ();
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( $this->equalTo ( $amountRenderClass ), $this->equalTo ( '' ), $this->equalTo ( $arguments ) )->will ( $this->returnValue ( $blockMock ) );
		
		$blockMock->expects ( $this->once () )->method ( 'setTemplate' )->with ( $this->equalTo ( $template ) );
		
		$result = $testedClass->createAmountRender ( $amountMock, $this->productMock, $this->priceMock, $methodData );
		$this->assertInstanceOf ( 'Magento\Framework\Pricing\Render\AmountRenderInterface', $result );
	}
	...
	public function testGetAdjustmentRendersNoRenderClass() {
		$typeId = 'simple';
		$priceCode = 'base_price_test';
		$this->productMock->expects ( $this->once () )->method ( 'getTypeId' )->will ( $this->returnValue ( $typeId ) );
		$this->priceMock->expects ( $this->once () )->method ( 'getPriceCode' )->will ( $this->returnValue ( $priceCode ) );
	...
	public function testGetAdjustmentRenders() {
		...
		$this->layoutMock->expects ( $this->once () )->method ( 'createBlock' )->with ( $this->equalTo ( $class ) )->will ( $this->returnValue ( $blockMock ) );
		
		$testedClass = $this->createTestedEntity ( $data );
		$result = $testedClass->getAdjustmentRenders ( $this->productMock, $this->priceMock );
		$this->assertArrayHasKey ( $code, $result );
		$this->assertInstanceOf ( 'Magento\Framework\View\Element\Template', $result [$code] );
	}
