...
class AbstractAdjustmentTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$constructorArgs = $objectManager->getConstructArguments ( 'Magento\Framework\Pricing\Render\AbstractAdjustment', [ 
				'priceCurrency' => $this->priceCurrency,
				'data' => $this->data 
		] );
		$this->model = $this->getMockBuilder ( 'Magento\Framework\Pricing\Render\AbstractAdjustment' )->setConstructorArgs ( $constructorArgs )->setMethods ( [ 
				'getData',
				'setData',
				'apply' 
	...
	public function testConvertAndFormatCurrency() {
		...
		$this->priceCurrency->expects ( $this->once () )->method ( 'convertAndFormat' )->with ( $amount, $includeContainer, $precision )->will ( $this->returnValue ( $result ) );
	...
	public function testRender() {
		...
		$this->model->expects ( $this->at ( 0 ) )->method ( 'getData' )->will ( $this->returnValue ( $this->data ) );
		$this->model->expects ( $this->at ( 1 ) )->method ( 'setData' )->with ( $mergedArguments );
		$this->model->expects ( $this->at ( 2 ) )->method ( 'apply' )->will ( $this->returnValue ( $renderText ) );
		$this->model->expects ( $this->at ( 3 ) )->method ( 'setData' )->with ( $this->data );
	...
	public function testGetPriceType() {
		...
		$amountRender->expects ( $this->once () )->method ( 'getSaleableItem' )->will ( $this->returnValue ( $sealableItem ) );
		$sealableItem->expects ( $this->once () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $priceInfo ) );
		$priceInfo->expects ( $this->once () )->method ( 'getPrice' )->with ( $priceCode )->will ( $this->returnValue ( $price ) );
	...
	public function testGetPrice() {
		$price = 100;
		$amountRender = $this->getMock ( 'Magento\Framework\Pricing\Render\Amount', [ ], [ ], '', false );
		$amountRender->expects ( $this->once () )->method ( 'getPrice' )->with ()->will ( $this->returnValue ( $price ) );
	...
	public function testGetSealableItem() {
		$sealableItem = $this->getMockForAbstractClass ( 'Magento\Framework\Pricing\SaleableInterface' );
		$amountRender = $this->getMock ( 'Magento\Framework\Pricing\Render\Amount', [ ], [ ], '', false );
		$amountRender->expects ( $this->once () )->method ( 'getSaleableItem' )->with ()->will ( $this->returnValue ( $sealableItem ) );
	...
	public function testGetAdjustment() {
		...
		$amountRender->expects ( $this->once () )->method ( 'getSaleableItem' )->will ( $this->returnValue ( $sealableItem ) );
		$sealableItem->expects ( $this->once () )->method ( 'getPriceInfo' )->will ( $this->returnValue ( $priceInfo ) );
		$priceInfo->expects ( $this->once () )->method ( 'getAdjustment' )->with ( $adjustmentCode )->will ( $this->returnValue ( $adjustment ) );
		
		$this->model->expects ( $this->at ( 0 ) )->method ( 'getData' )->will ( $this->returnValue ( $this->data ) );
		$this->model->expects ( $this->once () )->method ( 'getAdjustmentCode' )->will ( $this->returnValue ( $adjustmentCode ) );
		$this->model->render ( $amountRender );
	...
	public function testFormatCurrency() {
		...
		$this->priceCurrency->expects ( $this->once () )->method ( 'format' )->with ( $amount, $includeContainer, $precision )->will ( $this->returnValue ( $expected ) );
