...
class SubscriptionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->triggerFactoryMock = $this->getMock ( 'Magento\Framework\DB\Ddl\TriggerFactory', [ ], [ ], '', false, false );
		$this->viewCollectionMock = $this->getMockForAbstractClass ( 'Magento\Framework\Mview\View\CollectionInterface', [ ], '', false, false, true, [ ] );
		$this->viewMock = $this->getMockForAbstractClass ( 'Magento\Framework\Mview\ViewInterface', [ ], '', false, false, true, [ ] );
		
		$this->resourceMock->expects ( $this->any () )->method ( 'getTableName' )->willReturn ( $this->tableName );
		
		$this->model = new Subscription ( $this->resourceMock, $this->triggerFactoryMock, $this->viewCollectionMock, $this->viewMock, $this->tableName, 'columnName' );
	}
	...
	public function testCreate() {
		...
		$otherViewMock->expects ( $this->exactly ( 1 ) )->method ( 'getSubscriptions' )->will ( $this->returnValue ( [ 
				[ 
						'name' => $this->tableName 
				],
				[ 
		...
		$this->viewCollectionMock->expects ( $this->exactly ( 1 ) )->method ( 'getViewsByStateMode' )->with ( \Magento\Framework\Mview\View\StateInterface::MODE_ENABLED )->will ( $this->returnValue ( [ 
				$this->viewMock,
				$otherViewMock 
		] ) );
