...
class SubscriptionFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreate() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\Mview\View\SubscriptionInterface', [ 
				'some_data' 
		] )->will ( $this->returnValue ( $subscriptionInterfaceMock ) );
		$this->assertEquals ( $subscriptionInterfaceMock, $this->model->create ( [ 
				'some_data' 
		] ) );
	}
}
