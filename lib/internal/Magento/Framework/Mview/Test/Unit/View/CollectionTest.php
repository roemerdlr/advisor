...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	public function testLoadDataAndGetViewsByStateMode() {
		...
		$entityFactory = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\EntityFactoryInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$config = $this->getMockBuilder ( 'Magento\Framework\Mview\ConfigInterface' )->getMock ();
		
		$statesFactory = $this->getMockBuilder ( 'Magento\Framework\Mview\View\State\CollectionFactory' )->disableOriginalConstructor ()->setMethods ( [ 
				'create' 
		] )->getMock ();
		
		$states = $this->getMockBuilder ( 'Magento\Framework\Mview\View\State\Collection' )->setMethods ( [ 
				'getItems' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$state = $this->getMockForAbstractClass ( 'Magento\Framework\Mview\View\StateInterface', [ ], '', false, false, true, [ 
				'getViewId',
				'getMode',
				'__wakeup' 
		] );
		...
		$state->expects ( $this->any () )->method ( 'getMode' )->will ( $this->returnValue ( \Magento\Framework\Mview\View\StateInterface::MODE_DISABLED ) );
		
		$view = $this->getMockForAbstractClass ( 'Magento\Framework\Mview\ViewInterface', [ ], '', false, false, true, [ 
				'load',
				'setState',
				'getState',
				'__wakeup' 
		...
		$entityFactory->expects ( $this->any () )->method ( 'create' )->with ( 'Magento\Framework\Mview\ViewInterface' )->will ( $this->returnValue ( $view ) );
		
		$statesFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $states ) );
		
		$config->expects ( $this->once () )->method ( 'getViews' )->will ( $this->returnValue ( [ 
				$indexerIdOne => 1,
				$indexerIdSecond => 2 
		] ) );
		
		$states->expects ( $this->any () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				$state 
		] ) );
