...
class ProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->viewsFactoryMock = $this->getMock ( 'Magento\Framework\Mview\View\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
	...
	public function testClearChangelog() {
		$viewsMock = $this->getViewsMock ();
		$viewsMock->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( $this->getViews ( 'clearChangelog' ) ) );
		$viewsMock->expects ( $this->never () )->method ( 'getItemsByColumnValue' );
