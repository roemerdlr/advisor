...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->reader = $this->getMock ( 'Magento\Framework\Mview\Config\Reader', [ 
				'read' 
		], [ ], '', false );
		$this->cache = $this->getMockForAbstractClass ( 'Magento\Framework\Config\CacheInterface', [ ], '', false, false, true, [ 
				'test',
				'load',
				'save' 
		] );
		$this->stateCollection = $this->getMockForAbstractClass ( 'Magento\Framework\Mview\View\State\CollectionInterface', [ ], '', false, false, true, [ 
				'getItems' 
		] );
	}
	public function testConstructorWithCache() {
		$this->cache->expects ( $this->once () )->method ( 'test' )->with ( $this->cacheId )->will ( $this->returnValue ( true ) );
		$this->cache->expects ( $this->once () )->method ( 'load' )->with ( $this->cacheId )->will ( $this->returnValue ( serialize ( $this->views ) ) );
	...
	public function testConstructorWithoutCache() {
		...
		$stateExistent = $this->getMock ( 'Magento\Framework\Mview\Indexer\State', [ 
				'getViewId',
				'__wakeup',
				'delete' 
		], [ ], '', false );
		...
		$stateNonexistent = $this->getMock ( 'Magento\Framework\Mview\Indexer\State', [ 
				'getViewId',
				'__wakeup',
				'delete' 
		], [ ], '', false );
