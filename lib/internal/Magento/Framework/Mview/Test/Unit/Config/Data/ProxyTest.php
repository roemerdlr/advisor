...
class ProxyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->dataMock = $this->getMock ( 'Magento\Framework\Mview\Config\Data', [ ], [ ], '', false );
	}
	public function testMergeShared() {
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Framework\Mview\Config\Data' )->will ( $this->returnValue ( $this->dataMock ) );
		$this->dataMock->expects ( $this->once () )->method ( 'merge' )->with ( [ 
				'some_config' 
		] );
		
		$this->model = new Proxy ( $this->objectManagerMock, 'Magento\Framework\Mview\Config\Data', true );
