...
class Changelog implements ChangelogInterface {
	...
	public function create() {
		...
		$table = $this->connection->newTable ( $changelogTableName )->addColumn ( 'version_id', \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER, null, [ 
				'identity' => true,
				'unsigned' => true,
				'nullable' => false,
				'primary' => true 
		], 'Version ID' )->addColumn ( $this->getColumnName (), \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER, null, [ 
				'unsigned' => true,
				'nullable' => false,
				'default' => '0' 
		], 'Entity ID' );
	...
	public function clear($versionId) {
		...
		$this->connection->delete ( $changelogTableName, [ 
	...
	public function getList($fromVersionId, $toVersionId) {
		...
		$select = $this->connection->select ()->distinct ( true )->from ( $changelogTableName, [ 
				$this->getColumnName () 
		] )->where ( 'version_id > ?', ( int ) $fromVersionId )->where ( 'version_id <= ?', ( int ) $toVersionId );
	...
	public function getVersion() {
		...
		$row = $this->connection->fetchRow ( 'SHOW TABLE STATUS LIKE ?', [ 
				$changelogTableName 
		] );
