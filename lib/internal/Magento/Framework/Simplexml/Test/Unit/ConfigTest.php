...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function testValidateCacheChecksum() {
		...
		$this->config->setCacheChecksum ( 'CacheChecksum' );
		$cache = $this->getMock ( 'Magento\Framework\Simplexml\Config\Cache\File', [ 
				'load' 
		] );
		$cache->expects ( $this->once () )->method ( 'load' )->with ( 'cacheId__CHECKSUM' )->will ( $this->returnValue ( '415a5472d4f94b71ff80fd1c8e9eca7f' ) );
	...
	public function testSaveCache() {
		...
		$cache->expects ( $this->at ( 0 ) )->method ( 'save' )->with ( null, 'cacheId__CHECKSUM', [ 
				'cacheTags' 
		], 10 )->will ( $this->returnValue ( true ) );
		$cache->expects ( $this->at ( 1 ) )->method ( 'save' )->with ( $xml, 'cacheId', [ 
				'cacheTags' 
		], 10 )->will ( $this->returnValue ( true ) );
	...
	public function testApplyExtends() {
		...
		$config->applyExtends ();
		$this->assertEquals ( $config->getNode ( 'node_1/node_1_1' )->asArray (), $config->getNode ( 'node_3/node_1_1' )->asArray () );
