...
class ResourceLoaderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPopulateAclOnValidObjects() {
		...
		$acl = $this->getMock ( 'Magento\Framework\Acl', [ 
				'addResource' 
		], [ ], '', false );
		...
		$factoryObject = $this->getMock ( 'Magento\Framework\Acl\AclResourceFactory', [ 
				'createResource' 
		], [ ], '', false );
		...
		$resourceProvider->expects ( $this->once () )->method ( 'getAclResources' )->will ( $this->returnValue ( [ 
				[ 
	...
	public function testPopulateAclWithException() {
		...
		$factoryObject = $this->getMockBuilder ( 'Magento\Framework\Acl\AclResourceFactory' )->setMethods ( [ 
				'createResource' 
