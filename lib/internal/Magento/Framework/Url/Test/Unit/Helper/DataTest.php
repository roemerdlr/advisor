...
class DataTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetCurrentBase64Url() {
		...
		$urlBuilderMock->expects ( $this->once () )->method ( 'getCurrentUrl' )->will ( $this->returnValue ( $url ) );
		$encodedUrl = 'encodedUrl';
		$urlEncoder = $this->getMockBuilder ( 'Magento\Framework\Url\EncoderInterface' )->disableOriginalConstructor ()->getMock ();
		$urlEncoder->expects ( $this->once () )->method ( 'encode' )->will ( $this->returnValue ( $encodedUrl ) );
		$context = $this->objectManager->getObject ( 'Magento\Framework\App\Helper\Context', [ 
	...
	public function testGetEncodedUrl($url, $callNum) {
		...
		$urlBuilderMock->expects ( $this->exactly ( $callNum ) )->method ( 'getCurrentUrl' )->will ( $this->returnValue ( $encodingUrl ) );
		$encodedUrl = 'encodedUrl';
		$urlEncoder = $this->getMockBuilder ( 'Magento\Framework\Url\EncoderInterface' )->disableOriginalConstructor ()->getMock ();
		$urlEncoder->expects ( $this->once () )->method ( 'encode' )->will ( $this->returnValue ( $encodedUrl ) );
		$context = $this->objectManager->getObject ( 'Magento\Framework\App\Helper\Context', [ 
