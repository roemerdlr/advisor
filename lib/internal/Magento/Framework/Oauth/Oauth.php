...
class Oauth implements OauthInterface {
	 ...
	 */
	public function __construct(Helper\Oauth $oauthHelper, NonceGeneratorInterface $nonceGenerator, TokenProviderInterface $tokenProvider, \Zend_Oauth_Http_Utility $httpUtility = null) {
	...
	public function getAccessToken($params, $requestUrl, $httpMethod = 'POST') {
		...
		$consumer = $this->_tokenProvider->getConsumerByKey ( $params ['oauth_consumer_key'] );
		$tokenSecret = $this->_tokenProvider->validateRequestToken ( $params ['oauth_token'], $consumer, $params ['oauth_verifier'] );
	...
	protected function _validateSignature($params, $consumerSecret, $httpMethod, $requestUrl, $tokenSecret = null) {
		if (! in_array ( $params ['oauth_signature_method'], self::getSupportedSignatureMethods () )) {
			throw new OauthInputException ( new Phrase ( 'Signature method %1 is not supported', [ 
	...
	protected function _validateVersionParam($version) {
		...
		if ('1.0' != $version) {
			throw new OauthInputException ( new Phrase ( 'OAuth version %1 is not supported', [ 
					$version 
			] ) );
		}
	}
	...
	protected function _checkRequiredParams($protocolParams, $requiredParams) {
		...
		foreach ( $requiredParams as $param ) {
			if (! isset ( $protocolParams [$param] )) {
				$exception->addError ( new Phrase ( OauthInputException::REQUIRED_FIELD, [ 
						'fieldName' => $param 
				] ) );
			}
		}
