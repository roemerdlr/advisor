...
class InputExceptionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testConstructor() {
		...
		$inputException = new InputException ( new Phrase ( InputException::INVALID_FIELD_MIN_VALUE, $params ) );
		...
		$this->assertEquals ( 'The quantity value of "-100" must be greater than or equal to 0.', $inputException->getLogMessage () );
	}
	...
	public function testAddError() {
		...
		$inputException->addError ( new Phrase ( InputException::INVALID_FIELD_MIN_VALUE, [ 
				'fieldName' => 'weight',
				'value' => - 100,
				'minValue' => 1 
		] ) );
		...
		$this->assertEquals ( InputException::INVALID_FIELD_MIN_VALUE, $inputException->getRawMessage () );
		$this->assertEquals ( 'The weight value of "-100" must be greater than or equal to 1.', $inputException->getMessage () );
		$this->assertEquals ( 'The weight value of "-100" must be greater than or equal to 1.', $inputException->getLogMessage () );
		
		$inputException->addError ( new Phrase ( InputException::REQUIRED_FIELD, [ 
				'fieldName' => 'name' 
		] ) );
		$this->assertTrue ( $inputException->wasErrorAdded () );
		...
		$this->assertEquals ( 'The weight value of "-100" must be greater than or equal to 1.', $errors [0]->getMessage () );
		$this->assertEquals ( 'The weight value of "-100" must be greater than or equal to 1.', $errors [0]->getLogMessage () );
