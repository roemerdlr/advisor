...
class ClassLoaderWrapperTest extends \PHPUnit_Framework_TestCase {
	...
	public function testAdd() {
		...
		$this->autoloaderMock->expects ( $this->once () )->method ( 'add' )->with ( self::PREFIX, self::DIR, $prepend );
	...
	public function testAddPsr4() {
		...
		$this->autoloaderMock->expects ( $this->once () )->method ( 'addPsr4' )->with ( self::PREFIX, self::DIR, $prepend );
		
		$this->model->addPsr4 ( self::PREFIX, self::DIR, $prepend );
	}
	public function testAddDefault() {
		$this->autoloaderMock->expects ( $this->once () )->method ( 'add' )->with ( self::PREFIX, self::DIR, self::DEFAULT_PREPEND );
		
		$this->model->addPsr0 ( self::PREFIX, self::DIR );
	}
	public function testAddPsr4Default() {
		$this->autoloaderMock->expects ( $this->once () )->method ( 'addPsr4' )->with ( self::PREFIX, self::DIR, self::DEFAULT_PREPEND );
	...
	public function testSet() {
		...
		$this->autoloaderMock->expects ( $this->once () )->method ( 'set' )->with ( self::PREFIX, $paths );
	...
	public function testSetPsr4() {
		...
		$this->autoloaderMock->expects ( $this->once () )->method ( 'setPsr4' )->with ( self::PREFIX, $paths );
