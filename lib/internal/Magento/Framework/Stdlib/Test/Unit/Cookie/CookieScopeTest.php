...
class CookieScopeTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->requestMock->expects ( $this->any () )->method ( 'isSecure' )->willReturn ( true );
		$this->objectManager = new ObjectManager ( $this );
		$cookieMetadataFactory = $this->getMockBuilder ( 'Magento\Framework\Stdlib\Cookie\CookieMetadataFactory' )->disableOriginalConstructor ()->getMock ();
		$cookieMetadataFactory->expects ( $this->any () )->method ( 'createSensitiveCookieMetadata' )->will ( $this->returnCallback ( [ 
				$this,
				'createSensitiveMetadata' 
		] ) );
		$cookieMetadataFactory->expects ( $this->any () )->method ( 'createPublicCookieMetadata' )->will ( $this->returnCallback ( [ 
				$this,
				'createPublicMetadata' 
		] ) );
		$cookieMetadataFactory->expects ( $this->any () )->method ( 'createCookieMetadata' )->will ( $this->returnCallback ( [ 
				$this,
	...
	public function createSensitiveMetadata($metadata = []) {
		return $this->objectManager->getObject ( 'Magento\Framework\Stdlib\Cookie\SensitiveCookieMetadata', [ 
				'metadata' => $metadata,
	...
	public function createPublicMetadata($metadata = []) {
		return $this->objectManager->getObject ( 'Magento\Framework\Stdlib\Cookie\PublicCookieMetadata', [ 
				'metadata' => $metadata 
		] );
	}
	...
	public function createCookieMetadata($metadata = []) {
		return $this->objectManager->getObject ( 'Magento\Framework\Stdlib\Cookie\CookieMetadata', [ 
				'metadata' => $metadata 
		] );
	}
}
