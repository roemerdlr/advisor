...
class Validator {
	 ...
	 */
	public function __construct($yearMinValue = \Magento\Framework\Stdlib\DateTime::YEAR_MIN_VALUE, $yearMaxValue = \Magento\Framework\Stdlib\DateTime::YEAR_MAX_VALUE) {
	...
	public function validate($timestamp, $toDate) {
		...
		if ($transitionYear > $this->_yearMaxValue || $transitionYear < $this->_yearMinValue) {
			throw new ValidatorException ( new Phrase ( 'Transition year is out of system date range.' ) );
		}
		
		if (( int ) $timestamp > ( int ) $toDate) {
			throw new ValidatorException ( new Phrase ( 'Transition year is out of specified date range.' ) );
		}
	}
}
