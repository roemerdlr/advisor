...
class ScopePool {
	 ...
	 */
	public function __construct(\Magento\Framework\App\Config\Scope\ReaderPoolInterface $readerPool, DataFactory $dataFactory, \Magento\Framework\Cache\FrontendInterface $cache, \Magento\Framework\App\ScopeResolverPool $scopeResolverPool, $cacheId = 'default_config_cache') {
	...
	public function getScope($scopeType, $scopeCode = null) {
		...
		if (! isset ( $this->_scopes [$code] )) {
			...
			$this->_scopes [$code] = $this->_dataFactory->create ( [ 
					'data' => $data 
			] );
		}
