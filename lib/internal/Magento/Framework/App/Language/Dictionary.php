...
class Dictionary {
	 ...
	 */
	public function __construct(ReadFactory $directoryReadFactory, ComponentRegistrar $componentRegistrar, ConfigFactory $configFactory) {
	...
	public function getDictionary($languageCode) {
		...
		foreach ( $this->paths as $path ) {
			...
			if ($directoryRead->isExist ( 'language.xml' )) {
				...
				$languageConfig = $this->configFactory->create ( [ 
						'source' => $xmlSource 
				] );
	...
	private function collectInheritedPacks($languageConfig, &$result, $level = 0, array &$visitedPacks = []) {
		$packKey = implode ( '|', [ 
				$languageConfig->getVendor (),
				$languageConfig->getPackage () 
		] );
	 ...
	 * @param array $current        	
	 * @param array $next        	
	 * @return int @SuppressWarnings(PHPMD.UnusedPrivateMethod)
	 */
