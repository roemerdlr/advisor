...
class DictionaryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->configFactory = $this->getMockBuilder ( '\Magento\Framework\App\Language\ConfigFactory' )->setMethods ( [ 
				'create' 
	...
	public function testDictionaryGetter() {
		...
		$readMock->expects ( $this->any () )->method ( 'search' )->willReturn ( [ 
				$csvFileName 
		] );
		
		$this->componentRegistrar->expects ( $this->once () )->method ( 'getPaths' )->willReturn ( [ 
				'foo/en_us' 
		] );
