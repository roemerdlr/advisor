...
class HttpTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_routerListMock = $this->getMock ( 'Magento\Framework\App\Route\ConfigInterface\Proxy', [ 
				'getRouteFrontName',
				'getRouteByFrontName',
				'__wakeup' 
		], [ ], '', false );
		...
		$this->converterMock = $this->getMockBuilder ( 'Magento\Framework\Stdlib\StringUtils' )->disableOriginalConstructor ()->setMethods ( [ 
				'cleanString' 
		] )->getMock ();
	...
	private function getModel($uri = null) {
		...
		return $testFrameworkObjectManager->getObject ( 'Magento\Framework\App\Request\Http', [ 
	...
	public function testGetDistroBaseUrlPath($scriptName, $expected) {
		$this->assertEquals ( $expected, Http::getDistroBaseUrlPath ( [ 
	...
	public function serverVariablesProvider() {
		...
		$returnValue ['no SCRIPT_NAME'] = [ 
				$noScriptNameData,
				'http://localhost/' 
		];
		
		unset ( $noHostData ['HTTP_HOST'] );
		$returnValue ['no HTTP_HOST'] = [ 
				$noHostData,
				'http://localhost/' 
		];
		
		$httpsOffData ['HTTPS'] = 'off';
		$returnValue ['HTTPS off'] = [ 
				$httpsOffData,
				'http://sample.host.com/' 
		];
		
		unset ( $noHttpsData ['HTTPS'] );
		$returnValue ['no HTTPS'] = [ 
				$noHttpsData,
				'http://sample.host.com/' 
		];
	 ...
	 * @param string $serverHttps
	 *        	value of $_SERVER['HTTPS']
	 * @param string $headerOffloadKey
	 *        	<Name-Of-Offload-Header>
	 * @param string $headerOffloadValue
	 *        	value of $_SERVER[<Name-Of-Offload-Header>]
	 * @param int $configCall
	...
	public function testIsSecure($isSecure, $serverHttps, $headerOffloadKey, $headerOffloadValue, $configCall) {
		...
		$configMock = $this->getMockBuilder ( 'Magento\Framework\App\Config' )->disableOriginalConstructor ()->setMethods ( [ 
				'getValue' 
		] )->getMock ();
	 ...
	 * 
	 * @param string $method
	 *        	value of $_SERVER['REQUEST_METHOD']
	 */
	...
	public function isSecureDataProvider() {
		...
		return [ 
				...
				'blank HTTPS with proxy set https' => [ 
						true,
						'',
						'Header-From-Proxy',
						'https',
				...
				'blank HTTPS with proxy set http' => [ 
						false,
						'',
						'Header-From-Proxy',
						'http',
				...
				'HTTPS off with HTTP_ prefixed proxy set to https' => [ 
						true,
						'off',
						'HTTP_Header-From-Proxy',
						'https',
						1 
				] 
		];
