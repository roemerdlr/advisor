...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->fileDriver->expects ( $this->any () )->method ( 'isExists' )->will ( $this->returnValueMap ( [ 
				[ 
						__DIR__ . '/_files/config.php',
						true 
				],
				[ 
						__DIR__ . '/_files/custom.php',
						true 
				],
				[ 
						__DIR__ . '/_files/duplicateConfig.php',
						true 
				],
				[ 
						__DIR__ . '/_files/env.php',
						true 
				],
				[ 
						__DIR__ . '/_files/mergeOne.php',
						true 
				],
				[ 
						__DIR__ . '/_files/mergeTwo.php',
						true 
				],
				[ 
						__DIR__ . '/_files/nonexistent.php',
						false 
				] 
		] ) );
		...
		$this->configFilePool->expects ( $this->any () )->method ( 'getPaths' )->willReturn ( [ 
				'configKeyOne' => 'config.php',
				'configKeyTwo' => 'env.php' 
		] );
	}
	...
	public function testLoad() {
		...
		$this->configFilePool->expects ( $this->any () )->method ( 'getPath' )->will ( $this->returnValueMap ( $files ) );
		$object = new Reader ( $this->dirList, $this->driverPool, $this->configFilePool );
		$this->assertSame ( [ 
				'fooKey' => 'foo',
				'barKey' => 'bar',
	...
	public function testCustomLoad($file, $expected) {
		...
		$configFilePool->expects ( $this->any () )->method ( 'getPaths' )->willReturn ( [ 
				$file 
		] );
	...
	public function testMerging() {
		...
		$configFilePool->expects ( $this->any () )->method ( 'getPaths' )->willReturn ( [ 
				'configKeyOne' => 'mergeOne.php',
				'configKeyTwo' => 'mergeTwo.php' 
		] );
	...
	public function testMergingWithDuplicateEndValues() {
		...
		$configFilePool->expects ( $this->any () )->method ( 'getPaths' )->willReturn ( [ 
				'configKeyOne' => 'config.php',
				'configKeyTwo' => 'duplicateConfig.php' 
		] );
		$object = new Reader ( $this->dirList, $this->driverPool, $configFilePool );
		$object->load ();
	}
