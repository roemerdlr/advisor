...
class FactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreate() {
		...
		$this->assertInstanceOf ( 'Magento\Framework\Cache\FrontendInterface', $result, 'Created object must implement \Magento\Framework\Cache\FrontendInterface' );
		$this->assertInstanceOf ( 'Magento\Framework\Cache\Core', $result->getLowLevelFrontend (), 'Created object must have \Magento\Framework\Cache\Core frontend by default' );
		$this->assertInstanceOf ( 'Zend_Cache_Backend_BlackHole', $result->getBackend (), 'Created object must have backend as configured in backend options' );
	}
	...
	protected function _buildModelForCreate($enforcedOptions = [], $decorators = []) {
		...
		$dirMock->expects ( $this->any () )->method ( 'getAbsolutePath' )->will ( $this->returnValue ( 'DIR' ) );
		...
		$model = new \Magento\Framework\App\Cache\Frontend\Factory ( $objectManager, $filesystem, $resource, $enforcedOptions, $decorators );
