...
class TypeListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_config = $this->getMock ( 'Magento\Framework\Cache\ConfigInterface', [ 
				'getTypes',
				'getType' 
		], [ ], '', false );
		$this->_config->expects ( $this->any () )->method ( 'getTypes' )->will ( $this->returnValue ( $this->_typesArray ) );
		
		$cacheState = $this->getMock ( 'Magento\Framework\App\Cache\StateInterface', [ 
				'isEnabled',
				'setEnabled',
				'persist' 
		], [ ], '', false );
		...
		$factory = $this->getMock ( 'Magento\Framework\App\Cache\InstanceFactory', [ 
				'get' 
		], [ ], '', false );
		$factory->expects ( $this->any () )->method ( 'get' )->with ( self::CACHE_TYPE )->will ( $this->returnValue ( $cacheBlockMock ) );
	...
	public function testInvalidate() {
		...
		$this->_cache->expects ( $this->once () )->method ( 'load' )->with ( TypeList::INVALIDATED_TYPES )->will ( $this->returnValue ( [ ] ) );
		$expectedInvalidated = [ 
				self::TYPE_KEY => 1 
		];
		$this->_cache->expects ( $this->once () )->method ( 'save' )->with ( serialize ( $expectedInvalidated ), TypeList::INVALIDATED_TYPES );
	...
	public function testCleanType() {
		$this->_cache->expects ( $this->once () )->method ( 'load' )->with ( TypeList::INVALIDATED_TYPES )->will ( $this->returnValue ( serialize ( $this->_typesArray ) ) );
		$this->_config->expects ( $this->once () )->method ( 'getType' )->with ( self::TYPE_KEY )->will ( $this->returnValue ( [ 
