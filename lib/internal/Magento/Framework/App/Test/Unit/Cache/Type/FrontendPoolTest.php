...
class FrontendPoolTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_model = new FrontendPool ( $this->_objectManager, $this->deploymentConfig, $this->_cachePool, [ 
	...
	public function testGet($fixtureConfigData, $inputCacheType, $expectedFrontendId) {
		$this->deploymentConfig->expects ( $this->once () )->method ( 'getConfigData' )->with ( FrontendPool::KEY_CACHE )->will ( $this->returnValue ( $fixtureConfigData ) );
		...
		$this->_objectManager->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\App\Cache\Type\AccessProxy', $this->identicalTo ( [ 
	...
	public function getDataProvider() {
		$configData1 = [ 
				...
				'type' => [ 
						'fixture_cache_type' => [ 
								'frontend' => 'configured_frontend_id' 
						] 
				] 
		];
		$configData2 = [ 
