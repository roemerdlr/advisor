...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = (new ObjectManager ( $this ))->getObject ( 'Magento\Framework\App\Cache\Type\Config', [ 
				'cacheFrontendPool' => $cacheFrontendPoolMock 
		] );
		$this->frontendMock = $this->getMock ( 'Magento\Framework\Cache\FrontendInterface' );
		$cacheFrontendPoolMock->expects ( $this->once () )->method ( 'get' )->with ( \Magento\Framework\App\Cache\Type\Config::TYPE_IDENTIFIER )->willReturn ( $this->frontendMock );
	}
	...
	public function testSave() {
		...
		$this->frontendMock->expects ( $this->once () )->method ( 'save' )->with ( 'test_value', 'test_id', [ 
				'test_tag_one',
				'test_tag_two',
				\Magento\Framework\App\Cache\Type\Config::CACHE_TAG 
		], 111 )->will ( $this->returnValue ( $expectedResult ) );
		$actualResult = $this->model->save ( 'test_value', 'test_id', [ 
				'test_tag_one',
				'test_tag_two' 
		], 111 );
	...
	public function testCleanModeAll() {
		$expectedResult = new \stdClass ();
		$this->frontendMock->expects ( $this->once () )->method ( 'clean' )->with ( \Zend_Cache::CLEANING_MODE_MATCHING_TAG, [ 
				\Magento\Framework\App\Cache\Type\Config::CACHE_TAG 
		] )->will ( $this->returnValue ( $expectedResult ) );
		$actualResult = $this->model->clean ( \Zend_Cache::CLEANING_MODE_ALL, [ 
				'ignored_tag_one',
				'ignored_tag_two' 
		] );
		$this->assertSame ( $expectedResult, $actualResult );
	...
	public function testCleanModeMatchingTag() {
		...
		$actualResult = $this->model->clean ( \Zend_Cache::CLEANING_MODE_MATCHING_TAG, [ 
				'test_tag_one',
				'test_tag_two' 
		] );
		$this->assertSame ( $expectedResult, $actualResult );
	...
	public function testCleanModeMatchingAnyTag($fixtureResultOne, $fixtureResultTwo, $expectedResult) {
		$this->frontendMock->expects ( $this->at ( 0 ) )->method ( 'clean' )->with ( \Zend_Cache::CLEANING_MODE_MATCHING_TAG, [ 
				'test_tag_one',
				\Magento\Framework\App\Cache\Type\Config::CACHE_TAG 
		] )->will ( $this->returnValue ( $fixtureResultOne ) );
		$this->frontendMock->expects ( $this->at ( 1 ) )->method ( 'clean' )->with ( \Zend_Cache::CLEANING_MODE_MATCHING_TAG, [ 
				'test_tag_two',
				\Magento\Framework\App\Cache\Type\Config::CACHE_TAG 
