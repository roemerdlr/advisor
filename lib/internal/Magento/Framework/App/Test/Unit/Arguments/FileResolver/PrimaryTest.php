...
class PrimaryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGet(array $fileList, $scope, $filename) {
		...
		$iteratorFactory = $this->getMock ( 'Magento\Framework\Config\FileIteratorFactory', [ 
				'create' 
		], [ ], '', false );
		
		$filesystem->expects ( $this->once () )->method ( 'getDirectoryRead' )->with ( DirectoryList::CONFIG )->will ( $this->returnValue ( $directory ) );
