...
class CacheTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_cacheFrontendMock = $this->getMockForAbstractClass ( 'Magento\Framework\Cache\FrontendInterface', [ ], '', true, true, true, [ 
				'clean' 
		] );
		...
		$frontendPoolMock->expects ( $this->any () )->method ( 'current' )->will ( $this->returnValue ( $this->_cacheFrontendMock ) );
		$frontendPoolMock->expects ( $this->any () )->method ( 'get' )->with ( \Magento\Framework\App\Cache\Frontend\Pool::DEFAULT_FRONTEND_ID )->will ( $this->returnValue ( $this->_cacheFrontendMock ) );
	...
	protected function _initCacheTypeMocks() {
		...
		foreach ( $cacheTypes as $type ) {
			$this->_cacheTypeMocks [$type] = $this->getMock ( $type, [ 
					'clean' 
			], [ 
	...
	public function testCleanByTags() {
		...
		$this->_cacheFrontendMock->expects ( $this->once () )->method ( 'clean' )->with ( \Zend_Cache::CLEANING_MODE_MATCHING_ANY_TAG, $expectedTags )->will ( $this->returnValue ( true ) );
