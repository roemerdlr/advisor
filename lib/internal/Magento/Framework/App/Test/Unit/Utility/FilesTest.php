...
class FilesTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetConfigFiles() {
		$this->dirSearch->expects ( $this->once () )->method ( 'collectFiles' )->with ( ComponentRegistrar::MODULE, '/etc/some.file' )->willReturn ( [ 
				'/one/some.file',
				'/two/some.file',
				'some.other.file' 
		] );
		...
		$actual = Files::init ()->getConfigFiles ( 'some.file', [ 
				'some.other.file' 
		], false );
	...
	public function testGetLayoutConfigFiles() {
		$this->dirSearch->expects ( $this->once () )->method ( 'collectFiles' )->with ( ComponentRegistrar::THEME, '/etc/some.file' )->willReturn ( [ 
				'/one/some.file',
				'/two/some.file' 
		] );
