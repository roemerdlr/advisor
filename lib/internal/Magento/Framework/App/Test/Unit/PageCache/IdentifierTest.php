...
class IdentifierTest extends \PHPUnit_Framework_TestCase {
	...
	protected function getRequestMock($uri, $vary = null) {
		...
		$requestMock->expects ( $this->once () )->method ( 'getUriString' )->willReturn ( $uri );
		$requestMock->expects ( $this->once () )->method ( 'get' )->with ( $this->equalTo ( \Magento\Framework\App\Response\Http::COOKIE_VARY_STRING ) )->willReturn ( $vary );
		return $requestMock;
	}
	...
	protected function getContextMock($getVeryStringCalledTimes, $vary) {
		...
		$contextMock->expects ( $this->exactly ( $getVeryStringCalledTimes ) )->method ( 'getVaryString' )->willReturn ( $vary );
		return $contextMock;
	}
	...
	public function testGetValue($uri, $varyStringCookie, $varyStringContext, $expected) {
		...
		$model = $this->objectManager->getObject ( '\Magento\Framework\App\PageCache\Identifier', [ 
				'request' => $request,
				'context' => $context 
		] );
