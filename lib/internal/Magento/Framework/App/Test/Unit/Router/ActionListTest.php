...
class ActionListTest extends \PHPUnit_Framework_TestCase {
	...
	public function testConstructorCachedData() {
		$this->cacheMock->expects ( $this->once () )->method ( 'load' )->will ( $this->returnValue ( serialize ( 'data' ) ) );
		$this->cacheMock->expects ( $this->never () )->method ( 'save' );
		$this->moduleReaderMock->expects ( $this->never () )->method ( 'getActionFiles' );
		$this->actionList = $this->objectManager->getObject ( 'Magento\Framework\App\Router\ActionList', [ 
	...
	public function testConstructorNoCachedData() {
		...
		$this->moduleReaderMock->expects ( $this->once () )->method ( 'getActionFiles' )->will ( $this->returnValue ( 'data' ) );
		$this->actionList = $this->objectManager->getObject ( 'Magento\Framework\App\Router\ActionList', [ 
				'cache' => $this->cacheMock,
				'moduleReader' => $this->moduleReaderMock 
		] );
	...
	public function getDataProvider() {
		...
		$actionClass = $this->getMockClass ( 'Magento\Framework\App\ActionInterface', [ 
				'execute',
				'getResponse' 
		], [ ], $mockClassName );
