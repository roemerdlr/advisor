...
class FileResolverTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->iteratorFactory = $this->getMock ( 'Magento\Framework\Config\FileIteratorFactory', [ ], [ 
				'getPath' 
		], '', false );
		$this->filesystem = $this->getMock ( 'Magento\Framework\Filesystem', [ 
				'getDirectoryRead' 
		], [ ], '', false );
		$this->moduleReader = $this->getMock ( 'Magento\Framework\Module\Dir\Reader', [ ], [ 
				'getConfigurationFiles' 
		], '', false );
		$this->model = new \Magento\Framework\App\Config\FileResolver ( $this->moduleReader, $this->filesystem, $this->iteratorFactory );
	}
	...
	public function testGetPrimary($filename, $fileList) {
		...
		$this->iteratorFactory->expects ( $this->once () )->method ( 'create' )->with ( $fileList )->will ( $this->returnValue ( true ) );
	...
	public function testGetGlobal($filename, $fileList) {
		$scope = 'global';
		$this->moduleReader->expects ( $this->once () )->method ( 'getConfigurationFiles' )->with ( $filename )->will ( $this->returnValue ( $fileList ) );
