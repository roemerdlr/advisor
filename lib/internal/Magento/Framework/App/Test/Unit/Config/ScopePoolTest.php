...
class ScopePoolTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->_dataFactory = $this->getMockBuilder ( '\Magento\Framework\App\Config\DataFactory' )->disableOriginalConstructor ()->getMock ();
		$this->_cache = $this->getMock ( '\Magento\Framework\Cache\FrontendInterface' );
		$this->_object = $helper->getObject ( '\Magento\Framework\App\Config\ScopePool', [ 
	...
	public function testGetScope($scopeType, $scope, array $data, $cachedData) {
		...
		if (! $cachedData) {
			...
			$this->_cache->expects ( $this->once () )->method ( 'save' )->with ( serialize ( $data ), $cacheKey, [ 
		...
		$this->_dataFactory->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'data' => $data 
	...
	public function testClean() {
		$this->_cache->expects ( $this->once () )->method ( 'clean' )->with ( \Zend_Cache::CLEANING_MODE_MATCHING_TAG, [ 
				\Magento\Framework\App\Config\ScopePool::CACHE_TAG 
		] );
