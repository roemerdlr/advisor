...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testReadNoFiles() {
		$this->createModelAndVerifyConstructor ();
		$this->fileResolverMock->expects ( $this->at ( 0 ) )->method ( 'get' )->with ( 'config.xml', 'global' )->will ( $this->returnValue ( [ ] ) );
	...
	public function testReadValidConfig() {
		...
		$this->converterMock->expects ( $this->once () )->method ( 'convert' )->with ( $this->anything () )->will ( $this->returnValue ( $expectedConfig ) );
	...
	private function createModelAndVerifyConstructor() {
		...
		$this->model = $this->objectManager->getObject ( 'Magento\Framework\App\Config\Initial\Reader', [ 
