...
class WriterTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resource = $this->getMockBuilder ( 'Magento\Framework\App\Config\ConfigResource\ConfigInterface' )->disableOriginalConstructor ()->getMock ();
		$this->model = $this->objectManager->getObject ( 'Magento\Framework\App\Config\Storage\Writer', [ 
				'resource' => $this->resource 
		] );
	}
	public function testDelete() {
		$this->resource->expects ( $this->once () )->method ( 'deleteConfig' )->with ( 'path', ScopeConfigInterface::SCOPE_TYPE_DEFAULT, 0 );
		$this->model->delete ( 'path' );
	}
	public function testDeleteOptions() {
		$scope = 'scope';
		$scopeId = '1';
		$this->resource->expects ( $this->once () )->method ( 'deleteConfig' )->with ( 'path', $scope, $scopeId );
