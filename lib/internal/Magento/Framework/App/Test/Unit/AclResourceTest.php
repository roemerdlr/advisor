...
class AclResourceTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->connectionFactory = $this->getMockBuilder ( ConnectionFactoryInterface::class )->setMethods ( [ 
				'create' 
		] )->getMockForAbstractClass ();
		$this->config = $this->getMockBuilder ( 'Magento\Framework\App\ResourceConnection\ConfigInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'getConnectionName' 
		] )->getMock ();
		...
		$this->deploymentConfig->expects ( $this->any () )->method ( 'get' )->willReturnMap ( [ 
				[ 
	...
	public function getTableNameDataProvider() {
		return [ 
				...
				[ 
						...
						self::TABLE_PREFIX . 'tableName_tableSuffix' 
				] 
		];
	}
