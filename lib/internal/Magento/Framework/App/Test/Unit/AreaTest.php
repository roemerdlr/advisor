...
class AreaTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->object = $this->objectManager->getObject ( 'Magento\Framework\App\Area', [ 
	...
	public function testLoadTranslate() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Framework\Phrase\RendererInterface' )->will ( $this->returnValue ( $renderMock ) );
	...
	public function testLoad() {
		...
		$designMock->expects ( $this->once () )->method ( 'setArea' )->with ( $this->areaCode )->willReturnSelf ();
		$designMock->expects ( $this->once () )->method ( 'setDefaultDesignTheme' );
		$this->objectManagerMock->expects ( $this->exactly ( 2 ) )->method ( 'get' )->will ( $this->returnValueMap ( [ 
				[ 
	...
	private function verifyLoadConfig() {
		...
		$this->diConfigLoaderMock->expects ( $this->once () )->method ( 'load' )->with ( $this->areaCode )->will ( $this->returnValue ( $configs ) );
		$this->objectManagerMock->expects ( $this->once () )->method ( 'configure' )->with ( $configs );
	}
	public function testDetectDesign() {
		$this->designExceptionsMock->expects ( $this->never () )->method ( 'getThemeByRequest' );
	...
	public function testDetectDesignByRequest($value, $callNum, $callNum2) {
		$this->designExceptionsMock->expects ( $this->once () )->method ( 'getThemeByRequest' )->will ( $this->returnValue ( $value ) );
		$designMock = $this->getMockBuilder ( 'Magento\Framework\View\DesignInterface' )->disableOriginalConstructor ()->getMock ();
		$designMock->expects ( $this->exactly ( $callNum ) )->method ( 'setDesignTheme' );
		$this->objectManagerMock->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Framework\View\DesignInterface' )->will ( $this->returnValue ( $designMock ) );
		$this->designMock->expects ( $this->exactly ( $callNum2 ) )->method ( 'loadChange' )->with ( self::SCOPE_ID )->willReturnSelf ();
		$this->designMock->expects ( $this->exactly ( $callNum2 ) )->method ( 'changeDesign' )->with ( $designMock )->willReturnSelf ();
