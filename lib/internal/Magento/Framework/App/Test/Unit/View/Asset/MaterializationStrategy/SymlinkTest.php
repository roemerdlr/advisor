...
class SymlinkTest extends \PHPUnit_Framework_TestCase {
	...
	public function testPublishFile() {
		$rootDir = $this->getMockBuilder ( 'Magento\Framework\Filesystem\Directory\WriteInterface' )->getMock ();
		...
		$rootDir->expects ( $this->once () )->method ( 'createSymlink' )->with ( $sourcePath, $destinationPath, $targetDir )->willReturn ( true );
	...
	public function testIsSupported($path, $expectation) {
		$asset = $this->getMockBuilder ( 'Magento\Framework\View\Asset\LocalInterface' )->setMethods ( [ ] )->getMock ();
		$asset->expects ( $this->once () )->method ( 'getSourceFile' )->willReturn ( $path );
	...
	public function sourceFileDataProvider() {
		return [ 
				...
				[ 
						DirectoryList::TMP_MATERIALIZATION_DIR . '/path/to/file',
						false 
				] 
		];
	}
