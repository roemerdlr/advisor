...
class FileTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$filesystem->expects ( $this->once () )->method ( 'getDirectoryWrite' )->with ( 'fixture_dir' )->will ( $this->returnValue ( $this->directory ) );
		$this->object = new File ( $filesystem, 'fixture_dir', 'fixture_file.txt' );
	}
	public function testLoad() {
		$this->directory->expects ( $this->once () )->method ( 'readFile' )->with ( 'fixture_file.txt' )->will ( $this->returnValue ( '123' ) );
	...
	public function testLoadExceptionPropagation() {
		$this->directory->expects ( $this->once () )->method ( 'readFile' )->with ( 'fixture_file.txt' )->will ( $this->throwException ( new \Exception ( 'Exception to be propagated' ) ) );
		$this->object->load ();
	}
	...
	public function testLoadExceptionWrapping() {
		$filesystemException = new \Magento\Framework\Exception\FileSystemException ( new \Magento\Framework\Phrase ( 'File does not exist' ) );
	...
	public function testSave() {
		$this->directory->expects ( $this->once () )->method ( 'writeFile' )->with ( 'fixture_file.txt', 'input_data', 'w' );
