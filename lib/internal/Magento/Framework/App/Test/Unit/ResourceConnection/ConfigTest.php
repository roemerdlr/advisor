...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_readerMock = $this->getMock ( 'Magento\Framework\App\ResourceConnection\Config\Reader', [ ], [ ], '', false );
		...
		$this->_resourcesConfig = [ 
				...
				'extendedResourceName' => [ 
						...
						'extends' => 'validResource' 
				] 
		];
		
		$this->_initialResources = [ 
				'validResource' => [ 
						'connection' => 'validConnectionName' 
				] 
		];
		
		$this->_cacheMock->expects ( $this->any () )->method ( 'load' )->will ( $this->returnValue ( serialize ( $this->_resourcesConfig ) ) );
		
		$deploymentConfig = $this->getMock ( '\Magento\Framework\App\DeploymentConfig', [ ], [ ], '', false );
		$deploymentConfig->expects ( $this->once () )->method ( 'getConfigData' )->with ( 'resource' )->willReturn ( $this->_initialResources );
		
		$this->_model = new \Magento\Framework\App\ResourceConnection\Config ( $this->_readerMock, $this->_scopeMock, $this->_cacheMock, $deploymentConfig, 'cacheId' );
	}
	...
	public function testExceptionConstructor() {
		$deploymentConfig = $this->getMock ( '\Magento\Framework\App\DeploymentConfig', [ ], [ ], '', false );
		$deploymentConfig->expects ( $this->once () )->method ( 'getConfigData' )->with ( 'resource' )->willReturn ( [ 
				'validResource' => [ 
						'somekey' => 'validConnectionName' 
				] 
		] );
		
		new \Magento\Framework\App\ResourceConnection\Config ( $this->_readerMock, $this->_scopeMock, $this->_cacheMock, $deploymentConfig, 'cacheId' );
	}
	...
	public function getConnectionNameDataProvider() {
		return [ 
				...
				[ 
						...
						'connectionName' => 'validConnectionName' 
				],
				[ 
						'resourceName' => 'validResource',
						'connectionName' => 'validConnectionName' 
				] 
		];
	}
