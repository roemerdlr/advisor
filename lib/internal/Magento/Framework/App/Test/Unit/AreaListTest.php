...
class AreaListTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_resolverFactory = $this->getMock ( '\Magento\Framework\App\Area\FrontNameResolverFactory', [ ], [ ], '', false );
	}
	public function testGetCodeByFrontNameWhenAreaDoesNotContainFrontName() {
		$expected = 'expectedFrontName';
		$this->_model = new \Magento\Framework\App\AreaList ( $this->objectManagerMock, $this->_resolverFactory, [ 
				'testArea' => [ 
						'frontNameResolver' => 'testValue' 
				] 
		], $expected );
		
		$resolverMock = $this->getMock ( '\Magento\Framework\App\Area\FrontNameResolverInterface' );
		$this->_resolverFactory->expects ( $this->any () )->method ( 'create' )->with ( 'testValue' )->will ( $this->returnValue ( $resolverMock ) );
	...
	public function testGetFrontNameWhenAreaCodeAndFrontNameArentSet() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\App\AreaInterface', [ 
				'areaCode' => $code 
		] )->willReturn ( 'test' );
	...
	protected function getObjectManagerMockGetArea() {
		...
		$objectManagerMock->expects ( $this->any () )->method ( 'create' )->with ( $this->equalTo ( 'Magento\Framework\App\AreaInterface' ), $this->equalTo ( [ 
				'areaCode' => 'testArea' 
