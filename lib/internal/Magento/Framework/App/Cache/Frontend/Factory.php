...
class Factory {
	 ...
	 */
	public function __construct(\Magento\Framework\ObjectManagerInterface $objectManager, Filesystem $filesystem, \Magento\Framework\App\ResourceConnection $resource, array $enforcedOptions = [], array $decorators = []) {
	...
	public function create(array $options) {
		...
		$result = $this->_objectManager->create ( 'Magento\Framework\Cache\Frontend\Adapter\Zend', [ 
				'frontend' => \Zend_Cache::factory ( $frontend ['type'], $backend ['type'], $frontend, $backend ['options'], true, true, true ) 
		] );
	 ...
	 * Result array contain backend type ('type' key) and backend options ('options')
	 *
	 ...
	 *
	 * @param array $fastOptions
	 *        	fast level backend type and options
	 * @param array $cacheOptions
	 *        	all cache options
	 * @return array
	 */
	...
	protected function _getFrontendOptions(array $cacheOptions) {
		...
		$options ['type'] = isset ( $cacheOptions ['frontend'] ) ? $cacheOptions ['frontend'] : 'Magento\Framework\Cache\Core';
		return $options;
	}
}
