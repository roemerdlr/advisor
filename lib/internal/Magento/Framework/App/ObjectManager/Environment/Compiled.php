...
class Compiled extends AbstractEnvironment implements EnvironmentInterface {
	...
	protected function createFactory($arguments, $factoryClass) {
		return new $factoryClass ( $this->getDiConfig (), $arguments ['shared_instances'], $arguments );
	}
	...
	public function getDiConfig() {
		if (! $this->config) {
			$this->config = new \Magento\Framework\Interception\ObjectManager\Config\Compiled ( $this->getConfigData () );
		}
	...
	public function configureObjectManager(ConfigInterface $diConfig, &$sharedInstances) {
		...
		$sharedInstances ['Magento\Framework\Interception\PluginList\PluginList'] = $objectManager->create ( 'Magento\Framework\Interception\PluginListInterface', [ 
				'cache' => $objectManager->get ( 'Magento\Framework\App\Interception\Cache\CompiledConfig' ) 
		] );
		$objectManager->get ( 'Magento\Framework\App\Cache\Manager' )->setEnabled ( [ 
