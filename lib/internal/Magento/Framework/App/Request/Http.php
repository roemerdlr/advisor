...
class Http extends Request implements RequestInterface, RequestSafetyInterface {
	 ...
	 */
	public function __construct(CookieReaderInterface $cookieReader, StringUtils $converter, ConfigInterface $routeConfig, PathInfoProcessorInterface $pathInfoProcessor, ObjectManagerInterface $objectManager, $uri = null, $directFrontNames = []) {
	...
	public function getDistroBaseUrl() {
		...
		if (isset ( $headerScriptName ) && isset ( $headerHttpHost )) {
			$secure = ! empty ( $headerHttps ) && $headerHttps != 'off' || isset ( $headerServerPort ) && $headerServerPort == '443';
	...
	public function getFullActionName($delimiter = '_') {
		return $this->getRouteName () . $delimiter . $this->getControllerName () . $delimiter . $this->getActionName ();
	}
	...
	public function isSecure() {
		...
		$offLoaderHeader = trim ( ( string ) $config->getValue ( self::XML_PATH_OFFLOADER_HEADER, ScopeConfigInterface::SCOPE_TYPE_DEFAULT ) );
	...
	protected function initialRequestSecure($offLoaderHeader) {
		...
		return ! empty ( $offLoaderHeader ) && (isset ( $header ) && ($header === 'https') || isset ( $httpHeader ) && ($httpHeader === 'https'));
	}
}
