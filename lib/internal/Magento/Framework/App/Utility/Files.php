...
class Files {
	 ...
	 */
	public function __construct(ComponentRegistrar $componentRegistrar, DirSearch $dirSearch, ThemePackageList $themePackageList) {
	...
	private function getLibraryFiles($flags) {
		if ($flags & self::INCLUDE_LIBS) {
			...
			return $this->getFilesSubset ( $this->componentRegistrar->getPaths ( ComponentRegistrar::LIBRARY ), '*.php', $libraryExcludeDirs );
		}
		return [ ];
	}
	...
	private function getPubFiles($flags) {
		if ($flags & self::INCLUDE_PUB_CODE) {
			return array_merge ( glob ( BP . '/*.php', GLOB_NOSORT ), glob ( BP . '/pub/*.php', GLOB_NOSORT ) );
		}
		return [ ];
	}
	...
	private function getAppCodeFiles($flags) {
		if ($flags & self::INCLUDE_APP_CODE) {
			...
			return $this->getFilesSubset ( $paths, '*.php', array_merge ( $this->getModuleTestDirsRegex (), $excludePaths ) );
		}
		return [ ];
	}
	...
	public function getXmlFiles() {
		return array_merge ( $this->getMainConfigFiles (), $this->getLayoutFiles (), $this->getPageLayoutFiles (), $this->getConfigFiles (), $this->getDiConfigs ( true ), $this->getLayoutConfigFiles (), $this->getPageTypeFiles () );
	}
	 ...
	 * @param bool $asDataSet        	
	 * @return array @codingStandardsIgnoreStart
	 */
	public function getConfigFiles($fileNamePattern = '*.xml', $excludedFileNames = ['wsdl.xml', 'wsdl2.xml', 'wsi.xml'], $asDataSet = true) {
		$cacheKey = __METHOD__ . '|' . BP . '|' . serialize ( func_get_args () );
		if (! isset ( self::$_cache [$cacheKey] )) {
	 ...
	 * @return array
	 */
	public function getXmlCatalogFiles($fileNamePattern = '*.xsd', $excludedFileNames = [], $asDataSet = true) {
		$cacheKey = __METHOD__ . '|' . BP . '|' . serialize ( func_get_args () );
		if (! isset ( self::$_cache [$cacheKey] )) {
			$files = $this->getFilesSubset ( $this->componentRegistrar->getPaths ( ComponentRegistrar::MODULE ), $fileNamePattern, [ ] );
			$libraryExcludeDirs = [ ];
			foreach ( $this->componentRegistrar->getPaths ( ComponentRegistrar::LIBRARY ) as $libraryDir ) {
				$libraryExcludeDirs [] = str_replace ( '\\', '/', '#' . $libraryDir . '/Test#' );
				$libraryExcludeDirs [] = str_replace ( '\\', '/', '#' . $libraryDir ) . '/[\\w]+/Test#';
			}
			$files = array_merge ( $files, $this->getFilesSubset ( $this->componentRegistrar->getPaths ( ComponentRegistrar::LIBRARY ), $fileNamePattern, $libraryExcludeDirs ) );
			$files = array_merge ( $files, $this->getFilesSubset ( $this->componentRegistrar->getPaths ( ComponentRegistrar::THEME ), $fileNamePattern, [ ] ) );
			$files = array_filter ( $files, function ($file) use($excludedFileNames) {
				return ! in_array ( basename ( $file ), $excludedFileNames );
			} );
			self::$_cache [$cacheKey] = $files;
	...
	public function getLayoutConfigFiles($fileNamePattern = '*.xml', $asDataSet = true) {
		$cacheKey = __METHOD__ . '|' . BP . '|' . serialize ( func_get_args () );
		if (! isset ( self::$_cache [$cacheKey] )) {
			self::$_cache [$cacheKey] = $this->dirSearch->collectFiles ( ComponentRegistrar::THEME, "/etc/{$fileNamePattern}" );
		}
		if ($asDataSet) {
			return self::composeDataSets ( self::$_cache [$cacheKey] );
		}
	...
	private function collectThemeLayoutFiles(array $params, $location) {
		...
		foreach ( $this->themePackageList->getThemes () as $theme ) {
			...
			if (($area == '*' || $theme->getArea () === $area) && ($themePath == '*' || $themePath == '*/*' || $themePath == $currentThemeCode)) {
				$themeFiles = [ ];
				$this->_accumulateFilesByPatterns ( [ 
	...
	private function parseThemeFiles($themeFiles, $currentThemePath, $theme) {
		$files = [ ];
		$regex = '#^' . $currentThemePath . '/(?P<module>[a-z\d]+_[a-z\d]+)/layout/(override/((base/)|(theme/[a-z\d_]+/[a-z\d_]+/)))?' . '(?P<path>.+)$#i';
	...
	public function getPageTypeFiles($incomingParams = [], $asDataSet = true) {
		...
		if (! isset ( self::$_cache [__METHOD__] [$cacheKey] )) {
			self::$_cache [__METHOD__] [$cacheKey] = self::getFiles ( $this->getEtcAreaPaths ( $params ['namespace'], $params ['module'], $params ['area'] ), 'page_types.xml' );
		}
	...
	public function getJsFiles($area = '*', $themePath = '*/*', $namespace = '*', $module = '*') {
		...
		$files = self::getFiles ( array_merge ( [ 
				BP . "/lib/web/{mage,varien}" 
		], $themePaths, $moduleWebPaths ), '*.js' );
	...
	public function getStaticHtmlFiles($area = '*', $themePath = '*/*', $namespace = '*', $module = '*') {
		...
		$files = self::getFiles ( array_merge ( $themePaths, $moduleTemplatePaths ), '*.html' );
		$result = self::composeDataSets ( $files );
		self::$_cache [$key] = $result;
		return $result;
	}
	...
	private function accumulateThemeStaticFiles($area, $locale, $filePattern, &$result) {
		foreach ( $this->themePackageList->getThemes () as $themePackage ) {
			...
			if ($area == '*' || $area == $themeArea) {
				...
				$regex = '#^' . $themePath . '/((?P<module>[a-z\d]+_[a-z\d]+)/)?web/(i18n/(?P<locale>[a-z_]+)/)?(?P<path>.+)$#i';
				foreach ( $files as $file ) {
	...
	protected function _parseModuleStatic($file) {
		foreach ( $this->componentRegistrar->getPaths ( ComponentRegistrar::MODULE ) as $moduleName => $modulePath ) {
			if (preg_match ( '/^' . preg_quote ( "{$modulePath}/", '/' ) . 'view\/([a-z]+)\/web\/(.+)$/i', $file, $matches ) === 1) {
	...
	public function getJsFilesForArea($area) {
		...
		if ($area == 'adminhtml') {
			$adminhtmlPaths = [ 
					BP . "/lib/web/mage/{adminhtml,backend}" 
			];
	 ...
	 * The list excludes tool-specific files
	...
	public function getAllFiles() {
		...
		$paths = array_merge ( [ 
				...
				BP . '/lib',
				BP . '/pub' 
		], $this->componentRegistrar->getPaths ( ComponentRegistrar::LANGUAGE ), $this->componentRegistrar->getPaths ( ComponentRegistrar::THEME ), $this->getPaths () );
	...
	private function classFileExistsCheckContent($fullPath, $namespace, $className) {
		 ...
		 */
		if (realpath ( $fullPath ) == str_replace ( [ 
				'/',
				'\\' 
		], DIRECTORY_SEPARATOR, $fullPath ) || file_exists ( $fullPath )) {
			$fileContent = file_get_contents ( $fullPath );
			if (strpos ( $fileContent, 'namespace ' . $namespace ) !== false && (strpos ( $fileContent, 'class ' . $className ) !== false || strpos ( $fileContent, 'interface ' . $className ) !== false)) {
				return true;
			}
		}
		return false;
	...
	public function getModuleFile($namespace, $module, $file) {
		return $this->componentRegistrar->getPath ( ComponentRegistrar::MODULE, $namespace . '_' . $module ) . '/' . $file;
	}
	 ...
	 *        	Subdirectories to exlude, represented as regex
