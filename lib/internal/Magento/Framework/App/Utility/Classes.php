...
class Classes {
	...
	public static function collectClassesInConfig(\SimpleXMLElement $xml) {
		...
		$classes = array_merge ( $classes, self::getXmlNodeNames ( $xml, '/logging/*/expected_models/* | /logging/*/actions/*/expected_models/* | /config/*/di/preferences/*' ) );
	...
	public static function collectLayoutClasses(\SimpleXMLElement $xml) {
		...
		$classes = array_merge ( $classes, self::getXmlNodeValues ( $xml, '/layout//action/attributeType | /layout//action[@method="addTab"]/content
	...
	public static function collectModuleClasses($subTypePattern = '[A-Za-z]+') {
		$pattern = '/^' . preg_quote ( Files::init ()->getPathToSource (), '/' ) . '\/app\/code\/([A-Za-z]+)\/([A-Za-z]+)\/(' . $subTypePattern . '\/.+)\.php$/';
		$result = [ ];
	...
	public static function isAutogenerated($className) {
		if (preg_match ( '/.*\\\\[a-zA-Z0-9]{1,}(Factory|Proxy|SearchResults|DataBuilder|Extension|ExtensionInterface)$/', $className ) || preg_match ( '/Magento\\\\[\w]+\\\\(Test\\\\(Page|Fixture))\\\\/', $className )) {
			return true;
		}
