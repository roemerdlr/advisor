...
class ServiceInputProcessor implements ServicePayloadConverterInterface {
	 ...
	 */
	public function __construct(TypeProcessor $typeProcessor, ObjectManagerInterface $objectManager, AttributeValueFactory $attributeValueFactory, CustomAttributeTypeLocatorInterface $customAttributeTypeLocator, MethodsMap $methodsMap) {
	 ...
	 *        	name of the service class that we are trying to call
	 * @param string $serviceMethodName
	 *        	name of the method that we are trying to call
	 * @param array $inputArray
	 *        	data to send to method in key-value format
	 ...
	 * @param array $customAttributeValue
	 ...
	 * @param mixed $data        	
	 * @param string $type
	 *        	Convert given value to the this type
	 * @return mixed
	...
	protected function _removeSoapItemNode($value) {
		 ...
		 * within item node.
		 * If several Data object values are passed, they will be wrapped into
	...
	protected function processInputError($inputError) {
		if (! empty ( $inputError )) {
			...
			foreach ( $inputError as $errorParamField ) {
				$exception->addError ( new Phrase ( InputException::REQUIRED_FIELD, [ 
