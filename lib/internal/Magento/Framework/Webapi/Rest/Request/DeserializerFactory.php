...
class DeserializerFactory {
	 ...
	 */
	public function __construct(\Magento\Framework\ObjectManagerInterface $objectManager, array $deserializers = []) {
	...
	public function get($contentType) {
		...
		if (! isset ( $deserializerClass ) || empty ( $deserializerClass )) {
			throw new \Magento\Framework\Webapi\Exception ( new Phrase ( 'Server cannot understand Content-Type HTTP header media type %1', [ 
					$contentType 
			] ) );
		}
		...
		if (! $deserializer instanceof \Magento\Framework\Webapi\Rest\Request\DeserializerInterface) {
			throw new \LogicException ( 'The deserializer must implement "Magento\Framework\Webapi\Rest\Request\DeserializerInterface".' );
		}
		return $deserializer;
	}
}
