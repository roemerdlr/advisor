...
class ErrorProcessor {
	 ...
	 *        	Exception to convert to a WebAPI exception
	 *        	
	...
	public function maskException(\Exception $exception) {
		...
		if ($exception instanceof WebapiException) {
			...
			$maskedException = new WebapiException ( new Phrase ( $exception->getRawMessage () ), $exception->getCode (), $httpCode, $exception->getParameters (), get_class ( $exception ), $errors, $stackTrace );
		} else {
			...
			$maskedException = new WebapiException ( new Phrase ( $message ), $code, WebapiException::HTTP_INTERNAL_ERROR, [ ], '', null, $stackTrace );
		}
	 ...
	 * @param \Exception $exception        	
	 * @param int $httpCode        	
	 * @return void @SuppressWarnings(PHPMD.ExitExpression)
	 */
	...
	protected function _critical(\Exception $exception) {
		 ...
		 */
		"Report ID: {$reportId}; Message: {$exception->getMessage()}", $exception->getCode () );
	 ...
	 * @return void
	 */
	public function renderErrorMessage($errorMessage, $trace = 'Trace is not available.', $httpCode = self::DEFAULT_ERROR_HTTP_CODE) {
	...
	public function registerShutdownFunction() {
		register_shutdown_function ( [ 
				$this,
				self::DEFAULT_SHUTDOWN_FUNCTION 
		] );
		return $this;
	}
	...
	public function apiShutdownFunction() {
		...
		if ($error && $error ['type'] & $fatalErrorFlag) {
			...
			if ($this->_appState->getMode () == State::MODE_DEVELOPER) {
				...
				$this->renderErrorMessage ( new Phrase ( 'Server internal error. See details in report api/%1', [ 
						$reportId 
				] ) );
			}
		}
