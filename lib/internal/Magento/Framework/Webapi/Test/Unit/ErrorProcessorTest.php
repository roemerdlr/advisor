...
class ErrorProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		 ...
		 */
		$this->encoderMock = $this->getMockBuilder ( 'Magento\Framework\Json\Encoder' )->disableOriginalConstructor ()->setMethods ( [ 
				'encode' 
		] )->getMock ();
		 ...
		 * Initialize SUT.
		 */
		$this->_errorProcessor = new ErrorProcessor ( $this->encoderMock, $this->_appStateMock, $this->_loggerMock, $filesystemMock );
	...
	public function testRenderJson() {
		 ...
		 */
		$this->encoderMock->expects ( $this->once () )->method ( 'encode' )->will ( $this->returnCallback ( [ 
				$this,
				'callbackJsonEncode' 
	...
	public function testRenderXml() {
		...
		$actualResult = ob_get_contents ();
		ob_end_clean ();
		$expectedResult = '<?xml version="1.0"?><error><messages><error><data_item><code>500</code>' . '<message><![CDATA[Message]]></message></data_item></error></messages></error>';
	...
	public function testRenderXmlInDeveloperMode() {
		...
		$actualResult = ob_get_contents ();
		ob_end_clean ();
		$expectedResult = '<?xml version="1.0"?><error><messages><error><data_item><code>401</code><message>' . '<![CDATA[Message]]></message><trace><![CDATA[Trace message.]]></trace></data_item></error>' . '</messages></error>';
	...
	public function testMaskExceptionInDeveloperMode() {
		...
		$this->assertEquals ( $errorMessage, $maskedException->getMessage (), 'Exception was masked incorrectly in developer mode.' );
	}
	 ...
	 * @return void @dataProvider dataProviderForSendResponseExceptions
	 */
	...
	public function dataProviderForSendResponseExceptions() {
		return [ 
				'NoSuchEntityException' => [ 
						new NoSuchEntityException ( new Phrase ( NoSuchEntityException::MESSAGE_DOUBLE_FIELDS, [ 
								'fieldName' => 'detail1',
								'fieldValue' => 'value1',
				...
				'AuthorizationException' => [ 
						new AuthorizationException ( new Phrase ( AuthorizationException::NOT_AUTHORIZED, [ 
								'consumer_id' => '3',
								'resources' => '4' 
						] ) ),
	 ...
	 * @return void
	 */
	public function assertMaskedException($maskedException, $expectedHttpCode, $expectedMessage, $expectedDetails) {
		/**
		 * All masked exceptions must be WebapiException
		 */
		$expectedType = 'Magento\Framework\Webapi\Exception';
		$this->assertInstanceOf ( $expectedType, $maskedException, "Masked exception type is invalid: expected '{$expectedType}', given '" . get_class ( $maskedException ) . "'." );
		/** @var $maskedException WebapiException */
		$this->assertEquals ( $expectedHttpCode, $maskedException->getHttpCode (), "Masked exception HTTP code is invalid: expected '{$expectedHttpCode}', " . "given '{$maskedException->getHttpCode()}'." );
		$this->assertContains ( $expectedMessage, $maskedException->getMessage (), "Masked exception message is invalid: expected '{$expectedMessage}', " . "given '{$maskedException->getMessage()}'." );
