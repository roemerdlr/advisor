...
class DeserializerFactoryTest extends \PHPUnit_Framework_TestCase {
	public function testGetLogicExceptionEmptyRequestAdapter() {
		...
		$interpreterFactory = new \Magento\Framework\Webapi\Rest\Request\DeserializerFactory ( $this->getMock ( 'Magento\Framework\ObjectManagerInterface' ), [ ] );
	...
	public function testGet() {
		...
		$validInterpreterMock = $this->getMockBuilder ( 'Magento\Framework\Webapi\Rest\Request\Deserializer\Xml' )->disableOriginalConstructor ()->getMock ();
		
		$objectManagerMock = $this->getMock ( 'Magento\Framework\ObjectManagerInterface' );
		$objectManagerMock->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( $validInterpreterMock ) );
		
		$interpreterFactory = new \Magento\Framework\Webapi\Rest\Request\DeserializerFactory ( $objectManagerMock, $expectedMetadata );
	...
	public function testGetMagentoWebapiException() {
		...
		$this->setExpectedException ( 'Magento\Framework\Webapi\Exception', 'Server cannot understand Content-Type HTTP header media type text_xml' );
		$interpreterFactory = new \Magento\Framework\Webapi\Rest\Request\DeserializerFactory ( $this->getMock ( 'Magento\Framework\ObjectManagerInterface' ), $expectedMetadata );
	...
	public function testGetLogicExceptionInvalidRequestDeserializer() {
		...
		$invalidInterpreter = $this->getMockBuilder ( 'Magento\Framework\Webapi\Response\Rest\Renderer\Json' )->disableOriginalConstructor ()->getMock ();
