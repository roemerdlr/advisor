...
class XmlTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		 ...
		 */
		$this->_xmlParserMock = $this->getMock ( 'Magento\Framework\Xml\Parser', [ 
				'xmlToArray',
				'loadXML' 
		] );
		 ...
		 * Initialize SUT.
		 */
		$this->_xmlDeserializer = new \Magento\Framework\Webapi\Rest\Request\Deserializer\Xml ( $this->_xmlParserMock, $this->_appStateMock );
		parent::setUp ();
	}
	...
	public function testDeserialize() {
		 ...
		 * Initialize SUT.
		 */
		$this->assertEquals ( $expectedArray, $this->_xmlDeserializer->deserialize ( $validInputXml ), 'Request XML body was parsed incorrectly into array of params.' );
	}
	public function testHandleErrors() {
		 ...
		 */
		$this->assertAttributeEquals ( $firstErrorMessage, '_errorMessage', $this->_xmlDeserializer, 'Error message was not set to xml deserializer.' );
		/**
		 * Add error message
		 */
		 ...
		 */
		$this->assertAttributeEquals ( $expectedMessages, '_errorMessage', $this->_xmlDeserializer, 'Error messages were not set to xml deserializer.' );
	}
	...
	public function testDeserializeMagentoWebapiExceptionDeveloperModeOff() {
		 ...
		 * Prepare mocks for SUT constructor.
		 */
		$this->_appStateMock->expects ( $this->once () )->method ( 'getMode' )->will ( $this->returnValue ( 'production' ) );
		$errorMessage = 'End tag for "key1" was omitted.';
		$this->_xmlDeserializer->handleErrors ( null, $errorMessage, null, null );
		$this->_xmlParserMock->expects ( $this->once () )->method ( 'loadXML' );
		$invalidXml = '<?xml version="1.0"?><xml><key1>test1</xml>';
