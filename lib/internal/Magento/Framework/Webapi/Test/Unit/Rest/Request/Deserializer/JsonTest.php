...
class JsonTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		 ...
		 */
		$this->decoderMock = $this->getMockBuilder ( 'Magento\Framework\Json\Decoder' )->disableOriginalConstructor ()->setMethods ( [ 
				'decode' 
		] )->getMock ();
		 ...
		 * Initialize SUT.
		 */
		$this->_jsonDeserializer = new \Magento\Framework\Webapi\Rest\Request\Deserializer\Json ( $this->decoderMock, $this->_appStateMock );
		parent::setUp ();
	}
	...
	public function testDeserialize() {
		...
		$this->decoderMock->expects ( $this->once () )->method ( 'decode' )->will ( $this->returnValue ( $expectedDecodedJson ) );
		/**
		 * Initialize SUT.
		 */
		$this->assertEquals ( $expectedDecodedJson, $this->_jsonDeserializer->deserialize ( $inputEncodedJson ), 'Deserialization from JSON to array is invalid.' );
	}
	public function testDeserializeInvalidEncodedBodyExceptionDeveloperModeOff() {
		/**
		 * Prepare mocks for SUT constructor.
		 */
		$this->decoderMock->expects ( $this->once () )->method ( 'decode' )->will ( $this->throwException ( new \Zend_Json_Exception () ) );
