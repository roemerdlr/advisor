...
class RequestTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		 ...
		 */
		$this->_deserializerFactory = $this->getMockBuilder ( 'Magento\Framework\Webapi\Rest\Request\DeserializerFactory' )->setMethods ( [ 
				'deserialize',
				'get' 
		...
		$this->_request = $this->getMock ( 'Magento\Framework\Webapi\Rest\Request', [ 
				'getHeader',
				'getMethod',
				'isGet',
				'isPost',
	...
	protected function _prepareSutForGetBodyParamsTest($params) {
		...
		$deserializer = $this->getMockBuilder ( 'Magento\Framework\Webapi\Rest\Request\Deserializer\Json' )->disableOriginalConstructor ()->setMethods ( [ 
				'deserialize' 
		] )->getMock ();
	 ...
	 *        	Appropriate content type for header value
	 * @param string|boolean $exceptionMessage
	 *        	\Exception message (boolean FALSE if exception is not expected)
	 */
	...
	public function providerContentType() {
		return [
				...
				[ 
						'application/dialog.dot-info7+xml',
						'application/dialog.dot-info7+xml' 
				],
				[ 
