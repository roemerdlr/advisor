...
class ResponseTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		 ...
		 */
		$this->rendererMock = $this->getMockBuilder ( 'Magento\Framework\Webapi\Rest\Response\Renderer\Json' )->disableOriginalConstructor ()->getMock ();
		$rendererFactoryMock = $this->getMockBuilder ( 'Magento\Framework\Webapi\Rest\Response\RendererFactory' )->disableOriginalConstructor ()->getMock ();
		$rendererFactoryMock->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValue ( $this->rendererMock ) );
		$this->errorProcessorMock = $this->getMockBuilder ( 'Magento\Framework\Webapi\ErrorProcessor' )->disableOriginalConstructor ()->getMock ();
		$this->appStateMock = $this->getMock ( 'Magento\Framework\App\State', [ ], [ ], '', false );
		
		/**
		 * Init SUP.
		 */
		$this->responseRest = new \Magento\Framework\Webapi\Rest\Response ( $rendererFactoryMock, $this->errorProcessorMock, $this->appStateMock );
	}
	protected function tearDown() {
		unset ( $this->responseRest, $this->appStateMock, $this->appStateMock, $this->rendererMock, $this->errorProcessorMock );
	}
	...
	public function testSetWebapiExceptionException() {
		 ...
		 */
		$apiException = new \Magento\Framework\Webapi\Exception ( new Phrase ( 'Exception message.' ), 0, \Magento\Framework\Webapi\Exception::HTTP_UNAUTHORIZED );
		 ...
		 */
		$this->assertTrue ( $this->responseRest->hasExceptionOfType ( 'Magento\Framework\Webapi\Exception' ), 'Magento\Framework\Webapi\Exception was not set.' );
	}
	...
	public function testSendResponseRenderMessagesException() {
		 ...
		 */
		$this->errorProcessorMock->expects ( $this->any () )->method ( 'maskException' )->will ( $this->throwException ( $logicException ) );
		/**
		 * Assert that renderException method will be executed once with specified parameters.
		 */
		$this->errorProcessorMock->expects ( $this->once () )->method ( 'renderException' )->with ( $logicException, \Magento\Framework\Webapi\Exception::HTTP_INTERNAL_ERROR );
		/**
	...
	public function testSendResponseRenderMessagesHttpNotAcceptable() {
		$exception = new \Magento\Framework\Webapi\Exception ( new Phrase ( 'Message' ), 0, \Magento\Framework\Webapi\Exception::HTTP_NOT_ACCEPTABLE );
		/**
		 * Mock error processor to throw \LogicException in maskException method.
		 */
		$this->errorProcessorMock->expects ( $this->any () )->method ( 'maskException' )->will ( $this->throwException ( $exception ) );
		/**
		 * Assert that renderException method will be executed once with specified parameters.
		 */
		$this->errorProcessorMock->expects ( $this->once () )->method ( 'renderException' )->with ( $exception, \Magento\Framework\Webapi\Exception::HTTP_NOT_ACCEPTABLE );
		/**
		 * Set exception to Rest response to get in to the _renderMessages method.
		 */
		$this->responseRest->setException ( new \Magento\Framework\Webapi\Exception ( new Phrase ( 'Message.' ), 0, \Magento\Framework\Webapi\Exception::HTTP_BAD_REQUEST ) );
		$this->responseRest->sendResponse ();
	}
	...
	public function testSendResponseWithException() {
		...
		$this->rendererMock->expects ( $this->any () )->method ( 'render' )->will ( $this->returnCallback ( [ 
				$this,
