...
class ServiceInputProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$cache = $this->getMockBuilder ( 'Magento\Framework\App\Cache\Type\Webapi' )->disableOriginalConstructor ()->getMock ();
		...
		$this->methodsMap = $objectManager->getObject ( 'Magento\Framework\Reflection\MethodsMap', [ 
				'cache' => $cache,
		...
		$this->serviceInputProcessor = $objectManager->getObject ( 'Magento\Framework\Webapi\ServiceInputProcessor', [ 
				'typeProcessor' => $typeProcessor,
	...
	public function testSimpleProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'simple', $data );
	...
	public function testNonExistentPropertiesWithDefaultArgumentValue() {
		$data = [ ];
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'simpleDefaultValue', $data );
		$this->assertNotNull ( $result );
	...
	public function testNestedDataProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'nestedData', $data );
		$this->assertNotNull ( $result );
	...
	public function testSimpleArrayProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'simpleArray', $data );
		$this->assertNotNull ( $result );
		/** @var array $result */
		$this->assertEquals ( 1, count ( $result ) );
		/** @var array $ids */
	...
	public function testAssociativeArrayProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'associativeArray', $data );
		$this->assertNotNull ( $result );
		/** @var array $result */
		$this->assertEquals ( 1, count ( $result ) );
	...
	public function testArrayOfDataObjectProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'dataArray', $data );
		$this->assertNotNull ( $result );
		/** @var array $result */
		$this->assertEquals ( 1, count ( $result ) );
	...
	public function testNestedSimpleArrayProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'nestedSimpleArray', $data );
		$this->assertNotNull ( $result );
		/** @var array $result */
		$this->assertEquals ( 1, count ( $result ) );
	...
	public function testNestedAssociativeArrayProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'nestedAssociativeArray', $data );
		$this->assertNotNull ( $result );
		/** @var array $result */
		$this->assertEquals ( 1, count ( $result ) );
	...
	public function testNestedArrayOfDataObjectProperties() {
		...
		$result = $this->serviceInputProcessor->process ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\TestService', 'nestedDataArray', $data );
		$this->assertNotNull ( $result );
		/** @var array $result */
		$this->assertEquals ( 1, count ( $result ) );
	...
	public function customAttributesDataProvider() {
		return [ 
				...
				'customAttributeObject' => [ 
						...
						'expectedObject' => $this->getObjectWithCustomAttributes ( 'SimpleArray', [ 
								'ids' => [ 
										1,
										2,
										3,
				...
				'customAttributeObjectNonExistentCustomAttributeCodeValue' => [ 
						...
						'inputData' => [ 
								'param' => [ 
										'customAttributes' => [ 
												[ 
														'attribute_code' => 'nonExistentAttributeCode',
														'value' => [ 
																'ids' => [ 
																		1,
																		2,
	...
	protected function getObjectWithCustomAttributes($type, $value = []) {
		...
		switch ($type) {
			case 'integer' :
				...
				break;
			case 'SimpleArray' :
				$customAttributeValue = $objectManager->getObject ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\SimpleArray', [ 
						'data' => $value 
				] );
				break;
			case 'Simple[]' :
				$dataObjectSimple1 = $objectManager->getObject ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\Simple', [ 
						'data' => $value [0] 
				] );
				$dataObjectSimple2 = $objectManager->getObject ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\Simple', [ 
						'data' => $value [1] 
				] );
				...
				break;
			case 'emptyData' :
				return $objectManager->getObject ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\ObjectWithCustomAttributes', [ 
						'data' => [ ] 
				] );
			default :
				return null;
		...
		return $objectManager->getObject ( 'Magento\Framework\Webapi\Test\Unit\ServiceInputProcessor\ObjectWithCustomAttributes', [ 
				'data' => [ 
						'custom_attributes' => [ 
								TestService::CUSTOM_ATTRIBUTE_CODE => $objectManager->getObject ( 'Magento\Framework\Api\AttributeValue', [ 
										'data' => [ 
												'attribute_code' => TestService::CUSTOM_ATTRIBUTE_CODE,
