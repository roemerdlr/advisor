...
class Uploader {
	...
	/**
	 * Upload type.
	 * Used to right handle $_FILES array.
	 *
	 ...
	 * By default it is original file name, but when
	...
	/**
	 * Max Image Width resolution in pixels.
	 * For image resizing on client side
	 */
	const MAX_IMAGE_WIDTH = 1920;
	
	/**
	 * Max Image Height resolution in pixels.
	 * For image resizing on client side
	 */
	...
	protected function _validateFile() {
		...
		foreach ( $this->_validateCallbacks as $params ) {
			if (is_object ( $params ['object'] ) && method_exists ( $params ['object'], $params ['method'] ) && is_callable ( [ 
					$params ['object'],
					$params ['method'] 
			] )) {
	 ...
	 *        	Method name of $callbackObject. It must
	...
	private function _createDestinationFolder($destinationFolder) {
		...
		if (! (@is_dir ( $destinationFolder ) || @mkdir ( $destinationFolder, DriverInterface::WRITEABLE_DIRECTORY_MODE, true ))) {
	...
	public static function getDispretionPath($fileName) {
		...
		while ( $char < 2 && $char < strlen ( $fileName ) ) {
			if (empty ( $dispertionPath )) {
				...
				$dispertionPath = self::_addDirSeparator ( $dispertionPath ) . ('.' == $fileName [$char] ? '_' : $fileName [$char]);
			}
			$char ++;
		}
