...
class InlineTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->renderer = new \Magento\Framework\Phrase\Renderer\Inline ( $this->translator, $this->provider, $this->loggerMock );
	}
	public function testRenderIfInlineTranslationIsAllowed() {
		...
		$this->assertEquals ( $result, $this->renderer->render ( [ 
				$text 
		], [ ] ) );
	}
	public function testRenderIfInlineTranslationIsNotAllowed() {
		...
		$this->assertEquals ( $text, $this->renderer->render ( [ 
				$text 
		], [ ] ) );
	}
