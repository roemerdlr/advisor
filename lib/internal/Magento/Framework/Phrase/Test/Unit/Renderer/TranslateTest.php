...
class TranslateTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_renderer = $objectManagerHelper->getObject ( 'Magento\Framework\Phrase\Renderer\Translate', [ 
	...
	public function testRender() {
		...
		$this->_translator->expects ( $this->exactly ( 2 ) )->method ( 'getData' )->will ( $this->returnValue ( [ 
				$translatedText => $translate 
		] ) );
		
		$this->assertEquals ( $translate, $this->_renderer->render ( [ 
				$translatedText 
		], [ ] ) );
		$this->assertEquals ( $text, $this->_renderer->render ( [ 
				$text 
		], [ ] ) );
	}
