...
class AbstractExtensibleModelTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->resourceMock = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ 
				'_construct',
				'getConnection',
				'__wakeup',
				'commit',
		...
		$this->resourceCollectionMock = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\AbstractDb' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		$this->metadataServiceMock = $this->getMockBuilder ( 'Magento\Framework\Api\MetadataServiceInterface' )->getMock ();
		$this->metadataServiceMock->expects ( $this->any () )->method ( 'getCustomAttributesMetadata' )->willReturn ( [ 
		...
		$extensionAttributesFactory = $this->getMockBuilder ( 'Magento\Framework\Api\ExtensionAttributesFactory' )->setMethods ( [ 
				'extractExtensionAttributes' 
		] )->disableOriginalConstructor ()->getMock ();
		$extensionAttributesFactory->expects ( $this->any () )->method ( 'extractExtensionAttributes' )->willReturnArgument ( 1 );
	...
	public function testCustomAttributesWithNonEmptyCustomAttributes() {
		...
		], $this->model->getCustomAttributes (), "One custom attribute expected" );
	...
	public function testGetDataWithCustomAttributes() {
		...
		$this->addCustomAttributesToModel ( $attributesAsArray, $this->model );
		$this->assertEquals ( $modelData, $this->model->getData (), 'All model data should be represented as a flat array, including custom attributes.' );
		foreach ( $modelData as $field => $value ) {
			$this->assertEquals ( $value, $this->model->getData ( $field ), "Model data item '{$field}' was retrieved incorrectly." );
		}
	}
	...
	public function testSetCustomAttributesAsLiterals() {
		...
		$this->model->setData ( \Magento\Framework\Api\CustomAttributesDataInterface::CUSTOM_ATTRIBUTES, [ 
