...
class RemoveActionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testIsAllowed($modelToCheck, $protectedModel, $secureArea, $expectedResult) {
		...
		$registryMock->expects ( $this->once () )->method ( 'registry' )->with ( 'isSecureArea' )->will ( $this->returnValue ( $secureArea ) );
		
		$model = new \Magento\Framework\Model\ActionValidator\RemoveAction ( $registryMock, [ 
