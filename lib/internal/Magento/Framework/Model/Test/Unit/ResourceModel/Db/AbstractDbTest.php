...
class AbstractDbTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_resourcesMock = $this->getMock ( '\Magento\Framework\App\ResourceConnection', [ ], [ ], '', false );
		
		$this->relationProcessorMock = $this->getMock ( '\Magento\Framework\Model\ResourceModel\Db\ObjectRelationProcessor', [ ], [ ], '', false );
		$this->transactionManagerMock = $this->getMock ( '\Magento\Framework\Model\ResourceModel\Db\TransactionManagerInterface' );
		$contextMock = $this->getMock ( '\Magento\Framework\Model\ResourceModel\Db\Context', [ ], [ ], '', false );
		$contextMock->expects ( $this->once () )->method ( 'getResources' )->willReturn ( $this->_resourcesMock );
		$contextMock->expects ( $this->once () )->method ( 'getObjectRelationProcessor' )->willReturn ( $this->relationProcessorMock );
		$contextMock->expects ( $this->once () )->method ( 'getTransactionManager' )->willReturn ( $this->transactionManagerMock );
	 ...
	 * @param
	 *        	$fieldNameType
	 * @param $expectedResult @dataProvider
	...
	public function testAddUniqueFieldArray() {
		$this->assertInstanceOf ( '\Magento\Framework\Model\ResourceModel\Db\AbstractDb', $this->_model->addUniqueField ( [ 
				'someField' 
		] ) );
	}
	...
	public function testGetIdFieldname() {
		$data = 'MainTableName';
		$idFieldNameProperty = new \ReflectionProperty ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', '_idFieldName' );
	...
	public function testGetMainTable($tableName, $expectedResult) {
		...
		$this->_resourcesMock->expects ( $this->once () )->method ( 'getTableName' )->with ( $expectedResult )->will ( $this->returnValue ( $expectedResult ) );
	...
	public function testGetTable() {
		...
		$tablesProperty->setValue ( $this->_model, [ 
				$data 
		] );
	...
	public function testGetChecksum($checksum, $expected) {
		...
		$connectionMock->expects ( $this->once () )->method ( 'getTablesChecksum' )->with ( $checksum )->will ( $this->returnValue ( [ 
				$checksum => 'checksum' 
		] ) );
	...
	public function testResetUniqueField() {
		...
		$uniqueFields->setValue ( $this->_model, [ 
				'uniqueField1',
				'uniqueField2' 
		] );
	...
	public function testGetUniqueFields() {
		$uniqueFieldsReflection = new \ReflectionProperty ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', '_uniqueFields' );
	...
	public function testLoad() {
		...
		$abstractModelMock = $this->getMockForAbstractClass ( '\Magento\Framework\Model\AbstractModel', [ 
				$contextMock,
				$registryMock 
		], '', false, true, true, [ 
				'__wakeup' 
	...
	public function testDelete() {
		...
		$abstractModelMock->expects ( $this->once () )->method ( 'getData' )->willReturn ( [ 
				'data' => 'value' 
		] );
		...
		$this->relationProcessorMock->expects ( $this->once () )->method ( 'delete' )->with ( $this->transactionManagerMock, $connectionMock, 'tableName', 'idFieldName', [ 
				'data' => 'value' 
		] );
	...
	public function testGetDataChanged($getOriginData, $expected) {
		...
		$this->_resourcesMock->expects ( $this->once () )->method ( 'getTableName' )->with ( 'table' )->will ( $this->returnValue ( 'tableName' ) );
		...
		$connectionInterfaceMock->expects ( $this->any () )->method ( 'describeTable' )->with ( 'tableName' )->will ( $this->returnValue ( [ 
				'tableName' 
		] ) );
	...
	public function testPrepareDataForUpdate() {
		...
		$registryMock = $this->getMock ( '\Magento\Framework\Registry', [ ], [ ], '', false );
		$resourceMock = $this->getMock ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb', [ 
				'_construct',
				'getConnection',
				'__wakeup',
				'getIdFieldName' 
	...
	public function testSaveNewObject($pkIncrement) {
		 ...
		 */
		$model = $this->getMockBuilder ( 'Magento\Framework\Model\ResourceModel\Db\AbstractDb' )->disableOriginalConstructor ()->setMethods ( [ 
				'_prepareDataForSave',
				'getIdFieldName',
				'getConnection',
				'getMainTable' 
		...
		// Mocked behavior
		$connectionMock = $this->getMockBuilder ( '\Magento\Framework\DB\Adapter\AdapterInterface' )->disableOriginalConstructor ()->setMethods ( [ 
				'lastInsertId' 
		] )->getMockForAbstractClass ();
		$getConnectionInvokedCount = $pkIncrement ? 2 : 1;
		$model->expects ( $this->exactly ( $getConnectionInvokedCount ) )->method ( 'getConnection' )->willReturn ( $connectionMock );
		
		$idFieldName = 'id_field_name';
		$model->expects ( $this->once () )->method ( '_prepareDataForSave' )->willReturn ( [ 
				$idFieldName => 'id' 
		] );
		...
		$model->expects ( $this->exactly ( $getIdFieldNameInvokedCount ) )->method ( 'getIdFieldName' )->willReturn ( $idFieldName );
		
		// Only set object id if not PK autoincrement
		$setIdInvokedCount = $pkIncrement ? 1 : 0;
		$inputObject = $this->getMockBuilder ( '\Magento\Framework\Model\AbstractModel' )->disableOriginalConstructor ()->getMock ();
		...
		$reflectionMethod->invokeArgs ( $model, [ 
				$inputObject 
		] );
	}
