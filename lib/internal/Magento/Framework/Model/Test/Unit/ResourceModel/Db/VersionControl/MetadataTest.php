...
class MetadataTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->model = $this->getMock ( 'Magento\Framework\Model\AbstractModel', [ ], [ ], '', false );
		$this->resource = $this->getMockForAbstractClass ( 'Magento\Framework\DB\Adapter\AdapterInterface', [ ], "", false, false, true, [ 
				'getConnection',
				'getMainTable' 
		] );
		$this->connection = $this->getMockForAbstractClass ( 'Magento\Framework\DB\Adapter\AdapterInterface', [ ], "", false, false );
		$this->model->expects ( $this->any () )->method ( 'getResource' )->willReturn ( $this->resource );
		$this->resource->expects ( $this->any () )->method ( 'getConnection' )->willReturn ( $this->connection );
		$this->entityMetadata = $objectManager->getObject ( 'Magento\Framework\Model\ResourceModel\Db\VersionControl\Metadata' );
	}
	public function testGetFields() {
		$mainTable = 'main_table';
		$expectedDescribedTable = [ 
				'described_table' => null 
		];
