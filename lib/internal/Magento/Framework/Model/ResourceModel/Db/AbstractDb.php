...
abstract class AbstractDb extends AbstractResource {
	...
	public function delete(\Magento\Framework\Model\AbstractModel $object) {
		...
		try {
			...
			$this->objectRelationProcessor->delete ( $this->transactionManager, $connection, $this->getMainTable (), $this->getConnection ()->quoteInto ( $this->getIdFieldName () . '=?', $object->getId () ), $object->getData () );
	 ...
	 * @param \Magento\Framework\Model\AbstractModel $object        	
	 * @return $this
	 * @throws AlreadyExistsException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	protected function _checkUnique(\Magento\Framework\Model\AbstractModel $object) {
		...
		if (! empty ( $existent )) {
			if (count ( $existent ) == 1) {
				$error = new \Magento\Framework\Phrase ( '%1 already exists.', [ 
						$existent [0] 
				] );
			} else {
				$error = new \Magento\Framework\Phrase ( '%1 already exist.', [ 
						implode ( ', ', $existent ) 
				] );
			}
	...
	protected function updateObject(\Magento\Framework\Model\AbstractModel $object) {
		...
		if ($this->_isPkAutoIncrement) {
			...
			if ($this->getConnection ()->fetchOne ( $select ) !== false) {
				...
				$this->getConnection ()->insert ( $this->getMainTable (), $this->_prepareDataForSave ( $object ) );
			}
		}
	}
