...
class Validator extends \Zend_Validate_Abstract {
	...
	public function __construct(\Magento\Framework\Config\DomFactory $domConfigFactory, UrnResolver $urnResolver) {
		...
		$this->_xsdSchemas = [ 
				self::LAYOUT_SCHEMA_PAGE_HANDLE => $urnResolver->getRealPath ( 'urn:magento:framework:View/Layout/etc/page_layout.xsd' ),
				self::LAYOUT_SCHEMA_MERGED => $urnResolver->getRealPath ( 'urn:magento:framework:View/Layout/etc/layout_merged.xsd' ) 
		];
	}
