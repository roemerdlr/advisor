...
class MessageConfigurationsPool {
	 ...
	 */
	public function __construct(array $configurationsMap = []) {
		array_walk ( $configurationsMap, function (array &$configuration) {
			if (! isset ( $configuration ['renderer'] ) || ! is_string ( $configuration ['renderer'] )) {
				throw new \InvalidArgumentException ( 'Renderer should be defined.' );
			}
			
			if (isset ( $configuration ['data'] ) && ! is_array ( $configuration ['data'] )) {
	...
	public function getMessageConfiguration($identifier) {
		return ! isset ( $this->configurationsMap [$identifier] ) ? null : $this->configurationsMap [$identifier];
	}
}
