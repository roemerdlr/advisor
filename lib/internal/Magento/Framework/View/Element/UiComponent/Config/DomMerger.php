...
class DomMerger implements DomMergerInterface {
	/**
	 * Format of items in errors array to be used by default.
	 * Available placeholders - fields of \LibXMLError.
	 */
	 ...
	 */
	public function __construct(ValidationStateInterface $validationState, $schema, $isMergeSimpleXMLElement = false, array $contextXPath = [], array $idAttributes = []) {
					...
					if ($jLength > 0) {
						for($j = 0; $j < $jLength; ++ $j) {
							...
							if ($this->isMergeContext ( $insertedXPath )) {
								...
								} else if (! $this->isContextXPath ( [ 
										$rootItemXPath,
										$insertedXPath 
								] ) && ! $this->hasIdAttribute ( $rootItem ) && ! $this->hasIdAttribute ( $insertedItem )) {
	...
	public function createDomDocument($xml) {
		...
		if ($this->validationState->isValidationRequired () && $this->schema) {
			...
			if (count ( $errors )) {
				throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( implode ( "\n", $errors ) ) );
			}
		}
	...
	protected function renderErrorMessage(\LibXMLError $errorInfo) {
		...
		if (strpos ( $result, '%' ) !== false) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Error format "' . static::ERROR_FORMAT_DEFAULT . '" contains unsupported placeholders.' ) );
		}
	...
	public function getDom() {
		if (! isset ( $this->domDocument )) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Object DOMDocument should be created.' ) );
		}
