...
class Layout extends \Magento\Framework\Simplexml\Config implements \Magento\Framework\View\LayoutInterface {
	...
	public function getUpdate() {
		if (! $this->_update) {
			...
			$this->_update = $this->_processorFactory->create ( [ 
					'theme' => $theme 
			] );
		}
		return $this->_update;
	...
	public function renderElement($name, $useCache = true) {
		...
		$this->_eventManager->dispatch ( 'core_layout_render_element', [ 
				'element_name' => $name,
				'layout' => $this,
	...
	protected function displayElement($name) {
		...
		if ($display === '' || $display === false || $display === null || filter_var ( $display, FILTER_VALIDATE_BOOLEAN )) {
	...
	public function getBlockSingleton($type) {
		if (empty ( $type )) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Invalid block type' ) );
		}
	...
	public function executeRenderer($namespace, $staticType, $dynamicType, $data = []) {
		...
		if ($options = $this->getRendererOptions ( $namespace, $staticType, $dynamicType )) {
			...
			$block = $this->createBlock ( $options ['type'], '' )->setData ( $data )->assign ( $dictionary )->setTemplate ( $options ['template'] )->assign ( $data );
