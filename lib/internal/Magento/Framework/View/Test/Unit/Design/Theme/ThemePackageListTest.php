...
class ThemePackageListTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetThemeNonexistent() {
		$themeKey = 'theme';
		$this->registrar->expects ( $this->once () )->method ( 'getPath' )->with ( ComponentRegistrar::THEME, $themeKey )->willReturn ( null );
	...
	public function testGetThemes() {
		$this->registrar->expects ( $this->once () )->method ( 'getPaths' )->with ( ComponentRegistrar::THEME )->willReturn ( [ 
				'theme1' => 'path1',
				'theme2' => 'path2' 
		] );
		$themePackage = $this->getMock ( '\Magento\Framework\View\Design\Theme\ThemePackage', [ ], [ ], '', false );
		$this->factory->expects ( $this->exactly ( 2 ) )->method ( 'create' )->withConsecutive ( [ 
				'theme1',
				'path1' 
		], [ 
				'theme2',
