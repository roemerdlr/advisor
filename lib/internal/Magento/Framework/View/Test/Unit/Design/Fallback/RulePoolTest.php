...
class RulePoolTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$filesystemMock->expects ( $this->any () )->method ( 'getDirectoryRead' )->will ( $this->returnCallback ( function ($code) {
			$dirMock = $this->getMockForAbstractClass ( 'Magento\Framework\Filesystem\Directory\ReadInterface' );
			$dirMock->expects ( $this->any () )->method ( 'getAbsolutePath' )->will ( $this->returnCallback ( function ($path) use($code) {
		...
		$simpleFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $rule ) );
		
		$themeFactory = $this->getMock ( 'Magento\Framework\View\Design\Fallback\Rule\ThemeFactory', [ ], [ ], '', false );
		$themeFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $rule ) );
		$moduleFactory = $this->getMock ( 'Magento\Framework\View\Design\Fallback\Rule\ModuleFactory', [ ], [ ], '', false );
		$moduleFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $rule ) );
		$moduleSwitchFactory = $this->getMock ( 'Magento\Framework\View\Design\Fallback\Rule\ModularSwitchFactory', [ ], [ ], '', false );
		$moduleSwitchFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $rule ) );
		$this->model = new RulePool ( $filesystemMock, $simpleFactory, $themeFactory, $moduleFactory, $moduleSwitchFactory );
