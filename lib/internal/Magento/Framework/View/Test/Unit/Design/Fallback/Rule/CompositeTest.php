...
class CompositeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetPatternDirs() {
		...
		$ruleOne->expects ( $this->once () )->method ( 'getPatternDirs' )->with ( $inputParams )->will ( $this->returnValue ( [ 
				'rule_one/path/one',
				'rule_one/path/two' 
		] ) );
		
		$ruleTwo = $this->getMockForAbstractClass ( '\Magento\Framework\View\Design\Fallback\Rule\RuleInterface' );
		$ruleTwo->expects ( $this->once () )->method ( 'getPatternDirs' )->with ( $inputParams )->will ( $this->returnValue ( [ 
				'rule_two/path/one',
				'rule_two/path/two' 
		] ) );
		
		$object = new Composite ( [ 
				$ruleOne,
				$ruleTwo 
		] );
