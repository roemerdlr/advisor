...
class FactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreate() {
		...
		$themeMock->expects ( $this->any () )->method ( 'getType' )->will ( $this->returnValue ( \Magento\Framework\View\Design\ThemeInterface::TYPE_VIRTUAL ) );
		
		$newThemeMock = $this->getMock ( 'Magento\Theme\Model\Theme', [ ], [ ], '', false );
		
		$objectManager = $this->getMock ( 'Magento\Framework\ObjectManagerInterface' );
		$objectManager->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\View\Design\Theme\Domain\VirtualInterface', [ 
				'theme' => $themeMock 
	...
	public function testCreateWithWrongThemeType() {
		...
		$this->setExpectedException ( 'Magento\Framework\Exception\LocalizedException', sprintf ( 'Invalid type of theme domain model "%s"', $wrongThemeType ) );
