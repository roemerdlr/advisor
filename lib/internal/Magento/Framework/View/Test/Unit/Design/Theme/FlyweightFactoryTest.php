...
class FlyweightFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreateById($path, $expectedId) {
		...
		$this->themeProviderMock->expects ( $this->once () )->method ( 'getThemeById' )->with ( $expectedId )->will ( $this->returnValue ( $theme ) );
	...
	public function testCreateByPath() {
		...
		$this->themeProviderMock->expects ( $this->once () )->method ( 'getThemeByFullPath' )->with ( 'frontend/frontend/Magento/luma' )->will ( $this->returnValue ( $theme ) );
