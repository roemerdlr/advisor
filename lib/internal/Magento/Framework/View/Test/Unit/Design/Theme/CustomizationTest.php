...
class CustomizationTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->fileProvider = $this->getMock ( 'Magento\Framework\View\Design\Theme\FileProviderInterface', [ ], [ ], '', false );
		$collectionFactory = $this->getMock ( 'Magento\Theme\Model\ResourceModel\Theme\File\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		...
		$this->theme = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup',
				'save',
				'load' 
		], [ ], '', false );
	...
	public function testGetFiles() {
		$this->fileProvider->expects ( $this->once () )->method ( 'getItems' )->with ( $this->theme )->will ( $this->returnValue ( [ ] ) );
	...
	public function testGetFilesByType() {
		$type = 'sample-type';
		$this->fileProvider->expects ( $this->once () )->method ( 'getItems' )->with ( $this->theme, [ 
				'file_type' => $type 
	...
	public function testGenerationOfFileInfo() {
		$file = $this->getMock ( 'Magento\Theme\Model\Theme\File', [ 
				'__wakeup',
				'getFileInfo' 
		], [ ], '', false );
		$file->expects ( $this->once () )->method ( 'getFileInfo' )->will ( $this->returnValue ( [ 
				'sample-generation' 
		] ) );
		$this->assertEquals ( [ 
				[ 
	...
	public function testDelete() {
		...
		$this->assertInstanceOf ( 'Magento\Framework\View\Design\Theme\CustomizationInterface', $this->model->delete ( [ 
				1 
		] ) );
	}
}
