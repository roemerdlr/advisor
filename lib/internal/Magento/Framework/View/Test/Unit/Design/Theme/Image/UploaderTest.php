...
class UploaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$adapterFactory->expects ( $this->once () )->method ( 'create' )->will ( $this->returnValue ( $this->_transferAdapterMock ) );
		
		$uploaderFactory = $this->getMock ( 'Magento\Framework\File\UploaderFactory', [ 
				'create' 
		], [ ], '', false );
		$uploaderFactory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValue ( $this->_fileUploader ) );
		
		$this->_model = new \Magento\Framework\View\Design\Theme\Image\Uploader ( $this->_filesystemMock, $adapterFactory, $uploaderFactory );
	}
