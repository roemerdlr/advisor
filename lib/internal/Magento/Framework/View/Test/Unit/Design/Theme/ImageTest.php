...
class ImageTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_filesystemMock = $this->getMock ( 'Magento\Framework\Filesystem', [ 
				'getDirectoryWrite',
				'__wakeup' 
		], [ ], '', false, false );
		$this->_filesystemMock->expects ( $this->at ( 0 ) )->method ( 'getDirectoryWrite' )->with ( DirectoryList::MEDIA )->will ( $this->returnValue ( $this->_mediaDirectoryMock ) );
		$this->_filesystemMock->expects ( $this->at ( 1 ) )->method ( 'getDirectoryWrite' )->with ( DirectoryList::ROOT )->will ( $this->returnValue ( $this->_rootDirectoryMock ) );
		...
		$this->_themeMock = $this->getMock ( 'Magento\Theme\Model\Theme', [ 
				'__wakeup' 
		], [ ], '', false, false );
		$this->_uploaderMock = $this->getMock ( 'Magento\Framework\View\Design\Theme\Image\Uploader', [ ], [ ], '', false, false );
	...
	protected function _getImagePathMock() {
		...
		$imagePathMock->expects ( $this->any () )->method ( 'getPreviewImageDefaultUrl' )->will ( $this->returnValue ( $testBaseUrl . 'test_default_preview.png' ) );
		
		$testBaseDir = '/media_path/';
		$imagePathMock->expects ( $this->any () )->method ( 'getImagePreviewDirectory' )->will ( $this->returnValue ( $testBaseDir . 'theme/preview' ) );
		$imagePathMock->expects ( $this->any () )->method ( 'getTemporaryDirectory' )->will ( $this->returnValue ( $testBaseDir . 'tmp' ) );
	...
	public function testCreatePreviewImageCopy() {
		...
		$this->_mediaDirectoryMock->expects ( $this->once () )->method ( 'getRelativePath' )->will ( $this->returnArgument ( 0 ) );
		
		$this->_rootDirectoryMock->expects ( $this->once () )->method ( 'getRelativePath' )->with ( $previewImage )->will ( $this->returnValue ( $relativePath ) );
		
		$this->_rootDirectoryMock->expects ( $this->once () )->method ( 'copyFile' )->with ( $relativePath, $this->anything () )->will ( $this->returnValue ( true ) );
		
		$themeImageMock = $this->getMockBuilder ( 'Magento\Framework\View\Design\Theme\Image' )->disableOriginalConstructor ()->setMethods ( [ 
				'getPreviewImagePath' 
		] )->getMock ();
		$themeImageMock->expects ( $this->atLeastOnce () )->method ( 'getPreviewImagePath' )->will ( $this->returnValue ( $previewImage ) );
		
		$themeMock = $this->getMockBuilder ( 'Magento\Theme\Model\Theme' )->disableOriginalConstructor ()->setMethods ( [ 
				'getThemeImage',
				'getPreviewImage',
				'__wakeup' 
		] )->getMock ();
		$themeMock->expects ( $this->atLeastOnce () )->method ( 'getPreviewImage' )->will ( $this->returnValue ( $previewImage ) );
		$themeMock->expects ( $this->atLeastOnce () )->method ( 'getThemeImage' )->will ( $this->returnValue ( $themeImageMock ) );
	...
	public function testGetPreviewImageUrl() {
		...
		$this->imagePathMock->expects ( $this->any () )->method ( 'getPreviewImageUrl' )->with ( $this->_themeMock )->will ( $this->returnValue ( 'http://localhost/media_path/preview/image.png' ) );
