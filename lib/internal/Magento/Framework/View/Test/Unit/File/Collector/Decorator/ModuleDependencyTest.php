...
class ModuleDependencyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_moduleListMock->expects ( $this->any () )->method ( 'getNames' )->will ( $this->returnValue ( [ 
				'Fixture_ModuleB',
				'Fixture_ModuleA' 
		] ) );
		$this->_model = new \Magento\Framework\View\File\Collector\Decorator\ModuleDependency ( $this->_fileSource, $this->_moduleListMock );
	}
	...
	public function testGetFiles(array $fixtureFiles, array $expectedFiles, $message) {
		...
		$this->_fileSource->expects ( $this->once () )->method ( 'getFiles' )->with ( $theme, '*.xml' )->will ( $this->returnValue ( $fixtureFiles ) );
