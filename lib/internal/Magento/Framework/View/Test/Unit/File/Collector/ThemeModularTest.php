...
class ThemeModularTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->themeDirectory = $this->getMock ( 'Magento\Framework\Filesystem\Directory\Read', [ 
				'getAbsolutePath',
				'search' 
		], [ ], '', false );
		...
		$this->model = new \Magento\Framework\View\File\Collector\ThemeModular ( $this->fileFactory, $this->readDirFactory, $this->componentRegistrar, $this->pathPatternHelperMock, 'subdir' );
	}
	...
	public function testGetFiles($files, $filePath, $pathPattern) {
		...
		foreach ( $files as $key => $file ) {
			...
			$this->fileFactory->expects ( $this->at ( $key ) )->method ( 'create' )->with ( sprintf ( $handlePath, $file ['module'], $file ['handle'] ), $file ['module'], $theme )->willReturn ( $checkResult [$key] );
		}
