...
class HeadTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->headGenerator = $objectManagerHelper->getObject ( 'Magento\Framework\View\Page\Config\Generator\Head', [ 
	...
	public function testProcess() {
		...
		$this->pageConfigMock->expects ( $this->at ( 0 ) )->method ( 'addRemotePageAsset' )->with ( 'remoteCss', 'css', [ 
				'attributes' => [ 
						'media' => 'all' 
				] 
		] );
		$this->pageConfigMock->expects ( $this->at ( 1 ) )->method ( 'addRemotePageAsset' )->with ( 'remoteLink', Head::VIRTUAL_CONTENT_TYPE_LINK, [ 
				'attributes' => [ 
						'media' => 'all' 
				] 
		] );
		$this->pageConfigMock->expects ( $this->once () )->method ( 'addPageAsset' )->with ( 'name', [ 
				'attributes' => [ 
						'media' => 'print' 
				],
				'ie_condition' => 'lt IE 7' 
		...
		$this->pageConfigMock->expects ( $this->exactly ( 2 ) )->method ( 'setMetadata' )->withConsecutive ( [ 
				'name1',
				'content1' 
		], [ 
				'name2',
		...
		$this->pageConfigMock->expects ( $this->exactly ( 3 ) )->method ( 'setElementAttribute' )->withConsecutive ( [ 
				PageConfig::ELEMENT_TYPE_BODY,
				'body_attr_1',
				'body_value_1' 
		], [ 
