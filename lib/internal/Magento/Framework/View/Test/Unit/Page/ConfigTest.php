...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$locale->expects ( $this->any () )->method ( 'getLocale' )->willReturn ( Resolver::DEFAULT_LOCALE );
		$this->model = (new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this ))->getObject ( 'Magento\Framework\View\Page\Config', [ 
	...
	public function testSetBuilder() {
		$this->assertInstanceOf ( 'Magento\Framework\View\Page\Config', $this->model->setBuilder ( $this->builder ) );
	}
	public function testBuild() {
		$this->model->setBuilder ( $this->builder );
		$this->builder->expects ( $this->once () )->method ( 'build' )->will ( $this->returnValue ( 'Magento\Framework\View\LayoutInterface' ) );
	...
	public function testMediaTypeEmpty() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'design/head/default_media_type', 'store' )->will ( $this->returnValue ( 'default_media_type' ) );
	...
	public function testCharsetEmpty() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'design/head/default_charset', 'store' )->will ( $this->returnValue ( 'default_charset' ) );
	...
	public function testDescriptionEmpty() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'design/head/default_description', 'store' )->will ( $this->returnValue ( 'default_description' ) );
	...
	public function testKeywordsEmpty() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'design/head/default_keywords', 'store' )->will ( $this->returnValue ( 'default_keywords' ) );
	...
	public function testRobotsEmpty() {
		...
		$this->scopeConfig->expects ( $this->once () )->method ( 'getValue' )->with ( 'design/search_engine_robots/default_robots', 'store' )->will ( $this->returnValue ( 'default_robots' ) );
	...
	public function testAddPageAsset($file, $properties, $name, $expectedName) {
		$this->assetRepo->expects ( $this->once () )->method ( 'createAsset' )->with ( $file )->will ( $this->returnValue ( $this->asset ) );
		$this->pageAssets->expects ( $this->once () )->method ( 'add' )->with ( $expectedName, $this->asset, $properties );
		$this->assertInstanceOf ( 'Magento\Framework\View\Page\Config', $this->model->addPageAsset ( $file, $properties, $name ) );
	}
	...
	public function testAddRemotePageAsset($url, $contentType, $properties, $name, $expectedName) {
		$this->assetRepo->expects ( $this->once () )->method ( 'createRemoteAsset' )->with ( $url, $contentType )->will ( $this->returnValue ( $this->remoteAsset ) );
		$this->pageAssets->expects ( $this->once () )->method ( 'add' )->with ( $expectedName, $this->remoteAsset, $properties );
		$this->assertInstanceOf ( 'Magento\Framework\View\Page\Config', $this->model->addRemotePageAsset ( $url, $contentType, $properties, $name ) );
	}
	...
	public function testAddRss() {
		...
		$href = 'http://test.com';
		$expected = [ 
				'attributes' => 'rel="alternate" type="application/rss+xml" title="test title"' 
		];
