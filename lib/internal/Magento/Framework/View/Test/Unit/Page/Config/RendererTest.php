...
class RendererTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->assetsCollection = $this->getMockBuilder ( 'Magento\Framework\View\Asset\GroupedCollection' )->setMethods ( [ 
				'getGroups' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->assetInterfaceMock = $this->getMockForAbstractClass ( 'Magento\Framework\View\Asset\AssetInterface' );
		
		$this->titleMock = $this->getMockBuilder ( 'Magento\Framework\View\Page\Title' )->setMethods ( [ 
				'set',
				'get' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->objectManagerHelper = new ObjectManager ( $this );
		$this->renderer = $this->objectManagerHelper->getObject ( 'Magento\Framework\View\Page\Config\Renderer', [ 
	...
	public function testPrepareFavicon() {
		$filePath = 'file';
		$this->pageConfigMock->expects ( $this->exactly ( 3 ) )->method ( 'getFaviconFile' )->willReturn ( $filePath );
		
		$this->pageConfigMock->expects ( $this->exactly ( 2 ) )->method ( 'addRemotePageAsset' )->withConsecutive ( [ 
				$filePath,
	...
	public function testPrepareFaviconDefault() {
		...
		$this->pageConfigMock->expects ( $this->exactly ( 2 ) )->method ( 'addPageAsset' )->withConsecutive ( [ 
				$defaultFilePath,
				[ 
						'attributes' => [ 
								'rel' => 'icon',
	...
	public function testRenderAssets($groupOne, $groupTwo, $expectedResult) {
		...
		$groupMockOne->expects ( $this->any () )->method ( 'getProperty' )->willReturnMap ( [ 
				[ 
						GroupedCollection::PROPERTY_CAN_MERGE,
						true 
				],
				[ 
						GroupedCollection::PROPERTY_CONTENT_TYPE,
						$groupOne ['type'] 
				],
				[ 
						'attributes',
		...
		$groupMockTwo->expects ( $this->any () )->method ( 'getProperty' )->willReturnMap ( [ 
				[ 
						GroupedCollection::PROPERTY_CAN_MERGE,
						true 
				],
		...
		$this->assetsCollection->expects ( $this->once () )->method ( 'getGroups' )->willReturn ( [ 
				$groupMockOne,
				$groupMockTwo 
		] );
