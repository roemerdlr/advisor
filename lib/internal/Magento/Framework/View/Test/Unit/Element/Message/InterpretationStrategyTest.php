...
class InterpretationStrategyTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->renderersPool = $this->getMockBuilder ( 'Magento\Framework\View\Element\Message\Renderer\RenderersPool' )->disableOriginalConstructor ()->getMock ();
		$this->messageConfigurationsPool = $this->getMockBuilder ( 'Magento\Framework\View\Element\Message\MessageConfigurationsPool' )->disableOriginalConstructor ()->getMock ();
		$this->messageMock = $this->getMock ( 'Magento\Framework\Message\MessageInterface' );
		$this->renderer = $this->getMock ( 'Magento\Framework\View\Element\Message\Renderer\RendererInterface' );
		
		$this->interpretationStrategy = new InterpretationStrategy ( $this->renderersPool, $this->messageConfigurationsPool );
	}
	public function testInterpret() {
		...
		$renderer = $this->getMock ( 'Magento\Framework\View\Element\Message\Renderer\RendererInterface' );
		...
		$this->renderersPool->expects ( static::once () )->method ( 'get' )->with ( $messageConfiguration ['renderer'] )->willReturn ( $renderer );
		$renderer->expects ( static::once () )->method ( 'render' )->with ( $this->messageMock, $messageConfiguration ['data'] )->willReturn ( $renderedMessage );
		
		static::assertSame ( $renderedMessage, $this->interpretationStrategy->interpret ( $this->messageMock ) );
	}
	public function testInterpretNoConfigurationException() {
		$identifier = 'Prophesy';
		
		$this->messageMock->expects ( static::once () )->method ( 'getIdentifier' )->willReturn ( $identifier );
		$this->messageConfigurationsPool->expects ( static::once () )->method ( 'getMessageConfiguration' )->with ( $identifier )->willReturn ( null );
		
		$this->setExpectedException ( 'LogicException' );
		
		$this->renderersPool->expects ( static::never () )->method ( 'get' );
