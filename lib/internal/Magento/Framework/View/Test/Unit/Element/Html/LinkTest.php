...
class LinkTest extends \PHPUnit_Framework_TestCase {
	...
	public function getLinkAttributesDataProvider() {
		...
		$escaperMock = $this->getMockBuilder ( 'Magento\Framework\Escaper' )->setMethods ( [ 
				'escapeHtml' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$escaperMock->expects ( $this->any () )->method ( 'escapeHtml' )->will ( $this->returnArgument ( 0 ) );
		
		$urlBuilderMock = $this->getMockBuilder ( 'Magento\Framework\UrlInterface' )->setMethods ( [ 
				'getUrl' 
		] )->disableOriginalConstructor ()->getMockForAbstractClass ();
		
		$urlBuilderMock->expects ( $this->any () )->method ( 'getUrl' )->will ( $this->returnArgument ( 'http://site.com/link.html' ) );
		
		$validtorMock = $this->getMockBuilder ( 'Magento\Framework\View\Element\Template\File\Validator' )->setMethods ( [ 
				'isValid' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$scopeConfigMock = $this->getMockBuilder ( 'Magento\Framework\App\Config' )->setMethods ( [ 
				'isSetFlag' 
		] )->disableOriginalConstructor ()->getMock ();
		...
		$contextMock = $this->getMockBuilder ( 'Magento\Framework\View\Element\Template\Context' )->setMethods ( [ 
				'getEscaper',
				'getUrlBuilder',
				'getValidator',
				'getResolver',
		...
		$linkWithAttributes = $objectManagerHelper->getObject ( 'Magento\Framework\View\Element\Html\Link', [ 
				'context' => $contextMock 
		] );
		/** @var \Magento\Framework\View\Element\Html\Link $linkWithoutAttributes */
		$linkWithoutAttributes = $objectManagerHelper->getObject ( 'Magento\Framework\View\Element\Html\Link', [ 
				'context' => $contextMock 
		] );
