...
class EscapeRendererTest extends \PHPUnit_Framework_TestCase {
	public function testInterpret() {
		...
		$message->expects ( static::once () )->method ( 'getText' )->willReturn ( $messageText );
		$escaper->expects ( static::once () )->method ( 'escapeHtml' )->with ( $messageText )->willReturn ( $escapedMessageText );
