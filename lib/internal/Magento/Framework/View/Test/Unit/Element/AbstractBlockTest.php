...
class AbstractBlockTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetUiId($expectedResult, $nameInLayout, $methodArguments) {
		...
		$this->assertEquals ( $expectedResult, call_user_func_array ( [ 
				$block,
				'getUiId' 
		], $methodArguments ) );
	}
	...
	public function testGetVar() {
		...
		$config = $this->getMock ( 'Magento\Framework\Config\View', [ 
				'getVarValue' 
		], [ ], '', false );
		$module = uniqid ();
	...
	public function testIsScopePrivate() {
		...
		$block = $this->getMockForAbstractClass ( 'Magento\Framework\View\Element\AbstractBlock', [ 
				'context' => $contextMock 
		] );
