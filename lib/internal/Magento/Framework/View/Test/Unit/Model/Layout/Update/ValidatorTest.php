...
class ValidatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _createValidator($layoutUpdate, $isSchemaValid = true) {
		...
		$params = [ 
				'xml' => '<layout xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">' . trim ( $layoutUpdate ) . '</layout>',
		...
		$model = $this->_objectHelper->getObject ( 'Magento\Framework\View\Model\Layout\Update\Validator', [ 
	...
	public function testIsValidNotSecurityCheck($layoutUpdate, $isValid, $expectedResult, $messages) {
		...
		$this->assertEquals ( $expectedResult, $model->isValid ( $layoutUpdate, Validator::LAYOUT_SCHEMA_PAGE_HANDLE, false ) );
	...
	public function testIsValidSecurityCheck($layoutUpdate, $expectedResult, $messages) {
		...
		$this->assertEquals ( $model->isValid ( $layoutUpdate, Validator::LAYOUT_SCHEMA_PAGE_HANDLE, true ), $expectedResult );
