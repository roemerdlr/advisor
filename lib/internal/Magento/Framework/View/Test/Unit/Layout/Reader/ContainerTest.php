...
class ContainerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->container = $this->objectManagerHelper->getObject ( 'Magento\Framework\View\Layout\Reader\Container', [ 
	 ...
	 */
	public function testProcess($elementCurrent, $containerName, $structureElement, $expectedData, $getStructureCondition, $setStructureCondition, $setRemoveCondition) {
		...
		$scheduledStructureMock->expects ( $getStructureCondition )->method ( 'getStructureElementData' )->with ( $containerName )->willReturn ( $structureElement );
		$scheduledStructureMock->expects ( $setStructureCondition )->method ( 'setStructureElementData' )->with ( $containerName, $expectedData )->willReturnSelf ();
		$scheduledStructureMock->expects ( $setRemoveCondition )->method ( 'setElementToRemoveList' )->with ( $containerName );
	...
	public function processDataProvider() {
		return [ 
				'container' => [ 
						'elementCurrent' => $this->getElement ( '<container name="container" id="id_add" tag="body"/>', 'container' ),
				...
				'referenceContainer' => [ 
						'elementCurrent' => $this->getElement ( '<referenceContainer name="reference" htmlTag="span" htmlId="id_add" htmlClass="new" label="Add"/>', 'referenceContainer' ),
				...
				'referenceContainerNoRemove' => [ 
						'elementCurrent' => $this->getElement ( '<referenceContainer name="reference" remove="false"/>', 'referenceContainer' ),
						'containerName' => 'reference',
						'structureElement' => [ ],
						'expectedData' => [ 
								'attributes' => [ 
				...
				'referenceContainerRemove' => [ 
						'elementCurrent' => $this->getElement ( '<referenceContainer name="reference" remove="1"/>', 'referenceContainer' ),
						'containerName' => 'reference',
						'structureElement' => [ ],
						'expectedData' => [ ],
				...
				'referenceContainerRemove2' => [ 
						'elementCurrent' => $this->getElement ( '<referenceContainer name="reference" remove="true"/>', 'referenceContainer' ),
						'containerName' => 'reference',
						'structureElement' => [ ],
						'expectedData' => [ ],
						'getStructureCondition' => $this->never (),
				...
				'referenceContainerDisplayFalse' => [ 
						'elementCurrent' => $this->getElement ( '<referenceContainer name="reference" htmlTag="span" htmlId="id_add" htmlClass="new" label="Add"' . ' display="true"/>', 'referenceContainer' ),
						'containerName' => 'reference',
						'structureElement' => [ ],
						'expectedData' => [ 
								'attributes' => [ 
	...
	protected function getElement($xml, $elementType) {
		$xml = simplexml_load_string ( '<parent_element>' . $xml . '</parent_element>', 'Magento\Framework\View\Layout\Element' );
