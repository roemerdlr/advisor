...
class UpdaterTest extends \PHPUnit_Framework_TestCase {
	...
	public function testEvaluate() {
		...
		$this->_interpreter->expects ( $this->once () )->method ( 'evaluate' )->with ( [ 
				'value' => 'some text' 
		...
		$updater->expects ( $this->once () )->method ( 'update' )->with ( $evaluatedValue )->will ( $this->returnValue ( $updatedValue ) );
		
		$this->_objectManager->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Framework\View\Layout\Argument\UpdaterInterface' )->will ( $this->returnValue ( $updater ) );
