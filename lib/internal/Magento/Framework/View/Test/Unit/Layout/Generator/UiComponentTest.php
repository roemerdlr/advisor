...
class UiComponentTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->argumentInterpreterMock = $this->getMockBuilder ( 'Magento\Framework\Data\Argument\InterpreterInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		...
		$this->contextFactoryMock = $this->getMock ( 'Magento\Framework\View\Element\UiComponent\ContextFactory', [ ], [ ], '', false );
		$this->blockFactoryMock = $this->getMock ( 'Magento\Framework\View\Element\BlockFactory', [ ], [ ], '', false );
		
		$this->uiComponent = $this->objectManagerHelper->getObject ( 'Magento\Framework\View\Layout\Generator\UiComponent', [ 
	...
	public function testProcess() {
		...
		$this->uiComponentFactoryMock->expects ( $this->any () )->method ( 'create' )->with ( 'uiComponent', null, [ 
				'context' => $contextMock 
		] )->willReturn ( $componentMock );
		
		$this->blockFactoryMock->expects ( $this->once () )->method ( 'createBlock' )->with ( UiComponent::CONTAINER, [ 
				'component' => $componentMock 
		] )->willReturn ( $blockMock );
		
		$this->argumentInterpreterMock->expects ( $this->any () )->method ( 'evaluate' )->will ( $this->returnValueMap ( [ 
				[ 
						[ 
								'key_1' => 'value_1' 
						],
	...
	protected function prepareScheduledStructure() {
		$this->scheduledStructureMock->expects ( $this->any () )->method ( 'getElements' )->willReturn ( [ 
				UiComponent::TYPE => [ 
						UiComponent::TYPE,
						[ 
								'attributes' => [ 
