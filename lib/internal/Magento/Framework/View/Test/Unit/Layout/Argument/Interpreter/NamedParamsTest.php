...
class NamedParamsTest extends \PHPUnit_Framework_TestCase {
	...
	public function testEvaluate() {
		...
		$this->_interpreter->expects ( $this->at ( 0 ) )->method ( 'evaluate' )->with ( [ 
				'value' => 'value 1' 
		] )->will ( $this->returnValue ( 'value 1 (evaluated)' ) );
		$this->_interpreter->expects ( $this->at ( 1 ) )->method ( 'evaluate' )->with ( [ 
				'value' => 'value 2' 
		] )->will ( $this->returnValue ( 'value 2 (evaluated)' ) );
		$expected = [ 
				'param1' => 'value 1 (evaluated)',
				'param2' => 'value 2 (evaluated)' 
		];
