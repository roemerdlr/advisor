...
class HelperTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->dataStructureMock = $this->getMockBuilder ( 'Magento\Framework\View\Layout\Data\Structure' )->disableOriginalConstructor ()->getMock ();
	 ...
	 * @return void @dataProvider scheduleStructureDataProvider
	 */
	public function testScheduleStructure($currentNodeName, $actualNodeName, $unsetPathElementCount, $unsetStructureElementCount) {
		...
		$this->scheduledStructureMock->expects ( $this->once () )->method ( 'setStructureElement' )->with ( $actualNodeName, [ 
				$block,
				$currentNodeAs,
				$parentNodeName,
				$after,
		...
		$this->scheduledStructureMock->expects ( $this->once () )->method ( 'getPaths' )->will ( $this->returnValue ( [ 
				$potentialChild => $testPath . '/' . $currentNodeName . '/' 
		] ) );
	...
	public function testScheduleNonExistentElement() {
		...
		$this->scheduledStructureMock->expects ( $this->once () )->method ( 'getStructureElement' )->with ( $key )->willReturn ( [ ] );
	 ...
	 * @return void @dataProvider scheduleElementLogDataProvider
	 */
	public function testScheduleElementLog($loggerExpects, $stateMode) {
		...
		$this->scheduledStructureMock->expects ( $this->once () )->method ( 'getStructureElement' )->willReturn ( [ 
		...
		$this->scheduledStructureMock->expects ( $this->once () )->method ( 'hasStructureElement' )->with ( $parentName )->willReturn ( false );
		$this->dataStructureMock->expects ( $this->once () )->method ( 'hasElement' )->with ( $parentName )->willReturn ( false );
		$this->stateMock->expects ( $this->once () )->method ( 'getMode' )->willReturn ( $stateMode );
		$this->loggerMock->expects ( $loggerExpects )->method ( 'critical' )->with ( "Broken reference: the '{$key}' element cannot be added as child to '{$parentName}', " . 'because the latter doesn\'t exist' );
	...
	public function testScheduleElement($hasParent, $setAsChild, $toRemoveList, $siblingName, $isAfter, $toSortList) {
		...
		$this->scheduledStructureMock->expects ( $this->any () )->method ( 'getStructureElement' )->willReturnMap ( [ 
				[ 
						$key,
						null,
						[ 
		...
		$this->scheduledStructureMock->expects ( $this->any () )->method ( 'getStructureElementData' )->willReturnMap ( [ 
				[ 
						$key,
						null,
						$data 
		...
		$this->scheduledStructureMock->expects ( $this->once () )->method ( 'setElement' )->with ( $key, [ 
				$block,
				$data 
		] );
		$this->dataStructureMock->expects ( $this->once () )->method ( 'createElement' )->with ( $key, [ 
				'type' => $block 
		] );
