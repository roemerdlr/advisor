...
class MinifierTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$filesystem->expects ( $this->once () )->method ( 'getDirectoryRead' )->with ( DirectoryList::ROOT )->willReturn ( $this->appDirectory );
	...
	public function testGetPathToMinified() {
		...
		$this->appDirectory->expects ( $this->once () )->method ( 'getRelativePath' )->with ( $file )->willReturn ( $relativePath );
		$this->htmlDirectory->expects ( $this->once () )->method ( 'getAbsolutePath' )->with ( $relativePath )->willReturn ( $absolutePath );
...
TEXT;
		...
		$this->appDirectory->expects ( $this->once () )->method ( 'getRelativePath' )->with ( $file )->willReturn ( $relativePath );
		$this->appDirectory->expects ( $this->once () )->method ( 'readFile' )->with ( $relativePath )->willReturn ( $baseContent );
		
		$this->htmlDirectory->expects ( $this->once () )->method ( 'isExist' )->willReturn ( false );
		$this->htmlDirectory->expects ( $this->once () )->method ( 'create' );
		$this->htmlDirectory->expects ( $this->once () )->method ( 'writeFile' )->with ( $relativePath, $expectedContent );
	...
	public function testGetMinified() {
		...
		$htmlDriver->expects ( $this->once () )->method ( 'getRealPathSafety' )->willReturn ( $file );
		
		$this->appDirectory->expects ( $this->exactly ( 3 ) )->method ( 'getRelativePath' )->with ( $file )->willReturn ( $relativePath );
		$this->htmlDirectory->expects ( $this->at ( 1 ) )->method ( 'isExist' )->with ( $relativePath )->willReturn ( false );
		
		$this->htmlDirectory->expects ( $this->once () )->method ( 'getDriver' )->willReturn ( $htmlDriver );
