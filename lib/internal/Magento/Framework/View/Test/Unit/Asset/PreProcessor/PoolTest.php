...
class PoolTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->sorterMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\PreProcessor\Helper\SortInterface' )->getMockForAbstractClass ();
	}
	...
	private function getChainMock($type) {
		...
		$chainMock->expects ( self::once () )->method ( 'getTargetContentType' )->willReturn ( $type );
	...
	private function getPreprocessorMock($chainMock) {
		...
		$preprocessorMock = $this->getMockBuilder ( self::PREPROCESSOR_CLASS )->getMockForAbstractClass ();
		
		$preprocessorMock->expects ( self::once () )->method ( 'process' )->with ( $chainMock );
	...
	public function testProcess() {
		...
		$this->objectManagerMock->expects ( self::once () )->method ( 'get' )->with ( self::PREPROCESSOR_CLASS )->willReturn ( $this->getPreprocessorMock ( $chainMock ) );
	...
	public function testProcessDefault() {
		...
		$this->sorterMock->expects ( self::never () )->method ( 'sort' );
		
		$chainMock = $this->getChainMock ( self::CONTENT_TYPE );
		
		$this->objectManagerMock->expects ( self::once () )->method ( 'get' )->with ( self::DEFAULT_PREPROCESSOR )->willReturn ( $this->getPreprocessorMock ( $chainMock ) );
