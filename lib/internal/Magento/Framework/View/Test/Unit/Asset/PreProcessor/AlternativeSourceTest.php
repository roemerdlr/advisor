 ...
 *
 * @see \Magento\Framework\View\Asset\PreProcessor\AlternativeSource @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class AlternativeSourceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->sorterMock = $this->getMockBuilder ( SortInterface::class )->getMockForAbstractClass ();
		$this->objectManagerMock = $this->getMockBuilder ( ObjectManagerInterface::class )->getMockForAbstractClass ();
		$this->lockerProcessMock = $this->getMockBuilder ( LockerProcessInterface::class )->getMockForAbstractClass ();
		$this->assetBuilderMock = $this->getMockBuilder ( AssetBuilder::class )->disableOriginalConstructor ()->getMock ();
		$this->alternativeMock = $this->getMockBuilder ( ContentProcessorInterface::class )->getMockForAbstractClass ();
		$this->filenameResolverMock = $this->getMockBuilder ( FilenameResolverInterface::class )->getMockForAbstractClass ();
	}
	...
	public function testProcessException() {
		...
		$this->filenameResolverMock->expects ( self::once () )->method ( 'resolve' )->with ( self::FILE_PATH )->willReturn ( self::FILE_PATH );
		...
		$this->assetBuilderMock->expects ( self::once () )->method ( 'build' )->willReturn ( $this->getAssetNew () );
		
		$this->objectManagerMock->expects ( self::once () )->method ( 'get' )->with ( 'stdClass' )->willReturn ( new \stdClass () );
		
		$alternativeSource = new AlternativeSource ( $this->filenameResolverMock, $this->objectManagerMock, $this->lockerProcessMock, $this->sorterMock, $this->assetBuilderMock, 'lock', $alternatives );
		try {
	...
	private function getProcessorMock($asset) {
		$processorMock = $this->getMockBuilder ( ContentProcessorInterface::class )->getMockForAbstractClass ();
		
		$processorMock->expects ( self::once () )->method ( 'processContent' )->with ( $asset )->willReturn ( self::NEW_CONTENT );
	...
	private function getChainMock() {
		$chainMock = $this->getMockBuilder ( Chain::class )->disableOriginalConstructor ()->getMock ();
	...
	private function getChainMockExpects($content = '', $contentExactly = 1) {
		...
		$chainMock->expects ( self::once () )->method ( 'getContent' )->willReturn ( $content );
		$chainMock->expects ( self::exactly ( 3 ) )->method ( 'getAsset' )->willReturn ( $this->getAssetMockExpects () );
		$chainMock->expects ( self::exactly ( $contentExactly ) )->method ( 'setContent' )->willReturn ( self::NEW_CONTENT );
	...
	private function getAssetMock() {
		$assetMock = $this->getMockBuilder ( LocalInterface::class )->disableOriginalConstructor ()->getMock ();
	...
	private function getAssetMockExpects() {
		...
		$assetMock->expects ( self::once () )->method ( 'getContext' )->willReturn ( $this->getContextMock () );
		$assetMock->expects ( self::once () )->method ( 'getFilePath' )->willReturn ( self::FILE_PATH );
		$assetMock->expects ( self::once () )->method ( 'getModule' )->willReturn ( self::MODULE );
	...
	private function getContextMock() {
		$contextMock = $this->getMockBuilder ( FallbackContext::class )->disableOriginalConstructor ()->getMock ();
		
		$contextMock->expects ( self::once () )->method ( 'getAreaCode' )->willReturn ( self::AREA );
		$contextMock->expects ( self::once () )->method ( 'getThemePath' )->willReturn ( self::THEME );
		$contextMock->expects ( self::once () )->method ( 'getLocale' )->willReturn ( self::LOCALE );
