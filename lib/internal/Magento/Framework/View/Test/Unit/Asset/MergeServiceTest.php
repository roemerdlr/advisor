...
class MergeServiceTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_filesystem->expects ( $this->any () )->method ( 'getDirectoryWrite' )->will ( $this->returnValue ( $this->_directory ) );
		
		$this->_object = new \Magento\Framework\View\Asset\MergeService ( $this->_objectManager, $this->_config, $this->_filesystem, $this->_state );
	}
	...
	public function testGetMergedAssets(array $assets, $contentType, $appMode, $mergeStrategy) {
		...
		$this->_objectManager->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\View\Asset\Merged', [ 
				'assets' => $assets,
