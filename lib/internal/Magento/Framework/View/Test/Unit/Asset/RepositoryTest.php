...
class RepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->urlMock = $this->getMockBuilder ( 'Magento\Framework\UrlInterface' )->disableOriginalConstructor ()->getMock ();
		$this->designMock = $this->getMockBuilder ( 'Magento\Framework\View\DesignInterface' )->disableOriginalConstructor ()->getMock ();
		$this->listMock = $this->getMockBuilder ( 'Magento\Framework\View\Design\Theme\ListInterface' )->disableOriginalConstructor ()->getMock ();
		...
		$this->fileFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\FileFactory' )->disableOriginalConstructor ()->getMock ();
		$this->fallbackFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\File\FallbackContextFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->contextFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\File\ContextFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		$this->remoteFactoryMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\RemoteFactory' )->setMethods ( [ 
				'create' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$this->repository = new Repository ( $this->urlMock, $this->designMock, $this->listMock, $this->sourceMock, $this->httpMock, $this->fileFactoryMock, $this->fallbackFactoryMock, $this->contextFactoryMock, $this->remoteFactoryMock );
	}
	...
	public function testUpdateDesignParamsWrongTheme() {
		...
		$this->listMock->expects ( $this->once () )->method ( 'getThemeByFullPath' )->with ( 'area/nonexistent_theme' )->will ( $this->returnValue ( null ) );
	 ...
	 * @param array $params        	
	 * @param array $result        	
	 * @return void @dataProvider updateDesignParamsDataProvider
	 */
	public function testUpdateDesignParams($params, $result) {
		$this->listMock->expects ( $this->any () )->method ( 'getThemeByFullPath' )->willReturn ( 'ThemeID' );
	...
	public function testCreateAsset() {
		$this->listMock->expects ( $this->any () )->method ( 'getThemeByFullPath' )->willReturnArgument ( 0 );
		...
		$this->fallbackFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'baseUrl' => '',
				'areaType' => '',
				'themePath' => 'Default',
				'localeCode' => '',
		...
		$this->fileFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
		...
		$this->assertEquals ( $assetMock, $this->repository->createAsset ( 'test/file.js', [ 
				'module' => 'Test',
				'theme' => 'Default' 
		] ) );
	}
	...
	public function testGetStaticViewFileContext() {
		...
		$this->designMock->expects ( $this->any () )->method ( 'getDesignParams' )->willReturn ( [ 
	 ...
	 * @param string $module        	
	 * @return void @dataProvider createRelatedDataProvider
	 */
	public function testCreateRelated($filePath, $resultFilePath, $module) {
		...
		$originalAssetMock = $this->getMockBuilder ( 'Magento\Framework\View\Asset\File' )->disableOriginalConstructor ()->setMethods ( [ 
				'getModule',
				'getContext' 
		] )->getMock ();
	...
	public function testCreateArbitrary() {
		...
		$this->contextFactoryMock->expects ( $this->once () )->method ( 'create' )->with ( [ 
				'baseUrl' => '',
				'baseDirType' => 'dirType',
	...
	public function testGetUrl() {
		...
		$this->fileFactoryMock->expects ( $this->exactly ( 2 ) )->method ( 'create' )->with ( [ 
				'source' => $this->sourceMock,
				'context' => '',
				'filePath' => 'test/file.js',
				'module' => '',
