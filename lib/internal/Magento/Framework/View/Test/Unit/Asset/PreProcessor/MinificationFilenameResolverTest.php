...
class MinificationFilenameResolverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testResolve($isMin, $input, $expected) {
		$minificationMock = $this->getMockBuilder ( Minification::class )->disableOriginalConstructor ()->getMock ();
		
		$minificationMock->expects ( self::once () )->method ( 'isEnabled' )->with ( 'ext' )->willReturn ( $isMin );
