...
class MergedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_assetJsOne->expects ( $this->any () )->method ( 'getPath' )->will ( $this->returnValue ( 'script_one.js' ) );
		...
		$this->_assetJsTwo->expects ( $this->any () )->method ( 'getPath' )->will ( $this->returnValue ( 'script_two.js' ) );
	...
	public function testIteratorInterfaceMerge() {
		...
		$merged = new \Magento\Framework\View\Asset\Merged ( $this->_logger, $this->_mergeStrategy, $this->_assetRepo, $assets );
		$mergedAsset = $this->getMock ( 'Magento\Framework\View\Asset\File', [ ], [ ], '', false );
		$this->_mergeStrategy->expects ( $this->once () )->method ( 'merge' )->with ( $assets, $mergedAsset )->will ( $this->returnValue ( null ) );
	...
	public function testIteratorInterfaceMergeFailure() {
		...
		$assetBroken->expects ( $this->any () )->method ( 'getPath' )->will ( $this->throwException ( $mergeError ) );
		
		$merged = new \Magento\Framework\View\Asset\Merged ( $this->_logger, $this->_mergeStrategy, $this->_assetRepo, [ 
				$this->_assetJsOne,
				$this->_assetJsTwo,
				$assetBroken 
		] );
