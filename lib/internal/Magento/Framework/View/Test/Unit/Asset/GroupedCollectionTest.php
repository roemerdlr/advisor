...
class GroupedCollectionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$factory = $this->getMock ( 'Magento\Framework\View\Asset\PropertyGroupFactory', [ ], [ ], '', false, false );
		$factory->expects ( $this->any () )->method ( 'create' )->will ( $this->returnCallback ( [ 
				$this,
				'createAssetGroup' 
		] ) );
	...
	/**
	 * Return newly created asset group.
	 * Used as a stub for object manger's creation operation.
	 *
	...
	public function testGetGroups() {
		...
		$this->_object->add ( 'css_asset_one', $cssAsset, [ 
				'property' => 'test_value' 
		] );
		$this->_object->add ( 'css_asset_two', $cssAsset, [ 
				'property' => 'test_value' 
		] );
		
		// assets with identical properties but empty properties should be grouped together
		$this->_object->add ( 'css_asset_four', $cssAsset, [ 
				'property' => 'test_value2',
				'junk1' => null 
		] );
		$this->_object->add ( 'css_asset_five', $cssAsset, [ 
				'property' => 'test_value2',
				'junk2' => '' 
		] );
		
		// assets with different properties should go to different groups
		$this->_object->add ( 'css_asset_three', $cssAsset, [ 
				'property' => 'different_value' 
		] );
		$this->_object->add ( 'js_asset_one', $jsAsset, [ 
				'property' => 'test_value' 
		] );
		
		// assets with identical properties in a different order should be grouped
		$this->_object->add ( 'js_asset_two', $jsAsset, [ 
				'property1' => 'value1',
				'property2' => 'value2' 
		] );
		$this->_object->add ( 'js_asset_three', $jsAsset, [ 
				'property2' => 'value2',
				'property1' => 'value1' 
		] );
		
		// assets allowing merge should go to separate group regardless of having identical properties
		$this->_object->add ( 'asset_allowing_merge', $jsAssetAllowingMerge, [ 
				'property' => 'test_value' 
		] );
