...
class LockerProcessTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLockProcess($method) {
		$this->filesystemMock->expects ( self::once () )->method ( 'getDirectoryWrite' )->with ( DirectoryList::VAR_DIR )->willReturn ( $this->$method () );
	...
	public function testUnlockProcess() {
		$this->filesystemMock->expects ( self::once () )->method ( 'getDirectoryWrite' )->with ( DirectoryList::VAR_DIR )->willReturn ( $this->getTmpDirectoryMockFalse ( 1 ) );
	...
	protected function getTmpDirectoryMockTrue() {
		...
		$tmpDirectoryMock->expects ( self::atLeastOnce () )->method ( 'isExist' )->with ( $this->fileName )->willReturn ( true );
		
		$tmpDirectoryMock->expects ( self::atLeastOnce () )->method ( 'readFile' )->with ( $this->fileName )->willReturn ( time () - 25 );
		
		$tmpDirectoryMock->expects ( self::once () )->method ( 'writeFile' )->with ( $this->fileName, self::matchesRegularExpression ( '#\d+#' ) );
	...
	protected function getTmpDirectoryMockFalse($exactly = 0) {
		...
		$tmpDirectoryMock->expects ( self::atLeastOnce () )->method ( 'isExist' )->with ( $this->fileName )->willReturn ( false );
		
		$tmpDirectoryMock->expects ( self::never () )->method ( 'readFile' );
		
		$tmpDirectoryMock->expects ( self::exactly ( $exactly ) )->method ( 'delete' )->with ( $this->fileName );
	...
	private function getTmpDirectoryMock() {
		$tmpDirectoryMock = $this->getMockBuilder ( 'Magento\Framework\Filesystem\Directory\WriteInterface' )->getMockForAbstractClass ();
