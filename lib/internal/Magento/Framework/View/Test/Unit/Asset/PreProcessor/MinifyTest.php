...
class MinifyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->adapterMock = $this->getMockBuilder ( 'Magento\Framework\Code\Minifier\AdapterInterface' )->setMethods ( [ 
				'minify' 
		...
		$this->minify = new Minify ( $this->adapterMock, $this->minificationMock );
	}
	 ...
	 * @return void @dataProvider processDataProvider
	 */
	public function testProcess($targetPath, $originalPath, $minifyCalls, $setContentCalls, $isEnabled) {
		...
		$this->minificationMock->expects ( $this->any () )->method ( 'isEnabled' )->willReturnMap ( [ 
				[ 
						'css',
						$isEnabled 
				] 
		...
		$this->minificationMock->expects ( $this->any () )->method ( 'isMinifiedFilename' )->willReturnMap ( [ 
				[ 
						'test.min.css',
						true 
				],
