...
class Merged implements \Iterator {
	 ...
	 */
	public function __construct(\Psr\Log\LoggerInterface $logger, MergeStrategyInterface $mergeStrategy, \Magento\Framework\View\Asset\Repository $assetRepo, array $assets) {
		$this->logger = $logger;
		...
		foreach ( $assets as $asset ) {
			...
			if (! $this->contentType) {
				...
				throw new \InvalidArgumentException ( "Content type '{$asset->getContentType()}' cannot be merged with '{$this->contentType}'." );
			}
		}
		$this->assets = $assets;
	}
