...
class Minification {
	...
	public function isEnabled($contentType) {
		return $this->appState->getMode () != State::MODE_DEVELOPER && ( bool ) $this->scopeConfig->isSetFlag ( sprintf ( self::XML_PATH_MINIFICATION_ENABLED, $contentType ), $this->scope );
	}
	...
	public function addMinifiedSign($filename) {
		...
		if ($this->isEnabled ( $extension ) && ! $this->isExcluded ( $filename ) && ! $this->isMinifiedFilename ( $filename )) {
