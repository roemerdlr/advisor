...
class Pool {
	 ...
	 */
	public function __construct(ObjectManagerInterface $objectManager, Helper\SortInterface $sorter, $defaultPreprocessor, array $preprocessors = []) {
	...
	private function getPreProcessors($type) {
		...
		if (isset ( $this->preprocessors [$type] )) {
			...
			$preprocessors = [ 
					'default' => [ 
							self::PREPROCESSOR_CLASS => $this->defaultPreprocessor 
					] 
			];
		}
