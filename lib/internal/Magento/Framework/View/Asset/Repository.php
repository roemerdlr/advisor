...
class Repository {
	...
	public function createAsset($fileId, array $params = []) {
		...
		$context = $this->getFallbackContext ( UrlInterface::URL_TYPE_STATIC, $isSecure, $params ['area'], $themePath, $params ['locale'] );
	...
	public function getStaticViewFileContext() {
		...
		return $this->getFallbackContext ( UrlInterface::URL_TYPE_STATIC, $isSecure, $params ['area'], $themePath, $params ['locale'] );
	}
	 ...
	 * @return Remote @codeCoverageIgnore
	 */
	public function createRemoteAsset($url, $contentType) {
		return $this->remoteFactory->create ( [ 
				'url' => $url,
				'contentType' => $contentType 
		] );
	}
	...
	public static function extractModule($fileId) {
		...
		if (empty ( $result [0] )) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Scope separator "::" cannot be used without scope identifier.' ) );
		}
		return [ 
				$result [0],
				$result [1] 
