...
class ResponseTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->response = $this->getMock ( 'Magento\Framework\HTTP\PhpEnvironment\Response', [ 
				'getHeaders',
				'send',
				'clearHeader' 
		] );
	...
	public function testGetHeader() {
		...
		$this->headers->expects ( $this->once () )->method ( 'has' )->will ( $this->returnValue ( true ) );
		$this->headers->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( true ) );
		
		$this->assertTrue ( $this->response->getHeader ( 'testName' ) );
	}
	public function testGetHeaderWithoutHeader() {
		$this->response->expects ( $this->once () )->method ( 'getHeaders' )->will ( $this->returnValue ( $this->headers ) );
		$this->headers->expects ( $this->once () )->method ( 'has' )->will ( $this->returnValue ( false ) );
		$this->headers->expects ( $this->never () )->method ( 'get' )->will ( $this->returnValue ( false ) );
	...
	public function testSetHeaderWithoutReplacing() {
		$this->response->expects ( $this->once () )->method ( 'getHeaders' )->will ( $this->returnValue ( $this->headers ) );
		$this->response->expects ( $this->never () )->method ( 'clearHeader' )->with ( 'testName' );
	...
	public function testClearHeaderIfHeaderExistsAndWasFound() {
		$response = $this->response = $this->getMock ( 'Magento\Framework\HTTP\PhpEnvironment\Response', [ 
				'getHeaders',
				'send' 
		] );
		...
		$this->headers->expects ( $this->once () )->method ( 'has' )->with ( 'Header-name' )->will ( $this->returnValue ( true ) );
		$this->headers->expects ( $this->once () )->method ( 'get' )->with ( 'Header-name' )->will ( $this->returnValue ( $header ) );
		$this->headers->expects ( $this->once () )->method ( 'removeHeader' )->with ( $header )->will ( $this->returnValue ( true ) );
	...
	public function testSetRedirect() {
		...
		$response = $this->getMock ( 'Magento\Framework\HTTP\PhpEnvironment\Response', [ 
				'setHeader',
				'setHttpResponseCode',
				'sendHeaders' 
		], [ ], '', false );
		$response->expects ( $this->once () )->method ( 'setHeader' )->with ( 'Location', 'testUrl', true )->will ( $this->returnSelf () );
		$response->expects ( $this->once () )->method ( 'setHttpResponseCode' )->with ( 302 )->will ( $this->returnSelf () );
