...
class FlagTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$data = [ 
				'flag_code' => 'synchronize' 
		];
		$this->createInstance ( $data );
	}
	protected function createInstance(array $data = []) {
		$eventManager = $this->getMock ( 'Magento\Framework\Event\Manager', [ 
				'dispatch' 
		], [ ], '', false, false );
		$context = $this->getMock ( 'Magento\Framework\Model\Context', [ ], [ ], '', false, false );
		$context->expects ( $this->once () )->method ( 'getEventDispatcher' )->will ( $this->returnValue ( $eventManager ) );
		$registry = $this->getMock ( 'Magento\Framework\Registry', [ ], [ ], '', false, false );
		
		$connection = $this->getMock ( 'Magento\Framework\DB\Adapter\Adapter', [ 
				'beginTransaction' 
		], [ ], '', false, false );
		$connection->expects ( $this->any () )->method ( 'beginTransaction' )->will ( $this->returnSelf () );
		$appResource = $this->getMock ( 'Magento\Framework\App\ResourceConnection', [ ], [ ], '', false, false );
		$appResource->expects ( $this->any () )->method ( 'getConnection' )->will ( $this->returnValue ( $connection ) );
		...
		$resource = $this->getMock ( '\Magento\Framework\Flag\FlagResource', [ 
				'__wakeup',
				'load',
				'save',
				'addCommitCallback',
		...
		$resourceCollection = $this->getMockBuilder ( 'Magento\Framework\Data\Collection\AbstractDb' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		
		$this->flag = new \Magento\Framework\Flag ( $context, $registry, $resource, $resourceCollection, $data );
	}
