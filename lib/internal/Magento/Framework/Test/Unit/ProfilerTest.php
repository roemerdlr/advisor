...
class ProfilerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function _getDriverMock() {
		return $this->getMockBuilder ( 'Magento\Framework\Profiler\DriverInterface' )->setMethods ( [ 
				'start',
				'stop',
				'clear' 
	...
	public function testTags() {
		$driver = $this->_getDriverMock ();
		$driver->expects ( $this->at ( 0 ) )->method ( 'start' )->with ( 'root_level_timer', [ 
				'default_tag' => 'default' 
		] );
		$driver->expects ( $this->at ( 1 ) )->method ( 'start' )->with ( 'root_level_timer->some_other_timer', [ 
				'default_tag' => 'default',
				'type' => 'test' 
		] );
		
		\Magento\Framework\Profiler::add ( $driver );
		\Magento\Framework\Profiler::setDefaultTags ( [ 
				'default_tag' => 'default' 
		] );
		\Magento\Framework\Profiler::start ( 'root_level_timer' );
		\Magento\Framework\Profiler::start ( 'some_other_timer', [ 
	...
	public function passedFilterDataProvider() {
		return [ 
				...
				'more than one tag with expected' => [ 
						'timer',
						[ 
								'tag' => 'value',
								'type' => 'test' 
	...
	public function testApplyConfig() {
		...
		$this->assertAttributeEquals ( [ 
				$mockDriver 
		], '_drivers', 'Magento\Framework\Profiler' );
		$this->assertAttributeEquals ( [ 
				'tagName' => [ 
						'tagValue' 
				] 
		], '_tagFilters', 'Magento\Framework\Profiler' );
