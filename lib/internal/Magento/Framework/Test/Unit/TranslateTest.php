...
class TranslateTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->request = $this->getMockForAbstractClass ( '\Magento\Framework\App\RequestInterface', [ ], '', false, false, true, [ 
				'getParam',
				'getControllerModule' 
		] );
		...
		$this->translate = new Translate ( $this->viewDesign, $this->cache, $this->viewFileSystem, $this->moduleList, $this->modulesReader, $this->scopeResolver, $this->resource, $this->locale, $this->appState, $filesystem, $this->request, $this->csvParser, $this->packDictionary );
	}
	...
	public function testLoadData($area, $forceReload, $cachedData) {
		...
		$this->csvParser->expects ( $this->any () )->method ( 'getDataPairs' )->will ( $this->returnValueMap ( [ 
				[ 
						'/app/module/en_US.csv',
						0,
						1,
						$moduleData 
				],
				[ 
						'/app/module/en_GB.csv',
						0,
						1,
						$moduleData 
				],
	...
	public function dataProviderForTestLoadData() {
		$cachedData = [ 
				'cached 1' => 'translated 1',
				'cached 2' => 'translated 2' 
		];
		return [ 
				[ 
						'adminhtml',
	...
	public function testGetTheme() {
		...
		$this->request->expects ( $this->at ( 1 ) )->method ( 'getParam' )->with ( 'theme' )->will ( $this->returnValue ( $requestTheme ) );
	...
	protected function expectsSetConfig($themeId, $localeCode = 'en_US') {
		...
		$scope = new \Magento\Framework\DataObject ( [ 
				'code' => 'frontendCode',
				'id' => 1 
		] );
		$scopeAdmin = new \Magento\Framework\DataObject ( [ 
				'code' => 'adminCode',
				'id' => 0 
		] );
		$this->scopeResolver->expects ( $this->any () )->method ( 'getScope' )->will ( $this->returnValueMap ( [ 
				[ 
						null,
						$scope 
				],
		...
		$designTheme = new \Magento\Framework\DataObject ( [ 
				'id' => $themeId 
		] );
