...
class JoinProcessorHelper {
	 ...
	 */
	public function __construct(Config $config, JoinDataInterfaceFactory $joinDataInterfaceFactory) {
		$this->config = $config;
	...
	public function getSelectFieldsMap($attributeCode, $selectFields) {
		...
		foreach ( $selectFields as $selectField ) {
			...
			$selectFieldsAliases [] = [ 
					JoinDataInterface::SELECT_FIELD_EXTERNAL_ALIAS => $attributeCode . ($useFieldInAlias ? '.' . $selectField [Converter::JOIN_FIELD] : ''),
