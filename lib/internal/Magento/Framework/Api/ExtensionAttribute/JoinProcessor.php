...
class JoinProcessor implements \Magento\Framework\Api\ExtensionAttribute\JoinProcessorInterface {
	 ...
	 */
	public function __construct(\Magento\Framework\ObjectManagerInterface $objectManager, TypeProcessor $typeProcessor, ExtensionAttributesFactory $extensionAttributesFactory, JoinProcessorHelper $joinProcessorHelper) {
	...
	public function extractExtensionAttributes($extensibleEntityClass, array $data) {
		...
		$extensionData = [ ];
		foreach ( $joinDirectives as $attributeCode => $directive ) {
			$this->populateAttributeCodeWithDirective ( $attributeCode, $directive, $data, $extensionData, $extensibleEntityClass );
		}
