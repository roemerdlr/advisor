...
class SchemaLocatorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$urnResolverMock->expects ( $this->once () )->method ( 'getRealPath' )->with ( 'urn:magento:framework:Api/etc/extension_attributes.xsd' )->willReturn ( $this->urnResolver->getRealPath ( 'urn:magento:framework:Api/etc/extension_attributes.xsd' ) );
		$this->model = new \Magento\Framework\Api\ExtensionAttribute\Config\SchemaLocator ( $urnResolverMock );
	}
	public function testGetSchema() {
		$this->assertEquals ( $this->urnResolver->getRealPath ( 'urn:magento:framework:Api/etc/extension_attributes.xsd' ), $this->model->getSchema () );
	}
