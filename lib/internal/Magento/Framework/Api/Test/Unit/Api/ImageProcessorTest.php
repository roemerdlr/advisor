...
class ImageProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->directoryWriteMock = $this->getMockForAbstractClass ( 'Magento\Framework\Filesystem\Directory\WriteInterface' );
		...
		$this->uploaderMock = $this->getMockBuilder ( 'Magento\Framework\Api\Uploader' )->setMethods ( [ 
		...
		$this->imageProcessor = $this->objectManager->getObject ( 'Magento\Framework\Api\ImageProcessor', [ 
	...
	public function testSaveWithNoImageData() {
		$imageDataMock = $this->getMockBuilder ( 'Magento\Framework\Api\CustomAttributesDataInterface' )->disableOriginalConstructor ()->getMock ();
		$imageDataMock->expects ( $this->once () )->method ( 'getCustomAttributes' )->willReturn ( [ ] );
		
		$this->dataObjectHelperMock->expects ( $this->once () )->method ( 'getCustomAttributeValueByType' )->willReturn ( [ ] );
	...
	public function testSaveInputException() {
		...
		$this->contentValidatorMock->expects ( $this->once () )->method ( 'isValid' )->willReturn ( false );
	...
	public function testSaveWithNoPreviousData() {
		...
		$imageContent->expects ( $this->any () )->method ( 'getName' )->willReturn ( 'testFileName' );
