...
class GenerateMapperTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerate() {
		...
		$model = $this->getMock ( 'Magento\Framework\Api\Code\Generator\Mapper', [ 
				'_validateData' 
		], [ 
		...
		$this->ioObjectMock->expects ( $this->once () )->method ( 'generateResultFileName' )->with ( '\Magento\Framework\Api\Code\Generator\SampleMapper' )->will ( $this->returnValue ( 'SampleMapper.php' ) );
		$this->ioObjectMock->expects ( $this->once () )->method ( 'writeResultFile' )->with ( 'SampleMapper.php', $sampleMapperCode );
