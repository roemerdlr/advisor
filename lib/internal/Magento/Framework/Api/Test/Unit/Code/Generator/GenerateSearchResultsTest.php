...
class GenerateSearchResultsTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerate() {
		...
		$model = $this->getMock ( 'Magento\Framework\Api\Code\Generator\SearchResults', [ 
				'_validateData' 
		], [ 
		...
		$this->ioObjectMock->expects ( $this->once () )->method ( 'generateResultFileName' )->with ( '\Magento\Framework\Api\Code\Generator\SampleSearchResults' )->will ( $this->returnValue ( 'SampleSearchResults.php' ) );
		$this->ioObjectMock->expects ( $this->once () )->method ( 'writeResultFile' )->with ( 'SampleSearchResults.php', $sampleSearchResultBuilderCode );
