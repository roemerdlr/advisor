...
abstract class AbstractServiceCollection extends \Magento\Framework\Data\Collection {
	...
	public function addFieldToFilter($field, $condition) {
		if (is_array ( $field ) && is_array ( $condition ) && count ( $field ) != count ( $condition )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'When passing in a field array there must be a matching condition array.' ) );
		} elseif (is_array ( $field ) && ! count ( $field ) > 0) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'When passing an array of fields there must be at least one field in the array.' ) );
		}
	 ...
	 *
	 * @param string $field
	 *        	Field for new filter
	 * @param string|array $condition
	 *        	Condition for new filter.
	 * @return Filter
	 */
