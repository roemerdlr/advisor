...
class Ftp extends AbstractRollback {
	...
	protected function _initFtpClient() {
		try {
			...
			throw new \Magento\Framework\Backup\Exception\FtpConnectionFailed ( new \Magento\Framework\Phrase ( $e->getMessage () ) );
		}
	}
	
	/**
	 * Perform ftp validation.
	 * Check whether ftp account provided points to current magento installation
	 *
	 * @return void
	 * @throws \Magento\Framework\Exception\LocalizedException
	 */
	protected function _validateFtp() {
		...
		if (! is_file ( $validationFilePath )) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Unable to validate ftp account' ) );
		}
		...
		if (! $fileExistsOnFtp) {
			throw new \Magento\Framework\Backup\Exception\FtpValidationFailed ( new \Magento\Framework\Phrase ( 'Failed to validate ftp account' ) );
		}
	}
	...
	protected function _createTmpDir() {
		...
		if (false === $result) {
			throw new \Magento\Framework\Backup\Exception\NotEnoughPermissions ( new \Magento\Framework\Phrase ( 'Failed to create directory %1', [ 
					$tmpDir 
			] ) );
		}
	...
	protected function _cleanupFtp() {
		...
		$filesystemIterator = new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $rootDir ), \RecursiveIteratorIterator::CHILD_FIRST );
		
		$iterator = new \Magento\Framework\Backup\Filesystem\Iterator\Filter ( $filesystemIterator, $this->_snapshot->getIgnorePaths () );
	 ...
	 * @param string $tmpDir        	
	 * @return void
	 * @throws \Magento\Framework\Exception\LocalizedException @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	protected function _uploadBackupToFtp($tmpDir) {
		$filesystemIterator = new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $tmpDir ), \RecursiveIteratorIterator::SELF_FIRST );
		...
		foreach ( $filesystemIterator as $item ) {
			...
			if ($item->isDir ()) {
				...
				if (false === $result) {
					throw new \Magento\Framework\Backup\Exception\NotEnoughPermissions ( new \Magento\Framework\Phrase ( 'Failed to upload file %1 to ftp', [ 
							$item->__toString () 
					] ) );
				}
			}
