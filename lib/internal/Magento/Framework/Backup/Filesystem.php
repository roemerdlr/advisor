...
class Filesystem extends AbstractBackup {
	...
	public function rollback() {
		...
		$rollbackWorker = $this->_useFtp ? new \Magento\Framework\Backup\Filesystem\Rollback\Ftp ( $this ) : new \Magento\Framework\Backup\Filesystem\Rollback\Fs ( $this );
		$rollbackWorker->run ();
	...
	public function validateAvailableDiscSpace($backupDir, $size) {
		...
		if ($requiredSpace > $freeSpace) {
			throw new \Magento\Framework\Backup\Exception\NotEnoughFreeSpace ( new \Magento\Framework\Phrase ( 'Warning: necessary space for backup is ' . (ceil ( $requiredSpace ) / 1024) . 'MB, but your free disc space is ' . (ceil ( $freeSpace ) / 1024) . 'MB.' ) );
		}
	}
	...
	protected function _checkBackupsDir() {
		...
		if (! is_dir ( $backupsDir )) {
			...
			if (! is_writeable ( $backupsDirParentDirectory )) {
				throw new \Magento\Framework\Backup\Exception\NotEnoughPermissions ( new \Magento\Framework\Phrase ( 'Cant create backups directory' ) );
			}
		...
		if (! is_writable ( $backupsDir )) {
			throw new \Magento\Framework\Backup\Exception\NotEnoughPermissions ( new \Magento\Framework\Phrase ( 'Backups directory is not writeable' ) );
		}
	}
