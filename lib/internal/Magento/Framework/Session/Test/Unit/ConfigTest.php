...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->validatorMock = $this->getMockBuilder ( 'Magento\Framework\Validator\ValidatorInterface' )->disableOriginalConstructor ()->getMock ();
	...
	public function testSaveHandlerFromConfig() {
		$this->getModel ( $this->validatorMock );
		$this->assertSame ( self::SESSION_HANDLER_CONFIG, $this->config->getSaveHandler (), var_export ( $this->config->toArray (), 1 ) );
	}
	...
	public function testConstructor($isValidSame, $isValid, $expected) {
		$validatorMock = $this->getMockBuilder ( 'Magento\Framework\Validator\ValidatorInterface' )->disableOriginalConstructor ()->getMock ();
		if ($isValidSame) {
			$validatorMock->expects ( $this->any () )->method ( 'isValid' )->willReturn ( $isValid );
		} else {
			for($x = 0; $x < 6; $x ++) {
				if ($x % 2 == 0) {
					$validatorMock->expects ( $this->at ( $x ) )->method ( 'isValid' )->willReturn ( false );
				} else {
					$validatorMock->expects ( $this->at ( $x ) )->method ( 'isValid' )->willReturn ( true );
				}
			}
		}
	...
	protected function getModel($validator) {
		...
		$this->validatorFactoryMock = $this->getMockBuilder ( 'Magento\Framework\ValidatorFactory' )->disableOriginalConstructor ()->getMock ();
		$this->validatorFactoryMock->expects ( $this->any () )->method ( 'setInstanceName' )->willReturnSelf ();
		$this->validatorFactoryMock->expects ( $this->any () )->method ( 'create' )->willReturn ( $validator );
		...
		$this->configMock->method ( 'getValue' )->will ( $this->returnValueMap ( $getValueReturnMap ) );
		
		$filesystemMock = $this->getMock ( 'Magento\Framework\Filesystem', [ ], [ ], '', false );
		$dirMock = $this->getMockForAbstractClass ( 'Magento\Framework\Filesystem\Directory\WriteInterface' );
		$filesystemMock->expects ( $this->any () )->method ( 'getDirectoryWrite' )->will ( $this->returnValue ( $dirMock ) );
		
		$deploymentConfigMock = $this->getMock ( 'Magento\Framework\App\DeploymentConfig', [ ], [ ], '', false );
		$deploymentConfigMock->expects ( $this->at ( 0 ) )->method ( 'get' )->with ( Config::PARAM_SESSION_SAVE_METHOD, ini_get ( 'session.save_handler' ) ?  : 'files' )->willReturn ( self::SESSION_HANDLER_CONFIG );
		$deploymentConfigMock->expects ( $this->at ( 1 ) )->method ( 'get' )->with ( Config::PARAM_SESSION_SAVE_PATH )->will ( $this->returnValue ( null ) );
		$deploymentConfigMock->expects ( $this->at ( 2 ) )->method ( 'get' )->with ( Config::PARAM_SESSION_CACHE_LIMITER )->will ( $this->returnValue ( 'files' ) );
		
		$this->config = $this->helper->getObject ( 'Magento\Framework\Session\Config', [ 
