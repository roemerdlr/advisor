...
class DbTableTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCheckConnection() {
		...
		$connection->expects ( $this->atLeastOnce () )->method ( 'isTableExists' )->with ( $this->equalTo ( self::SESSION_TABLE ) )->will ( $this->returnValue ( true ) );
	...
	public function testCheckConnectionNoTable() {
		...
		$connection->expects ( $this->once () )->method ( 'isTableExists' )->with ( $this->equalTo ( self::SESSION_TABLE ) )->will ( $this->returnValue ( false ) );
	...
	protected function _prepareMockForRead($isDataEncoded) {
		...
		$connection->expects ( $this->once () )->method ( 'from' )->with ( self::SESSION_TABLE, [ 
				self::COLUMN_SESSION_DATA 
		] )->will ( $this->returnSelf () );
		$connection->expects ( $this->once () )->method ( 'where' )->with ( self::COLUMN_SESSION_ID . ' = :' . self::COLUMN_SESSION_ID )->will ( $this->returnValue ( self::SELECT_OBJECT ) );
		
		$sessionData = self::SESSION_DATA;
		if ($isDataEncoded) {
			$sessionData = base64_encode ( $sessionData );
		}
		$connection->expects ( $this->once () )->method ( 'fetchOne' )->with ( self::SELECT_OBJECT, [ 
	...
	protected function _prepareMockForWrite($sessionExists) {
		...
		if ($sessionExists) {
			...
			$connection->expects ( $this->once () )->method ( 'update' )->will ( $this->returnCallback ( [ 
					$this,
					'verifyUpdate' 
			] ) );
		} else {
			$connection->expects ( $this->once () )->method ( 'insert' )->will ( $this->returnCallback ( [ 
					$this,
					'verifyInsert' 
			] ) );
