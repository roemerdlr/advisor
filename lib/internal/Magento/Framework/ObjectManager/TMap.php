...
class TMap implements \IteratorAggregate, \Countable, \ArrayAccess {
	 ...
	 * @param array $array        	
	 */
	public function __construct($type, ObjectManagerInterface $objectManager, ConfigInterface $configInterface, array $array = []) {
		...
		array_walk ( $array, function ($item, $index) {
	...
	private function throwTypeException($inputType, $index) {
		...
		throw new \InvalidArgumentException ( sprintf ( $message, $this->type, $inputType, $index ) );
	}
