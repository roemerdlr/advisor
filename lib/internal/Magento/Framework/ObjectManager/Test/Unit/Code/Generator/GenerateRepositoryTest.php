...
class GenerateRepositoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerate() {
		...
		$model = $this->getMock ( 'Magento\Framework\ObjectManager\Code\Generator\Repository', [ 
				'_validateData' 
		], [ 
		...
		$this->ioObjectMock->expects ( $this->once () )->method ( 'generateResultFileName' )->with ( '\Magento\Framework\ObjectManager\Code\Generator\SampleRepository' )->willReturn ( 'SampleRepository.php' );
		
		$repositoryCode = file_get_contents ( __DIR__ . '/_files/SampleRepository.txt' );
		$this->ioObjectMock->expects ( $this->once () )->method ( 'writeResultFile' )->with ( 'SampleRepository.php', $repositoryCode );
