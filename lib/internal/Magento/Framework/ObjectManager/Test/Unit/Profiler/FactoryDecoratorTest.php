...
class FactoryDecoratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->objectManagerMock = $this->getMockBuilder ( 'Magento\Framework\ObjectManager\FactoryInterface' )->disableOriginalConstructor ()->getMock ();
		
		// Instantiate SUT
		$this->model = $objectManager->getObject ( 'Magento\Framework\ObjectManager\Profiler\FactoryDecorator', [ 
	...
	public function testCreate() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( self::CLASS_NAME, $arguments )->willReturn ( $baseObject );
	...
	public function testCreateNeglectGenerator() {
		...
		$this->objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( self::GENERATOR_NAME, $arguments )->willReturn ( $loggerMock );
