...
class TMapTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->omConfig = $this->getMockBuilder ( 'Magento\Framework\ObjectManager\ConfigInterface' )->getMockForAbstractClass ();
	}
	...
	public function testRead() {
		$tMap = $this->getSimpleInitialized ( 3 );
		$this->om->expects ( static::exactly ( 3 ) )->method ( 'create' )->willReturnMap ( [ 
				[ 
						'TClass',
						[ ],
						new \TClass () 
	...
	public function testEdit() {
		...
		$this->om->expects ( static::exactly ( 4 ) )->method ( 'create' )->willReturnMap ( [ 
				[ 
						'TClass',
						[ ],
						new \TClass () 
	...
	private function getSimpleInitialized($exactlyCalls = 3) {
		...
		$this->omConfig->expects ( static::exactly ( $exactlyCalls ) )->method ( 'getPreference' )->willReturnMap ( [ 
				[ 
						'TClass',
						'TClass' 
				],
		...
		$this->omConfig->expects ( static::exactly ( $exactlyCalls ) )->method ( 'getInstanceType' )->willReturnMap ( [ 
				[ 
						'TClass',
						'TClass' 
				],
		...
		return new TMap ( 'TInterface', $this->om, $this->omConfig, $testClasses );
	}
}
