...
class FactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function testResolveArgumentsException() {
		...
		$configMock->expects ( $this->once () )->method ( 'getArguments' )->will ( $this->returnValue ( [ 
				'firstParam' => 1 
		] ) );
		
		$definitionsMock = $this->getMock ( 'Magento\Framework\ObjectManager\DefinitionInterface' );
		$definitionsMock->expects ( $this->once () )->method ( 'getParameters' )->will ( $this->returnValue ( [ 
				[ 
						'firstParam',
						'string',
						true,
		...
		$this->factory = new Developer ( $configMock, null, $definitionsMock );
		$this->objectManager = new ObjectManager ( $this->factory, $this->config );
		$this->factory->setObjectManager ( $this->objectManager );
		$this->factory->create ( 'Magento\Framework\ObjectManager\Test\Unit\Factory\Fixture\OneScalar', [ 
				'foo' => 'bar' 
		] );
	}
	public function testCreateOneArg() {
		/** @var \Magento\Framework\ObjectManager\Test\Unit\Factory\Fixture\OneScalar $result */
		$result = $this->factory->create ( 'Magento\Framework\ObjectManager\Test\Unit\Factory\Fixture\OneScalar', [ 
				'foo' => 'bar' 
		] );
