...
class CompiledTest extends \PHPUnit_Framework_TestCase {
	public function testGetParametersWithUndefinedDefinition() {
		...
		$readerMock = $this->getMock ( '\Magento\Framework\Code\Reader\ClassReader', [ 
				'getConstructor' 
		], [ ], '', false );
		$readerMock->expects ( $this->once () )->method ( 'getConstructor' )->with ( $className )->willReturn ( $undefinedDefinitionSignature );
		$model = $objectManager->getObject ( 'Magento\Framework\ObjectManager\Test\Unit\Definition\CompiledStub', [ 
				'definitions' => [ 
						[ ],
						[ ] 
				],
