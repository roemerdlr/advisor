...
class Composite {
	/**
	 *
	 * @param array $declaredComponents
	 *        	Array of the components which should be registered in the following format:
	 *        	<pre>
	 *        	[
	...
	public function filterAndSortDeclaredComponents($declaredComponents) {
		 ...
		 */
		$declaredComponents = array_filter ( $declaredComponents, function ($component) {
			return (isset ( $component ['type'] ) && isset ( $component ['sortOrder'] ));
		} );
		/**
		 * Sort all components according to the provided sort order
		 */
		uasort ( $declaredComponents, function ($firstComponent, $secondComponent) {
