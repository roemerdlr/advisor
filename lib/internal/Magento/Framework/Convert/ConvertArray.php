	...
	private function _assocToXml(array $array, $rootName, \SimpleXMLElement $xml) {
		...
		foreach ( $array as $key => $value ) {
			if (! is_array ( $value )) {
				if (is_string ( $key )) {
					if ($key === $rootName) {
						throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Associative key must not be the same as its parent associative key.' ) );
					}
					$hasStringKey = true;
		...
		if ($hasNumericKey && $hasStringKey) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Associative and numeric keys must not be mixed at one level.' ) );
		}
		return $xml;
	}
}
