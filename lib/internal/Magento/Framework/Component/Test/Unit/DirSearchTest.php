...
class DirSearchTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCollectFilesNothingFound() {
		...
		$this->registrar->expects ( $this->exactly ( 2 ) )->method ( 'getPaths' )->with ( $componentType )->willReturn ( [ ] );
		$this->readFactory->expects ( $this->never () )->method ( 'create' );
	...
	public function testCollectFiles() {
		...
		$this->readFactory->expects ( $this->exactly ( 2 ) )->method ( 'create' )->willReturnMap ( [ 
				[ 
						'path1',
						DriverPool::FILE,
						$this->dir 
		...
		$this->dir->method ( 'search' )->with ( $pattern )->willReturnOnConsecutiveCalls ( [ 
				'one/file.xml' 
		], [ 
				'two/file.xml' 
		] );
