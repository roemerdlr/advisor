...
class Gd2 extends \Magento\Framework\Image\Adapter\AbstractAdapter {
	 ...
	 * @param int $fileType
	 *        	one of the constants IMAGETYPE_*
	 * @param
	 *        	bool &$isAlpha
	 * @param
	 *        	bool &$isTrueColor
	...
	public function resize($frameWidth = null, $frameHeight = null) {
		...
		if ($this->_imageHandler) {
			...
			imagecopyresampled ( $newImage, $this->_imageHandler, $dims ['dst'] ['x'], $dims ['dst'] ['y'], $dims ['src'] ['x'], $dims ['src'] ['y'], $dims ['dst'] ['width'], $dims ['dst'] ['height'], $this->_imageSrcWidth, $this->_imageSrcHeight );
		}
		$this->_imageHandler = $newImage;
	...
	public function watermark($imagePath, $positionX = 0, $positionY = 0, $opacity = 30, $tile = false) {
		...
		$this->_getFileAttributes ();
		$watermark = call_user_func ( $this->_getCallback ( 'create', $watermarkFileType, 'Unsupported watermark image format.' ), $imagePath );
		
		$merged = false;
		
		if ($this->getWatermarkWidth () && $this->getWatermarkHeight () && $this->getWatermarkPosition () != self::POSITION_STRETCH) {
			...
			imagecopyresampled ( $newWatermark, $watermark, 0, 0, 0, 0, $this->getWatermarkWidth (), $this->getWatermarkHeight (), imagesx ( $watermark ), imagesy ( $watermark ) );
	...
	public function crop($top = 0, $left = 0, $right = 0, $bottom = 0) {
		...
		imagecopyresampled ( $canvas, $this->_imageHandler, 0, 0, $left, $top, $newWidth, $newHeight, $newWidth, $newHeight );
	...
	protected function _createImageFromTtfText($text, $font) {
		...
		$black = imagecolorallocate ( $this->_imageHandler, 0, 0, 0 );
		$result = imagettftext ( $this->_imageHandler, $this->_fontSize, 0, 0, $height - abs ( $boundingBox [1] ), $black, $font, $text );
		if ($result === false) {
