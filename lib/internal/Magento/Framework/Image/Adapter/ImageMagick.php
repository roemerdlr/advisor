...
class ImageMagick extends \Magento\Framework\Image\Adapter\AbstractAdapter {
	 ...
	 * Check Imagick::COLOR_* constants
	 *
	 ...
	 * Will be usable later when create an option container
	 *
	 * @return $this
	 */
	...
	public function resize($frameWidth = null, $frameHeight = null) {
		...
		$newImage->newImage ( $dims ['frame'] ['width'], $dims ['frame'] ['height'], $this->_imageHandler->getImageBackgroundColor () );
		
		$this->_imageHandler->resizeImage ( $dims ['dst'] ['width'], $dims ['dst'] ['height'], \Imagick::FILTER_CUBIC, self::BLUR_FACTOR );
		
		if ($this->_imageHandler->getImageWidth () < $this->_options ['small_image'] ['width'] || $this->_imageHandler->getImageHeight () < $this->_options ['small_image'] ['height']) {
			$this->_imageHandler->sharpenImage ( $this->_options ['sharpen'] ['radius'], $this->_options ['sharpen'] ['deviation'] );
		}
		
		$newImage->compositeImage ( $this->_imageHandler, \Imagick::COMPOSITE_OVER, $dims ['dst'] ['x'], $dims ['dst'] ['y'] );
	...
	public function watermark($imagePath, $positionX = 0, $positionY = 0, $opacity = 30, $tile = false) {
		...
		if ($this->getWatermarkWidth () && $this->getWatermarkHeight () && $this->getWatermarkPosition () != self::POSITION_STRETCH) {
			$watermark->resizeImage ( $this->getWatermarkWidth (), $this->getWatermarkHeight (), \Imagick::FILTER_CUBIC, self::BLUR_FACTOR );
		}
