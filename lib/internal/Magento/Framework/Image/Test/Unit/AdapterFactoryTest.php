...
class AdapterFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->configMock = $this->getMock ( 'Magento\Framework\Image\Adapter\ConfigInterface', [ 
				'getAdapterAlias',
				'getAdapters' 
		], [ ], '', false );
		
		$this->configMock->expects ( $this->once () )->method ( 'getAdapters' )->will ( $this->returnValue ( [ 
				'GD2' => [ 
	...
	public function testCreate($alias, $class) {
		$objectManagerMock = $this->getMock ( 'Magento\Framework\ObjectManager\ObjectManager', [ 
				'create' 
		], [ ], '', false );
		$imageAdapterMock = $this->getMock ( $class, [ 
				'checkDependencies' 
		], [ ], '', false );
	...
	public function testCreateWithoutName() {
		...
		$imageAdapterMock = $this->getMock ( $adapterClass, [ 
				'checkDependencies' 
		], [ ], '', false );
		$imageAdapterMock->expects ( $this->once () )->method ( 'checkDependencies' );
		
		$objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( $adapterClass )->will ( $this->returnValue ( $imageAdapterMock ) );
