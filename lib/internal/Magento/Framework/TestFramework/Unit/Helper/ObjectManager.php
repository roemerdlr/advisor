...
class ObjectManager {
	...
	protected function _getResourceModelMock() {
		$resourceMock = $this->_testObject->getMock ( 'Magento\Framework\Module\ModuleResource', [ 
				'getIdFieldName',
				'__sleep',
				'__wakeup' 
		], [ ], '', false );
	...
	public function getObject($className, array $arguments = []) {
		if (is_subclass_of ( $className, '\Magento\Framework\Api\AbstractSimpleObjectBuilder' ) || is_subclass_of ( $className, '\Magento\Framework\Api\Builder' )) {
