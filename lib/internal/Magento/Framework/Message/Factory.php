...
class Factory {
	 ...
	 *        	The message type to create, must correspond to a message type under the
	 *        	namespace Magento\Framework\Message\
	 * @param string $text
	 *        	The text to inject into the message
	...
	public function create($type, $text = null) {
		...
		$message = $this->objectManager->create ( $className, $text === null ? [ ] : [ 
				'text' => $text 
		] );
