...
class ManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->messagesFactory = $this->getMockBuilder ( 'Magento\Framework\Message\CollectionFactory' )->disableOriginalConstructor ()->getMock ();
		$this->messageFactory = $this->getMockBuilder ( 'Magento\Framework\Message\Factory' )->disableOriginalConstructor ()->getMock ();
		$this->session = $this->getMockBuilder ( 'Magento\Framework\Message\Session' )->disableOriginalConstructor ()->setMethods ( [ 
				'getData',
				'setData' 
		] )->getMock ();
		...
		$this->model = new Manager ( $this->session, $this->messageFactory, $this->messagesFactory, $this->eventManager, $this->logger );
	}
	public function testGetDefaultGroup() {
		$this->assertEquals ( Manager::DEFAULT_GROUP, $this->model->getDefaultGroup () );
	}
	public function testGetMessages() {
		$messageCollection = $this->getMockBuilder ( 'Magento\Framework\Message\Collection' )->disableOriginalConstructor ()->setMethods ( [ 
				'addMessage' 
		] )->getMock ();
	...
	public function testGetMessagesWithClear() {
		...
		$this->session->expects ( $this->any () )->method ( 'getData' )->with ( Manager::DEFAULT_GROUP )->will ( $this->returnValue ( $messageCollection ) );
	...
	public function testAddException() {
		...
		$messageError = $this->getMockBuilder ( 'Magento\Framework\Message\Error' )->setConstructorArgs ( [ 
		...
		$this->session->expects ( $this->atLeastOnce () )->method ( 'getData' )->with ( Manager::DEFAULT_GROUP )->will ( $this->returnValue ( $messageCollection ) );
	...
	public function testAddMessage($type, $methodName) {
		...
		$message = 'Message';
		$messageCollection = $this->getMock ( 'Magento\Framework\Message\Collection', [ 
				'addMessage' 
		], [ ], '', false );
	...
	public function testAddUniqueMessagesWhenMessagesImplementMessageInterface($messages, $expectation) {
		...
		$this->session->expects ( $this->any () )->method ( 'getData' )->will ( $this->returnValue ( $messageCollection ) );
		$messageCollection->expects ( $this->once () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				new TestingMessage ( 'text' ) 
		] ) );
		$messageCollection->expects ( $this->$expectation () )->method ( 'addMessage' );
		$this->model->addUniqueMessages ( [ 
				$messages 
		] );
	}
	...
	public function testAddUniqueMessages($messages) {
		...
		$this->session->expects ( $this->any () )->method ( 'getData' )->will ( $this->returnValue ( $messageCollection ) );
		$messageCollection->expects ( $this->any () )->method ( 'getItems' )->will ( $this->returnValue ( [ 
				'message' 
		] ) );
