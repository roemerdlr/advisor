...
class ScopedTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_model = new \Magento\Framework\Config\Data\Scoped ( $this->_readerMock, $this->_configScopeMock, $this->_cacheMock, 'tag' );
	}
	...
	public function testGetScopeSwitchingWithNonCachedData() {
		 ...
		 */
		$this->_cacheMock->expects ( $this->once () )->method ( 'load' )->with ( 'adminhtml::tag' )->will ( $this->returnValue ( false ) );
		
		/**
		 * get data from reader
		 */
		$this->_readerMock->expects ( $this->once () )->method ( 'read' )->with ( 'adminhtml' )->will ( $this->returnValue ( $testValue ) );
