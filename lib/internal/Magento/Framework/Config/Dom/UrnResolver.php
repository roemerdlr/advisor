...
class UrnResolver {
	...
	public function getRealPath($schema) {
		...
		if (preg_match ( $modulePattern, $schema, $matches )) {
			...
			throw new NotFoundException ( new Phrase ( "Unsupported format of schema location: '%1'", [ 
					$schema 
			] ) );
		}
		$schemaPath = $package . '/' . $matches ['path'];
		if (empty ( $package ) || ! file_exists ( $schemaPath )) {
			throw new NotFoundException ( new Phrase ( "Could not locate schema: '%1' at '%2'", [ 
					$schema,
					$schemaPath 
			] ) );
		}
	 ...
	 * @param array $context        	
	 * @return resource
	 * @throws LocalizedException @SuppressWarnings(PHPMD.UnusedFormalParameter)
	 */
	public function registerEntityLoader($public, $system, $context) {
		if (strpos ( $system, 'urn:' ) === 0) {
			...
			if (file_exists ( $system )) {
				...
				throw new LocalizedException ( new Phrase ( "File '%system' cannot be found", [ 
						'system' => $system 
				] ) );
			}
		}
