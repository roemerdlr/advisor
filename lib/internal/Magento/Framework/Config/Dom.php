...
class Dom {
	...
	protected function _mergeNode(\DOMElement $node, $parentPath) {
		...
		if ($matchedNode) {
			// different node type
			if ($this->typeAttributeName && $node->hasAttribute ( $this->typeAttributeName ) && $matchedNode->hasAttribute ( $this->typeAttributeName ) && $node->getAttribute ( $this->typeAttributeName ) !== $matchedNode->getAttribute ( $this->typeAttributeName )) {
	...
	protected function _getMatchedNode($nodePath) {
		...
		if ($matchedNodes->length > 1) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( "More than one node matching the query: %1", [ 
					$nodePath 
			] ) );
	 ...
	 * @param string $schema
	 *        	Absolute schema file path or URN
	 * @param string $errorFormat        	
		...
		if (strpos ( $result, '%' ) !== false) {
			if (preg_match_all ( '/%.+%/', $result, $matches )) {
				...
				if (! empty ( $unsupported )) {
					throw new \InvalidArgumentException ( "Error format '{$format}' contains unsupported placeholders: " . join ( ', ', $unsupported ) );
				}
			}
		}
		return $result;
