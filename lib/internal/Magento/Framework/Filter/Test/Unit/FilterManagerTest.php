...
class FilterManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function initMocks() {
		...
		$this->_factoryMock = $this->getMock ( $factoryName, [ 
				'canCreateFilter',
				'createFilter' 
		], [ ], '', false );
		...
		$this->_config = $this->getMock ( '\Magento\Framework\Filter\FilterManager\Config', [ 
				'getFactories' 
		], [ ], '', false );
		$this->_config->expects ( $this->atLeastOnce () )->method ( 'getFactories' )->will ( $this->returnValue ( [ 
				$factoryName 
		] ) );
	...
	public function testCreateFilterInstance() {
		...
		$this->configureFactoryMock ( $filterMock, 'alias', [ 
				'123' 
		] );
		
		$method = new \ReflectionMethod ( 'Magento\Framework\Filter\FilterManager', 'createFilterInstance' );
		$method->setAccessible ( true );
		$this->assertEquals ( $filterMock, $method->invoke ( $this->_filterManager, 'alias', [ 
				'123' 
		] ) );
	}
	...
	public function testCreateFilterInstanceWrongAlias() {
		...
		$this->_factoryMock->expects ( $this->atLeastOnce () )->method ( 'canCreateFilter' )->with ( $this->equalTo ( $filterAlias ) )->will ( $this->returnValue ( false ) );
	...
	protected function configureFactoryMock($filter, $alias, $arguments = []) {
		$this->_factoryMock->expects ( $this->atLeastOnce () )->method ( 'canCreateFilter' )->with ( $this->equalTo ( $alias ) )->will ( $this->returnValue ( true ) );
		
		$this->_factoryMock->expects ( $this->atLeastOnce () )->method ( 'createFilter' )->with ( $this->equalTo ( $alias ), $this->equalTo ( $arguments ) )->will ( $this->returnValue ( $filter ) );
	}
	public function testCall() {
		...
		$this->assertEquals ( $value, $this->_filterManager->alias ( $value, [ 
				'123' 
		] ) );
	}
}
