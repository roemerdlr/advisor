...
EXPECTED_RESULT;
	...
	public function testAfterFilter() {
		...
		// more than once
		$this->templateFilter->addAfterFilterCallback ( [ 
				$callbackObject,
				'afterFilterCallbackMethod' 
		] );
		$this->templateFilter->addAfterFilterCallback ( [ 
	...
	public function varDirectiveDataProvider() {
		...
		$stub = $this->getMockBuilder ( '\Magento\Framework\DataObject' )->disableOriginalConstructor ()->disableProxyingToOriginalMethods ()->setMethods ( [ 
				'bar' 
		] )->getMock ();
		
		$stub->expects ( $this->once () )->method ( 'bar' )->will ( $this->returnCallback ( function ($arg) {
