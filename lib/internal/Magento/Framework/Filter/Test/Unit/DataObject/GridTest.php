...
class GridTest extends \PHPUnit_Framework_TestCase {
	public function testFilter() {
		$entityFactoryMock = $this->getMock ( 'Magento\Framework\Data\Collection\EntityFactoryInterface', [ ], [ ], '', false );
		$entityFactoryMock->expects ( $this->any () )->method ( 'create' )->with ( 'Magento\Framework\DataObject', [ ] )->will ( $this->returnCallback ( function () {
		...
		$filterMock->expects ( $this->exactly ( 4 ) )->method ( 'filter' )->will ( $this->returnCallback ( function ($input) {
			return '(' . $input . ')';
		} ) );
		...
		$fieldFilterMock->expects ( $this->exactly ( 2 ) )->method ( 'filter' )->will ( $this->returnCallback ( function ($input) {
			return '[' . $input . ']';
		} ) );
		...
		$this->assertEquals ( [ 
				'field1' => '(value11)',
				'field2' => '[(value12)]' 
		], $filteredGrid [0]->getData (), 'First grid item was filtered incorrectly.' );
		$this->assertEquals ( [ 
				'field3' => '(value23)',
				'field2' => '[(value22)]' 
		], $filteredGrid [1]->getData (), 'Second grid item was filtered incorrectly.' );
	}
}
