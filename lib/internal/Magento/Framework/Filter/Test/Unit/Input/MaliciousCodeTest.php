...
class MaliciousCodeTest extends \PHPUnit_Framework_TestCase {
	...
	public function testFilter($input, $expectedOutput) {
		$this->assertEquals ( $expectedOutput, $this->filter->filter ( $input ), 'Malicious code is not filtered out correctly.' );
	}
	...
	public function testAddExpression() {
		 ...
		 */
		'Custom malicious tag is removed customMalicious', $this->filter->filter ( "Custom \tmalicious tag\t\t is removed <customMalicious>customMalicious</customMalicious>" ), 'Custom filters are not applied correctly.' );
	}
	...
	public function testSetExpression() {
		 ...
		 */
		"Custom \tmalicious tag\t\t is removed customMalicious", $this->filter->filter ( "Custom \tmalicious tag\t\t is removed <customMalicious>customMalicious</customMalicious>" ), 'Native filters should have been replaced with custom ones.' );
	}
}
