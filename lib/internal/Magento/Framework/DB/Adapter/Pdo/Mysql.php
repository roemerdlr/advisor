...
class Mysql extends \Zend_Db_Adapter_Pdo_Mysql implements AdapterInterface {
	...
	/**
	 * SQL bind params.
	 * Used temporarily by regexp callback.
	 *
	 * @var array
	 */
	 ...
	 * Mysql specific property, designed only for backwards compatibility.
	 *
	 * @var array|null
	 */
	...
	/**
	 * Get adapter transaction level state.
	 * Return 0 if all transactions are complete
	 *
	 * @return int
	 */
	 ...
	 * @param mixed $bind
	 *        	An array of data or data itself to bind to the placeholders.
...
;
	 ...
	 * (e.g.
	 * "foo:bar"). And also changes named bind to positional one, because underlying library has problems
	 * with named binds.
	 *
	 ...
	 *
	 * @param string $sql        	
	 * @return array @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function dropForeignKey($tableName, $fkName, $schemaName = null) {
		...
		foreach ( [ 
			...
			if (isset ( $foreignKeys [$key] )) {
				$sql = sprintf ( 'ALTER TABLE %s DROP FOREIGN KEY %s', $this->quoteIdentifier ( $this->_getTableName ( $tableName, $schemaName ) ), $this->quoteIdentifier ( $foreignKeys [$key] ['FK_NAME'] ) );
	 ...
	 * @return $this
	 */
	public function purgeOrphanRecords($tableName, $columnName, $refTableName, $refColumnName, $onDelete = AdapterInterface::FK_ACTION_CASCADE) {
		$onDelete = strtoupper ( $onDelete );
		if ($onDelete == AdapterInterface::FK_ACTION_CASCADE || $onDelete == AdapterInterface::FK_ACTION_RESTRICT) {
			$sql = sprintf ( "DELETE p.* FROM %s AS p LEFT JOIN %s AS r ON p.%s = r.%s WHERE r.%s IS NULL", $this->quoteIdentifier ( $tableName ), $this->quoteIdentifier ( $refTableName ), $this->quoteIdentifier ( $columnName ), $this->quoteIdentifier ( $refColumnName ), $this->quoteIdentifier ( $refColumnName ) );
			$this->rawQuery ( $sql );
		} elseif ($onDelete == AdapterInterface::FK_ACTION_SET_NULL) {
			$sql = sprintf ( "UPDATE %s AS p LEFT JOIN %s AS r ON p.%s = r.%s SET p.%s = NULL WHERE r.%s IS NULL", $this->quoteIdentifier ( $tableName ), $this->quoteIdentifier ( $refTableName ), $this->quoteIdentifier ( $columnName ), $this->quoteIdentifier ( $refColumnName ), $this->quoteIdentifier ( $columnName ), $this->quoteIdentifier ( $refColumnName ) );
			$this->rawQuery ( $sql );
		}
	 ...
	 *        	string specific or universal array DB Server definition
	 * @param string $schemaName        	
	...
	public function addColumn($tableName, $columnName, $definition, $schemaName = null) {
		...
		$sql = sprintf ( 'ALTER TABLE %s ADD COLUMN %s %s %s', $this->quoteIdentifier ( $this->_getTableName ( $tableName, $schemaName ) ), $this->quoteIdentifier ( $columnName ), $definition, $primaryKey );
	...
	public function dropColumn($tableName, $columnName, $schemaName = null) {
		...
		$sql = sprintf ( 'ALTER TABLE %s %s', $this->quoteIdentifier ( $this->_getTableName ( $tableName, $schemaName ) ), implode ( ', ', $alterDrop ) );
	...
	public function changeColumn($tableName, $oldColumnName, $newColumnName, $definition, $flushData = false, $schemaName = null) {
		if (! $this->tableColumnExists ( $tableName, $oldColumnName, $schemaName )) {
			throw new \Zend_Db_Exception ( sprintf ( 'Column "%s" does not exist in table "%s".', $oldColumnName, $tableName ) );
		}
		...
		$sql = sprintf ( 'ALTER TABLE %s CHANGE COLUMN %s %s %s', $this->quoteIdentifier ( $tableName ), $this->quoteIdentifier ( $oldColumnName ), $this->quoteIdentifier ( $newColumnName ), $definition );
	...
	public function modifyColumn($tableName, $columnName, $definition, $flushData = false, $schemaName = null) {
		...
		$sql = sprintf ( 'ALTER TABLE %s MODIFY COLUMN %s %s', $this->quoteIdentifier ( $tableName ), $this->quoteIdentifier ( $columnName ), $definition );
	...
	public function getForeignKeys($tableName, $schemaName = null) {
		...
		if ($ddl === false) {
			...
			// collect CONSTRAINT
			$regExp = '#,\s+CONSTRAINT `([^`]*)` FOREIGN KEY ?\(`([^`]*)`\) ' . 'REFERENCES (`([^`]*)`\.)?`([^`]*)` \(`([^`]*)`\)' . '( ON DELETE (RESTRICT|CASCADE|SET NULL|NO ACTION))?' . '( ON UPDATE (RESTRICT|CASCADE|SET NULL|NO ACTION))?#';
			$matches = [ ];
		...
		foreach ( $tables as $table => $tableData ) {
			...
			foreach ( $tableData ['columns'] as $column => $columnDefinition ) {
				...
				foreach ( $droppedKeys as $options ) {
					...
					$this->addForeignKey ( $options ['FK_NAME'], $options ['TABLE_NAME'], $options ['COLUMN_NAME'], $options ['REF_TABLE_NAME'], $options ['REF_COLUMN_NAME'], ($onDelete) ? $onDelete : AdapterInterface::FK_ACTION_NO_ACTION );
				}
			}
	...
	public function getIndexList($tableName, $schemaName = null) {
		...
		if ($ddl === false) {
			...
			$sql = sprintf ( 'SHOW INDEX FROM %s', $this->quoteIdentifier ( $this->_getTableName ( $tableName, $schemaName ) ) );
...
;
	...
	public function saveDdlCache($tableCacheKey, $ddlType, $data) {
		...
		if ($this->_cacheAdapter) {
			$cacheId = $this->_getCacheId ( $tableCacheKey, $ddlType );
			$data = serialize ( $data );
			$this->_cacheAdapter->save ( $data, $cacheId, [ 
					self::DDL_CACHE_TAG 
			] );
		}
	...
	public function resetDdlCache($tableName = null, $schemaName = null) {
		...
		if ($tableName === null) {
			$this->_ddlCache = [ ];
			if ($this->_cacheAdapter) {
				$this->_cacheAdapter->clean ( \Zend_Cache::CLEANING_MODE_MATCHING_TAG, [ 
						self::DDL_CACHE_TAG 
				] );
			}
		} else {
	...
	public function getColumnCreateByDescribe($columnData) {
		...
		if ($columnData ['NULLABLE'] === false && ! ($type == Table::TYPE_TEXT && strlen ( $columnData ['DEFAULT'] ) != 0)) {
			$options ['nullable'] = false;
		}
		if ($columnData ['PRIMARY'] === true) {
			$options ['primary'] = true;
		}
		if (! is_null ( $columnData ['DEFAULT'] ) && $type != Table::TYPE_TEXT) {
	...
	public function createTableByDdl($tableName, $newTableName) {
		...
		foreach ( $describe as $columnData ) {
			...
			$table->addColumn ( $columnInfo ['name'], $columnInfo ['type'], $columnInfo ['length'], $columnInfo ['options'], $columnInfo ['comment'] );
		}
		...
		foreach ( $foreignKeys as $keyData ) {
			$fkName = $this->getForeignKeyName ( $newTableName, $keyData ['COLUMN_NAME'], $keyData ['REF_TABLE_NAME'], $keyData ['REF_COLUMN_NAME'] );
			$onDelete = $this->_getDdlAction ( $keyData ['ON_DELETE'] );
			
			$table->addForeignKey ( $fkName, $keyData ['COLUMN_NAME'], $keyData ['REF_TABLE_NAME'], $keyData ['REF_COLUMN_NAME'], $onDelete );
		}
	 ...
	 *
	 * @param array $column        	
	 * @return string @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	 ...
	 * @param array $data
	 *        	Column-value pairs or array of column-value pairs.
	 * @param array $fields
	 *        	update fields pairs or values
	 * @return int The number of affected rows.
	 * @throws \Zend_Db_Exception @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	 ...
	 *
	 * @param string|array|\Zend_Db_Expr $table
	 *        	The table to insert data into.
	 * @param array $data
	 *        	Column-value pairs or array of Column-value pairs.
	 * @return int The number of affected rows.
	...
	public function createTable(Table $table) {
		...
		$sqlFragment = array_merge ( $this->_getColumnsDefinition ( $table ), $this->_getIndexesDefinition ( $table ), $this->_getForeignKeysDefinition ( $table ) );
	 ...
	 * @throws \Zend_Db_Exception
	 * @return \Zend_Db_Pdo_Statement @SuppressWarnings(PHPMD.UnusedLocalVariable)
	 */
	 ...
	 *
	 * @param array $tablePairs
	 *        	array('oldName' => 'Name1', 'newName' => 'Name2')
	 *        	
	 * @return boolean
	 * @throws \Zend_Db_Exception
	 */
	...
	protected function _getForeignKeysDefinition(Table $table) {
		...
		if (! empty ( $relations )) {
			foreach ( $relations as $fkData ) {
				...
				$definition [] = sprintf ( '  CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s (%s) ON DELETE %s', $this->quoteIdentifier ( $fkData ['FK_NAME'] ), $this->quoteIdentifier ( $fkData ['COLUMN_NAME'] ), $this->quoteIdentifier ( $fkData ['REF_TABLE_NAME'] ), $this->quoteIdentifier ( $fkData ['REF_COLUMN_NAME'] ), $onDelete );
			}
		}
	 ...
	 *        	Table DDL Column type constant
	...
	protected function _getColumnDefinition($options, $ddlType = null) {
		...
		return sprintf ( '%s%s%s%s%s COMMENT %s %s', $cType, $cUnsigned ? ' UNSIGNED' : '', $cNullable ? ' NULL' : ' NOT NULL', $cDefault !== false ? $this->quoteInto ( ' default ?', $cDefault ) : '', $cIdentity ? ' auto_increment' : '', $this->quote ( $comment ), $after ? 'AFTER ' . $this->quoteIdentifier ( $after ) : '' );
	}
	 ...
	 *        	the table column name or array of ones
	 * @param string $indexType
	 *        	the index type
	 * @param string $schemaName        	
	 * @return \Zend_Db_Statement_Interface
	 * @throws \Zend_Db_Exception
	 * @throws \Exception @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	public function addIndex($tableName, $indexName, $fields, $indexType = AdapterInterface::INDEX_TYPE_INDEX, $schemaName = null) {
		...
		foreach ( $fields as $field ) {
			if (! isset ( $columns [$field] )) {
				$msg = sprintf ( 'There is no field "%s" that you are trying to create an index on "%s"', $field, $tableName );
		...
		while ( $cycle === true ) {
			try {
				...
				if (in_array ( strtolower ( $indexType ), [ 
						'primary',
						'unique' 
				] )) {
					$match = [ ];
	...
	public function dropIndex($tableName, $keyName, $schemaName = null) {
		...
		$sql = sprintf ( 'ALTER TABLE %s %s', $this->quoteIdentifier ( $this->_getTableName ( $tableName, $schemaName ) ), $cond );
	 ...
	 * @param string $onDelete        	
	 * @param bool $purge
	 *        	trying remove invalid data
	 * @param string $schemaName        	
	 * @param string $refSchemaName        	
	 * @return \Zend_Db_Statement_Interface @SuppressWarnings(PHPMD.ExcessiveParameterList)
	 */
	public function addForeignKey($fkName, $tableName, $columnName, $refTableName, $refColumnName, $onDelete = AdapterInterface::FK_ACTION_CASCADE, $purge = false, $schemaName = null, $refSchemaName = null) {
		...
		$query = sprintf ( 'ALTER TABLE %s ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s (%s)', $this->quoteIdentifier ( $this->_getTableName ( $tableName, $schemaName ) ), $this->quoteIdentifier ( $fkName ), $this->quoteIdentifier ( $columnName ), $this->quoteIdentifier ( $this->_getTableName ( $refTableName, $refSchemaName ) ), $this->quoteIdentifier ( $refColumnName ) );
	 ...
	 *
	 * @param array $column
	 *        	the column describe array
	 * @param mixed $value        	
	 * @return mixed @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	 ...
	 *        	OPTIONAL. Applies when $expression is NULL
	 * @return \Zend_Db_Expr
	 */
	 ...
	 * @param array $casesResults
	 *        	Cases and results
	 * @param string $defaultValue
	 *        	value to use if value doesn't confirm to any cases
	 * @return \Zend_Db_Expr
	 */
	 ...
	 *        	quoted field name or SQL statement
	 * @param int $interval        	
	 * @param string $unit        	
	 * @return \Zend_Db_Expr
	 */
	 ...
	 *
	 * @param string $date
	 *        	quoted date value or non quoted SQL statement(field)
	 * @param string $format        	
	 * @return \Zend_Db_Expr
	 */
	 ...
	 *
	 * @param \Zend_Db_Expr|string $stringExpression
	 *        	quoted field name or SQL statement
	 ...
	 *
	 * @param \Zend_Db_Expr|string $expressionField
	 *        	quoted field name or SQL statement
	 * @return \Zend_Db_Expr
	 */
	 ...
	 *
	 * @param string $tableName
	 *        	The table which is the subject of the trigger
	 * @param string $time
	 *        	Either "before" or "after"
	 * @param string $event
	 *        	The DB level event which activates the trigger, i.e. "update" or "insert"
	 * @return string @codeCoverageIgnore
	 */
	...
	public function selectsByRange($rangeField, \Magento\Framework\DB\Select $select, $stepCount = 100) {
		...
		if (empty ( $fromSelect )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Select object must have correct "FROM" part' ) );
		}
		...
		$selectRange = $this->select ()->from ( $tableName, [ 
		...
		while ( $min <= $max ) {
			...
			$partialSelect->where ( $this->quoteIdentifier ( $correlationName ) . '.' . $this->quoteIdentifier ( $rangeField ) . ' >= ?', $min )->where ( $this->quoteIdentifier ( $correlationName ) . '.' . $this->quoteIdentifier ( $rangeField ) . ' < ?', $min + $stepCount );
	 ...
	 * @return string
	 * @throws LocalizedException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 *         @SuppressWarnings(PHPMD.NPathComplexity)
	 */
	public function updateFromSelect(Select $select, $table) {
		...
		foreach ( $select->getPart ( \Magento\Framework\DB\Select::COLUMNS ) as $columnEntry ) {
			...
			$columns [] = sprintf ( '%s = %s', $this->quoteIdentifier ( [ 
					$tableAlias,
					$alias 
			] ), $column );
		}
		...
		if (! $columns) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'The columns for UPDATE statement are not defined' ) );
		}
	 ...
	 * @param Select $select        	
	 * @param string $table
	 *        	the table name or alias used in select
	 * @return string
	 */
	 ...
	 *        	array of tables names | table name
	 * @param string $schemaName
	 *        	schema name
	 * @return array
	 */
	...
	public function createTrigger(\Magento\Framework\DB\Ddl\Trigger $trigger) {
		if (! $trigger->getStatements ()) {
			throw new \Zend_Db_Exception ( ( string ) new \Magento\Framework\Phrase ( 'Trigger %1 has not statements available', [ 
					$trigger->getName () 
			] ) );
		}
