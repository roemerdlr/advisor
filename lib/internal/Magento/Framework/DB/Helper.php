...
class Helper extends \Magento\Framework\DB\Helper\AbstractHelper {
		...
		foreach ( $columns as $columnEntry ) {
			...
			if ($column instanceof \Zend_Db_Expr) {
				if ($alias !== null) {
					...
					$preparedColumns [strtoupper ( $alias )] = [ 
							null,
							$column,
							$alias 
					];
				...
				if ($column == \Magento\Framework\DB\Select::SQL_WILDCARD) {
					...
					foreach ( array_keys ( $tableColumns ) as $col ) {
						$preparedColumns [strtoupper ( $col )] = [ 
								$correlationName,
								$col,
								null 
						];
					...
					$preparedColumns [strtoupper ( $columnKey )] = [ 
							$correlationName,
							$column,
							$alias 
					];
	 ...
	 *        	Field alias which will be added with column group_concat expression
	 * @param string $fields        	
	...
	public function addGroupConcatColumn($select, $fieldAlias, $fields, $groupConcatDelimiter = ',', $fieldsDelimiter = '', $additionalWhere = '') {
		...
		$select->columns ( [ 
				$fieldAlias => new \Zend_Db_Expr ( sprintf ( 'GROUP_CONCAT(%s%s)', $fieldExpr, $separator ) ) 
		] );
		return $select;
	}
