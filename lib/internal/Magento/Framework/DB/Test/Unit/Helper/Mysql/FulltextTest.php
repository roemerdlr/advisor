...
class FulltextTest extends \PHPUnit_Framework_TestCase {
	public function testGetMatchQuery() {
		/** @var Fulltext $select */
		$select = (new ObjectManager ( $this ))->getObject ( 'Magento\Framework\DB\Helper\Mysql\Fulltext', [ 
		...
		$result = $select->getMatchQuery ( [ 
				'title',
				'description' 
		], 'some searchable text', Fulltext::FULLTEXT_MODE_NATURAL );
	...
	/**
	 *
	 * @param $isCondition @dataProvider
	 *        	matchProvider
	 */
	public function testMatch($isCondition) {
		...
		$select->expects ( $this->once () )->method ( $isCondition ? 'where' : 'orWhere' )->with ( $fullCondition );
		
		/** @var Fulltext $fulltext */
		$fulltext = (new ObjectManager ( $this ))->getObject ( 'Magento\Framework\DB\Helper\Mysql\Fulltext', [ 
				'resource' => $resource 
		] );
		
		$result = $fulltext->match ( $select, [ 
				'title',
				'description' 
		], 'some searchable text', $isCondition, Fulltext::FULLTEXT_MODE_NATURAL );
	...
	protected function getResourceMock() {
		$connection = $this->getMockBuilder ( 'Magento\Framework\DB\Adapter\AdapterInterface' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		$connection->expects ( $this->at ( 0 ) )->method ( 'quote' )->with ( $this->equalTo ( 'some searchable text' ) )->will ( $this->returnValue ( "'some searchable text'" ) );
