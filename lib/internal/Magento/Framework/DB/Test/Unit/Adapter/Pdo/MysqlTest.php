...
class MysqlTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_mockAdapter = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'beginTransaction',
				'getTransactionLevel' 
		], [ 
				'string' => $string,
		...
		$this->_mockAdapter->expects ( $this->any () )->method ( 'getTransactionLevel' )->will ( $this->returnValue ( 1 ) );
		
		$this->_adapter = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'getCreateTable',
				'_connect',
				'_beginTransaction',
				'_commit',
		...
		$resourceProperty = new \ReflectionProperty ( get_class ( $this->_adapter ), '_profiler' );
	...
	public function bigintResultProvider() {
		return [ 
				...
				[ 
						9223372036854775807.3423424234,
						'9223372036854775808' 
				],
				[ 
						2147483647 * pow ( 10, 10 ) + 12,
						'21474836470000001024' 
				],
				[ 
						9223372036854775807 * pow ( 10, 10 ) + 12,
						'92233720368547758080000000000' 
				],
				[ 
						(0.099999999999999999999999995 + 0.2 + 0.3 + 0.4 + 0.5) * 10,
						'15' 
				],
				[ 
						'21474836470000000012',
						'21474836470000001024' 
				],
				[ 
						0x5468792130ABCDEF,
	...
	public function testAsymmetricRollBackFailure() {
		try {
			...
			$this->assertEquals ( AdapterInterface::ERROR_ASYMMETRIC_ROLLBACK_MESSAGE, $e->getMessage () );
		}
	}
	...
	public function testAsymmetricCommitFailure() {
		try {
			...
			$this->assertEquals ( AdapterInterface::ERROR_ASYMMETRIC_COMMIT_MESSAGE, $e->getMessage () );
		}
	}
	...
	public function testIncompleteRollBackFailureOnCommit() {
		...
		try {
			...
			$this->assertEquals ( AdapterInterface::ERROR_ROLLBACK_INCOMPLETE_MESSAGE, $e->getMessage () );
			$this->_adapter->rollBack ();
		}
	}
	...
	public function testSelectsByRange() {
		...
		$this->_adapter->expects ( $this->once () )->method ( 'fetchRow' )->with ( $this->_adapter->select ()->from ( $tableName, [ 
	...
	public function testAddColumn($options, $expectedQuery) {
		$connectionMock = $this->getMock ( '\Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'tableColumnExists',
				'_getTableName',
				'rawQuery',
				'resetDdlCache',
	...
	public function addColumnDataProvider() {
		return [ 
				[ 
						...
						'expectedQuery' => 'ALTER TABLE `tableName` ADD COLUMN `columnName` int UNSIGNED ' . 'NOT NULL default  auto_increment COMMENT Some field AFTER `Previous field` ' 
				] 
		];
	}
	...
	public function testGetIndexName($name, $fields, $indexType, $expectedName) {
		...
		$this->assertTrue ( strpos ( $resultIndexName, $expectedName ) === 0, "Index name '$resultIndexName' did not begin with expected value '$expectedName'" );
	}
	public function getIndexNameDataProvider() {
		...
		return [ 
				...
				[ 
						...
						'',
						'SHORT_TABLE_NAME_FIELD1_FIELD2' 
				] 
		];
	}
}
