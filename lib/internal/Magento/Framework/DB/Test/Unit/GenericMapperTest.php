...
class GenericMapperTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->mapperFactoryMock = $this->getMock ( 'Magento\Framework\DB\MapperFactory', [ 
				'create' 
		], [ ], '', false );
		
		$this->geneticMapper = $objectManager->getObject ( 'Magento\Framework\DB\GenericMapper', [ 
	...
	public function testMapCriteriaList() {
		...
		$mapperInstanceMock = $this->getMockForAbstractClass ( 'Magento\Framework\DB\MapperInterface', [ ], '', false, true, true, [ 
				'map' 
		] );
		
		$criteriaMock->expects ( $this->any () )->method ( 'getMapperInterfaceName' )->will ( $this->returnValue ( 'mapper-name' ) );
		$this->mapperFactoryMock->expects ( $this->exactly ( 4 ) )->method ( 'create' )->with ( 'mapper-name', [ 
				'select' => $this->selectMock 
	...
	public function testMapFilters() {
		...
		$connectionMock = $this->getMockForAbstractClass ( 'Magento\Framework\DB\Adapter\AdapterInterface', [ ], '', false, true, true, [ 
				'quoteInto',
				'prepareSqlCondition' 
		] );
		
		/** @var \Magento\Framework\DB\GenericMapper|\PHPUnit_Framework_MockObject_MockObject $geneticMapper */
		$geneticMapper = $this->getMock ( 'Magento\Framework\DB\GenericMapper', [ 
				'getConnection',
				'getSelect' 
		], [ ], '', false );
		
		$geneticMapper->expects ( $this->any () )->method ( 'getConnection' )->will ( $this->returnValue ( $connectionMock ) );
		$geneticMapper->expects ( $this->exactly ( 4 ) )->method ( 'getSelect' )->will ( $this->returnValue ( $this->selectMock ) );
		$connectionMock->expects ( $this->exactly ( 2 ) )->method ( 'quoteInto' )->with ( 'test-field=?', 'test-condition' )->will ( $this->returnValue ( 'test-condition' ) );
		$this->selectMock->expects ( $this->once () )->method ( 'orWhere' )->with ( 'test-condition' );
		$this->selectMock->expects ( $this->exactly ( 3 ) )->method ( 'where' )->with ( 'test-condition' );
		$connectionMock->expects ( $this->any () )->method ( 'prepareSqlCondition' )->with ( 'test-field', 'test-condition' )->will ( $this->returnValue ( 'test-condition' ) );
