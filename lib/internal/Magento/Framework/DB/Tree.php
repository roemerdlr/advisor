...
class Tree {
	 ...
	 * @param string|int $nodeId        	
	 * @param array $data        	
	 * @return false|string @SuppressWarnings(PHPMD.ExitExpression)
	 */
	public function appendChild($nodeId, $data) {
		...
		if ($nodeId) {
			...
			try {
				...
				$this->_db->query ( $sql, [ 
						'left' => $info [$this->_left],
						'right' => $info [$this->_right] 
				] );
				$this->_db->insert ( $this->_table, $data );
				$this->_db->commit ();
	...
	public function checkNodes() {
		...
		$sql->from ( [ 
				...
				't1.' . $this->_id,
				new \Zend_Db_Expr ( 'COUNT(t1.' . $this->_id . ') AS rep' ) 
		] )->from ( [ 
				't2' => $this->_table 
		] )->from ( [ 
				't3' => $this->_table 
	 ...
	 * @param int $endLevel        	
	 * @return NodeSet @SuppressWarnings(PHPMD.ExitExpression)
	 */
