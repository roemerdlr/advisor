...
class Select extends \Zend_Db_Select {
	 ...
	 *        	The WHERE condition.
	 * @param string $value
	 *        	OPTIONAL A single value to quote into the condition.
	 ...
	 *
	 * @param bool $flag
	 *        	Whether or not the SELECT use STRAIGHT_JOIN (default true).
	 * @return $this
	 */
	...
	public function exists($select, $joinCondition, $isExists = true) {
		...
		$select->reset ( self::COLUMNS )->columns ( [ 
				new \Zend_Db_Expr ( '1' ) 
		] )->where ( $joinCondition );
