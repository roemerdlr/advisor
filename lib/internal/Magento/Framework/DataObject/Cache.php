...
class Cache {
	...
	public function save($object, $idx = null, $tags = null) {
		...
		if (isset ( $this->_objects [$idx] )) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Object already exists in registry (%1). Old object class: %2, new object class: %3', [ 
					$idx,
					get_class ( $this->_objects [$idx] ),
					get_class ( $object ) 
			] ) );
		}
	...
	public function reference($refName, $idx) {
		...
		if (isset ( $this->_references [$refName] )) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'The reference already exists: %1. New index: %2, old index: %3', [ 
					$refName,
					$idx,
