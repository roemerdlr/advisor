...
class CacheTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * @expectedExceptionMessage Object already exists in registry (#1).
	 * Old object class: stdClass
	 */
	public function testSaveWhenObjectAlreadyExistsInRegistry() {
		...
		$this->assertEquals ( [ 
				$newIdx => $object 
		], $this->cache->findByClass ( 'stdClass' ) );
		$this->assertEquals ( [ 
				$newIdx => $object 
		], $this->cache->getAllObjects () );
		$this->assertEquals ( $newIdx, $this->cache->find ( $object ) );
		$this->assertEquals ( [ 
				$newIdx => $object 
		], $this->cache->findByIds ( [ 
				$newIdx 
		] ) );
	...
	/**
	 * @expectedException \Magento\Framework\Exception\LocalizedException
	 * @expectedExceptionMessage The reference already exists: refName.
	 * New index: idx, old index: idx
	 */
	...
	public function testGetAndDeleteTags() {
		...
		$this->assertEquals ( [ 
				$newIdx => $object 
		], $this->cache->findByTags ( 'tags_array' ) );
