...
class Editor extends Textarea {
	...
	protected function _getPluginButtonsHtml($visible = true) {
		...
		if ($this->getConfig ( 'add_images' )) {
			$buttonsHtml .= $this->_getButtonHtml ( [ 
					...
					'onclick' => "MediabrowserUtility.openDialog('" . $this->getConfig ( 'files_browser_window_url' ) . "target_element_id/" . $this->getHtmlId () . "/" . (null !== $this->getConfig ( 'store_id' ) ? 'store/' . $this->getConfig ( 'store_id' ) . '/' : '') . "')",
			...
			foreach ( $this->getConfig ( 'plugins' ) as $plugin ) {
				if (isset ( $plugin ['options'] ) && $this->_checkPluginButtonOptions ( $plugin ['options'] )) {
					...
					if (! $visible) {
						...
						$buttonOptions = array_merge ( $buttonOptions, [ 
								'style' => 'display:none;' . $configStyle 
						] );
					}
	...
	protected function _wrapIntoContainer($html) {
		...
		$html = '<div id="editor' . $this->getHtmlId () . '"' . ($this->getConfig ( 'no_display' ) ? ' style="display:none;"' : '') . ($this->getConfig ( 'container_class' ) ? ' class="admin__control-wysiwig ' . $this->getConfig ( 'container_class' ) . '"' : '') . '>' . $html . '</div>';
