...
class Structure {
	...
	public function importElements(array $elements) {
		...
		foreach ( $elements as $elementId => $element ) {
			if (is_numeric ( $elementId )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( "Element ID must not be numeric: '%1'.", [ 
						$elementId 
				] ) );
			}
			...
			if (isset ( $element [self::GROUPS] )) {
				...
				foreach ( $groups as $groupName => $group ) {
					...
					if ($group !== array_flip ( $group )) {
						throw new LocalizedException ( new \Magento\Framework\Phrase ( "Invalid format of group '%1': %2", [ 
								$groupName,
								var_export ( $group, 1 ) 
						] ) );
					}
	...
	protected function _assertParentRelation($elementId) {
		...
		if (isset ( $element [self::PARENT] )) {
			...
			if (empty ( $this->_elements [$parentId] [self::CHILDREN] [$elementId] )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( "Broken parent-child relation: the '%1' is not in the nested set of '%2'.", [ 
						$elementId,
						$parentId 
				] ) );
			}
		...
		if (isset ( $element [self::CHILDREN] )) {
			...
			if ($children !== array_flip ( array_flip ( $children ) )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Invalid format of children: %1', [ 
						var_export ( $children, 1 ) 
				] ) );
			}
			foreach ( array_keys ( $children ) as $childId ) {
				$this->_assertElementExists ( $childId );
				if (! isset ( $this->_elements [$childId] [self::PARENT] ) || $elementId !== $this->_elements [$childId] [self::PARENT]) {
					throw new LocalizedException ( new \Magento\Framework\Phrase ( "Broken parent-child relation: the '%1' is supposed to have '%2' as parent.", [ 
							$childId,
							$elementId 
					] ) );
				}
	...
	public function createElement($elementId, array $data) {
		if (isset ( $this->_elements [$elementId] )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "Element with ID '%1' already exists.", [ 
					$elementId 
			] ) );
		}
	...
	public function renameElement($oldId, $newId) {
		...
		if (! $newId || isset ( $this->_elements [$newId] )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "Element with ID '%1' is already defined.", [ 
					$newId 
			] ) );
		}
	...
	public function setAsChild($elementId, $parentId, $alias = '', $position = null) {
		if ($elementId == $parentId) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "The '%1' cannot be set as child to itself.", [ 
					$elementId 
			] ) );
		}
		if ($this->_isParentRecursively ( $elementId, $parentId )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "The '%1' is a parent of '%2' recursively, therefore '%3' cannot be set as child to it.", [ 
					$elementId,
					$parentId,
					$elementId 
			] ) );
	 ...
	 *
	 * @param string $elementId
	 *        	ID of an element or its parent element
	...
	public function getChildren($parentId) {
		return isset ( $this->_elements [$parentId] [self::CHILDREN] ) ? $this->_elements [$parentId] [self::CHILDREN] : [ ];
	}
	 ...
	 *
	 * @param string $parentId
	 *        	Name of an element containing group
	 * @param string $groupName        	
	 * @return array
	 */
	...
	protected function _getChildOffset($parentId, $childId) {
		...
		if (false === $index) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "The '%1' is not a child of '%2'.", [ 
					$childId,
					$parentId 
			] ) );
		}
	...
	protected function _insertChild($targetParentId, $elementId, $offset, $alias) {
		...
		if (! empty ( $this->_elements [$elementId] [self::PARENT] )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "The element '%1' already has a parent: '%2'", [ 
					$elementId,
					$this->_elements [$elementId] [self::PARENT] 
			] ) );
		}
		$this->_assertElementExists ( $targetParentId );
		$children = $this->getChildren ( $targetParentId );
		if (isset ( $children [$elementId] )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "The element '%1' already a child of '%2'", [ 
					$elementId,
					$targetParentId 
			] ) );
		}
		if (false !== array_search ( $alias, $children )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "The element '%1' already has a child with alias '%2'", [ 
					$targetParentId,
					$alias 
			] ) );
		}
		...
		$this->_elements [$targetParentId] [self::CHILDREN] = array_merge ( array_slice ( $children, 0, $offset ), [ 
				$elementId => $alias 
	...
	private function _assertArray($value) {
		if (! is_array ( $value )) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( "An array expected: %1", [ 
					var_export ( $value, 1 ) 
			] ) );
		}
	}
