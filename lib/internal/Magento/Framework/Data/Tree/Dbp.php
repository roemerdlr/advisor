...
class Dbp extends \Magento\Framework\Data\Tree {
	...
	public function load($parentNode = null, $recursionLevel = 0) {
		if (! $this->_loaded) {
			...
			if ($parentPath) {
				$pathField = $this->_conn->quoteIdentifier ( [ 
						$this->_table,
						$this->_pathField 
				] );
			...
			if ($recursionLevel != 0) {
				$levelField = $this->_conn->quoteIdentifier ( [ 
						$this->_table,
						$this->_levelField 
				] );
	...
	public function loadNode($nodeId) {
		...
		if (is_numeric ( $nodeId )) {
			$condField = $this->_conn->quoteIdentifier ( [ 
					$this->_table,
					$this->_idField 
			] );
		} else {
	...
	public function move($node, $newParent, $prevNode = null) {
		...
		$reorderData = [ 
				$this->_orderField => new \Zend_Db_Expr ( "{$this->_orderField} + 1" ) 
		];
		try {
