...
class DbTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->fetchStrategyMock = $this->getMock ( 'Magento\Framework\Data\Collection\Db\FetchStrategy\Query', [ 
				'fetchAll' 
		], [ ], '', false );
		...
		$this->collection = new \Magento\Framework\Data\Test\Unit\Collection\DbCollection ( $this->entityFactoryMock, $this->loggerMock, $this->fetchStrategyMock );
	}
	...
	public function testSetAddOrder() {
		$adapter = $this->getMock ( 'Magento\Framework\DB\Adapter\Pdo\Mysql', [ 
				'fetchAll' 
		], [ ], '', false );
	...
	public function testAddFieldToFilter() {
		...
		$this->collection->addFieldToFilter ( 'is_imported', [ 
				'eq' => '1' 
		] );
	...
	public function testAddFieldToFilterWithMultipleParams() {
		...
		$adapter->expects ( $this->at ( 0 ) )->method ( 'prepareSqlCondition' )->with ( '`weight`', [ 
				'in' => [ 
						1,
						3 
				] 
		] )->will ( $this->returnValue ( 'weight in (1, 3)' ) );
		$adapter->expects ( $this->at ( 1 ) )->method ( 'prepareSqlCondition' )->with ( '`name`', [ 
				'like' => 'M%' 
		...
		$this->assertEquals ( "SELECT `test`.* FROM `test` WHERE ((weight in (1, 3)) OR (name like 'M%'))", $select->assemble () );
		
		$adapter->expects ( $this->at ( 0 ) )->method ( 'prepareSqlCondition' )->with ( '`is_imported`', $this->anything () )->will ( $this->returnValue ( 'is_imported = 1' ) );
		
		$this->collection->addFieldToFilter ( 'is_imported', [ 
				'eq' => '1' 
		] );
		$this->assertEquals ( "SELECT `test`.* FROM `test` WHERE ((weight in (1, 3)) OR (name like 'M%')) AND (is_imported = 1)", $select->assemble () );
	}
	...
	public function testAddFieldToFilterValueContainsQuestionMark() {
		...
		$select = $this->collection->getSelect ()->from ( 'test' );
		$this->collection->addFieldToFilter ( 'email', [ 
				'like' => 'value?' 
		] );
	...
	public function testFetchItem() {
		...
		$counter = 0;
		$statementMock = $this->getMock ( 'Zend_Db_Statement_Pdo', [ 
				'fetch' 
		], [ ], '', false );
	...
	public function testGetSize() {
		...
		$adapterMock->expects ( $this->once () )->method ( 'select' )->will ( $this->returnValue ( $selectMock ) );
		$adapterMock->expects ( $this->exactly ( 2 ) )->method ( 'quoteInto' )->will ( $this->returnValueMap ( [ 
				[ 
						'testField1=?',
						'testValue1',
						null,
		...
		$selectMock->expects ( $this->exactly ( 3 ) )->method ( 'where' )->will ( $this->returnValueMap ( [ 
				[ 
						'testValue2',
						$this->returnSelf () 
				],
	...
	public function testDistinct($flag, $expectedFlag) {
		...
		$adapterMock->expects ( $this->once () )->method ( 'select' )->will ( $this->returnValue ( $selectMock ) );
		$selectMock->expects ( $this->once () )->method ( 'distinct' )->with ( $expectedFlag );
	...
	public function testToOptionHash() {
		...
		$this->fetchStrategyMock->expects ( $this->once () )->method ( 'fetchAll' )->with ( $selectMock, [ ] )->will ( $this->returnValue ( [ 
				$data 
		] ) );
		...
		$objectMock->expects ( $this->any () )->method ( 'getData' )->will ( $this->returnValueMap ( [ 
				[ 
						null,
						null,
						10 
