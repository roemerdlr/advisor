...
class CacheTest extends \PHPUnit_Framework_TestCase {
	...
	private $_fixtureData = [ 
			[ 
					'column_one' => 'row_one_value_one',
					'column_two' => 'row_one_value_two' 
			],
			[ 
					'column_one' => 'row_two_value_one',
					'column_two' => 'row_two_value_two' 
			] 
	];
	protected function setUp() {
		$this->_select = $this->getMock ( 'Magento\Framework\DB\Select', [ 
				'assemble' 
		], [ ], '', false );
		$this->_select->expects ( $this->once () )->method ( 'assemble' )->will ( $this->returnValue ( 'SELECT * FROM fixture_table' ) );
		...
		$this->_object = new \Magento\Framework\Data\Collection\Db\FetchStrategy\Cache ( $this->_cache, $this->_fetchStrategy, 'fixture_', [ 
				'fixture_tag_one',
				'fixture_tag_two' 
		], 86400 );
	}
	...
	public function testFetchAllCached() {
		$this->_cache->expects ( $this->once () )->method ( 'load' )->with ( 'fixture_06a6b0cfd83bf997e76b1b403df86569' )->will ( $this->returnValue ( serialize ( $this->_fixtureData ) ) );
	...
	public function testFetchAllDelegation() {
		...
		$this->_fetchStrategy->expects ( $this->once () )->method ( 'fetchAll' )->with ( $this->_select, $bindParams )->will ( $this->returnValue ( $this->_fixtureData ) );
		$this->_cache->expects ( $this->once () )->method ( 'save' )->with ( serialize ( $this->_fixtureData ), $cacheId, [ 
				'fixture_tag_one',
				'fixture_tag_two' 
		], 86400 );
