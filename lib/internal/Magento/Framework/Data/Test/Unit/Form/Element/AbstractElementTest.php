...
class AbstractElementTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_factoryMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Factory', [ ], [ ], '', false );
		$this->_collectionFactoryMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\CollectionFactory', [ ], [ ], '', false );
	...
	public function testAddElement() {
		$elementId = 11;
		$elementMock = $this->getMockForAbstractClass ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ ], '', false, true, true, [ 
				'getId' 
		] );
	...
	public function testGetNameWithoutSuffix() {
		...
		$formMock->expects ( $this->any () )->method ( 'getFieldNameSuffix' )->will ( $this->returnValue ( null ) );
		$formMock->expects ( $this->never () )->method ( 'addSuffixToName' );
	...
	public function testRemoveField() {
		...
		$collectionMock = $this->getMock ( '\Magento\Framework\Data\Form\Element\Collection', [ 
				'remove' 
		], [ ], '', false );
	...
	public function testGetEscapedValueWithoutFilter() {
		...
		$this->assertEquals ( '&lt;a href=&quot;#hash_tag&quot;&gt;my \'quoted\' string&lt;/a&gt;', $this->_model->getEscapedValue () );
	}
	...
	public function testGetEscapedValueWithFilter() {
		...
		$filterMock = $this->getMock ( 'Magento\Framework\DataObject', [ 
				'filter' 
		], [ ], '', false );
	...
	public function getLabelHtmlDataProvider() {
		return [ 
				...
				[ 
						...
						'<label class="label admin__field-label" for="some-html-id" data-ui-id="form-element-label">' . '<span>some-label</span></label>' . "\n" 
				],
				[ 
						[ 
								'id_suffix' => 'suffix',
						...
						'<label class="label admin__field-label" for="some-html-idsuffix" data-ui-id="form-element-label">' . '<span>some-label</span></label>' . "\n" 
				] 
		];
	}
	...
	public function getElementHtmlDataProvider() {
		return [ 
				...
				[ 
						...
						'<label class="addbefore" for="html-id">some-html</label>' . '<input id="html-id" name="some-name"  data-ui-id="form-element-some-name" value="some-value" />' 
				],
				[ 
						[ 
								'html_id' => 'html-id',
				...
				[ 
						...
						'<input id="html-id" name="some-name"  data-ui-id="form-element-some-name" value="some-value" />' . '<label class="addafter" for="html-id">some-html</label>' 
				] 
		];
	}
}
