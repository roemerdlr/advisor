...
class FormKeyTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->formKey = new FormKey ( $this->mathRandomMock, $this->sessionMock );
	}
	public function testGetFormKeyNotExist() {
		...
		$this->sessionMock->expects ( $this->any () )->method ( 'getData' )->will ( $this->returnValueMap ( $valueMap ) );
		$this->mathRandomMock->expects ( $this->once () )->method ( 'getRandomString' )->with ( 16 )->will ( $this->returnValue ( 'random_string' ) );
	...
	public function testIsPresent() {
		$this->sessionMock->expects ( static::once () )->method ( 'getData' )->with ( FormKey::FORM_KEY )->willReturn ( 'Form key' );
