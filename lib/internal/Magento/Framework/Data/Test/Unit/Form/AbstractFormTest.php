...
class AbstractFormTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->factoryElementMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Factory', [ 
				'create' 
		], [ ], '', false );
		$this->factoryCollectionMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\CollectionFactory', [ 
				'create' 
		], [ ], '', false );
		$this->allElementsMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\Collection', [ ], [ ], '', false );
		$this->elementMock = $this->getMock ( 'Magento\Framework\Data\Form\Element\AbstractElement', [ ], [ ], '', false );
	...
	public function testAddField() {
		...
		$this->factoryElementMock->expects ( $this->once () )->method ( 'create' )->with ( 'hidden', [ 
				'data' => $config 
	...
	public function testAddFieldset() {
		...
		$this->factoryElementMock->expects ( $this->once () )->method ( 'create' )->with ( 'fieldset', [ 
				'data' => $config 
		] )->will ( $this->returnValue ( $this->elementMock ) );
	...
	public function testAddColumn() {
		...
		$this->factoryElementMock->expects ( $this->once () )->method ( 'create' )->with ( 'column', [ 
				'data' => $config 
		] )->will ( $this->returnValue ( $this->elementMock ) );
		$this->elementMock->expects ( $this->once () )->method ( 'setId' )->with ( 'hidden' );
	...
	public function testAddCustomAttribute() {
		$this->assertEquals ( $this->abstractForm, $this->abstractForm->addCustomAttribute ( 'attribute_key1', 'attribute_value1' ) );
		
		$form = clone $this->abstractForm;
		$this->assertNotEquals ( $form, $this->abstractForm->addCustomAttribute ( 'attribute_key2', 'attribute_value2' ) );
	}
