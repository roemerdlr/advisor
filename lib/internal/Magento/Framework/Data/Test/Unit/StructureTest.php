...
class StructureTest extends \PHPUnit_Framework_TestCase {
	 ...
	 * @return void @dataProvider importExportElementsDataProvider
	 */
		...
		return [ 
				...
				[ 
						[ 
								...
								'two' => [ 
										\Magento\Framework\Data\Structure::PARENT => 'one' 
								],
								'three' => [ 
										\Magento\Framework\Data\Structure::PARENT => 'one' 
								],
	 ...
	 *
	 * @param array $elements        	
	 * @return void @dataProvider importExceptionDataProvider
	...
	public function importExceptionDataProvider() {
		return [ 
				...
				'completely missing nested set' => [ 
						[ 
								'one' => [ 
										\Magento\Framework\Data\Structure::PARENT => 'two' 
								],
								'two' => [ ] 
						] 
				],
				...
				'broken reference back to parent' => [ 
						[ 
								...
								'two' => [ 
										\Magento\Framework\Data\Structure::PARENT => 'three' 
								],
								'three' => [ 
										\Magento\Framework\Data\Structure::PARENT => 'one' 
								] 
	 ...
	 *
	 * @param array $elements        	
	 * @return void @dataProvider importExceptionElementNotFoundDataProvider
	...
	public function importExceptionElementNotFoundDataProvider() {
		return [ 
				'non-existing parent' => [ 
						[ 
								'element' => [ 
										\Magento\Framework\Data\Structure::PARENT => 'unknown' 
								] 
						] 
				],
				'missing child' => [ 
	...
	public function testUnsetElement() {
		...
		$this->assertSame ( [ 
				5 
		], $this->_structure->getElement ( 'five' ) );
		...
		], $this->_structure->exportElements () );
	}
	 ...
	 * @return void @expectedException \InvalidArgumentException
	...
	public function testRenameElement() {
		...
		$this->assertSame ( [ 
				'three.5' => 'th',
				'two' => 'tw' 
		], $this->_structure->getChildren ( 'four.5' ) );
	...
	public function testSetAsChild() {
		...
		$this->assertEquals ( [ 
				'two' => 'two' 
		], $this->_structure->getChildren ( 'one' ) );
		$this->assertEquals ( [ 
				'three' => 'th' 
		], $this->_structure->getChildren ( 'four' ) );
		...
		$this->assertEquals ( [ 
				'six' => 's' 
		], $this->_structure->getChildren ( 'three' ) );
	}
	 ...
	 * @return void @dataProvider setAsChildOffsetDataProvider
	 */
	public function testSetAsChildOffset($offset, $expectedOffset) {
		...
		$this->assertSame ( $expectedOffset, $actualOffset, "The 'x' is expected to be at '{$expectedOffset}' offset, rather than '{$actualOffset}', in array: " . var_export ( $children, 1 ) );
	}
	 ...
	 * @param int $newOffset        	
	 * @param int $expectedOffset        	
	 * @return void @dataProvider reorderChildDataProvider
	 */
	 ...
	 * @param int $delta        	
	 * @param int $expectedOffset        	
	 * @return void @dataProvider reorderSiblingDataProvider
	 */
	...
	/**
	 *
	 * @return void @covers \Magento\Framework\Data\Structure::addToParentGroup
	...
	public function testGroups() {
		...
		$this->assertSame ( [ 
				'two',
				'three' 
		], $this->_structure->getGroupChildNames ( 'one', 'group1' ) );
		$this->assertSame ( [ 
				'two',
				'four' 
		], $this->_structure->getGroupChildNames ( 'one', 'group2' ) );
	...
	protected function _populateSampleStructure() {
		$this->_structure->importElements ( [ 
				'one' => [ ],
				'two' => [ 
						\Magento\Framework\Data\Structure::PARENT => 'four',
						'foo' => 'bar' 
				],
				'three' => [ 
						\Magento\Framework\Data\Structure::PARENT => 'four',
				...
				'five' => [ 
						\Magento\Framework\Data\Structure::PARENT => 'six',
						5 
				],
				'six' => [ 
						\Magento\Framework\Data\Structure::CHILDREN => [ 
	...
	protected function _populateSampleSortStructure() {
		$child = [ 
				\Magento\Framework\Data\Structure::PARENT => 'parent' 
		];
		$this->_structure->importElements ( [ 
				'parent' => [ 
						\Magento\Framework\Data\Structure::CHILDREN => [ 
