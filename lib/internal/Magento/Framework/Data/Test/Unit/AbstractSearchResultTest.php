...
class AbstractSearchResultTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->query->expects ( $this->any () )->method ( 'getCriteria' )->willReturn ( $this->criteria );
		$this->entityFactory = $this->getMockForAbstractClass ( 'Magento\Framework\Data\Collection\EntityFactoryInterface' );
	...
	public function testGetItems() {
		...
		$this->query->expects ( $this->once () )->method ( 'fetchAll' )->willReturn ( [ 
				$itemData 
		] );
		$this->entityFactory->expects ( $this->once () )->method ( 'create' )->with ( 'Magento\Framework\DataObject', [ 
				'data' => $itemData 
		] )->willReturn ( $testItem );
