...
class SearchResultProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->searchCriteriaMock = $this->getMockBuilder ( 'Magento\Framework\Api\CriteriaInterface' )->disableOriginalConstructor ()->getMock ();
		
		$this->searchResultCollectionMock = $this->getMockBuilder ( 'Magento\Framework\Data\AbstractSearchResult' )->disableOriginalConstructor ()->setMethods ( [ 
				'getSearchCriteria',
				'getItems',
				'getItemId' 
		] )->getMockForAbstractClass ();
		$this->searchResultCollectionMock->expects ( $this->any () )->method ( 'getSearchCriteria' )->willReturn ( $this->searchCriteriaMock );
	...
	public function testGetFirstItem() {
		...
		$this->searchResultCollectionMock->expects ( $this->once () )->method ( 'getItems' )->willReturn ( [ 
				$testItem,
				$testItem2 
		] );
	...
	public function testGetAllIds() {
		...
		$this->searchResultCollectionMock->expects ( $this->once () )->method ( 'getItemId' )->with ( $testItem )->willReturn ( 1 );
	...
	public function testGetColumnValues() {
		...
		$this->assertEquals ( [ 
				$columnValue 
		], $this->searchResultProcessor->getColumnValues ( $columnKey ) );
	}
