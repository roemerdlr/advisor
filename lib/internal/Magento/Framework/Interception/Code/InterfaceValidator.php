...
class InterfaceValidator {
	 ...
	 *
	 * @return void
	 * @throws ValidatorException @SuppressWarnings(PHPMD.CyclomaticComplexity)
	...
	public function validate($pluginClass, $interceptedType) {
		...
		foreach ( $plugin->getMethods ( \ReflectionMethod::IS_PUBLIC ) as $pluginMethod ) {
			...
			if (! $type->hasMethod ( $originMethodName )) {
				throw new ValidatorException ( new Phrase ( 'Incorrect interface in %1. There is no method [ %2 ] in %3 interface', [ 
						$pluginClass,
						$originMethodName,
						$interceptedType 
				] ) );
			...
			if (! $this->_argumentsReader->isCompatibleType ( $subject ['type'], $interceptedType ) || $subject ['type'] === null) {
				throw new ValidatorException ( new Phrase ( 'Invalid [%1] $%2 type in %3::%4. It must be compatible with %5', [ 
						$subject ['type'],
						$subject ['name'],
						$pluginClass,
			...
			switch ($methodType) {
				case self::METHOD_BEFORE :
					$this->validateMethodsParameters ( $pluginMethodParameters, $originMethodParameters, $pluginClass, $pluginMethod->getName () );
					break;
				case self::METHOD_AROUND :
					...
					if (! $this->_argumentsReader->isCompatibleType ( $proceed ['type'], '\\Closure' )) {
						throw new ValidatorException ( new Phrase ( 'Invalid [%1] $%2 type in %3::%4. It must be compatible with \\Closure', [ 
								$proceed ['type'],
								$proceed ['name'],
								$pluginClass,
								$pluginMethod->getName () 
				...
				case self::METHOD_AFTER :
					if (count ( $pluginMethodParameters ) > 1) {
						throw new ValidatorException ( new Phrase ( 'Invalid method signature. Detected extra parameters in %1::%2', [ 
								$pluginClass,
								$pluginMethod->getName () 
						] ) );
					}
	...
	protected function validateMethodsParameters(array $pluginParameters, array $originParameters, $class, $method) {
		if (count ( $pluginParameters ) != count ( $originParameters )) {
			throw new ValidatorException ( new Phrase ( 'Invalid method signature. Invalid method parameters count in %1::%2', [ 
					$class,
					$method 
			] ) );
		}
		foreach ( $pluginParameters as $position => $data ) {
			if (! $this->_argumentsReader->isCompatibleType ( $data ['type'], $originParameters [$position] ['type'] )) {
				throw new ValidatorException ( new Phrase ( 'Incompatible parameter type [%1 $%2] in %3::%4. It must be compatible with %5', [ 
						$data ['type'],
						$data ['name'],
						$class,
						$method,
