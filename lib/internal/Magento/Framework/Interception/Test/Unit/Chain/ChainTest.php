...
class ChainTest extends \PHPUnit_Framework_TestCase {
	...
	public function testInvokeNextBeforePlugin() {
		...
		$pluginMock = $this->getMock ( 'PluginClass', [ 
				'beforeMethod' 
		] );
		
		$pluginMock->expects ( $this->once () )->method ( 'beforeMethod' )->with ( $subjectMock, 1, 2 )->will ( $this->returnValue ( [ 
				'beforeMethodResult' 
		] ) );
		
		$this->_pluginListMock->expects ( $this->once () )->method ( 'getNext' )->with ( $type, $method, null )->will ( $this->returnValue ( [ 
				\Magento\Framework\Interception\DefinitionInterface::LISTENER_BEFORE => [ 
						'code' 
				] 
		] ) );
		
		$this->_pluginListMock->expects ( $this->once () )->method ( 'getPlugin' )->with ( $type, 'code' )->will ( $this->returnValue ( $pluginMock ) );
		
		$subjectMock->expects ( $this->once () )->method ( '___callParent' )->with ( 'method', [ 
				'beforeMethodResult' 
	...
	public function testInvokeNextAroundPlugin() {
		...
		$pluginMock->expects ( $this->once () )->method ( 'aroundMethod' )->with ( $this->anything () )->will ( $this->returnValue ( 'subjectMethodResult' ) );
	...
	public function testInvokeNextAfterPlugin() {
		...
		$pluginMock->expects ( $this->once () )->method ( 'afterMethod' )->with ( $subjectMock, 'subjectMethodResult' )->will ( $this->returnValue ( 'afterMethodResult' ) );
		...
		$this->assertEquals ( 'afterMethodResult', $this->_model->invokeNext ( $type, $method, $subjectMock, [ 
				1,
				2 
		] ) );
	}
