...
class InterceptorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->classGeneratorMock = $this->getMock ( '\Magento\Framework\Code\Generator\CodeGeneratorInterface', [ ], [ ], '', false );
	}
	public function testGetDefaultResultClassName() {
		...
		$this->classGeneratorMock->expects ( $this->once () )->method ( 'setName' )->willReturnSelf ();
		$this->classGeneratorMock->expects ( $this->once () )->method ( 'addProperties' )->willReturnSelf ();
		$this->classGeneratorMock->expects ( $this->once () )->method ( 'addMethods' )->willReturnSelf ();
		$this->classGeneratorMock->expects ( $this->once () )->method ( 'setClassDocBlock' )->willReturnSelf ();
