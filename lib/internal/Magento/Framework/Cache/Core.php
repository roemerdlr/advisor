...
class Core extends \Zend_Cache_Core {
	 ...
	 * @param int $priority
	 *        	integer between 0 (very low priority) and 10 (maximum priority) used by
	...
	protected function _decorateBackend(\Zend_Cache_Backend $backendObject) {
		...
		foreach ( $this->_specificOptions ['backend_decorators'] as $decoratorName => $decoratorOptions ) {
			if (! is_array ( $decoratorOptions ) || ! array_key_exists ( 'class', $decoratorOptions )) {
				\Zend_Cache::throwException ( "Concrete decorator options in '" . $decoratorName . "' should be an array containing 'class' key" );
			}
			...
			if (! class_exists ( $decoratorOptions ['class'] )) {
				\Zend_Cache::throwException ( "Class '" . $decoratorOptions ['class'] . "' specified in '" . $decoratorName . "' does not exist" );
			}
			...
			if (! $backendObject instanceof \Magento\Framework\Cache\Backend\Decorator\AbstractDecorator) {
				\Zend_Cache::throwException ( "Decorator in '" . $decoratorName . "' should extend \Magento\Framework\Cache\Backend\Decorator\AbstractDecorator" );
			}
		}
