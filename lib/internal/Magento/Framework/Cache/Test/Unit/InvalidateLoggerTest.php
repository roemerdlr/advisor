...
class InvalidateLoggerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->invalidateLogger = new \Magento\Framework\Cache\InvalidateLogger ( $this->requestMock, $this->loggerMock );
		$this->requestMock->expects ( $this->once () )->method ( 'getMethod' )->willReturn ( $this->method );
		$this->requestMock->expects ( $this->once () )->method ( 'getUriString' )->willReturn ( $this->url );
	}
	public function testCritical() {
		$this->loggerMock->expects ( $this->once () )->method ( 'critical' )->with ( 'message', [ 
				'method' => $this->method,
				'url' => $this->url,
	...
	public function testExecute() {
		$this->loggerMock->expects ( $this->once () )->method ( 'debug' )->with ( 'cache_invalidate: ', [ 
				'method' => $this->method,
				'url' => $this->url,
				'invalidateInfo' => $this->params 
		] );
	...
	public function testMakeParams() {
		...
		$this->assertEquals ( $expected, $method->invoke ( $this->invalidateLogger, $this->params ) );
	}
