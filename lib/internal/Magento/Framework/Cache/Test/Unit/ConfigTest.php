...
class ConfigTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_storage = $this->getMock ( 'Magento\Framework\Cache\Config\Data', [ 
				'get' 
		], [ ], '', false );
		$this->_model = new \Magento\Framework\Cache\Config ( $this->_storage );
	}
	public function testGetTypes() {
		$this->_storage->expects ( $this->once () )->method ( 'get' )->with ( 'types', [ ] )->will ( $this->returnValue ( [ 
				'val1',
				'val2' 
		] ) );
	...
	public function testGetType() {
		$this->_storage->expects ( $this->once () )->method ( 'get' )->with ( 'types/someType', [ ] )->will ( $this->returnValue ( [ 
				'someTypeValue' 
		] ) );
