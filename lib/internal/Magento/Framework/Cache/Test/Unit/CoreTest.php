...
class CoreTest extends \PHPUnit_Framework_TestCase {
	...
	public function setBackendExceptionProvider() {
		return [ 
				...
				'decorator setting is not an array' => [ 
						[ 
								'decorator' => 'string' 
						] 
				],
				'decorator setting is empty array' => [ 
						[ 
								'decorator' => [ ] 
						] 
				],
				'no class index in array' => [ 
						[ 
								'decorator' => [ 
										'somedata' 
								] 
	...
	public function testSave() {
		...
		$backendMock = $this->getMock ( 'Zend_Cache_Backend_BlackHole' );
		$backendMock->expects ( $this->once () )->method ( 'save' )->with ( $data, $this->anything (), $prefixedTags )->will ( $this->returnValue ( true ) );
		$frontend = new \Magento\Framework\Cache\Core ( [ 
				'disable_save' => false,
				'caching' => true,
	...
	public function testClean() {
		...
		$backendMock = $this->getMock ( 'Zend_Cache_Backend_BlackHole' );
		$backendMock->expects ( $this->once () )->method ( 'clean' )->with ( $mode, $prefixedTags )->will ( $this->returnValue ( $expectedResult ) );
		$frontend = new \Magento\Framework\Cache\Core ( [ 
				'caching' => true,
				'cache_id_prefix' => $prefix 
		] );
	...
	public function testGetIdsMatchingTags() {
		...
		$backendMock->expects ( $this->once () )->method ( 'getIdsMatchingTags' )->with ( $prefixedTags )->will ( $this->returnValue ( $ids ) );
		$backendMock->expects ( $this->any () )->method ( 'getCapabilities' )->will ( $this->returnValue ( [ 
				'tags' => true 
		] ) );
		$frontend = new \Magento\Framework\Cache\Core ( [ 
				'caching' => true,
