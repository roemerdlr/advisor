...
class DatabaseTest extends \PHPUnit_Framework_TestCase {
	...
	public function testInitializeWithException($options) {
		$this->objectManager->getObject ( 'Magento\Framework\Cache\Backend\Database', [ 
				'options' => $options 
		] );
	}
	...
	public function testLoad($options, $expected) {
		...
		$database = $this->objectManager->getObject ( 'Magento\Framework\Cache\Backend\Database', [ 
				'options' => $options 
		] );
	...
	public function loadDataProvider() {
		$connectionMock = $this->getMockBuilder ( 'Magento\Framework\DB\Adapter\Pdo\Mysql' )->setMethods ( [ 
				'select',
				'fetchOne' 
		] )->disableOriginalConstructor ()->getMock ();
		
		$selectMock = $this->getMock ( 'Magento\Framework\DB\Select', [ 
				'where',
				'from' 
		], [ ], '', false );
		...
		$connectionMock->expects ( $this->any () )->method ( 'fetchOne' )->will ( $this->returnValue ( 'loaded_value' ) );
	...
	public function getOptionsWithoutStoreData($connectionMock = null) {
		if (null === $connectionMock) {
			$connectionMock = $this->getMockBuilder ( 'Magento\Framework\DB\Adapter\Pdo\Mysql' )->disableOriginalConstructor ()->getMock ();
		}
	...
	protected function getSaveAdapterMock($result) {
		...
		$dbStatementMock = $this->getMockBuilder ( 'Zend_Db_Statement_Interface' )->setMethods ( [ 
				'rowCount' 
		] )->disableOriginalConstructor ()->getMockForAbstractClass ();
		
		$dbStatementMock->expects ( $this->any () )->method ( 'rowCount' )->will ( $this->returnValue ( $result ) );
		
		$connectionMock->expects ( $this->any () )->method ( 'quoteIdentifier' )->will ( $this->returnValue ( 'data' ) );
		
		$connectionMock->expects ( $this->any () )->method ( 'query' )->will ( $this->returnValue ( $dbStatementMock ) );
	...
	public function removeDataProvider() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'delete' )->will ( $this->returnValue ( true ) );
	...
	public function cleanDataProvider() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'query' )->will ( $this->returnValue ( false ) );
	...
	public function getIdsDataProvider() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'fetchCol' )->will ( $this->returnValue ( [ 
				'value_one',
				'value_two' 
		] ) );
	...
	public function testGetTags() {
		...
		$selectMock->expects ( $this->any () )->method ( 'distinct' )->will ( $this->returnSelf () );
	...
	public function testGetIdsMatchingTags() {
		...
		$selectMock->expects ( $this->any () )->method ( 'group' )->will ( $this->returnSelf () );
		
		$selectMock->expects ( $this->any () )->method ( 'having' )->will ( $this->returnSelf () );
	...
	public function testGetIdsNotMatchingTags() {
		...
		$connectionMock->expects ( $this->at ( 1 ) )->method ( 'fetchCol' )->will ( $this->returnValue ( [ 
				'some_value_one' 
		] ) );
		
		$connectionMock->expects ( $this->at ( 3 ) )->method ( 'fetchCol' )->will ( $this->returnValue ( [ 
				'some_value_two' 
		] ) );
		...
		$this->assertEquals ( [ 
				'some_value_one' 
		], $database->getIdsNotMatchingTags () );
	}
	...
	public function testGetMetadatas() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'fetchRow' )->will ( $this->returnValue ( [ 
				'expire_time' => '3',
				'update_time' => 2 
		] ) );
	...
	public function touchDataProvider() {
		...
		$connectionMock->expects ( $this->any () )->method ( 'update' )->will ( $this->returnValue ( false ) );
