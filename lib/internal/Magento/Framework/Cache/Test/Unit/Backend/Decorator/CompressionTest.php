...
class CompressionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testDecompressData() {
		...
		$this->assertEquals ( $this->_testString, $methodDecompress->invoke ( $this->_decorator, $methodCompress->invoke ( $this->_decorator, $this->_testString ) ) );
	}
	...
	public function testSaveLoad() {
		...
		$backend->expects ( $this->once () )->method ( 'save' )->will ( $this->returnCallback ( [ 
				__CLASS__,
				'mockSave' 
		] ) );
		
		$backend->expects ( $this->once () )->method ( 'load' )->will ( $this->returnCallback ( [ 
				__CLASS__,
				'mockLoad' 
		] ) );
