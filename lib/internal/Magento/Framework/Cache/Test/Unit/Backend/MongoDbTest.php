...
class MongoDbTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_collection = $this->getMock ( 'MongoCollection', [ 
				'find',
				'findOne',
				'distinct',
				'save',
		...
		$this->_model = $this->getMock ( 'Magento\Framework\Cache\Backend\MongoDb', [ 
				'_getCollection' 
		], [ ], '', false );
	...
	public function testGetIdsMatchingTags($method, $tags, $expectedInput) {
		$expectedOutput = new \ArrayIterator ( [ 
				'test1' => 'test1',
				'test2' => 'test2' 
		] );
		$expectedIds = [ 
		...
		$this->_collection->expects ( $this->once () )->method ( 'find' )->with ( $expectedInput )->will ( $this->returnValue ( $expectedOutput ) );
	...
	public function testLoad($doNotTestValidity) {
		...
		$this->_collection->expects ( $this->once () )->method ( 'findOne' )->with ( $this->logicalAnd ( $this->arrayHasKey ( '_id' ), $validityCondition ) )->will ( $this->returnValue ( [ 
				'data' => $binData 
		] ) );
	...
	public function testTest() {
		$cacheId = 'test_id';
		$time = time ();
		$this->_collection->expects ( $this->once () )->method ( 'findOne' )->with ( $this->logicalAnd ( $this->arrayHasKey ( '_id' ), $this->contains ( $cacheId ) ) )->will ( $this->returnValue ( [ 
				'mtime' => $time 
		] ) );
	...
	public function testSave() {
		...
		$this->assertTrue ( $this->_model->save ( 'test data', 'test_id', [ 
				'tag1',
				'tag2' 
		], 100 ) );
	}
	public function testRemove() {
		$cacheId = 'test';
		$this->_collection->expects ( $this->once () )->method ( 'remove' )->with ( [ 
				'_id' => $cacheId 
	...
	public function cleanAll() {
		$this->_collection->expects ( $this->once () )->method ( 'drop' )->will ( $this->returnValue ( [ 
				'ok' => true 
		] ) );
