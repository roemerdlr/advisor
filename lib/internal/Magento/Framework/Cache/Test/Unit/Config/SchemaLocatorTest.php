...
class SchemaLocatorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetSchema() {
		$this->urnResolverMock->expects ( $this->once () )->method ( 'getRealPath' )->with ( 'urn:magento:framework:Cache/etc/cache.xsd' )->willReturn ( $this->urnResolver->getRealPath ( 'urn:magento:framework:Cache/etc/cache.xsd' ) );
		$this->assertEquals ( $this->urnResolver->getRealPath ( 'urn:magento:framework:Cache/etc/cache.xsd' ), $this->schemaLocator->getSchema () );
	}
