...
class MongoDb extends \Zend_Cache_Backend implements \Zend_Cache_Backend_ExtendedInterface {
	 ...
	 * Used for composing queries to search by tags
	 */
	...
	public function getIds() {
		return array_keys ( iterator_to_array ( $this->_getCollection ()->find ( [ ], [ 
				'_id' 
		] ) ) );
	}
	...
	public function getIdsMatchingTags($tags = []) {
		...
		$result = $this->_getCollection ()->find ( $query, [ 
				'_id' 
		] );
	 ...
	 * @param string $cacheId
	 *        	Cache id
	 * @param boolean $notTestCacheValidity
	...
	public function load($cacheId, $notTestCacheValidity = false) {
		...
		$result = $this->_getCollection ()->findOne ( $query, [ 
				'data' 
		] );
	 ...
	 *        	If != false, set a specific lifetime (null => infinite lifetime)
	...
	public function clean($mode = \Zend_Cache::CLEANING_MODE_ALL, $tags = []) {
		...
		switch ($mode) {
			...
			case \Zend_Cache::CLEANING_MODE_OLD :
				$query = [ 
						'expire' => [ 
								'$ne' => self::EXPIRATION_TIME_INFINITE,
								'$lte' => time () 
						] 
				];
				break;
