...
class Database extends \Zend_Cache_Backend implements \Zend_Cache_Backend_ExtendedInterface {
	...
	protected function _getConnection() {
		if (! $this->_connection) {
			...
			if (! $connection instanceof \Magento\Framework\DB\Adapter\AdapterInterface) {
				\Zend_Cache::throwException ( 'DB Adapter should be declared and extend \Magento\Framework\DB\Adapter\AdapterInterface' );
			} else {
	...
	public function test($id) {
		if ($this->_options ['store_data']) {
			$select = $this->_getConnection ()->select ()->from ( $this->_getDataTable (), 'update_time' )->where ( 'id=:cache_id' )->where ( 'expire_time=0 OR expire_time>?', time () );
			return $this->_getConnection ()->fetchOne ( $select, [ 
					'cache_id' => $id 
			] );
		} else {
	...
	public function remove($id) {
		if ($this->_options ['store_data']) {
			return $this->_getConnection ()->delete ( $this->_getDataTable (), [ 
					'id=?' => $id 
			] );
		}
		return false;
	...
	public function clean($mode = \Zend_Cache::CLEANING_MODE_ALL, $tags = []) {
		...
		switch ($mode) {
			...
			case \Zend_Cache::CLEANING_MODE_OLD :
				if ($this->_options ['store_data']) {
					$result = $connection->delete ( $this->_getDataTable (), [ 
							'expire_time> ?' => 0,
							'expire_time<= ?' => time () 
					] );
				} else {
	...
	public function getIdsMatchingTags($tags = []) {
		$select = $this->_getConnection ()->select ()->from ( $this->_getTagsTable (), 'cache_id' )->distinct ( true )->where ( 'tag IN(?)', $tags )->group ( 'cache_id' )->having ( 'COUNT(cache_id)=' . count ( $tags ) );
		return $this->_getConnection ()->fetchCol ( $select );
	}
	...
	public function getIdsMatchingAnyTags($tags = []) {
		$select = $this->_getConnection ()->select ()->from ( $this->_getTagsTable (), 'cache_id' )->distinct ( true )->where ( 'tag IN(?)', $tags );
		return $this->_getConnection ()->fetchCol ( $select );
	}
	...
	public function touch($id, $extraLifetime) {
		if ($this->_options ['store_data']) {
			return $this->_getConnection ()->update ( $this->_getDataTable (), [ 
					'expire_time' => new \Zend_Db_Expr ( 'expire_time+' . $extraLifetime ) 
			], [ 
					'id=?' => $id,
					'expire_time = 0 OR expire_time>' => time () 
			] );
		} else {
			return true;
		}
	...
	protected function _cleanByTags($mode, $tags) {
		if ($this->_options ['store_data']) {
			...
			while ( $row = $stmt->fetch () ) {
				...
				$counter ++;
				if ($counter > 100) {
					$result = $result && $connection->delete ( $this->_getDataTable (), [ 
							'id IN (?)' => $ids 
					] );
					$ids = [ ];
					$counter = 0;
