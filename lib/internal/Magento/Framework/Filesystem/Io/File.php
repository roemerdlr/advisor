...
class File extends AbstractIo {
	...
	public static function rmdirRecursive($dir, $recursive = true) {
		if ($recursive) {
			$result = self::_recursiveCallback ( $dir, [ 
					'unlink' 
			], [ 
					'rmdir' 
			] );
	 ...
	 * @param string $dir        	
	 * @return true
	 * @throws \Exception @SuppressWarnings(PHPMD.ShortMethodName)
	 */
	 ...
	 * @param int $mode        	
	 * @return int|bool @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	 ...
	 *
	 * @param string $filename        	
	 * @return bool @SuppressWarnings(PHPMD.ShortMethodName)
	 */
	...
	public static function chmodRecursive($dir, $mode) {
		return self::_recursiveCallback ( $dir, [ 
				'chmod',
				[ 
						$mode 
				] 
		...
		if ($dirHandler) {
			while ( ($entry = readdir ( $dirHandler )) !== false ) {
				...
				if (is_file ( $fullPath )) {
					...
					$listItem ['leaf'] = true;
					if (isset ( $pathInfo ['extension'] ) && in_array ( strtolower ( $pathInfo ['extension'] ), [ 
							'jpg',
							'jpeg',
							'gif',
							'bmp',
