...
class ReadTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->fileFactory = $this->getMock ( 'Magento\Framework\Filesystem\File\ReadFactory', [ ], [ ], '', false );
		$this->read = new \Magento\Framework\Filesystem\Directory\Read ( $this->fileFactory, $this->driver, $this->path );
	}
	...
	public function testStat() {
		$this->driver->expects ( $this->once () )->method ( 'stat' )->will ( $this->returnValue ( [ 
				'some-stat-data' 
		] ) );
		$this->assertEquals ( [ 
				'some-stat-data' 
		], $this->read->stat ( 'correct-path' ) );
	}
