...
class WriteTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->fileFactory = $this->getMock ( 'Magento\Framework\Filesystem\File\WriteFactory', [ ], [ ], '', false );
		$this->path = 'PATH/';
		$this->write = new \Magento\Framework\Filesystem\Directory\Write ( $this->fileFactory, $this->driver, $this->path, 'cool-permissions' );
	}
	...
	private function assertIsFileExpectation($path) {
		$this->driver->expects ( $this->any () )->method ( 'getAbsolutePath' )->with ( $this->path, $path )->willReturn ( $this->getAbsolutePath ( $path ) );
		$this->driver->expects ( $this->any () )->method ( 'isFile' )->with ( $this->getAbsolutePath ( $path ) )->willReturn ( true );
	}
