...
class Write extends Read implements WriteInterface {
	 ...
	 */
	public function __construct(\Magento\Framework\Filesystem\File\WriteFactory $fileFactory, \Magento\Framework\Filesystem\DriverInterface $driver, $path, $createPermissions = null) {
	...
	protected function assertWritable($path) {
		if ($this->isWritable ( $path ) === false) {
			...
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The path "%1" is not writable', [ 
					$path 
			] ) );
		}
	}
	...
	protected function assertIsFile($path) {
		...
		if (! $this->driver->isFile ( $absolutePath )) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The "%1" file doesn\'t exist or not a file', [ 
					$absolutePath 
			] ) );
		}
	}
