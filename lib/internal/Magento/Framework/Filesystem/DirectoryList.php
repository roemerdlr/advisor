...
class DirectoryList {
	...
	public function __construct($root, array $config = []) {
		...
		$this->directories [self::SYS_TMP] = [ 
				self::PATH => realpath ( sys_get_temp_dir () ) 
		];
	...
	private function assertUrlPath($urlPath) {
		if (! preg_match ( '/^([a-z0-9_]+[a-z0-9\._]*(\/[a-z0-9_]+[a-z0-9\._]*)*)?$/', $urlPath )) {
			throw new \InvalidArgumentException ( "URL path must be relative directory path in lowercase with '/' directory separator: '{$urlPath}'" );
		}
	}
	...
	private function assertCode($code) {
		if (! isset ( $this->directories [$code] )) {
			throw new \Magento\Framework\Exception\FileSystemException ( new \Magento\Framework\Phrase ( 'Unknown directory type: \'%1\'', [ 
					$code 
			] ) );
		}
	}
