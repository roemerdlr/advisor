...
class File implements DriverInterface {
	...
	public function isExists($path) {
		...
		if ($result === null) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution %1', [ 
	...
	public function stat($path) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Cannot gather stats! %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileGetContents($path, $flag = null, $context = null) {
		...
		if (false === $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Cannot read contents from file "%1" %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function createDirectory($path, $permissions) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Directory "%1" cannot be created %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function rename($oldPath, $newPath, DriverInterface $targetDriver = null) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The "%1" path cannot be renamed into "%2" %3', [ 
					$oldPath,
					$newPath,
					$this->getWarningMessage () 
			] ) );
	...
	public function copy($source, $destination, DriverInterface $targetDriver = null) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The file or directory "%1" cannot be copied to "%2" %3', [ 
					$source,
					$destination,
					$this->getWarningMessage () 
			] ) );
	...
	public function symlink($source, $destination, DriverInterface $targetDriver = null) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Cannot create a symlink for "%1" and place it to "%2" %3', [ 
					$source,
					$destination,
					$this->getWarningMessage () 
			] ) );
	...
	public function deleteFile($path) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The file "%1" cannot be deleted %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function deleteDirectory($path) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The directory "%1" cannot be deleted %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function changePermissions($path, $permissions) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Cannot change permissions for path "%1" %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function changePermissionsRecursively($path, $dirPermissions, $filePermissions) {
		...
		$iterator = new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $path, $flags ), \RecursiveIteratorIterator::CHILD_FIRST );
		/** @var \FilesystemIterator $entity */
		foreach ( $iterator as $entity ) {
			if ($entity->isDir ()) {
	...
	public function touch($path, $modificationTime = null) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The file or directory "%1" cannot be touched %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function filePutContents($path, $content, $mode = null) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'The specified "%1" file could not be written %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function fileOpen($path, $mode) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'File "%1" cannot be opened %2', [ 
					$path,
					$this->getWarningMessage () 
			] ) );
		}
	...
	public function fileReadLine($resource, $length, $ending = null) {
		...
		if (false === $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'File cannot be read %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileGetCsv($resource, $length = 0, $delimiter = ',', $enclosure = '"', $escape = '\\') {
		...
		if ($result === null) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Wrong CSV handle %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileSeek($resource, $offset, $whence = SEEK_SET) {
		...
		if ($result === - 1) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of fileSeek %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileClose($resource) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of fileClose %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileWrite($resource, $data) {
		...
		if (false === $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of fileWrite %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function filePutCsv($resource, array $data, $delimiter = ',', $enclosure = '"') {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of filePutCsv %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileFlush($resource) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of fileFlush %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileLock($resource, $lockMode = LOCK_EX) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of fileLock %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
	...
	public function fileUnlock($resource) {
		...
		if (! $result) {
			throw new FileSystemException ( new \Magento\Framework\Phrase ( 'Error occurred during execution of fileUnlock %1', [ 
					$this->getWarningMessage () 
			] ) );
		}
		return $result;
