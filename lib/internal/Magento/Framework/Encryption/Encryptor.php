...
class Encryptor implements EncryptorInterface {
	...
	public function validateHashVersion($hash, $validateCount = false) {
		...
		return $validateCount ? end ( $hashVersions ) === self::HASH_VERSION_LATEST && count ( $hashVersions ) === 1 : end ( $hashVersions ) === self::HASH_VERSION_LATEST;
	}
	...
	public function encrypt($data) {
		...
		return $this->keyVersion . ':' . $this->cipher . ':' . (MCRYPT_MODE_CBC === $crypt->getMode () ? $crypt->getInitVector () . ':' : '') . base64_encode ( $crypt->encrypt ( ( string ) $data ) );
	}
	 ...
	 *        	NULL value means usage of the default key specified on constructor
