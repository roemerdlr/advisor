...
class Crypt {
	 ...
	 * @param string $cipher
	 *        	Cipher algorithm (one of the MCRYPT_ciphername constants)
	 * @param string $mode
	 *        	Mode of cipher algorithm (MCRYPT_MODE_modeabbr constants)
	 * @param string|bool $initVector
	 *        	Initial vector to fill algorithm blocks.
	...
	public function __construct($key, $cipher = MCRYPT_BLOWFISH, $mode = MCRYPT_MODE_ECB, $initVector = false) {
		...
		try {
			...
			if (strlen ( $key ) > $maxKeySize) {
				throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Key must not exceed %1 bytes.', [ 
						$maxKeySize 
				] ) );
			}
			...
			if (true === $initVector) {
				...
				throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Init vector must be a string of %1 bytes.', [ 
						$initVectorSize 
				] ) );
			}
