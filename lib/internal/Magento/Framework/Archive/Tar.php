...
class Tar extends \Magento\Framework\Archive\AbstractArchive implements \Magento\Framework\Archive\ArchiveInterface {
	...
	/**
	 * Skip first level parent directory.
	 * Example:
	...
	protected function _createTar($skipRoot = false, $finalize = false) {
		...
		if (is_dir ( $file )) {
			...
			if (false === $dirFiles) {
				throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Can\'t scan dir: %1', [ 
						$file 
				] ) );
			}
	...
	protected function _composeHeader($long = false) {
		...
		$a = function_exists ( 'posix_getgrgid' ) ? posix_getgrgid ( filegroup ( $file ) ) : [ 
				'name' => '' 
		];
			...
			if (in_array ( $header ['type'], [ 
				...
				if (! file_exists ( $dirname )) {
					...
					if (false === $mkdirResult) {
						throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to create directory %1', [ 
								$dirname 
						] ) );
					}
				}
	...
	protected function _extractFileHeader() {
		...
		if (isset ( $header ['name'] ) && $checksumOk) {
			...
			$realNameBlockSize = floor ( ($header ['size'] + self::TAR_BLOCK_SIZE - 1) / self::TAR_BLOCK_SIZE ) * self::TAR_BLOCK_SIZE;
	...
	public function extract($file, $source, $destination) {
		...
		while ( ! $archiveReader->eof () ) {
			...
			if ($header ['type'] != 5) {
				$skipBytes = floor ( ($header ['size'] + self::TAR_BLOCK_SIZE - 1) / self::TAR_BLOCK_SIZE ) * self::TAR_BLOCK_SIZE;
