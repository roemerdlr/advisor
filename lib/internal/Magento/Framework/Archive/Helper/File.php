...
class File {
	...
	public function open($mode = 'w+', $chmod = DriverInterface::WRITEABLE_FILE_MODE) {
		...
		if ($this->_isInWriteMode) {
			if (! is_writable ( $this->_fileLocation )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Permission denied to write to %1', [ 
						$this->_fileLocation 
				] ) );
			}
			
			if (is_file ( $this->_filePath ) && ! is_writable ( $this->_filePath )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( "Can't open file %1 for writing. Permission denied.", [ 
						$this->_fileName 
				] ) );
			}
		}
		...
		if ($this->_isReadableMode ( $mode ) && (! is_file ( $this->_filePath ) || ! is_readable ( $this->_filePath ))) {
			if (! is_file ( $this->_filePath )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( 'File %1 does not exist', [ 
						$this->_filePath 
				] ) );
			}
			
			if (! is_readable ( $this->_filePath )) {
				throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Permission denied to read file %1', [ 
						$this->_filePath 
				] ) );
			}
		}
	...
	protected function _open($mode) {
		...
		if (false === $this->_fileHandler) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to open file %1', [ 
					$this->_filePath 
			] ) );
		}
	}
	...
	protected function _write($data) {
		...
		if (false === $result) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to write data to %1', [ 
					$this->_filePath 
			] ) );
		}
	}
	...
	protected function _read($length) {
		...
		if (false === $result) {
			throw new LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to read data from %1', [ 
					$this->_filePath 
			] ) );
		}
