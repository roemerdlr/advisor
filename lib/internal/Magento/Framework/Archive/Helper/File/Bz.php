...
class Bz extends \Magento\Framework\Archive\Helper\File {
	...
	protected function _open($mode) {
		...
		if (false === $this->_fileHandler) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to open file %1', [ 
					$this->_filePath 
			] ) );
		}
	}
	...
	protected function _write($data) {
		...
		if (false === $result) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to write data to %1', [ 
					$this->_filePath 
			] ) );
		}
	}
	...
	protected function _read($length) {
		...
		if (false === $data) {
			throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( 'Failed to read data from %1', [ 
					$this->_filePath 
			] ) );
		}
