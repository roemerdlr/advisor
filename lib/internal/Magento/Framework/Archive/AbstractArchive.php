...
class AbstractArchive {
	...
	/**
	 * Read data from file.
	 * If file can't be opened, throw to exception.
	 *
	 * @param string $source        	
	 * @return string
	...
	protected function _readFile($source) {
		...
		if (is_file ( $source ) && is_readable ( $source )) {
			...
			if ($data === false) {
				throw new \Magento\Framework\Exception\LocalizedException ( new \Magento\Framework\Phrase ( "Can't get contents from: %1", [ 
						$source 
				] ) );
			}
		}
