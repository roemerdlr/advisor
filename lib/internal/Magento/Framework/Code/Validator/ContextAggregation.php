...
class ContextAggregation implements ValidatorInterface {
	...
	/**
	 * Validate class.
	 * Check declaration of dependencies that already declared in context object
	 *
	...
	public function validate($className) {
		...
		foreach ( $actualDependencies as $type ) {
			...
			if (is_subclass_of ( $type, '\Magento\Framework\ObjectManager\ContextInterface' )) {
				$contextDependencies = array_merge ( $contextDependencies, $this->_argumentsReader->getConstructorArguments ( new \ReflectionClass ( $type ), false, true ) );
			}
		}
		...
		if (false == empty ( $errors )) {
			...
			throw new \Magento\Framework\Exception\ValidatorException ( new \Magento\Framework\Phrase ( 'Incorrect dependency in class %1 in %2%3%4', [ 
					$className,
					$classPath,
					PHP_EOL,
