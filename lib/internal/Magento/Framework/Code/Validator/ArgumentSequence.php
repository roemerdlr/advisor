...
class ArgumentSequence implements ValidatorInterface {
	...
	public function validate($className) {
		...
		if (false == $this->_checkArgumentSequence ( $classArguments, $requiredSequence )) {
			...
			throw new \Magento\Framework\Exception\ValidatorException ( new \Magento\Framework\Phrase ( 'Incorrect argument sequence in class %1 in %2%3Required: $%4%5Actual  : $%6%7', [ 
					$className,
					$classPath,
					PHP_EOL,
	...
	protected function _buildsSequence(array $classArguments, array $parentArguments = []) {
		...
		foreach ( $parentArgumentList [self::REQUIRED] as $name => $argument ) {
			if (! isset ( $classArgumentList [self::OPTIONAL] [$name] )) {
				$output [$name] = isset ( $classArgumentList [self::REQUIRED] [$name] ) ? $classArgumentList [self::REQUIRED] [$name] : $argument;
			} else {
		...
		foreach ( $parentArgumentList [self::OPTIONAL] as $name => $argument ) {
			if (! isset ( $output [$name] )) {
				$output [$name] = isset ( $classArgumentList [self::OPTIONAL] [$name] ) ? $classArgumentList [self::OPTIONAL] [$name] : $argument;
			}
		}
