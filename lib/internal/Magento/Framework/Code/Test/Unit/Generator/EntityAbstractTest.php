...
class EntityAbstractTest extends \PHPUnit_Framework_TestCase {
	...
	public function testConstruct() {
		...
		$this->assertAttributeInstanceOf ( 'Magento\Framework\Code\Generator\ClassGenerator', '_classGenerator', $this->_model );
		$this->assertAttributeInstanceOf ( 'Magento\Framework\Code\Generator\DefinedClasses', 'definedClasses', $this->_model );
		...
		$ioObject = $this->getMockBuilder ( 'Magento\Framework\Code\Generator\Io' )->disableOriginalConstructor ()->getMock ();
		$codeGenerator = $this->getMockBuilder ( 'Magento\Framework\Code\Generator\ClassGenerator' )->disableOriginalConstructor ()->getMock ();
	...
	public function generateDataProvider() {
		return [ 
				'no_source_class' => [ 
						'errors' => [ 
								'Source class ' . self::SOURCE_CLASS . ' doesn\'t exist.' 
						],
				...
				'cant_create_result_directory' => [ 
						'errors' => [ 
								'Can\'t create directory ' . self::RESULT_DIRECTORY . '.' 
						],
						'validationSuccess' => false,
	 ...
	 */
	public function testGenerate($errors = [], $validationSuccess = true, $sourceClassExists = true, $resultClassExists = false, $makeResultDirSuccess = true, $resultFileExists = false, $willWriteCode = true) {
		if ($validationSuccess) {
			$arguments = $this->_prepareMocksForGenerateCode ( $willWriteCode );
		} else {
			$arguments = $this->_prepareMocksForValidateData ( $sourceClassExists, $resultClassExists, $makeResultDirSuccess, $resultFileExists );
		}
		$abstractGetters = [ 
				'_getClassProperties',
				'_getClassMethods' 
	 ...
	 */
	protected function _prepareMocksForValidateData($sourceClassExists = true, $resultClassExists = false, $makeResultDirSuccess = true, $resultFileExists = false) {
		// Configure DefinedClasses mock
		$definedClassesMock = $this->getMock ( 'Magento\Framework\Code\Generator\DefinedClasses' );
		$definedClassesMock->expects ( $this->once () )->method ( 'isClassLoadable' )->with ( self::SOURCE_CLASS )->willReturn ( $sourceClassExists );
		if ($resultClassExists) {
			$definedClassesMock->expects ( $this->once () )->method ( 'isClassLoadableFromDisc' )->with ( self::RESULT_CLASS )->willReturn ( $resultClassExists );
		}
		...
		if ($sourceClassExists && ! $resultClassExists) {
			$ioObject->expects ( $this->once () )->method ( 'makeResultFileDirectory' )->with ( self::RESULT_CLASS )->willReturn ( $makeResultDirSuccess );
		}
	...
	protected function _prepareMocksForGenerateCode($willWriteCode) {
		...
		$codeGenerator->expects ( $this->once () )->method ( 'setClassDocBlock' )->with ( $this->isType ( 'array' ) )->will ( $this->returnSelf () );
		
		$codeGenerator->expects ( $this->once () )->method ( 'generate' )->will ( $this->returnValue ( $willWriteCode ? self::RESULT_CODE : null ) );
