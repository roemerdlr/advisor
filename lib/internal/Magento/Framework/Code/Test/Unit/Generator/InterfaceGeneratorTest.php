...
class InterfaceGeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGenerate($additionalMethodsData, $expectedException, $expectedExceptionMessage) {
		...
		$this->interfaceGenerator->setClassDocBlock ( $this->interfaceDocBlock )->addMethods ( $methodsData )->setName ( 'SevenInterface' )->setNamespaceName ( 'Magento\SomeModule\Model' )->addUse ( 'Magento\SomeModule\Model\Two\Test', 'TestTwo' )->addUse ( 'Magento\SomeModule\Model\Three\Test', 'TestThree' )->setExtendedClass ( '\Magento\Framework\Code\Generator\CodeGeneratorInterface' );
		$generatedInterface = $this->interfaceGenerator->generate ();
		$expectedInterface = file_get_contents ( __DIR__ . '/../_files/app/code/Magento/SomeModule/Model/SevenInterface.php' );
		
		$this->assertStringEndsWith ( $generatedInterface, $expectedInterface, "Interface was generated incorrectly." );
	}
