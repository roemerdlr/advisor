...
class ConstructorIntegrityTest extends \PHPUnit_Framework_TestCase {
	...
	public function testValidateIfClassHasExtraArgumentInTheParentConstructor() {
		...
		$this->setExpectedException ( '\Magento\Framework\Exception\ValidatorException', 'Extra parameters passed to parent construct: $factory. File: ' . $fileName );
	...
	public function testValidateIfClassHasMissingRequiredArguments() {
		...
		$fileName = str_replace ( '\\', '/', $fileName );
		$this->setExpectedException ( '\Magento\Framework\Exception\ValidatorException', 'Missed required argument factory in parent::__construct call. File: ' . $fileName );
	...
	public function testValidateIfClassHasIncompatibleArguments() {
		...
		$fileName = str_replace ( '\\', '/', $fileName );
		$this->setExpectedException ( '\Magento\Framework\Exception\ValidatorException', 'Incompatible argument type: Required type: \Magento\SomeModule\Model\Proxy. ' . 'Actual type: \Magento\SomeModule\Model\ElementFactory; File: ' . PHP_EOL . $fileName );
	...
	public function testValidateWrongOrderForParentArguments() {
		...
		$fileName = str_replace ( '\\', '/', $fileName );
		$this->setExpectedException ( '\Magento\Framework\Exception\ValidatorException', 'Incompatible argument type: Required type: \Context. ' . 'Actual type: \ClassA; File: ' . PHP_EOL . $fileName );
		$this->_model->validate ( 'ClassArgumentWrongOrderForParentArguments' );
	}
	public function testValidateWrongOptionalParamsType() {
		$fileName = realpath ( __DIR__ ) . '/_files/ClassesForConstructorIntegrity.php';
		$fileName = str_replace ( '\\', '/', $fileName );
		$this->setExpectedException ( '\Magento\Framework\Exception\ValidatorException', 'Incompatible argument type: Required type: array. ' . 'Actual type: \ClassB; File: ' . PHP_EOL . $fileName );
