...
class NotProtectedExtensionTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->_scopeConfig->expects ( $this->atLeastOnce () )->method ( 'getValue' )->with ( $this->equalTo ( \Magento\MediaStorage\Model\File\Validator\NotProtectedExtension::XML_PATH_PROTECTED_FILE_EXTENSIONS ), $this->equalTo ( \Magento\Store\Model\ScopeInterface::SCOPE_STORE ), $this->equalTo ( null ) )->will ( $this->returnValue ( $this->_protectedList ) );
	...
	public function testInitialization() {
		$property = new \ReflectionProperty ( '\Magento\MediaStorage\Model\File\Validator\NotProtectedExtension', '_messageTemplates' );
		...
		$property = new \ReflectionProperty ( '\Magento\MediaStorage\Model\File\Validator\NotProtectedExtension', '_protectedFileExtensions' );
		$property->setAccessible ( true );
		$protectedList = [ 
				'exe',
				'php',
