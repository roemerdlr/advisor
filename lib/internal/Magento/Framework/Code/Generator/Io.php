...
class Io {
	 ...
	 */
	public function __construct(\Magento\Framework\Filesystem\Driver\File $filesystemDriver, $generationDirectory = null) {
	...
	public function generateResultFileName($className) {
		return $this->_generationDirectory . ltrim ( str_replace ( [ 
				'\\',
				'_' 
		], '/', $className ), '/' ) . '.php';
	}
	...
	public function writeResultFile($fileName, $content) {
		/**
		 * Rename is atomic on *nix systems, while file_put_contents is not.
		 * Writing to a
		...
		try {
			...
			if (! $this->fileExists ( $fileName )) {
				...
				/**
				 * Due to race conditions, file may have already been written, causing rename to fail.
				 * As long as
