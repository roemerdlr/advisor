...
class Url extends \Magento\Framework\DataObject implements \Magento\Framework\UrlInterface {
	...
	protected function _getConfig($path) {
		return $this->_scopeConfig->getValue ( $path, $this->_scopeType, $this->_getScope () );
	}
	...
	public function getBaseUrl($params = []) {
		 ...
		 */
		if ($this->_getType () == UrlInterface::URL_TYPE_LINK && $this->_getRequest ()->isDirectAccessFrontendName ( $this->_getRouteFrontName () )) {
	 ...
	 *
	 * @param string $data        	
	 * @return \Magento\Framework\UrlInterface @SuppressWarnings(PHPMD.CyclomaticComplexity)
	 */
	...
	protected function _setRouteName($data) {
		...
		$this->unsetData ( 'route_front_name' )->unsetData ( 'route_path' )->unsetData ( 'controller_name' )->unsetData ( 'action_name' );
	...
	public function addSessionParam() {
		$this->setQueryParam ( $this->_sidResolver->getSessionIdQueryParam ( $this->_session ), $this->_session->getSessionId () );
		return $this;
	}
	...
	public function getUrl($routePath = null, $routeParams = null) {
		...
		if ($isArray) {
			array_walk_recursive ( $routeParams, function ($item) use(&$isCached) {
				if (is_object ( $item )) {
					$isCached = false;
				}
			} );
