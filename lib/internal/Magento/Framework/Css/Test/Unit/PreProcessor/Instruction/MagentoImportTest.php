...
class MagentoImportTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->object = new \Magento\Framework\Css\PreProcessor\Instruction\MagentoImport ( $this->design, $this->fileSource, $this->errorHandler, $this->assetRepo, $this->themeList );
	}
		...
		return [ 
				'non-modular notation' => [ 
						...
						[ 
								[ 
										'module' => null,
										'filename' => 'some/file.css' 
								],
								[ 
										'module' => null,
										'filename' => 'theme/some/file.css' 
								] 
						],
	...
	public function testProcessNoImport() {
		...
		$this->assetRepo->expects ( $this->never () )->method ( 'createRelated' );
		$this->object->process ( $chain );
		$this->assertEquals ( $expectedContent, $chain->getContent () );
		$this->assertEquals ( 'css', $chain->getContentType () );
	}
