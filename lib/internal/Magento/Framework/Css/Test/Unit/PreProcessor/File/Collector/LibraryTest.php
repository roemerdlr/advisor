...
class LibraryTest extends \PHPUnit_Framework_TestCase {
	...
	public function setup() {
		...
		$this->fileSystemMock->expects ( $this->any () )->method ( 'getDirectoryRead' )->will ( $this->returnValueMap ( [ 
				[ 
						DirectoryList::LIB_WEB,
						Filesystem\DriverPool::FILE,
						$this->libraryDirectoryMock 
				] 
		] ) );
	...
	public function testGetFilesEmpty() {
		...
		$this->readFactoryMock->expects ( $this->never () )->method ( 'create' );
		$this->componentRegistrarMock->expects ( $this->never () )->method ( 'getPath' );
	 ...
	 *
	 * @param $libraryFiles array
	 *        	Files in lib directory
	 * @param $themeFiles array
	 *        	Files in theme
	 *        	*
	 * @return void
	 */
	public function testGetFiles($libraryFiles, $themeFiles) {
		$this->fileListMock->expects ( $this->any () )->method ( 'getAll' )->will ( $this->returnValue ( [ 
				'returnedFile' 
		] ) );
		...
		$this->componentRegistrarMock->expects ( $this->once () )->method ( 'getPath' )->with ( ComponentRegistrar::THEME, $themePath )->will ( $this->returnValue ( [ 
				'/path/to/theme' 
		] ) );
		...
		$this->assertEquals ( [ 
				'returnedFile' 
		], $this->library->getFiles ( $this->themeMock, $subPath ) );
	}
