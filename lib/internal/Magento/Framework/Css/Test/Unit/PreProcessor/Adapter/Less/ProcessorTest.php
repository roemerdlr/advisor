...
class ProcessorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->temporaryFileMock = $this->getMockBuilder ( 'Magento\Framework\Css\PreProcessor\File\Temporary' )->disableOriginalConstructor ()->getMock ();
		
		$this->processor = new Processor ( $this->loggerMock, $this->appStateMock, $this->assetSourceMock, $this->temporaryFileMock );
	}
	...
	public function testProcessContentException() {
		...
		$this->assetSourceMock->expects ( self::once () )->method ( 'getContent' )->with ( $assetMock )->willThrowException ( new \Exception ( self::ERROR_MESSAGE ) );
		...
		$this->temporaryFileMock->expects ( self::never () )->method ( 'createFile' );
	...
	public function testProcessContentEmpty() {
		...
		$this->assetSourceMock->expects ( self::once () )->method ( 'getContent' )->with ( $assetMock )->willReturn ( '' );
