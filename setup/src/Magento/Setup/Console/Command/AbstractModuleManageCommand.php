...
abstract class AbstractModuleManageCommand extends AbstractModuleCommand {
	...
	protected function configure() {
		$this->addOption ( self::INPUT_KEY_FORCE, 'f', InputOption::VALUE_NONE, 'Bypass dependencies check' );
		$this->addOption ( self::INPUT_KEY_ALL, null, InputOption::VALUE_NONE, ($this->isEnable () ? 'Enable' : 'Disable') . ' all modules' );
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		if (! empty ( $modulesToChange )) {
			...
			if (! $force) {
				...
				if ($constraints) {
					$output->writeln ( "<error>Unable to change status of modules because of the following constraints:</error>" );
			...
			if ($isEnable) {
				...
				$output->writeln ( '' );
				$output->writeln ( '<info>To make sure that the enabled modules are properly registered,' . " run 'setup:upgrade'.</info>" );
			} else {
			...
			if ($force) {
				$output->writeln ( '<error>Alert: You used the --force option.' . ' As a result, modules might not function properly.</error>' );
			}
		} else {
	...
	protected function validate(array $modules) {
		$messages = [ ];
		if (empty ( $modules )) {
			$messages [] = '<error>No modules specified. Specify a space-separated list of modules' . ' or use the --all option</error>';
		}
		return $messages;
	}
