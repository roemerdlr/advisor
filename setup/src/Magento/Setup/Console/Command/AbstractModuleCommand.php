...
abstract class AbstractModuleCommand extends AbstractSetupCommand {
	...
	protected function configure() {
		$this->addArgument ( self::INPUT_KEY_MODULES, InputArgument::IS_ARRAY | ($this->isModuleRequired () ? InputArgument::REQUIRED : InputArgument::OPTIONAL), 'Name of the module' );
		$this->addOption ( self::INPUT_KEY_CLEAR_STATIC_CONTENT, 'c', InputOption::VALUE_NONE, 'Clear generated static view files. Necessary, if the module(s) have static view files' );
	...
	protected function cleanup(InputInterface $input, OutputInterface $output) {
		...
		if ($input->getOption ( self::INPUT_KEY_CLEAR_STATIC_CONTENT )) {
			...
			$output->writeln ( '<info>Info: Some modules might require static view files to be cleared. Use the optional --' . self::INPUT_KEY_CLEAR_STATIC_CONTENT . ' option to clear them.</info>' );
		}
	}
}
