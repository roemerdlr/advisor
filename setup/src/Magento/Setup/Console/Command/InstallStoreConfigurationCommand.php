...
class InstallStoreConfigurationCommand extends AbstractSetupCommand {
	...
	protected function configure() {
		$this->setName ( 'setup:store-config:set' )->setDescription ( 'Installs the store configuration' )->setDefinition ( $this->getOptionsList () );
		parent::configure ();
	}
	...
	public function getOptionsList() {
		return [ 
				new InputOption ( StoreConfigurationDataMapper::KEY_BASE_URL, null, InputOption::VALUE_REQUIRED, 'URL the store is supposed to be available at' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_LANGUAGE, null, InputOption::VALUE_REQUIRED, 'Default language code' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_TIMEZONE, null, InputOption::VALUE_REQUIRED, 'Default time zone code' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_CURRENCY, null, InputOption::VALUE_REQUIRED, 'Default currency code' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_USE_SEF_URL, null, InputOption::VALUE_REQUIRED, 'Use rewrites' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_IS_SECURE, null, InputOption::VALUE_REQUIRED, 'Use secure URLs. Enable this option only if SSL is available.' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_BASE_URL_SECURE, null, InputOption::VALUE_REQUIRED, 'Base URL for SSL connection' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_IS_SECURE_ADMIN, null, InputOption::VALUE_REQUIRED, 'Run admin interface with SSL' ),
				new InputOption ( StoreConfigurationDataMapper::KEY_ADMIN_USE_SECURITY_KEY, null, InputOption::VALUE_REQUIRED, 'Whether to use a "security key" feature in Magento Admin URLs and forms' ) 
		];
	}
			...
			switch ($key) {
				case StoreConfigurationDataMapper::KEY_BASE_URL :
					...
					if (! $url->isValid ( $value )) {
						...
						$errors [] = '<error>' . 'Command option \'' . StoreConfigurationDataMapper::KEY_BASE_URL . '\': ' . $errorMsgs [Validator::INVALID_URL] . '</error>';
					}
					break;
				...
				case StoreConfigurationDataMapper::KEY_BASE_URL_SECURE :
					try {
						...
						if (! $url->isValid ( $value )) {
							$errorMsgs = $url->getMessages ();
							if (! empty ( $errorMsgs )) {
								$errors [] = '<error>' . 'Command option \'' . StoreConfigurationDataMapper::KEY_BASE_URL_SECURE . '\': ' . $errorMsgs [Validator::INVALID_URL] . '</error>';
							}
						}
						...
						$errors [] = '<error>' . 'Command option \'' . StoreConfigurationDataMapper::KEY_BASE_URL_SECURE . '\': ' . $e->getLogMessage () . '</error>';
					}
					break;
				...
				case StoreConfigurationDataMapper::KEY_ADMIN_USE_SECURITY_KEY :
					$errorMsg = $this->validateBinaryValue ( $value, StoreConfigurationDataMapper::KEY_ADMIN_USE_SECURITY_KEY );
					if ($errorMsg !== '') {
						$errors [] = $errorMsg;
					}
					break;
				case StoreConfigurationDataMapper::KEY_JS_LOGGING :
					$errorMsg = $this->validateBinaryValue ( $value, StoreConfigurationDataMapper::KEY_JS_LOGGING );
					if ($errorMsg !== '') {
						$errors [] = $errorMsg;
					}
					break;
