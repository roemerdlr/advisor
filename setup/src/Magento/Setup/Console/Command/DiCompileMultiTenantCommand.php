...
class DiCompileMultiTenantCommand extends AbstractSetupCommand {
	...
	protected function configure() {
		$options = [ 
				new InputOption ( self::INPUT_KEY_SERIALIZER, null, InputOption::VALUE_REQUIRED, 'Serializer function that should be used (' . self::SERIALIZER_VALUE_SERIALIZE . '|' . self::SERIALIZER_VALUE_IGBINARY . ') default: ' . self::SERIALIZER_VALUE_SERIALIZE ),
				new InputOption ( self::INPUT_KEY_EXTRA_CLASSES_FILE, null, InputOption::VALUE_REQUIRED, 'Path to file with extra proxies and factories to generate' ),
				new InputOption ( self::INPUT_KEY_GENERATION, null, InputOption::VALUE_REQUIRED, 'Absolute path to generated classes, <magento_root>/var/generation by default' ),
				new InputOption ( self::INPUT_KEY_DI, null, InputOption::VALUE_REQUIRED, 'Absolute path to DI definitions directory, <magento_root>/var/di by default' ),
				new InputOption ( self::INPUT_KEY_EXCLUDE_PATTERN, null, InputOption::VALUE_REQUIRED, 'Allows to exclude Paths from compilation (default is #[\\\\/]m1[\\\\/]#i)' ) 
		];
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$fileExcludePatterns = $input->getOption ( 'exclude-pattern' ) ? [ 
				$input->getOption ( self::INPUT_KEY_EXCLUDE_PATTERN ) 
		] : [ 
				'#[\\\\/]M1[\\\\/]#i' 
		];
	...
	public function generateCode($generationDir, $fileExcludePatterns, $input) {
		...
		$this->files ['additional'] = [ 
				$input->getOption ( self::INPUT_KEY_EXTRA_CLASSES_FILE ) 
		];
		...
		$this->generator = new \Magento\Framework\Code\Generator ( $generatorIo, [ 
			...
			switch ($key) {
				case self::INPUT_KEY_SERIALIZER :
					if (($value !== self::SERIALIZER_VALUE_SERIALIZE) && ($value !== self::SERIALIZER_VALUE_IGBINARY)) {
						$errors [] = '<error>Invalid value for command option \'' . self::INPUT_KEY_SERIALIZER . '\'. Possible values (' . self::SERIALIZER_VALUE_SERIALIZE . '|' . self::SERIALIZER_VALUE_IGBINARY . ').</error>';
					}
					break;
				case self::INPUT_KEY_EXTRA_CLASSES_FILE :
					if (! file_exists ( $value )) {
						$errors [] = '<error>Path does not exist for the value of command option \'' . self::INPUT_KEY_EXTRA_CLASSES_FILE . '\'.</error>';
					}
					break;
				...
				case self::INPUT_KEY_EXCLUDE_PATTERN :
					if (@preg_match ( $value, null ) === false) {
						$errors [] = '<error>Invalid pattern for command option \'' . self::INPUT_KEY_EXCLUDE_PATTERN . '\'.</error>';
					}
					break;
			}
		}
	...
	private function validateOutputPath($value, $type) {
		...
		if (file_exists ( $value ) && ! is_writeable ( $value )) {
			$errorMsg .= '<error>Non-writable directory is provided by the value of command option \'' . $type . '\'.</error>';
		}
		return $errorMsg;
	}
}
