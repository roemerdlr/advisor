...
/**
 * Checks for PHP readiness.
 * It is used by both Cron and Setup wizard.
 */
class PhpReadinessCheck {
	 ...
	 */
	public function __construct(ComposerInformation $composerInformation, PhpInformation $phpInformation, VersionParser $versionParser) {
	...
	public function checkPhpSettings() {
		...
		$settings = array_merge ( $this->checkXDebugNestedLevel (), $this->checkPopulateRawPostSetting () );
	...
	private function checkXDebugNestedLevel() {
		...
		if (in_array ( 'xdebug', $currentExtensions )) {
			...
			$message = sprintf ( 'Your current setting of xdebug.max_nesting_level=%d.
                 Magento 2 requires it to be set to %d or more.
                 Edit your config, restart web server, and try again.', $currentXDebugNestingLevel, $minimumRequiredXDebugNestedLevel );
	...
	private function checkPopulateRawPostSetting() {
		...
		$message = sprintf ( 'Your PHP Version is %s, but always_populate_raw_post_data = %d.
	        ...
	        Please open your php.ini file and set always_populate_raw_post_data to -1.
 	        If you need more help please call your hosting provider.', PHP_VERSION, intVal ( ini_get ( 'always_populate_raw_post_data' ) ) );
