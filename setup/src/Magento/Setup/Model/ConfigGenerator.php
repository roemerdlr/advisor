...
class ConfigGenerator {
	...
	public function createSessionConfig(array $data) {
		...
		if (isset ( $data [ConfigOptionsListConstants::INPUT_KEY_SESSION_SAVE] )) {
			$configData->set ( ConfigOptionsListConstants::CONFIG_PATH_SESSION_SAVE, $data [ConfigOptionsListConstants::INPUT_KEY_SESSION_SAVE] );
		}
	...
	public function createDefinitionsConfig(array $data) {
		...
		if (! empty ( $data [ConfigOptionsListConstants::INPUT_KEY_DEFINITION_FORMAT] )) {
			$configData->set ( ObjectManagerFactory::CONFIG_PATH_DEFINITION_FORMAT, $data [ConfigOptionsListConstants::INPUT_KEY_DEFINITION_FORMAT] );
		}
	...
	public function createDbConfig(array $data) {
		...
		if (isset ( $data [ConfigOptionsListConstants::INPUT_KEY_DB_PREFIX] )) {
			$configData->set ( ConfigOptionsListConstants::CONFIG_PATH_DB_PREFIX, $data [ConfigOptionsListConstants::INPUT_KEY_DB_PREFIX] );
		}
		
		foreach ( $optional as $key ) {
			if (isset ( $data [$key] )) {
				$configData->set ( ConfigOptionsListConstants::CONFIG_PATH_DB_CONNECTION_DEFAULT . '/' . self::$paramMap [$key], $data [$key] );
			}
		}
		
		$currentStatus = $this->deploymentConfig->get ( ConfigOptionsListConstants::CONFIG_PATH_DB_CONNECTION_DEFAULT . '/' . ConfigOptionsListConstants::KEY_ACTIVE );
		
		if ($currentStatus === null) {
			$configData->set ( ConfigOptionsListConstants::CONFIG_PATH_DB_CONNECTION_DEFAULT . '/' . ConfigOptionsListConstants::KEY_ACTIVE, '1' );
		}
