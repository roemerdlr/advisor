...
class UninstallCollector {
	 ...
	 */
	public function __construct(ObjectManagerProvider $objectManagerProvider, DataSetupFactory $dataSetupFactory) {
	...
	public function collectUninstall($filterModules = []) {
		...
		$result = $setup->getConnection ()->select ()->from ( $setup->getTable ( 'setup_module' ), [ 
				'module' 
		] );
