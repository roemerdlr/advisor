...
class ConfigModel {
	 ...
	 */
	public function __construct(ConfigOptionsListCollector $collector, Writer $writer, DeploymentConfig $deploymentConfig, FilePermissions $filePermissions) {
		...
		foreach ( $options as $moduleName => $option ) {
			...
			foreach ( $configData as $config ) {
				if (! $config instanceof ConfigData) {
					throw new \Exception ( 'In module : ' . $moduleName . 'ConfigOption::createConfig should return an array of ConfigData instances' );
				}
				
				if (isset ( $fileConfigStorage [$config->getFileKey ()] )) {
					$fileConfigStorage [$config->getFileKey ()] = array_replace_recursive ( $fileConfigStorage [$config->getFileKey ()], $config->getData () );
				} else {
