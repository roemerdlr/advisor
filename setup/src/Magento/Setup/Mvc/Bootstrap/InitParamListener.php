...
class InitParamListener implements ListenerAggregateInterface, FactoryInterface {
	...
	public function attach(EventManagerInterface $events) {
		...
		$this->listeners [] = $sharedEvents->attach ( 'Zend\Mvc\Application', MvcEvent::EVENT_BOOTSTRAP, [ 
				$this,
				'onBootstrap' 
		] );
	}
	...
	public function authPreDispatch($event) {
		...
		if (! in_array ( $controller, $this->controllersToSkip )) {
			...
			if ($serviceManager->get ( 'Magento\Framework\App\DeploymentConfig' )->isAvailable ()) {
				...
				$objectManager->create ( 'Magento\Backend\Model\Auth\Session', [ 
	...
	public function createFilesystem(DirectoryList $directoryList) {
		...
		return new Filesystem ( $directoryList, new Filesystem\Directory\ReadFactory ( $driverPool ), new Filesystem\Directory\WriteFactory ( $driverPool ) );
	}
}
