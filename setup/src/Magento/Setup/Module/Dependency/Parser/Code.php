...
class Code implements ParserInterface {
	...
	protected function checkOptions($options) {
		if (! isset ( $options ['files_for_parse'] ) || ! is_array ( $options ['files_for_parse'] ) || ! $options ['files_for_parse']) {
			throw new \InvalidArgumentException ( 'Parse error: Option "files_for_parse" is wrong.' );
		}
		
		if (! isset ( $options ['declared_namespaces'] ) || ! is_array ( $options ['declared_namespaces'] ) || ! $options ['declared_namespaces']) {
