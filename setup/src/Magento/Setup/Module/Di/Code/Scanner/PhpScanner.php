...
class PhpScanner implements ScannerInterface {
			...
			foreach ( $parameters as $parameter ) {
				...
				if (isset ( $matches [1] ) && substr ( $matches [1], - strlen ( $entityType ) ) == $entityType) {
					...
					if (! class_exists ( $sourceClassName ) && ! interface_exists ( $sourceClassName )) {
						$this->_log->add ( Log::CONFIGURATION_ERROR, $missingClassName, "Invalid {$entityType} for nonexistent class {$sourceClassName} in file {$file}" );
						continue;
					}
	...
	protected function getSourceClassName($missingClassName, $entityType) {
		...
		if ($entityType == ExtensionAttributesInterfaceGenerator::ENTITY_TYPE || $entityType == ExtensionAttributesGenerator::ENTITY_TYPE) {
			/**
			...
			if (substr ( $sourceClassName, - strlen ( $extensionAttributesSuffix ) ) == $extensionAttributesSuffix) {
				 ...
				 */
				$extensionAttributesClass = substr ( $sourceClassName, 0, - strlen ( ExtensionAttributesGenerator::ENTITY_TYPE ) );
	...
	protected function _fetchMissingExtensionAttributesClasses($reflectionClass, $file) {
		$missingExtensionInterfaces = $this->_findMissingClasses ( $file, $reflectionClass, 'setExtensionAttributes', ucfirst ( \Magento\Framework\Api\Code\Generator\ExtensionAttributesInterfaceGenerator::ENTITY_TYPE ) );
	...
	public function collectEntities(array $files) {
		...
		foreach ( $files as $file ) {
			...
			foreach ( $classes as $className ) {
				...
				$output = array_merge ( $output, $this->_fetchFactories ( $reflectionClass, $file ), $this->_fetchMissingExtensionAttributesClasses ( $reflectionClass, $file ) );
			}
		}
	...
	protected function _getDeclaredClasses($file) {
		...
		for($tokenIterator = 0; $tokenIterator < $count; $tokenIterator ++) {
			...
			if (($tokens [$tokenIterator] [0] == T_CLASS || $tokens [$tokenIterator] [0] == T_INTERFACE) && $tokens [$tokenIterator - 1] [0] != T_DOUBLE_COLON) {
