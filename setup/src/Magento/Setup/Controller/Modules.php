...
class Modules extends AbstractActionController {
	...
	public function validateAction() {
		...
		$constraints = $status->checkConstraints ( $params ['status'], [ 
				$params ['module'] 
		], array_diff ( $params ['selectedModules'], [ 
				$params ['module'] 
		] ) );
		if ($constraints) {
	...
	private function getConstraintsFailureMessage($isEnable, array $constraints) {
		...
		$message = " Unable to $updateType modules because of the following constraints:<br>" . PHP_EOL . implode ( "<br>" . PHP_EOL, $constraints );
		return $message;
	}
}
