...
class Environment extends AbstractActionController {
	 ...
	 */
	public function __construct(\Magento\Setup\Model\FilePermissions $permissions, \Magento\Framework\Filesystem $filesystem, \Magento\Setup\Model\CronScriptReadinessCheck $cronScriptReadinessCheck, \Magento\Setup\Model\PhpReadinessCheck $phpReadinessCheck) {
	...
	private function getPhpChecksInfo($type) {
		...
		try {
			...
			if (isset ( $jsonData [ReadinessCheck::KEY_PHP_CHECKS] ) && isset ( $jsonData [ReadinessCheck::KEY_PHP_CHECKS] [$type] )) {
	...
	public function cronScriptAction() {
		...
		if (isset ( $updaterCheck ['notice'] )) {
			$data ['updaterNoticeMessage'] = 'Notice from Updater Application Cron Script:<br/>' . $updaterCheck ['notice'];
		}
