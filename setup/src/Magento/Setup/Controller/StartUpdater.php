...
class StartUpdater extends AbstractActionController {
	 ...
	 */
	public function __construct(\Magento\Framework\Filesystem $filesystem, \Magento\Setup\Model\Navigation $navigation, \Magento\Setup\Model\Updater $updater, \Magento\Framework\Module\FullModuleList $moduleList) {
	...
	public function updateAction() {
		...
		if (isset ( $postPayload [self::KEY_POST_PACKAGES] ) && is_array ( $postPayload [self::KEY_POST_PACKAGES] ) && isset ( $postPayload [self::KEY_POST_JOB_TYPE] )) {
			...
			if (empty ( $errorMessage )) {
				...
				$errorMessage .= $this->updater->createUpdaterTask ( [ ], \Magento\Setup\Model\Updater::TASK_TYPE_MAINTENANCE_MODE, [ 
						'enable' => true 
				] );
