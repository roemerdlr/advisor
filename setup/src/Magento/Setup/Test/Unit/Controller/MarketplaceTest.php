...
class MarketplaceTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSaveAuthJsonAction() {
		$this->marketplaceManager->expects ( $this->once () )->method ( 'checkCredentialsAction' )->will ( $this->returnValue ( \Zend_Json::encode ( [ 
				'success' => true 
		] ) ) );
		$this->marketplaceManager->expects ( $this->once () )->method ( 'saveAuthJson' )->willReturn ( true );
	...
	public function testSaveAuthJsonActionWithError() {
		$this->marketplaceManager->expects ( $this->once () )->method ( 'checkCredentialsAction' )->will ( $this->throwException ( new \Exception () ) );
	...
	public function testCheckAuthAction() {
		$this->marketplaceManager->expects ( $this->once () )->method ( 'getAuthJsonData' )->will ( $this->returnValue ( [ 
				'username' => 'test',
				'password' => 'test' 
		] ) );
		$this->marketplaceManager->expects ( $this->once () )->method ( 'checkCredentialsAction' )->will ( $this->returnValue ( \Zend_Json::encode ( [ 
	...
	public function testCheckAuthActionWithError() {
		$this->marketplaceManager->expects ( $this->once () )->method ( 'getAuthJsonData' )->will ( $this->throwException ( new \Exception () ) );
		$jsonModel = $this->controller->checkAuthAction ();
		$this->assertInstanceOf ( '\Zend\View\Model\JsonModel', $jsonModel );
		$variables = $jsonModel->getVariables ();
		$this->assertArrayHasKey ( 'success', $variables );
	...
	public function testRemoveCredetinalsAction() {
		$this->marketplaceManager->expects ( $this->once () )->method ( 'removeCredentials' )->will ( $this->returnValue ( true ) );
