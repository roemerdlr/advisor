...
class DataOptionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testNoHasUninstallAction() {
		...
		$this->uninstallCollector->expects ( $this->never () )->method ( 'collectUninstall' )->with ( [ 
				"some_module" 
		] );
	...
	public function testHasUninstallAction($content, $expected, $result) {
		...
		$this->uninstallCollector->expects ( $this->once () )->method ( 'collectUninstall' )->with ( [ 
				"some_module" 
		] )->willReturn ( $expected );
	...
	public function hasUninstallActionDataProvider() {
		...
		return [ 
				...
				'module does not have uninstall class' => [ 
						$content,
						[ ],
						false 
				] 
