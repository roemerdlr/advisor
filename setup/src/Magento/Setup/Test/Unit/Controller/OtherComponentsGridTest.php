...
class OtherComponentsGridTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$magentoComposerApplicationFactory = $this->getMock ( 'Magento\Framework\Composer\MagentoComposerApplicationFactory', [ ], [ ], '', false );
		$magentoComposerApplicationFactory->expects ( $this->once () )->method ( 'createInfoCommand' )->willReturn ( $this->infoCommand );
		$this->controller = new OtherComponentsGrid ( $this->composerInformation, $magentoComposerApplicationFactory );
	}
	public function testComponentsAction() {
		...
		$this->infoCommand->expects ( $this->once () )->method ( 'run' )->willReturn ( [ 
