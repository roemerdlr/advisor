...
class InitParamListenerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testOnBootstrap() {
		...
		$initParams [AppBootstrap::INIT_PARAM_FILESYSTEM_DIR_PATHS] [DirectoryList::ROOT] = [ 
				'path' => '/test' 
		];
		$serviceManager->expects ( $this->once () )->method ( 'get' )->willReturn ( $initParams );
		$serviceManager->expects ( $this->exactly ( 2 ) )->method ( 'setService' )->withConsecutive ( [ 
				'Magento\Framework\App\Filesystem\DirectoryList',
				$this->isInstanceOf ( 'Magento\Framework\App\Filesystem\DirectoryList' ) 
		], [ 
				'Magento\Framework\Filesystem',
				$this->isInstanceOf ( 'Magento\Framework\Filesystem' ) 
		] );
	...
	public function testCreateServiceNotConsole() {
		...
		$serviceLocator->expects ( $this->once () )->method ( 'get' )->with ( 'Application' )->willReturn ( $mvcApplication );
	 ...
	 *        	Data that comes from Zend Framework Application config
	 * @param array $env
	 *        	Config that comes from SetEnv
	...
	public function testCreateService($zfAppConfig, $env, $cliParam, $expectedArray) {
		...
		$request->expects ( $this->any () )->method ( 'getContent' )->willReturn ( $cliParam ? [ 
				'install',
				'--magento-init-params=' . $cliParam 
		] : [ 
				'install' 
		] );
		$mvcApplication->expects ( $this->any () )->method ( 'getConfig' )->willReturn ( $zfAppConfig ? [ 
				InitParamListener::BOOTSTRAP_PARAM => $zfAppConfig 
		] : [ ] );
	...
	private function prepareEventManager() {
		$this->callbackHandler = $this->getMockBuilder ( 'Zend\Stdlib\CallbackHandler' )->disableOriginalConstructor ()->getMock ();
		...
		$sharedManager->expects ( $this->once () )->method ( 'attach' )->with ( 'Zend\Mvc\Application', MvcEvent::EVENT_BOOTSTRAP, [ 
				$this->listener,
				'onBootstrap' 
