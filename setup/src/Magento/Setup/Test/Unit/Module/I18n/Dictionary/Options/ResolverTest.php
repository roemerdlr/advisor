...
class ResolverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetOptions($directory, $withContext, $result) {
		...
		$componentRegistrar->expects ( $this->any () )->method ( 'getPaths' )->will ( $this->returnValueMap ( [ 
				[ 
						ComponentRegistrar::MODULE,
						[ 
								$root . '/app/code/module1',
								$root . '/app/code/module2' 
						] 
				],
				[ 
		...
		$resolver = $objectManagerHelper->getObject ( 'Magento\Setup\Module\I18n\Dictionary\Options\Resolver', [ 
				'directory' => $directory,
	...
	public function testGetOptionsWrongDir($directory, $withContext, $message) {
		$componentRegistrar = $this->getMock ( 'Magento\Framework\Component\ComponentRegistrar', [ ], [ ], '', false );
		$root = __DIR__ . '/_files/source';
		$componentRegistrar->expects ( $this->any () )->method ( 'getPaths' )->willReturn ( [ 
				$root . '/app/code/module1',
				$root . '/app/code/module2' 
		] );
		$directoryList = $this->getMock ( 'Magento\Framework\App\Filesystem\DirectoryList', [ ], [ ], '', false );
	...
	public function getOptionsWrongDirDataProvider() {
		return [ 
				[ 
						'not_exist',
						true,
						'Specified path is not a Magento root directory' 
				],
				[ 
						'not_exist',
						false,
