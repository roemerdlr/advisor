...
class GeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->contextualParserMock = $this->getMock ( 'Magento\Setup\Module\I18n\Parser\Contextual', [ ], [ ], '', false );
		...
		$this->factoryMock->expects ( $this->any () )->method ( 'createDictionaryWriter' )->will ( $this->returnValue ( $this->writerMock ) );
		
		$this->optionsResolverFactory = $this->getMock ( 'Magento\Setup\Module\I18n\Dictionary\Options\ResolverFactory', [ ], [ ], '', false );
		
		$objectManagerHelper = new \Magento\Framework\TestFramework\Unit\Helper\ObjectManager ( $this );
		$this->generator = $objectManagerHelper->getObject ( 'Magento\Setup\Module\I18n\Dictionary\Generator', [ 
	...
	public function testCreatingDictionaryWriter() {
		...
		$this->parserMock->expects ( $this->any () )->method ( 'getPhrases' )->will ( $this->returnValue ( [ 
				$phrase 
		] ) );
		$options = [ ];
		$optionResolver = $this->getMock ( 'Magento\Setup\Module\I18n\Dictionary\Options\Resolver', [ ], [ ], '', false );
		$optionResolver->expects ( $this->once () )->method ( 'getOptions' )->will ( $this->returnValue ( $options ) );
		$this->optionsResolverFactory->expects ( $this->once () )->method ( 'create' )->with ( $this->equalTo ( '' ), $this->equalTo ( false ) )->will ( $this->returnValue ( $optionResolver ) );
	...
	public function testUsingRightParserWhileWithoutContextParsing() {
		...
		$phrase = $this->getMock ( 'Magento\Setup\Module\I18n\Dictionary\Phrase', [ ], [ ], '', false );
		$this->parserMock->expects ( $this->once () )->method ( 'getPhrases' )->will ( $this->returnValue ( [ 
				$phrase 
		] ) );
