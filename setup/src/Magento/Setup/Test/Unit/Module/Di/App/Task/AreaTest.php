...
class AreaTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->areaInstancesNamesList = $this->getMockBuilder ( '\Magento\Setup\Module\Di\Code\Reader\Decorator\Area' )->disableOriginalConstructor ()->getMock ();
		$this->configReaderMock = $this->getMockBuilder ( 'Magento\Setup\Module\Di\Compiler\Config\Reader' )->disableOriginalConstructor ()->getMock ();
		$this->configWriterMock = $this->getMockBuilder ( 'Magento\Setup\Module\Di\Compiler\Config\WriterInterface' )->disableOriginalConstructor ()->getMock ();
		$this->configChain = $this->getMockBuilder ( 'Magento\Setup\Module\Di\Compiler\Config\ModificationChain' )->disableOriginalConstructor ()->getMock ();
	}
	public function testDoOperationEmptyPath() {
		$areaOperation = new Area ( $this->areaListMock, $this->areaInstancesNamesList, $this->configReaderMock, $this->configWriterMock, $this->configChain );
	...
	public function testDoOperationGlobalArea() {
		...
		$this->areaInstancesNamesList->expects ( $this->once () )->method ( 'getList' )->with ( $path )->willReturn ( $arguments );
		$this->configReaderMock->expects ( $this->once () )->method ( 'generateCachePerScope' )->with ( $this->isInstanceOf ( 'Magento\Setup\Module\Di\Definition\Collection' ), App\Area::AREA_GLOBAL )->willReturn ( $generatedConfig );
