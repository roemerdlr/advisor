...
class ProxyGeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testDoOperationWrongData($data) {
		$model = new ProxyGenerator ( $this->directoryScannerMock, $this->proxyScannerMock, $data );
		
		$this->directoryScannerMock->expects ( $this->never () )->method ( 'scan' );
		$this->proxyScannerMock->expects ( $this->never () )->method ( 'collectEntities' );
	...
	public function testDoOperation() {
		...
		$this->proxyScannerMock->expects ( $this->once () )->method ( 'collectEntities' )->with ( $files ['di'] )->willReturn ( [ ] );
