...
class ArgumentsResolverTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetResolvedArgumentsConstructorFormat() {
		...
		$this->diContainerConfig->expects ( $this->any () )->method ( 'isShared' )->willReturnMap ( [ 
				[ 
						'Type\Dependency',
						false 
				],
		...
		$type = 'Class';
		$this->diContainerConfig->expects ( $this->any () )->method ( 'getArguments' )->with ( $type )->willReturn ( [ ] );
		
		$this->assertSame ( $expectedResultDefault, $this->model->getResolvedConstructorArguments ( $type, $constructor ) );
	}
	...
	private function getConfiguredArguments() {
		return [ 
				'type_dependency_configured' => [ 
						'instance' => 'Type\Dependency\Configured' 
				],
				'type_dependency_shared_configured' => [ 
						'instance' => 'Type\Dependency\Shared\Configured' 
				],
				'global_argument' => [ 
						'argument' => 'global_argument_configured' 
				],
				'global_argument_def' => [ 
						'argument' => 'global_argument_configured' 
				],
