...
class InterceptionConfigurationBuilderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->interceptionConfig = $this->getMock ( 'Magento\Framework\Interception\Config\Config', [ 
				'hasPlugins' 
		], [ ], '', false );
		$this->pluginList = $this->getMock ( 'Magento\Setup\Module\Di\Code\Generator\PluginList', [ 
		...
		$this->typeReader = $this->getMock ( 'Magento\Setup\Module\Di\Code\Reader\Type', [ 
				'isConcrete' 
		], [ ], '', false );
		$this->model = new \Magento\Setup\Module\Di\Code\Generator\InterceptionConfigurationBuilder ( $this->interceptionConfig, $this->pluginList, $this->typeReader, $this->cacheManager );
	}
	...
	public function testGetInterceptionConfiguration($plugins) {
		...
		$this->typeReader->expects ( $this->any () )->method ( 'isConcrete' )->willReturnMap ( [ 
				[ 
						'Class1',
						true 
				],
		...
		$this->cacheManager->expects ( $this->once () )->method ( 'setEnabled' )->with ( [ 
				CompiledConfig::TYPE_IDENTIFIER 
		], true );
		$this->pluginList->expects ( $this->once () )->method ( 'setInterceptedClasses' )->with ( $definedClasses );
		$this->pluginList->expects ( $this->once () )->method ( 'setScopePriorityScheme' )->with ( [ 
				'global',
				'areaCode' 
		] );
		$this->pluginList->expects ( $this->once () )->method ( 'getPluginsConfig' )->willReturn ( [ 
				'instance' => $plugins 
		] );
