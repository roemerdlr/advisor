...
class ReaderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->argumentsResolverFactory = $this->getMock ( 'Magento\Setup\Module\Di\Compiler\ArgumentsResolverFactory', [ ], [ ], '', false );
		$this->argumentsResolver = $this->getMock ( 'Magento\Setup\Module\Di\Compiler\ArgumentsResolver', [ ], [ ], '', false );
		$this->argumentsResolverFactory->expects ( $this->any () )->method ( 'create' )->willReturn ( $this->argumentsResolver );
		$this->classReaderDecorator = $this->getMock ( 'Magento\Setup\Module\Di\Code\Reader\ClassReaderDecorator', [ ], [ ], '', false );
		$this->typeReader = $this->getMock ( 'Magento\Setup\Module\Di\Code\Reader\Type', [ ], [ ], '', false );
		
		$this->model = new \Magento\Setup\Module\Di\Compiler\Config\Reader ( $this->diContainerConfig, $this->configLoader, $this->argumentsResolverFactory, $this->classReaderDecorator, $this->typeReader );
	}
	public function testGenerateCachePerScopeGlobal() {
		...
		foreach ( $definitionCollection->getInstancesNamesList () as $instanceType ) {
			$isConcreteMap [] = [ 
					$instanceType,
					strpos ( $instanceType, 'Interface' ) === false 
			];
			
			$getResolvedConstructorArgumentsMap [] = [ 
					$instanceType,
					$definitionCollection->getInstanceArguments ( $instanceType ),
					$this->getResolvedArguments ( $definitionCollection->getInstanceArguments ( $instanceType ) ) 
			];
		}
	...
	private function getDefinitionsCollection() {
		...
		$definitionCollection->addDefinition ( 'ConcreteType1', [ 
				'argument1',
				'argument2' 
		] );
		$definitionCollection->addDefinition ( 'ConcreteType2', [ 
				'argument1',
				'argument2' 
		] );
		$definitionCollection->addDefinition ( 'Interface1', [ 
				null 
		] );
	...
	private function getResolvedArguments($arguments) {
		return empty ( $arguments ) ? null : array_map ( function ($argument) {
	...
	private function getResolvedVirtualConstructorArgumentsMap(Collection $definitionCollection, array $virtualTypes) {
		...
		foreach ( $virtualTypes as $virtualType => $concreteType ) {
			$getResolvedConstructorArgumentsMap [] = [ 
					...
					$this->getResolvedArguments ( $definitionCollection->getInstanceArguments ( $concreteType ) ) 
			];
		}
