...
class InterceptionCacheTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->interceptionsListMock = $this->getMockBuilder ( 'Magento\Setup\Module\Di\Code\Reader\Decorator\Interceptions' )->setMethods ( [ ] )->disableOriginalConstructor ()->getMock ();
	}
	public function testDoOperationEmptyData() {
		$data = [ ];
		
		$operation = new InterceptionCache ( $this->configMock, $this->interceptionsListMock, $data );
		$this->configMock->expects ( $this->never () )->method ( 'initialize' );
	...
	public function testDoOperationInitializeWithDefinitions() {
		...
		$this->interceptionsListMock->expects ( $this->any () )->method ( 'getList' )->willReturnMap ( [ 
				[ 
						'lib',
						[ 
								'Library\Class' 
		...
		$operation = new InterceptionCache ( $this->configMock, $this->interceptionsListMock, $data );
		$this->configMock->expects ( $this->once () )->method ( 'initialize' )->with ( $definitions );
