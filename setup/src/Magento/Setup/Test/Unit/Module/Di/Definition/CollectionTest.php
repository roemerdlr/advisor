...
class CollectionTest extends \PHPUnit_Framework_TestCase {
	...
	private function getExpectedDefinition() {
		return [ 
				self::INSTANCE_1 => $this->getArgument () 
		];
	}
	protected function setUp() {
		$this->collectionMock = $this->getMockBuilder ( '\Magento\Setup\Module\Di\Definition\Collection' )->setMethods ( [ ] )->getMock ();
	...
	public function testGetInstancesNamesList() {
		...
		$this->assertEquals ( [ 
				self::INSTANCE_1 
		], $this->model->getInstancesNamesList () );
	}
	...
	public function testAddCollection() {
		$this->model->addDefinition ( self::INSTANCE_1, $this->getArgument () );
		$this->collectionMock->expects ( $this->any () )->method ( 'getCollection' )->willReturn ( [ 
				self::INSTANCE_2 => $this->getArgument () 
		] );
