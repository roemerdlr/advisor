...
class OperationFactoryTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->factory = new OperationFactory ( $objectManagerProviderMock );
	}
	...
	public function testCreateSuccess($alias, $arguments, $instanceName) {
		$operationInstance = $this->getMockBuilder ( 'Magento\Setup\Module\Di\App\Task\OperationInterface' )->getMock ();
		
		$this->objectManagerMock->expects ( $this->once () )->method ( 'create' )->with ( $instanceName, [ 
				'data' => $arguments 
	...
	public function testCreateException() {
		...
		$this->setExpectedException ( 'Magento\Setup\Module\Di\App\Task\OperationException', sprintf ( 'Unrecognized operation "%s"', $notRegisteredOperation ), OperationException::UNAVAILABLE_OPERATION );
	...
	public function aliasesDataProvider() {
		return [ 
				[ 
						...
						[ ],
						'Magento\Setup\Module\Di\App\Task\Operation\Area' 
				],
				[ 
						OperationFactory::INTERCEPTION,
						null,
						'Magento\Setup\Module\Di\App\Task\Operation\Interception' 
				],
				[ 
