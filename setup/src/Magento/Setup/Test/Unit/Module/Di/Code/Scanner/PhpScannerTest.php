...
class PhpScannerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->_model = new \Magento\Setup\Module\Di\Code\Scanner\PhpScanner ( $this->_logMock = $this->getMock ( '\Magento\Setup\Module\Di\Compiler\Log\Log', [ ], [ ], '', false ) );
	...
	public function testCollectEntities() {
		$this->_logMock->expects ( $this->at ( 0 ) )->method ( 'add' )->with ( 4, 'Magento\SomeModule\Module\Factory', 'Invalid Factory for nonexistent class Magento\SomeModule\Module in file ' . $this->_testFiles [0] );
		$this->_logMock->expects ( $this->at ( 1 ) )->method ( 'add' )->with ( 4, 'Magento\SomeModule\Element\Factory', 'Invalid Factory declaration for class Magento\SomeModule\Element in file ' . $this->_testFiles [0] );
