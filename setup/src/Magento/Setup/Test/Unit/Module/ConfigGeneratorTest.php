...
class ConfigGeneratorTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCreateCryptConfigWithInput() {
		$testData = [ 
				ConfigOptionsListConstants::INPUT_KEY_ENCRYPTION_KEY => 'some-test_key' 
		];
	...
	public function testCreateSessionConfigWithInput() {
		...
		$this->assertEquals ( [ 
				'session' => [ 
						'save' => ConfigOptionsListConstants::SESSION_SAVE_FILES 
				] 
		], $returnValue->getData () );
		
		$testData = [ 
				ConfigOptionsListConstants::INPUT_KEY_SESSION_SAVE => 'db' 
		];
		$returnValue = $this->configGeneratorObject->createSessionConfig ( $testData );
		$this->assertEquals ( ConfigFilePool::APP_ENV, $returnValue->getFileKey () );
		$this->assertEquals ( [ 
				'session' => [ 
						'save' => ConfigOptionsListConstants::SESSION_SAVE_DB 
				] 
		], $returnValue->getData () );
	}
	...
	public function testCreateDefinitionsConfig() {
		$testData = [ 
				ConfigOptionsListConstants::INPUT_KEY_DEFINITION_FORMAT => 'test-format' 
		];
