...
class CompilerPreparationTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->serviceManagerMock = $this->getMockBuilder ( '\Zend\ServiceManager\ServiceManager' )->disableOriginalConstructor ()->getMock ();
		$this->inputMock = $this->getMockBuilder ( '\Symfony\Component\Console\Input\ArgvInput' )->disableOriginalConstructor ()->getMock ();
		...
		$this->model = (new ObjectManager ( $this ))->getObject ( '\Magento\Setup\Console\CompilerPreparation', [ 
	...
	public function testClearGenerationDirWhenNeeded($commandName, $isCompileCommand, $isHelpOption, $dirExists = false) {
		...
		$this->inputMock->expects ( $this->atLeastOnce () )->method ( 'hasParameterOption' )->with ( $this->logicalOr ( '--help', '-h' ) )->willReturn ( $isHelpOption );
		if ($isCompileCommand && ! $isHelpOption) {
			$this->filesystemDriverMock->expects ( $this->once () )->method ( 'isExists' )->willReturn ( $dirExists );
	...
	public function testGenerationDirectoryFromInitParams() {
		...
		$this->inputMock->expects ( $this->once () )->method ( 'getFirstArgument' )->willReturn ( DiCompileMultiTenantCommand::NAME );
		
		// Filesystem mock
		$this->filesystemDriverMock->expects ( $this->once () )->method ( 'isExists' )->willReturn ( true );
		$this->filesystemDriverMock->expects ( $this->once () )->method ( 'deleteDirectory' )->with ( $customGenerationDirectory );
		
		$this->serviceManagerMock->expects ( $this->once () )->method ( 'get' )->with ( InitParamListener::BOOTSTRAP_PARAM )->willReturn ( $mageInitParams );
		$this->model->handleCompilerEnvironment ();
	}
