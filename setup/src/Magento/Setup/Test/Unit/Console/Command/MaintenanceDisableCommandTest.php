...
class MaintenanceDisableCommandTest extends \PHPUnit_Framework_TestCase {
	...
	public function testExecute(array $input, array $validatorMessages, $expectedMessage) {
		$return = isset ( $input ['--ip'] ) ? ($input ['--ip'] !== [ 
				'none' 
		] ? $input ['--ip'] : [ ]) : [ ];
	...
	public function executeDataProvider() {
		return [ 
				[ 
						...
						[ ],
						'Disabled maintenance mode' . PHP_EOL . 'Set exempt IP-addresses: 127.0.0.1, 127.0.0.2' . PHP_EOL 
				],
				[ 
						[ 
								'--ip' => [ 
						...
						[ ],
						'Disabled maintenance mode' . PHP_EOL . 'Set exempt IP-addresses: none' . PHP_EOL 
				],
				[ 
						[ ],
						[ ],
