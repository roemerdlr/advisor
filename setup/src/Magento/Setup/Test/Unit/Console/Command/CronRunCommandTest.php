...
class CronRunCommandTest extends \PHPUnit_Framework_TestCase {
	...
	public function testExecuteErrorOnToggleInProgress() {
		...
		$this->status->expects ( $this->once () )->method ( 'toggleUpdateInProgress' )->willThrowException ( new \RuntimeException ( 'runtime exception' ) );
		$this->commandTester->execute ( [ ] );
	}
	...
	public function testExecuteFirstJobNotSupported() {
		$this->setUpPreliminarySuccess ();
		$this->queue->expects ( $this->exactly ( 2 ) )->method ( 'peek' )->willReturn ( [ 
				'name' => 'update' 
		] );
		$this->queue->expects ( $this->never () )->method ( 'popQueuedJob' );
		$this->commandTester->execute ( [ ] );
	...
	public function testExecuteJobFailed() {
		$this->setUpPreliminarySuccess ();
		$this->queue->expects ( $this->at ( 0 ) )->method ( 'peek' )->willReturn ( [ 
				'name' => 'setup:' 
		] );
		$this->queue->expects ( $this->at ( 1 ) )->method ( 'peek' )->willReturn ( [ 
				'name' => 'setup:' 
		] );
