...
class DiCompileMultiTenantCommandTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		$this->model = (new ObjectManager ( $this ))->getObject ( '\Magento\Setup\Console\Command\DiCompileMultiTenantCommand' );
	}
	...
	public function validateDataProvider() {
		return [ 
				[ 
						...
						'Invalid value for command option \'' . DiCompileMultiTenantCommand::INPUT_KEY_SERIALIZER . '\'. Possible values (serialize|igbinary).' 
				],
				[ 
						[ 
								'--' . DiCompileMultiTenantCommand::INPUT_KEY_EXTRA_CLASSES_FILE => '/wrong/file/path' 
						],
						'Path does not exist for the value of command option \'' . DiCompileMultiTenantCommand::INPUT_KEY_EXTRA_CLASSES_FILE . '\'.' 
				],
				[ 
						[ 
								'--' . DiCompileMultiTenantCommand::INPUT_KEY_GENERATION => '/wrong/path' 
						],
						'Path does not exist for the value of command option \'' . DiCompileMultiTenantCommand::INPUT_KEY_GENERATION . '\'.' 
				],
				[ 
						[ 
								'--' . DiCompileMultiTenantCommand::INPUT_KEY_DI => '/wrong/path' 
						],
						'Path does not exist for the value of command option \'' . DiCompileMultiTenantCommand::INPUT_KEY_DI . '\'.' 
				],
				[ 
						[ 
								'--' . DiCompileMultiTenantCommand::INPUT_KEY_EXCLUDE_PATTERN => '%wrongPattern' 
						],
						'Invalid pattern for command option \'' . DiCompileMultiTenantCommand::INPUT_KEY_EXCLUDE_PATTERN . '\'.' 
				] 
		];
	}
