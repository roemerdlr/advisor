...
class ConfigSetCommandTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$option->expects ( $this->any () )->method ( 'getName' )->will ( $this->returnValue ( 'db-host' ) );
		$this->configModel = $this->getMock ( 'Magento\Setup\Model\ConfigModel', [ ], [ ], '', false );
		$this->configModel->expects ( $this->exactly ( 2 ) )->method ( 'getAvailableOptions' )->will ( $this->returnValue ( [ 
				$option 
		] ) );
	...
	public function testExecuteNoInteractive() {
		$this->deploymentConfig->expects ( $this->once () )->method ( 'get' )->will ( $this->returnValue ( null ) );
		...
		$this->assertSame ( 'You saved the new configuration.' . PHP_EOL, $commandTester->getDisplay () );
	}
	...
	private function checkInteraction($interactionType) {
		...
		$dialog->expects ( $this->once () )->method ( 'askConfirmation' )->will ( $this->returnValue ( $interactionType ) );
		...
		$helperSet->expects ( $this->once () )->method ( 'get' )->with ( 'dialog' )->will ( $this->returnValue ( $dialog ) );
		...
		$this->assertSame ( $message, $commandTester->getDisplay () );
	}
}
