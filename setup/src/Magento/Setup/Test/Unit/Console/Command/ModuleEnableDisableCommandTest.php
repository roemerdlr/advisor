...
class ModuleEnableDisableCommandTest extends \PHPUnit_Framework_TestCase {
	...
	public function testExecute($isEnable, $clearStaticContent, $expectedMessage) {
		$this->status->expects ( $this->once () )->method ( 'getModulesToChange' )->with ( $isEnable, [ 
				'Magento_Module1',
				'Magento_Module2' 
		...
		$this->status->expects ( $this->once () )->method ( 'setIsEnabled' )->with ( $isEnable, [ 
				'Magento_Module1' 
		] );
		
		$this->cache->expects ( $this->once () )->method ( 'clean' );
		$this->cleanupFiles->expects ( $this->once () )->method ( 'clearCodeGeneratedClasses' );
		$this->cleanupFiles->expects ( $clearStaticContent ? $this->once () : $this->never () )->method ( 'clearMaterializedViewFiles' );
	...
	public function executeDataProvider() {
		return [ 
				'enable, do not clear static content' => [ 
						true,
						false,
						'%amodules have been enabled%aMagento_Module1%a' . 'Info: Some modules might require static view files to be cleared.%a' 
				],
				'disable, do not clear static content' => [ 
						false,
						false,
						'%amodules have been disabled%aMagento_Module1%a' . 'Info: Some modules might require static view files to be cleared.%a' 
				],
	...
	public function testExecuteEnableInvalidModule() {
		$this->status->expects ( $this->once () )->method ( 'getModulesToChange' )->with ( true, [ 
				'invalid' 
	...
	public function testExecuteDisableInvalidModule() {
		$this->status->expects ( $this->once () )->method ( 'getModulesToChange' )->with ( false, [ 
				'invalid' 
		] )->willThrowException ( new \LogicException ( 'Unknown module(s): invalid' ) );
	...
	public function testExecuteAll($isEnable, $expectedMessage) {
		$this->fullModuleList->expects ( $this->once () )->method ( 'getNames' )->will ( $this->returnValue ( [ 
				'Magento_Module1',
				'Magento_Module2' 
		] ) );
	...
	public function testExecuteWithConstraints($isEnable) {
		...
		$this->assertStringMatchesFormat ( 'Unable to change status of modules%aconstraint1%aconstraint2%a', $commandTester->getDisplay () );
	}
	...
	public function testExecuteForce($isEnable, $expectedMessage) {
		...
		$this->status->expects ( $this->never () )->method ( 'checkConstraints' );
		...
		$this->assertStringMatchesFormat ( $expectedMessage . '%amodules might not function properly%a', $commandTester->getDisplay () );
	}
