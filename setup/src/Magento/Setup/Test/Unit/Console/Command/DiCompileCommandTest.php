...
class DiCompileCommandTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->cacheMock = $this->getMockBuilder ( 'Magento\Framework\App\Cache' )->disableOriginalConstructor ()->getMock ();
		...
		$this->fileDriver = $this->getMockBuilder ( 'Magento\Framework\Filesystem\Driver\File' )->disableOriginalConstructor ()->getMock ();
		...
		$this->command = new DiCompileCommand ( $this->deploymentConfig, $this->directoryList, $this->manager, $objectManagerProvider, $this->filesystem, $this->fileDriver, $this->componentRegistrar );
	}
	...
	public function testExecuteNotInstalled() {
		...
		$tester = new CommandTester ( $this->command );
		$tester->execute ( [ ] );
		$this->assertEquals ( 'You cannot run this command because the Magento application is not installed.' . PHP_EOL, $tester->getDisplay () );
	}
	public function testExecute() {
		$this->directoryList->expects ( $this->atLeastOnce () )->method ( 'getPath' )->willReturn ( null );
		$this->objectManager->expects ( $this->once () )->method ( 'get' )->with ( 'Magento\Framework\App\Cache' )->willReturn ( $this->cacheMock );
		...
		$this->objectManager->expects ( $this->once () )->method ( 'create' )->with ( 'Symfony\Component\Console\Helper\ProgressBar' )->willReturn ( $progressBar );
