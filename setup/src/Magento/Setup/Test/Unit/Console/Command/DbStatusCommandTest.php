...
class DbStatusCommandTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$objectManagerProvider->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValue ( $objectManager ) );
		$objectManager->expects ( $this->any () )->method ( 'get' )->will ( $this->returnValue ( $this->dbVersionInfo ) );
	...
	public function testExecute(array $outdatedInfo, $expectedMessage) {
		...
		$this->dbVersionInfo->expects ( $this->once () )->method ( 'getDbVersionErrors' )->will ( $this->returnValue ( $outdatedInfo ) );
	...
	public function executeDataProvider() {
		return [ 
				...
				'DB is outdated' => [ 
						...
						'%amodule_a%aschema%a1%a->%a2' . "%aRun 'setup:upgrade' to update your DB schema and data%a" 
				],
				'code is outdated' => [ 
						[ 
								[ 
						...
						'%amodule_a%adata%a2.0.0%a->%a1.0.0' . '%aSome modules use code versions newer or older than the database%a' 
				],
				'both DB and code is outdated' => [ 
						...
						'%amodule_a%aschema%a1.0.0%a->%a2.0.0' . '%amodule_b%adata%a2.0.0%a->%a1.0.0' . '%aSome modules use code versions newer or older than the database%a' 
				] 
		];
	}
