...
class AdminAccountTest extends \PHPUnit_Framework_TestCase {
	...
	public function testSaveUserExistsNewAdminRole() {
		...
		$returnValueMap = [ 
				...
				[ 
						'SELECT * FROM authorization_role WHERE parent_id = :parent_id AND tree_level = :tree_level ' . 'AND role_type = :role_type AND user_id = :user_id ' . 'AND user_type = :user_type AND role_name = :role_name',
						[ 
								'parent_id' => 0,
								'tree_level' => 1,
								'role_type' => 'G',
