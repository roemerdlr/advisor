...
class ReadinessCheckTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->deploymentConfig->expects ( $this->once () )->method ( 'get' )->willReturn ( [ 
		...
		$this->phpReadinessCheck->expects ( $this->once () )->method ( 'checkPhpVersion' )->willReturn ( [ 
				'success' => true 
		] );
		$this->phpReadinessCheck->expects ( $this->once () )->method ( 'checkPhpExtensions' )->willReturn ( [ 
				'success' => true 
		] );
		$this->phpReadinessCheck->expects ( $this->once () )->method ( 'checkPhpSettings' )->willReturn ( [ 
				'success' => true 
		] );
		$this->expected = [ 
				ReadinessCheck::KEY_PHP_VERSION_VERIFIED => [ 
						'success' => true 
				],
				ReadinessCheck::KEY_PHP_EXTENSIONS_VERIFIED => [ 
						'success' => true 
				],
				ReadinessCheck::KEY_PHP_SETTINGS_VERIFIED => [ 
						'success' => true 
				] 
		];
	}
	public function testRunReadinessCheckNoDbAccess() {
		$this->dbValidator->expects ( $this->once () )->method ( 'checkDatabaseConnection' )->willThrowException ( new \Magento\Setup\Exception ( 'Connection failure' ) );
	...
	public function testRunReadinessCheckNoDbWriteAccess() {
		$this->dbValidator->expects ( $this->once () )->method ( 'checkDatabaseConnection' )->willThrowException ( new \Magento\Setup\Exception ( 'Database user username does not have write access.' ) );
		$this->write->expects ( $this->once () )->method ( 'isExist' )->willReturn ( false );
		$this->write->expects ( $this->never () )->method ( 'readFile' );
		$expected = [ 
				ReadinessCheck::KEY_READINESS_CHECKS => [ 
	...
	public function testRunReadinessCheck() {
		...
		$expected = [ 
				ReadinessCheck::KEY_READINESS_CHECKS => [ 
						ReadinessCheck::KEY_DB_WRITE_PERMISSION_VERIFIED => true 
				],
				ReadinessCheck::KEY_PHP_CHECKS => $this->expected,
				ReadinessCheck::KEY_CURRENT_TIMESTAMP => 100 
		];
