...
class JobStaticRegenerateTest extends \PHPUnit_Framework_TestCase {
	...
	public function testExecuteProductionMode() {
		...
		$modeObjectMock->expects ( $this->once () )->method ( 'getMode' )->will ( $this->returnValue ( State::MODE_PRODUCTION ) );
		
		$filesystemMock = $this->getFilesystemObjectMock ( [ 
				'regenerateStatic' 
		] );
		$filesystemMock->expects ( $this->once () )->method ( 'regenerateStatic' );
		
		$this->jobStaticRegenerate->expects ( $this->once () )->method ( 'getFilesystem' )->will ( $this->returnValue ( $filesystemMock ) );
		
		$this->jobStaticRegenerate->expects ( $this->once () )->method ( 'getModeObject' )->will ( $this->returnValue ( $modeObjectMock ) );
		
		$this->jobStaticRegenerate->expects ( $this->once () )->method ( 'getOutputObject' )->will ( $this->returnValue ( $this->getOutputObjectMock () ) );
	...
	public function testExecuteDevelopernMode() {
		...
		$modeObjectMock->expects ( $this->once () )->method ( 'getMode' )->will ( $this->returnValue ( State::MODE_DEVELOPER ) );
		
		$this->jobStaticRegenerate->expects ( $this->once () )->method ( 'getModeObject' )->will ( $this->returnValue ( $modeObjectMock ) );
		
		$statusObject = $this->getStatucObjectMock ( [ 
				'add' 
		] );
		$statusObject->expects ( $this->exactly ( 3 ) )->method ( 'add' );
		$this->jobStaticRegenerate->expects ( $this->exactly ( 3 ) )->method ( 'getStatusObject' )->will ( $this->returnValue ( $statusObject ) );
		
		$cacheObject = $this->getCacheObjectMock ( [ 
				'clean' 
		] );
		$cacheObject->expects ( $this->once () )->method ( 'clean' );
		$this->jobStaticRegenerate->expects ( $this->once () )->method ( 'getCacheObject' )->will ( $this->returnValue ( $cacheObject ) );
		
		$cleanFilesObject = $this->getCleanFilesObjectMock ( [ 
				'clearMaterializedViewFiles',
				'clearCodeGeneratedFiles' 
		] );
		$cleanFilesObject->expects ( $this->once () )->method ( 'clearMaterializedViewFiles' );
		$cleanFilesObject->expects ( $this->once () )->method ( 'clearCodeGeneratedFiles' );
		$this->jobStaticRegenerate->expects ( $this->exactly ( 2 ) )->method ( 'getCleanFilesObject' )->will ( $this->returnValue ( $cleanFilesObject ) );
