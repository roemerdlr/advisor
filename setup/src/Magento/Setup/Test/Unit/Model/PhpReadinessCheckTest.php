...
class PhpReadinessCheckTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCheckPhpVersionNoRequiredVersion() {
		$this->composerInfo->expects ( $this->once () )->method ( 'getRequiredPhpVersion' )->willThrowException ( new \Exception () );
		$expected = [ 
	...
	public function testCheckPhpVersionPrettyVersion() {
		...
		$this->versionParser->expects ( $this->at ( 1 ) )->method ( 'normalize' )->willThrowException ( new \UnexpectedValueException () );
	...
	public function testCheckPhpSettings() {
		...
		$xdebugMessage = sprintf ( 'Your current setting of xdebug.max_nesting_level=%d.
                 Magento 2 requires it to be set to %d or more.
                 Edit your config, restart web server, and try again.', 100, 50 );
		
		$this->setUpNoPrettyVersionParser ();
		$rawPostMessage = sprintf ( 'Your PHP Version is %s, but always_populate_raw_post_data = -1.
	        ...
	        Please open your php.ini file and set always_populate_raw_post_data to -1.
 	        If you need more help please call your hosting provider.', PHP_VERSION );
		$expected = [ 
				'responseType' => ResponseTypeInterface::RESPONSE_TYPE_SUCCESS,
				'data' => [ 
	...
	public function testCheckPhpSettingsFailed() {
		...
		$xdebugMessage = sprintf ( 'Your current setting of xdebug.max_nesting_level=%d.
                 Magento 2 requires it to be set to %d or more.
                 Edit your config, restart web server, and try again.', 100, 200 );
	...
	public function testCheckPhpExtensionsNoRequired() {
		$this->composerInfo->expects ( $this->once () )->method ( 'getRequiredExtensions' )->willThrowException ( new \Exception () );
		$expected = [ 
				'responseType' => ResponseTypeInterface::RESPONSE_TYPE_ERROR,
				'data' => [ 
	...
	public function testCheckPhpExtensions() {
		$this->composerInfo->expects ( $this->once () )->method ( 'getRequiredExtensions' )->willReturn ( [ 
				'a',
				'b',
				'c' 
		] );
