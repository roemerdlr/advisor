...
class MarketplaceManagerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->curlClientMock = $this->_getCurlClientMock ( [ 
				'setCredentials',
				'getStatus',
				'getBody',
				'post' 
		] );
		$this->filesystemMock = $this->_getFilesystemMock ( [ 
				'getDirectoryRead',
				'getDirectoryWrite' 
		] );
	}
	...
	public function testGetCheckCredentialUrl() {
		...
		$marketplaceManager->expects ( $this->once () )->method ( 'getCredentialBaseUrl' )->will ( $this->returnValue ( $this->checkingCredentialsUrl ) );
		
		$this->assertEquals ( $this->urlPrefix . $this->checkingCredentialsUrl . '/check_credentials', $marketplaceManager->getCheckCredentialUrl () );
	}
	...
	public function testGetCredentialBaseUrl() {
		...
		$this->serviceLocatorMock->expects ( $this->once () )->method ( 'get' )->with ( 'config' )->will ( $this->returnValue ( [ 
				'marketplace' => [ 
						'check_credentials_url' => $this->checkingCredentialsUrl 
				] 
		] ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'getServiceLocator' )->will ( $this->returnValue ( $this->serviceLocatorMock ) );
		
		$this->assertEquals ( $this->checkingCredentialsUrl, $marketplaceManager->getCredentialBaseUrl () );
	}
	...
	public function testGetPackagesJsonUrl() {
		...
		$this->assertEquals ( $this->urlPrefix . $this->checkingCredentialsUrl . '/packages.json', $marketplaceManager->getPackagesJsonUrl () );
	}
	...
	public function testCheckCredentialsAction() {
		...
		$this->curlClientMock->expects ( $this->once () )->method ( 'setCredentials' )->with ( 'username', 'password' );
		$this->curlClientMock->expects ( $this->once () )->method ( 'getStatus' );
		$marketplaceManager->expects ( $this->exactly ( 3 ) )->method ( 'getCurlClient' )->will ( $this->returnValue ( $this->curlClientMock ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'getPackagesJsonUrl' );
	...
	public function testCheckCredentialsActionWithException() {
		...
		$this->curlClientMock->expects ( $this->once () )->method ( 'setCredentials' )->with ( 'username', 'password' );
		$this->curlClientMock->expects ( $this->once () )->method ( 'post' )->will ( $this->throwException ( new \Exception () ) );
		
		$this->curlClientMock->expects ( $this->never () )->method ( 'getBody' );
		$marketplaceManager->expects ( $this->exactly ( 2 ) )->method ( 'getCurlClient' )->will ( $this->returnValue ( $this->curlClientMock ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'getPackagesJsonUrl' );
	...
	public function testGetPackagesJson() {
		...
		$this->curlClientMock->expects ( $this->once () )->method ( 'setCredentials' )->with ( 'username', 'password' );
		$this->curlClientMock->expects ( $this->once () )->method ( 'getBody' );
		$marketplaceManager->expects ( $this->exactly ( 3 ) )->method ( 'getCurlClient' )->will ( $this->returnValue ( $this->curlClientMock ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'getAuthJsonData' )->will ( $this->returnValue ( [ 
				'username' => 'username',
				'password' => 'password' 
		] ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'getPackagesJsonUrl' );
	...
	public function testSyncPackagesForInstall() {
		...
		$marketplaceManager->expects ( $this->once () )->method ( 'getPackagesJson' )->will ( $this->returnValue ( '{
                    "packages": {
		...
		$this->composerInformationMock->expects ( $this->once () )->method ( 'getInstalledMagentoPackages' )->will ( $this->returnValue ( [ 
				[ 
						"name" => "magento/testing-extension" 
				] 
		] ) );
		$this->composerInformationMock->expects ( $this->any () )->method ( 'getPackagesTypes' )->will ( $this->returnValue ( [ 
				'magento2-module' 
		] ) );
		$marketplaceManager->expects ( $this->exactly ( 2 ) )->method ( 'getComposerInformation' )->will ( $this->returnValue ( $this->composerInformationMock ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'savePackagesForInstallToCache' )->will ( $this->returnValue ( true ) );
	...
	public function testGetAuthJsonData() {
		...
		$marketplaceManager->expects ( $this->once () )->method ( 'getAuthJson' )->will ( $this->returnValue ( [ 
				'http-basic' => [ 
	...
	public function testGetAuthJson() {
		...
		$this->filesystemMock->expects ( $this->once () )->method ( 'getDirectoryRead' )->will ( $this->returnValue ( $directory ) );
		$directory->expects ( $this->once () )->method ( 'isExist' )->will ( $this->returnValue ( true ) );
		$directory->expects ( $this->once () )->method ( 'isReadable' )->will ( $this->returnValue ( true ) );
		$directory->expects ( $this->once () )->method ( 'readFile' )->will ( $this->returnValue ( true ) );
		$marketplaceManager->expects ( $this->once () )->method ( 'getFilesystem' )->will ( $this->returnValue ( $this->filesystemMock ) );
	...
	public function testRemoveCredentials() {
		...
		$marketplaceManager->expects ( $this->once () )->method ( 'getCredentialBaseUrl' )->will ( $this->returnValue ( $this->checkingCredentialsUrl ) );
		$marketplaceManager->expects ( $this->never () )->method ( 'getDirectory' )->will ( $this->returnValue ( $directory ) );
		$directory->expects ( $this->never () )->method ( 'writeFile' );
	...
	public function testSaveAuthJson() {
		...
		$directory = $this->_getDirectoryMock ();
		$marketplaceManager->expects ( $this->any () )->method ( 'getDirectory' )->will ( $this->returnValue ( $directory ) );
		$directory->expects ( $this->any () )->method ( 'writeFile' )->will ( $this->returnValue ( $directory ) );
	...
	public function testGetPackagesForInstallEmptyData() {
		...
		$marketplaceManager->expects ( $this->once () )->method ( 'loadPackagesForInstallFromCache' )->will ( $this->returnValue ( false ) );
	...
	public function testGetPackagesForInstall() {
		...
		$marketplaceManager->expects ( $this->once () )->method ( 'loadPackagesForInstallFromCache' )->will ( $this->returnValue ( [ 
				'packages' => [ 
						[ 
								'name' => 'test1',
								"type" => "magento2-module" 
