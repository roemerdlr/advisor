...
class DependencyReadinessCheckTest extends \PHPUnit_Framework_TestCase {
	...
	public function testRunReadinessCheckFailed() {
		$this->reqUpdDryRunCommand->expects ( $this->once () )->method ( 'run' )->with ( [ ], 'var' )->willThrowException ( new \RuntimeException ( 'Failed' . PHP_EOL . 'dependency readiness check' ) );
		$expected = [ 
				'success' => false,
				'error' => 'Failed<br/>dependency readiness check' 
		];
