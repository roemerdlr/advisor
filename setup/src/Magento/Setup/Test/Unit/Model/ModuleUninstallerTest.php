...
class ModuleUninstallerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->uninstaller = new ModuleUninstaller ( $objectManagerProvider, $this->remove, $this->collector, $setupFactory, $this->moduleRegistryUninstaller );
	...
	public function testUninstallRemoveData() {
		...
		$this->collector->expects ( $this->once () )->method ( 'collectUninstall' )->willReturn ( [ 
				'moduleA' => $uninstall,
				'moduleB' => $uninstall 
		] );
		...
		$this->uninstaller->uninstallData ( $this->output, [ 
				'moduleA',
				'moduleB' 
		] );
	}
	public function testUninstallRemoveCode() {
		...
		$this->uninstaller->uninstallCode ( $this->output, [ 
				'moduleA',
				'moduleB' 
		] );
	}
