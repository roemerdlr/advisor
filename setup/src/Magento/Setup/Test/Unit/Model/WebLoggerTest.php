...
class WebLoggerTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'readFile' )->with ( 'install.log' )->will ( $this->returnCallback ( [ 
				'Magento\Setup\Test\Unit\Model\WebLoggerTest',
				'readLog' 
		] ) );
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'writeFile' )->with ( 'install.log' )->will ( $this->returnCallback ( [ 
				'Magento\Setup\Test\Unit\Model\WebLoggerTest',
				'writeToLog' 
		] ) );
		$this->directoryWriteMock->expects ( $this->any () )->method ( 'isExist' )->will ( $this->returnCallback ( [ 
				'Magento\Setup\Test\Unit\Model\WebLoggerTest',
				'isExist' 
		] ) );
	...
	public function testConstructorLogFileSpecified() {
		...
		$filesystemMock->expects ( $this->once () )->method ( 'getDirectoryWrite' )->will ( $this->returnValue ( $directoryWriteMock ) );
	...
	public function testLogSuccess() {
		...
		$this->assertEquals ( '<span class="text-success">[SUCCESS] ' . 'Success1' . '</span><br>' . '<span class="text-success">[SUCCESS] ' . 'Success2' . '</span><br>', self::$log );
	}
	...
	public function testLog() {
		...
		$this->assertEquals ( '<span class="text-info">Message1</span><br><span class="text-info">Message2</span><br>', self::$log );
	}
	public function testLogAfterInline() {
		$this->webLogger->logInline ( '*' );
		$this->webLogger->log ( 'Message' );
		$this->assertEquals ( '<span class="text-info">*</span><br><span class="text-info">Message</span><br>', self::$log );
	}
	...
	public function testClear() {
		$this->directoryWriteMock->expects ( $this->once () )->method ( 'delete' )->will ( $this->returnCallback ( [ 
				'Magento\Setup\Test\Unit\Model\WebLoggerTest',
				'deleteLog' 
		] ) );
