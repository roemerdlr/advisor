...
class UninstallCollectorTest extends \PHPUnit_Framework_TestCase {
	...
	public function setUp() {
		...
		$this->adapterInterface = $this->getMockForAbstractClass ( 'Magento\Framework\DB\Adapter\AdapterInterface', [ ], '', false );
		...
		$objectManager->expects ( $this->any () )->method ( 'create' )->will ( $this->returnValueMap ( [ 
				[ 
						'Magento\A\Setup\Uninstall',
						[ ],
						$uninstallA 
				],
				[ 
						'Magento\B\Setup\Uninstall',
						[ ],
						$uninstallB 
				] 
		] ) );
	...
	public function testUninstallCollector() {
		...
		$this->adapterInterface->expects ( $this->once () )->method ( 'fetchAll' )->with ( $this->result )->willReturn ( [ 
				[ 
						'module' => 'Magento_A' 
				],
				[ 
		...
		$this->assertEquals ( [ 
				'Magento_A' => 'Uninstall Class A',
				'Magento_B' => 'Uninstall Class B' 
