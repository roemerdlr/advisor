...
class EavVariationsFixtureTest extends \PHPUnit_Framework_TestCase {
	...
	public function testExecute() {
		...
		$storeManagerMock->expects ( $this->once () )->method ( 'getStores' )->will ( $this->returnValue ( [ 
				$storeMock 
		] ) );
		
		$setMock = $this->getMock ( 'Magento\Eav\Model\Entity\Attribute\Set', [ ], [ ], '', false );
		$setMock->expects ( $this->once () )->method ( 'getDefaultGroupId' )->will ( $this->returnValue ( 2 ) );
		...
		$this->fixtureModelMock->expects ( $this->exactly ( 4 ) )->method ( 'getObjectManager' )->will ( $this->returnValue ( $objectManagerMock ) );
	...
	public function testNoFixtureConfigValue() {
		...
		$objectManagerMock = $this->getMock ( 'Magento\Framework\ObjectManager\ObjectManager', [ ], [ ], '', false );
		$objectManagerMock->expects ( $this->never () )->method ( 'create' )->with ( $this->equalTo ( 'Magento\Catalog\Model\ResourceModel\Eav\Attribute' ) )->willReturn ( $attributeMock );
