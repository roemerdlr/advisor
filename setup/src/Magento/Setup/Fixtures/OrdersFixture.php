...
class OrdersFixture extends Fixture {
	...
	/**
	 *
	 * {@inheritdoc} @SuppressWarnings(PHPMD)
	 */
	public function execute() {
		...
		$quoteTableName = $this->getTableName ( 'quote', 'Magento\Quote\Model\ResourceModel\Quote' );
		$quoteAddressTableName = $this->getTableName ( 'quote_address', 'Magento\Quote\Model\ResourceModel\Quote\Address' );
		$quoteItemTableName = $this->getTableName ( 'quote_item', 'Magento\Quote\Model\ResourceModel\Quote\Item' );
		$quoteItemOptionTableName = $this->getTableName ( 'quote_item_option', 'Magento\Quote\Model\ResourceModel\Quote\Item\Option' );
		$quotePaymentTableName = $this->getTableName ( 'quote_payment', 'Magento\Quote\Model\ResourceModel\Quote\Payment' );
		$quoteAddressRateTableName = $this->getTableName ( 'quote_shipping_rate', 'Magento\Quote\Model\ResourceModel\Quote\Address\Rate' );
		$reportEventTableName = $this->getTableName ( 'report_event', 'Magento\Reports\Model\ResourceModel\Event' );
		$salesOrderTableName = $this->getTableName ( 'sales_order', 'Magento\Sales\Model\ResourceModel\Order' );
		$salesOrderAddressTableName = $this->getTableName ( 'sales_order_address', 'Magento\Sales\Model\ResourceModel\Order' );
		$salesOrderGridTableName = $this->getTableName ( 'sales_order_grid', 'Magento\Sales\Model\ResourceModel\Order\Grid' );
		$salesOrderItemTableName = $this->getTableName ( 'sales_order_item', 'Magento\Sales\Model\ResourceModel\Order\Item' );
		$salesOrderPaymentTableName = $this->getTableName ( 'sales_order_payment', 'Magento\Sales\Model\ResourceModel\Order\Payment' );
		$salesOrderStatusHistoryTableName = $this->getTableName ( 'sales_order_status_history', 'Magento\Sales\Model\ResourceModel\Order\Status\History' );
		$eavEntityStoreTableName = $this->getTableName ( 'eav_entity_store', '\Magento\Eav\Model\ResourceModel\Entity\Store' );
		...
		foreach ( $stores as $store ) {
			...
			foreach ( $resultsCategories as $resultsCategory ) {
				...
				if (trim ( $resultsCategoryName ) != '') {
					...
					$simpleProductCollection = $this->fixtureModel->getObjectManager ()->create ( 'Magento\Catalog\Model\ResourceModel\Product\Collection' );
	...
	public function getConnection() {
		return $this->fixtureModel->getObjectManager ()->get ( 'Magento\Framework\App\ResourceConnection' )->getConnection ();
	}
}
